...
try {
  ...
  assert.match(feedbackHost.innerHTML, /请至少上传 1 张产前版样衣成果图片/, '空成果提交应明确提示缺少图片')

  formFields.imageIds.value = 'sample-front.jpg, sample-back.jpg'
  assert.equal(handlePcsEngineeringMasterDetailEvent(actionTarget('submit-pre-production-sample-result')), true)
  assert.match(feedbackHost.innerHTML, /制作数量必须大于 0/, '数量无效时应阻断提交')
  assert.equal(
  ...
  assert.equal(handlePcsEngineeringMasterDetailEvent(actionTarget('submit-pre-production-sample-result')), true)
  assert.match(feedbackHost.innerHTML, /请填写产前版样衣成果提交人/, '提交人为空时应阻断提交')
  assert.equal(
