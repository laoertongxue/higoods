...
test('平台侧印花加工单列表先查看平台事实，再进入 Web 详情', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '前置任务项', exact: true })).toBeVisible()
  await expect(page.getByRole('heading', { name: '后置任务项', exact: true })).toBeVisible()
})

test('平台侧染色加工单列表进入 PFOS 单据详情', async ({ page }) => {
  ...
  await expectNotPda(page)
  await expect(page.getByRole('heading', { level: 1, name: '染色加工单详情' })).toBeVisible()
  await expect(page.getByRole('heading', { name: '任务明细', exact: true })).toBeVisible()
})
...
test('工艺工厂侧印花加工单列表进入 Web 详情', async ({ page }) => {
  ...
  await expect(detailLink).toHaveText(/^PH-/, { timeout: 30_000 })
