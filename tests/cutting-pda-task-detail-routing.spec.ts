...
test('PDA 裁片任务详情主流程进入当前任务，不再把铺布录入作为直接主按钮', async ({ page }) => {
  ...
  await expect(orderCard).toContainText('去铺布')
  if (targetLine?.nextActionLabel && targetLine.nextActionLabel !== '进入当前任务') {
