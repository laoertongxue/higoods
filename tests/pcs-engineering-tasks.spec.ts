...
assert.match(revisionListHtml, /生成技术包|再次写入技术包/, '应提供技术包动作')

const revisionTask = listRevisionTasks().find((item) => isTechPackGenerationAllowedStatus(item.status))
...
assert.match(revisionDetailHtml, /改版方案/, '改版任务详情应渲染方案页签')
assert.match(revisionDetailHtml, /下游任务/, '改版任务详情应渲染下游任务页签')
assert.match(revisionDetailHtml, /技术包写回|\/technical-data\//, '改版任务详情应显示技术包关联')
