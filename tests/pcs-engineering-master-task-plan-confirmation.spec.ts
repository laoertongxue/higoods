...
assert.match(draftHtml, /本次工作安排确认/, '草稿生产准备单首屏必须是本次工作安排确认')
...
Object.defineProperty(globalThis, 'window', {
  configurable: true,
  value: {
    location: { pathname: `/pcs/production-preparation/orders/${draft.masterOrderId}` },
...
assert.equal(confirmed?.status, '已发布', '确认任务方案后生产准备单必须进入已发布')
