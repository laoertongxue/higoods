...
test('设计改款列表冷进入、刷新和核心列表操作五次性能', async ({ browser, baseURL }, testInfo) => {
  ...
  for (let iteration = 0; iteration < 5; iteration += 1) {
    ...
    await expect(table.locator('thead th').nth(2)).toHaveText(/状态/)
    const relation = table.locator('[data-design-revision-style-relation]').first()
    await expect(relation).not.toContainText('工作下达')
    await expect(relation).not.toContainText('设计稿上传')
    await expect(relation).not.toContainText('更新')
    const materialCell = table.locator('[data-design-revision-material-costs]').first()
    await expect(materialCell.locator('[data-cost-section]')).toHaveCount(3)
    await expect(materialCell.locator('[data-design-revision-material-row]')).toHaveCount(3)
    await expect(materialCell.locator('[data-design-revision-fee-row]')).toHaveCount(2)
