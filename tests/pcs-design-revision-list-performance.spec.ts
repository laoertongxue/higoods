...
test('设计改款列表冷进入、刷新和核心列表操作五次性能', async ({ browser, baseURL }, testInfo) => {
  ...
  for (let iteration = 0; iteration < 5; iteration += 1) {
    ...
    record('hideColumn', await measure(page, async () => { await page.locator(`[data-standard-list-column-key="owner"] input${actionSelector('toggle-column-visibility')}`).uncheck(); await expect(table.locator('th').getByText('买手', { exact: true })).toHaveCount(0) }))
    record('restoreColumns', await measure(page, async () => { await page.locator(actionSelector('restore-column-settings')).click(); await expect(table.locator('th').getByText('买手', { exact: true })).toHaveCount(1) }))
    ...
    record('dragColumn', await measure(page, async () => {
      await page.locator('[data-drag-source="owner"]').dragTo(page.locator('[data-drop-target="status"]'))
      const keys = await table.locator('thead th').evaluateAll((nodes) => nodes.map((node) => node.textContent?.trim()))
      expect(keys.indexOf('买手')).toBeLessThan(keys.indexOf('状态'))
    }, 'dragstart'))
