...
test('唛架详情页使用只读信息块，不再展示 disabled input 大表单，且头部按钮可导航', async ({ page }) => {
  ...
  await expect(page.getByTestId('cutting-marker-plan-detail-page').getByRole('button', { name: '去合并批次' })).toBeVisible()
  await page.getByTestId('cutting-marker-plan-detail-page').getByRole('button', { name: '去合并批次' }).click()
