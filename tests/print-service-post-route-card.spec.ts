...
test('后道列表进入统一打印预览并隐藏 Web 系统壳', async ({ page }) => {
  ...
  await expect(page.getByText('PO-202603-0001', { exact: true }).first()).toBeVisible()
  await expect(page.getByText('HiGood 后道工厂', { exact: true }).first()).toBeVisible()
  await expect(page.getByText('扫码收货 -> 质检 -> 后道 -> 复检 -> 交出').first()).toBeVisible()
...
test('专门后道工厂完整流程打印单包含正式单据区域', async ({ page }) => {
  ...
  await expect(page.getByText('后道任务流转卡').first()).toBeVisible()
  await expect(page.getByText('扫码收货 -> 质检 -> 后道 -> 复检 -> 交出').first()).toBeVisible()
  for (const label of ['扫码收货区', '质检区', '后道区', '复检区', '交出区', '差异记录区', '签字区', '二维码区']) {
