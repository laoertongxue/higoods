...
test('PDA 回货确认执行超 5%二次点数、授权和真实账号记录', async ({ page }, testInfo) => {
  ...
  await page.goto(`/fcs/craft/post-finishing/wait-process-warehouse?deliveryId=${encodeURIComponent(seed.pendingDeliveryId)}`)
...
test('PDA 后道扫码先核对再开始，实时数量与瑕疵字段完整', async ({ page }, testInfo) => {
  const seed = await seedBrowserFlow(page)
  await page.setViewportSize({ width: 360, height: 800 })
  await setSession(page)
  ...
  await expect(page.locator('[data-post-result-line]')).toHaveCount(5)
  await expect(page.getByRole('button', { name: '核对无误，开始后道' })).toBeVisible()
  await expect(page.locator('[data-post-result-field="passedQty"]')).toHaveCount(0)
  await expectImagesLoaded(page)
  ...
  await expect(page.locator('[data-post-result-field="defectReason"]')).toHaveCount(5)
  await expect(page.locator('[data-post-result-field="defectImageUrl"]')).toHaveCount(5)
  await expect(page.locator('[data-post-result-file="defectImage"]')).toHaveCount(5)
  await expect(page.locator('[data-post-result-field="responsibleParty"]')).toHaveCount(5)
  await expect(page.locator('[data-quantity-summary="post-chain-total"]')).toContainText('整单全链应有')
  await expect(page.locator('[data-difference-authorization-block="post"]')).toBeHidden()
  const first = page.locator('[data-post-result-line]').first().locator('[data-post-result-field="passedQty"]')
  await first.fill('19')
  await expect(page.locator('[data-quantity-summary="post-total"]')).toContainText('少 1 件，提交前需授权')
...
test('PDA 复检执行领取释放、错码阻断、重贴复扫并唯一生成出货单', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('status')).toContainText('复检完成，出货单 FCK-')
  await expect(page.locator('body')).toContainText('已生成唯一出货单：FCK-')
  await verifyPdaAtBothSizes(page)
})
...
test('Web 管理页、动态授权日志和四类打印均读取同一链路事实', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('body')).toContainText('后道任务')
  ...
  for (const item of prints) {
    ...
    await expect(page.locator('dl > div')).toHaveCount(4)
    await expectImagesLoaded(page)
