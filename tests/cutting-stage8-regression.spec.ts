...
const WEB_PAGES: Array<{ path: string; title: string; required: string[]; forbidden?: string[] }> = [
  ...
  },
  {
    ...
    title: '铺布单',
    required: ['计划层数', '实铺层数', '计划用量', '实际用量', '实际裁剪数量', '最近写回'],
  },
  {
  ...
  },
  {
    ...
    title: '菲票',
    required: ['待首打', '已首打', '需补打', '已作废', '特殊工艺', '承接工厂', '编号范围'],
...
test('阶段 8 菲票详情、打印、补打、作废使用独立页面并保留打印投影字段', async ({ page }) => {
  const errors = collectBrowserErrors(page)
  await openAndCheck(page, WEB_PAGES.find((item) => item.path === '/fcs/craft/cutting/fei-tickets')!)

  const detailPath = await readDataNav(
    page,
    'button.text-left[data-nav^="/fcs/craft/cutting/fei-tickets/"]:not([data-nav*="print-records"])',
  )
  await gotoWithRecovery(page, detailPath)
  await expect(page.locator('body')).toContainText('菲票详情')
  for (const text of ['来源信息', '裁片信息', '特殊工艺', '二维码信息', '打印记录']) {
    await expect(page.locator('body')).toContainText(text)
  }
  await expectNoMainHorizontalScroll(page)
  ...
  const reprintPath = `${detailPath}/reprint`
  ...
  const voidPath = `${detailPath}/void`
  await gotoWithRecovery(page, voidPath)
  await expect(page.locator('h1').filter({ hasText: '菲票作废' }).first()).toBeVisible()
