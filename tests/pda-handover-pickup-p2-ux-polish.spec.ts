...
test('待接收 P2 轻量精修保持动作聚焦和信息主次', async ({ page }) => {
  ...
  await expect(currentRecordSection.getByText(/请确认本次接收，或发起数量差异。|查看异常单进度与处理结果。|平台已给出最终结果。|本次接收已确认完成。|先查看记录与二维码。/)).toBeVisible()
