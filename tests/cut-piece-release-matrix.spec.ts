...
async function getCurrentMatrixVersion(page: Page): Promise<number> {
  return page.evaluate(async () => {
    const repository = await import('/src/data/fcs/cut-piece-release.ts')
    const version = repository.listCutPieceReleaseMatrixVersions('po-14671').at(-1)?.version
    if (version === undefined) throw new Error('生产单 PO14671 缺少裁片矩阵版本')
    return version
  })
}
...
test('确认摘要保存目标并验证同版本仓储契约的幂等、冲突和版本历史', async ({ page }) => {
...
test('离开详情并重新进入会恢复已保存视图并清空未保存瞬态', async ({ page }) => {
  await selectBlackTarget(page)
...
test('关闭页重开动作会更新当前页面仓储中的放行来源', async ({ page }) => {
...
test('裁片交接显示最低应回与多余裁片，车缝摘要只显示当前齐套', async ({ page }) => {
  ...
  await page.reload()
  const reloadedSnapshot = page.locator('[data-testid="cut-piece-release-handover-snapshot"]')
  await expect(reloadedSnapshot).toContainText('矩阵 V1')
  await expect(reloadedSnapshot).toContainText('Black / M')
  await expect(reloadedSnapshot).toContainText('200 件')
  await expect(reloadedSnapshot).toContainText('多 24 片')
