...
test('按颜色展示物料尺码矩阵与当前齐套数量', async ({ page }) => {
...
test('目标只能从各尺码已有物料候选中选择并自动显示差异', async ({ page }) => {
...
test('确认摘要保存目标并呈现幂等、冲突和版本变化反馈', async ({ page }) => {
  ...
  await expect(summary).toContainText('当前矩阵版本 V1')
...
test('保存时真实矩阵版本已变化会阻断并引导刷新', async ({ page }) => {
  await selectBlackTarget(page)
  await page.evaluate(async () => {
    ...
    repository.recordCutOrderReleaseStatusChange({
      ...
      status: '持续更新',
      occurredAt: '2026-06-03 15:30:00',
      operator: '裁床主管 Dewi',
      reason: '测试上游恢复后版本变化',
    })
  })
  await page.getByRole('button', { name: '确认目标' }).click()
  await expect(page.locator('[data-testid="cut-piece-release-target-summary"]')).toContainText('当前矩阵版本 V1')
  await page.getByRole('button', { name: '保存目标' }).click()
  await expect(page.getByText('当前裁片矩阵版本已变化，请刷新后重新确认目标。')).toBeVisible()
})

test('单元格点击或键盘聚焦显示部位计算抽屉且关闭时保留矩阵滚动位置', async ({ page }) => {
  await rememberPageRoot(page)
  ...
  await page.getByRole('button', { name: '关闭部位详情' }).click()
  await expect(drawer).toHaveCount(0)
  expect(await matrixScroll.evaluate((element) => element.scrollLeft)).toBe(before)
  await expectPageRootStable(page)
})

test('历史抽屉分页展示真实版本事件与来源追溯', async ({ page }) => {
