...
test('管理端、技术包、加工单、捆条菲票打印和辅料仓形成可见闭环', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: '工艺路线', exact: true }).click()
  ...
  const routeValidationMessage = page.waitForEvent('dialog')
  const routeConfirmation = page.getByRole('button', { name: '确认工艺路线', exact: true }).click()
  const routeValidationDialog = await routeValidationMessage
  expect(routeValidationDialog.message()).toContain('存在多个未连接的起点')
  await routeValidationDialog.dismiss()
  await routeConfirmation
  await expect(page.locator('body')).toContainText('路线待确认')
  ...
  await expect(bindingStripEditor).toContainText('如需选择盘扣，请先在工序路线增加“盘扣（对象：捆条）”并确认路线')
  ...
  await expect(buttonLoopToggles.nth(0)).toBeDisabled()
  await expect(buttonLoopToggles.nth(1)).toBeDisabled()
...
test('PDA 交接与执行按现场职责完成盘扣全流程', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('body')).toContainText(/特殊工艺加工填报已(?:同步|记录)|累计产出：24 个/)
  ...
  await expect(page.locator('body')).toContainText(/特殊工艺发起交出已(?:同步|记录)|累计交出：10 个/)
  await expect(page.locator('body')).toContainText('待交出：14 个')
  ...
  await expect(page.locator('body')).toContainText(/特殊工艺发起交出已(?:同步|记录)|累计交出：24 个/)
  await expect(page.locator('body')).toContainText('待交出：0 个')
  ...
  await expect(page.locator('body')).toContainText(/特殊工艺完成加工单已(?:同步|记录)|已完结/)
