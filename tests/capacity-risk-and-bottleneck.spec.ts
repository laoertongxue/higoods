...
test('任务产值风险页已切到产值主线并覆盖 6 类风险', async ({ page }) => {
  ...
  const mixedOrder = riskData.orderRows.find(
    (row) =>
      [row.allocatedOutputValue, row.frozenPendingOutputValue, row.unallocatedOutputValue, row.unscheduledOutputValue]
  ...
  await expect(riskTaskTable).toContainText('窗口供给产值')
  await expect(riskTaskTable).toContainText('其他已冻结产值')
  await expect(riskTaskTable).toContainText('当前任务计入后剩余产值')

  await page.getByRole('button', { name: '生产单风险' }).click()
  await expect(riskOrderTable).toBeVisible()
  await expect(riskOrderTable).toContainText('已冻结待确认产值')
  await expect(riskOrderTable).toContainText('未落厂产值')
  await expect(riskOrderTable).toContainText('未排期产值')
  if (mixedOrder) {
...
test('工艺瓶颈与待分配页已切到工艺 / 日期 / 待分配未排期三条产值主线', async ({ page }) => {
  const errors = collectPageErrors(page)
  const bottleneckData = buildCapacityBottleneckData()
  const craftWithGap = bottleneckData.craftRows.find((row) => row.maxGapValue > 0)
  const craftWithFrozen = bottleneckData.craftRows.find((row) => row.windowFrozenValue > 0)
  const craftWithUnallocated = bottleneckData.craftRows.find((row) => row.unallocatedValue > 0)
  const dateWithGap = bottleneckData.dateRows.find((row) => row.overloadedFactoryCount > 0)
  const dateWithUnallocated = bottleneckData.dateRows.find((row) => row.unallocatedValue > 0)
  ...
  await expect(craftTable).toContainText('待分配产值')
  await expect(craftTable).toContainText('未排期产值')
  ...
  await expect(dateTable).toContainText('当日待分配产值')
  if (dateWithGap) {
