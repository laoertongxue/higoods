...
async function fillMaterialChange(page: Page, reason: string): Promise<void> {
  await selectFirstBusinessOption(page.getByLabel('原面料'))
  await selectFirstBusinessOption(page.getByLabel('新面料'))

  await page.getByRole('button', { name: '剩余数量替换', exact: true }).click()
  await page.getByRole('button', { name: '全部数量替换', exact: true }).click()
  await page.getByRole('button', { name: '只处理当前生产单', exact: true }).click()
  await page.getByRole('button', { name: '后续生产单也替换', exact: true }).click()

  const confirmedQty = page.getByLabel('跟单确认用于生产的数量')
  const suggestedQty = Number((await page.getByText('建议替换生产数量').locator('..').textContent())?.match(/\d+/)?.[0])
  expect(suggestedQty).toBeGreaterThan(1)
  await confirmedQty.fill(String(suggestedQty - 1))
  await expect(confirmedQty).toHaveValue(String(suggestedQty - 1))
  await page.getByLabel('变更原因').fill(reason)
}
...
test('替换物料覆盖两种方式、两个范围和可调整建议数量', async ({ page }) => {
  await openContent(page, 'material')
  await fillMaterialChange(page, 'E2E 物料替换：系统面料用于当前及后续生产单。')
  await expect(page.getByText('适用批次', { exact: false })).toHaveCount(0)

  await page.getByRole('button', { name: '下一步' }).click()
  await expect(page.getByRole('heading', { name: '系统自动处理' })).toBeVisible()
  await completeVisibleDecisions(page, 'FULL')
  await expect(page.getByRole('heading', { name: '正式版本绑定调整', exact: true })).toBeVisible()
  await page.getByRole('button', { name: '下一步' }).click()
  await page.getByRole('button', { name: '确认执行' }).click()
  await expect(page.getByText('全部处理成功并已统一生效。')).toBeVisible()
  await expect(page.getByLabel('最终执行结果').getByText('正式版本绑定调整', { exact: true })).toBeVisible()
...
test('可见失败入口全部回滚，重试复用同一变更单', async ({ page }) => {
  ...
  await expect(page.getByText('生产单正在变更，请稍后再试', { exact: true })).toBeVisible()
...
test('两张表单及第三第四步在双视口无页面溢出、关键控件不重叠且无旧文案', async ({ page }) => {
  for (const viewport of [{ width: 1024, height: 768 }, { width: 1440, height: 900 }]) {
    ...
    await page.getByRole('button', { name: '替换物料', exact: true }).click()
    await expectNoDocumentOverflow(page)
    await expectNoOverlap(page.getByLabel('原面料'), page.getByLabel('新面料'))
    await fillMaterialChange(page, `E2E ${viewport.width} 视口布局验收。`)
    await moveToExecution(page)
    await expectNoDocumentOverflow(page)
