...
test('后道单列表工厂名称、详情和操作入口可用', async ({ page }) => {
  ...
  for (const tab of ['后道记录', '质检记录', '复检记录', '待加工仓', '交出记录', '流转记录']) {
    await page.getByRole('button', { name: tab, exact: true }).click()
    await expect(page).toHaveURL(new RegExp(`tab=${tab === '后道记录' ? 'post' : tab === '质检记录' ? 'qc' : tab === '复检记录' ? 'recheck' : tab === '待加工仓' ? 'warehouse' : tab === '交出记录' ? 'handover' : 'events'}`))
...
test('质检单、复检单和移动端后道任务使用同一工厂与同一批后道单', async ({ page }) => {
  ...
  await page.goto('/fcs/pda/exec/TASK-POST-019')
  await expect(page.getByRole('button', { name: '开始复检' })).toBeVisible()

  await page.goto('/fcs/pda/exec/TASK-POST-201')
  await expect(page.getByText('HD-2026-201')).toBeVisible()
