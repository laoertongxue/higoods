...
test('Web 后道加工单可用完整单号开始并查看产品与加工项目', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog', { name: '开始后道' })).toHaveCount(0)

  await page.locator('tbody [data-post-finishing-work-orders-action="open-start"]').first().click()
  await page.getByRole('button', { name: '核对无误，开始后道' }).click()
  await expect(page).toHaveURL(/\/fcs\/craft\/post-finishing\/work-orders\//)
  await expect(page.getByRole('heading', { name: '执行后道加工单' })).toBeVisible()
  await expect(page.getByText('后道中', { exact: true }).first()).toBeVisible()
