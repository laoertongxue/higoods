...
test('印花加工单列表完整保留线上信息并按投入、来源、产出和双状态重组', async ({ page }) => {
  ...
  const selectHeader = root.locator('th[data-column-key="select"]')
  ...
  await expect(root.locator('thead tr').last().locator('th').first()).toHaveAttribute('data-column-key', 'select')
  await expect(selectHeader).toHaveCSS('position', 'sticky')
  ...
  const selectHeaderBox = await selectHeader.boundingBox()
  const orderHeaderBox = await orderHeader.boundingBox()
  expect(selectHeaderBox).toBeTruthy()
  expect(orderHeaderBox).toBeTruthy()
  expect(selectHeaderBox!.x).toBeLessThan(orderHeaderBox!.x)
  expect(Math.round(orderHeaderBox!.x - selectHeaderBox!.x)).toBe(64)
  ...
  const positionsBeforeScroll = await Promise.all([selectHeader, orderHeader, inputHeader, actionsHeader].map(async (header) => (await header.boundingBox())!.x))
  await scrollSurface.evaluate((element) => {
    element.scrollLeft = 700
    element.dispatchEvent(new Event('scroll'))
  })
  const positionsAfterScroll = await Promise.all([selectHeader, orderHeader, inputHeader, actionsHeader].map(async (header) => (await header.boundingBox())!.x))
  expect(Math.abs(positionsAfterScroll[0] - positionsBeforeScroll[0])).toBeLessThanOrEqual(1)
  expect(Math.abs(positionsAfterScroll[1] - positionsBeforeScroll[1])).toBeLessThanOrEqual(1)
  expect(positionsAfterScroll[2]).toBeLessThan(positionsBeforeScroll[2] - 500)
  expect(Math.abs(positionsAfterScroll[3] - positionsBeforeScroll[3])).toBeLessThanOrEqual(1)
  ...
  const selectColumnSetting = root.locator('[data-standard-list-column-key="select"]')
  await expect(selectColumnSetting).toHaveAttribute('draggable', 'false')
  await expect(selectColumnSetting.locator('input')).toHaveCount(2)
