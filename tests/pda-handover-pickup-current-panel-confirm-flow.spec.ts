...
test('待工厂确认状态下不再出现可编辑确认数量，并可直接确认仓库交付数量', async ({ page }) => {
  ...
  const currentRecordSection = page.getByTestId('pickup-current-panel-card')
  await expect(currentRecordSection).toBeVisible()
  await expect(currentRecordSection.getByText('请确认本次接收，或发起数量差异。')).toBeVisible()
  ...
  await expect(currentRecordSection.getByRole('button', { name: '确认本次接收', exact: true })).toBeVisible()
  await expect(currentRecordSection.getByRole('button', { name: '数量有差异', exact: true })).toBeVisible()

  await currentRecordSection.getByRole('button', { name: '确认本次接收', exact: true }).click()

  await expect(currentRecordSection.getByText('本次接收已确认完成。').first()).toBeVisible()
  ...
  await expect(currentRecordSection.getByRole('button', { name: '确认本次接收', exact: true })).toHaveCount(0)
