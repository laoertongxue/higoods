...
import { listProjects, listProjectNodes, resetProjectRepository, updateProjectNodeRecord } from '../src/data/pcs-project-repository.ts'
...
assert.match(listHtml, new RegExp(project.projectName), '列表页应包含当前仓储中的演示项目数据')

const detailHtml = await renderPcsProjectDetailPage(project.projectId)

assert.match(detailHtml, /阶段与步骤/, '详情页应渲染阶段导航')
...
const sampleAcquireProject = listProjects().find((item) =>
  listProjectNodes(item.projectId).some(
    (candidate) => candidate.stepCode === 'SAMPLE_ACQUIRE' && candidate.currentStatus !== '未开始',
  ),
)
...
const sampleAcquireHtml = await renderPcsProjectStepDetailPage(sampleAcquireProject.projectId, sampleAcquireNode.projectNodeId)

assert.match(sampleAcquireHtml, /全量信息/, '样衣获取节点应渲染全量信息页签')
assert.match(sampleAcquireHtml, /样衣来源方式/, '样衣获取节点应渲染正式字段标签')
assert.match(sampleAcquireHtml, /保存正式字段|新增正式记录|新增记录/, '样衣获取节点应提供字段保存入口')

const decisionProject = listProjects().find((item) =>
  listProjectNodes(item.projectId).some(
    (candidate) => candidate.stepCode === 'TEST_CONCLUSION',
  ),
)
assert.ok(decisionProject, '应存在测款结论步骤的演示项目')

const decisionNode = listProjectNodes(decisionProject.projectId).find(
  (candidate) => candidate.stepCode === 'TEST_CONCLUSION',
)
assert.ok(decisionNode, '应存在测款结论步骤')
listProjectNodes(decisionProject.projectId)
  .filter((candidate) => candidate.projectNodeId !== decisionNode.projectNodeId)
  .forEach((candidate) => {
...
const decisionHtml = await renderPcsProjectStepDetailPage(decisionProject.projectId, decisionNode.projectNodeId)

assert.match(decisionHtml, /测款结论/, '测款结论节点应渲染正式字段分组')
assert.match(decisionHtml, /通过/, '测款结论节点应包含通过分支')
assert.match(decisionHtml, /不通过/, '测款结论节点应包含不通过分支')
assert.match(decisionHtml, /暂保留/, '测款结论节点应包含暂保留分支')
...
const sampleConfirmProject = listProjects().find((item) =>
  listProjectNodes(item.projectId).some(
    (candidate) => candidate.stepCode === 'SAMPLE_CONFIRM' && candidate.currentStatus !== '未开始',
  ),
)
...
const sampleConfirmHtml = await renderPcsProjectStepDetailPage(sampleConfirmProject.projectId, sampleConfirmNode.projectNodeId)

assert.match(sampleConfirmHtml, /样衣确认/, '样衣确认节点应渲染正式字段录入')
assert.doesNotMatch(sampleConfirmHtml, />继续调整</, '样衣确认节点不应再包含继续调整分支')

function findProjectByNode(stepCode: string, preferredProjectNamePart = '') {
  return listProjects().find((item) => {
    if (preferredProjectNamePart && !item.projectName.includes(preferredProjectNamePart)) return false
    return listProjectNodes(item.projectId).some((node) => node.stepCode === stepCode)
  })
}
...
assert.match(createHtml, /固定五步商品测款流程/, '创建页应渲染固定流程预览区域')
