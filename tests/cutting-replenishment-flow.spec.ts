...
test('补料审批通过后，会在原裁片任务的仓库配料领料中生成补料待配料', async ({ page }) => {
  ...
  await expect(page.getByText('已生成补料待配料，可继续去仓库配料领料处理。')).toBeVisible()
  ...
  await expect(page.getByText(targetRow!.lines[0].cutOrderNo || targetRow!.lines[0].cutOrderId).first()).toBeVisible()
