...
test('菲票打印按铺布单展示，并在详情中展开该铺布单下全部部位明细', async ({ page }) => {
  ...
  await expect(page.locator('body')).toContainText('菲票打印')
  await expect(page.locator('body')).toContainText('铺布单')
  await expect(page.locator('body')).toContainText('菲票明细')
  await expect(page.locator('button[data-nav*="/fcs/craft/cutting/fei-tickets/spreading%3A"]').first()).toBeVisible()
  ...
  await expect(page.locator('body')).toContainText('该铺布单下全部部位明细')
  await expect(page.locator('body')).toContainText('部位裁片编号范围')
  await expect(page.locator('body')).toContainText('单条详情')
