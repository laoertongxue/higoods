...
test('release acceptance：supervisor IA、铺布列表状态与菜单闭环可见', async ({ page }) => {
  ...
  await expect(page.locator('[data-menu-group-header="工作台"]')).toBeVisible()
  await expect(page.locator('[data-menu-group-header="裁床厂管理"]')).toBeVisible()
  await expect(page.locator('[data-menu-group-icon="裁床厂管理"]')).toBeVisible()
  ...
  expect(segments.closedLoop).toContain('中转袋流转')
  ...
  const spreadingTable = page.getByTestId('cutting-spreading-list-table')
  await expect(spreadingTable.getByRole('button', { name: '去补料管理' }).first()).toBeVisible()

  await (await getStageTab(page, '待打印菲票')).click()
  await expect(spreadingTable.getByRole('button', { name: '去打印菲票' }).first()).toBeVisible()

  await (await getStageTab(page, '待装袋')).click()
  await expect(spreadingTable.getByRole('button', { name: '去装袋' }).first()).toBeVisible()

  await (await getStageTab(page, '待入仓')).click()
  await expect(spreadingTable.getByRole('button', { name: '去裁片仓' }).first()).toBeVisible()
...
test('release acceptance：supervisor 详情页 next-step action bar、公式和上下游跳转闭环', async ({ page }) => {
  ...
  for (const expectation of stageExpectations) {
    ...
    if (expectation.stage === '待装袋') {
      ...
      await expect(page.locator('body')).toContainText('中转袋列表')
      await expect(page.locator('body')).toContainText('来源铺布')
      await expect(page.locator('body')).toContainText('来源唛架')
      await expect(page.locator('body')).toContainText('来源原始裁片单')
      await expect(page.locator('body')).toContainText('来源合并裁剪批次')
...
test('release acceptance：PDA 从任务到执行单元到铺布录入，写回后 supervisor 可见', async ({ page }) => {
  ...
  await expect(page.locator('body')).toContainText('排版项')
  await expect(page.locator('body')).not.toContainText('计划单元')
