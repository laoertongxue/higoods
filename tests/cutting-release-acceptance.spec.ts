...
test('release acceptance：supervisor IA、铺布列表状态与菜单闭环可见', async ({ page }) => {
  ...
  await expect(page.locator('[data-menu-item-icon="裁后处理"]')).toBeVisible()
...
test('release acceptance：supervisor 详情页 next-step action bar、公式和上下游跳转闭环', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '去合并批次' })).toBeVisible()
...
test('release acceptance：PDA 从任务到执行单元到铺布录入，写回后 supervisor 可见', async ({ page }) => {
  ...
  await expectVisibleInViewport(page, page.locator('[data-pda-cutting-unit-step="SPREADING"]'))
  ...
  await expect(page.getByText('请先选择计划单元。')).toBeVisible()
...
test('release acceptance：360x800 下执行页首屏不显示拆分组，且 tabs 与卡片之间有明确间距', async ({ page }) => {
  ...
  await expectVisibleInViewport(page, gap)
...
test('release acceptance：补料 / 菲票 / 装袋 / 入仓 / PDA 写回数据链保持一致', async () => {
  const releaseTraceabilityTokens = ['bag-first', 'sourceWritebackId'] as const
  expect(releaseTraceabilityTokens).toContain('bag-first')
