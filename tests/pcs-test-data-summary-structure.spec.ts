...
assert.equal(launchResult.ok, true, '新建实例后应允许完成上架')
assert.ok(launchResult.record?.upstreamChannelProductCode, '完成上架后应回填上游渠道商品编码')
...
const workItemHtml = renderPcsProjectWorkItemDetailPage(project!.projectId, summaryNode!.projectNodeId)
assert.match(workItemHtml, /渠道拆分/, '工作项详情页应展示渠道拆分字段')
assert.match(workItemHtml, /店铺拆分/, '工作项详情页应展示店铺拆分字段')
assert.match(workItemHtml, /渠道商品拆分/, '工作项详情页应展示渠道商品拆分字段')
