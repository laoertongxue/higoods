...
test('PDA 二次分拣、重新装袋、交出、特殊工艺交出和回仓均写入统一事件账，并能在 Web 回查', async ({ page }) => {
  test.setTimeout(120_000)
  const errors = collectPageErrors(page)
  await seedCuttingPdaSession(page)
  ...
  await expectRuntimeEvent(page, '待交出仓二次分拣')
  await expectRuntimeEvent(page, '待交出仓重新装袋')
  ...
  await verifyEventOnWebPage(page, '/fcs/craft/cutting/warehouse-management/wait-handover', '裁床待交出仓', '待交出仓二次分拣')
...
test('PDA 二次分拣和特殊工艺回仓缺少扫码时不写入事件账', async ({ page }) => {
  const errors = collectPageErrors(page)
  await seedCuttingPdaSession(page)
  ...
  await expect(page.locator('[data-pda-cut-handover-field="pickingTaskScan"]')).toBeVisible({ timeout: 30000 })
  await page.locator('[data-pda-cut-handover-action="confirm-picking"]').click()
  await expect(page.locator('body')).toContainText('请先扫描当前裁片配料任务码。')
  await expectNoRuntimeEvent(page, '待交出仓二次分拣')
  await expectNoRuntimeEvent(page, '待交出仓重新装袋')
