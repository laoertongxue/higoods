...
assert.equal(result.order!.buyerName, style.buyerName, '买手来自商品绑定，忽略建单传入值')
...
for (const value of ['验收品牌', '衬衫', '非毛织', '25-45', '非穆斯林', '设计款', 'print shirt-45-60印花衬衫', style.buyerName!]) assert.ok(detail.includes(value), value)
...
assert.ok(renderPcsTestingOrderListPage().includes(style.buyerName!))
...
assert.ok(renderProductInformation({ ...style, materialType: '', categoryCode: '', productConfigRefs: undefined }).includes('待完善'))
