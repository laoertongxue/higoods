...
const groupedRelationCount = detail!.relationSection.groups.reduce((sum, group) => sum + group.items.length, 0)
assert.equal(
...
assert.equal(groupedRelationCount, detail!.relationSection.totalCount, '项目详情页关系分组应完整覆盖正式关系记录')
...
assert.ok(detail!.relationSection.pendingCount >= 0, '项目详情页应区分正式关系数量与待处理关系数量')
