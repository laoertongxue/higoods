...
test('合并染色一次性全链路与加工单同一身份', async ({ page }) => {
  ...
  await first.check()
  await expect(overlay).toContainText('首张加工单确定染厂、面料、目标颜色和染色工序')
  ...
  await expect(secondOrderRow.getByText('已加入合并染色')).toHaveCount(0)
  await page.getByRole('button', { name: '合并染色', exact: true }).first().click()
  await expect(page).toHaveURL(new RegExp(`${COMBINED_PATH}$`))
  await page.locator('[data-combined-dyeing-workspace] tbody tr').filter({ hasText: taskNo }).getByText(taskNo, { exact: true }).click()
  ...
  await expect(historyRow).not.toContainText(taskNo)
