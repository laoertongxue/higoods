...
test('合并裁剪批次点击后从右侧打开详情侧边栏', async ({ page }) => {
  ...
  await expect(drawer.getByText('计划裁床组', { exact: true })).toBeVisible()
  await expect(drawer.getByText('计划裁剪日期', { exact: true })).toBeVisible()
  await expect(drawer.getByText('备注', { exact: true })).toBeVisible()
