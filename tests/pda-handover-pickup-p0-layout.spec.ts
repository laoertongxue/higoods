...
test('待接收列表与详情页完成 P0 信息收口', async ({ page }) => {
  ...
  await expect(page.getByText(/仓库回写生成接收记录/)).toBeVisible()
  ...
  await expect(headCard.getByRole('button', { name: '去接收', exact: true })).toBeVisible()
  ...
  expect(cardText).toContain('接收工厂')
  ...
  await headCard.getByRole('button', { name: '去接收', exact: true }).click()
  ...
  await expect(sectionHeaders.nth(0)).toHaveText('接收信息（接收头）')
  await expect(sectionHeaders.nth(1)).toHaveText('当前记录处理区')
  await expect(sectionHeaders.nth(2)).toHaveText('仓库已生成的接收记录')

  const headSection = page.locator('article').filter({ has: page.getByRole('heading', { name: '接收信息（接收头）' }) }).first()
  ...
  expect(headSectionText).toContain('接收工厂')
  expect(headSectionText).toContain('累计接收记录')
