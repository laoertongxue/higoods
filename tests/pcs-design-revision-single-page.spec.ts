...
test('新建直接填写全部内容，保存和直接提交全流程及五次性能', async ({ browser, baseURL }, testInfo) => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    await measure('targetSku', async () => { await page.locator(field('bomMaterialSkuId')).selectOption('dr_cotton_dye_print'); await expect(page.locator('[data-independent-bom-line]')).toContainText('潘通') }, 'change')
