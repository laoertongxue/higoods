...
test('待接收当前处理区压成单张主卡并保留关键动作', async ({ page }) => {
  ...
  await expect(currentPanelCard.getByText(/待工厂确认|已确认接收|差异处理中|已发起差异|已处理|待仓库发出|待工厂自提/).first()).toBeVisible()
  ...
  await expect(currentPanelCard.getByRole('button', { name: /确认本次接收|数量有差异|去异常定位与处理/ }).first()).toBeVisible()

  await expect(currentRecordSection.getByText('当前可执行动作')).toHaveCount(0)

  await expect(currentPanelCard.getByRole('button', { name: '确认本次接收', exact: true })).toBeVisible()
