...
test('中转袋流转列表页收简为 4 张联动统计卡并支持分页', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /中转袋总数/ }).click()
...
test('待装袋铺布 session 可进入装袋页，并按来源铺布 session 预筛', async ({ page }) => {
  ...
  await expect(page.locator('body')).toContainText('当前已装菲票数 / 容量（张）')
  await expect(page.locator('body')).toContainText('当前袋内成衣件数（件）')
