...
test('周转口袋流转列表页收简为 4 张联动统计卡并支持分页', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /待发出口袋数/ })).toBeVisible()
  ...
  await expect(body).not.toContainText('当前口袋没有进行中的使用周期，可直接开始装袋。')
  await expect(body).not.toContainText('当前口袋已完成装袋，等待打印装袋清单并发出。')
  ...
  await page.getByRole('button', { name: /待发出口袋数/ }).click()
  ...
  expect(readyStatuses.every((text) => text.includes('待发出'))).toBeTruthy()

  await page.getByRole('button', { name: /周转口袋总数/ }).click()
  await expect(body).toContainText('显示 1-10 条，共 18 条，第 1 / 2 页')

  const nextPage = page.getByRole('button', { name: '下一页', exact: true })
  await expect(nextPage).toBeVisible()
  await nextPage.click()
  await expect(body).toContainText('显示 11-18 条，共 18 条，第 2 / 2 页')
