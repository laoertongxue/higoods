...
assert.ok(
  tasks.some((task) => task.projectCode === 'PRJ-202603-005' && task.sampleCode === 'FOS-RESULT-25001' && !('projectNodeId' in task)),
  '首单样衣任务仓储读取旧缓存时，应合并 bootstrap 中新增的已完成首单样衣任务',
)
assert.ok(
  tasks.some((task) => task.projectCode === 'PRJ-202603-004' && task.firstOrderSampleTaskCode === 'FOS-20260425-002' && !('projectNodeId' in task)),
