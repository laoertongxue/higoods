...
import {
  ...
  type TaskOutputValueRiskConclusion,
...
} satisfies Record<TaskOutputValueRiskConclusion, (typeof riskRows)[number] | undefined>
...
const frozenPendingOrderRisk = riskOrderRows.find((row) => row.frozenPendingOutputValue > 0)
...
const craftUnallocatedRow = bottleneck.craftRows.find((row) => row.unallocatedValue > 0)
...
test('产能日历页面已经接入正常/紧张/超载/暂停动态状态，并同步到风险与瓶颈视角', async ({ page }) => {
  ...
  await expect(page.locator('[data-capacity-risk-order-table]')).toContainText('已冻结待确认产值')
