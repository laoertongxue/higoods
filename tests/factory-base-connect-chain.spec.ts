...
test('基础连接工艺已进入字典，并沿工厂档案能力链进入产能档案', async ({ page }) => {
  ...
  const quyaSheet = await openCraftDetail(page, '曲牙绣')
  ...
  const quyaCheckbox = factoryForm.locator('[data-factory-craft-toggle="CRAFT_3000004"]').first()
  ...
  await expect(readonlySection).toContainText('曲牙绣')
  ...
  const quyaCard = sewingDrawer.locator('[data-capacity-entry-card]').filter({ hasText: '曲牙绣' }).first()
