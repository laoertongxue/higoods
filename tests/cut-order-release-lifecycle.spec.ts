...
test.afterEach(async ({ page }) => {
  ...
  await page.evaluate(async () => {
    const integration = await import('/src/pages/process-factory/cutting/cut-order-release-integration.ts')
    integration.setCutOrderReleaseStatusWriterForTesting(null)
  }).catch(() => undefined)
})
...
async function navigateSpa(page: Page, path: string): Promise<void> {
  ...
  if (path.startsWith('/fcs/craft/cutting/cut-order-close')) {
    ...
    await page.evaluate(() => import('/src/main-handlers/fcs-handlers.ts'))
  } else if (path.startsWith('/fcs/craft/cutting/cut-piece-release')) {
    await page.waitForSelector('[data-cut-piece-release-page]')
  }
}

async function openClosePage(page: Page): Promise<void> {
  await page.goto('/', { waitUntil: 'load' })
  await page.waitForSelector('[data-shell-sidebar-root="true"]')
  await page.evaluate(async () => {
    await Promise.all([
      import('/src/pages/process-factory/cutting/cut-order-release-integration.ts'),
      import('/src/main-handlers/fcs-handlers.ts'),
    ])
  })
...
test('重新打开时阶段投影不存在必须回滚账本且不改变放行矩阵', async ({ page }) => {
  ...
  await expect(page.getByTestId('cut-order-close-feedback')).toContainText('阶段投影不存在，重新打开失败')
...
test('关闭与重开在预热后局部刷新反馈和阶段均小于 200ms', async ({ page }) => {
  ...
  const reopenPerformance = await measureCloseLifecycleClick(page, '重新打开裁片单', '已重新打开裁片单', '持续更新')
...
test('阶段成功后放行写入拒绝或抛错必须完整回滚阶段、账本和矩阵', async ({ page }) => {
  ...
  const before = await page.evaluate(async () => {
    ...
    const repository = await import('/src/data/fcs/cut-piece-release.ts')
    const progress = await import('/src/data/fcs/cutting/order-progress.ts')
    const pageModule = await import('/src/pages/process-factory/cutting/cut-order-release-integration.ts')
    repository.resetCutPieceReleasePrototypeStoreForTesting()
    repository.recordCutOrderReleaseStatusChange({
  ...
  await page.locator('[data-cutting-piece-close-field="closeDescription"]').fill('异常后立即重试成功')
  const retryPerformance = await measureCloseLifecycleClick(page, '确认关闭裁片单', '已关闭裁片单并保留历史记录', '已关闭')
  expect(retryPerformance.elapsed).toBeLessThan(200)
  const retryAudit = await page.evaluate(async () => {
    const repository = await import('/src/data/fcs/cut-piece-release.ts')
    const lifecycle = await import('/src/data/fcs/cutting/cut-order-close-records.ts')
    return {
      closeCount: lifecycle.listStoredCutOrderCloseRecords().length,
  ...
  expect(retryAudit).toEqual({ closeCount: 1, version: before.version + 1, status: '已冻结' })
})
