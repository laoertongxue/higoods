...
import { listLiveProductLinesBySession } from '../src/data/pcs-live-testing-repository.ts'
import { getVideoTestRecordById } from '../src/data/pcs-video-testing-repository.ts'
import { findProjectNodeByWorkItemTypeCode } from '../src/data/pcs-project-repository.ts'
import {
...
const launchedProject = prepareProjectWithLaunchedChannelProduct('项目详情测款关系展示项目')
const project = createProjectForBusinessChain('项目详情测款关系展示项目-补充')
assert.ok(findProjectNodeByWorkItemTypeCode(project.projectId, 'LIVE_TEST'), '测试项目应存在直播测款节点')
assert.ok(findProjectNodeByWorkItemTypeCode(project.projectId, 'VIDEO_TEST'), '测试项目应存在短视频测款节点')
createAndLaunchChannelProductForProject(project.projectId)
...
replaceLiveProductLineProjectRelations(liveLine!.liveLineId, [launchedProject.projectId])
replaceVideoRecordProjectRelations(videoRecord!.videoRecordId, [launchedProject.projectId])

const detail = buildProjectDetailViewModel(launchedProject.projectId)
assert.ok(detail, '应能读取真实项目详情')

const liveItems = listProjectLiveTestingRelationItems(launchedProject.projectId)
...
const videoItems = listProjectVideoTestingRelationItems(launchedProject.projectId)
...
const liveNode = findProjectNodeByWorkItemTypeCode(launchedProject.projectId, 'LIVE_TEST')!
const videoNode = findProjectNodeByWorkItemTypeCode(launchedProject.projectId, 'VIDEO_TEST')!

const liveNodeDetail = buildProjectNodeDetailViewModel(launchedProject.projectId, liveNode.projectNodeId)
const videoNodeDetail = buildProjectNodeDetailViewModel(launchedProject.projectId, videoNode.projectNodeId)
assert.ok(liveNodeDetail && videoNodeDetail, '应能打开直播测款和短视频测款节点详情')

assert.ok(
  listProjectNodeLiveTestingRelationItems(launchedProject.projectId, liveNode.projectNodeId).every((item) => item.sourceObjectType === '直播商品明细'),
  'LIVE_TEST 节点详情页只应展示直播商品明细关系',
)
assert.ok(
  listProjectNodeVideoTestingRelationItems(launchedProject.projectId, videoNode.projectNodeId).every((item) => item.sourceObjectType === '短视频记录'),
