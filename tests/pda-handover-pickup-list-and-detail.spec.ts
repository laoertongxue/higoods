...
test('待领料列表保留领料处理 CTA，进入详情后不再出现新增领料记录区块', async ({ page }) => {
  ...
  await expect(headCard.getByRole('button', { name: '领料处理', exact: true })).toBeVisible()

  await headCard.getByRole('button', { name: '领料处理', exact: true }).click()
