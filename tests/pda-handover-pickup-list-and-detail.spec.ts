...
test('待接收列表保留去接收 CTA，进入详情后不再出现新增接收记录区块', async ({ page }) => {
  ...
  await expect(page.getByText('接收记录由仓库配料回写后生成')).toBeVisible()
  ...
  await expect(page.getByText('新增接收记录')).toHaveCount(0)
  await expect(page.getByRole('button', { name: '确认新增接收记录', exact: true })).toHaveCount(0)
  await expect(page.getByText('仓库已生成的接收记录')).toBeVisible()
  await expect(page.getByText('接收记录二维码').first()).toBeVisible()
