...
test('改款打样从目标颜色到整单确认按团队自动接力并保存真实成果文件', async ({ page }) => {
  ...
  await expect(page.getByText('目标颜色已确认，并按颜色建立物料方案。')).toBeVisible()
  await expect(page.getByText('浏览器验收藏青', { exact: false }).first()).toBeVisible()
  await expect(page.getByRole('link', { name: '维护该颜色物料' })).toHaveCount(2)
  ...
  const revisionCostDecision = page.locator('[data-pcs-independent-sampling-field="customCostDecision"]')
  if (await revisionCostDecision.inputValue() === 'UNDECIDED') {
    await revisionCostDecision.selectOption('NO_CUSTOM_COST')
  }
  const handoffStartedAt = Date.now()
  await page.getByRole('button', { name: '确认 BOM 与价格并交给跟单' }).click()
  await expect(page.getByText('BOM 与价格已一次确认并交给跟单安排工作。')).toBeVisible({ timeout: 200 })
  expect(Date.now() - handoffStartedAt).toBeLessThanOrEqual(200)
...
test('设计打样使用相同四步骨架并由跟单下达销售展示样衣制作要求', async ({ page }) => {
  ...
  await page.getByRole('link', { name: '维护该颜色物料' }).click()
  ...
  await page.getByRole('button', { name: 'BOM 与价格' }).click()
  await page.locator('[data-pcs-independent-sampling-field="customCostDecision"]').selectOption('NO_CUSTOM_COST')
  await page.getByRole('button', { name: '确认 BOM 与价格并交给跟单' }).click()
  await expect(page.getByRole('heading', { name: '第二步：工作安排' })).toBeVisible()
  await expect(page.getByRole('heading', { name: '销售展示样衣制作要求' })).toBeVisible()
