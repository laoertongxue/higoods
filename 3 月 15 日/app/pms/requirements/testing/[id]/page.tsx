...
import { ArrowLeft, Save, Send, FileText, XCircle, CheckCircle, Clock, AlertTriangle, Info, Edit } from "lucide-react"

export default function TestingRequirementDetailPage() {
  const params = useParams()
  const id = params.id as string
  const [isEditing, setIsEditing] = useState(false)
  const [adjustedQty, setAdjustedQty] = useState(2600)
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button variant="ghost" size="icon" asChild>
                <Link href="/pms/requirements/testing">
                ...
                <CardContent>
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className="font-medium mt-1">{requirement.source}</div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">关联商品</div>
                      <div className="font-medium mt-1">{requirement.productName}</div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">SKU</div>
                      <div className="font-medium mt-1">{requirement.skuCode}</div>
                    </div>
                    <div>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    <div className="p-3 bg-muted/50 rounded-lg">
                      <div className="text-muted-foreground">当前直播订单数</div>
                      <div className="text-2xl font-bold text-primary mt-1">{requirement.orderCount} 单</div>
                    </div>
                    <div className="p-3 bg-muted/50 rounded-lg">
                      <div className="text-muted-foreground">测款达标阈值</div>
                      <div className="text-2xl font-bold mt-1">{requirement.threshold} 单</div>
                    </div>
                    <div className="p-3 bg-muted/50 rounded-lg">
                      <div className="text-muted-foreground">预估最终销量</div>
                      <div className="text-2xl font-bold text-orange-600 mt-1">
                        {requirement.estimatedSales.toLocaleString()} 件
                      </div>
                    </div>
                  </div>
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="text-sm">
                        <div className="font-medium text-blue-800">预估逻辑说明</div>
                        <div className="text-blue-600 mt-1">
                          首日转化率 {requirement.conversionRate}%，历史同款放量系数 {requirement.multiplier}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="space-y-3">
                      <div className="flex justify-between p-2 bg-muted/30 rounded">
                        ...
                        <span className="font-medium">{requirement.currentStock.toLocaleString()} 件</span>
                      </div>
                      <div className="flex justify-between p-2 bg-muted/30 rounded">
                        <span className="text-muted-foreground">已分配库存</span>
                        <span className="font-medium text-orange-600">
                          -{requirement.allocatedStock.toLocaleString()} 件
                        </span>
                      </div>
                      <div className="flex justify-between p-2 bg-muted/30 rounded">
                        <span className="text-muted-foreground">在途库存（预计{requirement.inTransitDate}到仓）</span>
                        <span className="font-medium text-green-600">
                          +{requirement.inTransitStock.toLocaleString()} 件
                        </span>
                      </div>
                      <div className="flex justify-between p-2 bg-muted/30 rounded">
                        <span className="text-muted-foreground">安全库存</span>
                        <span className="font-medium">{requirement.safetyStock.toLocaleString()} 件</span>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div className="flex justify-between p-2 bg-blue-50 rounded border border-blue-200">
                        ...
                        <span className="font-bold text-blue-800">
                          {requirement.availableStock.toLocaleString()} 件
                        </span>
                      </div>
                      <div className="flex justify-between p-2 bg-red-50 rounded border border-red-200">
                        <span className="text-red-800">系统计算缺口数量</span>
                        <span className="font-bold text-red-800">{requirement.gapQty.toLocaleString()} 件</span>
                      </div>
                      <div className="flex justify-between p-2 bg-green-50 rounded border border-green-200">
                        <span className="text-green-800">建议采购数量（含{requirement.redundancy}%冗余）</span>
                        <span className="font-bold text-green-800">{requirement.suggestedQty.toLocaleString()} 件</span>
                      </div>
                    </div>
                  </div>
                </CardContent>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      <tr className="border-b">
                        <td className="p-3 font-medium">{requirement.skuCode}</td>
                        <td className="p-3 text-muted-foreground">M / L / XL 混码</td>
                        <td className="p-3 text-right font-medium">{requirement.suggestedQty.toLocaleString()}</td>
                        <td className="p-3 text-right">
                          {isEditing ? (
                            ...
                            <span className="font-medium">{adjustedQty.toLocaleString()}</span>
                          )}
                        </td>
                        <td className="p-3">
                          {isEditing ? (
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between p-2 bg-muted/30 rounded">
                        ...
                        <span className="font-medium">¥{requirement.unitPrice} / 件</span>
                      </div>
                      <div className="flex justify-between p-2 bg-muted/30 rounded">
