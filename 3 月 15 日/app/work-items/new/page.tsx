...
export default function NewWorkItemPage() {
  ...
  const [selectedFieldModels, setSelectedFieldModels] = useState<string[]>([])
  ...
  const handleSubmit = () => {
    ...
    // Here would be the API call to save the work item
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-6 space-y-6 max-w-4xl">
            {/* Header */}
            <div>
              <h1 className="text-2xl font-semibold text-foreground">新增工作项</h1>
              <p className="text-sm text-muted-foreground mt-1">创建可复用的工作项定义</p>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pb-6">
              ...
              <Button onClick={handleSubmit}>创建工作项</Button>
            </div>
          </div>
        </main>
      </div>
      ...
      <AlertDialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => router.push("/work-items")}>确定取消</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
