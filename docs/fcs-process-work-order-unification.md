...
- 旧批次模块（已移除）是多张染色加工单的一次共同执行任务，不合并、拆分或改号原加工单；只允许生产单来源加工单参加。
...
- 正式生产单加工快照使用技术包 BOM 的稳定物料编码作为实际面料身份；BOM 行 ID 只用于追溯。多条物料编码去重、排序后形成稳定物料集合身份，因此不同技术包、不同 BOM 行或排列顺序下的同组实际面料仍可一致识别；缺少稳定物料编码时阻断生成加工单。
