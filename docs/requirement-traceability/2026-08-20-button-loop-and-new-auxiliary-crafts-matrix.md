...
| FLOW-004 | 总体设计 2.5 | 完成加工单前必须已接收且已有加工填报；完成不依赖交出数量，完成后仍可交出剩余产出 | WP-05、WP-08 | `button-loop-craft-flow.ts`、`process-action-writeback-service.ts` | 综合领域契约覆盖零填报阻断、完成后交出及状态不回退 | PDA 加工填报后出现完成按钮；累计交出为 0 时完成；完成后在交接继续交出且状态不回退 | 已验证 | 领域专项通过；Playwright 43258 同轮 16/16；`process-report-fei-ticket-ready-360x640.png`、`completed-order-handout-360x640.png` | 用户／2026-09-01，当前修复工作树；现实 iData 待现场验收 |
