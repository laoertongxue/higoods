...
本轮把印花、染色、裁片、特殊工艺的 Web 端状态操作和工厂端移动应用操作，统一联动到待加工仓、待交出仓、交出记录、接收方收货确认记录和差异记录。
...
待加工仓、待交出仓、交出记录、收货确认记录、差异记录使用同一事实源：
...
- 接收方确认收货数量一致时只生成收货确认记录，不生成差异记录。
- 接收方确认收货数量小于完成数量时进入部分交出，剩余数量继续待交出。
- 接收方确认收货数量不一致时生成差异记录。
