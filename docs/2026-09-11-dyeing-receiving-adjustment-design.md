...
| 证据 | 当前代码确认的事实 | 对本次的影响 |
|---|---|---|
| E01 | `src/data/app-shell-config.ts:511`、`src/router/routes-fcs.ts:384`：有加工单、合并染色、水溶、两个仓库等，无独立待接收菜单 | 新增工厂收货入口，保留原导航 |
...
现有多单卷清单交出仍为面料服务。纱线交出单行直接携带 pcs/管型/毛重/净重，不构造“一条假卷”来套用面料模型。合并染色也保留各染色单身份、各自毛重上限和各自标签，不能变成整个合染批次一个码或共享限额。
