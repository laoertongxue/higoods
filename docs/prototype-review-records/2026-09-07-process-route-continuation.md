...
印染入口和旧批次模块（已移除）最终浏览器5/5通过（`preparation-entry-rerun.log`）；印花下游全接收后人工完单场景1/1通过（`printing-manual-final.log`）。
...
### 2026-09-08 CASE01 公共送货身份与两批后道续验

负责范围：PROD-004、EXEC-008/010、POST-002/004/021、TEST-005。前段改动文件src/pages/pda-sewing-self-return.ts，公共PDA有效锁定模式的管理员仅负责开启原入口；实际登记人取原填写的具名送货人。匹配工厂、开启人和有效锁定状态才适用，PPIC不获授权，不修改领域权限门禁。230原失败、231无记录；233/234原成功两登记、235退出模式、237两批后道点数，每批唯一QC。资料照片是原型截图，非现场实拍，款式图仍不准确，不通过图片/物理设备门禁。

原始证据output/verification/seven-demand-acceptance/seven-case01-43256/238–240、243–247。两张QC固定三项均勾选且disabled；两张后道单原开工、数量保存、人工完成后生成两张复核单；后道辅料尚无调拨单，原提示保留，不以加工按钮允许操作证明实际辅料到齐。243确认原HO实收仍0、后道已确认10，是未收口缺陷，不伪造逐原HDR实收映射。

11:42现场工作区main / 272e26d56c70d32db961deccf8f05e68a200b0b3，当时干净，由其他操作形成，本任务未提交推送。43256服务原地重启，240当前main原后道完整存储冷读相等。前段公共身份技术契约、类型、构建、CodeGraph日志已归档technical-contracts/public-return-actor-*，版本切换后的原页面续验独立注明；未把旧构建视为新工作区完整验收。此段仅补证据文档与运行原页面，无新增业务代码。完整新单验收仍0。

252原成衣仓两批实收及冷读两原回执有效；252图片目视为未稳定筛选的其他种子行，不作本单UI通过证据。253只读终点核查两次自动审批超时未启动，未人工完成PO。当前main仅文档差异，本轮末段未改src/scripts，因此不重跑构建/CodeGraph或生成新技术收据；此前技术日志按其执行时间归档，不能代替本轮命名页面。


### 2026-09-08 12:39 独立车缝原回货实收关联修复

当前main / 272e26d56c70d32db961deccf8f05e68a200b0b3，同工作树43256浏览器；本轮只修改src/data/fcs/pda-handover-events.ts与src/pages/pda-handover-detail.ts，以及本方案/矩阵/验收记录。负责EXEC-008/010、POST-002，对应增量方案§4.3.2。

- 角色/任务：后道原登录管理员仅可核对本工厂接收来源的独立车缝记录，具名选择原送货单并二次确认；不能代车缝新交出、人工结束或再录收货。源工厂仍按原只读关联结果查看。
- 数量/来源：同一执行任务、生产单、工厂、原单位件、逐SKU颜色尺码登记量完全一致才可关联；拒绝错单、拆并批猜配、重复占用；关联不改原交出数量，实收/人/时间来自当前后道确认，失效不沿用旧值。
- 页面：原/fcs/pda/handover/HOH-RETURN-TASKGEN-202603-0103-002__ORDER，390×844。初次被后道旧页面范围挡住（255/256）后，只开放本厂原回货核对；258可选项分别只有对应DEL01/02。259顺序执行取消与两次二次确认；CLI提前返回弹窗状态，260额外dismiss已无弹窗，未把工具返回误当无写入，261原事实确认两次关联实际保存。未重复提交。
- 证据：261-original-link-comparison.json比较254原只读输入：原POST全文逐字节未变；原两HDR除postReturnLink外逐字段相等。262最终源码两次刷新完整facts相等，head及getOriginalHandoutQuantities均10实收、0待收，无关联选择/泛用收货按钮；262截图已目视核对，无横向溢出，S记录需内部纵向滚动查看。原图缺失仍未通过图片门禁，真实PDA/扫码枪/打印与性能200ms仍未验，不将截图表示为这些门禁通过。
- 技术：原始254业务账仅供隔离Node输入，未回灌浏览器；post-link-contract.log与post-link-cold.log通过来源/单位/角色/重复/存储回滚/冷读/当前确认订正与失效；最后补原数量读路径后重新运行。类型、最终构建通过，CodeGraph sync/status无pending；最终技术收据结果另记，不提前宣称。
- 高风险审查：原交出动作仍保留，新增只有关联身份/关联人/时间；读取结果不保存为第二笔收货。后道已关联实收禁止泛用写入或异议改量；原数量汇总复用同一投影且不重建单头。仅此独立SEW且接收方为后道场景生效，毛织/准备工艺及未关联记录路径保留。

该修复不关闭水溶/染色→车缝投入、正式后道辅料调拨、准确素材和真实设备；整体仍未通过。

263/265原PO于12:39:04人工完成并最终冷读；CASE01剩余投入/辅料分支仍未完成，不以PO状态代替产品验收。初次task receipt因沙箱Chromium不能启动而implemented，允许浏览器环境的同一列表治理已通过；最终收据归档时说明状态。

266本单仓收列表稳定两行已收、原记录数量6/4与接收人一致、无重复收货按钮，补足252误截图的命名列表证据。最终技术收据归档technical-contracts/post-link-task-receipt-final.json，仅证明本次修复检查，不代表101项或七单全部验收。

### 2026-09-08 101 条当前证据对账与专项维护（无新增用户可见影响）

本次新增负责 `scripts/check-process-order-task-relations.ts`，对应 GOV-003/006、EXEC-004～007、PROD-004。旧断言把所有 POST/IRON_PACK 一律禁止，与已明确三方合并责任冲突；改为核验原生产单、冻结版本、显式范围和 EXTERNAL_TASK，并保持开扣眼/装扣子禁生成。未改生产生成器、路由、UI、业务存储或原验收单。运行结果：218 条关系通过。前轮两个受管文件的最终原页面证据仍见本记录前文，未发生新的源代码实质修改。

新增 `docs/product-design/101条需求当前证据索引.md` 与机器报告绑定全部 101 个编号。37 项专项实际 35 通过、2 失败，缺失/不适用不能自动视为通过；当前整体原型验收结论仍未完成。旧染色部分入库及后道 50/42 图证据冲突未以修改门槛绕过。
