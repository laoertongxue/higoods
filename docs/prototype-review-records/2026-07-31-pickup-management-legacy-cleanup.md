...
| --- | --- |
| 审查日期 | 2026-07-31 |
| 相关需求 / 任务 | 清理废弃的接收节点级工作台（旧 pickup-management.ts 整条链路） |
| 涉及系统 | PFOS（裁床厂管理） |
| 涉及页面路径 | /fcs/craft/cutting/pickup-management（旧工作台与旧详情，均已移除入口） |
| 端类型 | 管理端 |
| 主要角色 | 裁床接收员、裁床仓管 |
| 主要任务 | 接收管理三列表（已配齐待接收 / 未配齐配料 / 已接收） |
...
| 检查项 | 结论 | 说明 |
| --- | --- | --- |
| 角色匹配 | 通过 | 保留的三列表对应裁床接收员 / 仓管现场协同，与 PDA 接收共用同一数据源 |
...
| 信息负荷 | 通过 | 删除详情页（信息已被列表行内与接收记录抽屉覆盖），页面数量收敛 |
| 文案 | 通过 | 无英文状态码；中文字段与状态沿用 pickupStatusLabelMap 统一字典 |
| 数量与状态 | 通过 | 接收状态（暂不可领 / 待接收 / 打回待仓库处理 / 已接收完结 / 按实完结）定义保留且被 material-prep 系列共用 |
| 扫码与识别 | 通过 | 不涉及扫码；PDA 接收链路未受影响 |
| 防错 | 通过 | "PC 不得硬编码收货人直接确认接收""PC 列表不得直接确认领取""接收记录必须展示节点版本"三条防错契约迁移至新列表页继续生效 |
| UI 样式 | 通过 | 仅删除，未改动既有三列表样式 |
| 组件交互 | 通过 | 接收记录抽屉、去接收 / 上报接收差异入口均保留在新列表页 |
| 协作关系 | 通过 | Web 三列表与 PDA 现场接收继续通过 pickup-management-runtime 共享节点事实 |
| 异常与追溯 | 通过 | 仓储回写异常、接收差异上报入口不受影响；e2e 用例 2 已改用新入口并通过 |
...
| --- | --- | --- | --- | --- |
| 旧节点级工作台无路由注册，仅剩不可达的详情页（链接只来自旧工作台），构成死链路 | 追溯不足 | 裁床接收员 | 整条链路删除：页面文件、详情路由、渲染器、事件分发、meta 条目、check 脚本契约、e2e 旧入口 | 否 |
| 接收详情路由 /pickup-management-detail 无任何入口可达 | 追溯不足 | 裁床接收员 | 一并删除；其信息（节点版本、接收会话、仓储同步状态）已由新列表行内与"查看接收记录"抽屉覆盖 | 否 |
| e2e 用例 2 依赖旧页面"办理接收入库"按钮，清理后必然失败 | 点错风险 | 裁床仓管 | 改为从 /fcs/craft/cutting/pickup-management/ready 列表"去接收"入口进入 PDA 并完成全链路断言 | 否 |
...
- `npx playwright test tests/cutting-warehouse-location-map.spec.ts -g "PDA 中转仓接收"`：通过
