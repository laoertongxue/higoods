...
| 工程主单持久化失败被降级为内存成功，技术包打回事实可能跨刷新不一致 | 追溯不足 | 跟单、专业团队 | 仅工程仓储事务期间传播持久化失败；事务直接恢复工程内存快照并 best effort 恢复持久化，恢复失败不覆盖原始异常；非事务仍保留原型内存降级 | 否 |
...
- 工程主单仓储事务内的持久化失败会触发工程与技术版本嵌套事务整体回滚；事务外仍允许在原型存储不可用时保留内存事实。
...
- 工程仓储持续故障注入：打回抛出原始写入异常，工程内存快照恢复，技术版本、版本日志和审核通知不提交。
- `npm run build`：通过
