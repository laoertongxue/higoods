...
| 新增机型、针型后，旧 v2 快照因字段缺失整体回退 Mock，遮蔽既有业务事实 | `事实丢失` | 毛织主管、仓管、设备主管 | 严格校验前执行同版本轻量迁移；只补空缺规格，已知设备按 ID/编号映射，未知设备使用确定性通用值；完整校验通过后原键只回写一次 | 否 |
...
- 合法旧 v2 快照只补齐缺失或空白的机型、针型，其他业务事实深度保持不变；错误 schema 与畸形快照不迁移、不覆盖，迁移后再次读取零回写。
...
- 定向回归覆盖：严格锁定不可维护兄弟单隔离、已完成锁定、未知锁定、达到 150% 但仍有关联、无效保存零写入、A/B 直达切换、设备定向链接、离开重进清理、同页局部保留、debounce 取消、两类陈旧确认零写入、机型针型完整性、合法旧 v2 全事实保留与一次迁移回写、未知设备确定性补值、二次读取幂等、错误 schema/畸形快照零覆盖、单快照规模性能。
