...
- 固化上下文加固：首次保存继续从系统配置读取当时最新汇率，并按目标技术版本 `linkedPartTemplateIds` 的既定顺序解析真实模板版本摘要；模板缺失、多余、重排或任一版本字段不符均拒绝。固化时间、操作人和汇率来源绑定同一规范构建对象，手工拼装或深克隆不能冒充；持久化后的历史快照仍只按冻结事实离线自检，不受后续汇率或模板变化影响。
