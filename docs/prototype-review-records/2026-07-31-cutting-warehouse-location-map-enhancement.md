...
| 同工厂同类型多仓共享布局和历史快照 | `协作断裂` | 主管、文员 | v3 存储键包含 `warehouseId`；层级 Mock 直接重写，旧 v1/v2 缓存失效并恢复默认，不做旧库位迁移。 | 否 |
...
- 当时的 `npm run check:cutting-warehouse-location-map-e2e`：通过（12/12）；当前最终结果见第 9 节的 16/16。
...
结论：任务 8 当时的两条长路由冷启动观察已由任务 9 的独立服务与受控真实 render／handler 浏览器方案覆盖，不再作为当前未闭环事项；当前结论以本记录“任务 9 最终浏览器收口”为准。
...
- 任务 8 当时两条 PDA 路由未进入断言的结果仅作为历史执行记录；任务 9 已改用受控真实模块页执行相同生产 render／handler，并保留真实路由视觉／布局验收。

### 8.7 加工领料会话与扫码停用返修

- 加工领料只消费与当前物料、生产订单和明确领料会话相符的占用事实；同格其他物料不进入来源编号或库位集合。
- 页面未明确领料会话且存在多次有效入仓时，整次阻断并提示先选择具体入仓记录，不追加出库事件、不扣减库存。
- 待交出仓稳定码与历史事件中的旧扫码文本复用同一即时校验；这只是运行事实读取兼容，不是旧 Mock／布局迁移。当前工厂、仓库、库区、货架、库位均启用且业务状态为空才允许加入；任一停用或占用均不改变已选数组。
- 动态检查覆盖真实加工领料处理器的显式会话、无显式多候选阻断、同格其他占用隔离，以及新旧扫码的库区/货架/库位三层停用状态。

### 8.8 本次领料批次现场选择返修

- 单一有效入仓批次由系统自动绑定，不增加员工选择步骤；多个有效批次才展示“本次领料批次”选择。
- 多批次默认停留在“请选择本次领料批次”，不替员工预选；未选择时阻断开始加工领料并给出同名短提示。
- 选项只展示入仓时间、带单位的剩余数量和完整库位编号；会话编号、事件编号及系统技术词不在现场文案中显示。
- 选择控件和主按钮触控高度不低于 44px；真实处理器从当前页面控件读取选择，确认后只扣减所选批次并保留其他批次。
- 动态门禁从真实页面 HTML 解析多批次控件及选项，再通过与真实 DOM 一致的 `closest`、容器 `querySelector` 路径触发处理器；不再通过测试专用 `pickupSessionId` 属性绕过页面。

## 9. 2026-08-02 任务 9 最终浏览器收口

### 9.1 角色、流程和 Mock 边界

- PFOS 裁床主管／文员负责 Web 层级维护和占用追踪；PDA 裁床仓管员只执行扫码、自由多选、取消、清空和确认，不出现新增库区、货架、库位或停用入口。
- 库位业务状态仅为“空闲／占用”；停用是库区、货架、库位主数据的可用性。确认时同时校验主数据启用状态和业务空闲状态，任一异常整组失败。
- 待加工仓 `WAIT_PROCESS` 与待交出仓 `WAIT_HANDOVER` 各自保存布局、投影占用和释放事实；同一业务对象占多个格时数量与票据按稳定身份只计一次。
- 当前 Mock 直接写稳定层级结构和 `warehouseLocations`，不迁移旧布局或旧 Mock，不提供布局别名。历史单库位事件仅兼容读取，不再由 Web／PDA 新事件双写。

### 9.2 浏览器验收与性能

- 正式命令 `npm run check:cutting-warehouse-location-map-e2e` 使用独立开发服务、固定端口 `43176` 且禁止复用已有服务；最终 `16/16` 通过，0 失败，总耗时 8.6 分钟。
- 真实 PFOS 路由覆盖空结构维护、分页生成、重复／配额／占用冲突、取消隔离、占用详情和页面壳；受控 PDA 模块页直接调用真实生产 render／handler，覆盖跨区／跨架／跨层自由多选、任意取消、逐项摘要、清空及中转袋入仓完整数组确认。
- `1366×768`、`1280×720`、`1024×768` 和 `390×844` 均无页面级横向溢出；PDA 390 宽度不显示管理维护入口。
- 维护入口、预览、保存首次反馈和占用抽屉均以浏览器计时断言 `<200ms`；真实路由冷启动仅作为测试环境耗时，不替代也不计入页面内响应门禁。
- 大层级视窗限制首屏格子量，末层末位置可达；取消生成不产生版本历史，旧异步任务不污染新弹窗。

### 9.3 非浏览器事实补充与结论

- Web 多库位原子冲突、特殊工艺部分交出／回仓、PDA 特殊工艺、整袋释放和多批次领料由真实生产处理器专项动态门禁覆盖；浏览器用例不伪造当前源数据中不存在的特殊工艺任务。
- `npm audit --audit-level=low` 固定依赖审计为 0 个已知漏洞；原型设计治理、列表页治理、库位图专项、PDA／Web 流转专项和生产构建均通过。
- 结论：通过。没有未闭环的任务 9 浏览器事实；无产品设计例外。仍按产品原型边界使用本地 Mock 和浏览器事件账，不实现真实后端、跨设备并发锁或正式角色鉴权。
