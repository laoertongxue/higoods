...
- 未启用、非待开始 / 进行中或没有当前有效染色物料的任务，不展示确认动作，服务层同步阻断且不写确认时间。
...
| 扫码与识别 | 通过 | 本次不涉及扫码。 |
| 防错 | 通过 | 未启用、无有效物料、缺行、重复行、错任务、非染色行、缺字段和越阶段操作均阻断。 |
...
| 未启用或空物料任务可能误写确认时间 | 点错风险 | 跟单 | 页面不展示确认动作，服务层按任务状态与有效物料集合双重阻断 | 否 |
| 确认后只读物料可能一次铺满页面 | 字段过载 | 跟单 | 确认前后统一使用每页 8 条的局部分页 | 否 |
...
- `npm run test -- tests/pcs-engineering-color-task-ui.spec.ts`：通过（覆盖确认入口隐藏、确认后首屏 8 条与第二页 2 条）
