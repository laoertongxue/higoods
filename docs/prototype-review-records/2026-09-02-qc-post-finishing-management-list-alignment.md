...
- 用户可见影响：有
- 判定依据：四张页面的筛选区、统计卡、字段、分页和操作区重排；生产单级对象统一为“后道生产任务”，质检后实际工序对象统一为“后道加工单”，“质检任务”更名为“质检单”；新增可操作的 SKU 重量弹窗及质检单、质检详情单打印；后道加工单增加流转卡并明确 PDA/Web 分工；“查看来源任务”按任务号定位任务分配工作台；删除重复的手工“生成质检单”入口；回货最终确认自动生成“待送检”质检单，待加工仓只激活同一单；两类列表共用线上主质检单 / 质检详情单版式；同时移除重复列表标题栏、压缩固定列宽，把操作入口改为最多三行的双列网格，补齐“我的动态授权码”的钥匙图标。
...
| 防错、危险确认与主管兜底 | 通过 | 后道生产任务不提供手工创建质检单；回货确认建单与待加工仓送检分开；质检/复检主管释放保留；后道 Web 接管仍走既有门禁。 |
...
| 后道生产任务丢失 SKU 重量与质检查看/打印 | 协作断裂 | 管理人员 | 恢复重量弹窗、关联回货/质检查看及两类打印入口 | 否 |
| “质检任务”与线上“质检单”命名不一致 | 读不懂 | 质检员 / 管理人员 | 菜单、页面和相关用户文案统一为质检单 | 否 |
| 后道生产任务保留“生成质检单”会与回货最终确认自动建单重复 | 点错风险 | 管理人员 | 删除手工入口；确认时生成待送检单，待加工仓送检只激活同一单 | 否 |
| “查看任务”进入通用工作台但没有定位当前任务 | 协作断裂 | 管理人员 | 携带后道任务号进入任务分配工作台并加载对应任务 | 否 |
| 后道生产任务和质检单列表打印内容与线上版式不一致 | 组件误用 | 管理人员 / 质检员 | 两个入口共用线上主质检单 / 质检详情单渲染器和同一业务事实 | 否 |
| 后道加工单只有 PDA 或只有 Web 都无法兜底 | 协作断裂 | 后道操作员 / 管理人员 | 列表同时保留 PDA 优先、Web 详情、打印和追溯 | 否 |
...
- 页面结构和交互已按线上基线完成本地验收；任务精确跳转与两类打印版式已在当前工作树完成最终 Chrome 复验。质检单在回货最终确认时自动生成，待加工仓送检只激活同一单，后道生产任务页不再提供重复入口。
...
- `npm run check:prototype-design-governance -- --all`：通过（由列表治理执行；23 个用户可见文件，0 个仅技术文件，2 份关联记录）
...
- `codegraph sync`：通过（Already up to date）；`codegraph status`：1577 个文件、48510 个节点、168946 条边，无待同步文件。
- `npm run workflow:verify -- --output /private/tmp/qc-post-finishing-management-list-alignment-task-receipt.json --task-boundary <scope>`：未运行（本轮密度实质修改后早先收据已失效；当前工作树含既有未隔离修改，未生成会吸收无关差异的任务收据）

### 浏览器证据

- 后道任务及重量保存：历史重量保存证据 `.playwright-cli/page-2026-09-02T06-59-36-300Z.yml`；本轮最终 Chrome 页面确认每行只有“查看任务、设置 SKU 重量、查看质检单、打印质检单、打印质检详情单”，无“生成质检单”。
- 查看任务：当前 Chrome 从 `SEW-TASK-QC-001` 行实点进入 `/fcs/dispatch/workbench?search_field=task&keyword=SEW-TASK-QC-001&source=post-finishing`；工作台仅 1 条结果，SPU `HG-QC-001`、生产单 `PO-QC-202608-001`、工厂 `PT Prima Printing Center` 一致，并已保留本轮截图。
...
- 质检单打印：当前 Chrome 分别从质检单列表和后道任务列表实点进入 `QC_ORDER`；版式包含线上主标题、条码、产品图与吊牌价、买手 / 单据类型 / 售卖类型、5 条面辅料与实图、尺码表。直接命名页共 6 张对象图片，0 个可见加载失败态，并已保留本轮截图。
- 质检详情单：当前 Chrome 分别从质检单列表和后道任务列表实点进入 `QC_DETAIL`；版式包含线上抬头字段及 5 条 SKU 的待加工、待质检、质检数量、日期签名列，并已保留本轮截图。
