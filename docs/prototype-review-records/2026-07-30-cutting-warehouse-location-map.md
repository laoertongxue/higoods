...
| 数量与状态 | 通过 | 多格占用不重复累计同一业务对象数量；启停不扩展为空闲/占用之外的业务状态。 |
| 文案 | 通过 | 删除“连续相邻”“只能从两端取消”等已失效提示；冲突反馈使用完整中文库位编号。 |
...
- 为保持任务 5 前的共享组件编译兼容，`WarehouseLocationMapShelf.locations` 暂时由 `levels` 只读展平派生，不作为布局或选择事实；任务 5 完成分层矩阵渲染后移除。
