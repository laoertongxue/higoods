...
| 编号识别与防错 | 通过 | 库区编码限定 A 到 Z；货架和层位使用正整数；创建和编辑均预览完整 `A-R02-L03-P02` 编号。 |
| 占用保护 | 通过 | 占用节点及影响占用后代的结构字段禁用，并显示具体占用库位及“先出库或移库”的修正动作；备注仍可修改。 |
...
| 性能证据 | 有条件通过 | Playwright 场景保留按钮到维护 DOM 小于 200 ms 的断言；当前定向浏览器运行已完成新增空库区，但随后一次运行受冷启动／等待链阻塞被主动终止，不能把完整浏览器场景记为通过，任务 9 继续最终实测。 |

### 验证与例外

- `npm run check:cutting-warehouse-location-map`：通过，覆盖单入口、正式表单局部函数、维护态动作和无 `prompt()` 契约。
- `npm run build`：通过。
- `npx playwright test tests/cutting-warehouse-location-map.spec.ts --list`：发现 10 项。
- 定向 Playwright：真实发现并修复运行时漏导入；修复后新增空库区已保存并局部刷新，继续到新增货架步骤时一次运行长时间无新增输出，按当前任务边界终止，未记为通过。
- 原型仍使用浏览器本地存储，不接真实后端、权限、审批和数据库锁；这是既定原型边界。
- 库位启停的复合编辑仍由任务 3 的独立领域动作负责；本表单本次接入编号结构和备注编辑，不扩写任务 7/8 的入仓写回。
