...
- WAIT_HANDOVER 真实工作台与生产 handler 完成“源临时袋菲票装袋 → 三库位入仓 → 三格占用 → 生产分配／分拣确认 → 释放源临时袋并建立不同编码的目标中转袋及独立使用周期 → 目标袋整袋交出 → 三格释放”；全程 1 袋／3 个库位只汇总一次，空仓后不再渲染占用汇总区。
...
- `npm run check:cutting-warehouse-location-map-e2e`：通过，19/19、退出码 0、总用时 14.4 分钟；覆盖 WAIT_PROCESS 与源临时袋不同于目标中转袋的 WAIT_HANDOVER 真实处理器场景。
