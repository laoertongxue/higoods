...
| 数量与菲票口径 | 通过 | 部分交出 5 片后回仓 5 片，A/B 每格都同步为 20 片，业务汇总仍只计 20 片。 |
...
| 端侧边界 | 通过 | 本次仅修复共享运行时事实投影，不改动 Web 选位交互和 PDA 页面。 |

### 验证与例外

- `npm run check:cutting-warehouse-location-map`：动态覆盖 A/B 两格各20片、部分交出5片、回仓5片到A格、无库位回仓事实沿用原范围、整袋交出释放A/B；另覆盖 F1/W1、F2/W2 同袋同周期隔离、无位置多仓歧义和混合 scope payload 防错。
