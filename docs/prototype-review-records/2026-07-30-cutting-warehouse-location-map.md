...
| 摘要隔离与性能 | 通过 | footprint 包含仓库 scope 和使用周期；同袋两个活动周期各自计数。库位合并为线性 Map 扫描，无 `findIndex` 回流。 |
...
- `npm run check:cutting-warehouse-location-map`：动态覆盖相同事件集合反序确定性、20→15→20 票级数量、A/B 完整 footprint、同袋不同周期摘要隔离、F1/W1 与 F2/W2 跨 scope 隔离、无位置歧义、混合 scope 防错及整袋交出释放全部库位；源码门禁阻止 `findIndex` 回流。
