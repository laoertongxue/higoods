...
- `levels`、层内 `locations` 和临时 flat `locations` 均为运行时冻结的只读数组；flat 字段是无 setter 的派生 getter，不保存第二套可分叉数组。
...
| 仓储识别与防错 | 通过 | 自由多选不再制造物理相邻限制；错仓、停用、占用和未知库位仍阻断，重校验反馈列出全部冲突。 |
...
| 只读单事实 | 通过 | `levels` 是唯一分层事实；兼容 flat getter 每次只从 `levels` 展平并冻结，不能 push、splice 或整体替换。 |
| 文案 | 通过 | 删除已失效的相邻限制和端点取消提示；冲突反馈使用完整中文库位编号。 |
| UI、组件与性能 | 有条件通过 | 本任务不实施任务 5 的正式分层矩阵 UI；共享组件暂读 `levels` 展平出的兼容 `locations`，选择逻辑不依赖展平顺序。 |
...
- 为保持任务 5 前的共享组件编译兼容，`WarehouseLocationMapShelf.locations` 暂时作为无 setter 的只读派生 getter 存在，不作为布局或选择事实；任务 5 必须迁移 model、shared component、warehouse-hub、PDA 待加工仓、裁床库位图页和 CodeGraph 发现的所有新增消费者后彻底移除。
- 本任务不改 PFOS 层级维护弹窗，不改 Web/PDA 状态数组和占用写回；这些仍按后续任务 6、7、8 的边界处理。
