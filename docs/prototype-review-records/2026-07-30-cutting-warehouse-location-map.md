...
| 摘要隔离与性能 | 通过 | footprint 包含仓库 scope 和使用周期；同袋两个活动周期各自计数。1,000 格且每格携带 1,000 格完整范围的动态检查确认只读取一份 footprint，再以线性 Map 合并；无 `findIndex` 或重复 `flatMap(warehouseLocations)` 回流。 |
...
- `npm run check:cutting-warehouse-location-map`：动态覆盖相同事件集合反序确定性、20→15→20 票级数量、A/B 完整 footprint、同袋不同周期摘要隔离、F1/W1 与 F2/W2 跨 scope 隔离、无位置歧义、混合 scope 防错及整袋交出释放全部库位；1,000 × 1,000 退化输入通过 getter 计数确认完整范围只读取一次，源码门禁阻止 `findIndex` 和重复展开完整 footprint 回流。
