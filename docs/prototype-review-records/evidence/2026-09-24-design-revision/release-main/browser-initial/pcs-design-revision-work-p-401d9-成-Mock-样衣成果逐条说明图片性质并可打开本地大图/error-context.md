...
- Name: pcs-design-revision-work-preview.spec.ts >> 已完成 Mock 样衣成果逐条说明图片性质并可打开本地大图
- Location: tests/pcs-design-revision-work-preview.spec.ts:144:1
...
Locator: getByRole('dialog').getByRole('status')
Expected: "图片加载中…"
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByRole('dialog').getByRole('status')
...
- main:
  ...
  - article:
    ...
    - button "查看大图"
    - link "下载":
      - /url: /materials/pcs-reviewed/hood-black.jpg
  ...
  - dialog "es-id-dr-002-display_sample-1-1.jpg大图":
    - button "关闭大图"
    - paragraph: es-id-dr-002-display_sample-1-1.jpg
    - button "关闭"
    - paragraph: Mock 演示图片 · 未核验为本次实际制作样衣的实拍照片。
    - img "es-id-dr-002-display_sample-1-1.jpg高清大图"
- button "查生产":
  - img
```
...
> 156 |   await expect(dialog.getByRole('status')).toHaveText('图片加载中…')
  ...
  175 |   await expect(page.getByRole('heading', { name: '成果记录', exact: true })).toBeVisible()
