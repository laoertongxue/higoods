async(page)=>{
 ...
 const images=async()=>{await p.waitForFunction(()=>[...document.querySelectorAll('[data-hpb-label] [data-real-qr]')].every(n=>n.querySelector('svg')));return p.evaluate(async()=>{await Promise.all([...document.querySelectorAll('[data-hpb-page] img')].map(i=>i.decode().catch(()=>{})));await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)))});};
