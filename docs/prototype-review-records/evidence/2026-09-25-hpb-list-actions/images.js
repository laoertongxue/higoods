async(page)=>{
 ...
 await p.locator('[data-hpb-action=detail]').first().click();await p.locator('[role=dialog] img').first().evaluate(img=>img.src='/missing-hpb-ui-material.png');await p.waitForFunction(()=>[...document.querySelectorAll('[role=dialog] span')].some(n=>n.textContent==='图片失败'&&!n.hidden));
