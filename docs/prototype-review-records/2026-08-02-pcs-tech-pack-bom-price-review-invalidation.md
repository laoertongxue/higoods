...
| UI 样式 | 通过 | 本次未改 UI。 |
| 组件交互 | 通过 | 本次未改组件。 |
| 协作关系 | 通过 | 版师、跟单审核结论保持；买手已有“审核-已通过／审核-未通过／无需审核”完成结论时，BOM 与价格变化后重建买手差异快照、追加日志并通知原指定买手；买手复审通过后直接恢复待发布。 |
...
| 买手正在等待审核或审核中时再次生成复审事实 | 点错风险 | 买手 | 只有买手状态已有“审核-已通过／审核-未通过／无需审核”完成结论时才能失效；待审核、审核中沿用当前节点，不重复生成日志和通知 | 否 |
| 系统判定“无需审核”后 BOM 与价格变化仍沿用旧结论 | 算不准 | 买手、跟单 | “无需审核”与审核通过、审核未通过统一视为既有完成结论；发生真实变化后重新生成买手差异、日志和通知 | 否 |
...
说明：本次把已确认的精确复审规则同时接入专用 BOM 与价格保存器和技术包 BOM 弹窗通用保存链，并将差异快照、审核日志、飞书通知抽取为审核生命周期公共能力；买手已有通过、未通过或无需审核结论时才失效，未提交、待审核、审核中的版本不被重复送审。技术版本事务同时覆盖差异节点、版本日志和审核通知，后续工程联动失败不会遗留孤儿副作用；事务测试按 localStorage key 区分快照初始化与回滚写入，逐仓验证只恢复一次。测试覆盖新增、删除、同价换 SKU、单位用量、打样数量、损耗率、用量单位、标准价、汇率、同值保存、不相关内容、三类无结论状态、无需审核完成态及六仓失败回滚；没有新增页面，也没有涉及专业任务返工、正式快照、工程主单关闭或历史任务。
...
- `tests/pcs-repository-sync-transaction.spec.ts`
...
- `node --import tsx tests/pcs-repository-sync-transaction.spec.ts`：通过
