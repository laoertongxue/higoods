...
当前代码的整件毛织任务明细按成衣 SKU 和“件”生成；部位毛织任务明细按“毛织部位 + 成衣颜色尺码”生成，计划片数为成衣计划件数乘单件所需部位片数。原设计草稿统一使用 `garmentSkuCode`，会导致部位毛织的计划量、150% 上限、库存和交出对象全部失真。
...
| 部位毛织 | 毛织部位 SKU | 成衣 SKU 计划件数 × 单件该部位所需片数 | 片 |
...
3. `patternId`、部位名称或单件片数缺失时，不生成可填报计划行，页面显示“技术包毛织部位资料不完整”。
4. 生成后的部位 SKU 在加工填报、待交出仓、交出和下游接收中保持同一编码。

示例：黑色 M 码成衣计划 100 件，每件需要 2 片袖片，则黑色 M 码袖片加工后 SKU 的计划量为 200 片，累计加工填报上限为 300 片。
...
同一加工单可多次交出，同一加工后 SKU 也可多次交出。保存交出与扣减默认库位库存必须是一次原子操作。部位毛织扣减裁片默认库位，整件毛织扣减成衣默认库位；库存不足时整次保存失败。

下游确认接收前允许直接修改交出数量，系统按差额扣减或返还库存。下游确认接收后交出数量锁定；下游实际接收数量与交出数量不一致时保留差异，不自动恢复毛织库存。
...
```text
当前库存(outputSku) = 累计加工填报入库 - 累计交出出库 ± 独立库存调整
0 < 本次交出数量 ≤ 对应默认库位当前库存(outputSku)
```
...
| 发起交出 | 加工单完成前且下游未确认 | 按差额扣减或返还裁片/成衣默认库位 | 修改后数量不大于 0、增加后库存不足，或下游已经确认 |
...
16. 部位毛织按部位 SKU 和片数计算计划量及 150% 上限。
...
- [ ] 部位毛织计划片数等于成衣计划件数乘单件部位片数。
- [ ] 单加工后 SKU 累计加工填报不超过计划数量的 150%。
- [ ] 单加工后 SKU 累计交出不超过累计加工填报及当前库存。
- [ ] 没有加工填报时不能发起交出。
- [ ] 没有交出记录时不显示完成加工单。
- [ ] 完成加工单不做系统数量充分性判断。
...
技术包只负责定义各加工后 SKU 必须具备哪些纱线；系统只判断这些纱线是否存在确认接收，不把纱线数量换算为最多可加工件数。整件毛织按成衣 SKU 和件管理，部位毛织按毛织部位 SKU 和片管理。毛织模块不再保留旧节点、菲票、统计、价格、“已排产”或独立完工入仓逻辑，从而消除当前 Web、PDA、仓库和设备之间的状态耦合。
