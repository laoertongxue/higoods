...
- 旧“横机成片、缝盘、两项旧后道、菲票打印”节点及相关代码、字段和文案的清理边界。
...
| 加工单详情 | `src/pages/process-factory/wool/work-order-detail.ts` 全文 | 页签直接包含“横机成片”“缝盘与旧后道”“毛织菲票” | 改为业务概览、款色用料、接收记录、加工填报、交出记录、横机关联和操作记录 |
...
| PDA 执行 | `src/pages/pda-exec-detail.ts` 毛织渲染、前置判断和事件处理相关段落 | PDA 仍按接单、开工、横机里程碑、缝盘、两项旧后道、菲票和自动完工运行 | 改为与 Web 同源的确认接收、加工填报、发起交出和人工完成 |
...
| 技术包类型 | `src/data/pcs-technical-data-version-types.ts:315-345` | 毛织工艺含菲票和旧后道必做等旧执行要求 | 删除毛织专属的菲票打印和旧后道节点配置；保留任务类型、交出对象等仍有效字段 |
...
| 技术包页面 | `src/pages/tech-pack/process-domain.ts:94-210, 532-646` | 毛织规则卡展示菲票规则、旧后道节点开关、固定交出说明 | 删除旧节点文案和开关；款色用料关系成为开工判断的唯一技术依据 |
| 技术包快照页 | `src/pages/fcs-production-tech-pack-snapshot.ts:430-465` | 执行要求列展示“打印毛织菲票”和毛织旧后道必做项 | 删除两项毛织执行要求 |
| 技术包样例 | `src/data/pcs-technical-data-version-bootstrap.ts:204-265` | 样例写死两项旧后道和打印菲票 | 改为接收、加工填报、交出语义 |
...
| 生产工艺产物 | `src/data/fcs/production-artifact-generation.ts:307-321` | 自动写入菲票和旧后道必做要求 | 删除旧要求的产物字段 |
| 里程碑配置 | `src/data/fcs/milestone-configs.ts:209-258` | 毛织配置明确要求“横机首批上报” | 删除毛织专属横机里程碑配置，避免 PDA 重新生成旧动作 |
| 工艺字典 | `src/data/fcs/process-craft-dict.ts` 毛织定义相关段落 | 毛织工艺备注仍写两项旧后道和部位打印菲票 | 只清理毛织工艺定义中的旧描述；后道统一以烫包为当前工序 |
...
| 毛织价格投影 | `src/data/fcs/wool-task-domain.ts:1049-1059, 2447-2453`、`src/data/fcs/page-adapters/task-execution-adapter.ts:308-313` | 价格由横机、缝盘和两项旧后道分钟估算，并投影为标准价和派工价 | 删除毛织价格结构、估价、页面展示、移动任务投影和结算投影；不以 0 或“待维护”占位 |
...
当前状态由 `deriveWoolOrderStatus()` 根据“横机成片、缝盘、两项旧后道、菲票打印、交出、仓库收货”串行推导。用户要求的三类可重复事实无法嵌入这条单向链路。
...
当前毛织价格由横机、缝盘和两项旧后道分钟估算，并继续投影到移动任务的标准价和派工价。删除旧节点后继续保留价格字段会形成没有业务来源的虚假价格。

#### 冲突八：旧完工入仓会自动推进全部节点

当前 `advanceWoolOrderToWarehouseInbound()` 会从接单、接收、排机开始循环推进，直到完成横机、缝盘、两项旧后道或菲票节点。新模型中加工填报本身即形成待交出仓入库事实，该旧函数及入口不能保留。
...
9. 不再自动写已接单、已开工、横机里程碑、菲票、旧后道必做项或价格。
