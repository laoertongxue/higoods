...
页面不让跟单手工维护生产进度。系统根据需求、物料、接收、裁剪、加工、完工、库存、结算等事实计算当前状态和可处理范围。

### 2.3 历史事实不可覆盖

已接收、已裁剪、已加工、已完工、已入库和已结算事实保持原值。生产单变更只能：
...
```mermaid
flowchart TD
  逐条数量["逐条读取变更前后数量"] --> 逐条事实["逐条读取已接收、已裁剪、已加工和已完工数量"]
  逐条事实 --> 判断进度{"关联单据当前进度"}
