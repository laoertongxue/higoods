...
| 后续顺序 | 物料／商品档案 → 商品测款 |
| 暂缓范围 | 样衣管理模块 |
| 设计依据 | 老系统制版、改版、花型、染色页面，当前技术包列表，现有生产准备时效规则及本轮业务确认 |
...
8. 新业务不允许绕过工程主单直接新增技术包，不提供导入历史技术包功能。
...
- BOM 已明确需要印花或染色时，对应花型／调色任务必须生成，跟单不能删除。
- 主单发布时一次性生成全部确认任务。
...
- BOM 新增印花或染色物料时，系统自动加入对应花型／调色任务。
- 缺少任务时自动生成并补齐固定前置依赖。
- 已完成的同类任务不重复新建，而是在原任务中新增“需求变更轮次”。
- 原已通过成果保持不变，只处理新增物料。
- 任务重新进入待处理／进行中。
- 生产准备时效保留第一次完成记录，当前有效完成时间更新为新增物料最终通过时间。
...
| 当前情况 | 处理方式 |
| --- | --- |
| 未开始 | 标记“需求已变更，无需执行”，不参与完成门禁 |
| 已开始、未提交 | 标记“需求变更停止”，保留开始时间、耗时和已填写内容 |
| 已提交、未审核 | 保留成果并标记“成果已提交，需求已变更”，不再审核，不进入正式成果库 |
| 已审核通过 | 保留审核和成果历史，成果标记“当前款式已停用”，新技术包不再引用 |

补充规则：

- 任务还有其他有效物料时继续执行。
- 所有物料均不再需要时，任务状态为“因需求变更结束”，不是人工取消。
- 生产准备时效区分“正常完成”和“因需求变更结束”。
...
当前阶段可以保留原页面确认准备项入口作为迁移过渡，但最终只能保留工程主单一个任务入口；生产准备时效变为完全只读。
...
- 选中任务后，高亮其全部上游和下游依赖。
- 点击任务卡片打开右侧详情抽屉。
- 页面不堆放业务说明文案；只保留操作所需提示、真实异常和门禁原因。

### 13.2 历史输入区

- 前期打样和历史专业成果显示在主工作台顶部“历史输入”区域。
- 复用成果以只读卡片展示。
- 使用虚线连接到其满足的工程任务。
...
1. 采购人员在采购系统下单。
2. 在 PCS 辅料下单任务中绑定采购单号。
3. 系统读取采购信息和实际下单时间。
4. 无效单号阻断；有效单号形成任务完成事实。

## 18. 实施顺序建议

1. 建立工程主单、专业任务及固定依赖的原型骨架。
2. 完成全宽泳道工作台和任务详情抽屉。
3. 打通 11 个生产准备项与专业任务节点。
4. 完成花型、调色的逐项审核和返工。
5. 完成 BOM 与价格及物料档案标准单价联动。
6. 完成辅料采购单绑定。
7. 串联现有技术包审核、发布和工程主单关闭门禁。
8. 将生产准备时效收口为只读投影。
