...
2. 部分生产场景下，多个前后连续的工序可以交给同一个工厂连续完成，例如车缝+后道。此时如果仍拆成多个独立任务，会增加分配、交接、接收和 PDA 操作成本。
...
6. PDA 步骤使用简化流程：接收 -> 开工 -> 关键节点上报 -> 交出 -> 完工。
