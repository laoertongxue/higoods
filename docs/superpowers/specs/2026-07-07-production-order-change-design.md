...
- 下游配料、接收、裁剪、印花、染色、车缝、后道、结算都能感知变更。
...
实际影响范围不是业务人员选择项，而是系统计算结果。系统根据变更内容、期望生效口径、当前进度、已生成单据、已接收、已消耗、已结算数据，计算受影响的颜色、尺码、批次、工序、数量和单据。
...
| 单据处理区 | 单据处理行 | 配料、接收、裁剪、印花、染色、菲票、车缝、结算怎么处理 |
...
| --- | --- | --- | --- |
| 第 1 步：选择生产单 | 搜索并选择生产单 | 展示该生产单完整事实：技术包、进度、色码批次、已生成单据、已接收、已消耗、已结算 | 不能只给一个下拉框，不展示当前事实 |
...
| 生产进度 | 配料、接收、裁剪、印花、染色、车缝、后道、发货准备 |
| 已生成单据 | 配料单、接收单、裁剪单、印花单、染色单、菲票、车缝 / 后道工单、结算单 |
...
3. 读取最新正式技术包版本。
4. 读取颜色、尺码、批次、计划数量。
5. 读取配料、接收、裁剪、印花、染色、车缝、后道进度。
6. 读取已生成单据。
7. 读取已接收、已消耗、已结算、已付款数据。
...
| 接收单 | 不动 / 部分改领 / 退旧领新 / 追加接收 | 已领未用可退旧，已用旧料不追回 |
...
示例：只变更 Black / M 面料时，只锁定 Black / M 的接收、裁剪、结算确认，不影响 White / S 继续生产。
...
| 角色 / 页面 | 展示内容 | 文案示例 |
| --- | --- | --- |
| 生产单列表 | 变更数量、审核状态、锁定范围、风险 | 有变更 2、审核中、Black / M 接收已锁定 |
| 生产单详情 | 当前版本、补丁、影响摘要 | 当前采用正式版 v1.1，存在 1 条物料补丁 |
| 配料 | 改配、补配、取消重开 | Black / M 后续改配 FAB-B02 |
| 接收 | 改领、退旧领新、追加接收 | 未领 700 米改领 FAB-B02 |
...
| 2 | 新技术包发布，已配料未接收 | 计算配料影响，建议改配 |
| 3 | 新技术包发布，已接收未裁剪 | 建议部分改领或退旧领新 |
