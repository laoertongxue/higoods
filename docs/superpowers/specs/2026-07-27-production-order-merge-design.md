...
- 「未领料」的生产单不可参与合并。
- 原生产单已有领料记录、物料批次、库位和数量仍归属于原生产单。
- 合并时不得复制领料记录或重复增加库存出库数量。
- 合并生产单引用来源单的有效领料事实，供裁床判断可用物料。
- 原生产单尚未完成的领料需求汇总到合并生产单继续执行，并保留来源需求明细。
...
2. 系统校验 SPU、技术包版本、领料、唛架、铺布、裁片及下游事实。
...
`PO000101` 和 `PO000102` 属于同一 SPU、同一技术包版本，均已部分领料，没有有效唛架、铺布或裁片事实。

合并后生成 `PO-H000001`。原领料事实仍在原生产单下，新的唛架、铺布、菲票、中转袋和车缝任务归属于 `PO-H000001`。
