...
- 一页一个主动作。
- 扫码是中转袋、菲票、库位和车缝任务的主要录入方式。
- 扫描后立即显示识别结果。
- 扫描成功后自动把焦点推进到下一步扫码对象。
- 手工编号输入只作为扫码失败时的主管兜底，不与主扫码动作同等突出。
- 编号同时配合生产单和袋内数量识别。
- 不让员工心算或填写交出数量。
- 不展示来源关系、系统状态码或复杂流水。
- 扫错时阻断并给出修正动作。
...
- 装袋时输入袋号和菲票号；入仓时输入袋号和库区库位；交出时输入袋号和车缝任务号。
- 输入编号后由系统查询并带出生产单、袋内数量、库位和接收工厂。
- 页面不依赖摄像头、二维码组件或扫码设备即可完成操作。
- 列表按中转袋一行展示。
- 支持按袋号、生产单、库区库位、状态、车缝任务和接收工厂查询。
- Web 与 PDA 使用相同的业务校验和袋级事实。
- 详情可查看袋内菲票，但不能在入仓或交出动作中勾选菲票。
- 数据列表必须使用分页。
