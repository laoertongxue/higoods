...
袋属于 `PO000123`，任务属于 `PO000124`。系统阻断并要求扫描正确任务，不允许通过修改数量或选择部分菲票继续。

### 15.4 重复扫描已交出袋

员工再次扫描 `BAG-0008`。系统显示该袋已于具体时间交给具体车缝任务，不新增交出记录。
...
- 角色与端类型：PDA 面向装袋、待交出仓和交出人员；Web 面向文员和主管。
- 任务清晰度：装袋、入仓、交出各自只有一个主动作。
- 扫码与识别：入仓扫袋和库位，交出扫袋和任务。
