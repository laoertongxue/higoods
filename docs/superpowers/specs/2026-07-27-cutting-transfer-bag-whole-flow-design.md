...
13. 裁床工厂 PDA「仓管」Tab 的「待交出仓」模块保留 5 个操作：「菲票装袋」「中转袋入仓」「中转袋交出」「特殊工艺回仓」「菲票打编号」。
14. 现有「交出装袋确认」统一更名为「中转袋交出」。
15. PDA 不再展示「待入仓菲票」列表或以菲票卡片作为操作入口。
16. 点击「菲票装袋」「中转袋入仓」「中转袋交出」后直接进入对应扫码操作页，不经过候选列表、任务详情或信息总览页。
17. 「特殊工艺回仓」「菲票打编号」保留为待交出仓模块的独立操作，本规格不改造其既有业务流程。
18. 菲票装袋成功后先提示「装袋成功」，随后清空本次袋号和菲票数据，刷新为下一次全新的菲票装袋界面。
...
- PDA 待交出仓模块保留 5 个操作，其中 3 个中转袋操作从入口直接进入扫码页，不经过待办或候选列表。
...
裁床工厂 PDA「仓管」Tab 的「待交出仓」模块显示以下 5 个操作按钮：
...
「特殊工艺回仓」「菲票打编号」保留为同一待交出仓模块内的独立操作入口，但不进入中转袋装袋、入仓、交出链路。本规格不改变这两个操作的既有业务流程。
...
- 点击「确认装袋」后，按钮进入提交中状态，防止重复点击。
- 保存成功后先提示「装袋成功」。
- 成功提示后清空当前袋号、已扫菲票、数量和最后扫描结果，将页面刷新为全新的菲票装袋界面，并重新等待扫描中转袋。
- 刷新后仍停留在「菲票装袋」页面，不返回待交出仓首页，也不进入待入仓菲票列表。
- 保存失败时保留当前袋号和已扫菲票，不刷新页面；只显示失败原因，允许修正后重新确认。
...
员工进入「菲票装袋」后直接扫描袋 `BAG-0008`，随后连续扫描本次要装入的菲票。页面只累计已扫数量并显示最后一张的扫描结果；员工点击「确认装袋」后，页面提示「装袋成功」，随即清空本次数据并刷新为等待扫描下一只中转袋的全新装袋界面。
...
- [ ] 裁床工厂 PDA「仓管」Tab 的「待交出仓」模块显示「菲票装袋」「中转袋入仓」「中转袋交出」「特殊工艺回仓」「菲票打编号」5 个操作。
- [ ] 「交出装袋确认」在页面、动作和新事件中统一更名为「中转袋交出」。
- [ ] 点击 3 个中转袋操作均直接进入对应扫码页。
- [ ] PDA 不再展示「待入仓菲票」列表或菲票候选卡片。
- [ ] PDA 菲票装袋严格为扫中转袋、扫菲票、确认装袋 3 步。
- [ ] PDA 3 个中转袋操作均为一页一个主动作。
- [ ] 「特殊工艺回仓」「菲票打编号」仍可从待交出仓模块直接进入原有操作流程。
- [ ] 菲票装袋成功后提示「装袋成功」，随后清空本次数据并刷新为新的菲票装袋界面。
- [ ] 菲票装袋保存失败时保留已扫数据，不错误清空页面。
- [ ] PDA 扫描成功后自动进入下一步扫码。
...
- 相关检查脚本
  ...
  - 增加待交出仓 5 个操作、3 个中转袋直达入口、无「待入仓菲票」、菲票装袋仅 3 步、装袋成功重置和无冗余信息卡片的门禁。
...
- 任务清晰度：待交出仓模块保留 5 个明确操作；其中装袋、入仓、交出 3 个中转袋操作直接进入扫码页且各自只有一个主动作。
