...
- 保留当前已有的“直喷加工单”，不改变其独立业务身份。
- 新增独立的“烫画加工单”，不能把烫画任务记录在直喷加工单中，也不能只在直喷加工单上增加一个工艺名称字段来兼容。
- 两类加工单分别建单、分别编号、分别展示工艺信息、分别执行和追踪状态；编号均由平台加工单编号能力生成，烫画加工单不得复用直喷加工单号。
- 两类加工单可以复用辅助工艺的公共仓库、交接、PDA 和状态展示能力，但业务类型、详情字段、打印内容和统计口径必须可明确区分。
- 同一生产单同时存在直喷和烫画时，分别生成直喷加工单和烫画加工单，不合并为一张辅助工艺加工单。
- 烫画加工单、直喷加工单均支持“裁片部位”和“成衣”两种作用对象，并按作用对象决定单位、菲票、上游和下游。
...
生产单冻结技术包后，系统按已确认工艺分别生成：

- 工艺为直喷：生成现有直喷加工单。
- 工艺为烫画：生成新增烫画加工单。
- 同时存在直喷和烫画：同时生成两张不同业务类型的加工单。

烫画加工单至少保存：烫画加工单号、来源生产单、冻结技术包版本、作用对象、关联裁片部位或成衣 BOM、适用 SKU、计划数量、单位、加工工厂、计划时间和执行状态。它进入辅助工艺的收货、加工、待交出、交出和下游收货链路，但不借用直喷加工单作为承载对象。
...
### 13.5 直喷、烫画加工单和辅助工艺仓库

- 保留现有直喷加工单入口，新增独立烫画加工单入口、列表和详情。
- 直喷与烫画分别使用自己的加工单号和业务类型，不合并为同一张单。
- 每类加工单均可以展示裁片任务和成衣任务。
...
flowchart TD
  ...
  GARMENT_BOM["成衣类型 BOM"] --> DIRECT["直喷加工单"]
  GARMENT_BOM --> HEAT["新增烫画加工单"]
  GARMENT_WH["成衣仓"] --> DIRECT
  GARMENT_WH --> HEAT
  DIRECT --> AUX_OUT["辅助工艺待交出仓"]
  HEAT --> AUX_OUT
  AUX_OUT --> POST["我方后道工厂"]
```
...
- 工序工艺字典中烫画、直喷作用对象收口。
- 保留现有直喷加工单，并新增独立烫画加工单及其列表、详情、编号和执行链路。
- BOM 成衣类型及其技术包交互。
- 技术包先染后印路线校验。
- 裁片和成衣两套辅助工艺生成与流转。
...
1. 现有直喷加工单继续独立存在，新增烫画加工单，二者不共用同一张业务单。
2. 同一生产单同时配置直喷和烫画时，分别生成两张加工单并使用各自加工单号。
3. 烫画加工单具备独立列表、详情、打印和状态追踪，并复用辅助工艺公共仓库与交接能力。
4. 裁片烫画、直喷按片生成并保留菲票和回裁床链路。
5. 成衣烫画、直喷按件生成，不出现裁片部位和菲票。
6. 成衣任务从成衣仓出库进入辅助工艺待加工仓。
7. 加工完成后进入辅助工艺待交出仓。
8. 交出接收方为我方后道工厂，收货后进入现有后道链路。
9. 差异、报废、货损、交出和实收数量可完整追溯。
...
- 无待定项、TODO 或占位符。
- 已明确保留现有直喷加工单并新增独立烫画加工单，两者不会合单或借单承载。
