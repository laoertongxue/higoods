...
- 独立车缝任务。
- 从车缝开始、连续执行到后道烫包结束的连续工序任务。
- 从裁片开始、连续执行到后道烫包结束的连续工序任务。
- 直接派单和竞价分配两种分配方式。
...
| 车缝到烫包连续任务 | 接单后120小时 | 接单后216小时 | 接单后240小时 |
| 裁片到烫包连续任务 | 接单后144小时 | 接单后216小时 | 接单后288小时 |
...
连续任务从车缝开始，并连续覆盖到后道烫包结束时，适用10天交付要求：

- 满120小时，累计有效实收比例不低于30%。
- 满216小时，累计有效实收比例不低于70%。
- 满240小时，累计有效实收比例不低于100%。

中间存在特殊工艺、装扣、烫包或其他连续工序时，不改变该分类。不能把该类型限制为恰好只有“车缝、后道”两个节点。

### 4.3 裁片到烫包连续任务

连续任务从裁片开始，并连续覆盖到后道烫包结束时，适用12天交付要求：

- 满144小时，累计有效实收比例不低于30%。
- 满216小时，累计有效实收比例不低于70%。
- 满288小时，累计有效实收比例不低于100%。

中间包含车缝、特殊工艺、装扣、烫包或其他连续工序时，仍按裁片到烫包连续任务处理。
...
- 车缝到烫包正确使用120、216、240小时节点。
- 裁片到烫包正确使用144、216、288小时节点。
