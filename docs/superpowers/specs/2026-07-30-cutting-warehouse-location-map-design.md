...
2. 待加工仓库存、领料会话和待交出仓部分事件主要保存库区、库位文本，没有完整稳定路径 ID。
...
两个页面现有“库区库位”页签统一改名为“库位图”。库存、领料记录、使用记录、退料、装袋、入仓、交出等既有页签和弹窗工作台保持不变。
...
多选库位表示一条领料会话的“存放范围”，不是把总数量复制成多条独立库存：

- 领料会话只保存一次总数量、卷数和物料明细。
...
| 待加工仓仓管 | PDA；必要时 iPad | 扫码/选位、连续多选、确认领料存放、调整剩余存放范围 | 跨货架选位、选择占用或停用库位 |
...
| 领料存放 | 领料会话、生产单、物料、总量、库位 ID 列表、操作人、时间、幂等键 |
...
| `src/data/fcs/cutting/production-material-prep.ts` | 领料会话、单库位文本、待加工运行时事件 | 增加一次性存放范围和稳定路径 |
| `src/pages/pda-warehouse-wait-process.ts` | `FAB-*` 硬编码、单库区/单库位下拉 | 迁移编码并接入连续多选 |
| `src/pages/process-factory/cutting/warehouse-hub.ts` | 两张静态库位表、PFOS 领料/入仓/回仓弹窗、现有页签 | 原路由原工作台内替换为库位图 |
