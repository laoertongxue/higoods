...
- 下游事实：裁床接收记录、裁床待加工仓入仓

本文档延续《中转仓配料与裁床接收完整关系设计》中“一张生产单对应一张持续汇总的裁床配料单、同一时刻最多一个有效待领节点、当前节点必须整批领取”的业务结论，并补充以下内容：
...
当前接收管理将未配齐、已配齐待领和历史已领放在同一页面中。业务人员需要先进入页面，再通过页签或筛选寻找任务。物料、数量和历史结果也有部分信息需要进入详情后才能核对。

本次目标是：

1. 将“接收管理”提升为工艺工厂运营系统 Web 的一级菜单。
2. 一级菜单下设置“已配齐待接收”“未配齐配料”“已接收”3 个二级菜单。
3. 让业务人员在列表直接看到生产单关联的全部物料，不必为核对物料进入详情。
4. 未配齐配料可以直接接收，但必须一次领走当前节点全部物料。
5. 已接收列表明确区分“已配齐后接收”和“未配齐先领”，并判断生产单是否最终完成全部接收。
6. 接收判断同时覆盖正常需求、染色、印花和多次补料。
...
| 角色 | 端类型 | 主要任务 | 页面要求 |
| --- | --- | --- | --- |
| 裁床仓管 | Web / PDA | 找到当前待领物料、核对位置和数量、确认接收 | 直接看到物料和位置，数量无需心算 |
| 裁床接收人员 | Web / PDA | 按生产单领取当前全部物料 | 主动作明确，不允许挑物料或挑库位 |
| 裁床主管 | Web | 查看缺口、处理差异、追溯接收结果 | 能看到完整累计关系和异常证据 |
...
| 累计有效配料数量 | 历次已经形成有效接收的数量，加上当前有效待领节点中的数量 |
| 历史有效已领数量 | 已经完成接收且未被作废、退回或冲销的数量 |
...
| 当前有可领物料，且所有有效需求行累计缺口均为 `0` | 已配齐待接收 |
...
3. 当前不存在尚未领取的有效待领节点。
4. 当前不存在新增但尚未完成接收的有效补料。

只要生产单产生过一条有效接收记录，就进入“已接收”列表。未达到以上条件时，显示“未完成全部接收”或“新增补料待领”。
...
- 原“裁前准备 > 接收管理”菜单入口移除，避免出现两个业务入口。
- 3 个二级菜单分别进入独立列表，不使用一个页面中的 3 个页签替代。
- PDA 保留统一“待接收 / 扫码接收”入口。
- Web 和 PDA 读取同一当前待领节点、物料、位置和接收记录。

### 8.2 页面归属

- “已配齐待接收”和“未配齐配料”按当前有效待领节点展示，同一个节点只能出现在其中一张列表。
- “已接收”按生产单累计展示，不与当前待办互斥。
- 生产单曾经未配齐领过物料，后来又形成新待领节点时，可以同时出现在“已接收”和一张当前待办列表。
...
| 接收事实 | 页面表达 |
| --- | --- |
| 第一次领取前已经累计配齐，且从未发生未配齐接收 | 已配齐后接收 |
| 曾经领取过未配齐节点 | 未配齐先领 |

同一生产单先未配齐接收，最后再领收尾节点时，接收路径仍显示“未配齐先领”，最终结果可以同时显示“全部领完”。

### 12.4 最终结果

| 条件 | 页面表达 |
| --- | --- |
| 所有正常需求和有效补料均完成全部接收 | 全部领完 |
| 已经领过物料，但仍有正常需求、有效补料或必需加工尚未完成 | 未完成全部接收 |
...
- 主操作：接收记录。
- 接收记录承接每次领取人、领取时间、物料明细、来源库位、托盘号、裁床接收位置和异常证据。
...
不得出现接收记录已经生成但库位未释放，或库位已释放但接收记录未生成的中间结果。
...
- 提供一级菜单和 3 张独立列表。
- 展示生产单累计关系、物料明细、补料、加工路线、库位和接收结果。
...
- Web、PDA、中转仓和裁床待加工仓读取同一接收事实。
- 每次接收可追溯领取人、时间、物料、来源位置和接收位置。
- 差异有结构化入口和主管兜底。
- 多次补料独立保留，不覆盖原需求或前次补料。

## 21. 验收标准

1. 工艺工厂运营系统 Web 出现“接收管理”一级菜单及 3 个二级菜单。
2. 原“裁前准备 > 接收管理”菜单入口不再重复展示。
3. 当前有效待领节点只出现在“已配齐待接收”或“未配齐配料”之一。
4. 未配齐配料可以执行整批接收。
5. 未配齐接收必须覆盖当前节点全部物料和全部库位。
6. 列表按生产单分组、按物料需求行展开。
7. 同一物料的多个库位及各库位数量在列表直接可见。
8. 无需加工、只染色、染色后印花的应配数量口径正确。
9. 染色后印花只取印花最终完成数量，不与染色数量相加。
10. 正常需求和每次补料独立展示、独立计算。
11. 新增补料会重新打开生产单总体接收状态。
12. 已配齐列表正确区分一次直接配齐和从未配齐升级配齐。
13. 当前未编号托盘显示“待领托盘（暂未编号）”。
14. 未来托盘有编号时可以展示并支持扫码。
15. 已接收列表正确显示“已配齐后接收”或“未配齐先领”。
16. 已接收列表正确显示“全部领完”“未完成全部接收”或“新增补料待领”。
17. 同一生产单的物料行不跨页。
18. 实物、数量、单位、库位或节点版本不一致时阻断接收。
19. 重复提交不生成第二条接收主记录。
20. PDA 菜单不拆分，但与 Web 读取同一当前任务和接收结果。

## 22. 最终设计结论

本次采用“一级菜单 + 3 张独立列表”的方案：

- “已配齐待接收”回答：哪些生产单已经配齐，现在可以整托领走。
- “未配齐配料”回答：哪些生产单当前虽未配齐，但已经有哪些物料、分别在哪些专属库位，可以整批领走。
- “已接收”回答：哪些生产单曾经领过物料，是已配齐后领还是未配齐先领，正常需求和全部补料是否最终都已完成接收。
