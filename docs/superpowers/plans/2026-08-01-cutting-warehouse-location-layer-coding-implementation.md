...
生成器必须校验：库区为单个大写字母；货架、层、位置为有限正整数，不设置固定业务上限；编号至少补足为两位，超过两位时保留完整十进制数。批量生成由任务 6 的动态设备资源预算负责阻断不可承载规模；该结构序号规则与任务 4 的“多选库位数量无固定上限”分别校验。Mock 使用稳定 ID，并按规格第 6 节生成两类仓库且相互隔离。
