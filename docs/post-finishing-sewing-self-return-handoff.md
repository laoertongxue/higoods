...
| 系统 | 菜单 / 页面 | 本次变化 | 主要使用人 |
|---|---|---|---|
| 工厂端移动应用 PDA | 交接 > 待接收 | 新增“车缝现场交货登记模式”入口与自助回货概览；待接收列表中展示“车缝自助回货”类记录，并与正常接收区分 | 后道工厂管理员、后道交接人员 |
| 工厂端移动应用 PDA | 车缝现场交货登记模式 | 新增锁定式自助登记页面，支持扫码、填写回货数量、上传现场凭证图片、提交自助回货、管理员验证退出 | 车缝厂送货人员、后道工厂管理员 |
| 工厂端移动应用 PDA | 交接 > 接收详情 | 支持车缝自助回货类接收记录确认；页面文案改为“来源车缝厂 -> 后道工厂”；明细按 SKU、尺码、颜色、数量展示 | 后道交接人员 |
...
- 只有后道工厂管理员可以在 PDA 的“交接 > 待接收”中看到“车缝现场交货登记模式”入口。
- 只有后道工厂管理员可以开启该模式。
...
- 生成车缝侧交出记录。
- 生成后道待加工仓待确认入库记录。
- 生成后道 PDA 待接收记录，供后道人员确认。
- 最近自助回货列表展示新提交记录。

### 5.3 PDA：交接 > 接收详情

针对车缝自助回货类记录，接收详情做了简化和业务化处理：
...
正常接收是“仓库 / 上游环节 -> 后道工厂”的交付确认。

车缝自助回货是“车缝厂 -> 后道工厂”的现场送货确认。

因此在 PDA 待接收列表和接收详情中必须区分：
...
- PDA 面向印尼本土操作人员，接收详情需要尽量减少重复信息，关键明细按 SKU、尺码、颜色、数量分行展示。

## 11. 验收标准

### 11.1 权限与入口

- 后道工厂管理员可以在 PDA“交接 > 待接收”看到“车缝现场交货登记模式”入口。
- 非管理员看不到或不能开启该模式。
- 非后道工厂公共 PDA 不能进入该模式。
...
- PDA“交接 > 待接收”可以看到车缝自助回货记录。
- 车缝自助回货记录与正常接收记录有明显区分。
- 来源展示为车缝厂，接收方展示为后道工厂。
- 接收详情不展示“一个任务一个接收单”提示。
- 接收详情中物料明细只保留一块，并按 SKU、尺码、颜色、数量展示。
- 接收详情可以确认本次回货，也可以处理数量差异或驳回。
