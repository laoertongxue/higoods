...
| ADJ-025 | 总体设计、实施计划、追踪矩阵与验收记录共同使用 ADJ-001～027。 |
...
1. 为每个 B 款颜色选择 A 款参考颜色；允许一个 A 款颜色对应多个 B 款颜色，也允许 B 款新增颜色没有来源颜色。
2. 对有来源的每条物料选择处理方式：沿用、替换、重新染色、重新印花、不使用。
3. 对 B 款新增需要的物料选择“新增”。
4. 每条 B 款物料维护单位用量、打样数量、损耗率、工艺要求。
5. 系统计算本次总需求量：`单位用量 × 打样数量 ×（1 + 损耗率）`。
6. 全部处理结论确认后，形成归属于 B 款的 BOM 与价格草稿。
...
工程主单、工程变更、改款打样、设计打样和各专业任务使用标准管理列表：摘要、筛选、列设置、分页、表格内部滚动和操作列。改款／设计与制版页面统一标题、筛选、表格密度、详情头部、当前动作区、成果区、操作记录和反馈方式。
...
1. ADJ-001～027 均须在追踪矩阵中绑定实现位置和当前证据。
