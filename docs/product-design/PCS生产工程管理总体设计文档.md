...
| 版本 | V5.0 |
| 日期 | 2026-08-13 |
| 状态 | V5.0 当前隔离分支已实现；原子需求、命名页面、PCS 总门禁、构建、任务范围治理和 CodeGraph 已验证；项目级任务收据仅被未修改的 PFOS 历史列表欠账阻断；待提交、推送和产品确认 |
| 权威调整基线 | 《PCS 生产工程管理完整调整方案》V5.0，以及用户对样衣制作要求、多颜色多尺码实际成果和专业任务规范入口的最终确认 |
...
8. 销售展示样衣和产前版样衣均先由跟单逐行下达“颜色、尺码、要求数量”，再由制作团队逐行提交实际成果。
9. 父单中的“进入任务”和专业任务列表进入同一份专业任务详情，避免同一任务出现两套操作页面。
...
| ADJ-025 | 总体设计、实施计划、追踪矩阵与验收记录共同使用当前完整范围 ADJ-001～050。 |
...
| ADJ-043 | 改款／设计打样的销售展示样衣制作要求由跟单在工作安排中逐行下达，并随任务生成后锁定。 |
| ADJ-044 | 工程主单的产前版样衣制作要求由跟单在确认工作安排前逐行下达，并随任务生成后锁定。 |
| ADJ-045 | 样衣制作要求和实际成果均采用“颜色＋尺码＋数量”的多行明细，要求件数与实际件数分别汇总和对照。 |
| ADJ-046 | 设计打样与改款打样使用相同四步团队流程，并完整覆盖销售展示样衣要求和专业执行。 |
| ADJ-047 | 父单的“进入任务”按真实任务类型进入制版、花型、调色或销售展示样衣的规范详情页。 |
| ADJ-048 | 销售展示样衣只能作为工程主单参考资料，不得替代产前版样衣或使其自动完成。 |
| ADJ-049 | 列表、父单和详情中的样衣数量均为实际制作数量之和，不得使用成果行数代替件数。 |
| ADJ-050 | 总体设计、实施计划、追踪矩阵、专项契约和命名页面验收同步使用本次样衣规则。 |
...
| 样衣制作要求 | 工作安排确认前由跟单逐行下达 | 销售展示样衣任务或产前版样衣任务 | 颜色、尺码、要求数量和可选说明；任务生成后锁定 |
| 样衣实际成果 | 制作团队开始任务后逐行提交 | 对应样衣任务和制作要求 | 实际颜色、尺码、数量、真实图片、纸样版本、制作说明和差异说明 |
...
销售展示样衣被选中时，跟单必须在确认工作安排前逐行下达制作要求。每行包含 B 款目标颜色、目标尺码、要求数量（件）和可选的制作要求说明。同一颜色与尺码组合不得重复；至少存在一条有效要求才可确认工作安排。确认成功后要求与专业任务一并生成并立即锁定。
...
销售展示样衣采用“跟单先下达要求，制作团队后提交实际成果”的两层事实：

| 事实 | 维护团队 | 维护时点 | 明细字段 | 锁定规则 |
| --- | --- | --- | --- | --- |
| 本次制作要求 | 跟单 | 工作安排确认前 | 要求颜色、要求尺码、要求数量（件）、制作要求说明 | 工作安排确认并生成任务后只读 |
| 本次实际成果 | 制作团队 | 任务开始后 | 对应要求、实际颜色、实际尺码、实际数量（件）、真实样衣图片、纸样版本、制作说明、差异说明 | 提交该轮成果后只读 |

一个要求可拆成多条实际成果，例如“黑色 M 2 件”可分两次各交 1 件；一个任务也可包含多个颜色和多个尺码。页面分别汇总要求总件数、实际总件数和差异，不能以成果记录条数代替实际件数。

实际颜色、尺码或数量与要求不同不应被系统改写。制作团队应填写真实结果；只要存在差异，就必须填写差异说明。差异说明完整后可以完成任务，本期不增加异常、审批或要求变更模块。

```mermaid
flowchart LR
  A["跟单逐行下达制作要求"] --> B{"要求完整且无重复"}
  B -- "否" --> A
  B -- "是" --> C["确认工作安排并生成任务"]
  C --> D["制作要求锁定"]
  D --> E["制作团队开始任务"]
...
2. 两种方式建立后都必须由跟单确认工作安排；其中产前版样衣被启用时，跟单必须先逐行下达制作要求。
3. 同一 SPU 只允许一张未关闭主单。
...
| 产前版样衣 | 制作团队 | 首单固定任务 | 适用的基码纸样 | 按已锁定要求逐行提交实际颜色、尺码、数量、真实照片和纸样版本 | 齐码纸样、技术包 |
...
产前版样衣与销售展示样衣使用相同的“制作要求／实际成果”数量模型，但它们是两张不同的任务：销售展示样衣只供打样和直播展示参考；工程主单必须重新下达产前版样衣要求、单独制作并提交成果，不能引用前者完成状态替代。
...
- 销售展示样衣：只读展示跟单下达的多行制作要求；制作团队逐行提交对应要求、实际颜色、实际尺码、实际数量、引用纸样版本、真实样衣照片、制作说明和必要的差异说明。
- 产前版样衣：只读展示工程主单中跟单下达的多行制作要求；制作团队逐行提交对应要求、实际颜色、实际尺码、实际数量、引用纸样版本、真实样衣照片、制作说明和必要的差异说明。
...
11. 销售展示样衣和产前版样衣各自覆盖多个颜色、多个尺码、一个要求拆成多条实际成果、实际数量少于／等于／多于要求及差异说明。
12. 改款和设计各至少一个完整销售展示样衣案例；父单“进入任务”与专业列表进入同一规范详情。
13. 销售展示样衣已完成但产前版样衣仍须独立下达要求并制作的边界案例。

## 13. 验收门禁

1. ADJ-001～050 均须在追踪矩阵中绑定实现位置和当前证据。
