...
| 文档版本 | V2.4 |
| 编制日期 | 2026-09-16 |
| 当前状态 | V2.4 花型成果存储与加工单绑定修复已实现，最终结果以同版本实施验收报告为准 |
...
1. 花型图与花型源文件合并为一个“花型成果文件”上传区，支持一次或分次上传多个文件；提交时同时校验至少一张可预览图片和一个 `.ai`、`.psd` 或 `.pdf` 文件。
2. 专业成果文件正文只保存在上传仓库；设计改款主记录、印花／染色加工单和 PDA 任务只保存文件元数据与引用，读取时再恢复文件正文，避免同一大文件重复占用浏览器存储。
3. 上传仓库和设计改款主记录改为“持久化成功后再更新内存”；浏览器存储空间不足时明确提示“本次操作未保存”，且当前页面状态不产生假成功。
4. 加工单绑定失败改为指出具体加工单、实际所属设计改款任务或专业任务；仅来源键过期且业务归属一致时自动校正，不再用笼统的“来源不一致”阻断。
5. 加工单记录缺失时展示恢复入口：资料完整可按当前物料与加工要求重新生成；销售展示样衣数量缺失时直接返回主任务补充，不执行注定失败的重建。
...
## 21. V2.4 花型成果存储与加工单绑定修复

### 21.1 目标与非目标

目标是修复花型成果上传、浏览器存储写满和加工单绑定失败三类直接问题，并让失败可诊断、可恢复。继续使用当前原型的本地上传仓库和既有印花／染色加工单，不新增后端、对象存储、异步上传或新的业务页面。

### 21.2 修复流程

```mermaid
flowchart LR
  A[花型团队选择多个成果文件] --> B{包含预览图和源文件?}
  B -- 否 --> C[就地提示缺少的文件类型]
  B -- 是 --> D[文件正文写入唯一上传仓库]
  D --> E[设计改款记录只保存文件引用]
  E --> F[买手审核]
...
stateDiagram-v2
  ...
  保存失败 --> 上传中: 清理草稿后重试
  已保存 --> 待买手审核: 提交成果
  待买手审核 --> 已完成: 加工单有效并审核通过
  待买手审核 --> 待恢复关联: 加工单不存在
  待恢复关联 --> 待买手审核: 重建关联成功
  待恢复关联 --> 待补充主任务: 缺少样衣数量等前置资料
  待补充主任务 --> 待恢复关联: 主任务资料补齐
```
...
|---|---|---|---|---|---|
| WP-14A | 花型图与花型文件统一多选 | `pcs-independent-sampling.ts`、上传组件 | 合并上传目的和提示，沿用原生多文件输入 | 命名页面真实选择 2 张图片和 1 份文件 | 同一区域显示 3 个文件并可提交 |
| WP-14B | 文件正文只保存一份 | 上传仓库、设计改款仓储、印花／染色仓储 | 保存引用、读取水合；印花业务视图中的同源预览图不重复落盘 | 专项契约检查三类持久化键 | 主记录和加工单不再包含重复 Base64 正文 |
| WP-14C | 存储写满不产生假成功 | 上传仓库、设计改款仓储 | 先写持久化，成功后替换内存；失败给出中文恢复提示 | 注入 `QuotaExceededError` | 上传和主记录均保持写入前状态 |
| WP-14D | 加工单绑定可诊断、可恢复 | 加工单适配器、专业任务审核、专业任务页 | 精确校验归属；旧来源键自动校正；缺单重建或返回补充资料 | 专项契约与浏览器失效引用场景 | 页面不再出现无对象、无恢复动作的笼统错误 |

### 21.5 原子需求追踪矩阵

| 需求编号 | 来源 | 原子需求 | 工作包 | 实现位置 | 自动化验证 | 页面验证 | 状态 | 证据位置 | 产品确认人和版本 |
|---|---|---|---|---|---|---|---|---|---|
| `ART-019` | 21.2 | 花型图和花型源文件必须位于同一上传区，且两类都支持多个文件 | WP-14A | `uploadPurposes`、`renderEngineeringFileUpload` | `pcs-design-revision-result-storage-and-binding.spec.ts` | 花型任务真实上传 2 张 JPG 与 1 份 PDF | 已验证 | `output/playwright/pcs-design-revision-artwork-storage-binding-fixed.png` | 待用户确认 V2.4 |
| `STORAGE-001` | 21.2 | 同一专业成果文件正文只允许保存在上传仓库一份，其他记录保存引用并可恢复读取 | WP-14B | `createPersistedRecords`、`compactDesignRevisionPrintSource`、`compactDesignRevisionDyeSource` | 专项契约检查主记录、印花记录无重复 Base64 | 刷新后 3 个文件仍可查看／下载 | 已验证 | 专项契约输出；浏览器存储长度与刷新页面记录 | 待用户确认 V2.4 |
| `STORAGE-002` | 21.3 | 上传或主记录写入超额时不得更新内存，必须提示本次未保存及恢复动作 | WP-14C | `writeGroups`、`writeRecords` | 两处注入 `QuotaExceededError` 均通过 | 不适用：浏览器配额由专项契约确定性模拟 | 已验证 | `pcs-design-revision-result-storage-and-binding.spec.ts` | 待用户确认 V2.4 |
| `BIND-001` | 21.3 | 绑定失败必须指出缺失加工单或冲突的任务归属；归属一致的旧来源键自动修正 | WP-14D | `validateResultBinding` | 缺单、错任务、旧来源键三种断言通过 | 页面显示 `PRINT-MISSING-UI` 具体单号 | 已验证 | 专项契约；失效引用浏览器快照 | 待用户确认 V2.4 |
| `BIND-002` | 21.3 | 加工单缺失且资料完整时可重建，资料不全时必须返回主任务补充 | WP-14D | `repairEngineeringIndependentProfessionalTaskProcessOrders`、专业任务恢复卡片 | 恢复后失效引用清零并记录日志 | 页面展示恢复卡片；缺样衣数量时给出具体阻断 | 已验证 | 专项契约；`output/playwright/pcs-design-revision-artwork-storage-binding-fixed.png` | 待用户确认 V2.4 |
| `BIND-003` | 21.3 | 审核绑定必须在任务状态落盘前完成，失败不能把成果误标为已通过 | WP-14D | `reviewEngineeringIndependentProfessionalTask` | 缺失加工单抛错后任务仍为待审核 | 浏览器失败后成果状态仍为“待审核” | 已验证 | 专项契约；失效引用页面快照 | 待用户确认 V2.4 |
