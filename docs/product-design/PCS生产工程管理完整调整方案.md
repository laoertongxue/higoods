...
> **历史决策记录**：本文已被《PCS设计改款合并、工程变更删除与生产准备时效收口调整方案》及当前总体设计取代，仅用于追溯 2026-08-13 当时的方案。本文中的“设计／改款双入口”“工程变更”等内容不再属于当前需求，也不得作为现行实现或验收依据。
...
| 文档版本 | V5.0 |
| 文档日期 | 2026-08-13 |
| 文档状态 | V5.0 当前隔离分支已实现；原子需求、命名页面、PCS 总门禁、构建、任务范围治理和 CodeGraph 已验证；项目级任务收据仅被未修改的 PFOS 历史列表欠账阻断；待提交、推送和产品验收 |
| 适用系统 | 商品中心系统（PCS） |
| 适用范围 | 独立改款打样、独立设计打样、工程主单、BOM 与价格、专业任务、工程变更、技术包、生产准备时效 |
| 核查基线 | 当前代码提交 `1decb915`；改款／设计打样、工程主单、销售展示样衣、产前版样衣、专业任务路由、专项测试及既有需求文档 |
...
16. 销售展示样衣和产前版样衣都必须先由跟单按“颜色＋尺码＋要求数量”下达制作要求，再由制作团队逐行提交实际成果。
17. “要求数量”和“实际制作数量”是两组独立业务事实，不得共用字段、相互覆盖或只保留一个总数。
18. 设计打样与改款打样使用同一套团队分步和样衣要求规则；区别仅在设计打样没有 A 款参考。
19. 独立打样详情中的“进入任务”必须进入该真实专业任务在专业任务体系中的同一详情页，不再进入第二套通用任务表单。
20. 销售展示样衣可以成为工程主单的参考输入，但任何情况下都不能换算、映射或复用为已完成的产前版样衣。
...
| ADJ-043 | 销售展示样衣只有制作团队事后填写的单个制作数量，没有跟单事前下达的颜色、尺码和要求数量 | 在改款／设计打样的“工作安排”中增加多行样衣制作要求，由跟单确认后随任务生成 | 工作要求补齐 |
| ADJ-044 | 产前版样衣只有单个实际制作总数，没有工程主单工作安排中的事前要求 | 在工程主单“确认工作安排”中增加产前版样衣多行制作要求，并随产前版样衣任务保存 | 工作要求补齐 |
| ADJ-045 | 当前样衣页面把颜色、尺码和数量作为一条实际成果填写，不能表达多个颜色、多个尺码分别制作多少件 | 制作要求和实际成果均采用“颜色＋尺码＋数量”的多行明细；合计由系统汇总 | 数量模型调整 |
| ADJ-046 | 现有设计打样只验证了页面骨架，没有完整覆盖颜色准备、工作安排、销售展示样衣要求和专业执行 | 设计打样与改款打样执行相同四步流程，并补齐独立验收案例 | 场景覆盖补齐 |
| ADJ-047 | 独立打样中的所有“进入任务”统一进入通用详情，和左侧制版、花型、调色、样衣专业任务入口不一致 | 每个按钮按真实任务类型进入对应专业任务详情；父单只负责协调、汇总和跳转 | 任务入口收口 |
| ADJ-048 | 当前实现存在把销售展示样衣对应为产前版样衣前期成果的逻辑 | 删除该替代关系；销售展示样衣只展示为参考资料，产前版样衣仍须单独下达要求、制作和提交 | 样衣边界纠正 |
| ADJ-049 | 当前部分列表把“成果记录条数”当成“实际制作数量” | 列表汇总实际数量之和，并同时展示要求合计、实际合计和差异 | 统计口径纠正 |
| ADJ-050 | 既有设计、专项测试仍固化单行结果、通用详情入口和销售样衣复用等旧口径 | 同步更新总体设计、实施计划、追踪矩阵、专项测试和浏览器验收场景 | 文档与契约纠正 |
...
17. 改款与设计打样的目标颜色、适用尺码先由买手确认；销售展示样衣的具体制作组合和要求数量再由跟单下达。
18. 产前版样衣必须在工程主单中独立生成、独立执行，不沿用销售展示样衣完成状态。

### 2.3 逐行核查后的当前实现缺口

本轮对独立打样数据对象、改款／设计详情页、工程主单工作安排、销售展示样衣提交、产前版样衣提交、专业任务路由、专业列表汇总、专项测试和既有文档进行了逐项核查。当前缺口不是单一页面问题，而是数据、页面、路由、测试和文档五个层面同时存在：

| 层面 | 当前实现 | 业务问题 | 本方案调整 |
| --- | --- | --- | --- |
| 独立打样工作安排 | 跟单只能勾选需要生成的任务类型 | 无处下达销售展示样衣的颜色、尺码和要求数量 | 工作安排增加“本次样衣制作要求”多行表格 |
| 设计打样 | 与改款共用专业任务结构，但只验证了页面骨架 | 设计打样同样缺少销售展示样衣要求，不能只修改款 | 改款、设计同时调整并分别验收 |
| 销售展示样衣成果 | 页面只填写一组颜色、尺码、制作数量 | 无法表达多个颜色、多个尺码分别做多少件 | 按颜色和尺码逐行提交实际成果 |
| 工程主单工作安排 | 只确认任务类型和前期成果 | 无处下达产前版样衣制作要求 | 主单工作安排增加产前版样衣要求表格 |
| 产前版样衣成果 | 只保存图片和一个实际总数量 | 缺少颜色、尺码、纸样版本；制作说明也没有形成有效结果事实 | 改为多行实际成果并完整保存说明和文件 |
| 专业任务入口 | 独立打样的所有“进入任务”进入同一通用详情 | 与专业任务列表中的制版、花型、调色等入口不一致 | 按任务类型进入同一个专业详情事实源 |
| 数量汇总 | 部分列表按成果记录条数计数 | 一条成果做 3 件可能仍显示 1 | 汇总实际件数，并展示要求／实际／差异 |
| 前期成果复用 | 销售展示样衣可能被对应为产前版样衣 | 混淆销售展示与大货产前确认 | 删除替代关系，只允许只读参考 |
| 自动化契约 | 仍验证单行结果、通用详情入口和销售样衣复用 | 测试会保护错误行为 | 按本方案重写专项契约和浏览器案例 |
| 需求文档 | 主要描述提交实际成果，没有写清事前要求 | 研发无法判断要求在哪里、由谁下达 | 总体设计、实施计划和矩阵同步原子化 |
...
11. 销售展示样衣和产前版样衣的制作要求、实际成果、数量差异和任务入口。
...
| 样衣制作要求 | 跟单在任务开始前下达本次要做什么 | 独立打样工作安排或工程主单工作安排 | 按目标颜色、尺码和要求数量形成的只读任务指令 |
| 样衣实际成果 | 制作团队完成后记录本次实际做了什么 | 销售展示样衣任务或产前版样衣任务 | 按实际颜色、尺码、数量、图片和纸样版本形成的成果明细 |
...
| 事实 | 形成时点 | 维护团队 | 任务中如何展示 | 是否可被另一事实覆盖 |
| --- | --- | --- | --- | --- |
| 本次制作要求 | 专业任务生成前 | 跟单团队 | 任务详情只读展示 | 不可以 |
| 本次实际成果 | 制作团队完成工作后 | 样衣制作团队 | 与要求逐行对照展示 | 不可以 |

制作要求不是成果预填值，实际成果也不是对要求的覆盖。系统必须永久保留“原本要求做什么”和“最后实际做了什么”。
...
| 字段 | 规则 |
| --- | --- |
| 目标颜色 | 只能选择买手已确认的 B 款目标颜色；不得直接拿 A 款参考颜色作为任务颜色 |
| 目标尺码 | 只能选择该目标颜色已确认适用的 B 款尺码 |
| 要求数量 | 大于 0 的整数，单位固定为“件” |
| 制作要求说明 | 可选，用于说明本行的特殊制作要求，不代替结构化数量 |

规则如下：

1. 同一任务可以包含任意多行颜色／尺码组合。
2. 不强制生成颜色与全部尺码的笛卡尔积；跟单只添加本次实际需要制作的组合。
3. 同一颜色、同一尺码不得重复出现两行；需要增加数量时直接修改该行。
4. 页面自动汇总要求件数，但总数只用于汇总，不能代替明细。
5. 改款打样的要求基于 B 款目标颜色和尺码；A 款颜色只保留为参考关系。
6. 设计打样没有 A 款，但使用完全相同的目标颜色、尺码和要求数量规则。
7. 产前版样衣使用工程主单目标款的正式颜色和尺码，不引用销售展示样衣的完成状态。
...
| 字段 | 规则 |
| --- | --- |
| 实际颜色 | 默认带出要求行颜色；如实物颜色不同，允许填写真实颜色，但必须说明差异 |
| 实际尺码 | 默认带出要求行尺码；如实物尺码不同，允许填写真实尺码，但必须说明差异 |
| 实际数量 | 大于 0 的整数，单位固定为“件” |
| 样衣图片 | 每个实际组合至少上传与该组合对应的真实样衣图片 |
| 使用的纸样版本 | 选择实际使用的已完成纸样版本；不得手填不存在的版本号 |
| 制作说明 | 记录本次实际制作情况；必须真实保存并进入操作记录 |
| 实际操作人和时间 | 团队成员开始、提交时由系统记录，不在任务生成前虚构个人 |

一个专业任务可以一次提交多条实际成果。页面必须展示要求合计、实际合计和逐行差异，不得把“成果有几行”显示为“做了几件”。

#### 7.6.4 要求确认、锁定和差异处理

1. 跟单确认工作安排前可以增加、删除和修改制作要求。
2. 确认工作安排时，系统同时校验至少存在一条有效样衣要求，并将要求写入新生成的样衣任务。
3. 工作安排确认和任务生成必须是同一个不可拆分动作；确认成功后制作要求立即锁定，不再允许从父单或专业任务页面修改。
4. 本阶段不增加要求变更、审批或异常模块；确认后确需改变制作目标时，不在当前任务中覆盖原要求。
5. 制作团队提交真实数量时，系统按颜色和尺码计算差异，不得为了与要求一致而改写实际事实。
6. 实际颜色、尺码或数量与要求不一致时允许提交真实成果，但该要求对应的至少一条实际成果必须填写差异说明；说明完整后任务可正常完成。
7. 样衣实物后续仍应按“一件样衣一个唯一码”进入样衣管理；唯一码、借用、流转和退回不在本次调整范围内。

#### 7.6.5 样衣要求与成果流程图

```mermaid
flowchart TD
    A["买手确认目标颜色、尺码与 BOM"] --> B["跟单进入工作安排"]
    B --> C["逐行填写颜色、尺码、要求数量"]
    C --> D{"要求是否完整"}
    D -->|"否"| C
    D -->|"是"| E["跟单确认工作安排"]
    E --> F["系统生成真实样衣任务并保存制作要求"]
    F --> G{"前置工作是否完成"}
    G -->|"否"| H["任务显示需要先完成的工作"]
    H --> G
    G -->|"是"| I["样衣制作团队开始制作"]
    I --> J["逐行提交实际数量、图片和纸样版本"]
    J --> K["系统计算要求／实际／差异"]
    K --> L["保存真实成果和实际操作记录"]
```
...
stateDiagram-v2
    [*] --> 待维护制作要求
    待维护制作要求 --> 制作要求已确认: 跟单确认工作安排
    制作要求已确认 --> 待前置: 存在未完成前置
    制作要求已确认 --> 待制作: 无未完成前置
    待前置 --> 待制作: 前置工作完成
    待制作 --> 制作中: 制作团队开始
    制作中 --> 待提交成果: 已完成实物制作
    待提交成果 --> 已完成: 提交实际成果且通过完成门禁
    已完成 --> [*]
```
...
```mermaid
sequenceDiagram
    participant Buyer as 买手团队
    participant System as PCS
    participant Merch as 跟单团队
    participant Sample as 样衣制作团队
    Buyer->>System: 确认目标颜色、适用尺码和 BOM
    System-->>Merch: 开放工作安排
    Merch->>System: 填写颜色、尺码、要求数量
    Merch->>System: 确认工作安排
    System->>System: 保存制作要求并生成专业任务
    System-->>Sample: 前置完成后显示待制作任务
    Sample->>System: 开始任务
    Sample->>System: 提交逐行实际数量、真实图片和纸样版本
    System->>System: 保存成果并计算数量差异
    System-->>Merch: 展示要求、实际和差异结果
```
...
flowchart TD
    ...
    G --> I["买手确认 B 款目标颜色、尺码和 BOM"]
    H --> I
    I --> J["跟单确认本次需要完成的工作"]
    J --> R{"是否制作销售展示样衣"}
    R -->|"是"| S["跟单填写颜色、尺码和要求数量"]
    R -->|"否"| T["无需样衣要求"]
    S --> U["一次性生成适用专业任务和制作要求"]
    T --> U
    U --> K["专业团队进入对应任务详情执行"]
    K --> V["真实上传纸样、图片或成果文件"]
    V --> L{"任务是否需要买手审核"}
...
当本次包含销售展示样衣时，跟单必须在同一阶段完成“本次样衣制作要求”：

1. 系统先读取买手已确认的 B 款目标颜色和适用尺码；
2. 跟单选择本次真正要做的颜色／尺码组合；
3. 跟单为每个组合填写要求数量，单位为件；
4. 要求不完整时不能确认工作安排；
5. 确认后，销售展示样衣任务携带这组只读要求进入专业任务体系。
...
当主单包含产前版样衣时，还必须显示“本次产前版样衣制作要求”。跟单按工程主单目标款颜色和尺码逐行填写要求数量。系统不得从销售展示样衣的实际结果自动生成产前版样衣要求，更不得用销售展示样衣完成状态完成产前版样衣。
...
6. 已选择销售展示样衣或产前版样衣但没有有效制作要求时，禁止确认工作安排。
7. 任务骨架、固定依赖和样衣制作要求必须在同一次确认动作中保存，不能先生成空任务再到处补要求。

### 10.4 工作安排的团队责任

| 阶段 | 当前处理团队 | 允许维护 | 不允许维护 |
| --- | --- | --- | --- |
| 目标颜色与 BOM 准备 | 买手团队 | B 款目标颜色、适用尺码、BOM 与价格 | 样衣要求数量 |
| 确认工作安排 | 跟单团队 | 专业任务选择、样衣颜色尺码组合、要求数量 | BOM 物料和标准单价 |
| 专业任务执行 | 对应专业团队 | 实际成果、真实文件、实际数量和说明 | 已确认制作要求 |

页面先显示当前团队，不在任务生成时预设个人；节点发生真实动作后再记录实际操作人。
...
系统推荐前期成果时必须保持成果类型不变：基码纸样只能对应基码纸样，花型只能对应花型，调色只能对应调色。销售展示样衣只进入“参考样衣”区域，不出现在“沿用为产前版样衣”的选择中。
...
“进入任务”不是进入父单里的另一套简化表单，而是进入该任务在共享专业任务体系中的唯一详情：
...
同一个任务从父单详情、专业任务列表或业务 Tab 进入时，必须打开同一个任务号、同一组状态、要求、成果和操作记录。
...
销售展示样衣和产前版样衣详情还必须在当前动作区下方依次展示：

1. “本次制作要求”：只读展示颜色、尺码、要求数量和要求合计；
2. “提交本次成果”：制作团队逐行填写实际数量并上传真实图片、选择纸样版本；
3. “要求与实际”：提交后展示逐行差异、实际合计、差异说明和历史轮次。
...
| 来源 | 独立改款或设计打样 |
| 当前团队 | 样衣制作团队 |
| 要求由谁下达 | 跟单团队在独立打样“工作安排”中下达 |
| 本次制作要求 | B 款目标颜色、尺码、要求数量和可选说明；至少一行 |
| 要求展示 | 任务详情只读展示，不允许制作团队修改 |
| 当前动作 | 制作并提交销售展示样衣 |
| 必填成果 | 按颜色／尺码逐行填写实际数量，真实上传与该组合对应的实物图片，选择实际使用的纸样版本并填写制作说明 |
| 完成规则 | 制作团队提交完整成果，并满足第 7.6.4 节最终确认的数量差异门禁后完成 |
...
| 来源 | 工程主单，或确有必要的工程变更 |
| 当前团队 | 样衣制作团队 |
| 要求由谁下达 | 跟单团队在工程主单“确认工作安排”中下达；工程变更按本次修改内容重新下达 |
| 本次制作要求 | 当前目标款颜色、尺码、要求数量和可选说明；至少一行 |
| 要求展示 | 任务详情只读展示，不允许制作团队修改 |
| 需要先完成 | 当前准备类型适用的全部基码纸样 |
| 当前动作 | 制作并提交产前版样衣 |
| 必填成果 | 按颜色／尺码逐行填写实际数量，真实上传与该组合对应的实物图片，选择实际使用的纸样版本并填写工艺／制作说明 |
| 完成规则 | 制作团队提交完整成果，并满足第 7.6.4 节最终确认的数量差异门禁后完成 |
| 后续 | 解锁适用的齐码纸样任务；成果进入技术包 |

### 13.3.1 两类样衣统一数量口径

1. 制作要求和实际成果都以“颜色＋尺码＋件数”作为最小明细。
2. 要求合计等于所有要求行数量之和；实际合计等于所有实际行数量之和。
3. 颜色尺码差异按同一组合的“实际数量－要求数量”计算。
4. 没有要求的实际组合必须单独标识，不能静默合并到其他组合。
5. 页面、列表、详情、日志和测试都不得用成果行数代替实际件数。
6. 销售展示样衣和产前版样衣分别统计，不跨任务合并。
...
- 跟单确认本次需要完成的专业工作；
- 选中销售展示样衣时，逐行维护目标颜色、目标尺码和要求数量；
- 要求合计和校验结果；
- 确认并生成任务动作；
- 已确认后的只读要求摘要。

“专业任务表格”的操作列按任务状态显示“开始任务”“继续处理”“审核成果”“查看结果”或“等待前面工作”，并进入对应专业任务唯一详情页。待前置任务不能显示成可直接开始。

### 17.4 工程主单详情

继续使用表格，删除花哨泳道表达。并行关系用同一阶段的多行任务表示，串行关系用“需要先完成”列表示。

草稿主单的工作安排区增加产前版样衣要求表格。跟单确认后，主单任务表格中的产前版样衣行展示要求合计，并可进入产前版样衣详情查看完整颜色尺码要求。主单不得显示“沿用销售展示样衣即可完成产前版样衣”的操作。

### 17.5 专业任务详情

当前通用“成果名称＋图片地址”表单改为专业业务表单。所有图片、`.prj` 纸样和附件使用第 13.9 节真实文件上传；款式和物料均展示真实缩略图并可打开高清大图。

销售展示样衣和产前版样衣详情采用相同页面骨架，但保持任务名称、来源、用途和后续去向不同：

1. 任务头：任务号、款式、来源单、状态和返回列表；
2. 当前动作：当前团队、需要先完成、完成后去向；
3. 本次制作要求：颜色、尺码、要求数量和要求合计；
4. 本次成果：多行实际颜色、尺码、数量、真实图片、纸样版本和说明；
5. 要求与实际：逐行差异及合计；
6. 历史成果和操作记录。

详情页只允许当前轮到的团队执行当前动作；其他团队可以查看，但不能修改本节点业务数据。
...
| 制作数量 | 在工作安排中叫“要求数量”；在成果提交中叫“实际制作数量” |
| 颜色／尺码 | 明确为“要求颜色／要求尺码”或“实际颜色／实际尺码”，不得使用无上下文的通用字段名 |
| 进入任务 | 按当前状态显示“开始任务／继续处理／审核成果／查看结果／等待前面工作” |
| 复用销售展示样衣 | 查看销售展示样衣作为参考；不得替代产前版样衣 |
...
- 跟单下达的每一行颜色、尺码和要求数量；
- 要求确认人、确认时间及任务开始前的调整记录；
- 制作团队提交的每一行实际颜色、尺码和数量；
- 要求合计、实际合计和逐行差异；
- 每个实际组合对应的真实图片和使用纸样版本；
- 差异说明；
- 销售展示样衣或产前版样衣的明确业务类型，禁止相互换算。
...
33. 改款打样的 B 款包含黑色 M 2 件、黑色 L 1 件、白色 M 1 件的销售展示样衣要求。
34. 设计打样没有 A 款，仍能由买手确认目标颜色／尺码，并由跟单下达多行销售展示样衣要求。
35. 工程主单包含至少两个颜色、多个尺码的产前版样衣要求。
36. 销售展示样衣实际数量与要求完全一致。
37. 销售展示样衣实际数量与要求存在差异，页面保留真实数量、差异和说明。
38. 产前版样衣使用已完成基码纸样版本并逐行上传真实图片。
39. 从独立打样详情和专业任务列表进入同一销售展示样衣任务，看到的任务号、状态、要求和成果完全一致。
40. 销售展示样衣已完成，但工程主单仍显示产前版样衣待制作且不能沿用完成状态。
41. 一条实际成果填写 3 件时，列表实际合计显示 3 件而不是 1 条。
42. 任务开始前跟单调整样衣要求并留下记录；任务开始后制作团队不能修改要求。
...
4. 后置任务缺少前置时系统自动补齐。
5. 页面不允许调整固定依赖。
6. 选择销售展示样衣后必须填写至少一条有效颜色、尺码和要求数量，否则不能确认。
7. 设计打样与改款打样使用相同的制作要求校验；设计打样不出现 A 款参考字段。
8. 工程主单包含产前版样衣时必须填写制作要求，不能从销售展示样衣自动带完成状态。
9. 工作安排确认后一次性生成任务和制作要求，不产生无要求的空样衣任务。
...
3. 产前版样衣必须等待适用基码纸样。
4. 产前版样衣完成后解锁适用齐码纸样。
5. 两类任务均先显示跟单下达的颜色、尺码和要求数量，再由制作团队填写实际成果。
6. 一张任务可包含多个颜色、多个尺码和不同数量。
7. 要求合计和实际合计按件数求和，不按成果行数计数。
8. 制作团队不能修改跟单下达的要求。
9. 实际结果必须保留真实图片、纸样版本、制作说明、实际操作人和时间。
10. 销售展示样衣只作为工程主单参考，不出现“沿用并完成产前版样衣”的选择。
...
1. 改款／设计详情的基码纸样“进入任务”与制版任务列表打开同一详情。
2. 花型“进入任务”与花型任务列表打开同一详情。
3. 纱线／面料调色“进入任务”与调色任务列表打开同一详情。
4. 销售展示样衣“进入任务”与销售展示样衣任务列表打开同一详情。
5. 工程主单产前版样衣“进入任务”与产前版样衣任务列表打开同一详情。
6. 从不同入口进入时，任务号、来源、当前团队、状态、制作要求、实际成果和日志一致。
7. 待前置任务显示“等待前面工作”，不能误导用户进入后开始执行。

### 21.11 样衣数量与结果真实性

1. 跟单可增加、删除和修改多个颜色尺码要求行。
2. 同一颜色尺码重复时阻止确认并指出重复行。
3. 要求数量为 0、负数、小数或空值时阻止确认。
4. 制作团队可以对多条颜色尺码组合提交实际件数。
5. 每个实际组合至少有对应真实图片，未成功保存时不能提交。
6. 实际纸样版本必须从已完成成果中选择，不能手填不存在版本。
7. 刷新页面后，要求明细、实际明细、合计、差异和说明保持一致。
8. 列表、父单和任务详情使用同一数量事实，不出现“详情 3 件、列表 1 件”。
9. 销售展示样衣和产前版样衣分别统计，互不冲抵。
10. 数量不一致时按第 7.6.4 节最终确认的门禁执行，并保留真实差异。

### 21.12 端到端回归组合

最终验收不能只验证单页，至少使用以下六组独立业务案例贯穿父单、专业任务、BOM、样衣和技术包：

| 案例 | 核心场景 | 必须验证 |
| --- | --- | --- |
| 改款 1 | B 款颜色比 A 款少 | 买手颜色映射、BOM 继承、跟单多尺码销售样衣要求、实际成果 |
| 改款 2 | B 款新增无来源颜色 | 新颜色新物料、花型／调色条件任务、销售样衣真实上传 |
| 设计 1 | 原创两色多尺码 | 无 A 款、买手目标颜色、跟单要求数量、销售样衣多行结果 |
| 设计 2 | 原创款包含花型和染色 | BOM 条件任务、团队接力、买手逐项审核和返工 |
| 工程主单 1 | 纯梭织首单 | 基码纸样、产前版样衣多行要求、齐码纸样、正式技术包 |
| 工程主单 2 | 毛织＋梭织首单 | 双基码并行、产前版样衣、双齐码、花型／调色并行、正式技术包 |

六组案例均须从父单进入专业任务，再从专业任务列表重复进入并核对同一事实；所有图片和纸样使用真实文件，页面刷新后状态、数量和文件保持一致。
...
3. 为 ADJ-001～ADJ-050 建立原子需求编号和验收证据位置。
4. 将当前被新方案推翻但仍标记“已验证”的需求改为“待实施”或“已实现待验证”。
5. 新增销售展示样衣要求、产前版样衣要求、要求／实际分离、唯一任务详情入口和禁止样衣替代等原子需求。
...
6. 在改款／设计打样工作安排中增加销售展示样衣多行制作要求。
7. 在工程主单工作安排中增加产前版样衣多行制作要求。
8. 确认工作安排时同时保存任务骨架、固定依赖和制作要求。
...
7. 建立销售展示样衣和产前版样衣的“制作要求—实际成果—数量差异”详情结构。
8. 支持多个颜色尺码实际成果，并按实际件数汇总。
9. 把父单中的“进入任务”全部改为对应专业任务唯一详情入口。
10. 删除销售展示样衣替代产前版样衣的映射和旧测试契约。
...
| 工作包 | 业务目标 | 必须调整的对象 | 最小验证证据 | 完成条件 |
| --- | --- | --- | --- | --- |
| WP-SAMPLE-01 | 建立独立的样衣制作要求事实 | 样衣要求明细、要求合计、确认人和时间 | 数据契约验证多颜色多尺码和刷新保持 | 要求可独立保存且不被成果覆盖 |
| WP-SAMPLE-02 | 在独立打样中由跟单下达销售展示样衣要求 | 改款工作安排、设计工作安排 | 改款和设计各一条完整浏览器流程 | 两类打样均不能生成无要求样衣任务 |
| WP-SAMPLE-03 | 在工程主单中由跟单下达产前版样衣要求 | 主单工作安排、产前版样衣任务 | 两张工程主单的多行要求案例 | 主单发布后任务携带完整要求 |
| WP-SAMPLE-04 | 支持制作团队提交多行真实成果 | 销售展示样衣详情、产前版样衣详情、真实上传 | 多颜色尺码、真实图片、纸样版本和刷新回归 | 实际成果完整、可追溯且不改写要求 |
| WP-SAMPLE-05 | 统一数量统计和差异 | 专业列表、父单、任务详情和日志 | 要求／实际／差异一致性契约 | 不再用成果行数代替件数 |
| WP-ROUTE-01 | 统一所有专业任务详情入口 | 父单任务表、专业任务列表、业务 Tab 和路由 | 每类任务双入口一致性测试 | 同一任务任何入口打开同一事实 |
| WP-BOUNDARY-01 | 永久分开销售展示样衣和产前版样衣 | 前期成果推荐、任务映射、主单工作安排、测试 | 销售样衣完成而产前样衣仍待制作案例 | 无替代入口、映射或测试残留 |
| WP-DOC-01 | 同步权威文档和追踪闭环 | 总体设计、实施计划、追踪矩阵、测试方案 | 正向和反向追踪记录 | ADJ-043～ADJ-050 均有实现和证据位置 |

### 22.2 实施依赖顺序

```mermaid
flowchart LR
    A["WP-DOC-01 文档与原子需求"] --> B["WP-SAMPLE-01 制作要求事实"]
    B --> C["WP-SAMPLE-02 独立打样工作安排"]
    B --> D["WP-SAMPLE-03 工程主单工作安排"]
    C --> E["WP-SAMPLE-04 多行实际成果"]
    D --> E
    E --> F["WP-SAMPLE-05 数量统计与差异"]
    C --> G["WP-ROUTE-01 唯一任务入口"]
    D --> G
    G --> H["WP-BOUNDARY-01 样衣边界收口"]
    F --> I["完整验收"]
    H --> I
```
...
| 调整项 | 实施工作包 | 主要验收章节 | 交付判断 |
| --- | --- | --- | --- |
| ADJ-043 | WP-SAMPLE-01、WP-SAMPLE-02 | 21.2、21.4、21.11 | 改款和设计均能下达销售展示样衣要求 |
| ADJ-044 | WP-SAMPLE-01、WP-SAMPLE-03 | 21.2、21.4、21.11 | 工程主单能下达产前版样衣要求 |
| ADJ-045 | WP-SAMPLE-04、WP-SAMPLE-05 | 21.4、21.11 | 多行要求和多行实际成果可保存、汇总和对照 |
| ADJ-046 | WP-SAMPLE-02、WP-DOC-01 | 21.2、21.9、21.12 | 两个设计打样端到端案例通过 |
| ADJ-047 | WP-ROUTE-01 | 21.10 | 父单与专业列表进入同一任务详情 |
| ADJ-048 | WP-BOUNDARY-01 | 21.4、21.12 | 销售展示样衣不能替代产前版样衣 |
| ADJ-049 | WP-SAMPLE-05 | 21.11 | 列表、父单、详情件数一致 |
| ADJ-050 | WP-DOC-01 | 23、24 | 文档、实现、测试和证据无旧口径残留 |
...
22. 改款和设计打样均能由跟单按颜色、尺码和要求数量下达销售展示样衣要求。
23. 工程主单能由跟单按颜色、尺码和要求数量下达产前版样衣要求。
24. 两类样衣任务都能逐行提交实际数量、真实图片和使用纸样版本，并保留要求／实际差异。
25. 父单“进入任务”和专业任务列表进入同一详情事实源。
26. 不存在销售展示样衣替代、映射或完成产前版样衣的代码、文案、Mock 和测试。
27. 列表、父单和详情按件数汇总，不使用成果记录条数冒充件数。
28. 第 7.6.4 节的数量差异完成门禁已经产品确认并写入总体设计和验收契约。
...
14. 销售展示样衣和产前版样衣都必须先有跟单下达的颜色、尺码和要求数量，制作团队只负责按要求制作并提交实际结果。
15. 要求数量和实际制作数量必须同时保留；页面必须让业务人员看清原要求、实际交付和差异。
16. 设计打样没有 A 款，但在目标颜色、工作安排、销售展示样衣要求和专业执行方面与改款打样遵循同一套规则。
17. “进入任务”进入真实专业任务唯一详情页；父单负责安排和汇总，不复制第二套执行表单。
18. 销售展示样衣只能作为参考，不能替代产前版样衣，也不能完成产前版样衣的前置或状态。

---

## 26. 已确认的数量差异完成门禁

样衣实际制作数量与跟单要求数量不一致时，采用以下已确认口径：

1. 制作团队必须保存实际颜色、实际尺码和实际件数，不得把实际数量改写成要求数量来制造一致。
2. 页面逐行显示要求、实际和差异，并汇总要求总件数、实际总件数和总差异。
3. 存在任一差异时必须填写差异说明；说明为空时阻断提交。
4. 差异说明填写完整后允许提交并完成本次制作任务，不增加审批流、异常模块或“待补齐”状态。
5. 要求明细保持锁定，制作团队只能新增、修改或删除本次实际交付明细，不能反向修改跟单要求。
6. 该口径已同步进入总体设计、实施计划、追踪矩阵、专项契约和命名浏览器场景。
