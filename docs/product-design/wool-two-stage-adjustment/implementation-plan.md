...
完成条件是矩阵每项达到约定可验收状态，并具有当前分支同一运行服务的自动化及页面证据。业务代码、构建或文档存在不等于完成。性能任何适用样本超出总体设计 §12.2 对应时限、图片不全、待办数量不一致或仍有旧单复活，都不能关闭任务。
...
**完成条件**：矩阵无未说明缺口，常规 <500ms、PDA 队列冷进入 ≤1000ms 的用户许可例外、真实图片和全链路证据全部通过；产品确认版本记录明确。未达门禁如实保持待验证／阻塞，不以“功能完成”关单。
...
- 首屏性能中间结果仍超当时的 200ms，完整路由及全入口 5 次测量尚未齐全，任务保持未完成。

## 8. 2026-09-19 继续收口

- 本轮负责 VERIFY-001～007、SCOPE-003 及所有受性能/页面证据门禁约束的原子需求；不更改用户已确认的两阶段业务规则。
- 工作树 `/private/tmp/higoods-wool-main-release-20260918`，分支 `codex/wool-main-release-20260918`，基准 HEAD `4ea0f03d81035608e3c9f4afe7933013ab28a57b`；本轮代码尚未提交。
- 以来源限定读取、一次进度快照、演示批处理及毛织PDA直接路由减少无关重复计算。撤回无明显收益的KOL懒初始化和构建预载实验，保留完整业务数据与权限。
- 补充14条真实浏览器用例、主管1024尺寸、逐片多节点超计划合法接收、绑定旧技术包与新默认版本对照、四批A4打印和图片失败恢复。
- 默认500ms；依用户最新指示，仅PDA任务队列冷进入允许≤1000ms，不扩大到该页操作。详细口径见设计§12.2。
- 最终源码、构建与原始日志见 `evidence/acceptance-500/final-source-manifest.json`；完整收口状态以验证索引和需求矩阵为准，历史200ms报告原样保留。

### 本轮完成登记

WP01～WP10已按126条原子需求逐条闭合；47个不同浏览器用例、266单元、17组契约及全部3695性能样本已取得通过证据。标准列表436页/17基线、真实列拖拽与原型治理通过；最终对抗式复审通过。最后增加的仓库迟到筛选修复、同批100片分60/40实际收货和分页测试等待修正均有直接回放；见最终验证索引。本轮状态为内部已验证，不代表产品接受或增量已再次推送main。
