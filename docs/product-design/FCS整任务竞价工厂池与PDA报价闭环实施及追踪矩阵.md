...
- 专项数据契约、命名 Web/PDA 浏览器场景、相邻直接派单回归、治理、构建、CodeGraph 和任务收据完成当前版本验证。
...
| 候选工厂 PDA 管理员/负责人 | 接收待报价消息、查看整个任务和价格范围、提交一次报价 | 仅能操作本工厂所在工厂池的招标单；提交后不可修改 |
...
| 招标单 | 招标单号 | 任务快照、业务分配日期、截止时间、价格范围、工厂池模式 | 取消后的招标单保留历史；新招标单使用新编号 |
...
- `手动选择部分工厂`：支持按工厂名称、编码、地址、能力关键字搜索，按工厂类型筛选，支持选择全部当前筛选结果和清空已选。
...
3. 报价页面明确显示“覆盖整个任务”，展示 SKU 数、任务总数量、币种、计价单位和允许报价范围。
4. 报价必须为正数并位于最低/最高限价内；同一工厂只能提交一次，提交后只读展示。
...
| WP-06 | 治理与交付 | 依赖锁文件、专项脚本、Playwright、本文、审查记录 | 按锁文件恢复本地依赖；将 Playwright 升级到兼容 Node 26 的当前稳定版本；原子矩阵、正反向追踪、当前版本证据和任务收据 | 依赖树、无目标警告构建、最终命令组 | 生产构建和 Playwright 执行均无 `caniuse-lite` 过期或 `module.register()` 提示，且无无说明待实施或待验证条目 |
...
| POOL-005 | 需求 2 | 手动模式支持选择全部筛选结果和清空已选 | WP-02 | 工厂池选择事件 | 页面源码契约 | 两个批量选择动作可见 | 已验证 | Playwright 场景 2 | 待用户验收/本分支 |
...
| PDA-005 | 需求 3 | PDA 展示最低/最高报价、币种和计价单位 | WP-03 | 报价弹窗 | 价格范围断言 | PDA 报价弹窗 | 已验证 | PDA 报价截图 | 待用户验收/本分支 |
| PDA-006 | 需求 3 | 报价必须为正数且位于允许区间 | WP-01/03 | `recordRuntimeTaskTenderQuote` | 超范围抛错断言 | 输入字段约束 | 已验证 | 专项脚本 | 待用户验收/本分支 |
...
| TOOL-001 | 用户补充/构建清洁 | 使用当前浏览器数据和 Node 26 兼容工具链，生产构建及命名页面测试不得输出目标两项警告 | WP-06 | `package.json`、`package-lock.json`、本地依赖安装树 | `npm ls`、`npm run build`、Playwright 列表与执行 | 不适用（构建工具链） | 已验证 | Vite 6.4.3、Tailwind 4.3.3、tsx 4.23.6、caniuse-lite 1.0.30001806、Playwright 1.61.1；构建和 Playwright 日志无目标警告 | 待用户验收/本分支 |
| QA-001 | 完成门禁 | 专项、命名页面、治理、构建、CodeGraph 和任务收据全部形成当前证据 | WP-06 | 检查脚本、审查记录 | 最终命令组 | Web/PDA 5 个场景 | 已验证 | 当前任务收据；本节最终验证结果 | 待用户验收/本分支 |
...
4. 最后一次实质修改后重新运行所有受影响专项、5 个 Playwright 场景、治理、构建、CodeGraph 和任务收据；此前绿灯自动失效。
...
| `workflow:verify` | `verified`；收据 `/private/tmp/higoods-fcs-whole-task-tender-receipt.json`，阻塞项 0 |
