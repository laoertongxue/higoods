async page => {
 ...
 const first=await print.evaluate(async()=>{const rows=await new Promise((resolve,reject)=>{const request=indexedDB.open('higood-cutting-records-v1');request.onsuccess=()=>{const db=request.result;const tx=db.transaction('records','readonly');const q=tx.objectStore('records').getAll();tx.oncomplete=()=>{db.close();resolve(q.result)};tx.onabort=()=>reject(tx.error)}});const s={tickets:rows.filter(r=>r.collection==='replacement-tickets').map(r=>r.value),prints:rows.filter(r=>r.collection==='replacement-prints').map(r=>r.value)};return {tickets:s.tickets.length,prints:s.prints.length}});
 ...
 const reprint=await print.evaluate(async()=>{const rows=await new Promise((resolve,reject)=>{const request=indexedDB.open('higood-cutting-records-v1');request.onsuccess=()=>{const db=request.result;const tx=db.transaction('records','readonly');const q=tx.objectStore('records').getAll();tx.oncomplete=()=>{db.close();resolve(q.result)};tx.onabort=()=>reject(tx.error)}});const s={tickets:rows.filter(r=>r.collection==='replacement-tickets').map(r=>r.value),prints:rows.filter(r=>r.collection==='replacement-prints').map(r=>r.value)};return {tickets:s.tickets.length,prints:s.prints.length,kinds:s.prints.map(x=>x.kind)}});
