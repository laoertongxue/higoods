async(page)=>{const context=await page.context().browser().newContext(),p=await context.newPage();await p.goto('http://127.0.0.1:43235/fcs/craft/cutting/replacement-fabric-fei-tickets');await p.locator('[data-hpb-page]').waitFor();const result=await p.evaluate(async()=>{
 ...
 try{
  ...
  try{await m.migrateLegacyCuttingEvents({otherPagesClosed:true,progress:message=>{if(message.startsWith('迁移中：'))throw Error('模拟迁移中断')}})}catch(e){result.interrupt=e.message}
  ...
  try{await m.migrateLegacyCuttingEvents({otherPagesClosed:true,progress:message=>{if(!message.includes('裁片领料历史'))localStorage.setItem(key,JSON.stringify({events:[event('changed'),event('late')]}))}})}catch(e){result.oldPage=e.message}
