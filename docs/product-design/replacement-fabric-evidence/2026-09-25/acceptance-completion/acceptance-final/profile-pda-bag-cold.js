async root=>{
 ...
 for(const name of ['pda-bag']){
  ...
  await p.goto('http://127.0.0.1:43235/fcs/pda/cutting/inbound/TASK-CUT-PDA-CUT-DONE-0307');await p.locator('[data-pda-cutting-inbound-workflow]').first().waitFor();
  ...
  const list=[...nodeById.values()].map(n=>({name:n.callFrame.functionName,url:n.callFrame.url,line:n.callFrame.lineNumber,self:(counts.get(n.id)||0)/1000,total:(total.get(n.id)||0)/1000}));result.push({name,self:list.toSorted((a,b)=>b.self-a.self).slice(0,25),total:list.filter(x=>x.url.includes('/src/')).toSorted((a,b)=>b.total-a.total).slice(0,65)});
 }
