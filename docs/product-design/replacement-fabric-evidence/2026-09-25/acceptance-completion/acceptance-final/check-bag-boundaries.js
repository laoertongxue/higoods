async page => {
  ...
  await runScenario(['BAG-002', 'BAG-003', 'BAG-004'], async (p, out) => {
    await importMixed(p); await login(p)
    out.ticket = await freshPrintedTicket(p, 'PO-202603-0002', 'SCAN-A')
    const otherOrderId = await p.evaluate(async () => {
      const fixtures=await import(performance.getEntriesByType('resource').map(entry=>entry.name).find(url=>new URL(url).pathname==='/src/data/fcs/cutting/simple-cut-piece-handover-fixtures.ts') || '/src/data/fcs/cutting/simple-cut-piece-handover-fixtures.ts');fixtures.ensureSimpleCutPieceHandoverFixtures(); // Explicit other-order + own-cutting assignment Mock, no confirmation facts.
      ...
      const row = source.listReplacementFabricOrderRows().find(row => row.order.productionOrderId !== 'PO-202603-0002' && !row.issues.length && row.materials.length)
      if (!row) throw Error('No distinct complete production order for cross-order boundary')
      return row.order.productionOrderId
    })
    out.foreignTicket = await freshPrintedTicket(p, otherOrderId, 'SCAN-FOREIGN')
    const initialRecords = JSON.stringify(await readRecords(p))
    await bagPage(p, 'BAG-BOUNDARY-SCAN-A'); await scan(p, out.ticket.ticketNo)
    await waitText(p, '已扫菲票 1 张')
    out.firstScan = await p.locator('[data-pda-cut-inbound-live]').innerText()
    await scan(p, out.ticket.ticketNo)
    await p.waitForFunction(() => /已扫过|已经扫过|无需重复扫描/.test(document.body.innerText))
    out.duplicate = await p.locator('[data-pda-cut-inbound-live]').innerText()
    assert(out.duplicate.includes('已扫菲票 1 张'), 'Duplicate scan increased ticket count')
    assert(initialRecords === JSON.stringify(await readRecords(p)), 'Scanning/duplicate scan persisted bag contents before confirmation')
    await scan(p, out.foreignTicket.ticketNo)
    await p.waitForFunction(() => /不属于当前袋生产单|不属于当前生产单/.test(document.body.innerText))
    out.crossOrder = await p.locator('[data-pda-cut-inbound-live]').innerText()
    assert(out.crossOrder.includes('已扫菲票 1 张'), 'Cross-order scan changed ticket count')
    assert(initialRecords === JSON.stringify(await readRecords(p)), 'Cross-order rejection persisted a record')
    // Reopen the unsaved scan round through the real route after the rejected scan.
    // The product deliberately blocks confirm while its last scan is invalid.
    out.recovery='Reopen unsaved round and scan the accepted ticket again; no business result is injected';
    await bagPage(p, 'BAG-BOUNDARY-SCAN-A'); await scan(p,out.ticket.ticketNo); await waitText(p,'已扫菲票 1 张');
    await saveBag(p)
    const afterSave = await readRecords(p)
    out.baggingEvents = savedEvents(afterSave).filter(event => event.eventType === '菲票装袋' && event.refs.transferBagCode === 'BAG-BOUNDARY-SCAN-A')
    assert(out.baggingEvents.length === 1 && out.baggingEvents[0].payload.feiTicketItems.length === 1 && out.baggingEvents[0].payload.feiTicketItems[0].feiTicketId === out.ticket.id, 'Confirmed bag did not save exactly the accepted ticket')
    await bagPage(p, 'BAG-BOUNDARY-SCAN-B'); await scan(p, out.ticket.ticketNo)
    await p.waitForFunction(() => document.body.innerText.includes('已在 BAG-BOUNDARY-SCAN-A'))
    out.crossBag = await p.locator('[data-pda-cut-inbound-live]').innerText()
    assert(out.crossBag.includes('已扫菲票 0 张'), 'Cross-bag occupied ticket entered second bag')
    assert(JSON.stringify(afterSave) === JSON.stringify(await readRecords(p)), 'Cross-bag rejection changed saved records')
    await p.screenshot({ path: '/private/tmp/higoods-replacement-fabric-release-20260925/output/playwright/hpb/acceptance-final/boundary-BAG002-003-004.png', fullPage: true })
  })
  await runScenario(['HAND-009'], async (p, out) => {
    await login(p)
    await p.goto(origin + '/fcs/craft/cutting/warehouse-management/wait-handover'); await p.locator('[data-simple-cut-open]').waitFor()
    out.sheet = await p.evaluate(async () => {
      ...
      const sheet = sheets.listDispatchTaskSheetAssignments().filter(a => a.productionOrderId === fixtures.SIMPLE_CUT_PIECE_DEMO_ORDER_ID).map(a => sheets.buildDispatchTaskSheetData(a.assignmentId)).find(s => s.supportsCutPieceHandover && s.assignment.skuLines.some(line => line.size === 'M'))
      if (!sheet) throw Error('Missing eligible simple-handover task fixture')
      return { taskSheetNo: sheet.taskSheetNo, orderId: sheet.assignment.productionOrderId }
    })
    out.ticket = await freshPrintedTicket(p, out.sheet.orderId, 'SIMPLE-BOUND')
    await bagPage(p, 'BAG-BOUNDARY-SIMPLE-OCCUPIED'); await scan(p, out.ticket.ticketNo); await waitText(p, '已扫菲票 1 张'); await saveBag(p)
    await p.setViewportSize({ width: 1366, height: 768 }); await p.goto(origin + '/fcs/craft/cutting/warehouse-management/wait-handover')
    await p.locator('[data-simple-cut-open]').click()
    await p.locator('[data-simple-cut-input]').fill(out.sheet.taskSheetNo); await p.locator('[data-simple-cut-action=read]').click()
    ...
    await p.locator('[data-simple-replacement-input]').fill('HIG:HPB:1:' + encodeURIComponent(out.ticket.id)); await p.locator('[data-simple-cut-action=scan-replacement]').click()
    await p.waitForFunction(() => document.querySelector('[data-simple-cut-content] [role=status]')?.textContent.includes('已在 BAG-BOUNDARY-SIMPLE-OCCUPIED'))
    out.feedback = await p.locator('[data-simple-cut-content] [role=status]').innerText()
    assert(await p.locator('[data-simple-cut-action="remove-replacement:' + out.ticket.id + '"]').count() === 0, 'Simple handover accepted an occupied ticket')
    assert(before === JSON.stringify(await readRecords(p)), 'Occupied simple scan changed saved records')
    await p.screenshot({ path: '/private/tmp/higoods-replacement-fabric-release-20260925/output/playwright/hpb/acceptance-final/boundary-HAND009.png', fullPage: true })
  })
