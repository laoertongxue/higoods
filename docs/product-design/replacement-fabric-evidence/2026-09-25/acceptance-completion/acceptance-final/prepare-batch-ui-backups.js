...
async page => {
 ...
 for(const scenario of ['unbound-fabric-omission-and-retry','both-bound-auto-aggregation']){
  ...
  try {
   ...
   const download=p.waitForEvent('download');await p.locator('[data-hpb-data-action=export]').click();const file=await download;
   const path='output/playwright/hpb/acceptance-final/batch-prerequisite-'+scenario+'.higcut';await file.saveAs(path);
   results.push({scenario,fixture,path,errors,infrastructureErrors,noHandoverPrerequisite:true});
  }catch(error){results.push({scenario,error:String(error),errors,infrastructureErrors})}finally{await context.close()}
 }
 return {results,failed:results.filter(r=>r.error||r.errors.length),purpose:'Explicit Mock prerequisite only; actual tested handover result is absent. Production preview must restore via real data-tools UI.'};
}
