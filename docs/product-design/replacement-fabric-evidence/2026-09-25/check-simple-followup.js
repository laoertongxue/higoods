async page => {
 ...
 await p.goto('http://127.0.0.1:43225/fcs/pda/auth/login');await p.getByRole('textbox',{name:'登录账号'}).fill('OWN-CUTTING-001_admin');await p.getByRole('textbox',{name:'密码',exact:true}).fill('123456');await p.getByRole('button',{name:'登录',exact:true}).click();await p.waitForURL('**/fcs/pda/exec');
 await p.goto('http://127.0.0.1:43225/fcs/craft/cutting/warehouse-management/wait-handover');await p.locator('[data-simple-cut-open]').waitFor();
 ...
 proof.sort((a,b)=>(b.payload.replacementFabricTickets?.length||0)-(a.payload.replacementFabricTickets?.length||0));const receiver=proof[0].payload.factoryId;await p.evaluate(async()=>{(await import('/src/data/fcs/store-domain-pda.ts')).clearPdaSession()});await p.goto('http://127.0.0.1:43225/fcs/pda/auth/login');await p.getByRole('textbox',{name:'登录账号'}).fill(receiver+'_admin');await p.getByRole('textbox',{name:'密码',exact:true}).fill('123456');await p.getByRole('button',{name:'登录',exact:true}).click();await p.waitForURL('**/fcs/pda/exec');
 await p.setViewportSize({width:360,height:800});
 await p.goto('http://127.0.0.1:43225/fcs/pda/handover/RECEIPT-'+proof[0].payload.handoverRecordId);
 ...
 await context.close();return {blocked,saved,second,afterSecond,proof,head};
}
