...
**确认记录**：用户于 2026-09-23 明确确认 §12 A1–A5 全部假设口径，并确认当时矩阵 82 条原子需求的产品确认人。两轮审查后补录的 TEST-025～027 与改回待确认的 GATE-001，已随版本接受一并确认；当前矩阵 **85/85** 产品确认人均已确认。交付状态：功能提交 `f62108bd` 合并 `main` 为 `1fce47c2` 并推送 `origin/main`（`delivered`）；用户 laoertongxue 于 2026-09-23 通过 GitHub commit comment 接受该版本（`accepted`，回执 https://github.com/laoertongxue/higoods/commit/1fce47c2e7845e05fb142d51b0d5d4f25782ed61#commitcomment-201603472）。本节无遗留待确认假设。
...
## 14. 2026-09-23 修复增量口径

本节来自用户本次追加要求，适用于 `codex/pcs-repair`；不沿用历史 `1fce47c2` 接受回执。

1. **TYPE-001**：全量 `tsc --noEmit` 的 68 条既有错误必须修复，不能以范围外排除或缩小检查范围作为通过依据。停用入口保持既有阻断语义。
2. **TYPE-002**：终止连续余料按实物账统一计算，报废只扣一次，超量必须阻断；已有下游印花绑定必须参与直接截断判断。
3. **IMG-AUDIT-001 / IMG-FILL-001**：由代理自行搜索商品与物料实拍素材，逐款号、SKU 颜色和物料 SKU 建立固定图片与来源记录；停止无关图片池自动分配。原型编码未提供真实供货源时，照片只作为品类、结构和可见颜色相符的原型选材，不能宣称验证了 HiGood 实物身份、供货商或尺寸。保留用户自有图片，只迁移登记的旧 Mock 图片。缩略图和对象标识同块，支持大图、关闭及可见失败状态。
4. **FCS-ENTRY-002**：后道 QC 的 3 组原型生产来源应共用需求、生产单、演示技术包、来源任务、主工厂及 15 个 SKU；款式每种颜色分别配图，既有浏览器 Mock 图片迁移不得改变质检结果或数量。
5. **PRINT-LIST-001 / TEST-024**：按用户确认保留八个业务列，时间/数量分列；573.6ms 及本次缺失逐项性能证据按用户明确例外记录，原始值保留，不改写为 `<500ms` 通过。

实施与证据分别见《PCS当前问题修复实施与核查》、矩阵 §0.1 以及本次原型审查记录 §8。
