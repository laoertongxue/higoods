...
- 模板：普通菲票删除特殊工艺字段；特殊工艺菲票主标题固定为“特殊工艺菲票——具体特殊工艺名称”，正文展示承接工厂，并明确展示生产单号（PO）和 SPU。黄色热敏纸只作为装纸提示和票面视觉样式，不进入业务标题。
- 演示数据：系统生成菲票稳定提供 12 张普通白纸菲票，覆盖 2 个生产单、2 个铺布批次、3 类部位和 S／M／L／XL 四个尺码；与已有辅助／特种工艺场景共同组成可直接验收的 Mock 集合。
...
12. 特殊工艺菲票标题必须读取当前菲票的具体工艺名称；一张票包含多个工艺时去重后使用“／”连接。首次打印与补打使用同一业务标题，打印模式和黄色热敏纸信息在标题之外单独表达。
13. 演示数据必须同时包含普通和特殊工艺菲票；普通场景不得依赖浏览器临时创建，并且生产单、铺布批次、部位、尺码、数量、二维码和白纸分类保持一致。
...
13. **特殊模板场景**：每张票的主标题按“特殊工艺菲票——具体特殊工艺名称”展示；多工艺名称去重后以“／”连接。正文独立展示非空的生产单号（PO）、SPU、辅助／特种工艺与承接工厂；黄色热敏纸只在装纸提示和票面样式中表达，长文本允许换行，二维码可扫描识别当前菲票。
14. **补打追溯场景**：补打必须填写原因，继承原纸色和模板；完成打印动作后记录打印／补打、原因、操作人、纸色、模板、标签尺寸和来源范围。
15. **相邻回归场景**：捆条菲票独立链路、原部位菲票来源追溯、菲票二维码、编号和中转袋关联不因本次调整改变。
16. **系统 Mock 覆盖场景**：清空浏览器运行时数据后，列表仍至少存在 12 张普通白纸菲票，且分别归属 2 个生产单和 2 个铺布批次，覆盖前片、后片、袖片及 S／M／L／XL；普通票的工艺数组为空，特殊工艺票仍可进入黄纸打印。
