...
try{
 ...
 assert.match(await surface.innerText(),/旧版本已有生产发料或实收/);assert.equal(await surface.locator('[data-tmf-version-action="confirm"]').count(),0)
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true)
 await page.screenshot({path:'output/playwright/tmf-webbing/version-replan-regression.png',fullPage:true})
 evidence.checks.push('下一次换版已发生生产实收，弹窗显示阻断且不提供整单重做确认')
 assert.deepEqual(evidence.errors,[])
}finally{writeFileSync('output/playwright/tmf-webbing/version-replan-regression-browser.json',JSON.stringify(evidence,null,2));await browser.close()}
