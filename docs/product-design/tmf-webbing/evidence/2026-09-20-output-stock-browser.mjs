...
function productionSource(id) {
 ...
 await page.evaluate(async()=>{
  const m=await import(performance.getEntriesByType('resource').find(e=>new URL(e.name).pathname==='/src/data/pms/tmf-material-purchases.ts')?.name??'/src/data/pms/tmf-material-purchases.ts')
  const issues=m.getTmfPurchaseState().productionIssues
  for(const i of issues){
   m.receiveTmfProductionPackage({issueId:i.id,packageId:i.packageId,demandId:i.demandId,receiverOrganizationId:i.receiverOrganizationId,pieces:i.dispatchedPieces},{id:i.receiverId,name:'实际领料人（数据验证）',role:'生产领料人'},`${i.id}:receive`)
  }
 })
 await page.reload();await stock.waitFor()
 await openStock('detail',0)
 ...
 evidence.checks.push('库存页两规格分配40/60条、释放10再分配、分配与发料超量及错包阻断；实际发100后仓内0且生产实收0/需求未满足；数据动作由指定领料人实收后页面回读满足，未将发出当实收')

 assert.deepEqual(evidence.errors,[])
 console.log(JSON.stringify(evidence,null,2))
}catch(error){evidence.failure=String(error);console.log(JSON.stringify(evidence,null,2));throw error}
finally{writeFileSync('output/playwright/tmf-webbing/output-stock-evidence.json',JSON.stringify(evidence,null,2));await browser.close()}
