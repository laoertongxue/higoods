...
function productionSource(id) {
  ...
  m.registerTmfProductionOrder(order,factory,'OUTPUT-register')
 ...
 for(const [index,qty,length,meters] of [[0,40,500,20],[1,60,700,42]]){
  ...
  await detail.locator('[name="quantity"]').fill(String(meters))
  ...
  await detail.locator('[name="quantity"]').fill(String(qty+1))
...
finally{writeFileSync('output/playwright/tmf-webbing/tipping-evidence.json',JSON.stringify(evidence,null,2));await browser.close()}
