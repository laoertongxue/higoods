...
try{
 ...
 await wd.locator('[name="rawLot"]').selectOption('RAW-LOT');await wd.locator('[name="rawSku"]').fill('YARN-DEMO-01');await wd.locator('[name="rawScan"]').fill('RAW-LOT');await wd.locator('[name="rawIssue"]').fill('RAW-ISS');await wd.locator('[name="quantity"]').fill('13');await wd.locator('[name="rawConfirmed"]').check()
 ...
 await wd.locator('[name="rawSku"]').fill('YARN-DEMO-01');await wd.locator('[name="rawScan"]').fill('RAW-RETURN');await wd.locator('[name="quantity"]').fill('2');await wd.locator('[name="rawConfirmed"]').check()
 ...
 await wa('raw-return-receive').click();await wd.locator('[name="rawSku"]').fill('YARN-DEMO-01');await wd.locator('[name="rawScan"]').fill('RAW-RETURN');await wd.locator('[name="quantity"]').fill('0.6');await wd.locator('[name="rawConfirmed"]').check();await wa('confirm').click()
...
}finally{await browser.close();writeFileSync('output/playwright/tmf-webbing/base-raw-warehouse-browser.json',JSON.stringify(evidence,null,2));console.log(JSON.stringify(evidence,null,2))}
