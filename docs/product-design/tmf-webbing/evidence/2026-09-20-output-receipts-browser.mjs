...
function productionSource(id) {
  ...
  globalThis.__tmfDebug={debugDemands:m.getTmfPurchaseState().demands.filter(d=>d.productionOrderId===order.productionOrderId).length,debugOutputs:m.getTmfPurchaseState().cutOutputs.length,debugPackages:m.getTmfPurchaseState().packages.length,debugHandovers:m.getTmfPurchaseState().outputHandovers.length}
 })
 const debug = await page.evaluate(()=>globalThis.__tmfDebug)
 ...
 evidence.pageDebug = await page.evaluate(async()=>{const m=await import('/src/data/pms/tmf-material-purchases.ts'); return {state:m.getTmfPurchaseState().outputHandovers.length,html:document.querySelector('[data-tmf-output-table]')?.textContent}})
...
finally{writeFileSync('output/playwright/tmf-webbing/output-receipt-evidence.json',JSON.stringify(evidence,null,2));await browser.close()}
