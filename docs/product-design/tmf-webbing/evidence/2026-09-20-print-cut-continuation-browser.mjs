...
function productionSource(id) {
 ...
 const beforeBinding=await page.evaluate(async()=>{
  ...
  m.assignPrintingWorkOrder('UPSTREAM-PRINT',{factoryId:'F090',operatorName:'测试计划'})
  const order=m.getPrintWorkOrderById('UPSTREAM-PRINT')
  return {source:order.sourceSnapshot,tmf:localStorage.getItem('higood-tmf-material-purchases-v1')}
 })
 await root.locator('[data-tmf-work-detail-action="tab"][data-id="requirements"]').click()
 await root.locator('[data-tmf-work-detail-action="tab"][data-id="upstream"]').click()
 ...
 const readTarget=()=>page.evaluate(async()=>{
  ...
  return {source:o.sourceSnapshot,kind:o.receiverKind,id:v.receivingTargetId,warehouse:o.targetTransferWarehouseId,continuation:o.productionTmfContinuation,tmf:localStorage.getItem('higood-tmf-material-purchases-v1')}
 })
 const bound=await readTarget()
 assert.equal(bound.kind,'FACTORY');assert.equal(bound.id,'FAC-TMF');assert.equal(bound.warehouse,'');assert.equal(bound.continuation.cutEntryId,'CUT')
 assert.deepEqual(bound.source,beforeBinding.source);assert.equal(bound.tmf,beforeBinding.tmf)
 await root.locator('[data-tmf-work-detail-action="link-tmf"]').click()
 assert.deepEqual(await readTarget(),bound)
 await page.reload();await root.waitFor()
 assert.deepEqual(await readTarget(),bound)
 await root.locator('[data-tmf-work-detail-action="tab"][data-id="upstream"]').click()
 await root.locator('[data-tmf-work-detail-action="link-tmf"]').waitFor()
 assert.match(await root.locator('[data-tmf-upstream-content]').innerText(),/后续接收：TMF - 辅料厂/)
 await page.screenshot({path:'output/playwright/tmf-webbing/print-cut-continuation.png',fullPage:true})
 evidence.checks.push('印花直接截断路线绑定FAC-TMF工厂，未伪造中转仓；来源快照和整个TMF数量账不变；重复确认与刷新保持绑定')
...
}catch(e){evidence.failure=String(e);throw e}finally{writeFileSync('output/playwright/tmf-webbing/print-cut-continuation-evidence.json',JSON.stringify(evidence,null,2));await browser.close()}
