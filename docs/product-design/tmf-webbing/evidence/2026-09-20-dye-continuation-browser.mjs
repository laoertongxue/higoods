...
function productionSource(id) {
 ...
 await page.evaluate(async()=>{
 const loaded=name=>performance.getEntriesByType('resource').find(e=>new URL(e.name).pathname===`/src/data/fcs/${name}.ts`)?.name??`/src/data/fcs/${name}.ts`
 const p=await import(loaded('printing-task-domain'))
 p.assignPrintingWorkOrder('UPSTREAM-PRINT',{factoryId:'F090',operatorName:'测试计划'})
 })
 await page.goto(`http://127.0.0.1:43188/fcs/craft/accessory/webbing/work-orders/${encodeURIComponent(key)}`)
 await page.locator('[data-tmf-work-detail-action="tab"][data-id="upstream"]').click()
 await page.locator('[data-tmf-work-detail-action="link-print"]').click()
 await page.getByText('已按采用路线确认印花接收方。实际交出及实收仍须分别登记。',{exact:true}).waitFor()
 await page.reload()
 const processing=await page.evaluate(async()=>{
 const loaded=name=>performance.getEntriesByType('resource').find(e=>new URL(e.name).pathname===`/src/data/fcs/${name}.ts`)?.name??`/src/data/fcs/${name}.ts`
 const d=await import(loaded('dyeing-task-domain'))
 const order=d.getDyeWorkOrderById('UPSTREAM-DYE')
 if(order.productionPrintContinuation?.orderId!=='UPSTREAM-PRINT'||d.getDyeDispatchPartner(order.dyeOrderId).id!=='F090')throw new Error('刷新丢失接续')
 d.acceptDyeWorkOrderPdaTask(order.taskId,'测试染厂')
 const vat=d.listDyeVatOptions('F090')[0]
 if(!vat)throw new Error('测试厂无染缸')
 d.planDyeVat(order.dyeOrderId,{dyeVatNo:vat.dyeVatNo,operatorName:'测试染厂'})
 d.startDyeing(order.dyeOrderId,{dyeVatNo:vat.dyeVatNo,inputQty:650,operatorName:'测试染厂',materialSku:'tmf-webbing-reference-white'})
 d.completeDyeing(order.dyeOrderId,{inputQty:650,outputQty:640,operatorName:'测试染厂'})
 for(const code of ['DEHYDRATE','DRY','SET','ROLL','PACK']){d.startDyeNode(order.dyeOrderId,code,'测试染厂');d.completeDyeNode(order.dyeOrderId,code,{outputQty:640,operatorName:'测试染厂'})}
 const result=d.getDyeWorkOrderById(order.dyeOrderId),nodes=d.listDyeExecutionNodeRecords(order.dyeOrderId)
 result.outputMaterial.imageUrl='/materials/tmf/webbing-real-box.jpg'
 const rolls=d.saveDyeOutputRolls(order.dyeOrderId,[{qty:640,vatNo:vat.dyeVatNo,remark:'测试实际640米蓝料'}])
 d.markDyeOutputRolls(order.dyeOrderId,[rolls[0].id],'print')
 const dispatch=d.createDyeDispatchDocument([{orderId:order.dyeOrderId,rollIds:[rolls[0].id]}],'测试交出员')
 d.scanDyeDispatchRoll(dispatch.id,rolls[0].barcode,'测试交出员')
 d.saveDyeDispatchTransport(dispatch.id,{driver:'测试司机',vehicle:'测试车辆',plate:'TEST-001',note:'测试交接'})
 let blocked=''
 try{d.finishDyeDispatchDocument(dispatch.id,'confirm')}catch(error){blocked=error.message}
 if(blocked)throw new Error('已有真实实拍替代图不应阻断交出：'+blocked)
 const finalDoc=d.listDyeDispatchDocuments().find(r=>r.id===dispatch.id)
 if(finalDoc.status==='草稿'||d.getDyeDispatchAvailableQty(order.dyeOrderId)!==0)throw new Error('有图交出未完成或数量未扣减')
 return {order:result,nodes,available:d.getDyeDispatchAvailableQty(order.dyeOrderId),blocked,dispatchStatus:finalDoc.status}
 })
 assert.equal(processing.available,0)
 const dyeNode=processing.nodes.find(n=>n.nodeCode==='DYE')
 assert.equal(dyeNode.inputQty,650);assert.equal(dyeNode.outputQty,640)
 evidence.processing={orderId:processing.order.dyeOrderId,outputSku:processing.order.outputMaterial.sku,input:dyeNode.inputQty,output:dyeNode.outputQty,loss:dyeNode.inputQty-dyeNode.outputQty,available:processing.available,blocked:processing.blocked,dispatchStatus:processing.dispatchStatus}
 evidence.checks.push('页面核对正式生产单染色→印花接收工厂，刷新保持；原生染色接单/排缸/开工扣用650→实际产出640→各后处理与包装640；真实实拍替代图可交出，差额10米保留')
 assert.deepEqual(evidence.errors,[])
}catch(e){evidence.failure=String(e);throw e}finally{writeFileSync('output/playwright/tmf-webbing/dye-continuation-evidence.json',JSON.stringify(evidence,null,2));await browser.close()}
