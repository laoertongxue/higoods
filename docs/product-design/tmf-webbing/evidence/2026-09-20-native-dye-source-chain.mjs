...
function productionSource(id) {
  ...
  pack.bomItems = [{ ...pack.bomItems[0], id: 'BOM-WB', type: '辅料', materialSkuId: 'tmf-webbing-reference-white', materialCode:'tmf-webbing-reference-white', unit:'米', unitConsumption:0.65, lossRate:0, applicableSkuCodes: [] }]
  const specification = {
 ...
 await page.screenshot({path:'output/playwright/tmf-webbing/native-dye-source.png'})
 ...
 await page.screenshot({path:'output/playwright/tmf-webbing/native-dye-issued.png'})


 const completed=await page.evaluate(async()=>{
 const load=async(path)=>import(performance.getEntriesByType('resource').find(e=>new URL(e.name).pathname===path)?.name??path)
 const dye=await load('/src/data/fcs/dyeing-task-domain.ts'),f=await load('/src/data/fcs/factory-receiving.ts')
 const id='UPSTREAM-DYE'
 dye.planDyeVat(id,{dyeVatNo:'N01-VAT',operatorName:'测试染色主管'})
 dye.startDyeing(id,{dyeVatNo:'N01-VAT',inputQty:650,materialSku:'tmf-webbing-reference-white',operatorName:'测试染色员'})
 dye.completeDyeing(id,{inputQty:650,outputQty:640,operatorName:'测试染色员'})
 for(const node of ['DEHYDRATE','DRY','SET','ROLL','PACK']){dye.startDyeNode(id,node,'测试染色员');dye.completeDyeNode(id,node,{outputQty:640,operatorName:'测试染色员'})}
 const rolls=dye.saveDyeOutputRolls(id,[{qty:640,vatNo:'N01-VAT'}]);dye.markDyeOutputRolls(id,[rolls[0].id],'print')
 return {input:f.listFactoryMaterialUses(id).flatMap(u=>u.lines).reduce((n,l)=>n+l.qty,0),output:dye.getDyeDispatchAvailableQty(id),image:dye.getDyeDispatchMaterial(id).imageUrl}
 })
 assert.deepEqual(completed,{input:650,output:640,image:''});evidence.completed=completed
 await page.goto('http://127.0.0.1:43188/fcs/craft/dyeing/pending-handover');await page.locator('[data-dye-output-page]').waitFor()
 const check=async()=>{
 await page.locator('[data-dye-output-field="keyword"]').fill('UPSTREAM-DYE');await page.locator('[data-dye-output-action="query"]').click()
 const row=page.locator('tbody tr').filter({has:page.locator('[data-dye-output-select-order="UPSTREAM-DYE"]')});await row.waitFor();assert.match(await row.innerText(),/640.00 米/);assert.match(await row.innerText(),/缺对应实物图/)
 await page.locator('[data-dye-output-select-order="UPSTREAM-DYE"]').check();await page.locator('[data-dye-output-action="create"]').click();await page.locator('[data-dye-output-action="save-document"]').click();await page.waitForFunction(()=>document.querySelector('[data-dye-output-modal-feedback]')?.textContent.includes('缺少染色后辅料 BLUE'))
 }
 await check();await page.reload();await page.locator('[data-dye-output-page]').waitFor();await check()
 const restored=await page.evaluate(async()=>{
 const load=async(path)=>import(performance.getEntriesByType('resource').find(e=>new URL(e.name).pathname===path)?.name??path)
 const dye=await load('/src/data/fcs/dyeing-task-domain.ts'),m=await load('/src/data/pms/tmf-material-purchases.ts'),f=await load('/src/data/fcs/factory-receiving.ts')
 return {input:f.listFactoryMaterialUses('UPSTREAM-DYE').flatMap(u=>u.lines).reduce((n,l)=>n+l.qty,0),output:dye.getDyeDispatchAvailableQty('UPSTREAM-DYE'),warehouse:m.getTmfPurchaseState().lots.find(l=>l.id==='BRIDGE-LOT').onHandMeters,docs:dye.listDyeDispatchDocuments().filter(d=>d.lines.some(l=>l.orderId==='UPSTREAM-DYE')).length}
 })
 assert.deepEqual(restored,{input:650,output:640,warehouse:350,docs:0});evidence.restored=restored
 evidence.checks.push('持久化生产主单与路线来源、PMS采购及实际仓库/染厂收发后原生650→640染色包装；刷新恢复650米实际用料、640米产出及350米基础仓余；缺BLUE图两次建单均阻断且零交出')
 evidence.scope='N01上游采购至原生染色包装及刷新恢复；PCS发布采用页面未操作，印花及下游未执行，真实素材和全入口性能未验'
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await page.screenshot({path:'output/playwright/tmf-webbing/native-dye-source-chain.png'})
 assert.deepEqual(evidence.errors,[])
}catch(e){evidence.failure=String(e);throw e}finally{writeFileSync('output/playwright/tmf-webbing/native-dye-source-chain.json',JSON.stringify(evidence,null,2));await browser.close()}
