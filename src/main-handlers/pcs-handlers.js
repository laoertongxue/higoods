const handlerModuleCache = new Map();
function isExactOrNestedPath(pathname, basePath) {
    return pathname === basePath || pathname.startsWith(`${basePath}/`);
}
function isAnyExactOrNestedPath(pathname, basePaths) {
    return basePaths.some((basePath) => isExactOrNestedPath(pathname, basePath));
}
function getCurrentPathname() {
    return window.location.pathname || '';
}
const PCS_HANDLER_SPECS = [
    {
...
function getActiveHandlerSpec(pathname = getCurrentPathname()) {
    return PCS_HANDLER_SPECS.find((spec) => spec.matches(pathname)) ?? null;
}
function invokeBooleanExport(module, exportName, arg) {
    if (!exportName) {
        return false;
    }
    ...
    return Boolean(handler(arg));
}
function invokeDialogStateExport(module, exportName) {
    if (!exportName) {
        return false;
    }
...
async function loadHandlerModule(spec) {
...
export async function dispatchPcsPageEvent(target) {
...
export async function dispatchPcsInputEvent(target) {
    const spec = getActiveHandlerSpec();
    if (!spec?.inputExport) {
        return false;
    }
...
export async function closePcsDialogsOnEscape() {
    const spec = getActiveHandlerSpec();
