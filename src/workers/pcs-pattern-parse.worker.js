...
import { buildParseSummary, buildDownsamplePlan, buildPerceptualHashFromRgba, decodeTiffToSampledRgba, detectHasAlphaFromRgba, downsampleRgba, getDominantColors, getPatternMimeTypeFromExt, guessColorModeFromPixels, parseJpegMetadata, parsePngMetadata, parseTiffMetadata, sha256Hex, tokenizePatternFilename, } from '../utils/pcs-pattern-library-core.ts';
function toResultBase(fileName, ext, mimeType, fileSize) {
    return {
...
async function buildPreviewBlobFromRgba(rgba, sourceWidth, sourceHeight, maxDimension) {
...
async function parseTiffBuffer(fileName, mimeType, buffer) {
    ...
    try {
        ...
        const previewBlob = await buildPreviewBlobFromRgba(previewDecoded.rgba, previewDecoded.width, previewDecoded.height, Math.max(previewDecoded.width, previewDecoded.height));
        const thumbnailPlan = buildDownsamplePlan(previewDecoded.width, previewDecoded.height, 320);
        const thumbnailRgba = downsampleRgba(previewDecoded.rgba, previewDecoded.width, previewDecoded.height, thumbnailPlan.width, thumbnailPlan.height);
        const thumbnailBlob = await buildPreviewBlobFromRgba(thumbnailRgba, thumbnailPlan.width, thumbnailPlan.height, Math.max(thumbnailPlan.width, thumbnailPlan.height));
        ...
        return {
            ...
            thumbnailBlob,
            parseStatus: 'success',
            parseSummary: buildParseSummary(ext, metadata.width ?? previewDecoded.originalWidth, metadata.height ?? previewDecoded.originalHeight, metadata.dpiX, metadata.dpiY, metadata.frameCount ?? previewDecoded.frameCount ?? 1),
            dominantColors,
        ...
        return {
            ...
            aspectRatio: metadata.width && metadata.height ? Number((metadata.width / Math.max(metadata.height, 1)).toFixed(4)) : undefined,
...
async function parseRasterBuffer(fileName, mimeType, ext, buffer) {
    const fileSize = buffer.byteLength;
    const filenameTokens = tokenizePatternFilename(fileName);
    const sha256 = await sha256Hex(buffer);
    const originalBlob = new Blob([buffer], { type: mimeType });
...
async function parseFileInWorker(request) {
