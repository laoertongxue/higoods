...
function buildShapeTags(features) {
    const tags = [];
    ...
    else if (features.taperRatio >= SHAPE_TAG_THRESHOLDS.taperBalancedMin &&
        features.taperRatio <= SHAPE_TAG_THRESHOLDS.taperBalancedMax) {
        pushUnique(tags, '两端接近等宽');
    }
    if (features.midWidth > Math.max(features.headWidth, features.tailWidth, 1) * SHAPE_TAG_THRESHOLDS.midWidthBoost) {
...
function buildSuitability(features) {
    const reasons = [];
    ...
    if (features.complexityLevel === 'high' ||
        ...
        if (features.cornerCount > 12)
            lowReasons.push('明显拐点过多');
        if (features.innerHoleCount > 2)
            lowReasons.push('内轮廓偏多');
        if (features.majorArcCount >= 3 && features.curvatureLevel === 'strong')
            lowReasons.push('多主曲边且弯曲明显');
        return {
...
function buildAutoDescription(features, tags) {
    const shape = tags.find((item) => ['长条形', '短宽形', '近矩形', '弧形', '明显弧形'].includes(item));
    const symmetry = tags.find((item) => item.startsWith('左右'));
    const curvature = tags.find((item) => item.includes('曲边') || item.includes('弧形'));
    const widthTrend = tags.find((item) => ['两端收窄', '两端接近等宽', '中段更宽'].includes(item));
    const structure = tags.find((item) => item === '含内轮廓') ??
...
export function buildPartShapeDescription(features) {
...
export function getTemplateMachineSuitabilityLabel(value) {
    if (value === 'high')
        return '高';
    if (value === 'medium')
        return '中';
