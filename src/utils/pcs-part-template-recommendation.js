...
function getNameSimilarity(source, target) {
    ...
    if (normalizedSource.includes(normalizedTarget) || normalizedTarget.includes(normalizedSource))
        return 0.8;
    if (normalizedSource.slice(0, 2) && normalizedTarget.includes(normalizedSource.slice(0, 2)))
        return 0.55;
    return 0;
}
...
function getSignatureSimilarity(left, right) {
...
function getRangeSimilarity(gap, highSimilarityGap, lowSimilarityGap) {
    if (gap <= highSimilarityGap)
        return 1;
    if (gap >= lowSimilarityGap)
        return 0;
...
function scoreNames(input, candidate) {
    ...
    return {
        ...
        reason: bestScore > 0 && bestPair
            ? `候选名接近：${bestPair[0]} 与模板名称 ${bestPair[1]} 接近`
            : undefined,
    };
}
function scoreSize(input, candidate) {
    if (!input.sizeCode || !candidate.sizeCode)
        return { score: 0, reason: undefined };
    if (normalizeText(input.sizeCode) !== normalizeText(candidate.sizeCode))
        return { score: 0, reason: undefined };
    return {
...
function scoreGeometry(input, candidate) {
    ...
    let score = 0;
    const reasons = [];
    const aspectRatioSimilarity = getRangeSimilarity(Math.abs(inputFeatures.aspectRatio - candidateFeatures.aspectRatio), 0.15, 0.5);
    ...
    const symmetrySimilarity = getRangeSimilarity(Math.abs(inputFeatures.symmetryScore - candidateFeatures.symmetryScore), 0.05, 0.25);
    score += WEIGHTS.symmetryScore * symmetrySimilarity;
    if (symmetrySimilarity >= 0.55) {
        reasons.push('几何接近：对称度接近');
    }
    const curvatureSimilarity = inputFeatures.curvatureLevel === candidateFeatures.curvatureLevel
        ? 1
        : Math.abs(['straight', 'slight', 'medium', 'strong'].indexOf(inputFeatures.curvatureLevel) -
            ['straight', 'slight', 'medium', 'strong'].indexOf(candidateFeatures.curvatureLevel)) === 1
            ? 0.5
            : 0;
    ...
    const taperSimilarity = getRangeSimilarity(Math.abs(inputFeatures.taperRatio - candidateFeatures.taperRatio), 0.08, 0.35);
    score += WEIGHTS.taperRatio * taperSimilarity;
    if (taperSimilarity >= 0.55) {
        reasons.push('宽度趋势接近：收窄比例相近');
    }
    const signatureSimilarity = getSignatureSimilarity(input.normalizedShapeSignature ?? inputFeatures.normalizedShapeSignature, candidate.normalizedShapeSignature ?? candidateFeatures.normalizedShapeSignature);
...
function scoreHistory(candidate) {
    const historyScore = clamp(candidate.reuseHitCount / 50 + candidate.hotStyleCount / 8 + candidate.cumulativeOrderQty / 5000, 0, 1);
    return {
...
export function scorePartTemplateRecommendation(input, candidate) {
    const reasons = [];
    ...
    const totalScore = clamp(Math.round(name.score + size.score + geometry.score + history.score), 0, 100);
    return {
