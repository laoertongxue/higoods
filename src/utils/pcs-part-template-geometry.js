...
const WIDTH_SAMPLE_POSITIONS = [0.1, 0.5, 0.9];
...
function unrotate(point, center, angle) {
    const cos = Math.cos(angle);
    const sin = Math.sin(angle);
    return {
...
function simplifyClosedBoundary(points) {
    if (points.length <= 8)
        return points;
    const bbox = getBoundingBox(points);
...
function rotateBoundary(points, center, angle) {
    return points.map((point) => rotate(point, center, angle));
}
function segmentIntersectsAtX(start, end, x) {
...
function computeWidthAtProjection(rotatedPoints, projectionX) {
    const intersections = [];
...
function estimatePointDistanceToBoundary(point, boundary) {
    return Math.min(...boundary.map((candidate) => Math.hypot(candidate.x - point.x, candidate.y - point.y)));
}
function computeSymmetryScore(rotatedPoints, majorAxisLength) {
    if (rotatedPoints.length === 0 || majorAxisLength <= EPSILON)
        return 1;
...
function normalizeTurnAngle(angle) {
...
function computeSegmentLength(points, startIndex, endIndex) {
    let total = 0;
...
function buildArcSegments(points, perimeter) {
    if (points.length < 4 || perimeter <= EPSILON)
        return [];
    const turnAngles = [];
    const segmentLabels = [];
    ...
    function closeCurrent(endIndex) {
        ...
        const avgSagitta = sagittas.length > 0 ? sagittas.reduce((sum, value) => sum + value, 0) / sagittas.length : 0;
        const estimatedRadius = maxSagitta > EPSILON
...
function buildNormalizedShapeSignature(points) {
    if (points.length === 0)
        return '';
...
export function buildGeometryHashFromSignature(signature) {
    return createSimpleHash(signature);
}
function computeComplexityLevel(params) {
    const score = params.pointCount / 24 +
...
function estimateNotchCount(points) {
    if (points.length < 3)
        return 0;
    let count = 0;
...
export function buildPartGeometryAnalysis(params) {
    if (!params.outerBoundary || params.outerBoundary.points.length < 3)
        return null;
    ...
    const widthSamples = WIDTH_SAMPLE_POSITIONS.map((ratio) => computeWidthAtProjection(rotated, rotatedBounds.minX + principalAxisLength * ratio));
    ...
    const avgSagitta = arcSegments.length > 0
        ? arcSegments.reduce((sum, segment) => sum + segment.sagitta, 0) / arcSegments.length
        : 0;
    const maxEstimatedRadius = arcSegments.reduce((max, segment) => {
        if (segment.estimatedRadius === null)
            return max;
        if (max === null)
            return segment.estimatedRadius;
        return Math.max(max, segment.estimatedRadius);
    }, null);
    const curvatureRatio = maxSagitta / principalAxisLength;
    const curvatureLevel = curvatureRatio < 0.02 ? 'straight' : curvatureRatio < 0.05 ? 'slight' : curvatureRatio < 0.1 ? 'medium' : 'strong';
    let curveLength = 0;
    ...
    const longestGrainLine = [...(params.grainLines ?? [])].sort((left, right) => distance(right.start, right.end) - distance(left.start, left.end))[0];
    const grainLineAngle = longestGrainLine
        ? normalizeAngleDegrees((Math.atan2(longestGrainLine.end.y - longestGrainLine.start.y, longestGrainLine.end.x - longestGrainLine.start.x) *
            180) /
            Math.PI)
        : null;
