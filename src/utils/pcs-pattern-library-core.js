...
export const DEFAULT_PATTERN_CATEGORY_TREE = [
    {
        value: '动物纹理',
        label: '动物纹理',
        children: [
...
export function clonePatternCategoryTree(tree) {
...
export function getPatternCategoryPrimaryOptions(tree) {
    return tree.map((node) => node.value);
}
export function getPatternCategorySecondaryOptions(tree, primary) {
    if (!primary)
        return [];
    return tree.find((node) => node.value === primary)?.children.map((child) => child.value) ?? [];
}
export function buildPatternCategoryPath(primary, secondary, missingLabel = '待补录') {
    if (!primary && !secondary)
        return missingLabel;
    if (!primary)
        return `${missingLabel} / ${secondary ?? missingLabel}`;
    return `${primary} / ${secondary || missingLabel}`;
}
export function formatPatternCategoryTreeText(tree) {
    return tree
        .map((node) => `${node.value} > ${node.children.map((child) => child.value).join('|')}`)
        .join('\n');
}
export function parsePatternCategoryTreeText(text, fallback = DEFAULT_PATTERN_CATEGORY_TREE) {
    const rows = text
        .split('\n')
        .map((row) => row.trim())
        .filter(Boolean);
...
export function getPatternMimeTypeFromExt(ext) {
...
export function tokenizePatternFilename(fileName) {
...
export function getPatternCategorySuggestions(input) {
...
export function isPatternLicenseUsable(status) {
    return status === 'authorized' || status === 'restricted';
}
export function canPatternBeReferenced(asset) {
    if (asset.parse_status !== 'success')
        return { allowed: false, reason: '解析成功后才允许正式引用' };
    if (asset.review_status !== 'approved')
        return { allowed: false, reason: '审核通过后才允许正式引用' };
    if (asset.lifecycle_status !== 'active')
        return { allowed: false, reason: '仅启用中的花型可正式引用' };
...
function mapRgbToColorLabel(red, green, blue) {
    ...
    const matched = COLOR_DICTIONARY.find((item) => {
        const inHueRange = item.hueMin <= item.hueMax
...
export function detectHasAlphaFromRgba(rgba) {
...
export function guessColorModeFromPixels(rgba, hasAlpha) {
    ...
    if (grayLikePixels / sampleCount > 0.88)
        return hasAlpha ? 'Gray + Alpha' : 'Gray';
    return hasAlpha ? 'RGBA' : 'RGB';
}
export function getDominantColors(rgba) {
...
function downsampleToGray(rgba, width, height, targetSize) {
    const values = [];
...
function discreteCosineTransform(values, size) {
...
export function buildPerceptualHashFromRgba(rgba, width, height) {
...
function getTiffValueOffset(entryOffset, type, count, littleEndian, view) {
    const totalSize = (TIFF_TYPE_SIZES[type] ?? 0) * count;
    return totalSize <= 4 ? entryOffset + 8 : view.getUint32(entryOffset + 8, littleEndian);
}
function readTiffValues(view, entryOffset, type, count, littleEndian) {
...
function parseTiffIfds(buffer) {
    const view = new DataView(buffer);
    if (view.byteLength < 8)
        throw new Error('TIFF 文件头长度不足');
    const byteOrder = String.fromCharCode(view.getUint8(0), view.getUint8(1));
    const littleEndian = byteOrder === 'II';
    if (!littleEndian && byteOrder !== 'MM')
        throw new Error('当前文件不是有效 TIFF');
    const magic = view.getUint16(2, littleEndian);
    if (magic !== 42)
        throw new Error('当前文件不是标准 TIFF');
    const ifds = [];
    ...
    while (ifdOffset > 0 && ifdOffset + 2 <= view.byteLength) {
        ...
        for (let index = 0; index < entryCount; index += 1) {
            ...
            if (tag === 256)
                ...
                parsed.bitsPerSample = values;
            else if (tag === 259)
                parsed.compression = first ?? 1;
            else if (tag === 262)
                parsed.photometric = first ?? 2;
            else if (tag === 273)
                parsed.stripOffsets = values;
            else if (tag === 277)
                parsed.samplesPerPixel = first ?? parsed.samplesPerPixel;
            else if (tag === 278)
                parsed.rowsPerStrip = first ?? parsed.rowsPerStrip;
            else if (tag === 279)
                parsed.stripByteCounts = values;
            else if (tag === 282)
                parsed.dpiX = first;
            else if (tag === 283)
                parsed.dpiY = first;
            else if (tag === 284)
                parsed.planarConfiguration = first ?? 1;
            else if (tag === 317)
                parsed.predictor = first ?? 1;
            else if (tag === 296)
                resolutionUnit = first ?? 2;
            else if (tag === 338)
                parsed.extraSamples = values;
        }
...
function createFixedLengthByteSink(expectedLength) {
...
function createRawByteReader(bytes) {
    let offset = 0;
    return {
        readByte() {
...
function createPackBitsByteReader(bytes) {
    let offset = 0;
...
function createTiffLzwByteReader(bytes) {
    ...
    const readCode = () => {
        while (bitsInBuffer < codeSize) {
            if (nextByteOffset >= bytes.length)
                return null;
...
export function decodeTiffLzw(bytes, expectedLength) {
...
function getTiffUnsupportedCompressionMessage(compression) {
    return `当前原型暂仅支持未压缩 / LZW / PackBits TIFF，当前压缩类型为 ${compression}`;
}
function getNormalizedBitsPerSample(primary) {
    if (primary.bitsPerSample.length > 0)
        return primary.bitsPerSample;
    return new Array(primary.samplesPerPixel).fill(8);
}
function getTiffBytesPerPixel(primary, bitsPerSample) {
    return Math.max(1, Math.round(bitsPerSample.reduce((sum, value) => sum + value, 0) / 8) || primary.samplesPerPixel || 1);
}
function validateTiffDecodeInput(primary, options = {}) {
...
function createTiffStripByteReader(source, compression) {
    if (compression === 1)
        return createRawByteReader(source);
    if (compression === 5)
        return createTiffLzwByteReader(source);
    if (compression === 32773)
        return createPackBitsByteReader(source);
    throw new Error(getTiffUnsupportedCompressionMessage(compression));
}
function reverseHorizontalPredictorInPlace(rowBytes, bytesPerPixel) {
...
export function reverseTiffHorizontalPredictor(bytes, width, rows, bytesPerPixel) {
...
function fillRowFromReader(reader, rowBuffer) {
    rowBuffer.fill(0);
...
function forEachDecodedTiffRow(buffer, primary, bytesPerPixel, onRow) {
    const bytes = new Uint8Array(buffer);
    ...
    for (let stripIndex = 0; stripIndex < primary.stripOffsets.length && currentRow < primary.height; stripIndex += 1) {
        ...
        if (!stripOffset || !stripLength || stripOffset + stripLength > bytes.length)
            continue;
...
function writeTiffPixelToRgba(source, sourceOffset, primary, target, targetOffset) {
    if (primary.photometric === 5) {
        const [red, green, blue] = cmykToRgb(source[sourceOffset] ?? 0, source[sourceOffset + 1] ?? 0, source[sourceOffset + 2] ?? 0, source[sourceOffset + 3] ?? 0);
...
function buildNearestSourceIndexMap(sourceLength, targetLength) {
...
function buildSourceRowTargets(sourceHeight, targetHeight) {
...
export function buildDownsamplePlan(width, height, maxDimension) {
...
export function downsampleRgba(rgba, sourceWidth, sourceHeight, targetWidth, targetHeight) {
...
export function decodeTiffToSampledRgba(buffer, options) {
    const { ifds } = parseTiffIfds(buffer);
...
export function decodeTiffToRgba(buffer, options = {}) {
    const { ifds } = parseTiffIfds(buffer);
    const validated = validateTiffDecodeInput(ifds[0], options);
    const { primary, bitsPerSample, bytesPerPixel } = validated;
...
export function buildParseSummary(ext, imageWidth, imageHeight, dpiX, dpiY, frameCount) {
...
export function getPatternSimilarityStatusText(phash, duplicateCount = 0) {
    if (!phash)
        return '视觉相似检测未完成';
    if (duplicateCount > 0)
        return `已命中 ${duplicateCount} 条疑似重复候选`;
...
export function buildPatternTagSuggestions(input) {
    const result = [];
    const normalizedTokens = input.tokens.map((token) => token.normalized);
    const pushSuggestion = (tag_name, tag_type, confidence, source = 'rule') => {
        if (!tag_name)
            return;
        if (result.some((item) => item.tag_name === tag_name && item.tag_type === tag_type))
            return;
        result.push({
...
export function getPatternSimilarityHit(input) {
    ...
    if (!Number.isFinite(distance) || distance > input.threshold)
        return null;
    return {
        assetId: input.assetId,
        versionId: input.versionId,
...
export function inferDuplicateStatus(hits) {
    if (hits.some((item) => item.duplicateType === 'sha256'))
        return 'merged';
    if (hits.length > 0)
        return 'suspected';
    return 'unique';
