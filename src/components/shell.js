...
import { getCurrentMenus, getCurrentSystem, getCurrentTabs, } from '../state/store';
import { shellIcons } from '../icons/shell-icons';
import { systems } from '../data/app-shell-config';
import { escapeHtml, toClassName } from '../utils';
function toKebabCaseIcon(name) {
    return name
...
function renderTopBar(state) {
    ...
    <header class="sticky top-0 z-40 flex h-14 items-center border-b bg-background px-4">
      ...
      <div class="mx-4 flex-1 overflow-hidden">
        ...
        return `
                <button
                  class="relative flex items-center gap-1.5 rounded-md px-3 py-2 text-sm font-medium transition-colors ${active
            ? 'text-blue-600'
...
function renderMenuItem(item, state, collapsed) {
    ...
    const baseButtonClass = toClassName('flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors', 'hover:bg-accent hover:text-accent-foreground', (isActive || childActive) && 'bg-blue-50 text-blue-600', collapsed && 'justify-center px-2');
    return `
    <div>
      <button
      ...
      ${hasChildren && expanded && !collapsed
        ? `
            <div class="ml-4 mt-1 space-y-1 border-l pl-3">
              ${item.children
            .map((child) => {
            const childClass = toClassName('flex w-full items-center gap-2 rounded-md px-3 py-1.5 text-sm transition-colors', 'hover:bg-accent hover:text-accent-foreground', child.href === state.pathname && 'bg-blue-50 text-blue-600');
            return `
                    <button
                      class="${childClass}"
...
function renderMenuGroup(group, index, state, collapsed) {
    ...
    <div>
      <button
        class="${toClassName('flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm font-semibold transition-colors', 'text-muted-foreground hover:bg-accent/50 hover:text-foreground', hasActive && 'text-primary')}"
...
function renderSidebarContent(state, collapsed, showCollapseButton) {
    const currentSystem = getCurrentSystem(state.pathname);
    const currentMenus = getCurrentMenus(state.pathname);
    return `
    <div class="flex h-full min-h-0 flex-col">
      <div class="${toClassName('flex h-14 shrink-0 items-center border-b', collapsed ? 'justify-center px-2' : 'justify-between px-4')}">
        ${collapsed
        ? ''
        : `
...
export function renderSidebar(state) {
    const desktop = `
    <aside class="${toClassName('hidden min-h-0 flex-col border-r bg-background transition-all duration-300 lg:flex', state.sidebarCollapsed ? 'w-16' : 'w-60')}">
...
function renderTabsBar(state) {
    ...
    <div class="border-b bg-muted/30">
      <div class="flex items-center">
        ...
        return `
                <div
                  class="group relative inline-flex items-center gap-2 border-r px-4 py-2 text-sm transition-colors ${active
...
export function renderAppShell(state, pageContent) {
    if (state.pathname.startsWith('/fcs/print/')
...
export function hydrateIcons(root = document) {
    const iconMap = { ...lucideIcons, ...shellIcons };
    root.querySelectorAll('[data-lucide]').forEach((element) => {
        const iconName = element.getAttribute('data-lucide');
        if (!iconName || !element.parentNode)
            return;
