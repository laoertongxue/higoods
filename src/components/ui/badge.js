 ...
 * 渲染徽章组件
 */
export function renderBadge(text, variant = 'neutral', icon) {
 ...
 * 渲染状态徽章（根据状态映射表）
 */
export function renderStatusBadge(status, statusMap) {
 ...
 * 渲染带计数的徽章
 */
export function renderCountBadge(count, variant = 'neutral') {
    const variantClasses = VARIANT_CLASSES[variant];
    return `
 ...
 * 渲染标签组（多个标签）
 */
export function renderBadgeGroup(badges, maxShow = 3) {
 ...
 * 渲染带图标的状态指示器（小圆点）
 */
export function renderStatusDot(status = 'neutral') {
    const colorMap = {
