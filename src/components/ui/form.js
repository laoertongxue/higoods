...
// ============ 基础样式 ============
const INPUT_BASE_CLASSES = 'flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ' +
    'ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium ' +
    'placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 ' +
    'focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50';
const SELECT_BASE_CLASSES = 'flex h-10 w-full items-center justify-between rounded-md border border-input ' +
    'bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground ' +
    'focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50';
const TEXTAREA_BASE_CLASSES = 'flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ' +
    'ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none ' +
    'focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ' +
    'disabled:cursor-not-allowed disabled:opacity-50';
const CHECKBOX_BASE_CLASSES = 'h-4 w-4 shrink-0 rounded border border-primary ring-offset-background ' +
...
export function renderInput(config) {
    const { id, name, value = '', placeholder = '', type = 'text', required = false, disabled = false, readonly = false, prefix, field, className = '', icon, } = config;
    const attrs = [];
    if (id)
        attrs.push(`id="${id}"`);
    if (name)
    ...
    if (readonly)
        attrs.push('readonly');
    if (prefix && field)
        attrs.push(toFieldAttr(prefix, field));
 ...
 * 渲染带标签的输入框
 */
export function renderLabeledInput(label, config, required = false) {
 ...
 * 渲染选择框
 */
export function renderSelect(config) {
    const { id, name, value = '', options, placeholder = '请选择', required = false, disabled = false, prefix, field, filter, className = '', } = config;
    const attrs = [];
    if (id)
        attrs.push(`id="${id}"`);
    if (name)
    ...
    if (prefix && field)
        attrs.push(toFieldAttr(prefix, field));
    if (prefix && filter)
        attrs.push(toFilterAttr(prefix, filter));
 ...
 * 渲染带标签的选择框
 */
export function renderLabeledSelect(label, config, required = false) {
 ...
 * 渲染文本域
 */
export function renderTextarea(config) {
    const { id, name, value = '', placeholder = '', rows = 4, required = false, disabled = false, prefix, field, className = '', } = config;
    const attrs = [];
    if (id)
        attrs.push(`id="${id}"`);
    if (name)
 ...
 * 渲染带标签的文本域
 */
export function renderLabeledTextarea(label, config, required = false) {
...
export function renderCheckbox(config) {
    const { id, name, checked = false, label, disabled = false, prefix, field, } = config;
    const checkboxId = id || `checkbox-${Math.random().toString(36).slice(2, 8)}`;
    const attrs = [`id="${checkboxId}"`];
    if (name)
        attrs.push(`name="${name}"`);
    if (checked)
        attrs.push('checked');
 ...
 */
export function renderFormField(config, content) {
 ...
 * 渲染表单组（带标题的一组字段）
 */
export function renderFormGroup(title, fields, description) {
    return `
    <div class="space-y-4">
      <div>
 ...
 * 渲染两列表单布局
 */
export function renderFormRow(leftField, rightField) {
    return `
 ...
 * 渲染表单分隔线
 */
export function renderFormDivider() {
    return '<div class="border-t my-4"></div>';
}
/**
 * 快速生成表单
 */
export function renderQuickForm(fields) {
    return fields
        .map(field => {
        switch (field.type) {
            case 'input':
                return renderLabeledInput(field.label, field.config, field.required);
            case 'select':
                return renderLabeledSelect(field.label, field.config, field.required);
            case 'textarea':
                return renderLabeledTextarea(field.label, field.config, field.required);
            case 'checkbox':
                return renderCheckbox(field.config);
            default:
                return '';
        }
    })
