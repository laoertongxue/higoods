...
function renderCell(
  ...
  const action = mode === 'SELECT' ? 'toggle-location' : occupied ? 'open-occupancy' : ''
  ...
  return `
    <div class="shrink-0">
      <button
        type="button"
        class="relative min-h-11 min-w-11 rounded-md border px-2 py-1.5 text-left text-xs ${statusClass} ${mode === 'SELECT' && occupied ? 'cursor-not-allowed opacity-70' : ''}"
...
function renderOccupancyDrawer(projection: WarehouseLocationMapProjection): string {
      ...
      <div class="rounded-lg border bg-card p-4">
        ...
        ${mode === 'SELECT' ? `
          <div class="mt-3 rounded-md bg-blue-50 px-3 py-2 text-sm text-blue-800">
            请选择同一货架内连续相邻的空闲库位；已选 ${selectedIds.size} 个。
          </div>
        ` : ''}
      ...
      ${projection.areas.map((area) => `
        <article class="rounded-lg border bg-card">
          <header class="flex items-center justify-between border-b px-4 py-3">
            <h3 class="font-semibold">库位组：${escapeHtml(area.areaName)}</h3>
            ${mode === 'LAYOUT' ? `
              <div class="flex gap-1">
                <button type="button" class="min-h-11 rounded-md border px-3 text-sm" data-skip-page-rerender="true" data-warehouse-map-action="move-area-left" data-area-id="${escapeHtml(area.areaId)}">左移库位组</button>
                <button type="button" class="min-h-11 rounded-md border px-3 text-sm" data-skip-page-rerender="true" data-warehouse-map-action="move-area-right" data-area-id="${escapeHtml(area.areaId)}">右移库位组</button>
              </div>
            ` : ''}
          </header>
          <div class="divide-y">
              ...
              <div class="grid gap-3 p-4 md:grid-cols-[9rem_minmax(0,1fr)]">
                ...
                <div class="overflow-x-auto pb-1">
                  <div class="flex min-w-max gap-2">
                    ${shelf.locations.map((cell) => renderCell(cell, mode, selectedIds)).join('')}
                  </div>
                </div>
              </div>
            `).join('')}
      ...
      ${projection.unlocatedOccupancies.length ? `
        <article class="rounded-lg border border-amber-200 bg-amber-50 p-4">
          ...
          <div class="mt-3 space-y-2">
            ${projection.unlocatedOccupancies.slice(0, 10).map((item) => `
          ...
          <div class="mt-3 text-xs text-amber-800">第 1 页 / 每页 10 条 / 共 ${projection.unlocatedOccupancies.length} 条</div>
        </article>
      ` : ''}
      ${renderOccupancyDrawer(projection)}
    </section>
  `
}
