...
import { renderButton, renderSecondaryButton } from './button.ts';
 ...
 * 渲染抽屉头部
 */
function renderDrawerHeader(config) {
 ...
 * 渲染抽屉底部
 */
export function renderDrawerFooter(config) {
    const buttons = [];
    if (config.extra) {
 ...
 */
export function renderDrawer(config, content, footer) {
 ...
 * 渲染带表单的抽屉（常用快捷方式）
 */
export function renderFormDrawer(config, formContent) {
 ...
 * 渲染详情抽屉（只有关闭按钮）
 */
export function renderDetailDrawer(config, content, extraButtons) {
...
export function renderSimpleDrawer(options) {
    return renderDrawer({
        ...
        width: options.width,
    }, options.content, options.footer);
}
