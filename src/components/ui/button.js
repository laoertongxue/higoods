...
export function renderButton(config) {
    const { label, icon, variant = 'secondary', size = 'md', action, disabled = false, className = '', type = 'button', } = config;
 ...
 * 渲染主要按钮（蓝色）
 */
export function renderPrimaryButton(label, action, icon) {
    return renderButton({ label, icon, variant: 'primary', action });
}
/**
 * 渲染次要按钮（边框）
 */
export function renderSecondaryButton(label, action, icon) {
    return renderButton({ label, icon, variant: 'secondary', action });
}
/**
 * 渲染危险按钮（红色）
 */
export function renderDangerButton(label, action, icon) {
    return renderButton({ label, icon, variant: 'danger', action });
}
/**
 * 渲染图标按钮
 */
export function renderIconButton(icon, action, title) {
    const actionAttr = action ? toActionAttr(action) : '';
 ...
 * 渲染按钮组
 */
export function renderButtonGroup(buttons, className = '') {
