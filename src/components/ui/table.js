...
export function renderTable(columns, data, options = {}) {
    const { emptyText = '暂无数据', striped = false, hoverable = true, compact = false, className = '', } = options;
 ...
 * 渲染表格头
 */
export function renderTableHeader(columns, cellPadding = 'p-4') {
 ...
 * 渲染表格行
 */
export function renderTableRow(columns, row, index, options = {}, cellPadding = 'p-4', striped = false, hoverable = true) {
    const { rowAction } = options;
    let rowAttrs = '';
    if (rowAction) {
        const dataValue = row[rowAction.dataKey];
 ...
 * 渲染表格操作列
 */
export function renderTableActions(buttons, rowIdAttr) {
 ...
 * 渲染表格操作列（紧凑版，只有图标）
 */
export function renderTableIconActions(buttons, rowIdAttr) {
 ...
 * 渲染表格选择框头部
 */
export function renderTableSelectHeader(prefix) {
 ...
 * 渲染表格选择框单元格
 */
export function renderTableSelectCell(prefix, id, checked = false) {
    const dataPrefix = toDataPrefix(prefix);
    return `
 ...
 * 渲染展开行
 */
export function renderExpandableRow(columnsCount, content, expanded = false) {
 ...
 * 渲染展开按钮
 */
export function renderExpandButton(prefix, id, expanded = false) {
 ...
 * 渲染简单表格（无复杂功能）
 */
export function renderSimpleTable(headers, rows, options = {}) {
