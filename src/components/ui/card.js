 ...
 * 渲染 KPI 卡片组
 */
export function renderKpiCardGroup(cards, columns = 4) {
 ...
 * 渲染信息展示卡片
 */
export function renderInfoCard(config) {
 ...
 * 渲染详情信息卡片（两列布局）
 */
export function renderDetailCard(title, details, columns = 2) {
 ...
 * 渲染加载状态
 */
export function renderLoadingCard(message = '加载中...') {
    return `
    <div class="${CARD_BASE_CLASSES}">
