...
// ============ 基础样式 ============
const PAGE_BUTTON_BASE = 'inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ' +
 ...
 * 渲染简单分页（只有上一页/下一页）
 */
function renderSimplePagination(current, totalPages, total, prefix, showTotal) {
 ...
 * 渲染完整分页（带页码）
 */
function renderFullPagination(current, totalPages, total, prefix, showTotal) {
 ...
 * 渲染页码跳转输入框
 */
export function renderPageJumper(prefix, totalPages) {
 ...
 * 渲染每页条数选择
 */
export function renderPageSizeSelect(prefix, current, options = [10, 20, 50, 100]) {
    const dataPrefix = toDataPrefix(prefix);
...
export function renderPaginationBar(config) {
    const { current, total, pageSize, prefix, showPageSize = true, pageSizeOptions = [10, 20, 50, 100], } = config;
    const totalPages = Math.ceil(total / pageSize);
...
export function renderTablePagination(config) {
    const { total, from, to, currentPage, totalPages, pageSize, actionPrefix, fieldPrefix = actionPrefix, pageSizeOptions = [10, 20, 50], } = config;
