      ...
      [
        {
          ...
          actionDetail: `工厂对接收记录 ${record.recordId} 发起数量差异`,
      ...
      [
        {
          id: `EAL-${caseId}-001`,
          action: 'CREATE',
          detail: `PDA 待接收记录 ${record.recordId} 发起数量差异`,
          at: record.warehouseHandedAt || record.submittedAt,
          by: head.targetName,
        },
      ],
...
export function createPdaPickupDisputeCase(
  ...
  if (!record) {
    return { record: null, exceptionCase: null, issues: ['未找到接收记录'] }
  }
  ...
  if (!head) {
    return { record: null, exceptionCase: null, issues: ['未找到对应接收单'] }
  }
...
export function updatePdaPickupDisputePlatformHandling(
  ...
  if (!fact) {
    return { record: null, exceptionCase: null, issues: ['未找到对应接收数量差异'] }
  }
