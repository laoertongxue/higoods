...
export function computeClaimDisputeQty(actualClaimQty, defaultClaimQty) {
    return Number((actualClaimQty - defaultClaimQty).toFixed(2));
}
export function canInitiateClaimDispute(actualClaimQty, defaultClaimQty) {
    return Number.isFinite(actualClaimQty) && Number.isFinite(defaultClaimQty) && actualClaimQty !== defaultClaimQty;
}
export function hasRequiredClaimDisputeEvidence(imageFiles = [], videoFiles = []) {
    return imageFiles.length + videoFiles.length > 0;
}
export function getClaimDisputeStatusMeta(status) {
    return statusMeta[status];
}
export function getClaimDisputeStatusLabel(status) {
    return getClaimDisputeStatusMeta(status).label;
}
export function buildPdaClaimDisputeSummary(record) {
    return `${getClaimDisputeStatusLabel(record.status)} · 差异 ${formatClaimQty(record.discrepancyQty)} · 证据 ${record.evidenceCount} 个`;
}
export function buildCraftClaimDisputeSummary(record) {
...
export function validateClaimDisputeInput(input) {
    const issues = [];
    if (!input.sourceTaskId.trim())
        issues.push('缺少来源任务');
    if (!input.cutOrderNo.trim())
        issues.push('缺少裁片单号');
    if (!input.productionOrderNo.trim())
        issues.push('缺少生产单号');
    if (!Number.isFinite(input.actualClaimQty))
        issues.push('请填写实际领取长度');
    const defaultQty = input.defaultClaimQty ?? input.configuredQty;
    if (!Number.isFinite(defaultQty))
        issues.push('缺少默认应领长度');
    if (!input.disputeReason.trim())
        issues.push('请填写异议原因');
    if (!hasRequiredClaimDisputeEvidence(input.imageFiles, input.videoFiles))
        issues.push('请至少上传一张图片或一个视频');
    if (!canInitiateClaimDispute(input.actualClaimQty, defaultQty))
        issues.push('实际领取长度与默认应领长度一致，无需发起异议');
    return issues;
}
export function validateClaimDisputeHandleInput(input) {
    const issues = [];
    if (!input.handleNote.trim())
        issues.push('请填写处理说明');
    if (!input.handleConclusion.trim())
        issues.push('请填写处理结论');
    return issues;
}
export function buildClaimDisputeRecord(input, disputeId, disputeNo, platformCaseId) {
...
export function applyClaimDisputeHandleResult(record, input) {
    return {
        ...record,
        status: input.status,
...
export function formatClaimQty(value) {
    return `${Number.isFinite(value) ? value : 0} 米`;
}
export function getClaimDisputeEvidenceHint() {
