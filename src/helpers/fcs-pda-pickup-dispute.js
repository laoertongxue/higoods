import { findPdaPickupHead, findPdaPickupRecord, getPdaPickupRecordsByHead, listPdaHandoverHeads, processPdaPickupQtyObjection, reportPdaPickupQtyObjection, resolvePdaPickupQtyObjection, } from '../data/fcs/pda-handover-events.ts';
import { appendCaseAction, appendCaseAuditLog, appendCaseStatusChangeAudit, markCaseResolved, } from '../data/fcs/progress-exception-lifecycle.ts';
...
function buildPickupDisputeSummary(head, record) {
    return `${head.processName}领料记录 ${record.recordId} 存在数量差异`;
}
function buildPickupDisputeDetail(head, record) {
    return [
        `领料单：${head.handoverId}`,
...
function buildSeedCase(head, record, caseId, existing) {
    const createdAt = existing?.createdAt || record.warehouseHandedAt || record.submittedAt;
    return {
        caseId,
        caseStatus: toCaseStatus(record),
        severity: Math.abs((record.warehouseHandedQty ?? record.qtyExpected) - (record.factoryReportedQty ?? record.finalResolvedQty ?? record.qtyExpected)) >= 5
            ? 'S2'
            : 'S3',
        category: 'MATERIAL',
...
export function ensurePdaPickupDisputeSeedCases() {
    ...
    heads.forEach((head) => {
        ...
        records.forEach((record) => {
            ...
            upsertProgressExceptionCase(buildSeedCase(head, record, record.exceptionCaseId, getProgressExceptionById(record.exceptionCaseId)));
        });
    });
}
export function getPdaPickupDisputeByCaseId(caseId) {
...
export function createPdaPickupDisputeCase(recordId, payload) {
...
export function updatePdaPickupDisputePlatformHandling(caseId, payload) {
