...
export function updatePatternTask(patternTaskId, patch) {
...
export function listPatternTaskPendingItems() {
    return loadSnapshot().pendingItems.map(clonePendingItem);
}
export function upsertPatternTaskPendingItem(item) {
    const snapshot = loadSnapshot();
    persistSnapshot({
        ...snapshot,
...
export function replacePatternTaskStore(tasks, pendingItems = []) {
    persistSnapshot({
        version: STORE_VERSION,
        tasks,
        pendingItems,
...
export function resetPatternTaskRepository() {
