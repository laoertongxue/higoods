...
export function getPatternTaskTeamName(teamCode) {
    return PATTERN_TASK_TEAMS.find((item) => item.teamCode === teamCode)?.teamName || '';
}
export function listPatternTaskMembersByTeam(teamCode) {
    return PATTERN_TASK_MEMBERS.filter((item) => item.teamCode === teamCode);
}
export function getPatternTaskMember(teamCode, memberId) {
    return listPatternTaskMembersByTeam(teamCode).find((item) => item.memberId === memberId) || null;
}
export function assertPatternTaskMemberInTeam(teamCode, memberId) {
    if (!teamCode)
        throw new Error('请先选择团队。');
    if (!memberId)
        throw new Error('请先选择花型师。');
