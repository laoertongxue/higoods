...
function pickWorkspaceValues(workspaceValues, preferredValues, fallbackValues, count) {
...
function getWorkspaceFallbackColors() {
    return pickWorkspaceValues(listProjectWorkspaceColors().map((item) => item.name), ['Black', 'White'], ['Black', 'White'], 2);
}
function getWorkspaceFallbackSizes() {
    return pickWorkspaceValues(listProjectWorkspaceSizes().map((item) => item.name), ['S', 'M'], ['S', 'M'], 2);
}
function createFallbackSkuLines(styleCode) {
...
function resolveColorCode(colorName) {
    ...
    if (normalized.includes('黄'))
        return 'YLW';
    if (normalized.includes('灰'))
        return 'GRY';
    if (normalized.includes('粉'))
        return 'PNK';
...
function resolvePrintName(styleName, index, explicit = '') {
    if (explicit.trim())
...
function resolveWeightText(styleName) {
    if (styleName.includes('外套') || styleName.includes('夹克'))
        return '0.85kg';
    if (styleName.includes('裙'))
        return '0.48kg';
    if (styleName.includes('裤'))
        return '0.62kg';
    return '0.36kg';
}
function resolveVolumeText(styleName) {
    if (styleName.includes('外套') || styleName.includes('夹克'))
        return '38*28*8cm';
    if (styleName.includes('裙'))
        return '34*24*5cm';
...
function normalizeRecord(record) {
    ...
    const archiveStatus = record.archiveStatus === 'INACTIVE' || record.archiveStatus === 'ARCHIVED' ? record.archiveStatus : 'ACTIVE';
    const mappingHealth = record.mappingHealth === 'MISSING' || record.mappingHealth === 'CONFLICT' ? record.mappingHealth : 'OK';
    return {
        ...cloneRecord(record),
        archiveStatus,
        mappingHealth,
    ...
    const mappingHealth = style.archiveStatus === 'ARCHIVED'
        ...
        : globalIndex % 7 === 0
            ...
            : globalIndex % 5 === 0 || !style.channelProductCount
                ? 'MISSING'
                : 'OK';
    const archiveStatus = style.archiveStatus === 'ARCHIVED' ? 'ARCHIVED' : globalIndex % 6 === 0 ? 'INACTIVE' : 'ACTIVE';
    const channelMappingCount = archiveStatus === 'ARCHIVED' ? 0 : Math.max(1, style.channelProductCount || 1);
    const listedChannelCount = archiveStatus === 'ACTIVE' && mappingHealth === 'OK'
        ? channelMappingCount
...
function buildSeedRecords(styles) {
    ...
    return styles.flatMap((style, styleIndex) => {
        ...
        const skuLines = seed?.demand.skuLines.map((item) => ({
...
function syncStyleArchiveSpecificationCount(styleId) {
...
export function createSkuArchiveBatch(records) {
...
export function updateSkuArchive(skuId, patch) {
    const snapshot = loadSnapshot();
...
export function replaceSkuArchiveStore(snapshot) {
    persistSnapshot(snapshot);
}
export function resetSkuArchiveRepository() {
