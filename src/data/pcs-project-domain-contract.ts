...
const sampleAcquireFields = [
  ...groupFields({
    ...
    fields: [
      ...
      { key: 'sampleLink', label: '采购地址 / 外采链接', type: 'url', sourceKind: '本地主数据', sourceRef: '样衣来源表单', meaning: '采购样衣地址或外采链接', logic: '国内采购样衣在项目录入时填写来源链接，不依赖旧系统转运批次导入。', required: false, conditionalRequired: 'sampleSourceType=外采' },
...
const patternTaskFields = [
  ...
  ...groupFields({
    ...
    title: '技术包关联',
    description: '正式技术包只由生产准备单生成；制版任务只记录已被采用的成果关联。',
    fields: [
      { key: 'linkedTechPackVersionId', label: '关联技术包版本', type: 'text', sourceKind: '制版任务', sourceRef: '制版任务正式对象.linkedTechPackVersionId', meaning: '采用本任务成果的生产准备单技术包版本', logic: '生产准备单生成技术包后回写，只读展示。', readonly: true, required: false },
  ...
  ...groupFields({
    ...
    fields: [
      ...
      { key: 'linkedTechPackVersionId', label: '关联技术包版本ID', type: 'text', sourceKind: '制版任务', sourceRef: '制版任务正式对象.linkedTechPackVersionId', meaning: '采用本任务成果的生产准备单技术包版本 ID', logic: '生产准备单生成技术包后回填，只读展示。', readonly: true },
      { key: 'linkedTechPackVersionCode', label: '关联技术包版本编码', type: 'text', sourceKind: '制版任务', sourceRef: '制版任务正式对象.linkedTechPackVersionCode', meaning: '采用本任务成果的生产准备单技术包版本编码', logic: '生产准备单生成技术包后回填，只读展示。', readonly: true },
      { key: 'linkedTechPackVersionLabel', label: '关联技术包版本标签', type: 'text', sourceKind: '制版任务', sourceRef: '制版任务正式对象.linkedTechPackVersionLabel', meaning: '采用本任务成果的生产准备单技术包版本标签', logic: '生产准备单生成技术包后回填，只读展示。', readonly: true },
      { key: 'linkedTechPackVersionStatus', label: '关联技术包版本状态', type: 'text', sourceKind: '制版任务', sourceRef: '制版任务正式对象.linkedTechPackVersionStatus', meaning: '采用本任务成果的生产准备单技术包版本状态', logic: '生产准备单生成技术包后回填，只读展示。', readonly: true },
...
const artworkTaskFields = [
  ...groupFields({
    ...
    fields: [
      { key: 'demandSourceType', label: '需求来源', type: 'single-select', sourceKind: '本地主数据', sourceRef: '花型任务表单', meaning: '花型需求来源', logic: '固定为设计改款任务或设计师款。' },
...
export const PCS_PROJECT_STEP_DEFINITIONS: PcsProjectStepDefinition[] = [
  ...
  },
  {
    ...
    instanceStatusDefinitions: [
      ...
      { statusName: '已上架待测款', entryConditions: ['上游渠道已有商品'], exitConditions: ['结论为通过后生效，或结论不通过作废'], businessMeaning: '上游渠道已有商品，可被直播和短视频测款引用。' },
    ...
    operationDefinitions: [
      {
        ...
        effects: ['记录判断', '通过时进入后续开发', '不通过时下架或作废当前渠道店铺商品并转入样衣处理', '暂保留时保持当前事实并等待稍后再判断', '回写渠道店铺商品与后续动作字段'],
        writebackRules: ['通过：回写 nextActionType=进入后续开发', '不通过：作废或下架当前渠道店铺商品，取消中间未完成节点，并回写 invalidatedChannelProductId、nextActionType=样衣退回处理', '暂保留：保留渠道店铺商品和已完成测款事实，不创建或重启测款节点，回写 nextActionType=稍后再判断'],
      },
    ],
    statusDefinitions: [
...
export const PCS_PROJECT_RELATED_INSTANCE_TYPES: PcsProjectRelatedInstanceTypeDefinition[] = [
  ...
  { typeCode: 'PATTERN_TASK', typeName: '制版任务', moduleName: '制版任务', businessMeaning: '判断通过后的制版推进任务。' },
