...
const sampleAcquireFields = [
  ...groupFields({
    ...
    fields: [
      ...
      { key: 'samplePurchaseSpecQty', label: '采购规格及数量', type: 'table', sourceKind: '本地主数据', sourceRef: '样衣来源表单.采购规格及数量', meaning: '记录外采页面或供应商侧的颜色、尺码、规格和采购件数', logic: '当前阶段还没有规格档案或渠道商品编码，只记录样衣采购所需的规格和数量；正式编码应在后续规格档案、渠道商品或店铺商品环节形成。', required: false },
    ],
  }),
]
...
export const PCS_PROJECT_TEMPLATE_SCHEMAS: PcsProjectTemplateSchema[] = [
  {
    ...
    updatedAt: CONTRACT_TIMESTAMP,
    status: 'active',
    scenario: '业务从商品列表手动创建国内采购样衣测款项目，记录采购样衣来源，到样核对后先做初步可行性判断，再准备商品上架，通过直播测款和可选短视频测款形成结论。',
    description: '覆盖手动立项、国内采购样衣获取、样衣结果核对、初步可行性判断、商品上架准备、市场测款、测款结论、款式档案和样衣处理。',
    phaseSchemas: [
      { phaseCode: 'PHASE_01', whyExists: '先手动创建商品项目，并记录采购样衣的供应商、采购地址、运费、收件人、区域和采购规格及数量。', nodeCodes: ['PROJECT_INIT', 'SAMPLE_ACQUIRE'] },
      { phaseCode: 'PHASE_02', whyExists: '采购样衣到样后先完成实物核对和样衣编号生成，再做初步可行性判断；判断进入测款后才准备渠道店铺商品。', nodeCodes: ['SAMPLE_INBOUND_CHECK', 'FEASIBILITY_REVIEW', 'CHANNEL_PRODUCT_LISTING'] },
  ...
  },
  {
    ...
    updatedAt: CONTRACT_TIMESTAMP,
    status: 'active',
    scenario: '业务基于旧款或参考款手动创建万隆改版出样衣测款项目，先确认样衣由改版任务出样，再通过工程开发与打样管理的改版任务承接改版需求并产出样衣，样衣核对和初步可行性判断后进入测款。',
    description: '覆盖手动立项、样衣来源确认、改版任务、样衣结果核对、初步可行性判断、商品上架、市场测款、测款结论、款式档案和样衣处理。',
    phaseSchemas: [
      { phaseCode: 'PHASE_01', whyExists: '先手动创建商品项目，并确认该项目样衣来源为万隆改版任务出样。', nodeCodes: ['PROJECT_INIT', 'SAMPLE_ACQUIRE'] },
      { phaseCode: 'PHASE_02', whyExists: '改版出样衣需求由改版任务承接，工程开发与打样管理产出样衣后，先核对实物到样，再做初步可行性判断。', nodeCodes: ['REVISION_TASK', 'SAMPLE_INBOUND_CHECK', 'FEASIBILITY_REVIEW'] },
      { phaseCode: 'PHASE_03', whyExists: '初步可行性判断进入测款后，先准备渠道店铺商品，再执行直播测款和可选短视频测款，并形成统一结论。', nodeCodes: ['CHANNEL_PRODUCT_LISTING', 'LIVE_TEST', 'VIDEO_TEST', 'TEST_DATA_SUMMARY', 'TEST_CONCLUSION'] },
