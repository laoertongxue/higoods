...
function normalizeTask(task: PlateMakingTaskRecord): PlateMakingTaskRecord {
  return {
    ...
    sampleReviewStatus: task.sampleReviewStatus || (task.status === '已确认' || task.status === '已生成技术包' || task.status === '已完成' ? '样板已通过' : task.status === '待确认' ? '待样板确认' : '未提交'),
    sampleReviewSubmittedAt: task.sampleReviewSubmittedAt || (task.status === '待确认' ? task.updatedAt : ''),
    sampleReviewSubmittedBy: task.sampleReviewSubmittedBy || '',
    sampleReviewerName: task.sampleReviewerName || '',
    sampleReviewAt: task.sampleReviewAt || task.sampleConfirmedAt || (task.status === '已确认' || task.status === '已生成技术包' || task.status === '已完成' ? task.updatedAt : ''),
    sampleReviewNote: task.sampleReviewNote || '',
    reworkReason: task.reworkReason || '',
    patternOutputSubmittedAt: task.patternOutputSubmittedAt || '',
    patternOutputSubmittedBy: task.patternOutputSubmittedBy || '',
