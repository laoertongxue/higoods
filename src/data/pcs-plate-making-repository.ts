...
function normalizeTask(task: PlateMakingTaskRecord): PlateMakingTaskRecord {
  return {
    ...
    sampleReviewStatus: task.sampleReviewStatus || (task.status === '已确认' || task.status === '已完成' ? '样板已通过' : task.status === '待确认' ? '待样板确认' : '未提交'),
    ...
    sampleReviewAt: task.sampleReviewAt || task.sampleConfirmedAt || (task.status === '已确认' || task.status === '已完成' ? task.updatedAt : ''),
