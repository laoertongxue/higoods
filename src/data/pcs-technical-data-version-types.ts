...
export type TechnicalProcessObjectType =
  | 'BOM_MATERIAL'
  | 'FABRIC'
  | 'YARN'
  | 'ACCESSORY'
...
export interface TechnicalProcessEntry {
  ...
  triggerField?: 'printRequirement' | 'dyeRequirement' | 'waterSolubleRequirement' | 'embroideryRequirement'
...
export interface TechnicalDataVersionContent {
  ...
  /** V1=仅 step/lane；V2=显式 predecessorEntryIds。旧内容只允许一次迁移读取。 */
  processRouteSchemaVersion?: number
  processRouteMigrationMarker?: string
