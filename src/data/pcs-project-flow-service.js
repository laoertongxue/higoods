import { getProjectById, getProjectNodeRecordById, getProjectNodeSequenceBlocker, getProjectNodeRecordByWorkItemTypeCode, listProjectNodes, listProjectPhases, updateProjectNodeRecord, updateProjectPhaseRecord, updateProjectRecord, approveProjectInit, } from './pcs-project-repository.ts';
...
import { submitProjectTestingSummary, } from './pcs-channel-product-project-repository.ts';
...
import { completeDecisionNodeWithResult, isProjectDecisionWorkItemCode, } from './pcs-project-decision-flow-service.ts';
function nowText() {
    const now = new Date();
...
export function isClosedProjectNodeStatus(status) {
    return status === '已完成' || status === '已取消';
}
function getSequenceBlockedResult(projectId, projectNodeId, messagePrefix = '请先完成前序工作项') {
...
export function syncProjectLifecycle(projectId, operatorName = '当前用户', timestamp = nowText()) {
...
export function completeProjectNode(projectId, projectNodeId, input = {}) {
    const blocker = getProjectNodeSequenceBlocker(projectId, projectNodeId);
    if (blocker)
        return null;
    ...
    const defaultResultType = node.workItemTypeCode === 'FIRST_SAMPLE'
        ? '首版样衣打样已完成'
        : node.workItemTypeCode === 'FIRST_ORDER_SAMPLE'
            ? '首单样衣打样已完成'
            : '节点完成';
    const defaultResultText = node.workItemTypeCode === 'FIRST_SAMPLE'
        ? '首版样衣打样已完成，商品项目节点同步完成。'
        : node.workItemTypeCode === 'FIRST_ORDER_SAMPLE'
            ? '首单样衣打样已完成，商品项目节点同步完成。'
            : `${node.workItemTypeName}已完成。`;
    updateProjectNodeRecord(projectId, projectNodeId, {
        currentStatus: '已完成',
...
export function activateProjectNode(projectId, projectNodeId, input = {}) {
    const blocker = getProjectNodeSequenceBlocker(projectId, projectNodeId);
    if (blocker)
...
export function cancelProjectNode(projectId, projectNodeId, reason, operatorName = '当前用户', timestamp = nowText()) {
    const node = getProjectNodeRecordById(projectId, projectNodeId);
    if (!node || isClosedProjectNodeStatus(node.currentStatus))
        return node;
    updateProjectNodeRecord(projectId, projectNodeId, {
...
export function markProjectNodeCompletedAndUnlockNext(projectId, projectNodeId, input = {}) {
...
export function approveProjectInitAndSync(projectId, operatorName = '当前用户') {
...
export function terminateProject(projectId, reason, operatorName = '当前用户', timestamp = nowText()) {
    const project = getProjectById(projectId);
    if (!project) {
        return {
            ok: false,
    ...
    listProjectNodes(projectId).forEach((node) => {
        if (isClosedProjectNodeStatus(node.currentStatus))
            return;
...
export function archiveProject(projectId, operatorName = '当前用户', timestamp = nowText()) {
    const nodes = listProjectNodes(projectId);
...
export function saveProjectNodeFormalRecord(input) {
    const operatorName = input.operatorName ?? '当前用户';
    ...
    if (input.completeAfterSave && node.workItemTypeCode === 'TEST_DATA_SUMMARY') {
        const summaryResult = submitProjectTestingSummary(input.projectId, {
    ...
    const saveResult = saveProjectInlineNodeFieldEntry(input.projectId, input.projectNodeId, {
        ...input.payload,
        businessDate,
        values,
        detailSnapshot,
    ...
    if (isProjectDecisionWorkItemCode(node.workItemTypeCode)) {
        const decisionValue = node.workItemTypeCode === 'FEASIBILITY_REVIEW'
        ...
        const decisionNote = node.workItemTypeCode === 'FEASIBILITY_REVIEW'
            ? String(values.reviewRisk || '').trim()
            : node.workItemTypeCode === 'SAMPLE_CONFIRM'
                ? String(values.confirmNote || '').trim()
                : String(values.conclusionNote || '').trim();
        try {
            return completeDecisionNodeWithResult(input.projectId, input.projectNodeId, decisionValue, operatorName, decisionNote, businessDate);
        }
        catch (error) {
            return {
                ok: false,
    ...
    const summaryNote = String(values.summaryText ||
        values.pricingNote ||
        values.costNote ||
        values.fitFeedback ||
        values.reviewRisk ||
        ...
        values.returnResult ||
        values.retainNote ||
        `${node.workItemTypeName}已完成。`).trim();
