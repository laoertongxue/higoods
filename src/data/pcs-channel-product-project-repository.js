import { findProjectByCode, getChannelNamesByCodes, getProjectById, getProjectNodeRecordByWorkItemTypeCode, listProjects, listProjectPhases, updateProjectNodeRecord, updateProjectPhaseRecord, updateProjectRecord, } from './pcs-project-repository.ts';
import { listProjectRelationsByProject, upsertProjectRelation, } from './pcs-project-relation-repository.ts';
...
import { findPcsChannelStoreMasterRecord, getDefaultPcsStoreIdByChannel, resolvePcsStoreCurrency, resolvePcsStoreDisplayName, } from './pcs-channel-store-master.ts';
import { getTechnicalDataVersionById, updateTechnicalDataVersionRecord, } from './pcs-technical-data-version-repository.ts';
import { createProjectImageAssetRecords, getProjectImageAssetById, upsertProjectImageAssets, } from './pcs-project-image-repository.ts';
import { getLatestProjectInlineNodeRecord, upsertProjectInlineNodeRecord, } from './pcs-project-inline-node-record-repository.ts';
import { STYLE_ARCHIVE_STATUS_RULES, resolveStyleArchiveBusinessStatus, } from './pcs-product-lifecycle-governance.ts';
...
import { cloneChannelListingImages, deriveListingImageIds, deriveListingMainImageId, normalizeChannelListingImages, validateChannelListingImagesForUpload, } from './pcs-channel-listing-image-utils.ts';
import { buildChannelListingSellerSku, buildUploadedUpstreamSkuId, countUploadedChannelListingSpecLines, cloneChannelListingSpecLines, normalizeChannelListingSpecLines, validateChannelListingSpecLinesForCreate, validateChannelListingSpecLinesForUpload, } from './pcs-channel-listing-spec-utils.ts';
function getTechPackSourceNodeBinding(projectId, createdFromTaskType) {
...
function getChannelMeta(channelCode, storeId = '') {
...
function getDefaultStoreId(channelCode) {
    return getDefaultPcsStoreIdByChannel(channelCode) || 'store-tiktok-01';
}
function resolveListingPayload(projectId, payload) {
    ...
    const defaultPriceAmount = typeof payload.defaultPriceAmount === 'number' && Number.isFinite(payload.defaultPriceAmount)
    ...
    return {
        ...
        defaultPriceAmount,
        currencyCode: payload.currencyCode?.trim() ||
...
function validateListingStoreChannel(targetChannelCode, targetStoreId) {
...
function buildTestingStatusText(seed) {
    if (seed.scenario === 'MEASURING')
        return '已完成上架，正在测款';
    if (seed.scenario === 'FAILED_ADJUST')
        return '历史测款结论待重新确认，当前渠道店铺商品已作废';
    if (seed.scenario === 'FAILED_PAUSED')
        return '历史测款结论待重新确认，当前渠道店铺商品已作废';
    if (seed.scenario === 'FAILED_ELIMINATED')
        return '测款结论为淘汰，当前渠道店铺商品已作废';
    if (seed.scenario === 'STYLE_PENDING_TECH')
        return '测款通过，已生成款式档案，待启用技术包';
    if (seed.scenario === 'STYLE_ACTIVE')
        return '测款通过，已关联款式档案并完成上游最终更新';
    return '历史测款渠道店铺商品，已失效';
}
function buildChannelProductId(projectCode, sequence) {
    return `channel_product_${projectCode.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase()}_${sequence}`;
}
function buildChannelProductCode(projectCode, sequence) {
    return `CP-${projectCode.slice(-7).replace(/-/g, '')}-${sequence}`;
}
function buildUpstreamCode(projectCode, sequence) {
    return `UP-${projectCode.slice(-7).replace(/-/g, '')}-${sequence}`;
}
function buildLegacySpecSnapshot(record) {
    ...
    const sellerSku = record.skuCode ||
...
function ensureLegacyListingImageAssets(record) {
    ...
    const legacyAssets = createProjectImageAssetRecords({
        ...
        mainFlag: index === 0,
        sortNo: index + 1,
    })), '系统迁移', record.updatedAt || record.createdAt || nowText());
...
function buildListingImagesFromPayload(input) {
    const imageInputs = input.listingImages.length > 0
        ? input.listingImages
...
function resolveListingImageAssetSnapshot(imageId) {
...
function getDefaultSpecProductImageId(input) {
    if (input.listingMainImageId)
        return input.listingMainImageId;
    if (input.listingImageIds[0])
        return input.listingImageIds[0];
    if (input.mainImageUrls[0] || input.detailImageUrls[0])
        return '';
    return '';
}
function hasListingSpecProductImages(specLines) {
    return specLines.every((item) => Boolean(item.productImageId));
}
function isNodeCompletableChannelProduct(record) {
    if (record.channelProductStatus === '已作废')
        return false;
    return (record.listingBatchStatus === '已上传待确认' ||
        record.listingBatchStatus === '已完成' ||
        record.channelProductStatus === '已上架待测款' ||
        record.channelProductStatus === '已生效');
}
function normalizeChannelProductRecord(record) {
    ...
    const specLines = normalizeChannelListingSpecLines({
        ...
        if (line.upstreamSkuId || !legacyUploaded)
            return line;
        return {
            ...line,
            lineStatus: '已上传',
    ...
    const listingBatchStatus = record.listingBatchStatus ||
        (record.channelProductStatus === '已上架待测款'
            ? '已完成'
            : upstreamProductId
                ? '已上传待确认'
    ...
    const listingImages = record.listingImages?.length > 0
    ...
    const derivedSkuName = primarySpec && (primarySpec.colorName || primarySpec.sizeName || primarySpec.printName)
...
function buildSeedRecord(seed) {
    ...
    const upstreamProductId = seed.channelProductStatus === '待上传'
        ? ''
    ...
    return {
        ...
        listingBatchStatus: seed.channelProductStatus,
...
function loadSnapshot() {
    ...
    try {
        ...
        memorySnapshot = hydrateSnapshot({
            version: STORE_VERSION,
            records: Array.isArray(parsed.records) ? parsed.records : seedSnapshot().records,
        });
        localStorage.setItem(STORAGE_KEY, JSON.stringify(memorySnapshot));
        return cloneSnapshot(memorySnapshot);
    }
...
function buildStyleRelation(projectId, styleId, operatorName) {
    const project = getProjectById(projectId);
    const style = getStyleArchiveById(styleId);
    const node = getProjectNodeRecordByWorkItemTypeCode(projectId, 'STYLE_ARCHIVE_CREATE');
    if (!project || !style || !node)
        return null;
    return {
...
function buildTechnicalVersionRelation(projectId, technicalVersionId, operatorName) {
    const project = getProjectById(projectId);
...
function buildChannelProductRelation(record, operatorName) {
...
function buildUpstreamSyncRelation(record, operatorName) {
    if (!record.upstreamChannelProductCode || record.upstreamSyncStatus !== '已更新')
        return null;
...
function applyScenarioStyleLinks() {
    ...
    currentRecords.forEach((record) => {
        ...
        const techPackNode = project && technicalVersion
            ...
            : null;
        if (!project || !style || !technicalVersion)
            return;
        ...
        updateStyleArchive(style.styleId, {
            ...
            channelProductCount: listProjectChannelProductsByProjectId(project.projectId).filter((item) => item.styleId === style.styleId).length || 1,
        ...
        if (styleNode) {
            updateProjectNodeRecord(project.projectId, styleNode.projectNodeId, {
                currentStatus: '已完成',
        ...
        if (techPackNode?.projectNodeId) {
            updateProjectNodeRecord(project.projectId, techPackNode.projectNodeId, {
                currentStatus: '进行中',
            ...
            syncProjectNodeInstanceRuntime(project.projectId, techPackNode.projectNodeId, DEMO_OPERATOR, (activated ? technicalVersion.publishedAt : technicalVersion.updatedAt) || project.updatedAt);
        }
    });
}
...
export function ensureProjectChannelProductDemoStateReady() {
    ensureDemoState();
}
export function createProjectChannelProductRelationBootstrapSnapshot(operatorName = DEMO_OPERATOR) {
    ensureDemoState();
    ...
    records.forEach((record) => {
        ...
        if (project?.linkedTechPackVersionId) {
            const technicalRelation = buildTechnicalVersionRelation(record.projectId, project.linkedTechPackVersionId, operatorName);
            if (technicalRelation)
                relations.push(technicalRelation);
        }
        const upstreamRelation = buildUpstreamSyncRelation(record, operatorName);
        if (upstreamRelation)
            relations.push(upstreamRelation);
    });
    return { relations, records };
}
function listValidChannelProducts(records) {
    return [...records]
        .filter((item) => item.channelProductStatus !== '已作废')
        .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
}
function buildSummaryText(currentRecord, styleStatus, activeCount) {
    if (!currentRecord)
        return '当前项目尚未创建渠道店铺商品。';
...
function getCurrentChannelProduct(records) {
...
function listLaunchReadyChannelProducts(records) {
    return listValidChannelProducts(records).filter((item) => item.listingBatchStatus === '已完成' ||
        item.channelProductStatus === '已上架待测款' ||
        item.channelProductStatus === '已生效');
}
function listTestingConclusionTargetRecords(records) {
    const launchReadyRecords = listLaunchReadyChannelProducts(records);
    if (launchReadyRecords.length > 0)
        return launchReadyRecords;
    return [...records]
        .filter((item) => Boolean(item.upstreamProductId || item.upstreamChannelProductCode))
        .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
}
function findOpenChannelProductByChannelStore(projectId, targetChannelCode, targetStoreId) {
    return (listValidChannelProducts(listProjectChannelProductsByProjectId(projectId)).find((item) => item.channelCode === targetChannelCode &&
        item.storeId === targetStoreId) || null);
}
...
function parseSequenceFromChannelProductCode(channelProductCode) {
    const matched = channelProductCode.match(/-(\d{2})$/);
    return matched ? Number(matched[1]) : 0;
}
function nextChannelProductSequence(projectId) {
    const records = listProjectChannelProductsByProjectId(projectId);
    const maxSequence = records.reduce((currentMax, item) => Math.max(currentMax, parseSequenceFromChannelProductCode(item.channelProductCode)), 0);
    return String(maxSequence + 1).padStart(2, '0');
}
function updateProjectPhaseStatuses(projectId, currentPhaseCode) {
    const phases = listProjectPhases(projectId);
    phases.forEach((phase) => {
        const nextStatus = phase.phaseCode === currentPhaseCode
            ? '进行中'
            : phase.phaseOrder <
...
function updateProjectCurrentPhase(projectId, phaseCode, operatorName = DEMO_OPERATOR) {
...
function getFormalTestingRelations(projectId) {
...
function buildTestingLinkPatch(relations) {
...
function normalizeTestingSourceChannelCode(channelLabel) {
    ...
    if (normalized === '虾皮' || normalized === 'shopee')
        return 'shopee';
    if (normalized === '微信小程序' || normalized === '微信视频号' || normalized === 'wechat-mini-program') {
        return 'wechat-mini-program';
    }
    if (normalized === '来赞达' || normalized === 'lazada')
        return 'lazada';
    return '';
}
function parseTestingSourceChannelLabel(channelLabel) {
...
function toBreakdownMetricLine(label, item) {
...
function sortBreakdownItems(items) {
    return [...items].sort((left, right) => {
        if (right.gmvAmount !== left.gmvAmount)
            return right.gmvAmount - left.gmvAmount;
        if (right.orderQty !== left.orderQty)
            return right.orderQty - left.orderQty;
        if (right.clickQty !== left.clickQty)
            return right.clickQty - left.clickQty;
        if (right.exposureQty !== left.exposureQty)
            return right.exposureQty - left.exposureQty;
        return left.label.localeCompare(right.label, 'zh-CN');
    });
}
function buildBreakdownItems(facts, resolveKey, resolveLabel) {
...
function buildTestingSummaryFacts(projectId, relations, channelProducts) {
    const parseRelationMeta = (note) => {
        if (!note)
            return {};
        try {
    ...
    const facts = relations
        ...
        if (relation.sourceObjectType === '直播商品明细') {
            ...
            const sourceChannel = parseTestingSourceChannelLabel(String(liveSession?.channelName ||
                meta.channelName ||
        ...
        const sourceChannel = parseTestingSourceChannelLabel(String(videoRecord?.channelName ||
            meta.channelName ||
            meta.targetChannelCode ||
            meta.account ||
            ''));
        ...
        return {
            relationId: relation.projectRelationId,
            sourceType: '短视频',
            sourceCode: videoRecord?.videoRecordCode ||
                relation.sourceObjectCode ||
...
function buildProjectTestingSummaryAggregate(projectId, relations = getFormalTestingRelations(projectId), channelProducts = listProjectChannelProductsByProjectId(projectId)) {
    const { liveRelationIds, liveRelationCodes, videoRelationIds, videoRelationCodes, facts } = buildTestingSummaryFacts(projectId, relations, channelProducts);
    const channelBreakdowns = buildBreakdownItems(facts, (item) => item.channelCode || item.channelName, (item) => item.channelName);
    const storeBreakdowns = buildBreakdownItems(facts, (item) => `${item.channelCode}::${item.storeId || item.storeName}`, (item) => `${item.channelName} / ${item.storeName}`);
    const channelProductBreakdowns = buildBreakdownItems(facts, (item) => item.channelProductCode, (item) => `${item.channelProductCode}（${item.channelName} / ${item.storeName}）`);
...
export function getProjectTestingSummaryAggregate(projectId) {
    return buildProjectTestingSummaryAggregate(projectId);
}
function getProjectNodeOrMessage(projectId, workItemTypeCode, nodeName) {
...
function buildInlineRecordId(projectNodeId, sourceDocId) {
    return `${projectNodeId}::${sourceDocId}`.replace(/[^a-zA-Z0-9:_-]/g, '_');
}
function buildInlineRecordCode(projectCode, suffix) {
    return `INR-${projectCode.slice(-3)}-${suffix}`;
}
function buildTestingSummaryInlineRecord(project, projectNodeId, summaryText, relations, channelProducts, operatorName, businessDate) {
    const primaryChannelProduct = getCurrentChannelProduct(channelProducts) || channelProducts[0];
    if (!primaryChannelProduct)
        return;
...
function buildTestingConclusionInlineRecord(project, projectNodeId, payload, channelProducts, summaryRecord, branchDetail, downstreamRefs, operatorName, businessDate) {
    const primaryChannelProduct = getCurrentChannelProduct(channelProducts) || channelProducts[0];
    if (!primaryChannelProduct)
        return;
...
function ensureListingPrerequisites(projectId) {
    const prerequisites = [
...
function replaceRecord(nextRecord, operatorName = DEMO_OPERATOR) {
    ...
    if (upstreamRelation)
        upsertProjectRelation(upstreamRelation);
}
function updateStyleArchiveCreateNode(projectId, patch) {
    const node = getProjectNodeRecordByWorkItemTypeCode(projectId, 'STYLE_ARCHIVE_CREATE');
    if (!node)
        return;
...
function isClosedProjectNodeStatus(status) {
    return status === '已完成' || status === '已取消';
}
function activateProjectNodeByWorkItemTypeCode(projectId, workItemTypeCode, patch, operatorName) {
    const node = getProjectNodeRecordByWorkItemTypeCode(projectId, workItemTypeCode);
    if (!node || isClosedProjectNodeStatus(node.currentStatus))
        return;
    updateProjectNodeRecord(projectId, node.projectNodeId, {
        currentStatus: patch.currentStatus || '进行中',
        ...patch,
    }, operatorName);
}
function activateTestingConclusionDecisionNode(projectId, operatorName, timestamp) {
    activateProjectNodeByWorkItemTypeCode(projectId, 'TEST_CONCLUSION', {
        currentStatus: '待确认',
...
function invalidateChannelProductRecord(record, input, operatorName) {
...
export function listProjectChannelProducts() {
    ensureDemoState();
    return loadSnapshot().records.map(cloneRecord).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
}
export function listProjectChannelProductsSnapshot() {
    return loadSnapshot().records.map(cloneRecord).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
}
export function listProjectChannelProductsByProjectId(projectId) {
    return listProjectChannelProducts().filter((item) => item.projectId === projectId);
}
export function repairChannelListingNodeInstanceConsistency(operatorName = '系统修复') {
    listProjects().forEach((project) => {
        ...
        updateProjectNodeRecord(project.projectId, listingNode.projectNodeId, {
...
export function buildProjectChannelProductChainSummary(projectId) {
    ...
    return {
        ...
        currentUpstreamChannelProductCode: currentRecord?.upstreamProductId || currentRecord?.upstreamChannelProductCode || '',
        currentChannelProductStatus: currentRecord?.listingBatchStatus === '已完成'
            ? '已上架待测款'
        ...
        linkedTechPackVersionStatus: getTechPackVersionStatusText(linkedVersion?.versionStatus || project.linkedTechPackVersionStatus || ''),
        ...
        canGenerateStyleArchive: Boolean(currentRecord &&
...
export function createProjectChannelProductFromListingNode(projectId, payload = {}, operatorName = '当前用户') {
    const project = getProjectById(projectId);
    if (!project) {
    ...
    if (project.targetChannelCodes.length > 0 &&
    ...
    const duplicateRecord = findOpenChannelProductByChannelStore(projectId, resolvedPayload.targetChannelCode, resolvedPayload.targetStoreId);
    const channelStoreError = validateListingStoreChannel(resolvedPayload.targetChannelCode, resolvedPayload.targetStoreId);
    ...
    const migratedListingImages = resolvedPayload.listingImageIds.length === 0 &&
        resolvedPayload.listingImages.length === 0 &&
        [...resolvedPayload.mainImageUrls, ...resolvedPayload.detailImageUrls].some(Boolean)
        ? createProjectImageAssetRecords(project, [...new Set([...resolvedPayload.mainImageUrls, ...resolvedPayload.detailImageUrls].map((item) => item.trim()).filter(Boolean))]
            .map((imageUrl, index) => ({
            imageUrl,
            imageName: `上架图片 ${index + 1}`,
            imageType: '上架图',
    ...
    const listingImages = buildListingImagesFromPayload({
        listingBatchId: channelProductId,
        listingImageIds: resolvedPayload.listingImageIds.length > 0
            ? resolvedPayload.listingImageIds
            : migratedListingImages.map((item) => item.imageId),
        listingImages: resolvedPayload.listingImages.length > 0
    ...
    updateProjectNodeRecord(projectId, listingNode.projectNodeId, {
        currentStatus: '进行中',
...
export function launchProjectChannelProductListing(channelProductId, operatorName = '当前用户') {
    ...
    const timestamp = nowText();
    const upstreamProductId = buildUpstreamCode(record.projectCode, String(parseSequenceFromChannelProductCode(record.channelProductCode)).padStart(2, '0'));
    ...
    const listingNode = getProjectNodeRecordByWorkItemTypeCode(record.projectId, 'CHANNEL_PRODUCT_LISTING');
    if (listingNode) {
        updateProjectNodeRecord(record.projectId, listingNode.projectNodeId, {
            currentStatus: '进行中',
...
export function markProjectChannelProductListingCompleted(channelProductId, operatorName = '当前用户') {
    const record = getProjectChannelProductById(channelProductId);
    if (!record) {
...
export function completeProjectChannelListingNode(projectId, operatorName = '当前用户') {
    const project = getProjectById(projectId);
    if (!project) {
...
export function submitProjectTestingSummary(projectId, payload = {}, operatorName = '当前用户') {
    const project = getProjectById(projectId);
    if (!project) {
    ...
    updateProjectNodeRecord(projectId, summaryNode.projectNodeId, {
        currentStatus: '已完成',
...
export function generateProjectTestingSummaryFromRelations(projectId, operatorName = '当前用户') {
    return submitProjectTestingSummary(projectId, {}, operatorName);
}
export function submitProjectTestingConclusion(projectId, payload, operatorName = '当前用户') {
    ...
    if (payload.conclusion === '通过') {
        ...
        updateProjectNodeRecord(projectId, conclusionNode.projectNodeId, {
            latestInstanceId: latestRecord.channelProductId,
            latestInstanceCode: latestRecord.channelProductCode,
            latestResultType: '测款通过',
            latestResultText: note,
            pendingActionType: '生成款式档案',
            pendingActionText: nextRecords.length > 1
        ...
        buildTestingConclusionInlineRecord(project, conclusionNode.projectNodeId, payload, nextRecords, summaryRecord, {
        ...
        return {
            ok: true,
            message: nextRecords.length > 1
                ? `已提交测款通过结论，当前 ${nextRecords.length} 个渠道店铺商品实例可关联同一款式档案。`
                : '已提交测款通过结论，当前可生成款式档案。',
            record: latestRecord,
        };
    }
    const nextRecords = targetRecords.map((record) => invalidateChannelProductRecord({ ...record, ...testingLinkPatch }, {
        scenario: 'FAILED_ELIMINATED',
        conclusion: '淘汰',
        reason: note,
...
export function invalidateProjectChannelProduct(channelProductId, operatorName = '当前用户') {
    const record = getProjectChannelProductById(channelProductId);
    if (!record) {
    ...
    const nextRecord = invalidateChannelProductRecord(record, {
        scenario: 'HISTORY_INVALIDATED',
...
export function bindStyleArchiveToProjectChannelProduct(projectId, input, operatorName = '当前用户') {
...
export function markProjectChannelProductConclusion(projectId, conclusion, operatorName = '当前用户') {
    const targetRecords = listValidChannelProducts(listProjectChannelProductsByProjectId(projectId));
    if (targetRecords.length === 0)
        return null;
    const timestamp = nowText();
...
export function syncProjectChannelProductAfterTechPackActivation(styleId, technicalVersion, operatorName = '当前用户') {
...
export function resetProjectChannelProductRepository() {
