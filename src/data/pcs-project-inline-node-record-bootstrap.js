...
function getTestingConclusionNextActionType(conclusion) {
    if (conclusion === '通过')
        return '生成款式档案';
    if (conclusion === '淘汰')
        return '样衣退回处理';
...
const PROJECT_SCENARIOS = {
    ...
    'PRJ-20251216-007': {
        ...
        sampleName: '基础毛织开衫试款样',
        sampleSourceType: '外采',
        sampleSupplierId: 'supplier-wool-a',
        ...
        evaluationParticipants: ['张丽', '毛织版师', '直播运营'],
        reviewRisk: '需观察纽扣牢度。',
        ...
        materialConfirmation: '毛织面料舒适度较高。',
        revisionRequired: false,
        actualSampleCost: 148,
        targetProductionCost: 124,
        costCompliance: '符合',
        priceRange: '169-219',
        pricingNote: '基础毛织款建议保留亲民价带。',
        pricingStrategy: '基础主销定价',
        finalPrice: 199,
        approvalStatus: '已通过',
    },
};
const TESTING_BRANCH_RECORDS = [
    {
...
function addDays(baseDateText, offsetDays, timeText) {
...
function getRecordCode(projectCode, workItemTypeCode) {
...
function buildSeedForNode(projectCode, workItemTypeCode) {
...
function buildProjectNodeLookup() {
    ...
    const nodeMap = new Map(snapshot.nodes.map((node) => [`${node.projectId}::${node.workItemTypeCode}`, node]));
...
function buildUpstreamChannelProductCode(projectCode, sequence) {
    return `UP-${projectCode.slice(-7).replace(/-/g, '')}-${sequence}`;
}
function buildRelationId(projectId, projectNodeId, sourceCode) {
...
function buildTestingBranchBootstrapRecords(projectMap, nodeMap) {
    const records = [];
...
function buildSampleCloseoutRecordCode(projectCode) {
    return `INR-${projectCode.slice(-3)}-RETURN`;
}
function buildSampleCloseoutBootstrapRecords(projectMap, nodeMap) {
    void projectMap;
    void nodeMap;
    return [];
}
function buildGenericInlineBusinessDate(project, node) {
    return node.updatedAt || node.lastEventTime || project.updatedAt || project.createdAt;
}
function buildFallbackSampleCode(projectCode) {
    return `SY-${projectCode.slice(-3)}-001`;
}
function buildGenericInlineSeed(project, node) {
...
function buildMissingCompletedInlineNodeRecords(projectIdMap, snapshot, existingRecords) {
    const existingNodeIds = new Set(existingRecords.map((record) => record.projectNodeId));
    const nextRecords = [];
    snapshot.nodes
        .filter((node) => node.currentStatus === '已完成' &&
        PCS_PROJECT_INLINE_NODE_RECORD_WORK_ITEM_TYPES.includes(node.workItemTypeCode))
        .forEach((node) => {
        if (existingNodeIds.has(node.projectNodeId))
            return;
        ...
        if (!seed)
            return;
        const recordCode = node.workItemTypeCode === 'SAMPLE_RETURN_HANDLE'
...
export function createBootstrapProjectInlineNodeRecordSnapshot(version) {
