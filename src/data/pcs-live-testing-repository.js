import { LIVE_PURPOSE_META, SESSION_STATUS_META, getLiveSessionItems, listLiveSessions, } from './pcs-testing.ts';
...
function parseSkuCodeSegments(skuCode) {
...
function buildSeedSnapshot() {
    ...
    const productLines = sessions.flatMap((session) => getLiveSessionItems(session.id).map((item, index) => {
...
function loadSnapshot() {
    ...
    try {
        ...
        memorySnapshot = hydrateSnapshot({
            version: LIVE_TESTING_STORE_VERSION,
            sessions: Array.isArray(parsed.sessions) ? parsed.sessions : buildSeedSnapshot().sessions,
...
export function getLiveTestingStoreSnapshot() {
    return loadSnapshot();
}
export function listLiveSessionRecords() {
    return loadSnapshot().sessions.map(cloneSession);
}
export function getLiveSessionRecordById(liveSessionId) {
    const record = loadSnapshot().sessions.find((item) => item.liveSessionId === liveSessionId);
    return record ? cloneSession(record) : null;
}
export function listLiveProductLines() {
    return loadSnapshot().productLines.map(cloneLine);
}
export function listLiveProductLinesBySession(liveSessionId) {
    return loadSnapshot()
        .productLines
