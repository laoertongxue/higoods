...
import { getProjectById, getProjectNodeRecordById, getProjectNodeSequenceBlocker, updateProjectNodeRecord, } from './pcs-project-repository.ts';
...
const ALLOWED_DETAIL_SNAPSHOT_KEYS = {
    SAMPLE_ACQUIRE: [
        'acquireMethod',
        'acquirePurpose',
        'applicant',
...
function isSupportedWorkItemTypeCode(value) {
    return PCS_PROJECT_INLINE_NODE_RECORD_WORK_ITEM_TYPES.includes(value);
}
...
function sanitizeObject(source, allowedKeys) {
...
function buildRecordId(projectCode, workItemTypeCode, sequence) {
...
function buildRecordCode(projectCode, workItemTypeCode, sequence) {
...
function normalizeFieldEntryValue(value) {
...
function hydrateSnapshot(snapshot) {
    const cleanedRecords = Array.isArray(snapshot?.records)
        ? migrateProjectDecisionInlineRecords(snapshot.records)
        : [];
    return {
        version: INLINE_NODE_RECORD_STORE_VERSION,
        records: cleanedRecords
            ...
            .filter((record) => {
            if (record.workItemTypeCode !== 'FEASIBILITY_REVIEW')
                return true;
...
function loadSnapshot() {
    ...
    try {
        ...
        memorySnapshot = mergeMissingSeedData(hydrateSnapshot(JSON.parse(raw)));
        localStorage.setItem(INLINE_NODE_RECORD_STORAGE_KEY, JSON.stringify(memorySnapshot));
        return cloneSnapshot(memorySnapshot);
    }
    catch {
...
export function saveProjectInlineNodeFieldEntry(projectId, projectNodeId, input, operatorName = '当前用户') {
    ...
    const workItemTypeCode = node.workItemTypeCode;
    ...
    const existingRecords = listProjectInlineNodeRecordsByNode(projectNodeId).filter((item) => item.workItemTypeCode === workItemTypeCode);
    ...
    const normalizedValues = Object.fromEntries(Object.entries(input.values || {}).map(([key, value]) => [key, normalizeFieldEntryValue(value)]));
...
export function replaceProjectInlineNodeRecordStore(records) {
    persistSnapshot({
        version: INLINE_NODE_RECORD_STORE_VERSION,
        records: records.map((record) => normalizeRecord(record)).sort(compareRecords),
    });
}
export function resetProjectInlineNodeRecordRepository() {
