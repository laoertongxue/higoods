...
import { getLiveProductLineById, listLiveProductLinesBySession, listLiveSessionRecords, } from './pcs-live-testing-repository.ts';
...
import { buildHistoricalLiveProductLineProjectRelation, buildHistoricalVideoRecordProjectRelation, normalizeLegacyLiveSessionHeaderRelation, } from './pcs-testing-relation-normalizer.ts';
export function createTestingRelationBootstrapSnapshot() {
    const channelSnapshot = createProjectChannelProductRelationBootstrapSnapshot();
    const relations = [...channelSnapshot.relations];
    const pendingItems = [];
    ...
    listVideoTestRecords().forEach((record) => {
        const legacyRefs = Array.from(new Set(getVideoItems(record.videoRecordId)
