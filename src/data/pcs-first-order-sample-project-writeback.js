...
import { getFirstOrderSampleTaskById, getLatestFirstOrderSampleTaskByProjectNode, listFirstOrderSampleTasks, updateFirstOrderSampleTask, upsertFirstOrderSampleTask, } from './pcs-first-order-sample-repository.ts';
import { getFirstSampleTaskById, listFirstSampleTasksByProject, } from './pcs-first-sample-repository.ts';
...
import { getFirstOrderSampleCompletionMissingFields, getFirstOrderSampleNodeEntryMissingFields, isFirstOrderSamplePassedStatus, } from './pcs-first-order-sample-field-policy.ts';
...
export const FIRST_ORDER_SAMPLE_CHAIN_MODE_OPTIONS = [
    '复用首版结论',
    '新增首单样衣确认',
    '替代布与正确布双确认',
];
export const FIRST_ORDER_SAMPLE_SPECIAL_REASON_OPTIONS = [
    '定位印',
    '大货量大',
    '工厂参照样',
    '正确布确认',
...
function normalizeSampleChainMode(value) {
    return FIRST_ORDER_SAMPLE_CHAIN_MODE_OPTIONS.includes(value)
        ? value
        : '';
}
function normalizeReasonCodes(values) {
    if (!Array.isArray(values))
        return [];
    return values.filter((item) => FIRST_ORDER_SAMPLE_SPECIAL_REASON_OPTIONS.includes(item));
}
...
function nextFirstOrderSampleTaskIdentity(timestamp) {
...
export function listFirstOrderSourceFirstSampleOptions(projectId) {
...
export function listFirstOrderTechPackVersionOptions(project) {
...
export function resolveFirstOrderSampleProjectNodeDefaults(projectId) {
...
function assertFirstOrderSampleProjectNode(projectId, projectNodeId) {
    const project = getProjectById(projectId);
    if (!project)
        return { project: null, node: null, error: '未找到商品项目，不能创建首单样衣打样任务。' };
    const node = getProjectNodeRecordById(projectId, projectNodeId);
    if (!node)
        return { project, node: null, error: '未找到首单样衣打样节点，不能创建任务。' };
...
export function buildFirstOrderProjectMeta(task) {
    return {
...
function buildFirstOrderSampleRelation(task, operatorName, timestamp) {
    return {
...
export function getFirstOrderSampleTaskForProjectNode(projectId, projectNodeId) {
    const task = getLatestFirstOrderSampleTaskByProjectNode(projectId, projectNodeId);
    if (task)
        return task;
    const relation = listProjectRelationsByProjectNode(projectId, projectNodeId).find((item) => item.sourceModule === '首单样衣打样' && item.sourceObjectType === '首单样衣打样任务');
...
export function createOrUpdateFirstOrderSampleTaskFromProjectNode(input) {
    ...
    const sampleChainMode = normalizedInput.sampleChainMode;
...
export function syncFirstOrderSampleTaskToProjectNode(input) {
    ...
    if (!task)
        return { ok: false, message: '未找到首单样衣打样任务，不能同步商品项目节点。', task: null, projectNode: null };
...
export function updateFirstOrderSampleTaskDetailAndSync(firstOrderSampleTaskId, patch, operatorName = '当前用户') {
    const current = getFirstOrderSampleTaskById(firstOrderSampleTaskId);
    if (!current)
        return { ok: false, message: '未找到首单样衣打样任务，不能保存详情。', task: null, projectNode: null };
    ...
    if (!updated)
        return { ok: false, message: '首单样衣打样任务保存失败。', task: null, projectNode: null };
    return syncFirstOrderSampleTaskToProjectNode({ firstOrderSampleTaskId: updated.firstOrderSampleTaskId, operatorName });
}
export function findFirstOrderSampleTaskRelations(firstOrderSampleTaskId) {
