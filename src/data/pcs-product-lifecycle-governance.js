export const STYLE_ARCHIVE_STATUS_RULES = {
    WAITING_BASE_INFO: {
...
export const TECH_PACK_AGGREGATE_STATUS_RULES = {
    UNCREATED: {
        key: 'UNCREATED',
        label: '未建立',
...
export const CHANNEL_PRODUCT_STATUS_RULES = {
    PENDING_LISTING: {
        key: 'PENDING_LISTING',
        label: '待上传',
        className: 'border-slate-200 bg-slate-50 text-slate-600',
...
export const STYLE_ARCHIVE_CONTROLLED_FIELD_RULES = [
    {
        title: '正式建档前维护字段',
...
export function isStyleArchiveFormalized(style) {
    return style.baseInfoStatus === '已建档' || style.baseInfoStatus === '已维护';
}
export function resolveStyleArchiveBusinessStatus(style) {
    if (style.archiveStatus === 'ARCHIVED')
        return 'ARCHIVED';
    if (style.archiveStatus === 'ACTIVE' || style.currentTechPackVersionId)
        return 'ACTIVE';
    if (isStyleArchiveFormalized(style))
        return 'WAITING_TECH_PACK';
    return 'WAITING_BASE_INFO';
}
export function resolveTechPackAggregateStatus(versions, currentVersionId) {
    if (versions.length === 0)
        return 'UNCREATED';
    if (versions.some((item) => item.versionStatus === 'DRAFT'))
        return 'DRAFT';
    if (currentVersionId)
        return 'ACTIVE';
...
export function resolveTechPackVersionBusinessStatus(record, currentVersionId) {
    if (record.versionStatus === 'ARCHIVED')
        return 'ARCHIVED';
    if (record.versionStatus === 'PUBLISHED' && currentVersionId === record.technicalVersionId)
        return 'ACTIVE';
    if (record.versionStatus === 'PUBLISHED')
        return 'PUBLISHED_PENDING';
    return 'DRAFT';
}
export function resolveChannelProductBusinessStatus(record) {
    if (record.channelProductStatus === '已作废')
        return 'INVALIDATED';
    if (record.channelProductStatus === '待上传')
        return 'PENDING_LISTING';
    if (record.channelProductStatus === '已上传待确认')
        return 'UPLOADED_PENDING_CONFIRM';
    if (record.channelProductStatus === '已完成')
        return 'LISTED_TESTING';
    if (record.channelProductStatus === '已上架待测款')
        return 'LISTED_TESTING';
    if (record.channelProductStatus === '已生效' && record.upstreamSyncStatus === '已更新')
        return 'ACTIVE_SYNCED';
    return 'ACTIVE_PENDING_SYNC';
}
export function normalizeStyleTechPackStatusText(status) {
    if (status === '草稿' || status === '待完善')
        return '草稿中';
    if (status === '已发布')
        return '已发布待启用';
    return status || '未建立';
}
export function listUnifiedProductLifecycleRuleRows() {
    return [
