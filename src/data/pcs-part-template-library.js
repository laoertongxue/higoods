import { suggestStandardPartName, } from '../utils/pcs-part-template-parser';
import { buildPartShapeDescription } from '../utils/pcs-part-template-shape-description';
...
function buildSeedPreviewSvg(label, fill) {
    return `
...
function pickStyleNames(seed, sourcePartName) {
    const stylePool = [
        '春夏爆款衬衣领',
        '基础商务衬衫',
        '轻熟通勤连衣裙',
...
function buildStats(seedInput, sourcePartName) {
...
function buildRecordFromPart(params) {
    ...
    return normalizeRecord({
        ...
        parserStatus: metrics && standardPartName
            ? '解析成功'
            : metrics
                ? '待人工矫正'
                : '解析异常',
...
export function listPartTemplatePackages() {
    return [...loadStore().packages];
}
export function listPartTemplateRecords() {
...
export function getPartTemplateRecordById(recordId) {
    return loadStore().records.find((record) => record.id === recordId) ?? null;
}
export function listSiblingPartTemplateRecords(templatePackageId) {
...
export function savePartTemplatePackage(params) {
    ...
    const records = params.rows.map((row) => buildRecordFromPart({
...
export function updatePartTemplateRecord(recordId, patch) {
    const store = loadStore();
...
export function recommendPartTemplateRecords(part, limit = 3) {
