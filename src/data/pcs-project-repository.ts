...
const SAMPLE_SOURCE_TYPES = ['外采', '委托打样'] as const
...
function cloneProject(project: PcsProjectRecord): PcsProjectRecord {
  ...
  return {
    ...
    targetChannelCodes: normalizePcsChannelCodes(baseProject.targetChannelCodes || []),
...
function getProjectCreateCatalogInternal(): ProjectCreateCatalog {
  ...
  return {
    ...
    channelOptions: PCS_CHANNEL_OPTIONS.map((item) => ({ ...item })),
...
function findChannelNames(codes: string[]): string[] {
  return codes
    .map((code) => PCS_CHANNEL_OPTIONS.find((item) => item.code === code)?.name ?? code)
    .filter(Boolean)
}
...
export function createProject(input: PcsProjectCreateDraft, operatorName = '当前用户'): ProjectCreateResult {
  ...
  const project: PcsProjectRecord = {
    ...
    targetChannelCodes: normalizePcsChannelCodes(input.targetChannelCodes),
    projectAlbumUrls: [...input.projectAlbumUrls],
    sampleSourceType: derivedSampleSourceType,
    sampleSupplierId: '',
    sampleSupplierName: '',
    sampleLink: '',
