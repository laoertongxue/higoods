...
function buildDateKey(dateText) {
...
function normalizeDomainStatus(value) {
    if (value === 'DRAFT' || value === 'COMPLETE')
        return value;
    return 'EMPTY';
}
function normalizeSourceTaskType(value, record) {
    if (value === 'REVISION' || value === 'PLATE' || value === 'ARTWORK' || value === 'MANUAL')
        return value;
    if ((record?.linkedRevisionTaskIds?.length ?? 0) > 0)
        return 'REVISION';
    if ((record?.linkedPatternTaskIds?.length ?? 0) > 0)
        return 'PLATE';
    if ((record?.linkedArtworkTaskIds?.length ?? 0) > 0)
        return 'ARTWORK';
    return 'REVISION';
}
...
function createEmptyContent(technicalVersionId) {
    return {
        technicalVersionId,
        patternFiles: [],
        patternDesc: '',
...
function normalizeContent(content) {
    return {
        ...
        colorMaterialMappings: cloneColorMappings(Array.isArray(content.colorMaterialMappings) ? content.colorMaterialMappings : []),
        ...
        legacyCompatibleCostPayload: content.legacyCompatibleCostPayload && typeof content.legacyCompatibleCostPayload === 'object'
            ? { ...content.legacyCompatibleCostPayload }
            : {},
    };
}
function getDomainStatus(count, versionStatus) {
    if (count <= 0)
        return 'EMPTY';
    return versionStatus === 'PUBLISHED' ? 'COMPLETE' : 'DRAFT';
}
export function buildTechnicalDataDerivedState(versionStatus, content) {
    ...
    if (bomItemCount > 0)
        ...
        missingItemCodes.push('BOM');
    if (patternFileCount > 0)
        completenessScore += 20;
    else
        missingItemCodes.push('PATTERN');
    if (processEntryCount > 0)
        completenessScore += 20;
    else
        missingItemCodes.push('PROCESS');
    if (gradingRuleCount > 0)
        completenessScore += 15;
    else
        missingItemCodes.push('GRADING');
    if (qualityRuleCount > 0)
        completenessScore += 15;
    else
        missingItemCodes.push('QUALITY');
    if (colorMaterialMappingCount > 0)
        completenessScore += 10;
    else
        missingItemCodes.push('COLOR_MATERIAL');
    return {
...
function applyDerivedFields(record, content) {
...
function normalizeRecord(rawRecord, contentMap) {
...
export function getNextStyleVersionMeta(styleId) {
...
export function getTechnicalDataVersionStoreSnapshot() {
    return loadSnapshot();
}
export function listTechnicalDataVersions() {
    return loadSnapshot().records.map(cloneRecord).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
}
export function getTechnicalDataVersionById(technicalVersionId) {
    const record = loadSnapshot().records.find((item) => item.technicalVersionId === technicalVersionId);
    return record ? cloneRecord(record) : null;
}
export function getTechnicalDataVersionContent(technicalVersionId) {
...
export function getTechnicalDataVersionContentById(technicalVersionId) {
    return getTechnicalDataVersionContent(technicalVersionId);
}
export function listTechnicalDataVersionsByStyleId(styleId) {
    return loadSnapshot()
        .records
...
export function listTechnicalDataVersionsByProjectId(projectId) {
    return loadSnapshot()
        .records
...
export function getCurrentTechPackVersionByStyleId(styleId) {
...
export function createTechnicalDataVersionDraft(record, content) {
...
export function updateTechnicalDataVersionRecord(technicalVersionId, patch) {
    const snapshot = loadSnapshot();
...
export function updateTechnicalDataVersionContent(technicalVersionId, patch) {
    const snapshot = loadSnapshot();
    ...
    if (contentIndex >= 0)
        nextContents.splice(contentIndex, 1, nextContent);
    else
    ...
    const nextRecords = [...snapshot.records];
    if (recordIndex >= 0) {
        nextRecords.splice(recordIndex, 1, normalizeRecord(snapshot.records[recordIndex], new Map([[technicalVersionId, nextContent]])));
    }
    persistSnapshot({
        version: TECHNICAL_VERSION_STORE_VERSION,
        records: nextRecords,
...
export function publishTechnicalDataVersionRecord(technicalVersionId, publishedAt, publishedBy) {
    const snapshot = loadSnapshot();
    ...
    const nextRecords = snapshot.records.map((item) => item.technicalVersionId === technicalVersionId
        ? normalizeRecord({
            ...item,
...
export function archiveTechnicalDataVersionRecord(technicalVersionId, updatedAt, updatedBy) {
...
export function listTechnicalDataVersionPendingItems() {
    return loadSnapshot().pendingItems.map(clonePendingItem);
}
export function pushTechnicalDataVersionPendingItem(item) {
    const snapshot = loadSnapshot();
...
export function replaceTechnicalDataVersionStore(snapshot) {
    persistSnapshot(snapshot);
}
export function resetTechnicalDataVersionRepository() {
