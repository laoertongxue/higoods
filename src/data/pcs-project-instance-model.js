import { getChannelNamesByCodes, getProjectById, listProjectNodes, } from './pcs-project-repository.ts';
import { listProjectReferenceImageViewModels, listProjectSampleShootImageViewModels, } from './pcs-project-image-view-model.ts';
import { getLatestProjectInlineNodeRecord, listProjectInlineNodeRecordsByProject, } from './pcs-project-inline-node-record-repository.ts';
...
function addField(fields, label, value, fieldKey) {
    const text = formatInstanceValue(value);
    if (!text)
        return;
    fields.push({ fieldKey, label, value: text });
}
function buildSummaryFromFields(fields, fallback) {
...
function buildFieldLabelMap(workItemTypeCode) {
...
function parseRelationMeta(note) {
    if (!note)
        return {};
...
function buildProjectRecordInstance(project, node) {
    const carrier = getPcsWorkItemRuntimeCarrierDefinition(node.workItemTypeCode);
    ...
    addField(fields, labelMap.get('projectAlbumUrls') || '参考图片', listProjectReferenceImageViewModels(project.projectId).map((item) => item.imageName), 'projectAlbumUrls');
...
function buildInlineRecordFields(record) {
    ...
    const fields = [];
    const sampleShootImageMap = record.workItemTypeCode === 'SAMPLE_SHOOT_FIT'
        ? new Map(listProjectSampleShootImageViewModels(record.projectId).map((item) => [item.imageId, item.imageName]))
        : null;
    getProjectWorkItemContract(record.workItemTypeCode).fieldDefinitions.forEach((field) => {
        let value = merged[field.fieldKey];
        if (sampleShootImageMap &&
            Array.isArray(value) &&
...
function buildInlineRecordInstance(record) {
    const carrier = getPcsWorkItemRuntimeCarrierDefinition(record.workItemTypeCode);
...
function buildFallbackRelationFields(relation, meta) {
    const fields = [];
...
function resolveChannelProductRelationObject(relation, meta) {
    const record = getProjectChannelProductById(relation.sourceObjectId);
    const sku = (record?.skuId ? getSkuArchiveById(record.skuId) : null) ||
...
function resolveRevisionTaskRelationObject(relation) {
...
function resolvePlateTaskRelationObject(relation) {
...
function resolvePatternTaskRelationObject(relation) {
...
function resolveFirstSampleRelationObject(relation) {
...
function resolveFirstOrderSampleRelationObject(relation) {
...
function resolveLiveRelationObject(relation) {
    ...
    const liveItem = session?.id && itemId
    ...
    const clickRateValue = (line?.exposureQty ?? 0) > 0 ? `${(((line?.clickQty ?? 0) / (line?.exposureQty ?? 1)) * 100).toFixed(1)}%` : '';
    const fields = [];
    ...
    addField(fields, '备注', session?.note || relation.note || `${line?.productTitle || relation.sourceTitle}直播测款已完成。`, 'note');
...
function resolveVideoRelationObject(relation) {
    ...
    const primaryItem = baseRecord?.id
    ...
    addField(fields, '视频链接', baseRecord?.videoUrl || (record ? `https://example.com/videos/${record.videoRecordCode}` : ''), 'videoUrl');
    ...
    addField(fields, '备注', baseRecord?.note || relation.note || `${record?.videoTitle || relation.sourceTitle}短视频测款已完成。`, 'note');
...
function resolveStyleArchiveRelationObject(relation, meta) {
...
function resolveTechnicalVersionRelationObject(relation, meta) {
...
function resolveProjectArchiveRelationObject(relation) {
...
function resolveRelationObjectSnapshot(relation) {
    const meta = parseRelationMeta(relation.note);
    ...
    if (relation.sourceObjectType === '改版任务')
        return resolveRevisionTaskRelationObject(relation);
    if (relation.sourceObjectType === '制版任务')
        return resolvePlateTaskRelationObject(relation);
    if (relation.sourceObjectType === '花型任务')
        return resolvePatternTaskRelationObject(relation);
    if (relation.sourceObjectType === '首版样衣打样任务')
        return resolveFirstSampleRelationObject(relation);
    if (relation.sourceObjectType === '首单样衣打样任务')
        return resolveFirstOrderSampleRelationObject(relation);
    if (relation.sourceObjectType === '直播商品明细')
        return resolveLiveRelationObject(relation);
    if (relation.sourceObjectType === '短视频记录')
        return resolveVideoRelationObject(relation);
    if (relation.sourceObjectType === '款式档案')
        return resolveStyleArchiveRelationObject(relation, meta);
    if (relation.sourceObjectType === '技术包版本')
        return resolveTechnicalVersionRelationObject(relation, meta);
    if (relation.sourceObjectType === '项目资料归档')
        return resolveProjectArchiveRelationObject(relation);
...
function buildRelationObjectInstance(relation) {
    const carrier = getPcsWorkItemRuntimeCarrierDefinition(relation.workItemTypeCode);
...
function createEmptyNodeInstanceModel(project, node) {
    const carrier = getPcsWorkItemRuntimeCarrierDefinition(node.workItemTypeCode);
    return {
        projectId: project.projectId,
        projectCode: project.projectCode,
...
export function getProjectInstanceModel(projectId) {
...
export function getProjectNodeInstanceModel(projectId, projectNodeId) {
    const project = getProjectById(projectId);
...
export function getProjectInstanceFieldValue(instance, fieldKey) {
    if (!instance)
        return '';
    return instance.fields.find((field) => field.fieldKey === fieldKey)?.value || '';
}
export function findLatestProjectInstance(projectId, matcher) {
    const model = getProjectInstanceModel(projectId);
    if (!model)
        return null;
    return model.instances.find(matcher) || null;
}
export function findLatestNodeInstance(projectId, projectNodeId, matcher) {
    const model = getProjectNodeInstanceModel(projectId, projectNodeId);
    if (!model)
        return null;
    return model.instances.find(matcher) || null;
}
export function getProjectNodeLatestInstanceRecord(projectId, projectNodeId) {
