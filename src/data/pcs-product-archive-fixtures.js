...
function normalizeEnglishName(styleName, seed) {
    const raw = styleName.trim();
    if (!raw)
        return pickBySeed(PRODUCT_TITLE_POOL, seed);
    if (/[A-Za-z]{3,}/.test(raw))
        return raw;
    return titleCaseWords(raw.replace(/[（()）]/g, ' '));
}
function resolveSizeBoost(size) {
...
function resolveBaseCost(styleName) {
    ...
    if (styleName.includes('裤') || /pants|jogger/i.test(styleName))
        return 84;
    if (styleName.includes('衬衫') || /shirt|kemeja/i.test(styleName))
        return 79;
    if (styleName.includes('毛衣') || styleName.includes('毛织') || /sweater|wool/i.test(styleName))
        return 88;
    return 68;
}
function resolvePackaging(styleName) {
    if (styleName.includes('外套') || styleName.includes('夹克') || /hoodie|jacket|jas/i.test(styleName)) {
...
export function buildStyleFixture(styleCode, styleName) {
...
export function buildSkuFixture(styleCode, styleName, colorName, sizeName) {
