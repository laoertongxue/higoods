...
function buildArchiveNo(sequence) {
...
export function createProjectArchiveBootstrapSnapshot(version) {
    const records = [];
    const documents = [];
    const files = [];
    const missingItems = [];
    ...
    listProjects().forEach((project) => {
        ...
        if (!style) {
            pendingItems.push(buildPendingItem(project.projectCode, project.linkedStyleId, '历史项目已存在款式档案关联字段，但正式款式档案不存在，暂不能初始化项目资料归档。'));
            return;
        }
        const archive = {
