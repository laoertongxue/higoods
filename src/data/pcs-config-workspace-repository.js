import { FLAT_DIMENSION_META, createInitialConfigData, } from './pcs-config-dimensions.ts';
...
function cloneSnapshot(snapshot) {
    return {
        ...
        flatOptions: Object.fromEntries(Object.entries(snapshot.flatOptions).map(([dimensionId, items]) => [dimensionId, items.map(cloneOption)])),
        categoryNodes: snapshot.categoryNodes.map(cloneCategoryNode),
    };
}
function makeLog(seed, action, detail, time) {
    return {
...
function buildCategoryLogs(itemName, seed) {
...
function loadSnapshot() {
    ...
    try {
        ...
        memorySnapshot = {
            version: STORE_VERSION,
            flatOptions: Object.fromEntries(Object.entries(parsed.flatOptions).map(([dimensionId, items]) => [dimensionId, Array.isArray(items) ? items.map(cloneOption) : []])),
...
function nextNumericCode(values) {
...
function appendOptionLog(option, dimensionName, action, detail, operatorName) {
    const time = nowText();
    const nextLog = {
...
function appendCategoryLog(node, action, detail, operatorName) {
    const time = nowText();
    const nextLog = {
...
export function listConfigWorkspaceSummaries() {
...
export function listConfigDimensionOptions(dimensionId) {
    return (loadSnapshot().flatOptions[dimensionId] || [])
        .map(cloneOption)
        .sort((left, right) => left.sortOrder - right.sortOrder || Number(left.code) - Number(right.code));
}
export function getConfigDimensionOption(dimensionId, optionId) {
    return listConfigDimensionOptions(dimensionId).find((item) => item.id === optionId) ?? null;
}
export function saveConfigDimensionOption(dimensionId, optionId, draft, operatorName = '当前用户') {
    const snapshot = loadSnapshot();
    ...
    if (!optionId) {
        ...
        const created = appendOptionLog(nextOption, dimensionName, '新增配置', `新增${dimensionName}「${draft.nameZh.trim()}」，并建立维护日志。`, operatorName);
    ...
    const updated = appendOptionLog({
        ...
        sortOrder: draft.sortOrder,
        status: draft.status,
    }, dimensionName, '编辑配置', `更新${dimensionName}「${draft.nameZh.trim()}」配置内容。`, operatorName);
    snapshot.flatOptions[dimensionId] = currentItems.map((item) => (item.id === optionId ? updated : item));
    persistSnapshot(snapshot);
    return updated;
}
export function listProductCategoryNodes() {
...
export function getProductCategoryNode(nodeId) {
...
export function listChildProductCategories(parentId) {
    return getCategoryChildren(parentId, listProductCategoryNodes());
}
export function canDeleteProductCategoryNode(nodeId) {
...
export function canDisableProductCategoryNode(nodeId) {
    const subtree = getCategorySubtree(nodeId, listProductCategoryNodes());
    return subtree.every((item) => item.productCount === 0);
}
export function saveProductCategoryNode(nodeId, parentId, draft, operatorName = '当前用户') {
    const snapshot = loadSnapshot();
    ...
    if (!nodeId) {
        ...
        const created = appendCategoryLog({
            ...
            updatedBy: operatorName,
            logs: [],
        }, '新增类目', `新增商品类目「${draft.name.trim()}」。`, operatorName);
    ...
    const updated = appendCategoryLog({
        ...
        sortOrder: draft.sortOrder,
        status: draft.status,
    }, '编辑类目', `更新商品类目「${draft.name.trim()}」的展示信息。`, operatorName);
    snapshot.categoryNodes = currentNodes.map((item) => (item.id === nodeId ? updated : item));
    persistSnapshot(snapshot);
    return updated;
}
export function deleteProductCategoryNode(nodeId, operatorName = '当前用户') {
    ...
    if (parent) {
        const parentUpdated = appendCategoryLog(parent, '子级调整', `已移除下级类目「${current.name}」。`, operatorName);
        snapshot.categoryNodes = snapshot.categoryNodes.map((item) => (item.id === parent.id ? parentUpdated : item));
    }
    persistSnapshot(snapshot);
}
export function resetConfigWorkspaceRepository() {
