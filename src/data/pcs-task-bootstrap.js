import { findProjectByCode, findProjectNodeByWorkItemTypeCode, } from './pcs-project-repository.ts';
import { findStyleArchiveByProjectId, findStyleArchiveByCode, listStyleArchives } from './pcs-style-archive-repository.ts';
function pendingItem(taskType, rawTaskCode, rawProjectField, rawSourceField, reason, discoveredAt) {
    return {
...
function firstSampleRelationMeta(task) {
...
function firstOrderRelationMeta(task) {
    return JSON.stringify({
...
function plateExecutionSeed(taskId, input) {
    return {
        ...
        : [
            {
                ...
                sampleRole: mode === '复用首版结论' ? '复用首版结论' : '正确布确认样',
                materialMode: mode === '复用首版结论' ? '沿用首版' : '正确布',
                quantity: 1,
                targetFactoryId: '',
                targetFactoryName: '',
                linkedSampleCode: reuseCode,
                status: reuseCode ? '已确认' : '待确认',
...
function createFirstSampleSeeds() {
    const tasks = [];
    const projectA = pickProjectByCode('PRJ-20251216-010');
    const projectB = pickProjectByCode('PRJ-20251216-003');
...
function createFirstOrderSeeds() {
    const tasks = [];
    const projectA = pickProjectByCode('PRJ-20251216-010');
    const projectB = pickProjectByCode('PRJ-20251216-003');
...
export function createTaskBootstrapSnapshot() {
...
export function createTaskRelationBootstrapSnapshot() {
    ...
    return {
        relations: [
            ...
            ...snapshot.plateTasks.map((task) => taskRelationRecord({
                projectId: task.projectId,
                projectCode: task.projectCode,
                projectNodeId: task.projectNodeId,
                workItemTypeCode: task.workItemTypeCode,
            ...
            ...snapshot.patternTasks.map((task) => taskRelationRecord({
                projectId: task.projectId,
                projectCode: task.projectCode,
                projectNodeId: task.projectNodeId,
                workItemTypeCode: task.workItemTypeCode,
            ...
            ...snapshot.firstSampleTasks.map((task) => taskRelationRecord({
                projectId: task.projectId,
                projectCode: task.projectCode,
                projectNodeId: task.projectNodeId,
                workItemTypeCode: task.workItemTypeCode,
            ...
            ...snapshot.firstOrderSampleTasks.map((task) => taskRelationRecord({
                projectId: task.projectId,
                projectCode: task.projectCode,
                projectNodeId: task.projectNodeId,
                workItemTypeCode: task.workItemTypeCode,
        ...
        pendingItems: [
            ...snapshot.revisionPendingItems.map((item) => relationPendingItem('改版任务', item.rawTaskCode, item.rawProjectField, item.reason, item.discoveredAt, item.rawSourceField)),
            ...snapshot.platePendingItems.map((item) => relationPendingItem('制版任务', item.rawTaskCode, item.rawProjectField, item.reason, item.discoveredAt, item.rawSourceField)),
            ...snapshot.patternPendingItems.map((item) => relationPendingItem('花型任务', item.rawTaskCode, item.rawProjectField, item.reason, item.discoveredAt, item.rawSourceField)),
            ...snapshot.firstSamplePendingItems.map((item) => relationPendingItem('首版样衣打样', item.rawTaskCode, item.rawProjectField, item.reason, item.discoveredAt, item.rawSourceField)),
            ...snapshot.firstOrderSamplePendingItems.map((item) => relationPendingItem('首单样衣打样', item.rawTaskCode, item.rawProjectField, item.reason, item.discoveredAt, item.rawSourceField)),
        ],
    };
}
