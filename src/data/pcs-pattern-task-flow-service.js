...
export function reviewPatternTaskByBuyer(patternTaskId, reviewStatus, reviewerName = '文锋', reviewNote = '') {
    ...
    if (!updated)
        throw new Error('买手确认更新失败。');
    return updated;
}
export function transferPatternTaskToChinaTeam(patternTaskId, reason, memberId = 'cn_bing_bing', operatorName = '当前用户') {
    const task = getPatternTaskById(patternTaskId);
    if (!task)
        throw new Error('未找到花型任务。');
    if (!reason.trim())
        throw new Error('转派中国团队必须填写原因。');
    const targetTeamCode = 'CN_TEAM';
    ...
    if (!updated)
        throw new Error('花型任务转派失败。');
    return updated;
}
export function assertPatternTaskAssignmentValid(task) {
