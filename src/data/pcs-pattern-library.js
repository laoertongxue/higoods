import { DEFAULT_PATTERN_CATEGORY_TREE, PatternTagService, buildPatternCategoryPath, canPatternBeReferenced, clonePatternCategoryTree, getPatternCategoryPrimaryOptions, getPatternCategorySecondaryOptions, getPatternSimilarityHit, inferDuplicateStatus, tokenizePatternFilename, } from '../utils/pcs-pattern-library-services.ts';
import { listProjectWorkspaceCategories, listProjectWorkspaceColors, listProjectWorkspaceStyles, } from './pcs-project-config-workspace-adapter.ts';
...
const LEGACY_CATEGORY_MIGRATION_MAP = {
    花卉: { primary: '植物与花卉' },
...
function normalizePatternLibraryConfig(config) {
...
function inferCategoryFromLegacy(category) {
    if (!category)
        return {};
    return LEGACY_CATEGORY_MIGRATION_MAP[category] ?? { primary: category };
}
function getAssetCategorySelection(asset) {
...
function migrateStoreSnapshot(store) {
    let migrated = false;
    const nextStore = cloneStore(store);
    nextStore.config = normalizePatternLibraryConfig(nextStore.config);
    if (nextStore.config.categories.join('|') !== (store.config?.categories ?? []).join('|')
    ...
    nextStore.assets = nextStore.assets.map((asset) => {
        ...
        const changed = asset.category !== normalizedCategory
...
function resolveDraftCategory(draft) {
...
function buildPreviewSvg(kind, palette) {
...
function buildPatternCode(sequence) {
    return `HX-${getTodayCode()}-${String(sequence).padStart(4, '0')}`;
}
function nextId(prefix, store) {
    store.sequence += 1;
...
export function waitForPatternLibraryPersistence() {
...
function enrichAsset(store, asset) {
...
function computeDuplicateCandidates(store, parsedFile, excludeAssetId) {
    const candidates = [];
    for (const version of store.versions.filter((item) => item.is_current)) {
        if (excludeAssetId && version.pattern_asset_id === excludeAssetId)
            continue;
...
async function persistBlobIfPresent(blob, kind, preferredKey) {
    if (!blob)
        return preferredKey;
    return patternRepo.saveBlob(blob, kind, preferredKey);
}
export async function persistPatternParsedFile(parsedFile) {
...
export async function getPatternBlob(blobKey) {
    if (!blobKey)
        return null;
    return patternRepo.getBlob(blobKey);
}
function createVersionRecord(store, assetId, parsedFile, versionNo) {
    return {
...
function buildPatternNameFromFilename(fileName) {
    return fileName.replace(/\.[^.]+$/, '');
}
function normalizeLicenseStatus(status) {
    return status ?? 'unverified';
}
function buildSourceReferences(store, assetId, draft) {
...
function buildDraftTags(store, assetId, versionId, draft) {
    ...
    const ruleTags = suggestions.map((suggestion) => ({
        id: nextId('pattern_tag', store),
        pattern_asset_id: assetId,
        pattern_file_version_id: versionId,
...
export const PATTERN_REVIEW_STATUS_LABELS = {
    draft: '草稿',
    pending: '待审核',
    approved: '已通过',
    rejected: '已驳回',
};
export const PATTERN_LIFECYCLE_STATUS_LABELS = {
    active: '启用中',
    inactive: '已停用',
    archived: '已归档',
};
export const PATTERN_DUPLICATE_STATUS_LABELS = {
    unchecked: '未检测',
    suspected: '疑似重复',
    unique: '唯一',
    merged: '已合并',
};
export const PATTERN_LICENSE_STATUS_LABELS = {
    unverified: '未确认',
    authorized: '已授权',
    restricted: '限制使用',
    expired: '已过期',
...
export function listPatternAssets() {
    const store = readStore();
...
export function listPatternAssetsForSelect() {
...
export function getPatternLibraryTabCounts() {
...
export function createPatternAsset(draft) {
    return mutateStore((store) => {
        ...
        const categorySelection = resolveDraftCategory(draft);
        if (draft.duplicateTargetAssetId &&
        ...
        const asset = {
            ...
            ai_summary: draft.parsedFile.parseWarnings.length > 0
        ...
        appendLog(store, assetId, '自动标签结果', '系统', `建议标签：${getAssetTags(store, assetId)
            .slice(0, 6)
            .map((tag) => `${tag.tag_type}-${tag.tag_name}`)
            .join(' / ') || '暂无'}`);
        if (duplicateCandidates.length > 0) {
            appendLog(store, assetId, '疑似重复检测', '系统', `命中 ${duplicateCandidates.length} 条疑似重复记录，最高相似度 ${Math.round(duplicateCandidates[0].hit.similarity * 100)}%。`);
        }
...
function addPatternVersion(input, inheritedStore) {
    const execute = (store) => {
        ...
        appendLog(store, input.assetId, '新建版本', input.updatedBy, `新增版本 ${version.version_no}，来源文件 ${input.parsedFile.originalFilename}${input.fromDuplicateMerge ? '，由重复文件合并而来' : ''}。`);
...
export function updatePatternAsset(assetId, input) {
    return mutateStore((store) => {
        const asset = store.assets.find((item) => item.id === assetId);
        if (!asset)
            throw new Error('花型不存在');
...
export function submitPatternAssetReview(assetId, operator) {
    return mutateStore((store) => {
        const asset = store.assets.find((item) => item.id === assetId);
        if (!asset)
            throw new Error('花型不存在');
...
export function approvePatternAsset(assetId, operator, comment = '审核通过，允许正式引用。') {
    return mutateStore((store) => {
        const asset = store.assets.find((item) => item.id === assetId);
        if (!asset)
            throw new Error('花型不存在');
...
export function rejectPatternAsset(assetId, operator, comment) {
    return mutateStore((store) => {
        const asset = store.assets.find((item) => item.id === assetId);
        if (!asset)
            throw new Error('花型不存在');
...
export function disablePatternAsset(assetId, operator) {
    return mutateStore((store) => {
        const asset = store.assets.find((item) => item.id === assetId);
        if (!asset)
            throw new Error('花型不存在');
...
export function restorePatternAsset(assetId, operator) {
    return mutateStore((store) => {
        const asset = store.assets.find((item) => item.id === assetId);
        if (!asset)
            throw new Error('花型不存在');
...
export function archivePatternAsset(assetId, operator) {
    return mutateStore((store) => {
        const asset = store.assets.find((item) => item.id === assetId);
        if (!asset)
            throw new Error('花型不存在');
...
export function batchUpdatePatternAssets(input) {
    return mutateStore((store) => {
        const updated = [];
        input.ids.forEach((id) => {
            ...
            if (!asset)
                return;
            if (input.maintenanceStatus)
                asset.maintenance_status = input.maintenanceStatus;
            if (input.reviewStatus)
                asset.review_status = input.reviewStatus;
            if (input.lifecycleStatus)
                asset.lifecycle_status = input.lifecycleStatus;
            asset.updated_by = input.updatedBy;
            asset.updated_at = nowIso();
...
export function getPatternTaskSummary(taskId) {
    const task = SOURCE_TASK_INDEX[taskId];
    return task ? { id: taskId, ...task } : null;
}
export function exportPatternLibraryRows() {
...
export function getPatternReferenceAvailability(assetId) {
    const asset = getPatternAssetById(assetId);
    if (!asset)
        return { allowed: false, reason: '花型不存在' };
    return canPatternBeReferenced(asset);
}
export function resetPatternLibraryStore() {
    memoryStore = migrateStoreSnapshot(seedStore()).store;
