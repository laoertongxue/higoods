...
import { activateTechPackVersionForStyle, } from './pcs-tech-pack-version-activation.ts';
import { generateTechPackVersionFromPatternTask, generateTechPackVersionFromPlateTask, generateTechPackVersionFromRevisionTask, syncProjectFromTechPackVersion, syncProjectSourceNodeFromTechPackVersion, syncStyleArchiveFromTechPackVersion, writeProjectRelationFromTechPackVersion, } from './pcs-tech-pack-task-generation.ts';
import { syncExistingProjectArchiveByProjectId } from './pcs-project-archive-sync.ts';
import { createTechnicalDataVersionDraft, getCurrentTechPackVersionByStyleId, getNextStyleVersionMeta, getNextTechnicalVersionIdentity, getTechnicalDataVersionById, getTechnicalDataVersionContent, updateTechnicalDataVersionContent, updateTechnicalDataVersionRecord, publishTechnicalDataVersionRecord, } from './pcs-technical-data-version-repository.ts';
...
function rebaseTechnicalVersionContent(content, baseTechnicalVersionId, nextTechnicalVersionId) {
...
export { generateTechPackVersionFromRevisionTask, generateTechPackVersionFromPlateTask, generateTechPackVersionFromPatternTask, activateTechPackVersionForStyle, };
export function saveTechnicalDataVersionContent(technicalVersionId, contentPatch, operatorName = '当前用户') {
    const record = getTechnicalDataVersionById(technicalVersionId);
    if (!record)
        throw new Error('未找到技术包版本。');
    if (record.versionStatus !== 'DRAFT')
        throw new Error('已发布的正式版本技术包不能编辑。');
...
export function publishTechnicalDataVersion(technicalVersionId, operatorName = '当前用户') {
    const record = getTechnicalDataVersionById(technicalVersionId);
    if (!record)
        throw new Error('未找到技术包版本。');
    if (record.versionStatus !== 'DRAFT')
        throw new Error('只有草稿技术包版本才能发布。');
...
export function createManualTechnicalDataVersionDraftFromCurrent(styleId, operatorName = '当前用户') {
    const style = getStyleArchiveById(styleId);
    if (!style)
        throw new Error('未找到正式款式档案，不能手动新增技术包版本。');
    const baseRecord = getCurrentTechPackVersionByStyleId(styleId);
    if (!baseRecord)
        throw new Error('当前款式没有当前生效技术包，不能手动新增版本。');
    const baseContent = getTechnicalDataVersionContent(baseRecord.technicalVersionId);
    if (!baseContent)
        throw new Error('当前生效技术包缺少内容，不能作为新版本初始化来源。');
