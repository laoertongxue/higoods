import { createEmptyProjectDraft, createProject, getProjectById, getProjectCreateCatalog, getProjectNodeRecordByWorkItemTypeCode, listActiveProjectTemplates, listProjects, listProjectNodes, updateProjectNodeRecord, updateProjectRecord, } from './pcs-project-repository.ts';
...
import { syncProjectNodeInstanceRuntime, syncProjectNodeInstancesByProject, } from './pcs-project-node-instance-registry.ts';
import { approveProjectInitAndSync, isClosedProjectNodeStatus, markProjectNodeCompletedAndUnlockNext, syncProjectLifecycle, terminateProject, } from './pcs-project-flow-service.ts';
import { findLatestProjectInstance, getProjectInstanceFieldValue, } from './pcs-project-instance-model.ts';
...
const INLINE_NODE_WORK_ITEM_CODE_SET = new Set([
    'SAMPLE_ACQUIRE',
...
function canUseSeedStorage() {
    return typeof localStorage !== 'undefined';
}
function getDemoSeedVersion() {
    if (!canUseSeedStorage())
        return '';
    try {
...
function findCatalogOptionByName(options, name) {
    return options.find((item) => item.name === name) ?? options[0] ?? null;
}
function findCategoryOptionByName(name) {
...
function serializeRelationNoteMeta(meta) {
...
function getCurrentChannelProductRelation(projectId) {
    return findLatestProjectInstance(projectId, (instance) => instance.sourceLayer === '正式业务对象' &&
...
function seedNodeStatus(projectId, workItemTypeCode, patch, operatorName = DEMO_OPERATOR) {
...
function buildQuickRecordPayload(project, node, input) {
        ...
        case 'TEST_DATA_SUMMARY':
            return {
                ...
                detailSnapshot: {
                    ...
                    upstreamChannelProductCode: String(getProjectInstanceFieldValue(currentChannelProduct, 'upstreamChannelProductCode') ||
                        buildFallbackUpstreamChannelProductCode(currentChannelProduct?.instanceCode || `${project.projectCode}-CP`, project.projectCode)),
...
function seedInlineRecordAndComplete(projectId, workItemTypeCode, input) {
    ...
    if (!payload)
        return;
    saveProjectInlineNodeFieldEntry(projectId, node.projectNodeId, {
...
function seedInlineRecord(projectId, workItemTypeCode, input) {
    const project = getProjectById(projectId);
    const node = getProjectNodeRecordByWorkItemTypeCode(projectId, workItemTypeCode);
    if (!project || !node)
        return;
...
function isInlineNodeWorkItemTypeCode(value) {
    return INLINE_NODE_WORK_ITEM_CODE_SET.has(value);
}
function getOrderedTemplateNodes(template) {
...
function getCoverageBlueprint(styleType, replicaIndex) {
...
function buildCoverageProjectName(styleType, workItemTypeName, replicaIndex) {
...
function getCoverageBusinessDate(nodeIndex, replicaIndex, offset = 0) {
    const day = 2 + ((nodeIndex * 3 + replicaIndex * 5 + offset) % 26);
    return `2026-04-${pad(day)}`;
}
function getCoverageTimestamp(nodeIndex, replicaIndex, hour = 10) {
...
function buildClickRate(clicks, exposure) {
    if (!exposure)
        return '';
    return `${((clicks / exposure) * 100).toFixed(1)}%`;
}
function getDemoStoreMeta(project, replicaIndex) {
...
function getCoverageObjectRefs(project, context) {
...
function buildLiveMetrics(nodeIndex, replicaIndex) {
...
function buildVideoMetrics(nodeIndex, replicaIndex) {
...
function updateNodeRuntimeMeta(projectId, nodeId, patch) {
    updateProjectNodeRecord(projectId, nodeId, {
...
function ensureFormalNodeRelation(project, node, nodeIndex, context) {
    ...
    switch (node.workItemTypeCode) {
        ...
        case 'FIRST_SAMPLE': {
            upsertDemoRelation({
                ...
                noteMeta: {
                    sourceTaskType: '制版任务',
                    sourceTaskId: refs.plateTaskId,
                    sourceTaskCode: refs.plateTaskCode,
                    sourceTechPackVersionId: refs.techPackVersionId,
                    sourceTechPackVersionCode: refs.techPackVersionCode,
                    sourceTechPackVersionLabel: refs.techPackVersionLabel,
                    ...
                    sampleImageIds: [`mock://sample-result/${project.projectCode}-first-sample-1`],
                    fitConfirmationSummary: nodeIndex < context.currentNodeIndex ? '版型确认通过。' : '',
                    artworkConfirmationSummary: nodeIndex < context.currentNodeIndex ? '外观效果确认通过。' : '',
                    productionReadinessNote: nodeIndex < context.currentNodeIndex ? '可进入首单样衣准备。' : '',
                    reuseAsFirstOrderBasisFlag: nodeIndex < context.currentNodeIndex,
                    reuseAsFirstOrderBasisConfirmedAt: nodeIndex < context.currentNodeIndex ? businessDate : '',
                    reuseAsFirstOrderBasisConfirmedBy: nodeIndex < context.currentNodeIndex ? project.ownerName : '',
                    reuseAsFirstOrderBasisNote: nodeIndex < context.currentNodeIndex ? '首版样衣可作为首单参照。' : '',
                    sourceType: '任务打样',
                    upstreamModule: '制版任务',
        ...
        case 'FIRST_ORDER_SAMPLE': {
            upsertDemoRelation({
                ...
                businessDate,
                noteMeta: {
                    sourceFirstSampleTaskId: refs.firstSampleTaskId,
                    sourceFirstSampleTaskCode: refs.firstSampleTaskCode,
                    sourceFirstSampleCode: `FS-RESULT-${project.projectCode.slice(-4)}-${context.replicaIndex + 1}`,
                    sourceTechPackVersionId: refs.techPackVersionId,
                    sourceTechPackVersionCode: refs.techPackVersionCode,
                    sourceTechPackVersionLabel: '首单确认版',
                    ...
                    samplePlanLines: [
                        {
                            lineId: 'reuse-first-sample-01',
                            sampleRole: '复用首版结论',
                            materialMode: '沿用首版',
                            quantity: 1,
                            targetFactoryId: '',
                            targetFactoryName: '',
                            linkedSampleCode: `FS-RESULT-${project.projectCode.slice(-4)}-${context.replicaIndex + 1}`,
                            status: nodeIndex < context.currentNodeIndex ? '已确认' : '待确认',
                            note: '复用首版样衣结论作为首单依据。',
                        },
                    ],
                    finalReferenceNote: nodeIndex < context.currentNodeIndex ? '首版样衣确认通过，首单阶段直接沿用。' : '',
                    sampleCode: nodeIndex < context.currentNodeIndex ? `FOS-RESULT-${project.projectCode.slice(-4)}-${context.replicaIndex + 1}` : '',
                    conclusionResult: nodeIndex < context.currentNodeIndex ? '通过' : '',
                    conclusionNote: nodeIndex < context.currentNodeIndex ? '首单样衣确认通过，可进入后续。' : '',
                    sourceType: '首单确认',
                    upstreamModule: '首版样衣打样',
                    upstreamObjectType: '首版样衣打样任务',
                    upstreamObjectId: refs.firstSampleTaskId,
                    upstreamObjectCode: refs.firstSampleTaskCode,
                    taskStatus: latestStatus,
                    confirmedAt: nodeIndex < context.currentNodeIndex ? businessDate : '',
                    confirmedBy: nodeIndex < context.currentNodeIndex ? project.ownerName : '',
                },
            });
            return;
        }
...
function hydrateProjectOutputRefs(project, context) {
    const refs = getCoverageObjectRefs(project, context);
    const patch = {};
...
function resolveProjectCurrentNodeIndex(nodes, project) {
    if (nodes.length <= 1)
        return 0;
...
function normalizeProjectNodeChain(project, nodes, currentNodeIndex, replicaIndex) {
...
function ensureProjectNodeDemoPayloads(project, _template, currentNodeIndex, replicaIndex) {
    ...
    orderedNodes.forEach((node, index) => {
        if (index === 0 || index > context.currentNodeIndex)
            return;
    ...
    syncProjectNodeInstancesByProject(project.projectId, DEMO_OPERATOR, getCoverageTimestamp(context.currentNodeIndex, replicaIndex, 18));
    syncProjectLifecycle(project.projectId, DEMO_OPERATOR, getCoverageTimestamp(context.currentNodeIndex, replicaIndex, 19));
}
function ensureTemplateCoverageProjects() {
    listActiveProjectTemplates().forEach((template) => {
        ...
        orderedNodes.forEach((node, nodeIndex) => {
            ...
            Array.from({ length: COVERAGE_PROJECTS_PER_NODE }).forEach((_, replicaIndex) => {
                ...
                const existingProject = listProjects().find((project) => project.projectName === projectName && project.templateId === template.id) ?? null;
                const blueprint = getCoverageBlueprint(styleType, replicaIndex);
                const project = existingProject ??
                    createProject(buildDemoDraft({
                        projectName,
                        styleType,
...
function ensureExistingProjectDemoConsistency() {
    ...
    listProjects().forEach((project, projectIndex) => {
        ...
        if (!template)
            return;
        ensureProjectNodeDemoPayloads(project, template, resolveProjectCurrentNodeIndex(listProjectNodes(project.projectId), project), projectIndex % COVERAGE_PROJECTS_PER_NODE);
    });
}
function ensureListingNodeDemoProject() {
    const existingProject = listProjects().find((project) => project.projectName === '2026夏季印花短袖快反项目');
    if (existingProject)
        return;
    const ongoingProject = createProject(buildDemoDraft({
    ...
    updateProjectRecord(ongoingProject.projectId, {
        updatedAt: '2026-04-12 18:40',
    }, DEMO_OPERATOR);
    syncProjectLifecycle(ongoingProject.projectId, DEMO_OPERATOR, '2026-04-12 18:40');
}
export function ensurePcsProjectDemoDataReady() {
    if (projectDemoSeedReady)
        return;
    ...
    if (!hasLegacyDemoSeed) {
        const pendingProject = createProject(buildDemoDraft({
        ...
        updateProjectRecord(pendingProject.projectId, {
        ...
        ensureListingNodeDemoProject();
        const decisionProject = createProject(buildDemoDraft({
        ...
        const terminatedProject = createProject(buildDemoDraft({
        ...
        const archivedProject = createProject(buildDemoDraft({
        ...
        listProjectNodes(archivedProject.projectId).forEach((node) => {
            if (node.workItemTypeCode === 'PROJECT_INIT' || isClosedProjectNodeStatus(node.currentStatus))
                return;
        ...
        [
            ...
            const project = createProject(buildDemoDraft({
        ...
        upsertDemoRelation({
            ...
            noteMeta: {
                sourceTaskType: '制版任务',
                sourceTaskId: `${archivedProject.projectId}-plate-001`,
                sourceTaskCode: `${archivedProject.projectCode}-PT-001`,
                sourceTechPackVersionId: `${archivedProject.projectId}-tdv-001`,
                sourceTechPackVersionCode: `${archivedProject.projectCode}-TDV-001`,
                sourceTechPackVersionLabel: '归档确认版',
                factoryId: 'factory-shenzhen-01',
                factoryName: '深圳工厂01',
                targetSite: '深圳',
                sampleMaterialMode: '正确布',
                ...
                sampleImageIds: [`mock://sample-result/${archivedProject.projectCode}-first-sample-1`],
                fitConfirmationSummary: '版型确认通过。',
                artworkConfirmationSummary: '花型确认通过。',
                productionReadinessNote: '可进入首单样衣准备。',
                reuseAsFirstOrderBasisFlag: true,
                reuseAsFirstOrderBasisConfirmedAt: '2026-04-05 18:40',
                reuseAsFirstOrderBasisConfirmedBy: archivedProject.ownerName,
                reuseAsFirstOrderBasisNote: '首版样衣可作为首单参照。',
                confirmedAt: '2026-04-05 18:40',
            },
        });
        ...
        updateProjectRecord(archivedProject.projectId, {
            projectStatus: '已归档',
