...
export function createOrUpdateFirstSampleTaskFromProjectNode(
  ...
  return {
    ok: true,
    message: existing
      ? '已更新首版样衣必要信息，并同步商品项目关系。'
      : '已创建首版样衣打样任务，并同步商品项目关系。',
    task: savedTask,
    projectNode,
  }
}
...
export function syncFirstSampleTaskToProjectNode(input: {
  ...
  if (!task) {
    return { ok: false, message: '未找到首版样衣打样任务，不能同步商品项目关系。', task: null, projectNode: null }
  }
  if (!task.projectNodeId) {
    return {
      ok: true,
      message: '首版样衣打样任务已保存，当前为独立任务，不占用商品项目节点。',
      task,
      projectNode: null,
    }
  }
