...
function createPlateSeeds(): { tasks: PlateMakingTaskRecord[]; pendingItems: PcsTaskPendingItem[] } {
  ...
  seedItems.forEach((item) => {
    ...
    tasks.push({
      ...
      upstreamObjectType: usesLegacyProjectNodeSource ? '项目步骤' : item.upstreamObjectType,
      upstreamObjectId,
      upstreamObjectCode,
      styleId: style?.styleId || '',
      styleCode,
...
function createPatternSeeds(): { tasks: PatternTaskRecord[]; pendingItems: PcsTaskPendingItem[] } {
  ...
  ;[
    ...
    tasks.push({
      ...
      stepCode: 'PATTERN_ARTWORK_TASK',
      stepName: '花型任务',
      sourceType: upstreamRevisionCode ? '改版任务' : '项目固定步骤',
      upstreamModule: upstreamRevisionCode ? '改版任务' : '商品项目',
      upstreamObjectType: upstreamRevisionCode ? '改版任务' : '项目步骤',
