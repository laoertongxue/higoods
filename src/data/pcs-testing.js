...
export const ACCOUNTING_STATUS_META = {
...
export const LIVE_PURPOSE_META = {
...
export const VIDEO_PURPOSE_META = {
    TEST: { label: '测款', color: 'bg-violet-100 text-violet-700' },
...
export const VIDEO_PLATFORM_META = {
...
const LEGACY_TESTING_PROJECT_REFERENCE_SEED = [];
...
export function getLiveSessionById(id) {
    const found = LIVE_SESSION_SEED.find((item) => item.id === id);
    return found ? clone(found) : null;
}
export function getLiveSessionItems(sessionId) {
    return clone(LIVE_ITEM_SEED[sessionId] ?? []);
}
export function getLiveSessionSamples(sessionId) {
    return clone(LIVE_SAMPLE_SEED[sessionId] ?? []);
}
export function getLiveSessionLogs(sessionId) {
...
export function getVideoItems(recordId) {
    return clone(VIDEO_ITEM_SEED[recordId] ?? []);
}
export function getVideoEvidence(recordId) {
    return clone(VIDEO_EVIDENCE_SEED[recordId] ?? []);
}
export function getVideoSamples(recordId) {
    return clone(VIDEO_SAMPLE_SEED[recordId] ?? []);
}
export function getVideoLogs(recordId) {
    return clone(VIDEO_LOG_SEED[recordId] ?? []);
}
export function listLegacyTestingProjectReferences() {
