export const FIRST_ORDER_SAMPLE_NODE_ENTRY_REQUIRED_FIELDS = [
...
export const FIRST_ORDER_SAMPLE_COMPLETION_REQUIRED_FIELDS = [
...
export function isFirstOrderSamplePassedStatus(status) {
    return status === '已通过';
}
export function getFirstOrderSampleNodeEntryMissingFields(input) {
...
export function getFirstOrderSampleCompletionMissingFields(task) {
    return FIRST_ORDER_SAMPLE_COMPLETION_REQUIRED_FIELDS
        .filter((item) => {
        const value = task[item.fieldKey];
        if (item.fieldKey === 'samplePlanLines')
            return !Array.isArray(value) || value.length === 0;
        if (item.fieldKey === 'conclusionResult')
            return value !== '通过';
        return value === undefined || value === null || String(value).trim() === '';
    })
        .map((item) => item.label);
