function cloneChannelListingImage(image) {
    return { ...image };
}
export function cloneChannelListingImages(images) {
    return images.map(cloneChannelListingImage);
}
export function buildChannelListingImageId(listingBatchId, index) {
...
export function normalizeChannelListingImages(input) {
    ...
    return input.imageInputs
        ...
        return {
            ...
            sortNo: typeof item.sortNo === 'number' && Number.isFinite(item.sortNo)
                ? item.sortNo
                : index + 1,
...
export function deriveListingImageIds(listingImages) {
    return listingImages
        .slice()
        .sort((left, right) => left.sortNo - right.sortNo)
        .map((item) => item.imageId);
}
export function deriveListingMainImageId(listingImages) {
    return (listingImages.find((item) => item.mainFlag)?.imageId ||
        listingImages.slice().sort((left, right) => left.sortNo - right.sortNo)[0]?.imageId ||
        '');
}
function isListingImageUsable(asset) {
    if (!asset)
        return false;
    if (!asset.usageScopes.includes('商品上架'))
        return false;
