...
import { getProjectImageAssetById, listProjectImageAssets, markProjectImageAssetUsableForStyleArchive, } from './pcs-project-image-repository.ts';
function uniqueStrings(values) {
    return Array.from(new Set(values.map((item) => item.trim()).filter(Boolean)));
}
function getImageSourceLabel(record) {
    if (record.sourceNodeCode === 'CHANNEL_PRODUCT_LISTING')
        return '商品上架图片';
...
function resolveImagePriority(record, listingImageIds) {
    if (listingImageIds.has(record.imageId) || record.sourceNodeCode === 'CHANNEL_PRODUCT_LISTING')
        return 1;
    if (record.sourceNodeCode === 'SAMPLE_SHOOT_FIT')
        return 2;
    if (record.imageType === '项目参考图')
...
function canBeStyleArchiveCandidate(record, listingImageIds) {
    if (!record.imageUrl)
        return false;
...
function isPendingReferenceImage(record) {
    return record.imageType === '项目参考图' && !record.usageScopes.includes('款式档案');
}
function ensureImageReadyForStyleArchive(record, operatorName, timestamp) {
    if (record.imageStatus === '已弃用' ||
...
export function listStyleArchiveImageCandidates(projectId) {
    const listingImageIds = new Set(listProjectChannelProductsByProjectId(projectId)
...
export function resolveStyleArchiveImageSelection(input) {
