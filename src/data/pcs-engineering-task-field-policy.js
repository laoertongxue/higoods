const REVISION_TASK_FIELD_POLICY = {
...
const PATTERN_TASK_FIELD_POLICY = {
...
export function getEngineeringTaskFieldPolicy(workItemTypeCode) {
    return FIELD_POLICY_MAP[workItemTypeCode];
}
export function getRevisionTaskCompletionMissingFields(task) {
    const missing = [];
    if (!(task.baseStyleCode || task.styleCode || task.productStyleCode).trim())
        missing.push('旧款信息');
    if ((task.revisionScopeNames || []).length === 0 && (task.revisionScopeCodes || []).length === 0)
        missing.push('改版范围');
    if (!(task.revisionSuggestionRichText || task.issueSummary).trim())
        missing.push('修改建议');
    if (task.liveRetestRequired) {
        if (task.liveRetestStatus === '待回直播验证')
            missing.push('回直播验证状态');
        if ((task.liveRetestRelationIds || []).length === 0 && !task.liveRetestSummary.trim())
            missing.push('回直播验证结论');
    }
    if (!task.generatedNewTechPackVersionFlag && !task.linkedTechPackVersionId)
        missing.push('新技术包版本');
    return missing;
}
export function getPlateTaskCompletionMissingFields(task) {
    const missing = [];
    if (!(task.patternMakerName || task.ownerName).trim())
        missing.push('版师');
    if (!task.productHistoryType)
        missing.push('产品历史属性');
    if (!task.patternArea)
        missing.push('打版区域');
    const hasPatternImage = (task.patternImageLineItems || []).length > 0;
    const hasPatternFile = (task.patternPdfFileIds || []).length > 0
        || (task.patternDxfFileIds || []).length > 0
        || (task.patternRulFileIds || []).length > 0;
    if (!hasPatternImage && !hasPatternFile && !task.patternVersion.trim())
        missing.push('纸样资料');
    return missing;
}
export function getPatternTaskCompletionMissingFields(task) {
    const missing = [];
    if (!task.artworkVersion.trim())
        missing.push('花型版次');
    if (task.buyerReviewStatus !== '买手已通过')
        missing.push('买手确认通过');
    if (task.completionImageIds.length === 0)
        missing.push('完成确认图片');
    return missing;
}
