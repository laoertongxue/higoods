...
import { listProjectWorkspaceCategories, listProjectWorkspaceStyles, } from './pcs-project-config-workspace-adapter.ts';
const WORKSPACE_CATEGORIES = listProjectWorkspaceCategories();
const WORKSPACE_STYLES = listProjectWorkspaceStyles();
function pickOptionByIndex(options, index) {
    if (options.length === 0) {
        return { id: '', code: '', name: '' };
    }
...
function findOptionByKeywords(options, keywords) {
    ...
    if (normalizedKeywords.length === 0)
        return null;
    return (options.find((option) => normalizedKeywords.some((keyword) => option.name.includes(keyword) || option.code.includes(keyword))) ?? null);
}
function resolveCategory(styleName) {
...
function resolvePriceRange(styleName) {
    const lowered = styleName.toLowerCase();
    if (styleName.includes('外套') ||
        styleName.includes('夹克') ||
        lowered.includes('jacket') ||
    ...
    if (styleName.includes('裙') || lowered.includes('dress'))
        return '¥299-499';
    if (styleName.includes('裤') || lowered.includes('pants') || lowered.includes('jogger'))
        return '¥199-399';
    return '¥159-299';
}
function resolveStyleTags(styleName) {
    const keywords = [];
    const lowered = styleName.toLowerCase();
    if (styleName.includes('印花') || styleName.includes('碎花') || lowered.includes('batik'))
        keywords.push('碎花');
    if (styleName.includes('运动'))
        keywords.push('街头');
    if (styleName.includes('休闲') || lowered.includes('casual') || lowered.includes('polo') || lowered.includes('t-shirt'))
        keywords.push('休闲');
    if (styleName.includes('商务') || styleName.includes('西装') || lowered.includes('formal') || lowered.includes('blazer'))
        keywords.push('通勤');
    if (styleName.includes('中式'))
        keywords.push('中式');
    if (styleName.includes('优雅') || styleName.includes('礼服') || lowered.includes('dress'))
        keywords.push('优雅');
    const tags = keywords
...
const EXTRA_STYLE_ARCHIVE_RECORDS = [
    {
...
export function createStyleArchiveBootstrapSnapshot(version) {
    return {
        version,
