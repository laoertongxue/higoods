import { getProjectById, getProjectNodeRecordById, listProjectNodes, listProjectPhases, updateProjectNodeRecord, updateProjectPhaseRecord, updateProjectRecord, } from './pcs-project-repository.ts';
...
const DECISION_RESULT_OPTIONS = ['通过', '淘汰'];
function nowText() {
    const now = new Date();
...
function buildResultText(nodeName, result, note) {
    return note.trim() || `${nodeName}已判定为${result}。`;
}
function syncProjectLifecycleAfterDecision(projectId, operatorName, timestamp) {
...
export function isProjectDecisionWorkItemCode(workItemTypeCode) {
    return DECISION_WORK_ITEM_CODES.includes(workItemTypeCode);
}
function assertDecisionResult(result) {
    if (!DECISION_RESULT_OPTIONS.includes(result)) {
        throw new Error('决策结果只能是通过或淘汰');
    }
}
export function advanceDecisionNodePassed(projectId, projectNodeId, operatorName = '当前用户', note = '', timestamp = nowText()) {
...
export function routeProjectToSampleReturnHandle(projectId, decisionNodeId, operatorName = '当前用户', note = '', timestamp = nowText()) {
    ...
    updateProjectNodeRecord(projectId, decisionNodeId, {
        currentStatus: '已完成',
        latestResultType: '淘汰',
        latestResultText: resultText,
        pendingActionType: '已完成',
    ...
    nodes.forEach((node, index) => {
        if (node.projectNodeId === decisionNodeId || node.projectNodeId === sampleReturnNode.projectNodeId)
            return;
        if (index <= decisionIndex)
            return;
        if (sampleReturnIndex >= 0 && index >= sampleReturnIndex)
            return;
    ...
    updateProjectNodeRecord(projectId, sampleReturnNode.projectNodeId, {
        currentStatus: '进行中',
...
export function advanceDecisionNodeEliminated(projectId, projectNodeId, operatorName = '当前用户', note = '', timestamp = nowText()) {
    return routeProjectToSampleReturnHandle(projectId, projectNodeId, operatorName, note, timestamp);
}
export function completeDecisionNodeWithResult(projectId, projectNodeId, result, operatorName = '当前用户', note = '', timestamp = nowText()) {
