import { getProjectTemplateById, getProjectTemplateVersion, hasTemplatePendingNodes, listProjectTemplates, } from './pcs-templates.ts';
...
import { buildProjectNodeRecordsFromTemplate, buildProjectPhaseRecordsFromTemplate, } from './pcs-project-node-factory.ts';
import { buildProjectWorkspaceCategoryOptions, findProjectWorkspaceOptionById, listProjectWorkspaceAges, listProjectWorkspaceBrands, listProjectWorkspaceCrowdPositioning, listProjectWorkspaceCrowds, listProjectWorkspaceProductPositioning, listProjectWorkspaceStyleCodes, listProjectWorkspaceStyles, } from './pcs-project-config-workspace-adapter.ts';
const PROJECT_STORAGE_KEY = 'higood-pcs-project-store-v2';
const PROJECT_STORE_VERSION = 2;
const PROJECT_TYPES = ['商品开发', '快反上新', '改版开发', '设计研发'];
const PROJECT_SOURCE_TYPES = ['企划提案', '渠道反馈', '测款沉淀', '历史复用', '外部灵感'];
const SAMPLE_SOURCE_TYPES = ['外采', '自打样', '委托打样'];
const PRIORITY_LEVELS = ['高', '中', '低'];
const STYLE_TYPES = ['基础款', '快时尚款', '改版款', '设计款'];
...
function stripProjectRuntimeFields(project) {
    const { progressDone: _progressDone, progressTotal: _progressTotal, nextWorkItemName: _nextWorkItemName, nextWorkItemStatus: _nextWorkItemStatus, pendingDecisionFlag: _pendingDecisionFlag, blockedFlag: _blockedFlag, blockedReason: _blockedReason, riskStatus: _riskStatus, riskReason: _riskReason, riskWorkItem: _riskWorkItem, riskDurationDays: _riskDurationDays, ...rest } = project;
    return rest;
}
...
function normalizeProject(project) {
    return {
        ...
        projectStatus: project.projectStatus === '已终止' ||
...
function normalizePhase(phase) {
    return {
        ...clonePhase(phase),
        phaseStatus: phase.phaseStatus === '已完成' || phase.phaseStatus === '已终止' || phase.phaseStatus === '进行中'
            ? phase.phaseStatus
            : '未开始',
    };
}
...
function getNodeAlignmentScore(node, expectedNodeId) {
    let score = 0;
    if (node.projectNodeId === expectedNodeId)
        score += 20;
    if (node.currentStatus === '已完成')
        score += 120;
    else if (node.currentStatus === '待确认')
        score += 90;
    else if (node.currentStatus === '进行中')
        score += 70;
    else if (node.currentStatus === '已取消')
        score += 30;
    if (node.latestInstanceId || node.latestInstanceCode)
        score += 40;
    if (node.latestResultType || node.latestResultText)
        score += 20;
    if (node.updatedAt || node.lastEventTime)
        score += 10;
    if (node.validInstanceCount > 0)
        score += 10;
    return score;
}
function alignProjectNodesWithTemplate(project, existingNodes) {
    ...
    return generatedNodes.map((generatedNode) => {
        const candidates = (candidatesByCode.get(generatedNode.workItemTypeCode) || [])
            ...
            .sort((left, right) => getNodeAlignmentScore(right, generatedNode.projectNodeId) -
...
function migrateRevisionTemplateProject(project) {
    if (project.templateId !== 'TPL-003')
...
function mergeMissingBootstrapData(snapshot) {
    ...
    const templateNodeCodeMap = new Map();
    mergedProjects.forEach((project) => {
        const template = getProjectTemplateById(project.templateId);
        if (!template)
            return;
        templateNodeCodeMap.set(project.projectId, new Set(buildProjectNodeRecordsFromTemplate({
            projectId: project.projectId,
            ownerId: project.ownerId,
            ownerName: project.ownerName,
            createdAt: project.createdAt,
    ...
    const templateProjectIds = new Set(mergedProjects
    ...
    const alignedNodes = mergedProjects
        ...
        .flatMap((project) => alignProjectNodesWithTemplate(project, mergedNodes.filter((node) => node.projectId === project.projectId)));
    mergedNodes = [
...
function hydrateSnapshot(snapshot) {
    ...
    if (decisionMigrated.projects.length === 0 &&
...
function getDurationDaysSince(dateText, endDateText = nowText()) {
...
function getOrderedProjectNodes(snapshot, projectId) {
...
function getProjectNodeSequenceBlockerFromSnapshot(snapshot, projectId, projectNodeId) {
...
function resetNodeToLockedPending(node, blocker, timestamp) {
...
function activateFirstOpenNode(node, timestamp) {
...
function syncProjectLifecycleInSnapshot(snapshot, projectId, operatorName, timestamp) {
    ...
    const completedNonInitCount = nodes.filter((node) => node.workItemTypeCode !== 'PROJECT_INIT' && node.currentStatus === '已完成').length;
    const allClosed = nodes.every((node) => isClosedNodeStatus(node.currentStatus));
    const nextProjectStatus = project.projectStatus === '已终止'
        ? '已终止'
        : allClosed
            ? '已归档'
            : completedNonInitCount === 0
                ...
                : '进行中';
    let changed = false;
    if (project.currentPhaseCode !== currentPhase.phaseCode ||
...
function repairProjectNodeSequenceInSnapshot(snapshot, projectId, operatorName, timestamp) {
    const project = snapshot.projects.find((item) => item.projectId === projectId);
    const orderedNodes = getOrderedProjectNodes(snapshot, projectId);
    if (!project || orderedNodes.length === 0)
        return false;
    let changed = false;
    ...
    if (firstOpenIndex >= 0) {
        ...
        orderedNodes.slice(firstOpenIndex + 1).forEach((node) => {
            if (node.currentStatus === '未开始' || isClosedNodeStatus(node.currentStatus))
                return;
...
function repairProjectNodeSequences(snapshot, operatorName = '系统修复', timestamp = nowText()) {
    snapshot.projects.forEach((project) => {
        repairProjectNodeSequenceInSnapshot(snapshot, project.projectId, operatorName, timestamp);
    });
    return snapshot;
}
function getNodeActivityTime(node) {
    if (!node)
        return '';
...
function deriveProjectRuntimeState(project, snapshot, referenceTime = nowText()) {
    ...
    const blockingNode = nodes.find((node) => !isClosedNodeStatus(node.currentStatus) && isBlockingIssue(node)) ??
    ...
    const blockedReason = blockedFlag
        ? blockingNode?.currentIssueText || blockingNode?.latestResultText || ''
        : project.projectStatus === '已终止'
            ? getTerminationReason(nodes)
            : '';
    const delayedPendingDecision = Boolean(pendingDecisionNode) && !blockedFlag && delayedPendingDecisionDays >= 2 && project.projectStatus === '进行中';
    const riskStatus = blockingDurationDays >= 2 || delayedPendingDecision ? '延期' : '正常';
...
function buildProjectViewRecord(project, snapshot, referenceTime = nowText()) {
    return {
        ...cloneProject(project),
        ...deriveProjectRuntimeState(project, snapshot, referenceTime),
    };
}
function getProjectCreateCatalogInternal() {
...
function buildProjectNodes(projectId, createdAt, ownerId, ownerName, template) {
...
export function getProjectCreateCatalog() {
...
export function validateProjectCreateDraft(draft) {
    const errors = [];
    ...
    if (!draft.styleType)
        errors.push('请选择款式类型。');
    if (!draft.projectSourceType)
        errors.push('请选择项目来源类型。');
    if (!draft.templateId)
        errors.push('请选择项目模板。');
    if (!draft.categoryId)
        errors.push('请选择一级分类。');
    if (!draft.brandId)
        errors.push('请选择品牌。');
    if (!draft.yearTag.trim())
        errors.push('请选择年份。');
    if (!draft.priceRangeLabel.trim())
        errors.push('请选择价格带。');
    if (draft.targetChannelCodes.length === 0)
        errors.push('请选择目标测款渠道。');
    if (!draft.ownerId)
        errors.push('请选择负责人。');
    if (catalog.teams.length > 0 && !draft.teamId)
        errors.push('请选择执行团队。');
    if (draft.templateId) {
...
export function getProjectIdentityById(projectId) {
...
export function findProjectByCode(projectCode) {
    const snapshot = readSnapshot();
    const project = snapshot.projects.find((item) => item.projectCode === projectCode);
    return project ? buildProjectViewRecord(project, snapshot) : null;
}
export function listProjectPhases(projectId) {
    return readSnapshot()
        .phases
        .filter((item) => item.projectId === projectId)
        .sort((a, b) => a.phaseOrder - b.phaseOrder)
...
export function listProjectNodes(projectId) {
    return readSnapshot()
        .nodes
        .filter((item) => item.projectId === projectId)
        .sort((a, b) => {
...
export function getProjectNodeSequenceBlocker(projectId, projectNodeId) {
...
export function repairAllProjectNodeSequences(operatorName = '系统修复', timestamp = nowText()) {
    const snapshot = readSnapshot();
...
export function findProjectNodeById(projectId, projectNodeId) {
...
export function getProjectNodeRecordById(projectId, projectNodeId) {
    const node = readSnapshot().nodes.find((item) => item.projectId === projectId && item.projectNodeId === projectNodeId);
    return node ? cloneNode(node) : null;
}
export function findProjectNodeByWorkItemTypeCode(projectId, workItemTypeCode) {
...
export function getProjectNodeRecordByWorkItemTypeCode(projectId, workItemTypeCode) {
    const node = readSnapshot()
        .nodes
        .filter((item) => item.projectId === projectId && item.workItemTypeCode === workItemTypeCode)
        .sort((a, b) => a.sequenceNo - b.sequenceNo)[0];
...
export function createProject(input, operatorName = '当前用户') {
...
export function approveProjectInit(projectId, operatorName = '当前用户') {
    const project = getProjectById(projectId);
    if (!project) {
        return {
            ok: false,
    ...
    const timestamp = nowText();
    const nextProject = updateProjectRecord(projectId, {
        projectStatus: '已立项',
        updatedAt: timestamp,
    }, operatorName);
    const nextProjectInitNode = updateProjectNodeRecord(projectId, projectInitNode.projectNodeId, {
        currentStatus: '已完成',
    ...
    const nextNode = updateProjectNodeRecord(projectId, sampleAcquireNode.projectNodeId, {
        currentStatus: '进行中',
...
export function updateProjectNodeRecord(projectId, projectNodeId, patch, operatorName = '系统回写') {
    const snapshot = readSnapshot();
    ...
    const nextProjects = snapshot.projects.map((project) => project.projectId === projectId
        ? {
            ...project,
...
export function updateProjectPhaseRecord(projectId, projectPhaseId, patch) {
    const snapshot = readSnapshot();
    const phaseIndex = snapshot.phases.findIndex((item) => item.projectId === projectId && item.projectPhaseId === projectPhaseId);
    if (phaseIndex < 0)
        return null;
...
export function updateProjectRecord(projectId, patch, operatorName = '系统回写') {
    const snapshot = readSnapshot();
...
export function updateProjectAlbumUrls(projectId, imageUrls, operatorName = '当前用户') {
...
export function replaceProjectStore(snapshot) {
    persistSnapshot(snapshot);
}
export function getProjectCategoryChildren(categoryId) {
    return findCategoryNodeById(categoryId)?.children.map((item) => ({ ...item })) ?? [];
}
export function listActiveProjectTemplates() {
    return listProjectTemplates().filter((template) => template.status === 'active');
}
export function getChannelNamesByCodes(codes) {
    return findChannelNames(codes);
}
export function getProjectOptionNameById(type, id) {
    if (type === 'brand') {
