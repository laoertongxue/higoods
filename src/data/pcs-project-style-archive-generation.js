import { bindStyleArchiveToProjectChannelProduct, listProjectChannelProductsByProjectId, } from './pcs-channel-product-project-repository.ts';
...
import { createStyleArchiveShell, findStyleArchiveByProjectId, getStyleArchiveById, listStyleArchives, updateStyleArchive, } from './pcs-style-archive-repository.ts';
import { resolveStyleArchiveImageSelection, } from './pcs-style-archive-image-selection.ts';
const STYLE_ARCHIVE_REQUIRED_FIELDS = [
...
function buildStyleRelation(projectId, style, operatorName) {
...
function getExistingStyle(projectId) {
    const project = getProjectById(projectId);
    if (!project)
        return null;
...
function collectMissingFields(style) {
...
export function getStyleArchiveGenerationStatus(projectId) {
    const project = getProjectById(projectId);
    if (!project) {
    ...
    const passed = conclusionNode?.latestResultType === '测款通过' ||
...
export function generateStyleArchiveFromProjectNode(projectId, operatorName = '当前用户', input) {
    ...
    try {
        ...
        bindStyleArchiveToProjectChannelProduct(project.projectId, {
            styleId: created.styleId,
            styleCode: created.styleCode,
            styleName: created.styleName,
        }, operatorName);
        const channelCount = listProjectChannelProductsByProjectId(project.projectId).filter((item) => item.channelProductStatus !== '已作废').length;
...
export function getStyleArchiveFormalizationCheck(styleId) {
...
export function formalizeStyleArchive(styleId, operatorName = '当前用户') {
