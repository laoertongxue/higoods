...
import { TECH_PACK_AGGREGATE_STATUS_RULES, resolveTechPackVersionBusinessStatus, } from './pcs-product-lifecycle-governance.ts';
...
import { getCurrentTechPackVersionByStyleId, getTechnicalDataVersionById, getTechnicalDataVersionContent, listTechnicalDataVersionsByProjectId, listTechnicalDataVersionsByStyleId, } from './pcs-technical-data-version-repository.ts';
export function getTechnicalVersionStatusLabel(status) {
    if (status === 'PUBLISHED')
        return '已发布待启用';
    if (status === 'ARCHIVED')
        return '已归档';
...
export function getTechnicalVersionBusinessStatusLabel(record, currentVersionId) {
    return TECH_PACK_AGGREGATE_STATUS_RULES[resolveTechPackVersionBusinessStatus(record, currentVersionId)].label;
}
export function getTechnicalDomainStatusLabel(status) {
    if (status === 'COMPLETE')
        return '已完成';
    if (status === 'DRAFT')
        return '草稿中';
...
export function canPublishTechnicalVersion(record) {
    return record.missingItemCodes.length === 0;
}
export function buildTechnicalVersionListByStyle(styleId) {
...
export function buildTechnicalVersionListByProject(projectId) {
...
export function buildTechnicalVersionDetailViewModel(technicalVersionId) {
    ...
    return {
        ...
        sourceProjectText: record.sourceProjectCode
            ? `${record.sourceProjectCode} · ${record.sourceProjectName}`
            : '未绑定商品项目',
        versionStatusLabel: getTechnicalVersionBusinessStatusLabel(record, getStyleArchiveById(record.styleId)?.currentTechPackVersionId || ''),
...
export function resolveFcsCompatibleTechnicalVersion(spuCode) {
    const style = getStyleArchiveById(spuCode) ??
...
export function buildTechnicalVersionSourceSummary(styleId) {
    const style = getStyleArchiveById(styleId);
    if (!style)
        return null;
