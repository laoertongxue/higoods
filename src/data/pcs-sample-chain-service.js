function nextLineId(prefix, index) {
    return `${prefix}-${String(index + 1).padStart(2, '0')}`;
}
function createPlanLine(lineId, sampleRole, materialMode, linkedSampleCode = '') {
    return {
        lineId,
        sampleRole,
        materialMode,
...
export function createReuseFirstSamplePlanLine(linkedSampleCode = '') {
    return createPlanLine('reuse-first-sample-01', '复用首版结论', '沿用首版', linkedSampleCode);
}
export function createDefaultSamplePlanLines(mode, linkedSampleCode = '') {
    if (mode === '替代布与正确布双确认') {
        return [
...
export function normalizeSamplePlanLines(mode, lines, linkedSampleCode = '') {
...
export function getFirstOrderSampleChainMissingFields(task) {
    const missing = [];
    if (!task.sampleChainMode)
        missing.push('首单确认方式');
    if (task.sampleChainMode === '复用首版结论') {
        if (!task.sourceFirstSampleTaskCode && !task.sourceFirstSampleTaskId)
            missing.push('来源首版任务');
        if (!task.samplePlanLines.some((line) => line.sampleRole === '复用首版结论'))
            missing.push('复用首版结论记录');
    }
    if (task.sampleChainMode === '新增首单样衣确认') {
        if (!task.samplePlanLines.some((line) => line.sampleRole === '正确布确认样'))
            missing.push('正确布首单确认样');
    }
    if (task.sampleChainMode === '替代布与正确布双确认') {
        if (!task.samplePlanLines.some((line) => line.sampleRole === '替代布确认样'))
            missing.push('替代布确认样');
        if (!task.samplePlanLines.some((line) => line.sampleRole === '正确布确认样'))
            missing.push('正确布确认样');
    }
