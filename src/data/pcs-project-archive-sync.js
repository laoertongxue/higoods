import { getProjectById, getProjectNodeRecordByWorkItemTypeCode, updateProjectRecord, } from './pcs-project-repository.ts';
...
import { deleteProjectArchiveDocument, getProjectArchiveById, getProjectArchiveByProjectId, listProjectArchiveDocumentsByArchiveId, listProjectArchiveFilesByArchiveId, listProjectArchives, replaceProjectArchiveDocuments, replaceProjectArchiveFiles, replaceProjectArchiveMissingItems, upsertProjectArchive, upsertProjectArchiveDocuments, upsertProjectArchiveFiles, upsertProjectArchivePendingItem, } from './pcs-project-archive-repository.ts';
function nowText() {
    const now = new Date();
...
function buildArchiveId(sequence) {
    return `project_archive_${String(sequence).padStart(4, '0')}`;
}
function buildArchiveNo(datePart, sequence) {
    return `ARC-${datePart}-${String(sequence).padStart(3, '0')}`;
}
function nextArchiveIdentity(timestamp) {
...
function buildPendingItem(projectCode, sourceCode, reason) {
    return {
...
function buildRelation(archive, operatorName) {
...
function dedupeManualDocuments(documents, files) {
    const keptDocuments = [];
    const keptFiles = [];
    const seen = new Set();
...
function computeArchiveSnapshot(archive) {
    ...
    const existingManualFiles = listProjectArchiveFilesByArchiveId(archive.projectArchiveId).filter((item) => existingManualDocuments.some((doc) => doc.archiveDocumentId === item.archiveDocumentId));
...
function markCollectedTechnicalVersions(documents, timestamp) {
    const versionIds = new Set(documents
...
function updateProjectAndNodeFromArchive(archive, operatorName, mode = 'sync') {
    updateProjectRecord(archive.projectId, {
...
export function createProjectArchive(projectId, operatorName = '商品中心') {
    ...
    const existingArchive = (project.projectArchiveId ? getProjectArchiveById(project.projectArchiveId) : null) ||
...
export function syncProjectArchive(projectArchiveId, operatorName = '商品中心') {
    const current = getProjectArchiveById(projectArchiveId);
    if (!current)
        throw new Error('未找到项目资料归档对象。');
    const timestamp = nowText();
...
export function syncExistingProjectArchiveByProjectId(projectId, operatorName = '系统同步') {
    ...
    try {
        ...
        upsertProjectArchivePendingItem(buildPendingItem(archive.projectCode, archive.archiveNo, error instanceof Error ? error.message : '项目资料归档自动同步失败。'));
    }
}
export function uploadProjectArchiveManualDocument(projectArchiveId, input, operatorName = '商品中心') {
    ...
    const existingManualFiles = listProjectArchiveFilesByArchiveId(projectArchiveId).filter((item) => existingManualDocuments.some((doc) => doc.archiveDocumentId === item.archiveDocumentId));
    const duplicateKey = [
        projectArchiveId,
        input.documentGroup,
        input.title.trim(),
...
export function deleteProjectArchiveManualDocument(projectArchiveId, archiveDocumentId, operatorName = '商品中心') {
    const archive = getProjectArchiveById(projectArchiveId);
    if (!archive)
        throw new Error('未找到项目资料归档对象。');
...
export function finalizeProjectArchive(projectArchiveId, operatorName = '商品中心') {
    const archive = getProjectArchiveById(projectArchiveId);
    if (!archive)
        throw new Error('未找到项目资料归档对象。');
