import { getProjectById, getProjectNodeRecordByWorkItemTypeCode, updateProjectNodeRecord, } from './pcs-project-repository.ts';
...
import { getFirstSampleTaskById, listFirstSampleTasks, upsertFirstSampleTask, upsertFirstSampleTaskPendingItem, } from './pcs-first-sample-repository.ts';
import { getPatternTaskById, listPatternTasks, updatePatternTask, upsertPatternTask, upsertPatternTaskPendingItem, } from './pcs-pattern-task-repository.ts';
import { assertPatternTaskMemberInTeam, getPatternTaskMember, getPatternTaskTeamName, } from './pcs-pattern-task-team-config.ts';
...
import { buildFirstSampleProjectMeta, syncFirstSampleTaskToProjectNode, } from './pcs-first-sample-project-writeback.ts';
import { getPlateMakingTaskById, listPlateMakingTasks, updatePlateMakingTask, upsertPlateMakingTask, upsertPlateMakingTaskPendingItem, } from './pcs-plate-making-repository.ts';
import { getFirstOrderSampleTaskById, listFirstOrderSampleTasks, upsertFirstOrderSampleTask, upsertFirstOrderSampleTaskPendingItem, } from './pcs-first-order-sample-repository.ts';
import { buildFirstOrderProjectMeta, syncFirstOrderSampleTaskToProjectNode, } from './pcs-first-order-sample-project-writeback.ts';
import { createDefaultSamplePlanLines, normalizeSamplePlanLines, } from './pcs-sample-chain-service.ts';
import { getRevisionTaskById, listRevisionTasks, updateRevisionTask, upsertRevisionTask, upsertRevisionTaskPendingItem, } from './pcs-revision-task-repository.ts';
...
function normalizePatternAssignment(input) {
...
function normalizePatternDemandSource(input) {
    if (input.demandSourceType)
        return input.demandSourceType;
    if (input.sourceType === '改版任务')
        return '改版任务';
    if (input.sourceType === '花型复用调色')
        return '设计师款';
    return '预售测款通过';
}
function plateExecutionFields(input, existing) {
    return {
...
function revisionExecutionFields(input, existing) {
...
function nextCode(prefix, currentCount) {
    return `${prefix}-${dateKey()}-${String(currentCount + 1).padStart(3, '0')}`;
}
function makePendingItem(taskType, rawTaskCode, rawProjectField, rawSourceField, reason) {
    return {
...
function makeRelationId(projectId, projectNodeId, sourceModule, sourceObjectId) {
    return `rel_${projectId}_${projectNodeId}_${sourceModule}_${sourceObjectId}`.replace(/[^a-zA-Z0-9]/g, '_');
}
function syncTaskCompletionToProjectNode(input) {
    const project = getProjectById(input.projectId);
    if (!project || !input.projectNodeId)
        return;
    const node = getProjectNodeRecordByWorkItemTypeCode(project.projectId, input.workItemTypeCode) ||
        null;
    if (!node || node.projectNodeId !== input.projectNodeId)
        return;
    upsertProjectRelation(relationPayload({
...
function getProjectOrPending(taskType, projectId, taskCode, rawSourceField) {
    const project = getProjectById(projectId);
    if (project)
        return { project, pendingItem: null };
    return {
        project: null,
        pendingItem: makePendingItem(taskType, taskCode, projectId, rawSourceField, '当前商品项目不存在，不能正式创建任务。'),
    };
}
function getNodeOrPending(taskType, projectId, projectCode, taskCode, workItemTypeCode) {
    const node = getProjectNodeRecordByWorkItemTypeCode(projectId, workItemTypeCode);
    if (node)
        return { node, pendingItem: null };
    return {
        node: null,
        pendingItem: makePendingItem(taskType, taskCode, projectCode, workItemTypeCode, '当前项目未配置对应项目节点，不能正式创建任务。'),
    };
}
function blockCancelledNode(taskType, taskCode, projectCode, node) {
...
function resolveUpstreamForProjectTemplate(project) {
    return {
        upstreamModule: '项目模板',
...
function ensureFormalSource(taskType, sourceType, upstreamObjectId, upstreamObjectCode, fallbackSourceField) {
    if (sourceType === '人工创建') {
...
function resolveRevisionStyle(input) {
    if (input.styleId)
        return getStyleArchiveById(input.styleId);
    return (findStyleArchiveByCode(input.styleCode || '') ||
        findStyleArchiveByCode(input.productStyleCode || '') ||
        findStyleArchiveByCode(input.spuCode || '') ||
        null);
}
function resolvePlateStyle(input) {
    if (input.styleId)
        return getStyleArchiveById(input.styleId);
    return (findStyleArchiveByCode(input.styleCode || '') ||
        findStyleArchiveByCode(input.productStyleCode || '') ||
...
function resolvePatternStyle(input) {
    if (input.styleId)
        return getStyleArchiveById(input.styleId);
    return (findStyleArchiveByCode(input.styleCode || '') ||
...
function hasRevisionPrintScope(input) {
    return Boolean(input.revisionScopeCodes?.includes('PRINT') ||
...
function updateRevisionNode(node, task, alreadyExists) {
    if (!alreadyExists) {
...
function updateTaskNode(node, task, input, alreadyExists) {
    if (!alreadyExists) {
        updateProjectNodeRecord(task.projectId, node.projectNodeId, {
            currentStatus: '进行中',
...
export function saveRevisionTaskDraft(input) {
...
export function savePlateMakingTaskDraft(input) {
    const now = nowTaskText();
...
export function savePatternTaskDraft(input) {
    const now = nowTaskText();
...
function buildFirstSampleCode(targetSite, count) {
    return `FS-RESULT-${targetSite === '雅加达' ? 'JKT' : 'SZ'}-${String(count + 21).padStart(4, '0')}`;
}
function buildFirstOrderSampleCode(targetSite, count) {
    return `FO-RESULT-${targetSite === '雅加达' ? 'JKT' : 'SZ'}-${String(count + 51).padStart(4, '0')}`;
}
function firstSampleChainFields(input, existing) {
...
function resolveSourceFirstSample(input, projectId) {
    const fromInput = {
    ...
    if (fromInput.sourceFirstSampleTaskId || fromInput.sourceFirstSampleTaskCode || fromInput.sourceFirstSampleCode)
        return fromInput;
    const matched = listFirstSampleTasks()
        .filter((task) => task.projectId === projectId)
        .find((task) => task.firstSampleTaskId === input.upstreamObjectId ||
...
function firstOrderChainFields(input, projectId, existing) {
    ...
    return {
        ...
        samplePlanLines: normalizeSamplePlanLines(sampleChainMode, input.samplePlanLines || existing?.samplePlanLines || defaultLines, sourceFirst.sourceFirstSampleCode),
...
export function saveFirstSampleTaskDraft(input) {
    const now = nowTaskText();
...
export function saveFirstOrderSampleTaskDraft(input) {
    const now = nowTaskText();
...
export function createRevisionTaskWithProjectRelation(input) {
    ...
    if (requiresProject) {
        ...
        const defaultUpstreamNode = getProjectNodeRecordByWorkItemTypeCode(project.projectId, 'TEST_CONCLUSION') ||
        ...
        const upstreamError = ensureFormalSource('改版任务', input.sourceType, input.referenceObjectId || input.referenceObjectCode || '', input.referenceObjectCode || input.referenceObjectName || '', input.referenceObjectId || input.referenceObjectCode || input.referenceObjectName || '');
        if (upstreamError) {
    ...
    const task = upsertRevisionTask({
        ...
        workItemTypeName: '改版任务',
        sourceType: input.sourceType,
        upstreamModule: input.sourceType === '既有商品改款'
            ? '款式档案'
    ...
    let relation = null;
    if (project && node) {
        relation = upsertProjectRelation(relationPayload({
            projectId: project.projectId,
            projectCode: project.projectCode,
            projectNodeId: node.projectNodeId,
            workItemTypeCode: 'REVISION_TASK',
...
function createPlateMakingTaskStandalone(input) {
...
function resolvePlateUpstream(project, input) {
    if (input.sourceType === '项目模板阶段')
        return resolveUpstreamForProjectTemplate(project);
...
export function createPlateMakingTaskWithProjectRelation(input) {
    const rawCode = input.plateTaskCode || input.plateTaskId || input.title;
    ...
    const relation = upsertProjectRelation(relationPayload({
        projectId: project.projectId,
        projectCode: project.projectCode,
        projectNodeId: node.projectNodeId,
        workItemTypeCode: 'PATTERN_TASK',
...
export function createPlateMakingTask(input) {
    if (input.projectId)
        return createPlateMakingTaskWithProjectRelation(input);
    return createPlateMakingTaskStandalone(input);
}
function resolvePatternUpstream(project, input) {
    if (input.sourceType === '项目模板阶段')
        return resolveUpstreamForProjectTemplate(project);
    if (input.sourceType === '花型复用调色') {
        return {
...
export function createPatternTaskWithProjectRelation(input) {
...
function createPatternTaskStandalone(input) {
    const rawCode = input.patternTaskCode || input.patternTaskId || input.title;
    const style = resolvePatternStyle(input);
    if (!style) {
...
export function createPatternTask(input) {
    if (input.projectId)
        return createPatternTaskWithProjectRelation(input);
    return createPatternTaskStandalone(input);
}
export function createRevisionTask(input) {
    return createRevisionTaskWithProjectRelation(input);
}
export function completeRevisionTaskWithProjectRelationSync(revisionTaskId, operatorName = '当前用户') {
    const task = getRevisionTaskById(revisionTaskId);
    if (!task)
        return { ok: false, task: null, message: '未找到改版任务。' };
    if (!task.projectId || !task.projectNodeId) {
        return { ok: false, task, message: '当前改版任务未关联正式商品项目节点。' };
    }
    if (task.status === '已取消')
        return { ok: false, task, message: '当前改版任务已取消，不能完成。' };
    ...
    if (!nextTask)
        return { ok: false, task, message: '改版任务更新失败。' };
...
export function completeRevisionTask(revisionTaskId, operatorName = '当前用户') {
    const task = getRevisionTaskById(revisionTaskId);
    if (!task)
        return { ok: false, task: null, message: '未找到改版任务。' };
    if (task.projectId && task.projectNodeId)
        return completeRevisionTaskWithProjectRelationSync(revisionTaskId, operatorName);
    if (task.status === '已取消')
        return { ok: false, task, message: '当前改版任务已取消，不能完成。' };
    const missing = getRevisionTaskCompletionMissingFields(task);
    if (missing.length > 0)
        return { ok: false, task, message: `缺少字段：${missing.join('、')}。` };
    const nextTask = updateRevisionTask(revisionTaskId, {
        status: '已完成',
...
export function completePlateMakingTaskWithProjectRelationSync(plateTaskId, operatorName = '当前用户') {
    const task = getPlateMakingTaskById(plateTaskId);
    if (!task)
        return { ok: false, task: null, message: '未找到制版任务。' };
    if (!task.projectId || !task.projectNodeId) {
        return { ok: false, task, message: '当前制版任务未关联正式商品项目节点。' };
    }
    if (task.status === '已取消')
        return { ok: false, task, message: '当前制版任务已取消，不能完成。' };
    ...
    if (!nextTask)
        return { ok: false, task, message: '制版任务更新失败。' };
    syncTaskCompletionToProjectNode({
        projectId: nextTask.projectId,
        projectNodeId: nextTask.projectNodeId,
        workItemTypeCode: 'PATTERN_TASK',
...
export function completePlateMakingTask(plateTaskId, operatorName = '当前用户') {
    const task = getPlateMakingTaskById(plateTaskId);
    if (!task)
        return { ok: false, task: null, message: '未找到制版任务。' };
    if (task.projectId && task.projectNodeId)
        return completePlateMakingTaskWithProjectRelationSync(plateTaskId, operatorName);
    if (task.status === '已取消')
        return { ok: false, task, message: '当前制版任务已取消，不能完成。' };
...
export function completePatternTaskWithProjectRelationSync(patternTaskId, operatorName = '当前用户') {
    const task = getPatternTaskById(patternTaskId);
    if (!task)
        return { ok: false, task: null, message: '未找到花型任务。' };
    if (!task.projectId || !task.projectNodeId) {
        return { ok: false, task, message: '当前花型任务未关联正式商品项目节点。' };
    }
    if (task.status === '已取消')
        return { ok: false, task, message: '当前花型任务已取消，不能完成。' };
    ...
    if (!nextTask)
        return { ok: false, task, message: '花型任务更新失败。' };
    syncTaskCompletionToProjectNode({
        projectId: nextTask.projectId,
        projectNodeId: nextTask.projectNodeId,
        workItemTypeCode: 'PATTERN_ARTWORK_TASK',
...
export function completePatternTask(patternTaskId, operatorName = '当前用户') {
    const task = getPatternTaskById(patternTaskId);
    if (!task)
        return { ok: false, task: null, message: '未找到花型任务。' };
    if (task.projectId && task.projectNodeId)
        return completePatternTaskWithProjectRelationSync(patternTaskId, operatorName);
    if (task.status === '已取消')
        return { ok: false, task, message: '当前花型任务已取消，不能完成。' };
...
export function syncExistingProjectEngineeringTaskNodes(operatorName = '系统同步') {
    listRevisionTasks()
...
export function createFirstSampleTaskWithProjectRelation(input) {
    ...
    const upstreamError = ensureFormalSource('首版样衣打样', input.sourceType, input.upstreamObjectId || '', input.upstreamObjectCode || '', manualProjectSource ? project.projectCode : '');
    if (upstreamError) {
...
export function createFirstOrderSampleTaskWithProjectRelation(input) {
    ...
    const manualProjectSource = input.sourceType === '人工创建';
    const upstreamError = ensureFormalSource('首单样衣打样', input.sourceType, input.upstreamObjectId || '', input.upstreamObjectCode || '', manualProjectSource ? project.projectCode : '');
    if (upstreamError) {
...
export function createDownstreamTasksFromRevision(revisionTaskId, selectedTypes) {
    ...
    const existingPatternTask = listPatternTasks().find((item) => item.upstreamObjectId === revisionTask.revisionTaskId || item.upstreamObjectCode === revisionTask.revisionTaskCode);
    ...
    return {
        ...
        createdTaskCodes: results.filter((item) => item.ok).map((item) => {
            const task = item.task;
            return (task.plateTaskCode ||
