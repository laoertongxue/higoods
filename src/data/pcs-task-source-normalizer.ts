...
function migrateLegacyProjectSourceType(value: string | null | undefined): string | null | undefined {
  if (value === '项目模板阶段' || value === '商品项目') return '商品项目'
  return value
}

export function normalizePlateTaskSourceType(value: string | null | undefined): PlateMakingTaskSourceType {
  const migratedValue = migrateLegacyProjectSourceType(value)
  if (migratedValue === '商品项目') return '商品项目'
