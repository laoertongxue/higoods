...
function createNodeStatus(statusName, entryConditions, exitConditions, businessMeaning) {
    return {
        statusName,
        entryConditions,
        exitConditions,
...
function createMultiInstanceDefinition(input) {
    return {
        ...input,
...
const PCS_PROJECT_MULTI_INSTANCE_DEFINITION_MAP = {
...
function resolveProjectWorkItemMultiInstanceDefinition(contract) {
    const found = PCS_PROJECT_MULTI_INSTANCE_DEFINITION_MAP[contract.workItemTypeCode];
    if (!contract.capabilities.canMultiInstance)
        return null;
    if (!found) {
...
const EXECUTE_NODE_STATUS_DEFINITIONS = [
...
const CHANNEL_LISTING_NODE_STATUS_DEFINITIONS = [
...
function buildStatusOptions(definition) {
...
function buildFieldGroups(definition) {
...
function createWorkItemConfig(definition) {
...
export const PCS_PROJECT_PHASE_CONTRACTS = [
    {
        phaseCode: 'PHASE_01',
        phaseName: '立项获取',
        phaseOrder: 1,
...
export const PCS_PROJECT_COMMON_INSTANCE_FIELDS = [
...
export const PCS_PROJECT_WORK_ITEM_CONTRACTS = [
    {
        workItemId: 'WI-001',
...
export const PCS_PROJECT_TEMPLATE_SCHEMAS = [
    {
        templateId: 'TPL-001',
...
export const PCS_PROJECT_CONFIG_SOURCE_MAPPINGS = [
...
export const PCS_PROJECT_RELATED_INSTANCE_TYPES = [
...
export const PCS_PROJECT_WORK_ITEM_LEGACY_MAPPINGS = [
...
export function listProjectPhaseContracts() {
...
export function getProjectPhaseContract(phaseCode) {
...
export function listProjectWorkItemContracts() {
...
export function getProjectWorkItemContract(workItemTypeCode) {
    const found = WORK_ITEM_MAP.get(workItemTypeCode);
    if (!found) {
        throw new Error(`未找到项目工作项契约：${workItemTypeCode}`);
    }
    return listProjectWorkItemContracts().find((item) => item.workItemTypeCode === workItemTypeCode);
}
export function getProjectWorkItemMultiInstanceDefinition(workItemTypeCode) {
    return getProjectWorkItemContract(workItemTypeCode).multiInstanceDefinition ?? null;
}
export function getProjectWorkItemContractById(workItemId) {
    const found = WORK_ITEM_ID_MAP.get(workItemId);
    if (!found)
        return null;
    return getProjectWorkItemContract(found.workItemTypeCode);
}
export function listProjectWorkItemFieldDefinitions(workItemTypeCode) {
    return getProjectWorkItemContract(workItemTypeCode).fieldDefinitions.map((field) => ({
        ...field,
        options: field.options ? [...field.options] : undefined,
    }));
}
export function listProjectWorkItemFieldGroups(workItemTypeCode) {
    const groups = new Map();
...
export function listProjectTemplateSchemas() {
...
export function getProjectTemplateSchema(templateId) {
    const found = TEMPLATE_MAP.get(templateId);
    if (!found) {
        throw new Error(`未找到项目模板契约：${templateId}`);
    }
    return listProjectTemplateSchemas().find((item) => item.templateId === templateId);
}
export function listProjectConfigSourceMappings() {
    return PCS_PROJECT_CONFIG_SOURCE_MAPPINGS.map((item) => ({ ...item }));
}
export function getProjectConfigSourceMapping(fieldKey) {
    const found = CONFIG_SOURCE_MAP.get(fieldKey);
    return found ? { ...found } : null;
}
export function listProjectRelatedInstanceTypes() {
    return PCS_PROJECT_RELATED_INSTANCE_TYPES.map((item) => ({ ...item }));
}
export function buildBuiltinProjectWorkItemConfigs() {
    return PCS_PROJECT_WORK_ITEM_CONTRACTS.map((item) => createWorkItemConfig(item));
}
export function buildBuiltinProjectTemplateMatrix() {
    return PCS_PROJECT_TEMPLATE_SCHEMAS.map((schema) => {
        ...
        const nodes = schema.phaseSchemas.flatMap((phase) => phase.nodeCodes.map((code, index) => {
...
export function resolveLegacyProjectWorkItemCode(legacyNameOrCode) {
    const normalized = legacyNameOrCode.trim();
    const matched = PCS_PROJECT_WORK_ITEM_LEGACY_MAPPINGS.find((item) => item.legacyName === normalized || item.legacyCode === normalized);
