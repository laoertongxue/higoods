...
const FALLBACK_PROJECTS = [
    ...
    { projectId: 'prj_20251216_019', projectCode: 'PRJ-20251216-019', projectName: '基础款毛织开衫待确认', styleType: '基础款', categoryName: '开衫', subCategoryName: '开衫', styleTagNames: ['休闲', '通勤'], targetChannelCodes: ['wechat-mini-program', 'tiktok-shop'], projectStatus: '进行中', currentPhaseName: '商品上架与市场测款', ownerName: '张丽', updatedAt: '2026-04-02 10:30', progressDone: 12, progressTotal: 20, nextWorkItemName: '测款结论判定', nextWorkItemStatus: '待确认', pendingDecisionFlag: true, riskStatus: '正常', riskReason: '历史测款结论待重新确认，请重新选择通过或淘汰。' },
...
function cloneProjectListRecord(record) {
    return {
        ...record,
...
function buildRuntimeRecord(project, snapshot) {
...
function readSnapshot() {
    ...
    try {
        ...
        if (!Array.isArray(parsed.projects) || !Array.isArray(parsed.nodes))
            return null;
        return parsed;
    }
    catch {
...
export function getChannelNamesByCodes(channelCodes) {
    return channelCodes.map((code) => CHANNEL_NAME_MAP[code] || code);
}
export function listProjectListRecords() {
