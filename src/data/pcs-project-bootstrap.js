...
import { listProjectWorkspaceAges, listProjectWorkspaceBrands, listProjectWorkspaceCategories, listProjectWorkspaceCrowdPositioning, listProjectWorkspaceCrowds, listProjectWorkspaceProductPositioning, listProjectWorkspaceStyleCodes, listProjectWorkspaceStyles, } from './pcs-project-config-workspace-adapter.ts';
...
const STYLE_TYPE_BRAND_INDEX = {
    基础款: 0,
    快时尚款: 1,
    改版款: 2,
    设计款: 3,
};
const STYLE_TYPE_PRODUCT_POSITIONING_INDEX = {
    基础款: 0,
    快时尚款: 4,
    改版款: 2,
    设计款: 1,
...
function resolveCategoryOption(seed, fallbackIndex) {
...
function resolveStyleOptions(seed, fallbackIndex) {
    const matched = seed.styleTags
        .map((tag) => WORKSPACE_STYLES.find((option) => option.name === tag))
        .filter((item) => Boolean(item));
    if (matched.length > 0)
        return matched;
    const inferredKeywords = seed.styleType === '设计款'
        ? ['秀场', '优雅']
        : seed.styleType === '改版款'
            ? ['复古', '街头']
            : seed.styleType === '快时尚款'
    ...
    return inferredKeywords
        ...
        .slice(0, 2)
        .concat(matched.length === 0 && inferredKeywords.length === 0 ? [pickOptionByIndex(WORKSPACE_STYLES, fallbackIndex)] : []);
}
function resolveWorkspaceBundle(seed) {
    const sequence = Number.parseInt(seed.projectCode.slice(-3), 10) || 0;
    const category = resolveCategoryOption(seed, sequence);
    const brand = pickOptionByIndex(WORKSPACE_BRANDS, STYLE_TYPE_BRAND_INDEX[seed.styleType] + sequence);
    const styleCode = findOptionByKeywords(WORKSPACE_STYLE_CODES, [seed.styleNumber, seed.projectName, category.name]) ??
        pickOptionByIndex(WORKSPACE_STYLE_CODES, sequence);
    const styleOptions = resolveStyleOptions(seed, sequence);
    const crowdPositioning = seed.targetAudienceTags.some((tag) => tag.includes('穆斯林'))
    ...
    const ages = [
        pickOptionByIndex(WORKSPACE_AGES, seed.styleType === '基础款' || seed.styleType === '快时尚款' ? 0 : 1),
    ];
    const crowds = [
        pickOptionByIndex(WORKSPACE_CROWDS, seed.styleType === '基础款' || seed.styleType === '快时尚款' ? 0 : 1),
    ];
    const productPositioning = [
        pickOptionByIndex(WORKSPACE_PRODUCT_POSITIONING, STYLE_TYPE_PRODUCT_POSITIONING_INDEX[seed.styleType]),
    ];
    return {
        category,
        brand,
...
function createScenarioProjectSeed(sequence, input) {
    return {
        ...input,
...
function buildPhaseStatus(seed, phaseOrder) {
    if (seed.projectStatus === '已归档')
        return '已完成';
    if (seed.projectStatus === '已终止') {
        if (phaseOrder < seed.currentPhaseOrder)
            return '已完成';
        if (phaseOrder === seed.currentPhaseOrder)
            return '已终止';
        return '未开始';
...
const LEGACY_BOOTSTRAP_NODE_CODE_MAP = {
...
function normalizeBootstrapNodeCode(code) {
    if (!code)
        return '';
    return LEGACY_BOOTSTRAP_NODE_CODE_MAP[code] ?? code;
}
function buildProjectFromSeed(seed, currentPhaseCode, currentPhaseName) {
...
function buildBootstrapRecords(seed) {
    const template = getProjectTemplateById(seed.templateId);
    if (!template) {
    ...
    const phases = sortedStages.map((stage) => ({
        ...
        finishedAt: seed.projectStatus === '已归档' || stage.phaseOrder < seed.currentPhaseOrder
            ? seed.updatedAt
            : '',
        ownerId: seed.ownerId,
        ownerName: seed.ownerName,
    ...
    const currentStageNodeIndexByCode = new Map(currentStageNodes.map((item, index) => [item.workItemTypeCode, index]));
    ...
    const nodes = orderedNodes.map((item) => {
        ...
        const hasLatestResult = normalizedLatestNodeCode === item.workItemTypeCode ||
        ...
        return {
            ...
            latestResultType: normalizedLatestNodeCode === item.workItemTypeCode
                ? seed.latestResultType || '最近结果'
                : status === '已完成'
                    ? '节点完成'
                    : '',
            latestResultText: normalizedLatestNodeCode === item.workItemTypeCode
...
export function createBootstrapProjectSnapshot(version) {
