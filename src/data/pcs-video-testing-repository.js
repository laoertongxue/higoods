...
function loadSnapshot() {
    ...
    try {
        ...
        memorySnapshot = hydrateSnapshot({
            version: VIDEO_TESTING_STORE_VERSION,
            records: Array.isArray(parsed.records) ? parsed.records : buildSeedSnapshot().records,
        });
        localStorage.setItem(VIDEO_TESTING_STORAGE_KEY, JSON.stringify(memorySnapshot));
        return cloneSnapshot(memorySnapshot);
    }
...
export function getVideoTestingStoreSnapshot() {
    return loadSnapshot();
}
export function listVideoTestRecords() {
    return loadSnapshot().records.map(cloneRecord);
}
export function getVideoTestRecordById(videoRecordId) {
    const record = loadSnapshot().records.find((item) => item.videoRecordId === videoRecordId);
    return record ? cloneRecord(record) : null;
}
export function resetVideoTestingRepository() {
