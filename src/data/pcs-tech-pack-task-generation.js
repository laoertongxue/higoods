...
import { getProjectById, getProjectNodeRecordByWorkItemTypeCode, updateProjectNodeRecord, updateProjectRecord, } from './pcs-project-repository.ts';
...
import { findStyleArchiveByCode, findStyleArchiveByProjectId, getStyleArchiveById, updateStyleArchive, } from './pcs-style-archive-repository.ts';
import { createTechnicalDataVersionDraft, getCurrentTechPackVersionByStyleId, getNextStyleVersionMeta, getNextTechnicalVersionIdentity, getTechnicalDataVersionById, getTechnicalDataVersionContent, listTechnicalDataVersionsByStyleId, updateTechnicalDataVersionContent, updateTechnicalDataVersionRecord, } from './pcs-technical-data-version-repository.ts';
function nowText() {
    const now = new Date();
...
function cloneContent(content, technicalVersionId) {
    return {
        technicalVersionId,
...
function createPlatePatternFiles(task, operatorName) {
...
function createPlateProcessEntries(task) {
    return [
        {
...
function createPlateSizeRows(task) {
...
function createPlateQualityRules(task) {
    return [
        {
...
function createPlateColorMappings(task) {
    return [
        {
...
function buildPlateGeneratedContent(task, technicalVersionId, operatorName, baseContent) {
...
function buildArtworkDesign(task) {
    return [
        {
...
function ensureTaskNode(projectId, workItemTypeCode, workItemTypeName) {
...
function getProjectNodeBindingByTaskType(projectId, taskType) {
    if (taskType === 'MANUAL') {
...
function buildChangeSummaryFromRevision(task) {
...
function buildProjectRelationId(technicalVersionId) {
    return `rel_tech_pack_${technicalVersionId}`;
}
function getStyleTechPackStatus(versions, currentTechPackVersionId) {
    if (currentTechPackVersionId && versions.some((item) => item.technicalVersionId === currentTechPackVersionId)) {
        return '已启用';
    }
    if (versions.some((item) => item.versionStatus === 'PUBLISHED'))
        return '已发布待启用';
    if (versions.length > 0)
        return '草稿中';
    return '未建立';
}
export function writeProjectRelationFromTechPackVersion(record, operatorName = '当前用户', sourceTaskType = record.createdFromTaskType) {
...
export function syncStyleArchiveFromTechPackVersion(record) {
...
export function syncProjectFromTechPackVersion(record) {
    if (!record.sourceProjectId)
        return;
    updateProjectRecord(record.sourceProjectId, {
...
export function syncProjectSourceNodeFromTechPackVersion(record, operatorName = '当前用户', action = 'CREATED', sourceTaskType = record.createdFromTaskType) {
    if (!record.sourceProjectId)
        return;
    const nodeBinding = getProjectNodeBindingByTaskType(record.sourceProjectId, sourceTaskType);
    if (!nodeBinding.projectNodeId)
        return;
    ...
    updateProjectNodeRecord(record.sourceProjectId, nodeBinding.projectNodeId, {
        currentStatus: '进行中',
        ...
        latestResultText: isManualVersion
            ? action === 'WRITTEN'
                ? '已手动维护技术包版本内容。'
...
function finalizeGeneration(record, action, logType, operatorName, sourceTaskType = record.createdFromTaskType) {
...
function createPlateGeneratedContentFallback(technicalVersionId) {
    return {
        technicalVersionId,
        patternFiles: [],
        patternDesc: '',
...
function ensureStyleArchive(task, errorText) {
    const style = (task.styleId ? getStyleArchiveById(task.styleId) : null) ||
...
function ensurePlateTaskReady(task) {
    ...
    const hasPatternOutput = Boolean(task.patternVersion)
...
function ensurePatternTaskReady(task) {
    if (!isTechPackGenerationAllowedStatus(task.status)) {
        throw new Error(getTechPackGenerationBlockedReason(task.status) || '当前花型任务尚未确认产出，不能写入技术包。');
    }
    return task;
}
function ensureRevisionTaskReady(task) {
    if (!isTechPackGenerationAllowedStatus(task.status)) {
        throw new Error(getTechPackGenerationBlockedReason(task.status) || '当前改版任务尚未确认产出，不能建立技术包版本。');
    }
    return task;
}
function getLatestPlateWritableVersion(styleId) {
    return (listTechnicalDataVersionsByStyleId(styleId).find((item) => item.versionStatus === 'DRAFT' && item.primaryPlateTaskId) || null);
}
function buildPatternLibraryRefs(task, baseVersion) {
...
function buildPatternAssetRefs(task, baseVersion) {
    const asset = (task.patternAssetId ? listPatternAssets().find((item) => item.id === task.patternAssetId) : null) ||
...
function syncPatternAssetTechPackLineage(task, record, operatorName) {
    const asset = (task.patternAssetId ? listPatternAssets().find((item) => item.id === task.patternAssetId) : null) ||
        listPatternAssets().find((item) => item.source_task_id === task.patternTaskId);
    if (!asset)
        return;
...
export function generateTechPackVersionFromPlateTask(plateTaskId, operatorName = '当前用户') {
    const task = getPlateMakingTaskById(plateTaskId);
    if (!task)
        throw new Error('未找到制版任务。');
    ...
    const style = ensureStyleArchive({ styleId: '', styleCode: task.productStyleCode, projectId: task.projectId, spuCode: task.spuCode }, '当前制版任务未绑定正式款式档案，不能建立技术包版本。');
...
export function generateTechPackVersionFromPatternTask(patternTaskId, operatorName = '当前用户') {
    const task = getPatternTaskById(patternTaskId);
    if (!task)
        throw new Error('未找到花型任务。');
    ...
    const style = ensureStyleArchive({ styleId: '', styleCode: task.productStyleCode, projectId: task.projectId, spuCode: task.spuCode }, '当前花型任务未绑定正式款式档案，不能写入技术包。');
...
export function generateTechPackVersionFromRevisionTask(revisionTaskId, operatorName = '当前用户') {
    const task = getRevisionTaskById(revisionTaskId);
    if (!task)
        throw new Error('未找到改版任务。');
    ensureRevisionTaskReady(task);
    ensureTaskProject(task, '当前改版任务未绑定正式商品项目，不能建立技术包版本。');
    const sourceNode = ensureTaskNode(task.projectId, task.workItemTypeCode, task.workItemTypeName);
    const style = ensureStyleArchive({ styleId: task.styleId, styleCode: task.styleCode || task.productStyleCode, projectId: task.projectId, spuCode: task.spuCode }, '当前改版任务未绑定正式款式档案，不能建立技术包版本。');
...
export function isTechPackGenerationAllowedStatus(status) {
    return status === '已确认' || status === '已完成';
}
export function getTechPackGenerationBlockedReason(status) {
    if (isTechPackGenerationAllowedStatus(status))
        return '';
    return '当前任务尚未确认产出，不能建立技术包版本。';
}
export function getRevisionTechPackActionLabel() {
    return '生成改版技术包版本';
}
export function getPlateTechPackActionLabel() {
    return ['生成', '技术包版本'].join('');
}
function buildPatternTechPackDisabledLabel(reason) {
    if (reason.includes('未绑定正式款式档案'))
        return '待关联款式档案';
    if (reason.includes('先完成制版任务生成技术包'))
        return '待生成技术包';
    if (reason.includes('尚未确认产出'))
        return '待任务确认';
    return '写入技术包花型';
}
function resolvePatternTechPackTarget(patternTaskId) {
    const task = getPatternTaskById(patternTaskId);
    if (!task) {
        return {
            mode: 'WRITE',
...
function getPatternTechPackDisabledReason(patternTaskId) {
    return resolvePatternTechPackTarget(patternTaskId).disabledReason;
}
function resolvePatternTaskTargetMode(patternTaskId) {
    return resolvePatternTechPackTarget(patternTaskId).mode;
}
export function getPatternTechPackActionLabel(patternTaskId) {
    return resolvePatternTaskTargetMode(patternTaskId) === 'NEW_VERSION' ? '生成花型新版本' : '写入技术包花型';
}
export function getPatternTechPackActionMeta(patternTaskId) {
...
export function getTechPackVersionById(technicalVersionId) {
    return getTechnicalDataVersionById(technicalVersionId);
}
export function buildTechPackVersionSourceTaskSummary(record) {
