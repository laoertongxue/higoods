import { getProjectTemplateVersion } from './pcs-templates.ts';
import { getPcsWorkItemDefinition } from './pcs-work-items.ts';
function buildPendingActionText(status, workItemName) {
    if (status === '已完成')
        return '节点已完成';
    if (status === '进行中')
        return `当前请处理：${workItemName}`;
    if (status === '待确认')
        return `当前待确认：${workItemName}`;
    if (status === '已取消')
        return '节点已取消';
    return '待开始执行';
}
function buildInitialNodeStatus(sequenceIndex, workItemTypeCode) {
    if (sequenceIndex === 0 && workItemTypeCode === 'PROJECT_INIT')
        return '进行中';
    return '未开始';
}
