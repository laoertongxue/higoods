...
function parseVersionNo(versionLabel, fallback) {
...
function resolveSeedScenario(seed) {
    if (seed.demand.spuCode === 'SPU-2024-011' || seed.demand.spuCode === 'SPU-2024-012')
        return 'WHOLE_WOOL';
    if (seed.demand.spuCode === 'SPU-TEE-084')
        return 'PART_WOOL';
    if (seed.demand.spuCode === 'SPU-2024-004')
        return 'GARMENT_HEAT_TRANSFER';
    return 'DEFAULT';
}
function buildSpecialCraftConfig(craftCode, craftName, selectedTargetObject = '裁片部位') {
    return {
...
function buildContent(seed) {
    ...
    const buildPieceRows = (pieces) => pieces.map((piece) => ({
        ...piece,
    ...
    const specialCraftsBySpuAndPiece = {
        'SPU-2024-005': {
            后片: [
    ...
    const getPieceSpecialCrafts = (pieceName) => specialCraftsBySpuAndPiece[demand.spuCode]?.[pieceName] ?? [];
    ...
    const resolveColorMaterialInfo = (color, index) => {
...
function buildProject018Content(technicalVersionId) {
...
export function createTechnicalDataVersionBootstrapSnapshot(version) {
