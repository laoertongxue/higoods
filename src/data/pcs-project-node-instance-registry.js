...
import { getProjectWorkItemMultiInstanceDefinition, } from './pcs-project-domain-contract.ts';
function nowText() {
    const now = new Date();
...
function filterPrimaryInstances(instances, definition) {
    if (!definition)
        return instances;
    ...
    return instances.filter((instance) => {
        if (!primarySourceKinds.has(instance.sourceKind))
            return false;
        if (instance.sourceKind !== 'RELATION_OBJECT')
            return true;
        if (primaryRelationObjectTypes.size === 0)
            return true;
        return primaryRelationObjectTypes.has(instance.sourceObjectType);
    });
}
function filterSupportingInstances(instances, definition, primaryInstances) {
    if (!definition)
        return [];
    ...
    return instances.filter((instance) => {
        if (primaryKeys.has(instance.projectNodeInstanceId))
            return false;
        if (instance.sourceKind !== 'RELATION_OBJECT')
            return false;
        if (supportingRelationObjectTypes.size === 0)
            return false;
        return supportingRelationObjectTypes.has(instance.sourceObjectType);
    });
}
function buildInlineRecordTitle(record) {
    if (record.workItemTypeCode !== 'SAMPLE_SHOOT_FIT') {
        return record.workItemTypeName;
    }
    const payload = (record.payload && typeof record.payload === 'object' ? record.payload : {});
    const imageCount = ['sampleFlatImageIds', 'sampleTryOnImageIds', 'sampleDetailImageIds'].reduce((count, key) => {
        const value = payload[key];
        return count + (Array.isArray(value) ? value.length : 0);
    }, 0);
    return imageCount > 0 ? `${record.workItemTypeName} / ${imageCount} 张图片` : record.workItemTypeName;
}
function buildRelationObjectTitle(relation) {
    if (relation.sourceTitle)
        return relation.sourceTitle;
...
export function listProjectNodeInstances(projectId, projectNodeId) {
...
export function getProjectNodeInstanceRuntimeSnapshot(projectId, projectNodeId) {
    const project = getProjectById(projectId);
...
export function getProjectNodeInstanceRegistry(projectId) {
    const project = getProjectById(projectId);
    if (!project)
        return null;
    const nodes = listProjectNodes(projectId);
...
export function syncProjectNodeInstanceRuntime(projectId, projectNodeId, operatorName = '系统实例注册中心', timestamp = nowText()) {
    ...
    const hasChanged = (currentNode.latestInstanceId || '') !== latestInstanceId ||
        (currentNode.latestInstanceCode || '') !== latestInstanceCode ||
        (currentNode.validInstanceCount || 0) !== validInstanceCount;
    if (!hasChanged) {
        return currentNode;
    }
    return updateProjectNodeRecord(projectId, projectNodeId, {
        latestInstanceId,
        latestInstanceCode,
        validInstanceCount,
        updatedAt: timestamp,
...
export function syncProjectNodeInstancesByProject(projectId, operatorName = '系统实例注册中心', timestamp = nowText()) {
