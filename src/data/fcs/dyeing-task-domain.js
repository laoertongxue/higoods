...
import { createFactoryHandoverRecord, ensureHandoverOrderForStartedTask, getHandoverOrderById, getPdaHandoverRecordsByHead, listHandoverOrdersByTaskId, writeBackHandoverRecord, } from './pda-handover-events.ts';
...
import { listActiveProcessCraftDefinitions } from './process-craft-dict.ts';
import { buildDictionaryCraftMockDocumentNo, DICTIONARY_CRAFT_MOCKS_PER_DEFINITION, getDictionaryCraftMockSource, } from './production-artifact-generation.ts';
export const DYE_WORK_ORDER_STATUS_LABEL = {
    WAIT_SAMPLE: '待样衣',
    WAIT_MATERIAL: '待原料',
    SAMPLE_TESTING: '打样中',
    SAMPLE_DONE: '打样完成',
...
export const SAMPLE_WAIT_TYPE_LABEL = {
    NONE: '不需要',
    WAIT_SAMPLE_GARMENT: '样衣',
    WAIT_COLOR_CARD: '色样',
};
export const DYE_NODE_LABEL = {
    SAMPLE: '打样',
    MATERIAL_READY: '备料',
    VAT_PLAN: '染缸安排',
    DYE: '染色',
...
export const DYE_REVIEW_STATUS_LABEL = {
    WAIT_RECEIVE: '交出待收货',
    PARTIAL_HANDOVER: '部分交出',
    FULL_HANDOVER: '全部交出',
    HANDOVER_DIFFERENCE: '收货差异',
...
const workOrderStore = new Map();
const nodeRecordStore = new Map();
const reviewRecordStore = new Map();
const vatScheduleStore = new Map();
...
function getProductionOrderQty(order) {
...
function getGeneratedDyeCraft(index) {
...
function listVisibleRawDyeWorkOrders() {
    ...
    for (const order of sorted) {
        if (reviewRecordStore.has(order.dyeOrderId)
...
function getVisibleDyeWorkOrderIds() {
    return new Set(listVisibleRawDyeWorkOrders().map((order) => order.dyeOrderId));
}
function toGeneratedDyeWorkOrder(order, index) {
...
function listGeneratedDyeWorkOrders() {
...
function isDyeingTask(task) {
    return task.processBusinessCode === 'DYE' || task.processCode === 'PROC_DYE' || task.processNameZh === '染色';
}
function getDyeingTasks() {
    return listPdaGenericProcessTasks()
        .filter(isDyeingTask)
        .sort((left, right) => left.taskId.localeCompare(right.taskId));
}
function getDyeingTaskById(taskId) {
    return getDyeingTasks().find((task) => task.taskId === taskId);
}
function syncLinkedTaskState(taskId, input) {
    const task = getDyeingTaskById(taskId);
    if (!task)
        return;
    if (input.status)
        task.status = input.status;
    if (typeof input.assignmentMode !== 'undefined')
        task.assignmentMode = input.assignmentMode;
    if (typeof input.assignmentStatus !== 'undefined')
        task.assignmentStatus = input.assignmentStatus;
    ...
    if (typeof input.acceptanceStatus !== 'undefined')
        task.acceptanceStatus = input.acceptanceStatus;
    if (typeof input.blockReason !== 'undefined')
        task.blockReason = input.blockReason;
    if (typeof input.blockRemark !== 'undefined')
        task.blockRemark = input.blockRemark;
    task.updatedAt = nowTimestamp();
}
function getPrimaryHandoverOrder(taskId) {
    const existing = listHandoverOrdersByTaskId(taskId);
    return existing[0] ?? null;
}
function syncTaskHandoverFields(taskId, handoverOrderId) {
    const task = getDyeingTaskById(taskId);
    if (!task)
        return;
...
function ensureStartedTaskHandover(taskId) {
    const task = getDyeingTaskById(taskId);
    if (!task?.startedAt)
        return undefined;
...
function ensureSeededHandoverRecord(input) {
    ...
    const head = getHandoverOrderById(handoverOrderId);
    if (!head)
        return { handoverOrderId, recordIds: [] };
...
function setNodeRecords(dyeOrderId, records) {
    nodeRecordStore.set(dyeOrderId, records.map((record) => ({ ...record })));
}
function appendNodeRecord(dyeOrderId, record) {
    const current = nodeRecordStore.get(dyeOrderId) ?? [];
    current.push({ ...record });
    nodeRecordStore.set(dyeOrderId, current);
}
function upsertNodeRecord(dyeOrderId, nodeCode, updater) {
    const current = nodeRecordStore.get(dyeOrderId) ?? [];
...
function getStatusDurationHours(order) {
...
function getCurrentOutputQty(order) {
    const nodes = nodeRecordStore.get(order.dyeOrderId) ?? [];
    return nodes.reduce((current, node) => Math.max(current, node.outputQty ?? 0), 0);
}
function getCurrentDyeVatNo(order) {
    return (getDyeExecutionNodeRecord(order.dyeOrderId, 'DYE')?.dyeVatNo
        || getDyeExecutionNodeRecord(order.dyeOrderId, 'VAT_PLAN')?.dyeVatNo);
}
function updateOrderTimestamp(order, at = nowTimestamp()) {
    order.updatedAt = at;
}
function resolveDyeReceiptStatus(input) {
    ...
    if (input.forceDifference || receivedQty > submittedQty)
        return 'HANDOVER_DIFFERENCE';
    if (receivedQty <= 0)
        return 'WAIT_RECEIVE';
    if (completedQty > 0 && receivedQty < completedQty)
        return 'PARTIAL_HANDOVER';
    return 'FULL_HANDOVER';
}
function createReviewFromHandover(order, head) {
    const records = getPdaHandoverRecordsByHead(head.handoverId);
...
function syncReviewFromHandover(order, head) {
...
function syncDyeOrderFromReview(order, review) {
    if (!review)
        return false;
...
function syncPreVatStatus(order) {
...
function addSeedWorkOrder(input) {
...
function getMutableWorkOrder(dyeOrderId) {
    syncDerivedWorkflow();
...
export function getDyeReviewRecordByOrderId(dyeOrderId) {
    syncDerivedWorkflow();
    if (!getVisibleDyeWorkOrderIds().has(dyeOrderId))
        return undefined;
    const review = reviewRecordStore.get(dyeOrderId);
    return review ? cloneReviewRecord(review) : undefined;
}
export function listDyeVatSchedules() {
    seedDomain();
    const visibleIds = getVisibleDyeWorkOrderIds();
...
export function listDyeVatOptions(factoryId) {
    return listFactoryDyeVatCapacities(factoryId);
}
export function listDyeFormulaRecords() {
    seedDomain();
    const visibleIds = getVisibleDyeWorkOrderIds();
...
export function getDyeOrderHandoverHead(dyeOrderId) {
    syncDerivedWorkflow();
    const order = workOrderStore.get(dyeOrderId);
...
export function getDyeOrderHandoverRecords(dyeOrderId) {
    const head = getDyeOrderHandoverHead(dyeOrderId);
    if (!head)
        return [];
    return getPdaHandoverRecordsByHead(head.handoverId);
}
export function getDyeOrderHandoverSummary(dyeOrderId) {
    const head = getDyeOrderHandoverHead(dyeOrderId);
    return {
...
export function getDyeWorkOrderSummary() {
    syncDerivedWorkflow();
    ...
    return {
        ...
        dyeingCount: orders.filter((order) => ['DYEING', 'DEHYDRATING', 'DRYING', 'SETTING', 'ROLLING', 'PACKING'].includes(order.status)).length,
...
export function listDyeReportRows() {
    syncDerivedWorkflow();
...
export function validateDyeStartPayload(input) {
    if (!input.dyeVatNo?.trim()) {
        return { ok: false, message: '请填写染缸编号' };
    }
    return { ok: true };
}
export function hasDirectPackingToReviewOrCompleteTransition() {
    return false;
}
export function startDyeSampleWait(dyeOrderId, input) {
...
export function completeDyeSampleWait(dyeOrderId, operatorName = '染色工厂') {
    const order = getMutableWorkOrder(dyeOrderId);
    const now = nowTimestamp();
...
export function startDyeMaterialWait(dyeOrderId, operatorName = '染色工厂') {
    const order = getMutableWorkOrder(dyeOrderId);
    const now = nowTimestamp();
...
export function completeDyeMaterialWait(dyeOrderId, operatorName = '染色工厂') {
    const order = getMutableWorkOrder(dyeOrderId);
    const now = nowTimestamp();
...
export function startDyeSampleTest(dyeOrderId, operatorName = '染色工厂') {
    const order = getMutableWorkOrder(dyeOrderId);
    const now = nowTimestamp();
...
export function completeDyeSampleTest(dyeOrderId, input) {
    const order = getMutableWorkOrder(dyeOrderId);
    const now = nowTimestamp();
...
export function startDyeMaterialReady(dyeOrderId, operatorName = '染色工厂') {
    const order = getMutableWorkOrder(dyeOrderId);
    const now = nowTimestamp();
...
export function completeDyeMaterialReady(dyeOrderId, input) {
    const order = getMutableWorkOrder(dyeOrderId);
...
export function planDyeVat(dyeOrderId, input) {
    const order = getMutableWorkOrder(dyeOrderId);
...
export function startDyeing(dyeOrderId, input) {
...
export function completeDyeing(dyeOrderId, input) {
    const order = getMutableWorkOrder(dyeOrderId);
    ...
    upsertNodeRecord(dyeOrderId, 'DYE', () => ({
        ...
        lossQty: (Number.isFinite(input.inputQty) ? Number(input.inputQty) : current.inputQty || order.plannedQty)
...
function getNodeStatusAfterStart(nodeCode) {
    switch (nodeCode) {
        case 'DEHYDRATE':
            return 'DEHYDRATING';
        case 'DRY':
...
function getNodeStatusAfterComplete(nodeCode) {
    switch (nodeCode) {
        case 'DEHYDRATE':
            return 'DRYING';
        case 'DRY':
...
export function startDyeNode(dyeOrderId, nodeCode, operatorName = '染色工厂') {
    const order = getMutableWorkOrder(dyeOrderId);
    const now = nowTimestamp();
...
export function completeDyeNode(dyeOrderId, nodeCode, input) {
    const order = getMutableWorkOrder(dyeOrderId);
...
export function submitDyeHandover(dyeOrderId, input = {}) {
    const order = getMutableWorkOrder(dyeOrderId);
...
function getMutableDyeReceiptReview(dyeOrderId) {
    const order = getMutableWorkOrder(dyeOrderId);
...
function applyDyeReceiptState(order, review) {
...
export function confirmDyeReceipt(dyeOrderId, input) {
...
export function markDyeReceiptDifference(dyeOrderId, input) {
...
export function approveDyeReview(dyeOrderId, input) {
    return confirmDyeReceipt(dyeOrderId, { receivedBy: input.reviewedBy, remark: input.remark });
}
export function rejectDyeReview(dyeOrderId, input) {
