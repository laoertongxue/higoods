import { listFactoryInternalWarehouses, listFactoryWarehouseNodeRows, listFactoryWarehouseStocktakeOrders, } from './factory-internal-warehouse.ts';
import { listProcessHandoverRecords, listWaitHandoverWarehouseRecords, listWaitProcessWarehouseRecords, } from './process-warehouse-domain.ts';
function normalizePrintWarehouseReceiptStatus(status) {
    if (status === '有差异' || status === '收货差异')
        return '收货差异';
    if (status === '已回写' || status === '已全部交出' || status === '全部交出')
        return '全部交出';
    if (status === '部分交出')
        return '部分交出';
    if (status === '待回写' || status === '待交出' || status === '已出库')
        return '交出待收货';
    return status;
}
function normalizePrintWarehouseReference(value) {
    return value
...
function matchesStatus(status, filterStatus) {
    if (!filterStatus || filterStatus === 'ALL')
        return true;
    return status === filterStatus;
}
function mapWaitProcessRecord(record) {
    return {
...
function mapWaitHandoverRecord(record) {
    return {
        stockItemId: record.warehouseRecordId,
...
function mapInboundRecord(record) {
...
function mapOutboundRecord(record) {
    return {
...
export function getPrintingWarehouseView(filters = {}) {
    const keyword = filters.keyword?.trim().toLowerCase() || '';
    const byFactory = (factoryId) => !filters.factoryId || factoryId === filters.factoryId;
    const waitProcessRecords = listWaitProcessWarehouseRecords({ craftType: 'PRINT' }).filter((item) => byFactory(item.targetFactoryId)
    ...
    const waitHandoverRecords = listWaitHandoverWarehouseRecords({ craftType: 'PRINT' }).filter((item) => byFactory(item.targetFactoryId)
        && matchesStatus(item.status, filters.status)
        && matchesKeyword([item.warehouseRecordNo, item.sourceWorkOrderNo, item.sourceTaskNo, item.batchNo, item.skuSummary], keyword));
    const handoverRecords = listProcessHandoverRecords({ craftType: 'PRINT' }).filter((item) => byFactory(item.handoverFactoryId)
        && matchesStatus(item.status, filters.status)
    ...
    const handoverOrderIds = new Set(outboundRecords.map((record) => record.handoverOrderId).filter(Boolean));
