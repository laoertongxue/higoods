import { getRuntimeTaskById, isRuntimeTaskExecutionTask, listRuntimeExecutionTasks, listRuntimeTasksByBaseTaskId, } from './runtime-process-tasks.ts';
import { listMaterialRequests, listMaterialRequestsByOrder, } from './material-request-drafts.ts';
import { listWarehouseExecutionDocsByOrder, listWarehouseExecutionDocsByRuntimeTaskId, listWarehouseExecutionDocsByMaterialRequestNo, listWarehouseInternalTransferOrdersByRuntimeTaskId, listWarehouseIssueOrdersByRuntimeTaskId, listWarehouseReturnOrdersByRuntimeTaskId, getWarehouseExecutionSummaryByOrder, } from './warehouse-material-execution.ts';
import { getPdaHandoutHeads, getPdaPickupHeads, getPdaPickupRecordsByHead, } from './pda-handover-events.ts';
// 生成异常号
...
export const progressExceptionCases = [];
...
function buildCoverageSeedContext(index, preferTender = false) {
...
function buildCoverageCaseId(index) {
    return `EX-MK-${String(index + 1).padStart(3, '0')}`;
}
const COVERAGE_SEED_DEFINITIONS = [
    {
...
function buildCoverageSeedCase(definition, index) {
    ...
    const sourceType = definition.sourceType === 'TENDER' && context.tenderId
        ? 'TENDER'
        : definition.sourceType === 'ORDER'
            ? 'ORDER'
            : 'TASK';
    const sourceId = sourceType === 'TENDER'
...
function ensureProgressExceptionCoverageSeeds() {
    COVERAGE_SEED_DEFINITIONS.forEach((definition, index) => {
        ...
        const existed = progressExceptionCases.some((item) => item.caseId === caseId || item.sourceModule === sourceModule);
        if (existed)
            return;
...
export function generateUrgeId() {
    urgeSeq += 1;
    return `UG-202603-${String(urgeSeq).padStart(4, '0')}`;
}
let urgeSeq = 9000;
export const initialUrges = [
...
function pickLatestTimestamp(values, fallback = mockNow) {
    const valid = values
...
function generateAutoProgressCaseId(candidateKey) {
    return `EX-AUTO-${shortStableKey(candidateKey)}`;
}
function toExceptionStatus(status) {
    return status;
}
function isDocClosed(status) {
    return status === 'CLOSED' || status === 'RETURNED' || status === 'RECEIVED';
}
function isDocActive(status) {
    return !isDocClosed(status);
}
function hasDocShortage(docs) {
    return docs.some((doc) => doc.lines.some((line) => line.shortQty > 0));
}
function hasPreparingStatus(docs) {
    return docs.some((doc) => doc.status === 'PLANNED' ||
        doc.status === 'PREPARING' ||
        doc.status === 'PARTIALLY_PREPARED' ||
        doc.status === 'READY');
}
function toIssueStatusFromExecution(input) {
    if (!input.hasExecutionDocs)
        return 'DRAFT';
    if (input.completedQty >= input.requestedQty && input.requestedQty > 0)
        return 'ISSUED';
    if (input.completedQty > 0)
        return 'PARTIAL';
    if (input.requestStatus === '待配送' || input.requestStatus === '待自提')
        return 'TO_ISSUE';
    return 'DRAFT';
}
...
function resolveRuntimeTaskForRequest(request) {
    const direct = getRuntimeTaskById(request.taskId);
    if (direct && isRuntimeTaskExecutionTask(direct))
        return direct;
    const byBase = listRuntimeTasksByBaseTaskId(request.taskId).filter((task) => isRuntimeTaskExecutionTask(task));
    if (byBase.length === 0)
        return null;
    if (byBase.length === 1)
        return byBase[0];
...
function getRequestsByRuntimeTask(task) {
...
function evaluateRuntimeStartReadiness(task) {
    ...
    const ready = pickupHeads.some((head) => {
        if (head.completionStatus === 'COMPLETED' || head.summaryStatus === 'WRITTEN_BACK')
            return true;
...
export function listProgressFacts() {
...
export function listProgressMaterialIssueRows() {
    return listMaterialRequests()
        ...
        const requestedQty = executionDocs.reduce((sum, doc) => sum + doc.lines.reduce((lineSum, line) => lineSum + line.plannedQty, 0), 0);
        const issuedQty = executionDocs.reduce((sum, doc) => sum +
            doc.lines.reduce((lineSum, line) => lineSum + (line.issuedQty ?? 0) + (line.transferredQty ?? 0), 0), 0);
...
export function listProgressMaterialIssueRowsByOrder(productionOrderId) {
    return listProgressMaterialIssueRows().filter((row) => row.productionOrderId === productionOrderId);
}
export function listProgressMaterialStatementDrafts() {
    ...
    return Array.from(grouped.entries())
        ...
        const status = allIssued ? 'CONFIRMED' : 'DRAFT';
        const newest = sortedRows.reduce((acc, row) => parseDateMs(row.updatedAt) > parseDateMs(acc.updatedAt) ? row : acc, sortedRows[0]);
...
function buildAutoCaseKey(runtimeTaskId, subCategoryKey) {
    return `${AUTO_PROGRESS_TAG}:${runtimeTaskId}:${subCategoryKey}`;
}
function createProgressExceptionCandidates() {
    ...
    facts.forEach((fact) => {
        ...
        if (!runtimeTask)
            return;
        const issueOrTransferDocs = fact.executionDocs.filter((doc) => doc.docType === 'ISSUE' || doc.docType === 'INTERNAL_TRANSFER');
        const returnDocs = fact.executionDocs.filter((doc) => doc.docType === 'RETURN');
        const skipMaterialChecks = fact.transitionFromPrev === 'SAME_FACTORY_CONTINUE' &&
        ...
        if (!skipMaterialChecks) {
            ...
            if (hasPreparingStatus(issueOrTransferDocs)) {
                candidates.push({
                    ...
                    linkedFactoryName,
                    summary: '仓库备料未完成',
                    detail: `${fact.processNameZh}（${fact.scopeLabel}）存在待备料执行单，仓库配料记录尚未收口`,
            ...
            const openByType = openIssueDocs.reduce((acc, doc) => {
        ...
        const shouldReturnToWarehouse = fact.executorKind === 'EXTERNAL_FACTORY' && fact.transitionToNext !== 'SAME_FACTORY_CONTINUE';
...
export function syncProgressFactsAndExceptions() {
    ...
    progressExceptionCases.forEach((item) => {
        if (!item.tags.includes(AUTO_PROGRESS_TAG))
            return;
        if (activeKeys.has(item.sourceModule || ''))
            return;
...
function isProgressFactBackedCase(item) {
    if (item.tags.includes(AUTO_PROGRESS_TAG))
        return true;
    if (item.sourceSystem === MOCK_PROGRESS_SOURCE_SYSTEM)
        return true;
    if (item.tags.includes(PROGRESS_EXCEPTION_MOCK_COVERAGE_TAG))
        return true;
    if (item.sourceModule === 'PDA_PICKUP_DISPUTE')
        return true;
    if (item.sourceSystem === 'RUNTIME_FLOW')
        return true;
    if (item.relatedTaskIds.some((taskId) => Boolean(getRuntimeTaskById(taskId))))
        return true;
    return false;
}
export function listProgressExceptions() {
...
export function getProgressExceptionById(caseId) {
    return listProgressExceptions().find((item) => item.caseId === caseId);
}
export function upsertProgressExceptionCase(updated) {
...
export function listProgressExceptionsByOrder(productionOrderId) {
    return listProgressExceptions().filter((item) => item.relatedOrderIds.includes(productionOrderId));
}
export function getProgressExceptionSummary() {
...
export function getProgressExceptionSummaryByOrder(productionOrderId) {
...
export function getOrderMaterialFlowSummary(productionOrderId) {
...
export function getOrderExecutionHealthSummary(productionOrderId) {
    ...
    return {
        ...
        handoutExceptionCount: exceptions.filter((item) => (item.caseStatus === 'OPEN' || item.caseStatus === 'IN_PROGRESS') &&
            item.subCategoryKey?.startsWith('HANDOUT_')).length,
        materialExceptionCount: exceptions.filter((item) => (item.caseStatus === 'OPEN' || item.caseStatus === 'IN_PROGRESS') &&
            item.unifiedCategory === 'MATERIAL').length,
...
export function getOrderProgressSummary(productionOrderId) {
    const facts = listProgressFactsByOrder(productionOrderId);
