...
export function resolveSupplementPrintPrerequisite(input: {
  ...
  // 人工完单与上下游分批承接分离；不要求所有来源或整单计划数量齐套。
  const actualInputs = input.upstream.filter((item) => item.fact.qualifiedQty > 0).map((item) => structuredClone(item.fact))
  const qualifiedOutputQty = actualInputs.reduce((sum, item) => sum + item.qualifiedQty, 0)
  if (qualifiedOutputQty <= 0) return { allowed: false, reason: '等待染色交出，当前没有可承接数量。', actualInputs: [] }
  if (actualInputs.some((item) => item.unit !== input.unit)) return { allowed: false, reason: '染色交出单位与印花投入单位不一致，不能开始印花。', actualInputs: [] }
...
function buildResolutionState(prerequisite: SupplementPrintPrerequisite) {
  return {
    ...
    upstream: prerequisite.dyeWorkOrderIds.map((dyeWorkOrderId) => {
      ...
      const handover = getDyeOrderHandoverSummary(dyeWorkOrderId)
      return {
        ...
        fact: {
          ...
          // 沿用已存在字段名；值是交出事实，不是中间质量放行或未交出的加工产出。
          qualifiedQty: handover.submittedQty,
          unit: order?.qtyUnit || '',
          batchSource: order?.handoverOrderNo || '未记录',
        },
      }
    }),
  }
