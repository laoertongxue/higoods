...
export function buildFactoryReceiptInboundRecords():FactoryWarehouseInboundRecord[]{
 return listFactoryReceipts().filter(r=>r.factoryId!=='OWN_WOOL_FACTORY').flatMap(receipt=>receipt.lines.filter(l=>l.qty>0).flatMap(line=>{
  ...
  return splits.map((split,i)=>{const p=getHistoricalReceiptPosition(receipt.factoryId,split),id=`FIN-${line.id}-${i+1}`;return {
   ...
   processCode:line.waterOrderId?'PROC_WATER_SOLUBLE':'PROC_DYE',processName:line.waterOrderId?'水溶':'染色',craftCode:line.waterOrderId?'WATER_SOLUBLE':'DYE',craftName:line.waterOrderId?'水溶':'染色',sourceRecordId:id,sourceRecordNo:line.sourceDocumentNo,sourceRecordType:line.sourceType==='HANDOUT'?'HANDOVER_RECEIVE':'TRANSFER_RECEIVE',sourceObjectName:line.origin.name,
   ...
   expectedQty:split.qty,receivedQty:split.qty,differenceQty:0,unit:line.unit,receiverName:receipt.operatorName,operatorUserId:receipt.operatorId,operatorFactoryId:receipt.factoryId,receivedAt:receipt.receivedAt,areaName:p.area.areaName,shelfNo:p.shelf.shelfNo,locationNo:p.location.locationNo,status:'已入库',photoList:[line.material.imageUrl],generatedStockItemId:`STK-${id}`,remark:`接收 ${receipt.id}；批次 ${line.material.batchNo}；${line.yarn?`${line.yarn.pcs} pcs，毛重 ${line.yarn.grossGrams/1000} kg，净重 ${line.qty} kg；`:''}${line.dyeOrderId?'已关联染色加工单':line.waterOrderId?'已关联水溶加工单':'备料来货，未关联加工单'}；${receipt.remark}`,
  } satisfies FactoryWarehouseInboundRecord})
 }))
}
export function getDyeFactoryReceiptProjection(orderId:string,unit:string,kind:'dye'|'water'='dye'){
 const allocations=listReceivingAllocations(),key=kind==='water'?'waterOrderId':'dyeOrderId'
