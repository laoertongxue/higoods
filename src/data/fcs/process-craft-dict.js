import { getFactorySupplyFormulaGuide, getFactorySupplyFormulaGuideByTemplate, getFactorySupplyFormulaTemplate, } from './process-craft-sam-explainer.ts';
export const PROCESS_ASSIGNMENT_GRANULARITY_LABEL = {
    ORDER: '按生产单',
    COLOR: '按颜色',
    SKU: '按SKU',
    DETAIL: '按明细行',
...
export const PROCESS_TARGET_OBJECT_NAME = {
    CUT_PIECE_PART: '裁片部位',
    FABRIC: '面料',
    ACCESSORY: '辅料',
    GARMENT_SEMI: '成衣半成品',
};
export const SPECIAL_CRAFT_CATEGORY_NAME = {
    AUXILIARY: '辅助工艺',
    SPECIAL: '特种工艺',
};
export const PROCESS_CRAFT_MANAGEMENT_DOMAIN_NAME = {
...
const SPECIAL_CRAFT_VISIBLE_FACTORY_TYPES_BY_LEGACY_VALUE = {
...
export const preparationProcessDefinitions = [
    {
...
export const modernSpecialCraftDefinitions = [
    {
...
export const cuttingCraftDefinitions = [
    {
...
function resolveSpecialCraftSupportedTargetObjects(item) {
    if (!item.isSpecialCraft)
        return [];
    return [...(SPECIAL_CRAFT_SUPPORTED_TARGET_OBJECTS_BY_LEGACY_VALUE[item.legacyValue] ?? ['CUT_PIECE'])];
}
function resolveSpecialCraftVisibleFactoryTypes(item) {
    if (!item.isSpecialCraft)
        return [];
    return [...(SPECIAL_CRAFT_VISIBLE_FACTORY_TYPES_BY_LEGACY_VALUE[item.legacyValue] ?? ['CENTRAL_SPECIAL'])];
}
export function getSpecialCraftTargetObjectLabel(targetObject) {
    return SPECIAL_CRAFT_TARGET_OBJECT_LABEL[targetObject];
}
export function getSpecialCraftSupportedTargetObjectLabels(targetObjects) {
    return targetObjects.map((item) => SPECIAL_CRAFT_TARGET_OBJECT_LABEL[item]);
}
export function isSpecialCraftTargetObjectLabel(value) {
    return value === '已裁部位' || value === '完整面料' || value === '成衣半成品';
}
export function normalizeSpecialCraftTargetObjectLabel(value) {
    if (value === '裁片')
        return '已裁部位';
    if (value === '面料')
        return '完整面料';
    if (value === '半成品' || value === '成衣' || value === '整件成衣')
        return '成衣半成品';
    return isSpecialCraftTargetObjectLabel(value) ? value : '';
}
export const PROCESS_ROLE_LABEL = {
    EXTERNAL_TASK: '对外任务',
    INTERNAL_CAPACITY_NODE: '产能节点',
};
export const DETAIL_SPLIT_MODE_LABEL = {
    COMPOSITE: '组合维度',
};
export const DETAIL_SPLIT_DIMENSION_LABEL = {
    PATTERN: '纸样',
    MATERIAL_SKU: '物料SKU',
    GARMENT_COLOR: '成衣颜色',
    GARMENT_SKU: '成衣SKU',
};
export const RULE_SOURCE_LABEL = {
    INHERIT_PROCESS: '继承工序规则',
    OVERRIDE_CRAFT: '工艺覆盖规则',
};
export const PROCESS_DOC_TYPE_LABEL = {
    DEMAND: '需求单',
    TASK: '任务单',
};
...
export const TASK_TYPE_MODE_LABEL = {
    PROCESS: '按工序',
    CRAFT: '按工艺',
};
export const SAM_CALC_MODE_LABEL = {
    DISCRETE: '离散型',
    CONTINUOUS: '连续型',
    BATCH: '批次型',
};
export const SAM_INPUT_UNIT_LABEL = {
    PIECE: '按件录入',
    METER: '按米录入',
    KG: '按公斤录入',
    BATCH: '按批次录入',
};
export const PUBLISHED_SAM_UNIT_LABEL = {
    MINUTE_PER_PIECE: '分钟/件',
    MINUTE_PER_BATCH: '分钟/批',
    MINUTE_PER_METER: '分钟/米',
    MINUTE_PER_DOZEN: '分钟/打',
};
export const CAPACITY_CONSTRAINT_SOURCE_LABEL = {
    DEVICE: '设备约束',
    STAFF: '人员约束',
    BOTH: '设备+人员共同约束',
};
export const SAM_FACTORY_FIELD_GROUP_LABEL = {
    DEVICE: '设备',
    STAFF: '人员',
    ADJUSTMENT: '调整',
};
export const SAM_FACTORY_FIELD_ORDER = [
    'deviceCount',
    'deviceShiftMinutes',
    'batchLoadCapacity',
    'batchLoadUnit',
...
export const SAM_FACTORY_FIELD_DICT = {
    deviceCount: {
        key: 'deviceCount',
        label: '设备数量',
        group: 'DEVICE',
...
const BATCH_PROCESS_FIELD_KEYS = [
    'deviceCount',
    'deviceShiftMinutes',
    'batchLoadCapacity',
    'batchLoadUnit',
...
const PROCESS_CURRENT_TEMPLATE_BY_CODE = {
    PRINT: 'C',
    DYE: 'D',
    CUT_PANEL: 'B',
    EMBROIDERY: 'B',
...
function resolveProcessCurrentTemplate(processCode) {
    return PROCESS_CURRENT_TEMPLATE_BY_CODE[processCode] ?? 'A';
}
const PROCESS_SAM_RULES = {
    PRINT: {
        samEnabled: true,
...
const CRAFT_SAM_RULE_OVERRIDES_BY_LEGACY_VALUE = {
    262144: {
        samCalcMode: 'DISCRETE',
        samDefaultInputUnit: 'PIECE',
        samConstraintSource: 'BOTH',
...
const REFERENCE_PUBLISHED_SAM_BY_CRAFT_NAME = {
...
function getReferencePublishedSamNote(unit) {
...
const CRAFT_SYSTEM_CODE_BY_LEGACY_VALUE = {
    1: 'PROC_POSITION_CUT',
    2: 'PROC_EMBROIDER',
    4: 'PROC_PLEAT',
    8: 'PROC_DALAN',
...
const CARRY_SUGGESTION_BY_PROCESS_CODE = {
    PRINT: '印花厂优先',
    DYE: '染色厂优先',
    CUT_PANEL: '裁片厂优先',
    EMBROIDERY: '绣花厂优先',
...
function toProcessDocType(documentLabel) {
    return documentLabel === '需求单' ? 'DEMAND' : 'TASK';
}
function toTaskTypeMode(isSpecialCraft) {
    return isSpecialCraft ? 'CRAFT' : 'PROCESS';
}
function toCraftCode(legacyValue) {
    return `CRAFT_${String(legacyValue).padStart(6, '0')}`;
}
export const processStageDefinitions = [
    {
        stageCode: 'PREP',
        stageName: '准备阶段',
        sort: 10,
...
function resolveProcessGranularity(processCode) {
...
function resolveProcessDefaultRule(processCode) {
...
export function getSamFactoryFieldDefinitionByKey(key) {
    return SAM_FACTORY_FIELD_DICT[key];
}
function formatSamFactoryFieldText(keys) {
    return listSamFactoryFieldDefinitions(keys)
        .map((item) => item.label)
        .join('、');
}
export const processDefinitions = processDefinitionSeeds.map((seed) => {
    ...
    const currentGuide = getFactorySupplyFormulaGuideByTemplate(resolveProcessCurrentTemplate(seed.processCode), seed.processName);
    return {
...
export const legacyProcessCraftMappings = [
...
const supplementalProcessCraftMappings = [
...
const modernSpecialCraftProcessMappings = modernSpecialCraftDefinitions
    .filter((craft) => !legacyProcessCraftMappings.some((item) => item.processCode === 'SPECIAL_CRAFT' && item.craftName === craft.craftName))
    .map((craft, index) => ({
...
function resolveProcessCraftManagementDomain(item) {
...
function resolveProcessCraftTargetObject(item) {
    const modernSpecialCraft = modernSpecialCraftDefinitions.find((craft) => craft.craftName === item.craftName);
    if (modernSpecialCraft) {
        return {
    ...
    const preparationProcess = preparationProcessDefinitions.find((process) => process.processName === item.craftName || process.processCode === item.processCode);
...
export const allProcessCraftDefinitions = [
    ...
    const resolvedRuleSource = forceOverrideBySpecialCraft
        ? 'OVERRIDE_CRAFT'
        : item.ruleSource ?? 'INHERIT_PROCESS';
    const resolvedAssignmentGranularity = resolvedRuleSource === 'OVERRIDE_CRAFT'
        ? item.assignmentGranularity ?? inheritedRule.assignmentGranularity
        : inheritedRule.assignmentGranularity;
    const resolvedDetailSplitMode = resolvedRuleSource === 'OVERRIDE_CRAFT'
        ? item.detailSplitMode ?? inheritedRule.detailSplitMode
        : inheritedRule.detailSplitMode;
    const resolvedDetailSplitDimensions = resolvedRuleSource === 'OVERRIDE_CRAFT'
    ...
    return {
        ...
        supportedTargetObjectLabels: getSpecialCraftSupportedTargetObjectLabels(resolveSpecialCraftSupportedTargetObjects(item)),
        ...
        samCurrentFieldKeys: craftCurrentTemplate === processCurrentTemplate ? undefined : [...craftCurrentGuide.currentFieldKeys],
        samCurrentFormulaLines: craftCurrentTemplate === processCurrentTemplate ? undefined : [...craftCurrentGuide.currentFormulaLines],
        samCurrentExplanationLines: craftCurrentTemplate === processCurrentTemplate ? undefined : [...craftCurrentGuide.currentExplanationLines],
        samCurrentExampleLines: craftCurrentTemplate === processCurrentTemplate ? undefined : [...craftCurrentGuide.currentExampleLines],
        samCurrentReason: craftCurrentTemplate === processCurrentTemplate ? undefined : craftCurrentGuide.currentReason,
        samFactoryFieldKeys: craftCurrentTemplate === processCurrentTemplate ? undefined : [...craftCurrentGuide.currentFieldKeys],
        samReason: craftCurrentTemplate === processCurrentTemplate ? undefined : craftCurrentGuide.currentReason,
    };
});
export const processCraftDefinitions = allProcessCraftDefinitions.filter((item) => item.isActive);
...
export function getProcessStageByCode(stageCode) {
...
export function getProcessDefinitionByCode(processCode) {
    return processDefinitionByCode.get(processCode);
}
export function getProcessDefinitionBySystemCode(systemProcessCode) {
    return processDefinitionBySystemCode.get(systemProcessCode);
}
export function isExternalTaskProcess(processCode) {
    return getProcessDefinitionByCode(processCode)?.generatesExternalTask ?? false;
}
export function isPostCapacityNode(processCode) {
...
export function listProcessesByStageCode(stageCode) {
    return listProcessDefinitions().filter((item) => item.stageCode === stageCode);
}
export function listProcessCraftDefinitions() {
    return processCraftDefinitions.slice();
}
export function listAllProcessCraftDefinitions() {
    return allProcessCraftDefinitions.slice();
}
export function listInactiveProcessCraftDefinitions() {
    return allProcessCraftDefinitions.filter((item) => !item.isActive);
}
export function listProcessCraftDefinitionsByStatus(isActive) {
    return allProcessCraftDefinitions.filter((item) => item.isActive === isActive);
}
export function listActiveProcessCraftDefinitions() {
    return processCraftDefinitions.slice();
}
export function getProcessCraftByCode(craftCode) {
    return processCraftByCode.get(craftCode);
}
export function getProcessCraftByLegacyValue(legacyValue) {
    return processCraftByLegacyValue.get(legacyValue);
}
export function getSpecialCraftSupportedTargetObjectsByCraftCode(craftCode) {
    return [...(getProcessCraftByCode(craftCode)?.supportedTargetObjects ?? [])];
}
export function getSpecialCraftVisibleFactoryTypesByCraftCode(craftCode) {
    return [...(getProcessCraftByCode(craftCode)?.visibleFactoryTypes ?? [])];
}
export function listCraftsByProcessCode(processCode) {
    return processCraftDefinitions.filter((item) => item.processCode === processCode);
}
export function listCraftsByStageCode(stageCode) {
    return processCraftDefinitions.filter((item) => item.stageCode === stageCode);
}
export function listSelectableSpecialCraftDefinitions() {
    return processCraftDefinitions.filter((item) => item.isSpecialCraft
        && item.processCode === 'SPECIAL_CRAFT');
}
function clonePreparationProcess(item) {
    return { ...item };
}
function cloneModernSpecialCraft(item) {
    return { ...item };
}
function cloneCuttingCraft(item) {
    return { ...item };
}
function findModernSpecialCraft(craftCodeOrName) {
    return modernSpecialCraftDefinitions.find((item) => item.craftCode === craftCodeOrName || item.craftName === craftCodeOrName);
}
export function listCraftsByManagementDomain(domain) {
    return modernSpecialCraftDefinitions
        .filter((item) => item.managementDomain === domain)
        .map((item) => cloneModernSpecialCraft(item));
}
export function getCraftManagementDomain(craftCodeOrName) {
    return findModernSpecialCraft(craftCodeOrName)?.managementDomain;
}
export function getCraftManagementDomainName(craftCodeOrName) {
    return findModernSpecialCraft(craftCodeOrName)?.managementDomainName;
}
export function listPreparationProcesses() {
    return preparationProcessDefinitions.map((item) => clonePreparationProcess(item));
}
export function listSpecialCrafts() {
    return modernSpecialCraftDefinitions.map((item) => cloneModernSpecialCraft(item));
}
export function listAuxiliaryCrafts() {
    return modernSpecialCraftDefinitions
        .filter((item) => item.craftCategory === 'AUXILIARY')
        .map((item) => cloneModernSpecialCraft(item));
}
export function listSpecialTypeCrafts() {
    return modernSpecialCraftDefinitions
        .filter((item) => item.craftCategory === 'SPECIAL')
        .map((item) => cloneModernSpecialCraft(item));
}
export function listCutPiecePartCrafts() {
    return modernSpecialCraftDefinitions
        .filter((item) => item.targetObject === 'CUT_PIECE_PART' && item.canSelectInPatternPiece)
        .map((item) => cloneModernSpecialCraft(item));
}
export function listFabricCrafts() {
    return modernSpecialCraftDefinitions
        .filter((item) => item.targetObject === 'FABRIC')
        .map((item) => cloneModernSpecialCraft(item));
}
export function listAccessoryCrafts() {
    return modernSpecialCraftDefinitions
        .filter((item) => item.targetObject === 'ACCESSORY')
        .map((item) => cloneModernSpecialCraft(item));
}
export function listCuttingCrafts() {
    return cuttingCraftDefinitions.map((item) => cloneCuttingCraft(item));
}
export function getCraftTargetObject(craftCodeOrName) {
    const modernCraft = findModernSpecialCraft(craftCodeOrName);
    if (modernCraft)
        return modernCraft.targetObject;
    const cuttingCraft = cuttingCraftDefinitions.find((item) => item.craftCode === craftCodeOrName || item.craftName === craftCodeOrName);
    if (cuttingCraft)
        return cuttingCraft.targetObject;
    const preparationProcess = preparationProcessDefinitions.find((item) => item.processCode === craftCodeOrName || item.processName === craftCodeOrName);
    if (preparationProcess)
        return preparationProcess.targetObject;
    ...
    if (legacyCraft) {
        ...
        if (migratedCraft)
            return migratedCraft.targetObject;
        if (legacyCraft.craftName === '洗水' || legacyCraft.craftName === '缩水')
            return 'FABRIC';
    }
    return undefined;
}
export function canSelectCraftInPatternPiece(craftCodeOrName) {
    const modernCraft = findModernSpecialCraft(craftCodeOrName);
    if (modernCraft)
        return modernCraft.canSelectInPatternPiece;
    const legacyCraft = getProcessCraftByCode(craftCodeOrName) ?? getProcessCraftByLegacyValue(Number(craftCodeOrName));
    if (!legacyCraft)
        return false;
    const migratedCraft = findModernSpecialCraft(legacyCraft.craftName);
...
function buildProcessCraftOption(definition) {
...
function dedupeProcessCraftOptions(options) {
...
export function getActiveProcessCraftOptions() {
    return dedupeProcessCraftOptions(listActiveProcessCraftDefinitions()
        .map((item) => buildProcessCraftOption(item))
        .filter((item) => Boolean(item)));
}
export function getCapacityProcessCraftOptions() {
    return getActiveProcessCraftOptions().filter((item) => {
        ...
        if (item.isCapacityNode)
            return true;
        return process?.capacityEnabled ?? false;
    });
}
export function getExternalTaskProcessCraftOptions() {
    return getActiveProcessCraftOptions().filter((item) => !item.isCapacityNode);
}
export function getCapacityNodeProcessCraftOptions() {
    return getActiveProcessCraftOptions().filter((item) => item.isCapacityNode);
}
export function getAssignmentGranularityByCraftCode(craftCode) {
    return processCraftByCode.get(craftCode)?.assignmentGranularity ?? 'ORDER';
}
export function getDetailSplitModeByCraftCode(craftCode) {
    return processCraftByCode.get(craftCode)?.detailSplitMode ?? 'COMPOSITE';
}
export function getDetailSplitDimensionsByCraftCode(craftCode) {
    return [...(processCraftByCode.get(craftCode)?.detailSplitDimensions ?? ['PATTERN', 'MATERIAL_SKU'])];
}
export function getRuleSourceByCraftCode(craftCode) {
    return processCraftByCode.get(craftCode)?.ruleSource ?? 'INHERIT_PROCESS';
}
export function getDefaultDocTypeByCraftCode(craftCode) {
    return processCraftByCode.get(craftCode)?.defaultDocType ?? 'TASK';
}
export function getTaskTypeModeByCraftCode(craftCode) {
    return processCraftByCode.get(craftCode)?.taskTypeMode ?? 'PROCESS';
}
export function isSpecialCraftByCraftCode(craftCode) {
    return processCraftByCode.get(craftCode)?.isSpecialCraft ?? false;
}
export function getResolvedProcessCraftSamRuleByCode(craftCode) {
    ...
    return {
        ...
        samCurrentFieldKeys: orderSamFactoryFieldKeys((craft.samCurrentFieldKeys ?? process.samCurrentFieldKeys)),
...
export const allProcessCraftDictRows = allProcessCraftDefinitions.map((item) => {
    const process = processDefinitionByCode.get(item.processCode);
    ...
    const samRule = getResolvedProcessCraftSamRuleByCode(item.craftCode) ??
        {
            samEnabled: true,
            samCalcMode: 'DISCRETE',
            samDefaultInputUnit: 'PIECE',
    ...
    return {
        ...
        processAssignmentGranularityLabel: PROCESS_ASSIGNMENT_GRANULARITY_LABEL[processAssignmentGranularity],
        ...
        samCurrentFieldText: formatSamFactoryFieldText(samRule.samCurrentFieldKeys),
        ...
        samFactoryFieldText: formatSamFactoryFieldText(samRule.samCurrentFieldKeys),
        samReason: samRule.samCurrentReason,
    };
});
export const processCraftDictRows = allProcessCraftDictRows.filter((item) => item.isActive);
export function listProcessCraftDictRows(includeHistorical = false) {
    return includeHistorical ? allProcessCraftDictRows.slice() : processCraftDictRows.slice();
}
export function getProcessCraftDictRowByCode(craftCode) {
    return allProcessCraftDictRows.find((item) => item.craftCode === craftCode);
}
function buildActiveProcessCraftRow(definition) {
...
function sortActiveProcessCraftRows(left, right) {
    ...
    if (leftStageSort !== rightStageSort)
        return leftStageSort - rightStageSort;
    const processCompare = left.processName.localeCompare(right.processName, 'zh-CN');
    if (processCompare !== 0)
        return processCompare;
    return left.craftName.localeCompare(right.craftName, 'zh-CN');
...
const activeProcessCraftRowByKey = new Map(activeProcessCraftRows.map((item) => [item.processCraftKey, item]));
export function getActiveProcessCraftRows() {
    return activeProcessCraftRows.slice();
}
export function getActiveProcessOptions() {
    const optionMap = new Map();
...
export function getActiveCraftOptionsByProcess(processCode) {
    return activeProcessCraftRows.filter((row) => (processCode ? row.processCode === processCode : true));
}
export function resolveProcessCraft(processCode, craftCode) {
...
export function assertProcessCraftExists(processCode, craftCode, context = '工序工艺校验') {
