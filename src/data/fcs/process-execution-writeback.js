import { completeColorTest, completePrinting, completeTransfer, getPrintExecutionNodeRecord, getPrintWorkOrderById, getPrintWorkOrderByTaskId, listPrintMachineOptions, startColorTest, startPrinting, startTransfer, } from './printing-task-domain.ts';
import { completeDyeMaterialReady, completeDyeMaterialWait, completeDyeNode, completeDyeSampleTest, completeDyeSampleWait, completeDyeing, getDyeExecutionNodeRecord, getDyeWorkOrderById, getDyeWorkOrderByTaskId, listDyeVatOptions, planDyeVat, startDyeMaterialReady, startDyeMaterialWait, startDyeNode as startDyeExecutionNode, startDyeSampleTest, startDyeSampleWait, startDyeing, } from './dyeing-task-domain.ts';
import { TEST_FACTORY_ID, TEST_FACTORY_NAME } from './factory-mock-data.ts';
import { createFactoryHandoverRecord, ensureHandoverOrderForStartedTask, writeBackHandoverRecord, } from './pda-handover-events.ts';
import { createSpecialCraftReturnHandover, getSpecialCraftBindingsByTaskOrderId, linkSpecialCraftCompletionToReturnWaitHandoverStock, listCuttingSpecialCraftFeiTicketBindings, recordSpecialCraftFeiTicketLossAndDamage, } from './cutting/special-craft-fei-ticket-flow.ts';
import { getSpecialCraftTaskOrderById, getSpecialCraftTaskWorkOrdersByTaskOrderId, listSpecialCraftTaskOrders, } from './special-craft-task-orders.ts';
import { applyPostFinishingActionFinish, applyPostFinishingActionStart, getPostFinishingWorkOrderById, getPostFinishingWorkOrderBySourceTaskId, receivePostFinishingAtManagedFactory, transferPostFinishingToManagedFactory, } from './post-finishing-domain.ts';
import { createProcessHandoverRecord, createWaitHandoverWarehouseRecord, createWaitProcessWarehouseRecord, writeBackProcessHandoverRecord, } from './process-warehouse-domain.ts';
function nowTimestamp(date = new Date()) {
    return date.toISOString().replace('T', ' ').slice(0, 19);
}
function requirePrintOrderByTaskId(taskId) {
    const order = getPrintWorkOrderByTaskId(taskId);
    if (!order)
        throw new Error(`未找到印花加工单：${taskId}`);
    return order;
}
function requireDyeOrderByTaskId(taskId) {
    const order = getDyeWorkOrderByTaskId(taskId);
    if (!order)
        throw new Error(`未找到染色加工单：${taskId}`);
    return order;
}
function ensurePrintHandoverOrderId(order) {
    if (order.handoverOrderId)
        return order.handoverOrderId;
    return ensureHandoverOrderForStartedTask(order.taskId).handoverOrderId;
}
function ensureDyeHandoverOrderId(order) {
    if (order.handoverOrderId)
        return order.handoverOrderId;
    return ensureHandoverOrderForStartedTask(order.taskId).handoverOrderId;
}
function buildProductionOrderNo(ids) {
    return ids?.[0] || '';
}
function createPrintWaitHandoverRecord(order, qty, operatedAt) {
...
function createDyeWaitHandoverRecord(order, qty, operatedAt) {
    return createWaitHandoverWarehouseRecord({
        craftType: 'DYE',
        craftName: '染色',
...
export function startPrintNode(taskId, nodeCode, payload = {}) {
    ...
    if (nodeCode === 'PRINT') {
        const printerNo = payload.printerNo
...
export function finishPrintNode(taskId, nodeCode, payload = {}) {
    const order = requirePrintOrderByTaskId(taskId);
    if (nodeCode === 'COLOR_TEST') {
...
export function submitPrintDelivery(taskId, payload = {}) {
    const order = requirePrintOrderByTaskId(taskId);
...
export function submitPrintHandover(taskId, payload = {}) {
    const order = requirePrintOrderByTaskId(taskId);
    const handoverOrderId = ensurePrintHandoverOrderId(order);
...
export function createPrintReviewRecord(taskId, payload = {}) {
    return submitPrintHandover(taskId, payload);
}
export function startDyeNode(taskId, nodeCode, payload = {}) {
...
export function finishDyeNode(taskId, nodeCode, payload = {}) {
    const order = requireDyeOrderByTaskId(taskId);
    if (nodeCode === 'SAMPLE') {
...
export function submitDyeDelivery(taskId, payload = {}) {
    const order = requireDyeOrderByTaskId(taskId);
...
export function submitDyeHandover(taskId, payload = {}) {
    const order = requireDyeOrderByTaskId(taskId);
    const handoverOrderId = ensureDyeHandoverOrderId(order);
...
export function createDyeReviewRecord(taskId, payload = {}) {
    return submitDyeHandover(taskId, payload);
}
export function startDyeSampleWaitWriteback(taskId, payload = {}) {
    const order = requireDyeOrderByTaskId(taskId);
...
export function finishDyeSampleWaitWriteback(taskId, payload = {}) {
    const order = requireDyeOrderByTaskId(taskId);
    completeDyeSampleWait(order.dyeOrderId, payload.operatorName || '染色工厂');
    return { workOrder: getDyeWorkOrderById(order.dyeOrderId) };
}
export function startDyeMaterialWaitWriteback(taskId, payload = {}) {
    const order = requireDyeOrderByTaskId(taskId);
    startDyeMaterialWait(order.dyeOrderId, payload.operatorName || '染色工厂');
    return { workOrder: getDyeWorkOrderById(order.dyeOrderId) };
}
export function finishDyeMaterialWaitWriteback(taskId, payload = {}) {
    const order = requireDyeOrderByTaskId(taskId);
    completeDyeMaterialWait(order.dyeOrderId, payload.operatorName || '染色工厂');
    return { workOrder: getDyeWorkOrderById(order.dyeOrderId) };
}
function findSpecialCraftTaskOrder(taskId) {
    const direct = getSpecialCraftTaskOrderById(taskId)
        || listSpecialCraftTaskOrders().find((taskOrder) => taskOrder.taskOrderNo === taskId
...
function getSpecialCraftBindings(taskId) {
    const taskOrder = findSpecialCraftTaskOrder(taskId);
    if (!taskOrder)
        return [];
    return getSpecialCraftBindingsByTaskOrderId(taskOrder.taskOrderId);
}
export function startSpecialCraftTask(taskId, payload = {}) {
    const taskOrder = findSpecialCraftTaskOrder(taskId);
    if (!taskOrder)
        throw new Error(`未找到特殊工艺任务：${taskId}`);
...
export function bindSpecialCraftFeiTicket(taskId, payload = {}) {
    ...
    if (!binding)
        throw new Error('未找到可绑定菲票');
    return { binding };
}
export function reportSpecialCraftDifference(taskId, payload) {
    const bindings = getSpecialCraftBindings(taskId);
    const binding = payload.bindingId
        ...
        : payload.feiTicketNo
            ...
            : bindings[0];
    if (!binding)
        throw new Error('未找到可上报差异的特殊工艺菲票');
...
export function finishSpecialCraftTask(taskId, payload = {}) {
    const taskOrder = findSpecialCraftTaskOrder(taskId);
    if (!taskOrder)
        throw new Error(`未找到特殊工艺任务：${taskId}`);
...
export function submitSpecialCraftHandover(taskId, payload = {}) {
    const taskOrder = findSpecialCraftTaskOrder(taskId);
    if (!taskOrder)
        throw new Error(`未找到特殊工艺任务：${taskId}`);
...
export function startPostFinishingAction(postOrderId, actionType, payload = {}) {
...
export function finishPostFinishingAction(postOrderId, actionType, payload = {}) {
...
export function transferPostFinishedGarmentsToManagedPostFactory(postOrderId, payload = {}) {
...
export function receivePostFinishedGarmentsAtManagedPostFactory(postOrderId, payload = {}) {
...
export function createPostFinishingHandoverWarehouseRecord(postOrderId, payload = {}) {
...
export function getPostFinishingWorkOrderForMobile(execId) {
