...
export const PRINT_WORK_ORDER_STATUS_LABEL = {
    WAIT_ARTWORK: '待花型',
...
export const PRINT_NODE_LABEL = {
    COLOR_TEST: '花型测试',
    PRINT: '打印',
    TRANSFER: '转印',
    HANDOVER: '交出',
};
export const PRINT_REVIEW_STATUS_LABEL = {
    WAIT_RECEIVE: '交出待收货',
    PARTIAL_HANDOVER: '部分交出',
    FULL_HANDOVER: '全部交出',
    HANDOVER_DIFFERENCE: '收货差异',
...
function getGeneratedPrintCraft(index) {
...
function buildPrintDemandId(craftCode, productionOrderId, mockIndex) {
    return buildDictionaryCraftMockDocumentNo('YHXQ', craftCode, productionOrderId, mockIndex);
}
function buildPrintWorkOrderNo(craftCode, productionOrderId, mockIndex) {
    return buildDictionaryCraftMockDocumentNo('YHJG', craftCode, productionOrderId, mockIndex);
}
function getGeneratedPrintContext(index) {
...
function getVisiblePrintWorkOrderIds() {
    return new Set(Array.from(workOrderStore.values())
...
function toGeneratedPrintWorkOrder(order, index) {
...
function listGeneratedPrintWorkOrders() {
...
function getPrintingTaskById(taskId) {
...
function getMachineSeed(factoryId, index = 0) {
...
function setNodeRecords(printOrderId, records) {
    nodeRecordStore.set(printOrderId, records.map((record) => ({ ...record })));
}
function appendNodeRecord(printOrderId, record) {
    const current = nodeRecordStore.get(printOrderId) ?? [];
    current.push({ ...record });
    nodeRecordStore.set(printOrderId, current);
}
function upsertNodeRecord(printOrderId, nodeCode, updater) {
    const current = nodeRecordStore.get(printOrderId) ?? [];
...
function createNodeRecordId(printOrderId, nodeCode) {
...
function seedReviewRecords() {
    ...
    if (!waitReviewOrder?.handoverOrderId)
        return;
    ...
    if (!completedOrder?.handoverOrderId)
        return;
    ...
    if (!rejectedOrder?.handoverOrderId)
        return;
...
function syncOrderFromReview(order, review) {
    if (!review)
        return false;
...
function getPrintCompletedQty(order) {
...
function ensureReviewForHandoverOrder(order, head) {
    const records = getPdaHandoverRecordsByHead(head.handoverId);
...
function syncDerivedWorkflow() {
    ...
    for (const order of workOrderStore.values()) {
        const head = (order.handoverOrderId ? getHandoverOrderById(order.handoverOrderId) : null)
...
function getMutableWorkOrder(printOrderId) {
    seedDomain();
    syncDerivedWorkflow();
...
function getMutableNodeRecord(printOrderId, nodeCode) {
    seedDomain();
...
export function getPrintWorkOrderStatusLabel(status) {
    return PRINT_WORK_ORDER_STATUS_LABEL[status];
}
export function getPrintReviewStatusLabel(status) {
    return PRINT_REVIEW_STATUS_LABEL[status];
}
export function listPrintWorkOrders() {
    syncDerivedWorkflow();
    return listGeneratedPrintWorkOrders().map((order) => cloneWorkOrder(order));
}
export function getPrintWorkOrderById(printOrderId) {
    syncDerivedWorkflow();
...
export function getPrintWorkOrderByTaskId(taskId) {
    syncDerivedWorkflow();
    const order = listGeneratedPrintWorkOrders().find((item) => item.taskId === taskId);
    return order ? cloneWorkOrder(order) : undefined;
}
export function listPrintExecutionNodeRecords(printOrderId) {
    seedDomain();
    const visibleIds = getVisiblePrintWorkOrderIds();
    if (printOrderId) {
        if (!visibleIds.has(printOrderId))
            return [];
...
export function getPrintExecutionNodeRecord(printOrderId, nodeCode) {
    const record = getMutableNodeRecord(printOrderId, nodeCode);
    return record ? cloneNodeRecord(record) : undefined;
}
export function listPrintReviewRecords() {
    syncDerivedWorkflow();
    const visibleIds = getVisiblePrintWorkOrderIds();
...
export function getPrintReviewRecordByOrderId(printOrderId) {
    syncDerivedWorkflow();
    if (!getVisiblePrintWorkOrderIds().has(printOrderId))
        return undefined;
    const review = reviewRecordStore.get(printOrderId);
    return review ? cloneReviewRecord(review) : undefined;
}
export function listPrintMachineOptions(factoryId) {
    return listFactoryPrintMachineCapacities(factoryId);
}
export function validatePrintStartPayload(input) {
    if (!input.printerNo?.trim()) {
        return { ok: false, message: '请选择打印机编号' };
    }
    return { ok: true };
}
export function validatePrintCompletePayload(input) {
...
export function validateReviewRejectPayload(reason) {
    if (!reason.trim()) {
        return { ok: false, message: '请填写收货差异原因' };
    }
    return { ok: true };
}
export function hasDirectTransferToReviewTransition() {
    return false;
}
export function startColorTest(printOrderId, operatorName = '印花工厂') {
...
export function completeColorTest(printOrderId, input) {
    const order = getMutableWorkOrder(printOrderId);
...
export function startPrinting(printOrderId, input) {
...
export function completePrinting(printOrderId, input) {
...
export function startTransfer(printOrderId, operatorName = '印花工厂') {
    const order = getMutableWorkOrder(printOrderId);
    const now = nowTimestamp();
...
export function completeTransfer(printOrderId, input) {
...
export function submitPrintHandover(printOrderId, input = {}) {
    const order = getMutableWorkOrder(printOrderId);
...
function getMutablePrintReceiptReview(printOrderId) {
    const order = getMutableWorkOrder(printOrderId);
...
function applyPrintReceiptState(order, review) {
...
export function confirmPrintReceipt(printOrderId, input) {
...
export function markPrintReceiptDifference(printOrderId, input) {
...
export function approvePrintReview(printOrderId, input) {
    return confirmPrintReceipt(printOrderId, { receivedBy: input.reviewedBy, remark: input.remark });
}
export function rejectPrintReview(printOrderId, input) {
...
export function listPrintingDashboardBuckets() {
...
export function getPrintWorkOrderSummary() {
    syncDerivedWorkflow();
...
export function getPrintOrderHandoverHead(printOrderId) {
    syncDerivedWorkflow();
    const order = workOrderStore.get(printOrderId);
    if (!order?.handoverOrderId)
        return undefined;
...
export function getPrintOrderHandoverRecords(printOrderId) {
    const head = getPrintOrderHandoverHead(printOrderId);
    if (!head)
        return [];
    return getPdaHandoverRecordsByHead(head.handoverId);
}
export function getPrintOrderHandoverSummary(printOrderId) {
    const head = getPrintOrderHandoverHead(printOrderId);
    return {
