...
function createMaterialDraftLogId() {
...
function toSkuScopeLines(lines) {
...
function getRuntimeTaskForDraft(task, draftTaskId) {
    if (draftTaskId) {
    ...
    if (!task)
        return null;
    const runtimeTasks = listRuntimeTasksByBaseTaskId(task.taskId).filter((item) => isRuntimeTaskExecutionTask(item));
    if (runtimeTasks.length === 1)
        return runtimeTasks[0];
    return null;
}
function getTaskSkuScope(order, task, runtimeTask) {
    if (runtimeTask) {
...
function formatPatternSpecText(widthCm, markerLengthM) {
    ...
    if (widthPart && markerPart)
        return `${widthPart} × ${markerPart}`;
    if (widthPart)
        return widthPart;
    if (markerPart)
        return markerPart;
...
function buildPatternEvidence(bomItem, patternFiles) {
    ...
    linkedPatterns.forEach((pattern, index) => {
        ...
        const totalCount = Number.isFinite(pattern.totalPieceCount) && Number(pattern.totalPieceCount) > 0
...
function inferMaterialCategory(item) {
...
function inferMaterialUnit(item, category) {
    const haystack = `${item.type} ${item.name} ${item.spec}`;
    if (category === '面料')
        return '米';
    if (haystack.includes('线'))
        return '卷';
    if (haystack.includes('纽扣'))
        return '颗';
    if (haystack.includes('拉链'))
        return '条';
    if (haystack.includes('标签'))
        return '张';
    return '个';
}
function resolveTaskType(task) {
    ...
    if (processName.includes('印花') || processCode === 'PROC_PRINT')
        return 'PRINT';
    if (processName.includes('染色') || processName.includes('染印') || processCode === 'PROC_DYE' || processCode === 'PROC_DYE_PRINT') {
        return 'DYE';
    }
    if (processName.includes('裁片') || processCode === 'PROC_CUT')
        return 'CUT';
    if (processName.includes('车缝') || processName.includes('缝纫') || processCode === 'PROC_SEW')
        return 'SEW';
    if (processName.includes('后道') ||
...
function getOrderProcessFlags(orderId) {
...
function buildBomCandidates(order, task, runtimeTask) {
    ...
    return techPack.bomItems
        ...
        const skuScopeLabel = applicableSkuCodes.length > 0 ? sourceSkuLabels.join('、') : '全部 SKU';
        const patternEvidence = buildPatternEvidence(item, techPack.patternFiles);
        const patternHint = patternEvidence.patternNames.length > 0
        ...
        const suggestedQty = normalizeQty(scopedQty * item.unitConsumption * (1 + item.lossRate / 100));
        return {
...
function buildSewUpstreamCandidates(order, task, runtimeTask) {
    const techPack = getProductionOrderTechPackSnapshot(order.productionOrderId);
    if (!techPack)
        return [];
    ...
    skuScope.forEach((sku) => {
        ...
        const candidateLines = mapping.lines.filter((line) => {
            ...
            if (!line.applicableSkuCodes || line.applicableSkuCodes.length === 0)
                return true;
        ...
        candidateLines.forEach((line, index) => {
            ...
            const sourceRuleLabel = mapping.status === 'AUTO_DRAFT' ? '款色映射草稿自动建议（待确认）' : '款色映射确认后自动建议';
            candidates.push({
                ...
                sourceBomItemCode: line.bomItemId && bomById.get(line.bomItemId)
...
function buildPostUpstreamCandidates(order, task, runtimeTask) {
...
function buildDraftCandidates(order, taskType, task, runtimeTask) {
...
function toDraftLines(draftId, candidates) {
...
function cloneRequest(record) {
    return { ...record };
}
function createRequestNo(sequence) {
    return `LLXQ202603${String(sequence).padStart(4, '0')}`;
}
function getDefaultRequestStatus(mode) {
    return mode === 'warehouse_delivery' ? '待配料' : '待自提';
}
function summarizeMaterials(lines) {
    if (lines.length === 0)
        return '无物料';
...
function applyTaskBinding(request) {
    const binding = {
...
function markDraftNotApplicable(taskId, operatorName = '跟单员', operateAt = toTimestamp()) {
...
function seedCreatedDraft(taskId, mode, createdAt, createdBy, requestStatus) {
    const baseTaskId = resolveBaseTaskId(taskId);
    const draft = materialRequestDrafts.find((item) => item.taskId === taskId) ??
        materialRequestDrafts.find((item) => resolveBaseTaskId(item.taskId) === baseTaskId);
    if (!draft)
...
const materialDraftOperationLogs = [];
const materialRequestDrafts = buildInitialDrafts();
const materialRequests = [];
function appendMaterialDraftOperationLog(log) {
    materialDraftOperationLogs.push({
        id: createMaterialDraftLogId(),
        ...log,
    });
}
function touchDraft(draft, by, at = toTimestamp()) {
    draft.updatedBy = by;
    draft.updatedAt = at;
}
function seedSystemAutoDraftLogs() {
...
if (seedEditableDraft && seedEditableDraft.lines[0]) {
    ...
    setMaterialDraftLineConfirmedQty(seedEditableDraft.draftId, seedEditableDraft.lines[0].lineId, Math.max(1, seedEditableDraft.lines[0].confirmedQty - 80), 'Mira Handayani');
}
...
function rebuildDraftLines(draft) {
...
export function listMaterialRequestDraftsByOrder(orderId) {
    return materialRequestDrafts
        .filter((draft) => draft.productionOrderId === orderId)
        .sort((a, b) => a.taskId.localeCompare(b.taskId))
        .map(cloneDraft);
}
export function getMaterialRequestDraftById(draftId) {
    const draft = getDraftById(draftId);
    return draft ? cloneDraft(draft) : null;
}
export function getMaterialRequestDraftSummaryByOrder(orderId) {
...
export function getMaterialDraftIndicatorsByOrder(orderId) {
    ...
    let materialDraftSummaryStatus = 'none';
    if (summary.status === 'pending')
        materialDraftSummaryStatus = 'pending';
    if (summary.status === 'partial_created')
        materialDraftSummaryStatus = 'partial_confirmed';
    if (summary.status === 'created')
        materialDraftSummaryStatus = 'confirmed';
...
export function setMaterialDraftNeedMaterial(draftId, needMaterial, operatorName = '跟单员') {
    const draft = getDraftById(draftId);
    if (!draft || draft.draftStatus === 'created')
        return;
...
export function setMaterialDraftMode(draftId, materialMode, operatorName = '跟单员') {
    const draft = getDraftById(draftId);
    if (!draft || draft.draftStatus === 'created')
        return;
    if (draft.materialMode === materialMode)
        return;
    const now = toTimestamp();
...
export function setMaterialDraftRemark(draftId, remark, operatorName = '跟单员') {
    const draft = getDraftById(draftId);
    if (!draft || draft.draftStatus === 'created')
        return;
    if (draft.remark === remark)
        return;
    draft.remark = remark;
    touchDraft(draft, operatorName, toTimestamp());
}
export function toggleMaterialDraftLine(draftId, lineId, selected, operatorName = '跟单员') {
    const draft = getDraftById(draftId);
    if (!draft || draft.draftStatus === 'created')
        return;
    const targetLine = draft.lines.find((line) => line.lineId === lineId);
    if (!targetLine || targetLine.selected === selected)
        return;
...
export function setMaterialDraftLineConfirmedQty(draftId, lineId, qty, operatorName = '跟单员') {
    const draft = getDraftById(draftId);
    if (!draft || draft.draftStatus === 'created')
        return;
    const targetLine = draft.lines.find((line) => line.lineId === lineId);
    ...
    if (!nextLine || nextLine.confirmedQty === targetLine.confirmedQty)
        return;
    const now = toTimestamp();
    touchDraft(draft, operatorName, now);
    appendMaterialDraftOperationLog({
...
export function restoreMaterialDraftSuggestion(draftId, operatorName = '跟单员') {
    const draft = getDraftById(draftId);
    if (!draft || draft.draftStatus === 'created')
        return;
...
export function listMaterialDraftSupplementOptions(draftId) {
    const draft = getDraftById(draftId);
    if (!draft)
        return [];
    const order = getOrderById(draft.productionOrderId);
    ...
    const runtimeTask = getRuntimeTaskById(draft.taskId) ?? getRuntimeTaskForDraft(task);
    const candidates = buildDraftCandidates(order, draft.taskType, task, runtimeTask);
    const selectedRefs = new Set(draft.lines.map((line) => `${line.sourceType}:${line.sourceRef}`));
    return candidates.filter((candidate) => !selectedRefs.has(`${candidate.sourceType}:${candidate.sourceRef}`));
}
export function addMaterialToDraft(draftId, optionKeys, operatorName = '跟单员') {
    const draft = getDraftById(draftId);
    if (!draft || draft.draftStatus === 'created')
        return 0;
...
export function confirmMaterialRequestDraft(draftId, operator) {
    const draft = getDraftById(draftId);
    if (!draft) {
...
export function listMaterialRequestsByOrder(orderNo) {
    return listMaterialRequests().filter((item) => item.productionOrderNo === orderNo);
}
export function listMaterialDraftOperationLogsByOrder(orderId) {
...
export function getTaskMaterialRequestBinding(taskId) {
...
export function getMaterialModeLabel(materialMode) {
...
export function getSupplementOptionDisplayRows(draftId) {
