import { FACTORY_ADMIN_ROLE_ID, FACTORY_ADMIN_ROLE_NAME, FACTORY_ONBOARDING_REQUIRED_FIELD_OPTIONS, canCreateFactoryProfile, canFactoryEditOnboarding, canFactoryEnterBusiness, canFactorySubmitOnboarding, getOnboardingNodeByStatus, isFactoryAccountLocked, normalizeOnboardingStatus, normalizeReviewResult, } from './factory-onboarding-domain.ts';
import { clearFactoryOnboardingApplicantSession, createEmptyFactoryOnboardingDraft, createFactoryOnboardingApplicantSession, findFactoryOnboardingApplicationByLoginId, getFactoryOnboardingApplicantSession, getFactoryOnboardingApplicationById, canStartNewOnboarding, buildAdminAccountFromFactoryShortName, getLockedFactoryNameReason, validateFactoryShortNameUnique, listFactoryOnboardingApplications, saveFactoryOnboardingApplication, setFactoryOnboardingApplicantSession, } from './factory-onboarding-store.ts';
import { clearPdaSession, getPdaSession, upsertOfficialFactoryAdminFromOnboarding, } from './store-domain-pda.ts';
...
import { getSampleVerificationByApplicationId, getSampleVerificationById, } from './factory-sample-verification-store.ts';
...
function buildCompletenessItem(itemCode, itemName, weight, completedCount, totalCount, missingReason) {
...
export function calculateOnboardingCompleteness(application) {
    ...
    const items = [
        buildCompletenessItem('IDENTITY', '基础身份信息', 15, [application.applicantName.trim(), application.identityNo.trim(), hasValidIdentityFile(application.identityFile) ? 'valid' : ''].filter(Boolean).length, 3, !application.applicantName.trim()
            ? '缺少姓名'
            : !application.identityNo.trim()
                ...
                : hasValidIdentityFile(application.identityFile)
                    ? '已完整'
                    : '缺少身份证复印件/电子文件'),
        buildCompletenessItem('FACTORY_BASE', '工厂基础信息', 25, [
        ...
        ].filter(Boolean).length, 6, !application.factoryShortName.trim()
            ? '缺少工厂简称'
            : !application.factoryCompanyName.trim()
                ? '缺少工厂/公司名称'
                : !application.address.trim()
        ...
        buildCompletenessItem('MACHINE_CAPABILITY_LINK', '机器与工序工艺关联', 10, machines.length > 0 ? linkedCompleted : 0, Math.max(1, machines.length || 1), machines.length <= 0
            ? '暂无机器明细'
...
export function getCompletenessMissingItems(application) {
...
function resolveFactoryTypeMatchCode(capability) {
    if (capability.processCode === 'CUT_PANEL' || ['普通裁', '激光定位裁', '定向裁', '定位裁'].includes(capability.craftName))
        return 'CUTTING_FACTORY';
    if (capability.processCode === 'PRINT')
        return 'PRINTING_FACTORY';
    if (capability.processCode === 'DYE')
        return 'DYEING_FACTORY';
    if (capability.processCode === 'POST_FINISHING' || ['质检', '复检', '包装', '熨烫'].includes(capability.craftName))
        return 'POST_FINISHING_FACTORY';
    if (capability.processCode === 'SEW')
        return 'SEWING_FACTORY';
...
export function inferFactoryTypesFromCapabilities(selectedCapabilities) {
...
export function getPrimaryFactoryType(matchResults) {
    ...
    if (uniqueCodes.length >= 3)
        return 'MULTI_CAPABILITY_FACTORY';
    return matchResults[0]?.factoryTypeCode || 'CUTTING_FACTORY';
}
export function buildFactoryTypeMatchReason(matchResults) {
...
function isSelectableCraft(processCode, craftCode) {
...
function getCapabilityKeySet(capabilities) {
    return new Set(capabilities.map((item) => `${item.processCode}:${item.craftCode}`));
}
export function getMachineValidationResult(machine, selectedCapabilities) {
...
export function applyMachineValidation(machines, selectedCapabilities) {
...
export function calculateNodeElapsedMinutes(nodeLog, now = nowTimestamp()) {
    if (nodeLog.nodeStatus === '未开始' || nodeLog.nodeStatus === '已终止')
        return 0;
...
export function formatNodeElapsedText(minutes) {
...
function buildNodeElapsedText(nodeLog, now = nowTimestamp()) {
    if (nodeLog.nodeStatus === '未开始')
        return '-';
    if (nodeLog.nodeStatus === '已终止')
        return '已终止';
    return formatNodeElapsedText(calculateNodeElapsedMinutes(nodeLog, now));
}
function buildCurrentFieldValueMap(application) {
    return {
...
function buildApplicationNo() {
    return `FON-${Date.now()}`;
}
function buildFactoryTempId() {
    return `FACTORY-TEMP-${Date.now()}`;
}
function createNodeLog(nodeName, nodeStatus, enteredAt, operator, remark, leftAt) {
    const base = {
...
function refreshNodeLogDerived(nodeLog, application, now = nowTimestamp()) {
...
export function refreshApplicationDerived(application, now = nowTimestamp()) {
...
function cloneDraftPayload(payload) {
    ...
    const machines = applyMachineValidation(payload.machines.map((machine, index) => ({
        ...machine,
...
function validateDraftIdentityForSave(payload, _confirmPassword) {
...
export function validateFactoryOnboardingDraftPayload(payload, _confirmPassword = '') {
    validateFactoryShortNameUnique(payload.factoryShortName, payload.applicationId);
    if (!payload.applicantName.trim())
        throw new Error('请填写姓名');
    if (!payload.identityNo.trim())
        throw new Error('请填写身份证号码/护照号码');
    if (!hasValidIdentityFile(payload.identityFile))
        throw new Error('请上传身份证复印件/电子文件');
    if (!payload.factoryCompanyName.trim())
        throw new Error('请填写工厂/公司名称');
    if (!canStartNewOnboarding(payload.factoryCompanyName, payload.applicationId)) {
        throw new Error(getLockedFactoryNameReason(payload.factoryCompanyName, payload.applicationId));
    }
    if (!payload.address.trim())
        throw new Error('请填写地址');
    if (!payload.mobilePhone.trim() || !hasBasicMobileValue(payload.mobilePhone))
        throw new Error('请填写手机号');
    if (!payload.sourceChannel.trim())
        throw new Error('请填写来源');
    if (!payload.ppicName.trim())
        throw new Error('请填写收到此通知的 PPIC 姓名');
    if (!payload.effectiveWorkerCount)
        throw new Error('请填写有效工人数量');
    if (payload.effectiveWorkerCount <= 0)
        throw new Error('有效工人数量必须大于 0');
    if (!payload.machineTotalCount)
        throw new Error('请填写机器数量');
    if (payload.machineTotalCount <= 0)
        throw new Error('机器数量必须大于 0');
    if (!payload.availableStartDate.trim())
        throw new Error('请选择可开始合作时间');
    if (payload.selectedCapabilities.length <= 0)
        throw new Error('请至少选择一个工序工艺');
    payload.selectedCapabilities.forEach((item) => {
        if (!item.processCode || !item.craftCode)
            throw new Error('请选择工序下的具体工艺');
        if (!isSelectableCraft(item.processCode, item.craftCode))
            throw new Error('请选择工序下的具体工艺');
    });
    if (payload.machines.length <= 0)
        throw new Error('请至少添加一条机器明细');
    payload.machines.forEach((machine) => {
        if (!machine.machineName.trim())
            throw new Error('请填写机器名称');
        if (!machine.machineCount)
            throw new Error('请填写机器数量');
        if (machine.machineCount <= 0)
            throw new Error('机器数量必须大于 0');
    });
...
function buildApplicationFromPayload(payload, status, currentNode) {
...
function getLatestAction(application, nodeName) {
...
export function getCurrentNodeLog(application) {
...
export function getNodeActionCount(application, nodeName) {
    return application.actionLogs.filter((item) => item.nodeName === nodeName).length;
}
export function getCurrentNodeElapsedText(application) {
    return getCurrentNodeLog(application)?.elapsedText || '-';
}
export function updateNodeLogOnTransition(application, fromNode, toNode, _actionName, operator, options = {}) {
...
function appendAction(application, input) {
...
function buildStatusTipLabel(status) {
    const map = {
        草稿: '草稿',
        待平台审核: '待平台审核',
        平台审核退回: '平台审核退回',
...
export function saveFactoryOnboardingDraft(payload, confirmPassword) {
    const normalized = cloneDraftPayload(payload);
    ...
    if (!canFactoryEditOnboarding(current.status))
        return current;
...
export function submitFactoryOnboardingApplication(payload, confirmPassword) {
    const normalized = cloneDraftPayload(payload);
    ...
    if (isResubmit) {
        ...
        if (pending) {
            ...
            application = {
                ...application,
                supplementRecords: application.supplementRecords.map((item) => item.supplementId === pending.supplementId
...
export function authenticateFactoryOnboardingAdmin(loginId, password) {
    ...
    if (normalizeLoginId(normalized.adminAccount.loginId) !== normalizeLoginId(loginId))
        return null;
    if (normalized.adminAccount.password.trim() !== password.trim())
        return null;
    if (normalized.accountLocked)
        return null;
    return normalized;
}
export function getFactoryOnboardingLoginFailureMessage(loginId, password) {
    const application = findFactoryOnboardingApplicationByLoginId(loginId);
    if (!application)
        return '账号或密码错误';
    const normalized = refreshApplicationDerived(application);
...
export function getFactoryOnboardingCurrentNodeSummary(application) {
...
export function getPdaCurrentAuthSession() {
...
export function buildPdaAuthLoginPath(returnTo) {
    return buildQueryPath('/fcs/pda/auth/login', { returnTo });
}
export function buildPdaAuthOnboardingPath(returnTo, applicationId) {
    return buildQueryPath('/fcs/pda/auth/onboarding', { returnTo, applicationId });
}
export function resolvePdaPostLoginRoute(session, returnTo) {
    if (!session)
        return buildPdaAuthLoginPath(returnTo);
    if (session.kind === 'PDA')
        return returnTo?.trim() || '/fcs/pda/exec';
    const application = refreshApplicationDerived(session.application);
    if (canFactoryEnterBusiness(application.status))
        return returnTo?.trim() || '/fcs/pda/exec';
    return buildPdaAuthOnboardingPath(returnTo, application.applicationId);
}
function getPdaReasonCode(status) {
    if (status === '草稿')
        return 'DRAFT';
    if (status === '平台审核退回')
        return 'RETURNED';
    ...
    if (status === '待样衣验证' || status === '待工厂确认收样' || status === '待工厂提交样衣审核' || status === '样衣审核退回')
        return 'WAITING_SAMPLE';
    if (status === '待平台审核样衣')
...
export function getPdaFactoryAccessState() {
...
export function ensurePdaAccessForRoute(targetRoute) {
    ...
    if (accessState.canAccessBusiness)
        return { allowed: true };
...
export function getPdaOnboardingApplicationFromSession() {
    return getPdaFactoryAccessState().onboardingApplication;
}
export function createPdaOnboardingSessionFromApplication(application) {
    return createFactoryOnboardingApplicantSession(application);
}
export function activateOnboardingSession(application) {
    setFactoryOnboardingApplicantSession(createFactoryOnboardingApplicantSession(application));
    clearPdaSession();
}
export function logoutPdaAccess() {
    clearPdaSession();
    clearFactoryOnboardingApplicantSession();
}
...
export function reviewFactoryOnboardingApplication(input) {
    ...
    let toStatus;
    let toNode;
    let openingNodeStatus = '进行中';
    let actionName;
    let transitionRemark;
    if (reviewResult === '已通过') {
        toStatus = '待样衣验证';
        toNode = '样衣验证';
        actionName = '平台初审已通过';
...
function getOnboardingDigits(value) {
    return value.replace(/\D/g, '').slice(-4).padStart(4, '0');
}
function mapOnboardingFactoryType(typeCode) {
    const map = {
...
function buildFactoryProcessAbilitiesFromOnboarding(application) {
    const grouped = new Map();
    application.selectedCapabilities.forEach((capability, index) => {
        const current = grouped.get(capability.processCode);
        if (current) {
            if (!current.craftCodes.includes(capability.craftCode))
                current.craftCodes.push(capability.craftCode);
            const craftNames = current.craftNames || [];
            if (!craftNames.includes(capability.craftName))
                current.craftNames = [...craftNames, capability.craftName];
...
function getPassedSampleVerification(application) {
...
export function canConvertOnboardingToOfficialFactory(application) {
    if (!application)
        return false;
    ...
    if (application.createdFactoryId)
        return false;
    if (!application.adminAccount || application.adminAccount.accountStatus === '已转正式')
        return false;
    if (application.selectedCapabilities.length <= 0 || application.machines.length <= 0)
        return false;
    const sampleVerification = getPassedSampleVerification(application);
    return sampleVerification?.status === '样衣审核通过';
}
export function validateOfficialFactoryConversion(application) {
    const errors = [];
    if (!canCreateFactoryProfile(application.status)) {
...
export function buildFactoryProfileFromOnboarding(application) {
...
export async function convertOnboardingAdminAccountToOfficial(application, createdFactory) {
...
export function createInitialCapacityProfileFromOnboarding(application, createdFactory) {
    return createCapacityProfileFromOnboardingStore(application, createdFactory);
}
export function buildOfficialFactoryConversionRecord(application, createdFactory, capacityProfile, operator, officialAdminAccountId) {
...
export async function convertOnboardingToOfficialFactory(applicationId, operator) {
    const application = getFactoryOnboardingApplicationById(applicationId);
    if (!application)
        throw new Error('未找到入驻申请');
...
export async function confirmFactoryOnboardingCooperation(input) {
    const result = await convertOnboardingToOfficialFactory(input.applicationId, input.operator);
    return result.application;
}
export function createDefaultMachineDraft(seed) {
    return {
...
export function createCapabilityFromSelection(processCode, craftCode) {
    if (!processCode || !craftCode)
        return null;
    const processName = getProcessName(processCode);
    const craftName = getCraftName(processCode, craftCode);
    if (!processName || !craftName || !isSelectableCraft(processCode, craftCode))
        return null;
    return {
        processCode,
        processName,
...
export function getOnboardingStatusActionLabel(application) {
    if (!application)
        return '提交入驻申请';
    if (application.status === '平台审核退回')
        return '重新提交入驻申请';
    if (application.status === '草稿')
        return '提交入驻申请';
    return '查看入驻进度';
}
export function canEditOnboardingApplication(application) {
    if (!application)
        return true;
    return canFactoryEditOnboarding(application.status);
}
export function canSubmitOnboardingApplication(application) {
    if (!application)
        return true;
    return canFactorySubmitOnboarding(application.status);
}
export function getOnboardingStatusTip(application) {
    if (!application)
        return '请补全入驻信息后提交平台审核。';
    return buildStatusTipLabel(normalizeOnboardingStatus(application.status));
}
export function getLatestReviewRecord(application) {
    if (!application || application.reviewRecords.length <= 0)
        return null;
    return [...application.reviewRecords].sort((left, right) => parseDateMs(right.reviewedAt) - parseDateMs(left.reviewedAt))[0] || null;
}
export function getLatestSupplementRecord(application) {
    if (!application || application.supplementRecords.length <= 0)
        return null;
    return [...application.supplementRecords].sort((left, right) => right.supplementRoundNo - left.supplementRoundNo)[0] || null;
}
export function listCurrentFieldValues(application) {
    if (!application)
        return [];
...
export { canCreateFactoryProfile, canFactoryEditOnboarding, canFactoryEnterBusiness, canFactorySubmitOnboarding, getOnboardingNodeByStatus, isFactoryAccountLocked, normalizeOnboardingStatus, };
