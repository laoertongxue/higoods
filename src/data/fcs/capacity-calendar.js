import { listCapacityCalendarOverrides, } from './capacity-calendar-overrides.ts';
import { CAPACITY_OVERLOAD_REMAINING_THRESHOLD, CAPACITY_TIGHT_THRESHOLD_RATIO, calculateCapacityRemainingStandardHours, } from './capacity-rules.ts';
import { createCapacityStandardTimeEvaluationContext, listCapacityCommitments, listCapacityFreezes, resolveCapacityUsageTaskIdentity, resolveCapacityStandardTimeWindow, resolveFactoryTaskStandardTimeJudgement, } from './capacity-usage-ledger.ts';
...
import { getFactoryMasterRecordById, listBusinessFactoryMasterRecords, } from './factory-master-store.ts';
import { assertProcessCraftExists, getActiveCraftOptionsByProcess, getActiveProcessOptions, getCapacityProcessCraftOptions, resolveProcessCraft, } from './process-craft-dict.ts';
import { listRuntimeExecutionTasks, resolveRuntimeAllocatableGroupPublishedSam, resolveRuntimeTaskPublishedSam, } from './runtime-process-tasks.ts';
import { productionOrders } from './production-orders.ts';
import { getProductionOrderProcessEntries } from './production-order-tech-pack-runtime.ts';
export const CAPACITY_CALENDAR_STATUS_LABEL = {
    NORMAL: '正常',
    TIGHT: '紧张',
    OVERLOADED: '超载',
    PAUSED: '暂停',
};
export const CAPACITY_CALENDAR_CONSTRAINT_STATUS_LABEL = {
    ...CAPACITY_CALENDAR_STATUS_LABEL,
    DATE_INCOMPLETE: '日期不足',
    SAM_MISSING: '工时缺失',
};
export const TASK_SAM_RISK_CONCLUSION_LABEL = {
    CAPABLE: '可承载',
    TIGHT: '紧张',
    EXCEEDS_WINDOW: '超出窗口',
    PAUSED: '暂停',
...
const FACTORY_CALENDAR_WINDOW_OPTIONS = [
    { value: 7, label: '未来 7 天' },
    { value: 15, label: '未来 15 天' },
    { value: 30, label: '未来 30 天' },
];
const TASK_RISK_CONCLUSION_OPTIONS = [
    { value: '', label: '全部风险结论' },
...
const CAPACITY_USAGE_SOURCE_LABEL = {
...
function normalizeTaskDemandIdentity(identity) {
    const resolved = assertProcessCraftExists(identity.processCode, identity.craftCode, `产能任务字典映射 ${identity.processCode} / ${identity.craftCode}`);
    return {
...
function resolvePostRollupCraftCode(task) {
    const childProcessCodes = Array.from(new Set((task.rolledUpChildProcessCodes ?? [])
        .map((item) => item.trim())
        .filter((item) => Boolean(item))
        .filter((item) => Boolean(resolveProcessCraft('POST_FINISHING', item)))));
    if (childProcessCodes.length === 0)
        return task.craftCode ?? '';
    if (childProcessCodes.length === 1)
        return childProcessCodes[0];
    const nodeSamByProcessCode = new Map();
    ...
    for (const entry of getProductionOrderProcessEntries(task.productionOrderId)) {
        if (!childProcessCodeSet.has(entry.processCode))
            continue;
        if (!resolveProcessCraft('POST_FINISHING', entry.processCode))
            continue;
        const standardTimeMinutes = Number(entry.standardTimeMinutes);
        if (!Number.isFinite(standardTimeMinutes) || standardTimeMinutes <= 0)
            continue;
        nodeSamByProcessCode.set(entry.processCode, roundSam((nodeSamByProcessCode.get(entry.processCode) ?? 0) + standardTimeMinutes));
    }
    return childProcessCodes
        ...
        if (rightSam !== leftSam)
            return rightSam - leftSam;
        return left.localeCompare(right);
    })[0];
}
function resolveDemandIdentityFromSourceEntry(task) {
    const sourceEntryId = task.sourceEntryId?.trim();
    if (!sourceEntryId)
        return null;
    const matchedEntry = getProductionOrderProcessEntries(task.productionOrderId).find((entry) => entry.id === sourceEntryId);
    if (!matchedEntry?.craftCode)
        return null;
...
function buildDemandIdentity(task) {
    ...
    const hasDirectDictionaryMapping = Boolean(task.processBusinessCode && rawCraftCode && resolveProcessCraft(task.processBusinessCode, rawCraftCode));
    if (!hasDirectDictionaryMapping) {
        const sourceEntryIdentity = resolveDemandIdentityFromSourceEntry(task);
        if (sourceEntryIdentity)
            return sourceEntryIdentity;
    }
...
function buildUnallocatedKey(input) {
    return [input.date, input.processCode, input.craftCode].join('::');
}
function buildUsageSchedule(input, reasonWhenMissing) {
...
function resolveUsageIdentity(input, taskMap, displayLabels) {
...
function shouldTrackUnallocatedDemand(task) {
...
function buildTaskWindowText(task, publishedSamTotal) {
...
function buildBottleneckSourceObjectKey(input) {
    return `${input.taskId}::${input.allocationUnitId ?? ''}`;
}
function buildTaskSchedule(task, publishedSamTotal) {
...
function resolveCommittedFactory(task) {
...
function shouldTrackDemand(task) {
    return task.status !== 'DONE' && task.status !== 'CANCELLED';
}
function resolveOverrideDisplayLabels() {
    const map = new Map();
...
function getOverrideSpecificity(override) {
    if (override.processCode && override.craftCode)
        return 3;
    if (override.processCode)
        return 2;
    return 1;
}
function buildOverrideScopeLabel(override, labels) {
    if (!override.processCode)
        return '整厂暂停';
...
function matchesPauseOverride(override, date, factoryId, processCode, craftCode) {
    if (override.overrideType !== 'PAUSE')
        return false;
    if (override.factoryId !== factoryId)
        return false;
    if (date < override.startDate || date > override.endDate)
        return false;
    if (override.processCode && override.processCode !== processCode)
        return false;
    if (override.craftCode && override.craftCode !== craftCode)
        return false;
    return true;
}
function resolvePauseOverride(overrides, overrideLabels, input) {
    const matched = overrides
        .filter((override) => matchesPauseOverride(override, input.date, input.factoryId, input.processCode, input.craftCode))
...
export function buildCapacityStatusBadge(status) {
    if (status === 'PAUSED')
        return { label: CAPACITY_CALENDAR_STATUS_LABEL[status], tone: 'danger' };
    if (status === 'OVERLOADED')
        return { label: CAPACITY_CALENDAR_STATUS_LABEL[status], tone: 'danger' };
    if (status === 'TIGHT')
        return { label: CAPACITY_CALENDAR_STATUS_LABEL[status], tone: 'warning' };
...
export function computeCapacityStatus(input) {
    ...
    return {
        ...
        reason: (input.baseSupplySam ?? availableSam) > 0
...
export function isCapacityPaused(input, overrides = listCapacityCalendarOverrides(), overrideLabels = resolveOverrideDisplayLabels()) {
    return Boolean(resolvePauseOverride(overrides, overrideLabels, input));
}
export function resolveFactoryWindowStatus(input, options) {
...
function buildComparisonRowMap(rows) {
...
export function createCapacityCalendarEvaluationContext(calendarData = buildCapacityCalendarData()) {
    return {
        calendarData,
        comparisonRowMap: buildComparisonRowMap(calendarData.comparisonRows),
    };
}
function compareConstraintPriority(status) {
    const order = {
        PAUSED: 5,
        OVERLOADED: 4,
        TIGHT: 3,
...
function buildConstraintReason(result) {
...
export function evaluateRuntimeTaskCapacityConstraint(input) {
    ...
    const allocations = schedule.allocations.map((allocation) => {
        ...
        const baselineCommitted = roundSam(Math.max((row?.committedSam ?? 0) - (currentTaskAlreadyCommitted ? allocation.demandSam : 0), 0));
    ...
    const worst = allocations.reduce((current, item) => {
        if (!current)
            return item;
    ...
    const decision = finalStatus === 'PAUSED' || finalStatus === 'OVERLOADED'
        ? 'BLOCK'
        : finalStatus === 'TIGHT'
            ? 'WARN'
            : 'ALLOW';
    const reasons = [worst?.reason].filter((item) => Boolean(item));
...
export function buildCapacityCalendarData() {
    const comparisonMap = new Map();
    const unallocatedMap = new Map();
    const unscheduledRows = [];
    const missingSamRows = [];
    const scheduledDateSet = new Set();
    const relevantCraftKeys = new Set();
    const overrideLabels = resolveOverrideDisplayLabels();
    const overrides = listCapacityCalendarOverrides();
    ...
    function pushUnscheduledRow(input) {
    ...
    for (const commitment of activeCommitments) {
        ...
        if (schedule.kind === 'UNSCHEDULED') {
            pushUnscheduledRow({
                ...
                factoryName: getFactoryMasterRecordById(commitment.factoryId)?.name
    ...
    for (const freeze of activeFreezes) {
        ...
        if (schedule.kind === 'UNSCHEDULED') {
            pushUnscheduledRow({
                ...
                assignmentMode: task?.assignmentMode ?? 'DIRECT',
                factoryName: getFactoryMasterRecordById(freeze.factoryId)?.name
                    ?? task?.assignedFactoryName
                    ?? freeze.factoryId,
    ...
    for (const task of tasks) {
        ...
        if (coveredTaskIds.has(task.taskId))
            continue;
        if (!shouldTrackUnallocatedDemand(task))
            continue;
        relevantCraftKeys.add(`${identity.processCode}::${identity.craftCode}`);
        const schedule = buildTaskSchedule(task, publishedSamTotal);
        if (schedule.kind === 'UNSCHEDULED') {
    ...
    const activeCraftKeys = relevantCraftKeys.size > 0 ? relevantCraftKeys : new Set();
    for (const factory of listBusinessFactoryMasterRecords()) {
        ...
        for (const { row, entry } of entries) {
            ...
            if (activeCraftKeys.size > 0 && !activeCraftKeys.has(craftKey))
                continue;
            const computed = computeFactoryCapacityEntryResult(row, entry.values, getFactoryCapacityEquipmentSummary(factory.id, row.processCode, row.craftCode));
    ...
    const comparisonRows = [...comparisonMap.values()]
        ...
        if (left.remainingSam !== right.remainingSam)
            return left.remainingSam - right.remainingSam;
    ...
    const unallocatedRows = [...unallocatedMap.values()].sort((left, right) => {
        if (left.date !== right.date)
            return left.date.localeCompare(right.date);
        if (right.demandSam !== left.demandSam)
            return right.demandSam - left.demandSam;
        const processCompare = left.processName.localeCompare(right.processName);
        if (processCompare !== 0)
            return processCompare;
        return left.craftName.localeCompare(right.craftName);
    ...
    unscheduledRows.sort((left, right) => {
        if (right.standardSamTotal !== left.standardSamTotal)
            return right.standardSamTotal - left.standardSamTotal;
...
function buildFactoryCalendarRowKey(input) {
    return [input.date, input.processCode, input.craftCode].join('::');
}
function resolveFactoryCalendarWindowDays(value) {
    if (value === 7 || value === 30)
        return value;
    return 15;
}
function buildFutureDateWindow(days, today = toDayStart(new Date())) {
    return Array.from({ length: days }, (_, index) => formatDateKey(addDays(today, index)));
}
function buildFactoryCalendarWindowText(startDate, endDate) {
    ...
    if (start || end)
        return start ?? end ?? '日期不足';
    return '日期不足';
}
function buildFactoryCalendarObjectKey(input) {
    return `${input.taskId}::${input.allocationUnitId ?? ''}`;
}
function sortFactoryCalendarRows(left, right) {
    if (left.date !== right.date)
        return left.date.localeCompare(right.date);
    const processCompare = left.processName.localeCompare(right.processName);
    if (processCompare !== 0)
...
function buildFactoryCalendarEmptySummary() {
    return {
        supplyTotal: 0,
        committedTotal: 0,
        frozenTotal: 0,
...
function buildFactoryCalendarSummary(rows) {
...
export function buildFactoryCalendarData(input) {
    ...
    const taskMap = new Map(listRuntimeExecutionTasks().map((task) => [task.taskId, task]));
    const displayLabels = resolveOverrideDisplayLabels();
    const overrides = listCapacityCalendarOverrides();
    const rowSeedMap = new Map();
    const processOptionMap = new Map();
    const craftOptionMap = new Map();
    ...
    function getRowSeed(identity, date) {
        ...
        const created = {
            ...
            committedSources: [],
            frozenSources: [],
            committedObjectKeys: new Set(),
            frozenObjectKeys: new Set(),
        };
    ...
    if (selectedFactoryId) {
        for (const { row, entry } of listFactoryCapacityEntries(selectedFactoryId)) {
            ...
            const dailySupplySam = roundSam(Math.max(computeFactoryCapacityEntryResult(row, entry.values, getFactoryCapacityEquipmentSummary(selectedFactoryId, row.processCode, row.craftCode)).resultValue ?? 0, 0));
        ...
        const appendUsageSources = (items, sourceKind) => {
            for (const item of items) {
                ...
                for (const allocation of schedule.allocations) {
                    if (!dateSet.has(allocation.date))
                        continue;
    ...
    const craftOptions = craftOptionsByFactory.filter((item) => selectedProcessCode ? item.processCode === selectedProcessCode : true);
    const rows = [...rowSeedMap.values()]
        .filter((row) => {
        if (selectedCraftCode)
            return row.craftCode === selectedCraftCode;
        if (selectedProcessCode)
            return row.processCode === selectedProcessCode;
        return true;
    })
        .map((row) => {
        ...
        return {
            ...
            committedSources: [...row.committedSources].sort((left, right) => {
                if (right.dailySam !== left.dailySam)
                    return right.dailySam - left.dailySam;
                return left.taskId.localeCompare(right.taskId);
            }),
...
function buildBottleneckCraftRowKey(input) {
    return [input.processCode, input.craftCode].join('::');
}
function buildCapacityBottleneckSummary(input) {
    return {
...
function aggregateCraftBottlenecks(dailyRows, unallocatedSamByCraft, unscheduledSamByCraft) {
    ...
    for (const row of dailyRows) {
        ...
        if (row.status === 'PAUSED')
            dateRow.status = 'PAUSED';
        else if (row.status === 'OVERLOADED' && dateRow.status !== 'PAUSED')
            dateRow.status = 'OVERLOADED';
        else if (row.status === 'TIGHT' && dateRow.status === 'NORMAL')
            dateRow.status = 'TIGHT';
    ...
    return [...craftMap.values()]
        ...
        const maxGapSam = roundSam(dateRows.reduce((max, item) => (item.remainingSam < 0 ? Math.max(max, Math.abs(item.remainingSam)) : max), 0));
        ...
        .sort((left, right) => {
        if (right.maxGapSam !== left.maxGapSam)
            return right.maxGapSam - left.maxGapSam;
        if (left.windowRemainingSam !== right.windowRemainingSam)
            return left.windowRemainingSam - right.windowRemainingSam;
        const processCompare = left.processName.localeCompare(right.processName);
        if (processCompare !== 0)
            return processCompare;
        return left.craftName.localeCompare(right.craftName);
...
function aggregateDateBottlenecks(dailyRows, unallocatedSamByDate) {
    ...
    for (const row of dailyRows) {
        const current = dateMap.get(row.date) ?? {
            ...
            frozenSam: 0,
            remainingSam: 0,
            overloadedFactories: new Set(),
            pausedFactories: new Set(),
            craftRemaining: new Map(),
            tightCrafts: new Set(),
            maxGapSam: 0,
            detailRows: [],
        };
...
function aggregateUnallocatedDemand(tasks) {
    return roundSam(tasks.reduce((sum, row) => sum + row.totalStandardTime, 0));
}
function aggregateUnscheduledDemand(tasks) {
    return roundSam(tasks.reduce((sum, row) => sum + row.totalStandardTime, 0));
}
function buildBottleneckProcessOptions(optionMap, selectedCraftOption, selectedProcessCode) {
...
function buildBottleneckCraftOptions(optionMap, selectedProcessCode) {
...
export function buildCapacityBottleneckData(input) {
    ...
    const unallocatedSamByCraft = new Map();
    const unallocatedSamByDate = new Map();
    const unscheduledSamByCraft = new Map();
    ...
    function getDailyRowSeed(date, factoryId, factoryName, identity) {
    ...
    for (const factory of factories) {
        for (const { row, entry } of listFactoryCapacityEntries(factory.id)) {
            ...
            registerIdentity(identity);
            const dailySupplySam = roundSam(Math.max(computeFactoryCapacityEntryResult(row, entry.values, getFactoryCapacityEquipmentSummary(factory.id, row.processCode, row.craftCode)).resultValue ?? 0, 0));
            for (const date of dates) {
    ...
    const appendUsageRows = (items, kind) => {
        for (const item of items) {
            const identity = resolveUsageIdentity(item, taskMap, displayLabels);
            registerIdentity(identity);
            const schedule = buildUsageSchedule({
    ...
    for (const task of taskMap.values()) {
        ...
        const unallocatedStageLabel = binding.kind === 'FROZEN_PENDING'
        ...
        unallocatedRows.push({
            ...
            assignmentStatusLabel: unallocatedStageLabel,
            frozenFactoryCount,
            note: binding.kind === 'FROZEN_PENDING'
    ...
    const { processOptions, selectedProcessCode, } = buildBottleneckProcessOptions(processOptionMap, selectedCraftOption, input?.processCode);
    const craftOptions = buildBottleneckCraftOptions(craftOptionMap, selectedProcessCode);
    const selectedCraftCode = craftOptions.find((item) => item.value === `${selectedProcessCode}::${input?.craftCode ?? ''}`)?.value.split('::')[1]
    ...
    const filteredDateRows = dateRows
        ...
        const craftRemaining = new Map();
        const overloadedFactories = new Set();
        const pausedFactories = new Set();
        const tightCrafts = new Set();
        let supplySam = 0;
        let committedSam = 0;
        let frozenSam = 0;
        let remainingSam = 0;
    ...
    const draft = {
        ...
        unallocatedRows: filteredUnallocatedRows.sort((left, right) => {
            if (right.totalStandardTime !== left.totalStandardTime)
...
function getTaskRiskSeverity(conclusion) {
    const order = {
        PAUSED: 7,
        EXCEEDS_WINDOW: 6,
        TIGHT: 5,
...
function buildRiskWindowOverlapFilter(windowDays, today = toDayStart(new Date())) {
...
function doesWindowOverlapRange(row, range) {
    if (row.conclusion === 'UNSCHEDULED')
        return true;
...
function summarizeUsageWindow(items) {
...
export function resolveTaskRiskWindow(task) {
    return resolveTaskWindow(task);
}
export function resolveTaskBindingState(task, activeCommitments, activeFreezes) {
    ...
    if (commitmentFactoryIds.length > 0) {
        ...
        return {
            ...
            reason: commitmentFactoryIds.length > 1
    ...
    if (freezeFactoryIds.length > 0) {
        ...
        return {
            ...
            factoryName: freezeFactoryIds.length === 1
            ...
            reason: freezeFactoryIds.length === 1
...
export function resolveTaskSamRisk(input) {
    ...
    const windowText = window.windowDays <= 0 && window.windowEndDate
    ...
    let conclusion = 'CAPABLE';
    if (windowJudgement.dateIncomplete || windowJudgement.status === 'SAM_MISSING')
        conclusion = 'UNSCHEDULED';
    else if (windowJudgement.status === 'PAUSED')
        conclusion = 'PAUSED';
    else if (windowJudgement.status === 'OVERLOADED')
        conclusion = 'EXCEEDS_WINDOW';
    else if (windowJudgement.status === 'TIGHT')
        conclusion = 'TIGHT';
    const remainingAfterCurrentSam = Number.isFinite(windowJudgement.remainingSam)
        ? roundSam(windowJudgement.remainingSam)
        : undefined;
    const baseReason = conclusion === 'PAUSED'
...
export function groupTasksByProductionOrder(taskRows) {
...
export function summarizeProductionOrderRisk(taskRows) {
    const grouped = groupTasksByProductionOrder(taskRows);
    const orderIndex = new Map(productionOrders.map((order, index) => [order.productionOrderId, index]));
    return [...grouped.entries()]
        ...
        return {
            ...
            allocatedStandardTime: roundSam(rows.reduce((sum, row) => sum
                + (row.conclusion === 'CAPABLE' || row.conclusion === 'TIGHT' || row.conclusion === 'EXCEEDS_WINDOW' || row.conclusion === 'PAUSED'
                    ? row.totalStandardTime
                    : 0), 0)),
            frozenPendingStandardTime: roundSam(rows.reduce((sum, row) => sum + (row.conclusion === 'FROZEN_PENDING' ? row.totalStandardTime : 0), 0)),
            unallocatedStandardTime: roundSam(rows.reduce((sum, row) => sum + (row.conclusion === 'UNALLOCATED' ? row.totalStandardTime : 0), 0)),
            unscheduledStandardTime: roundSam(rows.reduce((sum, row) => sum + (row.conclusion === 'UNSCHEDULED' ? row.totalStandardTime : 0), 0)),
...
export function resolveProductionOrderRisk(taskRows) {
    return summarizeProductionOrderRisk(taskRows);
}
function buildRiskProcessOptions(taskRows) {
...
function buildRiskCraftOptions(taskRows) {
...
function summarizeTaskRiskRows(rows) {
    return {
...
export function filterCapacityRiskTaskRows(input) {
    ...
    return input.rows.filter((row) => {
        ...
        if (input.processCode && row.processCode !== input.processCode)
            return false;
        if (selectedCraftCode && (row.processCode !== selectedCraftProcessCode || row.craftCode !== selectedCraftCode)) {
            return false;
        }
        if (input.conclusion && row.conclusion !== input.conclusion)
            return false;
        if (!doesWindowOverlapRange(row, range))
            return false;
        return true;
    });
}
