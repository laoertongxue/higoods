import { OWN_WOOL_FACTORY_ID, OWN_WOOL_FACTORY_NAME, } from './factory-mock-data.ts';
...
export const WOOL_KIND_LABEL = {
    WHOLE_GARMENT: '整件毛织',
    PART_PANEL: '部位毛织',
};
export const WOOL_STATUS_LABEL = {
    WAIT_ACCEPT: '未接单',
    WAIT_PICKUP: '待领料',
...
function normalizeWoolText(value) {
...
function isBusinessTimestamp(value) {
    return Boolean(value && /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}/.test(value));
}
function readWoolStore() {
...
function resolveProductionOrderQty(order) {
...
function getPatternPackageRows(patternFiles) {
...
function getMixedPatternPartWoolSources(patternFiles) {
...
function resolvePatternLinkedMaterial(pattern, order) {
...
function buildSnapshotDrivenWoolTasks() {
    const tasks = [];
...
function listGeneratedWoolTasks() {
    ...
    return [...snapshotDrivenTasks, ...runtimeTasks]
        .sort((left, right) => {
        if (left.productionOrderId !== right.productionOrderId)
...
function resolveWoolKind(task) {
    if (task.woolTaskType === 'PART_PANEL' || task.woolKind === 'PART_PANEL')
        return 'PART_PANEL';
    if (task.craftName === '部位毛织' || task.taskCategoryZh === '部位毛织')
        return 'PART_PANEL';
    return 'WHOLE_GARMENT';
}
function resolveOrderSkuSizeRange(task) {
...
function resolveOrderColorName(task) {
    const order = productionOrders.find((item) => item.productionOrderId === task.productionOrderId);
...
function resolvePlannedQty(task, kind) {
    if (kind === 'PART_PANEL') {
...
function resolveCompletedQty(task, plannedQty, kind) {
    if (task.status === 'DONE')
        return plannedQty;
...
function resolveYarnReceipt(task, kind, plannedQty) {
    ...
    const plannedWeightKg = roundQty(task.yarnPlannedWeightKg ?? (yarnBom ? plannedQty * Math.max(yarnBom.unitConsumption || 0, kind === 'PART_PANEL' ? 0.08 : 0.48) * (1 + Math.max(yarnBom.lossRate || 0, 0)) : plannedQty * (kind === 'PART_PANEL' ? 0.08 : 0.48)));
...
function makeNodeStatus(task, completedQty, plannedQty) {
    if (task.status === 'NOT_STARTED')
        return '未开始';
    if (completedQty >= plannedQty && plannedQty > 0)
        return '已完成';
    return '进行中';
}
function buildGeneratedWoolNodes(task, kind, plannedQty, completedQty, needsPackaging, machineNos) {
...
function resolveGeneratedStatus(task, kind) {
    if (task.status === 'DONE')
        return 'COMPLETED';
    if (task.acceptanceStatus !== 'ACCEPTED' && !task.acceptedAt && task.status === 'NOT_STARTED')
        return 'WAIT_ACCEPT';
    if (task.status === 'NOT_STARTED')
        return 'WAIT_PICKUP';
    if (kind === 'PART_PANEL')
        return 'WAIT_FEI_TICKET';
    if (task.packagingRequired)
        return 'IRONING';
    return 'WAIT_HANDOVER';
}
function parseSizeFromSku(skuCode) {
    const segments = skuCode.split('-').filter(Boolean);
    return segments[segments.length - 1] || '-';
}
function buildGeneratedPartPanels(task, completedRatio) {
...
function buildGeneratedWoolWorkOrder(task, index) {
...
function buildManualSeedWoolWorkOrders() {
    ...
    const makePrice = (kind, needsPackaging) => ({
        flatWoolMinutes: kind === 'PART_PANEL' ? 0.45 : 3.2,
        linkingMinutes: kind === 'WHOLE_GARMENT' ? 1.1 : undefined,
        ironingIncluded: kind === 'WHOLE_GARMENT',
        packagingIncluded: needsPackaging,
...
function getFlatWoolNode(order) {
    return order.nodes.find((node) => node.nodeName === '横机成片');
}
function hasWoolMachineSchedule(order) {
    return Boolean(order.machineScheduleId) || Boolean(getFlatWoolNode(order)?.machineNos?.length);
}
function getWoolMachineScheduleStatus(order) {
    const flatNode = getFlatWoolNode(order);
    if (flatNode?.status === '已完成')
        return '已完成';
    if (flatNode?.status === '进行中' || flatNode?.startedAt)
        return '生产中';
    return '待开工';
}
function getWoolMachineRiskText(order) {
    const flatNode = getFlatWoolNode(order);
...
function buildGeneratedWoolMachineSchedules() {
    const orders = listWoolWorkOrders();
    const schedules = orders.filter(hasWoolMachineSchedule).map((order, index) => {
        const machineStart = index * 4 + 1;
        const flatNode = getFlatWoolNode(order);
...
export function listWoolWorkOrders() {
    ...
    return Array.from(ordersById.values()).sort((left, right) => {
        if (left.status === 'WAIT_ACCEPT' && right.status !== 'WAIT_ACCEPT')
            return -1;
        if (right.status === 'WAIT_ACCEPT' && left.status !== 'WAIT_ACCEPT')
            return 1;
        return left.woolOrderNo.localeCompare(right.woolOrderNo, 'zh-CN');
    });
}
export function listWoolMachineSchedules() {
    return buildGeneratedWoolMachineSchedules();
}
function getMachineStatusBySchedule(schedule) {
    if (schedule.status === '生产中')
        return '生产中';
    if (schedule.status === '空闲')
        return '空闲';
    if (schedule.status === '已取消')
        return '空闲';
    if (schedule.status === '延误')
        return '已排产';
    return '已排产';
}
export function listWoolMachines() {
    const machinesByNo = new Map();
...
export function getWoolWorkOrderById(woolOrderId) {
    return listWoolWorkOrders().find((order) => order.woolOrderId === woolOrderId);
}
export function getWoolWorkOrderByTaskId(taskId) {
    return listWoolWorkOrders().find((order) => order.taskNo === taskId || order.woolOrderId === taskId);
}
function ensureWoolPickupReceived(order, operatorName, operatedAt) {
    if (order.yarnReceipt.receivedWeightKg > 0)
        return order.yarnReceipt;
    return {
        ...order.yarnReceipt,
...
function getWoolLinkingLossWeight(order) {
    const linkingNode = getWoolNode(order, '缝盘');
    if (!linkingNode)
        return 0;
    if (typeof linkingNode.yarnLossWeightKg === 'number')
        return roundQty(linkingNode.yarnLossWeightKg);
    if (linkingNode.status === '已完成')
        return roundQty(getWoolYarnProcessingUsageWeight(order) * 0.015);
    return 0;
}
function getWoolRecoveredYarnWeight(order) {
    return roundQty(order.yarnReceipt.recoveredWeightKg || 0);
}
export function getWoolYarnUsageSummary(order) {
...
function deriveWoolOrderStatus(order) {
    ...
    if (order.status === 'WAIT_ACCEPT' && !hasNodeProgress && !hasYarnReceived)
        return 'WAIT_ACCEPT';
    if (order.receiverWrittenQty !== undefined && order.receiverWrittenQty >= 0)
        return 'COMPLETED';
    if (order.status === 'HANDOVER_SUBMITTED')
        return 'HANDOVER_SUBMITTED';
    if (!flatNode || flatNode.status === '未开始') {
        if (!hasYarnReceived)
            return 'WAIT_PICKUP';
        if (!order.pickupCompletedAt)
            return 'PICKUP_IN_PROGRESS';
        if (!hasWoolMachineSchedule(order))
            return 'WAIT_MACHINE_SCHEDULE';
        return 'MACHINE_SCHEDULED';
    }
    if (flatNode.status === '进行中')
        return 'FLAT_WOOL';
    if (order.kind === 'PART_PANEL') {
        if (order.partPanels.every((panel) => panel.feiTicketStatus === '已打印'))
            return 'FEI_TICKET_PRINTED';
        return 'WAIT_FEI_TICKET';
    }
...
function normalizeWoolWorkOrderRuntime(order) {
...
export function getWoolAllowedActions(order) {
    switch (order.status) {
        case 'WAIT_ACCEPT':
...
export function acceptWoolWorkOrder(woolOrderId, operatorName = OWN_WOOL_FACTORY_NAME, operatedAt = nowTimestamp()) {
...
export function confirmWoolPickupRecord(woolOrderId, operatorName = OWN_WOOL_FACTORY_NAME, operatedAt = nowTimestamp()) {
    const order = getWoolWorkOrderById(woolOrderId);
    if (!order)
        return undefined;
...
export function completeWoolPickupHead(woolOrderId, operatorName = OWN_WOOL_FACTORY_NAME, operatedAt = nowTimestamp()) {
    const order = getWoolWorkOrderById(woolOrderId);
    if (!order)
        return undefined;
    const acceptedOrder = order.status === 'WAIT_ACCEPT'
...
export function scheduleWoolMachines(woolOrderId, operatorName = OWN_WOOL_FACTORY_NAME, operatedAt = nowTimestamp(), input = {}) {
    const order = getWoolWorkOrderById(woolOrderId);
    if (!order || !order.pickupCompletedAt)
        return order;
    ...
    const nodes = order.nodes.map((node) => node.nodeName === '横机成片'
        ? { ...node, machineNos }
        : node);
    const nextBase = {
        ...order,
...
function getAllowedNodeTransition(order, nodeName, nodeStatus) {
    if (nodeStatus === '未开始')
        return true;
    if (nodeName === '横机成片') {
        if (nodeStatus === '进行中')
            return order.status === 'MACHINE_SCHEDULED';
        if (nodeStatus === '已完成')
            return order.status === 'FLAT_WOOL';
        return false;
    }
    if (nodeName === '缝盘') {
        if (order.kind !== 'WHOLE_GARMENT')
            return false;
        if (nodeStatus === '进行中')
            return order.status === 'WAIT_LINKING';
        if (nodeStatus === '已完成')
            return order.status === 'LINKING';
        return false;
    }
    if (nodeName === '熨烫') {
        if (order.kind !== 'WHOLE_GARMENT')
            return false;
        if (nodeStatus === '进行中')
            return order.status === 'WAIT_IRONING';
        if (nodeStatus === '已完成')
            return order.status === 'IRONING';
        return false;
    }
    if (nodeName === '包装') {
        if (order.kind !== 'WHOLE_GARMENT' || !order.needsPackaging)
            return nodeStatus === '已跳过' && order.status === 'WAIT_PACKING';
        if (nodeStatus === '进行中')
            return order.status === 'WAIT_PACKING';
        if (nodeStatus === '已完成')
...
export function updateWoolWorkOrderNodeStatus(woolOrderId, nodeName, nodeStatus, operatorName = OWN_WOOL_FACTORY_NAME, operatedAt = nowTimestamp(), options = {}) {
    const order = getWoolWorkOrderById(woolOrderId);
    if (!order)
        return undefined;
    if (!getAllowedNodeTransition(order, nodeName, nodeStatus))
        return order;
    ...
    const partPanels = order.kind === 'PART_PANEL' && nodeName === '横机成片' && nodeStatus === '已完成'
        ? order.partPanels.map((panel) => ({ ...panel, completedPieces: panel.plannedPieces, feiTicketStatus: panel.feiTicketStatus === '已打印' ? '已打印' : '待打印' }))
        : order.partPanels;
...
export function recoverWoolYarnToWaitProcessWarehouse(woolOrderId, recoveredWeightKg, operatorName = OWN_WOOL_FACTORY_NAME, operatedAt = nowTimestamp()) {
    const order = getWoolWorkOrderById(woolOrderId);
    if (!order)
        return order;
...
export function recordWoolYarnRecovery(input) {
    ...
    const associatedOrders = allOrders.filter((order) => order.yarnReceipt.yarnSku === input.yarnSku
...
export function listWoolYarnRecoveryRecords() {
    return cloneValue(readWoolStore().yarnRecoveryRecords || []);
}
function buildWoolRecoveryRemark(record) {
    if (!record.associations.length)
        return record.remark || '未关联毛织加工单';
...
function getLegacyRecoveredWeight(order, recoveryRecords) {
    const recorded = recoveryRecords.some((record) => record.associations.some((association) => association.woolOrderId === order.woolOrderId));
    return recorded ? 0 : getWoolRecoveredYarnWeight(order);
}
function addRecoveryFlowsToGroup(group, yarnSku, recoveryRecords) {
    recoveryRecords
...
function addLegacyRecoveryFlow(group, order, recoveryRecords) {
    const recoveredWeightKg = getLegacyRecoveredWeight(order, recoveryRecords);
    if (recoveredWeightKg <= 0)
        return;
...
export function markWoolFeiTicketsPrinted(woolOrderId, operatorName = OWN_WOOL_FACTORY_NAME, operatedAt = nowTimestamp()) {
    const order = getWoolWorkOrderById(woolOrderId);
    if (!order || order.kind !== 'PART_PANEL')
        return order;
    if (order.status !== 'WAIT_FEI_TICKET')
        return order;
    const nextBase = {
        ...order,
    ...
    const next = { ...nextBase, status: deriveWoolOrderStatus(nextBase) };
    return saveWoolWorkOrder(next);
}
export function submitWoolHandover(woolOrderId, operatorName = OWN_WOOL_FACTORY_NAME, operatedAt = nowTimestamp()) {
    const order = getWoolWorkOrderById(woolOrderId);
    if (!order || (order.status !== 'WAIT_HANDOVER' && order.status !== 'FEI_TICKET_PRINTED'))
        return order;
    const nextBase = {
        ...order,
        status: 'HANDOVER_SUBMITTED',
...
export function confirmWoolHandoverReceipt(woolOrderId, operatorName = '接收仓库', operatedAt = nowTimestamp()) {
    const order = getWoolWorkOrderById(woolOrderId);
    if (!order || order.status !== 'HANDOVER_SUBMITTED')
        return order;
...
function getWoolTaskStatus(order) {
    if (order.status === 'WAIT_ACCEPT' || order.status === 'WAIT_PICKUP' || order.status === 'PICKUP_IN_PROGRESS' || order.status === 'WAIT_MACHINE_SCHEDULE' || order.status === 'MACHINE_SCHEDULED')
        return 'NOT_STARTED';
    if (order.status === 'WAIT_HANDOVER' || order.status === 'HANDOVER_SUBMITTED' || order.status === 'COMPLETED')
        return 'DONE';
    return 'IN_PROGRESS';
}
function getWoolTaskAcceptedAt(order) {
    if (order.status === 'WAIT_ACCEPT')
        return undefined;
    if (order.acceptedAt)
...
function getWoolTaskStartedAt(order) {
    return order.nodes.find((node) => node.startedAt)?.startedAt;
}
function getWoolTaskFinishedAt(order) {
    if (getWoolTaskStatus(order) !== 'DONE')
        return undefined;
    return [...order.nodes].reverse().find((node) => node.finishedAt)?.finishedAt || order.scheduledEndAt;
}
function getWoolMilestoneProofFiles(order) {
...
function buildWoolMobileTask(order) {
    ...
    const downstreamReceiver = order.kind === 'PART_PANEL'
        ? { receiverKind: 'WAREHOUSE', receiverId: 'WH-CUTTING-WAIT-HANDOVER', receiverName: '裁床待交出仓' }
        : { receiverKind: 'MANAGED_POST_FACTORY', receiverId: 'POST-FACTORY-OWN', receiverName: '后道工厂' };
    return {
        taskId: order.taskNo,
        taskNo: order.taskNo,
...
export function listWoolMobileProcessTasks() {
    return listWoolWorkOrders().map(buildWoolMobileTask);
}
function getWoolHandoverId(order, type) {
    return `${type === 'PICKUP' ? 'PKH' : 'HOH'}-${order.taskNo.replace(/^任务-/, '')}`;
}
export function listWoolHandoverHeadSeeds() {
    const pickupHeads = listWoolWorkOrders().map((order) => {
...
export function getWoolPickupRecordSeedsByHeadId(handoverId) {
...
export function getWoolHandoutRecordSeedsByHeadId(handoverId) {
...
function getDefaultWoolWarehouseAreas(mode) {
...
export function listWoolWarehouseAreas(mode) {
...
export function upsertWoolWarehouseArea(input) {
    ...
    if (index >= 0)
        store.areas[index] = next;
    else
        store.areas.unshift(next);
    store.locations = store.locations.map((location) => (location.warehouseMode === next.warehouseMode && location.areaId === next.areaId
        ? { ...location, areaName: next.areaName, updatedAt: next.updatedAt }
        : location));
    writeWoolStore(store);
    return cloneValue(next);
}
function getDefaultWoolWarehouseLocations(mode) {
    const prefix = mode === 'wait-process' ? 'KWP' : 'KWH';
...
export function listWoolWarehouseLocations(mode) {
...
export function upsertWoolWarehouseLocation(input) {
    const store = readWoolStore();
    const locationId = input.locationId || `KLOC-${input.warehouseMode}-${Date.now()}`;
    const existing = store.locations.find((location) => location.locationId === locationId)
    ...
    if (index >= 0)
        store.locations[index] = next;
    else
        store.locations.unshift(next);
    writeWoolStore(store);
    return cloneValue(next);
}
export function deleteWoolWarehouseLocation(locationId) {
    const store = readWoolStore();
    store.locations = store.locations.filter((location) => location.locationId !== locationId);
    writeWoolStore(store);
}
function buildWoolWaitProcessReceiptNo(order) {
    return `领料-${order.woolOrderNo}`;
}
function resolveWoolWaitProcessLocationText(order, fallback = '待分配库位') {
    if (order.yarnReceipt.waitProcessLocationText)
        return order.yarnReceipt.waitProcessLocationText;
    const area = findWoolWarehouseArea('wait-process', order.yarnReceipt.waitProcessAreaId);
    const location = listWoolWarehouseLocations('wait-process').find((item) => item.locationId === order.yarnReceipt.waitProcessLocationId
        && (!area || item.areaId === area.areaId));
    if (area && location)
        return `${area.areaName} / ${location.locationCode}`;
    if (area)
        return area.areaName;
    return fallback;
}
export function listWoolWaitProcessScanReceipts() {
...
export function lookupWoolWaitProcessScanReceipt(input) {
    const keyword = input.trim();
    if (!keyword)
        return undefined;
    const normalized = keyword.toLowerCase();
    const matched = listWoolWaitProcessScanReceipts().find((receipt) => (receipt.receiptNo.toLowerCase() === normalized
...
export function confirmWoolWaitProcessScanReceipt(input) {
    ...
    if (!receipt)
        throw new Error('未找到毛织领料单或送料二维码。');
    ...
    input.lines.forEach((lineInput) => {
        ...
        const location = locations.find((item) => item.locationId === lineInput.locationId
            && item.areaId === lineInput.areaId);
        if (!line || actualWeightKg <= 0 || !area)
            return;
    ...
    if (!updatedOrderIds.length)
        throw new Error('请至少填写一条有效的实收重量和库区。');
    writeWoolStore(store);
...
export function listWoolWaitProcessReceiptRecords() {
    return listWoolWorkOrders().map((order) => ({
...
export function listWoolWaitProcessUsageRecords() {
    return listWoolWorkOrders()
        .flatMap((order) => {
        const flatNode = getWoolNode(order, '横机成片');
        const linkingNode = getWoolNode(order, '缝盘');
...
function hasWoolInboundToHandover(order) {
    if (order.kind === 'PART_PANEL') {
...
export function listWoolWaitHandoverInboundRecords() {
    return listWoolWorkOrders()
...
export function listWoolWaitHandoverHandoutRecords() {
    return listWoolWorkOrders()
...
export function listWoolWarehouseInventory(mode) {
    ...
    return listWoolWorkOrders()
        ...
        const buildHandoutFlow = (qty, unit) => qty > 0
            ? [
                {
...
export function getWoolMachineScheduleSummary() {
...
export function getWoolWorkOrderKindLabel(kind) {
    return WOOL_KIND_LABEL[kind];
}
export function getWoolWorkOrderStatusLabel(status) {
    return WOOL_STATUS_LABEL[status];
}
export function getWoolWorkOrderSummary() {
    const orders = listWoolWorkOrders();
    return {
        total: orders.length,
        ...
        flatWoolCount: orders.filter((order) => order.status === 'FLAT_WOOL'
            || order.nodes.some((node) => node.nodeName === '横机成片' && node.status === '进行中')).length,
