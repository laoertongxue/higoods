...
import { confirmProductionOrderMainFactoryFromSewingTask, productionOrders, } from './production-orders.ts';
import { getProcessAssignmentGranularity, } from './process-types.ts';
import { calculatePublishedSamTotal, processTasks, sumTaskStandardTimeTotals, } from './process-tasks.ts';
import { buildTaskQrValue } from './task-qr.ts';
import { TEST_FACTORY_ID, TEST_FACTORY_NAME } from './factory-mock-data.ts';
import { listTaskAllocatableGroups, resolveTaskSplitDecision, validateAllocatableGroupAssignments, } from './task-split-dispatch.ts';
const runtimeTaskOverrides = new Map();
const runtimeTaskSplitPlans = new Map();
...
function resolveExecutorKindByFactoryId(factoryId) {
    if (!factoryId)
...
export function isRuntimeSewingTask(task) {
    return task.processCode === 'SEW' || task.processNameZh === '车缝';
}
function getOrderSkuLines(productionOrderId) {
...
function cloneTaskDetailRows(rows) {
...
export function resolveRuntimeTaskPublishedSam(task) {
    ...
    const fallbackTotal = publishedSamPerUnit && publishedSamUnit
...
export function resolveRuntimeAllocatableGroupPublishedSam(task, group) {
    ...
    const scopedDetailRows = detailRowKeySet.size > 0 ? detailRows.filter((row) => detailRowKeySet.has(row.rowKey)) : detailRows;
    return {
        ...base,
        publishedSamTotal: normalizePublishedSamNumber(calculatePublishedSamTotal({
...
function recalculateRuntimeTaskPublishedSamTotal(task, scopeQty, detailRows) {
...
function filterDetailRowsByScope(rows, scopeType, scopeSkuLines) {
    if (rows.length === 0)
        return [];
    if (scopeType === 'ORDER')
        return rows;
    ...
    return rows.filter((row) => {
        ...
        if (scopeType === 'SKU') {
            if (rowSku && !scopeSkuSet.has(rowSku))
                return false;
            if (rowColor && !scopeColorSet.has(rowColor))
                return false;
            return true;
        }
        if (scopeType === 'COLOR') {
...
function pickDetailRowsByKeys(rows, keys) {
    if (!keys.length)
        return cloneTaskDetailRows(rows);
    const keySet = new Set(keys);
    return cloneTaskDetailRows(rows).filter((row) => keySet.has(row.rowKey));
}
function deriveScopeSkuLinesByDetailRows(scopeSkuLines, detailRows) {
    if (!detailRows.length)
        return [...scopeSkuLines];
    const skuSet = new Set(detailRows
        .map((row) => row.dimensions.GARMENT_SKU)
        .filter((sku) => Boolean(sku)));
    const colorSet = new Set(detailRows
        .map((row) => row.dimensions.GARMENT_COLOR)
        .filter((color) => Boolean(color)));
    if (skuSet.size === 0 && colorSet.size === 0)
        return [...scopeSkuLines];
    return scopeSkuLines.filter((line) => {
        if (skuSet.size > 0 && skuSet.has(line.skuCode))
            return true;
        if (colorSet.size > 0 && colorSet.has(line.color))
            return true;
        return false;
    });
}
function applyRuntimeSplitPlans(tasks) {
    if (runtimeTaskSplitPlans.size === 0)
        return tasks;
    const planBySourceTaskId = new Map(runtimeTaskSplitPlans);
    const splitResultTaskIdsBySource = new Map();
    ...
    if (splitResultTaskIdsBySource.size === 0)
        return expanded;
    return expanded.map((task) => {
        const rewrittenDepends = task.dependsOnTaskIds.flatMap((dependsTaskId) => {
            ...
            if (!splitResultTaskIds || splitResultTaskIds.length === 0)
                return [dependsTaskId];
...
function buildOrderScopeTask(baseTask, skuLines) {
...
function buildColorScopeTasks(baseTask, skuLines) {
    if (!skuLines.length)
        return [buildOrderScopeTask(baseTask, [])];
...
function buildSkuScopeTasks(baseTask, skuLines) {
    if (!skuLines.length)
        return [buildOrderScopeTask(baseTask, [])];
    const baseDetailRows = getTaskDetailRows(baseTask);
...
function buildRuntimeTasksByGranularity(baseTask) {
    ...
    const _granularity = baseTask.assignmentGranularity
        ?? getProcessAssignmentGranularity(baseTask.processCode);
    void _granularity;
    return [buildOrderScopeTask(baseTask, skuLines)];
}
function findOrderScopeTask(tasks) {
    return tasks.find((task) => task.scopeType === 'ORDER');
}
function getRuntimeDependencyIds(currentTask, upstreamTasks) {
    ...
    if (currentTask.scopeType === 'SKU') {
        matched = upstreamTasks.filter((task) => {
            if (task.scopeType === 'SKU')
                return Boolean(task.skuCode && task.skuCode === currentTask.skuCode);
            if (task.scopeType === 'COLOR')
                return task.scopeKey === currentTask.skuColor;
            return false;
        });
    }
        ...
        matched = upstreamTasks.filter((task) => {
            if (task.scopeType === 'COLOR')
                return task.scopeKey === currentTask.scopeKey;
            if (task.scopeType === 'SKU')
                return task.skuColor === currentTask.scopeKey;
            return false;
        });
    }
    else {
...
function applyRuntimeDependencies(runtimeTasks) {
    const tasksByBaseId = new Map();
    ...
    return runtimeTasks.map((task) => {
        ...
        const runtimeDependsOn = upstreamBaseIds.flatMap((baseId) => getRuntimeDependencyIds(task, tasksByBaseId.get(baseId) ?? []));
        return {
            ...task,
...
function applyRuntimeOverrides(tasks) {
...
function shouldUseSameFactoryContinue(upstream, downstream) {
    if (upstream.scopeType !== 'SKU' || downstream.scopeType !== 'SKU')
        return false;
    if (!upstream.skuCode || !downstream.skuCode)
        return false;
    if (upstream.skuCode !== downstream.skuCode)
        return false;
    if (!upstream.assignedFactoryId || !downstream.assignedFactoryId)
        return false;
    if (upstream.assignedFactoryId !== downstream.assignedFactoryId)
        return false;
...
function computeTransitionsForOrder(tasks) {
    const byId = new Map(tasks.map((task) => [task.taskId, task]));
    const upstreamByTaskId = new Map();
    ...
    return tasks.map((task) => {
        ...
        let transitionFromPrev = 'NOT_APPLICABLE';
        ...
        let transitionToNext = 'NOT_APPLICABLE';
...
function compareRuntimeTask(a, b) {
    const orderCompare = a.productionOrderId.localeCompare(b.productionOrderId);
    if (orderCompare !== 0)
        return orderCompare;
    if (a.seq !== b.seq)
        return a.seq - b.seq;
    const scopeRank = { ORDER: 0, COLOR: 1, SKU: 2, DETAIL: 3 };
...
function buildRuntimeBaseTasksFromTaskFacts() {
...
function buildRuntimeProcessTasksBase() {
...
function buildRuntimeProcessTasks() {
...
function getMutableRuntimeTaskById(taskId) {
    return listRuntimeProcessTasks().find((task) => task.taskId === taskId) ?? null;
}
function patchRuntimeTask(taskId, patch) {
...
function updateRuntimeTaskWithAudit(taskId, patch, action, detail, by) {
    const current = getMutableRuntimeTaskById(taskId);
    if (!current)
        return null;
...
function buildSeedAuditLog(taskId, action, detail, by, at) {
    return {
...
function getSeedBaseAuditLogs(taskId) {
...
function seedRuntimeTaskOverride(taskId, patch, auditLogs = []) {
...
function ensureDispatchBoardSeedData() {
    if (dispatchBoardSeedReady)
        return;
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0001-001__ORDER', {
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0002-001__ORDER', {
        assignmentMode: 'DIRECT',
        assignmentStatus: 'ASSIGNED',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0002-002__ORDER', {
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0002-003__ORDER', {
        assignmentMode: 'DIRECT',
        assignmentStatus: 'ASSIGNED',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0002-005__ORDER', {
        assignmentMode: 'DIRECT',
        assignmentStatus: 'ASSIGNED',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0003-001__ORDER', {
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0004-001__ORDER', {
        assignmentMode: 'BIDDING',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0005-001__ORDER', {
        assignmentStatus: 'UNASSIGNED',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0006-001__ORDER', {
        assignmentStatus: 'UNASSIGNED',
    }, [
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0008-001__ORDER', {
        assignmentMode: 'DIRECT',
        assignmentStatus: 'ASSIGNED',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0009-001__ORDER', {
        assignmentMode: 'BIDDING',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0015-001__ORDER', {
        assignmentMode: 'BIDDING',
        assignmentStatus: 'ASSIGNING',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0015-002__ORDER', {
        assignmentStatus: 'UNASSIGNED',
        taskDeadline: '2026-04-12 18:00:00',
    }, [
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0015-004__ORDER', {
        assignmentMode: 'BIDDING',
        assignmentStatus: 'BIDDING',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0015-005__ORDER', {
        assignmentMode: 'BIDDING',
        assignmentStatus: 'BIDDING',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0015-006__ORDER', {
        assignmentMode: 'BIDDING',
        assignmentStatus: 'BIDDING',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0015-007__ORDER', {
        assignmentMode: 'DIRECT',
        assignmentStatus: 'ASSIGNED',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-0015-008__ORDER', {
        assignmentMode: 'BIDDING',
        assignmentStatus: 'AWARDED',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-083-001__ORDER', {
        assignmentMode: 'BIDDING',
        assignmentStatus: 'AWARDED',
    ...
    seedRuntimeTaskOverride('TASKGEN-202603-084-001__ORDER', {
        assignmentMode: 'BIDDING',
        assignmentStatus: 'ASSIGNING',
...
function getOrderIdsFromTaskIds(taskIds) {
...
export function listRuntimeProcessTasks() {
    ensureDispatchBoardSeedData();
    return buildRuntimeProcessTasks();
}
export function listRuntimeTasksByOrder(productionOrderId) {
    return listRuntimeProcessTasks().filter((task) => task.productionOrderId === productionOrderId);
}
export function listRuntimeExecutionTasksByOrder(productionOrderId) {
    return listRuntimeTasksByOrder(productionOrderId).filter((task) => isRuntimeTaskExecutionTask(task) && task.defaultDocType !== 'DEMAND');
}
export function getRuntimeTaskById(taskId) {
    return listRuntimeProcessTasks().find((task) => task.taskId === taskId) ?? null;
}
export function listRuntimeTasksByBaseTaskId(baseTaskId) {
    return listRuntimeProcessTasks().filter((task) => task.baseTaskId === baseTaskId);
}
export function listRuntimeTasksByStage(stageCode) {
    return listRuntimeProcessTasks().filter((task) => task.stageCode === stageCode);
}
export function listRuntimeTasksByProcess(processCode) {
    return listRuntimeProcessTasks().filter((task) => task.processBusinessCode === processCode || task.processCode === processCode);
}
function formatSplitStatusSummary(tasks) {
    if (tasks.length === 0)
        return '无拆分结果任务';
    ...
    if (done > 0)
        parts.push(`已完成${done}`);
    if (inProgress > 0)
        parts.push(`进行中${inProgress}`);
    if (pending > 0)
        parts.push(`待执行${pending}`);
    if (blocked > 0)
        parts.push(`暂停${blocked}`);
    if (cancelled > 0)
        parts.push(`已取消${cancelled}`);
    return parts.length > 0 ? parts.join(' / ') : '待执行';
}
function formatSplitFactorySummary(tasks) {
    if (tasks.length === 0)
        return '-';
    const names = Array.from(new Set(tasks
        .map((task) => task.assignedFactoryName?.trim())
        .filter((name) => Boolean(name))));
    return names.length > 0 ? names.join('、') : '-';
}
export function listRuntimeTaskSplitGroupsByOrder(productionOrderId) {
...
export function isRuntimeTaskExecutionTask(task) {
    return task.executionEnabled !== false && task.isSplitSource !== true;
}
export function listRuntimeExecutionTasks() {
    return listRuntimeProcessTasks().filter((task) => isRuntimeTaskExecutionTask(task));
}
function resolveTaskAssignmentGranularity(task) {
    return task.assignmentGranularity
        ?? getProcessAssignmentGranularity(task.processCode);
}
export function listRuntimeTaskAllocatableGroups(taskId) {
...
function clearRuntimeTaskSplitPlan(sourceTaskId) {
...
export function dispatchRuntimeTaskByDetailGroups(input) {
    const task = getRuntimeTaskById(input.taskId);
    if (!task)
        return { ok: false, message: '任务不存在或已被移除' };
    if (task.isSplitResult)
        return { ok: false, message: '拆分结果任务不支持再次按明细分配，请对来源任务操作' };
    ...
    if (!validation.valid)
        return { ok: false, message: validation.reason ?? '分配单元校验失败' };
    ...
    if (splitDecision.mode === 'SINGLE_FACTORY') {
        ...
        const updated = updateRuntimeTaskWithAudit(task.taskId, {
        ...
        }, 'DETAIL_DISPATCH', `按明细分配完成（同一工厂：${splitDecision.factoryName}），保持原任务执行`, input.by);
        if (!updated) {
    ...
    clearRuntimeTaskSplitPlan(task.taskId);
    const splitFactories = splitDecision.factories.map((factory) => ({
        ...factory,
    ...
    const sourceAuditLogs = appendRuntimeAudit(task, 'DETAIL_SPLIT', `按明细分配到多个工厂，生成 ${splitFactories.length} 条平级任务`, input.by);
    ...
    return {
        ...
        resultAssignments: splitFactories.flatMap((factory) => groups
...
export function createRuntimeTaskTenderByDetailGroups(input) {
    const task = getRuntimeTaskById(input.taskId);
    if (!task)
        return { ok: false, message: '任务不存在或已被移除' };
    if (task.isSplitResult)
        return { ok: false, message: '拆分结果任务不支持再次按明细创建招标单，请对来源任务操作' };
    const groups = listRuntimeTaskAllocatableGroups(task.taskId);
    if (groups.length <= 1) {
    ...
    const resultPlans = groups.map((group, index) => {
    ...
    patchRuntimeTask(task.taskId, {
        ...
        assignmentStatus: 'BIDDING',
        updatedAt: eventAt,
        auditLogs: appendRuntimeAudit(task, 'DETAIL_TENDER_SPLIT', `按明细创建招标单，生成 ${resultPlans.length} 条平级竞价任务`, input.by),
    });
...
export function setRuntimeTaskAssignMode(taskId, mode, by) {
    const task = getRuntimeTaskById(taskId);
    if (!task)
        return;
    if (mode === 'BIDDING') {
        const patch = {
            assignmentMode: 'BIDDING',
            assignmentStatus: task.assignmentStatus === 'UNASSIGNED' || task.assignmentStatus === 'ASSIGNED'
                ? 'BIDDING'
    ...
    updateRuntimeTaskWithAudit(taskId, {
        assignmentStatus: 'UNASSIGNED',
    }, 'SET_ASSIGN_MODE', '设为暂不分配', by);
    recomputeRuntimeTransitionsForOrder(task.productionOrderId);
}
export function batchSetRuntimeTaskAssignMode(taskIds, mode, by) {
    for (const taskId of taskIds) {
        setRuntimeTaskAssignMode(taskId, mode, by);
    }
}
export function upsertRuntimeTaskTender(taskId, payload, by) {
    const task = getRuntimeTaskById(taskId);
    if (!task)
        return null;
    const updated = updateRuntimeTaskWithAudit(taskId, {
        assignmentMode: 'BIDDING',
    ...
    }, 'BIDDING_START', `发起竞价 ${payload.tenderId}`, by);
    recomputeRuntimeTransitionsForOrder(task.productionOrderId);
...
export function validateRuntimeBatchDispatchSelection(taskIds) {
...
export function validateRuntimeFactoryAssignment(input) {
    ...
    for (const orderId of affectedOrders) {
        ...
        const assignedToFactory = orderTasks.filter((task) => task.assignedFactoryId === input.factoryId || input.taskIds.includes(task.taskId));
...
export function batchDispatchRuntimeTasks(input) {
...
export function applyRuntimeDirectDispatchMeta(input) {
    ...
    const updated = updateRuntimeTaskWithAudit(input.taskId, {
        assignmentMode: 'DIRECT',
        assignmentStatus: 'ASSIGNED',
    ...
    }, 'DISPATCH', input.autoAccept ? '自动分配直接派单，工厂已同步接单确认' : '已发起直接派单，待工厂确认', input.by);
...
export function recomputeRuntimeTransitionsForOrder(productionOrderId) {
...
export function getRuntimeAssignmentSummaryByOrder(productionOrderId) {
    ...
    const directAssignedCount = tasks.filter((task) => task.assignmentMode === 'DIRECT' && task.assignmentStatus === 'ASSIGNED').length;
    const biddingLaunchedCount = tasks.filter((task) => task.assignmentMode === 'BIDDING' &&
        (task.assignmentStatus === 'BIDDING' || task.assignmentStatus === 'ASSIGNING' || task.assignmentStatus === 'AWARDED')).length;
    const biddingAwardedCount = tasks.filter((task) => task.assignmentStatus === 'AWARDED').length;
    const assignedFactoryCount = new Set(tasks
        .filter((task) => task.assignmentStatus === 'ASSIGNED' || task.assignmentStatus === 'AWARDED')
        .map((task) => task.assignedFactoryId)
        .filter((factoryId) => Boolean(factoryId))).size;
...
export function getRuntimeTaskCountByOrder(productionOrderId) {
    return getRuntimeAssignmentSummaryByOrder(productionOrderId).totalTasks;
}
export function getRuntimeOrderStandardTimeTotal(productionOrderId) {
    return sumTaskStandardTimeTotals(listRuntimeExecutionTasksByOrder(productionOrderId));
}
export function getRuntimeTaskSummaryByOrder(productionOrderId) {
    const tasks = listRuntimeTasksByOrder(productionOrderId).filter((task) => isRuntimeTaskExecutionTask(task));
    const totalTasks = tasks.length;
...
export function getRuntimeBiddingSummaryByOrder(productionOrderId) {
    const tasks = listRuntimeTasksByOrder(productionOrderId).filter((task) => isRuntimeTaskExecutionTask(task));
