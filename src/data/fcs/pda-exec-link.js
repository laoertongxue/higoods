...
import { buildMilestoneRuleLabel, getMilestoneConfigForTask, getMilestoneProofRequirementLabel, getMilestoneTargetUnitByRuleType, getMilestoneTargetUnitLabel, } from './milestone-configs';
import { generateCaseId, getProgressExceptionById, listProgressExceptions, upsertProgressExceptionCase, } from './store-domain-progress.ts';
...
function mapPauseReasonToBlockReason(reasonCode) {
    if (reasonCode === 'MATERIAL_ISSUE')
        return 'MATERIAL';
    if (reasonCode === 'TECH_DOC_ISSUE')
        return 'TECH';
    if (reasonCode === 'EQUIPMENT_ISSUE')
        return 'EQUIPMENT';
    if (reasonCode === 'STAFF_ISSUE')
        return 'CAPACITY';
    if (reasonCode === 'CUTTING_ISSUE')
        return 'QUALITY';
    return 'OTHER';
}
function mapPauseReasonToExceptionReason(reasonCode) {
    if (reasonCode === 'MATERIAL_ISSUE')
        return 'BLOCKED_MATERIAL';
    if (reasonCode === 'TECH_DOC_ISSUE')
        return 'BLOCKED_TECH';
    if (reasonCode === 'EQUIPMENT_ISSUE')
        return 'BLOCKED_EQUIPMENT';
    if (reasonCode === 'STAFF_ISSUE')
...
function getPauseReasonLabel(reasonCode) {
    return PAUSE_REASON_OPTIONS.find((item) => item.code === reasonCode)?.label || '其他';
}
function normalizeMilestoneRuleType(raw) {
    return raw === 'AFTER_N_YARD' ? 'AFTER_N_YARD' : 'AFTER_N_PIECES';
}
function normalizeMilestoneTargetUnit(raw, ruleType) {
    if (raw === 'YARD' || raw === 'PIECE')
        return raw;
    return getMilestoneTargetUnitByRuleType(ruleType);
}
function ensureMilestoneDefaults(task) {
    ...
    const overdueExceptionEnabled = task.milestoneOverdueExceptionEnabled ??
    ...
    const ruleLabel = task.milestoneRuleLabel || config?.ruleLabel || buildMilestoneRuleLabel(ruleType, targetQty, targetUnit);
    const status = task.milestoneStatus || (task.milestoneReportedAt ? 'REPORTED' : 'PENDING');
    return {
        required: true,
        ruleType,
        ruleLabel,
...
export function getTaskMilestoneState(task) {
    return ensureMilestoneDefaults(task);
}
export function isTaskMilestoneRequired(task) {
    return ensureMilestoneDefaults(task).required;
}
export function isTaskMilestoneReported(task) {
    const milestone = ensureMilestoneDefaults(task);
    return milestone.required ? milestone.status === 'REPORTED' : true;
}
export function getTaskMilestoneWarningText(task) {
    const milestone = ensureMilestoneDefaults(task);
    if (!milestone.required || milestone.status === 'REPORTED')
        return '';
    return milestone.ruleLabel;
}
export function getTaskMilestoneProofHint(task) {
    const milestone = ensureMilestoneDefaults(task);
    return `当前要求：${milestone.ruleLabel}；凭证要求：${milestone.proofRequirementLabel}`;
}
export function isTaskMilestoneProofSatisfied(task, proofFiles) {
    const milestone = ensureMilestoneDefaults(task);
    if (!milestone.required)
        return true;
    if (milestone.proofRequirement === 'NONE')
        return true;
...
function findWritableTask(taskId) {
    return processTasks.find((item) => item.taskId === taskId) || getPdaTaskFlowTaskById(taskId) || undefined;
}
export function reportTaskMilestone(taskId, payload) {
    const task = findWritableTask(taskId);
    if (!task)
        return { ok: false, message: '任务不存在' };
    if (task.status !== 'IN_PROGRESS')
        return { ok: false, message: '仅进行中任务可上报关键节点' };
    const milestone = ensureMilestoneDefaults(task);
    if (!milestone.required)
        return { ok: false, message: '当前任务无需关键节点上报' };
    if (milestone.status === 'REPORTED')
        return { ok: false, message: '关键节点已上报' };
...
function createPauseException(task, payload) {
    const createdAt = nowTimestamp();
    const caseId = generateCaseId();
    const severity = payload.reasonCode === 'EQUIPMENT_ISSUE' ? 'S1' : 'S2';
...
function getActivePauseException(taskId) {
    return listProgressExceptions().find((item) => item.sourceType === 'FACTORY_PAUSE_REPORT' &&
        item.relatedTaskIds.includes(taskId) &&
        (item.caseStatus === 'OPEN' || item.caseStatus === 'IN_PROGRESS'));
}
function addHours(baseAt, hours) {
    const date = new Date(baseAt.replace(' ', 'T'));
    date.setHours(date.getHours() + hours);
    return nowTimestamp(date);
}
function isOpenMilestoneOverdueException(exceptionCase) {
    return (exceptionCase.reasonCode === 'MILESTONE_NOT_REPORTED' &&
        (exceptionCase.caseStatus === 'OPEN' || exceptionCase.caseStatus === 'IN_PROGRESS'));
}
function findTaskOpenMilestoneOverdueException(taskId) {
    return listProgressExceptions().find((item) => isOpenMilestoneOverdueException(item) && item.relatedTaskIds.includes(taskId));
}
function createMilestoneOverdueException(task, milestone, overdueAt, now) {
    const caseId = generateCaseId();
...
function resolveMilestoneOverdueException(exceptionCase, now) {
...
export function syncMilestoneOverdueExceptions(now = new Date()) {
...
export function reportTaskPause(taskId, payload) {
    const task = getTaskById(taskId);
    if (!task)
        return { ok: false, message: '任务不存在' };
    if (task.status !== 'IN_PROGRESS')
        return { ok: false, message: '仅进行中任务可上报暂停' };
    const existing = getActivePauseException(taskId);
    if (existing)
        return { ok: false, message: '当前任务已上报暂停，待平台处理' };
    const now = nowTimestamp();
...
export function recordPauseExceptionFollowUp(caseId, remark, by) {
    const exc = getCaseById(caseId);
    if (!exc || exc.sourceType !== 'FACTORY_PAUSE_REPORT')
        return { ok: false, message: '异常不存在' };
    if (exc.caseStatus === 'CLOSED' || exc.caseStatus === 'RESOLVED')
        return { ok: false, message: '异常已结束' };
    const now = nowTimestamp();
    const updated = {
        ...exc,
...
export function allowContinueFromPauseException(caseId, by) {
    const exc = getCaseById(caseId);
    if (!exc || exc.sourceType !== 'FACTORY_PAUSE_REPORT')
        return { ok: false, message: '异常不存在' };
    if (exc.caseStatus === 'CLOSED')
        return { ok: false, message: '异常已关闭' };
    if (exc.caseStatus === 'RESOLVED')
        return { ok: false, message: '异常已解决' };
    const now = nowTimestamp();
...
export function getPauseHandleStatus(task) {
    const pauseStatus = task.pauseStatus || 'NONE';
