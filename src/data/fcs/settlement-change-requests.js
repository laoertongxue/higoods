...
const OPEN_REQUEST_STATUSES = ['PENDING_REVIEW'];
...
function inferVersionChangeSource(sourceRequestId) {
    if (sourceRequestId === 'INIT')
        return '初始化';
    if (sourceRequestId === 'VERSION_MANUAL')
        return '平台新增版本';
    return 'PDA收款账号修改申请';
}
function inferVersionChangeItems(sourceRequestId) {
    if (sourceRequestId === 'INIT')
        return ['结算配置', '收款账号', '扣款规则'];
    if (sourceRequestId === 'VERSION_MANUAL')
        return ['结算配置', '扣款规则'];
    return ['收款账号'];
}
...
function resolveSettlementConfigSnapshot(factoryId) {
    const activeProfile = settlementProfiles.find((item) => item.factoryId === factoryId && item.isActive) ??
...
function resolveDeductionRuleSnapshots(factoryId) {
...
function buildEffectiveInfo(seed) {
...
function buildVersionRecord(seed) {
    const accountSnapshot = cloneAccountSnapshot(seed);
    return {
        ...seed,
        expiryAt: '',
...
const settlementEffectiveInfos = [
    {
...
const settlementVersionHistory = [
    {
        versionId: 'VER-0001',
        factoryId: 'ID-FAC-0001',
        factoryName: 'PT Sinar Garment Indonesia',
...
function createLog(actor, action, remark, createdAt) {
...
const settlementChangeRequests = [
    {
    ...
    },
    {
        ...
        logs: [
            ...
            createLog('平台运营-陈彦', '驳回申请', '开户名与证件信息不一致，请补充有效证明后重新提交', '2026-03-11 16:20'),
        ],
    },
];
const settlementInitDrafts = [
    {
        draftId: 'SID-0001',
...
function nextInitDraftId() {
    const id = `SID-${String(initDraftSeq).padStart(4, '0')}`;
    initDraftSeq += 1;
    return id;
}
function parseVersionNo(versionNo) {
    const matched = versionNo.match(/^V(\d+)$/);
    return matched ? Number(matched[1]) : 1;
}
function calcNextVersionNo(versionNo) {
    return `V${parseVersionNo(versionNo) + 1}`;
}
function getLatestVersionRecordByFactory(factoryId) {
    const records = settlementVersionHistory.filter((item) => item.factoryId === factoryId);
    if (records.length === 0)
        return null;
    return records.reduce((latest, item) => parseVersionNo(item.versionNo) > parseVersionNo(latest.versionNo) ? item : latest);
}
function normalizeFactoryVersionHistory(factoryId) {
...
function normalizeAllVersionHistory() {
...
function getLatestVersionNoByFactory(factoryId) {
...
function getRequestByIdOrNull(requestId) {
    return settlementChangeRequests.find((item) => item.requestId === requestId) ?? null;
}
function getEffectiveInfoByFactoryOrNull(factoryId) {
    return settlementEffectiveInfos.find((item) => item.factoryId === factoryId) ?? null;
}
function pushRequestLog(request, actor, action, remark) {
    request.logs.unshift(createLog(actor, action, remark, nowText()));
}
function summarizeChangedFields(before, after) {
    const changed = [];
    if (before.accountHolderName !== after.accountHolderName)
        changed.push('开户名');
    if (before.idNumber !== after.idNumber)
        changed.push('证件号');
    if (before.bankName !== after.bankName)
        changed.push('银行名称');
    if (before.bankAccountNo !== after.bankAccountNo)
        changed.push('银行账号');
    if (before.bankBranch !== after.bankBranch)
        changed.push('开户支行');
    return changed.length > 0 ? changed.join('、') : '信息确认';
}
function applyAccountSnapshot(target, snapshot) {
...
export function getSettlementEffectiveInfos() {
    return settlementEffectiveInfos;
}
export function getSettlementVersionHistory(factoryId) {
    if (!factoryId)
        return settlementVersionHistory;
    return settlementVersionHistory.filter((item) => item.factoryId === factoryId);
}
export function getSettlementChangeRequests() {
    return settlementChangeRequests;
}
export function listSettlementRequestsByFactory(factoryId) {
    return settlementChangeRequests
        .filter((item) => item.factoryId === factoryId)
        .slice()
        .sort((a, b) => b.submittedAt.localeCompare(a.submittedAt));
}
export function getSettlementInitDrafts() {
...
export function getSettlementEffectiveInfoByFactory(factoryId) {
    return getEffectiveInfoByFactoryOrNull(factoryId);
}
function parseDateTimeForCompare(value) {
    if (!value)
...
export function getSettlementEffectiveInfoByFactoryAt(factoryId, referenceAt) {
...
export function getSettlementRequestById(requestId) {
    return getRequestByIdOrNull(requestId);
}
export function getSettlementInitDraftByFactory(factoryId) {
    return settlementInitDrafts.find((item) => item.factoryId === factoryId) ?? null;
}
export function getSettlementLatestRequestByFactory(factoryId) {
    return listSettlementRequestsByFactory(factoryId)[0] ?? null;
}
export function getSettlementActiveRequestByFactory(factoryId) {
...
export function createSettlementVersionFromCurrent(payload) {
    ...
    if (!current)
        return { ok: false, message: '工厂尚未初始化结算资料，无法新增版本' };
...
export function createSettlementChangeRequest(payload) {
    const current = getEffectiveInfoByFactoryOrNull(payload.factoryId);
    if (!current)
        return { ok: false, message: '未找到当前生效结算资料' };
    const activeRequest = getSettlementActiveRequestByFactory(payload.factoryId);
    if (activeRequest)
        return { ok: false, message: '当前已有结算资料修改申请处理中' };
    ...
    pushRequestLog(request, payload.submittedBy, '提交申请', `工厂提交结算资料修改申请（变更项：${summarizeChangedFields(request.before, request.after)}）`);
...
export function verifySettlementRequest(requestId, operator, remark) {
    const request = getRequestByIdOrNull(requestId);
    if (!request)
        return { ok: false, message: '申请不存在' };
    if (request.status !== 'PENDING_REVIEW')
        return { ok: false, message: '当前状态不可记录核实' };
...
export function markSettlementRequestPrinted(requestId, operator) {
    const request = getRequestByIdOrNull(requestId);
    if (!request)
        return { ok: false, message: '申请不存在' };
    if (request.status !== 'PENDING_REVIEW') {
...
export function uploadSettlementSignedProof(requestId, operator, fileType) {
    const request = getRequestByIdOrNull(requestId);
    if (!request)
        return { ok: false, message: '申请不存在' };
    if (request.status !== 'PENDING_REVIEW') {
...
export function submitSettlementSignedProof(requestId, operator) {
    const request = getRequestByIdOrNull(requestId);
    if (!request)
        return { ok: false, message: '申请不存在' };
    if (request.status !== 'PENDING_REVIEW')
        return { ok: false, message: '当前状态不可提交签字附件' };
    if (request.signedProofFiles.length === 0)
        return { ok: false, message: '请先上传签字证明图片/附件' };
    pushRequestLog(request, operator, '提交签字附件', '签字证明附件已齐全');
    return { ok: true, message: '签字附件已提交，可执行审核', data: request };
}
// backward compatible exports
export function uploadSettlementSignedForm(requestId, operator, fileType) {
    return uploadSettlementSignedProof(requestId, operator, fileType);
}
export function submitSettlementSignedForms(requestId, operator) {
    return submitSettlementSignedProof(requestId, operator);
}
export function setSettlementRequestPaperArchived(requestId, paperArchived) {
    const request = getRequestByIdOrNull(requestId);
    if (!request)
        return { ok: false, message: '申请不存在' };
...
export function approveSettlementRequest(requestId, operator, reviewRemark) {
    const request = getRequestByIdOrNull(requestId);
    if (!request)
        return { ok: false, message: '申请不存在' };
    if (request.status !== 'PENDING_REVIEW')
        return { ok: false, message: '当前状态不可审核通过' };
    if (request.signedProofFiles.length === 0)
        return { ok: false, message: '请先上传签字证明图片/附件' };
    const current = getEffectiveInfoByFactoryOrNull(request.factoryId);
    if (!current)
        return { ok: false, message: '未找到当前生效信息' };
    ...
    const nextConfigSnapshot = cloneConfigSnapshot(latestRecord ? latestRecord.settlementConfigSnapshot : current.settlementConfigSnapshot);
    const nextRulesSnapshot = cloneDeductionRuleSnapshots(latestRecord ? latestRecord.defaultDeductionRulesSnapshot : current.defaultDeductionRulesSnapshot);
...
export function rejectSettlementRequest(requestId, operator, rejectReason) {
    const request = getRequestByIdOrNull(requestId);
    if (!request)
        return { ok: false, message: '申请不存在' };
    if (request.status !== 'PENDING_REVIEW') {
    ...
    if (!rejectReason.trim())
        return { ok: false, message: '请填写不通过原因' };
...
export function followupSettlementRequest(requestId, operator, followupRemark) {
    const request = getRequestByIdOrNull(requestId);
    if (!request)
        return { ok: false, message: '申请不存在' };
    if (request.status !== 'PENDING_REVIEW')
        return { ok: false, message: '当前状态不可记录处理备注' };
