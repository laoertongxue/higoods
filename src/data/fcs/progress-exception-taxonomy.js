...
export const REASON_TO_SUB_CATEGORY_KEY = {
...
const SUB_CATEGORY_KEY_SET = new Set(Object.keys(SUB_CATEGORY_LABEL));
export function isSubCategoryKey(value) {
    return SUB_CATEGORY_KEY_SET.has(value);
}
export function getUnifiedCategoryFromReason(reasonCode, legacyCategory) {
    if ([
        'TENDER_OVERDUE',
    ...
    if (legacyCategory === 'ASSIGNMENT')
        return 'ASSIGNMENT';
    if (legacyCategory === 'TECH_PACK')
        return 'TECH_PACK';
    if (legacyCategory === 'MATERIAL')
        return 'MATERIAL';
    if (legacyCategory === 'HANDOVER')
        return 'HANDOUT';
    return 'EXECUTION';
}
export function getDefaultSubCategoryKeyFromReason(reasonCode) {
    return REASON_TO_SUB_CATEGORY_KEY[reasonCode];
}
export function inferLegacySubCategoryKey(reasonCode, summary, detail) {
    const text = `${summary} ${detail}`;
    if (reasonCode === 'TECH_PACK_NOT_RELEASED') {
        if (/(缺失|缺少|缺项)/.test(text))
            return 'TECH_PACK_MISSING';
        if (/(确认|待批复|待评审)/.test(text))
            return 'TECH_PACK_PENDING_CONFIRM';
        return 'TECH_PACK_NOT_RELEASED';
    }
    if (reasonCode === 'MATERIAL_NOT_READY') {
        if (/(配料数量待补|配料不足|配料数量不足)/.test(text))
            return 'MATERIAL_PREP_PENDING';
        if (/(不足|缺口|不够)/.test(text))
            return 'MATERIAL_QTY_SHORT';
        if (/(多次|分批)/.test(text))
            return 'MATERIAL_MULTI_OPEN';
        return 'MATERIAL_NOT_READY';
    }
    if (reasonCode === 'HANDOVER_DIFF') {
        if (/异议/.test(text))
            return 'HANDOUT_OBJECTION';
        if (/混批/.test(text))
            return 'HANDOUT_MIXED';
        if (/(破损|损耗|丢失)/.test(text))
            return 'HANDOUT_DAMAGE';
        if (/(待查|待确认|待核实)/.test(text))
            return 'HANDOUT_PENDING_CHECK';
        return 'HANDOUT_DIFF';
    }
    return undefined;
}
export function getSubCategoryOptionsByCategory(category) {
    if (category === 'ALL') {
