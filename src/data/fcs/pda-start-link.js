...
import { getMilestoneConfigForTask, getMilestoneProofRequirementLabel, } from './milestone-configs';
import { generateCaseId, listProgressExceptions, upsertProgressExceptionCase, } from './store-domain-progress.ts';
import { getRuntimeTaskById, isRuntimeTaskExecutionTask, listRuntimeTasksByBaseTaskId, } from './runtime-process-tasks';
import { getPdaPickupHeads, getPdaPickupRecordsByHead, } from './pda-handover-events';
import { listWarehouseInternalTransferOrdersByRuntimeTaskId, listWarehouseIssueOrdersByRuntimeTaskId, listWarehouseReturnOrdersByRuntimeTaskId, } from './warehouse-material-execution';
import { getDefaultSubCategoryKeyFromReason, getUnifiedCategoryFromReason, } from './progress-exception-taxonomy';
import { markCaseResolved, maybeAutoCloseResolvedCase, } from './progress-exception-lifecycle';
import { isCuttingSpecialTask, listPdaCuttingExecutionRowsByTaskId, } from './pda-cutting-execution-source.ts';
...
import { getDyeWorkOrderByTaskId, getDyeWorkOrderStatusLabel, } from './dyeing-task-domain.ts';
import { getPrintWorkOrderByTaskId, getPrintWorkOrderStatusLabel, } from './printing-task-domain.ts';
const DEFAULT_START_DUE_HOURS = 48;
const SOON_THRESHOLD_MS = 24 * 60 * 60 * 1000;
function isCuttingReceiveReady(status) {
    return status.includes('来料已入仓') || status.includes('已回执') || status.includes('已领取');
}
function getCuttingStartPrerequisite(task) {
...
function getSpecialCraftTaskForStart(task) {
    return listSpecialCraftTaskOrders().find((taskOrder) => taskOrder.taskOrderId === task.taskId ||
        taskOrder.taskOrderNo === task.taskNo ||
        taskOrder.sourceTaskId === task.taskId ||
        taskOrder.sourceTaskNo === task.taskNo);
}
function getSpecialCraftStartPrerequisite(task) {
    ...
    const hasReceived = specialCraftTask.receivedQty > 0 ||
        Boolean(specialCraftTask.waitProcessStockItemIds?.length) ||
        ['IN_WAIT_PROCESS_WAREHOUSE', 'PROCESSING', 'COMPLETED', 'WAIT_HANDOVER', 'HANDED_OVER', 'WRITTEN_BACK'].includes(specialCraftTask.executionStatus || '');
    if (!hasReceived) {
        return {
            met: false,
            type: 'PICKUP',
...
function getPrintStartPrerequisite(task) {
    ...
    return {
        ...
        blocker: "已满足开工前置",
        hint: preparationHintMap[order.status] ||
            `印花加工单  已同步到 PDA，可先开工准备；实际开始印花前必须确认领料到位`,
    };
}
function getDyeStartPrerequisite(task) {
...
export function getStartPrerequisite(task) {
    ...
    const runtimeTasks = listRuntimeTasksByBaseTaskId(task.taskId).filter((runtimeTask) => isRuntimeTaskExecutionTask(runtimeTask));
    ...
    const hasSameFactoryContinue = runtimeTasks.some((runtimeTask) => runtimeTask.transitionFromPrev === 'SAME_FACTORY_CONTINUE');
...
export function getTaskStartRuleState(task) {
...
function evaluateRuntimeStartReadiness(task) {
    if (task.executorKind === 'WAREHOUSE_WORKSHOP') {
        ...
        const hasReadyTransfer = transferDocs.some((doc) => doc.status === 'RECEIVED' ||
            doc.status === 'CLOSED' ||
    ...
    if (task.transitionFromPrev === 'SAME_FACTORY_CONTINUE') {
        const upstreamDone = task.dependsOnTaskIds.every((upstreamTaskId) => {
            ...
            if (upstreamTask.status === 'DONE')
                return true;
            return listWarehouseReturnOrdersByRuntimeTaskId(upstreamTask.taskId).some((doc) => doc.status === 'RETURNED' || doc.status === 'CLOSED');
        });
        return upstreamDone
    ...
    const hasReadyPickup = pickupHeads.some((head) => {
        if (head.summaryStatus === 'WRITTEN_BACK' || head.completionStatus === 'COMPLETED')
            return true;
...
export function getStartDueBase(task) {
    if (task.assignmentMode === 'BIDDING') {
        const awardedAt = task.awardedAt;
        if (awardedAt) {
...
export function getTaskStartDueInfo(task, nowMs = Date.now()) {
...
function isOpenStartOverdueException(exceptionCase) {
    return (exceptionCase.reasonCode === 'START_OVERDUE' &&
        (exceptionCase.caseStatus === 'OPEN' || exceptionCase.caseStatus === 'IN_PROGRESS'));
}
function findTaskOpenStartOverdueException(taskId) {
    return listProgressExceptions().find((item) => isOpenStartOverdueException(item) && item.relatedTaskIds.includes(taskId));
}
function createStartOverdueException(task, startDueAt, now) {
...
function replaceException(updated) {
    upsertProgressExceptionCase(updated);
}
function resolveStartOverdueException(exceptionCase, now) {
...
export function syncPdaStartRiskAndExceptions(now = new Date()) {
    const nowMs = now.getTime();
    const nowAt = nowTimestamp(now);
    processTasks.forEach((task) => {
        if (!task.taskId.startsWith('PDA-EXEC-'))
            return;
...
export function formatRemainingHours(remainingMs) {
    const hours = Math.ceil(remainingMs / (60 * 60 * 1000));
    return String(Math.max(hours, 0));
}
export function formatStartDueSourceText(source, dueHours = DEFAULT_START_DUE_HOURS) {
