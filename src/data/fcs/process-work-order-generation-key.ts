...
export function normalizeProcessWorkOrderSourceSnapshot(source: ProcessWorkOrderSourceSnapshot): ProcessWorkOrderSourceSnapshot {
  ...
  if (source.sourceType === 'DESIGN_REVISION') {
    ...
    return {
      ...
      professionalTaskNo: optionalField(source.professionalTaskNo),
      // 新流程由目标物料 SKU 冻结加工属性；旧单的专业成果字段只供历史读取。
      ...
      targetMaterialSkuCode: optionalField(source.targetMaterialSkuCode),
      rawMaterialSkuCode: optionalField(source.rawMaterialSkuCode),
      dyedMaterialSkuCode: optionalField(source.dyedMaterialSkuCode),
      pantoneCode: optionalField(source.pantoneCode),
      patternCode: optionalField(source.patternCode),
      patternImageUrl: optionalField(source.patternImageUrl),
      inputMaterialSkuCode: optionalField(source.inputMaterialSkuCode) || optionalField(source.materialSkuCode),
      outputMaterialSkuCode: optionalField(source.outputMaterialSkuCode),
      ...
      materialCode: optionalField(source.materialCode),
      ...
      materialWidthCm: Number.isFinite(source.materialWidthCm) && Number(source.materialWidthCm) > 0 ? source.materialWidthCm : undefined,
      materialGsm: Number.isFinite(source.materialGsm) && Number(source.materialGsm) > 0 ? source.materialGsm : undefined,
