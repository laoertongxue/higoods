import { getFactoryWarehouseSummary, listFactoryWarehouseInboundRecords, listFactoryWarehouseOutboundRecords, listFactoryWarehouseStocktakeOrders, listFactoryWaitHandoverStockItems, listFactoryWaitProcessStockItems, } from './factory-internal-warehouse.ts';
...
import { listWoolWaitHandoverHandoutRecords, listWoolWaitHandoverInboundRecords, listWoolWaitProcessReceiptRecords, listWoolWarehouseInventory, } from './wool-task-domain.ts';
import { FULL_CAPABILITY_FACTORY_ID, listPostFinishingUpstreamHandovers, listPostFinishingWaitHandoverWarehouseRecords, listPostFinishingWaitProcessWarehouseRecords, } from './post-finishing-domain.ts';
...
function isSewingFactory(factoryId) {
...
export function getFactoryMobileTransferBagPackTasks(factoryId) {
    ...
    return listCuttingSewingTransferBags().filter((bag) => orderIds.has(bag.dispatchOrderId) && bag.dispatchStatus === '未交出');
}
export function getFactoryMobileTransferBagReceiveTasks(factoryId) {
...
export function getFactoryMobileWarehouseOverview(factoryId, factoryName) {
    ...
    if (factoryId === FULL_CAPABILITY_FACTORY_ID) {
        ...
        return {
            ...
            handoutCompletedOrderCount: waitHandoverRecords.filter((record) => (record.submittedHandoverGarmentQty >= record.waitHandoverGarmentQty
                && record.receivedHandoverGarmentQty >= record.submittedHandoverGarmentQty)).length,
            stocktakeWaitReviewCount: 0,
            stocktakeAdjustedCount: 0,
            transferBagPackTaskCount: 0,
            pendingTransferBagReceiveCount: 0,
...
export function getFactoryMobileWarehouseCards(factoryId, factoryName) {
