...
// 模板审计日志
export const templateAuditLogs = [
...
// 模板匹配算法
export function matchTemplates(processCodes, keywords) {
    const activeTemplates = getActiveTemplates();
    const results = [];
    for (const template of activeTemplates) {
        if (template.matchRule.mode !== 'AUTO')
            continue;
        let score = 0;
        ...
        const keywordMatched = templateKeywordsLower.filter(k => keywordsLower.some(kw => kw.includes(k) || k.includes(kw)));
