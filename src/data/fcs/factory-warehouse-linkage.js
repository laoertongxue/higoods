...
import { buildFactoryWaitHandoverStockItemFromOutboundRecord, buildFactoryWaitProcessStockItemFromInboundRecord, buildInboundRecordFromHandoverReceive, buildInboundRecordFromPickup, buildOutboundRecordFromHandoverRecord, decreasePendingFactoryWaitHandoverQty, findFactoryInternalWarehouseByFactoryAndKind, findFactoryWaitHandoverStockItemByHandoverRecordId, findPendingFactoryWaitHandoverStockItemByOrderId, findFactoryWarehouseOutboundRecordByHandoverRecordId, upsertFactoryWaitHandoverStockItem, upsertFactoryWaitProcessStockItem, upsertFactoryWarehouseInboundRecord, upsertFactoryWarehouseOutboundRecord, } from './factory-internal-warehouse.ts';
import { findPdaHandoverHead, findPdaHandoverRecord, findPdaPickupHead, findPdaPickupRecord, getHandoverOrderById, getPdaHeadSourceExecutionDoc, } from './pda-handover-events.ts';
...
function resolveWarehouse(factoryId, warehouseKind) {
...
function pickWarehousePosition(warehouse, status, seed) {
...
function resolvePickupBuilderBase(pickupRecordId, factory, warehouse) {
    ...
    if (!pickupHead || !sourceDoc || sourceDoc.docType !== 'ISSUE')
        return null;
    const pickupRecord = findPdaPickupRecord(pickupRecordId);
    if (!pickupRecord)
        return null;
    const issueDoc = sourceDoc;
...
export function linkPickupConfirmToInboundRecord(input) {
    ...
    const receivedQty = roundQty(input.receivedQty
    ...
    const status = differenceQty !== 0 ? '差异待处理' : '已入库';
    const baseRecord = builderBase
        || {
    ...
    const inboundRecord = {
        ...
        status,
        abnormalReason: differenceQty !== 0
...
export function linkHandoverReceiveToInboundRecord(input) {
    ...
    const head = (input.handoverOrderId ? getHandoverOrderById(input.handoverOrderId) : undefined)
...
export function linkTaskCompletionToWaitHandoverStock(input) {
    ...
    const existingPending = input.handoverOrderId
        ...
        : undefined;
    const waitHandoverQty = roundQty(Math.max((input.completedQty || 0) - (input.lossQty || 0) - (input.alreadyHandoverQty || 0), 0));
...
export function syncReceiverWritebackToOutboundRecord(input) {
    ...
    const differenceQty = roundQty(input.differenceQty
        ?? (input.receiverWrittenQty - existingOutbound.outboundQty));
    const status = differenceQty === 0 ? '已回写' : '差异';
...
export function syncQuantityObjectionToOutboundRecord(input) {
    ...
    const updatedWaitHandoverStockItem = upsertFactoryWaitHandoverStockItem({
        ...
        differenceQty: existingWaitItem?.differenceQty
