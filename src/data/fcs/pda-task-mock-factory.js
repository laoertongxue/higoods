...
import { PDA_MOBILE_PROCESS_DEFINITIONS, } from './pda-task-scenario-matrix.ts';
import { buildTaskQrValue } from './task-qr.ts';
import { TEST_FACTORY_ID, TEST_FACTORY_NAME } from './factory-mock-data.ts';
function getHandoutObjectTypeForProcess(key) {
    if (key === 'PRINTING')
        return 'CUT_PIECE';
    if (key === 'DYEING')
        return 'FABRIC';
...
function getHandoutQtyUnitForProcess(key) {
    if (key === 'PRINTING')
        return '片';
    if (key === 'DYEING')
        return '卷';
...
function getGarmentEquivalentQty(pieceQty, pieceCountPerGarment = 2) {
    return roundQty(pieceQty / pieceCountPerGarment);
}
const PROCESS_DEFINITION_BY_KEY = new Map(PDA_MOBILE_PROCESS_DEFINITIONS.map((item) => [item.key, item]));
...
function getProcessDef(profile) {
    return PROCESS_DEFINITION_BY_KEY.get(profile.key);
}
function isExternalMockProcess(profile) {
    return profile.key !== 'IRONING' && profile.key !== 'PACKAGING';
}
function resolveMockBusinessProcessCode(profile) {
    if (profile.key === 'SEWING')
        return 'SEW';
    if (profile.key === 'PRINTING')
        return 'PRINT';
    if (profile.key === 'DYEING')
        return 'DYE';
    if (profile.key === 'FINISHING')
        return 'POST_FINISHING';
    if (profile.key === 'QC')
        return 'QC';
    if (profile.key === 'IRONING')
        return 'IRONING';
    if (profile.key === 'PACKAGING')
        return 'PACKAGING';
    return profile.processCode;
}
function resolveMockStageCode(profile) {
...
function resolveMockStageName(profile) {
...
function resolveMockTaskReceiver(profile) {
    if (!isExternalMockProcess(profile))
        return null;
    if (profile.key === 'SEWING') {
        return {
...
function buildOwnerSuggestion(profile, assignmentMode) {
...
function createAuditLog(taskId, action, detail, at, by) {
    return {
...
function buildDirectTaskSet(profile, baseIndex) {
    const baseOrderNo = orderNo(baseIndex);
    const pendingAcceptDeadline = profile.key === 'SEWING'
        ? nowLike(28, '20:30:00')
        : nowLike(29, '10:00:00');
    return [
...
function buildBiddingTaskSet(profile, baseIndex) {
...
function buildHandoverSeeds(profile, tasks, baseIndex) {
    ...
    const stageCode = resolveMockStageCode(profile);
    const stageName = resolveMockStageName(profile);
    const openHandoutRecords = handoutObjectType === 'CUT_PIECE'
        ? [
            {
        ...
        : handoutObjectType === 'FABRIC'
            ...
            : [
                ...
                },
                {
                    ...
                    factorySubmittedAt: nowLike(28, '17:00:00'),
                    status: profile.key === 'SEWING' || profile.key === 'QC'
                        ? 'OBJECTION_REPORTED'
                        : 'PENDING_WRITEBACK',
                    warehouseReturnNo: profile.key === 'SEWING' || profile.key === 'QC'
                        ? `RET-DIFF-${profile.taskPrefix}-01`
                        : undefined,
                    warehouseWrittenQty: profile.key === 'SEWING' || profile.key === 'QC'
                        ? Math.max(Math.round(Math.max(Math.round(openHandoutTask.qty * 0.26), 48) * 0.92), 40)
                        : undefined,
                    warehouseWrittenAt: profile.key === 'SEWING' || profile.key === 'QC'
                        ? nowLike(28, '17:25:00')
                        : undefined,
                    receiverWrittenBy: profile.key === 'SEWING'
                        ? '后道收货员'
                        : profile.key === 'QC'
                            ? '成衣仓收货员'
                            : undefined,
                    factoryRemark: profile.key === 'SEWING'
                        ? '尾批已交我方后道工厂，数量异议处理中'
                        : profile.key === 'QC'
                            ? '数量异议待接收方确认'
                            : '尾批待接收方回写',
                    objectionReason: profile.key === 'SEWING'
                        ? '我方后道工厂回写数量少于工厂交出对象数量'
                        : profile.key === 'QC'
                            ? '抽检不合格数量差异'
                            : undefined,
                    objectionRemark: profile.key === 'SEWING'
                        ? '工厂复点与接收方回写不一致，待平台核定。'
                        : profile.key === 'QC'
                            ? '待复核差异责任'
                            : undefined,
    ...
    const doneHandoutRecords = handoutObjectType === 'CUT_PIECE'
        ? [
            {
    ...
    const openHandoutExpectedTotal = roundQty(openHandoutRecords.reduce((sum, record) => sum + record.plannedQty, 0));
    const openHandoutWrittenTotal = roundQty(openHandoutRecords.reduce((sum, record) => sum + (record.warehouseWrittenQty ?? 0), 0));
    const doneHandoutExpectedTotal = roundQty(doneHandoutRecords.reduce((sum, record) => sum + record.plannedQty, 0));
    const doneHandoutWrittenTotal = roundQty(doneHandoutRecords.reduce((sum, record) => sum + (record.warehouseWrittenQty ?? 0), 0));
    return {
        heads: [
            {
        ...
        pickupRecordsByHeadId: {
            [pickupHeadId]: [
                ...
                {
                    handoverId: pickupHeadId,
                    recordId: `${pickupHeadId}-007`,
                    taskId: pickupTask.taskId,
                    sequenceNo: 7,
                    materialCode: `${profile.taskPrefix}-MAT-007`,
                    materialSummary: `${profile.materialSummary}驳回批次`,
                    materialName: profile.materialSummary,
                    materialSpec: `${profile.processNameZh}驳回批次`,
                    skuCode: `${profile.taskPrefix}-SKU-007`,
                    skuColor: '标准色',
                    skuSize: '整单',
                    pieceName: '驳回批次',
                    qtyExpected: Math.max(Math.round(pickupTask.qty * 0.04), 10),
                    qtyActual: 0,
                    qtyUnit: '件',
                    submittedAt: nowLike(28, '13:20:00'),
                    status: 'REJECTED',
                    pickupMode: 'WAREHOUSE_DELIVERY',
                    qrCodeValue: `PICKUP-RECORD:${pickupHeadId}-007`,
                    warehouseHandedQty: Math.max(Math.round(pickupTask.qty * 0.04), 10),
                    warehouseHandedAt: nowLike(28, '13:40:00'),
                    warehouseHandedBy: '仓库扫码员',
                    factoryConfirmedAt: nowLike(28, '13:55:00'),
                    objectionReason: '到货内容与领料记录不一致',
                    objectionRemark: '工厂驳回本次领料，等待重新发料。',
                    followUpRemark: '驳回记录不进入待加工仓',
                    remark: '驳回后不生成入库记录',
                },
            ],
        },
...
const PDA_GENERIC_PROCESS_TASKS = [...directTasks, ...biddingTasks];
function cloneProcessTaskForStatistics(sourceTaskId, nextTaskId, productionOrderNo, qty) {
...
PDA_GENERIC_PROCESS_TASKS.push(cloneProcessTaskForStatistics('TASK-PRINT-000717', 'TASK-PRINT-000722', 'PO-202604-PRINT-011', 1460), cloneProcessTaskForStatistics('TASK-PRINT-000717', 'TASK-PRINT-000723', 'PO-202604-PRINT-012', 1380), cloneProcessTaskForStatistics('TASK-DYE-000726', 'TASK-DYE-000732', 'PO-202604-DYE-012', 1320), cloneProcessTaskForStatistics('TASK-DYE-000726', 'TASK-DYE-000733', 'PO-202604-DYE-013', 940));
const PDA_TEST_FACTORY_PROCESS_TASKS = PDA_GENERIC_PROCESS_TASKS
    .slice(0, 4)
    .map((task, index) => {
...
const handoverSeedCollections = PROCESS_PROFILES.filter((profile) => isExternalMockProcess(profile)).map((profile, index) => buildHandoverSeeds(profile, PDA_GENERIC_PROCESS_TASKS.filter((task) => task.mockProcessKey === profile.key), 400 + index * 3));
...
const PDA_GENERIC_BIDDING_TENDERS = PDA_GENERIC_PROCESS_TASKS
...
const PDA_GENERIC_QUOTED_TENDERS = PDA_GENERIC_PROCESS_TASKS
...
const PDA_GENERIC_AWARDED_TENDER_NOTICES = PDA_GENERIC_PROCESS_TASKS
...
export function listPdaGenericProcessTasks() {
    return [...PDA_GENERIC_PROCESS_TASKS, ...PDA_TEST_FACTORY_PROCESS_TASKS];
}
export function listPdaGenericBiddingTenderMocks() {
    return PDA_GENERIC_BIDDING_TENDERS.map((item) => ({ ...item }));
}
export function listPdaGenericQuotedTenderMocks() {
    return PDA_GENERIC_QUOTED_TENDERS.map((item) => ({ ...item }));
}
export function listPdaGenericAwardedTenderNoticeMocks() {
    return PDA_GENERIC_AWARDED_TENDER_NOTICES.map((item) => ({ ...item }));
}
export function listPdaGenericHandoverHeadSeeds() {
    return PDA_GENERIC_HANDOVER_HEADS.map((item) => ({ ...item }));
}
export function getPdaGenericPickupRecordSeedsByHeadId(handoverId) {
    return (PDA_GENERIC_PICKUP_RECORDS_BY_HEAD_ID[handoverId] ?? []).map((item) => ({ ...item }));
}
export function getPdaGenericHandoutRecordSeedsByHeadId(handoverId) {
    return (PDA_GENERIC_HANDOUT_RECORDS_BY_HEAD_ID[handoverId] ?? []).map((item) => ({ ...item }));
}
export function listPdaGenericTasksByFactory(factoryId) {
    return listPdaGenericProcessTasks().filter((task) => task.assignedFactoryId === factoryId);
}
export function listPdaGenericTasksByProcess(processKey) {
