...
export function calculateCapacityRemainingStandardHours(input) {
    return roundCapacityRuleValue(input.supplyStandardHours - input.committedStandardHours - input.frozenStandardHours);
}
export function isCapacityTight(input) {
    if (input.supplyStandardHours <= 0)
        return false;
    if (input.remainingStandardHours < CAPACITY_OVERLOAD_REMAINING_THRESHOLD)
        return false;
    return input.remainingStandardHours / input.supplyStandardHours < CAPACITY_TIGHT_THRESHOLD_RATIO;
}
export function roundCapacityRuleValue(value) {
    return Math.round(value * 1000) / 1000;
}
export function formatCapacityThresholdPercent(value) {
    return `${Math.round(value * 100)}%`;
}
export const CAPACITY_RULE_SECTIONS = [
    {
        key: 'SUPPLY_DEMAND',
        title: '供给与需求口径',
        lines: [
...
export const CAPACITY_THRESHOLD_RULES = [
    {
        label: '紧张阈值',
