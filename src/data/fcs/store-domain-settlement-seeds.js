...
export const initialMaterialStatementDrafts = [];
let statementAppealSeq = 1;
const FALLBACK_SETTLEMENT_CONFIG = {
    cycleType: 'WEEKLY',
    settlementDayRule: '每周五截止，次周三付款',
    pricingMode: 'BY_PIECE',
    currency: 'IDR',
};
const FALLBACK_RECEIVING_ACCOUNT = (partyId) => ({
...
function cloneSettlementConfigSnapshot(snapshot) {
    return { ...snapshot };
}
function cloneReceivingAccountSnapshot(snapshot) {
    return { ...snapshot };
}
function cloneDeductionRulesSnapshot(snapshots) {
    return snapshots.map((item) => ({ ...item }));
}
function resolveSettlementEffectiveFactoryId(partyId) {
    if (getSettlementEffectiveInfoByFactory(partyId))
        return partyId;
    const factoryById = getFactoryById(partyId);
    if (factoryById?.code && getSettlementEffectiveInfoByFactory(factoryById.code))
        return factoryById.code;
    const factoryByCode = getFactoryByCode(partyId);
    if (factoryByCode?.code && getSettlementEffectiveInfoByFactory(factoryByCode.code))
        return factoryByCode.code;
    return null;
}
function buildSettlementProfileSnapshot(partyId) {
...
function buildStatementPartyView(settlementPartyType, settlementPartyId) {
    const matchedFactory = getFactoryById(settlementPartyId);
    if (matchedFactory)
        return `${matchedFactory.name}（${settlementPartyId}）`;
...
function normalizeAppealRecord(appeal, draft) {
    return {
        ...
        resolutionResult: appeal.resolutionResult ?? ((appeal.status ?? 'SUBMITTED') === 'RESOLVED' ? 'UPHELD' : undefined),
        resolutionComment: appeal.resolutionComment ?? appeal.platformRemark,
    };
}
function getAppealRecords(statement) {
...
function normalizeStatementStatus(draft) {
    ...
    if (draft.status === 'READY_FOR_SETTLEMENT')
        return 'READY_FOR_PREPAYMENT';
    if (draft.status === 'CONFIRMED') {
        if (draft.factoryFeedbackStatus === 'FACTORY_CONFIRMED')
            return 'READY_FOR_PREPAYMENT';
        if (draft.factoryFeedbackStatus === 'RESOLVED') {
            if (draft.resolutionResult === 'REOPEN_REQUIRED')
                return 'CLOSED';
            return 'READY_FOR_PREPAYMENT';
        }
...
export function canStatementEnterSettlement(statement) {
    if (statement.status !== 'READY_FOR_PREPAYMENT')
        return false;
    return (statement.factoryFeedbackStatus === 'FACTORY_CONFIRMED' ||
        (statement.factoryFeedbackStatus === 'RESOLVED' && statement.resolutionResult === 'UPHELD'));
}
export function canStatementEnterPrepayment(statement) {
    return canStatementEnterSettlement(statement);
}
export function getStatementSettlementProgressView(statement) {
...
function resolveTaskNo(taskId) {
    if (!taskId)
        return undefined;
    return processTasks.find((item) => item.taskId === taskId)?.taskNo;
}
function resolveProductionOrderNo(productionOrderId) {
    if (!productionOrderId)
        return undefined;
    return productionOrders.find((item) => item.productionOrderId === productionOrderId)?.legacyOrderNo;
}
function enrichStatementDraftItemSeed(item, draft) {
    const cycleFields = item.settlementCycleId
        ? {
...
function finalizeStatementDraftFields(draft) {
    const ledgerIds = Array.from(new Set(draft.items.map((item) => item.sourceItemId)));
    const earningLedgerIds = Array.from(new Set(draft.items.filter((item) => item.sourceItemType === 'TASK_EARNING').map((item) => item.sourceItemId)));
    const deductionLedgerIds = Array.from(new Set(draft.items.filter((item) => item.sourceItemType === 'QUALITY_DEDUCTION').map((item) => item.sourceItemId)));
    const totalQty = draft.items.reduce((sum, item) => sum + (item.returnInboundQty ?? item.deductionQty ?? 0), 0);
    const totalEarningAmount = roundAmount(draft.items.reduce((sum, item) => sum + (item.earningAmount ?? 0), 0));
    const totalDeductionAmount = roundAmount(draft.items.reduce((sum, item) => sum + (item.qualityDeductionAmount ?? 0), 0));
    const netPayableAmount = roundAmount(draft.items.reduce((sum, item) => sum + (item.netAmount ?? item.deductionAmount ?? 0), 0));
    return {
        ...draft,
        ...
        settlementCurrency: draft.settlementCurrency ?? draft.settlementProfileSnapshot.settlementConfigSnapshot.currency,
        ledgerIds,
        earningLedgerIds,
        deductionLedgerIds,
...
function buildBatchSnapshotRefs(items) {
    const refs = new Map();
    for (const item of items) {
        if (!item.settlementProfileSnapshot)
            continue;
...
function cloneSettlementProfileSnapshot(snapshot) {
    return {
        ...snapshot,
...
function cloneStatementDraftItem(item) {
    return {
        ...item,
    };
}
function cloneGeneratedStatementDraft(draft) {
...
function cloneGeneratedPayableAdjustment(adjustment) {
    return {
        ...adjustment,
    };
}
function cloneGeneratedPreSettlementLedger(ledger) {
    return {
        ...ledger,
    };
}
function cloneGeneratedFeishuPaymentApproval(approval) {
    return {
        ...approval,
    };
}
function cloneGeneratedPaymentWriteback(writeback) {
    return {
        ...writeback,
    };
}
function cloneGeneratedSettlementBatch(batch) {
    ...
    return {
        ...
        settlementProfileVersionSummary: snapshotRefs.length === 0
            ? '未绑定结算资料版本'
...
export const initialStatementDrafts = settlementLinkedMockFactoryOutput.statementDrafts.map((draft) => cloneGeneratedStatementDraft(draft));
export const initialPayableAdjustments = settlementLinkedMockFactoryOutput.payableAdjustments.map((adjustment) => cloneGeneratedPayableAdjustment(adjustment));
export const initialTaskEarningLedgers = settlementLinkedMockFactoryOutput.taskEarningLedgers.map((ledger) => cloneGeneratedPreSettlementLedger(ledger));
export const initialStatementAdjustments = initialPayableAdjustments;
export const initialFeishuPaymentApprovals = settlementLinkedMockFactoryOutput.feishuPaymentApprovals.map((approval) => cloneGeneratedFeishuPaymentApproval(approval));
export const initialPaymentWritebacks = settlementLinkedMockFactoryOutput.paymentWritebacks.map((writeback) => cloneGeneratedPaymentWriteback(writeback));
export const initialSettlementBatches = settlementLinkedMockFactoryOutput.settlementBatches.map((batch) => cloneGeneratedSettlementBatch(batch));
export const initialPrepaymentBatches = initialSettlementBatches;
function isSameSettlementPartyId(left, right) {
    if (left === right)
        return true;
...
export function getStatementDraftById(statementId) {
    return initialStatementDrafts.find((item) => item.statementId === statementId) ?? null;
}
export function getStatementById(statementId) {
...
export function listStatements(options = {}) {
    ...
    return initialStatementDrafts.filter((item) => {
        if (options.settlementPartyId && !isSameSettlementPartyId(item.settlementPartyId, options.settlementPartyId))
            return false;
        if (options.settlementCycleId && item.settlementCycleId !== options.settlementCycleId)
            return false;
        if (options.status && options.status !== '__ALL__' && item.status !== options.status)
            return false;
        if (!keyword)
            return true;
        const haystack = [
...
export function listSettlementStatementsByParty(settlementPartyId) {
    return initialStatementDrafts.filter((item) => isSameSettlementPartyId(item.settlementPartyId, settlementPartyId));
}
export function listSettlementBatchesByParty(settlementPartyId) {
    return initialSettlementBatches.filter((batch) => batch.items.some((item) => isSameSettlementPartyId(item.settlementPartyId, settlementPartyId)));
}
export function listSettlementBatchesByStatement(statementId) {
    return initialSettlementBatches.filter((batch) => batch.statementIds.includes(statementId));
}
export function getPrepaymentBatchById(batchId) {
    return initialSettlementBatches.find((batch) => batch.batchId === batchId) ?? null;
}
export function listFeishuPaymentApprovals(batchId) {
    if (!batchId)
        return initialFeishuPaymentApprovals;
    return initialFeishuPaymentApprovals.filter((item) => item.batchId === batchId);
}
export function getFeishuPaymentApprovalById(approvalId) {
    return initialFeishuPaymentApprovals.find((item) => item.approvalId === approvalId) ?? null;
}
export function listPaymentWritebacks(batchId) {
    if (!batchId)
        return initialPaymentWritebacks;
    return initialPaymentWritebacks.filter((item) => item.batchId === batchId);
}
export function getPaymentWritebackById(writebackId) {
    return initialPaymentWritebacks.find((item) => item.writebackId === writebackId) ?? null;
}
function buildPayeeAccountSnapshotId(statement) {
    return `${statement.settlementProfileVersionNo}:${statement.settlementProfileSnapshot.receivingAccountSnapshot.bankAccountNo}`;
}
function isClosedBatchStatus(status) {
    return status === 'CLOSED';
}
function isPaidBatchStatus(status) {
    return status === 'PREPAID' || status === 'CLOSED';
}
function isApprovalActive(status) {
    return status !== 'REJECTED' && status !== 'CANCELED';
}
function nextFeishuApprovalStatus(status) {
    if (status === 'CREATED')
        return 'APPROVING';
    if (status === 'APPROVING')
...
function buildBatchStatementItems(statements) {
...
function updateStatementsForBatch(statements, batch, by, at) {
...
function buildBatchLifecyclePaymentCompat(batch, writeback, approval) {
    if (writeback) {
...
export function createPrepaymentBatch(input) {
    if (!input.statementIds.length)
        return { ok: false, message: '请至少选择一张对账单' };
    ...
    const occupied = initialSettlementBatches.find((batch) => !isClosedBatchStatus(batch.status) &&
...
export function applyPrepaymentBatchForPayment(input) {
    ...
    if (!batch)
        return { ok: false, message: '未找到对应预付款批次' };
...
export function syncFeishuPaymentApprovalStatus(input) {
    ...
    if (!approval)
        return { ok: false, message: '未找到对应飞书付款审批' };
    const batch = getPrepaymentBatchById(approval.batchId);
    if (!batch)
        return { ok: false, message: '未找到审批关联的预付款批次' };
    ...
    if (nextStatus === 'APPROVED_PENDING_PAYMENT')
...
export function createPaymentWriteback(input) {
    ...
    if (!batch)
        return { ok: false, message: '未找到对应预付款批次' };
    if (batch.paymentWritebackId)
        return { ok: false, message: '当前批次已存在正式打款回写，不能重复创建' };
    if (!batch.feishuApprovalId)
        return { ok: false, message: '当前批次尚未创建飞书付款审批' };
    const approval = getFeishuPaymentApprovalById(batch.feishuApprovalId);
    if (!approval)
        return { ok: false, message: '未找到对应飞书付款审批' };
    if (approval.status !== 'PAID')
        return { ok: false, message: '飞书付款审批未到已付款状态，不能创建打款回写' };
    const timestamp = input.at ?? nowText();
    const writeback = {
...
export function closePrepaymentBatch(input) {
    ...
    if (!batch)
        return { ok: false, message: '未找到对应预付款批次' };
    if (!isPaidBatchStatus(batch.status))
        return { ok: false, message: '仅已预付批次允许关闭' };
...
export function findOpenStatementByPartyAndCycle(settlementPartyId, settlementCycleId) {
    return (initialStatementDrafts.find((item) => item.status !== 'CLOSED' &&
        item.settlementCycleId === settlementCycleId &&
        isSameSettlementPartyId(item.settlementPartyId, settlementPartyId)) ?? null);
}
export function createStatementFromEligibleLedgers(input) {
    ...
    if (!input.items.length)
        return { ok: false, message: '当前工厂和结算周期暂无可入单的正式流水' };
    const timestamp = input.at ?? nowText();
...
export function updateStatementDraftRemark(input) {
    ...
    if (!statement)
        return { ok: false, message: '未找到对应对账单' };
    if (statement.status !== 'DRAFT')
        return { ok: false, message: '当前仅草稿可更新备注' };
    const timestamp = input.at ?? nowText();
...
export function syncStatementDraftFromBuild(input) {
    ...
    if (!statement)
        return { ok: false, message: '未找到对应对账单' };
    if (statement.status !== 'DRAFT')
        return { ok: false, message: '当前仅草稿可继续编辑' };
    if (!input.items.length)
        return { ok: false, message: '当前周期暂无可纳入的对账明细行' };
    const timestamp = input.at ?? nowText();
...
export function buildStatementSettlementProfileSnapshot(settlementPartyType, settlementPartyId) {
    if (settlementPartyType === 'FACTORY')
...
export function submitStatementFactoryConfirmation(input) {
    ...
    if (!statement)
        return { ok: false, message: '未找到对应对账单' };
    if (statement.status !== 'PENDING_FACTORY_CONFIRM')
...
export function submitStatementFactoryAppeal(input) {
    ...
    if (!statement)
        return { ok: false, message: '未找到对应对账单' };
    if (statement.status !== 'PENDING_FACTORY_CONFIRM')
        return { ok: false, message: '当前仅待工厂反馈的对账单可发起申诉' };
...
export function startStatementAppealHandling(input) {
    ...
    if (!appeal)
        return { ok: false, message: '当前没有待处理申诉' };
...
export function markStatementReadyForPrepayment(input) {
    ...
    if (!statement)
        return { ok: false, message: '未找到对应对账单' };
    if (statement.status === 'CLOSED')
        return { ok: false, message: '已关闭单据不能进入预付款' };
    if (statement.factoryFeedbackStatus !== 'FACTORY_CONFIRMED' &&
        !(statement.factoryFeedbackStatus === 'RESOLVED' && statement.resolutionResult === 'UPHELD')) {
...
export function listFactoryConfirmedStatementsEligibleForPrepayment() {
    return initialStatementDrafts.filter((statement) => canStatementEnterPrepayment(statement));
}
export function getSettlementVersionUsageStats(factoryId) {
    ...
    if (!effective)
        return { openStatementCount: 0, activeStatementCount: 0 };
    const relatedStatements = initialStatementDrafts.filter((item) => item.settlementProfileVersionNo === effective.versionNo &&
    ...
    const activeStatementCount = initialSettlementBatches.filter((item) => item.status !== 'CLOSED' && item.statementIds.some((statementId) => relatedStatementIds.has(statementId))).length;
    return {
        openStatementCount: relatedStatements.length,
        activeStatementCount,
    };
}
export const initialProductionOrderChanges = [
    {
