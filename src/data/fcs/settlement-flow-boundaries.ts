...
export const SETTLEMENT_FLOW_BOUNDARIES: Record<SettlementBoundaryId, SettlementBoundaryDefinition> = {
  ...
  statement: {
    ...
    excludes: ['不是扣款规则页', '不是接收事实页', '不是打款结果页'],
  ...
  'material-statement': {
    ...
    title: '车缝接收对账',
    layer: 'FLOW_OBJECT',
    definition: '车缝接收对账是基于车缝厂确认接收后形成的正式材料对账对象，用于沉淀材料应收、抵扣和对账对象管理。',
    responsibilities: ['沉淀正式材料对账口径', '区分接收事实与正式对账对象', '输出材料应收或抵扣结果', '保留后续进入结算主链的接入口'],
    upstream: ['裁片或辅料发出记录', '车缝厂领取确认', '退料入库', '实耗与损耗确认', '材料货权转移记录'],
    downstream: ['形成材料应收/抵扣口径', '当前阶段仅用于接收事实与对账对象管理，后续再进入结算主链'],
    excludes: ['不是仓储动作本身', '不是普通进度跟踪页'],
    pageIntro: '车缝接收对账承接车缝厂已确认的接收、退料和实耗结果，形成正式材料对账对象；当前阶段暂不计入本期应付结算生成。',
    routeNote: '接收事实发生在仓交接和执行链路，正式对账对象归属对账与结算；当前阶段暂不进入对账单待生成来源项。',
  },
  batch: {
    id: 'batch',
    title: '预付款批次',
...
export const SETTLEMENT_FLOW_FROZEN_RULES = {
  ...
  materialStatementScope:
    '车缝接收对账当前保留为正式对象，但暂不进入对账单待生成来源项，也不参与本期应付结算生成。',
