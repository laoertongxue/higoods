...
let prepareFailureForTest: { processCode: GeneratedProcessCode; occurrence: number; seen: number } | null = null
let rollbackFailureForTest: { processCode: GeneratedProcessCode; occurrence: number; seen: number } | null = null
...
export function setProcessWorkOrderGenerationPrepareFailureForTest(
  processCode: GeneratedProcessCode | null,
  occurrence = 1,
): void {
  prepareFailureForTest = processCode
    ? { processCode, occurrence: Math.max(Math.floor(occurrence), 1), seen: 0 }
    : null
}

export function setProcessWorkOrderGenerationRollbackFailureForTest(
  processCode: GeneratedProcessCode | null,
  occurrence = 1,
): void {
  rollbackFailureForTest = processCode
    ? { processCode, occurrence: Math.max(Math.floor(occurrence), 1), seen: 0 }
    : null
}

export interface PreparedProcessWorkOrderBatch {
  results: EnsuredProcessWorkOrders[]
  commit: () => EnsuredProcessWorkOrders[]
  rollback: () => void
}
...
function aggregateTransactionFailure(original: unknown, rollbackErrors: unknown[]): Error {
  ...
  return new AggregateError(
    ...
    `${originalError.message}；回滚时发生 ${rollbackErrors.length} 个异常`,
    { cause: originalError },
  )
}

function rollbackPreparedPlans(
  plans: Array<{ processCode: GeneratedProcessCode; plan: PreparedProcessWorkOrderRegistration }>,
): unknown[] {
  const rollbackErrors: unknown[] = []
  for (const { processCode, plan } of [...plans].reverse()) {
    try {
      plan.rollback()
      if (rollbackFailureForTest?.processCode === processCode) {
        rollbackFailureForTest.seen += 1
        if (rollbackFailureForTest.seen === rollbackFailureForTest.occurrence) {
          throw new Error(`模拟${processCode === 'DYE' ? '染色' : '印花'}加工单回滚失败`)
        }
      }
    } catch (error) {
...
export function ensureProcessWorkOrderBatch(
  ...
  return prepareProcessWorkOrderBatch(inputs).commit()
}
...
export function prepareProcessWorkOrderBatch(
  ...
  return {
    ...
    commit: () => {
      if (state === 'ROLLED_BACK') throw new Error('加工单批事务已回滚，不能再提交')
      if (state === 'COMMITTED') return structuredClone(results)
      const attempted: typeof prepared = []
      try {
        for (const item of prepared) {
          attempted.push(item)
          if (commitFailureForTest?.processCode === item.processCode) {
            commitFailureForTest.seen += 1
            if (commitFailureForTest.seen === commitFailureForTest.occurrence) {
              const processName = item.processCode === 'DYE' ? '染色' : '印花'
        ...
        state = 'ROLLED_BACK'
        throw aggregateTransactionFailure(error, rollbackPreparedPlans(attempted))
      }
    },
    rollback: () => {
      if (state === 'ROLLED_BACK') return
      const rollbackErrors = rollbackPreparedPlans(prepared)
      state = 'ROLLED_BACK'
      if (rollbackErrors.length) {
        throw new AggregateError(rollbackErrors, `加工单批事务回滚时发生 ${rollbackErrors.length} 个异常`)
      }
    },
  }
}
