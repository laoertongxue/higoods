...
function assertContinuousLengthSource(source:FactoryReceivingSource){
 if(source.lines.some(l=>l.measurementBasis!==undefined&&(l.measurementBasis!=='CONTINUOUS_LENGTH'||l.material.kind!=='ACCESSORY'||l.unit!=='米')))throw new Error('连续长度口径只能用于按米交出的辅料。')
}
...
export function getPrintingReceivingConflict(orderId:string,sku:string,origin:DyePartner):string|undefined {
 const data=read()
 const assigned=[...data.sources.flatMap(s=>s.lines.filter(l=>l.printingOrderId===orderId).map(l=>({sku:l.material.sku,origin:s.origin}))),...data.receipts.flatMap(r=>r.lines.filter(l=>l.printingOrderId===orderId||data.allocations.some(a=>a.receiptLineId===l.id&&a.printingOrderId===orderId)).map(l=>({sku:l.material.sku,origin:l.origin})))]
...
export function registerFactoryReceivingSource(source:FactoryReceivingSource){
 assertContinuousLengthSource(source)
...
export function approveFactoryTransfer(id:string,operator:string,at:string){const s=getFactoryReceivingSource(id);if(!s||s.type==='HANDOUT'||s.voidedAt)throw new Error('请选择有效的仓库调拨单。');if(!operator.trim()||!at)throw new Error('请确认审核人和时间。');if(s.id.startsWith('DR-MATERIAL-')&&s.lines.some(l=>l.sentQty<=0))throw new Error('设计改款备料必须先由仓库确认实际发料数量和卷码。');registerFactoryReceivingSource({...s,approvedAt:s.approvedAt||at,approvedBy:s.approvedBy||operator})}
...
export function prepareFactoryReceipt(input:FactoryReceiptInput):FactoryReceipt {
 ...
 const lines=input.lines.map((l,i):FactoryReceiptLine=>{
  ...
  if(line.printingOrderId){const error=getPrintingReceivingConflict(line.printingOrderId,line.material.sku,s.origin);if(error)throw new Error(error)}
  ...
  let qty:number,unit:'Yard'|'kg'|'片'|'米',yarn:FactoryReceiptLine['yarn']
  ...
  }else if(line.material.kind==='ACCESSORY'&&line.measurementBasis==='CONTINUOUS_LENGTH'){
   if(line.unit!=='米'||l.businessUnit!=='米'||!Number.isFinite(l.businessQty)||l.businessQty!<0||Math.abs(l.businessQty!*1000-Math.round(l.businessQty!*1000))>.000001)throw new Error('请填写实际米数，最多三位小数；未收到请明确填0。')
   if(l.weightKg!==undefined||l.grossKg!==undefined||l.tubes!==undefined||l.pcs!==undefined||l.rolls?.length)throw new Error('连续辅料按实测米数接收，不混填公斤、筒数或面料卷码。')
   qty=l.businessQty!;unit='米'
   const received=read().receipts.flatMap(r=>r.lines).filter(r=>r.sourceId===s.id&&r.sourceLineId===line.id).reduce((n,r)=>n+(r.businessUnit==='米'?r.businessQty??0:0),0)
   if(Math.round((received+qty)*1000)>Math.round(line.sentQty*1000))throw new Error('累计实收米数超过该批实际发出量，请核对未收余额。')
  ...
  if(line.material.kind==='ACCESSORY'&&line.measurementBasis!=='CONTINUOUS_LENGTH'&&convertReceiptQuantity(qty,'kg',line.unit)===undefined){if(l.businessUnit!==line.unit||!Number.isFinite(l.businessQty)||l.businessQty!<0||((qty===0)!==(l.businessQty===0)))throw new Error('请同时填写实收重量和原单单位的实测数量，零接收两项均填 0。')}
...
export function allocateFactoryReceivedMaterial(input:ReceivingAllocation){
 ...
 if(input.printingOrderId){const error=getPrintingReceivingConflict(input.printingOrderId,line.material.sku,line.origin);if(error)throw new Error(error)}
...
export interface FactoryMaterialUse {
 ...
 lines:Array<{receiptLineId:string;barcode?:string;qty:number;unit:'Yard'|'kg'|'米'}>
}
...
export function getReceiptMaterialUsed(receiptLineId:string,barcode?:string){
 return (read().materialUses??[]).reduce((total,use)=>total+use.lines.reduce((sum,line)=>sum+(line.receiptLineId===receiptLineId&&(barcode===undefined||line.barcode===barcode)?line.qty:0),0),0)
}
export function recordFactoryMaterialUsage(input:{id:string;printingOrderId?:string;dyeOrderId?:string; waterOrderId?:string;factoryId:string;operatorName:string;at:string;qty:number;unit:string;materialSku?:string;legacyAvailableQty:number}):void {
 ...
 const belongs=(o:{printingOrderId?:string;dyeOrderId?:string;waterOrderId?:string})=>Boolean(input.printingOrderId&&o.printingOrderId===input.printingOrderId||input.dyeOrderId&&o.dyeOrderId===input.dyeOrderId||input.waterOrderId&&o.waterOrderId===input.waterOrderId)
 ...
 if((input.dyeOrderId||input.printingOrderId)&&skus.size>1)throw new Error(`一张${input.printingOrderId?'印花':'染色'}加工单只能使用一种投入物料，请先核对已有来货关联。`)
