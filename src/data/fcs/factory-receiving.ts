...
function read():ReceivingData {
 ...
 if(raw){const saved=JSON.parse(raw) as ReceivingData;if(saved.version!==1||!['sources','deliveries','receipts','allocations'].every(k=>Array.isArray(saved[k as keyof ReceivingData])))throw new Error('接收记录格式不完整，请保留现场数据并联系主管。');validateStoredReceiving(saved);cache=saved}
