...
function sku(skuId, spuId, spuCode, spuName, colorName, sizeName, qty) {
    return {
...
let postFinishingTaskAcceptanceRecords = [];
function readPostFinishingTaskAcceptanceRecords() {
...
function persistPostFinishingTaskAcceptanceRecords() {
    if (typeof window !== 'undefined') {
        window.localStorage.setItem(POST_FINISHING_TASK_ACCEPTANCE_STORE_KEY, JSON.stringify(postFinishingTaskAcceptanceRecords));
    }
}
function hasPostFinishingTaskProgress(task) {
    return (task.currentStatus !== '待上游交出'
        && task.currentStatus !== '待收货') || task.receivedQty > 0 || task.qcOrderCount > 0 || task.postOrderCount > 0 || task.recheckOrderCount > 0 || task.waitHandoverQty > 0;
}
function getDefaultPostFinishingTaskAcceptance(task) {
...
export function getPostFinishingTaskAcceptance(task) {
...
export function acceptPostFinishingTask(postTaskId, operatorName = FULL_CAPABILITY_FACTORY_NAME, acceptedAt = nowText()) {
    ...
    if (index >= 0)
        postFinishingTaskAcceptanceRecords[index] = next;
    else
        postFinishingTaskAcceptanceRecords.unshift(next);
    persistPostFinishingTaskAcceptanceRecords();
    return cloneValue(next);
}
export function rejectPostFinishingTask(postTaskId, rejectReason, operatorName = FULL_CAPABILITY_FACTORY_NAME, rejectedAt = nowText()) {
    const task = getPostFinishingTaskById(postTaskId);
    if (!task)
        return null;
    const next = {
...
export function buildPostFinishingTaskId(productionOrderId) {
    return `POST-TASK-${productionOrderId.replace(/^PO-/, '')}`;
}
export function buildPostFinishingTaskNo(productionOrderNo) {
    return `后道任务-${productionOrderNo.replace(/^PO-/, '')}`;
}
function sumProductionOrderQty(order) {
    const demandQty = order.demandSnapshot.skuLines.reduce((sum, line) => sum + line.qty, 0);
    return order.planQty || demandQty;
}
function buildFallbackSkuLinesFromProductionOrder(order) {
...
function getSourceContextsForProductionOrder(productionOrderNo) {
...
function sumWaitProcessAvailableQty(records) {
    return roundQty(records.reduce((sum, record) => sum + Math.max(record.availableGarmentQty, 0), 0));
}
function sumQcOrderQty(records, status) {
    return roundQty(records
...
function resolvePostTaskStatus(input) {
    if (input.waitHandoverQty > 0)
        return '待交出';
    if (input.waitRecheckQty > 0)
        return '待复检';
    if (input.postDoingQty > 0)
        return '后道中';
    if (input.waitPostQty > 0)
        return '待后道';
    if (input.qcInProgressQty > 0)
        return '质检中';
    if (input.recheckDoneQty >= input.plannedQty && input.plannedQty > 0)
        return '已完成';
    if (input.waitQcQty > 0 || input.qcDoneQty > 0 || input.receivedQty > 0)
        return '待质检';
    return input.hasSourceContext ? '待收货' : '待上游交出';
}
function postTaskCurrentNode(status) {
    const nodeMap = {
        待上游交出: '等待车缝/毛织交出',
        待收货: '扫码收货',
        待质检: '质检',
...
function buildPostFinishingTaskView(order) {
...
const UPSTREAM_HANDOVER_RECORDS = SOURCE_CONTEXTS.slice(0, 4).map((context, index) => {
    const sourceFactoryType = context.sourceFactoryType === '毛织厂' ? '毛织任务' : '车缝任务';
...
let receiptRecords = SOURCE_CONTEXTS.map((context, index) => ({
...
function defect(defectItemId, qty) {
    return {
        defectItemId,
...
function receiptWarehouseRecordId(receipt, lineIndex) {
    return `PF-WP-${receipt.receiptId.replace('PF-RV-', 'RV-')}-${lineIndex + 1}`;
}
function defaultWarehouseAreaAndLocation(lineIndex) {
...
function buildQcWarehouseAllocations(receipt, qcOrderId) {
...
function buildQcOrder(index, context, receipt, options) {
    ...
    const qcResult = options.status !== '质检完成'
        ? '部分不合格'
        : defectiveQty <= 0
            ? '全数合规'
            : passedQty <= 0
...
function getContext(contextId) {
    ...
    if (!context)
        throw new Error(`未找到后道来源：${contextId}`);
    return context;
}
function nextIndexFrom(values, pattern) {
...
function cloneReceipt(record) {
    return { ...record, skuLines: record.skuLines.map(cloneSkuLine) };
}
function cloneQcOrder(order) {
    return {
        ...order,
...
function makeQualitySnapshot(qc) {
...
function makeActionRecord(input) {
    ...
    if (input.actionType === '扫码收货')
        action.receivedGarmentQty = input.acceptedQty;
    if (input.qc) {
...
function buildPostOrderFromQc(qc, index) {
...
function buildDirectRecheckFromQc(qc, index) {
    return {
...
function buildPostRecheck(qc, postOrderId, postOrderNo, index) {
    return {
        recheckOrderId: `PF-RC-${pad(index)}`,
        recheckOrderNo: `RC-POST-2026-${pad(index)}`,
        postTaskId: qc.postTaskId || buildPostFinishingTaskId(qc.productionOrderId),
...
function buildPendingRecheckFromQc(qc, index, postOrder) {
    return {
        recheckOrderId: `PF-RC-${pad(index)}`,
        recheckOrderNo: `RC-POST-2026-${pad(index)}`,
        postTaskId: qc.postTaskId || buildPostFinishingTaskId(qc.productionOrderId),
...
let postFinishingWorkOrders = qcOrders
    .filter((qc) => qc.qcStatus === '质检完成' && postFlags(qc).length > 0)
    .map((qc, index) => buildPostOrderFromQc(qc, qc.qcOrderId.endsWith('004') ? 4 : index + 1));
let sewingFactoryPostTasks = SOURCE_CONTEXTS.filter((context) => context.sourceTaskId).map((context, index) => {
    ...
    return {
        ...
        status: ['车缝完成', '后道完成', '已交后道工厂'][index % 3],
        needFactoryPostFinishing: Boolean(postOrder),
        managedPostFactoryId: FULL_CAPABILITY_FACTORY_ID,
        managedPostFactoryName: FULL_CAPABILITY_FACTORY_NAME,
    };
});
function ensurePostOrderFromQc(qc) {
...
function ensureDirectRecheckFromQc(qc) {
...
function ensurePostRecheckFromOrder(order) {
    ...
    if (!qc)
        throw new Error(`未找到后道单关联质检单：${order.qcOrderId}`);
...
export function listPostFinishingReceiptRecords() {
    return receiptRecords.map(cloneReceipt);
}
export function listPostFinishingTasks() {
    return productionOrders.map(buildPostFinishingTaskView);
}
export function getPostFinishingTaskById(postTaskId) {
...
export function getPostFinishingTaskByProductionOrder(productionOrderNoOrId) {
...
export function getPostFinishingTaskSkuLines(postTaskId) {
    const task = getPostFinishingTaskById(postTaskId);
    if (!task)
        return [];
    const contexts = getSourceContextsForProductionOrder(task.productionOrderNo);
    if (contexts.length)
        return contexts.flatMap((context) => context.skuLines.map(cloneSkuLine));
...
function findSourceContextForWarehouseRecord(record) {
    return SOURCE_CONTEXTS.find((context) => (context.productionOrderNo === record.sourceProductionOrderNo
        && context.spuId === record.spuId
        && (!context.sourceTaskNo || context.sourceTaskNo === record.sourceTaskNo)));
}
function sumQcInProgressQty(warehouseRecordId) {
    return qcOrders
...
export function listPostFinishingWaitQcSkuItems(input = {}) {
...
export function createPostFinishingQcOrder(input) {
    const targetTask = input.postTaskId ? getPostFinishingTaskById(input.postTaskId) : undefined;
    if (input.postTaskId && !targetTask)
        throw new Error('未找到后道任务，不能创建质检单。');
    ...
    if (!selected.length)
        throw new Error('请至少选择一个待质检 SKU。');
    const first = selected[0].item;
    if (targetTask && first.productionOrderNo !== targetTask.productionOrderNo)
        throw new Error('只能在当前后道任务下创建质检单。');
    const notSameOrder = selected.find(({ item }) => item.productionOrderNo !== first.productionOrderNo || item.spuId !== first.spuId);
    if (notSameOrder)
        throw new Error('一次质检单只能选择同一生产单下的同一款式 SKU。');
    const invalidQty = selected.find(({ item, qcQty }) => qcQty > item.waitQcQty);
    if (invalidQty)
        throw new Error(`SKU ${invalidQty.item.skuCode} 本次质检数量不能超过待质检数量。`);
    const context = SOURCE_CONTEXTS.find((item) => (item.productionOrderNo === first.productionOrderNo
        && item.spuId === first.spuId
        && (!item.sourceTaskNo || item.sourceTaskNo === first.sourceTaskNo)));
    if (!context)
        throw new Error('未找到该 SKU 对应的生产单和上游任务。');
...
export function completePostFinishingQcOrder(input) {
    ...
    if (!qc)
        throw new Error(`未找到质检单：${input.qcOrderId}`);
    ...
    if (postFlags(qc).length > 0)
        ensurePostOrderFromQc(qc);
    else
        ensureDirectRecheckFromQc(qc);
    refreshPostFinishingDerivedRecords();
    return cloneQcOrder(qc);
}
export function listPostFinishingQcOrderEntities() {
    return qcOrders.map(cloneQcOrder);
}
export function listPostFinishingRecheckOrderEntities() {
    return recheckOrders.map(cloneRecheck);
}
export function getPostFinishingRecheckOrderById(recheckOrderId) {
...
export function buildPostFinishingQcDeductionRecord(record) {
...
export function startSewingFactoryPostTask(taskId) {
    const task = sewingFactoryPostTasks.find((item) => item.taskId === taskId || item.postTaskId === taskId);
    if (!task)
        throw new Error(`未找到上游后道任务：${taskId}`);
    task.status = '后道中';
    return { ...task, skuLines: task.skuLines.map(cloneSkuLine) };
}
export function finishSewingFactoryPostTask(taskId) {
    const task = sewingFactoryPostTasks.find((item) => item.taskId === taskId || item.postTaskId === taskId);
    if (!task)
        throw new Error(`未找到上游后道任务：${taskId}`);
    task.status = '后道完成';
...
export function transferSewingFactoryPostTaskToManagedFactory(taskId) {
    const task = sewingFactoryPostTasks.find((item) => item.taskId === taskId || item.postTaskId === taskId);
    if (!task)
        throw new Error(`未找到上游后道任务：${taskId}`);
    task.status = '已交后道工厂';
...
function getMutablePostFinishingWorkOrder(postOrderId) {
    const order = postFinishingWorkOrders.find((item) => item.postOrderId === postOrderId);
    if (!order)
...
function getAction(order, actionType) {
    const normalized = normalizeActionType(actionType);
    if (normalized === '扫码收货')
        return order.receiveAction;
    if (normalized === '质检')
        return order.qcAction;
    if (normalized === '后道')
        return order.postAction;
    return order.recheckAction;
}
function startedStatus(actionType) {
    const normalized = normalizeActionType(actionType);
    if (normalized === '扫码收货')
        return '扫码收货中';
    if (normalized === '质检')
...
function doneStatus(actionType) {
    const normalized = normalizeActionType(actionType);
    if (normalized === '扫码收货')
        return '已入库';
    if (normalized === '质检')
...
function syncOrderStatusFields(order) {
...
function applyNextStatusAfterFinish(order, actionType) {
    const normalized = normalizeActionType(actionType);
    if (normalized === '扫码收货')
        order.currentStatus = '待质检';
    else if (normalized === '质检')
        order.currentStatus = order.requiresPostFinishing ? '待后道' : '待复检';
    else if (normalized === '后道')
...
export function applyPostFinishingActionFinish(input) {
    ...
    if (normalizeActionType(input.actionType) === '扫码收货')
        action.receivedGarmentQty = acceptedQty;
...
export function completePostFinishingWorkOrder(input) {
    const order = getMutablePostFinishingWorkOrder(input.postOrderId);
...
export function completePostFinishingRecheckOrder(input) {
    ...
    if (!recheck)
        throw new Error(`未找到复检单：${input.recheckOrderId}`);
...
function buildWaitProcessRecords() {
...
function applyQcAllocationsToWaitProcessRecords(records) {
...
let postFinishingHandoverSubmissions = [];
function getPostFinishingHandoverSubmissions() {
...
function persistPostFinishingHandoverSubmissions() {
...
function getRecheckLinePassedQty(recheck, line) {
...
function getHandoverSubmissionsForLine(recheck, line) {
    return getPostFinishingHandoverSubmissions().filter((item) => ((item.recheckOrderId === recheck.recheckOrderId || item.recheckOrderNo === recheck.recheckOrderNo)
        && item.skuLineId === line.skuLineId));
}
function buildWaitHandoverRecords() {
    return recheckOrders
        ...
        const handoverFlows = lineSubmissions.flatMap((submission, submissionIndex) => {
            ...
            if (typeof submission.receiverWrittenQty !== 'number')
                return [outFlow];
            return [
                outFlow,
                buildFlowRecord({
...
function readPostFinishingWarehouseStore() {
    if (typeof window === 'undefined')
...
function writePostFinishingWarehouseStore(store) {
    if (typeof window === 'undefined')
        return;
    window.localStorage.setItem(POST_FINISHING_WAREHOUSE_STORE_KEY, JSON.stringify(store));
}
function postFinishingWarehousePrefix(mode) {
    return mode === 'wait-process' ? 'PFP' : 'PFH';
}
function postFinishingWarehouseName(mode) {
    return mode === 'wait-process' ? '后道待加工仓' : '后道待交出仓';
}
function getDefaultPostFinishingWarehouseAreas(mode) {
...
export function listPostFinishingWarehouseAreas(mode) {
...
function findPostFinishingWarehouseArea(mode, areaIdOrName) {
    if (!areaIdOrName)
        return undefined;
    return listPostFinishingWarehouseAreas(mode).find((area) => area.areaId === areaIdOrName || area.areaName === areaIdOrName);
}
export function upsertPostFinishingWarehouseArea(input) {
    const store = readPostFinishingWarehouseStore();
...
function getDefaultPostFinishingWarehouseLocations(mode) {
    const prefix = postFinishingWarehousePrefix(mode);
...
export function listPostFinishingWarehouseLocations(mode) {
...
export function upsertPostFinishingWarehouseLocation(input) {
    const store = readPostFinishingWarehouseStore();
...
export function deletePostFinishingWarehouseLocation(locationId) {
    const store = readPostFinishingWarehouseStore();
    store.locations = store.locations.filter((location) => location.locationId !== locationId);
    writePostFinishingWarehouseStore(store);
}
export function listPostFinishingUpstreamHandovers() {
    return cloneValue(UPSTREAM_HANDOVER_RECORDS);
}
export function lookupPostFinishingUpstreamHandover(input) {
    const keyword = input.trim();
    if (!keyword)
        return undefined;
    const normalized = keyword.toLowerCase();
    const matched = UPSTREAM_HANDOVER_RECORDS.find((record) => (record.handoverRecordNo.toLowerCase() === normalized
...
export function confirmPostFinishingWarehouseReceipt(input) {
    ...
    const createdRecords = input.lines
        .map((lineInput, index) => {
        ...
        if (!line || actualQty <= 0 || !area)
            return null;
        const conflict = currentInventory.find((record) => (record.skuId === line.skuId
    ...
    if (!createdRecords.length)
        throw new Error('请至少填写一条有效的实收明细和库区。');
...
export function ensurePostFinishingHandoverWarehouseRecord(input) {
    ...
    if (!record)
        throw new Error('复检完成后才会进入后道待交出仓');
    return { ...record, flowRecords: record.flowRecords.map(cloneFlowRecord) };
}
export function listPostFinishingAvailableHandoverLines() {
    waitHandoverWarehouseRecords = buildWaitHandoverRecords();
...
export function recordPostFinishingHandoverSubmission(input) {
    const submittedQty = roundQty(input.submittedQty);
    if (!Number.isFinite(submittedQty) || submittedQty <= 0)
        throw new Error('交出数量必须大于 0');
    const line = listPostFinishingAvailableHandoverLines().find((record) => ((record.recheckOrderId === input.recheckOrderId || record.recheckOrderNo === input.recheckOrderNo)
        && record.skuLineId === input.skuLineId));
    if (!line)
        throw new Error('未找到可交出的后道复检库存明细');
    ...
    if (index >= 0)
        postFinishingHandoverSubmissions[index] = { ...postFinishingHandoverSubmissions[index], ...next };
    else
        postFinishingHandoverSubmissions.push(next);
    persistPostFinishingHandoverSubmissions();
    waitHandoverWarehouseRecords = buildWaitHandoverRecords();
    return waitHandoverWarehouseRecords.find((record) => ((record.recheckOrderId === input.recheckOrderId || record.recheckOrderNo === input.recheckOrderNo)
        && record.skuLineId === input.skuLineId));
}
export function writeBackPostFinishingHandoverSubmission(input) {
    ...
    return waitHandoverWarehouseRecords.find((record) => ((record.recheckOrderId === updated.recheckOrderId || record.recheckOrderNo === updated.recheckOrderNo)
        && record.skuLineId === updated.skuLineId));
}
export function listPostFinishingWorkOrders() {
    return postFinishingWorkOrders.map(cloneWorkOrder);
}
export function listPostFinishingActionRecords(actionType) {
...
export function listPostFinishingQcOrders() {
    return listPostFinishingActionRecords('质检');
}
export function listPostFinishingRecheckOrders() {
    return listPostFinishingActionRecords('复检');
}
export function listPostFinishingReceiveOrders() {
    return listPostFinishingActionRecords('扫码收货');
}
export function listSewingFactoryPostTasks() {
    return sewingFactoryPostTasks.map((task) => ({ ...task, skuLines: task.skuLines.map(cloneSkuLine) }));
}
export function listPostFinishingWaitProcessWarehouseRecords() {
...
export function listPostFinishingWaitHandoverWarehouseRecords() {
    waitHandoverWarehouseRecords = buildWaitHandoverRecords();
