...
import { buildProductionOrderDemandSnapshot, validateDemandTechPackOrderLink, } from './production-upstream-chain.ts';
import { buildProductionOrderTechPackSnapshot, cloneProductionOrderTechPackSnapshot, } from './production-tech-pack-snapshot-builder.ts';
export const PENDING_MAIN_FACTORY_ID = 'PENDING-SEWING-MAIN-FACTORY';
export function createFactorySnapshot(factory) {
    return {
        id: factory.id,
        code: factory.code,
        name: factory.name,
...
export function createPendingMainFactorySnapshot() {
    return {
...
export function isProductionOrderMainFactoryPending(order) {
    return order.mainFactoryStatus === 'PENDING_SEWING_ASSIGNMENT' || order.mainFactoryId === PENDING_MAIN_FACTORY_ID;
}
export function formatProductionOrderMainFactoryName(order) {
    return isProductionOrderMainFactoryPending(order) ? '待车缝任务分配确定' : order.mainFactorySnapshot.name;
}
function buildProductionOrderFromResolvedUpstream(seed, demand, techPackSnapshot) {
    ...
    return {
        ...
        lockedLegacy: seed.lockedLegacy ?? ['EXECUTING', 'COMPLETED', 'CANCELLED'].includes(seed.status),
...
export function buildProductionOrderFromSeed(seed) {
    ...
    if (!validation.ok || !validation.demand || !validation.techPackSnapshot) {
        throw new Error([`生产单 ${seed.productionOrderId} 上游链非法`, ...validation.issues.map((item) => item.message)].join('；'));
    }
    return buildProductionOrderFromResolvedUpstream(seed, validation.demand, validation.techPackSnapshot);
}
export function buildProductionOrderFromDemand(seed, demand, snapshotBy) {
...
function resolveMergedPriority(demands) {
    const weights = { NORMAL: 1, HIGH: 2, URGENT: 3 };
    return demands
        .map((demand) => demand.priority)
        .sort((a, b) => (weights[b] ?? 0) - (weights[a] ?? 0))[0] ?? 'NORMAL';
}
function resolveMergedDeliveryDate(demands) {
    return demands
        .map((demand) => demand.requiredDeliveryDate)
        .filter((value) => Boolean(value))
        .sort()[0] ?? null;
}
function buildMergedProductionOrderDemandSnapshot(demands) {
...
export function buildProductionOrderFromDemands(seed, demands, snapshotBy) {
    if (demands.length === 0) {
...
export function confirmProductionOrderMainFactoryFromSewingTask(input) {
    ...
    if (!order)
        return null;
    if (!isProductionOrderMainFactoryPending(order))
        return order;
    ...
    order.updatedAt = at;
    order.auditLogs.push(createAuditLog(`LOG-MAIN-FACTORY-${order.productionOrderId}-${order.auditLogs.length + 1}`, 'MAIN_FACTORY_CONFIRMED', `车缝任务分配给 ${input.factoryName || factory.name}，已回写为生产单主工厂和货权归属工厂`, at, input.by));
    return order;
}
export function getProductionOrderTechPackSnapshot(orderId) {
...
export const productionOrders = productionOrderSeeds.map((seed) => buildProductionOrderFromSeed(seed));
export const productionOrderStatusConfig = {
...
export const assignmentProgressStatusConfig = {
...
export const techPackStatusConfig = {
