...
import { patternMaterialFileTypeLabels, patternMaterialTypeLabels, } from './production-tech-pack-snapshot-types.ts';
import { listRuntimeExecutionTasksByOrder, } from './runtime-process-tasks.ts';
...
const POST_INCLUDED_PROCESS_NAMES = ['开扣眼', '装扣子', '熨烫', '包装'];
...
export const productionConfirmationStatusLabels = {
    PRINTABLE: '可打印',
    PRINTED: '已打印',
    VOIDED: '已作废',
};
const productionConfirmationRecords = [];
const productionConfirmationSnapshots = new Map();
...
function toConfirmationImage(label, url) {
...
function toLossRatePercent(value) {
    if (!Number.isFinite(value))
        return null;
...
function resolveUsageUnit(materialType) {
    if (materialType === '面料')
        return '米';
    return '件';
}
function resolveDemand(order) {
...
function resolvePatternMaterialTypeLabel(value) {
    return patternMaterialFileTypeLabels[value] || patternMaterialTypeLabels[value] || '暂无数据';
}
function buildConfirmationPatternFileName(item) {
...
function mapSizeMeasurements(rows) {
    return (rows ?? []).map((row) => ({
...
function toSnapshotImages(labelPrefix, urls) {
    return (urls ?? [])
        .map((url, index) => toConfirmationImage(`${labelPrefix}${index + 1}`, url))
        .filter((item) => Boolean(item));
}
function inferPatternMaterialType(order) {
    ...
    const combinedText = [
        ...
        .join(' ')
        .toLowerCase();
    if (combinedText.includes('毛织') || combinedText.includes('wool'))
        return 'WOOL';
    if (combinedText.includes('梭织') || combinedText.includes('布料') || combinedText.includes('woven'))
        return 'WOVEN';
    return 'UNKNOWN';
}
function getNextConfirmationVersion(productionOrderId) {
...
function buildConfirmationNo(productionOrderNo, confirmationVersion) {
    const suffix = productionOrderNo.replace(/^PO-/, '');
    return `PC-${suffix}-V${confirmationVersion}`;
}
function buildSnapshotId(productionOrderNo, confirmationVersion) {
...
export function getPostIncludedRemark() {
    return `内含：${POST_INCLUDED_PROCESS_NAMES.join('、')}`;
}
function isConfirmationExternalTask(task) {
    if (INTERNAL_POST_PROCESS_CODES.has(task.processBusinessCode || ''))
        return false;
    if (INTERNAL_POST_PROCESS_CODES.has(task.processCode))
        return false;
    return true;
}
...
function resolveAssignmentModeLabel(mode) {
    return mode === 'BIDDING' ? '竞价' : '派单';
}
function buildTaskAssignmentSnapshot(order) {
...
function buildBomSnapshot(order) {
    const techPackSnapshot = getProductionOrderTechPackSnapshot(order.productionOrderId);
    if (!techPackSnapshot)
        return [];
...
function buildSizeQtySnapshot(order) {
...
function buildPatternSnapshot(order) {
    const techPackSnapshot = getProductionOrderTechPackSnapshot(order.productionOrderId);
    if (!techPackSnapshot) {
    ...
    const sizeMeasurements = techPackSnapshot.sizeMeasurements.length
        ...
        : techPackSnapshot.sizeTable.flatMap((row) => ['S', 'M', 'L', 'XL']
...
function buildImageSnapshot(order) {
    const demand = resolveDemand(order);
    const techPackSnapshot = getProductionOrderTechPackSnapshot(order.productionOrderId);
    const styleArchive = findStyleArchiveByCode(order.demandSnapshot.spuCode);
    const techPackImages = techPackSnapshot?.imageSnapshot || null;
...
function buildStyleImageUrls(images) {
    return images.map((item) => item.url);
}
function buildSampleImageUrls(images) {
    return images.map((item) => item.url);
}
function buildProductMainImageUrl(images) {
    return images[0]?.url;
}
function buildProductionConfirmationSnapshotInternal(productionOrderId, confirmationVersion, confirmationNo, createdAt, createdBy) {
...
function createConfirmationRecord(input) {
    ...
    const snapshot = buildProductionConfirmationSnapshotInternal(input.productionOrderId, input.confirmationVersion, confirmationNo, input.createdAt, input.createdBy);
...
function initializeSeedConfirmations() {
    if (productionConfirmationRecords.length > 0)
        return;
    productionConfirmationRecords.push(createConfirmationRecord({
...
export function buildProductionConfirmationSnapshot(productionOrderId) {
    const version = getNextConfirmationVersion(productionOrderId);
    const order = productionOrders.find((item) => item.productionOrderId === productionOrderId);
    if (!order) {
        throw new Error(`未找到生产单：${productionOrderId}`);
    }
    return buildProductionConfirmationSnapshotInternal(productionOrderId, version, buildConfirmationNo(order.productionOrderNo, version), CONFIRMATION_CREATED_AT, CONFIRMATION_CREATED_BY);
}
export function listProductionConfirmationsByOrderId(productionOrderId) {
    return productionConfirmationRecords
        .filter((item) => item.productionOrderId === productionOrderId)
        .sort((a, b) => b.confirmationVersion - a.confirmationVersion);
}
export function getProductionConfirmationById(confirmationId) {
    return productionConfirmationRecords.find((item) => item.confirmationId === confirmationId);
}
export function getProductionConfirmationByOrderId(productionOrderId) {
    return listProductionConfirmationsByOrderId(productionOrderId)
        .find((item) => item.status !== 'VOIDED');
}
export function getProductionConfirmationSnapshotById(snapshotId) {
    return productionConfirmationSnapshots.get(snapshotId);
}
export function isProductionConfirmationPrintable(productionOrderId) {
    const order = productionOrders.find((item) => item.productionOrderId === productionOrderId);
    if (!order) {
...
export function getOrCreateProductionConfirmation(productionOrderId) {
...
export function voidProductionConfirmation(confirmationId, reason) {
