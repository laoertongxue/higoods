...
export const MILESTONE_RULE_TYPE_LABEL = {
    AFTER_N_PIECES: '完成第 N 件后上报',
    AFTER_N_YARD: '完成第 N Yard 后上报',
};
export const MILESTONE_TARGET_UNIT_LABEL = {
    PIECE: '件',
    YARD: 'Yard',
};
export const MILESTONE_PROOF_REQUIREMENT_LABEL = {
    NONE: '不要求凭证',
    IMAGE: '要求上传图片',
    VIDEO: '要求上传视频',
    IMAGE_OR_VIDEO: '图片或视频任选其一',
};
export const EXECUTION_FACTORY_TYPE_SCOPE_LABEL = {
    ALL: '全部工厂类型',
...
export const EXECUTION_TASK_TYPE_SCOPE_LABEL = {
    ALL: '全部任务类型',
    WHOLE_GARMENT: '整件毛织',
    PART_PANEL: '部位毛织',
    CUTTING: '裁片任务',
...
export function getMilestoneTargetUnitByRuleType(ruleType) {
    return ruleType === 'AFTER_N_YARD' ? 'YARD' : 'PIECE';
}
export function buildMilestoneRuleLabel(ruleType, targetQty, targetUnit) {
    const safeQty = Math.max(1, Math.floor(targetQty));
    const unit = targetUnit || getMilestoneTargetUnitByRuleType(ruleType);
    if (unit === 'YARD')
        return `完成第 ${safeQty} Yard 后上报`;
...
function nextMilestoneConfigId(processCode) {
...
function getExecutionFactoryTypeScopeLabel(scope) {
    return EXECUTION_FACTORY_TYPE_SCOPE_LABEL[scope];
}
function getExecutionTaskTypeScopeLabel(scope) {
...
function normalizeTargetQty(value) {
    if (!Number.isFinite(value))
        return 1;
    return Math.max(1, Math.floor(value));
}
function getConfigIndexById(id) {
    return milestoneConfigs.findIndex((config) => config.id === id);
}
function getConfigIndexByScope(processCode, processNameZh, factoryTypeScope, taskTypeScope) {
    return milestoneConfigs.findIndex((config) => config.processCode === processCode &&
        config.processNameZh === processNameZh &&
        config.factoryTypeScope === factoryTypeScope &&
        config.taskTypeScope === taskTypeScope);
}
export function listMilestoneConfigs() {
    return milestoneConfigs
        .map(cloneConfig)
        .sort((a, b) => a.processNameZh.localeCompare(b.processNameZh, 'zh-Hans-CN'));
}
export function getMilestoneConfigById(id) {
    const item = milestoneConfigs.find((config) => config.id === id);
    return item ? cloneConfig(item) : undefined;
}
export function getMilestoneConfigByProcess(processCode, processNameZh) {
    if (!processCode && !processNameZh)
        return undefined;
    const exact = milestoneConfigs.find((config) => {
        if (processCode && config.processCode !== processCode)
            return false;
        if (processNameZh && config.processNameZh !== processNameZh)
            return false;
        return true;
    });
    if (exact)
        return cloneConfig(exact);
    if (processCode) {
        const byCode = milestoneConfigs.find((config) => config.processCode === processCode);
        if (byCode)
            return cloneConfig(byCode);
    }
    if (processNameZh) {
        const byName = milestoneConfigs.find((config) => config.processNameZh === processNameZh);
        if (byName)
            return cloneConfig(byName);
    }
    return undefined;
}
...
export function resolveExecutionFactoryTypeScope(input) {
    ...
    if (factory?.id === OWN_WOOL_FACTORY_ID)
        return 'OWN_WOOL_FACTORY';
    if (factoryType === 'CENTRAL_WOOL' && factory?.id === OWN_WOOL_FACTORY_ID)
        return 'OWN_WOOL_FACTORY';
    if (includesAny(`${factoryType} ${explicitFactoryType}`, ['CUTTING', 'CUT']))
        return 'CUTTING_FACTORY';
    if (includesAny(`${factoryType} ${explicitFactoryType}`, ['FINISHING', 'POST', 'MANAGED_POST_FACTORY']))
        return 'POST_FACTORY';
    return 'PROCESS_FACTORY';
}
export function resolveExecutionTaskTypeScope(input) {
    ...
    const labelText = [
        ...
        .filter(Boolean)
        .join(' ');
    if (includesAny(codeText, ['WHOLE_GARMENT']) || labelText.includes('整件毛织'))
        return 'WHOLE_GARMENT';
    if (includesAny(codeText, ['PART_PANEL']) || labelText.includes('部位毛织'))
        return 'PART_PANEL';
    if (includesAny(codeText, ['CUTTING', 'CUT']) || labelText.includes('裁片'))
        return 'CUTTING';
    if (includesAny(codeText, ['PRINT', 'DYE']) || labelText.includes('印花') || labelText.includes('染色')) {
        return 'PRINT_DYE';
    }
    if (includesAny(codeText, ['SEW']) || labelText.includes('车缝'))
        return 'SEWING';
    if (includesAny(codeText, ['POST_FINISHING', 'IRON', 'PACK']) || labelText.includes('后道') || labelText.includes('整烫')) {
        return 'POST_FINISHING';
    }
    if (input.isSpecialCraft || labelText.includes('特殊工艺'))
        return 'SPECIAL_CRAFT';
    return 'ALL';
}
function scoreConfigMatch(config, factoryScope, taskScope) {
    let score = 0;
    if (config.factoryTypeScope === factoryScope)
        score += 20;
    else if (config.factoryTypeScope !== 'ALL')
        return -1;
    if (config.taskTypeScope === taskScope)
        score += 10;
    else if (config.taskTypeScope !== 'ALL')
        return -1;
    return score;
}
export function getMilestoneConfigForTask(input) {
...
export function listMilestoneProcessOptions() {
    const map = new Map();
    ...
    for (const task of processTasks) {
        if (!task.processCode || !task.processNameZh)
            continue;
        if (map.has(task.processCode))
            continue;
    ...
    return Array.from(map.values()).sort((a, b) => a.processNameZh.localeCompare(b.processNameZh, 'zh-Hans-CN'));
}
export function getMilestoneProofRequirementLabel(requirement) {
    return MILESTONE_PROOF_REQUIREMENT_LABEL[requirement];
}
export function getFactoryTypeScopeLabel(scope) {
    return getExecutionFactoryTypeScopeLabel(scope);
}
export function getTaskTypeScopeLabel(scope) {
    return getExecutionTaskTypeScopeLabel(scope);
}
export function getMilestoneStartRuleLabel(config) {
    if (!config.startRequired)
        return '不要求开工';
    return `要求开工，${config.startDueHours} 小时内确认`;
}
export function getMilestoneRuleTypeLabel(ruleType) {
    return MILESTONE_RULE_TYPE_LABEL[ruleType];
}
export function getMilestoneTargetUnitLabel(unit) {
    return MILESTONE_TARGET_UNIT_LABEL[unit];
}
export function getMilestoneOverdueRuleLabel(config) {
    if (!config.enabled || !config.overdueExceptionEnabled)
        return '未启用超时异常';
    return `开工后 ${config.overdueHours} 小时未上报进异常`;
}
export function createMilestoneConfig(payload, by) {
    if (getConfigIndexByScope(payload.processCode, payload.processNameZh, payload.factoryTypeScope, payload.taskTypeScope) >= 0) {
...
export function updateMilestoneConfig(id, payload, by) {
...
export function toggleMilestoneConfigEnabled(id, enabled, by) {
