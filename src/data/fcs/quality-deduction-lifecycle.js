import { qualityDeductionSharedCaseFacts, qualityDeductionSharedFormalLedgers, qualityDeductionSharedPendingDeductionRecords, qualityDeductionSharedSettlementAdjustments, } from './quality-deduction-shared-facts.ts';
...
function nextLifecycleId(prefix) {
    lifecycleSequence += 1;
...
export function getQualityDeductionNow() {
    return mockedNow ? new Date(mockedNow.getTime()) : new Date();
}
export function setQualityDeductionNowForTest(input) {
    const value = input instanceof Date ? input : new Date(input.replace(' ', 'T'));
    mockedNow = Number.isFinite(value.getTime()) ? value : null;
}
export function resetQualityDeductionNowForTest() {
    mockedNow = null;
}
export function formatQualityDeductionTimestamp(date = getQualityDeductionNow()) {
    return date.toISOString().replace('T', ' ').slice(0, 19);
}
export function parseQualityDeductionTimestamp(value) {
    if (!value)
        return null;
...
function getCaseFactByQcId(qcId) {
...
function syncPendingRegistry(caseFact) {
    if (!caseFact.pendingDeductionRecord)
        return;
    const index = qualityDeductionSharedPendingDeductionRecords.findIndex((item) => item.pendingRecordId === caseFact.pendingDeductionRecord?.pendingRecordId);
    if (index >= 0) {
...
function syncLedgerRegistry(caseFact) {
...
function clearLedgerRegistry(caseFact) {
    if (!caseFact.formalLedger)
        return;
...
function clearAdjustmentCompat(caseFact) {
    if (!caseFact.settlementAdjustment)
        return;
...
function resolveLedgerStatus(caseFact) {
    if (caseFact.formalLedger?.status)
        return caseFact.formalLedger.status;
    if (caseFact.settlementImpact.status === 'SETTLED')
        return 'PREPAID';
    if (caseFact.settlementImpact.includedSettlementBatchId)
        return 'INCLUDED_IN_PREPAYMENT_BATCH';
    if (caseFact.settlementImpact.includedSettlementStatementId)
        return 'INCLUDED_IN_STATEMENT';
    return 'GENERATED_PENDING_STATEMENT';
}
function resolveLedgerTrigger(caseFact) {
    if (caseFact.disputeCase?.adjudicationResult === 'PARTIALLY_ADJUSTED')
        return 'ADJUDICATION_PARTIAL_LIABILITY';
    if (caseFact.disputeCase?.adjudicationResult === 'UPHELD')
        return 'ADJUDICATION_FACTORY_LIABILITY';
    if (caseFact.pendingDeductionRecord?.status === 'SYSTEM_AUTO_CONFIRMED')
        return 'AUTO_CONFIRM';
    return 'FACTORY_CONFIRM';
}
function ensurePendingRecord(caseFact, nextStatus, at, by, comment) {
...
function buildLedger(caseFact, at, by) {
    const pending = caseFact.pendingDeductionRecord;
    if (!pending)
        return null;
    const adjudicatedWithLedger = caseFact.disputeCase?.adjudicationResult === 'UPHELD' ||
...
function syncCompatibilityFacts(caseFact) {
    const pending = caseFact.pendingDeductionRecord;
    ...
    impact.totalFinancialImpactAmount = roundAmount(impact.blockedProcessingFeeAmount + impact.effectiveQualityDeductionAmount);
}
export function deriveCaseStatus(caseFact) {
    return deriveQualityDeductionCaseStatus(caseFact);
}
export function findAutoConfirmCandidates(now = getQualityDeductionNow()) {
    return qualityDeductionSharedCaseFacts.filter((caseFact) => {
        const pending = caseFact.pendingDeductionRecord;
        if (!pending)
            return false;
        if (caseFact.qcRecord.isLegacy || caseFact.qcRecord.qcStatus === 'CLOSED')
            return false;
        if (pending.status !== 'PENDING_FACTORY_CONFIRM')
            return false;
...
export function resolveSettlementImpactAfterConfirmation(qcId, at = formatQualityDeductionTimestamp(), actorLabel = '工厂确认') {
...
export function autoConfirmOverdueQualityCases(now = getQualityDeductionNow()) {
    const processedQcIds = [];
    const skippedQcIds = [];
...
export function syncQualityDeductionLifecycle(now = getQualityDeductionNow()) {
    autoConfirmOverdueQualityCases(now);
    for (const caseFact of qualityDeductionSharedCaseFacts) {
        syncCompatibilityFacts(caseFact);
    }
}
export function adjudicateDisputeCase(input) {
    ...
    syncCompatibilityFacts(caseFact);
    caseFact.qcRecord.updatedAt = at;
    appendQcAuditLog(caseFact, `平台完成质量异议处理：${input.adjudicationComment.trim()}`, input.reviewerUserName, at, 'PLATFORM_ADJUDICATE_DISPUTE');
