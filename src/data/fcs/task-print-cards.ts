...
export function buildTaskDeliveryCardByRecord(
  ...
  return {
    ...
    card: {
      ...
      sourceType: head.sourceType || 'PRODUCTION_ORDER',
...
function mapDeliveryCardToPrintDoc(card: TaskDeliveryCardModel, record?: PdaHandoverRecord): TaskDeliveryCardPrintDoc {
  ...
  return {
    ...
    productionOrderNo: card.productionOrderNo,
    sourceType: card.sourceType,
    sourceSnapshot: card.sourceSnapshot ? structuredClone(card.sourceSnapshot) : undefined,
    stockMaterialId: card.stockMaterialId,
    stockMaterialName: card.stockMaterialName,
...
function buildRouteCardFromGenericTask(task: PdaGenericTaskMock): TaskRouteCardBuildResult {
  return {
    ...
    card: {
      ...
      summaryRemark: task.mockExecutionSummary || task.dispatchRemark || '按任务进度跟踪当前数据生成',
      supplementalItems: [
        { label: '来源类型', value: '任务进度跟踪任务' },
...
function buildRouteCardFromRuntimeTask(sourceId: string): TaskRouteCardBuildResult {
  ...
  return {
    ...
    card: {
      ...
      summaryRemark: task.dispatchRemark || task.blockRemark || '按任务进度跟踪当前数据生成',
      supplementalItems: [
        { label: '来源类型', value: '任务进度跟踪任务' },
        { label: '任务范围', value: task.scopeLabel || task.stageName || '整单任务' },
        { label: '分配状态', value: ASSIGNMENT_STATUS_LABEL[task.assignmentStatus] },
...
function buildPostFinishingTaskRouteRows(task: PostFinishingTaskView): TaskRouteCardRecordRow[] {
  return ensureRouteRecordNodes([
    ...
    },
    {
      ...
      station: task.managedPostFactoryName,
      operator: '后道工',
      remark: task.postOrderCount > 0 ? `后道加工单 ${task.postOrderCount} 张` : '质检未勾选实际工序时可直接进入复检',
    },
    {
...
function buildRouteCardFromPostFinishingTask(sourceId: string): TaskRouteCardBuildResult {
  ...
  if (!task) return { ok: false, title: TASK_ROUTE_CARD_NAME, message: `未找到后道生产任务：${sourceId}` }
  ...
  return {
    ...
    card: {
      ...
      summaryRemark: '生产单级后道生产任务流转卡，质检单、后道加工单、复检单均归属该任务。',
      titleOverride: '后道生产任务流转卡',
      summaryRowsOverride: [
        { label: '后道生产任务编号', value: task.postTaskNo },
      ...
      supplementalItems: [
        ...
        { label: '子单据', value: `质检单 ${task.qcOrderCount} 张 / 后道加工单 ${task.postOrderCount} 张 / 复检单 ${task.recheckOrderCount} 张` },
      ],
...
function buildRouteCardFromPostFinishingWorkOrder(sourceId: string): TaskRouteCardBuildResult {
  ...
  if (!order) return { ok: false, title: TASK_ROUTE_CARD_NAME, message: `未找到后道加工单：${sourceId}` }
  ...
  return {
    ...
    card: {
      ...
      summaryRemark: '按扫码收货、质检、实际工序、复检节点生成后道加工单流转卡',
      titleOverride: '后道加工单流转卡',
      summaryRowsOverride: [
        { label: '后道加工单号', value: order.postOrderNo },
        { label: '生产单号', value: order.sourceProductionOrderNo },
        ...
        { label: '已完成实际工序成衣件数', value: formatQtyText(order.postAction.acceptedGarmentQty, order.postAction.qtyUnit) },
        { label: '当前状态', value: order.currentStatus },
      ],
      supplementalItems: [
        { label: '任务模式', value: order.routeMode },
        { label: '当前流程', value: order.isPostDoneBySewingFactory ? '扫码收货 -> 质检 -> 复检 -> 交出' : '扫码收货 -> 质检 -> 实际工序 -> 复检 -> 交出' },
