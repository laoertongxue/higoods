...
export interface ProductionOrder {
  ...
  sewingFactorySnapshots?: FactorySnapshot[]
...
export function formatProductionOrderMainFactoryName(order: Pick<ProductionOrder, 'mainFactorySnapshot' | 'mainFactoryId' | 'mainFactoryStatus'>): string {
  ...
  return order.mainFactorySnapshot.name
}
...
function getProductionOrderSewingFactorySnapshots(order: ProductionOrder): FactorySnapshot[] {
  return order.sewingFactorySnapshots ?? []
}
...
export function listProductionOrderSewingFactories(productionOrderId: string): FactorySnapshot[] {
  ...
  return (order ? getProductionOrderSewingFactorySnapshots(order) : []).map((factory) => ({
    ...factory,
    tags: [...factory.tags],
  }))
}
...
export function registerProductionOrderSewingFactory(input: {
  ...
  const factorySnapshot = resolveProductionOrderFactorySnapshot(input.factoryId)
  if (!factorySnapshot) return null

  const at = input.at ?? new Date().toISOString().replace('T', ' ').slice(0, 19)
  const currentSnapshots = [...getProductionOrderSewingFactorySnapshots(order)]
  const alreadyRecorded = currentSnapshots.some((item) => item.id === factorySnapshot.id)
  const hadValidMainFactory = currentSnapshots.some((item) => item.id === order.mainFactoryId)
  if (alreadyRecorded && hadValidMainFactory) return order

  if (!alreadyRecorded) currentSnapshots.push(factorySnapshot)
  order.sewingFactorySnapshots = currentSnapshots
  const shouldAutoSelectMainFactory = !currentSnapshots.some((item) => item.id === order.mainFactoryId)
  if (shouldAutoSelectMainFactory) {
    const firstSewingFactory = currentSnapshots[0]
    order.mainFactoryId = firstSewingFactory.id
    order.mainFactorySnapshot = { ...firstSewingFactory, tags: [...firstSewingFactory.tags] }
    order.ownerPartyType = 'FACTORY'
    order.ownerPartyId = firstSewingFactory.id
    ...
    order.ownerReason = '第一家有效车缝承接工厂自动成为生产单主工厂。'
  }
  order.updatedAt = at
  order.auditLogs.push(
    createAuditLog(
      `LOG-SEWING-FACTORY-${order.productionOrderId}-${order.auditLogs.length + 1}`,
      shouldAutoSelectMainFactory ? 'SEWING_FACTORY_REGISTERED_AND_MAIN_SELECTED' : 'SEWING_FACTORY_REGISTERED',
      shouldAutoSelectMainFactory
        ? `已登记车缝承接工厂 ${factorySnapshot.name}，并自动设为生产单主工厂。`
        : `已登记车缝承接工厂 ${factorySnapshot.name}；当前主工厂仍为 ${formatProductionOrderMainFactoryName(order)}。`,
      at,
      input.by,
    ),
  )
...
export function selectProductionOrderMainFactory(input: {
  ...
  const order = productionOrders.find((item) => item.productionOrderId === input.productionOrderId)
  if (!order || !input.reason.trim()) return null

  const factorySnapshot = getProductionOrderSewingFactorySnapshots(order).find((factory) => factory.id === input.factoryId)
  if (!factorySnapshot) return null

  const at = input.at ?? new Date().toISOString().replace('T', ' ').slice(0, 19)
  const previousMainFactoryName = formatProductionOrderMainFactoryName(order)
  order.mainFactoryId = factorySnapshot.id
  order.mainFactorySnapshot = { ...factorySnapshot, tags: [...factorySnapshot.tags] }
  order.mainFactoryStatus = 'CONFIRMED'
  order.mainFactorySource = 'SEWING_TASK_ASSIGNMENT'
  order.mainFactoryConfirmedAt = at
  order.mainFactoryConfirmedBy = input.by
  order.ownerPartyType = 'FACTORY'
  order.ownerPartyId = factorySnapshot.id
  order.ownerReason = input.reason.trim()
  order.updatedAt = at
  order.auditLogs.push(
    createAuditLog(
      `LOG-MAIN-FACTORY-${order.productionOrderId}-${order.auditLogs.length + 1}`,
      'MAIN_FACTORY_SELECTED',
      `生产单主工厂调整：${previousMainFactoryName} → ${factorySnapshot.name}。原因：${input.reason.trim()}`,
      at,
      input.by,
    ),
  )
...
export function confirmProductionOrderMainFactoryFromSewingTask(input: {
  ...
  return registerProductionOrderSewingFactory(input)
}
...
const productionOrderSeeds: ProductionOrderSeed[] = [
  {
    ...
    ledgerDetails: {
      ...
      taskFactories: [
        ...
        { taskType: '烫包', taskNo: 'IRONPACK-143664', factory: 'alwi collection (504)', factoryType: '后道厂', status: '等待处理', plannedDoneAt: '2026-07-04', actualDoneAt: '-', completedQty: '0', issue: '前序未完成', action: '查看' },
  ...
  },
  {
    ...
    ledgerDetails: {
      materialIssues: [
        { materialType: '面料', materialName: 'Hoodie 抓绒主面料', materialSku: 'tdv_demand_SPU_2024_005-bom-main', requiredQty: '3,150 yd', purchasedQty: '3,150 yd', arrivedQty: '3,150 yd', preparedQty: '3,150 yd', issuedQty: '0 yd', factorySignedQty: '0 yd', shortageQty: '0 yd', expectedArrival: '-', status: '已确认配料' },
        { materialType: '辅料', materialName: '前中拉链', materialSku: 'tdv_demand_SPU_2024_005-bom-accessory-zipper', requiredQty: '2,500 条', purchasedQty: '2,500 条', arrivedQty: '3,100 条', preparedQty: '2,500 条', issuedQty: '0 条', factorySignedQty: '0 条', shortageQty: '0 条', expectedArrival: '-', status: '已确认配料' },
        { materialType: '辅料', materialName: '主唛', materialSku: 'tdv_demand_SPU_2024_005-bom-accessory-main-label', requiredQty: '2,500 套', purchasedQty: '2,500 套', arrivedQty: '2,800 套', preparedQty: '2,500 套', issuedQty: '0 套', factorySignedQty: '0 套', shortageQty: '0 套', expectedArrival: '-', status: '已确认配料' },
        { materialType: '辅料', materialName: '洗护唛', materialSku: 'tdv_demand_SPU_2024_005-bom-accessory-care-label', requiredQty: '2,500 套', purchasedQty: '2,500 套', arrivedQty: '2,600 套', preparedQty: '2,500 套', issuedQty: '0 套', factorySignedQty: '0 套', shortageQty: '0 套', expectedArrival: '-', status: '已确认配料' },
        { materialType: '纱线', materialName: '缝纫线', materialSku: 'tdv_demand_SPU_2024_005-bom-yarn-stitching', requiredQty: '30 公斤', purchasedQty: '30 公斤', arrivedQty: '42 公斤', preparedQty: '30 公斤', issuedQty: '0 公斤', factorySignedQty: '0 公斤', shortageQty: '0 公斤', expectedArrival: '-', status: '已确认配料' },
      ],
      taskFactories: [
        { taskType: '裁片', taskNo: 'TASKGEN-202603-0002-001', factory: 'PT Mulia Cutting Center', factoryType: '裁片厂', status: '已完成', plannedDoneAt: '2026-08-10', actualDoneAt: '2026-08-10', completedQty: '2,500', issue: '-', action: '查看' },
        { taskType: '车缝', taskNo: 'TASKGEN-202603-0002-002', factory: '待分配', factoryType: '车缝厂', status: '待分配', plannedDoneAt: '-', actualDoneAt: '-', completedQty: '0', issue: '等待分配', action: '分配任务' },
        { taskType: '烫包', taskNo: 'TASKGEN-202603-0002-003', factory: '待分配', factoryType: '后道阶段工厂', status: '待分配', plannedDoneAt: '-', actualDoneAt: '-', completedQty: '0', issue: '等待分配', action: '分配任务' },
      ],
      keyTimes: [
      ...
      quantityQuality: [
        { quantityType: '生产计划', plannedQty: '2,500', currentQty: '2,500', unit: '件', diff: '0', status: '正常', note: '生产单计划数量' },
        { quantityType: '裁片目标', plannedQty: '2,500', currentQty: '2,500', unit: '件', diff: '0', status: '已确认', note: '裁床已确认颜色与尺码目标' },
        { quantityType: '裁片齐套', plannedQty: '2,500', currentQty: '2,300', unit: '件', diff: '-200', status: '部分齐套', note: '按 Grey 各尺码当前最小齐套数汇总' },
        { quantityType: '裁床已确认放行', plannedQty: '2,500', currentQty: '2,320', unit: '件', diff: '-180', status: '风险放行', note: '含 20 件风险放行，分配时只提示不阻断' },
        { quantityType: '工厂签收', plannedQty: '2,500', currentQty: '0', unit: '件', diff: '-2,500', status: '未签收', note: '车缝任务尚未分配' },
        { quantityType: '回货数量', plannedQty: '2,500', currentQty: '0', unit: '件', diff: '-2,500', status: '未回货', note: '等待车缝任务分配后开工' },
        { quantityType: '质检通过', plannedQty: '2,500', currentQty: '0', unit: '件', diff: '-2,500', status: '未通过', note: '质检为回货流程节点，不是后道阶段工序' },
      ],
    },
    riskFlags: [],
    auditLogs: [
  ...
  },
  {
    ...
    taskBreakdownSummary: { isBrokenDown: true, taskTypesTop3: ['裁片', '车缝', '烫包'], lastBreakdownAt: '2026-03-04 11:20:00', lastBreakdownBy: 'Ahmad Wijaya' },
    ledgerDetails: {
      materialIssues: [
      ...
      taskFactories: [
        ...
        { taskType: '染色加工单', taskNo: 'DY-143691', factory: 'PT Cahaya Dyeing Sejahtera', factoryType: '染厂', status: '处理中', plannedDoneAt: '2026-07-01', actualDoneAt: '-', completedQty: '260', issue: '面料部分未到仓', action: '查看' },
        ...
        { taskType: '烫包', taskNo: 'IRONPACK-143693', factory: 'PT Cahaya Dyeing Sejahtera', factoryType: '后道工厂', status: '等待处理', plannedDoneAt: '2026-07-05', actualDoneAt: '-', completedQty: '0', issue: '包装袋未到仓', action: '查看' },
      ],
      keyTimes: [
