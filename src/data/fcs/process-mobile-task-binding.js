...
import { getDyeWorkOrderById, listDyeWorkOrders, } from './dyeing-task-domain.ts';
import { listPdaTaskFlowTasks, } from './pda-cutting-execution-source.ts';
import { getWoolWorkOrderById, getWoolWorkOrderByTaskId, listWoolMobileProcessTasks, listWoolWorkOrders, } from './wool-task-domain.ts';
import { listPdaGenericTasksByProcess } from './pda-task-mock-factory.ts';
import { getPostFinishingWorkOrderById, getPostFinishingTaskById, listPostFinishingTasks, } from './post-finishing-domain.ts';
import { getPrintWorkOrderById, listPrintWorkOrders, } from './printing-task-domain.ts';
...
function mapPostFinishingStatusToTaskStatus(status) {
    ...
    if (status.includes('中'))
        return 'IN_PROGRESS';
    if (status.includes('已交出') || status.includes('已回写') || status.includes('已完成'))
        return 'DONE';
    if (status === '待后道' || status === '待复检' || status === '待交出')
        return 'IN_PROGRESS';
    return 'NOT_STARTED';
}
function mapPostFinishingTaskToTask(task, seq) {
...
export function listPostFinishingMobileExecutionTasks() {
    return listPostFinishingTasks().map((task, index) => mapPostFinishingTaskToTask(task, index + 1));
}
function mapSpecialCraftExecutionStatus(status) {
    if (status === 'PROCESSING')
        return 'IN_PROGRESS';
    if (status === 'DIFFERENCE' || status === 'OBJECTION' || status === 'ABNORMAL')
        return 'BLOCKED';
    if (status === 'COMPLETED' || status === 'WAIT_HANDOVER' || status === 'HANDED_OVER' || status === 'WRITTEN_BACK')
        return 'DONE';
    return 'NOT_STARTED';
}
function mapSpecialCraftTaskOrderToMobileTask(taskOrder, seq) {
...
function isSpecialCraftTask(task) {
    return getMobileTaskProcessType(task) === 'SPECIAL_CRAFT';
}
export function listPdaMobileExecutionTasks() {
    listPrintWorkOrders();
    listDyeWorkOrders();
...
export function getPdaMobileExecutionTaskById(taskId) {
    return listPdaMobileExecutionTasks().find((task) => task.taskId === taskId) ?? null;
}
export function getMobileTaskProcessType(task) {
    if (!task)
        return 'UNKNOWN';
    ...
    const craftFields = [
        ...
        .filter(Boolean)
        .join(' ');
    if (/PROC_PRINT|PRINT\b|印花|转印/.test(explicitFields))
        return 'PRINT';
    if (/PROC_DYE|DYE\b|染色/.test(explicitFields))
        return 'DYE';
    if (/PROC_CUT|CUTTING|裁片|定位裁/.test(explicitFields))
        return 'CUTTING';
    if (/PROC_WOOL|WOOL|毛织|毛织/.test(explicitFields))
        return 'WOOL';
    if (/POST_FINISH|后道/.test(explicitFields))
        return 'POST_FINISHING';
    if (/SEW|车缝/.test(explicitFields))
        return 'SEWING';
    if (/SPECIAL|特殊工艺|绣花|打揽|打条|激光切|烫画|直喷|捆条/.test(craftFields))
        return 'SPECIAL_CRAFT';
    return 'UNKNOWN';
}
export function getMobileTaskFactoryId(task) {
    return String(task?.assignedFactoryId || '');
}
export function getMobileTaskAcceptanceState(task) {
    if (!task)
        return '未知';
    const origin = getTaskOrigin(task);
    if (origin === 'AWARDED_PENDING')
        return '待接单';
    if (origin.endsWith('REJECTED'))
        return '已拒单';
...
export function getMobileTaskBiddingState(task) {
    if (!task)
        return '未知';
    const origin = getTaskOrigin(task);
    if (origin === 'BIDDING_PENDING')
        return '待报价';
    if (origin === 'BIDDING_QUOTED')
        return '待定标';
    if (origin.startsWith('AWARDED'))
        return '已中标';
...
export function getMobileTaskExecutionState(task) {
    if (!task)
        return '未知';
    const origin = getTaskOrigin(task);
    if (task.status === 'CANCELLED' || origin.endsWith('CANCELLED'))
        return '已关闭';
    if (task.status === 'DONE')
        return '已完工';
    if (task.status === 'BLOCKED' || origin.endsWith('BLOCKED'))
        return '生产暂停';
...
export function isTaskInBiddingOrAwarding(task) {
    const biddingState = getMobileTaskBiddingState(task);
    return biddingState === '待报价' || biddingState === '已报价' || biddingState === '待定标';
}
function isTaskRejected(task) {
    return getMobileTaskAcceptanceState(task) === '已拒单' || getTaskOrigin(task).endsWith('REJECTED');
}
function isTaskClosed(task) {
    return getMobileTaskExecutionState(task) === '已关闭';
}
export function isTaskExecutable(task) {
    if (!task || !isTaskAccepted(task) || isTaskRejected(task) || isTaskInBiddingOrAwarding(task) || isTaskClosed(task))
        return false;
    return EXECUTABLE_STATES.has(getMobileTaskExecutionState(task));
}
export function isTaskVisibleInMobileExecutionList(task, currentFactoryId = TEST_FACTORY_ID) {
    if (!task)
        return false;
    ...
    if (!isTaskAccepted(task))
        return false;
    if (isTaskRejected(task) || isTaskInBiddingOrAwarding(task) || isTaskClosed(task))
        return false;
    return OPENABLE_STATES.has(getMobileTaskExecutionState(task));
}
function isSpecialCraftOperationMatched(task, expectedOperationName) {
    if (!expectedOperationName)
        return getMobileTaskProcessType(task) === 'SPECIAL_CRAFT';
    const haystack = [
        task.processNameZh,
        task.processBusinessName,
...
export function getMobileTaskAccessResult(task, currentFactoryId = TEST_FACTORY_ID) {
    if (!task) {
...
function getPrintTask(order) {
    return order.taskId ? getPdaMobileExecutionTaskById(order.taskId) : null;
}
function getDyeTask(order) {
    return order.taskId ? getPdaMobileExecutionTaskById(order.taskId) : null;
}
function getWoolTask(order) {
    return getPdaMobileExecutionTaskById(order.taskNo);
}
function selectBestCuttingTask(orderId) {
...
function getSpecialCraftTaskOrderByTaskOrderId(taskOrderId) {
    return getSpecialCraftTaskOrderById(taskOrderId) ?? null;
}
export function validatePrintWorkOrderMobileTaskBinding(printOrderId) {
...
export function validateDyeWorkOrderMobileTaskBinding(dyeOrderId) {
...
export function validateWoolWorkOrderMobileTaskBinding(woolOrderId) {
...
export function validateCuttingOrderMobileTaskBinding(cuttingOrderId) {
    const snapshot = buildFcsCuttingDomainSnapshot();
    const order = snapshot.cutOrders.find((item) => item.cutOrderId === cuttingOrderId || item.cutOrderNo === cuttingOrderId);
...
export function validateSpecialCraftTaskOrderMobileTaskBinding(taskOrderId) {
...
export function validateSpecialCraftMobileTaskBinding(workOrderId) {
...
export function validatePostFinishingMobileTaskBinding(postOrderId) {
...
export function validateProcessMobileTaskBinding(params) {
    if (params.processType === 'PRINT')
        return validatePrintWorkOrderMobileTaskBinding(params.sourceId);
    if (params.processType === 'DYE')
        return validateDyeWorkOrderMobileTaskBinding(params.sourceId);
    if (params.processType === 'WOOL')
        return validateWoolWorkOrderMobileTaskBinding(params.sourceId);
    if (params.processType === 'CUTTING')
        return validateCuttingOrderMobileTaskBinding(params.sourceId);
    if (params.processType === 'SPECIAL_CRAFT')
        return validateSpecialCraftMobileTaskBinding(params.sourceId);
    if (params.processType === 'POST_FINISHING')
        return validatePostFinishingMobileTaskBinding(params.sourceId);
    return validateBinding({
...
function listKnownCuttingOrderIds() {
    const ids = new Set();
    const snapshot = buildFcsCuttingDomainSnapshot();
    const cutOrders = Array.isArray(snapshot.cutOrders)
        ? snapshot.cutOrders
        : Object.values(snapshot.cutOrdersById || {});
    cutOrders.forEach((order) => ids.add(order.cutOrderId));
    return [...ids];
}
export function listInvalidProcessMobileTaskBindings(filter = {}) {
    const results = [];
...
export function assertValidProcessMobileTaskBinding(params) {
