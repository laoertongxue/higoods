...
function resolveInspectionScene(batch) {
    if (batch.processType === 'SEW')
        return 'SEW_RETURN_RECEIVING_QC';
    if (batch.processType === 'PRINT')
        return 'PRINT_RECEIVING_QC';
    if (batch.processType === 'DYE' || batch.processType === 'DYE_PRINT')
        return 'DYE_RECEIVING_QC';
    if (batch.processType === 'CUT_PANEL')
        return 'CUT_PIECE_RECEIVING_QC';
    return 'SEW_RETURN_RECEIVING_QC';
}
function resolveInspectionMethod(batch) {
    if (batch.processType === 'SEW')
        return 'SAMPLING';
    if (batch.processType === 'PRINT' || batch.processType === 'DYE' || batch.processType === 'DYE_PRINT')
        return 'SAMPLING';
    return 'FULL_INSPECTION';
}
let returnInboundGeneratedSeq = 1;
function ensureUniqueId(prefix, hasId) {
    let attempts = 0;
...
export function createReturnInboundBatchRecord(input) {
...
export function updateReturnInboundBatchStatus(input) {
    const index = input.batches.findIndex((item) => item.batchId === input.batchId);
    if (index < 0)
        return null;
    const ts = input.now ?? nowTimestamp();
...
export function isReturnInboundInspection(qc) {
    return (qc.inspectionScene === 'RETURN_INBOUND' ||
...
export function isSewReturnInboundQc(qc, batches = []) {
...
export function requiresFinalLiabilityDecision(qc, batches = []) {
    return isSewReturnInboundQc(qc, batches);
}
export function findReturnInboundBatchForQc(qc, batches) {
...
export function resolveReturnInboundTaskId(qc, batch) {
    return batch?.sourceTaskId || qc.refTaskId || (qc.refType === 'TASK' ? qc.refId : undefined);
}
export function createQcFromReturnInboundBatch(input) {
    ...
    const defects = input.defectItems && input.defectItems.length > 0
        ? input.defectItems
        : result === 'FAIL'
            ? [
                {
    ...
    const qc = {
        ...
        sourceOrderId: (input.sourceBusinessType ?? input.batch.sourceType) === 'DYE_PRINT_ORDER'
...
export function upsertDeductionBasisFromReturnInboundQc(input) {
    if (input.qc.result !== 'FAIL')
        return null;
    const ts = input.now ?? nowTimestamp();
    ...
    const decisionStage = isSewReturnInboundQc(input.qc, [input.batch])
        ? 'SEW_RETURN_INBOUND_FINAL'
        : 'GENERAL';
    const deductionDecision = input.qc.deductionDecision ?? (input.qc.disposition === 'ACCEPT' ? 'NO_DEDUCT' : 'DEDUCT');
    ...
    const existing = input.basisItems.find((item) => item.sourceBatchId === input.batch.batchId ||
    ...
    if (existing) {
        const updated = {
            ...
            liabilityStatusSnapshot: input.qc.liabilityStatus === 'DRAFT' ? 'PENDING' : input.qc.liabilityStatus,
    ...
    const basis = {
        ...
        summary,
        evidenceRefs: [],
        status: input.qc.liabilityStatus ?? 'DRAFT',
        deepLinks: {
            qcHref: `/fcs/quality/qc-records/${input.qc.qcId}`,
            taskHref: taskId ? `/fcs/pda/task-receive/${taskId}` : undefined,
        },
...
export function blockTaskForReturnInboundQc(input) {
    const ts = input.now ?? nowTimestamp();
    if (input.task.status === 'BLOCKED' && input.task.blockReason === 'QUALITY')
        return;
...
export function applyReturnInboundPassWriteback(input) {
    ...
    const refType = input.batch.sourceType === 'DYE_PRINT_ORDER' ? 'DYE_PRINT_ORDER' : 'RETURN_BATCH';
    const event = {
