import { getFactoryByCode } from './indonesia-factories.ts';
import { listFormalQualityDeductionLedgers, traceQualityDeductionLedgerSource, } from './quality-deduction-repository.ts';
import { getSettlementEffectiveInfoByFactory, getSettlementEffectiveInfoByFactoryAt, getSettlementVersionHistory, } from './settlement-change-requests.ts';
import { deriveSettlementCycleFields } from './store-domain-statement-grain.ts';
function buildDeductionEntryHrefByBasisId(basisId) {
...
function addDays(dateText, days, hour = 10) {
...
function createFactorySnapshot(factory) {
    return {
        id: factory.id,
        code: factory.code,
        name: factory.name,
...
function createAuditLog(id, action, detail, at, by) {
    return { id, action, detail, at, by };
}
function buildDemandSnapshot(factoryIndex, orderIndex, totalQty, requiredDeliveryDate) {
    return {
...
function buildAssignmentProgress() {
    return {
        status: 'DONE',
...
function buildTaskBreakdownSummary(createdAt) {
    return {
        isBrokenDown: true,
...
function chooseStatementStatus(cycleIndex) {
    if (cycleIndex <= 2)
...
function chooseBatchStatus(factoryIndex) {
    const statusOrder = [
        'PREPAID',
...
function chooseFactoryFeedbackStatus(statementStatus, factoryIndex, cycleIndex) {
    if (statementStatus === 'DRAFT')
        return 'NOT_SENT';
    const mod = (factoryIndex + cycleIndex) % 4;
    if (statementStatus === 'PENDING_FACTORY_CONFIRM') {
        if (mod === 0)
            return 'FACTORY_APPEALED';
        if (mod === 1)
            return 'PLATFORM_HANDLING';
        return 'PENDING_FACTORY_CONFIRM';
    }
    if (mod === 0)
        return 'RESOLVED';
...
function mapFactoryByCode(code) {
...
function getFactoryEffectiveInfo(factory) {
...
function chooseSettlementVersionRecord(factory, referenceAt) {
...
function cloneDeductionRuleSnapshots(snapshots) {
    return snapshots.map((item) => ({ ...item }));
}
function buildSettlementSnapshotForFactoryAt(factory, referenceAt) {
...
function createProductionOrders(factories) {
    const orders = [];
...
function createProcessTasks(orders) {
    const taskContexts = [];
    let taskSeq = 1;
    const linkedTaskStandardTimePerUnitBySeq = [0.96, 1.08, 1.18, 1.32, 0.88];
...
function buildBatchQtyPlan(totalQty, batchCount) {
...
function createReturnInboundBatches(taskContexts) {
    const batches = [];
    let batchSeq = 1;
        ...
        for (let batchIndex = 0; batchIndex < batchCount; batchIndex += 1) {
            ...
            batches.push({
                batch: {
                    ...
                    sewPostProcessMode: batchIndex % 2 === 0 ? 'SEW_FACTORY_INCLUDES_POST' : 'MANAGED_POST_FACTORY_EXECUTES',
                    postExecutionMode: batchIndex % 2 === 0 ? 'SEW_FACTORY_INCLUDES_POST' : 'MANAGED_POST_FACTORY_EXECUTES',
...
function buildEvidenceRefs(batchId, count) {
    return Array.from({ length: count }).map((_, index) => ({
        name: `${batchId} 检查证据-${index + 1}`,
        type: index === count - 1 ? '文档' : '图片',
    }));
}
function createQcScenarios(batchContexts) {
    const scenarios = [];
    let qcSeq = 1;
    let basisSeq = 1;
    let disputeSeq = 1;
    for (const batchContext of batchContexts) {
        ...
        const failBase = {
            ...
            deductionDecisionRemark: selector === 3
                ? '争议处理中，暂不计入本周期'
                : selector === 5
                    ? '非工厂责任，不进入扣款'
                    : '按工厂责任进入结算',
        ...
        const basisStatus = selector === 3 ? 'DISPUTED' : 'CONFIRMED';
        const basisSummary = selector === 3
            ? '工厂已发起异议，当前先冻结结算'
            : isHistoricalSettled
                ? '历史周期已确认并进入结算'
                : '当前周期已确认，可纳入结算';
...
function buildCarryOverAdjustments(qcScenarios, cycleOrder) {
    const adjustments = [];
    let adjustmentSeq = 1;
    for (const [index, scenario] of qcScenarios.entries()) {
        if (!scenario.basis || !scenario.dispute)
            continue;
        ...
        if (cycleIndex < 0 || cycleIndex >= cycleOrder.length - 1)
            continue;
...
function buildOtherAdjustments(factories, cycleOrder) {
    const adjustments = [];
...
function buildStatementLineFromBatch(batchContext, lineIndex) {
...
function buildStatementLineFromQualityLedger(ledger) {
    ...
    const cycleFields = deriveSettlementCycleFields(settlementPartyId, trace?.caseFact.qcRecord.inboundAt ?? ledger.generatedAt);
    ...
    return {
        item: {
            ...
            remark: trace?.disputeCase?.adjudicationResult
...
function buildStatementAppeal(statementId, cycleIndex, factoryIndex) {
...
function createStatementSourceRows(taskEarningLedgers, qualityLedgers) {
    const taskRows = taskEarningLedgers.map((ledger) => ({
        sourceItemId: ledger.ledgerId,
        sourceType: 'TASK_EARNING',
    ...
    const qualityRows = qualityLedgers.map((ledger) => {
        const trace = traceQualityDeductionLedgerSource(ledger.ledgerId);
        const settlementPartyId = ledger.settlementPartyId ?? ledger.factoryId;
        const cycle = deriveSettlementCycleFields(settlementPartyId, trace?.caseFact.qcRecord.inboundAt ?? ledger.generatedAt);
        return {
            sourceItemId: ledger.ledgerId,
...
function createStatementDrafts(factories, batchContexts, qualityLedgers) {
    const linesByFactoryCycle = new Map();
    ...
    for (const [statementSeq, key] of keys.entries()) {
        ...
        const appealRecord = factoryFeedbackStatus === 'FACTORY_APPEALED' || factoryFeedbackStatus === 'PLATFORM_HANDLING' || factoryFeedbackStatus === 'RESOLVED'
            ? {
                ...buildStatementAppeal(statementId, cycleIndex === -1 ? 0 : cycleIndex, factoryIndex),
                factoryId: factory.id,
                settlementCycleId,
                status: factoryFeedbackStatus === 'PLATFORM_HANDLING'
                    ? 'PLATFORM_HANDLING'
        ...
        const lineItems = lines
            .filter((line) => {
            if (line.item.sourceItemType !== 'QUALITY_DEDUCTION')
                return true;
            ...
            return (left.returnInboundBatchNo ?? left.sourceRefLabel ?? '').localeCompare(right.returnInboundBatchNo ?? right.sourceRefLabel ?? '');
        });
        const keptLines = lines.filter((line) => lineItems.some((item) => item.sourceItemId === line.item.sourceItemId));
        ...
        const earningLedgerIds = Array.from(new Set(lineItems.filter((item) => item.sourceItemType === 'TASK_EARNING').map((item) => item.sourceItemId)));
        const deductionLedgerIds = Array.from(new Set(lineItems.filter((item) => item.sourceItemType === 'QUALITY_DEDUCTION').map((item) => item.sourceItemId)));
        ...
        statements.push({
            ...
            factoryFeedbackAt: factoryFeedbackStatus === 'NOT_SENT'
                ? undefined
                : addDays(createdAt, 1, 11),
            factoryFeedbackBy: factoryFeedbackStatus === 'FACTORY_CONFIRMED' || factoryFeedbackStatus === 'FACTORY_APPEALED'
                ? `工厂财务-${factoryIndex + 1}`
                : factoryFeedbackStatus === 'RESOLVED'
                    ? '平台运营'
                    : '平台运营',
            factoryFeedbackRemark: factoryFeedbackStatus === 'FACTORY_CONFIRMED'
                ? '工厂已确认本期正式流水口径'
            ...
            readyForPrepaymentAt: status === 'READY_FOR_PREPAYMENT' && factoryFeedbackStatus !== 'FACTORY_APPEALED' && factoryFeedbackStatus !== 'PLATFORM_HANDLING'
...
function mapBatchToApprovalStatus(status) {
    if (status === 'FEISHU_APPROVAL_CREATED')
        return 'APPROVING';
    if (status === 'FEISHU_PAID_PENDING_WRITEBACK' || status === 'PREPAID' || status === 'CLOSED')
        return 'PAID';
    if (status === 'FEISHU_APPROVAL_REJECTED')
        return 'REJECTED';
    if (status === 'FEISHU_APPROVAL_CANCELED')
        return 'CANCELED';
    return null;
}
function createPrepaymentChain(statements) {
    const readyByFactory = new Map();
    ...
    if (factoryIds[1])
        statusPlans.push({ factoryId: factoryIds[1], status: 'FEISHU_PAID_PENDING_WRITEBACK', statementCount: 2 });
    if (factoryIds[2])
        statusPlans.push({ factoryId: factoryIds[2], status: 'FEISHU_APPROVAL_CREATED', statementCount: 2 });
    if (factoryIds[3])
        statusPlans.push({ factoryId: factoryIds[3], status: 'READY_TO_APPLY_PAYMENT', statementCount: 2 });
    if (factoryIds[4])
        statusPlans.push({ factoryId: factoryIds[4], status: 'FEISHU_APPROVAL_REJECTED', statementCount: 1 });
    ...
    for (const [index, plan] of statusPlans.entries()) {
        ...
        const first = pool[0];
        const items = pool.map((statement) => ({
        ...
        const snapshotRefs = Array.from(new Map(items
            .filter((item) => item.settlementProfileSnapshot)
            .map((item) => [item.settlementProfileSnapshot.versionNo, item.settlementProfileSnapshot])).values());
        ...
        const appliedForPaymentAt = plan.status === 'READY_TO_APPLY_PAYMENT' || plan.status === 'DRAFT'
            ? undefined
            : addDays(createdAt, 1, 10);
        let approvalId;
        let approvalNo;
        let paymentWritebackId;
        let prepaidAt;
        let closedAt;
        let paymentAmount;
        let paymentAt;
        ...
        let paymentUpdatedBy;
        let paymentSyncStatus;
        if (approvalStatus) {
            ...
            const approval = {
                ...
                createdBy: '财务共享',
                latestSyncedAt: plan.status === 'FEISHU_APPROVAL_CREATED'
                    ? addDays(createdAt, 2, 11)
                    : approvalStatus === 'PAID'
                        ? addDays(createdAt, 3, 16)
                        : addDays(createdAt, 2, 14),
                approvedAt: approvalStatus === 'PAID' || approvalStatus === 'APPROVED_PENDING_PAYMENT'
...
function createTaskEarningLedgers(batchContexts) {
    return batchContexts.map((batchContext, index) => {
        const settlementInfo = getSettlementEffectiveInfoByFactoryAt(batchContext.taskContext.factory.code, batchContext.batch.inboundAt) ?? getSettlementEffectiveInfoByFactory(batchContext.taskContext.factory.code);
        ...
        return {
            ...
            status: 'OPEN',
            sourceReason: batchContext.taskContext.pricingSourceType === 'BIDDING'
                ? '竞价中标价 × 回货数量'
                : '派单价 × 回货数量',
            remark: `回货批次 ${batchContext.batch.batchId} 形成任务收入流水`,
        };
...
function syncStatementsWithPrepaymentBatches(statements, batches, approvals, writebacks) {
...
function reserveStatementBuildScope(statements) {
...
function buildCycleOrder(factories) {
    const keys = new Set();
...
function buildSettlementLinkedMockFactory() {
    ...
    const statementDrafts = reserveStatementBuildScope(createStatementDrafts(factories, returnInboundBatchContexts, qualityLedgers));
    const { batches: settlementBatches, approvals: feishuPaymentApprovals, writebacks: paymentWritebacks } = createPrepaymentChain(statementDrafts);
    const syncedStatementDrafts = syncStatementsWithPrepaymentBatches(statementDrafts, settlementBatches, feishuPaymentApprovals, paymentWritebacks);
