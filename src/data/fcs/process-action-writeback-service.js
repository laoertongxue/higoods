import { PRINT_WORK_ORDER_STATUS_LABEL, completeColorTest, completePrinting, completeTransfer, getPrintWorkOrderById, startColorTest, startPrinting, startTransfer, submitPrintHandover, } from './printing-task-domain.ts';
import { DYE_WORK_ORDER_STATUS_LABEL, completeDyeMaterialReady, completeDyeMaterialWait, completeDyeNode, completeDyeSampleTest, completeDyeSampleWait, completeDyeing, getDyeWorkOrderById, planDyeVat, startDyeMaterialReady, startDyeSampleTest, startDyeing, submitDyeHandover, } from './dyeing-task-domain.ts';
...
import { getSpecialCraftTaskWorkOrderById, updateSpecialCraftTaskWorkOrderWebStatus, } from './special-craft-task-orders.ts';
import { applyPostFinishingActionFinish, applyPostFinishingActionStart, getPostFinishingWorkOrderById, } from './post-finishing-domain.ts';
...
import { createProcessHandoverDifferenceRecord, createProcessHandoverRecord, listWaitProcessWarehouseRecords, } from './process-warehouse-domain.ts';
...
import { getProcessObjectType, getProcessQtyUnit, getQuantityLabel, } from './process-quantity-labels.ts';
...
const processActionOperationRecords = [];
...
export const PROCESS_ACTION_DEFINITIONS = [
    {
...
function normalizeActionCode(actionCode) {
    return ACTION_CODE_ALIASES[actionCode] || actionCode;
}
export function getCanonicalProcessActionCode(actionCode) {
    return normalizeActionCode(actionCode);
}
export function listProcessActionDefinitions(sourceType) {
    return sourceType ? PROCESS_ACTION_DEFINITIONS.filter((item) => item.sourceType === sourceType) : [...PROCESS_ACTION_DEFINITIONS];
}
export function getProcessActionDefinition(sourceType, actionCode) {
...
export function getProcessActionStatusSnapshot(sourceType, sourceId) {
    ...
    if (sourceType === 'CUTTING') {
        ...
        const record = cutPieceOrderRecords.find((item) => item.cutOrderId === sourceId || item.cutOrderNo === sourceId || item.id === sourceId);
...
function validateBinding(sourceType, sourceId) {
    const binding = sourceType === 'PRINT'
...
function toPlatformStatus(sourceType, sourceId, nextStatus) {
...
function getFormFieldValue(fields, aliases) {
    if (!fields)
        return '';
...
function assertPositiveFormNumber(fields, aliases, label) {
    const value = getFormFieldValue(fields, aliases);
    if (!value || !Number.isFinite(Number(value)) || Number(value) <= 0)
        throw new Error(`请填写${label}`);
}
function assertNonEmptyFormValue(fields, aliases, label) {
    if (!getFormFieldValue(fields, aliases))
        throw new Error(`请填写${label}`);
}
function assertPrintTransferPickupReady(printOrderId) {
    ...
    if (!hasPickup)
        throw new Error('开始转印前必须先在交接模块完成本单领料记录');
}
function assertActionSpecificFields(payload, definition) {
...
function resolveSpecialCraftObjectMeta(targetObject) {
    const normalized = String(targetObject || '');
    if (normalized.includes('成衣'))
        return { objectType: '成衣', qtyUnit: '件' };
    if (normalized.includes('面料'))
        return { objectType: '面料', qtyUnit: '米' };
    return { objectType: '裁片', qtyUnit: '片' };
}
function assertRequiredFields(payload, definition, qty) {
    const fields = payload.formData || {};
    for (const field of definition.requiredFields) {
        ...
        if (field.includes('数量') || field.includes('米数') || field.includes('卷数') || field.includes('长度')) {
            if (!Number.isFinite(qty) || qty <= 0)
                throw new Error('请填写带对象和单位的操作数量');
            continue;
        }
        if (field === '单位') {
            if (!payload.qtyUnit?.trim())
...
function getActionObjectType(sourceType, payload) {
...
function getActionQtyUnit(sourceType, payload, snapshot) {
...
export function executePrintAction(payload) {
    ...
    if (actionCode === 'PRINT_PATTERN_READY') {
        ...
        const usedMaterialQty = Number(fields['实际使用原料面料米数']
            || fields['实际使用原料裁片数量']
            || fields['实际使用原料']
...
export function executeDyeAction(payload) {
    const operatorName = payload.operatorName || (payload.sourceChannel === '移动端' ? '移动端操作员' : 'Web 端操作员');
...
export function executeSpecialCraftAction(payload) {
    ...
    const nextStatus = definition.toStatus;
    const objectMeta = resolveSpecialCraftObjectMeta(workOrder.targetObject);
...
function getPostFinishingActionType(actionCode, currentStatus) {
    if (actionCode.includes('RECEIVE'))
        return '扫码收货';
    if (actionCode.includes('QC'))
        return '质检';
    if (actionCode.includes('PROCESS'))
        return '后道';
    if (actionCode.includes('RECHECK'))
        return '复检';
    if (currentStatus?.includes('质检'))
        return '质检';
    if (currentStatus?.includes('后道'))
        return '后道';
    if (currentStatus?.includes('复检') || currentStatus === '待交出')
        return '复检';
    return '扫码收货';
}
function getPostFinishingRejectedQty(payload, actionCode) {
    const fields = payload.formData || {};
    if (actionCode === 'POST_QC_FINISH')
        return Number(fields['不合格数量'] || fields['质检不合格成衣件数'] || 0);
    if (actionCode === 'POST_RECHECK_FINISH')
        return Number(fields['复检不合格成衣件数'] || 0);
    return 0;
}
export function executePostFinishingAction(payload) {
    const definition = getProcessActionDefinition('POST_FINISHING', payload.actionCode);
    if (!definition)
        throw new Error('后道动作未注册');
    ...
    if (actionCode.endsWith('_START')) {
        ...
        return { updatedWorkOrderId: updated.postOrderId, nextStatus: updated.currentStatus };
    }
    if (actionCode === 'POST_REPORT_DIFFERENCE') {
        ...
        return { updatedWorkOrderId: updated.postOrderId, nextStatus: '有差异' };
    }
...
function buildOperationRemark(payload, definition) {
    const fields = payload.formData || {};
    ...
    if (rollCount)
        parts.push(`卷数：${rollCount}`);
    if (rollLength)
        parts.push(`每卷长度：${rollLength}`);
    return parts.filter(Boolean).join('；');
}
export function createProcessActionOperationRecord(payload, definition, previousStatus = '', nextStatus = '', result = {}) {
    ...
    const quantityContext = {
        ...
        qtyPurpose: normalizeActionCode(payload.actionCode).includes('HANDOVER') ? '已交出' : '已完成',
    };
    const record = {
...
export function executeProcessAction(payload) {
    ...
    const partial = hydratedPayload.sourceType === 'PRINT'
    ...
    const linkedPartial = {
        ...partial,
        affectedWarehouseRecordId: linkage.updatedWaitHandoverWarehouseRecordId ||
...
export function executeMobileProcessAction(payload) {
    return executeProcessAction({ ...payload, sourceChannel: '移动端' });
}
export function listProcessActionOperationRecords(filter = {}) {
    return processActionOperationRecords.filter((record) => {
        if (filter.sourceChannel && record.sourceChannel !== filter.sourceChannel)
            return false;
...
export function getProcessActionOperationRecordsBySource(sourceType, sourceId) {
    return listProcessActionOperationRecords({ sourceType, sourceId });
}
export function getProcessActionOperationRecordsByTask(taskId) {
