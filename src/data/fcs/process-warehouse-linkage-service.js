...
import { applySpecialCraftDifferenceToFeiTickets, createProcessHandoverDifferenceRecord, createProcessHandoverRecord, createWaitHandoverWarehouseRecord, createWaitProcessWarehouseRecord, getProcessHandoverDifferenceRecordById, getProcessHandoverRecordById, getProcessWarehouseRecordById, listProcessHandoverRecords, listWaitHandoverWarehouseRecords, } from './process-warehouse-domain.ts';
...
function emptyLinkageResult(actionResult, message = '当前动作不需要仓交出联动') {
    return {
        success: true,
...
function derivePlatformStatus(actionResult) {
...
function resolvePrintContext(actionResult) {
...
function resolveDyeContext(actionResult) {
...
function findCuttingOrder(sourceId) {
    return cutPieceOrderRecords.find((item) => item.cutOrderId === sourceId ||
        item.cutOrderNo === sourceId ||
        item.id === sourceId ||
        item.cutPieceOrderNo === sourceId);
}
function findGeneratedCutOrder(sourceId) {
    return buildFcsCuttingDomainSnapshot().cutOrders.find((item) => item.cutOrderId === sourceId || item.cutOrderNo === sourceId);
}
function resolveCuttingFeiTicketIds(cutOrderId) {
    if (!cutOrderId)
        return [];
    return listSpreadingResultGeneratedFeiTicketsByCutOrderId(cutOrderId).map((ticket) => ticket.feiTicketNo);
}
function resolveCuttingContextFromGeneratedOrder(actionResult, order) {
...
function resolveCuttingContext(actionResult) {
...
function resolveSpecialCraftContext(actionResult) {
    ...
    const objectType = targetObject.includes('成衣')
        ? '成衣'
...
function resolvePostFinishingContext(actionResult) {
...
function ensureWaitProcessWarehouseRecord(context, actionResult, currentActionName) {
...
function ensureWaitHandoverWarehouseRecord(context, actionResult) {
...
function ensureHandoverRecord(context, actionResult, warehouseRecordId) {
    ...
    if (existed)
        return existed;
    const fallbackWarehouse = warehouseRecordId ||
...
function ensureSpecialCraftDifference(context, actionResult) {
    ...
    const warehouse = getProcessWarehouseRecordById(handover?.warehouseRecordId || '') ||
...
export function applyPrintWarehouseLinkageAfterAction(actionResult) {
    const context = resolvePrintContext(actionResult);
    const base = emptyLinkageResult(actionResult);
    if (!context)
        return mergeResult(base, { success: false, message: '未找到印花加工单，不能执行仓联动' });
    if (!['PRINT_FINISH_TRANSFER', 'PRINT_SUBMIT_HANDOVER'].includes(actionResult.actionCode))
        return base;
...
export function applyDyeWarehouseLinkageAfterAction(actionResult) {
    const context = resolveDyeContext(actionResult);
    const base = emptyLinkageResult(actionResult);
    if (!context)
        return mergeResult(base, { success: false, message: '未找到染色加工单，不能执行仓联动' });
    if (!['DYE_FINISH_PACKING', 'DYE_SUBMIT_HANDOVER'].includes(actionResult.actionCode))
        return base;
    const waitHandover = ensureWaitHandoverWarehouseRecord(context, actionResult);
    let result = mergeResult(base, {
        createdWaitHandoverWarehouseRecordId: waitHandover.warehouseRecordId,
...
export function applyCuttingWarehouseLinkageAfterAction(actionResult) {
    const context = resolveCuttingContext(actionResult);
    const base = emptyLinkageResult(actionResult);
    if (!context)
        return mergeResult(base, { success: false, message: '未找到裁片单，不能执行仓联动' });
    ...
    if (!['CUTTING_CONFIRM_INBOUND', 'CUTTING_SUBMIT_HANDOVER'].includes(actionResult.actionCode))
        return base;
...
export function applySpecialCraftWarehouseLinkageAfterAction(actionResult) {
    const context = resolveSpecialCraftContext(actionResult);
    const base = emptyLinkageResult(actionResult);
    if (!context)
        return mergeResult(base, { success: false, message: '未找到特殊工艺单，不能执行仓联动' });
    ...
    if (!['SPECIAL_CRAFT_FINISH_PROCESS', 'SPECIAL_CRAFT_SUBMIT_HANDOVER'].includes(actionResult.actionCode))
        return base;
    const waitHandover = ensureWaitHandoverWarehouseRecord(context, actionResult);
    let result = mergeResult(base, {
        createdWaitHandoverWarehouseRecordId: waitHandover.warehouseRecordId,
...
export function applyPostFinishingWarehouseLinkageAfterAction(actionResult) {
    const context = resolvePostFinishingContext(actionResult);
    const base = emptyLinkageResult(actionResult);
    if (!context)
        return mergeResult(base, { success: false, message: '未找到后道单，不能执行仓联动' });
    if (['POST_RECEIVE_FINISH', 'POST_QC_FINISH', 'POST_PROCESS_FINISH'].includes(actionResult.actionCode)) {
        const actionName = actionResult.actionCode === 'POST_RECEIVE_FINISH'
            ? '后道待质检'
            : actionResult.actionCode === 'POST_QC_FINISH'
                ? (actionResult.nextStatus === '待复检' ? '后道待复检' : '后道待后道')
                : '后道待复检';
        const waitProcess = ensureWaitProcessWarehouseRecord({ ...context, targetWarehouseName: '后道待加工仓', warehouseLocation: '后道待加工仓-H01' }, actionResult, actionName);
        return mergeResult(base, {
            createdWaitProcessWarehouseRecordId: waitProcess.warehouseRecordId,
            updatedWaitProcessWarehouseRecordId: waitProcess.warehouseRecordId,
    ...
    if (actionResult.actionCode !== 'POST_RECHECK_FINISH')
        return base;
    const waitHandover = ensureWaitHandoverWarehouseRecord(context, actionResult);
    let result = mergeResult(base, {
        createdWaitHandoverWarehouseRecordId: waitHandover.warehouseRecordId,
...
export function applyWarehouseLinkageAfterAction(actionResult) {
    if (!actionResult.success)
        return emptyLinkageResult(actionResult, '动作未成功，不执行仓交出联动');
    if (actionResult.sourceType === 'PRINT')
        return applyPrintWarehouseLinkageAfterAction(actionResult);
    if (actionResult.sourceType === 'DYE')
        return applyDyeWarehouseLinkageAfterAction(actionResult);
    if (actionResult.sourceType === 'CUTTING')
        return applyCuttingWarehouseLinkageAfterAction(actionResult);
    if (actionResult.sourceType === 'SPECIAL_CRAFT')
