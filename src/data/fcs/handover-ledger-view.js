import { getPdaCompletedHeads, getPdaHandoutHeads, getPdaHandoverRecordsByHead, getPdaPickupHeads, getPdaPickupRecordsByHead, } from './pda-handover-events';
import { getRecordReceiverWrittenAt, getRecordReceiverWrittenQty, getReceiverDisplayName, } from './task-handover-domain';
import { processTasks } from './process-tasks';
import { listRuntimeProcessTasks } from './runtime-process-tasks';
function parseDateMs(value) {
    if (!value)
        return Number.NaN;
    return new Date(value.replace(' ', 'T')).getTime();
}
function formatQtyDiff(diff, unit) {
    if (diff === 0)
        return `差异 0 ${unit}`;
    const sign = diff > 0 ? '-' : '+';
    return `差异 ${sign}${Math.abs(diff)} ${unit}`;
}
function buildPickupHeadRow(head) {
...
function buildPickupRecordRow(head, record) {
...
function buildHandoutHeadRow(head) {
...
function buildHandoutRecordRow(head, record) {
    let eventTypeCode = 'HANDOUT_RECORD';
    let eventTypeLabel = '交出记录';
    let statusCode = 'HANDOUT_RECORD_PENDING_WRITEBACK';
    let statusLabel = '待收货确认';
    let statusGroup = 'PENDING';
    let statusTone = 'warning';
    let nextActionHint = '等待接收方确认实收对象数量';
    if (record.handoverRecordStatus === 'WRITTEN_BACK_MATCHED'
        || record.handoverRecordStatus === 'WRITTEN_BACK_DIFF'
        || record.handoverRecordStatus === 'DIFF_ACCEPTED') {
        eventTypeCode = 'WAREHOUSE_CONFIRMED';
        eventTypeLabel = '接收方确认';
        statusCode =
            record.handoverRecordStatus === 'WRITTEN_BACK_DIFF'
                ? 'HANDOUT_RECORD_DIFF'
                : record.handoverRecordStatus === 'DIFF_ACCEPTED'
        ...
        nextActionHint =
            record.handoverRecordStatus === 'WRITTEN_BACK_DIFF' ? '等待工厂确认差异' : '接收方收货确认已完成';
    }
    ...
    const writtenText = typeof writtenQty === 'number' ? `${writtenQty} ${head.qtyUnit}` : '待收货确认';
    return {
...
function buildCompletedHeadRow(head) {
    return {
...
function buildSameFactoryContinueRow(task) {
    return {
        ...
        nextActionHint: task.status === 'DONE'
            ? '连续工序已完成，按后续工序流转'
            : '同厂同 SKU 连续工序，中间无需回仓再领料',
        handoverId: `RTC-${task.taskId}`,
    };
}
function buildWarehouseWorkshopRow(task) {
    return {
...
export function getHandoverLedgerRows() {
    const rows = [];
    ...
    const runtimeRows = listRuntimeProcessTasks()
        ...
        .map((task) => task.executorKind === 'WAREHOUSE_WORKSHOP'
...
export function getHandoverPreviewStats(rows) {
    return {
        ...
        pendingWarehouseConfirm: rows.filter((row) => row.sourceType === 'HANDOUT_RECORD' && row.statusCode === 'HANDOUT_RECORD_PENDING_WRITEBACK').length,
        pendingObjections: rows.filter((row) => row.sourceType === 'HANDOUT_RECORD' &&
            (row.statusCode === 'HANDOUT_OBJECTION_REPORTED' || row.statusCode === 'HANDOUT_OBJECTION_PROCESSING')).length,
    };
}
function getLatestOccurredAt(rows) {
    return rows
        .map((row) => row.occurredAt)
        .filter(Boolean)
        .sort((a, b) => parseDateMs(b) - parseDateMs(a))[0] || '';
}
function deriveProcessSectionStatus(events) {
    if (events.length === 0) {
        return {
            label: '暂无事件',
            tone: 'muted',
    ...
    if (statusCodes.has('HANDOUT_RECORD_PENDING_WRITEBACK') ||
    ...
    if (statusCodes.has('PICKUP_PENDING') ||
...
function getSectionPriority(label) {
    switch (label) {
        case '有异议':
            return 1;
        case '异议处理中':
...
function resolveFocusByStatusLabel(label) {
    if (label === '待领料')
        return 'pickup';
    if (label === '已领料待交出')
        return 'handout';
    ...
    if (label === '有异议' || label === '异议处理中')
        return 'objection';
    return undefined;
}
function buildOrderProcessSections(orderId, orderRows) {
...
function deriveOrderBottleneck(sections) {
    const sorted = [...sections].sort((a, b) => getSectionPriority(a.processStatusLabel) - getSectionPriority(b.processStatusLabel) ||
        a.seq - b.seq);
...
export function getHandoverOrderTimelineViews(rows) {
    ...
    return orderIds
        ...
        return {
            ...
            pendingCount: processSections.filter((section) => section.processStatusLabel !== '已完成' && section.processStatusLabel !== '暂无事件').length,
            objectionCount: processSections.filter((section) => section.processStatusLabel === '有异议' ||
                section.processStatusLabel === '异议处理中').length,
...
export function getHandoverOrderTimelineViewById(rows, productionOrderId) {
    return getHandoverOrderTimelineViews(rows).find((item) => item.productionOrderId === productionOrderId);
}
export function getProductionOrderHandoverSummary(productionOrderId, rows = getHandoverLedgerRows()) {
...
export function getTaskHandoverSummary(taskId, rows = getHandoverLedgerRows()) {
...
export function buildHandoverPageLink(options = {}) {
    const params = new URLSearchParams();
    if (options.tab)
        params.set('tab', options.tab);
    if (options.productionOrderId)
        params.set('po', options.productionOrderId);
    if (options.taskId)
...
export function buildHandoverOrderDetailLink(options) {
    const params = new URLSearchParams();
    if (options.tab)
        params.set('tab', options.tab);
    if (options.taskId)
