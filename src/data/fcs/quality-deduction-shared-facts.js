...
import { RETURN_INBOUND_QC_CHAIN_SCENARIOS, } from './return-inbound-quality-chain-facts.ts';
...
const MOCK_FX_RATE_BY_SETTLEMENT_CURRENCY = {
    CNY: 1,
    IDR: 2150,
    USD: 0.14,
};
const LEGACY_QC_RAW_CASES = [
    {
        qcId: 'QC-020',
...
const RAW_CASES = [...RETURN_INBOUND_QC_CHAIN_SCENARIOS, ...LEGACY_QC_RAW_CASES];
...
function resolveInspectionQtys(scenario) {
    const inspectedQty = normalizeQty(scenario.inspectedQty || scenario.returnedQty);
    const defectQty = normalizeQty(scenario.defectItems?.reduce((sum, item) => sum + item.qty, 0) ?? scenario.affectedQty);
    ...
    const qcResult = unqualifiedQty <= 0
        ? 'QUALIFIED'
        : qualifiedQty > 0
            ? 'PARTIALLY_QUALIFIED'
            : 'UNQUALIFIED';
...
function createEvidenceAssets(qcId, assets) {
    return assets.map((asset, index) => ({
        assetId: `${qcId}-ASSET-${String(index + 1).padStart(2, '0')}`,
        name: asset.name,
        assetType: asset.type === '视频'
            ? 'VIDEO'
            : asset.type === '图片'
                ? 'IMAGE'
                : 'DOCUMENT',
...
function resolveLiabilityStatus(scenario, override, unqualifiedQty) {
    const status = override.liabilityStatus ?? 'PENDING';
    const baseFactoryQty = override.factoryLiabilityQty ??
        (status === 'FACTORY' ? unqualifiedQty : status === 'MIXED' ? Math.max(Math.floor(unqualifiedQty * 0.6), 1) : 0);
    const baseNonFactoryQty = override.nonFactoryLiabilityQty ??
        (status === 'NON_FACTORY'
            ? unqualifiedQty
            : status === 'MIXED'
...
function resolveProcessLabel(scenario) {
...
function createQcRecordFact(scenario) {
    ...
    const writebackCompletedAt = Object.prototype.hasOwnProperty.call(override, 'writebackCompletedAt')
        ? override.writebackCompletedAt
        : scenario.writeback?.completedAt;
    const writebackCompletedBy = Object.prototype.hasOwnProperty.call(override, 'writebackCompletedBy')
        ? override.writebackCompletedBy
        : scenario.writeback?.completedBy;
    const evidenceAssets = createEvidenceAssets(scenario.qcId, [
    ...
    return {
        qcId: scenario.qcId,
        qcNo: scenario.qcId,
        routeAliases: Array.from(new Set([
            scenario.qcId,
            scenario.batchId,
            scenario.taskId,
...
function createFactoryResponseFact(scenario, qcRecord) {
    ...
    return {
        ...
        responseAction: status === 'CONFIRMED'
            ? 'CONFIRM'
        ...
        isOverdue: Boolean(override.responseDeadlineAt) &&
...
function createDeductionBasisFact(scenario, qcRecord) {
    if (!scenario.basis)
        return null;
    const override = CASE_OVERRIDES[scenario.qcId] ?? {};
    ...
    const blockedProcessingFeeAmount = resolveBlockedProcessingFeeAmount(scenario, override, blockedSettlementQty);
    const proposedQualityDeductionAmount = override.proposedQualityDeductionAmount ?? scenario.dispute?.requestedAmount ?? scenario.basis.deductionAmount ?? 0;
    const effectiveQualityDeductionAmount = override.effectiveQualityDeductionAmount ?? scenario.dispute?.finalAmount ?? scenario.basis.deductionAmount ?? 0;
    return {
        ...
        effectiveAt: status === 'EFFECTIVE' || status === 'ADJUSTED'
...
function createDisputeCaseFact(scenario) {
    if (!scenario.dispute)
        return null;
    const override = CASE_OVERRIDES[scenario.qcId] ?? {};
    const status = override.disputeStatus ?? 'IN_REVIEW';
    const resolved = status === 'UPHELD' || status === 'PARTIALLY_ADJUSTED' || status === 'REVERSED' || status === 'CLOSED';
    return {
        ...
        disputeEvidenceAssets: createEvidenceAssets(scenario.dispute.disputeId, scenario.basis?.evidenceRefs ?? [{ name: scenario.dispute.summary, type: '文档' }]),
        ...
        adjudicationResult: override.adjudicationResult ??
...
function createSettlementImpactFact(scenario, qcRecord, deductionBasis) {
    const override = CASE_OVERRIDES[scenario.qcId] ?? {};
    const blockedSettlementQty = override.blockedSettlementQty ?? qcRecord.factoryLiabilityQty;
    const blockedProcessingFeeAmount = resolveBlockedProcessingFeeAmount(scenario, override, blockedSettlementQty);
    const effectiveQualityDeductionAmount = override.effectiveQualityDeductionAmount ??
    ...
    return {
        ...
        blockedProcessingFeeAmount,
        effectiveQualityDeductionAmount,
        candidateSettlementCycleId: override.candidateSettlementCycleId ?? scenario.settlementImpact.settlementBatchId,
        includedAt: override.includedAt,
        includedSettlementStatementId: override.includedSettlementStatementId,
        includedSettlementBatchId: override.includedSettlementBatchId ??
        ...
        settledAt: Object.prototype.hasOwnProperty.call(override, 'settledAt')
            ? override.settledAt
...
function createSettlementAdjustmentFact(scenario) {
    void scenario;
    return null;
}
function resolveFactorySettlementCurrency(factoryId) {
    if (!factoryId)
        return 'CNY';
    return getSettlementEffectiveInfoByFactory(factoryId)?.settlementConfigSnapshot.currency ?? 'CNY';
}
function resolveMockFxRate(originalCurrency, settlementCurrency) {
    if (originalCurrency === settlementCurrency)
        return 1;
    return MOCK_FX_RATE_BY_SETTLEMENT_CURRENCY[settlementCurrency] ?? 1;
}
function resolvePendingRecordStatus(caseFact) {
...
function buildPendingQualityDeductionRecord(scenario, qcRecord, deductionBasis, factoryResponse, disputeCase) {
    if (!factoryResponse || qcRecord.factoryLiabilityQty <= 0 || !deductionBasis)
        return null;
    const settlementCurrency = resolveFactorySettlementCurrency(qcRecord.returnFactoryId);
    const originalAmount = disputeCase?.adjudicationResult === 'PARTIALLY_ADJUSTED'
    ...
    return {
        ...
        pendingReasonSummary: factoryResponse.factoryResponseStatus === 'PENDING_RESPONSE'
...
function resolveLedgerStatusFromCompat(caseFact) {
    const impact = caseFact.settlementImpact;
    if (impact.status === 'SETTLED')
        return 'PREPAID';
    if (impact.includedSettlementBatchId)
        return 'INCLUDED_IN_PREPAYMENT_BATCH';
    if (impact.includedSettlementStatementId)
        return 'INCLUDED_IN_STATEMENT';
    return 'GENERATED_PENDING_STATEMENT';
}
function resolveLedgerTrigger(pendingRecord, disputeCase) {
    if (disputeCase?.adjudicationResult === 'PARTIALLY_ADJUSTED')
        return 'ADJUDICATION_PARTIAL_LIABILITY';
    if (disputeCase?.adjudicationResult === 'UPHELD')
        return 'ADJUDICATION_FACTORY_LIABILITY';
    if (pendingRecord.status === 'SYSTEM_AUTO_CONFIRMED')
        return 'AUTO_CONFIRM';
    return 'FACTORY_CONFIRM';
}
function buildFormalQualityDeductionLedger(scenario, qcRecord, pendingRecord, disputeCase, settlementImpact) {
    if (!pendingRecord)
        return null;
    const adjudicatedWithLedger = disputeCase?.adjudicationResult === 'UPHELD' || disputeCase?.adjudicationResult === 'PARTIALLY_ADJUSTED';
    if (pendingRecord.status === 'PENDING_FACTORY_CONFIRM' ||
    ...
    if (disputeCase?.adjudicationResult === 'REVERSED')
        return null;
    const amount = disputeCase?.adjudicationResult === 'PARTIALLY_ADJUSTED'
    ...
    return {
        ...
        generatedAt: disputeCase?.adjudicatedAt ??
            pendingRecord.handledAt ??
            pendingRecord.generatedAt,
        generatedBy: disputeCase?.reviewerUserName ??
        ...
        comment: disputeCase?.adjustmentReasonSummary ??
            disputeCase?.adjudicationComment ??
            pendingRecord.handledComment,
    };
}
function createCaseFact(scenario) {
    ...
    const pendingDeductionRecord = buildPendingQualityDeductionRecord(scenario, qcRecord, deductionBasis, factoryResponse, disputeCase);
    const formalLedger = buildFormalQualityDeductionLedger(scenario, qcRecord, pendingDeductionRecord, disputeCase, settlementImpact);
    return {
        qcRecord,
        factoryResponse,
        deductionBasis,
...
export const qualityDeductionSharedCaseFacts = RAW_CASES.map(createCaseFact);
export const qualityDeductionSharedQcRecords = qualityDeductionSharedCaseFacts.map((item) => item.qcRecord);
export const qualityDeductionSharedFactoryResponses = qualityDeductionSharedCaseFacts
    .map((item) => item.factoryResponse)
    .filter((item) => item !== null);
export const qualityDeductionSharedDeductionBases = qualityDeductionSharedCaseFacts
    .map((item) => item.deductionBasis)
    .filter((item) => item !== null);
export const qualityDeductionSharedDisputeCases = qualityDeductionSharedCaseFacts
    .map((item) => item.disputeCase)
    .filter((item) => item !== null);
export const qualityDeductionSharedPendingDeductionRecords = qualityDeductionSharedCaseFacts
    .map((item) => item.pendingDeductionRecord)
    .filter((item) => item !== null);
export const qualityDeductionSharedFormalLedgers = qualityDeductionSharedCaseFacts
    .map((item) => item.formalLedger)
    .filter((item) => item !== null);
export const qualityDeductionSharedSettlementImpacts = qualityDeductionSharedCaseFacts.map((item) => item.settlementImpact);
export const qualityDeductionSharedSettlementAdjustments = qualityDeductionSharedCaseFacts
    .map((item) => item.settlementAdjustment)
    .filter((item) => item !== null);
export function getQualityDeductionCaseStatus(caseFact) {
    return deriveQualityDeductionCaseStatus(caseFact);
}
export function validateQualityDeductionSharedFacts(cases = qualityDeductionSharedCaseFacts) {
    const issues = [];
    for (const caseFact of cases) {
        ...
        if (deductionBasis) {
            ...
            if (deductionBasis.status === 'ADJUSTED' &&
        ...
        if (settlementImpact.totalFinancialImpactAmount !==
