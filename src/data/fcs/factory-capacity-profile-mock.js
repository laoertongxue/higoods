...
import { getCapacityProcessCraftOptions, getProcessDefinitionByCode, getProcessCraftDictRowByCode, listCraftsByProcessCode, listProcessStages, } from './process-craft-dict.ts';
import { getFactorySupplyFormulaTemplate } from './process-craft-sam-explainer.ts';
...
const POST_CAPACITY_NODE_MACHINE_TYPE = {
    BUTTONHOLE: '锁眼机',
    BUTTON_ATTACH: '装扣机',
    IRONING: '整烫工位',
    PACKAGING: '包装工位',
};
const POST_CAPACITY_NODE_REFERENCE_CRAFTS = {
    BUTTONHOLE: ['开扣眼'],
    BUTTON_ATTACH: ['机打扣', '四爪扣', '布包扣', '手缝扣'],
    IRONING: ['熨烫'],
    PACKAGING: ['包装'],
};
function isPostCapacityNodeProcess(processCode) {
    return POST_CAPACITY_NODE_CODES.includes(processCode);
}
function buildPostCapacityNodeRow(processCode) {
...
function resolveAbilitySupportedRows(ability) {
    if ((ability.status ?? 'ACTIVE') === 'DISABLED')
        return [];
...
function cloneEquipmentAbility(ability) {
    return {
...
function createEmptyProfile(factoryId) {
...
function cloneEquipments(equipments) {
    return equipments.map((item) => cloneEquipment(item));
}
function resolveFactorySupportedCraftRows(factory) {
    const stageWeight = new Map(listProcessStages().map((stage, index) => [stage.stageCode, index]));
...
function getBaseSeed(factoryId, craftCode) {
    return [...`${factoryId}-${craftCode}`].reduce((total, char) => total + char.charCodeAt(0), 0);
}
function buildDefaultFieldValue(key, row, factoryId) {
    ...
    switch (key) {
        ...
        case 'deviceEfficiencyValue':
            if (row.samCalcMode === 'BATCH')
                return Number((0.7 + (seed % 4) * 0.05).toFixed(2));
            if (row.samCalcMode === 'CONTINUOUS')
                return Number((0.72 + (seed % 6) * 0.03).toFixed(2));
        ...
        case 'staffEfficiencyValue':
            if (row.samCalcMode === 'BATCH')
                return Number((0.75 + (seed % 5) * 0.03).toFixed(2));
            if (row.samCalcMode === 'CONTINUOUS')
                return Number((0.78 + (seed % 4) * 0.03).toFixed(2));
...
function buildDefaultValues(fieldKeys, row, factoryId) {
    return fieldKeys.reduce((result, key) => {
        result[key] = buildDefaultFieldValue(key, row, factoryId);
        return result;
    }, {});
}
function getSeedNumericValue(values, key) {
    const rawValue = values[key];
    const numericValue = Number(rawValue);
    return Number.isFinite(numericValue) ? numericValue : 0;
}
function resolveEquipmentEfficiencyUnit(row) {
    if (row.processCode === 'PRINT')
        return '米/分钟';
    if (row.processCode === 'DYE')
        return '批/分钟';
    return '件/分钟';
}
function resolveEquipmentEfficiencyUnitByCodes(processCode, craftCode) {
    const row = getProcessCraftDictRowByCode(craftCode);
    if (row)
        return resolveEquipmentEfficiencyUnit(row);
...
function buildEquipmentAbilityFromRow(row, efficiencyValue, efficiencyUnit = resolveEquipmentEfficiencyUnit(row)) {
    return {
...
function buildEquipmentAbilityFromCodes(processCode, craftCode, efficiencyValue, efficiencyUnit) {
    const row = getProcessCraftDictRowByCode(craftCode);
...
function isCountableEquipmentStatus(status) {
    return status === 'AVAILABLE' || status === 'IN_USE';
}
function buildDerivedEquipmentFromEntry(factoryId, row, entry) {
    const quantity = Math.max(1, getSeedNumericValue(entry.values, 'deviceCount'));
    const singleShiftMinutes = Math.max(1, getSeedNumericValue(entry.values, 'deviceShiftMinutes') || getSeedNumericValue(entry.values, 'staffShiftMinutes'));
    const efficiencyValue = Math.max(0.01, getSeedNumericValue(entry.values, 'deviceEfficiencyValue') || getSeedNumericValue(entry.values, 'staffEfficiencyValue'));
    return {
...
function buildCustomFactoryEquipments(factoryId) {
...
function ensureFactoryEquipmentArchive(factoryId, entries, supportedRows) {
    pruneOrphanProfiles();
    const existing = equipmentsByFactoryId.get(factoryId);
    if (existing)
        return cloneEquipments(existing);
    const customEquipments = buildCustomFactoryEquipments(factoryId);
    const coveredKeys = new Set(customEquipments.flatMap((equipment) => equipment.abilityList.map((ability) => `${ability.processCode}::${ability.craftCode}`)));
    const rowMap = new Map(supportedRows.map((row) => [`${row.processCode}::${row.craftCode}`, row]));
    const generatedEquipments = entries
        .map((entry) => {
        const row = rowMap.get(`${entry.processCode}::${entry.craftCode}`);
        if (!row)
            return null;
        if (coveredKeys.has(`${row.processCode}::${row.craftCode}`))
            return null;
...
function normalizeEntry(existingEntry, row, factoryId, preserveEmptyValues = false) {
    const baseValues = buildDefaultValues(row.samCurrentFieldKeys, row, factoryId);
    const existingValues = existingEntry?.values ?? {};
    const nextValues = row.samCurrentFieldKeys.reduce((result, key) => {
...
const profilesByFactoryId = new Map();
const equipmentsByFactoryId = new Map();
function pruneOrphanProfiles() {
...
function ensureProfile(factoryId) {
    pruneOrphanProfiles();
    const factory = getFactoryMasterRecordById(factoryId);
    if (!factory) {
...
export function listFactoryCapacityProfiles() {
    pruneOrphanProfiles();
    return listFactoryMasterRecords().map((factory) => ensureProfile(factory.id));
}
export function getFactoryCapacityProfileByFactoryId(factoryId) {
    return ensureProfile(factoryId);
}
function mapOnboardingConditionToEquipmentStatus(condition) {
    if (condition === '维修中')
        return 'MAINTENANCE';
    if (condition === '停用')
        return 'STOPPED';
...
function buildInitialEntryFromRow(row) {
    const emptyValues = row.samCurrentFieldKeys.reduce((result, key) => {
        result[key] = '';
        return result;
    }, {});
    return {
...
export function listFactoryCapacityProfileStoreIds() {
    pruneOrphanProfiles();
    return [...profilesByFactoryId.keys()].sort((left, right) => left.localeCompare(right));
}
export function listFactoryCapacitySupportedCraftRows(factoryId) {
    const factory = getFactoryMasterRecordById(factoryId);
    if (!factory)
        return [];
    return resolveFactorySupportedCraftRows(factory);
}
export function listFactoryCapacityEntries(factoryId) {
...
export function listFactoryCapacityEquipments(factoryId) {
    if (factoryId) {
        const profile = ensureProfile(factoryId);
...
function findMatchedEquipmentAbility(equipment, processCode, craftCode) {
    return equipment.abilityList.find((ability) => ability.processCode === processCode && ability.craftCode === craftCode);
}
export function getFactoryCapacityEquipmentSummary(factoryId, processCode, craftCode) {
    const equipments = listFactoryCapacityEquipments(factoryId);
    const matchedEquipments = equipments.filter((equipment) => equipment.abilityList.some((ability) => ability.processCode === processCode && ability.craftCode === craftCode));
    const summary = matchedEquipments.reduce((result, equipment) => {
        ...
        if (equipment.status === 'MAINTENANCE')
            result.maintenanceEquipmentCount += quantity;
        if (equipment.status === 'STOPPED')
            result.stoppedEquipmentCount += quantity;
        if (equipment.status === 'FROZEN')
            result.frozenEquipmentCount += quantity;
        ...
        if (!result.efficiencyUnit)
            result.efficiencyUnit = ability.efficiencyUnit;
        return result;
    }, {
        factoryId,
        processCode,
    ...
    if (summary.countableEquipmentCount > 0) {
        summary.averageSingleShiftMinutes = Number((summary.eligibleShiftMinutesTotal / summary.countableEquipmentCount).toFixed(2));
    }
    if (summary.eligibleShiftMinutesTotal > 0) {
        summary.weightedEfficiencyValue = Number((summary.eligibleDeviceCapacityTotal / summary.eligibleShiftMinutesTotal).toFixed(4));
    }
...
export function replaceFactoryCapacityEquipments(factoryId, equipments) {
    const sanitized = cloneEquipments(equipments).map((equipment, index) => ({
        ...
        abilityList: equipment.abilityList.filter((ability) => getCapacityProcessCraftOptions().some((option) => option.processCode === ability.processCode && option.craftCode === ability.craftCode)),
    }));
    equipmentsByFactoryId.set(factoryId, sanitized);
}
export function replaceFactoryCapacityProfileEntries(factoryId, entries) {
    const profile = ensureProfile(factoryId);
    const supportedPairSet = new Set(listFactoryCapacitySupportedCraftRows(factoryId).map((row) => `${row.processCode}:${row.craftCode}`));
    const next = {
        factoryId,
        entries: entries
...
function getNumericValue(values, key) {
...
export function updateFactoryCapacityEntryValue(factoryId, processCode, craftCode, fieldKey, value) {
    const profile = ensureProfile(factoryId);
...
export function updateFactoryCapacityEntryNote(factoryId, processCode, craftCode, note) {
    const profile = ensureProfile(factoryId);
    const next = cloneProfile(profile);
    const entry = next.entries.find((item) => item.processCode === processCode && item.craftCode === craftCode);
    if (!entry)
...
export function hasCapacityFieldValue(value) {
    return value !== undefined && value !== null && String(value).trim() !== '';
}
export function calculateFactoryCapacityCompletion(factoryId) {
...
function getEntryNumericValue(entry, fieldKey) {
...
function resolveEquipmentStatus(factoryId, processCode) {
    const factory = getFactoryMasterRecordById(factoryId);
    if (!factory)
        return 'STOPPED';
    if ((factory.processAbilities.find((item) => item.processCode === processCode)?.status ?? 'ACTIVE') === 'PAUSED')
        return 'FROZEN';
    if (factory.status === 'paused')
        return 'FROZEN';
    if (factory.status === 'inactive' || factory.status === 'blacklist')
        return 'STOPPED';
    return 'AVAILABLE';
}
export function listFactoryPostCapacityNodes(factoryId) {
    const entries = new Map(listFactoryCapacityEntries(factoryId)
        .filter(({ row }) => isPostCapacityNodeProcess(row.craftCode))
        .map(({ row, entry }) => [row.craftCode, { row, entry }]));
    const equipments = listFactoryCapacityEquipments(factoryId);
    return POST_CAPACITY_NODE_CODES.flatMap((nodeCode) => {
        const resolved = entries.get(nodeCode);
        if (!resolved)
            return [];
        const matchedEquipments = equipments.filter((equipment) => equipment.abilityList.some((ability) => ability.processCode === 'POST_FINISHING' && ability.craftCode === nodeCode));
...
export function listFactoryPrintMachineCapacities(factoryId) {
...
export function listFactoryDyeVatCapacities(factoryId) {
    return listFactoryCapacityEquipments(factoryId)
...
export function auditFactoryCapacityProfile(factoryId) {
    const factory = getFactoryMasterRecordById(factoryId);
    if (!factory)
        return [];
    const profile = ensureProfile(factoryId);
    ...
    supportedRows.forEach((row) => {
        ...
        const actualKeys = Object.keys(entry.values);
        ...
        const result = computeFactoryCapacityEntryResult(row, entry.values, getFactoryCapacityEquipmentSummary(factory.id, row.processCode, row.craftCode));
...
export function auditAllFactoryCapacityProfiles() {
