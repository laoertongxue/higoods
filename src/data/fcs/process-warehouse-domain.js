import { getSpecialCraftTaskWorkOrdersByTaskOrderId, listSpecialCraftTaskOrders, } from './special-craft-task-orders.ts';
import { applySpecialCraftHandoverDifferenceToFeiTickets } from './cutting/special-craft-fei-ticket-flow.ts';
import { listPostFinishingWorkOrders } from './post-finishing-domain.ts';
function cloneWarehouseRecord(record) {
    return {
        ...record,
...
function createRecordNo(prefix, index) {
    return `${prefix}-${String(index).padStart(4, '0')}`;
}
function normalizeStatus(recordType, status) {
    if (status)
        return status;
    return recordType === 'WAIT_PROCESS' ? '已入仓' : '待交出';
}
function buildWarehouseRecord(payload, recordType, index) {
...
function getObjectQtyLabel(objectType) {
    if (objectType === '面料')
        return '面料米数';
    if (objectType === '裁片')
        return '裁片数量';
...
export function formatProcessObjectQty(qty, unit) {
    return `${roundQty(qty)} ${unit}`;
}
export function getProcessWarehouseQtyLabel(prefix, objectType) {
    return `${prefix}${getObjectQtyLabel(objectType)}`;
}
function buildSeedProcessWarehouseRecords() {
    const commonFactory = {
...
function buildSpecialCraftWarehouseRecords(taskOrders) {
    const records = [];
    const resolveLocation = (craftName, warehouseKind, seed) => {
    ...
    taskOrders.forEach((taskOrder) => {
        ...
        if (taskOrder.status === '待领料' || taskOrder.status === '已入待加工仓' || taskOrder.status === '加工中') {
            ...
            records.push(buildWarehouseRecord({
                ...common,
        ...
        if (taskOrder.waitHandoverQty > 0 || ['待交出', '已交出', '全部交出', '收货差异', '差异', '异议中'].includes(taskOrder.status)) {
            ...
            }, 'WAIT_HANDOVER', records.length + 1));
        }
    });
    return records;
}
function postWaitAction(order) {
    if (order.currentStatus.includes('接收'))
        return '待接收入仓';
    if (order.currentStatus.includes('复检'))
        return '待复检';
    if (order.currentStatus.includes('质检'))
        return '待质检';
    if (!order.isPostDoneBySewingFactory && order.currentStatus.includes('后道'))
        return '待后道';
    if (order.currentStatus === '待交出' || order.currentStatus.includes('已'))
        return '待复检';
    return '待接收入仓';
}
function buildPostFinishingWarehouseRecords(orders) {
    const records = [];
...
function buildInitialWarehouseRecords() {
    return [
...
function buildInitialHandoverRecords(warehouseRecords) {
    const records = [];
    warehouseRecords
    ...
    const ensureDemoCoverage = (craftType, craftName) => {
        const warehouses = warehouseRecords.filter((record) => record.recordType === 'WAIT_HANDOVER'
            && record.craftType === craftType
            && (!craftName || record.craftName === craftName));
        if (!warehouses.length)
            return;
        const demoRecordedAt = nowText();
        ['交出待收货', '全部交出', '收货差异'].forEach((status) => {
            let existedCount = records.filter((record) => record.craftType === craftType
                && (!craftName || record.craftName === craftName)
                && record.status === status).length;
...
function buildInitialReviewRecords(handoverRecords) {
...
function resolveDifferenceType(record) {
    if (record.remark?.includes('报废'))
        return '报废';
    if (record.remark?.includes('货损'))
        return '货损';
    if (record.remark?.includes('破损'))
        return '破损';
    if (record.diffObjectQty < 0)
        return '少收';
    if (record.diffObjectQty > 0)
        return '多收';
    return '其他';
}
function buildDifferenceRecordFromHandover(record, index) {
    const status = index % 3 === 0 ? '待处理' : index % 3 === 1 ? '处理中' : '已确认差异';
    const nextAction = status === '处理中'
        ? '平台处理'
        : status === '已确认差异'
            ? '确认差异继续流转'
            : '要求重新交出';
    const specialCraftDifferenceTypes = ['报废', '货损', '少收', '多收'];
    return {
...
function buildInitialDifferenceRecords(handoverRecords) {
...
const processWarehouseRecords = buildInitialWarehouseRecords();
const processHandoverRecords = buildInitialHandoverRecords(processWarehouseRecords);
const processHandoverDifferenceRecords = buildInitialDifferenceRecords(processHandoverRecords);
const processWarehouseReviewRecords = buildInitialReviewRecords(processHandoverRecords);
...
processWarehouseReviewRecords.forEach((review) => {
    ...
    if (difference)
        review.relatedDifferenceRecordId = difference.differenceRecordId;
...
function matchesWarehouseFilter(record, filter = {}) {
    if (filter.recordType && record.recordType !== filter.recordType)
        return false;
    if (filter.craftType && record.craftType !== filter.craftType)
        return false;
    if (filter.craftName && record.craftName !== filter.craftName)
        return false;
    if (filter.sourceWorkOrderId && record.sourceWorkOrderId !== filter.sourceWorkOrderId)
        return false;
    if (filter.sourceTaskId && record.sourceTaskId !== filter.sourceTaskId)
        return false;
    if (filter.targetFactoryId && record.targetFactoryId !== filter.targetFactoryId)
        return false;
    if (filter.status && record.status !== filter.status)
        return false;
    return true;
}
function matchesHandoverFilter(record, filter = {}) {
    if (filter.craftType && record.craftType !== filter.craftType)
        return false;
    if (filter.craftName && record.craftName !== filter.craftName)
        return false;
    if (filter.sourceWorkOrderId && record.sourceWorkOrderId !== filter.sourceWorkOrderId)
        return false;
    if (filter.warehouseRecordId && record.warehouseRecordId !== filter.warehouseRecordId)
        return false;
    if (filter.status && record.status !== filter.status)
        return false;
    return true;
}
function matchesReviewFilter(record, filter = {}) {
    if (filter.craftType && record.craftType !== filter.craftType)
        return false;
    if (filter.craftName && record.craftName !== filter.craftName)
        return false;
    if (filter.sourceWorkOrderId && record.sourceWorkOrderId !== filter.sourceWorkOrderId)
        return false;
    if (filter.handoverRecordId && record.handoverRecordId !== filter.handoverRecordId)
        return false;
    if (filter.reviewStatus && record.reviewStatus !== filter.reviewStatus)
        return false;
    return true;
}
function matchesDifferenceFilter(record, filter = {}) {
    if (filter.craftType && record.craftType !== filter.craftType)
        return false;
    if (filter.craftName && record.craftName !== filter.craftName)
        return false;
...
export function getProcessHandoverRecordById(recordId) {
...
export function getProcessHandoverDifferenceRecordById(differenceRecordId) {
...
export function getProcessWarehouseReviewRecordById(reviewRecordId) {
...
export function getWarehouseRecordsByWorkOrderId(workOrderId) {
    return listProcessWarehouseRecords({ sourceWorkOrderId: workOrderId });
}
export function getHandoverRecordsByWorkOrderId(workOrderId) {
    return listProcessHandoverRecords({ sourceWorkOrderId: workOrderId });
}
export function getReviewRecordsByWorkOrderId(workOrderId) {
    return listProcessWarehouseReviewRecords({ sourceWorkOrderId: workOrderId });
}
export function getDifferenceRecordsByWorkOrderId(workOrderId) {
    return listProcessHandoverDifferenceRecords({ sourceWorkOrderId: workOrderId });
}
export function getHandoverRecordsByWarehouseRecordId(warehouseRecordId) {
    return listProcessHandoverRecords({ warehouseRecordId });
}
export function getDifferenceRecordsByHandoverRecordId(handoverRecordId) {
    return listProcessHandoverDifferenceRecords({ handoverRecordId });
}
function upsertWarehouseRecord(payload, recordType) {
    ...
    const existed = processWarehouseRecords.find((record) => record.recordType === recordType
...
export function createWaitProcessWarehouseRecord(payload) {
    return upsertWarehouseRecord(payload, 'WAIT_PROCESS');
}
export function createWaitHandoverWarehouseRecord(payload) {
    return upsertWarehouseRecord(payload, 'WAIT_HANDOVER');
}
export function updateWarehouseRecordQty(recordId, payload) {
    const record = processWarehouseRecords.find((item) => item.warehouseRecordId === recordId);
    if (!record)
        return undefined;
...
export function markWarehouseRecordInProcess(recordId, payload = {}) {
    const record = processWarehouseRecords.find((item) => item.warehouseRecordId === recordId);
    if (!record)
        return undefined;
    record.status = '加工中';
...
export function markWarehouseRecordReadyToHandover(recordId, payload = {}) {
    const record = processWarehouseRecords.find((item) => item.warehouseRecordId === recordId);
    if (!record)
        return undefined;
    record.status = '待交出';
...
export function closeWarehouseRecord(recordId, payload = {}) {
    const record = processWarehouseRecords.find((item) => item.warehouseRecordId === recordId);
    if (!record)
        return undefined;
    record.status = '已关闭';
...
export function createProcessHandoverRecord(payload) {
    const warehouse = payload.warehouseRecordId
        ...
        : processWarehouseRecords.find((record) => record.recordType === 'WAIT_HANDOVER'
            && record.craftType === payload.craftType
            && record.sourceWorkOrderId === payload.sourceWorkOrderId);
...
export function createProcessHandoverDifferenceRecord(payload) {
...
export function writeBackProcessHandoverRecord(handoverRecordId, payload) {
...
export function createProcessWarehouseReviewRecord(payload) {
    ...
    const handover = processHandoverRecords.find((record) => record.handoverRecordId === next.handoverRecordId);
    if (handover)
        handover.relatedReviewRecordId = next.reviewRecordId;
    const warehouse = processWarehouseRecords.find((record) => record.warehouseRecordId === handover?.warehouseRecordId);
...
export function handleProcessHandoverDifference(differenceRecordId, payload) {
...
export function applySpecialCraftDifferenceToFeiTickets(differenceRecordId, payload) {
    const difference = processHandoverDifferenceRecords.find((record) => record.differenceRecordId === differenceRecordId);
    if (!difference || difference.craftType !== 'SPECIAL_CRAFT')
        return difference ? cloneDifferenceRecord(difference) : undefined;
...
export function getProcessWarehouseSummary(filter = {}) {
