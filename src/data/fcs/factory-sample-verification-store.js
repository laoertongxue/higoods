...
let cachedSampleVerifications = null;
...
function normalizeSampleReviewRecord(record) {
    ...
    const toStatus = String(record.toStatus || (sampleReviewResult === '已通过' ? '样衣审核通过' : '样衣审核退回'));
    const toNode = String(record.toNode || (sampleReviewResult === '未通过' ? '工厂提交样衣审核' : '样衣验证完成'));
    return {
        ...
        requiredResubmitItems: Array.isArray(record.requiredResubmitItems) ? [...record.requiredResubmitItems] : sampleReviewResult === '未通过' ? ['样衣照片'] : [],
        reviewer: String(record.reviewer || '平台样衣审核员'),
        reviewedAt: String(record.reviewedAt || nowTimestamp()),
        fromStatus: String(record.fromStatus || '待平台审核样衣'),
        toStatus,
        fromNode: String(record.fromNode || '平台审核样衣'),
        toNode,
        relatedSubmissionRoundNo: Number(record.relatedSubmissionRoundNo || 1),
        sampleQualityConclusion: record.sampleQualityConclusion,
        capacityConclusion: record.capacityConclusion,
        bossIdentityNoAtReview: record.bossIdentityNoAtReview ? String(record.bossIdentityNoAtReview) : undefined,
        bossIdentityFilesAtReview: Array.isArray(record.bossIdentityFilesAtReview) ? record.bossIdentityFilesAtReview.map(cloneReferenceFile) : [],
        bossIdentitySourceAtReview: record.bossIdentitySourceAtReview,
...
function cloneSampleVerification(item) {
    return {
        ...item,
...
function createFactorySubmissionFile(seed, kind, index = 1) {
...
function fallbackApplication(seed) {
    return {
...
function getApplicationSnapshot(seed) {
    return getFactoryOnboardingApplicationById(`FOA-${String(seed).padStart(4, '0')}`) || fallbackApplication(seed);
}
function getSeedPurposes(seed) {
    if (seed % 3 === 0)
        return ['检验车缝能力', '检验质量稳定性', '检验交期配合度'];
    if (seed % 3 === 1)
        return ['检验裁床能力', '检验特殊工艺能力'];
    return ['检验后道能力', '检验质量稳定性'];
}
function getSeedSampleStatus(seed) {
    if (seed <= 18)
        return '待工厂确认收样';
    if (seed <= 21)
        return '待工厂提交样衣审核';
...
function getSeedSampleNode(status) {
    if (status === '待工厂确认收样')
        return '工厂确认收样';
...
function getSeedBossIdentity(seed, status, hasSubmitted, hasReviewed) {
    if (!hasSubmitted) {
        return {
...
function createSeedSampleVerification(seed) {
    ...
    const issueMethod = seed <= 18 ? '现场发放' : '快递发放';
    ...
    const sampleReviewRecords = status === '样衣审核退回' || status === '样衣审核拒绝' || status === '样衣审核通过'
        ? Array.from({ length: status === '样衣审核退回' ? 2 : 1 }, (_, index) => {
            ...
            return {
                ...
                toStatus: status,
                fromNode: '平台审核样衣',
                toNode: status === '样衣审核退回' ? '工厂提交样衣审核' : '样衣验证完成',
...
function createSeedSampleVerifications() {
    return Array.from({ length: 18 }, (_, index) => index + 16).map(createSeedSampleVerification);
}
function ensureSampleVerificationStore() {
    if (cachedSampleVerifications)
        return cachedSampleVerifications;
    const stored = readStoredJson(SAMPLE_VERIFICATION_STORE_KEY);
...
function persistSampleVerifications(items) {
    cachedSampleVerifications = items.map(cloneSampleVerification);
    writeStoredJson(SAMPLE_VERIFICATION_STORE_KEY, cachedSampleVerifications);
}
export function listSampleVerifications() {
    return ensureSampleVerificationStore().map(cloneSampleVerification);
}
export function getSampleVerificationById(verificationId) {
...
export function getSampleVerificationByApplicationId(applicationId) {
    const matched = ensureSampleVerificationStore()
        .filter((item) => item.applicationId === applicationId)
        .sort((left, right) => right.updatedAt.localeCompare(left.updatedAt))[0];
    return matched ? cloneSampleVerification(matched) : null;
}
export function createSampleVerificationFromOnboarding(application, payload) {
...
export function updateSampleVerification(verificationId, patch) {
