...
const TEMPLATE_A_CRAFT_NAMES = ['基础连接', '手缝扣'];
...
const TEMPLATE_C_CRAFT_NAMES = ['丝网印', '数码印', '压褶', '打条', '激光切', '捆条', '橡筋定长切割'];
const TEMPLATE_D_CRAFT_NAMES = ['匹染', '色织', '缩水', '洗水'];
export const FACTORY_SUPPLY_TEMPLATE_BY_CRAFT_NAME = Object.fromEntries([
...
export const CURRENT_STAGE_ALLOWED_FIELD_KEYS = [
    'deviceCount',
    'deviceShiftMinutes',
...
export const EXPECTED_SAM_CURRENT_FIELD_KEYS_BY_TEMPLATE = {
...
function buildTemplateAGuide(craftName) {
    return {
        template: 'A',
        idealFormulaLines: [
        ...
        currentReason: '理想完整口径会保留人员效率单位做解释字段；当前阶段只收敛人数、单人默认日有效分钟、人员标准效率值和工厂效率系数，系统据此自动算出默认日可供给发布工时 SAM。',
    };
}
function buildTemplateBGuide(craftName) {
    return {
        template: 'B',
        idealFormulaLines: [
        ...
        currentReason: '理想完整口径会保留设备效率单位和人员效率单位做解释字段；当前阶段只保留最小必要数值字段，系统据此自动算出默认日可供给发布工时 SAM。',
    };
}
function buildTemplateCGuide(craftName) {
    return {
        template: 'C',
        idealFormulaLines: [
            '设备侧日供给能力 = 设备数量 × 单台默认日有效分钟 × 设备标准效率值',
        ...
        currentReason: '理想完整口径会保留设备效率单位和人员效率单位做连续型速度解释；当前阶段只保留最小必要数值字段，系统据此自动算出默认日可供给发布工时 SAM。',
    };
}
function buildTemplateDGuide(craftName) {
    return {
        template: 'D',
        idealFormulaLines: [
        ...
        currentReason: '理想完整口径会保留装载量单位和人员效率单位做批次口径解释；当前阶段只保留最小必要数值字段，系统据此自动算出默认日可供给发布工时 SAM。',
    };
}
export function getFactorySupplyFormulaTemplate(craftName) {
...
export function getExpectedSamCurrentFieldKeysByTemplate(template) {
    return [...EXPECTED_SAM_CURRENT_FIELD_KEYS_BY_TEMPLATE[template]];
}
export function getFactorySupplyFormulaGuideByTemplate(template, subjectName) {
    switch (template) {
        case 'A':
...
export function getFactorySupplyFormulaGuide(craftName) {
