import { buildDictionaryCraftMockDocumentNo, listGeneratedProductionDemandArtifacts, } from '../production-artifact-generation.ts';
import { productionOrders } from '../production-orders.ts';
import { listProcessWorkOrders, } from '../process-work-order-domain.ts';
import { validateDyeWorkOrderMobileTaskBinding, validatePrintWorkOrderMobileTaskBinding, } from '../process-mobile-task-binding.ts';
import { getPlatformStatusForProcessWorkOrder, } from '../process-platform-status-adapter.ts';
...
function getPrepArtifacts(processCode) {
...
function toDemandStatus(requiredQty, satisfiedQty, handoverCompleted) {
    if (satisfiedQty <= 0)
        return '待满足';
    if (satisfiedQty < requiredQty)
        return '部分满足';
    if (handoverCompleted)
        return '已完成交接';
    return '已满足';
}
function toLinkedOrderStatus(status) {
    if (status === '待交出')
        return '待交出';
    if (status === '交出待收货')
        return '交出待收货';
    ...
    if (status === '部分交出')
        return '部分交出';
    if (status === '收货差异' || status === '异常')
        return '收货差异';
    if (status === '全部交出' || status === '已完成' || status === '已关闭')
        return '已收货';
    return '进行中';
}
function calcOrderStatus(requiredQty, satisfiedQty, index) {
    if (satisfiedQty <= 0)
        return '待接收来料';
    if (satisfiedQty < requiredQty * 0.4)
        return '加工中';
    if (satisfiedQty < requiredQty)
        return '部分交出';
    return index % 4 === 0 ? '已关闭' : '全部交出';
}
function legacyOrderStatusToPlatformStatus(status) {
    switch (status) {
        case '待接收来料':
        case '待开工':
            return '待开工';
...
function calcReceiptStatus(requiredQty, satisfiedQty) {
    if (satisfiedQty <= 0)
        return '待接收';
    if (satisfiedQty < requiredQty)
        return '部分接收';
...
function toWarehouseName(index) {
    return index % 2 === 0 ? '雅加达裁片仓' : '万隆裁片仓';
}
function toRequirementText(meta, artifact) {
...
function toMaterialCode(meta, artifact, index) {
...
function resolveArtifactMockIndex(artifact, fallbackIndex) {
...
function buildPrepDocumentNo(prefix, artifact, fallbackIndex) {
    if (artifact.craftCode) {
        return buildDictionaryCraftMockDocumentNo(prefix, artifact.craftCode, artifact.orderId, resolveArtifactMockIndex(artifact, fallbackIndex));
    }
    return `${prefix}${artifact.orderId.replace(/\D/g, '').slice(-8)}${pad(fallbackIndex + 1, 2)}`;
}
function buildFacts(processCode) {
    ...
    artifacts.forEach((artifact, index) => {
        ...
        const createMode = index % 2 === 0 ? '按需求创建' : '按备货创建';
        ...
        const traceLine = satisfiedQty > 0
            ? [
                {
        ...
        const sources = satisfiedQty > 0
            ...
            : [];
        const linkedOrders = satisfiedQty > 0
            ? [
                {
                    processOrderNo: orderNo,
                    createMode,
        ...
        const orderFact = {
            ...
            stockMaterial: createMode === '按备货创建'
                ? {
                    materialCode: toMaterialCode(meta, artifact, index),
                    materialName: `${meta.materialLabel} ${order?.demandSnapshot.spuName ?? artifact.orderId}`,
                    unit: meta.unit,
...
function toDemandStatusFromOrder(order) {
    if (order.status === 'COMPLETED')
        return '已完成交接';
    if (order.handoverRecords.length > 0)
        return '部分满足';
    return '待满足';
}
function toReceiptStatusFromOrder(order) {
    if (order.handoverRecords.length === 0)
        return '待接收';
    if (order.status === 'COMPLETED')
        return '已接收';
...
function mapUnifiedWorkOrderToPrepOrder(order) {
...
function toDemandFactFromProcessOrder(processCode, orderFact, index) {
    const meta = META_BY_PROCESS[processCode];
    ...
    const traceLines = satisfiedQty > 0
        ? [
            {
...
function buildFactsFromProcessWorkOrders(processCode) {
...
export function listPrepRequirementDemands(processCode) {
    return cloneDemands(processCode === 'PRINT' ? PRINT_FACTS.demands : DYE_FACTS.demands);
}
export function listPrepProcessOrders(processCode) {
