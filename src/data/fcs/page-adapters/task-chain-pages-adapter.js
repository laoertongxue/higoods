import { isRuntimeTaskExecutionTask, listRuntimeExecutionTasks, listRuntimeProcessTasks, listRuntimeTasksByBaseTaskId, } from '../runtime-process-tasks.ts';
import { getTaskTypeDisplayName, listExecutionTaskFacts, } from './task-execution-adapter.ts';
...
function resolveTenderStatus(tasks, deadline) {
    ...
    const hasBidding = tasks.some((task) => task.assignmentMode === 'BIDDING'
...
export function listTaskChainTasks() {
    return listExecutionTaskFacts()
        .filter((task) => task.defaultDocType !== 'DEMAND')
        .sort(compareTask);
}
export function getTaskChainTaskById(taskId) {
    return listTaskChainTasks().find((task) => task.taskId === taskId);
}
export function listTaskChainRuntimeTasksByBaseTaskId(baseTaskId) {
    const byBase = listRuntimeTasksByBaseTaskId(baseTaskId).filter((task) => isRuntimeTaskExecutionTask(task));
    if (byBase.length > 0)
        return byBase;
    return listRuntimeProcessTasks().filter((task) => task.taskId === baseTaskId && isRuntimeTaskExecutionTask(task));
}
export function resolveTaskChainTenderId(task) {
...
export function listTaskChainTenders() {
...
export function getTaskChainTenderById(tenderId) {
    return listTaskChainTenders().find((item) => item.tenderId === tenderId);
}
export function getTaskChainTaskDisplayName(task) {
