import { getDyeWorkOrderById, getDyeWorkOrderStatusLabel, listDyeExecutionNodeRecords, } from './dyeing-task-domain.ts';
import { findPdaHandoverRecord, getHandoverOrderById, getPdaHandoverRecordsByHead, getPdaPickupRecordsByHead, listPdaHandoverHeads, } from './pda-handover-events.ts';
import { listPdaGenericProcessTasks } from './pda-task-mock-factory.ts';
import { getRecordDiffQty, getRecordReceiverWrittenAt, getRecordReceiverWrittenBy, getRecordReceiverWrittenQty, getHandoverObjectTypeLabel, getHandoverOrderStatusLabel, getHandoverRecordQrDisplayValue, getReceiverDisplayName, getRecordSubmittedQty, } from './task-handover-domain.ts';
import { getPrintWorkOrderById, getPrintWorkOrderStatusLabel, listPrintExecutionNodeRecords, } from './printing-task-domain.ts';
import { buildProductionConfirmationSnapshot, } from './production-confirmation.ts';
import { buildTaskQrValue } from './task-qr.ts';
import { getRuntimeTaskById, listRuntimeExecutionTasksByOrder, } from './runtime-process-tasks.ts';
import { getSpecialCraftTaskOrderById, } from './special-craft-task-orders.ts';
import { getPostFinishingTaskById, getPostFinishingWorkOrderById, } from './post-finishing-domain.ts';
import { getCuttingMarkerPlanRefTaskPrintSourceById, getCuttingCutOrderTaskPrintSourceById, } from './cutting-task-print-source.ts';
...
function toText(value, fallback = '待确认') {
...
function formatQtyText(value, unit, fallback = '—') {
    if (!Number.isFinite(value))
        return fallback;
    const formatted = Number(value).toLocaleString('zh-CN', { maximumFractionDigits: 2 });
    return `${formatted} ${unit || ''}`.trim();
}
function getTaskQuantityLabel(processName, unit, purpose) {
...
function buildSystemPlaceholderImage(title = '暂无商品图') {
    const svg = `
...
function safeBuildProductionSnapshot(productionOrderId) {
    try {
...
function firstImage(title, images, sourceLabel) {
...
function resolveTaskSpecificImage(snapshot, processName, craftName) {
    ...
    if (text.includes('特殊') || text.includes('绣') || text.includes('洗') || text.includes('烫')) {
        return (firstImage('工艺参考图', snapshot?.imageSnapshot.artworkImages, '任务特有图：工艺参考图')
...
function resolvePrintImage(input) {
    ...
    return (firstImage('商品图片', snapshot?.imageSnapshot.productImages, '商品主图')
...
function getRuntimeTaskProductionOrderId(taskId, fallbackOrderNo) {
    const task = getRuntimeTaskById(taskId);
    return task?.productionOrderId || fallbackOrderNo;
}
function getHandoverRecordStatusLabel(record) {
    if (record.handoverRecordStatus === 'SUBMITTED_WAIT_WRITEBACK' || record.status === 'PENDING_WRITEBACK')
        return '待收货';
    if (record.handoverRecordStatus === 'WRITTEN_BACK_MATCHED' || record.status === 'WRITTEN_BACK')
        return '已收货';
    if (record.handoverRecordStatus === 'WRITTEN_BACK_DIFF')
        return '差异待确认';
    if (record.handoverRecordStatus === 'DIFF_ACCEPTED')
        return '已接受差异';
    if (record.handoverRecordStatus === 'OBJECTION_REPORTED' || record.status === 'OBJECTION_REPORTED')
        return '已发起异议';
    if (record.handoverRecordStatus === 'OBJECTION_PROCESSING' || record.status === 'OBJECTION_PROCESSING')
        return '异议处理中';
    if (record.handoverRecordStatus === 'OBJECTION_RESOLVED' || record.status === 'OBJECTION_RESOLVED')
        return '异议已处理';
    return '待收货';
}
function getHandoverSummaryStatusLabel(status) {
    if (status === 'NONE')
        return '暂无记录';
    if (status === 'SUBMITTED')
        return '已提交';
...
function findRecordInHead(head, handoverRecordId) {
    return getPdaHandoverRecordsByHead(head.handoverId).find((record) => record.recordId === handoverRecordId
        || record.handoverRecordId === handoverRecordId
        || record.handoverRecordNo === handoverRecordId);
}
function fallbackRecordLine(record) {
    return {
        lineId: `${record.recordId}-LINE-001`,
        handoverRecordId: record.handoverRecordId || record.recordId,
        objectType: (record.objectType || 'FINISHED_GARMENT'),
...
function buildDeliveryCardLines(record) {
...
function findHandoutRecordContext(handoverRecordId) {
    for (const head of listPdaHandoverHeads().filter((item) => item.headType === 'HANDOUT')) {
        const record = getPdaHandoverRecordsByHead(head.handoverId).find((item) => item.recordId === handoverRecordId
...
export function buildTaskDeliveryCardByRecord(handoverOrderId, handoverRecordId) {
...
export function buildTaskDeliveryCardByRecordId(handoverRecordId) {
    ...
    return buildTaskDeliveryCardByRecord(context.head.handoverOrderId || context.head.handoverId, context.record.handoverRecordId || context.record.recordId);
}
function buildCardTitle(processName, cardName) {
    const normalized = processName.trim();
    if (!normalized || normalized === '待确认')
        return cardName;
    return `${normalized}${cardName}`;
}
function mapDeliveryCardToPrintDoc(card, record) {
    ...
    if (typeof writtenQty === 'number')
        writebackRows.push({ label: `接收方收货${writtenQtyLabel}`, value: `${writtenQty} ${card.qtyUnit}` });
    if (writtenAt)
        writebackRows.push({ label: '收货时间', value: writtenAt });
    if (writtenBy)
        writebackRows.push({ label: '收货人', value: writtenBy });
    if (typeof diffQty === 'number')
        writebackRows.push({ label: diffQtyLabel, value: `${diffQty} ${card.qtyUnit}` });
    if (record?.handoverRecordStatus)
        writebackRows.push({ label: '异议状态', value: getHandoverRecordStatusLabel(record) });
    const remarkRows = [
...
export function buildTaskDeliveryCardPrintDocByRecordId(handoverRecordId) {
    const context = findHandoutRecordContext(handoverRecordId);
    const result = context
        ? buildTaskDeliveryCardByRecord(context.head.handoverOrderId || context.head.handoverId, context.record.handoverRecordId || context.record.recordId)
...
function buildRuntimeRouteRows(task) {
...
function buildPendingRouteRow(node) {
    return {
        rowId: `pending-${node}`,
        node,
        startedAt: '—',
...
function ensureRouteRecordNodes(rows, nodeNames) {
...
function buildHandoverRouteRows(taskId) {
...
function buildRouteCardFromGenericTask(task) {
    return {
        ok: true,
        card: {
...
function buildRouteCardFromHandoverTask(head) {
    return {
        ok: true,
        card: {
            cardName: TASK_ROUTE_CARD_NAME,
...
function buildRouteCardFromPrintWorkOrder(sourceId) {
    const order = getPrintWorkOrderById(sourceId);
    if (!order)
        return { ok: false, title: TASK_ROUTE_CARD_NAME, message: `未找到印花加工单：${sourceId}` };
...
function buildRouteCardFromDyeWorkOrder(sourceId) {
    const order = getDyeWorkOrderById(sourceId);
    if (!order)
        return { ok: false, title: TASK_ROUTE_CARD_NAME, message: `未找到染色加工单：${sourceId}` };
...
function mapSpecialCraftNodeRecord(record) {
    return {
        rowId: record.nodeRecordId,
        node: record.nodeName,
...
function buildRouteCardFromSpecialCraftTaskOrder(sourceId) {
    const order = getSpecialCraftTaskOrderById(sourceId);
    if (!order)
        return { ok: false, title: TASK_ROUTE_CARD_NAME, message: `未找到特殊工艺加工单：${sourceId}` };
    return {
        ok: true,
        card: {
            cardName: TASK_ROUTE_CARD_NAME,
...
function mapPostFinishingActionRecord(record) {
    return {
...
function buildPostFinishingTaskRouteRows(task) {
...
function buildRouteCardFromPostFinishingTask(sourceId) {
    const task = getPostFinishingTaskById(sourceId);
    if (!task)
        return { ok: false, title: TASK_ROUTE_CARD_NAME, message: `未找到后道任务：${sourceId}` };
    return {
        ok: true,
        card: {
            cardName: TASK_ROUTE_CARD_NAME,
...
function buildRouteCardFromPostFinishingWorkOrder(sourceId) {
    const order = getPostFinishingWorkOrderById(sourceId);
    if (!order)
        return { ok: false, title: TASK_ROUTE_CARD_NAME, message: `未找到后道单：${sourceId}` };
    const actionRecords = [order.receiveAction, order.qcAction, order.postAction, order.recheckAction].filter(Boolean);
    return {
        ok: true,
        card: {
            cardName: TASK_ROUTE_CARD_NAME,
...
function buildRouteCardFromCuttingCutOrder(sourceId) {
    const source = getCuttingCutOrderTaskPrintSourceById(sourceId);
    if (!source)
        return { ok: false, title: TASK_ROUTE_CARD_NAME, message: `未找到裁片单：${sourceId}` };
    return {
        ok: true,
        card: {
            cardName: TASK_ROUTE_CARD_NAME,
...
function buildRouteCardFromCuttingMarkerPlanRef(sourceId) {
    const source = getCuttingMarkerPlanRefTaskPrintSourceById(sourceId);
    if (!source)
        return { ok: false, title: TASK_ROUTE_CARD_NAME, message: `未找到唛架方案：${sourceId}` };
    return {
        ok: true,
        card: {
            cardName: TASK_ROUTE_CARD_NAME,
...
export function buildTaskRouteCardBySource(sourceType, sourceId) {
    switch (sourceType) {
        case 'RUNTIME_TASK':
...
export function isTaskRouteCardSourceType(value) {
    return (value === 'RUNTIME_TASK'
...
function mapRouteCardToPrintDoc(card) {
