const capacityCalendarOverrides = [
    {
        id: 'CAP-OVR-0001',
        factoryId: 'ID-F030',
...
function cloneOverride(item) {
    return { ...item };
}
function normalizeOverrideInput(input) {
    return {
...
export function listCapacityCalendarOverrides() {
...
export function getCapacityCalendarOverrideById(id) {
    const item = capacityCalendarOverrides.find((record) => record.id === id);
    return item ? cloneOverride(item) : undefined;
}
export function createCapacityCalendarOverride(input) {
...
export function updateCapacityCalendarOverride(id, input) {
...
export function removeCapacityCalendarOverride(id) {
    const index = capacityCalendarOverrides.findIndex((record) => record.id === id);
    if (index >= 0) {
        capacityCalendarOverrides.splice(index, 1);
    }
}
export function expireCapacityCalendarOverride(id, endDate) {
