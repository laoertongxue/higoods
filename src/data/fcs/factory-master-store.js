...
function createOnboardingOfficialSeedFactories() {
...
function mergeSeedFactories(factories) {
...
function loadFactoryMasterRecords() {
    ...
    try {
        ...
        return mergeSeedFactories(parsed).map((factory) => cloneFactory(factory));
    }
    catch {
        return mergeSeedFactories(mockFactories).map((factory) => cloneFactory(factory));
    }
}
function persistFactoryMasterRecords() {
    writeBrowserStorageItem(getBrowserLocalStorage(), FACTORY_MASTER_STORE_KEY, JSON.stringify(factoryMasterRecords.map((factory) => cloneFactory(factory))));
}
let factoryMasterRecords = loadFactoryMasterRecords();
export function listFactoryMasterRecords() {
    return factoryMasterRecords.map((factory) => cloneFactory(factory));
}
export function listBusinessFactoryMasterRecords(input = {}) {
...
function isSewingFactory(factory) {
    if (factory.status !== 'active')
        return false;
    if (factory.processAbilities.some((item) => item.processCode === 'SEW'))
        return true;
    return ['CENTRAL_GARMENT', 'SATELLITE_SEWING', 'THIRD_SEWING'].includes(factory.factoryType);
}
export function listSewingFactoryMasterRecords() {
...
export function getFactoryMasterRecordById(factoryId) {
...
export function upsertFactoryMasterRecord(factory) {
    const nextFactory = cloneFactory(factory);
    const currentIndex = factoryMasterRecords.findIndex((item) => item.id === nextFactory.id);
    if (currentIndex >= 0) {
        factoryMasterRecords = factoryMasterRecords.map((item, index) => index === currentIndex ? nextFactory : item);
...
export function removeFactoryMasterRecord(factoryId) {
