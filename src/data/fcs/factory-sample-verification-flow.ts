...
} from './factory-onboarding-flow.ts'
import {
...
import {
  ...
  type FactoryBossIdentitySource,
  type FactorySampleIssuePayload,
  type FactorySampleReceivePayload,
  type FactorySampleReferenceFile,
  type FactorySampleReviewPayload,
  type FactorySampleReviewRecord,
  type FactorySampleSubmissionPayload,
  type FactorySampleVerification,
  type FactorySampleVerificationNode,
  type FactorySampleVerificationStatus,
...
function formatTimestamp(date = nowDate()): string {
...
function buildActionLogId(actionName: string, at: string): string {
  return `ACT-${actionName}-${at.replace(/[-: T]/g, '')}`
}

function buildNodeLogId(nodeName: string, at: string): string {
  return `NODE-${nodeName}-${at.replace(/[-: T]/g, '')}`
}

export function buildSampleVerificationNo(): string {
  const now = nowDate()
  return `SV-${formatDateCode(now)}-${pad(now.getTime() % 1000, 3)}`
}

function buildSampleBatchNo(): string {
  const now = nowDate()
  return `SY-${formatDateCode(now)}-${pad(now.getTime() % 1000, 3)}`
}

export function createSampleIssuePayload(input: Partial<FactorySampleIssuePayload> = {}): FactorySampleIssuePayload {
  const now = nowDate()
  return {
...
export function validateSampleIssuePayload(payload: FactorySampleIssuePayload): void {
  const errors: string[] = []
  if (!payload.sampleBatchNo.trim()) errors.push('请填写样衣批次号')
  if (!payload.styleNo.trim()) errors.push('请填写款号')
  if (!payload.sampleName.trim()) errors.push('请填写样衣名称')
  if (!payload.sampleQuantity) errors.push('请填写样衣件数')
  if (Number(payload.sampleQuantity) <= 0) errors.push('样衣件数必须大于 0')
  if (!payload.sampleDescription.trim()) errors.push('请填写样衣说明')
  if (payload.verificationPurpose.length <= 0) errors.push('请选择验证目的')
  if (!payload.issueMethod || !FACTORY_SAMPLE_ISSUE_METHOD_OPTIONS.includes(payload.issueMethod as never)) errors.push('请选择发放方式')
  if (payload.issueMethod === '快递发放' && !payload.courierCompany?.trim()) errors.push('请填写快递公司')
  if (payload.issueMethod === '快递发放' && !payload.trackingNo?.trim()) errors.push('请填写快递单号')
  if (!payload.issuedAt.trim()) errors.push('请选择发放时间')
  if (!payload.issuedBy.trim()) errors.push('请填写发放人')
  if (!payload.expectedSubmitAt.trim()) errors.push('请选择预计提交样衣审核时间')
  if (errors.length > 0) throw new Error(errors.join('\n'))
}

function cloneReferenceFiles(files: FactorySampleReferenceFile[]): FactorySampleReferenceFile[] {
  return files.map((item) => ({ ...item }))
}

function hasReferenceFiles(files: FactorySampleReferenceFile[] | null | undefined): boolean {
...
function resolveBossIdentitySource(params: {
  existingSource?: FactoryBossIdentitySource
  hadFactoryIdentity: boolean
  hasPlatformSupplement: boolean
}): FactoryBossIdentitySource | undefined {
  if (params.existingSource === '工厂提交和平台补录') return '工厂提交和平台补录'
  if (params.hasPlatformSupplement && (params.hadFactoryIdentity || params.existingSource === '工厂提交')) return '工厂提交和平台补录'
  if (params.hasPlatformSupplement) return '平台补录'
  if (params.hadFactoryIdentity || params.existingSource === '工厂提交') return '工厂提交'
  return params.existingSource
}

function resolveBossIdentityForReview(
  verification: FactorySampleVerification,
  payload: FactorySampleReviewPayload,
  reviewer: string,
  operatedAt: string,
): {
...
function buildSampleActionLog(params: {
  actionName: FactorySampleVerification['actionLogs'][number]['actionName']
  ...
  remark: string
}): FactorySampleVerification['actionLogs'][number] {
  return {
...
function updateSampleNodeLogs(
  ...
  const closed = verification.nodeLogs.map((log) => {
    if (log.nodeName !== params.fromNode || log.leftAt) return log
    return {
      ...log,
      nodeStatus: '已完成' as const,
  ...
  if (openIndex >= 0) {
    return closed.map((log, index) => index === openIndex
      ? { ...log, nodeStatus: '进行中' as const, operator: params.operator, remark: params.openingRemark }
      : log)
  }
  return [
    ...closed,
...
function appendOnboardingSampleAction(params: {
  ...
  actionName: FactoryOnboardingActionLog['actionName']
...
function updateOnboardingNodeLogsForReceive(application: FactoryOnboardingApplication, operatedAt: string, operator: string): FactoryOnboardingNodeLog[] {
  ...
  const nextSampleNode: FactoryOnboardingNodeLog = {
    ...(sampleNode || {
...
function updateOnboardingNodeLogsForSampleSubmit(application: FactoryOnboardingApplication, operatedAt: string, operator: string): FactoryOnboardingNodeLog[] {
  const closedSampleNodes = application.nodeLogs.map((log) => {
    if (log.nodeName !== '样衣验证' || log.leftAt) return log
    return {
      ...log,
      nodeStatus: '已完成' as const,
      leftAt: operatedAt,
  ...
  if (openIndex >= 0) {
    return closedSampleNodes.map((item, index) => index === openIndex
      ? { ...item, nodeStatus: '进行中' as const, operator, lastActionAt: operatedAt, remark: '待平台审核样衣。' }
      : item)
  }
  return [
    ...closedSampleNodes,
...
function updateSampleNodeLogsForPlatformReview(
  verification: FactorySampleVerification,
  params: {
    toStatus: FactorySampleVerificationStatus
    toNode: FactorySampleVerificationNode
  ...
  if (openIndex >= 0) {
    return closed.map((log, index) => index === openIndex
      ? { ...log, nodeStatus: openStatus as FactoryOnboardingNodeLog['nodeStatus'], operator: params.operator, remark: params.openingRemark }
      : log)
  }
  return [
    ...closed,
...
function updateOnboardingNodeLogsForSampleReview(
  application: FactoryOnboardingApplication,
  params: {
    toStatus: FactoryOnboardingStatus
    toNode: FactoryOnboardingNode
  ...
  if (openIndex >= 0) {
    return closed.map((log, index) => index === openIndex
      ? {
          ...log,
          nodeStatus: openStatus as FactoryOnboardingNodeLog['nodeStatus'],
          operator: params.operator,
          lastActionAt: params.operatedAt,
          actionCount: Math.max(1, log.actionCount || 0) + 1,
          remark: params.openingRemark,
...
function requireOnboardingApplication(applicationId: string): FactoryOnboardingApplication {
...
function assertSupportedFiles(files: FactorySampleReferenceFile[], allowed: string[], errors: string[]): void {
  if (files.some((file) => !allowed.includes(file.fileType))) {
    errors.push('文件格式不支持，请重新上传')
  }
}

export function validateFactorySampleSubmission(payload: FactorySampleSubmissionPayload): void {
  const errors: string[] = []
  if (!payload.factorySamplePhotos || payload.factorySamplePhotos.length <= 0) errors.push('请上传样衣照片')
  if (!payload.factorySampleVideos || payload.factorySampleVideos.length <= 0) errors.push('请上传样衣视频')
  if (!payload.factoryCraftDescription.trim()) errors.push('请填写工艺说明')
  if (!payload.factorySitePhotos || payload.factorySitePhotos.length <= 0) errors.push('请上传工厂照片')
  if (!payload.factorySiteVideos || payload.factorySiteVideos.length <= 0) errors.push('请上传工厂视频')
...
export function getFactorySampleVerificationForOnboarding(applicationId: string): FactorySampleVerification | null {
  return getSampleVerificationByApplicationId(applicationId)
}

function buildSampleNodeLog(application: FactoryOnboardingApplication, operatedAt: string, operator: string): FactoryOnboardingNodeLog {
...
function appendSampleIssueAction(application: FactoryOnboardingApplication, operatedAt: string, operator: string): FactoryOnboardingActionLog {
...
export function issueSampleForOnboarding(
  applicationId: string,
  payload: FactorySampleIssuePayload,
  operator: string,
): { application: FactoryOnboardingApplication; sampleVerification: FactorySampleVerification } {
  const application = getFactoryOnboardingApplicationById(applicationId)
  if (!application) throw new Error('未找到入驻申请')
...
export function confirmFactoryReceivedSample(
  verificationId: string,
  payload: FactorySampleReceivePayload,
  operator: string,
): { application: FactoryOnboardingApplication; sampleVerification: FactorySampleVerification } {
  const verification = getSampleVerificationById(verificationId)
  if (!verification) throw new Error('未找到样衣验证记录')
  if (verification.status !== '待工厂确认收样') throw new Error('当前状态不能确认收到样衣。')

  const errors: string[] = []
  if (!payload.factoryReceivedAt.trim()) errors.push('请选择确认收样时间')
  if (!payload.factoryReceivedBy.trim()) errors.push('请填写确认收样人')
  if (errors.length > 0) throw new Error(errors.join('\n'))
  ...
  if (!updatedVerification) throw new Error('样衣验证记录更新失败')
...
export function submitFactorySampleReview(
  verificationId: string,
  payload: FactorySampleSubmissionPayload,
  operator: string,
): { application: FactoryOnboardingApplication; sampleVerification: FactorySampleVerification } {
  const verification = getSampleVerificationById(verificationId)
  if (!verification) throw new Error('未找到样衣验证记录')
...
export function validateSampleReviewPayload(payload: FactorySampleReviewPayload, verification?: FactorySampleVerification): void {
  const errors: string[] = []
  const reviewResult = normalizeSampleReviewResult(payload.sampleReviewResult)
  if (!payload.sampleReviewResult || !FACTORY_SAMPLE_REVIEW_RESULT_OPTIONS.includes(reviewResult as never)) errors.push('请选择样衣审核结果')
  if (!payload.sampleReviewOpinion.trim()) errors.push('请填写样衣审核意见')
  ...
  if (reviewResult === '已通过') {
    ...
    if (!bossIdentityNo) errors.push('请填写老板身份证号码/护照号码')
    if (!hasReferenceFiles(bossIdentityFiles)) errors.push('请上传老板身份证复印件或照片')
  }
  if (errors.length > 0) throw new Error(errors.join('\n'))
}

export function getLatestSampleReviewRecord(verificationId: string): FactorySampleReviewRecord | null {
  const verification = getSampleVerificationById(verificationId)
  if (!verification || verification.sampleReviewRecords.length <= 0) return null
...
export function canReviewFactorySample(verification: FactorySampleVerification | null | undefined): boolean {
  return verification?.status === '待平台审核样衣'
}

export function canFactoryResubmitSample(verification: FactorySampleVerification | null | undefined): boolean {
  return verification?.status === '样衣审核退回'
}

export function reviewFactorySample(
  verificationId: string,
  payload: FactorySampleReviewPayload,
  operator: string,
): { application: FactoryOnboardingApplication; sampleVerification: FactorySampleVerification; sampleReviewRecord: FactorySampleReviewRecord } {
  const verification = getSampleVerificationById(verificationId)
  if (!verification) throw new Error('未找到样衣验证记录')
  if (!canReviewFactorySample(verification)) throw new Error('当前状态不能进行样衣审核')
  ...
  const sampleToStatus: FactorySampleVerificationStatus = reviewResult === '已通过'
    ? '样衣审核通过'
    : '样衣审核退回'
  const sampleToNode: FactorySampleVerificationNode = resubmitAllowed ? '工厂提交样衣审核' : '样衣验证完成'
  const applicationToStatus: FactoryOnboardingStatus = reviewResult === '已通过'
    ? '样衣审核通过待转正式'
    : '样衣审核退回'
  const applicationToNode: FactoryOnboardingNode = reviewResult === '已通过'
    ? '正式合作'
    : '样衣验证'
  ...
  const requiredResubmitItems = resubmitAllowed
    ...
    : []
  const sampleReviewRecord: FactorySampleReviewRecord = {
    ...
    sampleQualityConclusion: payload.sampleQualityConclusion && FACTORY_SAMPLE_QUALITY_CONCLUSION_OPTIONS.includes(payload.sampleQualityConclusion as never)
      ? payload.sampleQualityConclusion
      : undefined,
    capacityConclusion: payload.capacityConclusion && FACTORY_SAMPLE_CAPACITY_CONCLUSION_OPTIONS.includes(payload.capacityConclusion as never)
...
export function getSampleStatusText(status: FactorySampleVerificationStatus | string | null | undefined): string {
  const matched = FACTORY_SAMPLE_VERIFICATION_STATUS_OPTIONS.find((item) => item === status)
  return matched || '未登记样衣'
}

export function getSampleNodeText(currentNode: FactorySampleVerificationNode | string | null | undefined): string {
