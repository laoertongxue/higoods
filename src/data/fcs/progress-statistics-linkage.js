...
import { listFactoryInternalWarehouses, listFactoryWaitHandoverStockItems, listFactoryWaitProcessStockItems, listFactoryWarehouseInboundRecords, listFactoryWarehouseOutboundRecords, listFactoryWarehouseStocktakeDifferenceReviews, listFactoryWarehouseStocktakeOrders, } from './factory-internal-warehouse.ts';
import { getPdaHandoverRecordsByHead, listPdaHandoverHeads, listQuantityObjections, listReceiverWritebacks, } from './pda-handover-events.ts';
import { getSpecialCraftTasksByProductionOrder, listSpecialCraftTaskWorkOrderLines, listSpecialCraftTaskWorkOrders, listSpecialCraftTaskOrders, } from './special-craft-task-orders.ts';
...
const PROGRESS_URGENCY_SORT_WEIGHT = {
    十万火急: 4,
    '紧急 A': 3,
    '紧急 B': 2,
    C: 1,
...
const sewingDispatchProgressSnapshotCache = new Map();
function sum(values) {
...
function shouldIncludeFactoryInSnapshots(factoryId, options = {}) {
    if (!factoryId || options.includeTestFactories)
        return true;
    return !getFactoryMasterRecordById(factoryId)?.isTestFactory;
}
function shouldIncludeOrderInSnapshots(order, options = {}) {
    return shouldIncludeFactoryInSnapshots(order.mainFactoryId, options);
}
let cuttingSpecialCraftProjectionBindingsCache = null;
let cuttingSpecialCraftReturnStatusCache = null;
const handoverProgressSnapshotCache = new Map();
const progressBlockingReasonsCache = new Map();
const productionProgressSnapshotCache = new Map();
const cuttingProgressSnapshotCache = new Map();
function getOpenDifferenceStatus(status) {
    return status === '待处理' || status === '处理中' ? status : undefined;
}
function buildCuttingSpecialCraftProjectionBindings() {
    if (cuttingSpecialCraftProjectionBindingsCache)
        return cuttingSpecialCraftProjectionBindingsCache;
    const taskOrders = listSpecialCraftTaskOrders();
    const taskOrdersByProductionOrderId = new Map();
    ...
    const workOrderLineByDemandLineId = new Map(listSpecialCraftTaskWorkOrderLines().map((line) => [line.demandLineId, line]));
    const operationById = new Map();
    const getScanOperation = (operationId) => {
        if (!operationById.has(operationId)) {
            const operation = getSpecialCraftOperationById(operationId);
            if (operation?.requiresFeiTicketScan)
                operationById.set(operationId, operation);
        }
        return operationById.get(operationId);
    };
    const generatedTickets = productionOrders.flatMap((order) => listSpreadingResultGeneratedFeiTicketsByProductionOrderId(order.productionOrderId));
    const ticketByNo = new Map(generatedTickets.map((ticket) => [ticket.feiTicketNo, ticket]));
    const bindings = [];
    ...
    generatedTickets.forEach((ticket) => {
        ...
        (taskOrdersByProductionOrderId.get(ticket.productionOrderId) || []).forEach((taskOrder) => {
            if (normalizeText(taskOrder.partName) && normalizeText(taskOrder.partName) !== ticketPartName)
                return;
            if (normalizeText(taskOrder.fabricColor) && normalizeText(taskOrder.fabricColor) !== ticketColor)
                return;
            (taskOrder.demandLines || []).forEach((line) => {
                if (normalizeText(line.sizeCode) !== normalizeText(ticket.skuSize))
                    return;
                const operation = getScanOperation(line.operationId);
                if (!operation)
                    return;
                if (!secondaryCraftSet.has(normalizeText(operation.operationName)))
                    return;
...
function seedCuttingSpecialCraftProjectionBindings(bindings) {
    ...
    const patchBinding = (binding, patch) => {
        if (binding)
...
function listCuttingSpecialCraftFeiTicketBindingsForProjection() {
    return buildCuttingSpecialCraftProjectionBindings();
}
let cuttingSpecialCraftProjectionBindingsByProductionOrderCache = null;
let specialCraftTaskWorkOrderLinesCache = null;
function getCuttingSpecialCraftProjectionBindingsByProductionOrder(productionOrderId) {
...
function getSpecialCraftTaskWorkOrderLinesCached() {
...
function buildCuttingSpecialCraftReturnStatusFromBindings(productionOrderId, orderBindings) {
    ...
    return {
        ...
        allReturned: lastByFeiTicket.size > 0
...
export function getCuttingSpecialCraftReturnStatusByProductionOrders(productionOrderIds) {
    if (!cuttingSpecialCraftReturnStatusCache) {
        ...
        groups.forEach((bindings, productionOrderId) => {
            if (cuttingSpecialCraftReturnStatusCache.has(productionOrderId))
                return;
    ...
    productionOrderIds.forEach((productionOrderId) => {
        result.set(productionOrderId, cuttingSpecialCraftReturnStatusCache.get(productionOrderId)
...
export function compareProductionProgressByDefaultDueDate(left, right) {
    ...
    if (!leftDueDate && rightDueDate)
        return 1;
    const urgencyCompare = (PROGRESS_URGENCY_SORT_WEIGHT[right.urgencyLevel || ''] ?? -1)
...
function getTechPackSpecialCraftSourceSummary(order) {
    ...
    const selectedTargetObjectSummary = unique(allCrafts
        .map((craft) => craft.selectedTargetObject)
        .filter((value) => Boolean(value)));
    const supportedTargetObjectSummary = unique(allCrafts.flatMap((craft) => [
...
function resolveUrgencyLevel(order) {
...
function getMaterialPrepRows(order) {
    return cuttingMaterialPrepGroups.flatMap((group) => group.productionOrderNo === order.productionOrderNo ? group.materialLines : []);
}
function resolveMaterialPrepStatus(order) {
    const lines = getMaterialPrepRows(order);
    if (!lines.length)
        return order.techPackSnapshot ? '有配料数量' : '无配料数量';
    if (lines.every((line) => line.configStatus === 'CONFIGURED'))
        return '有配料数量';
    if (lines.some((line) => line.configuredLength > 0 || line.configuredRollCount > 0))
        return '配料数量不足';
    return '无配料数量';
}
function resolveCuttingPickupStatus(order) {
    const lines = getMaterialPrepRows(order);
    if (lines.some((line) => line.receiveStatus === 'RECHECK' || line.discrepancyStatus !== 'NONE'))
        return '差异待处理';
    if (!lines.length)
        return order.status === 'EXECUTING' || order.status === 'COMPLETED' ? '有领料记录' : '无领料记录';
    if (lines.some((line) => line.receivedLength > 0 || line.receivedRollCount > 0))
        return '有领料记录';
    return '无领料记录';
}
function resolveFeiTicketStatus(order) {
    ...
    if (expectedLines > 0 && tickets.length < expectedLines)
        return '部分生成';
    return '已生成';
}
function resolveCuttingStatus(order) {
    const tickets = listSpreadingResultGeneratedFeiTicketsByProductionOrderId(order.productionOrderId);
    if (tickets.length > 0)
        return '已裁剪';
...
function resolveCuttingWaitHandoverStatus(order) {
    ...
    if (waitHandoverItems.length === 0)
        return '未入仓';
    if (ticketCount > 0 && waitHandoverItems.length < ticketCount)
        return '部分入仓';
    return '已入裁床厂待交出仓';
}
function resolveSpecialCraftStatus(order) {
    ...
    if (returnStatus.totalNeedSpecialCraftFeiTickets > 0 && returnStatus.allReturned)
        return '已回仓';
    if (returnStatus.objectionCount > 0)
        return '异议中';
    if (returnStatus.differenceCount > 0)
        return '差异';
    if (returnStatus.waitReturnCount > 0)
        return '待回仓';
    if (returnStatus.receivedBySpecialFactoryCount > 0 || tasks.some((task) => task.status === '加工中'))
        return '加工中';
    return '待交出';
}
function resolveSpecialCraftReturnStatus(order) {
    const status = getCuttingSpecialCraftReturnStatusByProductionOrder(order.productionOrderId);
    if (status.totalNeedSpecialCraftFeiTickets === 0)
        return '不需要回仓';
    if (status.allReturned)
        return '已回仓';
    if (status.objectionCount > 0)
        return '异议中';
    if (status.differenceCount > 0)
        return '差异';
    if (status.returnedCount > 0)
        return '部分回仓';
    return '未回仓';
}
export function buildSewingDispatchProgressSnapshot(order) {
    ...
    const dispatchRatio = tickets.length === 0
        ? 0
        : order.status === 'COMPLETED'
            ...
            : order.status === 'EXECUTING'
                ...
                : 0;
    const cumulativeDispatchedGarmentQty = Math.min(totalProductionQty, Math.max(0, Math.round((ticketQty || totalProductionQty) * dispatchRatio)));
    ...
    const writtenBackTransferBagCount = transferBagCount > 0 && order.status === 'COMPLETED'
        ? transferBagCount
        : Math.floor(transferBagCount * 0.45);
    const partialWritebackTransferBagCount = writtenBackTransferBagCount > 0 && writtenBackTransferBagCount < transferBagCount ? 1 : 0;
...
export function getCuttingSewingDispatchProgressByProductionOrder(productionOrderId) {
    const order = productionOrders.find((item) => item.productionOrderId === productionOrderId);
    if (order)
        return buildSewingDispatchProgressSnapshot(order);
    return {
...
function resolveSewingDispatchStatus(snapshot) {
    if (snapshot.differenceTransferBagCount > 0)
        return '差异';
    if (snapshot.objectionTransferBagCount > 0)
        return '异议中';
    if (snapshot.cumulativeDispatchedGarmentQty <= 0)
        return '未交出';
    if (snapshot.remainingGarmentQty > 0)
        return '部分交出';
    return '已全部交出';
}
function resolveSewingReceiveStatus(snapshot) {
    if (snapshot.differenceTransferBagCount > 0)
        return '差异';
    if (snapshot.objectionTransferBagCount > 0)
        return '异议中';
    if (snapshot.writtenBackTransferBagCount <= 0)
        return '未回写';
    if (snapshot.writtenBackTransferBagCount < snapshot.dispatchedTransferBagCount)
        return '部分回写';
    return '已回写';
}
export function buildHandoverProgressSnapshot(order) {
...
function resolveHandoverStatus(snapshot) {
    if (snapshot.handoverRecordCount === 0)
        return '无交接';
    if (snapshot.objectionCount > 0)
        return '异议中';
    if (snapshot.differenceCount > 0)
        return '差异';
    if (snapshot.waitWritebackCount > 0)
        return '待回写';
    return '已回写';
}
function makeBlockingReason(order, index, sourceModule, sourceRecordNo, blockingLabel, nextActionLabel, relatedRoute, severity = '普通') {
    return {
...
export function buildProgressBlockingReasons(order) {
    ...
    const add = (sourceModule, sourceRecordNo, blockingLabel, nextActionLabel, relatedRoute, severity = '普通') => {
        reasons.push({
            productionOrderId: order.productionOrderId,
            productionOrderNo: order.productionOrderNo,
            sourceModule,
    ...
    if (materialPrepStatus === '无配料数量')
        add('待加工入仓', order.productionOrderNo, '面料未完成中转仓配料', '查看待加工仓', '/fcs/craft/cutting/warehouse-management/wait-process', '加急');
    if (materialPrepStatus === '配料数量不足')
        add('待加工入仓', order.productionOrderNo, '面料已完成部分中转仓配料', '查看待加工仓', '/fcs/craft/cutting/warehouse-management/wait-process', '普通');
    if (pickupStatus === '无领料记录')
        add('待加工入仓', order.productionOrderNo, '面料未形成裁床领料记录', '查看待加工仓', '/fcs/craft/cutting/warehouse-management/wait-process');
    if (pickupStatus === '差异待处理')
        add('领料', order.productionOrderNo, '领料差异待处理', '处理差异', '/fcs/progress/handover', '紧急');
    if (cuttingStatus !== '已裁剪')
        add('裁床', order.productionOrderNo, cuttingStatus === '裁剪中' ? '裁剪未完成' : '唛架未完成', '裁剪', '/fcs/craft/cutting/production-progress');
    if (feiStatus !== '已生成')
        add('菲票', order.productionOrderNo, '菲票未生成', '打印菲票', '/fcs/craft/cutting/fei-tickets');
    if (waitHandoverStatus !== '已入裁床厂待交出仓')
        add('裁床', order.productionOrderNo, '裁片未入裁床厂待交出仓', '裁片入仓', '/fcs/craft/cutting/production-progress');
    if (specialReturnStatus === '差异')
        add('特殊工艺', order.productionOrderNo, '特殊工艺差异', '处理差异', '/fcs/craft/cutting/warehouse-management/wait-handover?tab=special-craft-return', '紧急');
    if (specialReturnStatus === '异议中')
        add('特殊工艺', order.productionOrderNo, '特殊工艺异议中', '处理异议', '/fcs/craft/cutting/warehouse-management/wait-handover?tab=special-craft-return', '紧急');
    ...
    if (handoverSnapshot.differenceCount > 0)
        add('交接', order.productionOrderNo, '交接差异', '处理差异', '/fcs/progress/handover', '紧急');
    if (handoverSnapshot.objectionCount > 0)
        add('交接', order.productionOrderNo, '数量异议中', '处理异议', '/fcs/progress/handover', '紧急');
    const result = reasons.map((reason, index) => makeBlockingReason(order, index + 1, reason.sourceModule, reason.sourceRecordNo, reason.blockingLabel, reason.nextActionLabel, reason.relatedRoute, reason.severity));
...
function resolveNextAction(snapshot) {
    if (snapshot.blockingReasons.length > 0)
        return snapshot.blockingReasons[0].nextActionLabel;
    if (snapshot.sewingDispatchStatus === '未交出' || snapshot.sewingDispatchStatus === '部分交出')
        return '新增交出记录';
    if (snapshot.sewingReceiveStatus !== '已回写')
        return '等待车缝回写';
    return '已完成当前节点';
}
export function buildProductionProgressSnapshot(order) {
    ...
    const transferBagCombinedWritebackStatus = (() => {
        ...
        if (recordStatuses.includes('异议中'))
            return '异议中';
        if (recordStatuses.includes('差异'))
            return '差异';
        if (recordStatuses.includes('部分回写'))
            return '部分回写';
        if (recordStatuses.includes('已回写'))
            return '已回写';
        if (recordStatuses.includes('待回写'))
            return '待回写';
        return '无交接';
    })();
    const partialSnapshot = {
        ...
        specialCraftDifferenceWarning: specialCraftBindings.some((binding) => binding.receiveDifferenceStatus === '待处理'
        ...
        canProceedToSewingDispatch: specialCraftReturnStatus !== '差异'
...
function metric(status, plannedQty, completedQty, differenceQty, abnormalCount, lastUpdatedAt = DEMO_TODAY) {
    return { status, plannedQty, completedQty, differenceQty, abnormalCount, lastUpdatedAt };
}
export function buildCuttingProgressSnapshot(order) {
    ...
    const bundleLines = getSpecialCraftTaskWorkOrderLinesCached().filter((line) => relatedWorkOrderIds.has(line.workOrderId));
    const techPackSpecialCraftSource = getTechPackSpecialCraftSourceSummary(order);
    ...
    const snapshot = {
        ...
        transferBagPackStatus: sewing.dispatchedTransferBagCount <= 0
            ? '待装袋'
            : sewing.packedTransferBagCount < sewing.transferBagCount
                ? '装袋中'
                : '已装袋',
        transferBagCombinedWritebackStatus: sewing.objectionTransferBagCount > 0
            ? '异议中'
...
export function buildSpecialCraftProgressSnapshots(options = {}) {
    ...
    const productionOrderMap = new Map(productionOrders.map((order) => [order.productionOrderId, order]));
    const groups = new Map();
    tasks.forEach((task) => {
    ...
    bindings.forEach((binding) => {
        ...
        if (!existing)
            return;
        if (binding.specialCraftFlowStatus === '待发料' || binding.specialCraftFlowStatus === '待绑定' || binding.specialCraftFlowStatus === '待确认顺序')
            existing.waitDispatchFeiTicketCount += 1;
        if (binding.specialCraftFlowStatus === '已发料')
            existing.dispatchedFeiTicketCount += 1;
        if (binding.specialCraftFlowStatus === '已接收')
            existing.receivedFeiTicketCount += 1;
        if (binding.specialCraftFlowStatus === '加工中')
            existing.processingFeiTicketCount += 1;
        if (binding.specialCraftFlowStatus === '待回仓' || binding.specialCraftFlowStatus === '已完成')
            existing.waitReturnFeiTicketCount += 1;
        if (binding.specialCraftFlowStatus === '已回仓')
            existing.returnedFeiTicketCount += 1;
        ...
        if (binding.objectionStatus === '异议中')
            existing.objectionFeiTicketCount += 1;
    ...
    return Array.from(groups.values()).filter((snapshot) => shouldIncludeFactoryInSnapshots(snapshot.factoryId, options));
}
export function buildFactoryWarehouseProgressSnapshots(options = {}) {
    ...
    return listFactoryInternalWarehouses()
        ...
        .reduce((result, warehouse) => {
        if (result.some((item) => item.factoryId === warehouse.factoryId))
            return result;
...
export function buildProductionProgressKpiSummary(snapshots = getProductionProgressSnapshots()) {
    return {
...
export function getProductionProgressSnapshotByOrder(productionOrderId) {
    const order = productionOrders.find((item) => item.productionOrderId === productionOrderId);
    return order ? buildProductionProgressSnapshot(order) : undefined;
}
let defaultProductionProgressSnapshotsCache = null;
export function getProductionProgressSnapshots(options = {}) {
    ...
    const snapshots = sortProductionProgressByDefaultDueDate(productionOrders
...
let defaultCuttingProgressSnapshotsCache = null;
function isDefaultProgressStatisticsOptions(options) {
    return Object.keys(options).length === 0;
}
export function getCuttingProgressSnapshots(options = {}) {
...
export function getSpecialCraftProgressSnapshots(options = {}) {
    return buildSpecialCraftProgressSnapshots(options);
}
export function getFactoryWarehouseProgressSnapshots(options = {}) {
    return buildFactoryWarehouseProgressSnapshots(options);
}
export function getProgressStatisticsDashboard(options = {}) {
...
export function assertProgressStatisticsConsistency() {
    ...
    snapshots.forEach((snapshot) => {
        if (snapshot.sewingDispatchStatus === '已全部交出' && snapshot.totalQty > 0) {
            ...
            if (sewing.cumulativeDispatchedGarmentQty > snapshot.totalQty)
                throw new Error(`${snapshot.productionOrderNo} 累计已交出件数超过生产总数`);
            if (sewing.remainingGarmentQty < 0)
                throw new Error(`${snapshot.productionOrderNo} 剩余未交出件数小于 0`);
        }
    });
    const sewingFactoryWarehouse = getFactoryWarehouseProgressSnapshots().find((item) => item.factoryName.includes('车缝厂'));
    if (sewingFactoryWarehouse)
        throw new Error('车缝厂不应进入工厂内部仓统计');
