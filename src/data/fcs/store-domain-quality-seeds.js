import { returnInboundChainBasisSeeds, returnInboundChainBatches, returnInboundChainQualityInspections, } from './return-inbound-quality-chain-facts.ts';
function mergeById(items, key) {
    const map = new Map();
...
export const initialQualityInspections = [...returnInboundChainQualityInspections];
// 18 QC seed records — 8 PASS / 10 FAIL
export const QC_SEEDS = [
    // ── PASS 8 条 ──
...
export const seedProductionOrder = {
    productionOrderId: 'PO-SEED-0001',
...
export const initialDeductionCandidates = [];
// ── initialDeductionBasisItems ───────────────
export const initialDeductionBasisItems = [];
export const BASIS_SEEDS = [
...
export const initialAllocationByTaskId = {
...
export const initialAllocationEvents = [];
// ── initialReturnBatches ─────────────────────
export const initialReturnBatches = [];
// ── initialReturnInboundBatches（回货入仓新主模型） ─────────────
export const initialReturnInboundBatches = mergeById([
    ...returnInboundChainBatches,
], 'batchId');
// ── legacyDyePrintOrdersSnapshot（兼容快照，非主真相） ─────────────
export const legacyDyePrintOrdersSnapshot = [
...
export function listDyePrintOrdersStore() {
