export const PLATFORM_PROCESS_STATUS_OPTIONS = [
    '待下发',
    '待接单',
    '待开工',
    '准备中',
...
export const PLATFORM_PROCESS_STATUS_ORDER = {
    异常: 1,
    收货差异: 2,
    收货确认中: 3,
    交出待收货: 4,
...
export const PLATFORM_PROCESS_STATUS_CLASS = {
    ...
    待交出: 'border-orange-200 bg-orange-50 text-orange-700',
    交出待收货: 'border-orange-200 bg-orange-50 text-orange-700',
    收货确认中: 'border-purple-200 bg-purple-50 text-purple-700',
    部分交出: 'border-amber-200 bg-amber-50 text-amber-700',
    全部交出: 'border-green-200 bg-green-50 text-green-700',
    收货差异: 'border-red-200 bg-red-50 text-red-700',
...
const PRINT_STATUS_MAP = {
    ...
    HANDOVER_WAIT_RECEIVE: 'WAIT_WRITEBACK',
    WAIT_WRITEBACK: 'WAIT_WRITEBACK',
    交出待收货: 'WAIT_WRITEBACK',
    PARTIAL_HANDOVER: 'PARTIAL_HANDOVER',
    部分交出: 'PARTIAL_HANDOVER',
    FULL_HANDOVER: 'FULL_HANDOVER',
    全部交出: 'FULL_HANDOVER',
    HANDOVER_DIFFERENCE: 'HANDOVER_DIFFERENCE',
    收货差异: 'HANDOVER_DIFFERENCE',
    RECEIPT_CONFIRMING: 'RECEIPT_CONFIRMING',
    收货确认中: 'RECEIPT_CONFIRMING',
    REJECTED: 'HANDOVER_DIFFERENCE',
    AUDIT_REJECTED: 'HANDOVER_DIFFERENCE',
    QUANTITY_DIFFERENCE: 'HANDOVER_DIFFERENCE',
    HAS_DIFFERENCE: 'HANDOVER_DIFFERENCE',
    已驳回: 'HANDOVER_DIFFERENCE',
    数量差异: 'HANDOVER_DIFFERENCE',
    有差异: 'HANDOVER_DIFFERENCE',
    COMPLETED: 'COMPLETED',
    已完成: 'COMPLETED',
    CLOSED: 'CLOSED',
    已关闭: 'CLOSED',
};
const DYE_STATUS_MAP = {
    WAIT_RELEASE: 'WAIT_RELEASE',
    待下发: 'WAIT_RELEASE',
    WAIT_ACCEPT: 'WAIT_ACCEPT',
    待接单: 'WAIT_ACCEPT',
...
const SPECIAL_CRAFT_STATUS_MAP = {
    WAIT_RELEASE: 'WAIT_RELEASE',
    待下发: 'WAIT_RELEASE',
    WAIT_ACCEPT: 'WAIT_ACCEPT',
    待接单: 'WAIT_ACCEPT',
    ...
    已交出: 'WAIT_WRITEBACK',
    WAIT_WRITEBACK: 'WAIT_WRITEBACK',
    WRITTEN_BACK: 'RECEIPT_CONFIRMING',
    交出待收货: 'WAIT_WRITEBACK',
    收货确认中: 'RECEIPT_CONFIRMING',
    RECEIPT_CONFIRMING: 'RECEIPT_CONFIRMING',
...
const POST_FINISHING_STATUS_MAP = {
    待扫码收货: 'PREPARING',
    接收中: 'PREPARING',
    已接收: 'PREPARING',
    待质检: 'PREPARING',
...
const RISK_HINTS = {
    ...
    PROCESSING: {
        riskLabel: '工厂内部加工中',
        actionHint: '等待工厂完成加工后交出',
        ownerHint: '工艺工厂',
    },
    WAIT_DELIVERY: {
        riskLabel: '工厂已完成加工，待交出',
        actionHint: '催办工厂发起交出',
        ownerHint: '工艺工厂',
    },
    WAIT_WRITEBACK: {
        riskLabel: '接收方尚未确认收货',
        actionHint: '跟进接收方确认收货数量',
        ownerHint: '接收方',
    },
    RECEIPT_CONFIRMING: {
        riskLabel: '等待收货确认',
    ...
    PARTIAL_HANDOVER: {
        riskLabel: '接收方已确认部分收货',
        actionHint: '继续交出剩余数量或处理差异',
        ownerHint: '工艺工厂 / 接收方',
    },
    FULL_HANDOVER: {
        riskLabel: '接收方已确认全部收货',
        actionHint: '查看交出和收货记录',
        ownerHint: '平台',
    },
    HANDOVER_DIFFERENCE: {
        riskLabel: '交出数量与实收数量存在差异',
        actionHint: '核对交出记录和收货数量',
        ownerHint: '平台 / 接收方 / 工艺工厂',
    },
    EXCEPTION: {
        riskLabel: '存在差异或异常，需要处理',
...
function resolveCode(map, status, fallback) {
    const raw = normalizeStatus(status);
    if (!raw)
        return fallback;
    return map[raw] ?? fallback;
}
function buildResult(code, context) {
...
export function mapPrintStatusToPlatformStatus(status, context = {}) {
...
export function mapDyeStatusToPlatformStatus(status, context = {}) {
...
export function mapCuttingStatusToPlatformStatus(status, context = {}) {
...
export function mapSpecialCraftStatusToPlatformStatus(status, context = {}) {
...
export function mapPostFinishingStatusToPlatformStatus(status, context = {}) {
...
export function mapCraftStatusToPlatformStatus(params) {
...
export function getPlatformStatusForProcessWorkOrder(workOrder) {
...
function getRuntimeTaskProcessType(task) {
    if (task.processCode.includes('PRINT'))
        return 'PRINT';
    if (task.processCode.includes('DYE'))
        return 'DYE';
    if (task.processCode.includes('CUT'))
        return 'CUTTING';
    if (task.processCode.includes('POST') || task.processNameZh.includes('后道'))
        return 'POST_FINISHING';
    if (task.processCode.includes('SPECIAL') || task.processCode.includes('CRAFT'))
...
function getRuntimeTaskStatusCode(task) {
    ...
    if (task.status === 'IN_PROGRESS')
        return 'PROCESSING';
    if (task.assignmentStatus === 'UNASSIGNED' || task.assignmentStatus === 'ASSIGNING')
        return 'WAIT_RELEASE';
...
export function getPlatformStatusForRuntimeTask(task) {
    const processType = getRuntimeTaskProcessType(task);
    const statusCode = getRuntimeTaskStatusCode(task);
    const craftStatusLabel = task.status === 'NOT_STARTED'
        ? '待开工'
...
export function listPlatformStatusOptions() {
