...
function normalizeDyeWarehouseReceiptStatus(status) {
...
function normalizeDyeWarehouseReference(value) {
    return value
...
export function getDyeingWarehouseView(filters = {}) {
    const keyword = filters.keyword?.trim().toLowerCase() || '';
    const byFactory = (factoryId) => !filters.factoryId || factoryId === filters.factoryId;
    const waitProcessRecords = listWaitProcessWarehouseRecords({ craftType: 'DYE' }).filter((item) => byFactory(item.targetFactoryId)
    ...
    const waitHandoverRecords = listWaitHandoverWarehouseRecords({ craftType: 'DYE' }).filter((item) => byFactory(item.targetFactoryId)
        && matchesStatus(item.status, filters.status)
        && matchesKeyword([item.warehouseRecordNo, item.sourceWorkOrderNo, item.sourceTaskNo, item.batchNo, item.skuSummary], keyword));
    const handoverRecords = listProcessHandoverRecords({ craftType: 'DYE' }).filter((item) => byFactory(item.handoverFactoryId)
        && matchesStatus(item.status, filters.status)
