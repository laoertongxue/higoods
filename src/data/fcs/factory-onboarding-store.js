...
import { DEFAULT_FACTORY_ONBOARDING_PPIC, getAvailableOnboardingPpicOptions as listAvailableOnboardingPpicOptions, getOnboardingPpicName as resolveOnboardingPpicName, getOnboardingPpicOptionById, } from './factory-onboarding-ppic.ts';
import { FACTORY_ADMIN_ROLE_ID, FACTORY_ADMIN_ROLE_NAME, canFactoryEnterBusiness, getOnboardingNodeByStatus, isFactoryAccountLocked, normalizeOnboardingStatus, normalizeReviewResult, } from './factory-onboarding-domain.ts';
...
export function normalizeFactoryShortName(factoryShortName) {
...
export function buildAdminAccountFromFactoryShortName(input) {
    ...
    const accountStatus = input.accountLocked || isFactoryAccountLocked(status)
        ? '已锁定'
...
function cloneAdminAccount(account) {
    return { ...account };
}
function cloneCapability(item) {
    return { ...item };
}
function cloneMachine(item) {
    return { ...item };
}
function cloneNodeLog(item) {
    return { ...item };
}
function cloneActionLog(item) {
    return { ...item };
}
function cloneReviewRecord(item) {
    return { ...item };
}
function cloneSupplementRecord(item) {
    return {
        ...item,
...
function cloneTransferRecord(item) {
    return { ...item };
}
function cloneConversionRecord(item) {
    return { ...item };
}
function clonePpicChangeLog(item) {
    return { ...item };
}
...
function resolveMachineValidation(machine, capabilities) {
...
function applyMachineValidations(machines, capabilities) {
...
function resolveFactoryTypeCode(capability) {
...
function inferFactoryTypes(capabilities) {
...
function getPrimaryFactoryType(matchResults) {
...
function buildFactoryTypeMatchReason(matchResults) {
...
function normalizeNodeName(nodeName, status) {
    if (nodeName === '填写入驻申请' || nodeName === '平台审核' || nodeName === '样衣验证' || nodeName === '样衣审核' || nodeName === '正式合作' || nodeName === '完成')
        return nodeName;
    if (nodeName === '填写入驻信息' || nodeName === '提交平台审核' || nodeName === '补充资料')
        return status === '待平台审核' ? '平台审核' : '填写入驻申请';
    if (nodeName === '确认' + '合作')
        return '样衣验证';
    if (nodeName === '生成工厂' + '档案')
        return '完成';
...
function normalizeActionName(actionName) {
    ...
    if (actionName === legacyApproved || actionName === '平台初审通过')
        return '平台初审已通过';
    if (actionName === legacyReturned || actionName === '平台退回补充' + '资料' || actionName === '平台初审退回')
        return '平台初审未通过';
    if (actionName === legacyRejected || actionName === '平台拒绝入驻')
        return '平台初审拒绝';
    return actionName;
}
function decorateApplication(rawApplication) {
    ...
    const identityFile = raw.identityFile || null;
    const adminAccount = buildAdminAccountFromFactoryShortName({
        factoryShortName,
        applicantName,
        mobilePhone,
        adminAccount: raw.adminAccount,
        status,
        accountLocked: Boolean(raw.accountLocked),
    });
    const selectedCapabilities = (raw.selectedCapabilities || []).map(cloneCapability);
    const machines = applyMachineValidations((raw.machines || []).map(cloneMachine), selectedCapabilities);
    ...
    const normalizedNodeLogs = (raw.nodeLogs || []).map((item) => ({
        ...item,
        nodeName: normalizeNodeName(String(item.nodeName), status),
    }));
    const normalizedActionLogs = (raw.actionLogs || []).map((item) => ({
        ...item,
    ...
    const normalizedReviewRecords = (raw.reviewRecords || []).map((item) => ({
        ...item,
    ...
    const rawTransferRecords = (raw.transferRecords || []).map(cloneTransferRecord);
    const rawConversionRecords = (raw.conversionRecords || []).map(cloneConversionRecord);
    ...
    const ppicChangeLogs = (raw.ppicChangeLogs || []).map(clonePpicChangeLog);
    return {
        ...
        assignedPpicAt: raw.assignedPpicAt,
        assignedPpicBy: raw.assignedPpicBy,
        ppicChangeLogs,
        machineTotalCount: Number(raw.machineTotalCount) || 0,
        effectiveWorkerCount: Number(raw.effectiveWorkerCount) || 0,
        availableStartDate: String(raw.availableStartDate || ''),
        ...
        lastSubmittedAt: (raw.lastSubmittedAt || raw.submittedAt),
        reviewedAt: raw.reviewedAt,
        sampleVerifiedAt: raw.sampleVerifiedAt,
        sampleIssuedAt: raw.sampleIssuedAt,
        sampleExpectedSubmitAt: raw.sampleExpectedSubmitAt,
        convertedAt: (raw.convertedAt || raw.contractedAt),
        createdFactoryId: raw.createdFactoryId,
        nodeLogs: normalizedNodeLogs,
        actionLogs: normalizedActionLogs,
        reviewRecords: normalizedReviewRecords,
        supplementRecords: (raw.supplementRecords || []).map(cloneSupplementRecord),
        accountLocked,
        accountLockedReason: raw.accountLockedReason || (accountLocked ? '历史账号状态不可用。' : undefined),
        factoryNameLocked: Boolean(raw.factoryNameLocked),
        lockedAt: raw.lockedAt,
        sampleVerificationId: raw.sampleVerificationId,
        ...
        contractedAt: (raw.convertedAt || raw.contractedAt),
    };
}
function createApplicationNo(seed) {
    return `FON-${String(20260500 + seed).padStart(8, '0')}`;
}
function createTempFactoryId(seed) {
    return `FACTORY-TEMP-${String(seed).padStart(4, '0')}`;
}
function resolveCraftByName(processName, craftName) {
...
function createCapability(processName, craftName, remark = '') {
    return {
...
function formatElapsedText(minutes, nodeStatus) {
    if (nodeStatus === '未开始')
        return '-';
    if (nodeStatus === '已终止')
        return '已终止';
...
function createIdentityFile(seed) {
...
function createBasePayload(seed, status, capabilityNames, condition, machineScenario = 'valid') {
    ...
    const machines = [createMachine({ seed: seed * 10 + 1, capability: baseCapability, condition })];
...
function createSeedApplication(seed, status, capabilityNames, condition, machineScenario = 'valid') {
    ...
    if (status === '平台审核退回') {
        const requiredFields = seed % 3 === 1 ? ['机器明细', '工序工艺能力'] : seed % 3 === 2 ? ['地址', '可开始合作时间'] : ['身份证复印件/电子文件', '手机号'];
    ...
    let sampleStatus;
    let sampleVerificationId;
    let sampleVerifiedAt;
    if ([
        '待样衣验证',
        '待工厂确认收样',
        '待工厂提交样衣审核',
...
function createSeedApplications() {
    ...
    const scenarios = ['valid', 'missingProcess', 'missingCraft', 'capabilityMismatch'];
    return statuses.flatMap((status, statusIndex) => [0, 1, 2].map((offset) => {
        const seed = statusIndex * 3 + offset + 1;
        const capabilityNames = CAPABILITY_SETS[(statusIndex + offset) % CAPABILITY_SETS.length];
        const condition = offset === 0 ? '可用' : offset === 1 ? '维修中' : '停用';
...
function ensureApplicationStore() {
    ...
    const stored = readStoredJson(APPLICATION_STORE_KEY);
...
export function listFactoryOnboardingApplications() {
    return ensureApplicationStore().map(cloneApplication);
}
export function getFactoryOnboardingApplicationById(applicationId) {
...
export function findFactoryOnboardingApplicationByLoginId(loginId) {
...
export function saveFactoryOnboardingApplication(application) {
...
export function replaceFactoryOnboardingApplications(applications) {
    persistApplications(applications);
}
export function updateFactoryOnboardingApplication(applicationId, updater) {
    const current = getFactoryOnboardingApplicationById(applicationId);
    if (!current)
        return null;
    return saveFactoryOnboardingApplication(updater(current));
}
export function isFactoryOnboardingLoginIdTaken(loginId, excludeApplicationId) {
    const normalized = normalizeLoginId(loginId);
    if (!normalized)
        return false;
    if (findFactoryPdaUserByLoginId(normalized))
        return true;
...
export function isFactoryShortNameTaken(factoryShortName, currentApplicationId) {
    ...
    const duplicatedApplication = ensureApplicationStore().some((item) => item.applicationId !== currentApplicationId &&
    ...
    return listFactoryMasterRecords().some((factory) => {
        if (current && (factory.id === current.createdFactoryId || factory.onboardingApplicationId === current.applicationId))
            return false;
...
export function validateFactoryShortNameUnique(factoryShortName, currentApplicationId) {
    if (!factoryShortName.trim())
        throw new Error('请填写工厂简称');
    if (isFactoryShortNameTaken(factoryShortName, currentApplicationId))
        throw new Error('工厂简称已存在，请更换');
}
...
export function getOnboardingPpicName(ppicId) {
    return resolveOnboardingPpicName(ppicId);
}
export function assignDefaultPpicForOnboarding(applicationId, operator = '系统默认分配') {
    const application = getFactoryOnboardingApplicationById(applicationId);
    if (!application)
        throw new Error('未找到入驻申请');
    if (application.assignedPpicId)
        return application;
...
export function updateOnboardingPpic(applicationId, nextPpicId, operator, changeReason = '') {
    const application = getFactoryOnboardingApplicationById(applicationId);
    if (!application)
        throw new Error('未找到入驻申请');
    if (!nextPpicId.trim())
        throw new Error('请选择 PPIC');
    const nextPpic = getOnboardingPpicOptionById(nextPpicId);
    if (!nextPpic || nextPpic.status !== '启用')
        throw new Error('该 PPIC 不可用，请重新选择');
    if (application.assignedPpicId === nextPpic.ppicId)
        return application;
...
export function isFactoryCompanyNameLocked(factoryCompanyName, excludeApplicationId) {
    void factoryCompanyName;
    void excludeApplicationId;
    return false;
}
export function canStartNewOnboarding(factoryCompanyName, excludeApplicationId) {
    return !isFactoryCompanyNameLocked(factoryCompanyName, excludeApplicationId);
}
export function getLockedFactoryNameReason(factoryCompanyName, excludeApplicationId) {
    void factoryCompanyName;
    void excludeApplicationId;
    return '';
}
export function isFactoryOnboardingFactoryCompanyNameLocked(factoryCompanyName, excludeApplicationId) {
    return isFactoryCompanyNameLocked(factoryCompanyName, excludeApplicationId);
}
export function createEmptyFactoryOnboardingDraft() {
    return {
        factoryShortName: '',
        applicantName: '',
        identityNo: '',
...
export function getFactoryOnboardingApplicantSession() {
    const stored = readStoredJson(APPLICANT_SESSION_KEY);
    if (!stored)
        return null;
    if (!stored.applicationId || !stored.loginId || !stored.factoryName)
        return null;
    return { ...stored };
}
export function setFactoryOnboardingApplicantSession(session) {
    if (!session) {
...
export function clearFactoryOnboardingApplicantSession() {
    removeBrowserStorageItem(getStorage(), APPLICANT_SESSION_KEY);
}
export function createFactoryOnboardingApplicantSession(application) {
    return {
