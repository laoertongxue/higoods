import type { OutputValueCurrentFieldKey } from './process-craft-dict.ts'
...
export interface FactorySupplyFormulaGuide {
  ...
  currentFieldKeys: OutputValueCurrentFieldKey[]
...
const TEMPLATE_D_CRAFT_NAMES = ['水溶', '匹染', '色织', '缩水', '洗水'] as const
...
export const CURRENT_STAGE_ALLOWED_FIELD_KEYS: OutputValueCurrentFieldKey[] = [
  'deviceCount',
  'deviceShiftMinutes',
...
export const EXPECTED_OUTPUT_VALUE_CURRENT_FIELD_KEYS_BY_TEMPLATE: Record<FactorySupplyFormulaTemplate, OutputValueCurrentFieldKey[]> = {
...
const DEVICE_COUNT_LABEL = getOutputValueBusinessFieldLabel('deviceCount')
const DEVICE_SHIFT_MINUTES_LABEL = getOutputValueBusinessFieldLabel('deviceShiftMinutes')
const DEVICE_EFFICIENCY_VALUE_LABEL = getOutputValueBusinessFieldLabel('deviceEfficiencyValue')
const STAFF_COUNT_LABEL = getOutputValueBusinessFieldLabel('staffCount')
const STAFF_SHIFT_MINUTES_LABEL = getOutputValueBusinessFieldLabel('staffShiftMinutes')
const STAFF_EFFICIENCY_VALUE_LABEL = getOutputValueBusinessFieldLabel('staffEfficiencyValue')
const BATCH_LOAD_CAPACITY_LABEL = getOutputValueBusinessFieldLabel('batchLoadCapacity')
const CYCLE_MINUTES_LABEL = getOutputValueBusinessFieldLabel('cycleMinutes')
const SETUP_MINUTES_LABEL = getOutputValueBusinessFieldLabel('setupMinutes')
const SWITCH_MINUTES_LABEL = getOutputValueBusinessFieldLabel('switchMinutes')
const EFFICIENCY_FACTOR_LABEL = getOutputValueBusinessFieldLabel('efficiencyFactor')
...
function buildTemplateAGuide(craftName: string): FactorySupplyFormulaGuide {
  return {
    ...
    currentExampleLines: [
      ...
      '默认日可供给产值 = 2688 × 0.9 = 2419.2',
    ],
    currentReason:
      '理想完整口径会保留人员效率单位做解释字段；当前阶段只收敛人数、单人默认日有效分钟、人员标准效率值和工厂效率系数，系统据此自动算出默认日可供给产值。',
  }
}
...
function buildTemplateBGuide(craftName: string): FactorySupplyFormulaGuide {
  return {
    ...
    currentFieldKeys: EXPECTED_OUTPUT_VALUE_CURRENT_FIELD_KEYS_BY_TEMPLATE.B,
    currentFormulaLines: [
      ...
      `默认日可供给产值 = （基础日能力 - ${SETUP_MINUTES_LABEL} - ${SWITCH_MINUTES_LABEL}）× ${EFFICIENCY_FACTOR_LABEL}`,
    ],
    currentExplanationLines: [
    ...
    currentExampleLines: [
      ...
      '基础日能力 = 1176',
      '默认日可供给产值 = (1176 - 20 - 10) × 0.95 = 1088.7',
    ],
    currentReason:
      '理想完整口径会保留设备效率单位和人员效率单位做解释字段；当前阶段只保留最小必要数值字段，系统据此自动算出默认日可供给产值。',
  }
}
...
function buildTemplateCGuide(craftName: string): FactorySupplyFormulaGuide {
  return {
    ...
    currentFieldKeys: EXPECTED_OUTPUT_VALUE_CURRENT_FIELD_KEYS_BY_TEMPLATE.C,
    currentFormulaLines: [
      `设备侧日能力 = ${DEVICE_COUNT_LABEL} × ${DEVICE_SHIFT_MINUTES_LABEL} × ${DEVICE_EFFICIENCY_VALUE_LABEL}`,
      `人员侧日能力 = ${STAFF_COUNT_LABEL} × ${STAFF_SHIFT_MINUTES_LABEL} × ${STAFF_EFFICIENCY_VALUE_LABEL}`,
      '基础日能力 = 设备侧日能力 和 人员侧日能力 里较小的那个',
    ...
    currentExampleLines: [
      ...
      '基础日能力 = 1008',
      '默认日可供给产值 = (1008 - 15 - 10) × 0.9 = 884.7',
    ],
    currentReason:
      '理想完整口径会保留设备效率单位和人员效率单位做连续型速度解释；当前阶段只保留最小必要数值字段，系统据此自动算出默认日可供给产值。',
  }
}
...
function buildTemplateDGuide(craftName: string): FactorySupplyFormulaGuide {
  return {
    ...
    currentFieldKeys: EXPECTED_OUTPUT_VALUE_CURRENT_FIELD_KEYS_BY_TEMPLATE.D,
    currentFormulaLines: [
    ...
    currentExplanationLines: [
      ...
      '固定准备分钟、切换准备分钟和工厂效率系数一起决定当天可兑现的默认日可供给产值。',
    ],
    currentExampleLines: [
      ...
      '基础日能力 = 934',
      '默认日可供给产值 = (934 - 20 - 10) × 0.9 = 813.6',
    ],
    currentReason:
      '理想完整口径会保留装载量单位和人员效率单位做批次口径解释；当前阶段只保留最小必要数值字段，系统据此自动算出默认日可供给产值。',
  }
}
...
export function getExpectedOutputValueCurrentFieldKeysByTemplate(
  template: FactorySupplyFormulaTemplate,
): OutputValueCurrentFieldKey[] {
  return [...EXPECTED_OUTPUT_VALUE_CURRENT_FIELD_KEYS_BY_TEMPLATE[template]]
}
