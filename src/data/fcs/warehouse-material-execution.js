import { listMaterialRequestDraftsByOrder, listMaterialRequests, listMaterialRequestsByOrder, } from './material-request-drafts.ts';
...
function inferProcessCodeByTaskType(taskType) {
    if (taskType === 'PRINT')
        return 'PROC_PRINT';
    if (taskType === 'DYE')
        return 'PROC_DYE';
...
function getDraftLinesForRequest(request) {
    const drafts = listMaterialRequestDraftsByOrder(request.productionOrderNo);
    const draft = drafts.find((item) => item.taskId === request.taskId &&
...
function mapRequestStatusToIssueStatus(requestStatus, targetType) {
    if (requestStatus === '待配料')
        return 'PREPARING';
    if (requestStatus === '待配送' || requestStatus === '待自提')
        return 'READY';
...
function derivePieceName(line) {
    if (!line.pieceSummaryText)
        return undefined;
    const first = line.pieceSummaryText.split('；')[0]?.trim();
    return first || undefined;
}
function deriveSkuScope(line, task) {
...
function createIssueOrTransferFromRequest(request) {
    ...
    const targetType = task?.executorKind === 'WAREHOUSE_WORKSHOP' ? 'WAREHOUSE_WORKSHOP' : 'EXTERNAL_FACTORY';
    ...
    const baseDoc = {
        ...
        remark: targetType === 'WAREHOUSE_WORKSHOP'
            ? '仓内后道执行，使用仓内流转单'
    ...
    if (targetType === 'EXTERNAL_FACTORY') {
        ...
        const issueLines = lines.map((line, index) => {
...
function deriveReturnStatus(task) {
    if (task.status === 'DONE')
        return 'RETURNED';
...
function buildReturnOrdersForOrder(productionOrderId) {
    ...
    return runtimeTasks
        ...
        const returnedQty = status === 'RETURNED' || status === 'CLOSED'
            ? plannedQty
            : status === 'IN_TRANSIT'
...
export function listWarehouseExecutionDocsByRuntimeTaskId(runtimeTaskId) {
    const docs = [
...
export function getWarehouseExecutionDocById(docId, productionOrderId) {
...
export function listWarehouseExecutionDocsByMaterialRequestNo(materialRequestNo) {
    const docs = [
...
function getDocLineMetrics(doc) {
...
export function getWarehouseExecutionLineStatsByOrder(productionOrderId) {
...
export function getWarehouseExecutionSummaryByOrder(productionOrderId) {
