...
// 不作为页面 UI/交互改造入口。
// =============================================
import { listProgressMaterialIssueRows, listProgressMaterialStatementDrafts, } from './store-domain-progress.ts';
import { listRuntimeExecutionTasks, upsertRuntimeTaskTender, } from './runtime-process-tasks';
function buildLegacyMaterialIssueSheetsFromRuntime() {
...
export function listMaterialIssueSheetsFromRuntime() {
    return buildLegacyMaterialIssueSheetsFromRuntime()
        .map((item) => ({ ...item }))
        .sort((left, right) => left.issueId.localeCompare(right.issueId));
}
export function listMaterialStatementDraftsFromRuntime() {
...
const localMaterialStatementAdditions = [];
const localMaterialStatementOverrides = new Map();
function cloneMaterialStatementDraft(draft) {
    return {
        ...draft,
        issueIds: [...draft.issueIds],
        items: draft.items.map((item) => ({ ...item })),
...
export function listMaterialStatementDraftsForSettlement() {
    const baseDrafts = listMaterialStatementDraftsFromRuntime().map(cloneMaterialStatementDraft);
    const merged = baseDrafts.map((draft) => localMaterialStatementOverrides.get(draft.materialStatementId) ?? draft);
...
export function appendMaterialStatementDraftForSettlement(draft) {
    localMaterialStatementAdditions.push(cloneMaterialStatementDraft(draft));
}
export function getMutableMaterialStatementDraftForSettlement(materialStatementId) {
    const localAddition = localMaterialStatementAdditions.find((item) => item.materialStatementId === materialStatementId);
    if (localAddition)
        return localAddition;
    const localOverride = localMaterialStatementOverrides.get(materialStatementId);
    if (localOverride)
        return localOverride;
    const baseDraft = listProgressMaterialStatementDrafts().find((item) => item.materialStatementId === materialStatementId);
    if (!baseDraft)
        return null;
...
function resolveTenderStatus(tasks, deadline) {
    const hasAwarded = tasks.some((task) => task.assignmentStatus === 'AWARDED');
    if (hasAwarded)
        return 'AWARDED';
    const hasActiveBidding = tasks.some((task) => task.assignmentMode === 'BIDDING'
    ...
    if (Number.isFinite(deadlineMs) && Number.isFinite(mockNowMs) && deadlineMs < mockNowMs)
        return 'OVERDUE';
    return 'OPEN';
}
function resolveTenderDeadline(tasks) {
...
function buildLegacyTendersFromRuntime() {
    // 兼容层口径：仅基于当前实际执行任务生成竞价视图，避免拆分来源任务误入执行主线。
    const runtimeTasks = listRuntimeExecutionTasks().filter((task) => Boolean(task.tenderId) && task.defaultDocType !== 'DEMAND');
    ...
    return Array.from(grouped.entries())
        ...
        const invitedFactoryIds = Array.from(new Set(tasks.map((task) => task.assignedFactoryId).filter((factoryId) => Boolean(factoryId))));
...
export function listTendersFromRuntime() {
...
export function getTenderByIdFromRuntime(tenderId) {
    return listTendersFromRuntime().find((item) => item.tenderId === tenderId);
}
export function listTenderOrdersFromRuntime() {
...
export function extendTenderDeadlineFromRuntime(tenderId, extendHours, by) {
...
// ─── 兼容常量导出（非主真相） ──────────────────
export const initialTenders = buildLegacyTendersFromRuntime();
export const initialTenderOrders = listTenderOrdersFromRuntime();
// ─── 历史领料单快照（兼容读取，非主真相） ───────────────
// 主流程请使用 listMaterialIssueSheetsFromRuntime 获取实时映射。
export const legacyMaterialIssueSheetsSnapshot = buildLegacyMaterialIssueSheetsFromRuntime();
// 兼容读取：旧页面若仍按“初始常量”语义读取，可改用本 getter 获取实时映射结果。
export function getInitialMaterialIssueSheetsLegacy() {
    return listMaterialIssueSheetsFromRuntime().map((item) => ({ ...item }));
}
export function getInitialMaterialStatementDraftsLegacy() {
...
export const initialQcStandardSheets = [
