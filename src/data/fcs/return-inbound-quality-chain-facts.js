...
function resolveScenarioReceivedQty(scenario) {
    if (typeof scenario.receivedQty === 'number')
        return scenario.receivedQty;
...
function resolveScenarioDeclaredQty(scenario) {
    return scenario.declaredQty ?? scenario.returnedQty;
}
function resolveScenarioInspectionScene(scenario) {
    if (scenario.inspectionScene)
        return scenario.inspectionScene;
    if (scenario.processType === 'SEW')
        return 'SEW_RETURN_RECEIVING_QC';
    if (scenario.processType === 'PRINT')
        return 'PRINT_RECEIVING_QC';
    if (scenario.processType === 'DYE' || scenario.processType === 'DYE_PRINT')
        return 'DYE_RECEIVING_QC';
    if (scenario.processType === 'CUT_PANEL')
        return 'CUT_PIECE_RECEIVING_QC';
    return 'SEW_RETURN_RECEIVING_QC';
}
function resolveScenarioInspectionMethod(scenario) {
    if (scenario.inspectionMethod)
        return scenario.inspectionMethod;
    if (scenario.processType === 'SEW') {
...
function resolveScenarioNextAction(scenario) {
    if (scenario.nextAction)
        return scenario.nextAction;
    if (scenario.result !== 'PASS')
        return 'WAIT_EXCEPTION_HANDLE';
    const route = resolveScenarioRoute(scenario);
    if (scenario.processType !== 'SEW')
        return 'HANDOVER_FINISHED_WAREHOUSE';
    if (route?.requiresPostExecution)
        return 'ENTER_POST_PROCESS';
    return 'ENTER_FINAL_RECHECK';
}
function resolveScenarioHandoverOrderId(scenario) {
    return scenario.handoverOrderId ?? `HORD-${scenario.taskId}`;
}
function resolveScenarioHandoverRecordIds(scenario) {
    return scenario.handoverRecordIds?.length ? [...scenario.handoverRecordIds] : [`HREC-${scenario.batchId}-01`];
}
function createQcAuditLogs(scenario) {
    const logs = [
        {
...
function resolveBatchStatus(scenario) {
    if (scenario.result === 'PASS') {
...
function createInspection(scenario) {
    ...
    const inspectedQty = scenario.inspectedQty ??
        (inspectionMethod === 'FULL_INSPECTION'
            ? receivedQty
            : inspectionMethod === 'SAMPLING'
                ? scenario.samplingQty ?? Math.max(1, Math.round(receivedQty * 0.2))
                : receivedQty);
    const unqualifiedQty = scenario.unqualifiedQty ??
        (scenario.result === 'PASS'
            ? 0
            : Math.min(scenario.affectedQty ?? scenario.defectItems?.reduce((sum, item) => sum + item.qty, 0) ?? inspectedQty, inspectedQty));
    const qualifiedQty = scenario.qualifiedQty ??
        (scenario.result === 'PASS'
            ? inspectedQty
    ...
    return {
        ...
        inspectionMethod,
        inspectionResult: scenario.result === 'PASS'
            ? 'PASS'
            : qualifiedQty > 0
                ? 'PARTIAL_PASS'
                : 'FAIL',
        ...
        declaredQty,
        receivedQty,
        samplingQty: inspectionMethod === 'SAMPLING'
            ? scenario.samplingQty ?? Math.max(1, Math.round(receivedQty * 0.2))
            : undefined,
        ...
        settlementFreezeReason: scenario.settlementImpact.status === 'NO_IMPACT' || scenario.settlementImpact.status === 'READY' || scenario.settlementImpact.status === 'SETTLED'
            ? ''
...
function createBatch(scenario) {
    ...
    return {
        ...
        processLabel: scenario.processType === 'SEW' && postExecutionMode
...
function createBasis(scenario) {
    if (!scenario.basis)
        return null;
    const liabilityStatusSnapshot = scenario.liabilityStatus === 'DRAFT'
        ? 'PENDING'
...
const LEGACY_RETURN_INBOUND_QC_CHAIN_SCENARIOS = [
    {
...
export const RETURN_INBOUND_QC_CHAIN_SCENARIOS = [...LEGACY_RETURN_INBOUND_QC_CHAIN_SCENARIOS];
const POST_FINAL_RECHECK_SEEDS = [
    {
...
export const returnInboundChainQualityInspections = [
    ...RETURN_INBOUND_QC_CHAIN_SCENARIOS.map(createInspection),
    ...POST_FINAL_RECHECK_SEEDS,
];
export const returnInboundChainBatches = RETURN_INBOUND_QC_CHAIN_SCENARIOS.map(createBatch);
export const returnInboundChainBasisSeeds = RETURN_INBOUND_QC_CHAIN_SCENARIOS
    .map(createBasis)
