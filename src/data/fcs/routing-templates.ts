...
export const routingTemplates: RoutingTemplate[] = [
  ...
  // 6. 含定位裁（激光切）/绣花
  {
    ...
    status: 'ACTIVE',
    version: 'v1.0',
    description: '适用于需要定位裁（激光切）和绣花的复杂款式，裁床由主裁床工厂执行，绣花建议竞价',
    tags: ['定位裁（激光切）', '绣花', '裁床工艺', '复杂'],
    applicableCategory: '毛织',
    matchRule: {
      mode: 'AUTO',
      requiredProcessCodes: ['PROC_CUT', 'PROC_EMBROIDER', 'PROC_SEW'],
      optionalProcessCodes: ['PROC_FINISHING'],
      keywords: ['激光', '绣花', '刺绣', '镂空', '特种'],
    },
    steps: [
      { seq: 1, processCode: 'PROC_CUT', craftName: '定位裁（激光切）', assignmentMode: 'DIRECT', ownerSuggestion: { kind: 'MAIN_FACTORY' } },
      { seq: 2, processCode: 'PROC_EMBROIDER', assignmentMode: 'BIDDING', ownerSuggestion: { kind: 'RECOMMENDED_FACTORY_POOL', recommendedTier: 'CENTRAL', recommendedTypes: ['EMBROIDERY'] }, dependsOnSeq: [1] },
      { seq: 3, processCode: 'PROC_SEW', assignmentMode: 'DIRECT', ownerSuggestion: { kind: 'MAIN_FACTORY' }, dependsOnSeq: [2] },
      { seq: 4, processCode: 'PROC_FINISHING', assignmentMode: 'DIRECT', ownerSuggestion: { kind: 'MAIN_FACTORY' }, dependsOnSeq: [3] },
    ],
