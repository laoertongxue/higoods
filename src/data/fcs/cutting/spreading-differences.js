...
function sumRolls(rolls, getter) {
    return round(rolls.reduce((sum, roll) => sum + Math.max(Number(getter(roll) || 0), 0), 0));
}
function resolveSessionForOrder(order, sessions) {
    return (sessions.find((session) => session.spreadingSessionId === order.spreadingOrderId) ||
...
function resolveOrderStatusFromSession(session) {
    if (session.cuttingStatus === 'CUTTING_DONE')
        return 'CUT_DONE';
    if (session.cuttingStatus === 'CUTTING')
        return 'CUTTING';
    if (session.cuttingStatus === 'WAITING_CUTTING')
...
function buildOrdersFromSessions(sessions) {
    if (!sessions.length)
        return [];
    ...
    return sessions.map((session) => {
        ...
        const plannedGarmentQty = round(session.planUnits?.reduce((sum, unit) => sum + Math.max(Number(unit.plannedCutGarmentQty || 0), 0), 0) ||
            session.theoreticalCutGarmentQty ||
            session.theoreticalActualCutPieceQty ||
            0);
        const plannedMaterialUsage = round(session.theoreticalSpreadTotalLength ||
...
function getActualCutQty(session) {
    if (!session)
        return 0;
    return round(Number(session.actualCutGarmentQty ?? session.actualCutPieceQty ?? 0) ||
        session.rolls.reduce((sum, roll) => sum + Number(roll.actualCutGarmentQty ?? roll.actualCutPieceQty ?? 0), 0));
}
function buildDifferenceBase(order, session) {
    ...
    const detectedBy = session?.operators[0]?.operatorName ||
...
function createDifference(order, session, input) {
...
function buildDifferencesFromOrders(orders, sessions) {
    ...
    return orders.flatMap((order) => {
        ...
        const abnormalRolls = session?.rolls.filter((roll) => !roll.rollNo ||
...
function buildDifferencesFromStageWritebacks(orders) {
...
function buildDifferencesFromPdaFeedbacks(orders) {
...
function buildSeedDifferences(orders) {
    const sourceRecords = listGeneratedCutOrderSourceRecords();
    const fallbackSource = sourceRecords[0];
    const pb2440Sources = sourceRecords.filter((record) => ['CUT-260307-102-01', 'CUT-260307-102-02', 'CUT-260307-102-03'].includes(record.cutOrderNo));
...
function uniqueByDifferenceId(differences) {
    const seen = new Set();
    return differences.filter((difference) => {
        if (seen.has(difference.differenceId))
            return false;
...
export function listSpreadingDifferencesBySpreadingOrder(spreadingOrderIdOrNo, input) {
    return listSpreadingDifferences(input).filter((difference) => difference.spreadingOrderId === spreadingOrderIdOrNo ||
        difference.spreadingOrderNo === spreadingOrderIdOrNo ||
        difference.sourceObjectId === spreadingOrderIdOrNo);
}
export function listSpreadingDifferencesByProductionOrder(productionOrderIdOrNo, input) {
    return listSpreadingDifferences(input).filter((difference) => difference.productionOrderIds.includes(productionOrderIdOrNo) ||
        difference.productionOrderNos.includes(productionOrderIdOrNo));
}
export function buildSpreadingReplenishmentHandlingObjects(differences = listSpreadingDifferences()) {
