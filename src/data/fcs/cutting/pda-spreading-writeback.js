...
function normalizeSpreadingMode(value) {
    if (value === 'HIGH_LOW')
        return 'HIGH_LOW';
    if (value === 'FOLD_HIGH_LOW')
        return 'FOLD_HIGH_LOW';
    if (value === 'FOLD_NORMAL' || value === 'FOLD')
        return 'FOLD_NORMAL';
    return 'NORMAL';
}
function mapWritebackModeToSessionMode(mode) {
    if (mode === 'HIGH_LOW')
        return 'high_low';
...
function mapSessionModeToWritebackMode(mode) {
    if (mode === 'high_low')
        return 'HIGH_LOW';
    if (mode === 'fold_high_low')
        return 'FOLD_HIGH_LOW';
...
function buildWritebackTimestampId(prefix, input = new Date(), suffix = Math.random().toString(16).slice(2, 6)) {
...
export function buildSettlementReserveFields(payload) {
...
export function normalizePdaWritebackPayload(rawPayload) {
    const raw = (rawPayload ?? {});
    const writebackId = String(raw.writebackId || buildWritebackTimestampId('pda-writeback'));
    const planUnits = toArray(raw.planUnits).map((item, index) => ({
    ...
    const rollItems = toArray(raw.rollItems).map((item, index) => {
    ...
    const operatorItems = toArray(raw.operatorItems).map((item, index) => ({
    ...
    const normalized = {
        ...
        recordType: raw.recordType || '开始铺布',
        cutOrderIds: uniqueStrings(toArray(raw.cutOrderIds)),
        cutOrderNos: uniqueStrings(toArray(raw.cutOrderNos)),
        markerPlanId: String(raw.markerPlanId || ''),
        markerPlanNo: String(raw.markerPlanNo || ''),
        productionOrderNos: uniqueStrings(toArray(raw.productionOrderNos)),
        ...
        rollItems,
        operatorItems,
        validationIssues: uniqueStrings(toArray(raw.validationIssues)),
        warningMessages: uniqueStrings(toArray(raw.warningMessages)),
...
export function validatePdaWritebackPayload(writeback) {
    const issues = [];
    ...
    const invalidOperator = writeback.operatorItems.find((item) => !item.operatorName || !item.operatorAccountId || !item.rollWritebackItemId);
    if (invalidOperator) {
...
export function matchWritebackToSpreadingContext(writeback, context) {
...
export function compareWritebackWithExistingSession(writeback, sessions) {
...
export function resolvePdaWritebackStatus(writeback, context, sessions, honorTerminalStatus = true) {
...
function toSpreadingSourceChannel(session) {
    if (!session)
        return 'PDA_WRITEBACK';
    if (session.sourceChannel === 'PDA_WRITEBACK' || session.sourceChannel === 'MIXED')
        return session.sourceChannel;
    return 'MIXED';
}
function sessionMatchesWritebackContext(session, writeback) {
    if (writeback.contextType === 'marker-plan-ref') {
        if (writeback.markerPlanId && session.markerPlanId === writeback.markerPlanId)
            return true;
        if (writeback.markerPlanNo && session.markerPlanNo === writeback.markerPlanNo)
            return true;
        return false;
    }
    return writeback.cutOrderIds.some((id) => session.cutOrderIds.includes(id));
}
function findWritebackCandidateSessions(store, writeback) {
    return store.sessions.filter((session) => sessionMatchesWritebackContext(session, writeback));
}
function findTargetSession(store, writeback) {
    ...
    if (writeback.markerId || writeback.markerNo) {
        const byMarker = store.sessions.find((session) => sessionMatchesWritebackContext(session, writeback)
            && ((writeback.markerId && session.markerId === writeback.markerId) || (writeback.markerNo && session.markerNo === writeback.markerNo)));
        if (byMarker)
            return byMarker;
    }
    const inProgress = store.sessions.find((session) => sessionMatchesWritebackContext(session, writeback) && session.status === 'IN_PROGRESS');
    if (inProgress)
        return inProgress;
    return null;
}
function createSessionFromWriteback(writeback, now = new Date()) {
...
function buildRollFromWriteback(writeback, item, sessionId, existingRollRecordId) {
    return {
...
function buildOperatorFromWriteback(writeback, item, sessionId, rollRecordId, existingOperatorRecordId) {
    return {
...
export function applyWritebackToSpreadingSession(options) {
    ...
    let nextSession = {
        ...
        materialSkuSummary: targetSession.materialSkuSummary
        ...
        planUnits: Array.isArray(targetSession.planUnits) && targetSession.planUnits.length
    ...
    const rollRecordIdByWritebackItemId = new Map();
    ...
    for (const item of options.writeback.operatorItems) {
        const rollRecordId = rollRecordIdByWritebackItemId.get(item.rollWritebackItemId)
            || nextSession.rolls.find((roll) => roll.sourceWritebackId === options.writeback.writebackId)?.rollRecordId
            || '';
        const existingIndex = nextSession.operators.findIndex((operator) => (operator.operatorAccountId
        ...
        if (existingIndex === -1) {
            const nextOperator = buildOperatorFromWriteback(options.writeback, item, nextSession.spreadingSessionId, rollRecordId);
            nextSession = {
                ...nextSession,
        ...
        const nextOperator = buildOperatorFromWriteback(options.writeback, item, nextSession.spreadingSessionId, rollRecordId || existingOperator.rollRecordId, existingOperator.operatorRecordId);
        nextSession = {
            ...nextSession,
    ...
    const audit = buildWritebackAuditTrail({
        ...
        note: createdRollCount || updatedRollCount || createdOperatorCount || updatedOperatorCount
...
export function buildPdaSupplementDraft(writeback) {
    return {
...
export function serializePdaWritebackStorage(store) {
    return JSON.stringify(store);
}
export function deserializePdaWritebackStorage(raw) {
    if (!raw)
    ...
    try {
        ...
        return {
            ...
            auditTrails: toArray(parsed?.auditTrails).map((item, index) => ({
                auditTrailId: String(item.auditTrailId || `audit-${index + 1}`),
                writebackId: String(item.writebackId || ''),
                action: item.action || 'IMPORT',
                actionBy: String(item.actionBy || '系统'),
                actionAt: String(item.actionAt || ''),
                targetSessionId: String(item.targetSessionId || ''),
                note: String(item.note || ''),
            })),
        };
    }
...
export function hydrateIncomingPdaWritebacks(storage) {
    return deserializePdaWritebackStorage(storage.getItem(CUTTING_PDA_WRITEBACK_STORAGE_KEY));
}
export function buildPdaWritebackStats(writebacks) {
...
export function buildPdaWritebackTraceMatrix(options) {
    return options.writebacks
        .flatMap((writeback) => {
        const matchedSession = options.sessions.find((session) => session.spreadingSessionId === writeback.spreadingSessionId
            || session.sourceWritebackId === writeback.writebackId
            || session.rolls.some((roll) => roll.sourceWritebackId === writeback.writebackId)
            || session.operators.some((operator) => operator.sourceWritebackId === writeback.writebackId)) || null;
        return writeback.rollItems.map((rollItem) => {
            const matchedRoll = matchedSession?.rolls.find((roll) => roll.sourceWritebackId === writeback.writebackId
                ...
                || null;
            const matchedOperatorItem = writeback.operatorItems.find((item) => item.rollWritebackItemId === rollItem.rollWritebackItemId) || null;
            const matchedOperator = matchedSession?.operators.find((operator) => operator.sourceWritebackId === writeback.writebackId
                && operator.rollRecordId === (matchedRoll?.rollRecordId || operator.rollRecordId))
                || matchedSession?.operators.find((operator) => operator.rollRecordId === (matchedRoll?.rollRecordId || '')
        ...
        .sort((left, right) => left.writebackId.localeCompare(right.writebackId, 'zh-CN')
