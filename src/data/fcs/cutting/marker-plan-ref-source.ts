import { listCutOrderSourceRecords, normalizeMarkerPlanRefId } from './cut-order-source.ts'

export const CUTTING_MARKER_PLAN_REF_LEDGER_STORAGE_KEY = 'cuttingMarkerPlanRefLedger'

export interface MarkerPlanRefSourceRecord {
  markerPlanId: string
  markerPlanNo: string
...
interface StoredMarkerPlanRefItem {
  cutOrderId?: string
  cutOrderNo?: string
...
interface StoredMarkerPlanRefRecord {
  markerPlanId?: string
  markerPlanNo?: string
  items?: StoredMarkerPlanRefItem[]
}
...
function readStoredMarkerPlanRefSourceRecords(): MarkerPlanRefSourceRecord[] {
  if (typeof localStorage === 'undefined') return []

  try {
    const raw = localStorage.getItem(CUTTING_MARKER_PLAN_REF_LEDGER_STORAGE_KEY)
    if (!raw) return []
    const parsed = JSON.parse(raw)
    if (!Array.isArray(parsed)) return []

    return parsed
      .filter((item): item is StoredMarkerPlanRefRecord => Boolean(item && typeof item === 'object'))
      .map((item) => {
        const markerPlanNo = typeof item.markerPlanNo === 'string' ? item.markerPlanNo.trim() : ''
        const markerPlanId = typeof item.markerPlanId === 'string' && item.markerPlanId.trim() ? item.markerPlanId.trim() : normalizeMarkerPlanRefId(markerPlanNo)
...
function buildSystemMarkerPlanRefSourceRecords(): MarkerPlanRefSourceRecord[] {
  const grouped = new Map<string, MarkerPlanRefSourceRecord>()
  const sourceRecords = listCutOrderSourceRecords()

  sourceRecords.forEach((record) => {
    if (!record.markerPlanNo) return
    const markerPlanId = record.markerPlanId || normalizeMarkerPlanRefId(record.markerPlanNo)
  ...
  Array.from(rowsBySpuAndProductionOrder.values())
    ...
    .forEach((rows, index) => {
      ...
      const markerPlanId = normalizeMarkerPlanRefId(markerPlanNo)
...
export function listMarkerPlanRefSourceRecords(): MarkerPlanRefSourceRecord[] {
  const merged = new Map<string, MarkerPlanRefSourceRecord>()
  ...
  const sourceRecords = [...buildSystemMarkerPlanRefSourceRecords(), ...readStoredMarkerPlanRefSourceRecords()]
    .map((record) => normalizeRecordToFormalCuttingSources(record, rowById, rowByNo))
    .filter((record): record is MarkerPlanRefSourceRecord => record !== null)

  sourceRecords.forEach((record) => {
    const key = record.markerPlanId || normalizeMarkerPlanRefId(record.markerPlanNo)
    if (!key) return
