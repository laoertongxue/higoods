import { CUTTING_QR_VERSION, buildCarrierQrPayload, buildFeiTicketQrPayload, buildCutOrderQrPayload, deserializeCuttingQrPayload, serializeCuttingQrPayload, } from './qr-payload.ts';
function sanitizeFragment(value) {
    return value.trim().replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'na';
}
export function buildCutOrderQrValue(cutOrderId) {
    return `FCS:CUT_ORDER:v1:${sanitizeFragment(cutOrderId)}`;
}
export function getCutOrderByQrValue(qrValue) {
    if (!qrValue.startsWith('FCS:CUT_ORDER:v1:'))
        return null;
    const cutOrderId = qrValue.slice('FCS:CUT_ORDER:v1:'.length).trim();
    return cutOrderId ? { cutOrderId } : null;
}
export function buildCuttingTraceabilityId(prefix, issuedAt, ...parts) {
...
export function encodeCutOrderQr(input) {
    const payload = buildCutOrderQrPayload(input);
    return { payload, qrValue: serializeCuttingQrPayload(payload) };
}
export function encodeFeiTicketQr(input) {
    const payload = buildFeiTicketQrPayload(input);
    return { payload, qrValue: serializeCuttingQrPayload(payload) };
}
export function encodeCarrierQr(input) {
    const payload = buildCarrierQrPayload(input);
    return { payload, qrValue: serializeCuttingQrPayload(payload) };
}
export function parseCuttingTraceQr(value) {
    return deserializeCuttingQrPayload(value);
}
export function validateFeiCraftSequence(payload, currentCraftType, completedCrafts = []) {
...
export function summarizeTraceabilityPayload(payload) {
