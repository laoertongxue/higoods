import { buildSpreadingTraceAnchors, deriveSpreadingSupervisorStage, } from '../../../pages/process-factory/cutting/marker-spreading-model.ts';
...
export { CUTTING_MARKER_SPREADING_LEDGER_STORAGE_KEY, buildSpreadingTraceAnchors, buildReplenishmentPreview, createEmptyStore, deserializeMarkerSpreadingStorage, serializeMarkerSpreadingStorage, upsertSpreadingSession, } from '../../../pages/process-factory/cutting/marker-spreading-model.ts';
...
function buildSeedTransferBagStore() {
    ...
    const feiTraceById = new Map(feiTraceRows.map((row) => [row.feiTicketId, row]));
...
function resolveSessionStageKey(session, options) {
    const lifecycleOverrides = session.prototypeLifecycleOverrides || null;
    const pendingReplenishmentConfirmation = lifecycleOverrides?.replenishmentStatusLabel === '待补料确认'
    ...
    const feiTicketReady = lifecycleOverrides?.feiTicketStatusLabel
        ? lifecycleOverrides.feiTicketStatusLabel === '已打印菲票'
        : options.hasFeiTicket;
    const baggingReady = lifecycleOverrides?.baggingStatusLabel
        ? lifecycleOverrides.baggingStatusLabel === '已装袋'
        : options.hasBagging;
    const warehouseReady = lifecycleOverrides?.warehouseStatusLabel
...
function gateDownstreamAnchorsByStage(stageKey, anchors) {
...
export function buildCuttingSpreadingFlowMatrix() {
    const prototypeData = readMarkerSpreadingPrototypeData();
    const anchors = buildSpreadingTraceAnchors(prototypeData.store);
    const anchorBySessionId = new Map(anchors.map((item) => [item.spreadingSessionId, item]));
    const replenishmentRows = buildReplenishmentFlowTraceMatrix();
    const replenishmentRowsBySessionId = replenishmentRows.reduce((acc, row) => {
        if (!row.sourceSpreadingSessionId)
            return acc;
        if (!acc[row.sourceSpreadingSessionId])
            acc[row.sourceSpreadingSessionId] = [];
        acc[row.sourceSpreadingSessionId].push(row);
        return acc;
    }, {});
    const feiRows = buildSpreadingDrivenFeiTicketTraceMatrix();
    const feiRowsBySessionId = feiRows.reduce((acc, row) => {
        if (!row.sourceSpreadingSessionId)
            return acc;
        if (!acc[row.sourceSpreadingSessionId])
            acc[row.sourceSpreadingSessionId] = [];
    ...
    const transferRowsBySessionId = transferRows.reduce((acc, row) => {
        if (!row.sourceSpreadingSessionId)
            return acc;
        if (!acc[row.sourceSpreadingSessionId])
            acc[row.sourceSpreadingSessionId] = [];
    ...
    const warehouseRowsBySessionId = warehouseRows.reduce((acc, row) => {
        if (!row.spreadingSessionId)
            return acc;
        if (!acc[row.spreadingSessionId])
            acc[row.spreadingSessionId] = [];
    ...
    return prototypeData.store.sessions
        ...
        const warehouse = warehouseRowsBySessionId[session.spreadingSessionId]?.length
            ? warehouseRowsBySessionId[session.spreadingSessionId] || []
            : warehouseRows.filter((item) => session.cutOrderIds.includes(item.cutOrderId)
        ...
        const fallbackReplenishmentRequestId = stageKey === 'WAITING_REPLENISHMENT' && session.replenishmentWarning && !session.replenishmentWarning.handled
        ...
        return {
            ...
            replenishmentRequestId: (stageKey === 'WAITING_REPLENISHMENT' ? pendingRows[0] : pendingRows[0] || approvedRows[0])?.replenishmentRequestId
        ...
        .sort((left, right) => left.sessionNo.localeCompare(right.sessionNo, 'zh-CN')
