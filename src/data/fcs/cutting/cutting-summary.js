...
const progressRecordMap = new Map(cuttingOrderProgressRecords.map((record) => [record.productionOrderId, record]));
...
function sumSkuQty(order) {
    return order.demandSnapshot.skuLines.reduce((sum, line) => sum + Number(line.qty || 0), 0);
}
function deriveUrgencyLevel(order, progressRecord) {
    if (progressRecord)
        return progressRecord.urgencyLevel;
    const requiredDeliveryDate = order.demandSnapshot.requiredDeliveryDate;
    if (!requiredDeliveryDate)
        return 'C';
    if (requiredDeliveryDate <= '2026-03-24')
        return 'AA';
    if (requiredDeliveryDate <= '2026-03-28')
        return 'A';
    if (requiredDeliveryDate <= '2026-04-05')
        return 'B';
    return 'C';
}
function buildIssueFlags(issues) {
    return unique(issues.map((item) => item.title));
}
function issueSourceMatches(issue, filter) {
    if (filter === 'ALL')
        return true;
    if (filter === 'PREP')
        return issue.sourcePage === 'MATERIAL_PREP';
    if (filter === 'EXECUTION')
        return issue.sourcePage === 'CUT_PIECE_ORDER';
    if (filter === 'REPLENISHMENT')
        return issue.sourcePage === 'REPLENISHMENT';
    if (filter === 'WAREHOUSE')
        return issue.sourcePage === 'WAREHOUSE';
    return issue.sourcePage === 'SAMPLE';
}
function getProductionCutOrderSources(productionOrderId) {
    return cutOrderSourceRecords.filter((item) => item.productionOrderId === productionOrderId);
}
function getProductionFabricWarehouseRecords(productionOrderId) {
    return formalFabricWarehouseRecords.filter((item) => item.productionOrderId === productionOrderId);
}
function getProductionCutPieceWarehouseRecords(productionOrderId) {
    return formalCutPieceWarehouseRecords.filter((item) => item.productionOrderId === productionOrderId);
}
function getProductionSampleWarehouseRecords(productionOrderId) {
    return formalSampleWarehouseRecords.filter((item) => item.productionOrderId === productionOrderId);
}
function buildMaterialSummary(progressRecord) {
...
function buildReceiveSummary(progressRecord) {
    const materialLines = progressRecord.materialLines;
...
function buildMarkerSummary(progressRecord) {
...
function buildSpreadingSummary(progressRecord) {
...
function buildReplenishmentSummary(progressRecord) {
...
function buildWarehouseSummary(records) {
    return {
...
function buildSampleSummary(records) {
    return {
...
function buildPlatformStageSummary(args) {
    if (args.replenishmentSummary.pendingReviewCount > 0)
        return '补料建议待审核，需先收口执行缺口。';
    if (args.warehouseSummary.cutPiecePendingInboundCount > 0 || args.warehouseSummary.unassignedZoneCount > 0)
        return '裁片仓入仓 / 分区仍待收口。';
    if (args.sampleSummary.sampleWaitingReturnCount > 0)
        return '样衣流转尚未收口，需继续回仓。';
    if (args.receiveSummary.receivedPartialCount > 0 || args.receiveSummary.notReceivedCount > 0)
        return '裁床领料入待加工仓仍在执行中。';
...
function buildOverallSummaryStatus(args) {
    if (args.replenishmentSummary.pendingReviewCount > 0 || args.replenishmentSummary.needMoreInfoCount > 0)
        return 'PENDING_REPLENISHMENT';
    if (args.sampleSummary.sampleWaitingReturnCount > 0 || args.sampleSummary.overdueReturnCount > 0)
        return 'PENDING_SAMPLE_RETURN';
    if (args.warehouseSummary.cutPiecePendingInboundCount > 0 || args.warehouseSummary.unassignedZoneCount > 0 || args.warehouseSummary.waitingHandoverCount > 0)
        return 'PENDING_WAREHOUSE_HANDOVER';
    if (args.markerSummary.pendingMarkerCount > 0 || args.spreadingSummary.pendingSpreadingCount > 0)
        return 'PENDING_EXECUTION_CLOSURE';
    if (args.materialSummary.partiallyConfiguredCount > 0 || args.receiveSummary.receivedPartialCount > 0 || args.receiveSummary.notReceivedCount > 0)
        return 'PENDING_PREP_CLOSURE';
...
function buildRecord(order) {
    ...
    if (!progressRecord && cutOrderSources.length === 0)
        return null;
    ...
    const overallRiskLevel = issues.some((item) => item.level === 'HIGH')
        ? 'HIGH'
...
export const cuttingSummaryRecords = productionOrders
    .map((order) => buildRecord(order))
    .filter((record) => record !== null);
export function cloneCuttingSummaryRecords() {
...
export function filterIssuesBySource(records, filter) {
    if (filter === 'ALL')
        return records;
