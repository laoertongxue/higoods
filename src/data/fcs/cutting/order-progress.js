...
function pickProfile(orderIndex) {
    return DEMO_STAGE_PROFILES[orderIndex % DEMO_STAGE_PROFILES.length];
}
function buildSkuRequirementLines(order) {
...
function deriveUrgencyLevel(requiredDeliveryDate) {
...
function estimateLength(requiredQty) {
    return Number(Math.max(requiredQty * 0.42, 1).toFixed(2));
}
function estimateRollCount(requiredQty) {
    return Math.max(Math.ceil(requiredQty / 1800), 1);
}
function buildPieceProgressLines(generated, profile) {
    if (!profile.hasSpreadingRecord)
        return undefined;
    const pieces = generated.pieceRows.length
        ? generated.pieceRows
        : [{ partCode: 'piece-main', partName: '裁片', pieceCountPerUnit: 1 }];
    return generated.skuScopeLines.flatMap((skuLine) => pieces.map((piece) => {
...
function buildProjectedMaterialLine(generated, profile) {
...
function buildProjectedRecord(order, generatedCutOrderRecords, orderIndex) {
...
export const cuttingOrderProgressRecords = formalCuttingProductionOrders
    .map((order, orderIndex) => {
    const generatedCutOrderRecords = generatedCutOrders.filter((item) => item.productionOrderId === order.productionOrderId);
    if (generatedCutOrderRecords.length === 0)
        return null;
    return buildProjectedRecord(order, generatedCutOrderRecords, orderIndex);
})
    .filter((record) => record !== null);
export function updateCuttingOrderProgressWebStage(cutOrderId, payload) {
    const record = cuttingOrderProgressRecords.find((item) => item.materialLines.some((line) => line.cutOrderId === cutOrderId ||
