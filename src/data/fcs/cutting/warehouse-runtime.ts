...
function buildFormalWarehouseRuntimeCache(): FormalWarehouseRuntimeCache {
  ...
  const fabric: CuttingFabricStockRecord[] = cutOrderSources.map((source, index) => {
    ...
    return {
      ...
      productionOrderNo,
      markerPlanId: cutOrderRef?.activeMarkerPlanSourceId || '',
      markerPlanNo: cutOrderRef?.activeMarkerPlanSourceNo || '',
      ...
      note: usage.fabricState === 'NEED_RECHECK' ? '当前裁床接收 / 使用记录存在差异，需继续核对。' : '当前仓务读链已切换到正式裁片单主码。',
    }
  })
  ...
  const cutPiece: CutPieceWarehouseRecord[] = cutOrderSources.map((source, index) => {
    ...
    return {
      ...
      cutPieceOrderNo: source.cutOrderNo,
      materialSku: source.materialSku,
      groupNo: cutOrderRef?.activeMarkerPlanSourceNo || `${source.materialSku}-主组`,
      zoneCode,
  ...
  if (firstNeedRecheck) {
    alerts.push({
      ...
      level: 'HIGH',
      title: '裁床仓余量待核对',
      description: `${firstNeedRecheck.materialSku} 当前存在裁床接收或余量差异待核对。`,
