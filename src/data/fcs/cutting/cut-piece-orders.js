...
function normalizeCutPieceOrderRecord(record) {
    ...
    const matchedCutOrders = Object.values(cuttingCoreRegistry.cutOrdersById).filter((item) => item.productionOrderNo === (productionRef?.productionOrderNo || record.productionOrderNo)
...
export const cutPieceOrderRecords = rawCutPieceOrderRecords.map(normalizeCutPieceOrderRecord);
export function cloneCutPieceOrderRecords() {
...
export function updateCutPieceOrderWebStage(cutOrderId, payload) {
    const record = cutPieceOrderRecords.find((item) => item.cutOrderId === cutOrderId || item.cutOrderNo === cutOrderId || item.id === cutOrderId);
    if (!record)
        return undefined;
