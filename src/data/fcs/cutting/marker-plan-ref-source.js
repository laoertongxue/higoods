...
function readStoredMarkerPlanRefSourceRecords() {
...
function buildSystemMarkerPlanRefSourceRecords() {
    const grouped = new Map();
    const sourceRecords = listCutOrderSourceRecords();
    sourceRecords.forEach((record) => {
        if (!record.markerPlanNo)
            return;
    ...
    const rowsBySpuAndProductionOrder = new Map();
    sourceRecords.forEach((record) => {
...
function normalizeRecordToFormalCuttingSources(record, rowById, rowByNo) {
...
export function listMarkerPlanRefSourceRecords() {
    const merged = new Map();
    const formalCutOrderRows = listCutOrderSourceRecords();
    const rowById = new Map(formalCutOrderRows.map((row) => [row.cutOrderId, row]));
    const rowByNo = new Map(formalCutOrderRows.map((row) => [row.cutOrderNo, row]));
