...
import { normalizeCarrierCycleItemBinding, normalizeTransferBagDispatchManifest, normalizeTransferCarrierCycleRecord, normalizeTransferCarrierRecord, normalizeTransferCarrierSeedTicket, } from './transfer-carrier-normalizer.ts';
...
function buildCycleStatus(cycleStatus) {
    if (cycleStatus === 'READY_TO_DISPATCH' || cycleStatus === 'PACKING' || cycleStatus === 'DRAFT')
        return 'loaded';
    if (cycleStatus === 'DISPATCHED' || cycleStatus === 'PENDING_SIGNOFF')
        return 'handed_over';
    if (cycleStatus === 'WAITING_RETURN' || cycleStatus === 'RETURN_INSPECTING')
        return 'returned';
    if (cycleStatus === 'CLOSED')
        return 'recycled';
    if (cycleStatus === 'EXCEPTION_CLOSED')
        return 'disabled';
    return 'idle';
}
function mergeById(seedItems, storedItems, idKey) {
...
function normalizeMergedSewingTasks(seedTasks, storedTasks) {
...
function syncCycleFactoryNames(usages, sewingTasks) {
...
function syncManifestFactoryNames(manifests, usages) {
...
function buildCarrierCycleId(carrierId, sewingTaskId, startedAt) {
    return buildCuttingTraceabilityId('carrier-cycle', startedAt, carrierId, sewingTaskId);
}
function buildCarrierBindingId(cycleId, feiTicketId, boundAt) {
    return buildCuttingTraceabilityId('carrier-bind', boundAt, cycleId, feiTicketId);
}
function buildDispatchManifestId(cycleId, createdAt) {
...
export function createCarrierCycleBinding(options) {
    const normalizedTicket = normalizeTransferCarrierSeedTicket(options.ticket);
    return {
...
export function buildSystemSeedTransferBagRuntime(options) {
    ...
    function pickInboundTempTickets(count) {
        ...
        printedTickets.forEach((ticket) => {
            if (picked.length >= count)
                return;
            if (!picked.some((item) => item.feiTicketId === ticket.feiTicketId))
                picked.push(ticket);
        });
        return picked;
    }
    function pushUsageAudit(cycle, actionAt, action, actionBy, note) {
        auditTrail.push({
    ...
    function pushReturnAudit(cycle, actionAt, action, actionBy, payloadSummary, note) {
    ...
    function addSeedCycle(options) {
        ...
        const cycleBindings = selectedTickets.map((ticket) => createCarrierCycleBinding({
            cycle,
            carrier,
            ticket,
        ...
        if (options.manifestAt && cycleBindings.length) {
            manifests.push(createCarrierDispatchManifest({
                cycle,
        ...
        if (options.returnAt) {
            ...
            pushReturnAudit(cycle, options.returnAt, '回收登记', options.receivedBy || '回收验收员', `回收点：${options.returnWarehouseName || '裁片仓回收点'}`, options.returnNote || '已完成回收登记。');
        }
        if (options.inspectedAt && options.reusableDecision) {
            ...
            pushReturnAudit(cycle, options.inspectedAt, '完成回收', options.inspectedBy || '回收验收员', `袋况：${options.conditionStatus || 'GOOD'}`, options.conditionNote || '袋况检查完成。');
        }
        if (options.closedAt && options.closureStatus && options.nextBagStatus) {
            ...
            pushReturnAudit(cycle, options.closedAt, '关闭本次周转', options.closedBy || '周转班长', `下一状态：${options.nextBagStatus}`, options.closureReason || '已关闭本轮周转。');
        }
        const keepActive = options.active ?? ['DRAFT', 'PACKING', 'READY_TO_DISPATCH', 'DISPATCHED', 'PENDING_SIGNOFF', 'WAITING_RETURN', 'RETURN_INSPECTING'].includes(options.cycleStatus);
        updateMasterRecord({
...
export function deserializeTransferBagRuntimeStorage(raw) {
    if (!raw)
        return emptyStore();
    try {
        ...
        const cyclesById = Object.fromEntries(rawCycles.map((item) => {
...
export function serializeTransferBagRuntimeStorage(store) {
    return JSON.stringify(store);
}
export function mergeTransferBagRuntimeStores(seed, stored) {
    const sewingTasks = normalizeMergedSewingTasks(seed.sewingTasks, stored.sewingTasks);
    const usages = syncCycleFactoryNames(mergeById(seed.usages, stored.usages, 'cycleId'), sewingTasks);
    return {
        masters: mergeById(seed.masters, stored.masters, 'carrierId'),
        usages,
        bindings: mergeById(seed.bindings, stored.bindings, 'bindingId'),
        manifests: syncManifestFactoryNames(mergeById(seed.manifests, stored.manifests, 'manifestId'), usages),
        sewingTasks,
...
export function deserializeTransferBagSelectedTicketIds(raw) {
    if (!raw)
        return [];
    try {
        const parsed = JSON.parse(raw);
...
export function serializeTransferBagSelectedTicketIds(ids) {
    return JSON.stringify(ids);
}
export function buildTransferBagRuntimeTraceMatrix(store) {
    return store.bindings
        ...
        .sort((left, right) => left.usageId.localeCompare(right.usageId, 'zh-CN')
        || left.feiTicketId.localeCompare(right.feiTicketId, 'zh-CN'));
}
export function buildSpreadingDrivenTransferBagTraceMatrix(store) {
    return buildTransferBagRuntimeTraceMatrix(store).filter((row) => Boolean(row.sourceSpreadingSessionId));
}
export function parseCarrierQrValue(value) {
    const payload = parseCuttingTraceQr(value);
    if (!payload || payload.codeType !== 'CARRIER')
        return null;
    return {
