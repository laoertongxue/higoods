import { buildCuttingMarkerPlanRefRouteCardPrintDocument, buildCuttingCutOrderRouteCardPrintDocument, buildDyeingWorkOrderRouteCardPrintDocument, buildLegacyTaskRouteCardPrintDocument, buildPostFinishingTaskRouteCardPrintDocument, buildPrintingWorkOrderRouteCardPrintDocument, buildRuntimeTaskRouteCardPrintDocument, buildSpecialCraftTaskOrderRouteCardPrintDocument, renderTaskRouteCardTemplate, } from '../../pages/print/templates/task-route-card-template.ts';
import { buildPostFinishingRouteCardPrintDocument, renderPostFinishingRouteCardTemplate, } from '../../pages/print/templates/post-finishing-route-card-template.ts';
import { buildTaskDeliveryCardPrintDocument, renderTaskDeliveryCardTemplate, } from '../../pages/print/templates/task-delivery-card-template.ts';
import { buildIssueSlipPrintDocument, buildMaterialPrepSlipPrintDocument, buildPickupSlipPrintDocument, buildSupplementMaterialSlipPrintDocument, renderMaterialSlipTemplate, } from '../../pages/print/templates/material-slip-template.ts';
import { buildCuttingOrderQrLabelPrintDocument, buildFeiTicketLabelPrintDocument, buildFeiTicketReprintLabelPrintDocument, buildFeiTicketVoidLabelPrintDocument, buildHandoverQrLabelPrintDocument, buildTransferBagLabelPrintDocument, renderLabelPrintTemplate, } from '../../pages/print/templates/label-print-template.ts';
import { buildProductionConfirmationPrintDocument, renderProductionConfirmationTemplate, } from '../../pages/print/templates/production-material-confirmation-template.ts';
import { buildHandoverDifferenceRequestPrintDocument, buildMasterDataChangeRequestPrintDocument, buildQualityDeductionConfirmationPrintDocument, buildQualityDisputeProcessingPrintDocument, buildSettlementChangeRequestPrintDocument, renderHandoverDifferenceRequestTemplate, renderMasterDataChangeRequestTemplate, renderQualityDeductionConfirmationTemplate, renderQualityDisputeProcessingTemplate, renderSettlementChangeRequestTemplate, } from '../../pages/print/templates/business-request-form-template.ts';
...
export function getPrintTemplateForRequest(input) {
    const matches = printTemplateRegistry.filter((template) => template.documentType === input.documentType
...
export function renderPrintDocument(document) {
...
export const requiredPrintDocumentTypes = [
    'TASK_ROUTE_CARD',
    'TASK_DELIVERY_CARD',
    'MATERIAL_PREP_SLIP',
    'PICKUP_SLIP',
...
export function validatePrintTemplateRegistry() {
    const issues = [];
    for (const documentType of requiredPrintDocumentTypes) {
        ...
        for (const template of templates) {
            if (!template.templateCode)
                issues.push(`模板缺少编码：${documentType}`);
            if (!template.templateName || /^[A-Za-z0-9_ -]+$/.test(template.templateName)) {
                issues.push(`模板缺少中文名称：${template.templateCode}`);
            }
            if (template.supportedSourceTypes.length === 0)
                issues.push(`模板缺少来源类型：${template.templateCode}`);
            if (typeof template.buildDocument !== 'function')
                issues.push(`模板缺少文档构建函数：${template.templateCode}`);
            if (typeof template.render !== 'function')
                issues.push(`模板缺少渲染函数：${template.templateCode}`);
        }
    }
    return issues;
}
