...
function cloneSkuRecord(record) {
    return { ...record };
}
function cloneUsageRecord(record) {
    return { ...record };
}
...
function normalizeSkuRecord(record) {
    return {
...
function buildUsageRecord(materialId, index, input) {
...
function buildSeedSnapshot() {
    ...
    const logRecords = records.flatMap((record, index) => [
        {
...
function loadSnapshot() {
    ...
    try {
        ...
        memorySnapshot = hydrateSnapshot({
            ...
            skuRecords: parsed.skuRecords,
            usageRecords: (parsed.usageRecords || []),
            logRecords: (parsed.logRecords || []),
        });
        localStorage.setItem(MATERIAL_ARCHIVE_STORAGE_KEY, JSON.stringify(memorySnapshot));
        return cloneSnapshot(memorySnapshot);
    }
...
function buildMaterialCode(kind, name) {
...
export function createMaterialSkuRecord(materialId, input) {
    const snapshot = loadSnapshot();
...
export function updateMaterialSkuRecord(materialSkuId, input) {
    const snapshot = loadSnapshot();
    ...
    const nextMaterialSkuCode = buildNextMaterialSkuCode(material.materialCode, normalizedInput, existingSkuRecords.filter((item) => item.materialSkuId !== materialSkuId));
