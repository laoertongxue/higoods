import { buildPlatformAttentionItems, buildPlatformChainSections, buildPlatformCuttingDetailRoute, } from './detail.helpers';
import { buildPlatformCuttingRuntimeOverviewData, } from './overview.adapter';
export function buildPlatformCuttingDetailView(recordId) {
    ...
    const issues = row.issues.map((issue) => ({
        title: issue.title,
        level: issue.level,
        sourceLabel: issue.sourcePage === 'MATERIAL_PREP'
            ? '仓库配料'
    ...
    const latestFactoryActionText = row.recentFactoryActionAt !== '-'
