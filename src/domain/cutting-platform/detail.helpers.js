export function buildPlatformCuttingDetailRoute(recordId) {
    return `/fcs/progress/cutting-overview/${encodeURIComponent(recordId)}`;
}
export function buildPlatformIssueSourceLabel(issue) {
    if (issue.sourcePage === 'MATERIAL_PREP')
        return '仓库配料';
    if (issue.sourcePage === 'CUT_PIECE_ORDER')
        return '裁片执行';
    if (issue.sourcePage === 'REPLENISHMENT')
        return '补料管理';
    if (issue.sourcePage === 'WAREHOUSE')
        return '仓库管理';
    return '样衣流转';
}
export function buildPlatformChainSections(row) {
    return [
        {
            key: 'pickup',
            title: '配料 / 领料',
            ...
            latestActionText: row.pickupSummary.latestScannedAt !== '-'
        ...
        },
        {
            ...
            latestActionText: row.record.spreadingSummary.latestSpreadingAt
        ...
        },
        {
            ...
            latestActionText: row.record.replenishmentSummary.suggestionCount > 0
        ...
        },
        {
            ...
            latestActionText: row.record.warehouseSummary.latestInboundAt
        ...
        },
        {
            ...
            latestActionText: row.record.sampleSummary.latestSampleActionAt
...
export function buildPlatformAttentionItems(row) {
    const items = [];
