...
const factoryCodeMap = {
    '晋江盛鸿裁片厂': 'CUT-F001',
    '石狮恒泰裁片厂': 'CUT-F002',
    '泉州嘉盛裁片厂': 'CUT-F003',
    '泉州协同裁片组': 'CUT-F004',
};
function buildFactoryCode(factoryName) {
    return factoryCodeMap[factoryName] || `CUT-${factoryName.length}`;
}
function buildPickupSummary(row) {
    return {
...
function buildExecutionSummary(row) {
    return {
...
function buildReplenishmentSummary(row) {
    return {
...
function buildWarehouseSummary(row) {
    return {
...
function groupExceptions(row, allExceptions) {
    return allExceptions.filter((item) => item.productionOrderNo === row.productionOrderNo && item.cuttingTaskNo === row.cuttingTaskNo);
}
function buildExceptionImpactSummary(row, exceptions) {
    ...
    const settlementAttentionExceptionCount = openExceptions.filter((item) => {
        return (item.exceptionType === 'RECEIVE_DISCREPANCY' ||
...
function buildOperatorSummary(row) {
    ...
    seeds.forEach((item, index) => {
        const current = map.get(item.operatorName) ||
            {
        ...
        if (item.key === 'pickup')
            current.pickupActionCount += 1;
        if (item.key === 'spreading')
            current.spreadingActionCount += 1;
        if (item.key === 'inbound')
            current.inboundActionCount += 1;
        if (item.key === 'replenishment')
            current.replenishmentFeedbackCount += 1;
...
function buildGroupSummary(row, operators) {
...
function buildSettlementFocusLevel(row, impact) {
    if (impact.highRiskExceptionCount > 0 || row.hasPendingReplenishment || row.hasReceiveRecheck)
        return 'HIGH_FOCUS';
    if (impact.hasSettlementImpact || row.hasPendingInbound || row.hasPendingHandover || row.pendingIssueCount > 0)
        return 'FOCUS';
    return 'NORMAL';
}
function buildScoreFocusLevel(row, impact) {
    if (impact.highRiskExceptionCount > 0 || impact.insufficientEvidenceCount > 0 || impact.repeatedRecheckCount > 1)
        return 'HIGH_ATTENTION';
    if (row.hasPendingReplenishment || row.hasSampleRisk || row.highRiskIssueCount > 0)
        return 'LOW_SCORE_RISK';
    if (impact.hasScoreImpact || row.pendingIssueCount > 0)
        return 'WATCH';
    return 'NORMAL';
}
function buildRecommendedScoreBand(scoreFocusLevel) {
    if (scoreFocusLevel === 'HIGH_ATTENTION')
        return 'LIMIT';
    if (scoreFocusLevel === 'LOW_SCORE_RISK')
        return 'WATCH';
    if (scoreFocusLevel === 'WATCH')
        return 'GOOD';
    return 'EXCELLENT';
}
function buildReviewStatus(settlementFocusLevel, scoreFocusLevel, impact) {
    if (impact.insufficientEvidenceCount > 0)
        return 'NEED_MORE_INFO';
    if (settlementFocusLevel === 'NORMAL' && scoreFocusLevel === 'NORMAL' && impact.openExceptionCount === 0)
        return 'CONFIRMED';
    if (impact.highRiskExceptionCount > 0)
        return 'REVIEWING';
    return 'PENDING';
}
export function buildPlatformCuttingSettlementInputViews(rows = buildPlatformCuttingRuntimeOverviewData().rows, allExceptions = buildPlatformCuttingExceptionViews(rows)) {
    return rows.map((row) => {
