export const cuttingSettlementFocusMeta = {
...
export const cuttingScoreFocusMeta = {
    NORMAL: { label: '普通', className: 'bg-slate-100 text-slate-700' },
...
export const cuttingInputReviewStatusMeta = {
...
export const cuttingRecommendedScoreBandMeta = {
...
export function filterCuttingSettlementInputViews(rows, filters) {
    ...
    return rows.filter((row) => {
        const matchesKeyword = keyword.length === 0 ||
            [
                ...
                .toLowerCase()
                .includes(keyword);
        const matchesSettlementFocus = filters.settlementFocusLevel === 'ALL' || row.settlementInput.settlementFocusLevel === filters.settlementFocusLevel;
        const matchesScoreFocus = filters.scoreFocusLevel === 'ALL' || row.scoreInput.scoreFocusLevel === filters.scoreFocusLevel;
        ...
        const matchesAttentionType = filters.attentionType === 'ALL' ||
        ...
        const matchesExceptionImpact = filters.exceptionImpact === 'ALL' ||
...
export function hasCuttingSettlementInputFilters(filters) {
...
export function buildCuttingSettlementInputStats(rows) {
    return {
...
export function buildCuttingSettlementFocusBuckets(rows) {
    ...
    return {
        ...
        reviewRows: pendingRows
            .filter((row) => row.settlementInput.exceptionImpactSummary.repeatedRecheckCount > 0 ||
            row.settlementInput.exceptionImpactSummary.insufficientEvidenceCount > 0)
            .slice(0, 3),
        executionRows: pendingRows
            .filter((row) => row.settlementInput.replenishmentSummary.highRiskCount > 0 ||
            row.settlementInput.warehouseSummary.pendingInboundCount > 0 ||
            row.settlementInput.warehouseSummary.waitingHandoverCount > 0)
            .slice(0, 3),
    };
}
export function buildCuttingSettlementInputEmptyStateText(hasFilters, mode) {
    if (mode === 'focus')
        return '当前没有需要平台优先确认的裁片结算与评分输入。';
    return hasFilters ? '未找到符合筛选条件的裁片结算与评分输入。' : '当前没有可展示的裁片结算与评分输入。';
}
export function buildOperatorContributionHeadline(view) {
    if (view.settlementInput.operatorSummary.length === 0)
        return '当前没有可展示的个人执行留痕。';
...
export function buildGroupContributionHeadline(view) {
    if (view.settlementInput.groupSummary.length === 0)
        return '当前没有可展示的小组执行留痕。';
...
export function buildSettlementReviewText(view) {
