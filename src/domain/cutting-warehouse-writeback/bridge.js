...
import { appendCutPieceWarehouseWritebackRecord, appendSampleWarehouseWritebackRecord, } from '../../data/fcs/cutting/warehouse-writeback-ledger.ts';
...
function buildSuccess(writebackId) {
    return {
        success: true,
        writebackId,
        issues: [],
...
function validateFormalIdentity(input) {
    ...
    if (!productionOrder)
        issues.push('未找到对应生产单，无法提交正式仓务写回。');
    if (!cutOrder)
        issues.push('未找到对应裁片单，无法提交正式仓务写回。');
...
export function submitCutPieceWarehouseWriteback(payload) {
    const issues = validateFormalIdentity(payload);
    if (!payload.warehouseRecordId.trim())
        issues.push('缺少裁片仓记录主键，无法提交。');
    if (!payload.cutOrderId.trim())
        issues.push('缺少裁片单 ID，无法提交。');
    if (!payload.productionOrderId.trim())
        issues.push('缺少生产单 ID，无法提交。');
    if (!payload.operatorAccountId.trim())
        issues.push('缺少仓务操作人账号，无法提交。');
    if (issues.length > 0)
        return buildFailure(issues, payload.writebackId);
    appendCutPieceWarehouseWritebackRecord(payload, getBrowserLocalStorage() || undefined);
    return buildSuccess(payload.writebackId);
}
export function submitSampleWarehouseWriteback(payload) {
    const issues = validateFormalIdentity(payload);
    if (!payload.sampleRecordId.trim())
        issues.push('缺少样衣仓记录主键，无法提交。');
    if (!payload.cutOrderId.trim())
        issues.push('缺少裁片单 ID，无法提交。');
    if (!payload.productionOrderId.trim())
        issues.push('缺少生产单 ID，无法提交。');
