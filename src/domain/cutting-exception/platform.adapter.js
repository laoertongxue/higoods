...
import { cuttingExceptionClosureConditionTexts, cuttingExceptionTriggerConditionTexts, cuttingExceptionTypeMeta, } from './helpers';
import { listCuttingPickupViewsByProductionOrder, } from '../pickup/page-adapters/cutting-shared';
function buildExceptionNo(row, type, index) {
...
function pickRelevantPickupView(views, predicate) {
    return views.find(predicate) ?? views[0] ?? null;
}
function deriveOwner(type, row) {
...
function deriveSuggestedRoute(type, row) {
    if (type === 'RECEIVE_DISCREPANCY' || type === 'MISSING_EVIDENCE')
        return row.routes.materialPrep;
    if (type === 'MARKER_NOT_MAINTAINED' || type === 'SPREADING_DATA_INSUFFICIENT')
        return row.routes.cutOrders;
    if (type === 'REPLENISHMENT_PENDING')
        return row.routes.replenishment;
    return row.routes.fabricWarehouse;
}
function deriveRiskLevel(type, row) {
    if (type === 'MISSING_EVIDENCE')
        return 'HIGH';
    if (type === 'REPLENISHMENT_PENDING' && row.record.replenishmentSummary.highRiskCount > 0)
        return 'HIGH';
    if (type === 'RECEIVE_DISCREPANCY' && row.overallRiskLevel === 'HIGH')
        return 'HIGH';
    if (type === 'INBOUND_PENDING' && (row.urgencyLevel === 'AA' || row.urgencyLevel === 'A'))
        return 'HIGH';
    if (type === 'SAMPLE_OVERDUE' && row.record.sampleSummary.overdueReturnCount > 0)
        return 'HIGH';
    if (type === 'HANDOVER_PENDING' && row.urgencyLevel === 'AA')
        return 'HIGH';
    if (type === 'ZONE_UNASSIGNED' || type === 'MARKER_NOT_MAINTAINED' || type === 'SPREADING_DATA_INSUFFICIENT')
        return 'MEDIUM';
    return row.overallRiskLevel === 'LOW' ? 'LOW' : 'MEDIUM';
}
function deriveInitialStatus(type, row) {
    if (type === 'RECEIVE_DISCREPANCY' && row.hasPhotoEvidence)
        return 'WAITING_CONFIRM';
    if (type === 'ZONE_UNASSIGNED')
        return 'WAITING_CONFIRM';
    if (type === 'REPLENISHMENT_PENDING' || type === 'HANDOVER_PENDING' || type === 'SAMPLE_OVERDUE')
        return 'IN_PROGRESS';
    return 'OPEN';
}
function buildCommonException(row, type, index, view, overrides) {
...
function buildExceptionsForRow(row) {
    ...
    if (row.hasReceiveRecheck) {
        rows.push(buildCommonException(row, 'RECEIVE_DISCREPANCY', rows.length + 1, recheckView || latestView, {
    ...
    if (row.hasReceiveRecheck && !row.hasPhotoEvidence) {
        rows.push(buildCommonException(row, 'MISSING_EVIDENCE', rows.length + 1, recheckView || latestView, {
    ...
    if (row.record.markerSummary.pendingMarkerCount > 0) {
        rows.push(buildCommonException(row, 'MARKER_NOT_MAINTAINED', rows.length + 1, latestView, {
    ...
    if (row.record.spreadingSummary.pendingSpreadingCount > 0) {
        rows.push(buildCommonException(row, 'SPREADING_DATA_INSUFFICIENT', rows.length + 1, latestView, {
    ...
    if (row.record.replenishmentSummary.pendingReviewCount > 0 || row.record.replenishmentSummary.needMoreInfoCount > 0) {
        rows.push(buildCommonException(row, 'REPLENISHMENT_PENDING', rows.length + 1, latestView, {
    ...
    if (row.record.warehouseSummary.cutPiecePendingInboundCount > 0) {
        rows.push(buildCommonException(row, 'INBOUND_PENDING', rows.length + 1, latestView, {
    ...
    if (row.record.warehouseSummary.unassignedZoneCount > 0) {
        rows.push(buildCommonException(row, 'ZONE_UNASSIGNED', rows.length + 1, latestView, {
    ...
    if (row.record.sampleSummary.sampleWaitingReturnCount > 0 || row.record.sampleSummary.overdueReturnCount > 0) {
        rows.push(buildCommonException(row, 'SAMPLE_OVERDUE', rows.length + 1, latestView, {
    ...
    if (row.record.warehouseSummary.waitingHandoverCount > 0) {
        rows.push(buildCommonException(row, 'HANDOVER_PENDING', rows.length + 1, latestView, {
...
export function buildPlatformCuttingExceptionViews(rows = buildPlatformCuttingRuntimeOverviewData().rows) {
