export const cuttingExceptionTypeMeta = {
...
export const cuttingExceptionRiskMeta = {
...
export const cuttingExceptionStatusMeta = {
...
export const cuttingExceptionSourceLayerLabels = {
    PLATFORM: '平台',
    PCS: 'PCS',
    FACTORY_APP: '工厂端',
};
export const cuttingExceptionSourcePageLabels = {
    ORDER_PROGRESS: '订单进度',
    MATERIAL_PREP: '仓库配料',
    CUT_PIECE_ORDER: '裁片单',
    REPLENISHMENT: '补料管理',
...
export const cuttingExceptionOwnerRoleLabels = {
    PLATFORM: '平台',
    CUTTING_FACTORY_OPS: '裁片厂运营',
    FIELD_EXECUTION: '现场执行',
};
export const cuttingExceptionTriggerConditionTexts = {
...
export const cuttingExceptionClosureConditionTexts = {
...
export function filterCuttingExceptions(rows, filters) {
...
export function buildCuttingExceptionStats(rows) {
...
export function buildCuttingExceptionFocusRows(rows) {
    return [...rows]
        .filter((row) => row.status !== 'CLOSED')
        .sort((left, right) => {
        const score = (row) => {
            if (row.exceptionType === 'MISSING_EVIDENCE' && row.riskLevel === 'HIGH')
                return 0;
            if (row.exceptionType === 'RECEIVE_DISCREPANCY' && row.riskLevel === 'HIGH')
                return 1;
            if (row.exceptionType === 'REPLENISHMENT_PENDING' && row.riskLevel === 'HIGH')
                return 2;
            if (row.exceptionType === 'INBOUND_PENDING')
                return 3;
            if (row.exceptionType === 'ZONE_UNASSIGNED')
                return 4;
            if (row.exceptionType === 'SAMPLE_OVERDUE')
                return 5;
            if (row.exceptionType === 'HANDOVER_PENDING')
                return 6;
...
export function buildCuttingExceptionEmptyStateText(hasFilters, mode) {
    if (mode === 'focus')
        return '当前没有需要平台优先推进的裁片专项异常。';
    return hasFilters ? '未找到符合筛选条件的裁片专项异常。' : '当前没有可展示的裁片专项异常。';
}
export function hasCuttingExceptionFilters(filters) {
...
export function buildCuttingExceptionFollowupSummary(rows) {
    const openRows = rows.filter((row) => row.status !== 'CLOSED');
    const threshold = Date.now() - 48 * 60 * 60 * 1000;
    const longOpenRows = openRows.filter((row) => {
        if (!row.latestActionAt || row.latestActionAt === '-')
            return false;
...
export function buildExceptionLatestActionText(row) {
    if (!row.latestActionAt || row.latestActionAt === '-')
        return row.latestActionSummary;
