...
import { buildPickupEvidenceSummary, buildPickupPrintVersionSummary, buildPickupQrBindingSummary, buildPickupReceiptSummary, buildPickupResultSummary, getLatestPickupScanRecord, pickupReceiptStatusLabels, } from '../helpers';
function mapMaterialType(materialType) {
...
function normalizeScanStatus(statusText) {
    if (statusText.includes('成功'))
        return 'MATCHED';
    if (statusText.includes('驳回'))
        return 'RECHECK_REQUIRED';
    if (statusText.includes('照片'))
        return 'PHOTO_SUBMITTED';
    if (statusText.includes('取消'))
        return 'CANCELLED';
    return 'NOT_SCANNED';
}
function buildPickupSlipNo(boundObjectNo) {
    if (boundObjectNo.startsWith('CP-'))
        return `PK-${boundObjectNo.slice(3)}`;
    if (boundObjectNo.startsWith('CPO-'))
        return `PS-${boundObjectNo.slice(4)}`;
    return `PK-${boundObjectNo}`;
}
function buildPrintVersionNo(boundObjectNo, printCount) {
    const normalized = boundObjectNo.replace(/^CP-/, '').replace(/^CPO-/, '');
    return `PV-${normalized}-V${Math.max(printCount, 1)}`;
}
function buildQtySummaryFromNumbers(unitLabel, itemCount, rollCount, length, summaryText) {
    return {
        unitLabel,
        itemCount,
        rollCount,
...
function parseNumericParts(input) {
...
function buildScanRecordsFromReceiveRecords(pickupSlipNo, qrCodeValue, boundObjectNo, receiveRecords) {
    return receiveRecords.map((record) => ({
        ...
        resultStatus: record.resultStatus === 'MATCHED'
            ? 'MATCHED'
            : record.resultStatus === 'PHOTO_SUBMITTED'
                ? 'PHOTO_SUBMITTED'
                : 'RECHECK_REQUIRED',
        receivedQtySummary: buildQtySummaryFromNumbers('卷', record.receivedRollCount, record.receivedRollCount, record.receivedLength, `实领 ${record.receivedRollCount} 卷 / ${record.receivedLength} 米`),
        photoProofCount: record.photoProofCount,
        note: record.note,
    }));
}
function buildEvidenceList(pickupSlipNo, latestScanRecord, discrepancyNote, photoProofCount) {
    if (!latestScanRecord)
        return [];
    const evidences = [];
...
function deriveSlipStatus(printCount, configuredRollCount, configuredLength, receiveStatus, discrepancyStatus) {
    if (discrepancyStatus === 'RECHECK_REQUIRED' || discrepancyStatus === 'PHOTO_SUBMITTED')
        return 'RECHECK_REQUIRED';
    if (receiveStatus === 'RECEIVED')
        return 'RECEIVED';
    if (receiveStatus === 'PARTIAL')
        return 'PARTIAL_RECEIVED';
    if (printCount > 0 || configuredRollCount > 0 || configuredLength > 0)
        return 'READY_TO_PICKUP';
    return 'PENDING_PRINT';
}
function buildView(slip, latestPrintVersion, qrBinding, scanRecords, evidences) {
...
function buildPdaScanRecords(detail) {
    return detail.pickupLogs.map((log) => ({
        ...
        resultStatus: normalizeScanStatus(log.resultLabel) === 'NOT_SCANNED' ? 'CANCELLED' : normalizeScanStatus(log.resultLabel),
        receivedQtySummary: buildQtySummaryFromText(detail.actualReceivedQtyText || '待扫码回写'),
        photoProofCount: log.photoProofCount,
        note: log.note,
    }));
}
function buildQtySummaryFromText(input) {
...
export function buildCuttingPickupViewFromMaterialPrepLine(line, group) {
    const pickupSlipNo = buildPickupSlipNo(line.cutPieceOrderNo);
    const latestPrintVersion = line.printCount > 0
        ? {
            pickupSlipNo,
        ...
        : null;
    const qrBinding = line.qrStatus === 'GENERATED'
        ? {
...
export function buildCuttingPickupViewFromCutPieceRecord(record) {
    const matched = findMaterialPrepLine(record.cutPieceOrderNo, record.materialSku);
    if (matched)
        return buildCuttingPickupViewFromMaterialPrepLine(matched.line, matched.group);
    const pickupSlipNo = buildPickupSlipNo(record.cutPieceOrderNo);
    const qrBinding = record.qrStatus === 'GENERATED'
        ? {
        ...
        : null;
    const latestPrintVersion = record.printSlipStatus === 'PRINTED'
        ? {
            pickupSlipNo,
        ...
        : null;
    const scanRecords = record.latestReceiveScanAt && record.latestReceiverName
        ? [
            {
                ...
                resultStatus: record.discrepancyStatus === 'PHOTO_SUBMITTED'
                    ? 'PHOTO_SUBMITTED'
...
export function buildCuttingPickupViewFromPdaDetail(detail, latestPrintVersion, qrBinding) {
...
export function listCuttingPickupViewsByProductionOrder(productionOrderNo) {
