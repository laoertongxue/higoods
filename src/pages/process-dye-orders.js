...
const ORDERS = listPrepProcessOrders('DYE').map((item) => ({
...
function scheduleScrollToSection(section) {
    if (typeof window === 'undefined')
        return;
    window.setTimeout(() => {
        const node = document.querySelector(`[data-dye-order-section="${section}"]`);
      ...
      <div class="flex flex-col gap-4 xl:flex-row">
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-3 flex flex-wrap gap-2 border-t pt-3">
            ...
            <button class="inline-flex h-8 items-center rounded-md border px-3 text-xs hover:bg-muted" data-dye-order-action="open-batches" data-order-no="${escapeHtml(order.orderNo)}">查看收货批次</button>
      ...
      <aside class="absolute inset-y-0 right-0 w-full overflow-y-auto border-l bg-background shadow-2xl sm:max-w-[780px]">
        ...
        <div class="space-y-4 px-6 py-5">
          ...
          <section class="rounded-lg border bg-card p-4">
            ...
            <p class="mt-3 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-700">执行提示：平台主状态用于跟单收口，工厂内部状态保留染色现场细节点，需求是否完成仍以收货批次关联满足为准。</p>
          </section>
...
export function renderProcessDyeOrdersPage() {
      ...
      <section class="rounded-lg border bg-card p-4">
        <div class="flex flex-wrap items-end gap-3">
          <div class="min-w-[220px] flex-1">
            ...
            <input class="h-9 w-full rounded-md border bg-background px-3 text-sm" placeholder="加工单号 / 工厂 / 需求单号 / 收货批次号" value="${escapeHtml(state.keyword)}" data-dye-order-field="keyword" />
          </div>
...
export function handleProcessDyeOrdersEvent(target) {
    if (state.materialPickerOpen && !target.closest('[data-dye-order-material-combobox="true"]')) {
        state.materialPickerOpen = false;
    }
    const createFieldNode = target.closest('[data-dye-order-create-field]');
    ...
    const fieldNode = target.closest('[data-dye-order-field]');
    ...
    const actionNode = target.closest('[data-dye-order-action]');
    if (!actionNode)
        return false;
...
export function isProcessDyeOrdersDialogOpen() {
