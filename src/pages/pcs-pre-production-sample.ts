...
import {
  ...
  savePreProductionSampleTaskDraft,
  type PreProductionSampleTaskCreateInput,
...
} from '../data/pcs-pre-production-sample-repository.ts'
import { recordSampleLedgerEvent } from '../data/pcs-sample-project-writeback.ts'
import type { PreProductionSampleTaskRecord } from '../data/pcs-pre-production-sample-types.ts'
import { PRE_PRODUCTION_SOURCE_TYPE_LIST } from '../data/pcs-task-source-normalizer.ts'
import { escapeHtml } from '../utils.ts'

interface PreProductionPageState {
  keyword: string
  statusFilter: string
...
let state: PreProductionPageState = {
  keyword: '',
  statusFilter: 'all',
  selectedTaskId: null,
  detailOpen: false,
...
function getTasks(): PreProductionSampleTaskRecord[] {
  return listPreProductionSampleTasks().filter((item) => {
    if (state.keyword) {
      const keyword = state.keyword.toLowerCase()
      if (
        ![
          item.preProductionSampleTaskCode,
          item.projectCode,
          item.projectName,
          item.sampleCode,
          item.title,
...
function getSelectedTask(): PreProductionSampleTaskRecord | null {
  if (!state.selectedTaskId) return null
  return getPreProductionSampleTaskById(state.selectedTaskId)
}

function getSampleName(task: PreProductionSampleTaskRecord): string {
  return `${task.projectName}-产前版样衣`
}

function updateTask(taskId: string, patch: Partial<PreProductionSampleTaskRecord>) {
  updatePreProductionSampleTask(taskId, patch)
}

function writePreProductionEvent(task: PreProductionSampleTaskRecord, eventType: 'SHIP_OUT' | 'RECEIVE_ARRIVAL' | 'CHECKIN_VERIFY') {
  recordSampleLedgerEvent({
    ledgerEventId: `${task.preProductionSampleTaskId}::${eventType}`,
    eventType,
...
export function shipPreProductionSampleTask(taskId: string): boolean {
  const task = getPreProductionSampleTaskById(taskId)
  if (!task || task.status !== '待发样') return false
  writePreProductionEvent(task, 'SHIP_OUT')
  updateTask(taskId, { status: '在途', trackingNo: task.trackingNo || `PP-TRACK-${task.preProductionSampleTaskCode}`, updatedAt: nowText(), updatedBy: '当前用户' })
  state.notice = `已为 ${task.preProductionSampleTaskCode} 记录发样事件，并同步回写样衣台账。`
  return true
}

export function receivePreProductionSampleTask(taskId: string): boolean {
  const task = getPreProductionSampleTaskById(taskId)
  if (!task || !['在途', '待发样'].includes(task.status)) return false
  writePreProductionEvent(task, 'RECEIVE_ARRIVAL')
  updateTask(taskId, { status: '已到样待入库', updatedAt: nowText(), updatedBy: '当前用户' })
  state.notice = `已为 ${task.preProductionSampleTaskCode} 记录到样事件，并同步回写项目节点。`
  return true
}

export function stockInPreProductionSampleTask(taskId: string): boolean {
  const task = getPreProductionSampleTaskById(taskId)
  if (!task || task.status !== '已到样待入库') return false
  writePreProductionEvent(task, 'CHECKIN_VERIFY')
  updateTask(taskId, { status: '验收中', updatedAt: nowText(), updatedBy: '当前用户' })
  state.notice = `已为 ${task.preProductionSampleTaskCode} 记录入库事件，并同步生成样衣资产关系。`
  return true
}
  ...
  return uiDrawer(
    {
      ...
      closeAction: { prefix: 'pre-production', action: 'close-create' },
      width: 'md',
    },
    `
      <div class="space-y-4">
        <div>
          ...
          <select class="h-9 w-full rounded-md border bg-background px-3 text-sm" data-pre-production-field="project">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <select class="h-9 w-full rounded-md border bg-background px-3 text-sm" data-pre-production-field="sourceType">
              ${PRE_PRODUCTION_SOURCE_TYPE_LIST.map((item) => `<option value="${item}" ${state.createForm.sourceType === item ? 'selected' : ''}>${item}</option>`).join('')}
            </select>
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium">优先级</label>
            <select class="h-9 w-full rounded-md border bg-background px-3 text-sm" data-pre-production-field="priorityLevel">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <select class="h-9 w-full rounded-md border bg-background px-3 text-sm" data-pre-production-field="site">
          ...
          <div>
            ...
            <input class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.createForm.factoryName)}" data-pre-production-field="factoryName" />
          </div>
        </div>
        <div class="grid gap-4 md:grid-cols-2">
          <div>
            <label class="mb-1 block text-sm font-medium">负责人</label>
            <input class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.createForm.ownerName)}" data-pre-production-field="ownerName" />
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium">预计到样时间</label>
            <input class="h-9 w-full rounded-md border px-3 text-sm" type="datetime-local" value="${escapeHtml(state.createForm.expectedArrival)}" data-pre-production-field="expectedArrival" />
          </div>
        </div>
        <div class="grid gap-4 md:grid-cols-2">
          <div>
            <label class="mb-1 block text-sm font-medium">纸样版本</label>
            <input class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.createForm.patternVersion)}" data-pre-production-field="patternVersion" />
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium">花型版本</label>
            <input class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.createForm.artworkVersion)}" data-pre-production-field="artworkVersion" />
          </div>
        </div>
        ${state.createForm.sourceType === '人工创建' ? '' : `
          <div class="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.createForm.upstreamObjectCode)}" data-pre-production-field="upstreamObjectCode" placeholder="请输入正式上游编号" />
            </div>
            <div>
              <label class="mb-1 block text-sm font-medium">上游对象主键</label>
              <input class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.createForm.upstreamObjectId)}" data-pre-production-field="upstreamObjectId" placeholder="可选，用于精确追溯" />
            </div>
          </div>
        `}
      </div>
    `,
    {
      cancel: { prefix: 'pre-production', action: 'close-create', label: '取消' },
      extraActions: [
        { prefix: 'pre-production', action: 'save-draft', label: '保存草稿', variant: 'secondary' },
      ],
      confirm: { prefix: 'pre-production', action: 'submit-create', label: '创建并开始', variant: 'primary' },
    },
  )
}
...
function renderDetailDrawer(): string {
  ...
  const actions = []
  if (task.status === '待发样') actions.push(`<button class="inline-flex h-8 items-center rounded-md bg-primary px-3 text-xs text-primary-foreground" data-pre-production-action="ship-task">发样</button>`)
  if (task.status === '在途') actions.push(`<button class="inline-flex h-8 items-center rounded-md bg-primary px-3 text-xs text-primary-foreground" data-pre-production-action="receive-task">到样签收</button>`)
  if (task.status === '已到样待入库') actions.push(`<button class="inline-flex h-8 items-center rounded-md bg-primary px-3 text-xs text-primary-foreground" data-pre-production-action="stockin-task">核对入库</button>`)
  return uiDetailDrawer(
    {
      title: '产前版样衣详情',
      subtitle: task.preProductionSampleTaskCode,
      closeAction: { prefix: 'pre-production', action: 'close-detail' },
      width: 'md',
    },
    `
      <div class="grid gap-3 text-sm md:grid-cols-2">
        ...
        <div class="rounded-lg border bg-muted/20 p-3"><div class="text-xs text-muted-foreground">纸样版本</div><div class="mt-1 font-medium">${escapeHtml(task.patternVersion || '—')}</div></div>
...
function buildCreateInput(): PreProductionSampleTaskCreateInput {
  ...
  const sourceObjectType =
    ...
    state.createForm.sourceType === '改版任务'
      ...
      : state.createForm.sourceType === '首版样衣打样'
        ? '首版样衣打样任务'
        : ''
  return {
    projectId: state.createForm.projectId,
    title: `产前版样衣-${project?.projectName || '待定项目'}`,
    sourceType: state.createForm.sourceType as PreProductionSampleTaskCreateInput['sourceType'],
...
export function handlePreProductionSampleEvent(target: Element): boolean {
  const actionNode = target.closest<HTMLElement>('[data-pre-production-action]')
  const action = actionNode?.dataset.preProductionAction
  ...
  if (action === 'ship-task' && state.selectedTaskId) return shipPreProductionSampleTask(state.selectedTaskId)
  if (action === 'receive-task' && state.selectedTaskId) return receivePreProductionSampleTask(state.selectedTaskId)
  if (action === 'stockin-task' && state.selectedTaskId) return stockInPreProductionSampleTask(state.selectedTaskId)
  if (action === 'save-draft') {
    const draft = savePreProductionSampleTaskDraft(buildCreateInput())
    state.notice = `已保存产前版样衣草稿：${draft.preProductionSampleTaskCode}。草稿不会写入项目关系。`
    state.createOpen = false
    state.createForm = createDefaultForm()
    return true
  }
  if (action === 'submit-create') {
    const result = createPreProductionSampleTaskWithProjectRelation(buildCreateInput())
    state.notice = result.message
    if (result.ok) {
      state.createOpen = false
      state.createForm = createDefaultForm()
      state.selectedTaskId = result.task.preProductionSampleTaskId
    }
    return true
  }
  return false
...
export function handlePreProductionSampleInput(target: Element): boolean {
  const field = (target as HTMLElement).dataset.preProductionField
  ...
  if (field === 'site') {
    ...
    if (!state.createForm.factoryName) {
      state.createForm.factoryName = state.createForm.targetSite === '雅加达' ? '雅加达工厂01' : '深圳工厂02'
    }
    return true
  }
  ...
  return `
    <div class="space-y-4">
      <header class="flex items-center justify-between">
        <div>
          <p class="text-xs text-gray-500">工程开发与打样管理 / 产前版样衣</p>
          <h1 class="text-xl font-semibold">产前版样衣</h1>
          <p class="mt-1 text-sm text-gray-500">正式创建后会同步写入任务仓储、项目关系和项目节点，发样与到样动作继续通过样衣台账事件回写。</p>
        </div>
        <button class="inline-flex h-9 items-center rounded-md bg-blue-600 px-4 text-sm text-white hover:bg-blue-700" data-pre-production-action="open-create">新建产前版样衣</button>
      </header>
      ...
      <section class="rounded-lg border bg-white p-4">
        <div class="grid gap-4 md:grid-cols-3">
          <input class="h-9 rounded-md border px-3 text-sm" placeholder="任务编号 / 项目编号 / 样衣编号" value="${escapeHtml(state.keyword)}" data-pre-production-field="keyword" />
          <select class="h-9 rounded-md border px-3 text-sm" data-pre-production-field="status">
      ...
      <section class="rounded-lg border bg-white overflow-hidden">
        <table class="w-full text-sm">
          ...
          <tbody>
            ${tasks.length === 0 ? `
              <tr><td colspan="9" class="px-4 py-10 text-center text-sm text-gray-500">暂无产前版样衣任务</td></tr>
            ` : tasks.map((task) => `
              <tr class="border-b last:border-b-0 hover:bg-gray-50">
                <td class="px-4 py-3">
                  <div class="font-medium">${escapeHtml(task.preProductionSampleTaskCode)}</div>
                ...
                <td class="px-4 py-3">
                  <div class="flex justify-end gap-2">
                    <button class="inline-flex h-8 items-center rounded-md border px-3 text-xs hover:bg-gray-50" data-pre-production-action="open-detail" data-task-id="${task.preProductionSampleTaskId}">查看</button>
                  </div>
                </td>
              </tr>
            `).join('')}
