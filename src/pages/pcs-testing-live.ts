...
import {
  getLiveLineOwnershipSummary,
  getLiveSessionOwnershipSummary,
...
function getFilteredSessions(): LiveSession[] {
  ...
  return sessions.filter((session) => {
    ...
    if (!matchTestingOwnershipFilter(getLiveSessionOwnershipSummary(session.id), state.ownershipFilter)) return false
...
function renderOwnershipQuickFilters(): string {
  const options: Array<{ key: TestingOwnershipFilter; label: string; desc: string }> = [
    ...
    { key: 'formal', label: '正式项目测款', desc: '只看已纳入正式商品项目链路的测款' },
            ...
            .map(
              (option) => `
                <button
                  ...
                  data-pcs-live-action="set-ownership-filter"
    ...
    <section class="rounded-lg border bg-card p-4">
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-7">
        ...
        <div>
          ...
          <select class="h-9 w-full rounded-md border bg-background px-3 text-sm" data-pcs-live-field="ownershipFilter">
...
function renderRows(rows: LiveSession[]): string {
  if (rows.length === 0) {
    return `
      <tr>
        <td colspan="12" class="px-4 py-12 text-center text-sm text-muted-foreground">
      ...
      return `
        <tr class="border-b last:border-b-0 hover:bg-muted/40">
          ...
          <td class="px-3 py-3 align-top text-xs">
            <span class="inline-flex rounded-full border px-2 py-0.5 ${ownership.badgeTone}">${ownership.label}</span>
            <p class="mt-1 text-muted-foreground">${escapeHtml(ownership.detailText)}</p>
          </td>
...
function renderLineRelationDrawer(): string {
  ...
  const projectOptions = listLiveProductLineProjectRelationCandidates(line.liveLineId)
  ...
  const content = `
    <div class="space-y-4">
      ...
      <div class="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-700">
        仅已完成商品上架且渠道商品状态为“已上架待测款”的项目，才允许建立正式直播测款关系。
      </div>
      <div class="space-y-2">
        ...
        <div class="max-h-[220px] space-y-2 overflow-y-auto rounded-md border bg-background p-2">
          ${
            enabledProjects.length > 0
              ? enabledProjects.map(renderLiveRelationCandidate).join('')
          ...
          ? `
            <div class="space-y-2">
              ...
              <div class="max-h-[200px] space-y-2 overflow-y-auto rounded-md border bg-background p-2">
                ${disabledProjects.map(renderLiveRelationCandidate).join('')}
              </div>
            </div>
          `
          : ''
...
function renderLiveRelationCandidate(project: TestingProjectRelationCandidate): string {
  ...
  if (!project.eligible) {
    return `
      <div class="rounded-md border border-dashed bg-muted/30 px-3 py-2 text-sm text-muted-foreground">
        ...
        <p class="mt-2 text-[11px] text-amber-700">${escapeHtml(project.disabledReason || '当前项目不满足正式直播测款门禁。')}</p>
      </div>
    `
  }
...
export function handlePcsLiveSessionsEvent(target: HTMLElement): boolean {
  ...
  if (action === 'save-line-relation') {
    ...
    if (result.errors.length === 0) {
      ...
      state.relationTargetLineId = null
      state.relationSelectedProjectIds = []
      state.notice = '直播商品明细的项目关联已更新，结果已写入正式项目关系记录。'
    } else {
