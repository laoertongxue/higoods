import { listFactoryWaitHandoverStockItems, updateWaitHandoverStockLocation, } from '../data/fcs/factory-internal-warehouse.ts';
...
import { listWoolWaitHandoverHandoutRecords, listWoolWaitHandoverInboundRecords, listWoolWarehouseInventory, } from '../data/fcs/wool-task-domain.ts';
import { renderPdaFrame } from './pda-shell';
import { buildWarehouseDifferenceText, escapeAttr, formatWarehouseDateTime, getCurrentFactoryWarehouseByKind, getMobileWarehouseRuntimeContext, getWaitHandoverWritebackStatusLabel, getWarehouseQrDisplayText, getWarehousePositionOptions, renderCompactFieldList, renderMobilePageEmptyState, renderSectionFilterChips, renderStatusPill, renderWarehouseSummaryHeader, resolveTaskRoute, resolveWaitHandoverRoute, resolveWarehouseOutboundRecordRoute, } from './pda-warehouse-shared';
...
function openLocationEditor(stockItemId) {
...
function getPostFinishingWaitHandoverRows() {
    return listPostFinishingWaitHandoverWarehouseRecords();
}
function getPostFinishingWaitHandoverStatus(row) {
    if (row.diffGarmentQty !== 0)
        return '差异';
    if (row.submittedHandoverGarmentQty <= 0)
        return '待交出';
    if (row.submittedHandoverGarmentQty >= row.waitHandoverGarmentQty && row.receivedHandoverGarmentQty >= row.submittedHandoverGarmentQty)
        return '已回写';
    if (row.submittedHandoverGarmentQty >= row.waitHandoverGarmentQty)
        return '已交出';
    return '部分交出';
}
function renderPostFinishingWaitHandoverDetailDrawer() {
...
function renderPostFinishingWaitHandoverPage() {
...
function renderWoolWaitHandoverPage() {
...
export function renderPdaWarehouseWaitHandoverPage() {
    const runtime = getMobileWarehouseRuntimeContext();
    if (!runtime)
        return renderPdaFrame(renderMobilePageEmptyState('未登录', '请先登录工厂端移动应用。'), 'warehouse', { disableTodoAutoOpen: true });
    if (runtime.factoryId === FULL_CAPABILITY_FACTORY_ID)
        return renderPostFinishingWaitHandoverPage();
    if (runtime.factoryId === OWN_WOOL_FACTORY_ID)
        return renderWoolWaitHandoverPage();
    const rows = getRows();
    const content = `
    <div class="space-y-4 px-4 pb-5 pt-4">
...
export function handlePdaWarehouseWaitHandoverEvent(target) {
    ...
    const value = fieldNode instanceof HTMLInputElement || fieldNode instanceof HTMLSelectElement || fieldNode instanceof HTMLTextAreaElement
        ? fieldNode.value
