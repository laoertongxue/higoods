...
const columns: StandardListColumn<SewingCutPieceResponsibilityProjection>[] = [
  ...
  { key: 'kit', title: '齐套／回货责任', width: 220, required: true, render: (row) => `<b>严格${row.strictCompleteKitQty.toLocaleString()}件</b><p class="mt-1 text-xs text-blue-700">参考${row.effectiveCompleteKitQty.toLocaleString()}件／责任${row.returnResponsibilityQty.toLocaleString()}件</p>` },
  {
...
function renderDetailContent(projection: SewingCutPieceResponsibilityProjection): string {
  ...
  const rowHtml = projection.lines.map((line) => {
    ...
    if (situation === 'HANDED_WITH_DEBT' && line.excludedFromEffectiveKit && line.exclusionVersionId) {
      action = `<button class="font-semibold text-blue-700" data-sewing-cut-piece-action="open-cancel-exclusion" data-assignment-id="${escapeHtml(context.assignmentId)}" data-exclusion-version-id="${escapeHtml(line.exclusionVersionId)}">取消参考标记</button>`
    } else if (situation === 'HANDED_WITH_DEBT' && eligibility.eligible) {
      action = `<button class="font-semibold text-blue-700" data-sewing-cut-piece-action="open-exclusion" data-assignment-id="${escapeHtml(context.assignmentId)}" data-requirement-line-id="${escapeHtml(line.requirementLineId)}">标记参考核查</button>`
  ...
  const exclusionHistory = exclusions.length
    ...
    : '<p class="rounded border border-dashed p-4 text-sm text-slate-500">当前没有人为标记参考核查。</p>'
...
function renderDialog(): string {
  ...
  if (state.dialog.kind === 'CANCEL_EXCLUSION') {
    ...
    return dialogShell('取消参考核查', `${projection.context.taskNo || projection.context.runtimeTaskId} · ${exclusion.partName}`, body, '<button class="rounded border px-4 py-2 text-sm" data-sewing-cut-piece-action="close-dialog">取消</button><button class="rounded bg-blue-600 px-4 py-2 text-sm text-white" data-sewing-cut-piece-action="submit-cancel-exclusion">确认取消参考标记</button>', 'max-w-xl')
  }
  const dialog = state.dialog
  const projection = getSewingCutPieceResponsibilityProjection(dialog.assignmentId)
  ...
  return dialogShell('人为标记参考核查', `${line.color} / ${line.size} · ${line.partName}`, body, '<button class="rounded border px-4 py-2 text-sm" data-sewing-cut-piece-action="close-dialog">取消</button><button class="rounded bg-blue-600 px-4 py-2 text-sm text-white" data-sewing-cut-piece-action="submit-exclusion">确认排除</button>', 'max-w-xl')
}
...
export async function handleSewingCutPieceHandoverEvent(target: HTMLElement, event?: Event): Promise<boolean> {
  ...
  if (action.startsWith('switch-tab:')) {
    ...
    try {
      const projection = getSewingCutPieceResponsibilityProjection(dialog.assignmentId)
      const line = projection.lines.find((item) => item.requirementLineId === dialog.requirementLineId)
      if (!line) throw new Error('未找到待标记参考核查')
    ...
    try {
      ...
      state.dialog = { ...dialog, error: error instanceof Error ? error.message : '取消参考标记失败' }
    }
