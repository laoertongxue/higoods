...
function matchesSituation(row: SewingOutsourcingReturnTrackingRow, situation: ReturnSituation): boolean {
  ...
  if (!focus) return situation === 'DUE_FOLLOW_UP' || (situation === 'POST_FINISHING_PENDING' && row.pendingPostFinishingQty > 0)
...
const columns: StandardListColumn<SewingOutsourcingReturnTrackingRow>[] = [
  ...
  { key: 'milestone', title: '当前责任节点', width: 270, required: true, render: (row) => { const node = focusMilestone(row); if (!node) return '<span class="text-slate-500">待有效接单，尚未起算</span>'; return `<b class="${statusTone(node.status)}">${Math.round(node.ratio * 100)}% · ${escapeHtml(statusLabels[node.status])}</b><p class="mt-1 text-xs">计划${node.targetQty}件 · 工厂承担${node.attributableTargetQty}件</p><p class="mt-1 text-xs text-slate-600">后道按期确认${node.confirmedQtyByDeadline}件 · 工厂待回${node.factoryPendingQty}件 · 裁床待补${node.cuttingShortfallQty}件</p><p class="mt-1 text-[11px] text-slate-500">截止 ${escapeHtml(node.deadlineAt)}</p>` } },
