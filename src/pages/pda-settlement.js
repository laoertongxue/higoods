...
import { listFutureMobileFactoryQcBuckets, listFutureMobileFactorySoonOverdueQcItems, } from '../data/fcs/quality-deduction-selectors';
import { getPreSettlementLedgerById, listPreSettlementLedgers, tracePreSettlementLedgerSource, } from '../data/fcs/pre-settlement-ledger-repository';
import { createSettlementChangeRequest, getSettlementActiveRequestByFactory, getSettlementEffectiveInfoByFactory, getSettlementLatestRequestByFactory, getSettlementStatusClass, getSettlementStatusLabel, getSettlementVersionHistory, listSettlementRequestsByFactory, } from '../data/fcs/settlement-change-requests';
import { getPrepaymentBatchById, getStatementDraftById, getStatementSettlementProgressView, listFeishuPaymentApprovals, listPaymentWritebacks, listSettlementBatchesByParty, listSettlementBatchesByStatement, listSettlementStatementsByParty, submitStatementFactoryAppeal, submitStatementFactoryConfirmation, } from '../data/fcs/store-domain-settlement-seeds';
...
function getCurrentSettlementSearchParams() {
    const pathname = appStore.getState().pathname;
    const [, query] = pathname.split('?');
    return new URLSearchParams(query || '');
}
function getCurrentFactoryContext() {
    const runtime = getPdaRuntimeContext();
    const currentFactoryId = runtime?.factoryId || '';
    const matchedFactory = indonesiaFactories.find((item) => item.id === currentFactoryId || item.code === currentFactoryId) ??
...
function getCurrentEffectiveSettlementInfo(factoryCode) {
...
function formatAmountWithSettlement(input) {
    ...
    if (!display.isConverted)
        return display.settlementAmountLabel;
...
function renderStatusBadge(text, variant) {
...
function renderRow(label, value, opts = {}) {
    ...
    <div class="flex items-center justify-between gap-3 py-0.5">
      ...
      <span class="${toClassName('text-xs text-right tabular-nums', opts.bold ? 'font-semibold text-foreground' : 'text-foreground', opts.red ? 'text-red-600' : '', opts.green ? 'text-emerald-700' : '', opts.orange ? 'text-amber-700' : '')}">${escapeHtml(value)}</span>
    </div>
  `;
}
function renderCard(title, body, className = '') {
    return `
...
function renderDrawer(title, body, closeAction) {
    return `
...
function getRemainingDeadlineSummary(deadline) {
...
function getStatementStatusLabel(status) {
    ...
    if (status === 'PENDING_FACTORY_CONFIRM')
        return '待工厂确认';
    if (status === 'FACTORY_CONFIRMED')
        return '工厂已确认';
    if (status === 'READY_FOR_PREPAYMENT')
        return '待入预付款';
    if (status === 'IN_PREPAYMENT_BATCH')
        return '已入预付款批次';
    if (status === 'PREPAID')
        return '已预付';
    return '已关闭';
}
function getStatementStatusVariant(status) {
    if (status === 'DRAFT')
        return 'amber';
    if (status === 'PENDING_FACTORY_CONFIRM')
        return 'blue';
    if (status === 'FACTORY_CONFIRMED' || status === 'READY_FOR_PREPAYMENT' || status === 'PREPAID')
        return 'green';
    if (status === 'IN_PREPAYMENT_BATCH')
        return 'blue';
    return 'gray';
}
function getFactoryFeedbackLabel(status) {
    if (status === 'NOT_SENT')
        return '未下发';
    if (status === 'PENDING_FACTORY_CONFIRM')
        return '待工厂反馈';
    if (status === 'FACTORY_CONFIRMED')
        return '工厂已确认';
    if (status === 'FACTORY_APPEALED')
        return '工厂已申诉';
    if (status === 'PLATFORM_HANDLING')
        return '平台处理中';
    return '已处理完成';
}
function getFactoryFeedbackVariant(status) {
    if (status === 'PENDING_FACTORY_CONFIRM')
        return 'amber';
    if (status === 'FACTORY_CONFIRMED')
        return 'green';
...
function getLedgerStatusVariant(status) {
    if (status === 'OPEN')
        return 'amber';
    if (status === 'IN_STATEMENT')
        return 'blue';
...
function getLedgerPriceSourceLabel(type) {
    if (type === 'DISPATCH')
        return '派单价';
    if (type === 'BID')
        return '中标价';
...
function getPrepaymentBatchStatusLabel(status) {
    if (status === 'DRAFT')
        return '草稿';
    if (status === 'READY_TO_APPLY_PAYMENT')
        return '待申请付款';
    if (status === 'FEISHU_APPROVAL_CREATED')
        return '已提交飞书付款审批';
    if (status === 'FEISHU_PAID_PENDING_WRITEBACK')
        return '飞书已付款待回写';
    if (status === 'PREPAID')
        return '已预付';
    if (status === 'CLOSED')
        return '已关闭';
    if (status === 'FEISHU_APPROVAL_REJECTED')
        return '审批已驳回';
    return '审批已取消';
}
function getPrepaymentBatchStatusVariant(status) {
    if (status === 'DRAFT' || status === 'READY_TO_APPLY_PAYMENT')
        return 'amber';
    if (status === 'FEISHU_APPROVAL_CREATED')
        return 'blue';
    if (status === 'FEISHU_PAID_PENDING_WRITEBACK')
        return 'purple';
    if (status === 'PREPAID' || status === 'CLOSED')
        return 'green';
    if (status === 'FEISHU_APPROVAL_REJECTED')
        return 'red';
    return 'gray';
...
function getFeishuStatusVariant(status) {
    if (status === 'CREATED' || status === 'APPROVING' || status === 'APPROVED_PENDING_PAYMENT')
        return 'blue';
    if (status === 'PAID')
        return 'green';
    if (status === 'REJECTED')
...
function getQualityResponseVariant(label) {
    if (label.includes('待工厂处理'))
        return 'amber';
    if (label.includes('自动确认'))
        return 'blue';
    if (label.includes('已确认'))
        return 'green';
    if (label.includes('异议'))
        return 'red';
    return 'gray';
}
function getQualityDisputeVariant(label) {
    if (label.includes('待平台处理') || label.includes('平台处理中') || label.includes('异议'))
        return 'amber';
    if (label.includes('最终维持工厂责任'))
        return 'red';
    if (label.includes('最终部分工厂责任'))
        return 'blue';
    if (label.includes('最终非工厂责任'))
        return 'purple';
    if (label.includes('已关闭') || label.includes('无异议'))
        return 'gray';
    return 'gray';
}
function getQualitySettlementVariant(label) {
    if (label.includes('正式质量扣款流水') || label.includes('预结算单'))
        return 'blue';
    if (label.includes('预付款批次') || label.includes('已预付'))
        return 'purple';
    if (label.includes('待确认') || label.includes('待平台处理'))
        return 'amber';
    if (label.includes('已生成正式质量扣款流水'))
        return 'green';
    return 'gray';
}
function getQualityCycleCollections(factoryId) {
...
function buildSnapshotDifferenceNote(input) {
    ...
    if (!effectiveVersionNo)
        return undefined;
    const different = [statementSnapshotVersionNo, batchSnapshotVersionNo].some((value) => value && value !== effectiveVersionNo);
    if (!different)
        return undefined;
    return `当前生效：${effectiveVersionNo}；单据使用：${statementSnapshotVersionNo ?? batchSnapshotVersionNo ?? '—'}。新版本用于后续新单据，本周期已生成单据继续沿用原快照。`;
}
function getSettlementCycleSummaries(factory) {
    ...
    const ensureCycle = (cycleId, cycleLabel, startAt, endAt) => {
    ...
    ledgers.forEach((ledger) => {
        const summary = ensureCycle(ledger.settlementCycleId, ledger.settlementCycleLabel, ledger.settlementCycleStartAt, ledger.settlementCycleEndAt);
        summary.ledgers.push(ledger);
        if (ledger.ledgerType === 'TASK_EARNING')
            summary.taskLedgers.push(ledger);
        if (ledger.ledgerType === 'QUALITY_DEDUCTION')
            summary.qualityLedgers.push(ledger);
    });
    statements.forEach((statement) => {
        const cycle = deriveSettlementCycleFields(factory.factoryId, statement.createdAt);
        const summary = ensureCycle(statement.settlementCycleId || cycle.settlementCycleId, statement.settlementCycleLabel || cycle.settlementCycleLabel, statement.settlementCycleStartAt || cycle.settlementCycleStartAt, statement.settlementCycleEndAt || cycle.settlementCycleEndAt);
        summary.statements.push(statement);
    });
    batches.forEach((batch) => {
        batch.items.forEach((item) => {
            if (!item.settlementCycleId || !item.settlementCycleLabel)
                return;
            const summary = ensureCycle(item.settlementCycleId, item.settlementCycleLabel, item.settlementCycleLabel.slice(-23, -13) || '', item.settlementCycleLabel.slice(-10) || '');
            if (!summary.batches.some((current) => current.batchId === batch.batchId)) {
                summary.batches.push(batch);
            }
        });
    });
    const attachQualityItems = (view, targetKey) => {
        for (const [cycleId, items] of qualityCollections[view].entries()) {
            ...
            const summary = ensureCycle(cycleId, cycle.settlementCycleLabel, cycle.settlementCycleStartAt, cycle.settlementCycleEndAt);
            summary[targetKey].push(...items);
        }
    };
    ...
    return Array.from(cycleMap.values())
        ...
        summary.batchSnapshotVersionNo = joinVersionText(summary.batches.flatMap((item) => [item.payeeAccountSnapshotVersion, item.settlementProfileVersionSummary]));
        ...
        .sort((left, right) => {
        if (left.cycleEndAt !== right.cycleEndAt)
            return left.cycleEndAt < right.cycleEndAt ? 1 : -1;
        return left.cycleId.localeCompare(right.cycleId, 'zh-CN');
    });
}
function getDefaultSettlementCycleId() {
    const context = getCurrentFactoryContext();
    return getSettlementCycleSummaries(context)[0]?.cycleId ?? '';
}
function resolveSettlementCycleId(candidate) {
    const context = getCurrentFactoryContext();
    const summaries = getSettlementCycleSummaries(context);
    if (candidate && summaries.some((item) => item.cycleId === candidate))
        return candidate;
    return summaries[0]?.cycleId ?? '';
}
function buildSettlementListHref() {
    return '/fcs/pda/settlement';
}
function buildSettlementDetailHref(detailTab = state.detailTab, cycleId = resolveSettlementCycleId(state.selectedCycleId), options) {
...
function syncSettlementStateFromRoute() {
    const routeKey = appStore.getState().pathname;
    if (state.lastRouteSyncKey === routeKey)
        return;
    ...
    if (ledgerStatusView && ['all', 'open', 'in-statement', 'in-prepayment-batch', 'prepaid'].includes(ledgerStatusView)) {
        state.ledgerStatusView = ledgerStatusView;
    }
...
function resetStatementAppealForm() {
...
function resetSettlementRequestForm() {
    const context = getCurrentFactoryContext();
...
function validateSettlementRequestForm() {
    const errors = {};
    if (!state.settlementRequestForm.accountHolderName.trim())
        errors.accountHolderName = '请填写开户名';
    if (!state.settlementRequestForm.idNumber.trim())
        errors.idNumber = '请填写证件号';
    if (!state.settlementRequestForm.bankName.trim())
        errors.bankName = '请填写银行名称';
...
function getSettlementRequestListByCurrentFactory(factoryCode) {
    return listSettlementRequestsByFactory(factoryCode);
}
function getSettlementRequestForDrawer(factoryCode) {
...
function getChangedSettlementFields(request) {
    const changed = [];
    if (request.before.accountHolderName !== request.after.accountHolderName)
        changed.push('开户名');
    if (request.before.idNumber !== request.after.idNumber)
        changed.push('证件号');
    if (request.before.bankName !== request.after.bankName)
        changed.push('银行名称');
    if (request.before.bankAccountNo !== request.after.bankAccountNo)
        changed.push('银行账号');
    if (request.before.bankBranch !== request.after.bankBranch)
        changed.push('开户支行');
    return changed.length > 0 ? changed.join('、') : '资料核对';
}
function getRequestNextStepText(request) {
    if (request.status === 'PENDING_REVIEW')
        return '平台正在审核申请，待线下签字资料齐备后继续处理。';
    if (request.status === 'APPROVED')
        return '申请已通过，新版本用于后续新单据，已生成单据继续沿用原快照。';
    return request.rejectReason || '申请未通过，可根据平台意见重新发起。';
}
function buildPdaQualityDetailHref(qcId, cycleId, qualityView) {
    const params = new URLSearchParams();
    params.set('back', 'settlement');
    if (qualityView)
        params.set('view', qualityView);
    if (cycleId)
        params.set('cycleId', cycleId);
    return `/fcs/pda/quality/${encodeURIComponent(qcId)}?${params.toString()}`;
}
function renderSettlementProfileEntryCard() {
    const context = getCurrentFactoryContext();
    const effective = getCurrentEffectiveSettlementInfo(context.factoryCode);
...
function renderSettlementRequestHistoryList(requests) {
              ...
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="inline-flex rounded border px-2 py-0.5 text-[10px] ${getSettlementStatusClass(request.status)}">${escapeHtml(getSettlementStatusLabel(request.status))}</span>
                  </div>
...
function renderSettlementVersionHistoryList(records) {
    if (records.length === 0) {
              ...
              <div class="flex items-center justify-between gap-3">
                <div>
                  <div class="flex items-center gap-2">
                    ...
                    <span class="inline-flex rounded border px-2 py-0.5 text-[10px] ${record.status === 'EFFECTIVE' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-zinc-200 bg-zinc-100 text-zinc-700'}">${escapeHtml(record.status === 'EFFECTIVE' ? '生效中' : '已失效')}</span>
                  </div>
...
function renderSettlementProfileDiffCard(summary) {
    if (!summary)
        return '';
...
function renderSettlementRequestDrawer() {
    ...
    if (mode === 'create') {
        ...
        ${state.settlementRequestErrorText
            ? `<div class="rounded-md border ${Object.keys(state.settlementRequestErrors).length > 0 ? 'border-red-200 bg-red-50 text-red-700' : 'border-blue-200 bg-blue-50 text-blue-700'} px-3 py-2 text-xs">${escapeHtml(state.settlementRequestErrorText)}</div>`
            : ''}
        <div class="space-y-3">
          ...
          <label class="block space-y-1">
            ...
            <textarea class="min-h-[72px] w-full rounded-md border px-3 py-2 text-xs" data-pda-sett-field="request.submitRemark">${escapeHtml(state.settlementRequestForm.submitRemark)}</textarea>
          </label>
        </div>
...
function getCycleQualityItems(summary, view) {
    if (view === 'pending')
        return summary.pendingQualityItems;
    if (view === 'soon')
        return summary.soonQualityItems;
    if (view === 'disputing')
        return summary.disputingQualityItems;
    if (view === 'processed')
        return summary.processedQualityItems;
    return summary.historyQualityItems;
}
function matchesQualityKeyword(item, keyword) {
...
function matchesLedgerKeyword(ledger, keyword) {
    const normalized = keyword.trim().toLowerCase();
    if (!normalized)
        return true;
    return [
...
function getFilteredCycleLedgers(summary) {
    return summary.ledgers.filter((ledger) => {
        if (state.ledgerTypeView === 'task-earning' && ledger.ledgerType !== 'TASK_EARNING')
            return false;
        if (state.ledgerTypeView === 'quality-deduction' && ledger.ledgerType !== 'QUALITY_DEDUCTION')
            return false;
        if (state.ledgerStatusView === 'open' && ledger.status !== 'OPEN')
            return false;
        if (state.ledgerStatusView === 'in-statement' && ledger.status !== 'IN_STATEMENT')
            return false;
        if (state.ledgerStatusView === 'in-prepayment-batch' && ledger.status !== 'IN_PREPAYMENT_BATCH')
            return false;
        if (state.ledgerStatusView === 'prepaid' && ledger.status !== 'PREPAID')
            return false;
        return matchesLedgerKeyword(ledger, state.ledgerKeyword);
    });
}
function getLedgerDetailViewModel(ledgerId) {
    if (!ledgerId)
        return null;
...
function renderCycleCard(summary) {
    ...
    const snapshotVersionLabel = summary.statementSnapshotVersionNo || summary.batchSnapshotVersionNo
...
function renderOverviewTab(summary) {
...
function renderQualityItemCard(item, cycleId) {
...
function renderQualityTab(summary) {
    ...
    <div class="space-y-3 p-4">
      ${renderCard('质检扣款处理区', `
        ...
        .map((item) => `
                  <button
                    class="inline-flex shrink-0 items-center rounded-full border px-3 py-1.5 text-xs font-medium ${state.qualityView === item.key ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background text-muted-foreground'}"
    ...
    <article class="rounded-lg border bg-card px-3 py-3 shadow-none">
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
          ...
          <div class="mt-1 text-[10px] text-muted-foreground">${escapeHtml(ledger.ledgerType === 'TASK_EARNING'
        ? `${ledger.taskNo ?? '未绑定任务'} · ${ledger.returnInboundBatchNo ?? '未绑定回货批次'}`
        : `${ledger.qcRecordId ?? '未绑定质检'} · ${ledger.pendingDeductionRecordId ?? '未绑定待确认记录'}`)}</div>
...
function renderLedgerDrawer() {
    ...
    const sourceSection = ledger.ledgerType === 'TASK_EARNING'
        ? `
    ...
    return renderDrawer(`正式流水 · ${ledger.ledgerNo}`, `
      ${renderCard('基本信息', `
...
function renderLedgersTab(summary) {
    ...
    <div class="space-y-3 p-4">
      ${renderCard('正式流水查看区', `
        ...
        .map(([value, label]) => `
                    <button
                      class="rounded-full border px-3 py-1.5 text-xs ${state.ledgerTypeView === value ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background text-muted-foreground'}"
        ...
        .map(([value, label]) => `
                    <button
                      class="rounded-full border px-3 py-1.5 text-xs ${state.ledgerStatusView === value ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background text-muted-foreground'}"
...
function getStatementSplitAmounts(statement) {
    return {
...
function getStatementAppealRecords(statement) {
    if (statement.appealRecords?.length)
        return statement.appealRecords.slice().reverse();
    if (statement.factoryAppealRecord)
        return [statement.factoryAppealRecord];
    return [];
}
function renderStatementCard(statement, summary) {
    ...
    const approval = getBatchApproval(batch);
    const writeback = getBatchWriteback(batch);
    const canRespond = statement.status === 'PENDING_FACTORY_CONFIRM' && statement.factoryFeedbackStatus === 'PENDING_FACTORY_CONFIRM';
    return `
    <article class="rounded-lg border bg-card px-3 py-3 shadow-none">
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
...
function renderPrepaymentBatchCard(batch) {
    const approval = getBatchApproval(batch);
    const writeback = getBatchWriteback(batch);
    return `
    <article class="rounded-lg border bg-card px-3 py-3 shadow-none">
...
function renderStatementDetailSection(title, rows) {
    return renderCard(title, rows);
}
function renderStatementDrawer() {
    if (!state.statementDrawerMode || !state.statementDetailId)
        return '';
    ...
    const snapshotDiffNote = currentEffective && currentEffective.versionNo !== statement.settlementProfileVersionNo
    ...
    if (state.statementDrawerMode === 'payment') {
        return renderDrawer(`预付款结果 · ${statement.statementNo ?? statement.statementId}`, `
        ${renderCard('预付款结果', `
            ${renderRow('预付款批次号', batch?.batchNo ?? batch?.batchId ?? '当前未入预付款批次')}
            ${renderRow('预付款批次状态', batch ? getPrepaymentBatchStatusLabel(batch.status) : '当前未入预付款批次')}
            ${renderRow('飞书付款审批编号', approval?.approvalNo ?? '当前未申请')}
    ...
    if (state.statementDrawerMode === 'appeal') {
        return renderDrawer(`发起对账单异议 · ${statement.statementNo ?? statement.statementId}`, `
        <div class="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-[10px] leading-5 text-blue-700">
        ...
        <div class="space-y-3">
          ...
          <div class="space-y-1.5">
            ...
            <textarea class="min-h-[96px] w-full rounded-md border bg-background px-3 py-2 text-xs" data-pda-sett-field="statement-appeal-description">${escapeHtml(state.statementAppealForm.description)}</textarea>
          </div>
          <div class="space-y-1.5">
    ...
    return renderDrawer(`对账单 · ${statement.statementNo ?? statement.statementId}`, `
      ${renderStatementDetailSection('基本信息 / 概况', `
        ...
        `)}
      ${renderStatementDetailSection('金额概况', `
        ...
        `)}
      ${renderStatementDetailSection('任务收入流水明细', taskItems.length > 0
        ? taskItems
            .map((item) => `
      ...
      ${renderStatementDetailSection('质量扣款流水明细', qualityItems.length > 0
        ? qualityItems
            .map((item) => `
                  <div class="rounded-md border bg-muted/20 px-3 py-2">
                    <div class="flex items-center justify-between gap-3">
      ...
      ${renderStatementDetailSection('工厂反馈', `
        ...
        : ''}
        `)}
      ${renderStatementDetailSection('结算资料快照', `
        ...
        `)}
      ${renderStatementDetailSection('后续预付款信息', `
...
function renderStatementsTab(summary) {
    return `
    <div class="space-y-3 p-4">
      ${renderCard('对账与预付款结果', `
          <div class="rounded-md border bg-muted/20 px-3 py-2 text-[10px] leading-5 text-muted-foreground">
        ...
        `)}
      ${renderCard('对账单', summary.statements.length > 0
      ...
      ${renderCard('预付款', summary.batches.length > 0
...
function renderCycleDetail(summary) {
    const detailTabs = [
        ['overview', '总览'],
        ['quality', '质检扣款'],
        ['ledgers', '正式流水'],
    ...
    if (state.detailTab === 'quality')
        content = renderQualityTab(summary);
    if (state.detailTab === 'ledgers')
        content = renderLedgersTab(summary);
    if (state.detailTab === 'statements')
        content = renderStatementsTab(summary);
    return `
    ...
    <div class="border-b bg-background px-4 py-3">
      <div class="flex gap-2 overflow-x-auto pb-1">
        ...
        .map(([key, label]) => `
              <button
                class="inline-flex shrink-0 items-center rounded-full border px-3 py-1.5 text-xs font-medium ${state.detailTab === key ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background text-muted-foreground'}"
                data-pda-sett-action="switch-detail-tab"
                data-tab="${key}"
                data-cycle-id="${escapeHtml(summary.cycleId)}"
              >
...
function renderSettlementContent() {
    ...
    if (state.pageMode === 'cycle-detail') {
        ...
        if (summary)
            return renderCycleDetail(summary);
    }
    return renderCycleListPage();
}
export function renderPdaSettlementPage() {
    if (!getPdaRuntimeContext()) {
        return renderPdaLoginRedirect();
    }
    return renderPdaFrame(renderSettlementContent(), 'settlement');
}
export function handlePdaSettlementEvent(target) {
    if (!ensurePdaSessionForAction())
        return true;
    const actionNode = target.closest('[data-pda-sett-action]');
    const fieldNode = target.closest('[data-pda-sett-field]');
    if (fieldNode) {
    ...
    if (action === 'open-quality-workbench') {
        const cycleId = resolveSettlementCycleId(actionNode.dataset.cycleId || state.selectedCycleId);
        const view = actionNode.dataset.view ?? 'pending';
        state.pageMode = 'cycle-detail';
        state.selectedCycleId = cycleId;
    ...
    if (action === 'set-ledger-type-view') {
        const nextValue = actionNode.dataset.value;
        const cycleId = resolveSettlementCycleId(actionNode.dataset.cycleId || state.selectedCycleId);
        if (!nextValue)
            return true;
        state.pageMode = 'cycle-detail';
