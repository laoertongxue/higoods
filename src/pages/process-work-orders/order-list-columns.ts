...
function renderOrderProduct(row: DyeWorkOrderOnlineRow): string {
  ...
  return `<div class="divide-y divide-gray-200 text-xs" data-dye-cell="order-product">
    <section class="space-y-1 pb-3" data-dye-section="documents">${field('加工厂',factoryName)}<div class="rounded border border-blue-100 bg-blue-50 px-2 py-1 leading-5 text-blue-800" data-dye-demand-source><span>需求来源：</span><span class="font-medium">${escapeHtml(row.sourceLabel)}</span></div><div><span class="text-muted-foreground">染色加工单：</span>${renderDetail(row)}</div>${field('任务单',row.taskNo)}${row.productionDemandId ? field('生产需求单', row.productionDemandId) : ''}${row.sourceType === 'DESIGN_REVISION' ? field('设计改款任务', row.designRevisionTaskNo) : field('生产单',row.matchedProductionOrderNo || row.productionOrderNo || '待匹配')}${row.matchStatus ? `<div class="pt-1">${renderBadge(row.matchStatusLabel, row.matchStatus === 'MATCHED' ? 'success' : row.matchStatus === 'MATCH_FAILED' ? 'danger' : row.matchStatus === 'CANCELLED' ? 'neutral' : 'warning')}</div>` : ''}${field('售卖类型',row.salesType)}</section>
...
return [
  ...
  },
  {
    ...
    render: row => `<div class="text-xs space-y-1" data-dye-cell="requirements">${field('工艺',row.processName)}${field('类型',row.headVatOrRedye)}${field('染色要求',row.targetColorName || (row.sourceType === 'DESIGN_REVISION' ? '按目标物料 SKU 执行' : '按调色任务执行'))}${field('深浅',row.shade || '工艺未指定')}${field('温度',row.temperature ? `${row.temperature}℃` : '工艺未指定')}${field('包含水溶',row.requiresWaterSoluble ? '是' : '否')}${row.professionalTaskNo && row.sourceType !== 'DESIGN_REVISION' ? field('调色任务', row.professionalTaskNo) : ''}${row.professionalResultVersion && row.sourceType !== 'DESIGN_REVISION' ? field('调色结果', `${row.professionalResultId || '成果'} · ${row.professionalResultVersion}`) : ''}${renderDyeResultAttachments(row)}${typeof row.estimatedUnitConsumption === 'number' ? field('预估单耗', String(row.estimatedUnitConsumption)) : ''}${typeof row.estimatedLossRate === 'number' ? field('损耗率', `${(row.estimatedLossRate * 100).toFixed(2)}%`) : ''}${row.matchFailureReason ? `<p class="text-red-700">${escapeHtml(row.matchFailureReason)}</p>` : ''}</div>`,
  },
  {
...
return [
  ...
  { key: 'requirement', title: '加工要求', freezeable: true, width: 290, sortable: true, sortValue: order => order.requirement.craftName,
    render: order => `<div class="space-y-2 text-xs"><p>工艺：<strong>${escapeHtml(order.requirement.craftName || '待确认')}</strong></p><p>加工方式：${escapeHtml(order.requirement.type || '待确认')}</p><p>印花面别：${escapeHtml(order.requirement.printSide)}</p>${order.demandSource.type !== 'DESIGN_REVISION' && order.professionalTaskNo ? `<p>花型任务：${escapeHtml(order.professionalTaskNo)}</p>` : ''}${order.demandSource.type !== 'DESIGN_REVISION' && order.professionalResultVersion ? `<p>花型成果：${escapeHtml(order.professionalResultId || '成果')} · ${escapeHtml(order.professionalResultVersion)}</p>` : ''}${typeof order.estimatedUnitConsumption === 'number' ? `<p>预估单耗：${order.estimatedUnitConsumption}</p>` : ''}${typeof order.estimatedLossRate === 'number' ? `<p>损耗率：${(order.estimatedLossRate * 100).toFixed(2)}%</p>` : ''}${order.matchFailureReason ? `<p class="text-red-700">${escapeHtml(order.matchFailureReason)}</p>` : ''}${order.requirement.shade ? `<p>深浅：${escapeHtml(order.requirement.shade)}</p>` : ''}${order.requirement.temperature ? `<p>温度：${escapeHtml(order.requirement.temperature)}</p>` : ''}<p>设备：${escapeHtml(order.printerNo || '尚未安排')}</p>${order.demandSource.type === 'DESIGN_REVISION' ? '' : renderArtworkTaskOutput(order)}<div class="space-y-2 border-t pt-2">${renderPattern(order.requirement.frontPattern, '正面花型')}${order.requirement.printSide === '双面' ? order.requirement.insidePattern ? renderPattern(order.requirement.insidePattern, '反面花型') : '<p class="text-amber-700">反面花型待补充</p>' : ''}</div></div>` },
