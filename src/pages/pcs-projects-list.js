import { getChannelNamesByCodes, listProjectListRecords, } from '../data/pcs-project-list-store.ts';
...
function getFilteredProjects() {
    ...
    return buildProjectListViewModels()
        ...
        if (state.list.styleType !== '全部' && project.styleType !== state.list.styleType)
            return false;
        if (state.list.status !== '全部' && project.projectStatus !== state.list.status)
            return false;
        if (state.list.owner !== '全部负责人' && project.ownerName !== state.list.owner)
            return false;
        if (state.list.phase !== '全部阶段' && (project.currentPhaseName || '-') !== state.list.phase)
            return false;
        if (state.list.riskStatus !== '全部' && project.riskStatus !== state.list.riskStatus)
            return false;
        if (state.list.pendingDecisionOnly && !project.pendingDecisionFlag)
            return false;
        if (!matchesDateRange(project.updatedAt, state.list.dateRange))
            return false;
        if (!keyword)
            return true;
        const haystack = [
...
function buildPhaseOptions(projects) {
    return ['全部阶段', ...Array.from(new Set(projects.map((item) => item.project.currentPhaseName || '-')))];
}
function renderProjectProgress(project) {
    const percent = project.project.progressTotal === 0 ? 0 : Math.round((project.project.progressDone / project.project.progressTotal) * 100);
    return `
    <div class="space-y-1">
...
function renderPagination(totalPages) {
    ...
    <div class="flex items-center justify-between border-t bg-white px-4 py-3">
      ...
      <div class="flex items-center gap-2">
        ...
        : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50')}" data-pcs-project-list-action="set-page" data-page="${page}">${page}</button>
            `)
        .join('')}
...
function renderToolbar(filteredCount, projects) {
      ...
      <div class="mt-4 flex flex-wrap items-center gap-4">
        <div class="flex flex-wrap items-center gap-2">
          ...
          ${STYLE_TYPE_OPTIONS.map((option) => `
              <button type="button" class="${toClassName('inline-flex h-8 items-center rounded-md px-3 text-xs', state.list.styleType === option ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200')}" data-pcs-project-list-action="set-style-filter" data-value="${escapeHtml(option)}">${escapeHtml(option)}</button>
            `).join('')}
        </div>
        <div class="flex flex-wrap items-center gap-2">
...
function renderProjectTable(projects, totalPages) {
    return `
...
export function handlePcsProjectListInput(target) {
    const fieldNode = target.closest('[data-pcs-project-list-field]');
    if (!fieldNode)
        return false;
...
export function handlePcsProjectListEvent(target) {
    const actionNode = target.closest('[data-pcs-project-list-action]');
    if (!actionNode)
        return false;
...
export function isPcsProjectListDialogOpen() {
    return false;
}
