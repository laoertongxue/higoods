import { calculateFactoryCapacityCompletion, computeFactoryCapacityEntryResult, getFactoryCapacityProfileByFactoryId, getFactoryCapacityEquipmentSummary, listFactoryCapacityEntries, listFactoryCapacityEquipments, replaceFactoryCapacityEquipments, replaceFactoryCapacityProfileEntries, } from '../data/fcs/factory-capacity-profile-mock';
import { getFactoryMasterRecordById, listFactoryMasterRecords, } from '../data/fcs/factory-master-store';
import { factoryAbilityScopeLabel, factoryAbilityStatusLabel, factoryEquipmentStatusLabel, factoryStatusConfig, factoryTypeConfig, } from '../data/fcs/factory-types';
import { SAM_CALC_MODE_LABEL, getCapacityProcessCraftOptions, getProcessDefinitionByCode, listProcessStages, listProcessesByStageCode, } from '../data/fcs/process-craft-dict';
...
function getVisibleFactories() {
    ...
    if (state.searchKeyword.trim()) {
        ...
        result = result.filter((factory) => [factory.name, factory.code, factory.contact, factory.phone].some((value) => (value ?? '').toLowerCase().includes(keyword)));
    }
...
function resetDraftState(factoryId) {
...
function buildEquipmentSummary(factoryId, processCode, craftCode) {
    ...
    const matchedEquipments = getCurrentEquipments(factoryId).filter((equipment) => equipment.abilityList.some((ability) => ability.processCode === processCode && ability.craftCode === craftCode));
    const summary = matchedEquipments.reduce((result, equipment) => {
        const ability = equipment.abilityList.find((item) => item.processCode === processCode && item.craftCode === craftCode);
        if (!ability)
            return result;
        result.totalEquipmentCount += equipment.quantity;
        if (equipment.status === 'MAINTENANCE')
            result.maintenanceEquipmentCount += equipment.quantity;
        if (equipment.status === 'STOPPED')
            result.stoppedEquipmentCount += equipment.quantity;
        if (equipment.status === 'FROZEN')
            result.frozenEquipmentCount += equipment.quantity;
...
function getSupportedProcessCount(factory) {
    return factory.processAbilities.filter((ability) => ability.status !== 'DISABLED' && ability.canReceiveTask !== false).length;
}
function getSupportedCraftCount(factory) {
    return factory.processAbilities.reduce((total, item) => total + item.craftCodes.length + (item.capacityNodeCodes?.length ?? 0), 0);
}
...
function getInferredFactoryTypeLabel(value) {
    const map = {
        CUTTING_FACTORY: '裁床厂',
...
function renderCapacityTableRows(factories) {
    if (!factories.length) {
        return `
      <tr>
...
function renderReadonlyProcessAbilities(factory) {
    const abilities = factory.processAbilities
        ...
        const detailTags = [
            ...(ability.craftNames ?? []),
            ...((ability.capacityNodeCodes ?? []).map((code) => getProcessDefinitionByCode(code)?.processName ?? code)),
        ];
        return `
        <article class="rounded-lg border bg-card px-4 py-3" data-capacity-readonly-ability="${escapeHtml(ability.processCode)}">
          ...
          ${detailTags.length
            ? `<div class="mt-3 flex flex-wrap gap-2">${detailTags
                .map((tag) => `<span class="inline-flex rounded-full border border-blue-200 bg-blue-50 px-2 py-1 text-xs text-blue-700">${escapeHtml(tag)}</span>`)
                .join('')}</div>`
            : ''}
        </article>
      `;
...
function resolveEquipmentGroupLabel(equipment) {
    if (equipment.equipmentType === 'PRINT_MACHINE')
        return '印花打印机';
    if (equipment.equipmentType === 'DYE_VAT')
        return '染缸';
    if (equipment.equipmentType === 'POST_NODE')
        return '后道';
    if (equipment.abilityList.some((ability) => ability.processCode === 'POST_FINISHING'))
        return '后道';
    return '设备';
}
function resolveEquipmentEfficiencyDisplay(equipment) {
    if (!equipment.abilityList.length)
        return '暂无数据';
    const firstAbility = equipment.abilityList[0];
    return `${firstAbility.efficiencyValue} ${firstAbility.efficiencyUnit}`.trim();
}
function renderEquipmentAbilityText(equipment) {
    if (!equipment.abilityList.length)
        return '暂无数据';
...
function renderEquipmentReadonlyTable(factoryId) {
...
function renderEquipmentEditorTable(factoryId) {
    ...
    <div class="space-y-3">
      ...
      <div class="overflow-hidden rounded-lg border">
        ...
        ? equipments
            .map((equipment) => {
            const abilityValueSet = new Set(equipment.abilityList.map((ability) => `${ability.processCode}::${ability.craftCode}`));
            const firstAbility = equipment.abilityList[0];
            return `
                ...
                .map(([value, label]) => `<option value="${escapeHtml(value)}" ${equipment.status === value ? 'selected' : ''}>${escapeHtml(label)}</option>`)
                .join('')}
                            </select>
                          </td>
                          <td class="px-3 py-3 text-right">
...
function renderEquipmentSection(factoryId) {
    return `
...
function renderTopReadonlyInfo(factory) {
...
function getCurrentPhaseFieldLabel(fieldKey) {
    if (fieldKey === 'deviceCount')
        return '设备数量（来自设备档案）';
    if (fieldKey === 'deviceShiftMinutes')
        return '单班时长（来自设备档案）';
    if (fieldKey === 'deviceEfficiencyValue')
        return '效率（来自设备档案）';
    return getSamBusinessFieldLabel(fieldKey);
}
function getCurrentPhaseFieldDescription(fieldKey) {
    return getSamBusinessFieldDescription(fieldKey);
}
function isEquipmentLinkedField(fieldKey) {
    return fieldKey === 'deviceCount' || fieldKey === 'deviceShiftMinutes' || fieldKey === 'deviceEfficiencyValue';
}
function renderReadonlyFieldBlock(label, value, helperText) {
    return `
...
function renderCurrentFieldControl(factoryId, processCode, craftCode, fieldKey, value) {
...
function renderCalculationLines(factoryId, row, values) {
    const result = computeFactoryCapacityEntryResult(row, values, buildEquipmentSummary(factoryId, row.processCode, row.craftCode));
    return `
...
function renderCapacityEntryCard(factoryId, row, values) {
    ...
    <article class="space-y-4 rounded-xl bg-muted/20 p-5" data-capacity-entry-card="${row.craftCode}">
      <div class="flex items-start justify-between gap-4">
        <div>
          ...
          <p class="mt-1 text-xs text-muted-foreground">${escapeHtml(row.processName)} · ${escapeHtml(SAM_CALC_MODE_LABEL[row.samCalcMode])}</p>
        </div>
      ...
      <div class="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
        ...
        .map((fieldKey) => renderCurrentFieldControl(factoryId, row.processCode, row.craftCode, fieldKey, values[fieldKey]))
        .join('')}
          </div>
        </section>
...
function renderCurrentStageSection(factory) {
...
function renderCapacityDetailDrawer() {
...
function createEmptyEquipment(factoryId) {
    return {
...
function resolveEquipmentTypeByAbilities(abilityValues) {
    if (abilityValues.some((value) => value.startsWith('PRINT::')))
        return 'PRINT_MACHINE';
    if (abilityValues.some((value) => value.startsWith('DYE::')))
        return 'DYE_VAT';
    if (abilityValues.some((value) => value.startsWith('POST_FINISHING::')))
        return 'POST_NODE';
    return 'GENERAL';
}
function updateDraftEntryValue(processCode, craftCode, fieldKey, value) {
    const targetEntry = state.draftEntries.find(({ row }) => row.processCode === processCode && row.craftCode === craftCode);
    if (!targetEntry)
        return;
    targetEntry.entry.values[fieldKey] = value;
}
function updateDraftEquipmentField(equipmentId, field, value) {
    ...
    if (field === 'status') {
        targetEquipment.status = String(value);
        return;
    }
    ...
    if (field === 'abilityList') {
        ...
        const currentAbilityMap = new Map(targetEquipment.abilityList.map((ability) => [`${ability.processCode}::${ability.craftCode}`, ability]));
        ...
        const optionMap = new Map(getCapacityProcessCraftOptions().map((option) => [`${option.processCode}::${option.craftCode}`, option]));
...
export function renderFactoryCapacityProfilePage() {
...
export function handleFactoryCapacityProfileEvent(target) {
    const pathname = appStore.getState().pathname.split('?')[0];
    if (pathname !== '/fcs/factories/capacity-profile')
        return false;
    const draftField = target.closest('[data-capacity-draft-field-key]');
    if (draftField instanceof HTMLInputElement && state.activeFactoryId && state.detailMode === 'edit') {
        ...
        const fieldKey = draftField.dataset.capacityDraftFieldKey;
        if (!processCode || !craftCode || !fieldKey)
            return true;
        updateDraftEntryValue(processCode, craftCode, fieldKey, normalizeCapacityValue(draftField.value));
        return true;
    }
    const equipmentField = target.closest('[data-capacity-equipment-field]');
    if (equipmentField instanceof HTMLInputElement
        || equipmentField instanceof HTMLSelectElement
        || equipmentField instanceof HTMLTextAreaElement) {
        if (!state.activeFactoryId || state.detailMode !== 'edit')
            return true;
    ...
    if (filterNode instanceof HTMLInputElement || filterNode instanceof HTMLSelectElement) {
        ...
        if (filter === 'search')
            state.searchKeyword = filterNode.value;
        if (filter === 'status')
            state.statusFilter = filterNode.value;
        if (filter === 'type')
            state.typeFilter = filterNode.value;
        state.currentPage = 1;
        return true;
    }
    ...
    if (action === 'save-detail') {
        if (!state.activeFactoryId)
            return true;
        replaceFactoryCapacityProfileEntries(state.activeFactoryId, state.draftEntries.map(({ entry }) => entry));
...
export function isFactoryCapacityProfileDialogOpen() {
