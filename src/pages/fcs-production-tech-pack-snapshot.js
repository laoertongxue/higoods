...
function renderReadonlyMaintainerStatus(value) {
    const normalized = String(value ?? '').trim();
    if (normalized === '已解析待确认')
        return '已完成';
...
function renderPatternImagePreview(pattern) {
    ...
    const previewUrl = pattern.markerImage?.previewUrl && pattern.markerImage.previewUrl.startsWith('data:')
...
function renderPieceColorQuantitySummary(row) {
...
function renderPieceInstanceCraftSummary(pattern, sourcePieceId) {
    ...
    const labels = instances
        ...
        .slice(0, 3);
    if (labels.length === 0)
        return `<span class="text-muted-foreground">已配置 0 / 共 ${escapeHtml(String(instances.length))} 片</span>`;
...
function renderMaterialPatternLinks(content) {
...
function renderPatternPieceTable(patternRows) {
    const rows = patternRows.flatMap((pattern) => (pattern.pieceRows ?? []).map((piece) => ({ pattern, piece })));
    if (rows.length === 0)
        return renderEmptyState('暂无部位明细。');
    return `
    <div class="overflow-x-auto rounded-lg border">
...
function renderPatternTab(source) {
    return `
...
function renderBomTab(items) {
    if (items.length === 0)
        return renderEmptyState('暂无物料清单。');
    return `
    <div class="overflow-x-auto rounded-lg border">
...
function renderProcessTab(rows) {
    if (rows.length === 0)
        return renderEmptyState('暂无工序工艺。');
    return `
    <div class="overflow-x-auto rounded-lg border">
...
function renderSizeTab(rows) {
    if (rows.length === 0)
        return renderEmptyState('暂无放码规则。');
    return `
    <div class="overflow-x-auto rounded-lg border">
...
function normalizeSnapshotMaterialType(value) {
    if (value === '面料' || value === '辅料' || value === '半成品' || value === '包装材料')
        return value;
    return '其他';
}
function resolveColorMappingLineFromSources(line, content) {
    const bom = (line.bomItemId ? content.bomItems.find((item) => item.id === line.bomItemId) : null)
        || content.bomItems.find((item) => item.name === line.materialName)
        || null;
    if (!bom)
        return null;
    const linkedPatterns = content.patternFiles.filter((pattern) => pattern.recordKind !== 'PACKAGE'
        && (pattern.linkedBomItemId === bom.id || (bom.linkedPatternIds ?? []).includes(pattern.id)));
    const pattern = (line.patternId ? linkedPatterns.find((item) => item.id === line.patternId) : null)
    ...
    const piece = (line.pieceId ? pieces.find((item) => item.id === line.pieceId) : null)
...
function buildResolvedColorMappingRows(content) {
    if (content.colorMaterialMappings.length === 0)
        return [];
...
function renderColorMappingTab(rows) {
    if (rows.length === 0)
        return renderEmptyState('暂无款色用料对应。');
    return `
    <div class="space-y-3">
      ${rows
        .map((row) => `
...
function renderDesignTab(rows) {
    if (rows.length === 0)
        return renderEmptyState('暂无花型设计。');
    return `
...
function renderAttachmentsTab(rows) {
    if (rows.length === 0)
        return renderEmptyState('暂无附件。');
    return `
    <div class="overflow-x-auto rounded-lg border">
      <table class="w-full text-sm">
        <thead class="border-b bg-muted/20 text-xs text-muted-foreground">
...
function renderTabContent(tab, source) {
    if (tab === 'bom')
        return renderBomTab(source.content.bomItems);
    if (tab === 'process')
        return renderProcessTab(source.content.processEntries);
    if (tab === 'size')
        return renderSizeTab(source.content.sizeTable);
    if (tab === 'color-mapping')
        return renderColorMappingTab(buildResolvedColorMappingRows(source.content));
    if (tab === 'design')
        return renderDesignTab(source.content.patternDesigns);
    if (tab === 'attachments')
        return renderAttachmentsTab(source.content.attachments);
    return renderPatternTab(source);
}
function getSourceTechPack(sourceTechPackVersionId) {
...
export function renderFcsProductionTechPackSnapshotPage(productionOrderId) {
