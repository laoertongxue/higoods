import { escapeHtml, state, renderBadge, renderStatCard, includesKeyword, renderFormDialog, lifecycleStatusLabel, lifecycleAllowedNext, initialDeductionBasisItems, initialStatementDrafts, initialSettlementBatches, getLegacyLikeQualityInspections, getLegacyLikeDyePrintOrders, listRuntimeTasksByOrder, deriveLifecycleStatus, getOrderById, } from './context';
function getOrdersWithLifecycleSummary() {
    ...
    const basisToStatement = new Map();
    ...
    const statementToBatch = new Map();
    ...
    return state.orders.map((order) => {
        ...
        const planStatus = order.planStatus === 'RELEASED'
            ? '计划已下发'
        ...
        const relatedDyes = legacyLikeDyePrintOrders.filter((dye) => dye.productionOrderId === order.productionOrderId);
        let dyeStatus = '无染印';
        if (relatedDyes.length > 0) {
            const hasFailInProcess = relatedDyes.some((dye) => {
                const failBatches = dye.returnBatches.filter((batch) => {
                    const linkedQcId = batch.qcId ?? batch.linkedQcId;
        ...
        const basisItems = initialDeductionBasisItems.filter((basis) => basis.productionOrderId === order.productionOrderId);
        let settlement = '无扣款';
...
function renderStatusChangeDialog() {
    const order = getOrderById(state.statusSelectedOrderId);
    if (!state.statusDialogOpen || !order)
        return '';
...
export function renderProductionStatusPage() {
    ...
    const filteredOrders = summarizedOrders.filter((order) => {
        ...
        if (keyword) {
            const matched = includesKeyword(order.productionOrderId.toLowerCase(), keyword) ||
        ...
        if (state.statusFilter !== 'ALL' && order._lifecycleStatus !== state.statusFilter)
            return false;
        return true;
    });
    const countBy = (status) => summarizedOrders.filter((order) => order._lifecycleStatus === status).length;
    const stats = {
        draft: countBy('DRAFT'),
        ...
        settlementReady: summarizedOrders.filter((order) => ['可进入结算', '已进入批次'].includes(order._settlementStatusZh)).length,
    };
    ...
    const planClass = (value) => value === '未计划' ? 'bg-white text-slate-700' : 'bg-slate-100 text-slate-700';
    const deliveryClass = (value) => value === '已配置' ? 'bg-blue-100 text-blue-700' : 'bg-white text-slate-700';
    const dyeStatusClass = (value) => {
        if (value === '可继续')
            return 'bg-blue-100 text-blue-700';
        if (value === '不合格处理中')
    ...
    const settlementClass = (value) => {
        if (value === '已进入批次' || value === '可进入结算')
            return 'bg-blue-100 text-blue-700';
        if (value === '冻结中')
            return 'bg-red-100 text-red-700';
        if (value === '有扣款依据')
    ...
    <div class="flex flex-col gap-6">
        ...
        .map((item) => `
              <article class="rounded-lg border bg-card">
                <div class="px-4 pb-1 pt-4">
                  <p class="text-xs font-medium leading-snug text-muted-foreground">${escapeHtml(item.label)}</p>
      ...
      <section class="flex flex-wrap gap-3">
        <input data-prod-field="statusKeyword" value="${escapeHtml(state.statusKeyword)}" class="w-64 rounded-md border px-3 py-2 text-sm" placeholder="关键词（生产单号 / 款号 / 工厂）" />
        <select data-prod-field="statusFilter" class="w-40 rounded-md border px-3 py-2 text-sm">
          ...
          ${Object.keys(lifecycleStatusLabel)
        .map((status) => `<option value="${status}" ${state.statusFilter === status ? 'selected' : ''}>${escapeHtml(lifecycleStatusLabel[status])}</option>`)
        .join('')}
        </select>
      </section>
        ...
        : `
            ...
            return `
                        <tr class="border-b last:border-0">
                          ...
                          <td class="px-3 py-3">${renderBadge(lifecycleStatusLabel[order._lifecycleStatus], lifecycleClass(order._lifecycleStatus))}</td>
                          <td class="px-3 py-3">${renderBadge(order._planStatusZh, planClass(order._planStatusZh))}</td>
                          <td class="px-3 py-3">${renderBadge(order._deliveryStatusZh, deliveryClass(order._deliveryStatusZh))}</td>
                          <td class="px-3 py-3 text-center">${order._taskCount > 0 ? order._taskCount : '—'}</td>
                          <td class="px-3 py-3 text-center">${order._blockedTaskCount > 0
                ? renderBadge(String(order._blockedTaskCount), 'bg-red-100 text-red-700')
                : '—'}</td>
                          <td class="px-3 py-3">${renderBadge(order._dyePrintStatusZh, dyeStatusClass(order._dyePrintStatusZh))}</td>
                          <td class="px-3 py-3">${renderBadge(order._settlementStatusZh, settlementClass(order._settlementStatusZh))}</td>
                          <td class="max-w-[140px] truncate px-3 py-3 text-xs text-muted-foreground" title="${escapeHtml(remarkText)}">${escapeHtml(remarkText)}</td>
                          <td class="px-3 py-3 text-xs text-muted-foreground">${escapeHtml(updatedAt)}</td>
                          <td class="px-3 py-3 text-right">
                            <div class="flex justify-end gap-1 flex-wrap">
                              <button class="rounded border px-2 py-1 text-xs hover:bg-muted ${order._lifecycleStatus === 'CLOSED' ? 'pointer-events-none opacity-50' : ''}" data-prod-action="open-status-change" data-order-id="${order.productionOrderId}">状态变更</button>
                              <button class="rounded px-2 py-1 text-xs hover:bg-muted" data-prod-action="open-order-detail" data-order-id="${order.productionOrderId}">生产单</button>
...
export { getOrdersWithLifecycleSummary, renderStatusChangeDialog, };
