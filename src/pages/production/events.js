import { state, currentUser, closeAllProductionDialogs, showPlanMessage, toTimestamp, nextLocalEntityId, productionOrderStatusConfig, changeAllowedNext, lifecycleAllowedNext, getFilteredDemands, listOrdersFromDemandGeneratableDemands, getFilteredOrders, getPaginatedOrders, getPlanFactoryOptions, deriveLifecycleStatus, getOrderById, openAppRoute, PAGE_SIZE, PLAN_EMPTY_FORM, DELIVERY_EMPTY_FORM, CHANGE_CREATE_EMPTY_FORM, CHANGE_STATUS_EMPTY_FORM, nextChangeId, addMaterialToDraft, restoreMaterialDraftSuggestion, confirmMaterialRequestDraft, setMaterialDraftNeedMaterial, toggleMaterialDraftLine, setMaterialDraftMode, setMaterialDraftRemark, setMaterialDraftLineConfirmedQty, } from './context';
import { getDemandCurrentTechPackInfo } from '../../data/fcs/production-tech-pack-snapshot-builder.ts';
import { openDemandBatchGenerate, openDemandSingleGenerate, openOrdersFromDemandGenerateDialog, performDemandGenerate, performOrdersFromDemandGenerate, } from './demand-domain';
function openCurrentTechPackEntry(spuCode) {
    const info = getDemandCurrentTechPackInfo({ spuCode });
    const deferOpenAppRoute = (pathname, key, title) => {
        window.setTimeout(() => {
            openAppRoute(pathname, key, title);
        }, 0);
    };
    if (info.styleId && info.currentTechPackVersionId) {
        deferOpenAppRoute(`/pcs/products/styles/${encodeURIComponent(info.styleId)}/technical-data/${encodeURIComponent(info.currentTechPackVersionId)}`, `pcs-tech-pack-${info.currentTechPackVersionId}`, `技术包版本 ${info.currentTechPackVersionCode || spuCode}`);
        return;
    }
    if (info.styleId) {
        deferOpenAppRoute(`/pcs/products/styles/${encodeURIComponent(info.styleId)}?tab=technical`, `pcs-style-${info.styleId}`, `款式档案 ${info.styleCode || spuCode}`);
        return;
    }
    deferOpenAppRoute('/pcs/products/styles', 'pcs-style-archives', '款式档案');
}
function openOrderTechPackSnapshot(productionOrderId) {
    openAppRoute(`/fcs/production/orders/${encodeURIComponent(productionOrderId)}/tech-pack`, `order-tech-pack-${productionOrderId}`, `技术包快照 ${productionOrderId}`);
}
function updateProductionField(field, node) {
    ...
    if (field.startsWith('materialDraftMode:')) {
        ...
        if (draftId && (value === 'warehouse_delivery' || value === 'factory_pickup')) {
            setMaterialDraftMode(draftId, value, currentUser.name);
        }
        return;
    }
    ...
    if (field === 'demandStatusFilter') {
        state.demandStatusFilter = value;
        return;
    }
    if (field === 'demandTechPackFilter') {
        state.demandTechPackFilter = value;
        return;
    }
    if (field === 'demandHasOrderFilter') {
        state.demandHasOrderFilter = value;
        return;
    }
    if (field === 'demandPriorityFilter') {
        state.demandPriorityFilter = value;
        return;
    }
    ...
    if (field === 'demandOwnerPartyType') {
        ...
        state.demandOwnerPartyType = value;
        return;
    }
    ...
    if (field === 'ordersStatusFilter') {
        state.ordersStatusFilter = value === 'ALL' ? [] : [value];
        state.ordersCurrentPage = 1;
        state.ordersActionMenuId = null;
        return;
    }
    if (field === 'ordersTechPackFilter') {
        state.ordersTechPackFilter = value;
        state.ordersCurrentPage = 1;
        state.ordersActionMenuId = null;
        return;
    }
    if (field === 'ordersBreakdownFilter') {
        state.ordersBreakdownFilter = value;
        state.ordersCurrentPage = 1;
        state.ordersActionMenuId = null;
        return;
    }
    if (field === 'ordersAssignmentProgressFilter') {
        state.ordersAssignmentProgressFilter = value;
        state.ordersCurrentPage = 1;
        state.ordersActionMenuId = null;
        return;
    }
    if (field === 'ordersAssignmentModeFilter') {
        state.ordersAssignmentModeFilter = value;
        state.ordersCurrentPage = 1;
        state.ordersActionMenuId = null;
        return;
    }
    if (field === 'ordersBiddingRiskFilter') {
        state.ordersBiddingRiskFilter = value;
        state.ordersCurrentPage = 1;
        state.ordersActionMenuId = null;
        return;
    }
    if (field === 'ordersTierFilter') {
        state.ordersTierFilter = value;
        state.ordersCurrentPage = 1;
        state.ordersActionMenuId = null;
        return;
    }
    if (field === 'ordersHasMaterialDraftFilter') {
        state.ordersHasMaterialDraftFilter = value;
        state.ordersCurrentPage = 1;
        state.ordersActionMenuId = null;
        return;
    }
    if (field === 'ordersHasConfirmedMaterialRequestFilter') {
        state.ordersHasConfirmedMaterialRequestFilter = value;
        state.ordersCurrentPage = 1;
        state.ordersActionMenuId = null;
        return;
    }
    ...
    if (field === 'deliveryStatusFilter') {
        state.deliveryStatusFilter = value;
        return;
    }
    ...
    if (field === 'changesTypeFilter') {
        state.changesTypeFilter = value;
        return;
    }
    if (field === 'changesStatusFilter') {
        state.changesStatusFilter = value;
        return;
    }
    ...
    if (field === 'changesCreateType') {
        state.changesCreateForm.changeType = value;
        return;
    }
    ...
    if (field === 'changesStatusNext') {
        state.changesStatusForm.nextStatus = value;
    ...
    if (field === 'detailSimulateStatus') {
        state.detailSimulateStatus = value;
    }
}
export function handleProductionEvent(target) {
    const fieldNode = target.closest('[data-prod-field]');
    ...
    const actionNode = target.closest('[data-prod-action]');
    if (!actionNode) {
    ...
    if (action === 'toggle-material-draft-needed') {
        ...
        if (!draftId || !(actionNode instanceof HTMLInputElement))
            return true;
    ...
    if (action === 'toggle-material-draft-line') {
        ...
        if (!draftId || !lineId || !(actionNode instanceof HTMLInputElement))
            return true;
    ...
    if (action === 'toggle-demand-select-all') {
        ...
        const shouldClear = filteredDemands.length > 0 &&
    ...
    if (action === 'toggle-orders-demand-select-all') {
        ...
        if (shouldClear) {
            ...
            state.ordersFromDemandSelectedIds = new Set(demands.map((demand) => demand.demandId));
        }
        return true;
    }
    ...
    if (action === 'hold-demand' || action === 'unhold-demand' || action === 'cancel-demand') {
        ...
        if (!demandId)
            return true;
        const targetStatus = action === 'hold-demand' ? 'HOLD' : action === 'unhold-demand' ? 'PENDING_CONVERT' : 'CANCELLED';
    ...
    if (action === 'save-plan-edit') {
        ...
        state.orders = state.orders.map((order) => {
            if (order.productionOrderId !== orderId)
                return order;
    ...
    if (action === 'save-changes-create') {
        ...
        if (Object.keys(errors).length > 0)
            return true;
        const targetOrder = state.orders.find((order) => order.productionOrderId === state.changesCreateForm.productionOrderId);
        if (!targetOrder) {
        ...
        const changeType = state.changesCreateForm.changeType;
        const newChange = {
            changeId,
    ...
    if (action === 'open-changes-status') {
        ...
        const currentStatus = actionNode.dataset.currentStatus;
        if (!changeId || !currentStatus)
            return true;
    ...
    if (action === 'save-changes-status') {
        ...
        state.changes = state.changes.map((change) => {
            if (change.changeId !== state.changesStatusTarget?.changeId)
                return change;
            return {
                ...change,
                status: state.changesStatusForm.nextStatus,
    ...
    if (action === 'save-status-change') {
        ...
        const now = toTimestamp();
        const nextStatus = state.statusNext;
        state.orders = state.orders.map((item) => {
            if (item.productionOrderId !== state.statusSelectedOrderId)
                return item;
            return {
                ...item,
    ...
    if (action === 'detail-apply-simulate') {
        ...
        const lockedStatuses = ['EXECUTING', 'COMPLETED', 'CANCELLED'];
        state.orders = state.orders.map((order) => {
            if (order.productionOrderId !== orderId)
                return order;
            const auditLog = {
...
export function handleProductionSubmit(_form) {
    return false;
}
export function isProductionDialogOpen() {
    return (state.demandDetailId !== null ||
