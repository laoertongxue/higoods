import { escapeHtml, state, renderBadge, includesKeyword, getLegacyLikeDyePrintOrders, listRuntimeTasksByOrder, keyProcessKeywords, getPlanFactoryOptions, getPlanWeekRange, getOrderById, } from './context';
function getPlanDownstreamMap() {
    ...
    for (const order of state.orders) {
        ...
        const keyProcessCount = tasks.filter((task) => keyProcessKeywords.some((keyword) => task.processNameZh.includes(keyword))).length;
...
function renderPlanEditDialog() {
    if (!getOrderById(state.planEditOrderId))
        return '';
      ...
      <div class="absolute left-1/2 top-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
        ...
        <div class="space-y-4 px-6 py-5">
          <label class="space-y-1">
            ...
            <input type="date" data-prod-field="planFormStartDate" value="${escapeHtml(state.planForm.planStartDate)}" class="w-full rounded-md border px-3 py-2 text-sm" />
          </label>

          <label class="space-y-1">
            <span class="text-sm">计划结束日期 <span class="text-destructive">*</span></span>
            <input type="date" data-prod-field="planFormEndDate" value="${escapeHtml(state.planForm.planEndDate)}" class="w-full rounded-md border px-3 py-2 text-sm" />
          </label>

          <label class="space-y-1">
            <span class="text-sm">计划数量 <span class="text-destructive">*</span></span>
            <input type="number" min="1" data-prod-field="planFormQty" value="${escapeHtml(state.planForm.planQty)}" class="w-full rounded-md border px-3 py-2 text-sm" placeholder="请输入计划数量" />
          </label>
          ...
          <label class="space-y-1">
            ...
            <select data-prod-field="planFormFactoryId" class="w-full rounded-md border px-3 py-2 text-sm">
              ...
              ${factoryOptions
        .map((factory) => `<option value="${factory.id}" ${state.planForm.planFactoryId === factory.id ? 'selected' : ''}>${escapeHtml(factory.name)}</option>`)
        .join('')}
            </select>
          </label>
          ...
          <label class="space-y-1">
            ...
            <input data-prod-field="planFormRemark" value="${escapeHtml(state.planForm.planRemark)}" class="w-full rounded-md border px-3 py-2 text-sm" placeholder="可选备注" />
          </label>
        </div>
...
export function renderProductionPlanPage() {
    ...
    const stats = {
        ...
        partialReady: state.orders.filter((order) => downstreamMap.get(order.productionOrderId)?.readyStatus === '部分准备').length,
    };
    ...
    <div class="flex flex-col gap-6">
      ...
      <section class="flex flex-wrap gap-3">
        <input data-prod-field="planKeyword" value="${escapeHtml(state.planKeyword)}" class="w-60 rounded-md border px-3 py-2 text-sm" placeholder="关键词（生产单号 / 款号 / 工厂）" />
        ...
        <select data-prod-field="planFactoryFilter" class="w-48 rounded-md border px-3 py-2 text-sm">
          ...
          ${planFactoryOptions
        .map((factory) => `<option value="${factory.id}" ${state.planFactoryFilter === factory.id ? 'selected' : ''}>${escapeHtml(factory.name)}</option>`)
        .join('')}
        </select>
      </section>
        ...
        : `
            ...
            return `
                          <tr class="border-b last:border-0">
                            ...
                            <td class="px-3 py-3">${renderBadge(planStatus === 'UNPLANNED' ? '未计划' : planStatus === 'PLANNED' ? '已计划' : '计划已下发', planStatusView[planStatus])}</td>
                            <td class="px-3 py-3">${renderBadge(isBrokenDown ? '已拆解' : '未拆解', isBrokenDown ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-700')}</td>
                            <td class="px-3 py-3 text-center">${downstream?.taskCount ?? 0}</td>
                            <td class="px-3 py-3 text-center">${downstream?.keyProcessCount ?? 0}</td>
                            <td class="px-3 py-3">${renderBadge(downstream?.hasDyePrint ? '有' : '无', downstream?.hasDyePrint ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-700')}</td>
                            <td class="px-3 py-3">${renderBadge(downstream?.readyStatus ?? '未准备', readyClass[downstream?.readyStatus ?? '未准备'])}</td>
                            <td class="px-3 py-3 text-xs text-muted-foreground">${escapeHtml(order.planUpdatedAt ?? '—')}</td>
                            <td class="px-3 py-3">
                              <div class="flex flex-wrap gap-1">
                                <button class="inline-flex h-8 items-center rounded-md border px-3 text-xs hover:bg-muted" data-prod-action="open-plan-edit" data-order-id="${order.productionOrderId}">编辑计划</button>
                                ${planStatus !== 'RELEASED'
                ? `<button class="inline-flex h-8 items-center rounded-md border px-3 text-xs hover:bg-muted" data-prod-action="release-plan" data-order-id="${order.productionOrderId}">下发计划</button>`
                : ''}
                                <button class="inline-flex h-8 items-center rounded-md px-3 text-xs hover:bg-muted" data-prod-action="open-order-detail" data-order-id="${order.productionOrderId}">查看生产单</button>
                              </div>
                            </td>
                          </tr>
                        `;
...
export { getPlanDownstreamMap, renderPlanEditDialog, };
