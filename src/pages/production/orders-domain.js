import { escapeHtml, state, renderBadge, renderEmptyRow, renderStatCard, safeText, tierLabels, typeLabels, riskFlagConfig, assignmentProgressStatusConfig, taskStatusLabel, taskStatusClass, getOrderById, getOrderDisplayBreakdownSnapshot, getOrderDisplayAssignmentSnapshot, getOrderStandardTimeSnapshot, getOrderMaterialDisplaySummary, getOrderMaterialIndicators, getOrderTechPackSnapshotDisplay, formatStandardTimeMinutes, getFilteredOrders, getPaginatedOrders, getProcessTaskById, getDraftStatusLabel, getMaterialRequestDraftById, getMaterialRequestDraftSummaryByOrder, listMaterialDraftOperationLogsByOrder, getSupplementOptionDisplayRows, getTaskTypeLabel, listMaterialRequestDraftsByOrder, listRuntimeTaskSplitGroupsByOrder, PAGE_SIZE, productionOrderStatusConfig, formatProductionOrderMainFactoryName, isProductionOrderMainFactoryPending, } from './context.ts';
import { renderOrdersFromDemandDialog, renderDemandConfirmDialog, } from './demand-domain.ts';
import { getProductionConfirmationByOrderId, isProductionConfirmationPrintable, } from '../../data/fcs/production-confirmation.ts';
import { buildProductionConfirmationPrintLink, } from '../../data/fcs/fcs-route-links.ts';
function getOrderConfirmationPreviewState(order) {
...
function renderOrderRiskFlags(flags) {
    if (flags.length === 0) {
...
function renderOrderAssignmentOverview(order) {
    ...
    if (assignment.assignmentSummary.directCount > 0) {
        lines.push(`派单 ${assignment.assignmentSummary.directCount} / 已确认 ${assignment.directDispatchSummary.assignedFactoryCount} / 待确认 ${Math.max(0, assignment.assignmentSummary.directCount - assignment.directDispatchSummary.assignedFactoryCount)}`);
    }
    if (assignment.assignmentSummary.biddingCount > 0) {
        lines.push(`竞价 ${assignment.assignmentSummary.biddingCount} / 已发起 ${assignment.assignmentProgress.biddingLaunchedCount} / 待定标 ${Math.max(0, assignment.assignmentSummary.biddingCount - assignment.assignmentProgress.biddingAwardedCount)}`);
    }
    lines.push(`总计 ${total}`);
    return `
    <div class="space-y-0.5 text-xs">
...
function getOrderListStatusDisplay(order) {
...
function renderOrderDemandSnapshotDrawer() {
    ...
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      ...
      <section class="absolute inset-y-0 right-0 w-full border-l bg-background shadow-2xl sm:max-w-[520px]" data-dialog-panel="true">
        ...
        ? `<section class="space-y-2 border-t pt-4">
                  ...
                  <p class="rounded-md bg-muted/50 px-3 py-2 text-sm text-muted-foreground">${escapeHtml(order.demandSnapshot.constraintsNote)}</p>
                </section>`
        : ''}
        </div>
      </section>
...
function renderOrderLogsDialog() {
...
function getOrderSplitAuditLogs(order) {
...
function getOrderMergedAuditLogs(order) {
...
function renderOrderMaterialSummary(order) {
...
function renderMaterialDraftTaskCard(draft) {
...
function renderAddDraftMaterialsDialog() {
...
function renderMaterialDraftDrawer() {
...
export function renderProductionOrdersPage() {
    ...
    const selectedAll = state.ordersSelectedIds.size === pagedOrders.length && pagedOrders.length > 0;
    const materialReminderStats = filteredOrders.reduce((acc, order) => {
        const indicators = getOrderMaterialDisplaySummary(order);
        if (indicators.stage === 'PREVIEW')
            acc.preview += 1;
        if (indicators.stage === 'ACTUAL_PENDING' || indicators.stage === 'ACTUAL_PARTIAL')
            acc.pendingOnly += 1;
        if (indicators.stage === 'ACTUAL_CONFIRMED')
            acc.confirmed += 1;
        return acc;
    ...
    <div class="space-y-4">
        ...
        <div class="grid gap-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
          ...
          <div>
            ...
            <select data-prod-field="ordersStatusFilter" class="mt-1 h-9 w-full rounded-md border px-3 text-sm">
              ...
              ${Object.keys(productionOrderStatusConfig)
        .filter((status) => status !== 'READY_FOR_BREAKDOWN')
        .map((status) => `<option value="${status}" ${state.ordersStatusFilter.length === 1 && state.ordersStatusFilter[0] === status
        ? 'selected'
        ...
        .join('')}
          ...
          <div>
            ...
            <select data-prod-field="ordersAssignmentProgressFilter" class="mt-1 h-9 w-full rounded-md border px-3 text-sm">
              ...
              ${Object.keys(assignmentProgressStatusConfig)
        .map((status) => `<option value="${status}" ${state.ordersAssignmentProgressFilter === status ? 'selected' : ''}>${escapeHtml(assignmentProgressStatusConfig[status].label)}</option>`)
        .join('')}
            </select>
          </div>
          ...
          <div>
            ...
            <select data-prod-field="ordersAssignmentModeFilter" class="mt-1 h-9 w-full rounded-md border px-3 text-sm">
              ...
              <option value="DIRECT_ONLY" ${state.ordersAssignmentModeFilter === 'DIRECT_ONLY' ? 'selected' : ''}>仅派单</option>
              <option value="BIDDING_ONLY" ${state.ordersAssignmentModeFilter === 'BIDDING_ONLY' ? 'selected' : ''}>仅竞价</option>
          ...
          <div>
            ...
            <select data-prod-field="ordersBiddingRiskFilter" class="mt-1 h-9 w-full rounded-md border px-3 text-sm">
              ...
              <option value="NEAR_DEADLINE" ${state.ordersBiddingRiskFilter === 'NEAR_DEADLINE' ? 'selected' : ''}>临近截止(&lt;24h)</option>
          ...
          <div>
            ...
            <select data-prod-field="ordersTierFilter" class="mt-1 h-9 w-full rounded-md border px-3 text-sm">
              ...
              ${Object.keys(tierLabels)
        .map((tier) => `<option value="${tier}" ${state.ordersTierFilter === tier ? 'selected' : ''}>${escapeHtml(tierLabels[tier])}</option>`)
        .join('')}
            </select>
          </div>
            ...
            <thead>
              <tr class="bg-muted/50">
                <th class="w-10 px-3 py-3 text-left">
                  <input type="checkbox" data-prod-action="toggle-orders-select-all" ${selectedAll ? 'checked' : ''} />
                </th>
            ...
            return `
                          <tr class="cursor-pointer border-b last:border-0 hover:bg-muted/30" data-prod-action="open-order-detail" data-order-id="${order.productionOrderId}">
                            <td class="px-3 py-3" data-prod-action="noop">
                              <input type="checkbox" data-prod-action="toggle-orders-select" data-order-id="${order.productionOrderId}" ${state.ordersSelectedIds.has(order.productionOrderId) ? 'checked' : ''} />
                            </td>
                            <td class="px-3 py-3">
                              <button class="h-auto p-0 font-mono text-sm text-blue-600 hover:underline" data-prod-action="open-order-detail" data-order-id="${order.productionOrderId}">${escapeHtml(order.productionOrderId)}</button>
                            </td>
                            ...
                            <td class="px-3 py-3">
                              <div class="space-y-1">
                                <div class="text-sm text-muted-foreground">${escapeHtml(techPackSnapshotDisplay.techPackVersionText)}</div>
                                <div class="flex items-center gap-2 text-xs text-muted-foreground">
                                  ${renderBadge(techPackSnapshotDisplay.techPackReadyStatus, techPackSnapshotDisplay.techPackReadyClassName)}
                            ...
                            <td class="px-3 py-3">
                              ${renderBadge(assignmentProgressStatusConfig[assignment.assignmentProgress.status]?.label ?? assignment.assignmentProgress.status, assignmentProgressStatusConfig[assignment.assignmentProgress.status]?.color ?? 'bg-slate-100 text-slate-700')}
                            </td>
                            <td class="px-3 py-3">
                ...
                ? `
                                        <div class="absolute right-0 z-50 mt-1 min-w-[150px] rounded-md border bg-background p-1 shadow-lg">
                                          ...
                                          <button
                                            class="flex w-full items-center rounded px-2 py-1.5 text-left text-sm ${getOrderConfirmationPreviewState(order).available ? 'hover:bg-muted' : 'pointer-events-none opacity-50'}"
      ...
      <footer class="flex items-center justify-between text-sm">
        <p class="text-muted-foreground">共 ${filteredOrders.length} 条记录${state.ordersSelectedIds.size > 0 ? `，已选 ${state.ordersSelectedIds.size} 项` : ''}</p>
        <div class="flex items-center gap-2">
          <button class="inline-flex h-8 w-8 items-center justify-center rounded-md border ${state.ordersCurrentPage === 1 ? 'pointer-events-none opacity-50' : 'hover:bg-muted'}" data-prod-action="orders-prev-page" aria-label="上一页">
            <i data-lucide="chevron-left" class="h-4 w-4"></i>
          </button>
          <span>${state.ordersCurrentPage} / ${totalPages || 1}</span>
          <button class="inline-flex h-8 w-8 items-center justify-center rounded-md border ${state.ordersCurrentPage >= totalPages ? 'pointer-events-none opacity-50' : 'hover:bg-muted'}" data-prod-action="orders-next-page" aria-label="下一页">
...
export { renderOrderRiskFlags, renderOrderAssignmentOverview, renderOrderDemandSnapshotDrawer, renderOrderLogsDialog, getOrderMaterialIndicators, getOrderSplitAuditLogs, getOrderMergedAuditLogs, renderOrderMaterialSummary, renderMaterialDraftTaskCard, renderAddDraftMaterialsDialog, renderMaterialDraftDrawer, };
