import { escapeHtml, state, renderBadge, safeText, changeTypeLabels, changeStatusLabels, changeStatusClass, changeAllowedNext, getLegacyLikeQualityInspections, getLegacyLikeDyePrintOrders, listRuntimeTasksByOrder, initialDeductionBasisItems, initialStatementDrafts, initialSettlementBatches, buildSettlementSummary, } from './context';
function getChangeSummaryMap() {
    ...
    for (const change of state.changes) {
        ...
        const openQcCount = legacyLikeQualityInspections.filter((inspection) => inspection.productionOrderId === orderId && inspection.status !== 'CLOSED').length;
        const basisCount = initialDeductionBasisItems.filter((basis) => basis.productionOrderId === orderId || basis.sourceOrderId === orderId).length;
        const relatedStatements = initialStatementDrafts.filter((statement) => {
            return statement.items.some((item) => item.productionOrderId === orderId || item.sourceOrderId === orderId);
        });
        ...
        const batchCount = initialSettlementBatches.filter((batch) => batch.statementIds.some((statementId) => statementIds.has(statementId))).length;
...
function renderChangeCreateDialog() {
    if (!state.changesCreateOpen)
        return '';
    return `
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      ...
      <div class="absolute left-1/2 top-1/2 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
        ...
        return `<option value="${order.productionOrderId}" ${state.changesCreateForm.productionOrderId === order.productionOrderId
            ? 'selected'
            : ''}>${escapeHtml(order.productionOrderId)}${styleNo ? ` · ${escapeHtml(styleNo)}` : ''}</option>`;
    })
        .join('')}
            </select>
        ...
        : ''}
          ...
          <label class="space-y-1">
            ...
            <select data-prod-field="changesCreateType" class="w-full rounded-md border px-3 py-2 text-sm">
              ...
              ${Object.keys(changeTypeLabels)
        .map((type) => `<option value="${type}" ${state.changesCreateForm.changeType === type ? 'selected' : ''}>${escapeHtml(changeTypeLabels[type])}</option>`)
        .join('')}
            </select>
        ...
        : ''}
          ...
          <div class="grid grid-cols-2 gap-3">
            <label class="space-y-1">
              ...
              <input data-prod-field="changesCreateBeforeValue" value="${escapeHtml(state.changesCreateForm.beforeValue)}" class="w-full rounded-md border px-3 py-2 text-sm" placeholder="可选" />
            </label>
            <label class="space-y-1">
              <span class="text-sm">变更后</span>
              <input data-prod-field="changesCreateAfterValue" value="${escapeHtml(state.changesCreateForm.afterValue)}" class="w-full rounded-md border px-3 py-2 text-sm" placeholder="可选" />
            </label>
          </div>

          <label class="space-y-1">
            <span class="text-sm">影响范围</span>
            <input data-prod-field="changesCreateImpactScope" value="${escapeHtml(state.changesCreateForm.impactScopeZh)}" class="w-full rounded-md border px-3 py-2 text-sm" placeholder="可选，如：生产排程、结算对象" />
          </label>

          <label class="space-y-1">
            <span class="text-sm">变更原因 <span class="text-destructive">*</span></span>
            <textarea data-prod-field="changesCreateReason" class="min-h-[72px] w-full rounded-md border px-3 py-2 text-sm" placeholder="请填写变更原因">${escapeHtml(state.changesCreateForm.reason)}</textarea>
        ...
        : ''}
          ...
          <label class="space-y-1">
            ...
            <textarea data-prod-field="changesCreateRemark" class="min-h-[72px] w-full rounded-md border px-3 py-2 text-sm" placeholder="可选">${escapeHtml(state.changesCreateForm.remark)}</textarea>
          </label>
        </div>
...
function renderChangeStatusDialog() {
    if (!state.changesStatusOpen || !state.changesStatusTarget)
        return '';
      ...
      <div class="absolute left-1/2 top-1/2 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
        ...
        <div class="space-y-4 px-6 py-5">
          <label class="space-y-1">
            ...
            <div class="rounded-md border bg-muted/40 px-3 py-2 text-sm">${escapeHtml(changeStatusLabels[currentStatus])}</div>
          </label>
        ...
        .map((status) => `<option value="${status}" ${state.changesStatusForm.nextStatus === status ? 'selected' : ''}>${escapeHtml(changeStatusLabels[status])}</option>`)
        .join('')}
            </select>
        ...
        : ''}
          ...
          <label class="space-y-1">
            <span class="text-sm">备注</span>
            <textarea data-prod-field="changesStatusRemark" class="min-h-[70px] w-full rounded-md border px-3 py-2 text-sm" placeholder="可选">${escapeHtml(state.changesStatusForm.remark)}</textarea>
          </label>
        </div>
...
export function renderProductionChangesPage() {
    const summaryMap = getChangeSummaryMap();
    const filteredChanges = state.changes.filter((change) => {
        if (state.changesTypeFilter !== 'ALL' && change.changeType !== state.changesTypeFilter)
            return false;
        if (state.changesStatusFilter !== 'ALL' && change.status !== state.changesStatusFilter)
            return false;
    ...
    const stats = {
        ...
        withDyePrint: state.changes.filter((change) => (summaryMap.get(change.changeId)?.dyePrintCount ?? 0) > 0).length,
        withOpenQc: state.changes.filter((change) => (summaryMap.get(change.changeId)?.openQcCount ?? 0) > 0).length,
    ...
    <div class="flex flex-col gap-6">
      ...
      <section class="flex flex-wrap gap-3">
        <input data-prod-field="changesKeyword" value="${escapeHtml(state.changesKeyword)}" class="w-72 rounded-md border px-3 py-2 text-sm" placeholder="关键词（变更单号 / 生产单号 / 变更原因）" />
        ...
        <select data-prod-field="changesTypeFilter" class="w-40 rounded-md border px-3 py-2 text-sm">
          ...
          ${Object.keys(changeTypeLabels)
        .map((type) => `<option value="${type}" ${state.changesTypeFilter === type ? 'selected' : ''}>${escapeHtml(changeTypeLabels[type])}</option>`)
        .join('')}
        </select>
        ...
        <select data-prod-field="changesStatusFilter" class="w-36 rounded-md border px-3 py-2 text-sm">
          ...
          ${Object.keys(changeStatusLabels)
        .map((status) => `<option value="${status}" ${state.changesStatusFilter === status ? 'selected' : ''}>${escapeHtml(changeStatusLabels[status])}</option>`)
        .join('')}
        </select>
      </section>
        ...
        : `
            ...
            return `
                          <tr class="border-b last:border-0">
                            ...
                            <td class="max-w-[100px] truncate px-3 py-3" title="${escapeHtml(safeText(change.beforeValue))}">${escapeHtml(safeText(change.beforeValue))}</td>
                            <td class="max-w-[100px] truncate px-3 py-3" title="${escapeHtml(safeText(change.afterValue))}">${escapeHtml(safeText(change.afterValue))}</td>
                            <td class="max-w-[120px] truncate px-3 py-3" title="${escapeHtml(safeText(change.impactScopeZh))}">${escapeHtml(safeText(change.impactScopeZh))}</td>
                            <td class="max-w-[140px] truncate px-3 py-3" title="${escapeHtml(change.reason)}">${escapeHtml(change.reason)}</td>
                            <td class="px-3 py-3">${renderBadge(changeStatusLabels[change.status], changeStatusClass[change.status])}</td>
                            ...
                            <td class="whitespace-nowrap px-3 py-3 text-sm text-muted-foreground">${escapeHtml(buildSettlementSummary(summary.statementCount, summary.batchCount))}</td>
                            <td class="whitespace-nowrap px-3 py-3 text-xs text-muted-foreground">${escapeHtml(safeText(change.updatedAt ?? change.createdAt))}</td>
                            <td class="px-3 py-3">
                              <div class="flex items-center gap-1 flex-wrap">
                                ${changeAllowedNext[change.status].length > 0
                ? `<button class="inline-flex h-8 items-center rounded-md border px-3 text-xs hover:bg-muted" data-prod-action="open-changes-status" data-change-id="${change.changeId}" data-current-status="${change.status}">状态变更</button>`
                : ''}
                                <button class="inline-flex h-8 items-center rounded-md px-3 text-xs hover:bg-muted" data-prod-action="open-order-detail" data-order-id="${change.productionOrderId}">查看生产单</button>
...
export { getChangeSummaryMap, renderChangeCreateDialog, renderChangeStatusDialog, };
