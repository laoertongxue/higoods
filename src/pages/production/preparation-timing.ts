...
function materialCatalog(): PreparationMaterialLine[] {
  ...
  for (const record of currentPreparationRecords()) {
...
function renderStatsColumnSettings(kind: StatsListKind, month: string, params: URLSearchParams): string {
  ...
  if (kind === 'monthly') {
    return renderStandardListColumnSettings({ title: '月度统计列设置', columns: createMonthlyStatsColumns(month, params), preferences: state.columnPreferences, eventPrefix: statsEventPrefix(kind), maxFrozenWidth: statsMaxFrozenWidth })
  }
  return renderStandardListColumnSettings({ title: '明细统计列设置', columns: createDetailStatsColumns(month), preferences: state.columnPreferences, eventPrefix: statsEventPrefix(kind), maxFrozenWidth: statsMaxFrozenWidth })
}
