import { escapeHtml, state, renderBadge, safeText, lifecycleStatusLabel, getOrderById, listRuntimeTasksByOrder, getLegacyLikeQualityInspections, initialAllocationByTaskId, } from './context';
function getDeliverySummaryMap() {
    ...
    for (const order of state.orders) {
        ...
        map.set(order.productionOrderId, {
            planStatus: order.planStatus === 'RELEASED'
                ? '计划已下发'
...
function renderDeliveryEditDialog() {
    if (!getOrderById(state.deliveryEditOrderId))
        return '';
    return `
      ...
      <div class="absolute left-1/2 top-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
        ...
        <div class="space-y-4 px-6 py-5">
          <label class="space-y-1">
            ...
            <input value="${escapeHtml(state.deliveryForm.productionOrderId)}" disabled class="w-full rounded-md border px-3 py-2 font-mono text-sm" />
          </label>

          <label class="space-y-1">
            <span class="text-sm">交付仓ID <span class="text-destructive">*</span></span>
            <input data-prod-field="deliveryFormWarehouseId" value="${escapeHtml(state.deliveryForm.deliveryWarehouseId)}" class="w-full rounded-md border px-3 py-2 text-sm" placeholder="请输入交付仓ID" />
          </label>

          <label class="space-y-1">
            <span class="text-sm">交付仓名称</span>
            <input data-prod-field="deliveryFormWarehouseName" value="${escapeHtml(state.deliveryForm.deliveryWarehouseName)}" class="w-full rounded-md border px-3 py-2 text-sm" placeholder="可选，留空则以仓库ID显示" />
          </label>

          <label class="space-y-1">
            <span class="text-sm">配置说明</span>
            <textarea data-prod-field="deliveryFormWarehouseRemark" rows="3" class="min-h-[80px] w-full rounded-md border px-3 py-2 text-sm" placeholder="可选备注">${escapeHtml(state.deliveryForm.deliveryWarehouseRemark)}</textarea>
          </label>
        </div>
...
export function renderProductionDeliveryWarehousePage() {
    ...
    const rows = state.orders.filter((order) => {
        if (state.deliveryStatusFilter !== 'ALL') {
            ...
            if (status !== state.deliveryStatusFilter)
                return false;
        }
        const keyword = state.deliveryKeyword.trim().toLowerCase();
        if (!keyword)
            return true;
        const styleCode = order.styleCode ?? order.demandSnapshot.spuCode;
    ...
    const stats = {
        unset: state.orders.filter((order) => !order.deliveryWarehouseStatus || order.deliveryWarehouseStatus === 'UNSET').length,
        ...
        deliverable: state.orders.filter((order) => summaryMap.get(order.productionOrderId)?.deliverableStatus === '可交付').length,
        deliverableQtyTotal: state.orders.reduce((sum, order) => sum + (summaryMap.get(order.productionOrderId)?.deliverableQty ?? 0), 0),
    };
    const statusClass = {
    ...
    <div class="flex flex-col gap-6">
      ...
      <section class="flex flex-wrap items-center gap-3">
        <input data-prod-field="deliveryKeyword" value="${escapeHtml(state.deliveryKeyword)}" class="w-72 rounded-md border px-3 py-2 text-sm" placeholder="关键词（生产单号 / 款号 / 仓库名称）" />
      ...
      <section class="rounded-lg border bg-card">
        ...
        : `
            ...
            const planStatus = (summary?.planStatus ?? '未计划');
            ...
            return `
                ...
                ? `${escapeHtml(order.deliveryWarehouseName)}${order.deliveryWarehouseId ? ` (${escapeHtml(order.deliveryWarehouseId)})` : ''}`
                : order.deliveryWarehouseId
                    ...
                    : '—'}
                            ...
                            <td class="px-3 py-3">${renderBadge(lifecycleStatus, lifecycleClass[lifecycleStatus] ?? 'bg-slate-100 text-slate-700')}</td>
                            <td class="whitespace-nowrap px-3 py-3 text-sm">${escapeHtml(summary?.taskSummary ?? '未拆解')}</td>
                            <td class="px-3 py-3">${renderBadge(summary?.deliverableStatus ?? '未配置交付仓', deliverableClass[summary?.deliverableStatus ?? '未配置交付仓'])}</td>
                            <td class="px-3 py-3 text-right text-sm">${(summary?.deliverableQty ?? 0).toLocaleString()}</td>
                            <td class="max-w-40 truncate px-3 py-3 text-sm text-muted-foreground" title="${escapeHtml(safeText(order.deliveryWarehouseRemark))}">${escapeHtml(safeText(order.deliveryWarehouseRemark))}</td>
                            <td class="whitespace-nowrap px-3 py-3 text-sm text-muted-foreground">${escapeHtml(safeText(order.deliveryWarehouseUpdatedAt))}</td>
                            <td class="px-3 py-3">
                              <div class="flex gap-2 whitespace-nowrap">
                                <button class="inline-flex h-8 items-center rounded-md border px-3 text-sm hover:bg-muted" data-prod-action="open-delivery-edit" data-order-id="${order.productionOrderId}">配置交付仓</button>
                                <button class="inline-flex h-8 items-center rounded-md px-3 text-sm hover:bg-muted" data-prod-action="open-order-detail" data-order-id="${order.productionOrderId}">查看生产单</button>
...
export { getDeliverySummaryMap, renderDeliveryEditDialog, };
