import { escapeHtml, currentUser, state, renderBadge, renderEmptyRow, safeText, getDemandById, getFilteredDemands, getBatchSelectedDemandIds, getTechPackSnapshotForDemand, renderDemandOperations, listOrdersFromDemandGeneratableDemands, getOrdersFromDemandSelectedIds, toTimestamp, nextLocalEntityId, nextProductionOrderId, openAppRoute, renderStatCard, demandPriorityConfig, demandStatusConfig, } from './context.ts';
import { buildProductionOrderFromDemands, PENDING_MAIN_FACTORY_ID, } from '../../data/fcs/production-orders';
function renderDemandDetailDrawer() {
          ...
          <section>
            ...
            <div class="space-y-3">
              <div class="flex items-center gap-2">
                ${renderBadge(techPackInfo.displayStatusLabel, techPackInfo.displayStatusClassName)}
...
function getSelectedBatchTargetDemands() {
    return getBatchSelectedDemandIds()
        .map((demandId) => state.demands.find((item) => item.demandId === demandId) ?? null)
        .filter((item) => item !== null);
}
function getDemandMergeValidation(targetDemands) {
    const generatableIds = new Set(targetDemands
        .filter((demand) => {
...
function renderDemandBatchGenerateDialog() {
    if (!state.demandBatchDialogOpen)
        return '';
    ...
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      ...
      <section class="absolute left-1/2 top-1/2 w-full max-w-2xl -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
                              ...
                              <tr class="border-b last:border-0">
                                ...
                                <td class="px-3 py-2">
                                  <div class="flex flex-wrap items-center gap-1">
                                    ${renderBadge(info.displayStatusLabel, info.displayStatusClassName)}
                                    <span class="text-xs text-muted-foreground">${escapeHtml(info.versionCode || '-')}</span>
                                    <span class="text-xs text-muted-foreground">${escapeHtml(info.versionLabel || '暂无当前生效版本')}</span>
                                  </div>
                                </td>
                                <td class="px-3 py-2">
        ...
        <footer class="flex items-center justify-end gap-2 border-t px-6 py-4">
          ...
          <button class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 ${mergeValidation.canMerge ? '' : 'pointer-events-none opacity-50'}" data-prod-action="open-demand-generate-confirm">确认合并生成</button>
        </footer>
      </section>
    </div>
  `;
}
function renderDemandSingleGenerateDialog(singleDemand) {
    if (!singleDemand)
        return '';
    return `
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
...
function renderOrdersFromDemandDialog() {
      ...
      <section class="absolute left-1/2 top-1/2 w-full max-w-2xl -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
        ...
        : demands
            ...
            return `
                              <tr class="border-b last:border-0">
                                <td class="px-3 py-2">
                                  <input type="checkbox" data-prod-action="toggle-orders-demand-select" data-demand-id="${demand.demandId}" ${selected ? 'checked' : ''} />
                                </td>
                                ...
                                <td class="px-3 py-2">
                                  <div class="flex flex-wrap items-center gap-1">
                                    ${renderBadge(techPack.displayStatusLabel, techPack.displayStatusClassName)}
                                    <span class="text-xs text-muted-foreground">${escapeHtml(techPack.versionCode || '-')}</span>
                                    <span class="text-xs text-muted-foreground">${escapeHtml(techPack.versionLabel || '暂无当前生效版本')}</span>
                                  </div>
                                </td>
                                <td class="px-3 py-2 text-right">${demand.requiredQtyTotal.toLocaleString()}</td>
                              </tr>
        ...
        <footer class="flex items-center justify-end gap-2 border-t px-6 py-4">
          ...
          <button class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 ${selectedIds.length === 0 ? 'pointer-events-none opacity-50' : ''}" data-prod-action="open-orders-demand-generate-confirm">确认生成</button>
        </footer>
      </section>
    </div>
  `;
}
function renderDemandConfirmDialog() {
    if (!state.demandGenerateConfirmOpen)
        return '';
    return `
...
export function renderProductionDemandInboxPage() {
    ...
    <div class="space-y-4">
      <header class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          <div class="flex items-center gap-2">
            <button
              class="relative inline-flex h-5 w-9 items-center rounded-full border transition-colors ${state.demandOnlyUngenerated ? 'border-blue-600 bg-blue-600' : 'bg-muted'}"
              data-prod-action="toggle-demand-only-ungenerated"
              aria-pressed="${state.demandOnlyUngenerated ? 'true' : 'false'}"
            >
              <span class="inline-block h-4 w-4 rounded-full bg-background shadow transition-transform ${state.demandOnlyUngenerated ? 'translate-x-4' : 'translate-x-0.5'}"></span>
            </button>
            <span class="text-sm">只看未生成</span>
          </div>
          <button class="inline-flex items-center rounded-md border px-3 py-2 text-sm hover:bg-muted ${selectedVisibleDemandIds.length === 0 ? 'pointer-events-none opacity-50' : ''}" data-prod-action="open-demand-batch">
      ...
      <section class="grid grid-cols-1 gap-3 md:grid-cols-3">
        ${renderStatCard('待转单', state.demands.filter((demand) => demand.demandStatus === 'PENDING_CONVERT').length)}
        ${renderStatCard('已转单', state.demands.filter((demand) => demand.demandStatus === 'CONVERTED').length)}
        ${renderStatCard('已挂起', state.demands.filter((demand) => demand.demandStatus === 'HOLD').length)}
      </section>
        ...
        <div class="grid gap-4 md:grid-cols-4 lg:grid-cols-6">
          ...
          <label class="space-y-1">
            ...
            <select data-prod-field="demandStatusFilter" class="mt-1 w-full rounded-md border px-3 py-2 text-sm">
              ...
              <option value="PENDING_CONVERT" ${state.demandStatusFilter === 'PENDING_CONVERT' ? 'selected' : ''}>待转单</option>
          ...
          <label class="space-y-1">
            ...
            <select data-prod-field="demandTechPackFilter" class="mt-1 w-full rounded-md border px-3 py-2 text-sm">
              ...
              <option value="INCOMPLETE" ${state.demandTechPackFilter === 'INCOMPLETE' ? 'selected' : ''}>不可转单</option>
      ...
      <div class="overflow-x-auto rounded-lg border">
        <table class="w-full min-w-[1200px] text-sm">
          <thead>
            <tr>
              <th class="w-10 bg-muted/50 px-3 py-3 text-left text-xs text-muted-foreground"><input type="checkbox" data-prod-action="toggle-demand-select-all" ${selectedAll ? 'checked' : ''} /></th>
        ...
        : filteredDemands
            ...
            return `
                        <tr class="border-b last:border-0 ${selected ? 'bg-muted/30' : ''}">
                          <td class="px-3 py-3"><input type="checkbox" data-prod-action="toggle-demand-select" data-demand-id="${demand.demandId}" ${selected ? 'checked' : ''} /></td>
                          ...
                          <td class="px-3 py-3 font-mono text-sm">
                            <div class="flex items-center gap-1">
                              ...
                              <button class="inline-flex h-5 w-5 items-center justify-center rounded opacity-50 hover:bg-muted hover:opacity-100" data-prod-action="copy-demand-legacy" data-legacy-no="${escapeHtml(demand.legacyOrderNo)}">
                          ...
                          <td class="px-3 py-3">
                            ...
                            <p class="max-w-[160px] truncate" title="${escapeHtml(demand.spuName)}">${escapeHtml(demand.spuName)}</p>
                          </td>
                          <td class="px-3 py-3">${renderBadge(demandPriorityConfig[demand.priority].label, demandPriorityConfig[demand.priority].className)}</td>
                          <td class="px-3 py-3">${renderBadge(demandStatusConfig[demand.demandStatus].label, demandStatusConfig[demand.demandStatus].className)}</td>
                          <td class="px-3 py-3">
                            <div class="space-y-1">
                              <div class="flex items-center gap-1">
                                ${renderBadge(techPack.displayStatusLabel, techPack.displayStatusClassName)}
                              ...
                              <div class="text-xs text-muted-foreground">${escapeHtml(techPack.versionLabel || '暂无当前生效版本')}</div>
                            </div>
                          </td>
                ...
                ? `<button class="h-auto p-0 font-mono text-sm text-blue-600 hover:underline" data-prod-action="open-order-detail" data-order-id="${demand.productionOrderId}">${escapeHtml(demand.productionOrderId)}</button>`
...
function resetDemandGenerateForm() {
...
function openDemandBatchGenerate() {
...
function openOrdersFromDemandGenerateDialog() {
    resetDemandGenerateForm();
...
function openDemandSingleGenerate(demandId) {
    resetDemandGenerateForm();
    state.demandSingleGenerateId = demandId;
    state.demandBatchDialogOpen = false;
}
function performDemandGenerate() {
    const demandIds = (() => {
        if (state.demandSingleGenerateId)
            return [state.demandSingleGenerateId];
        const targetDemands = getSelectedBatchTargetDemands();
    ...
    state.demands = state.demands.map((demand) => {
        if (!generatedDemandIds.has(demand.demandId))
            return demand;
        return {
            ...demand,
    ...
    state.demandSingleGenerateId = null;
    resetDemandGenerateForm();
    openAppRoute(`/fcs/production/orders/${createdOrder.productionOrderId}`, `po-${createdOrder.productionOrderId}`, `生产单管理 ${createdOrder.productionOrderId}`);
}
function performOrdersFromDemandGenerate() {
    ...
    const targetDemands = demandIds
        ...
        if (!demand)
            return false;
        const techPack = getTechPackSnapshotForDemand(demand);
        return (!demand.hasProductionOrder &&
...
export { renderDemandDetailDrawer, renderDemandBatchGenerateDialog, renderDemandSingleGenerateDialog, renderOrdersFromDemandDialog, renderDemandConfirmDialog, resetDemandGenerateForm, openDemandBatchGenerate, openOrdersFromDemandGenerateDialog, openDemandSingleGenerate, performDemandGenerate, performOrdersFromDemandGenerate, };
