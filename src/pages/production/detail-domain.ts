  ...
  if (!task) {
    return `
      <article class="rounded-lg border bg-card p-4">
        <h3 class="mb-2 text-sm font-medium text-muted-foreground">后道生产任务</h3>
        ${renderBadge('未生成', 'bg-amber-100 text-amber-700')}
        <p class="mt-2 text-xs text-muted-foreground">生产单缺少后道生产任务</p>
      </article>
    `
  }
...
function renderOrderPostFinishingTaskTab(order: ProductionOrder): string {
  ...
  if (!task) {
    return `
      <section class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
        <p class="font-medium">未找到生产单级后道生产任务</p>
        <p class="mt-1">请检查生产单 Mock 数据与后道生产任务生成链路。</p>
      </section>
    `
  }
  ...
  return `
    <section class="rounded-lg border bg-card p-4 space-y-4">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 class="text-base font-semibold">后道生产任务</h3>
          <p class="mt-1 text-xs text-muted-foreground">该记录串联收货、质检、实际工序、复检与交接；质检和复检不是生产工序。</p>
        </div>
        <div class="flex flex-wrap gap-2">
          <button class="rounded-md border px-3 py-1.5 text-sm hover:bg-muted" data-nav="${escapeHtml(buildPostFinishingTaskLink(task.postTaskId))}">查看后道生产任务</button>
          <button class="rounded-md border px-3 py-1.5 text-sm hover:bg-muted" data-nav="/fcs/craft/post-finishing/wait-process-warehouse">后道待加工仓</button>
      ...
      <div class="grid gap-3 md:grid-cols-3 lg:grid-cols-6">
        <article class="rounded-md border bg-muted/20 px-3 py-3">
          <p class="text-xs text-muted-foreground">后道生产任务编号</p>
  ...
  const tabs: Array<{ key: OrderDetailTab; label: string }> = [
    ...
    ...(
      isKolGotoProductionOrder(order)
        ? []
        : [{ key: 'post-finishing' as const, label: '后道生产任务' }]
    ),
...
function getOrderSourceDemandSnapshots(order: ProductionOrder): ProductionOrder['sourceDemandSnapshots'] {
  return order.sourceDemandSnapshots?.length ? order.sourceDemandSnapshots : [order.demandSnapshot]
}

function getOrderSourceDemandIdsText(order: ProductionOrder): string {
...
function renderOrderDetailTabContent(order: ProductionOrder): string {
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <section class="rounded-lg border bg-card p-4">
          ...
          <div class="grid grid-cols-2 gap-3 text-sm">
            ...
            <div><p class="text-xs text-muted-foreground">需求编号</p><p class="font-mono whitespace-normal break-words">${escapeHtml(
              sourceDemandIdsText,
            )}</p></div>
  ...
  if (state.detailTab === 'demand-snapshot') {
    const sourceDemandRows = sourceDemandSnapshots.flatMap((snapshot) =>
      snapshot.skuLines.map((sku) => ({ snapshot, sku })),
    )
    ...
    return `
      <section class="rounded-lg border bg-card p-4 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
          <div class="md:col-span-2"><p class="text-xs text-muted-foreground">需求编号</p><p class="font-mono whitespace-normal break-words">${escapeHtml(
            sourceDemandIdsText,
          )}</p></div>
    ...
    <div class="space-y-4">
      <header class="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p class="text-sm text-muted-foreground">关联需求：${escapeHtml(getOrderSourceDemandIdsText(order))} | 旧单号：${escapeHtml(
            order.legacyOrderNo,
          )}</p>
        ...
        <article class="rounded-lg border bg-card p-4">
          ...
          <p class="text-xs text-muted-foreground">${
            isProductionOrderMainFactoryPending(order)
              ? '车缝任务分配后回写'
              : escapeHtml(order.mainFactorySnapshot.code)
          }</p>
