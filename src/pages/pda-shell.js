...
let shownTodoSessionKey = null;
let currentTodoSessionKey = null;
let currentPdaPathKey = null;
...
function syncTodoModalAutoOpen(disableAutoOpen = false) {
    const runtime = getPdaRuntimeContext();
...
function getTabTitle(activeTab, headerTitle) {
    if (headerTitle)
        return headerTitle;
    if (!activeTab)
        return '工厂端移动应用';
...
function renderTodoTrigger(activeCount) {
    return `
    <button
      type="button"
...
function renderPdaTopBar(activeTab, headerTitle) {
    const runtime = getPdaRuntimeContext();
...
export function renderPdaBottomNav(activeTab) {
    return `
    <nav class="absolute bottom-0 left-0 right-0 z-10 flex h-[72px] items-center justify-around border-t bg-background px-1" data-pda-bottom-nav="true">
      ${MOBILE_APP_TABS.map((tab) => {
        ...
        return `
          <button
            type="button"
            class="flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 rounded-lg px-1 py-2 transition-colors ${toClassName(active ? 'text-primary' : 'text-muted-foreground hover:text-foreground')}"
            data-nav="${tab.href}"
...
export function renderPdaFrame(content, activeTab, options = {}) {
...
export function handlePdaShellEvent(target) {
    const actionNode = target.closest('[data-pda-shell-action]');
...
export function closePdaShellDialogsOnEscape() {
    if (!todoModalOpen)
        return false;
    todoModalOpen = false;
    return true;
