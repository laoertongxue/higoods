...
import { copyProjectTemplate, countTemplateStages, countTemplateWorkItems, createProjectTemplate, getProjectTemplateById, getStatusLabel, listProjectTemplates, toggleProjectTemplateStatus, updateProjectTemplate, } from '../data/pcs-templates.ts';
...
import { buildTemplateBusinessSummary, getTemplateNodeEditRule } from '../data/pcs-template-domain-view-model.ts';
const STYLE_TYPE_OPTIONS = ['基础款', '快时尚款', '改版款', '设计款'];
const STATUS_FILTER_OPTIONS = ['全部状态', '启用', '停用'];
...
function createExpandedStageMap(stages) {
    return Object.fromEntries(stages.map((stage) => [stage.phaseCode, true]));
}
function buildRecommendedDraft(styleType) {
...
function ensureEditorState(mode, templateId) {
    const queryParams = getCurrentQueryParams();
    const routeKey = `${mode}:${templateId ?? 'new'}:${queryParams.toString()}`;
    if (state.editor.routeKey === routeKey)
        return;
    if (mode === 'edit') {
...
function applyRecommendedDraft(styleType) {
...
function getFilteredTemplates() {
    ...
    return getTemplates().filter((template) => {
        ...
        const matchesStyleType = state.list.styleType === '全部款式类型' || template.styleType.includes(state.list.styleType);
...
function getStageNodes(phaseCode, includeDisabled = true) {
    return state.editor.nodes
        .filter((node) => node.phaseCode === phaseCode && (includeDisabled || node.enabledFlag !== false))
        .sort((a, b) => a.sequenceNo - b.sequenceNo);
}
function getTemplateStageNodes(template, phaseCode, includeDisabled = true) {
    return template.nodes
        .filter((node) => node.phaseCode === phaseCode && (includeDisabled || node.enabledFlag !== false))
        .sort((a, b) => a.sequenceNo - b.sequenceNo);
}
function getDefaultRoleNames(node) {
    return getPcsWorkItemDefinition(node.workItemId)?.roleNames ?? [];
}
function getEffectiveRoleNames(node) {
...
function getFieldTemplateLabels(workItemId) {
    const definition = getPcsWorkItemDefinition(workItemId);
    if (!definition)
        return [];
    return Array.from(new Set(definition.fieldGroups.map((group) => group.title).filter(Boolean)));
}
function getEditorStage(phaseCode) {
    return state.editor.stages.find((stage) => stage.phaseCode === phaseCode) ?? null;
}
function getStageSelectorItems(phaseCode) {
    ...
    return listSelectableTemplateWorkItems(phaseCode)
        .filter((item) => allowedCodes.has(item.workItemTypeCode))
        .filter((item) => {
      ...
      <div class="grid gap-4 md:grid-cols-4">
        ...
        <label class="space-y-1">
          ...
          <select class="h-10 w-full rounded-md border border-slate-200 px-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" data-pcs-template-field="list-styleType">
            ...
            ${STYLE_TYPE_OPTIONS.map((option) => `<option value="${escapeHtml(option)}" ${state.list.styleType === option ? 'selected' : ''}>${escapeHtml(option)}</option>`).join('')}
          </select>
        </label>
        <label class="space-y-1">
          ...
          <select class="h-10 w-full rounded-md border border-slate-200 px-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" data-pcs-template-field="list-status">
            ${STATUS_FILTER_OPTIONS.map((option) => `<option value="${escapeHtml(option)}" ${state.list.status === option ? 'selected' : ''}>${escapeHtml(option)}</option>`).join('')}
          </select>
        </label>
                        ...
                        <tr class="hover:bg-slate-50">
                          ...
                          <td class="px-4 py-3 align-top">
                            <div class="flex flex-wrap gap-1.5">
                              ${template.styleType
            .map((item) => `<span class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs text-slate-600">${escapeHtml(item)}</span>`)
            .join('')}
                            </div>
                          </td>
...
function renderBaseInfoCard(label, value, helper) {
    return `
...
function renderDetailSummary(template) {
...
function renderPendingNodes(template) {
    if (template.pendingNodes.length === 0)
        return '';
    return `
...
function renderTemplateWorkItemRows(template, stage) {
...
function renderEditorPendingNodes() {
    if (state.editor.pendingNodes.length === 0)
        return '';
    return `
    <section class="rounded-lg border border-amber-200 bg-amber-50 p-4">
...
function renderRoleBadges(node) {
    ...
    return allRoles
        ...
        <button
          type="button"
          class="${toClassName('inline-flex rounded-full border px-2 py-0.5 text-xs transition', active
...
function renderFieldTemplateBadges(workItemId) {
    const labels = getFieldTemplateLabels(workItemId);
    if (labels.length === 0) {
        return '<span class="text-xs text-slate-400">无独立字段模板</span>';
    }
    return labels
        .map((label) => `<span class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs text-slate-600">${escapeHtml(label)}</span>`)
        .join('');
}
function renderEditorNodeCard(node) {
    const definition = getPcsWorkItemDefinition(node.workItemId);
    const editRule = getTemplateNodeEditRule(state.editor.styleType, node.workItemTypeCode);
    return `
...
function renderEditorStage(stage) {
...
function renderDialogShell(title, description, body, footer) {
    return `
...
function renderToggleDialog() {
    ...
    return renderDialogShell(`确认${targetStatus}`, `确定要${targetStatus}模板「${template.name}」吗？`, `<div class="rounded-lg border bg-slate-50 p-4 text-sm leading-6 text-slate-600">模板状态切换仅影响后续项目选用，不会删除现有模板结构和工作项配置。</div>`, `
...
function renderCancelDialog() {
    if (!state.cancelDialogOpen)
        return '';
    return renderDialogShell('确认取消', '当前编辑的内容将不会被保存，确定要取消吗？', '<div class="rounded-lg border bg-slate-50 p-4 text-sm leading-6 text-slate-600">取消后将返回模板列表，当前页面内的改动不会写入模板数据。</div>', `
...
function renderSelectorDialog() {
    ...
    return renderDialogShell('从工作项库选择', stage ? `选择需要补回到「${stage.phaseName}」阶段的工作项。` : '选择需要补回的工作项。', items.length === 0
        ...
        : items
            ...
            return `
              <button
                type="button"
                class="${toClassName('flex w-full items-start gap-3 rounded-lg border p-3 text-left transition', selected ? 'border-blue-600 bg-blue-50' : 'border-slate-200 bg-white hover:bg-slate-50')}"
              ...
              >
                ...
                <span class="min-w-0 flex-1">
                  ...
                  <span class="mt-2 flex flex-wrap gap-2">
                    ${item.roleNames
                .map((role) => `<span class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs text-slate-600">${escapeHtml(role)}</span>`)
                .join('')}
                  </span>
...
export function renderPcsTemplateListPage() {
    return `
...
export function renderPcsTemplateDetailPage(templateId) {
...
export function renderPcsTemplateEditorPage(templateId) {
...
export function handlePcsTemplatesInput(target) {
    const fieldNode = target.closest('[data-pcs-template-field]');
    if (!fieldNode)
        return false;
...
export function handlePcsTemplatesEvent(target) {
    const actionNode = target.closest('[data-pcs-template-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'selector-confirm') {
        ...
        state.editor.selectorSelectedIds.forEach((workItemId) => {
            ...
            if (!definition)
                return;
            sequenceNo += 1;
            const editRule = getTemplateNodeEditRule(state.editor.styleType, definition.workItemTypeCode);
            state.editor.nodes = [
                ...state.editor.nodes,
                {
                    templateNodeId: '',
...
export function isPcsTemplatesDialogOpen() {
