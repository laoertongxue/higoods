...
import { getTransferBagContentDisplayItems, getTransferBagScanSummaryByQr, listCuttingSewingTransferBags, } from '../data/fcs/cutting/sewing-dispatch.ts';
import { renderPdaFrame } from './pda-shell';
function getQueryValue(name) {
    const query = (appStore.getState().pathname.split('?')[1] || '');
    return new URLSearchParams(query).get(name) || '';
}
function getCurrentBagNo(routeBagNo) {
    return routeBagNo || getQueryValue('bagNo') || listCuttingSewingTransferBags()[0]?.transferBagNo || '';
}
export function renderPdaTransferBagDetailPage(routeBagNo) {
