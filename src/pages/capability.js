import { capabilityTagsFull, tagCategories, } from '../data/fcs/capability-mock-data';
import { tagStatusConfig, } from '../data/fcs/capability-types';
...
function getPagedTags(filteredTags) {
...
function renderTagFormDialog() {
    if (state.dialog.type !== 'tag-form')
        return '';
...
function renderCategoryDisableConfirmDialog() {
    if (state.categoryDisableConfirmId === null)
        return '';
...
function renderCategoryDialog() {
    if (state.dialog.type !== 'category-manage')
        return '';
        ...
        ? `
            ...
            : ''}
                  ...
                  <div class="flex gap-2">
                    <button class="rounded-md bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700" data-cap-action="submit-category-form">${editingCategory ? '保存' : '创建'}</button>
        ...
        : state.categories
            ...
            return `
                            <tr class="border-b last:border-0">
                              ...
                              <td class="px-3 py-2">
                                <span class="inline-flex rounded border px-2 py-0.5 text-xs ${getStatusBadgeClass(category.status)}">${escapeHtml(status.label)}</span>
                              </td>
...
function renderDisableConfirmDialog() {
    if (state.dialog.type !== 'disable-confirm')
        return '';
...
function renderPagination(total) {
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-1">
        ...
        .map((page) => `<button class="rounded-md border px-3 py-1 text-sm ${page === state.currentPage ? 'bg-blue-600 text-white' : 'hover:bg-muted'}" data-cap-action="goto-page" data-page="${page}">${page}</button>`)
        .join('')}
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
          <label class="space-y-2">
            ...
            <select data-cap-filter="category" class="w-full rounded-md border px-3 py-2 text-sm">
              ...
              ${state.categories
        .map((category) => `<option value="${category.id}" ${state.filterCategory === category.id ? 'selected' : ''}>${escapeHtml(category.name)}</option>`)
        .join('')}
            </select>
          </label>
...
function updateCategoryNameForTags(categoryId, categoryName) {
    state.tags = state.tags.map((tag) => (tag.categoryId === categoryId ? { ...tag, categoryName } : tag));
}
function toggleCategoryStatus(categoryId) {
    state.categories = state.categories.map((category) => category.id === categoryId
        ? {
            ...category,
...
export function handleCapabilityEvent(target) {
    const filterNode = target.closest('[data-cap-filter]');
    if (filterNode instanceof HTMLInputElement || filterNode instanceof HTMLSelectElement) {
        ...
        if (filter === 'system')
            state.filterSystemTag = value;
        state.currentPage = 1;
        state.tagActionMenuId = null;
        state.categoryActionMenuId = null;
        return true;
    }
    const fieldNode = target.closest('[data-cap-field]');
    ...
    const actionNode = target.closest('[data-cap-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'confirm-disable-tag') {
        ...
        if (!tagId)
            return true;
        state.tags = state.tags.map((tag) => tag.id === tagId
            ? {
                ...tag,
                status: 'inactive',
    ...
    if (action === 'submit-category-form') {
        ...
        if (state.categoryEditingId) {
            state.categories = state.categories.map((category) => category.id === state.categoryEditingId
                ? {
                    ...category,
                    name,
                }
    ...
    if (action === 'confirm-disable-category') {
        if (!state.categoryDisableConfirmId)
            return true;
...
export function handleCapabilitySubmit(form) {
...
export function isCapabilityDialogOpen() {
