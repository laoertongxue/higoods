...
function getMockPrintUnit(index) {
    return index % 2 === 0 ? 'Yard' : '米';
}
function buildMockAdditionRecords(demandId, baseQty, unit, index) {
    if (index % 4 === 0) {
        return [
            {
...
const DEMAND_FACTS = listPrepRequirementDemands('PRINT').map((item, index) => {
...
function deriveStatus(demand) {
    const satisfiedQty = sumSatisfiedQty(demand);
    if (satisfiedQty === 0)
        return '待满足';
    if (satisfiedQty < getFinalRequiredQty(demand))
        return '部分满足';
    if (demand.handoverCompleted)
        return '已完成交接';
    return '已满足';
}
function isWarehousePrepared(demand) {
...
function getFilteredDemands() {
    ...
    return DEMANDS.filter((demand) => {
        ...
        if (state.statusFilter !== '全部' && status !== state.statusFilter)
            return false;
        if (!keyword)
            return true;
        const haystack = [
...
function scheduleScrollToSourcesSection() {
    if (typeof window === 'undefined')
        return;
    window.setTimeout(() => {
        const section = document.querySelector('[data-print-req-section="sources"]');
...
function renderSourceLines(demand, options) {
...
function renderAdditionSummary(demand) {
...
function renderAdditionRecordsTable(demand) {
    if (demand.printQtyAdditionRecords.length === 0) {
...
function renderDetailDrawer() {
    ...
    const focusedPreparationOrderNo = state.focusedPreparationOrderNo ?? demand.sources[0]?.preparationOrderNo ?? null;
    const focusedPreparation = demand.sources.find((source) => source.preparationOrderNo === focusedPreparationOrderNo) ?? demand.sources[0] ?? null;
    return `
...
function renderBatchViewerDialog() {
...
function renderAddQuantityDialog() {
...
export function renderProcessPrintRequirementsPage() {
    const statusOptions = ['全部', '待满足', '部分满足', '已满足', '已完成交接'];
    return `
    <div class="space-y-4">
...
function openDetail(demandId, focusSources, preparationOrderNo) {
...
function createOrderFromDemand(demandId) {
...
export function handleProcessPrintRequirementsEvent(target) {
    const addFieldNode = target.closest('[data-print-req-add-field]');
    ...
    const fieldNode = target.closest('[data-print-req-field]');
    ...
    const actionNode = target.closest('[data-print-req-action]');
    if (!actionNode)
        return false;
...
export function isProcessPrintRequirementsDialogOpen() {
