...
import { PATTERN_LICENSE_STATUS_LABELS, PATTERN_LIFECYCLE_STATUS_LABELS, PATTERN_PARSE_STATUS_LABELS, PATTERN_REVIEW_STATUS_LABELS, approvePatternAsset, archivePatternAsset, createPatternVersion, disablePatternAsset, getPatternBlob, getPatternAssetById, getPatternCategorySecondaryList, getPatternLibraryConfig, getPatternReferenceAvailability, listPatternApplicableCategoryOptions, listPatternVersions, persistPatternParsedFile, rejectPatternAsset, restorePatternAsset, submitPatternAssetReview, updatePatternAsset, waitForPatternLibraryPersistence, } from '../data/pcs-pattern-library';
...
function revokeVersionUrls() {
    revokePreviewUrls();
    revokeStorageUrls();
}
function ensureVersionPreview(version) {
    if (!version)
        return;
...
function ensureVersionStorageUrl(version) {
    if (!version)
        return;
    if (version.file_url)
        return;
    if (state.storageUrls[version.id])
        return;
    if (state.storageLoadingVersionIds.includes(version.id))
        return;
...
function getSelectedVersion(assetId) {
...
function renderTabContent() {
    if (state.activeTab === '基础信息')
        return renderBasicInfoTab();
    if (state.activeTab === '文件与版本')
        return renderVersionTab();
    if (state.activeTab === '标签与识别结果')
        return renderTagsTab();
    if (state.activeTab === '引用关系')
        return renderReferencesTab();
    if (state.activeTab === '版权与授权')
        return renderLicenseTab();
    return renderLogsTab();
}
export function renderPcsPatternLibraryDetailPage(assetId) {
    syncState(assetId);
    const asset = getPatternAssetById(assetId);
    if (!asset) {
        return `
...
export function handlePcsPatternLibraryDetailEvent(target) {
    const actionNode = target.closest('[data-pattern-library-detail-action]');
...
export function handlePcsPatternLibraryDetailInput(target) {
    const field = target.dataset.patternLibraryDetailField;
    if (!field)
        return false;
...
export function isPcsPatternLibraryDetailDialogOpen() {
    return false;
}
