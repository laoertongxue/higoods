...
import { PATTERN_DUPLICATE_STATUS_LABELS, PATTERN_LICENSE_STATUS_LABELS, PATTERN_LIFECYCLE_STATUS_LABELS, PATTERN_PARSE_STATUS_LABELS, PATTERN_REVIEW_STATUS_LABELS, archivePatternAsset, batchUpdatePatternAssets, disablePatternAsset, exportPatternLibraryRows, getPatternBlob, getPatternCategorySecondaryList, getPatternLibraryConfig, getPatternLibraryStats, getPatternLibraryTabCounts, getPatternReferenceAvailability, getPatternAssetById, listPatternAssets, listSimilarPatternAssets, restorePatternAsset, submitPatternAssetReview, } from '../data/pcs-pattern-library';
const state = {
    currentTab: '全部',
    search: '',
    usageType: '全部',
...
function ensureVersionPreview(record) {
    const version = record.currentVersion;
    if (!version)
        return;
    if (version.preview_url || version.thumbnail_url)
        return;
    if (state.previewUrls[version.id])
        return;
...
function getVersionPreviewUrl(record) {
...
function getBgClass(background) {
    if (background === '深色')
        return 'bg-slate-800';
    if (background === '网格')
        return 'bg-[linear-gradient(45deg,#f8fafc_25%,transparent_25%),linear-gradient(-45deg,#f8fafc_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#e2e8f0_75%),linear-gradient(-45deg,transparent_75%,#e2e8f0_75%)] bg-[length:24px_24px] bg-[position:0_0,0_12px,12px_-12px,-12px_0px]';
    return 'bg-slate-50';
}
function getFilteredRecords() {
    ...
    return listPatternAssets().filter((record) => {
        if (state.currentTab === '待补录' && record.maintenance_status !== '待补录')
            return false;
        if (state.currentTab === '待审核' && record.review_status !== 'pending')
            return false;
        if (state.currentTab === '解析失败' && record.parse_status !== 'failed')
            return false;
        if (state.currentTab === '疑似重复' && record.duplicate_status !== 'suspected')
            return false;
        if (state.currentTab === '已归档' && record.lifecycle_status !== 'archived')
            return false;
        const text = [
            record.pattern_code,
            record.pattern_name,
        ...
        if (keyword && !text.includes(keyword))
            return false;
        if (state.usageType !== '全部' && record.usage_type !== state.usageType)
            return false;
        if (state.categoryPrimary !== '全部' && (record.category_primary ?? record.category) !== state.categoryPrimary)
            return false;
        if (state.categorySecondary !== '全部' && (record.category_secondary ?? '') !== state.categorySecondary)
            return false;
        if (state.styleTag !== '全部' && !record.style_tags.includes(state.styleTag))
            return false;
        if (state.primaryColor !== '全部' && !record.color_tags.includes(state.primaryColor))
            return false;
        if (state.hotFlag !== '全部' && (record.hot_flag ? '是' : '否') !== state.hotFlag)
            return false;
        if (state.licenseStatus !== '全部' && record.license_status !== state.licenseStatus)
            return false;
        if (state.maintenanceStatus !== '全部' && record.maintenance_status !== state.maintenanceStatus)
            return false;
        if (state.parseStatus !== '全部' && record.parse_status !== state.parseStatus)
            return false;
        if (state.reviewStatus !== '全部' && record.review_status !== state.reviewStatus)
            return false;
        if (state.sourceType !== '全部' && record.source_type !== state.sourceType)
            return false;
        if (state.uploadedBy !== '全部' && record.updated_by !== state.uploadedBy && record.created_by !== state.uploadedBy)
            return false;
        if (state.referencedFlag !== '全部' && (record.reference_count > 0 ? '已引用' : '未引用') !== state.referencedFlag)
            return false;
        if (state.duplicateFlag !== '全部' && (record.duplicate_status === 'suspected' ? '是' : '否') !== state.duplicateFlag)
            return false;
...
function getStatusBadge(text, tone) {
    const map = {
...
function renderPreviewSurface(record) {
...
function renderPreviewDrawer() {
    if (!state.previewDrawerOpen || !state.previewAssetId)
        return '';
    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        <select class="h-10 w-full rounded-md border px-3 text-sm" data-pattern-library-field="batchReviewStatus">
          ${['保持不变', 'draft', 'pending', 'approved', 'rejected'].map((value) => `<option value="${value}" ${state.batchReviewStatus === value ? 'selected' : ''}>${value === '保持不变' ? value : PATTERN_REVIEW_STATUS_LABELS[value]}</option>`).join('')}
        </select>
      </div>
      <div>
        ...
        <select class="h-10 w-full rounded-md border px-3 text-sm" data-pattern-library-field="batchLifecycleStatus">
          ${['保持不变', 'active', 'inactive', 'archived'].map((value) => `<option value="${value}" ${state.batchLifecycleStatus === value ? 'selected' : ''}>${value === '保持不变' ? value : PATTERN_LIFECYCLE_STATUS_LABELS[value]}</option>`).join('')}
        </select>
      </div>
    </div>
  `;
...
export function handlePcsPatternLibraryEvent(target) {
    const actionNode = target.closest('[data-pattern-library-action]');
    ...
    if (action === 'confirm-batch-update') {
        batchUpdatePatternAssets({
            ...
            maintenanceStatus: state.batchMaintenanceStatus === '保持不变' ? undefined : state.batchMaintenanceStatus,
            reviewStatus: state.batchReviewStatus === '保持不变' ? undefined : state.batchReviewStatus,
            lifecycleStatus: state.batchLifecycleStatus === '保持不变' ? undefined : state.batchLifecycleStatus,
            updatedBy: '档案管理员',
        });
        state.batchDrawerOpen = false;
    ...
    if (action === 'set-preview-mode') {
        state.previewMode = actionNode.dataset.previewMode || '原图';
        return true;
    }
    if (action === 'set-preview-background') {
        state.previewBackground = actionNode.dataset.previewBackground || '浅色';
        return true;
    }
...
export function handlePcsPatternLibraryInput(target) {
    const field = target.dataset.patternLibraryField;
    if (!field)
        return false;
    ...
    if (field === 'licenseStatus')
        state.licenseStatus = value;
    if (field === 'maintenanceStatus')
        state.maintenanceStatus = value;
    if (field === 'parseStatus')
        state.parseStatus = value;
    if (field === 'reviewStatus')
        state.reviewStatus = value;
    if (field === 'sourceType')
        state.sourceType = value;
    if (field === 'uploadedBy')
        state.uploadedBy = value;
    if (field === 'updatedWithin')
        state.updatedWithin = value;
    if (field === 'referencedFlag')
        state.referencedFlag = value;
    if (field === 'duplicateFlag')
        state.duplicateFlag = value;
    if (field === 'batchMaintenanceStatus')
        state.batchMaintenanceStatus = value;
    if (field === 'batchReviewStatus')
        state.batchReviewStatus = value;
    if (field === 'batchLifecycleStatus')
        state.batchLifecycleStatus = value;
    return true;
}
export function isPcsPatternLibraryDialogOpen() {
    return state.previewDrawerOpen || state.batchDrawerOpen;
}
export function renderPcsPatternLibraryPage() {
    if (!state.notice) {
