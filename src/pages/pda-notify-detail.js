import { initialNotifications } from '../data/fcs/store-domain-progress.ts';
...
function markNotificationRead(notificationId) {
    ...
    if (index < 0)
        return;
    if (initialNotifications[index].readAt)
        return;
...
function renderNotificationDetail(notificationId) {
...
export function renderPdaNotifyDetailPage(id) {
...
export function handlePdaNotifyDetailEvent(target) {
    if (!ensurePdaSessionForAction())
        return true;
    const actionNode = target.closest('[data-pda-notify-detail-action]');
