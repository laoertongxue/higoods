...
import { FLAT_DIMENSION_META, } from '../data/pcs-config-dimensions.ts';
import { canDeleteProductCategoryNode, canDisableProductCategoryNode, deleteProductCategoryNode, getConfigDimensionOption, getProductCategoryNode, listChildProductCategories, listConfigDimensionOptions, listConfigWorkspaceSummaries, listRootProductCategories, saveConfigDimensionOption, saveProductCategoryNode, } from '../data/pcs-config-workspace-repository.ts';
...
const state = {
    activeDimension: PRODUCT_CATEGORY_META.id,
    search: '',
    statusFilter: '全部状态',
    notice: null,
    flatModal: null,
    ...
    expandedCategoryIds: new Set(['product-category-1', 'product-category-2']),
};
...
function renderAuditPanel(dimensionName, updatedAt, updatedBy, logCount) {
    return `
...
function renderModalShell(title, subtitle, body, closeAction, footer = '') {
    return `
...
function renderEditableInput(label, field, value, required = false, placeholder = '') {
    return `
    <label class="block">
...
function renderEditableSelect(label, field, value) {
    return `
    <label class="block">
...
function getFilteredFlatOptions(dimensionId) {
    ...
    return items.filter((item) => {
        ...
        const matchStatus = state.statusFilter === '全部状态' ||
...
function renderFlatDimensionContent(dimensionId) {
    ...
    <div class="space-y-4">
      ...
      <section class="flex flex-wrap items-center gap-3">
        ...
        <select class="h-11 rounded-xl border border-slate-200 bg-white px-4 text-sm text-slate-700 outline-none focus:border-blue-500" data-pcs-config-workspace-field="status-filter">
          ${['全部状态', '启用中', '停用中'].map((item) => `<option value="${escapeHtml(item)}" ${state.statusFilter === item ? 'selected' : ''}>${escapeHtml(item)}</option>`).join('')}
        </select>
...
function renderCategoryRuleNotice() {
    return `
    <section class="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">
      <div class="flex items-start gap-3">
...
function renderCategoryActionButton(icon, action, dataset, danger = false, disabled = false) {
...
function renderFlatLogsModal() {
    ...
    if (state.flatModal.mode === 'logs') {
        return renderModalShell(`${meta.name}日志详情`, `${option.name_zh} ｜ ${option.code}`, `
        <div class="space-y-8">
    ...
    return renderModalShell(`编辑${meta.name}`, `${option.name_zh} ｜ ${option.code}`, `
      <div class="space-y-8">
        <div class="grid gap-8 md:grid-cols-2">
          ${renderReadonlyField('Code（系统自增）', option.code)}
...
function renderFlatCreateModal() {
    if (!state.flatModal || state.flatModal.mode !== 'create')
        return '';
    const meta = getDimensionMeta(state.flatModal.dimensionId);
    return renderModalShell(`新建${meta.name}`, `${meta.description}`, `
      <div class="space-y-8">
        <div class="grid gap-8 md:grid-cols-2">
...
function renderCategoryModal() {
    ...
    if (state.categoryModal.mode === 'logs' && node) {
        return renderModalShell('商品类目日志详情', `${node.name} ｜ ${node.code}`, `
        <div class="space-y-8">
          <div class="grid gap-8 md:grid-cols-2">
    ...
    const subtitle = state.categoryModal.mode === 'create-root'
        ...
        : state.categoryModal.mode === 'create-child'
            ...
            : `${node?.name || '-'} ｜ ${node?.code || '-'}`;
    return renderModalShell(title, subtitle, `
      <div class="space-y-8">
        <div class="grid gap-8 md:grid-cols-2">
...
function openFlatModal(mode, dimensionId, optionId = null) {
...
function openCategoryModal(mode, nodeId = null, parentId = null) {
...
function saveFlatModal() {
    ...
    const result = saveConfigDimensionOption(dimensionId, state.flatModal.mode === 'create' ? null : state.flatModal.optionId, draft, '当前用户');
    closeAllDialogs();
...
function saveCategoryModal() {
    ...
    try {
        const result = saveProductCategoryNode(state.categoryModal.mode === 'edit' ? state.categoryModal.nodeId : null, state.categoryModal.mode === 'create-root' ? null : state.categoryModal.mode === 'create-child' ? state.categoryModal.parentId : getProductCategoryNode(state.categoryModal.nodeId || '')?.parentId || null, draft, '当前用户');
        if (result.parentId) {
...
export function renderPcsConfigWorkspacePage() {
    return `
    <div class="p-4">
      ${renderNotice()}
...
export function handlePcsConfigWorkspaceEvent(target) {
    const actionNode = target.closest('[data-pcs-config-workspace-action]');
    ...
    if (action === 'switch-dimension') {
        const dimensionId = actionNode.dataset.dimensionId;
        if (dimensionId) {
    ...
    if (action === 'open-flat-create') {
        const dimensionId = actionNode.dataset.dimensionId;
        if (dimensionId)
            openFlatModal('create', dimensionId);
        return true;
    }
    if (action === 'open-flat-edit') {
        ...
        if (dimensionId && optionId)
            openFlatModal('edit', dimensionId, optionId);
        return true;
    }
    if (action === 'open-flat-logs') {
        ...
        const optionId = actionNode.dataset.optionId || null;
        if (dimensionId && optionId)
            openFlatModal('logs', dimensionId, optionId);
        return true;
    }
    ...
    if (action === 'toggle-category-expand') {
        ...
        if (state.expandedCategoryIds.has(nodeId))
            state.expandedCategoryIds.delete(nodeId);
        else
            state.expandedCategoryIds.add(nodeId);
        return true;
    }
    ...
    if (action === 'open-category-edit') {
        ...
        if (nodeId)
            openCategoryModal('edit', nodeId);
        return true;
    }
    if (action === 'open-category-logs') {
        const nodeId = actionNode.dataset.nodeId || null;
        if (nodeId)
            openCategoryModal('logs', nodeId);
        return true;
    }
...
export function handlePcsConfigWorkspaceInput(target) {
    const fieldNode = target.closest('[data-pcs-config-workspace-field]');
    const field = fieldNode?.dataset.pcsConfigWorkspaceField;
    if (!fieldNode || !field)
        return false;
    const value = 'value' in fieldNode ? String(fieldNode.value) : '';
    if (field === 'search') {
        state.search = value;
        return true;
    }
    ...
    if (field === 'flat-name-zh') {
        state.flatDraft.nameZh = value;
        return true;
    }
    if (field === 'flat-name-en') {
        state.flatDraft.nameEn = value;
        return true;
    }
    if (field === 'flat-sort-order') {
        state.flatDraft.sortOrder = value;
        return true;
    }
    if (field === 'flat-status') {
        state.flatDraft.status = value;
        return true;
    }
    if (field === 'category-name') {
        state.categoryDraft.name = value;
        return true;
    }
    if (field === 'category-sort-order') {
        state.categoryDraft.sortOrder = value;
        return true;
    }
    if (field === 'category-status') {
        state.categoryDraft.status = value;
        return true;
    }
    return false;
}
export function isPcsConfigWorkspaceDialogOpen() {
    return Boolean(state.flatModal || state.categoryModal);
}
export function resetPcsConfigWorkspaceState() {
