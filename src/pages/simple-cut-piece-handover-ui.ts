import { loadReplacementFabricState } from '../data/fcs/cutting/replacement-fabric-repository.ts'
import { resolveReplacementFabricScan } from '../data/fcs/cutting/replacement-fabric-scan.ts'
import { resolveReplacementFabricTaskContext } from '../data/fcs/cutting/replacement-fabric-source.ts'
import { evaluateReplacementFabricCoverage, type ReplacementFabricState, type ReplacementFabricTicket } from '../data/fcs/cutting/replacement-fabric-fei-tickets.ts'
import { replacementFabricBagTicket } from '../data/fcs/cutting/mixed-transfer-bag-ticket.ts'
import { renderMixedBagTicket } from '../components/ui/mixed-bag-contents.ts'
...
interface State { replacementTickets: ReplacementFabricTicket[]; replacementFacts?: ReplacementFabricState; raw: string; preview?: SimpleCutPieceHandoverPreview; result?: Result; busy: boolean; feedback: string; page: number; commandId: string }
const fresh = (): State => ({ replacementTickets: [], raw: '', busy: false, feedback: '', page: 1, commandId: '' })
const states: Record<Mode, State> = { WEB: fresh(), PDA: fresh() }
const button = (label: string, action: string, disabled = false, primary = false, fullWidth = false) => renderButton({ label, disabled, variant: primary ? 'primary' : 'secondary', className: fullWidth ? 'min-h-12 w-full rounded-xl text-base' : 'min-h-10', action: { prefix: 'simple-cut', action, skipPageRerender: true } })
...
function renderReplacementSection(preview: SimpleCutPieceHandoverPreview, state: State): string {
  const context = resolveReplacementFabricTaskContext(preview.sheet)
  const coverage = evaluateReplacementFabricCoverage(context, state.replacementFacts?.receipts || [], state.replacementTickets)
  return `<section class="space-y-2 rounded border p-3"><h3 class="font-semibold">随裁片交出的换片布</h3><p class="text-xs">每张固定 5 Yard；本任务已交过的面料无需重复交。</p>${context.issues.map(issue => `<p class="text-sm text-red-700">${e(issue)}</p>`).join('')}<div class="space-y-1 text-sm">${context.requiredMaterials.map(material => `<p>${e(material.name)} · ${e(material.code)} · ${e(material.color)}：${coverage.previous.has(material.key) ? '此前已交' : coverage.current.has(material.key) ? '本次随交' : '<strong class="text-amber-700">还需换片布</strong>'}</p>`).join('') || '<p>无需换片布</p>'}</div><div class="flex gap-2"><input data-simple-replacement-input data-skip-page-rerender="true" class="h-11 min-w-0 flex-1 rounded border px-2" placeholder="扫描换片布菲票二维码">${button('加入','scan-replacement',state.busy)}</div>${state.replacementTickets.map(ticket => `<div>${renderMixedBagTicket(replacementFabricBagTicket(ticket))}${button('移出本次', 'remove-replacement:' + ticket.id)}</div>`).join('')}<p class="text-sm">本次 ${state.replacementTickets.length} 张 / ${state.replacementTickets.length * 5} Yard</p></section>`
}
...
export function renderSimpleCutPieceContent(mode: Mode): string {
  ...
  if (state.result) {
    ...
    if (mode === 'PDA') {
      ...
      const time = Number.isNaN(date.getTime()) ? event.occurredAt : date.toLocaleString('zh-CN', { hour12: false })
      return `<div data-simple-cut-content class="flex min-h-0 flex-1 flex-col bg-slate-50"><div data-simple-cut-scroll class="min-h-0 flex-1 overflow-y-auto p-4"><section class="rounded-2xl bg-white p-5 text-center"><div class="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-green-100 text-green-700" aria-hidden="true"><svg viewBox="0 0 24 24" class="h-8 w-8" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m5 12 4 4L19 6"/></svg></div><h3 class="text-xl font-semibold text-green-700">已交出 · 工厂已接收</h3><p class="mt-5 text-3xl font-bold text-slate-900">${p.totalPieceQty}<span class="ml-1 text-base font-medium">片</span></p><p class="mt-1 text-sm text-slate-500">本次交出 ${p.tickets.length} 张裁片票</p><p class="mt-1 text-sm">换片布 ${p.replacementFabricTickets?.length || 0} 张 / ${(p.replacementFabricTickets?.length || 0)*5} Yard</p></section><dl class="mt-3 space-y-4 rounded-2xl bg-white p-4 text-sm">${[['接收工厂',p.factoryName],['领取 PPIC',p.ppicName],['任务单号',p.taskSheetNo],['交出时间',time]].map(([label,value])=>`<div class="flex items-start justify-between gap-4"><dt class="shrink-0 text-slate-500">${label}</dt><dd class="min-w-0 break-words text-right font-medium">${e(value)}</dd></div>`).join('')}</dl></div><footer class="shrink-0 border-t bg-white p-3">${button('继续扫描','reset',false,true,true)}</footer></div>`
    }
    return `<div data-simple-cut-content class="space-y-4 p-4"><h3 class="text-lg font-semibold text-green-700">已交出 · 工厂已接收</h3><dl class="space-y-2 break-words text-sm"><div>交出单：${e(p.handoverOrderNo)}</div><div>交出记录：${e(p.handoverRecordNo)}</div><div>${p.tickets.length} 张裁片票 / ${p.totalPieceQty} 片</div><div>换片布 ${p.replacementFabricTickets?.length || 0} 张 / ${(p.replacementFabricTickets?.length || 0)*5} Yard</div><div>接收工厂：${e(p.factoryName)}</div><div>领取 PPIC：${e(p.ppicName)}</div><div>交出人：${e(event.operatorName || '')}</div><div>时间：${e(event.occurredAt)}</div></dl><a class="block text-blue-700 underline" href="/fcs/craft/cutting/handover-records/${encodeURIComponent(p.handoverRecordId)}">查看交出记录</a>${button('继续下一单','reset',false,true)}</div>`
  }
  return `<div data-simple-cut-content class="flex min-h-0 flex-1 flex-col"><div class="min-h-0 flex-1 space-y-3 overflow-y-auto p-4" data-simple-cut-scroll><div><label for="simple-cut-input-${mode}" class="block text-sm font-medium">任务单号</label><div class="mt-1 flex gap-2"><input id="simple-cut-input-${mode}" data-simple-cut-input data-skip-page-rerender="true" class="h-11 min-w-0 flex-1 rounded border px-3 text-sm" placeholder="输入或扫描任务单号" value="${e(state.raw)}" ${state.busy?'disabled':''}>${mode==='PDA'?button('扫描','read',state.busy,true):''}</div></div>${mode==='WEB'?`<div>${button('读取任务','read',state.busy,true)}</div>`:''}<p role="status" aria-live="polite" class="text-sm ${state.busy?'text-blue-700':'text-red-700'}">${e(state.feedback)}</p>${mode==='WEB'&&!permission?`<p class="text-xs text-slate-600">当前仓管账号：${e(simpleCutPieceActor(mode).operatorName)} · ${e(simpleCutPieceActor(mode).operatorId)}</p>`:''}${permission?`<p class="rounded bg-amber-50 p-2 text-sm">${e(permission)} <a class="underline" href="/fcs/pda/auth/login?returnTo=${encodeURIComponent(typeof location==='undefined'?'':location.pathname+location.search)}">登录裁床账号</a></p>`:''}${state.preview?renderPreview(state.preview,state,mode):'<p class="py-6 text-center text-sm text-slate-500">读取后核对工厂、PPIC 和裁片清单，再确认交出。</p>'}</div><footer class="shrink-0 space-y-2 border-t bg-white p-3"><p class="text-xs">确认后，本批裁片视为 PPIC 已领取、工厂已接收。确认包含全部分页清单。</p>${button(state.busy?'处理中…':`确认交出（${state.preview?.tickets.length||0} 张菲票 / ${state.preview?.totalAvailablePieceQty||0} 片）`,'confirm',state.busy||!!permission||!state.preview?.tickets.length||!!state.preview?.blockingReasons.length,true,mode==='PDA')}</footer></div>`
}
...
export function prepareSimpleCutPiecePage(mode: Mode, raw = ''): void {
  ...
  setTimeout(() => {
    ...
    host.addEventListener('keydown', event => {
      ...
      if (event.key === 'Enter' && (event.target as HTMLElement).matches('[data-simple-replacement-input]')) {event.preventDefault();event.stopPropagation();void perform(mode,'scan-replacement',host)}
...
async function perform(mode: Mode, action: string, host: HTMLElement): Promise<void> {
  ...
  if(action.startsWith('remove-replacement:')) {state.replacementTickets=state.replacementTickets.filter(ticket=>ticket.id!==action.slice('remove-replacement:'.length));refresh(mode);return}
  if(action==='scan-replacement'&&state.preview){
    state.busy=true
    try {const raw=host.querySelector<HTMLInputElement>('[data-simple-replacement-input]')?.value || '';const ticket=await resolveReplacementFabricScan(raw,state.preview.sheet.assignment.productionOrderId)
      if(state.replacementTickets.some(item=>item.id===ticket.id))throw new Error('本次已扫描这张票，无需重复。')
      const context=resolveReplacementFabricTaskContext(state.preview.sheet)
      if(!context.requiredMaterials.some(material=>material.key===ticket.material.key))throw new Error('本任务不使用这种面料，请核对实物。')
      state.replacementTickets.push(ticket);state.feedback='已加入换片布，确认交出时将与裁片一起保存。'
    } catch(error){state.feedback=(error as Error).message}finally{state.busy=false;refresh(mode)}return
  }
  ...
  try {
    if(action==='read'){state.preview=undefined;state.replacementTickets=[];state.replacementFacts=await loadReplacementFabricState();state.preview=resolveSimpleCutPieceHandover(state.raw);state.raw=state.preview.sheet.taskSheetNo;state.commandId=`SIMPLE:${state.preview.sheet.taskSheetNo}:${state.preview.candidateFingerprint}`;state.page=1;state.feedback=''}
    else if(state.preview){state.result=await confirmSimpleCutPieceHandover({taskSheetNo:state.preview.sheet.taskSheetNo,candidateFingerprint:state.preview.candidateFingerprint,commandId:state.commandId,replacementFabricTicketIds:state.replacementTickets.map(ticket=>ticket.id),actor:simpleCutPieceActor(mode)});state.feedback='';document.dispatchEvent(new CustomEvent('simple-cut-piece-handover-saved'))}
