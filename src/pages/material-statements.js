...
import { appendMaterialStatementDraftForSettlement, getMutableMaterialStatementDraftForSettlement, listMaterialIssueSheetsFromRuntime, listMaterialStatementDraftsForSettlement, } from '../data/fcs/store-domain-dispatch-process';
...
function syncMaterialStatementsStateFromPath() {
    ...
    if (keyword !== null)
        state.objectKeyword = keyword;
    if (detail !== null)
        state.detailStatementId = detail;
}
function getIssueSheets() {
...
function showMaterialStatementsToast(message, tone = 'success') {
...
function getLinkedStatementId(materialStatementId) {
    const linked = initialStatementDrafts.find((statement) => statement.status !== 'CLOSED'
        && statement.items.some((item) => item.sourceItemId === materialStatementId));
    return linked?.statementId ?? null;
}
function getOccupiedIssueIds() {
    const set = new Set();
...
function generateMaterialStatementDraft(input, by) {
    ...
    for (const issueId of issueIds) {
        if (occupiedIds.has(issueId))
            return { ok: false, message: '存在已纳入未关闭来源对象的领料事实' };
    }
    ...
    materialStatementSeq += 1;
    const items = issueIds.map((issueId) => {
        const issue = getIssueById(issueId);
        return {
...
function confirmMaterialStatementDraft(materialStatementId, by) {
    const draft = getMutableMaterialStatementDraftForSettlement(materialStatementId);
    if (!draft)
        return { ok: false, message: `领料对账对象 ${materialStatementId} 不存在` };
    if (draft.status === 'CONFIRMED')
        return { ok: true };
    if (draft.status === 'CLOSED')
        return { ok: false, message: '已关闭的对象不允许确认' };
    draft.status = 'CONFIRMED';
    draft.updatedAt = nowTimestamp();
    draft.updatedBy = by;
    return { ok: true };
}
function closeMaterialStatementDraft(materialStatementId, by) {
    const draft = getMutableMaterialStatementDraftForSettlement(materialStatementId);
    if (!draft)
        return { ok: false, message: `领料对账对象 ${materialStatementId} 不存在` };
    if (draft.status === 'CLOSED')
...
function getFilteredObjects(view) {
...
function renderViewChip(view, count) {
    const label = view === 'PENDING_BUILD'
        ? '待生成'
        : view === 'DRAFT'
            ? '草稿中'
            : view === 'CONFIRMED'
    ...
    return `
    <button
      class="${toClassName('inline-flex items-center rounded-full border px-4 py-2 text-sm', state.activeView === view ? 'border-blue-300 bg-blue-50 text-blue-700' : 'hover:bg-muted')}"
...
export function renderMaterialStatementsPage() {
    ...
    const currentObjects = state.activeView === 'DRAFT'
...
export function handleMaterialStatementsEvent(target) {
    const poolFilterNode = target.closest('[data-mst-pool-filter]');
    ...
    const objectFilterNode = target.closest('[data-mst-object-filter]');
    if (objectFilterNode instanceof HTMLInputElement) {
        if (objectFilterNode.dataset.mstObjectFilter === 'keyword')
            state.objectKeyword = objectFilterNode.value;
        return true;
    }
    const fieldNode = target.closest('[data-mst-field]');
    if (fieldNode instanceof HTMLTextAreaElement || fieldNode instanceof HTMLInputElement) {
        if (fieldNode.dataset.mstField === 'remark')
            state.remark = fieldNode.value;
        return true;
    }
    const actionNode = target.closest('[data-mst-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'generate') {
        ...
        const result = generateMaterialStatementDraft({ productionOrderId: selectedOrder, issueIds: state.selected, remark: state.remark.trim() || undefined }, '管理员');
        if (!result.ok) {
    ...
    if (action === 'confirm') {
        ...
        if (result.ok) {
            ...
            showMaterialStatementsToast(result.message ?? '操作失败', 'error');
        return true;
    }
    if (action === 'close') {
        const materialStatementId = actionNode.dataset.materialStatementId;
...
export function isMaterialStatementsDialogOpen() {
