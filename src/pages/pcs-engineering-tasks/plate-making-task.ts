...
export function handlePlateMakingTaskInput(target: Element): boolean {
  const uploadInput = target.closest<HTMLInputElement>('[data-plate-upload-input]')
  if (uploadInput) {
    ...
    const purpose = uploadInput.dataset.uploadPurpose === 'PATTERN_PREVIEW' ? 'PATTERN_PREVIEW' : 'PATTERN_SOURCE'
    void uploadEngineeringTaskFiles({ taskId, itemId: uploadInput.dataset.itemId || 'TASK', purpose, files: uploadInput.files, actor: { userId: operator.operatorId, userName: operator.operatorName, teamName: operator.teamName }, roundNo: detail.task.currentRoundNo })
      .then(() => { const host = document.querySelector<HTMLElement>(`[data-plate-detail="${CSS.escape(taskId)}"]`); if (host) host.outerHTML = renderPcsPlateMakingTaskDetailPage(taskId) })
      .catch((error) => { const feedback = document.querySelector<HTMLElement>(`[data-plate-form="${CSS.escape(taskId)}"] [data-plate-feedback]`); if (feedback) { feedback.hidden = false; feedback.className = 'mx-5 mt-4 rounded-md bg-red-50 px-3 py-2 text-sm text-red-700'; feedback.textContent = error instanceof Error ? error.message : '上传失败，请重试。' } })
    return true
  }
...
export function handlePlateMakingTaskEvent(target: HTMLElement): boolean {
  ...
  const uploadPreview = target.closest<HTMLElement>('[data-plate-upload-preview]')
  if (uploadPreview) {
