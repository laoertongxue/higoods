...
function renderColorDetailPage(taskId: string): string {
  ...
  const canSubmitResults = Boolean(task.colorRequirementConfirmedAt) && ['待开始', '进行中', '返工中'].includes(task.status)
  ...
  return `<div class="space-y-5 p-4" data-engineering-review-detail="color:${escapeHtml(task.taskId)}">${header}<div data-review-ui-feedback role="alert" hidden></div>${renderTaskSummaryCard(task, master)}${renderTaskMasterCard(master)}${stepOne}${stepTwo}${stepThree}${renderTaskReworkRoundsCard(task)}${renderTaskDependencyCard(task)}${renderTaskLogsCard(task, master, 'color')}</div>`
}
