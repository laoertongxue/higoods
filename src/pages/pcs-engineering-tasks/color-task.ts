...
function renderColorDetailPage(taskId: string): string {
  ...
  const stepOneRows = paged.lines.length > 0
    ? paged.lines.map((line) => `<tr class="border-t border-slate-100"><td class="px-4 py-3 text-sm font-medium text-slate-800">${escapeHtml(line.materialSkuId)}</td><td class="px-4 py-3 text-sm text-slate-600">${escapeHtml(line.materialName)}</td><td class="px-4 py-3 text-sm text-slate-600">${escapeHtml(line.materialType)}</td><td class="px-4 py-3 text-sm text-slate-600">${escapeHtml(line.productColor || '-')}</td></tr>`).join('')
  ...
  const canConfirmRequirements = bomLines.length > 0 && ['待开始', '进行中'].includes(task.status)
