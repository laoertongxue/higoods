...
import { getPreSettlementLedgerById, listPreSettlementLedgers, tracePreSettlementLedgerSource, } from '../data/fcs/pre-settlement-ledger-repository.ts';
...
function syncPreSettlementLedgerStateFromPath() {
    const params = getCurrentSearchParams();
    const view = params.get('view');
    const keyword = params.get('keyword');
    if (view === 'task')
        state.activeView = 'TASK_EARNING';
    else if (view === 'quality')
        state.activeView = 'QUALITY_DEDUCTION';
    else if (view === 'open' || view === 'pending')
        state.activeView = 'OPEN';
    else if (view === 'statement' || view === 'bound')
        state.activeView = 'IN_STATEMENT';
    else if (view === 'batch' || view === 'payment' || view === 'effective' || view === 'void')
        state.activeView = 'IN_PREPAYMENT';
    else if (view === 'all')
        state.activeView = 'ALL';
    if (keyword !== null)
        state.keyword = keyword;
}
function showLedgerToast(message, tone = 'success') {
...
function getViewCount(view, ledgers) {
    return ledgers.filter((ledger) => matchesView(ledger, view)).length;
}
function matchesView(ledger, view) {
    if (view === 'ALL')
        return true;
    if (view === 'TASK_EARNING')
        return ledger.ledgerType === 'TASK_EARNING';
    if (view === 'QUALITY_DEDUCTION')
        return ledger.ledgerType === 'QUALITY_DEDUCTION';
    if (view === 'OPEN')
        return ledger.status === 'OPEN';
    if (view === 'IN_STATEMENT')
        return ledger.status === 'IN_STATEMENT';
...
function getFilteredLedgers() {
    ...
    return ledgers.filter((ledger) => {
        if (!matchesView(ledger, state.activeView))
            return false;
        if (state.filterFactory !== '__ALL__' && ledger.factoryId !== state.filterFactory)
            return false;
        if (state.filterCycle !== '__ALL__' && ledger.settlementCycleId !== state.filterCycle)
            return false;
        if (state.filterType !== '__ALL__' && ledger.ledgerType !== state.filterType)
            return false;
        if (state.filterStatus !== '__ALL__' && ledger.status !== state.filterStatus)
            return false;
        if (!keyword)
            return true;
        const haystack = [
...
function getFactoryOptions(ledgers) {
    return Array.from(new Map(ledgers.map((ledger) => [ledger.factoryId, ledger.factoryName])).entries())
        .map(([id, name]) => ({ id, name }))
        .sort((left, right) => left.name.localeCompare(right.name, 'zh-CN'));
}
function getCycleOptions(ledgers) {
    return Array.from(new Map(ledgers.map((ledger) => [
    ...
    ])).values()).sort((left, right) => (left.endAt < right.endAt ? 1 : -1));
}
function renderStatsCard(label, value, subLabel) {
    return `
...
function renderViewChip(view, count) {
    return `
    <button
      class="${toClassName('inline-flex h-9 items-center rounded-full border px-4 text-sm', state.activeView === view ? 'border-blue-300 bg-blue-50 text-blue-700' : 'hover:bg-muted')}"
...
function renderTypeCell(ledger) {
    return `
...
function renderStatusCell(ledger) {
    return `
...
function renderLedgerRows(ledgers) {
    if (!ledgers.length) {
...
function renderLedgerDetail(trace) {
    if (!trace)
        return '';
...
export function renderAdjustmentsPage() {
    ...
    <div class="flex flex-col gap-6 p-6">
      ...
      <section class="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        ...
        ${renderStatsCard('已入预付款批次 / 已预付', allLedgers.filter((item) => item.status === 'IN_PREPAYMENT_BATCH' || item.status === 'PREPAID').length)}
      </section>
...
export function handleAdjustmentsEvent(target) {
    const filterNode = target.closest('[data-adj-filter]');
    if (filterNode instanceof HTMLInputElement || filterNode instanceof HTMLSelectElement) {
        ...
        if (field === 'factory')
            state.filterFactory = filterNode.value;
        if (field === 'cycle')
            state.filterCycle = filterNode.value;
        if (field === 'type')
            state.filterType = filterNode.value;
        if (field === 'status')
            state.filterStatus = filterNode.value;
        return true;
    }
    const actionNode = target.closest('[data-adj-action]');
    if (!actionNode)
        return false;
...
export function isAdjustmentsDialogOpen() {
