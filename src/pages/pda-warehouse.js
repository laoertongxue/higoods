...
import { listWoolWaitHandoverHandoutRecords, listWoolWaitHandoverInboundRecords, listWoolWaitProcessReceiptRecords, listWoolWaitProcessUsageRecords, listWoolWarehouseInventory, listWoolWarehouseLocations, } from '../data/fcs/wool-task-domain.ts';
import { getMobileWarehouseRuntimeContext, renderMobileWarehouseLoginRedirect, renderWarehouseActionCards, renderWarehouseSummaryHeader, } from './pda-warehouse-shared';
import { escapeHtml } from '../utils';
function renderWoolWarehouseTabs(active = 'home') {
    const tabClass = (key) => active === key
...
function renderWoolWarehouseHome() {
...
function renderPostFinishingWarehouseHome(factoryName) {
...
export function renderPdaWarehousePage() {
    const runtime = getMobileWarehouseRuntimeContext();
    if (!runtime)
        return renderMobileWarehouseLoginRedirect();
...
export function handlePdaWarehouseEvent(_target) {
    return false;
}
