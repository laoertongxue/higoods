...
const DEMANDS = listPrepRequirementDemands('DYE').map((item) => ({
...
function deriveStatus(demand) {
    ...
    if (satisfiedQty === 0)
        return '待满足';
    if (satisfiedQty < demand.requiredQty)
        return '部分满足';
...
function scheduleScrollToSourcesSection() {
    if (typeof window === 'undefined')
        return;
    window.setTimeout(() => {
        const section = document.querySelector('[data-dye-req-section="sources"]');
...
function renderBatchViewerDialog() {
  ...
  `;
    return renderDialog({
        title: '仓配明细预览',
        closeAction: { prefix: 'dye-req', action: 'close-batches' },
        width: 'lg',
    }, contentHtml);
}
export function renderProcessDyeRequirementsPage() {
...
export function handleProcessDyeRequirementsEvent(target) {
    const fieldNode = target.closest('[data-dye-req-field]');
    ...
    const actionNode = target.closest('[data-dye-req-action]');
    if (!actionNode)
        return false;
...
export function isProcessDyeRequirementsDialogOpen() {
