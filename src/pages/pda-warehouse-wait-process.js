import { listFactoryWaitProcessStockItems, updateWaitProcessStockLocation, } from '../data/fcs/factory-internal-warehouse.ts';
import { OWN_WOOL_FACTORY_ID } from '../data/fcs/factory-mock-data.ts';
import { FULL_CAPABILITY_FACTORY_ID, listPostFinishingWaitProcessWarehouseRecords, } from '../data/fcs/post-finishing-domain.ts';
import { getWoolWorkOrderById, getWoolYarnUsageSummary, listWoolWaitProcessReceiptRecords, listWoolWaitProcessUsageRecords, listWoolWorkOrders, listWoolWarehouseInventory, recoverWoolYarnToWaitProcessWarehouse, } from '../data/fcs/wool-task-domain.ts';
import { escapeHtml } from '../utils';
import { renderPdaFrame } from './pda-shell';
import { buildWarehouseDifferenceText, escapeAttr, formatWarehouseDateTime, getCurrentFactoryWarehouseByKind, getWaitProcessSourceActionLabel, getWaitProcessSourceStatusLabel, getMobileWarehouseRuntimeContext, getWarehousePositionOptions, renderCompactFieldList, renderMobilePageEmptyState, renderSectionFilterChips, renderStatusPill, renderWarehouseSummaryHeader, resolveTaskRoute, resolveWaitProcessSourceRoute, resolveWarehouseInboundRecordRoute, } from './pda-warehouse-shared';
...
function getPostFinishingWaitProcessRows() {
...
function renderPostFinishingFlowSummary(record) {
    return record.flowRecords
        .slice(-3)
        .map((flow) => `${flow.flowType}${flow.qty}${flow.qtyUnit}`)
        .join(' / ') || '-';
}
function renderPostFinishingWaitProcessDetailDrawer() {
...
function renderPostFinishingWaitProcessPage() {
...
function renderWoolWaitProcessPage() {
...
export function renderPdaWarehouseWaitProcessPage() {
    const runtime = getMobileWarehouseRuntimeContext();
    if (!runtime)
    ...
    if (runtime.factoryId === FULL_CAPABILITY_FACTORY_ID)
        return renderPostFinishingWaitProcessPage();
    if (runtime.factoryId === OWN_WOOL_FACTORY_ID)
        return renderWoolWaitProcessPage();
    const rows = getRows();
    const content = `
    <div class="space-y-4 px-4 pb-5 pt-4">
...
export function handlePdaWarehouseWaitProcessEvent(target) {
