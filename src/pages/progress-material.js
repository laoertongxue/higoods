...
import { getTaskTypeLabel, listMaterialRequests, listMaterialRequestsByOrder, } from '../data/fcs/material-request-drafts';
import { getWarehouseExecutionDocById, getWarehouseExecutionSummaryByOrder, listWarehouseExecutionDocsByMaterialRequestNo, listWarehouseExecutionDocsByOrder, } from '../data/fcs/warehouse-material-execution';
import { getRuntimeTaskById, isRuntimeTaskExecutionTask, listRuntimeTasksByBaseTaskId, } from '../data/fcs/runtime-process-tasks';
import { buildReplenishmentProjection } from './process-factory/cutting/replenishment-projection';
import { buildCutOrderLink, buildMaterialPrepLink, buildProductionOrderLink, buildReplenishmentLink, } from '../data/fcs/fcs-route-links';
...
const STATUS_VARIANT_CLASS_MAP = {
...
const hasShortageOptions = [
    { value: 'ALL', label: '全部' },
    { value: 'YES', label: '是' },
    { value: 'NO', label: '否' },
];
const CUTTING_MATERIAL_TYPE_LABEL = {
    PRINT: '印花面料',
    DYE: '染色面料',
    SOLID: '纯色面料',
    LINING: '里布',
...
function buildQuery(params) {
    ...
    if (params.po)
        search.set('po', params.po);
    if (params.docId)
        search.set('docId', params.docId);
...
function renderProgressBar(percent, widthClass) {
...
function resolveRuntimeTaskForRequest(request) {
    ...
    if (direct && isRuntimeTaskExecutionTask(direct))
        return direct;
    const baseMatches = listRuntimeTasksByBaseTaskId(request.taskId).filter((task) => isRuntimeTaskExecutionTask(task));
...
function getFilteredOrderRows(rows) {
    ...
    return rows.filter((row) => {
        ...
        if (state.hasShortage === 'YES' && row.summary.shortLineCount === 0)
            return false;
        if (state.hasShortage === 'NO' && row.summary.shortLineCount > 0)
            return false;
        if (state.deliveryDateFrom && row.requiredDeliveryDate < state.deliveryDateFrom)
            return false;
        if (state.deliveryDateTo && row.requiredDeliveryDate > state.deliveryDateTo)
            return false;
        return true;
    });
}
function filterMaterialRequestsByKeyword(rows) {
    const keyword = state.keyword.trim().toLowerCase();
    if (!keyword)
        return rows;
    return rows.filter((row) => {
...
function getAggregatedSummary(rows) {
...
function buildCuttingMaterialProgressRows(poId) {
    const replenishmentRows = buildReplenishmentProjection().viewModel.rows;
    const replenishmentMap = replenishmentRows.reduce((accumulator, row) => {
    ...
    return cuttingMaterialPrepGroups
        .flatMap((group) => group.materialLines.map((line) => ({
        ...
        differenceText: line.configuredRollCount === line.receivedRollCount
            ? '—'
...
function renderCuttingMaterialProgressSection(poId) {
...
function renderMaterialRequestSection(rows) {
    return `
...
function renderWarehouseDocsSection(orderIds) {
    const docs = orderIds
        ...
        : docs
            ...
            return `
                        <tr class="border-b last:border-b-0">
                          ...
                          <td class="px-3 py-2">${shortLineCount > 0
                ? renderBadge(String(shortLineCount), 'border-red-200 bg-red-50 text-red-700')
                : '<span class="text-muted-foreground">0</span>'}</td>
                          <td class="px-3 py-2 text-sm text-muted-foreground">${escapeHtml(doc.updatedAt)}</td>
                          <td class="px-3 py-2 text-right">
                            <button class="inline-flex h-8 items-center rounded-md px-2 text-sm hover:bg-muted" data-material-action="open-doc-detail" data-doc-id="${escapeHtml(doc.id)}">
                              查看详情
...
function renderMaterialListView() {
      ...
      <section class="rounded-lg border bg-card">
        ...
        : rows
            ...
            const latestStatus = row.latestDoc
                ...
                : '<span class="text-muted-foreground">未生成执行单</span>';
            return `
                          <tr class="cursor-pointer border-b hover:bg-muted/50" data-material-action="select-po" data-po-id="${escapeHtml(row.productionOrderId)}">
                            ...
                            <td class="px-3 py-2 text-right">
                              <button class="inline-flex h-8 items-center rounded-md px-2 text-sm hover:bg-muted" data-material-action="select-po" data-po-id="${escapeHtml(row.productionOrderId)}">
                                查看详情
                                <i data-lucide="chevron-right" class="ml-1 h-4 w-4"></i>
                              </button>
                            </td>
...
function renderDocDrawer(poId) {
    if (!state.drawerOpen || !state.selectedDocId)
        return '';
        ...
        ? `
                ...
                <section>
                  ...
                  <div class="grid grid-cols-2 gap-4 rounded-lg bg-muted/50 p-4">
                    ...
                    <div>
                      ...
                      <div class="font-medium">${escapeHtml(doc.targetType === 'WAREHOUSE_WORKSHOP'
            ? `${TARGET_TYPE_LABEL[doc.targetType]} · ${doc.warehouseName ?? '-'}`
            : `${TARGET_TYPE_LABEL[doc.targetType]} · ${doc.targetFactoryName ?? '-'}`)}</div>
                    </div>
                    <div>
                  ...
                  <div class="overflow-x-auto rounded-lg border">
                    <table class="w-full min-w-[980px] text-sm">
                      ...
                      <tbody>
                        ${doc.lines
            .map((line) => {
            const finishedQty = doc.docType === 'RETURN'
                ? line.returnedQty
            ...
            return `
                              <tr class="border-b last:border-b-0">
                                ...
                                <td class="px-3 py-2 text-right">${line.shortQty > 0
...
function renderMaterialDetailView(poId, docIdFromQuery) {
    ...
    const docs = state.docTypeFilter === 'ALL'
        ? executionDocs
        : executionDocs.filter((doc) => doc.docType === state.docTypeFilter);
    const shortageLines = docs.flatMap((doc) => doc.lines
        ...
        : materialRequests
            ...
            return `
                          <tr class="border-b last:border-b-0">
                            ...
                            <td class="px-3 py-2 font-mono text-xs">${escapeHtml(row.materialRequestNo)}</td>
                            <td class="px-3 py-2">${escapeHtml(row.materialModeLabel)}</td>
                            <td class="px-3 py-2">${latestDoc
                ? renderBadge(EXECUTION_STATUS_LABEL[latestDoc.status], STATUS_VARIANT_CLASS_MAP[latestDoc.status])
                : '<span class="text-muted-foreground">未生成执行单</span>'}</td>
                            <td class="px-3 py-2 text-sm text-muted-foreground">${escapeHtml(row.updatedAt)}</td>
                          </tr>
                        `;
        })
...
function syncDetailStateByQuery(poId, docIdFromQuery) {
...
export function renderProgressMaterialPage() {
...
export function handleProgressMaterialEvent(target) {
    const fieldNode = target.closest('[data-material-field]');
    ...
    const actionNode = target.closest('[data-material-action]');
    if (!actionNode)
        return false;
...
export function isProgressMaterialDrawerOpen() {
