...
import { buildCuttingExceptionEmptyStateText, buildCuttingExceptionFocusRows, buildCuttingExceptionStats, buildExceptionLatestActionText, cuttingExceptionOwnerRoleLabels, cuttingExceptionRiskMeta, cuttingExceptionSourceLayerLabels, cuttingExceptionSourcePageLabels, cuttingExceptionStatusMeta, cuttingExceptionTypeMeta, filterCuttingExceptions, hasCuttingExceptionFilters, } from '../domain/cutting-exception/helpers';
...
function getCuttingRouteActionLabel(route) {
    const normalizedRoute = normalizeRoute(route);
    if (normalizedRoute === materialPrepPath)
        return '去待加工仓';
    if (normalizedRoute === replenishmentPath)
        return '去补料管理';
    if (normalizedRoute === cutOrdersPath)
        return '去裁片单';
    if (normalizedRoute === fabricWarehousePath)
        return '去裁床仓';
    if (normalizedRoute === productionProgressPath)
        return '去生产单进度';
    return '打开关联页面';
}
const state = {
...
function buildSummaryCard(label, value, _hint, accentClass) {
    return `
...
function getPriorityBuckets(rows) {
    ...
    return {
        ...
        warehouseRows: openRows
            .filter((row) => row.exceptionType === 'INBOUND_PENDING' ||
...
function openDetail(exceptionNo) {
    state.activeExceptionNo = exceptionNo;
    state.processDraft = null;
}
function openProcess(exceptionNo, targetStatus) {
...
function buildActionSummary(targetStatus, note) {
    const suffix = note.trim() ? `：${note.trim()}` : '';
    if (targetStatus === 'IN_PROGRESS')
        return `平台已标记处理中${suffix}`;
    if (targetStatus === 'WAITING_CONFIRM')
        return `平台已标记待确认${suffix}`;
...
function renderFocusColumn(title, _description, rows, emptyText) {
    return `
      ...
      <div class="space-y-6 text-sm">
        ...
        <section class="rounded-lg border p-4">
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            ${getQuickRoutes(row)
        .map((item) => `<button class="rounded-md border px-3 py-2 text-sm hover:bg-muted" data-cutting-exception-action="go-route" data-route="${item.route}">${escapeHtml(item.label)}</button>`)
        .join('')}
          </div>
        </section>
      </div>
    ...
    return uiDrawer({
      ...
      <div class="space-y-6 text-sm">
        ...
        <section class="space-y-4">
          ${renderFilterSelect('目标状态', 'status', draft.targetStatus, [
        { value: 'IN_PROGRESS', label: '处理中' },
        { value: 'WAITING_CONFIRM', label: '待确认' },
        { value: 'CLOSED', label: '已关闭' },
...
export function renderProgressCuttingExceptionCenterPage() {
    refreshRuntimeRows();
    return `
...
export function handleProgressCuttingExceptionCenterEvent(target) {
    const fieldNode = target.closest('[data-cutting-exception-field]');
    if (fieldNode) {
        const field = fieldNode.dataset.cuttingExceptionField;
        if (!field)
            return false;
    ...
    const processFieldNode = target.closest('[data-cutting-exception-process-field]');
    if (processFieldNode) {
        const field = processFieldNode.dataset.cuttingExceptionProcessField;
        if (!field)
            return false;
        const input = processFieldNode;
        updateDraftField(field, input.value);
        return true;
    }
    const actionNode = target.closest('[data-cutting-exception-action]');
    ...
    const targetStatus = actionNode?.dataset.targetStatus;
...
export function isProgressCuttingExceptionCenterDialogOpen() {
