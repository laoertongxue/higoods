...
import { PLATFORM_PROCESS_STATUS_CLASS, listPlatformStatusOptions, } from '../data/fcs/process-platform-status-adapter.ts';
...
const ORDERS = listPrepProcessOrders('PRINT').map((item) => ({
    ...
    quantityDisplayFields: item.quantityDisplayFields?.map((field) => normalizeQuantityDisplayField(field.label, field.value, field.unit, field.text)),
...
const ORDER_STATUS_CLASS = PLATFORM_PROCESS_STATUS_CLASS;
...
const WAITING_RECEIVE_STATUSES = ['待下发', '待接单', '待开工', '准备中'];
const IN_PROCESSING_STATUSES = ['加工中', '待交出', '交出待收货', '收货确认中', '部分交出', '收货差异', '异常'];
const DONE_STATUSES = ['全部交出', '已完成'];
...
function normalizePlannedQtyLabel(label) {
    if (!label || label.includes('计划印花'))
        return '需求单印花数量';
    return label;
}
function normalizeQuantityDisplayField(label, value, unit, text) {
    const normalizedLabel = normalizePlannedQtyLabel(label);
    if (normalizedLabel === label)
        return { label, value, unit, text };
    return {
        label: normalizedLabel,
        value,
        unit,
        text: `${normalizedLabel}：${formatQty(value, unit)}`,
    };
}
function toPrintLengthUnit(unit) {
    return unit === '米' ? '米' : 'Yard';
}
function formatQty(qty, unit) {
    return `${qty.toLocaleString()} ${unit}`;
}
function getSelectedCreateDemands() {
    const demandPool = getDemandOptions();
    return demandPool.filter((item) => state.createForm.selectedDemandIds.includes(item.demandId));
}
function syncCreateQtyFromSelectedDemands() {
...
function getReturnedQty(order) {
    return order.batches.reduce((sum, batch) => sum + batch.returnedQty, 0);
}
function getBatchCount(order) {
    return order.batches.length;
}
function getDemandNosText(order) {
    if (order.linkedDemands.length === 0)
        return '-';
...
function getOrderStockMaterial(order) {
    if (order.stockMaterial)
        return order.stockMaterial;
    if (order.linkedDemands.length === 0)
        return null;
    const uniq = new Map();
...
function getFilteredOrders() {
    ...
    return ORDERS.filter((order) => {
        ...
        if (state.statusFilter !== '全部' && order.status !== state.statusFilter)
            return false;
        if (state.modeFilter !== '全部' && order.createMode !== state.modeFilter)
            return false;
        if (!keyword)
            return true;
        const haystack = [
...
function scheduleScrollToSection(section) {
    if (typeof window === 'undefined')
        return;
    window.setTimeout(() => {
        const node = document.querySelector(`[data-print-order-section="${section}"]`);
        node?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 0);
}
function getDemandOptions() {
    const base = ORDERS.flatMap((order) => order.linkedDemands.map((demand) => ({
        demandId: demand.demandId,
        sourceProductionOrderId: demand.sourceProductionOrderId,
        materialCode: demand.materialCode,
        materialName: demand.materialName,
    ...
    for (const item of merged) {
        if (!byId.has(item.demandId))
            byId.set(item.demandId, item);
    }
    return Array.from(byId.values());
}
function getStockMaterialOptions() {
...
function upsertDynamicDemandOption(option) {
    if (state.dynamicDemandOptions.some((item) => item.demandId === option.demandId))
        return;
...
function consumeCreateIntentIfExists() {
...
function toDateTimeValue(source) {
    const match = source.match(/^(\d{4}-\d{2}-\d{2}) (\d{2}:\d{2})/);
    if (!match)
        return '';
    return `${match[1]}T${match[2]}`;
}
function fromDateTimeValue(value) {
    if (!value)
        return '';
...
function renderPlatformResultFields(order) {
...
function renderResultFlag(label, active, id) {
...
function renderFollowUpLinks(order) {
...
function renderPlatformSyncSection(order) {
    return `
...
function renderCreateDemandPick() {
    const options = getDemandOptions();
    if (options.length === 0) {
...
function getSelectedCreateMaterial() {
    if (!state.createForm.selectedMaterialKey)
        return null;
...
export function renderProcessPrintOrdersPage() {
    consumeCreateIntentIfExists();
    const statusOptions = ['全部', ...listPlatformStatusOptions()];
    const modeOptions = ['全部', '按需求创建', '按备货创建'];
    return `
    <div class="space-y-4">
...
function submitCreate() {
    ...
    ORDERS.unshift({
        ...
        updatedAt: now,
        linkedDemands,
        stockMaterial: form.createMode === '按备货创建' && selectedMaterial
            ? {
...
export function handleProcessPrintOrdersEvent(target) {
    if (state.materialPickerOpen && !target.closest('[data-print-order-material-combobox="true"]')) {
        state.materialPickerOpen = false;
    }
    const createFieldNode = target.closest('[data-print-order-create-field]');
    if (createFieldNode instanceof HTMLInputElement || createFieldNode instanceof HTMLTextAreaElement || createFieldNode instanceof HTMLSelectElement) {
        ...
        if (key === 'createMode') {
            state.createForm.createMode = createFieldNode.value;
        ...
        if (key === 'plannedFeedUnit') {
            state.createForm.plannedFeedUnit = createFieldNode.value;
            return true;
        }
    ...
    const fieldNode = target.closest('[data-print-order-field]');
    ...
    if (fieldNode instanceof HTMLSelectElement) {
        if (fieldNode.dataset.printOrderField === 'modeFilter') {
            state.modeFilter = fieldNode.value;
            state.page = 1;
            closeDetail();
            return true;
        }
        ...
        if (fieldNode.dataset.printOrderField === 'pageSize') {
            state.pageSize = Number(fieldNode.value);
            state.page = 1;
            closeDetail();
            return true;
        }
    }
    const actionNode = target.closest('[data-print-order-action]');
    if (!actionNode)
        return false;
...
export function isProcessPrintOrdersDialogOpen() {
