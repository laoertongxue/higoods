...
export function closePdaWarehouseInboundDialogsOnEscape(): boolean {
    ...
    <div class="fixed inset-0 z-[120]">
      ...
      <section class="absolute inset-x-0 bottom-[72px] rounded-t-3xl border bg-background px-4 py-4 shadow-2xl">
        ...
        <div class="mt-4 rounded-2xl border bg-card px-4 py-4 shadow-sm">
          ${isKolGoto ? `<div class="mb-4">${renderPdaWarehouseBusinessImage({ imageUrl: row.photoList[0], imageAlt: `${row.itemName}（${row.materialSku || row.inboundRecordNo}）实物图`, openAction: 'open-inbound-image', className: 'h-28 w-28' })}</div>` : ''}
          ${renderCompactFieldList(isKolGoto
            ? [
                { label: '入库单号', value: row.inboundRecordNo },
                { label: '来源动作', value: '加工领料' },
                { label: '生成方式', value: '加工领料自动写入' },
                { label: '所属任务', value: row.taskNo || '-' },
                { label: '物料', value: `${row.itemKind} / ${row.itemName}` },
                { label: '物料编码', value: row.materialSku || '-' },
                { label: '入库数量', value: `${row.receivedQty} ${row.unit}` },
                { label: '默认库位', value: `${row.areaName} / ${row.shelfNo} / ${row.locationNo}` },
          ...
          <div class="mt-4 flex gap-2">
            ${isKolGoto ? '' : `<button type="button" class="flex-1 rounded-xl border px-3 py-2.5 text-sm" data-nav="${escapeAttr(resolveInboundSourceRoute(row))}">查看来源</button>`}
            ${
              row.differenceQty !== 0
    ...
    <div class="space-y-4 px-4 pb-5 pt-4">
      ${isKolGoto
        ? `<section class="rounded-2xl border bg-card px-4 py-4 shadow-sm">
            <div class="flex items-start justify-between gap-3">
              <div>
                <div class="text-lg font-semibold text-foreground">加工领料入库记录</div>
                <div class="mt-1 text-xs leading-5 text-muted-foreground">每次加工领料提交后，系统在唯一待加工仓的默认库位自动写入；无需仓管再次接收。</div>
              </div>
              <div class="shrink-0 rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-medium text-primary">${rows.length} 条</div>
            </div>
            <div class="mt-3 rounded-xl bg-muted/50 px-3 py-2 text-xs text-muted-foreground">默认库位：默认库区 / 默认货架 / 默认库位</div>
          </section>`
        : `${renderWarehouseSummaryHeader('入库记录', '中转仓接收、回收入仓或交接接收后，自动生成入库记录。', runtime.overview)}
        ...
        ${
          rows.length > 0
                  ...
                  (row) => `
                    <article class="rounded-2xl border bg-card px-4 py-4 shadow-sm">
                      <div class="flex items-start gap-3">
                        ${isKolGoto ? renderPdaWarehouseBusinessImage({ imageUrl: row.photoList[0], imageAlt: `${row.itemName}（${row.materialSku || row.inboundRecordNo}）实物图`, openAction: 'open-inbound-image' }) : ''}
                        <div class="min-w-0 flex-1">
                      ...
                      <div class="mt-3 space-y-1.5 text-xs text-muted-foreground">
                        ${isKolGoto
                          ? `<div>来源动作：加工领料</div><div>物料：${escapeHtml(`${row.itemKind} / ${row.itemName}`)}</div><div>物料编码：${escapeHtml(row.materialSku || '-')}</div><div>入库数量：${row.receivedQty} ${escapeHtml(row.unit)}</div><div>默认库位：${escapeHtml(row.areaName)} / ${escapeHtml(row.shelfNo)} / ${escapeHtml(row.locationNo)}</div>`
                      ...
                      <div class="mt-4 flex flex-wrap gap-2">
                        ...
                        ${isKolGoto ? '' : `<button type="button" class="rounded-full border px-3 py-1.5 text-xs" data-nav="${escapeAttr(resolveInboundSourceRoute(row))}">查看来源</button>`}
                        ${
                          row.differenceQty !== 0
            ...
            : renderMobilePageEmptyState(
                '暂无入库记录',
                isKolGoto
                  ? '完成一次加工领料后，系统会自动生成入库记录。'
                  : '中转仓接收、回收入仓或交接接收后，会自动生成入库记录。',
              )
        }
      </section>
      ${renderDetailDrawer()}
      ${renderPdaWarehouseImagePreview({ imageUrl: state.imageUrl, imageAlt: state.imageAlt, closeAction: 'close-inbound-image' })}
    </div>
  `
