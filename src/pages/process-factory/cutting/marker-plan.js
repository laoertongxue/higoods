...
import { spreadingOrderStatusMeta } from './marker-spreading-model.ts';
import { buildMaterialLedgerProjectionMap, listStoredMarkerPlanLockLedger, saveStoredMarkerPlanLockLedger, } from '../../../data/fcs/cutting/material-ledger.ts';
import { buildMarkerSchemeFromPlan } from './marker-scheme-adapter.ts';
import { buildMarkerDemandMatchSummary, buildCombinedMarkerPlanContextCandidate, buildMarkerPlanGoSpreadingPath, buildMarkerPlanModeOptions, createMarkerPlanFromContext, deserializeMarkerPlanStorage, findMarkerPlanContextById, findMarkerPlanContextForPlan, getMarkerPlanInitialEditTab, getMarkerPlanReferencedWarning, getMarkerPlanStorageKey, hydrateMarkerPlan, serializeMarkerPlanStorage, } from './marker-plan-model.ts';
import { MARKER_SIZE_CODES, buildMarkerExplodedPieceQtyFormula, buildMarkerFinalUnitUsageFormula, buildMarkerFoldedEffectiveWidthFormula, buildMarkerPlanSystemUnitUsageFormula, buildMarkerPlannedSpreadLengthFormula, buildMarkerTotalPiecesFormula, markerLayoutStatusMeta, markerImageStatusMeta, markerMappingStatusMeta, markerPlanModeMeta, markerPlanStatusMeta, } from './marker-plan-domain.ts';
...
let markerPlanTopInfoResizeObserver = null;
let markerPlanStickyResizeBound = false;
let markerPlanStickySyncToken = 0;
let markerPlanProjectionCache = null;
...
function parseRoute() {
    ...
    if (basePath === LIST_PATH)
        return { kind: 'LIST', id: '' };
    if (basePath === CREATE_PATH)
        return { kind: 'CREATE', id: '' };
    const editMatch = basePath.match(/^\/fcs\/craft\/cutting\/marker-edit\/([^/]+)$/);
    if (editMatch)
        return { kind: 'EDIT', id: decodeURIComponent(editMatch[1]) };
...
function updateMarkerPlanStickyOffsetFromDom() {
    if (typeof document === 'undefined')
        return;
    const root = document.documentElement;
    const shell = document.querySelector('[data-marker-plan-top-shell]');
    if (!shell) {
...
function syncMarkerPlanStickyOffset() {
    ...
    const bindStickyOffset = (remainingRetries = 8) => {
        if (markerPlanStickySyncToken !== syncToken)
            return;
        const shell = document.querySelector('[data-marker-plan-top-shell]');
        if (!shell) {
...
function clearMarkerPlanStickyOffset() {
    if (typeof document === 'undefined')
        return;
...
function getPlanSourceTypeText(row) {
    return row.cutOrderNos.length > 1 ? '组合裁片单' : '裁片单';
}
function getPlanSourceNoText(row) {
    const sourceNos = row.cutOrderNos.filter(Boolean);
    if (!sourceNos.length)
        return '—';
    if (sourceNos.length > 2)
        return `${sourceNos.slice(0, 2).join(' / ')} 等 ${sourceNos.length} 个`;
    return sourceNos.join(' / ');
}
function getPlanTechPackText(row) {
    return [row.techPackStatus || '正式版', row.techPackVersion].filter(Boolean).join(' ') || '正式版';
}
function getPlanBedCountText(row) {
    const beds = row.beds || [];
    if (!beds.length)
        return '0 / 0';
...
function getPlanBedModeText(row) {
    const modeLabels = Array.from(new Set((row.beds || [])
...
function getPlanSpreadingStatusText(row) {
    return row.readyForSpreading ? '可交接铺布' : '未交接铺布';
}
function getPlanDemandMatchSummary(plan) {
    if ('demandMatchSummary' in plan)
        return plan.demandMatchSummary;
    return buildMarkerDemandMatchSummary(plan);
}
function getDemandMatchStatusMeta(status) {
    if (status === '已匹配')
        return { label: status, className: 'bg-emerald-100 text-emerald-700 border border-emerald-200' };
    if (status === '有不足')
        return { label: status, className: 'bg-amber-100 text-amber-700 border border-amber-200' };
    if (status === '有超出')
        return { label: status, className: 'bg-blue-100 text-blue-700 border border-blue-200' };
    if (status === '有差异')
        return { label: status, className: 'bg-rose-100 text-rose-700 border border-rose-200' };
...
function getConfirmationStatusMeta(status) {
    if (status === '已确认')
        return { label: '已确认', className: 'bg-emerald-100 text-emerald-700 border border-emerald-200' };
    if (status === '需调整')
        return { label: '需调整', className: 'bg-rose-100 text-rose-700 border border-rose-200' };
    return { label: '待确认', className: 'bg-amber-100 text-amber-700 border border-amber-200' };
}
function getListTabMeta(listTab) {
...
function getPlanListTabCount(listTab, rows) {
    if (listTab === 'EXCEPTIONS')
        return rows.filter(hasPlanExceptionIssue).length;
    if (listTab === 'ALL')
        return rows.length;
    if (listTab === 'WAITING_LAYOUT')
        return rows.filter((row) => row.layoutStatus !== 'done').length;
    if (listTab === 'DEMAND_DIFF')
        return rows.filter((row) => row.demandMatchSummary.status !== '已匹配' && row.demandMatchSummary.status !== '待编辑唛架').length;
    if (listTab === 'WAITING_CONFIRM')
        return rows.filter((row) => row.layoutStatus === 'done' && row.confirmationStatus !== '已确认').length;
    return rows.filter((row) => row.status === listTab).length;
}
function buildMarkerPlanListTabOptions(viewModel = getViewModel()) {
...
function getProjection() {
    ...
    if (markerPlanProjectionCache?.key === key)
        return markerPlanProjectionCache.projection;
...
function normalizeCutOrderSelectionKey(cutOrderIds) {
    return Array.from(new Set(cutOrderIds.map((id) => String(id || '').trim()).filter(Boolean))).sort().join('|');
}
function parseCutOrderIdFromContextKey(contextKey) {
...
function readCreateCutOrderIdsFromParams(params = getCurrentSearchParams()) {
    const fromSelectedIds = params.getAll('selectedCutOrderIds').flatMap((value) => String(value || '')
        .split(',')
        .map((item) => item.trim())
        .filter(Boolean));
...
function isCuttableCreateSource(params = getCurrentSearchParams()) {
    return params.get('source') === '可排唛架裁片单';
}
function buildContextKeysFromCutOrderIds(cutOrderIds) {
    return cutOrderIds.map((cutOrderId) => `cut-order:${cutOrderId}`);
}
function getCuttableItemsById() {
    return buildCuttablePoolProjection().viewModel.itemsById;
}
function buildSourceLockRowsFromItems(items) {
...
function buildSourceLockRowsFromSelectedCutOrders(cutOrderIds) {
    ...
    const rows = buildSourceLockRowsFromItems(items.filter((item) => Boolean(item)));
...
function validateSourceCutOrderCombination(rows) {
    if (!rows.length)
        return { ok: false, message: '请先选择可排唛架裁片单。' };
...
function upsertDraftLockRows(plan) {
    const rows = plan.sourceCutOrderLockRows || [];
    if (!rows.length)
        return;
    const now = nowText();
    ...
    rows.forEach((row) => {
        const existingIndex = nextRecords.findIndex((record) => record.markerPlanDraftId === plan.id
...
function releaseDraftLocksForPlan(plan, releaseReason = '取消草稿') {
    const now = nowText();
    const nextRecords = listStoredMarkerPlanLockLedger().map((record) => {
        if (record.markerPlanDraftId !== plan.id || record.lockStatus !== '草稿锁定')
            return record;
        return {
            ...record,
            lockStatus: '已释放',
...
function confirmDraftLocksForPlan(plan) {
    const now = nowText();
...
function getContextMap(viewModel = getViewModel()) {
    return Object.fromEntries(viewModel.contexts.map((context) => [context.contextKey, context]));
}
function getSelectedDrawerContexts(viewModel = getViewModel()) {
...
function validateMarkerPlanSourceSelection(contexts) {
    if (!contexts.length)
        return { ok: false, message: '请先选择可排唛架裁片单。' };
...
function syncContextDrawerSelectionDom(viewModel = getViewModel()) {
    if (typeof document === 'undefined')
        return;
    ...
    const selectedSummary = selectedContexts.length
        ...
        : '请先选择可排唛架裁片单';
    const summaryNode = document.querySelector('[data-marker-plan-selection-summary]');
    const messageNode = document.querySelector('[data-marker-plan-selection-message]');
    const confirmButton = document.querySelector('[data-marker-plan-action="confirm-context-create"]');
    ...
    if (confirmButton)
        confirmButton.disabled = !selectionValidation.ok;
    document.querySelectorAll('[data-marker-plan-context-row]').forEach((row) => {
        const contextKey = row.dataset.contextKey || '';
        row.classList.toggle('bg-blue-50/40', state.selectedContextKeys.includes(contextKey));
    });
}
function getMarkerPlanPageRoot() {
    if (typeof document === 'undefined')
        return null;
...
function removeContextDrawerDom() {
    if (typeof document === 'undefined')
        return;
    document.querySelector('[data-marker-plan-context-drawer-shell]')?.remove();
}
function getDraftContext(viewModel = getViewModel()) {
    if (!state.draftPlan)
        return null;
    return findMarkerPlanContextForPlan(viewModel.contexts, state.draftPlan) ?? null;
}
function getDetailPlan(viewModel = getViewModel(), id = parseRoute().id) {
    return viewModel.plansById[id] ?? null;
}
function isEditingReferencedPlan(viewModel = getViewModel()) {
    const route = parseRoute();
    if (route.kind !== 'EDIT')
        return false;
    return Boolean(viewModel.plansById[route.id]?.isReferencedBySpreading);
}
function confirmReferencedStructuralEdit(viewModel = getViewModel()) {
    if (!isEditingReferencedPlan(viewModel))
        return true;
    if (state.referencedStructureEditConfirmed)
        return true;
    const confirmed = window.confirm('当前方案唛架已被铺布引用，修改配比、分配、唛架结构或模式可能影响后续铺布，是否继续？');
    if (confirmed)
        state.referencedStructureEditConfirmed = true;
    return confirmed;
}
...
function buildEditPath(id) {
    return `${EDIT_BASE_PATH}/${encodeURIComponent(id)}`;
}
function buildDetailPath(id) {
...
function getPlanColorOptions(plan, context) {
...
function isHighLowBedMode(mode) {
    return mode === 'high_low' || mode === 'fold_high_low';
}
function isFoldBedMode(mode) {
    return mode === 'fold_normal' || mode === 'fold_high_low';
}
function getMarkerBedModeOptions() {
    return ['normal', 'high_low', 'fold_normal', 'fold_high_low'];
}
function getMarkerBedModePrefix(mode) {
    return isHighLowBedMode(mode) ? 'B' : 'A';
}
function getPlanDemandRows(plan) {
    return buildMarkerSchemeFromPlan(plan).demandRows;
}
function getPlanSizeOptions(plan) {
    ...
    fromDemand.forEach((item) => {
        ...
        if (key && !map.has(key))
            map.set(key, { code: String(item.code || key), name: key });
    });
    return Array.from(map.values());
}
function getPlanColorOptionsFromDemand(plan, context) {
    void context;
...
function buildBedCoverageRows(plan, colorCode, selectedSizeNames) {
...
function buildBedSizeSummary(coverageRows) {
    return coverageRows
        .map((row) => row.sizeName || row.sizeCode)
        .filter(Boolean)
        .join(' / ');
}
function getMarkerMatrixSizeColumns(plan) {
...
function getMarkerMatrixSizeDemandMap(plan) {
    const map = {};
...
function getMarkerMatrixColorRows(plan) {
    return Array.from(new Set(getPlanDemandRows(plan).map((row) => row.colorName || row.colorCode).map((item) => String(item || '').trim()).filter(Boolean)));
}
function getMarkerMatrixMaterialForColor(plan, colorName) {
...
function createMarkerMatrixRows(plan, bed) {
    ...
    return baseRows.map((existing, index) => {
        ...
        const sizeValues = Object.fromEntries(sizeColumns.map((size) => [size, Math.max(Math.round(safeNumber(existing?.sizeValues?.[size])), 0)]));
        const sizePiecePerLayer = normalizeMarkerSizePiecePerLayer(bed, sizeColumns);
        const totalQty = sizeColumns.reduce((total, size) => total + Math.max(safeNumber(sizeValues[size]), 0) * Math.max(safeNumber(sizePiecePerLayer[size]), 0), 0);
        return {
...
function isHighLowMatrixMode(mode) {
    return mode === 'high_low' || mode === 'fold_high_low';
}
function normalizeMarkerSizePiecePerLayer(bed, sizeColumns) {
    return Object.fromEntries(sizeColumns.map((size) => [size, Math.max(Math.round(safeNumber(bed.sizePiecePerLayer?.[size])), 0)]));
}
function getMarkerMatrixCellLayer(row, size) {
    return Math.max(Math.round(safeNumber(row.sizeValues?.[size])), 0);
}
function getMarkerMatrixCellActualLayer(mode, layerCount) {
    const normalized = Math.max(Math.round(safeNumber(layerCount)), 0);
    return isFoldBedMode(mode) ? normalized / 2 : normalized;
}
function getMarkerMatrixRowActiveLayer(mode, row, sizeColumns) {
    return Math.max(...sizeColumns.map((size) => getMarkerMatrixCellActualLayer(mode, getMarkerMatrixCellLayer(row, size))), 0);
}
function getMarkerMatrixRowLength(bed, row) {
    return isHighLowMatrixMode(bed.bedMode) ? Math.max(safeNumber(row.markerLength), 0) : Math.max(safeNumber(bed.markerLength), 0);
}
function getMarkerMatrixBedLengthSummary(bed) {
    if (!isHighLowMatrixMode(bed.bedMode))
        return Math.max(safeNumber(bed.markerLength), 0);
...
function getMatrixActualLayer(mode, demandLayer) {
    const normalized = Math.max(Math.round(safeNumber(demandLayer)), 0);
    return isFoldBedMode(mode) ? normalized / 2 : normalized;
}
function getMarkerMatrixColumnLayerTotals(rows, sizeColumns) {
    return Object.fromEntries(sizeColumns.map((size) => [
        size,
        rows.reduce((total, row) => total + getMarkerMatrixCellLayer(row, size), 0),
    ]));
}
function getMarkerMatrixColumnPieceTotals(rows, sizeColumns, sizePiecePerLayer) {
    return Object.fromEntries(sizeColumns.map((size) => [
        size,
        rows.reduce((total, row) => total + getMarkerMatrixCellPlannedQty(row, size, sizePiecePerLayer), 0),
    ]));
}
function getMarkerMatrixCellPlannedQty(row, size, sizePiecePerLayer) {
    return getMarkerMatrixCellLayer(row, size) * Math.max(Math.round(safeNumber(sizePiecePerLayer[size])), 0);
}
function getMarkerMatrixActualLayerTotal(mode, rows, sizeColumns) {
    const columnTotals = sizeColumns.map((size) => rows.reduce((total, row) => {
...
function validateMarkerMatrixRows(mode, rows, sizeColumns, sizePiecePerLayer) {
    const errors = [];
    ...
    sizeColumns.forEach((size) => {
        ...
        columnValues.forEach((value) => {
            if (value > 0 && ended)
                errors.push(`${size} 列不能出现有层数的行之间夹空行`);
            if (value > 0)
                started = true;
            if (started && value <= 0)
                ended = true;
...
function buildCoverageRowsFromMarkerMatrix(plan, rows, sizePiecePerLayer) {
    const plannedByColorSize = new Map();
    rows.forEach((row) => {
        ...
        Object.entries(row.sizeValues || {}).forEach(([size, value]) => {
            ...
            plannedByColorSize.set(key, (plannedByColorSize.get(key) || 0) + Math.max(Math.round(safeNumber(value)), 0) * Math.max(Math.round(safeNumber(sizePiecePerLayer[size])), 0));
        });
    });
    const allocatedByColorSize = new Map();
...
function recalculateSchemeBed(bed, plan) {
    ...
    const plannedGarmentQty = matrixRows.reduce((total, row) => total + sizeColumns.reduce((rowTotal, size) => rowTotal + getMarkerMatrixCellPlannedQty(row, size, sizePiecePerLayer), 0), 0);
...
function buildDefaultSchemeBed(plan, context, mode = 'normal') {
    ...
    const sizeColumns = getMarkerMatrixSizeColumns(plan);
    const sizePiecePerLayer = Object.fromEntries(sizeColumns.map((size) => [size, 0]));
    const matrixRows = (colors.length ? colors : ['主色']).map((color, index) => ({
        rowId: `${plan.id}-new-marker-row-${Date.now()}-${index + 1}`,
        colorCode: color,
        colorName: color,
        markerLength: 0,
        sizeValues: Object.fromEntries(sizeColumns.map((size) => [size, 0])),
        patternValues: {},
        totalQty: 0,
    }));
...
function normalizeSchemeBeds(plan, beds) {
...
function applySchemeBedsToPlan(plan, beds) {
...
function getCurrentPlanById(planId, viewModel = getViewModel()) {
    if (planId && viewModel.plansById[planId])
        return viewModel.plansById[planId];
    const route = parseRoute();
    if ((route.kind === 'CREATE' || route.kind === 'EDIT') && state.draftPlan)
        return state.draftPlan;
    if (route.kind === 'DETAIL' && viewModel.plansById[route.id])
        return viewModel.plansById[route.id];
    return null;
}
function getContextForPlan(plan, viewModel = getViewModel()) {
    if (!plan)
        return null;
...
function buildMappingDraftForRow(row) {
    return {
...
function getMappingTargetRow(plan) {
    if (!plan || !state.mappingTargetRowId)
        return null;
...
function buildCurrentListExportRows(viewModel = getViewModel()) {
...
function navigateToCreateWithContext(context) {
    const params = new URLSearchParams({
...
function navigateToCreateWithContexts(contexts) {
...
function buildDetailPathWithTab(id, tab) {
    return buildRouteWithQuery(buildDetailPath(id), {
        tab,
    });
}
function buildEditPathWithTab(id, tab) {
    return buildRouteWithQuery(buildEditPath(id), {
        tab,
    });
}
function hydrateDraft(nextPlan, context) {
    if (!context)
        return nextPlan;
...
function applyMarkerMode(plan, context, markerMode) {
    void context;
    const nextPlan = {
        ...plan,
        markerMode,
        foldConfig: markerMode === 'fold_normal' || markerMode === 'fold_high_low'
            ? plan.foldConfig || {
                originalEffectiveWidth: 168,
                foldAllowance: 2,
                foldDirection: '对边折入',
...
function syncStateFromRoute(viewModel = getViewModel()) {
    ...
    if (route.kind === 'CREATE') {
        ...
        if (selectedCutOrderIds.length && isCuttableCreateSource(params)) {
            ...
            if (!schemeValidation.ok)
                setFeedback('warning', schemeValidation.message);
            return;
        }
        state.draftPlan = null;
        state.activeTab = 'basic';
...
function renderActionButton(label, attrs, variant = 'secondary', disabled = false) {
...
function renderReadonlyField(label, value, formula = '') {
    return `
...
function renderInputField(label, value, field, type = 'text') {
    return `
...
function renderTextareaField(label, value, field) {
    return `
...
function renderSelectField(label, field, options, value, dataAttr = 'data-marker-plan-basic-field') {
    return `
...
function renderPlanHeaderActions(route, plan) {
    if (route === 'LIST') {
        return `
...
function buildGoCutOrdersPath(plan) {
...
function buildGoCutOrdersPathFromContext(context) {
    return buildRouteWithQuery(getCanonicalCuttingPath('cut-orders'), {
...
function buildGoMaterialPrepPath(plan) {
...
function buildGoMarkerPlanRefPath(plan) {
    return buildRouteWithQuery(getCanonicalCuttingPath('marker-list'), {
        focusBatchId: plan.markerPlanId || undefined,
    });
}
function buildGoMarkerPlanRefPathFromContext(context) {
    return buildRouteWithQuery(getCanonicalCuttingPath('marker-list'), {
        focusBatchId: context.markerPlanId || undefined,
    });
}
function resolveCurrentPlan(viewModel = getViewModel(), planId = '') {
    if (planId && viewModel.plansById[planId])
        return viewModel.plansById[planId];
    if ((parseRoute().kind === 'CREATE' || parseRoute().kind === 'EDIT') && state.draftPlan)
        return state.draftPlan;
    if (parseRoute().kind === 'DETAIL' && viewModel.plansById[parseRoute().id])
        return viewModel.plansById[parseRoute().id];
    return null;
}
function validateDraftForCompletion() {
...
function getProblemTabForPlan(row) {
    if (row.mappingStatus === 'issue')
        return 'explosion';
    if (row.layoutStatus !== 'done')
        return 'layout';
    if (row.confirmationStatus !== '已确认')
        return 'explosion';
    return 'basic';
}
function hasPlanExceptionIssue(row) {
    return (row.mappingStatus === 'issue' ||
        row.layoutStatus !== 'done' ||
        row.confirmationStatus === '需调整');
}
function renderTopInfoChip(label, attrs, tone = 'blue') {
    return renderWorkbenchFilterChip(label, attrs, tone);
}
function renderTopInfoValue(label, value, formula = '') {
    return `
...
function renderTopInfoStatus(label, badgeHtml) {
    return `
    <article class="rounded-lg border bg-background px-3 py-3">
      <div class="text-xs text-muted-foreground">${escapeHtml(label)}</div>
...
function renderPlanTopInfo(plan, context, options = {}) {
    ...
    const systemUnitUsageFormula = 'systemUnitUsageFormula' in plan
        ? plan.systemUnitUsageFormula
        : buildMarkerPlanSystemUnitUsageFormula(plan);
    const finalUnitUsageFormula = 'finalUnitUsageFormula' in plan
        ? plan.finalUnitUsageFormula
        : buildMarkerFinalUnitUsageFormula(plan.systemUnitUsage, plan.manualUnitUsage);
    const plannedSpreadLengthFormula = 'plannedSpreadLengthFormula' in plan
    ...
    const cutOrderChips = plan.cutOrderNos.length
        ? plan.cutOrderNos
            .map((cutOrderNo, index) => renderTopInfoChip(cutOrderNo, `data-marker-plan-action="go-cut-orders" data-cut-order-id="${escapeHtml(plan.cutOrderIds[index] || '')}" data-cut-order-no="${escapeHtml(cutOrderNo)}" data-production-order-id="${escapeHtml(plan.productionOrderIds[index] || plan.productionOrderIds[0] || '')}" data-production-order-no="${escapeHtml(plan.productionOrderNos[index] || plan.productionOrderNos[0] || '')}"`))
            .join('')
        : '<span class="text-xs text-muted-foreground">—</span>';
    const mergeChip = plan.markerPlanNo
        ? renderTopInfoChip(plan.markerPlanNo, `data-marker-plan-action="go-marker-plan-ref" data-marker-plan-ref-id="${escapeHtml(plan.markerPlanId)}" data-marker-plan-ref-no="${escapeHtml(plan.markerPlanNo)}"`, 'amber')
        : '<span class="text-xs text-muted-foreground">—</span>';
    const productionChips = plan.productionOrderNos.length
        ? plan.productionOrderNos
            .map((productionOrderNo, index) => renderTopInfoChip(productionOrderNo, `data-marker-plan-action="go-production-progress" data-production-order-id="${escapeHtml(plan.productionOrderIds[index] || '')}" data-production-order-no="${escapeHtml(productionOrderNo)}" data-style-code="${escapeHtml(plan.styleCode || '')}" data-spu-code="${escapeHtml(plan.spuCode || '')}" data-material-sku="${escapeHtml(plan.sourceMaterialSku || '')}" data-cut-order-id="${escapeHtml(plan.cutOrderIds[index] || '')}" data-cut-order-no="${escapeHtml(plan.cutOrderNos[index] || '')}" data-marker-plan-ref-id="${escapeHtml(plan.markerPlanId || '')}" data-marker-plan-ref-no="${escapeHtml(plan.markerPlanNo || '')}"`, 'emerald'))
            .join('')
        : '<span class="text-xs text-muted-foreground">—</span>';
    const summaryItems = [
    ...
    <section data-marker-plan-tab-shell class="rounded-lg border bg-card/95 p-2 shadow-sm">
      <div class="flex flex-wrap gap-2">
        ...
        .map((tab) => `
              <button
                type="button"
                class="inline-flex cursor-pointer items-center rounded-md px-3 py-3 text-sm font-medium ${normalizedActiveTab === tab.key
...
function renderMarkerPlanTabLabel(tab) {
    const labels = {
        basic: '来源裁片单',
        explosion: '需求匹配与确认',
        layout: '编辑唛架',
...
function getCreateStepFromActiveTab(activeTab) {
    if (activeTab === 'layout')
        return 'layout';
...
function getCreateStepTargetTab(step) {
    if (step === 'layout')
        return 'layout';
    if (step === 'match')
        return 'explosion';
...
function getCreateStepStatus(plan, step) {
    if (step === 'source')
        return plan ? '已选择' : '当前';
    if (!plan)
        return '未开始';
    if (step === 'layout')
        return markerLayoutStatusMeta[plan.layoutStatus]?.label || '待编辑';
    if (step === 'match')
        return getPlanDemandMatchSummary(plan).status;
    return '未开始';
}
function renderCreateStepNav(plan) {
    ...
    <section class="rounded-xl border bg-card p-4" data-testid="marker-plan-create-step-nav">
      <div class="grid gap-2 md:grid-cols-3 xl:grid-cols-6">
        ...
        return `
              <button
                type="button"
                class="rounded-lg border px-3 py-3 text-left transition ${active
...
function renderBalanceStatusBadge(status) {
...
function renderSizeRatioReadonlyGrid(plan) {
    return `
...
function renderBalanceTable(balanceRows) {
    return `
...
function renderExplosionStatusCards(summary) {
    const cards = [
...
function renderExplosionIssueRows(plan, readOnly = false) {
...
function renderExplosionTab(plan, context = null, readOnly = false) {
    const summary = plan.explosionSummary || {
...
function renderMappingRepairDrawer(plan, context) {
    if (!state.mappingDrawerOpen || !plan)
        return '';
    ...
    const partOptions = Array.from(new Map(plan.pieceExplosionRows
        .map((row) => ({
    ...
    <div class="space-y-4" data-testid="marker-plan-mapping-drawer">
      <div class="grid gap-3 md:grid-cols-2">
        ...
        ${renderSelectField('目标 SKU', 'targetSku', skuOptions.map((skuCode) => ({ value: skuCode, label: skuCode })), state.mappingDraft.targetSku, 'data-marker-plan-mapping-field')}
        ${renderSelectField('颜色归属方式', 'colorMode', [
        { value: 'follow-source', label: '沿用来源颜色' },
        { value: 'specified', label: '指定颜色' },
    ], state.mappingDraft.colorMode, 'data-marker-plan-mapping-field')}
        ${renderSelectField('指定颜色', 'specifiedColor', colorOptions.map((color) => ({ value: color, label: color })), state.mappingDraft.specifiedColor, 'data-marker-plan-mapping-field')}
        ${renderSelectField('部位', 'partCode', partOptions.map((item) => ({ value: item.value, label: item.label })), state.mappingDraft.partCode, 'data-marker-plan-mapping-field')}
        ${renderSelectField('纸样', 'patternCode', patternOptions.map((patternCode) => ({ value: patternCode, label: patternCode })), state.mappingDraft.patternCode, 'data-marker-plan-mapping-field')}
...
function renderFoldConfigEditor(foldConfig) {
    if (!foldConfig)
        return '';
...
function renderFoldConfigReadonly(foldConfig) {
    if (!foldConfig)
        return '';
    return `
    <section class="space-y-3 rounded-lg border bg-background p-3" data-testid="marker-plan-fold-config">
...
function renderLayoutTab(plan, context) {
...
function renderLayoutReadonlyTab(plan) {
    const hasFoldBed = buildMarkerSchemeFromPlan(plan).beds.some((bed) => isFoldBedMode(bed.bedMode));
    return `
...
function renderSchemeDemandMatrix(plan) {
    ...
    const getQty = (color, size) => scheme.demandRows
...
function renderMarkerMatrixReadonlyTable(plan, bed) {
    const sizeColumns = getMarkerMatrixSizeColumns(plan);
...
function renderSchemeBedsOverview(plan) {
    const scheme = buildMarkerSchemeFromPlan(plan);
...
function renderMarkerPlanSpreadingOrders(plan) {
...
function renderMarkerMatrixEditor(plan, bed) {
    const sizeColumns = getMarkerMatrixSizeColumns(plan);
    const sizeDemandMap = getMarkerMatrixSizeDemandMap(plan);
...
function renderSchemeBedEditor(plan, context) {
    void context;
    const scheme = buildMarkerSchemeFromPlan(plan);
...
function renderDetailPreviewAndFlow(plan, context) {
    return `
    ${renderDetailStepNav(plan)}
    ${renderPlanTopInfo(plan, context, { showActionRow: false })}
    ${renderCreateSourceSummary(plan, context, { title: '来源裁片单' })}
...
function renderDetailStepNav(plan) {
...
function renderListTabs(listTab, viewModel = getViewModel()) {
    return `
...
function getListFilterLabels() {
    const labels = [`视图：${getListTabMeta(state.listTab).label}`];
    if (state.filters.keyword)
        labels.push(`关键词：${state.filters.keyword}`);
    if (state.filters.contextNo)
        labels.push(`裁片单号：${state.filters.contextNo}`);
    if (state.filters.markerNo)
        labels.push(`方案编号：${state.filters.markerNo}`);
    if (state.filters.contextType !== 'ALL') {
        ...
        if (option)
            labels.push(`裁片单范围：${option.label}`);
    }
    if (state.filters.mode !== 'ALL')
        labels.push(`唛架模式：${markerPlanModeMeta[state.filters.mode].label}`);
    if (state.filters.status !== 'ALL')
        labels.push(`主状态：${markerPlanStatusMeta[state.filters.status].label}`);
    if (state.filters.ready === 'YES')
        labels.push('可交接铺布：是');
    if (state.filters.ready === 'NO')
        labels.push('可交接铺布：否');
    return labels;
}
function renderListStateBar() {
    ...
    <div data-testid="marker-plan-list-state-bar">
      ${renderWorkbenchStateBar({
        summary: '当前视图条件',
        chips: labels.map((label, index) => renderWorkbenchFilterChip(label, index === 0
            ? `data-marker-plan-action="switch-list-tab" data-list-tab="${state.listTab}"`
            : 'data-marker-plan-action="reset-filters"', index === 0 ? 'amber' : 'blue')),
        clearAttrs: 'data-marker-plan-action="reset-list-view"',
    })}
    </div>
  `;
}
function filterPlans(rows, tab) {
    ...
    return rows.filter((row) => {
        if (tab === 'WAITING_LAYOUT' && row.layoutStatus === 'done')
            return false;
        if (tab === 'DEMAND_DIFF' && (row.demandMatchSummary.status === '已匹配' || row.demandMatchSummary.status === '待编辑唛架'))
            return false;
        if (tab === 'WAITING_CONFIRM' && !(row.layoutStatus === 'done' && row.confirmationStatus !== '已确认'))
            return false;
        if (tab === 'READY_FOR_SPREADING' && !row.readyForSpreading)
            return false;
        if (tab === 'EXCEPTIONS' && !hasPlanExceptionIssue(row))
            return false;
        if (contextNo) {
            const contextKeywords = [row.contextNo, row.markerPlanNo, ...row.cutOrderNos]
                .filter(Boolean)
                .map((item) => item.toLowerCase());
            if (!contextKeywords.some((item) => item.includes(contextNo)))
                return false;
        }
        if (markerNo && !row.markerNo.toLowerCase().includes(markerNo))
            return false;
        if (state.filters.contextType !== 'ALL' && row.contextType !== state.filters.contextType)
            return false;
        if (state.filters.mode !== 'ALL' && row.markerMode !== state.filters.mode)
            return false;
        if (state.filters.status !== 'ALL' && row.status !== state.filters.status)
            return false;
        if (state.filters.ready === 'YES' && !row.readyForSpreading)
            return false;
        if (state.filters.ready === 'NO' && row.readyForSpreading)
            return false;
        if (!keyword)
            return true;
        const keywords = [
      ...
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-[repeat(4,minmax(0,1fr))_auto] xl:items-end">
        ...
        <label class="space-y-2">
          ...
          <select data-marker-plan-filter-field="status" class="h-10 w-full rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-blue-500">
            ...
            ${Object.keys(markerPlanStatusMeta)
...
function renderStats(viewModel = getViewModel()) {
...
function renderPlanRowsTable(rows, exceptionOnly = false) {
    ...
    <section class="rounded-lg border bg-card [&_td]:px-3 [&_td]:py-2 [&_th]:px-3 [&_th]:py-2" data-testid="${exceptionOnly ? 'marker-plan-exception-list' : 'marker-plan-list-table'}" data-marker-plan-main-card="true">
      ...
      ${renderStickyTableScroller(`
        ...
        ? rows
            ...
            return `
                      <tr class="border-t align-top ${exceptionOnly ? 'cursor-pointer hover:bg-muted/10' : ''}" ${exceptionOnly
...
function renderListPage(viewModel = getViewModel()) {
    clearMarkerPlanStickyOffset();
...
function renderEditorWarning(plan) {
    if (!plan)
        return '';
...
function renderEditBoundaryNotice(plan) {
    if (!plan)
        return '';
...
function renderCreateSourceSummary(plan, context, options = {}) {
...
function getSourceLockRows(plan) {
    return plan.sourceCutOrderLockRows || [];
}
function getSourceLockSummary(plan) {
...
function renderCreateCombinationRuleStep(plan) {
    const rows = getSourceLockRows(plan);
...
function renderMarkerPlanLockSummary(plan) {
...
function renderMarkerPlanImageSection(plan, readOnly = false) {
...
function renderCreateDemandStep(plan, context) {
    return `
...
function renderDemandMatchStep(plan, context, readOnly = false) {
...
function renderCreateEditorBody(plan, context) {
    if (!plan) {
        return `
...
function renderCreatePage(viewModel = getViewModel()) {
...
function renderEditPage(viewModel = getViewModel(), id = parseRoute().id) {
...
function renderDetailPage(viewModel = getViewModel(), id = parseRoute().id) {
...
export function renderCraftCuttingMarkerListPage() {
    const viewModel = getViewModel();
    syncStateFromRoute(viewModel);
    return renderListPage(viewModel);
}
export function renderCraftCuttingMarkerCreatePage() {
    const viewModel = getViewModel();
    syncStateFromRoute(viewModel);
    return renderCreatePage(viewModel);
}
export function renderCraftCuttingMarkerPlanEditPage(id) {
    const viewModel = getViewModel();
    syncStateFromRoute(viewModel);
    return renderEditPage(viewModel, id || parseRoute().id);
}
export function renderCraftCuttingMarkerPlanDetailPage(id) {
    const viewModel = getViewModel();
    syncStateFromRoute(viewModel);
    return renderDetailPage(viewModel, id || parseRoute().id);
}
function persistDraftPlan() {
...
function loadPlanAsDraftForAction(viewModel, planId) {
    if (!planId || state.draftPlan?.id === planId)
        return Boolean(state.draftPlan);
    ...
    state.draftPlan = context ? hydrateMarkerPlan(plan, context) : plan;
    return true;
}
function saveDraftPlan(stayOnPage = true, successMessage) {
...
function createFromSelectedContext(viewModel = getViewModel()) {
...
function handleAction(action, node) {
    ...
    if (action === 'go-edit') {
        ...
        if (!planId)
            return false;
        appStore.navigate(buildEditPathWithTab(planId, node.dataset.tabKey));
        return true;
    }
    if (action === 'go-detail') {
        const planId = node.dataset.planId || route.id;
        if (!planId)
            return false;
        appStore.navigate(buildDetailPathWithTab(planId, node.dataset.tabKey));
        return true;
    }
    ...
    if (action === 'go-cut-order-context') {
        const contextType = node.dataset.contextType;
    ...
    if (action === 'go-production-progress') {
        ...
        if (!plan)
            return false;
        appStore.navigate(buildGoProductionProgressPath({
    ...
    if (action === 'go-merge-context') {
        ...
        if (!contextType || !contextId)
            return false;
        const context = findMarkerPlanContextById(viewModel.contexts, contextType, contextId);
        if (!context || !context.markerPlanId)
            return false;
    ...
    if (action === 'switch-list-tab') {
        state.listTab = node.dataset.listTab || 'ALL';
        return true;
    }
    if (action === 'switch-create-step') {
        const createStep = node.dataset.createStep || 'source';
    ...
    if (action === 'switch-tab') {
        state.activeTab = node.dataset.tabKey || 'basic';
        if (state.draftPlan) {
            state.draftPlan = {
                ...state.draftPlan,
                lastVisitedTab: state.activeTab,
    ...
    if (action === 'complete-plan') {
        if (node.dataset.planId && !loadPlanAsDraftForAction(viewModel, node.dataset.planId))
            return true;
    ...
    if (action === 'confirm-plan') {
        ...
        if (handled)
            setFeedback('success', '已确认当前唛架方案，可交接铺布。');
        return handled;
    }
    if (action === 'mark-plan-adjust') {
        ...
        if (handled)
            setFeedback('warning', '已标记为需调整，请回到唛架编辑器修改。');
        return handled;
    }
    ...
    if (action === 'add-scheme-bed') {
        const mode = node.dataset.bedMode || 'normal';
    ...
    if (action === 'save-mapping') {
        ...
        return updateDraft((plan) => ({
            ...plan,
            pieceExplosionRows: plan.pieceExplosionRows.map((row) => row.id === targetRowId
                ? {
                    ...row,
...
export function handleCraftCuttingMarkerPlanEvent(target) {
    const route = parseRoute();
    if (route.kind === 'OTHER')
        return false;
    const viewModel = getViewModel();
    const filterFieldNode = target.closest('[data-marker-plan-filter-field]');
    if (filterFieldNode) {
        const field = filterFieldNode.dataset.markerPlanFilterField;
        const input = filterFieldNode;
        if (field === 'keyword')
            state.filters.keyword = input.value;
        if (field === 'contextNo')
            state.filters.contextNo = input.value;
        if (field === 'markerNo')
            state.filters.markerNo = input.value;
        if (field === 'contextType')
            state.filters.contextType = input.value;
        if (field === 'mode')
            state.filters.mode = input.value;
        if (field === 'status')
            state.filters.status = input.value;
        if (field === 'ready')
            state.filters.ready = input.value;
        return true;
    }
    const contextFieldNode = target.closest('[data-marker-plan-context-field]');
    if (contextFieldNode) {
        const field = contextFieldNode.dataset.markerPlanContextField;
        const input = contextFieldNode;
        if (field === 'contextKeyword') {
            state.contextKeyword = input.value;
            return true;
        }
    }
    const basicFieldNode = target.closest('[data-marker-plan-basic-field]');
    if (basicFieldNode) {
        const field = basicFieldNode.dataset.markerPlanBasicField;
        const input = basicFieldNode;
        if (!field)
            return false;
        if (field === 'markerMode') {
    ...
    const textareaFieldNode = target.closest('[data-marker-plan-textarea-field]');
    if (textareaFieldNode) {
        const field = textareaFieldNode.dataset.markerPlanTextareaField;
        const input = textareaFieldNode;
        if (!field)
            return false;
        if (field === 'remark')
            return updateDraft((plan) => ({ ...plan, remark: input.value }));
        if (field === 'confirmationRemark')
            return updateDraft((plan) => ({ ...plan, confirmationRemark: input.value }));
    }
    const matrixCellNode = target.closest('[data-marker-plan-matrix-cell]');
    if (matrixCellNode) {
        ...
        const input = matrixCellNode;
        if (!bedId || !rowId || !sizeName)
            return false;
        if (!confirmReferencedStructuralEdit(viewModel))
            return true;
        return updateDraft((plan) => {
            ...
            const beds = buildMarkerSchemeFromPlan(plan).beds.map((bed) => {
                ...
                const rows = createMarkerMatrixRows(plan, bed).map((row) => {
                    ...
                    if (bed.bedMode === 'normal' || bed.bedMode === 'fold_normal') {
                        sizeValues = Object.fromEntries(sizeColumns.map((size) => [size, nextValue]));
                    }
                    return {
                        ...row,
                        sizeValues,
    ...
    if (matrixRowLengthNode) {
        ...
        return updateDraft((plan) => {
            const beds = buildMarkerSchemeFromPlan(plan).beds.map((bed) => {
                if (bed.bedId !== bedId)
                    return bed;
                const rows = createMarkerMatrixRows(plan, bed).map((row) => row.rowId === rowId
                    ? {
                        ...row,
    ...
    const bedFieldNode = target.closest('[data-marker-plan-bed-field]');
    if (bedFieldNode) {
        const field = bedFieldNode.dataset.markerPlanBedField;
        const bedId = bedFieldNode.dataset.bedId || '';
        const input = bedFieldNode;
        if (!field || !bedId)
            return false;
        if (!confirmReferencedStructuralEdit(viewModel))
            return true;
        return updateDraft((plan) => {
            const beds = buildMarkerSchemeFromPlan(plan).beds.map((bed) => {
                ...
                if (field === 'bedMode') {
                    const nextMode = input.value;
                    const nextBedNo = bed.bedNo === '' || /^[AB]-\d+$/.test(bed.bedNo)
                ...
                if (field === 'plannedLayerCount' || field === 'markerPieceQtyPerLayer') {
                    return { ...bed, [field]: Math.max(Math.round(safeNumber(input.value)), 0) };
                }
                if (field === 'markerLength') {
                    return { ...bed, markerLength: Math.max(safeNumber(input.value), 0) };
                }
                if (field === 'bedNo')
                    return { ...bed, bedNo: input.value, bedName: input.value };
                return { ...bed, remark: input.value };
            });
            return applySchemeBedsToPlan(plan, beds);
        });
    }
    const foldFieldNode = target.closest('[data-marker-plan-fold-field]');
    if (foldFieldNode) {
        const field = foldFieldNode.dataset.markerPlanFoldField;
        const input = foldFieldNode;
        if (!field)
            return false;
        if (!confirmReferencedStructuralEdit(viewModel))
            return true;
        return updateDraft((plan) => ({
            ...
            foldConfig: {
                ...
                [field]: field === 'foldDirection'
                    ? input.value
                    : Math.max(safeNumber(input.value), 0),
            },
        }));
    }
    const mappingFieldNode = target.closest('[data-marker-plan-mapping-field]');
    if (mappingFieldNode) {
        const field = mappingFieldNode.dataset.markerPlanMappingField || '';
        const input = mappingFieldNode;
        if (field === 'targetSku')
            state.mappingDraft.targetSku = input.value;
        if (field === 'colorMode')
            state.mappingDraft.colorMode = input.value;
        if (field === 'specifiedColor')
            state.mappingDraft.specifiedColor = input.value;
        if (field === 'partCode')
            state.mappingDraft.partCode = input.value;
        if (field === 'patternCode')
            state.mappingDraft.patternCode = input.value;
        if (field === 'piecePerGarment')
            state.mappingDraft.piecePerGarment = Math.max(Math.round(safeNumber(input.value)), 0);
        if (field === 'note')
            state.mappingDraft.note = input.value;
        return true;
    }
    const actionNode = target.closest('[data-marker-plan-action]');
    const action = actionNode?.dataset.markerPlanAction;
    if (!action || !actionNode)
        return false;
    return handleAction(action, actionNode);
}
export function isCraftCuttingMarkerPlanDialogOpen() {
