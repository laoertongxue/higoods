...
function repackContent(model: WaitHandoverActionDialogModel): string {
  ...
  return `<div class="min-w-0 space-y-4" data-wait-handover-repack-workspace>
    ...
    <section class="hidden rounded-lg border p-4" data-wait-handover-repack-step="confirm"><div class="flex items-start gap-3"><span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-700 text-sm font-semibold text-white">5</span><div><h3 class="font-semibold">5 汇总并确认</h3><p class="mt-1 text-xs text-muted-foreground">核对结果袋、重新入仓来源袋、空闲袋以及菲票和裁片数量；确认后结果袋直接交出。</p></div></div><div class="mt-4" data-wait-handover-repack-total-preview></div><div class="mt-3 rounded-md bg-blue-50 p-3 text-xs text-blue-800">系统先完成重装，再逐只交出结果袋；剩余来源袋完成入仓；清空来源袋变为空闲。</div><div class="mt-3">${field('交出操作人', 'operatorName', '', '请输入实际交出人姓名')}</div><input type="hidden" data-wait-handover-field="repackBatchId" value="${escapeHtml(model.repackBatchId)}" /><div class="mt-4"><button type="button" class="rounded-md border px-4 py-2 text-sm" data-skip-page-rerender="true" data-wait-handover-action="repack-back">上一步</button></div></section>
  </div>`
}
