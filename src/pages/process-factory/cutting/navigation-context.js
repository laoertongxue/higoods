...
const warehouseStatusLabelMap = {
    PENDING_INBOUND: '待入仓',
    INBOUNDED: '已入仓',
    WAITING_HANDOVER: '待交接',
    HANDED_OVER: '已交接',
};
const focusTabLabelMap = {
    printed: '已打印菲票',
    records: '打印记录',
    split: '拆分明细',
    spreadings: '铺布记录',
...
export function serializeCuttingDrillContext(context) {
    if (!context)
        return {};
    return {
...
export function readCuttingDrillContextFromLocation(input) {
    const params = typeof input === 'string' ? new URLSearchParams(input) : input;
    if (!params)
        return null;
    const context = {
        sourcePageKey: pickString(params, 'sourcePageKey'),
...
export function buildCuttingDrillContext(payload, sourcePageKey, extra) {
...
function getTargetPath(target, context) {
    if (target === 'summary')
        return getCanonicalCuttingPath('summary');
    if (target === 'replenishment')
        return getCanonicalCuttingPath('replenishment');
    if (target === 'specialProcesses')
        return getCanonicalCuttingPath('special-processes');
    if (target === 'materialPrep')
        return getCanonicalCuttingPath('warehouse-management-wait-process');
    ...
    if (target === 'cutOrders')
        return getCanonicalCuttingPath('cut-orders');
    if (target === 'productionProgress')
        return getCanonicalCuttingPath('production-progress');
    ...
    if (target === 'cutPieceWarehouse')
        return getCanonicalCuttingPath('cut-piece-warehouse');
    if (target === 'fabricWarehouse')
        return getCanonicalCuttingPath('fabric-warehouse');
    if (target === 'markerPlanRefs')
        return getCanonicalCuttingPath('marker-list');
    if (target === 'cuttablePool')
        return getCanonicalCuttingPath('cuttable-pool');
    if (target === 'markerSpreading') {
        return getCanonicalCuttingPath('spreading-list');
    }
    if (target === 'feiTickets') {
        if (context.printableUnitId || context.printableUnitNo) {
            if (context.focusTab === 'records')
                return getCanonicalCuttingPath('fei-ticket-records');
            if (context.focusTab === 'printed' || context.ticketId || context.ticketNo)
                return getCanonicalCuttingPath('fei-ticket-printed');
            if (context.autoOpenDetail)
                return getCanonicalCuttingPath('fei-ticket-detail');
        }
        return getCanonicalCuttingPath('fei-tickets');
    }
    return getCanonicalCuttingPath('summary');
}
export function buildCuttingRouteWithContext(target, context) {
    return buildRouteWithQuery(getTargetPath(target, context), serializeCuttingDrillContext(context));
}
export function getCuttingNavigationActionLabel(target) {
    return actionLabelMap[target];
}
export function getCuttingSourcePageLabel(sourcePageKey) {
    if (!sourcePageKey)
        return '外部页面';
    return sourcePageLabelMap[sourcePageKey];
}
export function buildCuttingDrillChipLabels(context) {
    if (!context)
        return [];
    const labels = [
...
export function buildCuttingDrillSummary(context) {
    if (!context)
        return '';
    if (context.sourcePageKey === 'cutting-summary')
        return '当前已恢复裁剪结果核查定位';
    return `来源：${getCuttingSourcePageLabel(context.sourcePageKey)}`;
}
export function hasSummaryReturnContext(context) {
    return context?.sourcePageKey === 'cutting-summary';
}
export function buildReturnToSummaryContext(context) {
    if (!hasSummaryReturnContext(context))
        return null;
    return {
        sourcePageKey: context?.sourcePageKey,
        sourceSection: context?.sourceSection,
