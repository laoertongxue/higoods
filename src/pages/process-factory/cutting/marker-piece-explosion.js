...
function buildAllocationSummaryText(lines) {
    if (!lines.length)
        return '待补分配';
    return lines
...
function getMaterialIdentityFromSourceRow(row, materialSku) {
...
export function resolveMarkerTechPackLink(options) {
    ...
    for (const candidate of candidates) {
        if (!candidate.productionOrderId)
            continue;
...
export function buildMarkerAllocationSourceRows(marker, rowsById) {
    return marker.cutOrderIds
        .map((id) => rowsById[id])
        .filter((row) => Boolean(row));
}
function resolveSkuCode(techPack, color, sizeLabel) {
...
function resolveMappingLinesForSku(techPack, color, skuCode) {
    const colorMapping = resolveColorMapping(techPack, color);
    if (!colorMapping)
        return { colorMapping: null, mappingLines: [] };
    const mappingLines = colorMapping.lines.filter((line) => !(line.applicableSkuCodes || []).length || line.applicableSkuCodes?.includes(skuCode));
    return { colorMapping, mappingLines };
}
function lineMatchesMaterial(line, materialSku) {
    if (!normalizeText(materialSku))
        return false;
    return equalsLoose(line.materialCode, materialSku) || equalsLoose(line.materialName, materialSku);
}
function buildPieceRowsFromPatternFallback(techPack, line, skuCode) {
    if (!line.patternId)
        return [];
    const pattern = (techPack.patternFiles || []).find((item) => item.id === line.patternId) || null;
    if (!pattern?.pieceRows?.length)
        return [];
...
export function buildMarkerPieceExplosionViewModel(options) {
    ...
    const allocationSizeMap = new Map();
    ...
    const skuSummaryRows = [];
    const pieceDetailRows = [];
    ...
    allocationLines.forEach((allocationLine) => {
        ...
        allocationSizeMap.set(allocationLine.sizeLabel, (allocationSizeMap.get(allocationLine.sizeLabel) || 0) + plannedGarmentQty);
        const techPackLink = resolveMarkerTechPackLink({
            marker: options.marker,
            sourceRow,
        });
        const baseAllocationRow = {
        ...
        const mappingStatus = materialMatched ? 'MATCHED' : 'MATERIAL_PENDING_CONFIRM';
        ...
        const explodedPieceTotal = pieceRows.reduce((sum, pieceRow) => sum + plannedGarmentQty * Math.max(pieceRow.pieceCountPerUnit || 0, 0), 0);
        allocationRows.push({
            ...baseAllocationRow,
            skuCode,
            mappingStatus,
    ...
    const allocationSizeSummary = sizeKeys.map((sizeLabel) => {
        const requiredQty = options.marker.sizeDistribution.find((item) => item.sizeLabel === sizeLabel)?.quantity || 0;
