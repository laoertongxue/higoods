...
export function validateMarkerSchemeSourceCandidates(candidates) {
    const messages = [];
    ...
    if (!candidates.length)
        messages.push('请选择来源单据');
    if (spuCodes.length > 1)
        messages.push('已选单据不属于同一 SPU');
    if (markerPlanGroupKeys.length > 1)
        messages.push('已选单据不满足同 SPU、同纸样文件、同有效幅宽、同历史组合组');
...
export function buildMarkerSchemeBedsFromPlan(plan) {
    return plan.beds?.length ? plan.beds : [];
}
export function buildMarkerSchemeFromPlan(plan) {
