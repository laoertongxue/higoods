import { buildProductionPieceTruthCompletion, productionPieceTruthCompletionMetaMap, } from '../../../domain/fcs-cutting-piece-truth/index.ts';
import { getMarkerPlanRefStatusMeta, } from './marker-plan-ref-model.ts';
import { getFeiTicketStatusMeta } from './fei-tickets-model.ts';
import { buildCuttingCheckResult, cuttingCheckSectionLabelMap, } from './cutting-summary-checks.ts';
const numberFormatter = new Intl.NumberFormat('zh-CN');
export const cuttingSummaryRiskMetaMap = {
    HIGH: {
        key: 'HIGH',
        label: '高风险',
...
export const cuttingSummaryIssueMetaMap = {
    MATERIAL_PREP: {
        key: 'MATERIAL_PREP',
        label: '领料问题',
        className: 'bg-amber-100 text-amber-700 border border-amber-200',
...
function summarizeTicketStatus(owners, records) {
    if (!owners.length)
        return '待建立票据主体';
...
function summarizeBagUsageStatus(usages, returnUsages) {
    if (!usages.length && !returnUsages.length)
        return '未进入中转袋流转';
...
function summarizeMaterialPrep(rows) {
    if (!rows.length)
        return '未进入配料/领料';
...
function summarizeSpreading(sessions, replenishments) {
    if (!sessions.length) {
        const pending = replenishments.filter((item) => ['PENDING_REVIEW', 'PENDING_SUPPLEMENT', 'APPROVED_PENDING_ACTION', 'IN_ACTION'].includes(item.statusMeta.key)).length;
...
function summarizeReplenishment(items) {
    if (!items.length)
        return '暂无补料建议';
...
function isReservedSpecialProcess(item) {
    return !item.typeExecutionMeta.enabledForExecution;
}
function isOpenSpecialProcess(item) {
    if (isReservedSpecialProcess(item))
        return true;
    return !['DONE', 'CANCELLED'].includes(item.status);
}
function summarizeSpecialProcess(items) {
    if (!items.length)
        return '未创建';
...
function mapSectionKeyToIssueType(sectionKey) {
    if (sectionKey === 'MATERIAL_PREP')
        return 'MATERIAL_PREP';
    if (sectionKey === 'SPREADING' || sectionKey === 'REPLENISHMENT')
        return 'SPREADING_REPLENISH';
    if (sectionKey === 'FEI_TICKETS')
        return 'TICKET_QR';
    if (sectionKey === 'WAREHOUSE_HANDOFF')
        return 'WAREHOUSE_HANDOFF';
    return 'SPECIAL_PROCESS';
}
function summarizeCheckSections(sections) {
...
const feiPrintJobStatusLabelMap = {
    PRINTED: '已打印',
    REPRINTED: '已补打',
    CANCELLED: '已取消',
};
function deriveOverallRiskLevel(options) {
    if (options.completionState === 'HAS_EXCEPTION' || options.blockerItems.some((item) => item.severity === 'HIGH'))
        return 'HIGH';
    if (options.blockerItems.length || options.completionState === 'DATA_PENDING')
        return 'MEDIUM';
    if (options.productionRiskTags.length)
        return 'MEDIUM';
    return 'LOW';
}
function buildSummarySourceObjects(options) {
    const blockerCountBySourceNo = options.row.blockerItems.reduce((result, item) => {
        result[item.sourceNo] = (result[item.sourceNo] || 0) + 1;
        return result;
    }, {});
    const resolveMaterialIdentity = (materialSku, cutOrderNo) => {
        const matched = options.cutOrderRows.find((row) => row.cutOrderNo === cutOrderNo && row.materialSku === materialSku)
    ...
    const cutOrderObjects = options.cutOrderRows.map((item) => ({
    ...
    const markerPlanRefObjects = options.markerPlanRefs.map((item) => ({
    ...
    const ticketOwners = options.ticketOwners.map((item) => ({
    ...
    const printJobs = options.printJobs.map((item) => ({
    ...
    const bagUsages = options.bagUsages.map((item) => ({
    ...
    const replenishments = options.replenishments.map((item) => ({
    ...
    const specialProcesses = options.specialProcesses.map((item) => ({
...
export function buildCuttingSummaryRows(options) {
    const cutOrderRowsByProduction = options.productionRows.reduce((result, productionRow) => {
    ...
    return options.productionRows.map((productionRow) => {
        ...
        const markerPlanRefs = options.markerPlanRefs.filter((batch) => batch.items.some((item) => cutOrderIdSet.has(item.cutOrderId) || item.productionOrderNo === productionRow.productionOrderNo));
        ...
        const spreadingSessions = options.markerStore.sessions.filter((session) => (session.cutOrderIds || []).some((cutOrderId) => cutOrderIdSet.has(cutOrderId)));
        ...
        const sampleItems = options.sampleWarehouseView.items.filter((item) => item.relatedProductionOrderNo === productionRow.productionOrderNo || item.styleCode === productionRow.styleCode);
        ...
        const completionMeta = buildProductionPieceTruthCompletion(pieceTruth, {
            ...
            hasObjectPending: checkResult.blockerItems.length > 0 ||
        ...
        return {
            ...
            warehouseIssueCount: fabricStocks.filter((item) => item.riskTags.length > 0).length +
...
function summarizeIssueSeverity(rows) {
    if (rows.some((row) => row.blockerItems.some((item) => item.severity === 'HIGH')))
        return 'HIGH';
    if (rows.some((row) => row.blockerItems.some((item) => item.severity === 'MEDIUM')))
        return 'MEDIUM';
    if (rows.some((row) => row.overallRiskLevel === 'HIGH'))
        return 'HIGH';
    if (rows.some((row) => row.overallRiskLevel === 'MEDIUM'))
        return 'MEDIUM';
    return 'LOW';
}
export function buildCuttingSummaryIssues(rows) {
    return Object.values(cuttingSummaryIssueMetaMap)
        .map((meta) => {
        const relatedRows = filterSummaryByIssueType(rows, meta.key).filter((row) => row.blockerItems.some((item) => mapSectionKeyToIssueType(item.sectionKey) === meta.key) ||
            row.checkSections.some((section) => mapSectionKeyToIssueType(section.sectionKey) === meta.key && section.stateKey === 'DATA_PENDING'));
        if (!relatedRows.length)
            return null;
        const relatedBlockers = relatedRows.flatMap((row) => row.blockerItems.filter((item) => mapSectionKeyToIssueType(item.sectionKey) === meta.key));
        const primaryActionLabel = relatedRows.flatMap((row) => row.nextActions.filter((action) => mapSectionKeyToIssueType(action.sectionKey) === meta.key))[0]
...
export function filterSummaryByIssueType(rows, issueType) {
    if (issueType === 'ALL')
        return rows;
    return rows.filter((row) => row.issueTypes.includes(issueType));
}
export function buildCuttingTraceTree(detail) {
    ...
    const replenishmentNodes = detail.replenishments.map((item) => ({
    ...
    const specialProcessNodes = detail.specialProcesses.map((item) => ({
...
export function buildSummaryDetailPanelData(rowId, options) {
    ...
    const markerPlanRefs = options.markerPlanRefs.filter((batch) => batch.items.some((item) => cutOrderIdSet.has(item.cutOrderId) || item.productionOrderNo === row.productionOrderNo));
    ...
    const sampleItems = options.sampleWarehouseView.items.filter((item) => item.relatedProductionOrderNo === row.productionOrderNo || item.styleCode === row.styleCode);
...
export function buildSummaryDashboardCards(dashboard) {
    return [
        {
...
export function buildSummarySearchIndex(rows) {
    return rows.map((row) => ({
        rowId: row.rowId,
        tokens: row.keywordIndex,
    }));
}
export function buildCuttingSummaryViewModel(options) {
    ...
    const dashboard = {
        ...
        warehouseIssueCount: options.fabricWarehouseView.summary.abnormalItemCount +
