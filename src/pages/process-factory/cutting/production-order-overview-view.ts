...
function statusClass(status: string): string {
  if (['已完成', '印花完成', '染色完成', '配料完成', '已经拆解', '唛架完成', '铺布完成', '裁剪完成', '已入仓', '发货完成', '领取完成', '已经接单'].includes(status)) {
    return 'bg-emerald-50 text-emerald-700'
  }
  if (['进行中', '印花中', '染色中', '部分配料', '部分领取', '铺布中'].includes(status) || status.includes('未完成')) {
        ...
        <table class="min-w-[2280px] w-full table-fixed text-sm" data-testid="cutting-production-progress-main-table">
          ...
          <thead class="sticky top-0 z-20 border-b bg-muted/95 text-muted-foreground backdrop-blur">
            <tr class="border-b">
              ...
              <th colspan="2" class="border-r-2 border-slate-300 bg-rose-50 px-3 py-2 text-center font-semibold text-rose-700">印染</th>
              <th colspan="3" class="border-r-2 border-slate-300 bg-amber-50 px-3 py-2 text-center font-semibold text-amber-700">中转仓</th>
              <th colspan="5" class="bg-blue-50 px-3 py-2 text-center font-semibold text-blue-700">裁床厂</th>
            </tr>
            <tr>
              ${PRODUCTION_ORDER_OVERVIEW_HEADERS
                .map((label, index) => `<th class="${index === 0 ? 'sticky left-0 z-30 bg-muted/95' : index === 1 ? 'sticky left-[240px] z-30 bg-muted/95' : ''} ${[2,4,7].includes(index) ? 'border-l-2 border-slate-300' : ''} px-3 py-2 text-left font-medium">${label}</th>`)
                .join('')}
            </tr>
          </thead>
          <tbody>
            ${pagination.items.length ? pagination.items.map((row) => `
              <tr class="border-b align-top last:border-b-0 hover:bg-muted/20" data-production-order-id="${escapeHtml(row.productionOrderId)}">
                ...
                <td class="border-l-2 border-slate-200 px-3 py-3">${renderStatus(row, row.printingStatus, 'printing')}</td>
                <td class="px-3 py-3">${renderStatus(row, row.dyeingStatus, 'dyeing')}</td>
                <td class="border-l-2 border-slate-200 px-3 py-3">${renderStatus(row, row.breakdownStatus, 'breakdown')}</td>
                <td class="px-3 py-3">${renderStatus(row, row.materialPrepStatus, 'materialPrep')}</td>
                <td class="px-3 py-3">${renderFactoryLines(row)}</td>
                <td class="border-l-2 border-slate-200 px-3 py-3">${renderStatus(row, row.markerStatus, 'marker')}</td>
