...
import { CUTTING_QR_VERSION, deserializeCuttingQrPayload, serializeCuttingQrPayload, } from '../../../data/fcs/cutting/qr-payload.ts';
...
const GENERATED_QR_META_FIELD = ['qr', 'Payload'].join('');
function readGeneratedQrVersion(record) {
    if (!record)
        return undefined;
    const qrMeta = record[GENERATED_QR_META_FIELD];
...
function createReservedProcessSlot(processKey) {
    return {
        processKey,
        enabled: false,
        payloadVersion: null,
...
export function getDefaultReservedProcessPayload(existing) {
    const keys = ['embroidery', 'template', 'strip', 'dyeMark'];
...
export function getDefaultReservedTracePayload(existing) {
    return {
...
function inferReservedProcess(payload) {
...
function normalizeBasePayload(input) {
    ...
    return {
        ...
        pieceSetNoRange: generated?.pieceSetNoRange
...
export function buildFeiQrPayload(options) {
    ...
    return {
        ...
        reservedProcess: getDefaultReservedProcessPayload(options.ticketRecord.reservedProcess
            || inferReservedProcess(payload)),
        reservedTrace: getDefaultReservedTracePayload(options.ticketRecord.reservedTrace || null),
    };
}
export function buildFeiQrPayloadSummary(payload) {
    return {
...
export function validateFeiQrPayload(payload) {
    const warnings = [];
    if (!payload.cutOrderId || !payload.cutOrderNo)
        warnings.push('当前菲票缺少裁片单主码引用。');
    if (!payload.productionOrderNo)
        warnings.push('当前菲票缺少生产单号。');
    if (!payload.materialSku)
        warnings.push('当前菲票缺少面料 SKU。');
    if (!payload.fabricRollNo)
        warnings.push('当前菲票缺少面料卷号。');
    if (!payload.fabricColor)
        warnings.push('当前菲票缺少布料颜色。');
    if (!payload.skuSize)
        warnings.push('当前菲票缺少尺码。');
    if (!payload.partName)
        warnings.push('当前菲票缺少裁片部位。');
    if (!payload.feiTicketNo)
        warnings.push('当前菲票缺少菲票号。');
    const craftValidation = payload.secondaryCrafts.length
        ...
        : null;
    if (craftValidation && !craftValidation.allowed)
        warnings.push(craftValidation.reason);
    return {
...
export function serializeFeiQrPayload(payload) {
    return serializeCuttingQrPayload(toCanonicalPayload(payload));
}
export function deserializeFeiQrPayload(value) {
    const payload = deserializeCuttingQrPayload(value);
    if (!payload || payload.codeType !== 'FEI_TICKET')
        return null;
    return {
        ...payload,
        schemaName: FEI_QR_SCHEMA_NAME,
...
export function buildReservedProcessBadges(payload) {
    return Object.keys(payload.reservedProcess).map((key) => {
...
export function buildFeiQrSchemaMeta(record) {
...
export function buildTransferBagReservedBridge(payload) {
    return {
...
export function buildQrNavigationPayload(payload) {
    return {
        cutOrderId: payload.cutOrderId,
        cutOrderNo: payload.cutOrderNo,
