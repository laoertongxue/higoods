...
export function addMarkerSizeRow(draft) {
    draft.sizeDistribution = [...draft.sizeDistribution, { sizeLabel: '', quantity: 0 }];
}
export function removeMarkerSizeRow(draft, index) {
    draft.sizeDistribution = draft.sizeDistribution.filter((_, itemIndex) => itemIndex !== index);
}
export function addMarkerAllocationLine(draft, sourceRows, createMarkerAllocationLine) {
...
export function removeMarkerAllocationLine(draft, index) {
    draft.allocationLines = (draft.allocationLines || []).filter((_, itemIndex) => itemIndex !== index);
}
export function addMarkerLineItem(draft, createMarkerLineItem) {
    draft.lineItems = [...(draft.lineItems || []), createMarkerLineItem(draft.lineItems?.length || 0)];
}
export function removeMarkerLineItem(draft, index) {
    draft.lineItems = (draft.lineItems || []).filter((_, itemIndex) => itemIndex !== index);
}
export function addHighLowCuttingRow(draft, createHighLowCuttingRow) {
...
export function removeHighLowCuttingRow(draft, index) {
    draft.highLowCuttingRows = (draft.highLowCuttingRows || []).filter((_, itemIndex) => itemIndex !== index);
}
export function addHighLowPatternKey(draft) {
...
export function removeHighLowPatternKey(draft, index) {
...
export function addHighLowPatternRow(draft, createHighLowPatternRow) {
...
export function removeHighLowPatternRow(draft, index) {
    draft.highLowPatternRows = (draft.highLowPatternRows || []).filter((_, itemIndex) => itemIndex !== index);
}
export function addSpreadingRoll(draft, createRollDraft) {
    draft.rolls = [...draft.rolls, createRollDraft(draft)];
}
export function removeSpreadingRoll(draft, index) {
...
export function addSpreadingOperator(draft, createOperatorDraft) {
    draft.operators = [...draft.operators, createOperatorDraft(draft)];
}
export function addSpreadingOperatorForRoll(draft, rollRecordId, createOperatorDraftForRoll) {
    draft.operators = [...draft.operators, createOperatorDraftForRoll(draft, rollRecordId)];
}
export function removeSpreadingOperator(draft, index) {
    draft.operators = draft.operators
        .filter((_, itemIndex) => itemIndex !== index)
        .map((operator, itemIndex) => ({ ...operator, sortOrder: itemIndex + 1 }));
}
export function handleMarkerSpreadingDraftAction(context) {
