...
export const summaryStatusMeta = {
...
export const updatedSourceMeta = {
    PLATFORM: '平台侧',
    PCS: '工艺工厂运营系统',
    FACTORY_APP: '工厂端',
};
export const issueSourceMeta = {
...
export function filterCuttingSummaryRecords(records, filters) {
    ...
    return records.filter((record) => {
        ...
        const matchesSource = filters.issueSource === 'ALL' ||
            record.issues.some((issue) => {
                if (filters.issueSource === 'PREP')
                    return issue.sourcePage === 'MATERIAL_PREP';
                if (filters.issueSource === 'EXECUTION')
                    return issue.sourcePage === 'CUT_PIECE_ORDER';
                if (filters.issueSource === 'REPLENISHMENT')
                    return issue.sourcePage === 'REPLENISHMENT';
                if (filters.issueSource === 'WAREHOUSE')
...
export function buildSummaryOverview(records) {
    return {
...
export function buildPriorityRecords(records) {
    const statusWeight = {
...
export function buildEmptyStateText(hasFilters, mode) {
    if (mode === 'priority')
        return '当前没有需要优先处理的重点问题生产单。';
    return hasFilters ? '未找到符合筛选条件的裁剪结果核查记录。' : '当前没有可展示的裁剪结果核查记录。';
}
export function hasSummaryFilters(filters) {
...
export function buildMaterialReceiveText(record) {
