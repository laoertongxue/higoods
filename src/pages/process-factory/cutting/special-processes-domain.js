...
export function getSpecialProcessOutputLabels(processType) {
...
export function deriveSpecialProcessTypeExecutionMeta(processType) {
    if (processType === 'BINDING_STRIP') {
        return {
...
export function hydrateScopeLineFromSource(scopeLine, sourceOptions) {
    const matched = sourceOptions.find((item) => item.sourceCutOrderId === scopeLine.sourceCutOrderId) ||
...
function hasExecutionContent(logs) {
...
export function validateSpecialProcessExecutionTransition(options) {
    ...
    if (!options.scopeLines.length)
        return { ok: false, message: '请先补齐作用范围。' };
    if (!options.payload)
        return { ok: false, message: '请先补齐工艺参数。' };
    if (options.nextStatus === 'DRAFT')
    ...
    if (options.nextStatus === 'DONE') {
        if (!hasExecutionContent(options.executionLogs))
            return { ok: false, message: '请先补录执行记录。' };
...
export function normalizeFollowupStatus(value) {
    if (value === 'DONE' || value === 'SKIPPED' || value === 'PENDING')
        return value;
    return 'PENDING';
}
