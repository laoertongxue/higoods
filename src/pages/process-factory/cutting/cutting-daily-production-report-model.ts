...
function buildFulfillmentRows(sources: CuttingSummaryBuildOptions, query: CuttingDailyProductionReportQuery): CuttingDailyDetailRow[] {
  const fulfillmentRows = filterRowsByScope(
    ...
    quantityText: `${formatQty(row.orderQty, '件')} / 可放行 ${formatQty(row.pieceCompletionSummary.key === 'COMPLETED' ? row.orderQty : Math.max(row.orderQty - row.pieceGapQty, 0), '件')}`,
  ...
  return filterRowsByScope(
    ...
    return sourceRows.map((line) => {
      ...
      return buildDetailRow({
        ...
        quantityText: `${formatQty(line.configuredQty, 'yard', 2)} / 已领 ${formatQty(line.claimedQty, 'yard', 2)}`,
        occurredAt: row.latestClaimRecordAt || row.printedAt,
...
function buildTicketRows(sources: CuttingSummaryBuildOptions, query: CuttingDailyProductionReportQuery): CuttingDailyDetailRow[] {
  ...
  const bagRows = sources.transferBagView.usages.map((usage) => buildDetailRow({
    ...
    quantityText: `${formatQty(usage.packedTicketCount, '张')} / ${formatQty(usage.summary.quantityTotal, '片')}`,
...
function buildWarehouseRows(sources: CuttingSummaryBuildOptions, query: CuttingDailyProductionReportQuery): CuttingDailyDetailRow[] {
  const cutPieceRows = sources.cutPieceWarehouseView.items.map((item) => buildDetailRow({
    id: `warehouse-piece:${item.warehouseItemId}`,
    tab: 'warehouse',
    ...
    occurredAt: item.inWarehouseAt || item.updatedAt || '',
  ...
  const handoverRows = listHandoverRecords().map((record) => {
    ...
    return buildDetailRow({
      ...
      quantityText: formatQty(sum(record.currentHandedOverSummary.map((item) => Number(item.qty || 0))), '片'),
...
function buildReportFromSources(
  ...
  const metricGroups: CuttingDailyMetricGroup[] = [
    ...
    },
    {
      ...
      metrics: [
        ...
        metric({ key: 'materials.pickup', label: '今日中转仓接收数量', value: materialPickupRows.length, unit: '条记录', tab: 'materials', recordCount: materialPickupRows.length, helperText: '按现有 claimRecords 和 PDA pickup 事件投影统计。' }),
        metric({ key: 'materials.processingIssue', label: '今日加工接收数量', value: summarizeUnits(materialProcessingRows, '卷'), unit: '卷', tab: 'materials', recordCount: materialProcessingRows.length, helperText: '待加工仓加工接收按可回溯记录展示。' }),
    ...
    },
    {
      ...
      metrics: [
        ...
        metric({ key: 'warehouse.handoverPieces', label: '今日交出裁片数量', value: sum(handoverRecords.map((record) => sum(record.currentHandedOverSummary.map((item) => Number(item.qty || 0))))), unit: '片', tab: 'warehouse', recordCount: warehouseHandoverRows.length, helperText: '交出单与交出记录分开统计，此处为交出记录数量。' }),
  ...
  const coverageRows = [
    ...
    coverage({ moduleKey: 'materials', moduleName: '配料与接收', recordCount: materialRows.length, latestRecordedAt: latestText(materialRows.map((row) => row.occurredAt)) }),
    ...
    coverage({ moduleKey: 'wait-process', moduleName: '待加工仓', recordCount: 0, status: 'UNAVAILABLE', reason: '当前记录不足，暂不能可靠回溯加工接收与回收入仓。' }),
