...
import { renderCompactKpiCard, renderStickyFilterShell, renderStickyTableScroller, renderWorkbenchFilterChip, renderWorkbenchSecondaryPanel, renderWorkbenchStateBar, } from './layout.helpers.ts';
import { getCanonicalCuttingMeta, getCanonicalCuttingPath, renderCuttingPageHeader } from './meta.ts';
import { buildCuttingSummaryIssues, cuttingSummaryIssueMetaMap, cuttingSummaryRiskMetaMap, filterSummaryByIssueType, } from './summary-model.ts';
import { getWarehouseSearchParams } from './warehouse-shared.ts';
import { buildCuttingDrillChipLabels, buildCuttingDrillSummary, buildCuttingRouteWithContext, getCuttingNavigationActionLabel, buildCuttingDrillContext, readCuttingDrillContextFromLocation, } from './navigation-context.ts';
import { cuttingCheckSectionLabelMap, } from './cutting-summary-checks.ts';
import { buildFcsCuttingSummaryDetailProjection, buildFcsCuttingSummaryProjection, } from './runtime-projections.ts';
import { renderMaterialIdentityBlock } from './material-identity.ts';
import { getCuttingProgressSnapshots, getCuttingSpecialCraftReturnStatusByProductionOrders, } from '../../../data/fcs/progress-statistics-linkage.ts';
import { getCuttingSewingDispatchProgressByProductionOrder as getRuntimeSewingDispatchProgressByProductionOrder, listAvailableCutPieceInventoryForSewingDispatch, listCuttingSewingDispatchBatches, listCuttingSewingDispatchOrders, listCuttingSewingTransferBags, } from '../../../data/fcs/cutting/sewing-dispatch.ts';
...
const completionSortWeight = {
    HAS_EXCEPTION: 0,
    DATA_PENDING: 1,
    IN_PROGRESS: 2,
    COMPLETED: 3,
};
const blockerLevelMetaMap = {
    HIGH: {
        label: '高风险',
...
const sourceObjectTypeLabelMap = {
    CUT_ORDER: '裁片单',
    MARKER_PLAN: '唛架方案',
    REPLENISHMENT: '补料建议',
    FEI_OWNER: '打票主体',
...
function getSessionActualMaterialMeter(session) {
...
function getRowSpreadingSessions(row, sources) {
    const cutOrderIds = new Set(row.relatedCutOrderIds);
    const markerPlanNos = new Set(row.relatedMarkerPlanNos);
    return sources.markerStore.sessions.filter((session) => (session.cutOrderIds || []).some((cutOrderId) => cutOrderIds.has(cutOrderId)) ||
...
function makePartResultKey(input) {
    return [input.color || '', input.size || input.sizeCode || '', input.partName || '', input.materialSku || '']
        .map((value) => String(value || '').trim().toLowerCase())
        .join('::');
}
function buildSummaryPartResultLines(row, transferBags) {
    ...
    const upsert = (input) => {
        ...
        if (!current.cutOrderNo && input.cutOrderNo)
            current.cutOrderNo = input.cutOrderNo;
        if (!current.materialSku && input.materialSku)
            current.materialSku = input.materialSku;
    ...
    return Array.from(grouped.values())
        .map((line) => ({
        ...line,
        remainingGapQty: Math.max(line.requiredPieceQty - Math.max(line.actualCutQty, line.waitHandoverStockQty + line.handedOverPieceQty), 0),
    }))
        .sort((left, right) => `${left.materialSku}-${left.color}-${left.size}-${left.partName}`.localeCompare(`${right.materialSku}-${right.color}-${right.size}-${right.partName}`, 'zh-CN'));
}
...
function findSummaryMaterialIdentity(detail, materialSku, sourceNo) {
    return (detail.sourceObjects.find((item) => item.sourceNo === sourceNo && item.materialSku === materialSku)
...
function getPrefilterFromQuery() {
    ...
    const issueType = params.get('issueType') || undefined;
...
function rowMatchesPrefilter(row, prefilter) {
    if (!prefilter)
        return true;
    if (prefilter.productionOrderNo && row.productionOrderNo !== prefilter.productionOrderNo)
        return false;
    if (prefilter.cutOrderNo && !row.relatedCutOrderNos.includes(prefilter.cutOrderNo))
        return false;
    if (prefilter.markerPlanNo && !row.relatedMarkerPlanNos.includes(prefilter.markerPlanNo))
        return false;
    if (prefilter.ticketNo && !row.relatedTicketNos.includes(prefilter.ticketNo))
        return false;
    if (prefilter.bagCode && !row.relatedBagCodes.includes(prefilter.bagCode))
        return false;
    if (prefilter.usageNo && !row.relatedUsageNos.includes(prefilter.usageNo))
        return false;
    if (prefilter.suggestionId && !row.relatedSuggestionIds.includes(prefilter.suggestionId))
        return false;
    if (prefilter.processOrderNo && !row.relatedProcessOrderNos.includes(prefilter.processOrderNo))
        return false;
    if (prefilter.materialSku && !row.relatedMaterialSkus.includes(prefilter.materialSku))
        return false;
    if (prefilter.issueType && !row.issueTypes.includes(prefilter.issueType))
        return false;
    if (prefilter.blockerSection && !row.blockerItems.some((item) => item.sectionKey === prefilter.blockerSection))
        return false;
    return true;
}
function getBlockerSourceFallback(row, sourceType) {
    if (sourceType === 'CUT_ORDER')
        return row.relatedCutOrderNos.length > 0;
    if (sourceType === 'MARKER_PLAN')
        return row.relatedMarkerPlanNos.length > 0;
    if (sourceType === 'REPLENISHMENT')
        return row.relatedSuggestionIds.length > 0;
    if (sourceType === 'FEI_OWNER')
        return row.relatedCutOrderNos.length > 0 && row.relatedTicketNos.length > 0;
    if (sourceType === 'FEI_PRINT_JOB')
        return Boolean(row.latestPrintJobNo);
    if (sourceType === 'BAG_USAGE')
        return row.relatedUsageNos.length > 0;
    return row.relatedProcessOrderNos.length > 0;
}
function getSourceNumberTokens(row) {
    return [
...
function getHighestBlockerWeight(row) {
    if (row.blockerItems.some((item) => item.severity === 'HIGH'))
        return 3;
    if (row.blockerItems.some((item) => item.severity === 'MEDIUM'))
        return 2;
    if (row.blockerItems.some((item) => item.severity === 'LOW'))
        return 1;
    return 0;
}
function getFilteredRows(viewModel, options = {}) {
    ...
    return filterSummaryByIssueType(viewModel.rows, options.ignoreIssueType ? 'ALL' : state.filters.issueType)
        .filter((row) => {
        if (!rowMatchesPrefilter(row, state.prefilter))
            return false;
        if (state.filters.riskLevel !== 'ALL' && row.overallRiskLevel !== state.filters.riskLevel)
            return false;
        if (state.filters.currentStage !== 'ALL') {
            const matchesStage = state.filters.currentStage === 'NOT_STARTED'
        ...
        if (state.filters.completionState !== 'ALL' && row.completionState !== state.filters.completionState)
            return false;
        if (state.filters.pendingReplenishmentOnly && row.pendingReplenishmentCount === 0)
            return false;
        if (state.filters.pendingTicketsOnly && row.unprintedOwnerCount === 0)
            return false;
        if (state.filters.pendingBagOnly && row.openBagUsageCount === 0)
            return false;
        if (state.filters.specialProcessOnly && row.openSpecialProcessCount === 0)
            return false;
        if (state.filters.blockerSection !== 'ALL') {
            ...
            const hasRelevantSection = row.checkSections.some((section) => section.sectionKey === state.filters.blockerSection && section.stateKey !== 'NOT_APPLICABLE');
        ...
        if (keyword && !row.keywordIndex.some((token) => token.includes(keyword)))
            return false;
        if (materialSkuKeyword &&
            ![
        ...
        if (sourceNoKeyword && !getSourceNumberTokens(row).some((token) => token.includes(sourceNoKeyword)))
            return false;
        return true;
    })
        .sort((left, right) => {
...
function getActiveRowId(rows, issues) {
    if (state.activeRowId && rows.some((row) => row.rowId === state.activeRowId))
...
function syncStateWithQuery(viewModel) {
    ...
    if (state.prefilter?.issueType)
        state.filters.issueType = state.prefilter.issueType;
    if (state.prefilter?.blockerSection)
        state.filters.blockerSection = state.prefilter.blockerSection;
    if (state.prefilter?.issueType) {
...
function buildSummaryDrillContext(payload, extra) {
...
function navigateWithPayload(target, payload, extra) {
    appStore.navigate(buildCuttingRouteWithContext(target, buildSummaryDrillContext(payload, extra)));
    return true;
}
function getResultLines(rows, aggregates) {
    return rows
        .map((row) => aggregates.resultLineByProductionId.get(row.productionOrderId))
        .filter((line) => Boolean(line));
}
function renderCuttingResultOverview(rows, aggregates) {
    const resultLines = getResultLines(rows, aggregates);
    const totals = resultLines.reduce((summary, line) => {
...
function buildSummaryRenderAggregates(rows, sources) {
    ...
    const snapshotByProductionId = new Map(snapshots.map((snapshot) => [snapshot.productionOrderId, snapshot]));
    ...
    const partResultLinesByProductionId = new Map(rows.map((row) => [row.productionOrderId, buildSummaryPartResultLines(row, transferBags)]));
    const resultLineByProductionId = new Map(rows.map((row) => [
    ...
    return {
        ...
        sewingDispatchByProductionId: new Map(rows.map((row) => [row.productionOrderId, getRuntimeSewingDispatchProgressByProductionOrder(row.productionOrderId)])),
        cuttingProgressSnapshots: snapshots,
        resultLineByProductionId,
        partResultLinesByProductionId,
    };
}
function renderSpecialCraftReturnOverview(rows, aggregates) {
    const summaries = rows
        .map((row) => aggregates.specialReturnByProductionId.get(row.productionOrderId))
        .filter((item) => Boolean(item && item.totalNeedSpecialCraftFeiTickets > 0));
    if (summaries.length === 0)
        return '';
    const aggregated = summaries.reduce((result, item) => {
...
function renderSewingDispatchOverview(rows, aggregates) {
    const summaries = rows
        .map((row) => aggregates.sewingDispatchByProductionId.get(row.productionOrderId))
        .filter((item) => Boolean(item));
    const total = summaries.reduce((result, item) => {
...
function renderProgressStatisticsSummary(rows, aggregates) {
...
function renderFilterInput(label, field, value, placeholder) {
    return `
    <label class="space-y-2">
      <span class="text-sm font-medium text-foreground">${escapeHtml(label)}</span>
      <input
    ...
    <div class="space-y-3">
      ...
      <div class="grid gap-3 xl:grid-cols-6">
        ...
        <div class="flex flex-wrap items-end gap-2 xl:col-span-2">
          ${renderWorkbenchFilterChip(state.filters.pendingReplenishmentOnly ? '已选：只看待补料' : '只看待补料', 'data-cutting-summary-action="toggle-replenishment"', 'amber')}
          ${renderWorkbenchFilterChip(state.filters.pendingTicketsOnly ? '已选：只看待打印菲票' : '只看待打印菲票', 'data-cutting-summary-action="toggle-tickets"', 'blue')}
          ${renderWorkbenchFilterChip(state.filters.pendingBagOnly ? '已选：只看待交接 / 待回仓' : '只看待交接 / 待回仓', 'data-cutting-summary-action="toggle-bags"', 'rose')}
          ${renderWorkbenchFilterChip(state.filters.specialProcessOnly ? '已选：只看特殊工艺' : '只看特殊工艺', 'data-cutting-summary-action="toggle-special"', 'emerald')}
...
function renderCheckStateBar(rows) {
...
function renderResultMainTable(rows, aggregates) {
    return `
    <section class="space-y-3 rounded-lg border bg-card p-4">
      <div class="flex items-center justify-between gap-3">
        <div>
      ...
      ${renderStickyTableScroller(rows.length
        ? `
...
function renderResultConclusion(detail, aggregates) {
...
function renderPartResultSection(detail, aggregates) {
...
function renderCheckConclusion(detail) {
    const whyBlocked = detail.primaryBlocker?.blockerReason || detail.completionMeta.detailText || detail.row.primaryBlockerReason || '当前暂无风险说明。';
    return `
    <article class="rounded-lg border p-3">
...
function renderPieceTruthSkuSection(detail) {
...
function renderPieceTruthGapSection(detail) {
    const rows = detail.pieceTruth.gapRows.filter((row) => row.mappingStatus !== 'MATCHED' || row.gapCutQty > 0 || row.gapInboundQty > 0);
    return `
    <article class="rounded-lg border p-3">
      <div class="flex items-center justify-between gap-3">
...
function renderPieceTruthIssueSection(detail) {
...
function renderBlockerSeverity(level) {
...
function renderBlockerList(detail) {
    return `
    <article class="rounded-lg border p-3">
      <div class="flex items-center justify-between gap-3">
            ...
            : '<span class="text-xs text-muted-foreground">—</span>'}
                            ...
                            <td class="px-3 py-3 text-right">
                              <button type="button" class="rounded-md border px-2 py-1 text-xs hover:bg-muted" data-cutting-summary-action="navigate-blocker" data-row-id="${detail.row.rowId}" data-blocker-id="${item.blockerId}">
                                ${escapeHtml(getCuttingNavigationActionLabel(item.navigationTarget))}
                              </button>
                            </td>
                          </tr>
                        `)
...
function renderSourceObjects(detail) {
    ...
    <article class="rounded-lg border p-3">
      ...
      ${groups.length
        ? `<div class="mt-3 space-y-3">
            ...
            }, { compact: true, imageSizeClass: 'h-9 w-9', showCategory: false })}</div>`
            : ''}
                                  </div>
...
function renderSectionStates(detail) {
    return `
    <article class="rounded-lg border p-3">
      <div class="flex items-center justify-between gap-3">
...
function renderDetailPanel(detail, aggregates) {
    if (!detail) {
        return `
...
function renderTraceNode(node, rowId) {
    const targetMap = {
...
function renderTracePanel(detail) {
...
function getDetailByRowId(rowId) {
    if (!rowId)
        return null;
    return buildFcsCuttingSummaryDetailProjection(rowId, buildProjection());
}
function getRowAction(rowId, actionId) {
    const row = getRowById(rowId);
    if (!row || !actionId)
        return null;
    return row.nextActions.find((item) => item.actionId === actionId) || null;
}
function getRowBlocker(rowId, blockerId) {
    const row = getRowById(rowId);
    if (!row || !blockerId)
        return null;
    return row.blockerItems.find((item) => item.blockerId === blockerId) || null;
}
function getRowSection(rowId, sectionKey) {
    const row = getRowById(rowId);
    if (!row || !sectionKey)
        return null;
    return row.checkSections.find((item) => item.sectionKey === sectionKey) || null;
}
function getSourceObject(rowId, sourceType, sourceId) {
    const detail = getDetailByRowId(rowId);
    if (!detail || !sourceType || !sourceId)
        return null;
    return detail.sourceObjects.find((item) => item.sourceType === sourceType && item.sourceId === sourceId) || null;
}
export function renderCraftCuttingSummaryPage() {
    return renderPage();
}
export function handleCraftCuttingSummaryEvent(target) {
    const filterFieldNode = target.closest('[data-cutting-summary-field]');
    if (filterFieldNode) {
        const field = filterFieldNode.dataset.cuttingSummaryField;
        if (!field)
            return false;
        state.filters = {
            ...state.filters,
    ...
    const actionNode = target.closest('[data-cutting-summary-action]');
    ...
    if (action === 'navigate-next-action') {
        ...
        if (!nextAction)
            return false;
        return navigateWithPayload(nextAction.target, nextAction.payload, {
    ...
    if (action === 'navigate-blocker') {
        ...
        if (!blocker)
            return false;
        return navigateWithPayload(blocker.navigationTarget, blocker.navigationPayload, {
    ...
    if (action === 'navigate-section-action') {
        ...
        if (!section)
            return false;
        return navigateWithPayload(section.defaultAction.target, section.defaultAction.payload, {
            blockerSection: section.sectionKey,
            sourceSection: 'section-state',
        });
    }
    if (action === 'navigate-source-object') {
        const sourceObject = getSourceObject(actionNode.dataset.rowId || state.activeRowId || undefined, actionNode.dataset.sourceType, actionNode.dataset.sourceId);
        if (!sourceObject)
            return false;
        return navigateWithPayload(sourceObject.navigationTarget, sourceObject.navigationPayload, {
...
export function isCraftCuttingSummaryDialogOpen() {
    return false;
}
