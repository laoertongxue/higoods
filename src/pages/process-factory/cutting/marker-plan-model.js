...
import { getCurrentTechPackVersionByStyleId, getTechnicalDataVersionContentById, } from '../../../data/pcs-technical-data-version-repository.ts';
import { buildMarkerAllocationSourceRows, buildMarkerPieceExplosionViewModel, } from './marker-piece-explosion.ts';
import { CUTTING_MARKER_SPREADING_LEDGER_STORAGE_KEY, deserializeMarkerSpreadingStorage, } from './marker-spreading-model.ts';
import { DEFAULT_SINGLE_SPREAD_FIXED_LOSS, MARKER_PLAN_STORAGE_KEY, MARKER_SIZE_CODES, buildMarkerAllocationDiffFormula, buildMarkerAllocationSumFormula, buildMarkerPlanCombinationGroupKey, buildMarkerFinalUnitUsageFormula, buildMarkerPlanSystemUnitUsageFormula, buildMarkerPlannedSpreadLengthFormula, buildMarkerSkuExplodedPieceQtyFormula, buildMarkerTotalPiecesFormula, computeMarkerAllocationDiffBySize, computeMarkerAllocationSumBySize, computeMarkerExplodedPieceQty, computeMarkerFoldedEffectiveWidth, computeMarkerFoldWidthCheckPassed, computeMarkerLayoutLineSpreadLength, computeMarkerPlanFinalUnitUsage, computeMarkerPlanSystemUnitUsageFromBeds, computeMarkerPlanSystemUnitUsage, computeMarkerPlanTotalPieces, createEmptySizeRatioRows, deriveMarkerAllocationStatus, deriveMarkerImageStatus, deriveMarkerMappingStatus, deriveMarkerPlanDefaultTab, deriveMarkerPlanStatus, deriveMarkerReadyForSpreading, markerLayoutStatusMeta, markerMappingStatusMeta, markerPlanModeMeta, markerPlanStatusMeta, normalizeMarkerSizeCode, } from './marker-plan-domain.ts';
...
function listReferencedCutOrderIdsFromSpreadingStorage(storage = typeof localStorage === 'undefined' ? null : localStorage) {
    if (!storage)
        return [];
    try {
        const store = deserializeMarkerSpreadingStorage(storage.getItem(CUTTING_MARKER_SPREADING_LEDGER_STORAGE_KEY));
        return uniqueStrings(store.sessions.flatMap((session) => [
            ...(session.cutOrderIds || []),
...
function listReferencedCutOrderIdsFromMarkerStore(store) {
...
function createMarkerNo(existingPlans, now = new Date()) {
...
function buildContextKey(contextType, contextId) {
    return `${contextType}:${contextId}`;
}
export function serializeMarkerPlanStorage(records) {
    return JSON['stringify'](records);
}
export function getMarkerPlanStorageKey() {
...
export function buildMarkerPlanModeOptions() {
    return ['normal', 'high_low', 'fold_normal', 'fold_high_low'].map((value) => ({
        value,
...
export function buildMarkerPlanGoSpreadingPath(plan) {
...
export function deserializeMarkerPlanStorage(raw) {
    if (!raw)
        return [];
    try {
        ...
        if (!Array.isArray(parsed))
            return [];
        return parsed.filter((record) => Boolean(record && typeof record === 'object' && typeof record.id === 'string' && typeof record.markerNo === 'string'));
    }
    catch {
        return [];
    }
}
function buildSizeRatioRowsFromSourceRecords(sourceRows) {
    const qtyMap = Object.fromEntries(MARKER_SIZE_CODES.map((sizeCode) => [sizeCode, 0]));
...
function buildMaterialPrepRowMap(rows) {
    return Object.fromEntries(rows.map((row) => [row.cutOrderId, row]));
}
function buildCutOrderRowMap(rows) {
    return Object.fromEntries(rows.map((row) => [row.cutOrderId, row]));
}
function buildProductionRowMap(rows) {
    return Object.fromEntries(rows.map((row) => [row.productionOrderId, row]));
}
function buildGeneratedRowMap() {
...
function resolveMarkerPlanEffectiveWidthFromGeneratedRows(rows) {
...
function buildMarkerPlanPatternKeyFromGeneratedRows(rows) {
...
function splitContextSummary(values) {
    return uniqueStrings(values.flatMap((value) => String(value || '')
        .split('/')
        .map((item) => item.trim())));
}
function getContextFactoryName(rows) {
...
function buildFormalTechPackStatusLabel(snapshots) {
    ...
    if (versionLabels.length === 1)
        return `正式版 ${versionLabels[0]}`;
    return `正式版 ${versionLabels.join(' / ')}`;
}
function buildContextPrepStatusLabel(cutOrderRows) {
    const prepKeys = uniqueStrings(cutOrderRows.map((row) => row.materialPrepStatus.key));
    if (!prepKeys.length)
        return '无配料数量';
    if (prepKeys.length === 1)
        return cutOrderRows[0]?.materialPrepStatus.label || '无配料数量';
    if (prepKeys.includes('NOT_CONFIGURED'))
        return '无配料数量';
    if (prepKeys.includes('PARTIAL'))
        return '配料数量不足';
    return '有配料数量';
}
function buildContextClaimStatusLabel(cutOrderRows) {
    const claimKeys = uniqueStrings(cutOrderRows.map((row) => row.materialClaimStatus.key));
    if (!claimKeys.length)
        return '无领料记录';
    if (claimKeys.length === 1)
        return cutOrderRows[0]?.materialClaimStatus.label || '无领料记录';
    if (claimKeys.includes('NOT_RECEIVED'))
        return '无领料记录';
    if (claimKeys.includes('PARTIAL'))
        return '领料数量不足';
    return '有领料记录';
}
function buildContextPrepClaimSummaryText(cutOrderRows) {
...
export function buildMarkerPlanContextCandidates(sources) {
    ...
    const cutOrderContexts = sources.cutOrderRows
        .map((row) => buildCutOrderContextCandidate({
        row,
    ...
    return cutOrderContexts.sort((left, right) => `${left.styleCode}-${left.contextNo}`.localeCompare(`${right.styleCode}-${right.contextNo}`, 'zh-CN'));
}
export function buildCombinedMarkerPlanContextCandidate(contexts) {
    ...
    const sourceCutOrderRows = uniqueByKey(selectedContexts.flatMap((context) => context.sourceCutOrderRows), (row) => row.cutOrderId);
    if (!sourceCutOrderRows.length)
        return null;
    const sourceMaterialPrepRows = uniqueByKey(selectedContexts.flatMap((context) => context.sourceMaterialPrepRows), (row) => row.cutOrderId);
    const sourceGeneratedRows = uniqueByKey(selectedContexts.flatMap((context) => context.sourceGeneratedRows), (row) => row.cutOrderId);
    ...
    return {
        ...
        sourceFactoryName: getContextFactoryName(selectedContexts.flatMap((context) => context.productionOrderIds.map((productionOrderId) => ({
            productionOrderId,
...
function buildMockImageSvg(label, accent) {
...
function buildDefaultFoldConfig(context) {
...
function buildAutoAllocationRows(context, sizeRatioRows) {
    const remaining = Object.fromEntries(MARKER_SIZE_CODES.map((sizeCode) => [sizeCode, 0]));
...
function buildSchemeDemandRowsFromContext(context) {
    return context.sourceGeneratedRows.flatMap((sourceRow) => sourceRow.skuScopeLines.map((skuLine, index) => {
...
function buildSizeRatioRowsFromDemandRows(demandRows) {
    const qtyMap = Object.fromEntries(MARKER_SIZE_CODES.map((sizeCode) => [sizeCode, 0]));
...
function buildCoverageRowsFromDemandRows(rows, colorName) {
    return rows
...
function buildDefaultSchemeBeds(options) {
    ...
    const matrixRows = activeColors.map((colorName, index) => {
        const sizeValues = Object.fromEntries(sizeNames.map((sizeName) => [sizeName, 0]));
        return {
    ...
    return [
        {
            ...
            sizePiecePerLayer: Object.fromEntries(sizeNames.map((sizeName) => [sizeName, 0])),
            plannedLayerCount,
            markerLength,
...
function getSeedPiecePerLayer(demands) {
    ...
    for (let candidate = Math.min(demandGcd, 80); candidate >= 1; candidate -= 1) {
        if (demandGcd % candidate === 0)
            return candidate;
    }
    return 1;
}
function getSeedLayerCount(demandQty, piecePerLayer, bedMode) {
    ...
    if (!isFoldMarkerBedMode(bedMode))
        return layerCount;
    return layerCount % 2 === 0 ? layerCount : layerCount + 1;
}
function buildReadySeedSchemeBeds(plan, context) {
    ...
    const demandByColorSize = new Map();
    demandRows.forEach((row) => {
    ...
    const sizePiecePerLayer = Object.fromEntries(activeSizes.map((sizeName) => [
        sizeName,
    ...
    const matrixRows = activeColors.map((colorName, index) => {
        const rawLayers = activeSizes.map((sizeName) => getSeedLayerCount(demandByColorSize.get(buildDemandMatchKey(colorName, sizeName)) || 0, sizePiecePerLayer[sizeName], bedMode));
        const rowLayer = Math.max(...rawLayers, 0);
        const sizeValues = Object.fromEntries(activeSizes.map((sizeName) => [sizeName, rowLayer]));
        const totalQty = activeSizes.reduce((total, sizeName) => total + Math.max(Math.round(safeNumber(sizeValues[sizeName])), 0) * Math.max(Math.round(safeNumber(sizePiecePerLayer[sizeName])), 0), 0);
        return {
...
function adaptPlanToMarkerExplosionInput(plan) {
    const allocationLines = plan.allocationRows.map((row) => ({
        allocationId: row.id,
...
function buildPieceExplosionRows(plan, context) {
    ...
    const previousOverrides = Object.fromEntries((plan.pieceExplosionRows || [])
...
function buildResolvedPieceExplosionOverrides(plan, context) {
...
function hydrateFoldConfig(foldConfig) {
    if (!foldConfig)
        return null;
...
function isFoldMarkerBedMode(mode) {
    return mode === 'fold_normal' || mode === 'fold_high_low';
}
function isHighLowMarkerBedMode(mode) {
    return mode === 'high_low' || mode === 'fold_high_low';
}
function normalizeMarkerSizePiecePerLayer(bed, sizeNames) {
    return Object.fromEntries(sizeNames.map((sizeName) => [sizeName, Math.max(Math.round(safeNumber(bed.sizePiecePerLayer?.[sizeName])), 0)]));
}
function getMarkerMatrixCellLayer(row, sizeName) {
    return Math.max(Math.round(safeNumber(row.sizeValues?.[sizeName])), 0);
}
function getMarkerMatrixCellPlannedQty(row, sizeName, sizePiecePerLayer) {
...
function normalizeMarkerMatrixRows(bed, sizeNames) {
    ...
    return rows.map((row, index) => {
        const sizeValues = Object.fromEntries(sizeNames.map((sizeName) => [
            sizeName,
            Math.max(Math.round(safeNumber(row.sizeValues?.[sizeName])), 0),
        ]));
        const totalQty = sizeNames.reduce((total, sizeName) => total + Math.max(Math.round(safeNumber(sizeValues[sizeName])), 0) * Math.max(Math.round(safeNumber(sizePiecePerLayer[sizeName])), 0), 0);
        return {
...
function buildDemandMatchKey(colorName, sizeName) {
    return `${String(colorName || '').trim()}::${String(sizeName || '').trim()}`;
}
function getDemandMatchRowStatus(diffQty) {
    if (diffQty < 0)
        return '不足';
    if (diffQty > 0)
        return '超出';
...
function getDemandMatchStatus(summary) {
    if (summary.markerTotalQty <= 0)
        return '待编辑唛架';
    if (summary.shortageQty > 0 && summary.surplusQty > 0)
        return '有差异';
    if (summary.shortageQty > 0)
        return '有不足';
    if (summary.surplusQty > 0)
        return '有超出';
    return '已匹配';
}
export function buildMarkerDemandMatchSummary(plan) {
    ...
    const rows = Array.from(demandMap.values())
        ...
        .sort((left, right) => `${left.colorName}-${left.sizeName}`.localeCompare(`${right.colorName}-${right.sizeName}`, 'zh-CN'));
...
function getMarkerMatrixActualLayerTotal(bed, rows, sizeNames) {
    const columnTotals = sizeNames.map((sizeName) => rows.reduce((total, row) => {
...
function getMarkerMatrixRowActualLayer(bed, row, sizeNames) {
...
export function hydrateMarkerPlan(plan, context) {
    ...
    const beds = (plan.beds || []).map((bed, index) => {
        const sizeNames = uniqueStrings((schemeDemandRows.length ? schemeDemandRows : bed.coverageRows || [])
        ...
        const spreadTotalLength = isHighLowMarkerBedMode(bed.bedMode)
            ? highLowMatrixRows.reduce((total, row) => total + computeMarkerLayoutLineSpreadLength({ markerLength: Math.max(safeNumber(row.markerLength), 0), repeatCount: getMarkerMatrixRowActualLayer(bed, row, sizeNames) }, plan.singleSpreadFixedLoss), 0)
            : computeMarkerLayoutLineSpreadLength({ markerLength, repeatCount: plannedLayerCount }, plan.singleSpreadFixedLoss);
        const unitFabricUsage = plannedGarmentQty > 0 ? roundTo(spreadTotalLength / plannedGarmentQty, 3) : 0;
    ...
    const netLength = roundTo(beds.length
    ...
    const layoutStatus = beds.length > 0 && beds.every((bed) => bed.readyForSpreading) ? 'done' : 'pending';
    const confirmationStatus = plan.confirmationStatus || '待确认';
...
function buildBalanceRows(plan) {
...
function buildPlanViewRow(plan, context, referencedCutOrderIds) {
...
function createPlanFromContext(options) {
    ...
    const netLength = roundTo(beds.length ? sum(beds.map((bed) => bed.markerLength)) : Math.max(Math.ceil(totalPieces / plannedLayerCount) * DEFAULT_MARKER_GARMENT_LENGTH_M, 1), 2);
    const plan = {
        id: planId,
        markerNo,
...
export function cloneMarkerPlanAsNewDraft(source, existingPlans) {
    const now = new Date();
    const planId = `marker-plan-${now.getTime()}-${Math.random().toString(36).slice(2, 8)}`;
...
function buildSeedVariants(contexts) {
    const modes = ['normal', 'high_low', 'fold_normal', 'fold_high_low'];
    const variants = ['ready', 'layout', 'ready', 'image', 'ready', 'manual'];
    const preferredContexts = contexts.filter((context) => context.contextType === 'cut-order' && context.spuCode !== 'SPU-2024-010');
    const uniqueContexts = preferredContexts.filter((context, index, all) => all.findIndex((item) => item.contextKey === context.contextKey) === index);
...
function applySeedVariant(plan, variant, context) {
    let nextPlan = {
        ...plan,
        imageRecords: [],
    ...
    if (variant === 'unbalanced' && nextPlan.allocationRows[0]) {
        nextPlan = {
            ...nextPlan,
            imageRecords: [],
            allocationRows: nextPlan.allocationRows.map((row, index) => index === 0 ? { ...row, garmentQty: Math.max(row.garmentQty - 1, 0) } : row),
        };
    }
    if (variant === 'mapping' && nextPlan.allocationRows[0]) {
        nextPlan = {
            ...
            allocationRows: nextPlan.allocationRows.map((row, index) => index === 0
                ? {
                    ...row,
    ...
    if (variant === 'layout') {
        nextPlan = {
            ...nextPlan,
            imageRecords: [],
            beds: nextPlan.beds?.map((bed, index) => index === 0
                ? {
                    ...bed,
                    markerLength: 0,
                    markerPieceQtyPerLayer: 0,
                ...
                : bed) || [],
            foldConfig: nextPlan.foldConfig && (nextPlan.markerMode === 'fold_normal' || nextPlan.markerMode === 'fold_high_low')
                ? {
                    ...nextPlan.foldConfig,
    ...
    if (variant === 'ready' || variant === 'manual') {
        ...
        nextPlan = {
            ...nextPlan,
            beds: variant === 'manual'
                ? readyBeds.flatMap((bed) => [0, 1, 2].map((offset) => {
                    ...
                    return {
                        ...
                        highLowMatrixRows: (bed.highLowMatrixRows || []).map((row, rowIndex) => ({
                            ...
                            sizeValues: Object.fromEntries(Object.entries(row.sizeValues || {}).map(([sizeName, value]) => [
                                sizeName,
...
function rekeySeedMarkerPlan(plan, stablePlanId) {
...
function buildSystemSeedMarkerPlans(contexts) {
    const plans = [];
...
export function buildMarkerPlanViewModel(sources, storedPlans = []) {
    ...
    const referencedCutOrderIds = new Set(uniqueStrings([
...
export function buildMarkerPlanMockCoverageReport(sources, storedPlans = [], options = {}) {
    const contexts = buildMarkerPlanContextCandidates(sources);
    ...
    const hydratedPlans = mergedPlans
        ...
        .filter((plan) => Boolean(plan));
    const usedContextKeys = new Set(hydratedPlans.map((plan) => buildContextKey(plan.contextType, plan.contextType === 'marker-plan-ref' ? plan.markerPlanId : plan.cutOrderIds[0])));
    const pendingContexts = contexts.filter((context) => !usedContextKeys.has(context.contextKey));
    const modeCounts = Object.fromEntries(Object.keys(markerPlanModeMeta).map((key) => [key, 0]));
    const statusCounts = Object.fromEntries(Object.keys(markerPlanStatusMeta).map((key) => [key, 0]));
...
export function buildMarkerPlanContextMap(contexts) {
    return Object.fromEntries(contexts.map((context) => [context.contextKey, context]));
}
export function buildMarkerPlanContextKey(contextType, contextId) {
    return buildContextKey(contextType, contextId);
}
export function findMarkerPlanContextById(contexts, contextType, contextId) {
    const contextKey = buildContextKey(contextType, contextId);
    return contexts.find((context) => context.contextKey === contextKey) ?? null;
}
export function findMarkerPlanContextForPlan(contexts, plan) {
    if (plan.contextType !== 'marker-plan-ref' && (plan.cutOrderIds.length > 1 || plan.markerPlanNo)) {
        ...
        const markerPlanContexts = contexts.filter((context) => context.contextType === 'marker-plan-ref' && markerPlanNos.has(context.markerPlanNo));
        ...
        const cutOrderContexts = plan.cutOrderIds
            ...
            .map((cutOrderId) => contexts.find((context) => context.contextType === 'cut-order' &&
            context.cutOrderIds.includes(cutOrderId)) || null)
            .filter((context) => Boolean(context));
        const sourceContexts = uniqueByKey([...markerPlanContexts, ...cutOrderContexts], (context) => context.contextKey);
...
export function getMarkerPlanReferencedWarning(plan) {
    return plan.isReferencedBySpreading ? plan.referenceWarningText : '';
}
function listUsedContextKeysForPlan(contexts, plan) {
...
export function getMarkerPlanInitialEditTab(plan) {
