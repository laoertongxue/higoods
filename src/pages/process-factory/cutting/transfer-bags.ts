...
function renderMasterStatusActions(options: {
  ...
  return actions.map((action) => {
    if (action === '查看详情') {
      return `<button type="button" class="rounded-md border px-2.5 py-1.5 text-xs hover:bg-muted" aria-label="查看袋内菲票明细和差异类型" data-nav="${escapeHtml(detailHref)}">查看详情</button>`
    }
...
const transferBagListColumns: StandardListColumn<TransferBagMasterListRow>[] = [
  ...
  },
  {
    ...
    sortable: true,
    render: ({ item, carrierRecord }) => `
      <div class="font-medium text-foreground">${escapeHtml(carrierRecord?.currentLocation || '—')}</div>
      <div class="mt-1 text-xs text-muted-foreground">${escapeHtml(carrierRecord?.enabled === false ? '已报废' : '可流转')}</div>
    `,
    sortValue: ({ item, carrierRecord }) =>
      carrierRecord?.currentLocation || '',
  },
  {
    key: 'load',
    title: '当前装载',
