import { buildPrintableUnitViewModel, buildSystemSeedFeiTicketLedger, } from './fei-tickets-model.ts';
import { buildBagUsageAuditTrail, applyPocketBindingLocksToTicketRecords, buildSystemSeedTransferBagStore, buildTransferBagViewModel, mergeTransferBagStores, } from './transfer-bags-model.ts';
import { buildSpreadingTraceAnchors, findSpreadingTraceAnchor, finalizeSpreadingCompletion, upsertSpreadingSession, } from './marker-spreading-model.ts';
import { buildMarkerSpreadingPrototypeStore } from './marker-spreading-utils.ts';
import { applyWritebackToSpreadingSession, buildMockPdaWritebacks, } from '../../../data/fcs/cutting/pda-spreading-writeback.ts';
import { buildTransferBagReturnViewModel, } from './transfer-bag-return-model.ts';
import { parseCuttingTraceQr, } from '../../../data/fcs/cutting/qr-codes.ts';
...
let defaultTraceabilityProjectionCache = null;
const snapshotTraceabilityProjectionCache = new WeakMap();
function castTicketRecords(input) {
    return input;
}
function castPrintJobs(input) {
    return input;
}
function castTransferBagStore(input) {
    return input;
}
function mergeTicketRecords(seed, stored) {
...
function mergePrintJobs(seed, stored) {
...
function buildTraceabilitySpreadingContext(session, materialPrepRows, markerPlanRefs) {
    const relatedRows = materialPrepRows.filter((row) => session.cutOrderIds.includes(row.cutOrderId));
    const batch = (session.markerPlanId
    ...
    return {
        ...
        techPackSpuCode: uniqueStrings(relatedRows.map((row) => row.techPackSpuCode)).length === 1
...
function ensureTraceabilityTicketRecords(options) {
    ...
    options.spreadingStore.sessions
        ...
        relatedRows.forEach((row, rowIndex) => {
            ...
            if (ticketIds.has(ticketRecordId))
                return;
            ...
            const markerPlanRef = (session.markerPlanId
...
function hydrateTraceabilitySpreadingStore(options) {
    ...
    targetSessions.forEach((targetSession, index) => {
        ...
        if (!context)
            return;
        const writebackDraft = buildMockPdaWritebacks({ context, sessions: [session] }).find((item) => item.sourceAccountId && item.contextType === context.contextType && item.rollItems.length > 0) || null;
        if (!writebackDraft)
            return;
        ...
        const writeback = {
            ...
            operatorItems: writebackDraft.operatorItems.map((item, operatorIndex) => ({
                ...
                rollWritebackItemId: rollItems[operatorIndex]?.rollWritebackItemId || rollItems[0]?.rollWritebackItemId || `${writebackId}-roll-1`,
            })),
        };
        ...
        if (latestSession.contextType === 'marker-plan-ref' && latestSession.status !== 'DONE') {
            const markerRecord = options.markerStore?.markers?.find((item) => item.markerId === latestSession.markerId) || null;
...
function ensureTraceabilityBagFirstSeed(options) {
    ...
    const targetSessions = rankedDoneSessions.reduce((accumulator, session) => {
        if (accumulator.some((item) => item.spreadingSessionId === session.spreadingSessionId))
            return accumulator;
    ...
    targetSessions.forEach((session, index) => {
        ...
        const traceAnchor = traceAnchors.find((item) => item.spreadingSessionId === sessionId) ||
        ...
        const candidateTickets = options.ticketRecords.filter((ticket) => session.cutOrderIds.includes(ticket.cutOrderId));
        const existingBindings = nextStore.bindings.filter((binding) => session.cutOrderIds.includes(binding.cutOrderId));
        ...
        const existingUsageMatches = existingUsageIds.some((usageId) => {
            ...
            const markerPlanId = uniqueStrings(bindings.map((binding) => {
            ...
            const materialSku = uniqueStrings(bindings.map((binding) => {
                const ticket = options.ticketRecords.find((item) => item.ticketRecordId === binding.ticketRecordId);
                return ticket?.materialSku || '';
            }))[0] || '';
            const color = uniqueStrings(bindings.map((binding) => {
                const ticket = options.ticketRecords.find((item) => item.ticketRecordId === binding.ticketRecordId);
                return ticket?.color || '';
            }))[0] || '';
        ...
        if (existingUsageMatches)
            return;
        const unboundTickets = candidateTickets.filter((ticket) => !nextStore.bindings.some((binding) => binding.ticketRecordId === ticket.ticketRecordId));
        const tickets = (unboundTickets.length ? unboundTickets : candidateTickets).sort((left, right) => left.ticketNo.localeCompare(right.ticketNo, 'zh-CN'));
        if (!tickets.length)
            return;
        const bag = nextStore.masters.find((item) => !usedBagIds.has(item.bagId) && item.currentStatus === 'IDLE') ||
            nextStore.masters.find((item) => !usedBagIds.has(item.bagId)) ||
            nextStore.masters.find((item) => item.currentStatus === 'IDLE') ||
            nextStore.masters[0] ||
            null;
        const sewingTask = (session.markerPlanNo
        ...
        const bindings = tickets.map((ticket, ticketIndex) => ({
        ...
        nextStore = {
            ...nextStore,
            masters: nextStore.masters.map((item) => item.bagId === bag.bagId
                ? {
                    ...item,
...
export function buildCuttingTraceabilityProjectionContext(snapshot, storeOverride) {
    ...
    const mergedRawTicketRecords = mergeTicketRecords(seedLedger.ticketRecords, castTicketRecords(effectiveSnapshot.feiTicketState.ticketRecords));
...
export function resolveCarrierScanInput(input, store) {
    const normalized = input.trim();
    if (!normalized)
        return null;
    const parsed = parseCarrierQrValue(normalized);
    if (parsed) {
        return (store.masters.find((item) => item.carrierId === parsed.carrierId) ||
...
export function resolveFeiTicketScanInput(input, ticketRecords) {
    const normalized = input.trim();
    if (!normalized)
        return null;
    const parsed = parseCuttingTraceQr(normalized);
    if (parsed?.codeType === 'FEI_TICKET') {
        return (ticketRecords.find((item) => item.ticketRecordId === parsed.feiTicketId) ||
            ticketRecords.find((item) => item.ticketNo === parsed.feiTicketNo) ||
            null);
    }
    return (ticketRecords.find((item) => item.ticketNo === normalized) ||
...
export function buildSpreadingBagWarehouseTraceProjection(options) {
    const usageMap = Object.fromEntries(options.transferBagViewModel.usages.map((item) => [item.usageId, item]));
    return options.warehouseItems
        ...
        .sort((left, right) => {
        const leftScore = (left.spreadingSessionId ? 4 : 0) +
            (left.sourceWritebackId ? 2 : 0) +
            (left.bagFirstSatisfied ? 1 : 0);
        const rightScore = (right.spreadingSessionId ? 4 : 0) +
