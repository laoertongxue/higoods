...
export function filterReplenishmentRecords(records, filters) {
...
export function buildReplenishmentSummary(records) {
    return {
    ...
    return [...records]
        ...
        const weight = (record) => {
            ...
            if (record.reviewStatus === 'PENDING')
                score += 20;
            if (record.reviewStatus === 'NEED_MORE_INFO')
                score += 10;
            if (record.impactFlags.includes('PENDING_PREP_REQUIRED'))
                score += 5;
            return score;
        };
...
export function buildRiskTags(record) {
    if (Array.isArray(record))
        return [];
    const tags = new Set();
    if (record.riskLevel === 'HIGH')
        tags.add('HIGH_GAP');
    if (record.suggestionSourceTypes.includes('RECEIVE_DISCREPANCY'))
        tags.add('RECEIVE_DIFF');
    if (!record.hasMarkerImage)
        tags.add('MARKER_PENDING');
    if (record.spreadingRecordCount === 0)
        tags.add('SPREADING_PENDING');
    if (record.reviewStatus === 'PENDING')
        tags.add('PENDING_REVIEW');
    if (record.reviewStatus === 'NEED_MORE_INFO')
        tags.add('NEED_MORE_INFO');
    return Array.from(tags);
}
export function buildImpactSummary(record) {
    if (!record.impactFlags.length)
        return '当前审核结果不触发额外联动。';
    return Array.from(new Set(record.impactFlags.map((flag) => impactFlagMeta[flag].label))).join(' / ');
}
export function buildGapSummary(record) {
...
export function buildEmptyStateText(filters) {
    if (filters.reviewStatus === 'PENDING')
        return '当前筛选条件下暂无待审核补料建议。';
    if (filters.riskLevel === 'HIGH')
        return '当前筛选条件下暂无高风险补料建议。';
    if (filters.impactFilter === 'PENDING_PREP_REQUIRED')
        return '当前筛选条件下暂无需回中转仓配料的补料建议。';
