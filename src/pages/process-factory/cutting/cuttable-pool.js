...
import { areCutOrdersReadyForMarkerPlan, buildCuttableSelectionSummary, buildQuickMarkerPlanBuckets, buildCuttablePoolStats, filterCuttablePoolGroups, validateCuttableSelection, } from './cuttable-pool-model.ts';
...
import { paginateItems, renderCompactKpiCard, renderStickyFilterShell, renderWorkbenchFilterChip, renderWorkbenchPagination, renderWorkbenchStateBar, } from './layout.helpers.ts';
...
function syncStateFromPath() {
    ...
    if (urgencyLevel && urgencyLevel in urgencyMeta)
        nextPrefilter.urgencyLevel = urgencyLevel;
    if (riskOnly)
        nextPrefilter.riskOnly = true;
...
function getSelectedItems(viewModel = getViewModel()) {
...
function normalizeCuttableOnlyFilters() {
...
function renderActionBar(viewModel = getViewModel()) {
...
function renderViewModeSwitch() {
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      ${options
        .map((option) => renderWorkbenchFilterChip(option.label, `data-cuttable-pool-action="set-view-mode" data-view-mode="${option.key}"`, state.viewMode === option.key ? 'blue' : 'emerald'))
        .join('')}
    </div>
  `;
}
function getPrefilterLabels() {
    ...
    if (prefilter.productionOrderNo)
        labels.push(`来自裁床生产单总览：${prefilter.productionOrderNo}`);
    if (prefilter.styleCode)
        labels.push(`预筛同款：${prefilter.styleCode}`);
    if (prefilter.spuCode)
        labels.push(`预筛 SPU：${prefilter.spuCode}`);
    if (prefilter.urgencyLevel)
        labels.push(`预筛紧急度：${urgencyMeta[prefilter.urgencyLevel].label}`);
    if (prefilter.riskOnly)
        labels.push('预筛：只看风险生产单');
    return labels;
}
function getFilterLabels() {
    ...
    if (state.filters.productionOrderKeyword)
        labels.push(`生产单：${state.filters.productionOrderKeyword}`);
    if (state.filters.spuKeyword)
        labels.push(`SPU：${state.filters.spuKeyword}`);
    if (state.filters.styleKeyword)
        labels.push(`款式：${state.filters.styleKeyword}`);
    if (state.filters.materialKeyword)
        labels.push(`面料：${state.filters.materialKeyword}`);
    if (state.filters.patternKeyword)
        labels.push(`纸样：${state.filters.patternKeyword}`);
    if (state.filters.effectiveWidthKeyword)
        labels.push(`有效幅宽：${state.filters.effectiveWidthKeyword}`);
    if (state.filters.historyCombinationGroupKeyword)
        labels.push(`历史组合组：${state.filters.historyCombinationGroupKeyword}`);
    ...
    if (state.filters.urgencyLevel !== 'ALL')
        labels.push(`紧急程度：${urgencyMeta[state.filters.urgencyLevel].label}`);
    return labels;
}
...
function renderPatternBlock(item) {
...
function renderLedgerBlock(item) {
...
function renderCutOrderRows(order) {
    return order.items
        .map((item) => {
...
function renderPoolPagination(total) {
...
function renderStyleGroups(groups) {
    if (!groups.length) {
    ...
    <section class="space-y-3">
      <div class="space-y-4">
        ...
        ? group.markerPlanBuckets
            .map((bucket) => renderBadge(`${bucket.materialSku} · ${bucket.cuttableCount}/${bucket.totalCount}`, bucket.cuttableCount > 0
...
function renderProductionOrderFlat(groups) {
...
function renderEmptyStateIfNeeded(groups) {
    if (groups.length)
        return '';
    return `
...
function renderSelectedCutOrdersBar(viewModel = getViewModel()) {
...
export function renderCraftCuttingCuttablePoolPage() {
    syncStateFromPath();
...
function createMarkerPlanAndGo() {
    ...
    if (summary.historyCombinationGroup)
        query.set('historyCombinationGroup', summary.historyCombinationGroup);
...
export function handleCraftCuttingCuttablePoolEvent(target) {
    const fieldNode = target.closest('[data-cuttable-pool-field]');
    if (fieldNode) {
        const field = fieldNode.dataset.cuttablePoolField;
        if (!field)
            return false;
        const input = fieldNode;
        if (field === 'keyword')
            state.filters.keyword = input.value;
        if (field === 'productionOrder')
            state.filters.productionOrderKeyword = input.value;
        if (field === 'spu')
            state.filters.spuKeyword = input.value;
        if (field === 'style')
            state.filters.styleKeyword = input.value;
        if (field === 'material')
            state.filters.materialKeyword = input.value;
        if (field === 'pattern')
            state.filters.patternKeyword = input.value;
        if (field === 'effectiveWidth')
            state.filters.effectiveWidthKeyword = input.value;
        if (field === 'historyCombinationGroup')
            state.filters.historyCombinationGroupKeyword = input.value;
        if (field === 'availableRange')
            state.filters.availableRange = input.value;
        if (field === 'urgency')
            state.filters.urgencyLevel = input.value;
        resetPagination();
        return true;
    }
    const pageSizeNode = target.closest('[data-cuttable-pool-page-size]');
    if (pageSizeNode) {
    ...
    const actionNode = target.closest('[data-cuttable-pool-action]');
    const action = actionNode?.dataset.cuttablePoolAction;
    if (!action)
        return false;
    if (action === 'set-view-mode') {
        const nextMode = actionNode.dataset.viewMode;
        if (!nextMode)
            return false;
    ...
    if (action === 'go-cut-order-detail') {
        ...
        if (!item)
            return false;
        appStore.navigate(buildRouteWithQuery(getCanonicalCuttingPath('cut-orders'), {
...
export function isCraftCuttingCuttablePoolDialogOpen() {
    return false;
}
