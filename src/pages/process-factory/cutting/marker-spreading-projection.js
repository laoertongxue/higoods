import { buildSpreadingSessionIdentityForMarkerBed, buildMarkerSpreadingViewModel, buildSpreadingPlanUnitDisplayLabel, createSpreadingDraftFromMarker, resolveSpreadingOrderStatusFromSession, } from './marker-spreading-model.ts';
...
import { findMarkerPlanContextForPlan, } from './marker-plan-model.ts';
import { markerPlanModeMeta } from './marker-plan-domain.ts';
export function buildSpreadingPlanUnitProjectionLabel(planUnit) {
    return buildSpreadingPlanUnitDisplayLabel(planUnit);
}
function buildQtyFormula(total, plannedLayers, totalPieces) {
    return `${Math.max(Math.round(total), 0)} 件 = ${Math.max(Math.round(plannedLayers), 0)} 层 × ${Math.max(Math.round(totalPieces), 0)} 件/层`;
}
function distributeBedPlannedQtyByCoverage(bed) {
...
function buildSizeDistributionFromBed(bed, distributedQty) {
    if (!bed.coverageRows.length) {
...
function buildMarkerRecordFromPlanBed(plan, context, bed) {
    ...
    const allocationLines = bed.coverageRows.map((row, index) => ({
    ...
    const normalLineItems = bed.bedMode === 'normal' || bed.bedMode === 'fold_normal'
        ? [
            {
        ...
        : [];
    const highLowCuttingRows = bed.bedMode === 'high_low' || bed.bedMode === 'fold_high_low'
        ? [
            {
                ...
                sizeValues: {
                    ...
                    onesize: 0,
                    plusonesize: 0,
                    ...Object.fromEntries(bed.coverageRows.map((row, index) => [
    ...
    const highLowPatternRows = highLowPatternKeys.length
        ? [
            {
...
function buildSpreadingContextFromPlanContext(context, plan) {
    return {
        contextType: context.contextType === 'marker-plan-ref' ? 'marker-plan-ref' : 'cut-order',
...
function buildCreateSourceRowsFromPlan(plan, context) {
    if (!context)
        return [];
    const scheme = buildMarkerSchemeFromPlan(plan);
    const spreadingContext = buildSpreadingContextFromPlanContext(context, plan);
    const contextSummary = spreadingContext.contextType === 'marker-plan-ref'
...
function buildSpreadingCreateSourceRows(planProjection) {
...
function findExistingSpreadingSession(store, identity, plan, bed) {
    return (store.sessions.find((session) => session.spreadingSessionId === identity.spreadingSessionId) ||
        store.sessions.find((session) => session.sessionNo === identity.sessionNo) ||
        store.sessions.find((session) => (session.sourceSchemeId === plan.id || session.markerPlanId === plan.id) &&
            (session.sourceBedId === bed.bedId || session.sourceBedNo === bed.bedNo)) ||
        null);
}
function parseEffectiveWidthText(value) {
...
function buildSpreadingOrderFromPlanBed(plan, context, bed, session, index) {
    const sourceOrder = context.sourceCutOrderRows[0] || null;
    ...
    const markerImageUrl = plan.schemeImage?.previewUrl ||
...
function buildConfirmedPlanSpreadingArtifacts(planProjection, baseStore) {
    const generatedSessions = new Map();
    const replacedSessionIds = new Set();
    const spreadingOrders = new Map();
    planProjection.viewModel.plans
        ...
        scheme.beds
            ...
            if (existingSession)
                replacedSessionIds.add(existingSession.spreadingSessionId);
            const markerRecord = buildMarkerRecordFromPlanBed(plan, context, bed);
    ...
    return {
        ...
        spreadingOrders: Array.from(spreadingOrders.values()).sort((left, right) => `${right.confirmedAt || right.createdAt}::${right.spreadingOrderNo}`.localeCompare(`${left.confirmedAt || left.createdAt}::${left.spreadingOrderNo}`, 'zh-CN')),
    };
}
function groupSpreadingOrdersByMarkerPlanId(spreadingOrders) {
    return spreadingOrders.reduce((accumulator, order) => {
        accumulator[order.markerPlanId] = accumulator[order.markerPlanId] || [];
        accumulator[order.markerPlanId].push(order);
        return accumulator;
    }, {});
}
function groupSpreadingOrdersByProductionOrderId(spreadingOrders) {
    return spreadingOrders.reduce((accumulator, order) => {
...
export function buildMarkerSpreadingProjection(options = {}) {
    ...
    const baseStore = options.store ??
        context.snapshot.markerSpreadingState.store;
    ...
    return {
        ...
        rows: context.sources.materialPrepRows,
        rowsById: Object.fromEntries(context.sources.materialPrepRows.map((row) => [row.cutOrderId, row])),
        rowsByProductionOrderNo: context.sources.materialPrepRows.reduce((accumulator, row) => {
