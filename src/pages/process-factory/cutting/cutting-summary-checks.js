export const cuttingCheckSectionLabelMap = {
    MATERIAL_PREP: '待加工仓',
    SPREADING: '唛架铺布',
    REPLENISHMENT: '补料',
    FEI_TICKETS: '打印菲票',
...
export const cuttingCheckCompletionMetaMap = {
    COMPLETED: {
        key: 'COMPLETED',
        label: '已闭环',
...
function buildMaterialPrepSection(options) {
    ...
    const stateKey = !options.materialPrepRows.length
        ? 'DATA_PENDING'
        : blockers.length
            ? 'BLOCKED'
            : 'DONE';
...
function buildSpreadingSection(options) {
    const blockers = [];
...
function buildReplenishmentSection(options) {
    ...
    const doneCount = options.replenishments.filter((item) => ['NO_ACTION', 'REJECTED', 'COMPLETED'].includes(item.statusMeta.key)).length;
    const stateKey = !options.replenishments.length
        ? 'NOT_APPLICABLE'
        : blockers.length
            ? 'BLOCKED'
            : 'DONE';
...
function buildFeiTicketSection(options) {
    const blockers = [];
...
function buildWarehouseSection(options) {
    const blockers = [];
    ...
    const totalCount = options.cutPieceItems.length +
...
function buildSpecialProcessSection(options) {
    ...
    const doneCount = options.specialProcesses.filter((item) => item.typeExecutionMeta.enabledForExecution && ['DONE', 'CANCELLED'].includes(item.status) && item.followupPendingCount === 0).length;
    let stateKey = 'NOT_APPLICABLE';
...
export function buildCuttingCheckResult(options) {
    ...
    const nextActions = dedupeActions([
        ...blockerItems.map((item) => buildAction({
    ...
    const completionKey = blockerItems.length
        ? 'BLOCKED'
    ...
    return {
        ...
        keySourceObjects: uniqueStrings(blockerItems.map((item) => item.sourceNo).slice(0, 3)),
    };
}
