...
export function renderFactoryWarehouseStandardTabs(tabs, idPrefix) {
    return `
...
export function renderWarehouseFlowButton(title, flows) {
...
export function renderWarehouseLocationToolbar(scopeLabel) {
    return `
    <button
      type="button"
...
export function renderWarehouseLocationActions(scopeLabel, locationLabel) {
    return `
...
function removeFlowModal() {
    document.getElementById(FLOW_MODAL_ID)?.remove();
}
function openFlowModal(title, flows) {
    removeFlowModal();
  ...
  `;
    ...
    modal.addEventListener('click', (event) => {
        const actionNode = event.target.closest('[data-warehouse-flow-action]');
        if (actionNode?.dataset.warehouseFlowAction === 'close') {
            removeFlowModal();
        }
    });
}
export function isFactoryWarehouseFlowModalOpen() {
    return Boolean(document.getElementById(FLOW_MODAL_ID));
}
export async function handleFactoryWarehouseSharedEvent(target) {
    const flowNode = target.closest('[data-warehouse-flow-action]');
    if (flowNode) {
        ...
        if (action === 'open') {
            ...
            let flows = [];
            try {
                flows = JSON.parse(decodeURIComponent(flowNode.dataset.flowJson || '[]'));
            }
            catch {
                flows = [];
            }
    ...
    const locationNode = target.closest('[data-factory-warehouse-location-action]');
    if (locationNode) {
...
export function closeFactoryWarehouseSharedDialogs() {
    if (!isFactoryWarehouseFlowModalOpen())
        return false;
    removeFlowModal();
    return true;
}
