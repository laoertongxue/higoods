...
export function closeProcessWebStatusActionDialog() {
    if (typeof document === 'undefined')
        return;
    document.getElementById(DIALOG_ID)?.remove();
}
export function isQuantityField(label) {
...
export function getDefaultFieldValue(label, context) {
    if (label.includes('操作人') ||
        label.includes('交出人') ||
        label.includes('接收人') ||
        label.includes('质检人') ||
        label.includes('复检人') ||
    ...
    if (label.includes('时间'))
        return context.operatedAt;
    if (label.includes('花型号') || label.includes('花型版本'))
        return 'PAT-20260328-A';
    if (label.includes('调色结果'))
        return '调色通过';
    if (label.includes('打印机编号'))
        return 'PRN-01';
    if (label.includes('染缸号'))
        return 'VAT-01';
    if (label.includes('染缸容量'))
        return '600';
    if (label.includes('色号'))
        return 'CLR-001';
    if (label.includes('打样结果'))
        return '打样通过';
    if (label.includes('原料面料 SKU'))
        return 'FAB-DYE-001';
    if (label.includes('领料记录'))
        return '已完成领料记录';
    if (label.includes('关联菲票'))
        return '按绑定菲票';
    if (label.includes('差异类型'))
        return '数量差异';
    if (label.includes('原因'))
        return '现场数量差异';
    if (label === '单位')
        return context.qtyUnit;
    if (label.includes('卷数'))
        return '12';
    if (isQuantityField(label))
        return context.objectQty;
    return '';
}
function parseFields(value) {
    return (value || '').split('|').map((item) => item.trim()).filter(Boolean);
}
function resolveObjectQtyField(fields) {
    return Object.entries(fields).find(([field]) => isQuantityField(field)
        && !field.includes('包装数量')
        && !field.includes('染缸容量')
        && !field.includes('卷数')
        && field !== '单位') ?? Object.entries(fields).find(([field]) => isQuantityField(field) && field !== '单位');
}
export function openProcessWebStatusActionDialog(config) {
    if (typeof document === 'undefined')
        return;
...
export function collectProcessWebStatusActionFields(dialog, toast) {
    const fields = {};
    const inputs = Array.from(dialog.querySelectorAll('[data-process-web-status-action-field]'));
...
export function confirmProcessWebStatusAction(callbacks) {
    ...
    try {
        const result = executeProcessWebAction({
            sourceType: (dialog.dataset.sourceType || 'PRINT_WORK_ORDER'),
...
export function handleProcessWebStatusActionDialogEvent(target, callbacks) {
    const actionNode = target.closest('[data-process-web-status-action]');
    if (!actionNode)
        return null;
