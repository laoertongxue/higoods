...
function createPage(mode: Mode) {
  ...
  const semiColumns: StandardListColumn<Row>[] = [
    ...
    { key: 'requirement', title: '加工要求', width: 275, render: r => `<div class="text-xs space-y-1">${f('类型',escapeHtml(r.purchase.accessoryType))}${f('生产标准',escapeHtml(r.purchase.productionStandard))}${r.purchase.baseMaterialRecipe ? f('原料配方',`${escapeHtml(r.purchase.baseMaterialRecipe.materialSkuId)} · ${r.purchase.baseMaterialRecipe.quantityPerMeter} ${escapeHtml(r.purchase.baseMaterialRecipe.unit)}/米`)+f('配方说明',escapeHtml(r.purchase.baseMaterialRecipe.specification)) : '<span class="text-red-700">历史记录缺原料配方，禁止产出</span>'}${f('品质要求','按采购行规格与确认样')}${f('目标仓',escapeHtml(r.purchase.warehouse))}</div>` },
