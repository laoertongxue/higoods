...
function createPage(mode: Mode) {
  ...
  const allRows = (): Row[] => {
    ...
    return data.orders.filter((purchase) => purchase.status !== '待采购').map((purchase) => {
      ...
      const status = base?.cancelledAt || purchase.status === '已关闭' ? '已终止' : base?.changePending ? '变更待处理' : !base ? '待生成' : !base.acceptedAt ? '待确认接收' : base.producedMeters < base.plannedMeters ? '加工中' : received < dispatched ? '交出待实收' : dispatched < base.producedMeters ? '待交出' : '已收齐'
  ...
  const purchaseColumns: StandardListColumn<Row>[] = [
    ...
    { key: 'style', title: '款式', width: 210, required: true, render: r => `<div class="flex gap-2 items-center"><img class="h-12 w-12 rounded border object-cover" src="${escapeHtml(r.purchase.styleImageUrl || r.purchase.materialImageUrl)}" alt="${escapeHtml(r.purchase.styleName || r.purchase.styleCode)}"><div><div>${escapeHtml(r.purchase.styleName || '款式待补齐')}</div><div class="text-xs text-slate-500">${escapeHtml(r.purchase.styleCode || '款号待补齐')}</div></div></div>` },
    ...
    { key: 'factory', title: '供应商／承接工厂', width: 190, render: r => `<div>${escapeHtml(r.purchase.supplierName)}</div><div class="text-xs text-slate-500">TMF - 辅料厂</div>` },
  ...
  const semiColumns: StandardListColumn<Row>[] = [
    ...
    { key: 'input', title: '加工投入／上游', width: 260, required: true, render: r => `<div class="text-xs space-y-1">${f('上游单据',escapeHtml(r.purchase.purchaseOrderNo))}${f('供应方',escapeHtml(r.purchase.supplierName))}${f('基础原料','按原料收发账实物确认')}${f('目标半成品',escapeHtml(r.purchase.materialCode))}</div>` },
    { key: 'requirement', title: '加工要求', width: 245, render: r => `<div class="text-xs space-y-1">${f('类型',escapeHtml(r.purchase.accessoryType))}${f('生产标准',escapeHtml(r.purchase.productionStandard))}${f('品质要求','按采购行规格与确认样')}${f('目标仓',escapeHtml(r.purchase.warehouse))}</div>` },
    ...
    { key: 'time', title: '时间', width: 230, sortable: true, sortValue: r => r.base?.dueDate ?? '', render: r => `<div class="text-xs space-y-1">${f('采购下达',escapeHtml(r.purchase.orderDate))}${f('计划完成',escapeHtml(r.base?.dueDate ?? r.purchase.expectedArrivalDate))}${f('确认接收',escapeHtml(r.base?.acceptedAt ?? '尚未确认'))}${f('加工完成',escapeHtml(r.base?.completedAt ?? '尚未完成'))}</div>` },
    ...
    { key: 'actions', title: '操作', width: 170, required: true, actionColumn: true, render: r => `<div class="flex flex-col gap-1 items-start">${!r.base?.acceptedAt ? action('accept','确认接收',r.purchase.purchaseOrderNo) : ''}${r.base?.acceptedAt && !r.base.cancelledAt ? action('report','加工填报',r.purchase.purchaseOrderNo) : ''}${r.base && !r.base.changePending && !r.base.cancelledAt && r.base.producedMeters > r.dispatched ? action('dispatch','发起交出',r.purchase.purchaseOrderNo) : ''}${action('detail','查看详情',r.purchase.purchaseOrderNo)}</div>` },
  ]
  ...
  const openDialog = (name: string, id: string, issueId = '') => {
    ...
    const headers: Record<string, string> = { 'purchase-return':'交出未用采购退货','purchase-return-receive':'TMF登记退货实收','purchase-resolve':'核对采购变更处置', 'raw-issue':'基础原料实际发出','raw-return-receive':'原料退回实收',raw:'半成品加工原料账', 'raw-receive':'基础原料实际接收', 'raw-consume':'基础原料实际耗用', 'raw-return':'未用原料交回', detail: '采购及半成品加工详情', generate: '生成半成品加工单', accept: '确认接收', report: '加工填报', dispatch: '发起交出', receive: '登记本次实际收货' }
  ...
  return { render: () => {
    ...
    return `<div data-tmf-base-page="${mode}">${renderStandardListPage({ title, primaryActionsHtml: `<div class="flex gap-2 items-center"><span class="text-xs text-slate-500">${receiving || rawWarehouse ? '原料／辅料仓 · 仓管演示身份' : 'TMF - 辅料厂 · 主管演示身份'}</span>${purchasePage ? '<button class="rounded border px-3 py-1.5 text-sm" data-nav="/pms/material-purchase-orders">PMS 面辅料采购</button>' : ''}</div>`,
      ...
      filtersHtml: `<div class="rounded-lg border bg-white p-3"><div class="flex gap-3 flex-wrap"><label class="text-xs">采购单 / 物料 / ${purchasePage ? '款式 / 半成品加工单' : '半成品加工单 / 批次'}<input name="keyword" value="${escapeHtml(state.keyword)}" class="block rounded border p-2 mt-1 w-72 text-sm"></label><label class="text-xs">进度<select name="status" class="block rounded border p-2 mt-1 text-sm"><option value="">全部</option>${(receiving ? ['待实收','部分实收','已收齐'] : ['待生成','待确认接收','加工中','待交出','交出待实收','已收齐','变更待处理','已终止']).map((s) => `<option ${s === state.status ? 'selected' : ''}>${s}</option>`).join('')}</select></label></div><div class="mt-3 flex gap-2">${action('query','查询')}${action('reset','重置')}${action('export','导出')}</div></div>`,
