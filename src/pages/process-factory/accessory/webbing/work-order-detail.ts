...
import { getTmfDefectiveBalance, scrapTmfDefectiveOutput, allocateTmfPrintHandover, receiveTmfPrintMaterial, getTmfPurchaseState, reloadTmfPurchaseRuntime, receiveTmfMergedProcessingMaterial, reportTmfMergedCutOutput, receiveTmfProcessingMaterial, reportTmfCutOutput, packTmfOutput, dispatchTmfOutputPackage, receiveTmfTipMaterial, reportTmfTipping, dispatchTmfContinuousReturn, getTmfTipMaterialBalance, dispatchTmfTipMaterialReturn } from '../../../../data/pms/tmf-material-purchases.ts'
...
async function loadUpstream(){
 ...
 try{
  ...
  const continuation=(dye?.matchedOrderId&&print?.matchedOrderId?action('link-print','按路线确认染色→印花接收',JSON.stringify([dye.matchedOrderId,print.matchedOrderId])):'')+(printToCut?action('link-tmf','按路线确认印花→织带厂接收',JSON.stringify([printToCut.matchedOrderId,order.routeEntryId])):'')
  surface.innerHTML=rows.length?`<p class="text-sm">按采用版本、BOM和路线核对既有加工单。计划量不是实际产出；来源关联不代表已交出或本厂已收到。</p>${continuation}${rows.map(r=>`<article class="rounded border bg-white p-4"><h3 class="font-medium">${e(r.processName)} · ${e(r.state)}</h3><p class="text-sm break-all">${e(r.inputSku)} → ${e(r.outputSku)}</p><p class="text-xs text-amber-700">印染物料对应实图待补</p>${r.rows.map(o=>`<div class="mt-3 border-t pt-2"><p>${e(o.no)} · ${e(o.factoryName||'待分配工厂')} · 计划 ${o.plannedQty} ${e(o.unit)}</p><p class="text-amber-700 text-sm">${e(o.problems.join('；'))}</p>${o.downstreamFactoryName?`<p class="text-sm">后续接收：${e(o.downstreamFactoryName)}${o.downstreamEntryId?` · 截断节点 ${e(o.downstreamEntryId)}`:''}；实收另行核对</p>`:''}<button data-nav="${e(o.route)}" class="border rounded px-3 py-2 text-sm">查看${e(r.processName)}加工单</button></div>`).join('')||'<p class="mt-2 text-sm text-amber-700">未找到对应来源，请核对生产单的印染加工单生成结果。</p>'}</article>`).join('')}`:'<p class="border rounded bg-white p-4">本单采用路线无需染色或印花，可按截断要求备料。</p>'
  const data=getTmfPurchaseState()
  const returns=rows.flatMap(r=>r.rows).filter(o=>o.downstreamEntryId===order.routeEntryId).flatMap(o=>printing.getPrintOrderHandoverRecords(o.id).filter(h=>h.handoverRecordStatus!=='VOIDED'&&h.factorySubmittedAt&&(h.submittedQty??0)>0).map(h=>({orderId:o.id,no:o.no,h,id:h.handoverRecordId||h.recordId})))
  surface.insertAdjacentHTML('beforeend',returns.map(r=>{const allocated=data.processingIssues.filter(i=>i.printHandover?.recordId===r.id).reduce((n,i)=>n+i.dispatchedMeters,0);const available=Math.round(((r.h.submittedQty??0)-allocated)*1000)/1000;return `<article class="border rounded bg-white p-4 mt-3"><h3 class="font-medium">印花实际交出 · ${e(r.id)}</h3><p>${e(r.no)} · ${e(r.h.skuCode||'')} · 交出 ${r.h.submittedQty??0} 米；已分配 ${allocated} 米；剩余可分配 ${available} 米</p><p>原单实收 ${r.h.receiverWrittenQty??r.h.warehouseWrittenQty??0} 米；按需求实收后才可截断。</p>${available>0?action('allocate-print','按需求分配投入',JSON.stringify([r.orderId,r.id])):''}</article>`}).join(''))
...
function open(name:string,id:string){
 ...
 if(name==='receive-tip'||name==='return-tip'){
  ...
  content=`<p>${e(m.materialSkuId)} · 来源发料 ${e(m.sourceDocumentNo)}</p><p class="text-xs">${m.stockLotId?`来源实收批次 ${e(m.stockLotId)} · ${e(data.tipMaterialLots.find(l=>l.id===m.stockLotId)?.warehouseId??'来源仓缺失')}`:'历史发料缺来源批次，未补造库存'}</p><p>上游已发 ${m.dispatchedQty}，本厂已收 ${m.receivedQty} ${m.unit}</p><p class="text-amber-700 text-xs">该辅材实物图待补</p>`+input('sku','扫描或输入实际辅材 SKU')+input('quantity',`本次实际${name==='return-tip'?'交回':'收到'}（${m.unit}）`,'number')
 ...
 const surface=root()!.querySelector('[data-tmf-detail-dialog]')!;surface.innerHTML=renderDialog({title:({scrap:'不良产出报废确认','allocate-print':'印花交出分配加工投入','merged-receive':'合并批投入实收','merged-cut':'合并批截断填报','return-tip':'未用辅材交回',return:'连续余料交回','receive-tip':'端头辅材实收',tip:'实际打头填报',receive:'投入实收',cut:'截断产出填报',pack:'合格产出装包',dispatch:'交回辅料仓'} as Record<string,string>)[name],width:'lg',closeAction:{prefix,action:'close',skipPageRerender:true}},`<div class="max-h-[60vh] overflow-y-auto">${content}</div><p role="alert" data-tmf-detail-error class="text-red-700 text-sm mt-2"></p>`,action('close','关闭')+action('confirm','确认'));hydrateIcons(surface);surface.setAttribute('tabindex','-1');(surface as HTMLElement).focus({preventScroll:true})
}
 ...
 el.addEventListener('click',async event=>{
  ...
  try{
   if(name==='tab'){tab=button.dataset.id!;el.querySelectorAll(`[data-${prefix}-action="tab"]`).forEach(b=>b.classList.toggle('bg-blue-50',b===button));refresh()}
    ...
    const surface=el.querySelector('[data-tmf-upstream-content]')
    if(surface?.isConnected){const result=document.createElement('p');result.setAttribute('role','status');result.textContent='已按采用路线确认交给 TMF 辅料厂。实际交出及投入实收仍须分别登记。';surface.prepend(result)}
   }
    ...
    if(currentAction==='allocate-print'){
     ...
     await allocateTmfPrintHandover({orderId,recordId,lines:lines.filter(l=>l.meters>0)},actor,operationId)
    }
   ...
   }else if(name&&['scrap','allocate-print','merged-receive','merged-cut','receive','cut','pack','dispatch','receive-tip','tip','return','return-tip'].includes(name))open(name,button.dataset.id||'')
