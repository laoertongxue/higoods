...
import { getTmfPurchaseState, getTmfProductionDisposition, getTmfOutputPackageBalance, getTmfTerminationReview, changeTmfProductionControl, type TmfProductionControl } from '../../../../data/pms/tmf-material-purchases.ts'
import { openTmfTerminationDispositionDialog } from './termination-disposition-dialog.ts'
...
export function openTmfProductionControlDialog(surface:HTMLElement,productionOrderId:string,onSaved:()=>void):void {
 ...
 surface.onclick=event=>{
  ...
  if(action==='termination'){openTmfTerminationDispositionDialog(surface,productionOrderId,()=>{surface.replaceChildren();onSaved()});return}
  if(action==='save')try{
