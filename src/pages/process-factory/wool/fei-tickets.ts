...
interface WoolFeiTicketRow {
  order: WoolWorkOrder
  panel: WoolPartPanel
}

function listRows(): WoolFeiTicketRow[] {
  return listWoolWorkOrders()
    .filter((order) => order.kind === 'PART_PANEL')
    .flatMap((order) => order.partPanels.map((panel) => ({ order, panel })))
}

function renderSummary(rows: WoolFeiTicketRow[]): string {
...
function renderRows(rows: WoolFeiTicketRow[]): string {
  return rows
    .map(({ order, panel }) => {
...
export function renderCraftWoolFeiTicketsPage(): string {
  const rows = listRows()
