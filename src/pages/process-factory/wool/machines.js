...
import { listWoolMachines, } from '../../../data/fcs/wool-task-domain.ts';
import { buildWoolMachineScheduleLink, buildWoolMachinesLink, buildWoolWorkOrdersLink, } from '../../../data/fcs/fcs-route-links.ts';
import { paginateWoolItems, renderBadge, renderMetricCard, renderPageHeader, renderPaginationControls, renderSection, renderTable, } from './shared';
function renderMachineStatusBadge(status) {
    const tone = status === '生产中'
        ? 'info'
        : status === '已排产'
            ? 'warning'
            : status === '空闲'
...
function buildMachineDialogLink(dialog, machineNo) {
    const params = new URLSearchParams();
    params.set('dialog', dialog);
    if (machineNo)
        params.set('machineNo', machineNo);
...
function renderSummaryCards(machines) {
    return `
...
export function renderCraftWoolMachinesPage() {
    ...
    <div class="space-y-4 p-4">
      ...
      ${renderFilters()}
      ${renderSection('横机设备清单', renderTable(['横机编号', '设备名称', '机台组', '针型', '适用任务', '状态', '当前任务', '日产能', '位置', '负责人', '备注', '操作'], renderMachineRows(paging.rows), 'min-w-[1700px]') + renderPaginationControls(paging, '台横机'))}
      ${renderMachineDialog(machines)}
    </div>
  `;
}
export function handleCraftWoolMachinesEvent(target) {
    const actionNode = target.closest('[data-wool-machine-action]');
    if (!actionNode)
        return false;
    const action = actionNode.dataset.woolMachineAction;
    const machineNo = actionNode.dataset.machineNo;
    if (action === 'create')
        window.alert('原型动作：新增横机设备');
    if (action === 'edit')
        window.alert(`原型动作：编辑横机 ${machineNo || ''}`);
    if (action === 'maintenance')
        window.alert(`原型动作：登记维修 ${machineNo || ''}`);
    if (action === 'filter')
        window.alert('原型动作：按当前条件筛选横机');
    if (action === 'reset')
        window.alert('原型动作：重置筛选条件');
    return true;
}
