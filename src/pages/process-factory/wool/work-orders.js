...
import { WOOL_STATUS_LABEL, acceptWoolWorkOrder, completeWoolPickupHead, confirmWoolHandoverReceipt, confirmWoolWaitProcessScanReceipt, confirmWoolPickupRecord, deleteWoolWarehouseLocation, getWoolWorkOrderSummary, getWoolAllowedActions, getWoolYarnUsageSummary, listWoolWarehouseAreas, listWoolWarehouseInventory, listWoolWarehouseLocations, listWoolMobileProcessTasks, listWoolWaitProcessScanReceipts, listWoolWorkOrders, lookupWoolWaitProcessScanReceipt, markWoolFeiTicketsPrinted, recordWoolYarnRecovery, recoverWoolYarnToWaitProcessWarehouse, scheduleWoolMachines, submitWoolHandover, upsertWoolWarehouseArea, upsertWoolWarehouseLocation, updateWoolWorkOrderNodeStatus, } from '../../../data/fcs/wool-task-domain.ts';
...
import { formatNumber, formatQty, paginateWoolItems, renderBadge, renderPaginationControls, renderKindBadge, renderMetricCard, renderPageHeader, renderSection, renderStatusBadge, } from './shared';
...
function getOrderExecutionTask(order) {
    return listWoolMobileProcessTasks().find((task) => task.taskId === order.taskNo);
}
function getStartStatus(order) {
    const task = getOrderExecutionTask(order);
    if (!task)
        return { label: '未同步', tone: 'muted', helper: '移动端任务未生成' };
    const rule = getTaskStartRuleState(task);
    const due = getTaskStartDueInfo(task);
    if (!rule.required)
        return { label: '不要求开工', tone: 'muted', helper: rule.proofRequirementLabel };
    if (task.startedAt)
        return { label: '已开工', tone: 'success', helper: task.startedAt };
    const tone = due.startRiskStatus === 'OVERDUE' ? 'danger' : due.startRiskStatus === 'DUE_SOON' ? 'warning' : 'info';
    return {
...
function getMilestoneStatus(order) {
    const task = getOrderExecutionTask(order);
    if (!task)
        return { label: '未同步', tone: 'muted', helper: '移动端任务未生成' };
    const milestone = getTaskMilestoneState(task);
    if (!milestone.required)
        return { label: '不要求节点', tone: 'muted', helper: milestone.proofRequirementLabel };
...
function renderExecutionReportCell(order) {
    const task = getOrderExecutionTask(order);
...
function listFilteredWoolWorkOrders(filters = getCurrentFilters()) {
    return listWoolWorkOrders().filter((order) => {
        if (!matchesKeyword(order.productionOrderNo, filters.productionOrder.trim()))
            return false;
        if (!matchesKeyword(`${order.woolOrderNo} ${order.woolOrderId}`, filters.woolOrder.trim()))
            return false;
        if (!matchesKeyword(`${order.styleNo} ${order.styleName}`, filters.style.trim()))
            return false;
        if (filters.kind && order.kind !== filters.kind)
            return false;
        if (filters.factory && order.factoryId !== filters.factory)
            return false;
...
function renderOrdersTable(filters) {
...
export function renderCraftWoolWorkOrdersPage() {
...
function readFilterField(field) {
    const node = document.querySelector(`[data-wool-filter-field="${field}"]`);
    return node?.value.trim() || '';
}
function navigateWorkOrderFilters() {
...
function promptKgValue(label, currentValue = 0, allowZero = true) {
...
function renderWarehouseStatusOptions(currentStatus = '启用') {
    return ['启用', '停用']
        .map((status) => `<option value="${status}" ${status === currentStatus ? 'selected' : ''}>${status}</option>`)
        .join('');
}
function readWarehouseFormField(modal, field) {
    return modal.querySelector(`[data-warehouse-form-field="${field}"]`)?.value.trim() || '';
}
function openWoolWarehouseAreaDialog(input) {
  ...
  `);
    ...
    modal.addEventListener('click', (event) => {
        const action = event.target.closest('[data-warehouse-form-action]')?.dataset.warehouseFormAction;
        if (!action)
            return;
        if (action === 'close') {
            removeWoolWarehouseFormDialog();
        ...
        if (action === 'submit-area') {
            ...
            upsertWoolWarehouseArea({
                ...
                status: readWarehouseFormField(modal, 'status') || '启用',
...
function renderWoolReceiptAreaOptions(selectedAreaId = '') {
...
function renderWoolReceiptDetails(receipt) {
    if (!receipt) {
        return `
...
function loadWoolReceiptDetails(modal) {
    const keyword = modal.querySelector('[data-wool-receipt-field="receiptNo"]')?.value.trim() || '';
    const details = modal.querySelector('[data-wool-receipt-details]');
    if (!details)
        return;
    const receipt = lookupWoolWaitProcessScanReceipt(keyword);
    details.innerHTML = renderWoolReceiptDetails(receipt);
    if (!receipt)
        window.alert('未找到该毛织领料单或送料二维码。');
}
function syncWoolReceiptLocationSelect(row) {
    const areaId = row.querySelector('[data-wool-receipt-field="areaId"]')?.value || '';
    const locationSelect = row.querySelector('[data-wool-receipt-field="locationId"]');
    if (!locationSelect)
        return;
    locationSelect.innerHTML = renderWoolReceiptLocationOptions(areaId);
}
function getWoolReceiptEvidenceType(modal) {
    const selected = modal.querySelector('[data-wool-receipt-field="evidenceType"]:checked')?.value;
    return selected === 'video' ? 'video' : 'image';
}
function syncWoolReceiptEvidenceFileName(modal) {
    const evidenceType = getWoolReceiptEvidenceType(modal);
    const fileInput = modal.querySelector(`[data-wool-receipt-field="${evidenceType === 'image' ? 'imageFile' : 'videoFile'}"]`);
    const fileNameNode = modal.querySelector('[data-wool-receipt-evidence-name]');
    if (!fileNameNode)
        return;
...
function syncWoolReceiptEvidencePanels(modal) {
    const evidenceType = getWoolReceiptEvidenceType(modal);
    modal.querySelectorAll('[data-wool-receipt-evidence-label]').forEach((label) => {
    ...
    modal.querySelectorAll('[data-wool-receipt-evidence-panel]').forEach((panel) => {
        panel.classList.toggle('hidden', panel.dataset.woolReceiptEvidencePanel !== evidenceType);
    });
    const inactiveInput = modal.querySelector(`[data-wool-receipt-field="${evidenceType === 'image' ? 'videoFile' : 'imageFile'}"]`);
    if (inactiveInput)
        inactiveInput.value = '';
    syncWoolReceiptEvidenceFileName(modal);
}
function readWoolReceiptEvidenceText(modal) {
    const evidenceType = getWoolReceiptEvidenceType(modal);
    const fileInput = modal.querySelector(`[data-wool-receipt-field="${evidenceType === 'image' ? 'imageFile' : 'videoFile'}"]`);
...
function openWoolYarnReceiptDialog() {
  ...
  `);
    ...
    modal.addEventListener('change', (event) => {
        const field = event.target.closest('[data-wool-receipt-field]')?.dataset.woolReceiptField;
        ...
        if (field !== 'areaId')
            return;
        const row = event.target.closest('[data-wool-receipt-line-id]');
        if (row)
            syncWoolReceiptLocationSelect(row);
    });
    modal.addEventListener('click', (event) => {
        const actionNode = event.target.closest('[data-wool-receipt-action]');
        ...
        if (action === 'fill-receipt') {
            const input = modal.querySelector('[data-wool-receipt-field="receiptNo"]');
            if (input)
                input.value = actionNode.dataset.receiptNo || '';
        ...
        if (action === 'submit') {
            const receiptNo = modal.querySelector('[data-wool-receipt-field="receiptNo"]')?.value.trim() || '';
            const evidenceText = readWoolReceiptEvidenceText(modal);
            if (!evidenceText) {
                window.alert('请上传称重照片或到货视频，二选一。');
                return;
            }
            const rows = Array.from(modal.querySelectorAll('[data-wool-receipt-line-id]'));
            const lines = rows.map((row) => ({
                receiptLineId: row.dataset.woolReceiptLineId || '',
                actualWeightKg: Number(row.querySelector('[data-wool-receipt-field="actualWeightKg"]')?.value || 0),
                areaId: row.querySelector('[data-wool-receipt-field="areaId"]')?.value || '',
                locationId: row.querySelector('[data-wool-receipt-field="locationId"]')?.value || '',
                evidenceText,
            }));
            try {
...
function openWoolYarnRecoveryDialog() {
  ...
  `);
    ...
    if (!modal)
        return;
    const yarnSelect = modal.querySelector('[data-recovery-field="yarnSku"]');
    const syncRows = () => {
        const yarnSku = yarnSelect?.value || '';
        modal.querySelectorAll('[data-recovery-order-row]').forEach((row) => {
            const matched = row.dataset.yarnSku === yarnSku;
            row.classList.toggle('hidden', !matched);
            if (!matched) {
                const checkbox = row.querySelector('[data-recovery-order-checkbox]');
                if (checkbox)
    ...
    syncRows();
    modal.addEventListener('click', (event) => {
        const actionNode = event.target.closest('[data-recovery-modal-action]');
        ...
        if (action === 'submit') {
            const yarnSku = yarnSelect?.value || '';
            const qtyInput = modal.querySelector('[data-recovery-field="qty"]');
            ...
            const associationOrderIds = Array.from(modal.querySelectorAll('[data-recovery-order-checkbox]:checked'))
...
export async function handleCraftWoolEvent(target) {
    const workflowNode = target.closest('[data-wool-workflow-action]');
    if (workflowNode?.dataset.woolOrderId && workflowNode.dataset.woolWorkflowAction) {
        ...
        if (action === 'ACCEPT')
            acceptWoolWorkOrder(orderId, 'Web端操作员');
        if (action === 'CONFIRM_PICKUP')
            confirmWoolPickupRecord(orderId, 'Web端操作员');
        if (action === 'COMPLETE_PICKUP')
            completeWoolPickupHead(orderId, 'Web端操作员');
        if (action === 'SCHEDULE_MACHINE')
            scheduleWoolMachines(orderId, 'Web端排产员');
        ...
        if (action === 'REPORT_FLAT_MILESTONE')
            updateWoolWorkOrderNodeStatus(orderId, '横机成片', '进行中', 'Web端操作员');
        if (action === 'COMPLETE_FLAT')
            updateWoolWorkOrderNodeStatus(orderId, '横机成片', '已完成', 'Web端操作员');
        if (action === 'START_LINKING')
            updateWoolWorkOrderNodeStatus(orderId, '缝盘', '进行中', 'Web端操作员');
        ...
        if (action === 'START_IRONING')
            updateWoolWorkOrderNodeStatus(orderId, '熨烫', '进行中', 'Web端操作员');
        if (action === 'COMPLETE_IRONING')
            updateWoolWorkOrderNodeStatus(orderId, '熨烫', '已完成', 'Web端操作员');
        if (action === 'START_PACKING')
            updateWoolWorkOrderNodeStatus(orderId, '包装', '进行中', 'Web端操作员');
        if (action === 'COMPLETE_PACKING')
            updateWoolWorkOrderNodeStatus(orderId, '包装', '已完成', 'Web端操作员');
        if (action === 'SKIP_PACKING')
            updateWoolWorkOrderNodeStatus(orderId, '包装', '已跳过', 'Web端操作员');
        if (action === 'PRINT_FEI_TICKET')
            markWoolFeiTicketsPrinted(orderId, 'Web端操作员');
        if (action === 'SUBMIT_HANDOVER')
            submitWoolHandover(orderId, 'Web端操作员');
        if (action === 'CONFIRM_HANDOVER_RECEIPT')
            confirmWoolHandoverReceipt(orderId, 'Web端仓库');
        return true;
    }
    const actionNode = target.closest('[data-wool-action]');
    if (!actionNode)
        return false;
    ...
    if ((action === 'node-start' || action === 'node-complete' || action === 'node-skip') && actionNode.dataset.woolOrderId && actionNode.dataset.nodeName) {
        const nextStatus = action === 'node-start' ? '进行中' : action === 'node-complete' ? '已完成' : '已跳过';
    ...
    if (action === 'edit-area') {
        ...
        openWoolWarehouseAreaDialog({
            ...
            status: actionNode.dataset.status || '启用',
