...
import { DEFAULT_PAGE_SIZE_OPTIONS, paginateRows, parsePageSize, } from '../../../utils/paging.ts';
import { getWoolWorkOrderKindLabel, getWoolWorkOrderStatusLabel, } from '../../../data/fcs/wool-task-domain.ts';
...
export function renderTable(headers, rows, minWidthClass = 'min-w-[1180px]') {
    return `
...
function buildPaginationHref(pageParam, page, pageSizeParam, pageSize) {
...
export function paginateWoolItems(items, pageParam, defaultPageSize = 10, pageSizeParam = `${pageParam}Size`) {
...
export function renderPaginationControls(paging, label = '条记录') {
    const pages = new Set([
        1,
        paging.totalPages,
        paging.currentPage,
...
export function renderPaginatedTable(headers, items, renderRow, minWidthClass, pageParam, label = '条记录', defaultPageSize = 10) {
