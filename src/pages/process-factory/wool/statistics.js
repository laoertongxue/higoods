...
import { listWoolMachineSchedules, listWoolWorkOrders, } from '../../../data/fcs/wool-task-domain.ts';
import { buildWoolMachineScheduleLink, buildWoolWorkOrderDetailLink, buildWoolWorkOrdersLink, } from '../../../data/fcs/fcs-route-links.ts';
import { formatNumber, formatQty, renderBadge, renderKindBadge, renderMetricCard, renderPageHeader, renderPaginatedTable, renderSection, renderStatusBadge, } from './shared';
function getNode(order, nodeName) {
    return order.nodes.find((node) => node.nodeName === nodeName);
}
function renderNodeBadge(node, fallback = '无节点') {
    if (!node)
        return renderBadge(fallback, 'muted');
    const tone = node.status === '已完成' ? 'success' : node.status === '进行中' ? 'info' : node.status === '已跳过' ? 'muted' : 'warning';
    return renderBadge(node.status, tone);
}
function renderNodeCell(order, nodeName) {
...
function renderSummaryCards() {
    ...
    const nodeCount = (nodeName, status) => wholeOrders.concat(partOrders).filter((order) => {
        const node = getNode(order, nodeName);
...
function renderWholeTable() {
    ...
    return renderPaginatedTable(['毛织单号', '任务类型', '款式', '横机成片', '缝盘', '熨烫', '包装', '交出对象', '当前状态', '操作'], orders, (order) => `
      ...
      `, 'min-w-[1560px]', 'statWholePage', '条整件毛织');
}
function renderPartTable() {
    const orders = listWoolWorkOrders().filter((order) => order.kind === 'PART_PANEL');
    return renderPaginatedTable(['毛织单号', '任务类型', '款式', '横机成片', '成片片数', '菲票行数', '交出对象', '当前状态', '操作'], orders, (order) => {
      ...
      `;
    }, 'min-w-[1360px]', 'statPartPage', '条部位毛织');
}
function renderRiskRows() {
    ...
    return renderSection('风险与待处理', renderPaginatedTable(['毛织单号', '任务类型', '款式', '风险类型', '说明', '交出对象', '操作'], rows, ({ order, risk }) => `
        <tr class="border-b align-top last:border-b-0">
          <td class="px-3 py-3 font-mono text-xs">${escapeHtml(order.woolOrderNo)}</td>
          <td class="px-3 py-3">${renderKindBadge(order.kind)}</td>
      ...
      `, 'min-w-[1280px]', 'statRiskPage', '条风险'));
}
export function renderCraftWoolStatisticsPage() {
    return `
    <div class="space-y-4 p-4">
      ${renderPageHeader('毛织统计', '围绕周哥毛织厂的整件节点、部位菲票、交出分流和差异待处理做演示统计。', renderTopActions())}
      ${renderSummaryCards()}
      ${renderSection('整件毛织节点进度', `
