...
import { WOOL_KIND_LABEL, listWoolWarehouseAreas, listWoolWaitHandoverHandoutRecords, listWoolWaitHandoverInboundRecords, listWoolWaitProcessReceiptRecords, listWoolWaitProcessUsageRecords, listWoolWarehouseInventory, listWoolWarehouseLocations, } from '../../../data/fcs/wool-task-domain.ts';
import { buildWoolWorkOrderDetailLink, } from '../../../data/fcs/fcs-route-links.ts';
import { formatNumber, formatQty, renderBadge, renderMetricCard, renderPageHeader, renderPaginatedTable, renderSection, renderTable, } from './shared';
import { renderWarehouseFlowButton } from '../shared/warehouse-standard.ts';
function getMode() {
    return appStore.getState().pathname.includes('wait-handover') ? 'wait-handover' : 'wait-process';
}
function getCurrentTab(mode) {
    const [, queryString = ''] = (appStore.getState().pathname || '').split('?');
    const requested = new URLSearchParams(queryString).get('tab');
...
function buildWarehouseLink(mode, overrides) {
...
function renderTabs(mode, activeTab) {
...
function renderInventoryDetailButton(mode, item) {
...
function renderInventoryDetailDialog(mode, items) {
...
function renderInventoryTab(mode) {
    ...
    const table = renderPaginatedTable(headers, items, (item) => {
        const flowLines = item.flowRecords.map((flow) => ({
        ...
        `;
    }, mode === 'wait-process' ? 'min-w-[900px]' : 'min-w-[1180px]', mode === 'wait-process' ? 'waitProcessInventoryPage' : 'waitHandoverInventoryPage', '条库存');
...
function renderReceiptsTab() {
    ...
    return renderSection('领料记录', renderPaginatedTable(['领料单', '毛织单号', '生产单', '来源', '纱线 SKU', '应收重量', '实收重量', '差异', '库区库位', '照片视频', '确认时间', '状态'], records, (record) => `
      <tr class="border-b last:border-b-0">
      ...
      `, 'min-w-[1620px]', 'waitProcessReceiptsPage', '条领料记录'));
}
function renderUsageTab() {
    const records = listWoolWaitProcessUsageRecords();
    return renderSection('加工用料记录', renderPaginatedTable(['用料单', '记录类型', '毛织单号', '生产单', '纱线 SKU', '重量', '对应节点', '操作人', '操作时间', '状态'], records, (record) => `
      <tr class="border-b last:border-b-0">
      ...
      `, 'min-w-[1320px]', 'waitProcessUsagePage', '条用料记录'));
}
function renderHandoutsTab() {
    const records = listWoolWaitHandoverHandoutRecords();
    return renderSection('交出记录', renderPaginatedTable(['交出单', '毛织单号', '生产单', '交出对象', '交出数量', '回写数量', '交出时间', '状态'], records, (record) => `
      <tr class="border-b last:border-b-0">
      ...
      `, 'min-w-[1100px]', 'waitHandoverHandoutsPage', '条交出记录'));
}
function renderInboundsTab() {
    const records = listWoolWaitHandoverInboundRecords();
    return renderSection('加工入仓记录', renderPaginatedTable(['入仓单', '毛织单号', '生产单', '类型', '入仓对象', '入仓数量', '操作人', '入仓时间', '状态'], records, (record) => `
      <tr class="border-b last:border-b-0">
      ...
      `, 'min-w-[1200px]', 'waitHandoverInboundsPage', '条入仓记录'));
}
function renderLocationsTab(mode) {
    const areas = listWoolWarehouseAreas(mode);
    const locations = listWoolWarehouseLocations(mode);
    const areaSection = renderSection('库区管理', renderPaginatedTable(['库区编号', '库区名称', '负责人', '状态', '更新时间', '备注', '操作'], areas, (area) => `
      <tr class="border-b last:border-b-0">
      ...
      `, 'min-w-[980px]', mode === 'wait-process' ? 'waitProcessAreasPage' : 'waitHandoverAreasPage', '个库区'), `<button type="button" class="rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700" data-wool-action="add-area" data-warehouse-mode="${escapeHtml(mode)}">新增库区</button>`);
    const locationSection = renderSection('库位管理', renderPaginatedTable(['库区', '库位', '负责人', '状态', '更新时间', '备注', '操作'], locations, (location) => `
      <tr class="border-b last:border-b-0">
      ...
      `, 'min-w-[1100px]', mode === 'wait-process' ? 'waitProcessLocationsPage' : 'waitHandoverLocationsPage', '个库位'), `<button type="button" class="rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700" data-wool-action="add-location" data-warehouse-mode="${escapeHtml(mode)}">新增库位</button>`);
    return `<div class="space-y-4">${areaSection}${locationSection}</div>`;
}
function renderActiveTab(mode, activeTab) {
    if (activeTab === 'receipts')
        return renderReceiptsTab();
    if (activeTab === 'usage')
        return renderUsageTab();
    if (activeTab === 'handouts')
        return renderHandoutsTab();
    if (activeTab === 'inbounds')
        return renderInboundsTab();
    if (activeTab === 'locations')
        return renderLocationsTab(mode);
    return renderInventoryTab(mode);
}
export function renderCraftWoolWaitProcessWarehousePage() {
    return renderWoolWarehousePage('wait-process');
}
export function renderCraftWoolWaitHandoverWarehousePage() {
    return renderWoolWarehousePage('wait-handover');
}
function renderWoolWarehousePage(mode = getMode()) {
    const title = mode === 'wait-handover' ? '毛织待交出仓' : '毛织待加工仓';
    const subtitle = mode === 'wait-handover'
        ? '库存由加工入仓形成，交出给后道工厂或裁床待交出仓后扣减。'
        : '库存对象为纱线，来源于染厂、印花厂或面辅料仓送料入仓；开工领用、缝盘损耗扣减，损耗纱线回收后回收入仓。';
    const activeTab = getCurrentTab(mode);
    return `
    <div class="space-y-4 p-4">
      ${renderPageHeader(title, subtitle, mode === 'wait-process'
