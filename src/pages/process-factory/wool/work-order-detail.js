...
import { buildWoolPartPanelFeiTicketSourceId, getWoolWorkOrderById, getWoolAllowedActions, getWoolYarnUsageSummary, getWoolWorkOrderKindLabel, getWoolWorkOrderStatusLabel, listWoolMobileProcessTasks, } from '../../../data/fcs/wool-task-domain.ts';
import { buildFeiTicketLabelPrintLink, buildWoolMachineScheduleLink, buildWoolMachinesLink, buildWoolWaitHandoverWarehouseLink, buildWoolWaitProcessWarehouseLink, buildWoolWorkOrderDetailLink, } from '../../../data/fcs/fcs-route-links.ts';
...
import { formatMoney, formatQty, renderBadge, renderField, renderKindBadge, renderPageHeader, renderPaginatedTable, renderSection, renderStatusBadge, } from './shared';
function getDetailTabs(order) {
    const tabs = [
        { key: 'base', label: '基本信息' },
        { key: 'yarn', label: '领料信息' },
    ...
    tabs.push({ key: 'handover', label: '送货交出' }, { key: 'price', label: '价格信息' }, { key: 'evidence', label: '异常证据' });
    return tabs;
}
function getCurrentTab(order) {
...
function renderDetailTabs(order, activeTab) {
    return `
...
function getStartStatusLabel(order) {
    if (order.status === 'WAIT_ACCEPT')
        return '未接单';
    if (order.status === 'WAIT_PICKUP')
        return '待领料';
...
function renderNodeActions(order, nodeName) {
    const actions = getWoolAllowedActions(order).filter((action) => action.nodeName === nodeName);
    if (!actions.length)
        return '<span class="text-xs text-muted-foreground">当前不可操作</span>';
    return `
...
function getMilestoneStatusLabel(order) {
    const task = getOrderExecutionTask(order);
    if (!task)
        return '移动端任务未同步';
...
function getMilestoneQtyText(order) {
    const task = getOrderExecutionTask(order);
    if (!task)
        return '未同步';
    const milestone = getTaskMilestoneState(task);
    if (!milestone.reportedQty)
        return '未上报';
    const unit = order.kind === 'PART_PANEL' ? '片' : '件';
    return formatQty(milestone.reportedQty, unit);
}
function renderDetailWorkflowActions(order) {
...
function renderExecutionReportSection(order) {
    const task = getOrderExecutionTask(order);
    if (!task) {
        return renderSection('开工与关键节点', '<div class="text-sm text-muted-foreground">当前毛织加工单尚未同步到工厂端移动任务。</div>');
    }
...
function renderYarnTab(order) {
    const receipt = order.yarnReceipt;
    const yarnUsage = getWoolYarnUsageSummary(order);
    return renderSection('领料信息', `
      <div class="grid gap-3 text-sm md:grid-cols-3">
...
function renderMachineTab(order) {
    const yarnUsage = getWoolYarnUsageSummary(order);
    const nodes = order.nodes.filter((node) => node.nodeName === '横机成片');
    return renderSection('横机成片', `
      ...
      ${renderPaginatedTable(['节点', '状态', '计划数量', '完成数量', '开工领用纱线', '横机编号', '操作人', '开始时间', '完成时间', 'Web操作'], nodes, (node) => `
        ...
        `, 'min-w-[1400px]', 'detailMachinePage', '条横机节点')}
    `);
}
function renderWholeTab(order) {
    const yarnUsage = getWoolYarnUsageSummary(order);
    const nodes = order.nodes.filter((node) => node.nodeName !== '横机成片');
    return renderSection('整件毛织节点', `
      ...
      ${renderPaginatedTable(['节点', '状态', '计划数量', '完成数量', '纱线损耗', '操作人', '开始时间', '完成时间', '备注', 'Web操作'], nodes, (node) => `
          <tr class="border-b last:border-b-0">
            <td class="px-3 py-3">${escapeHtml(node.nodeName)}</td>
        ...
        `, 'min-w-[1400px]', 'detailWholePage', '条整件节点')}
    `);
}
function renderFeiTab(order) {
    const panels = order.partPanels;
    return renderSection('部位毛织菲票', `
      ...
      ${renderPaginatedTable(['部位', '颜色', '尺码', '计划片数', '完成片数', '菲票号', '打印状态', '后续去向', '操作'], panels, (panel) => {
      ...
      `;
    }, 'min-w-[1280px]', 'detailFeiPage', '行菲票')}
    `);
}
function renderHandoverTab(order) {
    const handoverActions = getWoolAllowedActions(order).filter((action) => action.code === 'SUBMIT_HANDOVER' || action.code === 'CONFIRM_HANDOVER_RECEIPT');
    return renderSection('送货交出', `
      <div class="grid gap-3 text-sm md:grid-cols-3">
        ${renderField('交出对象', order.downstreamTarget)}
...
function renderPriceTab(order) {
    return renderSection('价格信息', `
      <div class="grid gap-3 text-sm md:grid-cols-3">
...
function renderEvidenceTab(order) {
    return renderSection('异常证据', renderPaginatedTable(['事项', '说明', '提交人', '提交时间'], order.evidenceItems, (item) => `
        <tr class="border-b last:border-b-0">
      ...
      `, 'min-w-[960px]', 'detailEvidencePage', '条证据'));
}
function renderActiveTab(order, activeTab) {
    if (activeTab === 'yarn')
        return renderYarnTab(order);
    if (activeTab === 'machine')
        return renderMachineTab(order);
    if (activeTab === 'whole' && order.kind === 'WHOLE_GARMENT')
        return renderWholeTab(order);
    if (activeTab === 'fei' && order.kind === 'PART_PANEL')
        return renderFeiTab(order);
    if (activeTab === 'handover')
        return renderHandoverTab(order);
    if (activeTab === 'price')
        return renderPriceTab(order);
    if (activeTab === 'evidence')
        return renderEvidenceTab(order);
    return renderBaseTab(order);
}
export function renderCraftWoolWorkOrderDetailPage(woolOrderId) {
    ...
    return `
    <div class="space-y-4 p-4">
      ${renderPageHeader('毛织加工单详情', `${order.woolOrderNo} / ${getWoolWorkOrderKindLabel(order.kind)} / ${order.downstreamTarget}`, `
          <div class="flex flex-wrap gap-2">
