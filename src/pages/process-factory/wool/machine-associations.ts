...
function isMaintainableOrder(order: WoolWorkOrder): boolean {
  return getWoolAllowedActions(order.woolOrderId).includes('ASSOCIATE_MACHINE')
}
...
export function buildWoolMachineAssociationWorkbenchModel(input: {
  ...
  return {
    ...
    orderOptions,
    selectedOrder,
    machines: listWoolMachineViews().map((machine) => {
...
function readinessSummary(order: WoolWorkOrder): string {
  const action = getWoolAllowedActions(order.woolOrderId).includes('REPORT_PROCESS')
  const currentMachines = listWoolMachineAssociations(order.woolOrderId).length
  return `${action ? '存在可继续填报 SKU' : '仅维护现有关联'}；当前 ${currentMachines} 台横机`
}
...
function associationDialog(): string {
  ...
  const orderOptions = model.orderOptions.map((order) =>
    `<option value="${escapeHtml(order.woolOrderId)}" ${selectedWoolOrderId === order.woolOrderId ? 'selected' : ''}>${escapeHtml(`${order.woolOrderNo}｜${order.kind === 'WHOLE_GARMENT' ? '整件毛织' : '部位毛织'}｜${readinessSummary(order)}`)}</option>`,
  ).join('')
...
function renderWorkspace(): string {
  ...
  return renderStandardListPage({
    title: '横机生产关联',
    primaryActionsHtml: `<div class="flex gap-2">${renderPrimaryButton('关联生产单', { prefix: EVENT_PREFIX, action: 'open-association' }, 'link')}${`<button type="button" class="h-9 rounded-md border px-4 text-sm" data-nav="${escapeHtml(buildWoolMachinesLink())}">横机设备</button>`}</div>`,
