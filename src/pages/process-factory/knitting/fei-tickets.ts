...
interface KnittingFeiTicketRow {
  order: KnittingWorkOrder
  panel: KnittingPartPanel
}

function listRows(): KnittingFeiTicketRow[] {
  return listKnittingWorkOrders()
    .filter((order) => order.kind === 'PART_PANEL')
    .flatMap((order) => order.partPanels.map((panel) => ({ order, panel })))
}

function renderSummary(rows: KnittingFeiTicketRow[]): string {
  ...
  return `
    <section class="grid gap-3 md:grid-cols-4">
      ${renderMetricCard('针织菲票行', String(rows.length), '仅部位针织')}
    ...
    <section class="grid gap-3 md:grid-cols-2">
      ...
      ${renderMetricCard('打印模板', '针织菲票标签', '复用统一菲票打印预览')}
      ${renderMetricCard('后续去向', '裁床待交出仓', '部位针织不进缝盘、熨烫、包装')}
    </section>
  `
}

function renderRows(rows: KnittingFeiTicketRow[]): string {
  return rows
    .map(({ order, panel }) => {
      const statusTone = panel.feiTicketStatus === '已打印' ? 'success' : 'warning'
      const detailHref = buildKnittingWorkOrderDetailLink(order.knittingOrderId, 'fei')
      const printSourceId = buildKnittingPartPanelFeiTicketSourceId(order, panel)
      return `
        <tr class="border-b align-top last:border-b-0">
          ...
          <td class="px-3 py-3">
            <div class="flex flex-wrap gap-2">
              ...
              <button type="button" class="rounded-md border px-2 py-1 text-xs hover:bg-muted" data-nav="${escapeHtml(buildKnittingWaitHandoverWarehouseLink(order.knittingOrderId))}">查看交出仓</button>
            </div>
          </td>
        </tr>
      `
