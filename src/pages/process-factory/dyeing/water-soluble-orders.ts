...
type Overlay =
  | { type: 'detail' | 'supervisor' | 'handover'; orderId: string }
...
function renderCard(order: WaterSolubleWorkOrder): string {
  ...
  return `<article class="rounded-xl border bg-card p-4 shadow-sm" data-testid="factory-water-soluble-card" data-order-id="${escapeHtml(order.waterOrderId)}">
    ...
    <div class="mt-4 space-y-2">${renderPrimaryAction(order)}<button data-skip-page-rerender="true" class="w-full text-center text-sm text-blue-600 hover:underline" data-factory-water-soluble-action="open-detail" data-order-id="${escapeHtml(order.waterOrderId)}">查看任务详情与记录</button></div>
  </article>`
}
