...
import * as dye from '../../../data/fcs/dyeing-task-domain.ts'
import * as water from '../../../data/fcs/water-soluble-output.ts'
import type { ProcessDispatchDocument as DyeDispatchDocument } from '../../../data/fcs/process-output-types.ts'
import type { OutputWorkOrderRow } from '../../../data/fcs/process-output-view.ts'
import { openWaterOutputBarcode, handleWaterOutputBarcode } from './water-output-barcode.ts'
import { getPdaSession } from '../../../data/fcs/store-domain-pda.ts'
const getDyeDispatchStatus = dye.getDyeDispatchStatus
const dyeApi={rolls:dye.getDyeOutputRolls,docs:dye.listDyeDispatchDocuments,create:dye.createDyeDispatchDocument,finish:dye.finishDyeDispatchDocument,available:dye.getDyeDispatchAvailableQty,scan:dye.scanDyeDispatchRoll,transport:dye.saveDyeDispatchTransport,isAvailable:dye.isDyeRollAvailable}
const waterApi={rolls:water.getWaterOutputRolls,docs:water.listWaterDispatchDocuments,create:water.createWaterDispatchDocument,finish:water.finishWaterDispatchDocument,available:water.getWaterDispatchAvailableQty,scan:water.scanWaterDispatchRoll,transport:water.saveWaterDispatchTransport,isAvailable:water.isWaterRollAvailable}
let process:'dye'|'water'='dye',api=dyeApi
...
type Item = { id: string; order?: OutputWorkOrderRow; doc?: DyeDispatchDocument }
...
function objectImage(url: string, name: string, code: string): string {
  if (!url.trim()) return `<div class="min-w-0 break-words"><div class="font-medium">${e(name)}</div><div class="mt-1 text-xs text-muted-foreground">${e(code)}</div><p class="mt-1 text-xs text-amber-700">缺对应实物图，请补齐资料</p></div>`
...
function info(id: string): OutputWorkOrderRow { const row = rows.find(row=>row.orderId===id); if (!row) throw new Error('原加工单不存在，请重新查询。'); return row }
function stateOf(row: OutputWorkOrderRow): string {
  if (row.blockedReason) return row.blockedReason
  const rolls = api.rolls(row.orderId).filter(roll=>!roll.dispatchId)
  if (rolls.some(roll=>roll.qty<=0)) return '卷长未维护'
  if (rolls.some(roll=>!roll.printedAt)) return '条码未全部打印'
  if (rolls.some(roll=>api.isAvailable(row.orderId,roll))) return '已维护并打印'
  return rolls.length ? '已加入交出单' : process==='water'&&api.available(row.orderId)>0?'尚未维护卷码':'已全部交出'
}
function allItems(): Item[] {
  return mode==='documents' ? api.docs().map(doc=>({id:doc.id,doc})) : rows.filter(row=>!row.isYarn && row.completedQty>0 && api.available(row.orderId)>0).map(order=>({id:order.orderId,order}))
}
...
function columns(): StandardListColumn<Item>[] {
  if(mode==='pending')return [
    ...
    {key:'actions',title:'操作',width:145,required:true,actionColumn:true,render:i=>`<div class="space-y-2">${btn('维护条码','barcodes',i.id)}${btn('合入已有单','join-row',i.id,false,!api.rolls(i.id).some(r=>api.isAvailable(i.id,r)))}</div>`},
  ]
  return [
    ...
    {key:'orders',title:'加工单 / 任务 / 接收方',width:290,freezeable:true,render:i=>i.doc!.lines.map(l=>`<div class="mb-2 border-b pb-2 last:border-0">${link(l.orderNo,orderRoute(l.orderId))}${line('任务',l.taskNo)}${line('接收方',l.receiver)}</div>`).join('')},
    {key:'quantity',title:'交出数量',width:210,sortable:true,sortValue:i=>dyeDispatchQuantities(i.doc!).rolls,render:i=>{const q=dyeDispatchQuantities(i.doc!);return line('总卷数',i.doc!.historical?'历史未登记卷码':`${q.rolls} 卷`)+line('原单数量',q.text)+line('SKU 数量',q.skus)+line('长度换算',q.hasLength?`${q.meters.toFixed(2)} 米`:'不适用（非长度单位）')}},
    {key:'time',title:'时间 / 运输',width:230,sortable:true,sortValue:i=>i.doc!.createdAt,render:i=>{const d=i.doc!;return line('建单',d.createdAt||'历史未登记建单时间')+line('交出',d.handedOverAt||'尚未交出')+line('司机',d.transport.driver||'尚未登记')+line('车牌',d.transport.plate||'尚未登记')}},
    {key:'status',title:'状态 / 扫卷进度',width:160,sortable:true,sortValue:i=>getDyeDispatchStatus(i.doc!),render:i=>badge(getDyeDispatchStatus(i.doc!),i.doc!.status==='已交出')+`<div class="mt-2">${i.doc!.historical?line('来源','历史通用交接'):line('已扫码',`${i.doc!.scans.length} / ${dyeDispatchQuantities(i.doc!).rolls} 卷`)}</div>`},
...
function listBody(): string {
  ...
  return renderStandardListPage({title:processLabel()+(mode==='pending'?'待交出列表':'交出单据'),primaryActionsHtml:`<div class="flex gap-4 text-sm">${link(mode==='pending'?'交出单据':'待交出列表',route(mode==='pending'?'documents':'pending'))}${process==='dye'?link('纱线整单交出','/fcs/craft/dyeing/yarn-shipments'):link('水溶加工单','/fcs/craft/dyeing/water-soluble-orders')}</div>`,statusTabsHtml:tabs,filtersHtml:filters(),statsHtml:renderStandardListStats(mode==='pending'?[{label:'待交加工单',value:items.length},{label:'可建单加工单',value:items.filter(i=>api.rolls(i.id).some(r=>api.isAvailable(i.id,r))).length},{label:'已选卷数',value:selected.size}]:[{label:'交出单据',value:items.length},{label:'等待交出',value:items.filter(i=>i.doc!.status==='草稿').length},{label:'已交出',value:items.filter(i=>i.doc!.status==='已交出').length}],{compact:true}),listTitle:`共 ${items.length} 条${mode==='pending'?` · 已选 ${selected.size} 卷`:''}`,listActionsHtml:`<div class="flex flex-wrap gap-2">${mode==='pending'?btn('批量生成交出单','create','',true,!selected.size)+btn('合入已有交出单','join','',false,!selected.size):''}${btn('列设置','open-column-settings')}</div>`,tableHtml:renderStandardListTable({columns:cols,rows:paging.rows,preferences,sort,eventPrefix:prefix,skipPageRerender:true,emptyText:'暂无符合条件的记录'}),paginationHtml:renderTablePagination({total:paging.total,from:paging.from,to:paging.to,currentPage:paging.currentPage,totalPages:paging.totalPages,pageSize:paging.pageSize,actionPrefix:prefix,fieldPrefix:prefix,pageSizeOptions:sizes,skipPageRerender:true}),overlaysHtml:showColumns?renderStandardListColumnSettings({title:'列表列设置',columns:cols,preferences,eventPrefix:prefix,maxFrozenWidth:550,skipPageRerender:true}):''})
}
function detail(doc: DyeDispatchDocument): string {
  const q=dyeDispatchQuantities(doc)
  return `<div class="grid grid-cols-3 gap-3 rounded-lg border bg-slate-50 p-3 text-sm">${line('工厂 / 接收方',`${doc.lines[0].factoryName} → ${[...new Set(doc.lines.map(l=>l.receiver))].join(' / ')}`)}${line('总数',doc.historical?`卷码未登记 · ${q.text}`:`${q.rolls} 卷 · ${q.text}`)}${line('扫卷进度',doc.historical?'历史未登记':`${doc.scans.length} / ${q.rolls}`)}${line('状态',getDyeDispatchStatus(doc))}${line('建单',`${doc.operator} / ${doc.createdAt||'历史未登记建单时间'}`)}${line('交出',doc.handedOverAt||'尚未交出')}</div>
    ${doc.historical?'<p class="my-3 text-sm text-amber-700">历史交接未登记卷码，按原单数量追溯。</p>':''}${process==='water'&&doc.status==='草稿'&&!getPdaSession()?'<p class="my-3 text-sm">确认实物交出需本厂交接员或管理员账号。<a class="text-blue-600" href="/pda/login">登录交接账号</a></p>':''}${doc.status==='草稿'?`<section class="my-4 rounded-lg border p-3"><h3 class="mb-3 font-semibold">扫码核对实物</h3><div class="grid grid-cols-[1fr_2fr_auto] items-end gap-3">${field('scanner','操作人',doc.operator)}${field('scan','卷码','','text','扫描卷码后按 Enter')}${btn('核对卷码','scan',doc.id,true)}</div></section>`:''}
    <div class="my-4">${table(['加工单 / 商品',process==='water'?'任务 / 物料 SKU':'任务 / 面料 SKU',process==='water'?'卷码':'卷码 / 缸号','数量','扫码核对'],doc.lines.flatMap(l=>(l.rolls.length?l.rolls:[{barcode:'历史未登记卷码',vatNo:'',qty:l.historicalQty??0}]).map(r=>{const row=info(l.orderId),scan=doc.scans.find(s=>s.barcode===r.barcode);return [e(l.orderNo)+`<div class="mt-2">${objectImage(row.productImageUrl,row.productName,row.productCode)}</div>`,line('任务',l.taskNo)+objectImage(row.outputImageUrl,row.materialName,l.sku),`<span class="select-all break-all">${e(r.barcode)}</span>${process==='dye'?line('缸号',r.vatNo||'未分缸'):''}`,`${r.qty.toFixed(2)} ${e(l.unit)}`,scan?badge('已扫码',true)+line('操作人',scan.operator)+line('时间',scan.at):doc.historical?'历史未登记':badge('待扫码')]})))}</div>
    ...
    ${doc.status==='已交出'?`<section class="mt-4 rounded-lg border p-3"><h3 class="mb-2 font-semibold">下游实际接收</h3>${doc.lines.map(l=>{const row=info(l.orderId),actual=l.receivingSourceId?getSourceActualReceipts(l.receivingSourceId):[];const records=row.handoverRecords.filter(r=>(r.handoverRecordId||r.recordId)===l.handoverRecordId);const qty=actual.length?actual.reduce((n,r)=>n+r.qty,0):records.reduce((n,r)=>n+(r.receiverWrittenQty??r.warehouseWrittenQty??0),0);return `<div class="mb-2">${e(l.orderNo)} → ${e(l.receiver)} · 实收 ${qty.toFixed(2)} ${e(actual.length?actual[0].unit:l.unit)} ${l.receivingSourceId?link('查看待接收单',`/fcs/craft/dyeing/pending-receipts?sourceId=${encodeURIComponent(l.receivingSourceId)}`):link('查看原交接记录',orderRoute(l.orderId))}</div>`}).join('')}</section>`:''}
...
function selectedSummary(){return table(['加工单','工厂','所选卷数','本次数量'],selections().map(s=>{const r=info(s.orderId);return [e(r.workOrderNo),e(r.factoryName),String(s.rollIds.length),`${api.rolls(s.orderId).filter(r=>s.rollIds.includes(r.id)).reduce((n,r)=>n+r.qty,0).toFixed(2)} ${e(r.qtyUnit)}`]}))}
function overlayBody(): string {
  if(overlay==='create'||overlay==='join'){
    ...
    const docs=api.docs().filter(d=>d.status==='草稿'&&d.lines[0]?.factoryId===selectedFactory)
  ...
  const doc=api.docs().find(d=>d.id===docId);if(!doc)return '单据不存在'
  return overlay==='print'?`${btn('打印','print-frame','',true)}${btn('返回详情','detail',docId)}<iframe class="mt-3 h-[65vh] w-full rounded border bg-slate-200" title="${processLabel()}交出单打印预览" data-dye-output-print></iframe>`:detail(doc)
}
function refreshOverlay(){
  ...
  if(overlay==='print'){const frame=host.querySelector<HTMLIFrameElement>('iframe')!;frame.srcdoc=renderDyeDispatchPrint(api.docs().find(d=>d.id===docId)!,rows,processLabel())}
  host.querySelector<HTMLInputElement>('[data-dye-output-field="scan"]')?.focus()
}
function refreshList(){rows=loadRows();const root=document.querySelector('[data-dye-output-list]');if(root){root.innerHTML=listBody();hydrateIcons(root as HTMLElement)}syncSelection()}
function syncSelection(){const root=document.querySelector('[data-dye-output-list]');if(!root)return;root.querySelectorAll<HTMLInputElement>('[data-dye-output-select]').forEach(el=>el.checked=selected.has(el.dataset.dyeOutputSelect!));root.querySelectorAll<HTMLInputElement>('[data-dye-output-select-order]').forEach(el=>{const rolls=api.rolls(el.dataset.dyeOutputSelectOrder!).filter(r=>api.isAvailable(el.dataset.dyeOutputSelectOrder!,r));const count=rolls.filter(r=>selected.has(`${el.dataset.dyeOutputSelectOrder}|${r.id}`)).length;el.checked=rolls.length>0&&count===rolls.length;el.indeterminate=count>0&&count<rolls.length});root.querySelectorAll<HTMLButtonElement>('[data-dye-output-action="create"],[data-dye-output-action="join"]').forEach(el=>el.disabled=!selected.size);const title=root.querySelector('[data-standard-list-table-section] > header h2');if(mode==='pending'&&title)title.textContent=`共 ${filtered().length} 条 · 已选 ${selected.size} 卷`;const head=root.querySelector<HTMLInputElement>('[data-dye-output-action="select-page"]');if(head){const checks=[...root.querySelectorAll<HTMLInputElement>('[data-dye-output-select]:not(:disabled)')];head.checked=checks.length>0&&checks.every(el=>el.checked);head.indeterminate=!head.checked&&checks.some(el=>el.checked)}const stats=root.querySelectorAll('[data-standard-list-stats] strong');if(mode==='pending'&&stats[2])stats[2].textContent=String(selected.size)}
...
function renderPage(next:Mode,kind:'dye'|'water'='dye'):string{
  const changed=process!==kind;process=kind;api=kind==='water'?waterApi:dyeApi
  mode=next;rows=loadRows();if(changed||lastMode!==mode||typeof document==='undefined'||!document.querySelector('[data-dye-output-page]')){page=1;sort=null;overlay='';showColumns=false;selected.clear();keyword=receiver=status=ready=dateFrom=dateTo='';factory='';showMore=false;if(process==='water'&&mode==='pending')status='待建单据'}
  ...
  return `<div data-dye-output-page data-output-process="${process}" data-skip-page-rerender="true" class="min-w-0"><p role="alert" data-dye-output-feedback class="px-4 text-sm text-blue-700">${e(feedback)}</p><main data-dye-output-list class="min-w-0">${listBody()}</main><div data-dye-output-overlays></div></div>`
}
export function renderWaterSolublePendingHandoverPage(){return renderPage('pending','water')}
export function renderWaterSolubleHandoverDocumentsPage(){return renderPage('documents','water')}
...
export function handleDyeOutputEvent(target:HTMLElement,event?:Event):boolean{
  ...
  if(target.matches('[data-dye-output-select],[data-dye-output-select-order]')){if(event?.type==='click')return true;const input=target as HTMLInputElement;const keys=input.dataset.dyeOutputSelect?[input.dataset.dyeOutputSelect]:api.rolls(input.dataset.dyeOutputSelectOrder!).filter(r=>api.isAvailable(input.dataset.dyeOutputSelectOrder!,r)).map(r=>`${input.dataset.dyeOutputSelectOrder}|${r.id}`);keys.forEach(k=>input.checked?selected.add(k):selected.delete(k));syncSelection();return true}
...
if(typeof document!=='undefined'){
  document.addEventListener('keydown',event=>{if(!document.querySelector('[data-dye-output-page]')||(event.target as HTMLElement)?.closest('[data-pda-image-preview-root]'))return;if(event.key==='Escape'&&overlay&&!document.querySelector('[data-dye-barcode-root], [data-water-barcode-root]')){overlay='';refreshOverlay()}if(event.key==='Enter'&&(event.target as HTMLElement)?.matches('[data-dye-output-field="scan"]')){event.preventDefault();document.querySelector<HTMLButtonElement>('[data-dye-output-action="scan"]')?.click()}})
