...
import { buildCapacityProfileLink, buildDyeingWorkOrderDetailLink, buildHandoverOrderLink, buildTaskDetailLink, } from '../../../data/fcs/fcs-route-links.ts';
import { getDyeOrderHandoverSummary, getDyeReviewRecordByOrderId, listDyeFormulaRecords, listDyeReportRows, listDyeReviewRecords, listDyeVatSchedules, listDyeWorkOrders, } from '../../../data/fcs/dyeing-task-domain.ts';
...
import { formatDyeQty, getDyeVatSummary, getSelectedDyeOrderId, renderActionButton, renderMetricCard, renderPageHeader, renderReviewStatusBadge, renderSection, renderWorkOrderStatusBadge, } from './shared';
function renderNodeOverview() {
    const statistics = getDyeingExecutionStatistics();
    const countByLabel = (label) => statistics.statusCounts[label] || 0;
    return renderSection('节点分布', `
...
function renderDurationTable() {
    const rows = listDyeReportRows()
      ...
      `;
        ...
        .join('');
    return renderSection('等待原因与节点耗时', `
...
function renderReviewTable(selectedId) {
...
function renderSelectedDetail(selectedId) {
...
export function renderCraftDyeingReportsPage() {
