...
function renderBody() {
  ...
  <div class="mb-3 flex flex-wrap items-center justify-between gap-3 text-xs"><span>总卷数：<b>${rolls.length}</b>　总长度：<b>${rolls.reduce((n,r) => n+r.qty,0).toFixed(2)}</b> ${e(row.qtyUnit)}　总重量：<b>${rolls.reduce((n,r) => n+r.weightKg,0).toFixed(3)}</b> kg</span><label>每页 <select data-dye-barcode-field="pageSize" class="rounded border p-1">${[10,20,50].map(n=>`<option ${n===pageSize?'selected':''}>${n}</option>`).join('')}</select> 条</label></div>
  ${table([`<input aria-label="全选本页条码" type="checkbox" data-dye-barcode-action="select-page" ${visible.length && visible.every(r => selected.has(r.id)) ? 'checked' : ''}>`, 'ID', '条码', '关联单号', 'SKU / 物料', '状态', '入库状态', '卷号', `卷长 (${e(row.qtyUnit)})`, '重量 (KG)', '克重 (g/m²)', '幅宽 (cm)', '入库仓库 / 库位', '入库时间', '打印人', '打印时间', '操作'], visible.map(roll => [
...
function closeInner() { root()?.querySelector('[data-dye-barcode-inner]')?.remove(); importRows = []; printingIds = [] }
...
function editor(mode: typeof editorMode, ids: string[]) {
  ...
  modal(batch ? `批量修改 ${ids.length} 卷` : mode === 'copy' ? '新增条码' : mode === 'edit' ? '编辑条码' : '补充条码',
    `${roll ? `<p class="mb-2 text-xs text-slate-500">参照条码：${e(roll.barcode)}</p>` : ''}<p class="mb-3 text-xs text-amber-700">${batch ? '只修改填写的字段，留空保持各卷原值。' : '填写长度或重量，按幅宽、克重即时换算另一项，结果保留 2 位小数。'}</p><div class="grid grid-cols-2 gap-3" data-dye-barcode-editor>
    ...
    ${input('weightKg','重量 (KG)', batch ? '' : fresh ? '0' : get('weightKg',0),'number')}${input('gsm','克重 (g/m²)',get('gsm',row.weightGsm || 0),'number')}${input('widthCm','幅宽 (cm)',get('widthCm',parseFloat(row.width) || 0),'number')}${input('vatNo','缸号',get('vatNo'))}<label class="col-span-2 text-sm">备注<textarea data-dye-barcode-field="remark" class="mt-1 min-h-20 w-full rounded border p-2" placeholder="打印时追加到标签末尾">${e(get('remark'))}</textarea></label></div>`, button('确定','save')+button('取消','close-inner'))
}
function convert(fieldName: string) {
  if (editorMode === 'batch' || !['qty','meters','weightKg','gsm','widthCm'].includes(fieldName)) return
  const row = rowInfo(), factor = dyeLengthMeters(1,row.qtyUnit)
  if (factor === null) return
  const width = Number(value('widthCm')), gsm = Number(value('gsm')), source = Number(value(fieldName))
  if (!value(fieldName) || !Number.isFinite(source) || source < 0) { showError('请输入有效的非负数字。'); return }
  let qty = Number(value('qty'))
  if (fieldName === 'meters') qty = source / factor
  if (fieldName === 'weightKg') { if (!(width > 0 && gsm > 0)) { showError('先填写大于 0 的幅宽和克重，再换算重量。'); return }; qty = source * 1000 / (width/100*gsm*factor) }
  if (!Number.isFinite(qty) || qty < 0) return
  const set = (name: string, n: number) => { const el = root()?.querySelector<HTMLInputElement>(`[data-dye-barcode-field="${name}"]`); if (el && name !== fieldName) el.value = n.toFixed(2) }
  set('qty', qty); set('meters',qty*factor)
  if (width > 0 && gsm > 0) set('weightKg',qty*factor*width/100*gsm/1000)
  showError('')
}
...
export function openDyeBarcodeDialog(id: string, snapshot?: DyeWorkOrderOnlineRow) {
  ...
  el.addEventListener('keydown', event=>{if(event.key==='Escape'){event.stopPropagation();el.querySelector('[data-dye-barcode-inner]') ? closeInner() : el.remove()}})
  el.addEventListener('click', event=>{if(event.target===el) el.remove(); else if((event.target as HTMLElement).matches('[data-dye-barcode-inner]')) closeInner()})
  ...
  document.querySelector('[data-dye-work-orders-root]')?.appendChild(el); el.focus()
}
export function handleDyeBarcodeEvent(target: HTMLElement): boolean {
  ...
  try {
    if(action==='close'){root()?.remove();return true}
    ...
    if(action==='save'){
      ...
      for(const [key,label] of [['qty','卷长'],['weightKg','重量'],['gsm','克重'],['widthCm','幅宽']] as const){if(editorMode==='batch'&&!value(key))continue;patch[key]=number(key,label)}
    ...
    if(action==='validate-import'){
      ...
      const parsed=lines.map((line,i)=>{const cells=line.split(line.includes('\t')?'\t':',').map(v=>v.trim());if(cells.length<4||cells.slice(0,4).some(v=>v===''||!Number.isFinite(Number(v))||Number(v)<0)||Number(cells[0])<=0)throw new Error(`第 ${i+1} 行：请填写有效卷长、重量、克重、幅宽；卷长须大于 0。`);return{qty:Number(cells[0]),weightKg:Number(cells[1]),gsm:Number(cells[2]),widthCm:Number(cells[3]),vatNo:cells[4]||'',remark:cells.slice(5).join(',')}})
      importRows=parsed;root()!.querySelector('[data-dye-barcode-import-preview]')!.innerHTML=`<p class="my-2 text-sm text-green-700">校验通过 ${parsed.length} 卷，总长度 ${parsed.reduce((s,r)=>s+r.qty,0).toFixed(2)} ${e(rowInfo().qtyUnit)}</p>`+table(['卷长','重量','克重','幅宽','缸号'],parsed.slice(0,10).map(r=>[String(r.qty),String(r.weightKg),String(r.gsm),String(r.widthCm),e(r.vatNo)]));return true
    }
