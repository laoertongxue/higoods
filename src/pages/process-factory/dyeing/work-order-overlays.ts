...
function renderView(dyeOrderId: string): string {
  ...
  const body = `<div class="max-h-[68vh] space-y-4 overflow-y-auto pr-1">
    <section class="space-y-3"><h3 class="font-medium">基本信息</h3><div class="flex gap-3">${renderObjectImage(row.productImageUrl, `${row.productCode} ${row.productName}`)}<div class="grid flex-1 grid-cols-2 gap-3">${field('商品', `${row.productCode} · ${row.productName}`)}${field('需求来源', `${row.sourceLabel} · ${row.productionOrderNo || row.purchaseOrderNo}`)}${field('生产需求单', row.purchaseOrderNo)}${field('任务', row.taskNo)}${field('当前加工厂', row.factoryName)}</div></div><div class="flex gap-4">${renderBadge(`接收：${row.receiptKnown ? row.receiptStatusLabel : '历史待补录'}`, axisTone(row.receiptStatus))}${renderBadge(`加工：${row.processingStatusLabel}`, axisTone(row.processingStatus))}${renderBadge(`交出：${row.handoverStatusLabel}`, axisTone(row.handoverStatus))}</div></section>
...
function renderEdit(dyeOrderId: string, error = ''): string {
  ...
  const body = `<div class="max-h-[68vh] space-y-4 overflow-y-auto pr-1" data-skip-page-rerender="true">
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      ${formField('生产需求单', renderInput({ value: row.purchaseOrderNo, readonly: true }))}
