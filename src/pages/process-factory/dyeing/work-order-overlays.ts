...
function renderView(dyeOrderId: string): string {
  ...
  const body = `<div class="max-h-[68vh] space-y-4 overflow-y-auto pr-1">
    <section class="space-y-3"><h3 class="font-medium">基本信息</h3><div class="flex gap-3">${renderObjectImage(row.productImageUrl, `${row.productCode} ${row.productName}`)}<div class="grid flex-1 grid-cols-2 gap-3">${field('商品', `${row.productCode} · ${row.productName}`)}${field('需求来源', `${row.sourceLabel} · ${row.designRevisionTaskNo || row.productionOrderNo || row.purchaseOrderNo}`)}${row.sourceType === 'DESIGN_REVISION' ? field('设计改款任务', row.designRevisionTaskNo) : field('生产需求单', row.purchaseOrderNo)}${field('任务', row.taskNo)}${field('当前加工厂', row.factoryName)}</div></div><div class="flex gap-4">${renderBadge(`接收：${row.receiptKnown ? row.receiptStatusLabel : '历史待补录'}`, axisTone(row.receiptStatus))}${renderBadge(`加工：${row.processingStatusLabel}`, axisTone(row.processingStatus))}${renderBadge(`交出：${row.handoverStatusLabel}`, axisTone(row.handoverStatus))}</div></section>
    <section class="space-y-3 border-t pt-3"><h3 class="font-medium">加工投入／上游</h3><div class="flex gap-3">${renderObjectImage(row.materialImageUrl, row.materialName)}<div class="grid flex-1 grid-cols-2 gap-3">${field('投入物料', row.rawMaterialSku !== row.materialName && !/[\u4e00-\u9fff]/.test(row.rawMaterialSku) ? `${row.materialName} · ${row.rawMaterialSku}` : row.materialName)}${row.upstreamPartners.flatMap(partner=>dyePartnerFields(partner)).map(([label,value])=>field(label,value)).join('')}${field('计划投入', qty(row.plannedQty))}${field('已接收', row.receiptKnown ? qty(row.receivedInputQty) : '历史接收待补录')}${field('实际使用', row.usageKnown ? qty(row.rawMaterialQty) : '待补录')}</div></div><div class="text-xs">关联上游：${links(row.upstreamLinks) || '暂无上游加工单'}<br>来源单据：${escapeHtml(row.inputSourceDocumentNos.join(' / ') || '待生成')}</div><details><summary class="cursor-pointer text-sm text-blue-700">供料记录 ${row.receiptRecords.length} 笔</summary>${table(['接收单','上游交接记录','实收数量','接收人','接收时间'], row.receiptRecords.map(record => [record.receiptId, getPreparationMaterialSourceDocumentNo(record.upstreamRecordId) || '历史来源单据待补录', qty(record.qty), record.receiverName, record.receivedAt]))}</details></section>
    ...
    <section class="space-y-3 border-t pt-3"><h3 class="font-medium">时间与备注</h3>${renderDyeWorkOrderTimes(row.timeSections, true)}<div class="mt-3">${field('备注', row.remark)}</div></section>
  </div>`
...
function renderEdit(dyeOrderId: string, error = ''): string {
  ...
  const body = `<div class="max-h-[68vh] space-y-4 overflow-y-auto pr-1" data-skip-page-rerender="true">
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      ${row.sourceType === 'DESIGN_REVISION' ? formField('设计改款任务', renderInput({ value: row.designRevisionTaskNo, readonly: true })) : formField('生产需求单', renderInput({ value: row.purchaseOrderNo, readonly: true }))}
