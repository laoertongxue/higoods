...
function renderFactoryTabs(rows: DyeWorkOrderOnlineRow[]): string {
  ...
  return `<div class="mb-3 flex items-end gap-3 border-b border-slate-300 bg-slate-50 px-3 pt-3"><div role="tablist" aria-label="加工厂切换" class="flex min-w-0 flex-1 flex-wrap gap-1">${tabs}</div></div>`
}
...
function filteredRows(allRows: DyeWorkOrderOnlineRow[]): DyeWorkOrderOnlineRow[] {
  return baseFilteredRows(allRows)
}
