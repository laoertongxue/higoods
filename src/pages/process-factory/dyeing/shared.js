...
import { getDyeExecutionNodeRecord, getDyeWorkOrderStatusLabel, } from '../../../data/fcs/dyeing-task-domain.ts';
export function getDyeingSearchParams() {
    const pathname = appStore.getState().pathname || '';
    const query = pathname.split('?')[1] || '';
    return new URLSearchParams(query);
}
export function getSelectedDyeOrderId(fallbackId = '') {
    return getDyeingSearchParams().get('dyeOrderId') || fallbackId;
}
export function buildDyeingHref(path, dyeOrderId) {
    if (!dyeOrderId)
        return path;
    return `${path}?dyeOrderId=${encodeURIComponent(dyeOrderId)}`;
}
export function formatDyeQty(value, unit) {
    return `${value ?? 0} ${escapeHtml(unit || '米')}`;
}
export function formatDyeTime(value) {
...
export function getDyeNodeRecord(orderId, nodeCode) {
    return getDyeExecutionNodeRecord(orderId, nodeCode);
}
export function getDyeVatSummary(order) {
