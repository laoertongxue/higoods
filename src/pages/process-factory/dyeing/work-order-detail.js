...
import { buildDyeingWorkOrderDetailLink, buildHandoverDifferenceRequestPrintLink, buildHandoverOrderLink, buildTaskDetailLink, } from '../../../data/fcs/fcs-route-links.ts';
...
import { executeProcessWebAction, getAvailableDyeWebActions, getUnifiedOperationRecordsForProcessWorkOrder, } from '../../../data/fcs/process-web-status-actions.ts';
...
function renderNodeTable(orderId) {
    const order = getProcessWorkOrderById(orderId);
    if (!order)
        return '';
...
export function renderCraftDyeingWorkOrderDetailPage(dyeOrderId) {
    ...
    const formulaRows = dye.formulaRecords
        .flatMap((formula) => formula.lines.map((line) => `
        <tr class="border-b last:border-b-0">
    ...
    const sections = {
      ...
      `),
        exception: renderSection('染色交出差异处理', `
    ...
    return `
    <div class="space-y-4 p-4">
      ${renderPageHeader('染色加工单详情', 'Web 端查看加工单主详情；染色配方和染色统计都是加工单下的信息视图。', `
          <div class="flex flex-wrap gap-2">
            <button class="rounded-md border px-3 py-2 text-sm hover:bg-muted" data-nav="/fcs/craft/dyeing/work-orders">返回染色加工单</button>
