...
export function dyeDispatchQuantities(doc: DyeDispatchDocument) {
  ...
  let meters = 0, rolls = 0, hasLength = false
  for (const line of doc.lines) {
    const qty = line.historicalQty ?? line.rolls.reduce((sum, roll) => sum + roll.qty, 0)
    units.set(line.unit, (units.get(line.unit) || 0) + qty)
    const length=dyeLengthMeters(qty,line.unit);if(length!==null){meters+=length;hasLength=true}
    rolls += line.rolls.length
  }
  return {rolls, meters, hasLength, yards: meters / .9144, skus: new Set(doc.lines.map(line => line.sku)).size, text: [...units].map(([unit, qty]) => `${qty.toFixed(2)} ${unit}`).join(' / ')}
}

/** 依据线上 SURAT JALAN 字段组织；非长度单位独立保留，不能相加成长度。 */
export function renderDyeDispatchPrint(doc: DyeDispatchDocument, orders: (OutputWorkOrderRow | DyeWorkOrderOnlineRow)[], processLabel = '染色'): string {
  const q = dyeDispatchQuantities(doc)
  const cell = (value: string | number) => `<td>${e(value)}</td>`
  const rows = doc.lines.map((line, index) => {
    const row = orders.find(order => ('orderId' in order ? order.orderId : order.dyeOrderId) === line.orderId)!
    const qty = line.historicalQty ?? line.rolls.reduce((sum, roll) => sum + roll.qty, 0), meters = dyeLengthMeters(qty, line.unit)
    const designRevisionTaskNo = 'sourceType' in row && row.sourceType === 'DESIGN_REVISION' ? row.designRevisionTaskNo : ''
    return `<tr>${cell(index+1)}${cell(meters === null ? '不适用' : (meters/.9144).toFixed(2))}${cell(meters === null ? '不适用' : meters.toFixed(2))}${cell(doc.historical?'未登记':line.rolls.length)}<td class="object"><img src="${e(localProductFixtureImageUrl(row.productImageUrl))}" alt="${e(row.productName)}"><b>${e(row.productName)}</b><br>${e(row.productCode)}</td><td class="object"><img src="${e(localProductFixtureImageUrl(row.outputImageUrl))}" alt="${e(row.materialName)}"><b>${e(row.materialName)}</b><br>${e(line.sku)}</td>${cell(line.orderNo)}${cell(designRevisionTaskNo ? '设计改款任务' : row.purchaseOrderNo)}${cell(designRevisionTaskNo || row.productionOrderNo || '备料，未关联生产单')}${cell(`${qty.toFixed(2)} ${line.unit}\n${line.receiver}`)}</tr>`
  }).join('')
  return `<!doctype html><html lang="zh"><head><meta charset="utf-8"><title>${e(processLabel)}交出单 ${e(doc.id)}</title><style>
    ...
    <header><h1>${e(doc.lines[0]?.factoryName || '')}</h1><div class="barcode">${renderCode128Barcode(doc.id,doc.id)}<div>${e(doc.id)}</div></div><div><h2>SURAT JALAN</h2><div>${e(processLabel)}交出单</div><p>${e(doc.handedOverAt || doc.createdAt)}</p></div></header>
    <div class="meta"><div><p><b>KEPADA / PENERIMA</b>${e([...new Set(doc.lines.map(line=>line.receiver))].join(' / '))}</p><p><b>JUMLAH PESANAN</b>${doc.lines.length} 单 · ${doc.historical?'未登记卷码':`${q.rolls} Roll`} · ${q.skus} SKU</p><p><b>STATUS / 状态</b><span class="${doc.status==='草稿'?'draft':''}">${e(getDyeDispatchStatus(doc))}${doc.status==='草稿'?'（尚未交出）':''}</span></p></div><div><p><b>JENIS KENDARAAN</b>${e(doc.transport.vehicle || '尚未登记')}</p><p><b>DRIVER / SUPIR</b>${e(doc.transport.driver || '尚未登记')}</p><p><b>NO. POLISI</b>${e(doc.transport.plate || '尚未登记')}</p></div></div>
    <table><thead><tr>${['NO','YARD','METER','ROLL','SPU / 商品','JENIS KAIN / 面料 SKU',`${processLabel === '染色' ? 'ID DYE' : 'ID PRINT'} / 加工单`,'PURCHASE NO / 需求','来源任务 / 生产单','KETERANGAN / 数量及接收方'].map(title=>`<th>${title}</th>`).join('')}</tr></thead><tbody>${rows}</tbody><tfoot><tr><td>合计</td><td>${q.hasLength?q.yards.toFixed(2):'不适用'}</td><td>${q.hasLength?q.meters.toFixed(2):'不适用'}</td><td>${doc.historical?'未登记':q.rolls}</td><td colspan="6">${e(q.text)} · ${doc.lines.length} Pesanan</td></tr></tfoot></table>
