import { buildSpecialCraftWorkOrderDetailPath, getSpecialCraftManagementDomainBySlug, listOperationDefinitionsByManagementDomain, } from '../../../data/fcs/special-craft-operations.ts';
import { listProcessHandoverRecords, listWaitHandoverWarehouseRecords, listWaitProcessWarehouseRecords, } from '../../../data/fcs/process-warehouse-domain.ts';
import { escapeHtml } from '../../../utils.ts';
import { paginateItems, renderCompactKpiCard, renderStickyFilterShell, renderStickyTableScroller, renderWorkbenchFilterChip, renderWorkbenchPagination, renderWorkbenchStateBar, } from '../cutting/layout.helpers.ts';
import { formatQty, renderEmptyState, renderSpecialCraftPageLayout, renderStatusBadge, } from './shared.ts';
import { renderFactoryWarehouseStandardTabs, renderWarehouseFlowButton, } from '../shared/warehouse-standard.ts';
...
const warehouseStateByPage = new Map();
const PHYSICAL_AREA_OPTIONS = [
...
const specialCraftWarehouseDomainMeta = {
...
function setWarehouseState(domainSlug, mode, patch) {
    const key = getWarehouseStateKey(domainSlug, mode);
...
function findOperationByCraftName(operations, craftName) {
    return operations.find((operation) => operation.operationName === craftName);
}
function filterSpecialCraftRecordsByDomain(records, craftNames) {
    return records.filter((record) => record.craftType === 'SPECIAL_CRAFT' && craftNames.has(record.craftName));
}
function resolvePhysicalAreaMeta(craftName) {
    if (craftName.includes('直喷') || craftName.includes('烫画'))
        return PHYSICAL_AREA_OPTIONS[0];
    if (craftName.includes('绣') || craftName.includes('贝壳') || craftName.includes('曲牙'))
        return PHYSICAL_AREA_OPTIONS[1];
    if (craftName.includes('打条') || craftName.includes('压褶'))
        return PHYSICAL_AREA_OPTIONS[2];
    return PHYSICAL_AREA_OPTIONS[3];
}
function buildLocationText(craftName, locationSeed) {
...
function recordMatchesWarehouseState(record, state) {
    if (state.operationName !== '全部' && record.craftName !== state.operationName)
        return false;
    if (state.factoryId !== '全部') {
        const factoryId = 'targetFactoryId' in record ? record.targetFactoryId : record.handoverFactoryId;
        if (factoryId !== state.factoryId)
            return false;
    }
    if (state.physicalAreaName !== '全部' && resolvePhysicalAreaMeta(record.craftName).areaName !== state.physicalAreaName)
        return false;
    return recordMatchesKeyword(record, state.keyword);
}
function buildOperationOptions(operations) {
    return [{ value: '全部', label: '全部' }, ...operations.map((operation) => ({ value: operation.operationName, label: operation.operationName }))];
}
function buildFactoryOptions(records) {
    const seen = new Set();
    const options = records
        .map((record) => {
        if ('targetFactoryId' in record)
            return { value: record.targetFactoryId, label: record.targetFactoryName };
...
function renderWarehouseFilterSelect(label, field, value, options, domainSlug, mode) {
    return `
...
function renderWarehouseFilters(domainSlug, mode, state, operations, activeRecords) {
    ...
    <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
      ...
      ${renderWarehouseFilterSelect('物理库区', 'physicalAreaName', state.physicalAreaName, [{ value: '全部', label: '全部' }, ...PHYSICAL_AREA_OPTIONS.map((item) => ({ value: item.areaName, label: item.areaName }))], domainSlug, mode)}
    </div>
  `);
}
function renderWarehouseStateBar(domainSlug, mode, state) {
    const chips = [
...
function renderDomainWorkOrderAction(operations, record) {
    const operation = findOperationByCraftName(operations, record.craftName);
    if (!operation)
        return `<span class="text-sm text-slate-400">未匹配工艺</span>`;
...
function buildSpecialWarehouseFlowLines(record) {
...
function renderWaitProcessRows(records, operations) {
    if (!records.length)
        return `<tr><td colspan="8" class="py-10 text-center text-muted-foreground">当前筛选条件下暂无待加工库存。</td></tr>`;
    return records
        .map((record) => {
...
function renderWaitHandoverRows(records, operations) {
    if (!records.length)
        return `<tr><td colspan="8" class="py-10 text-center text-muted-foreground">当前筛选条件下暂无待交出库存。</td></tr>`;
    return records
        .map((record) => {
        const locationText = buildLocationText(record.craftName, record.warehouseRecordNo);
        return `
...
function renderHandoverRows(records) {
    if (!records.length)
        return `<tr><td colspan="8" class="py-10 text-center text-muted-foreground">当前筛选条件下暂无交出记录。</td></tr>`;
    return records
        .map((record) => `
      <tr class="align-top hover:bg-muted/20">
        <td class="px-3 py-3">
...
function buildDomainLocationRows(operations, mode) {
...
function renderLocationRows(rows) {
    if (!rows.length)
        return `<tr><td colspan="7" class="py-10 text-center text-muted-foreground">当前筛选条件下暂无库区库位。</td></tr>`;
    const seen = new Set();
    return rows
        .filter((row) => {
...
function renderPaginatedTable(input) {
...
function renderSpecialCraftDomainWarehousePageByMode(domainSlug, mode) {
    ...
    const allWaitProcessRecords = mode === 'wait-process'
        ? filterSpecialCraftRecordsByDomain(listWaitProcessWarehouseRecords({ craftType: 'SPECIAL_CRAFT' }), craftNames)
        : [];
    const allWaitHandoverRecords = mode === 'wait-handover'
        ? filterSpecialCraftRecordsByDomain(listWaitHandoverWarehouseRecords({ craftType: 'SPECIAL_CRAFT' }), craftNames)
        : [];
    const allHandoverRecords = mode === 'wait-handover'
        ? filterSpecialCraftRecordsByDomain(listProcessHandoverRecords({ craftType: 'SPECIAL_CRAFT' }), craftNames)
        : [];
    ...
    const locationRows = buildDomainLocationRows(operations, mode).filter((row) => {
        if (state.operationName !== '全部' && row.operationName !== state.operationName)
            return false;
        if (state.physicalAreaName !== '全部' && row.physicalAreaName !== state.physicalAreaName)
            return false;
...
export function renderSpecialCraftDomainWaitProcessWarehousePage(domainSlug) {
    return renderSpecialCraftDomainWarehousePageByMode(domainSlug, 'wait-process');
}
export function renderSpecialCraftDomainWaitHandoverWarehousePage(domainSlug) {
    return renderSpecialCraftDomainWarehousePageByMode(domainSlug, 'wait-handover');
}
export function handleSpecialCraftWarehouseEvent(target) {
    const pageSizeNode = target.closest('[data-special-craft-warehouse-page-size]');
    if (pageSizeNode) {
        const domainSlug = pageSizeNode.dataset.domainSlug;
        const mode = pageSizeNode.dataset.warehouseMode;
        if (!domainSlug || !mode)
            return false;
        setWarehouseState(domainSlug, mode, { pageSize: Number(pageSizeNode.value) || 20, page: 1 });
        return true;
    }
    const fieldNode = target.closest('[data-special-craft-warehouse-field]');
    if (fieldNode) {
        const domainSlug = fieldNode.dataset.domainSlug;
        const mode = fieldNode.dataset.warehouseMode;
        const field = fieldNode.dataset.specialCraftWarehouseField;
        if (!domainSlug || !mode || !field)
            return false;
        setWarehouseState(domainSlug, mode, { [field]: fieldNode.value, page: 1 });
        return true;
    }
    const actionNode = target.closest('[data-special-craft-warehouse-action]');
    ...
    const mode = actionNode.dataset.warehouseMode;
    if (!domainSlug || !mode)
        return false;
