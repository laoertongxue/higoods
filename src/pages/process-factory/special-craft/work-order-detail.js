import { buildSpecialCraftTaskDetailPath, buildSpecialCraftWorkOrderDetailPath, getSpecialCraftOperationBySlug, } from '../../../data/fcs/special-craft-operations.ts';
import { buildHandoverDifferenceRequestPrintLink } from '../../../data/fcs/fcs-route-links.ts';
import { getUnifiedOperationRecordsForProcessWorkOrder, } from '../../../data/fcs/process-web-status-actions.ts';
import { getSpecialCraftTaskOrderById, getSpecialCraftTaskWorkOrderById, getSpecialCraftTaskWorkOrderLinesByWorkOrderId, } from '../../../data/fcs/special-craft-task-orders.ts';
import { getSpecialCraftFeiTicketFlowEventsByWorkOrderId, listCuttingSpecialCraftFeiTicketBindings, listSpecialCraftQtyDifferenceReports, } from '../../../data/fcs/cutting/special-craft-fei-ticket-flow.ts';
import { applySpecialCraftDifferenceToFeiTickets, getDifferenceRecordsByWorkOrderId, getHandoverRecordsByWorkOrderId, getWarehouseRecordsByWorkOrderId, handleProcessHandoverDifference, } from '../../../data/fcs/process-warehouse-domain.ts';
...
const fastSpecialCraftActionDefs = [
    {
...
function getFastSpecialCraftWebActions(status) {
...
function getCurrentDetailTab() {
    const [, queryString = ''] = (appStore.getState().pathname || '').split('?');
    const tab = new URLSearchParams(queryString).get('tab');
    return specialCraftDetailTabs.some((item) => item.key === tab) ? tab : 'base';
}
function renderDetailTabs(baseHref, activeTab) {
    return `
...
function resolveWorkOrderObjectMeta(workOrder) {
...
function localizeSpecialCraftActionText(value, objectMeta) {
    if (objectMeta.objectType === '裁片')
        return value;
    return value.replaceAll('裁片', objectMeta.objectLabel);
}
function localizeSpecialCraftActionFields(fields, objectMeta) {
    return fields
        .filter((field) => objectMeta.objectType === '裁片' || field !== '关联菲票')
        .map((field) => localizeSpecialCraftActionText(field, objectMeta));
}
function showSpecialCraftToast(message) {
...
function renderWebActionPanel(workOrderId, currentStatus, actions, objectQty, objectMeta) {
...
function renderWebOperationRecords(records) {
...
function resolveDifferenceAction() {
    const [, queryString = ''] = (appStore.getState().pathname || '').split('?');
...
function applyDifferenceActionFromUrl() {
...
export function renderSpecialCraftWorkOrderDetailPage(operationSlug, workOrderId) {
    ...
    const webOperationRecords = activeTab === 'events'
        ? getUnifiedOperationRecordsForProcessWorkOrder('SPECIAL_CRAFT_WORK_ORDER', workOrder.workOrderId, workOrder.taskOrderId)
        : [];
    ...
    const sections = {
        ...
        receive: renderSection('接收记录', waitProcessRows
        ...
        process: renderSection('加工记录', lineRows || quantityRows
        ...
        difference: renderSection('差异记录', reportRows || unifiedDifferenceRows
        ...
        handover: renderSection('交出记录', waitHandoverRows || unifiedHandoverRows
...
export function handleSpecialCraftWorkOrderDetailEvent(target) {
    ...
    if (dialogHandled !== null)
        return dialogHandled;
    const actionNode = target.closest('[data-special-craft-web-action]');
    if (!actionNode)
        return false;
