import { buildSpecialCraftTaskDetailPath, buildSpecialCraftPreferredWarehousePath, getSpecialCraftOperationBySlug, } from '../../../data/fcs/special-craft-operations.ts';
...
import { formatQty, formatSpecialCraftFactoryLabel, renderEmptyState, renderSpecialCraftFactoryContextBlockedLayout, renderSpecialCraftPageLayout, resolveSpecialCraftFactoryContextGuard, renderStatusBadge, } from './shared.ts';
...
const taskListStateByOperation = new Map();
...
function getTaskListState(operationId) {
...
function buildTaskOrderLightweightSummary(taskOrder) {
    ...
    return {
        ...
        returnStatus: taskOrder.waitHandoverQty > 0
            ? '待回仓'
...
function buildFactoryOptions(taskOrders) {
    const seen = new Set();
...
function buildTaskFilters(state) {
    return {
...
function renderTaskFilterSelect(label, field, value, options, operationId) {
    return `
...
function renderTaskQuickFilterRow(operationId, state) {
    const options = [
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      ${options
        .map((option) => renderWorkbenchFilterChip(option.label, `data-special-craft-task-action="set-status" data-status="${option.value}" data-operation-id="${operationId}"`, state.status === option.value ? option.tone : 'blue'))
        .join('')}
    </div>
  `;
}
function renderTaskFilters(operationId, state, allTaskOrders) {
...
function renderTaskStateBar(operationId, state) {
    const chips = [
...
function renderStatsCards(taskOrders) {
...
function renderTaskOrdersTable(operationId, taskOrders, state) {
...
export function renderSpecialCraftTaskOrdersPage(operationSlug) {
    const operation = getSpecialCraftOperationBySlug(operationSlug);
    if (!operation)
        return renderMissingOperation();
...
export function handleSpecialCraftTaskOrdersEvent(target) {
    const pageSizeNode = target.closest('[data-special-craft-task-page-size]');
    if (pageSizeNode) {
        const operationId = pageSizeNode.dataset.operationId;
        if (!operationId)
            return false;
        setTaskListState(operationId, { pageSize: Number(pageSizeNode.value) || 20, page: 1 });
        return true;
    }
    const fieldNode = target.closest('[data-special-craft-task-field]');
    if (fieldNode) {
        const operationId = fieldNode.dataset.operationId;
        const field = fieldNode.dataset.specialCraftTaskField;
        if (!operationId || !field)
            return false;
        setTaskListState(operationId, { [field]: fieldNode.value, page: 1 });
        return true;
    }
    const actionNode = target.closest('[data-special-craft-task-action]');
