...
import { canFactorySeeSpecialCraftOperation, } from '../../../data/fcs/special-craft-operations.ts';
...
export function formatSpecialCraftFactoryLabel(factoryName, factoryId) {
    return formatFactoryDisplayName(factoryName, factoryId || undefined);
}
export function renderStatusBadge(label) {
    const tone = label.includes('差异') || label.includes('异议') || label.includes('异常')
        ? 'red'
...
export function renderFilterGrid(items) {
    return `
...
export function renderTable(headers, rows, minWidthClass = 'min-w-[1520px]') {
    return `
...
function getSpecialCraftFactoryContextFactoryId() {
...
export function resolveSpecialCraftFactoryContextGuard(operation) {
...
export function renderSpecialCraftFactoryContextBlockedLayout(input) {
    return renderSpecialCraftPageLayout({
        ...
        content: renderEmptyState(input.factoryName
...
export function renderLinkedRecord(recordNo, href, fallback = '—') {
    if (!recordNo || !href)
        return fallback;
