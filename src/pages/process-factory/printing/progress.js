...
import { getPrintOrderHandoverSummary, getPrintReviewRecordByOrderId, getPrintReviewStatusLabel, listPrintWorkOrders, } from '../../../data/fcs/printing-task-domain.ts';
import { buildHandoverOrderLink, buildPrintingWorkOrderDetailLink, buildTaskDetailLink, } from '../../../data/fcs/fcs-route-links.ts';
import { formatPrintQty, formatPrintTime, getPrintQuantityLabel, getPrintNodeRecord, getSelectedPrintOrderId, renderActionButton, renderPageHeader, renderSection, renderReviewStatusBadge, renderWorkOrderStatusBadge, } from './shared';
function renderOrderList(selectedId) {
...
function renderNodeCard(title, body) {
...
export function renderCraftPrintingProgressPage() {
    ...
    <div class="space-y-4">
      ...
      <section class="grid gap-4 xl:grid-cols-2">
        ${renderNodeCard('花型测试', `
            <div class="flex items-center gap-2">${colorTestNode?.finishedAt
        ...
        : colorTestNode?.startedAt
            ...
            : '<span class="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs text-slate-700">待花型图</span>'}</div>
