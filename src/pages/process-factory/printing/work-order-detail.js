...
import { buildHandoverDifferenceRequestPrintLink, buildHandoverOrderLink, buildPrintingWorkOrderDetailLink, buildTaskDetailLink, } from '../../../data/fcs/fcs-route-links.ts';
...
import { executeProcessWebAction, getAvailablePrintWebActions, getUnifiedOperationRecordsForProcessWorkOrder, } from '../../../data/fcs/process-web-status-actions.ts';
...
import { getDifferenceRecordsByWorkOrderId, getHandoverRecordsByWorkOrderId, getReviewRecordsByWorkOrderId, handleProcessHandoverDifference, } from '../../../data/fcs/process-warehouse-domain.ts';
import { formatFactoryDisplayName } from '../../../data/fcs/factory-mock-data.ts';
import { appStore } from '../../../state/store.ts';
import { formatPrintProcessQty, formatPrintQty, formatPrintTime, getPrintQuantityLabel, renderBadge, renderPageHeader, renderSection, } from './shared';
const printDetailTabs = [
    { key: 'base', label: '基本信息' },
...
function getCurrentPrintDetailTab() {
    const [, queryString = ''] = (appStore.getState().pathname || '').split('?');
    const tab = new URLSearchParams(queryString).get('tab');
    return printDetailTabs.some((item) => item.key === tab) ? tab : 'base';
}
function renderDetailTabs(orderId, activeTab) {
...
function renderWebActionPanel(orderId, currentStatus, actions, platformStatus) {
...
function renderNodeRows(orderId) {
    const order = getProcessWorkOrderById(orderId);
    if (!order)
        return '';
...
function applyWebActionFromUrl(orderId) {
    const [, queryString = ''] = (appStore.getState().pathname || '').split('?');
    const params = new URLSearchParams(queryString);
...
function renderDifferenceRows(records, orderId) {
...
export function renderCraftPrintingWorkOrderDetailPage(printOrderId) {
    ...
    const sections = {
      ...
      `),
        ...
        exception: renderSection('交出差异处理', `
    ...
    return `
    <div class="space-y-4 p-4">
      ${renderPageHeader('印花加工单详情', 'Web 端查看加工单主详情；移动端入口仅用于一线执行。', `
          <div class="flex flex-wrap gap-2">
            <button class="rounded-md border px-3 py-2 text-sm hover:bg-muted" data-nav="/fcs/craft/printing/work-orders">返回印花加工单</button>
