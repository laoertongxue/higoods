...
function selectionActions() {
  return `<div class="flex flex-wrap items-center gap-2" data-dispatch-selection-actions>${button('全选本页可建单卷', 'select-page')}${button('清空', 'clear')}<span class="text-xs" data-dispatch-selected>已选 ${selected.size} 卷</span>${button('批量生成交出单', 'create', '', selected.size === 0)}${button('合入已有草稿', 'merge', '', selected.size === 0)}</div>`
}
...
export function renderPrintingDispatchPrintDocument(doc: PrintingDispatchDocument): string {
  const lines = getPrintingDispatchLines(doc)
  const image = (identity: { imageUrl: string; imageAlt: string }) => identity.imageUrl ? `<img src="${e(localProductFixtureImageUrl(identity.imageUrl))}" alt="${e(identity.imageAlt)}">` : '<span class="missing">缺少对应图片</span>'
...
function showSelectionCount(): void {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-printing-dispatch="create"], [data-printing-dispatch="merge"]').forEach(button => { button.disabled = selected.size === 0 })
}
