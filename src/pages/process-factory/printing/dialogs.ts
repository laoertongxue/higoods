...
function renderProductionOperation(order: PrintingWorkOrderBusinessRecord, start: boolean): string {
  const facts = getPrintingWorkflowFacts(order.workOrderId)
  const targetSkuDeterminesPattern = order.demandSource.type === 'DESIGN_REVISION' && Boolean(order.requirement.frontPattern.patternNo)
  const choices = [...(targetSkuDeterminesPattern ? [] : [['ARTWORK:FINISH','确认花型版本'], ['SAMPLE:FINISH','确认样品/米样']]), ['PRINT:START','开始打印'], ['PRINT:FINISH','填报打印完成'], ...(facts.requiresTransfer ? [['TRANSFER:START','开始转印'],['TRANSFER:FINISH','填报转印完成']] : [])]
  ...
  ${!start ? field(`本工序累计完成（${order.output.qtyUnit}）`, inputControl('productionQty', '', {type:'number',step:'0.01'}), targetSkuDeterminesPattern ? '仅打印/转印完成时填写；开始动作无需数量' : '仅打印/转印完成时填写；开始和确认花型/样品无需数量') : ''}${field('操作人',inputControl('productionOperator','印花执行员'))}</div><p class="mt-4 text-xs text-slate-500">每次保存保留操作人和时间；已有接收、其他工序和交接事实不会被覆盖。</p>` })
}
...
function renderEditInformation(order: PrintingWorkOrderBusinessRecord): string {
  ...
  const blocker = order.processingStatus === 'CANCELLED' || order.processingStatus === 'PROCESS_COMPLETED' ? '本单已结束，不能变更印花要求。'
    ...
    : order.output.completedQty > 0 && (order.barcodes.some(roll => roll.quantityConfirmed === false || roll.lengthY <= 0) || order.barcodes.reduce((sum, roll) => sum + roll.lengthY, 0) < order.output.completedQty - 0.01) ? '存在尚未测量的产出卷，请先在产出卷条码中补齐实际数量。' : ''
  const targetSkuDeterminesPattern = order.demandSource.type === 'DESIGN_REVISION' && Boolean(order.requirement.frontPattern.patternNo)
  const locked = Boolean(blocker) || targetSkuDeterminesPattern
