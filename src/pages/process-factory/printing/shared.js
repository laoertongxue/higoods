...
import { getPrintExecutionNodeRecord, getPrintWorkOrderStatusLabel, } from '../../../data/fcs/printing-task-domain.ts';
import { getQuantityLabel, formatProcessQuantityWithUnit, } from '../../../data/fcs/process-quantity-labels.ts';
export function getPrintingSearchParams() {
    const pathname = appStore.getState().pathname || '';
    const query = pathname.split('?')[1] || '';
    return new URLSearchParams(query);
}
export function getSelectedPrintOrderId(fallbackId = '') {
    return getPrintingSearchParams().get('printOrderId') || fallbackId;
}
export function buildPrintingHref(path, printOrderId) {
    if (!printOrderId)
        return path;
...
export function getPrintQuantityContext(order, qtyPurpose = '计划', operationCode) {
    return {
        processType: 'PRINT',
...
export function getPrintQuantityLabel(order, qtyPurpose = '计划', operationCode) {
    return getQuantityLabel(getPrintQuantityContext(order, qtyPurpose, operationCode));
}
export function formatPrintProcessQty(order, value, qtyPurpose = '计划', operationCode) {
...
export function renderBadge(label, tone = 'muted') {
...
export function renderWorkOrderStatusBadge(status) {
    const tone = status === 'FULL_HANDOVER'
        ? 'success'
...
export function renderReviewStatusBadge(status) {
    const tone = status === 'FULL_HANDOVER'
        ? 'success'
        : status === 'HANDOVER_DIFFERENCE'
            ? 'danger'
            : status === 'WAIT_RECEIVE'
                ? 'warning'
                : 'info';
    const label = status === 'FULL_HANDOVER'
        ? '全部交出'
        : status === 'HANDOVER_DIFFERENCE'
            ? '收货差异'
...
export function renderMetricCard(title, value, helper = '') {
    return `
...
export function renderSection(title, body, actionHtml = '') {
    return `
...
export function renderActionButton(input) {
    const toneClass = input.tone === 'primary'
...
export function getPrintNodeRecord(orderId, nodeCode) {
    return getPrintExecutionNodeRecord(orderId, nodeCode);
}
export function getPrintPrinterSummary(order) {
...
export function getPrintTransferSummary(order) {
