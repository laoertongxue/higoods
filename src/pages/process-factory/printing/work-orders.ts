...
const columns: StandardListColumn<PrintWorkOrder>[] = [
  ...
  { key: 'ready', title: '开工准备', width: 185, render: (order) => { const ready = getStartPrerequisiteByTaskId(order.taskId); return `<div class="space-y-1"><div class="font-medium">${escapeHtml(ready?.statusLabel || '按加工单状态判断')}</div><div class="text-xs text-muted-foreground">接收到位后开工</div></div>` } },
