...
export function printingWorkOrderTimeGroups(order: PrintingWorkOrderBusinessRecord): Array<{ title: string; fields: Array<[string, string]> }> {
  ...
  const creationFields: Array<[string, string]> = order.demandSource.type === 'DESIGN_REVISION'
    ? [['加工单创建', order.orderedAt || '历史未记录']]
    : [['需求创建', facts.demandCreatedAt || (order.demandSource.type === 'STOCK' ? '不适用（备货）' : '历史未记录')], ['生产单生成', facts.productionOrderCreatedAt || (order.demandSource.type === 'STOCK' ? '不适用（备货）' : '历史未记录')], ['加工单创建', order.orderedAt || '历史未记录']]
  return [
    { title: '单据创建', fields: creationFields },
