...
export function renderCraftPrintingStatisticsPage(): string {
  ...
  const factories = [...new Set(rows.map((row) => row.printFactoryName))].map((factory) => {
    ...
    return `<tr class="border-t"><td class="p-3">${escapeHtml(factory)}</td><td class="p-3 text-right">${data.orderCount}</td><td class="p-3 text-right">${escapeHtml(formatPrintingSummaryMetric(data, 'plannedInputQty'))}</td><td class="p-3 text-right">${escapeHtml(formatPrintingSummaryMetric(data, 'usedInputQty'))}</td><td class="p-3 text-right">${escapeHtml(formatPrintingSummaryMetric(data, 'completedOutputQty'))}</td><td class="p-3 text-right">${escapeHtml(formatPrintingSummaryMetric(data, 'handedOverQty'))}</td><td class="p-3 text-right">${escapeHtml(formatPrintingSummaryMetric(data, 'receivedQty'))}</td></tr>`
  }).join('')
  return `<div class="space-y-4 p-4"><header><h1 class="text-xl font-semibold">印花统计</h1><p class="mt-1 text-sm text-slate-500">加工状态与交出状态分别统计；不使用旧花型/打印/转印细分状态作为现场指标。</p></header>
    <section class="grid gap-3 md:grid-cols-3 xl:grid-cols-6">${metric('印花加工单', String(summary.orderCount))}${metric('计划投入', formatPrintingSummaryMetric(summary, 'plannedInputQty'))}${metric('实际使用', formatPrintingSummaryMetric(summary, 'usedInputQty'))}${metric('完成', formatPrintingSummaryMetric(summary, 'completedOutputQty'))}${metric('已交出', formatPrintingSummaryMetric(summary, 'handedOverQty'))}${metric('已接收', formatPrintingSummaryMetric(summary, 'receivedQty'))}</section>
    ...
    <section class="overflow-hidden rounded-xl border bg-white"><header class="border-b p-4"><h2 class="font-semibold">按加工厂</h2></header><div class="overflow-x-auto"><table class="min-w-[900px] w-full text-left text-sm"><thead class="bg-slate-50 text-xs text-slate-500"><tr><th class="p-3">加工厂</th><th class="p-3 text-right">单量</th><th class="p-3 text-right">计划投入（按单位）</th><th class="p-3 text-right">实际使用（按单位）</th><th class="p-3 text-right">完成（按单位）</th><th class="p-3 text-right">已交出（按单位）</th><th class="p-3 text-right">已接收（按单位）</th></tr></thead><tbody>${factories}</tbody></table></div></section>
  </div>`
}
