...
function renderCurrentActions(order: PrintingWorkOrderBusinessRecord): string {
  if (order.historicalInputQuantityUnknown) return actionButton('补录历史累计投入', 'receive-input', order, 'primary')
    + (['HANDOVER_WAIT_RECEIVE', 'PARTIAL_RECEIVED'].includes(order.handoverStatus) ? actionButton('接收', 'receive-handover', order) : '')
    + '<span class="self-center text-sm text-amber-800">历史投入未记录；已产出和原交接数量保留，补录前不能填报新的加工完成。</span>'
  ...
  const canReceiveOutput = ['PARTIAL_HANDOVER', 'HANDOVER_WAIT_RECEIVE', 'PARTIAL_RECEIVED'].includes(order.handoverStatus)
  const canCompleteDocument = order.handoverStatus === 'RECEIVED' && !isPrintingWorkOrderBusinessCompleted(order)
  const primaryAction = canHandOver ? 'handover' : canReceiveOutput ? 'receive-handover'
  ...
  if (canReceiveOutput) actions.push(actionButton('接收', 'receive-handover', order, tone('receive-handover')))
...
function renderInputIdentity(order: PrintingWorkOrderBusinessRecord): string {
  const specification = ['面料', '花边', '织带'].includes(order.plannedInput.objectType)
    ...
    : ''
  return `<div class="flex gap-4">${imageButton(order.plannedInput)}<div class="min-w-0 space-y-1 text-sm"><p class="font-semibold">[${escapeHtml(order.plannedInput.objectType)}] ${escapeHtml(order.plannedInput.materialName)}</p><p>SPU：${escapeHtml(order.plannedInput.spu)}</p><p class="break-all font-mono text-xs">计划 SKU：${escapeHtml(order.plannedInput.sku)}</p><p class="break-all font-mono text-xs">实际 SKU：${escapeHtml(order.actualInput.actualSku || (order.historicalInputQuantityUnknown ? '历史未记录' : '未接收'))}</p>${specification}</div></div>`
}
...
function renderOutputIdentity(order: PrintingWorkOrderBusinessRecord): string {
  const specification = ['面料', '花边', '织带'].includes(order.output.objectType)
    ...
    : ''
  return `<div class="flex gap-4">${imageButton(order.output)}<div class="min-w-0 space-y-1 text-sm"><p class="font-semibold">[${escapeHtml(order.output.objectType)}] ${escapeHtml(order.output.materialName)}</p><p>SPU：${escapeHtml(order.output.spu)}</p><p class="break-all font-mono text-xs font-medium text-emerald-700">产出 SKU：${escapeHtml(order.output.sku)}</p>${specification}<p class="text-xs text-slate-500">产出对象固定，投入换料不会自动改变</p></div></div>`
}
...
function renderBarcodes(order: PrintingWorkOrderBusinessRecord): string {
  ...
  return `<div class="overflow-x-auto"><table class="min-w-[900px] w-full text-left text-sm"><thead class="bg-slate-50 text-xs text-slate-500"><tr><th class="p-3">条码</th><th class="p-3">产出 SKU</th><th class="p-3">卷号</th><th class="p-3 text-right">数量(${escapeHtml(order.output.qtyUnit)})</th><th class="p-3 text-right">重量(KG)</th><th class="p-3">状态</th><th class="p-3">仓库/入库</th></tr></thead><tbody>${rows}</tbody></table></div>${order.barcodes.length > 5 ? `<p class="mt-3 text-xs text-slate-500">仅预览前 5 卷；共 ${order.barcodes.length} 卷。</p>` : ''}`
}
...
function renderDetail(order: PrintingWorkOrderBusinessRecord): string {
  ...
  return `<div class="space-y-4 p-4" data-printing-detail-workspace>
    ...
    ${section('3. 计划加工投入与实际加工投入', `<div class="grid gap-6 lg:grid-cols-2"><div><h3 class="mb-3 text-sm font-semibold">计划加工投入</h3>${renderInputIdentity(order)}<div class="mt-4 grid gap-3 sm:grid-cols-2">${field('计划投入', `${formatPrintingQty(order.plannedInput.plannedQty)} ${inputUnit}`)}${field('来源仓', order.plannedInput.sourceWarehouseName)}${field('当前库存', `${formatPrintingQty(order.plannedInput.currentStockQty)} ${inputUnit}`)}${field('待印数量', `${formatPrintingQty(order.plannedInput.pendingPrintQty)} ${inputUnit}`)}</div></div><div><h3 class="mb-3 text-sm font-semibold">实际加工投入</h3><div class="grid gap-3 sm:grid-cols-2">${field('实际 SKU', order.actualInput.actualSku || (order.historicalInputQuantityUnknown ? '历史未记录' : '未接收'))}${field('接收数量/卷', order.historicalInputQuantityUnknown ? '历史未记录' : `${formatPrintingQty(order.actualInput.receivedQty)} ${inputUnit} / ${order.actualInput.receivedRollCount} 卷`)}${field('使用数量/卷', `${formatPrintingQty(order.actualInput.usedQty)} ${inputUnit} / ${(order.historicalRollQuantitiesUnknown || (order.actualInput.usedQty > 0 && order.actualInput.usedRollCount === 0)) ? '历史卷数未记录' : `${order.actualInput.usedRollCount} 卷`}`)}${field('接收人', order.actualInput.receiverName)}${field('接收时间', order.actualInput.receivedAt || '—')}</div></div></div>`, order.output.completedQty === 0 ? actionButton('调整加工投入', 'change-input', order) : '')}
    ${section('4. 投入调整历史', renderChangeHistory(order))}
    ${section('5. 印花要求', renderRequirement(order))}
    ${section('6. 固定加工产出', `<div class="grid gap-6 lg:grid-cols-2">${renderOutputIdentity(order)}<div class="grid gap-4 sm:grid-cols-2">${field('计划完成', `${formatPrintingQty(order.output.plannedQty)} ${outputUnit}`)}${field('实际完成', `${formatPrintingQty(order.output.completedQty)} ${outputUnit}`)}${field('完成卷数', order.historicalRollQuantitiesUnknown ? '历史未记录' : `${order.output.completedRollCount} 卷`)}${field('加工状态', PRINTING_PROCESSING_STATUS_LABEL[order.processingStatus])}</div></div>`)}
    ${section('7. 数量与卷数', `<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">${field('计划投入', `${formatPrintingQty(order.plannedInput.plannedQty)} ${inputUnit}`)}${field('实际接收投入', order.historicalInputQuantityUnknown ? '历史未记录' : `${formatPrintingQty(order.actualInput.receivedQty)} ${inputUnit} / ${order.actualInput.receivedRollCount} 卷`)}${field('实际使用投入', `${formatPrintingQty(order.actualInput.usedQty)} ${inputUnit} / ${(order.historicalRollQuantitiesUnknown || (order.actualInput.usedQty > 0 && order.actualInput.usedRollCount === 0)) ? '历史卷数未记录' : `${order.actualInput.usedRollCount} 卷`}`)}${field('加工完成', `${formatPrintingQty(order.output.completedQty)} ${outputUnit} / ${order.historicalRollQuantitiesUnknown ? '历史卷数未记录' : `${order.output.completedRollCount} 卷`}`)}${field('已交出', `${formatPrintingQty(order.handover.handedOverQty)} ${outputUnit}`)}${field('已接收', `${formatPrintingQty(order.handover.receivedQty)} ${outputUnit}`)}${field('差异/异议', `${formatPrintingQty(order.handover.diffQty)} ${outputUnit} / ${order.handover.objectionQty} 条`)}${field('历史损耗', `${formatPrintingQty(order.historicalLossQty)} ${inputUnit}`, '仅兼容历史展示，不要求现场填报')}</div>`)}
    ${section('8. 加工厂与执行时间', `<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">${field('加工厂', `${order.printFactoryName}（${order.printFactoryId}）`)}${field('打印机', order.printerNo)}${field('下单时间', order.orderedAt)}${field('投入接收时间', order.inputReceivedAt || '—')}${field('加工完成时间', order.completedAt || '—')}${field('交货时间', order.deliveryAt || '—')}${field('转印完成数量', `${formatPrintingQty(order.transferCompletedQty)} ${outputUnit}`, '兼容执行事实，不作为状态')}${field('待接收数量', `${formatPrintingQty(order.pendingWritebackQty)} ${outputUnit}`)}</div>`)}
    ${section('9. 交出与接收', `<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">${field('交出状态', PRINTING_HANDOVER_STATUS_LABEL[order.handoverStatus])}${field('交出单', order.handover.handoverNo || '—')}${field('下游接收人', order.handover.receiverName)}${field('交出时间', order.handover.handedOverAt || '—')}${field('下游接收时间', order.handover.receivedAt || '—')}${field('累计交出', `${formatPrintingQty(order.handover.handedOverQty)} ${outputUnit}`)}${field('累计接收', `${formatPrintingQty(order.handover.receivedQty)} ${outputUnit}`)}${field('差异/说明', order.handover.differenceReason || '无')}</div>`, ['WAIT_HANDOVER', 'PARTIAL_HANDOVER'].includes(order.handoverStatus) ? actionButton('交出', 'handover', order) : ['HANDOVER_WAIT_RECEIVE', 'PARTIAL_RECEIVED'].includes(order.handoverStatus) ? actionButton('接收', 'receive-handover', order) : '')}
