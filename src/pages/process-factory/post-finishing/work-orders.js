import { buildPostFinishingWaitProcessWarehouseLink, buildPostFinishingWorkOrderDetailLink, buildUnifiedPrintPreviewRouteLink, } from '../../../data/fcs/fcs-route-links.ts';
import { completePostFinishingWorkOrder, listPostFinishingWorkOrders, } from '../../../data/fcs/post-finishing-domain.ts';
...
function filterRows(records, filters) {
    return records.filter((order) => {
        if (filters.status !== '全部' && order.currentStatus !== filters.status && order.postStatus !== filters.status)
            return false;
        if (filters.factory !== '全部' && order.managedPostFactoryName !== filters.factory)
            return false;
...
function renderPostItems(order) {
...
function registerPostWorkOrderActions() {
    if (typeof window === 'undefined')
        return;
    const win = window;
    win.__postCompleteWorkOrder = (postOrderId) => {
...
export function renderPostFinishingWorkOrdersPage() {
  ...
  `).join('');
    ...
    <div class="space-y-4 p-4">
      ...
      ${renderPostSection('后道单列表', `${renderPostTable(['后道单号', '关联质检单', '生产单', '来源工厂', '后道工厂', '款式衣服', 'SKU 明细', '后道项目', '后道数量', '后道状态', '最近更新', '操作'], rows || '<tr><td colspan="12" class="px-3 py-8 text-center text-sm text-muted-foreground">暂无后道单</td></tr>', 'min-w-[1500px]')}<div class="mt-4">${renderPostPagination(pagination)}</div>`)}
    </div>
  `;
}
