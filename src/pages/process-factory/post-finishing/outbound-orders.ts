...
export function renderPostFinishingOutboundOrdersPage(): string {
  ...
  return `<div class="space-y-4 p-4" data-testid="post-finishing-outbound-orders-page">${renderPostFinishingPageHeader('后道出货单', '复检完成自动且唯一生成；FCK 出货单号是仓库现场唯一收货身份')}<form class="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-3"><label class="text-xs text-muted-foreground">出货单 / 复检单 / 后道单 / 质检单 / 送货单 / 生产单<input name="keyword" value="${escapeHtml(query().get('keyword') || '')}" class="mt-1 h-9 w-full rounded-md border px-3 text-sm" /></label><label class="text-xs text-muted-foreground">实际接收人<input name="receiver" value="${escapeHtml(query().get('receiver') || '')}" class="mt-1 h-9 w-full rounded-md border px-3 text-sm" /></label><label class="text-xs text-muted-foreground">差异授权人<input name="authorizer" value="${escapeHtml(query().get('authorizer') || '')}" class="mt-1 h-9 w-full rounded-md border px-3 text-sm" /></label><button class="h-9 rounded-md border text-sm md:col-span-3">筛选</button></form><div class="overflow-x-auto rounded-xl border bg-card"><table class="min-w-[1100px] w-full text-left text-sm"><thead class="bg-slate-50 text-xs text-muted-foreground"><tr><th class="px-3 py-2">后道出货单</th><th class="px-3 py-2">复检单 / 生产单</th><th class="px-3 py-2">出货数量</th><th class="px-3 py-2">实际接收人</th><th class="px-3 py-2">差异授权人</th><th class="px-3 py-2">状态</th><th class="px-3 py-2">操作</th></tr></thead><tbody class="divide-y">${rows || '<tr><td colspan="7" class="p-10 text-center text-muted-foreground">暂无后道出货单。</td></tr>'}</tbody></table></div></div>`
}
