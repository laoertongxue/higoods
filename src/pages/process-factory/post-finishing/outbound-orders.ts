...
function image(line: { sku: Pick<OutboundRecord['lines'][number]['sku'], 'skuCode' | 'colorName' | 'sizeName' | 'imageUrl' | 'spuName'> }): string {
...
function renderDetail(record: NonNullable<ReturnType<typeof getPostFinishingFullFlowOutboundOrder>>): string {
  ...
  const directFromQc = record.sourceType === '质检直达'
  const sourceLocation = directFromQc ? `${factoryName}-质检交接区` : `${factoryName}-后道待交出仓`
  const sourceAction = directFromQc ? '质检完成 → 成衣仓待接收' : '处理后数量条码复核 → 后道待交出仓 → 成衣仓待接收'
  const traceNodes = [record.deliveryOrderNo, record.qcTaskNo, record.postTaskNo, record.recheckOrderNo, record.outboundOrderNo].filter(Boolean)
  const rows = record.lines.map((line) => `<tr>
    <td class="px-3 py-3">${record.lines.indexOf(line) + 1}</td><td class="px-3 py-3">${image(line)}</td><td class="px-3 py-3"><span class="rounded border border-blue-200 bg-blue-50 px-2 py-1 text-xs text-blue-700">成品</span></td><td class="px-3 py-3 font-mono text-xs">${escapeHtml(line.sku.skuCode)}</td><td class="px-3 py-3">${escapeHtml(line.sku.spuName)}</td><td class="px-3 py-3">${escapeHtml(line.sku.colorName)}</td><td class="px-3 py-3">${escapeHtml(line.sku.sizeName)}</td><td class="px-3 py-3">—</td><td class="px-3 py-3">—</td><td class="px-3 py-3">—</td><td class="px-3 py-3">${line.outboundQty}</td><td class="px-3 py-3">${line.receivedQty ?? 0}</td><td class="px-3 py-3">件</td><td class="px-3 py-3">${displayStatus === '已确认' ? '成衣仓已确认接收' : '待成衣仓确认接收'}</td>
  </tr>`).join('')
  return `<div class="space-y-4">
    <section class="rounded-xl border bg-card p-4"><h3 class="font-semibold">单头信息</h3><dl class="mt-4 grid gap-x-8 gap-y-3 text-sm md:grid-cols-2"><div><dt class="text-xs text-muted-foreground">出货单号</dt><dd class="font-mono">${escapeHtml(record.outboundOrderNo)}</dd></div><div><dt class="text-xs text-muted-foreground">状态</dt><dd>${renderPostStatusBadge(displayStatus)}</dd></div><div><dt class="text-xs text-muted-foreground">工厂</dt><dd>${escapeHtml(factoryName)}</dd></div><div><dt class="text-xs text-muted-foreground">交出位置</dt><dd>${escapeHtml(sourceLocation)}</dd></div><div><dt class="text-xs text-muted-foreground">接收仓</dt><dd>成衣仓</dd></div><div><dt class="text-xs text-muted-foreground">来源动作</dt><dd>${escapeHtml(sourceAction)}</dd></div><div><dt class="text-xs text-muted-foreground">任务范围 / 后道责任</dt><dd>${escapeHtml(`${record.responsibility.taskTypeLabel} · ${record.responsibility.responsibilityLabel}`)}</dd></div><div><dt class="text-xs text-muted-foreground">交接来源</dt><dd>${escapeHtml(record.sourceType)}</dd></div><div><dt class="text-xs text-muted-foreground">生产单号</dt><dd class="font-mono">${escapeHtml(record.productionOrderNo)}</dd></div><div><dt class="text-xs text-muted-foreground">任务单号</dt><dd class="font-mono">${escapeHtml(delivery?.sewingTaskNo || '—')}</dd></div><div><dt class="text-xs text-muted-foreground">来源对象</dt><dd>${directFromQc ? '质检单' : '处理后交出复核单'}</dd></div><div><dt class="text-xs text-muted-foreground">来源单号</dt><dd class="font-mono">${escapeHtml(directFromQc ? record.qcTaskNo : record.recheckOrderNo || '—')}</dd></div><div><dt class="text-xs text-muted-foreground">后道加工单</dt><dd class="font-mono">${escapeHtml(record.postTaskNo || '不适用（质检直达成衣仓）')}</dd></div><div><dt class="text-xs text-muted-foreground">库区 / 库位</dt><dd>${escapeHtml(directFromQc ? '不进入后道待交出仓' : `${waitHandover?.areaName || '后道待交出仓'} / ${waitHandover?.locationCode || '待分配'}`)}</dd></div><div><dt class="text-xs text-muted-foreground">应交数量</dt><dd>${record.lines.reduce((sum, line) => sum + line.outboundQty, 0)} 件</dd></div><div><dt class="text-xs text-muted-foreground">成衣仓实收</dt><dd>${record.lines.reduce((sum, line) => sum + (line.receivedQty || 0), 0)} 件</dd></div><div><dt class="text-xs text-muted-foreground">生成时间</dt><dd>${escapeHtml(new Date(record.createdAt).toLocaleString('zh-CN'))}</dd></div><div><dt class="text-xs text-muted-foreground">成衣仓接收人</dt><dd>${escapeHtml(record.receivedBy?.actorName || '待接收')}</dd></div><div class="md:col-span-2"><dt class="text-xs text-muted-foreground">备注</dt><dd>${escapeHtml(receipt?.differenceReason || (directFromQc ? 'QC 确认无需我方后道处理；仅生成成衣仓待接收事实，尚未代收' : '处理后数量与条码复核完成后生成'))}</dd></div><div class="md:col-span-2"><dt class="text-xs text-muted-foreground">追溯链路</dt><dd class="font-mono text-xs">${traceNodes.map(escapeHtml).join(' → ')}</dd></div></dl></section>
...
const outboundColumns: StandardListColumn<OutboundRow>[] = [
  ...
  { key: 'source', title: '交接来源', width: 120, required: true, render: (row) => `<div class="text-xs font-medium">${escapeHtml(row.record.sourceType)}</div><div class="mt-1 text-[11px] text-muted-foreground">${escapeHtml(row.record.postTaskNo || '质检直达成衣仓')}</div>` },
...
// schema 1 仅用于旧链接只读查看；历史链接迁移验收完成后连同 archive 键删除。
// 不加入当前列表、数量统计、出货和收货命令，也不以旧记录补造当前业务事实。
function renderArchivedOutbound(id: string): string {
  try {
    const archive = JSON.parse(globalThis.localStorage?.getItem('higood-fcs-post-finishing-outbound-orders-v1:archive') || 'null')
    if (archive?.schemaVersion !== 1) return ''
    const records: unknown = JSON.parse(archive.raw)
    if (!Array.isArray(records)) return ''
    const record = records.find((item) => item && (item.outboundOrderId === id || item.outboundOrderNo === id) && Array.isArray(item.lines))
    if (!record) return ''
    const text = (value: unknown) => escapeHtml(String(value ?? '—'))
    const rows = record.lines.map((line: Record<string, unknown>) => `<tr>
      <td class="p-3"><div class="flex items-center gap-2">${line.skuImageUrl ? image({ sku: { imageUrl: String(line.skuImageUrl), skuCode: String(line.skuCode || ''), spuName: String(line.spuName || ''), colorName: String(line.colorName || ''), sizeName: String(line.sizeName || '') } }) : '<span class="text-amber-700">历史图片缺失</span>'}<span>${text(line.spuName)}<br>${text(line.skuCode)}</span></div></td>
      <td class="p-3">${text(line.colorName)} / ${text(line.sizeName)}</td><td class="p-3">${text(line.plannedQty)} ${text(line.qtyUnit)}</td><td class="p-3">${text(line.inboundQty)} ${text(line.qtyUnit)}</td>
    </tr>`).join('')
    return `<section class="space-y-4 rounded-xl border bg-white p-4" data-testid="archived-post-outbound">
      <h2 class="font-semibold">历史出货单 · ${text(record.outboundOrderNo || record.outboundOrderId)}</h2>
      <p class="text-sm text-amber-700">历史记录仅供查看。数量和状态保留原记录，不参与当前出货或入库。</p>
      <p class="text-sm">生产单：${text(record.productionOrderNo)}　原状态：${text(record.status)}　来源复检单：${text(record.recheckOrderNo)}</p>
      <div class="overflow-x-auto"><table class="w-full text-sm"><thead><tr class="border-b text-left"><th class="p-3">款式 / SKU</th><th class="p-3">颜色 / 尺码</th><th class="p-3">原出货数量</th><th class="p-3">原入库数量</th></tr></thead><tbody>${rows}</tbody></table></div>
    </section>`
  } catch { return '' }
}
...
export function renderPostFinishingOutboundOrderDetailPage(id: string): string {
  ...
  return `<div class="space-y-4 p-4">${renderPostFinishingPageHeader('后道出货单详情', record?.outboundOrderNo || '', actions)}${record ? renderDetail(record) : renderArchivedOutbound(id) || '<div class="rounded-xl border border-red-200 bg-red-50 p-6 text-red-700">未找到后道出货单。</div>'}</div>`
}
