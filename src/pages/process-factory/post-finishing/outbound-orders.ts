...
function renderDetail(record: NonNullable<ReturnType<typeof getPostFinishingFullFlowOutboundOrder>>): string {
  ...
  return `<div class="space-y-4">
    <section class="rounded-xl border bg-card p-4"><h3 class="font-semibold">单头信息</h3><dl class="mt-4 grid gap-x-8 gap-y-3 text-sm md:grid-cols-2"><div><dt class="text-xs text-muted-foreground">出货单号</dt><dd class="font-mono">${escapeHtml(record.outboundOrderNo)}</dd></div><div><dt class="text-xs text-muted-foreground">状态</dt><dd>${renderPostStatusBadge(displayStatus)}</dd></div><div><dt class="text-xs text-muted-foreground">工厂</dt><dd>${escapeHtml(factoryName)}</dd></div><div><dt class="text-xs text-muted-foreground">出库仓</dt><dd>${escapeHtml(`${factoryName}-后道待加工仓`)}</dd></div><div><dt class="text-xs text-muted-foreground">接收仓</dt><dd>${escapeHtml(`${factoryName}-后道待交出仓`)}</dd></div><div><dt class="text-xs text-muted-foreground">来源动作</dt><dd>复检完成 → 后道待交出仓</dd></div><div><dt class="text-xs text-muted-foreground">任务范围 / 后道责任</dt><dd>${escapeHtml(`${record.responsibility.taskTypeLabel} · ${record.responsibility.responsibilityLabel}`)}</dd></div><div><dt class="text-xs text-muted-foreground">复检来源</dt><dd>${escapeHtml(record.sourceType)}</dd></div><div><dt class="text-xs text-muted-foreground">生产单号</dt><dd class="font-mono">${escapeHtml(record.productionOrderNo)}</dd></div><div><dt class="text-xs text-muted-foreground">任务单号</dt><dd class="font-mono">${escapeHtml(delivery?.sewingTaskNo || '—')}</dd></div><div><dt class="text-xs text-muted-foreground">来源对象</dt><dd>复检单</dd></div><div><dt class="text-xs text-muted-foreground">交接单号</dt><dd class="font-mono">${escapeHtml(record.recheckOrderNo)}</dd></div><div><dt class="text-xs text-muted-foreground">后道加工单</dt><dd class="font-mono">${escapeHtml(record.postTaskNo || '不适用（质检直达复检）')}</dd></div><div><dt class="text-xs text-muted-foreground">库区 / 库位</dt><dd>${escapeHtml(`${waitHandover?.areaName || '后道待交出仓'} / ${waitHandover?.locationCode || '待分配'}`)}</dd></div><div><dt class="text-xs text-muted-foreground">计划数量</dt><dd>${record.lines.reduce((sum, line) => sum + line.outboundQty, 0)} 件</dd></div><div><dt class="text-xs text-muted-foreground">入库数量</dt><dd>${record.lines.reduce((sum, line) => sum + (line.receivedQty || 0), 0)} 件</dd></div><div><dt class="text-xs text-muted-foreground">出库时间</dt><dd>${escapeHtml(new Date(record.createdAt).toLocaleString('zh-CN'))}</dd></div><div><dt class="text-xs text-muted-foreground">操作人</dt><dd>${escapeHtml(record.receivedBy?.actorName || '系统自动生成')}</dd></div><div class="md:col-span-2"><dt class="text-xs text-muted-foreground">备注</dt><dd>${escapeHtml(receipt?.differenceReason || '复检完成后自动生成')}</dd></div><div class="md:col-span-2"><dt class="text-xs text-muted-foreground">追溯链路</dt><dd class="font-mono text-xs">${escapeHtml(record.deliveryOrderNo)} → ${escapeHtml(record.qcTaskNo)} → ${escapeHtml(record.postTaskNo || '加工单不适用')} → ${escapeHtml(record.recheckOrderNo)} → ${escapeHtml(record.outboundOrderNo)}</dd></div></dl></section>
...
const outboundColumns: StandardListColumn<OutboundRow>[] = [
  ...
  { key: 'source', title: '复检来源', width: 110, required: true, render: (row) => `<div class="text-xs font-medium">${escapeHtml(row.record.sourceType)}</div><div class="mt-1 text-[11px] text-muted-foreground">${escapeHtml(row.record.postTaskNo || '加工单不适用')}</div>` },
