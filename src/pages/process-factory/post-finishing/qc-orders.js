import { buildPostFinishingQcDeductionRecord, completePostFinishingQcOrder, createPostFinishingQcOrder, getPostFinishingTaskById, listPostFinishingQcOrders, listPostFinishingWaitQcSkuItems, } from '../../../data/fcs/post-finishing-domain.ts';
...
function registerQcPageActions() {
    if (typeof window === 'undefined')
        return;
    const win = window;
    win.__postCreateQcOrder = () => {
        const postTaskId = getCurrentPostTaskId() || undefined;
        const station = document.querySelector('[data-qc-create-station]')?.value || '后道质检台 A';
        const allocations = Array.from(document.querySelectorAll('[data-qc-source-row]'))
            .filter((row) => row.querySelector('[data-qc-source-check]')?.checked)
            .map((row) => ({
            warehouseRecordId: row.dataset.warehouseRecordId || '',
            qcQty: Number(row.querySelector('[data-qc-source-qty]')?.value || 0),
        }))
    ...
    win.__postCompleteQcOrder = (qcOrderId) => {
        ...
        const completed = completePostFinishingQcOrder({
            ...
            qcResult: result,
            unqualifiedDisposition: result === '全数合规' ? '' : valueOf('[data-qc-disposition]'),
            unqualifiedReasonSummary: result === '全数合规' ? '' : valueOf('[data-qc-reason]'),
            rootCauseType: result === '全数合规' ? '' : valueOf('[data-qc-root-cause]'),
            responsiblePartyType: result === '全数合规' ? '' : valueOf('[data-qc-responsible-party]'),
            responsiblePartyName: result === '全数合规' ? '' : valueOf('[data-qc-responsible-name]'),
            deductionDecision: result === '全数合规' ? '' : valueOf('[data-qc-deduction-decision]'),
    ...
    win.__syncQcCompleteForm = (select) => {
        const allGood = select.value === '全数合规';
        document.querySelectorAll('[data-qc-defect-section], [data-qc-liability-section]').forEach((section) => {
            section.hidden = allGood;
        });
        document.querySelectorAll('[data-qc-defect-field]').forEach((field) => {
...
function renderPageHeader() {
    ...
    return renderPostFinishingPageHeader('质检单', task ? `${task.postTaskNo} / ${task.productionOrderNo}` : '', `<button type="button" class="rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700" data-nav="${escapeHtml(linkWith({ createQc: '1', completeQc: undefined, viewQc: undefined }))}">创建质检单</button>`);
}
...
function normalizeResult(value) {
    if (value === '全数合格' || value === '全数合规')
        return '全数合规';
    if (value === '全数不合格')
        return '全数不合格';
    return '部分不合格';
}
function renderSelect(label, options, selected = '', attrs = '') {
    return `
    <label class="space-y-1 text-sm">
...
function renderInput(label, value = '', attrs = '') {
    return `
    <label class="space-y-1 text-sm">
      <span class="text-xs text-muted-foreground">${escapeHtml(label)}</span>
...
function renderPostNeeds(record, readonly = false) {
...
function renderCompleteQcDialog() {
    const recordId = getCompleteId();
    if (!recordId)
        return '';
    const record = listPostFinishingQcOrders().find((item) => item.actionRecordId === recordId);
    if (!record)
        return renderModal('完成质检', '<div class="text-sm text-muted-foreground">未找到质检单</div>');
...
function renderPrintButton(record) {
...
function renderViewDialog() {
    ...
    const record = listPostFinishingQcOrders().find((item) => item.actionRecordId === recordId);
    if (!record)
        return renderModal('质检单详情', '<div class="text-sm text-muted-foreground">未找到质检单</div>');
    const snapshot = buildPostFinishingQcDeductionRecord(record);
...
function filterQc(records, filters) {
    return records.filter((record) => {
        if (filters.source !== '全部' && filters.source !== '质检单')
            return false;
        if (filters.status !== '全部' && record.status !== filters.status)
            return false;
        if (filters.factory !== '全部' && record.targetFactoryName !== filters.factory)
            return false;
...
function filterWaitQc(records, filters) {
    return records.filter((record) => {
        if (filters.source !== '全部' && record.sourceFactoryType !== filters.source)
            return false;
        if (filters.factory !== '全部' && record.sourceFactoryName !== filters.factory)
            return false;
...
export function renderPostFinishingQcOrdersPage() {
    registerQcPageActions();
    const filters = getPostListFilters();
    const postTaskId = getCurrentPostTaskId() || undefined;
    const waitItems = listPostFinishingWaitQcSkuItems({ postTaskId });
    const allQc = listPostFinishingQcOrders().filter((record) => (!postTaskId || record.warehouseAllocations?.some((allocation) => allocation.postTaskId === postTaskId)));
    ...
    const activeSection = activeTab === 'wait'
        ? renderPostSection('待质检列表', `${renderPostTable(['SKU', '款式衣服', '生产单 / 上游任务', '上游工厂', '库区 / 库位', '当前库存', '待质检数量', '质检中数量', '操作'], waitRows || '<tr><td colspan="9" class="px-3 py-8 text-center text-sm text-muted-foreground">暂无待质检库存</td></tr>', 'min-w-[1280px]')}<div class="mt-4">${renderPostPagination(waitPagination)}</div>`)
        : renderPostSection('质检单列表', `${renderPostTable(['质检单号', '后道单号', '生产单', '来源工厂', '质检台', '款式衣服', 'SKU 明细', '质检数量', '合格数量', '不合格数量', '质检结果', '责任方', '扣款决策', '状态', '质检人', '操作'], qcRows || '<tr><td colspan="16" class="px-3 py-8 text-center text-sm text-muted-foreground">暂无质检单</td></tr>', 'min-w-[1800px]')}<div class="mt-4">${renderPostPagination(pagination)}</div>`);
    return `
