      ...
      <section class="w-full max-w-2xl rounded-lg border bg-background shadow-2xl">
        ...
        <div class="grid gap-3 p-4 md:grid-cols-2">
          ...
          <label class="text-sm">
            ...
            <input class="mt-1 h-9 w-full rounded-md border bg-background px-3 text-sm" value="${escapeHtml(input.managerName || DEFAULT_POST_FINISHING_WAREHOUSE_ACCOUNT)}" data-post-warehouse-form-field="managerName" />
          </label>
  ...
  modal.addEventListener('click', (event) => {
    ...
    if (action === 'submit-area') {
      ...
      upsertPostFinishingWarehouseArea({
        ...
        areaCode,
        areaName,
        managerName: readWarehouseFormField(modal, 'managerName') || DEFAULT_POST_FINISHING_WAREHOUSE_ACCOUNT,
...
export function handlePostFinishingEvent(target: HTMLElement, event?: Event): boolean {
  ...
  if (action === 'add-area') {
    ...
    openPostFinishingWarehouseAreaDialog({
      ...
      managerName: DEFAULT_POST_FINISHING_WAREHOUSE_ACCOUNT,
      remark: 'Web新增库区',
    })
    return true
  }
  ...
  if (action === 'edit-area') {
    ...
    openPostFinishingWarehouseAreaDialog({
      ...
      managerName: actionNode.dataset.managerName || DEFAULT_POST_FINISHING_WAREHOUSE_ACCOUNT,
