...
import { confirmPostFinishingWarehouseReceipt, deletePostFinishingWarehouseLocation, listPostFinishingUpstreamHandovers, listPostFinishingWarehouseAreas, listPostFinishingWarehouseLocations, lookupPostFinishingUpstreamHandover, upsertPostFinishingWarehouseArea, upsertPostFinishingWarehouseLocation, } from '../../../data/fcs/post-finishing-domain.ts';
...
function showPostFinishingToast(message) {
...
function openPostFinishingWarehouseAreaDialog(input) {
  ...
  `);
    ...
    modal.addEventListener('click', (event) => {
        const action = event.target.closest('[data-post-warehouse-form-action]')?.dataset.postWarehouseFormAction;
        if (!action)
            return;
        if (action === 'close') {
            removePostFinishingWarehouseFormDialog();
...
function renderReceiptAreaOptions(selectedAreaId = '') {
...
function renderReceiptLocationOptions(areaId, selectedLocationId = '') {
...
function renderReceiptDetails(handover) {
    if (!handover) {
        return `
...
function loadReceiptDetails(modal) {
    const keyword = modal.querySelector('[data-post-receipt-field="handoverNo"]')?.value.trim() || '';
    const details = modal.querySelector('[data-post-receipt-details]');
    if (!details)
        return;
    const handover = lookupPostFinishingUpstreamHandover(keyword);
    details.innerHTML = renderReceiptDetails(handover);
    if (!handover)
        showPostFinishingToast('未找到该交出记录，请核对编号或二维码。');
}
function syncReceiptLocationSelect(row) {
    const areaId = row.querySelector('[data-post-receipt-field="areaId"]')?.value || '';
    const locationSelect = row.querySelector('[data-post-receipt-field="locationId"]');
    if (!locationSelect)
        return;
    locationSelect.innerHTML = renderReceiptLocationOptions(areaId);
}
function openPostFinishingReceiptDialog() {
  ...
  `);
    ...
    modal.addEventListener('change', (event) => {
        const field = event.target.closest('[data-post-receipt-field]')?.dataset.postReceiptField;
        if (field !== 'areaId')
            return;
        const row = event.target.closest('[data-post-receipt-line-id]');
        if (row)
            syncReceiptLocationSelect(row);
    });
    modal.addEventListener('click', (event) => {
        const actionNode = event.target.closest('[data-post-receipt-action]');
        ...
        if (action === 'fill-handover') {
            const input = modal.querySelector('[data-post-receipt-field="handoverNo"]');
            if (input)
                input.value = actionNode.dataset.handoverNo || '';
        ...
        if (action === 'scan-demo') {
            const input = modal.querySelector('[data-post-receipt-field="handoverNo"]');
            if (input)
                input.value = handovers[0]?.qrCode || '';
            loadReceiptDetails(modal);
            return;
        }
        if (action === 'load') {
        ...
        if (action === 'submit') {
            ...
            const handoverNo = modal.querySelector('[data-post-receipt-field="handoverNo"]')?.value.trim() || '';
            const rows = Array.from(modal.querySelectorAll('[data-post-receipt-line-id]'));
            const lines = rows.map((row) => ({
                handoverLineId: row.dataset.postReceiptLineId || '',
                actualQty: Number(row.querySelector('[data-post-receipt-field="actualQty"]')?.value || 0),
                areaId: row.querySelector('[data-post-receipt-field="areaId"]')?.value || '',
                locationId: row.querySelector('[data-post-receipt-field="locationId"]')?.value || '',
            }));
            try {
...
export function handlePostFinishingEvent(target) {
    ...
    if (dialogResult !== null)
        return dialogResult;
    const actionNode = target.closest('[data-post-finishing-action]');
    if (!actionNode)
        return false;
