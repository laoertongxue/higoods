...
import { completePostFinishingWorkOrder, getPostFinishingSourceLabel, getPostFinishingWorkOrderById, } from '../../../data/fcs/post-finishing-domain.ts';
import { appStore } from '../../../state/store.ts';
import { escapeHtml } from '../../../utils.ts';
import { formatGarmentQty, renderPostAction, renderPostFinishingPageHeader, renderPostSection, renderPostStatusBadge, renderPostTable, } from './shared.ts';
const DETAIL_TABS = [
    { key: 'base', label: '基本信息' },
...
function getCurrentTab() {
    ...
    return DETAIL_TABS.some((tab) => tab.key === value) ? value : 'base';
}
function buildDetailHref(postOrderId, tab) {
    return `/fcs/craft/post-finishing/work-orders/${encodeURIComponent(postOrderId)}?tab=${tab}`;
}
function renderTabs(postOrderId, activeTab) {
    return `
...
function registerPostWorkOrderDetailActions() {
    if (typeof window === 'undefined')
        return;
    const win = window;
    win.__completePostFinishingWorkOrderFromDetail = (postOrderId) => {
    ...
    win.__reportPostFinishingWorkOrderException = (postOrderNo) => {
...
function renderPostItemRows(order) {
    const itemSet = new Set(order.postProcessItems);
    const rows = ['开扣眼', '装扣子', '熨烫', '包装'].map((item) => `
    <tr class="align-top">
...
function renderResultRows(order) {
...
function renderTabBody(order) {
    const activeTab = getCurrentTab();
    if (activeTab === 'sku') {
        return renderPostSection('SKU 明细', renderPostTable(['SKU', '款式衣服', '颜色', '尺码', '后道数量'], renderSkuRows(order) || renderEmptyRow(5, '暂无 SKU 明细'), 'min-w-[980px]'));
    }
    if (activeTab === 'items') {
        return renderPostSection('后道项目', renderPostTable(['后道项目', '是否需要'], renderPostItemRows(order), 'min-w-[560px]'));
    }
    if (activeTab === 'result') {
        return renderPostSection('执行结果', renderPostTable(['后道状态', '操作人', '开始时间', '完成时间', '后道数量', '完成数量', '异常数量', '备注'], renderResultRows(order), 'min-w-[1160px]'));
    }
    const baseRows = [
...
export function renderPostFinishingWorkOrderDetailPage(postOrderId) {
