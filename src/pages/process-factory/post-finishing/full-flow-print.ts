...
function outboundDocumentPage(outbound: NonNullable<ReturnType<typeof getPostFinishingFullFlowOutboundOrder>>): string {
  ...
  return `<div class="min-h-screen bg-slate-100 p-4 print:bg-white print:p-0" data-testid="post-finishing-full-flow-print" data-print-document-no="${escapeHtml(outbound.outboundOrderNo)}"><div class="mx-auto max-w-[210mm] bg-white p-[12mm] shadow print:shadow-none" data-print-sheet="a4"><div class="flex items-start justify-between gap-4 border-b-2 border-black pb-4"><div><h1 class="text-2xl font-bold">后道出货单</h1><div class="mt-2 font-mono text-lg font-semibold">${escapeHtml(outbound.outboundOrderNo)}</div></div><div class="w-44" data-business-document-barcode="${escapeHtml(outbound.outboundOrderNo)}">${renderCode128Barcode(outbound.outboundOrderNo, '后道出货单号条码')}</div></div><table class="mt-4 w-full border-collapse text-xs"><tbody>${[
    ...
    ['任务范围', `${outbound.responsibility.taskTypeLabel} · ${outbound.responsibility.responsibilityLabel}`, '复检来源', outbound.sourceType],
    ['后道加工单', outbound.postTaskNo || '不适用（质检直达复检）', '复检单', outbound.recheckOrderNo],
