...
export function renderPostFinishingQcOrdersPage(): string {
  ...
  return `<div class="space-y-4 p-4" data-testid="post-finishing-qc-management-page">
    ...
    <section class="rounded-xl border bg-card p-4"><div><h2 class="font-semibold">质检任务管理</h2><p class="mt-1 text-xs text-muted-foreground">主管可筛选全部任务并释放错误占用；质检员从本页输入任务号领取。</p></div><form class="mt-4 grid gap-3 md:grid-cols-3" action="/fcs/craft/post-finishing/qc-orders"><label class="text-xs text-muted-foreground">单号 / 生产单<input name="keyword" value="${escapeHtml(params().get('keyword') || '')}" class="mt-1 h-9 w-full rounded-md border px-3 text-sm" /></label><label class="text-xs text-muted-foreground">状态<select name="status" class="mt-1 h-9 w-full rounded-md border px-3 text-sm"><option value="">全部</option>${['待质检','质检中','质检完成'].map((value) => `<option ${value === status ? 'selected' : ''}>${value}</option>`).join('')}</select></label><label class="text-xs text-muted-foreground">当前质检员<input name="owner" value="${escapeHtml(params().get('owner') || '')}" class="mt-1 h-9 w-full rounded-md border px-3 text-sm" /></label><button class="h-9 rounded-md border px-4 text-sm md:col-span-3">筛选</button></form></section>
...
export function handlePostFinishingQcOrdersEvent(target: HTMLElement, event?: Event): boolean {
  ...
  try {
    if (action === 'full-flow-claim-qc-from-list') {
      ...
      appStore.navigate(`/fcs/craft/post-finishing/qc-workbench?actor=${actor}&taskNo=${encodeURIComponent(claimed.qcTaskNo)}`)
      return true
    }
