...
const columns: StandardListColumn<RecheckOrderRow>[] = [
  ...
  { key: 'source', title: '来源', width: 80, required: true, render: () => '后道加工单' },
  ...
  { key: 'post', title: '关联后道加工单', width: 150, required: true, render: (row) => `<span class="break-all font-mono ${row.record.postTaskNo ? 'text-blue-700' : 'text-muted-foreground'}">${escapeHtml(row.record.postTaskNo || '—')}</span>` },
  ...
  { key: 'actions', title: '操作', width: 230, required: true, actionColumn: true, render: (row) => {
    ...
    return `<div class="grid grid-cols-2 gap-x-3 gap-y-2"><a data-nav="/fcs/craft/post-finishing/recheck-orders?id=${encodeURIComponent(row.record.recheckOrderId)}" class="whitespace-nowrap text-xs font-medium text-blue-700 hover:underline">${isOwner && row.record.status !== '复检完成' ? '执行复检' : '查看复检详情'}</a><a data-nav="/fcs/craft/post-finishing/print?type=QC_ORDER&id=${encodeURIComponent(row.record.qcTaskNo)}" class="whitespace-nowrap text-xs text-blue-600 hover:underline">打印质检单</a><a data-nav="/fcs/craft/post-finishing/print?type=QC_DETAIL&id=${encodeURIComponent(row.record.qcTaskNo)}" class="whitespace-nowrap text-xs text-blue-600 hover:underline">打印质检单详情</a>${isOwner && row.record.status !== '复检完成' ? `<button type="button" class="whitespace-nowrap text-left text-xs text-amber-700 hover:underline" data-post-finishing-recheck-action="release" data-recheck-id="${escapeHtml(row.record.recheckOrderId)}">退领</button>` : ''}${isSupervisor && row.record.claimedBy && row.record.status !== '复检完成' ? `<button type="button" class="whitespace-nowrap text-left text-xs text-amber-700 hover:underline" data-post-finishing-action="full-flow-supervisor-release-recheck" data-recheck-id="${escapeHtml(row.record.recheckOrderId)}">主管释放</button>` : ''}</div>`
  } },
]
...
export function renderPostFinishingRecheckOrderDetailPage(recheckOrderId: string): string {
  ...
  const headerActions = record ? `<div class="flex flex-wrap items-center justify-end gap-2">${renderPostFinishingQcPrintActions(record.qcTaskNo)}${record.claimedBy?.actorId === currentRechecker().actorId && record.status !== '复检完成' ? `<button type="button" class="inline-flex h-9 items-center rounded-md border border-amber-300 px-3 text-sm text-amber-800" data-post-finishing-recheck-action="release" data-recheck-id="${escapeHtml(record.recheckOrderId)}">退领复检单</button>` : ''}<a data-nav="/fcs/craft/post-finishing/recheck-orders" class="inline-flex h-9 items-center rounded-md border bg-white px-3 text-sm">返回复检单列表</a></div>` : `<a data-nav="/fcs/craft/post-finishing/recheck-orders" class="inline-flex h-9 items-center rounded-md border bg-white px-3 text-sm">返回复检单列表</a>`
...
export function renderPostFinishingRecheckOrdersPage(): string {
  ...
  return renderStandardListPage({
    title: '复检单',
    primaryActionsHtml: `<div class="flex flex-wrap items-center justify-end gap-2">${renderPostFinishingQcPrintActions()}<button type="button" class="inline-flex h-9 items-center rounded-md bg-blue-600 px-4 text-sm font-medium text-white" data-post-finishing-recheck-action="open-claim" data-skip-page-rerender="true">领取复检单</button></div>`,
