import { buildPostFinishingTaskLink, buildTaskRouteCardPrintLink, } from '../../../data/fcs/fcs-route-links.ts';
import { getPostFinishingTaskById, listPostFinishingQcOrderEntities, listPostFinishingRecheckOrderEntities, listPostFinishingTasks, listPostFinishingWaitQcSkuItems, listPostFinishingWorkOrders, } from '../../../data/fcs/post-finishing-domain.ts';
import { escapeHtml } from '../../../utils.ts';
import { formatGarmentQty, getPostListFilters, paginatePostRows, postFilterTextMatches, renderPostAction, renderPostFilterPanel, renderPostFinishingPageHeader, renderPostMetricCard, renderPostPagination, renderPostSection, renderPostStatusBadge, renderPostTable, } from './shared.ts';
function currentParams() {
    return typeof window === 'undefined' ? new URLSearchParams() : new URLSearchParams(window.location.search);
}
function createQcLink(task) {
    return `/fcs/craft/post-finishing/qc-orders?postTaskId=${encodeURIComponent(task.postTaskId)}&createQc=1`;
}
function taskQcListLink(task) {
...
function filterTasks(tasks, filters) {
    return tasks.filter((task) => {
        ...
        if (filters.status !== '全部' && task.currentStatus !== filters.status)
            return false;
        if (filters.source !== '全部' && sourceLabel !== filters.source)
            return false;
        if (filters.factory !== '全部' && task.managedPostFactoryName !== filters.factory)
            return false;
...
export function renderPostFinishingTasksPage() {
    ...
    <div class="space-y-4 p-4">
      ...
      ${renderPostSection('后道任务列表', `${renderPostTable(['后道任务', '生产单 / 技术包', '款式衣服', '计划数量', '未质检', '已质检', '待交出', '上游来源', '操作'], rows || '<tr><td colspan="9" class="px-3 py-8 text-center text-sm text-muted-foreground">暂无后道任务</td></tr>', 'min-w-[1280px]')}<div class="mt-4">${renderPostPagination(pagination)}</div>`)}
