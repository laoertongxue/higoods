...
const columns: StandardListColumn<PostFinishingPostTask>[] = [
  ...
  { key: 'actions', title: '操作', width: 420, required: true, actionColumn: true, render: (task) => `<div class="flex justify-end gap-3 whitespace-nowrap"><a data-nav="/fcs/pda/post-finishing/execute?id=${encodeURIComponent(task.postTaskNo)}" class="text-xs font-medium text-blue-700 hover:underline">PDA执行（优先）</a><a data-nav="/fcs/craft/post-finishing/work-orders/${encodeURIComponent(task.postTaskId)}" class="text-xs font-medium text-amber-700 hover:underline">Web应急处理</a><a data-nav="/fcs/craft/post-finishing/print?type=POST_ORDER&id=${encodeURIComponent(task.postTaskId)}" class="text-xs text-blue-600 hover:underline">打印</a><a data-nav="/fcs/craft/post-finishing/audit-records?deliveryId=${encodeURIComponent(task.deliveryId)}" class="text-xs text-blue-600 hover:underline">追溯</a></div>` },
]
...
export function renderPostFinishingWorkOrdersPage(): string {
  ...
  return renderStandardListPage({
    title: '后道单',
    primaryActionsHtml: '<div class="flex items-center gap-3"><span class="text-xs text-muted-foreground">现场优先使用 PDA；设备不可用时从列表进入 Web 应急处理</span><a data-nav="/fcs/pda/post-finishing/execute" class="inline-flex h-9 items-center rounded-md bg-blue-600 px-4 text-sm font-medium text-white">打开 PDA 后道加工</a></div>',
