...
const columns: StandardListColumn<WorkOrderRow>[] = [
  { key: 'documents', title: '单据', width: 280, required: true, freezeable: true, render: (row) => `<div>后道：<strong class="font-mono">${escapeHtml(row.task.postTaskNo)}</strong></div><div class="mt-1 text-xs">质检：<span class="font-mono text-blue-700">${escapeHtml(row.task.qcTaskNo)}</span></div><div class="mt-1 text-xs">生产：<span class="font-mono text-blue-700">${escapeHtml(row.task.productionOrderNo)}</span></div><div class="mt-1 text-[11px] text-muted-foreground">第 ${row.task.returnIndex} 次回货 · ${escapeHtml(row.task.deliveryOrderNo)}</div>` },
  { key: 'factory', title: '工厂', width: 210, required: true, render: (row) => `<div><span class="text-xs text-muted-foreground">来源：</span>${escapeHtml(row.delivery?.sewingFactoryName || '—')}</div><div class="mt-1"><span class="text-xs text-muted-foreground">后道：</span>${escapeHtml(row.delivery?.managedPostFactoryName || '—')}</div>` },
  { key: 'sku', title: 'SKU 明细', width: 330, required: true, render: renderSkuLines },
  { key: 'process', title: '后道项目', width: 210, required: true, render: (row) => `<div class="flex flex-wrap gap-1">${row.task.processItems.map((item) => `<span class="rounded border border-blue-200 bg-blue-50 px-1.5 py-0.5 text-xs text-blue-700">${escapeHtml(item)}</span>`).join('')}</div>` },
  { key: 'total', title: '总数量', width: 110, required: true, align: 'center', render: (row) => `${row.totalQty} 件` },
  { key: 'defect', title: '后道不合格', width: 130, required: true, align: 'center', render: (row) => `${row.defectQty} 件` },
  { key: 'status', title: '后道状态', width: 130, required: true, render: (row) => renderPostStatusBadge(row.task.status) },
  { key: 'time', title: '时间', width: 230, required: true, render: (row) => `<div><span class="text-xs text-muted-foreground">创建：</span>${escapeHtml(row.qcTask?.completedAt ? new Date(row.qcTask.completedAt).toLocaleString('zh-CN') : '—')}</div><div class="mt-1"><span class="text-xs text-muted-foreground">开始：</span>${escapeHtml(row.task.startedAt ? new Date(row.task.startedAt).toLocaleString('zh-CN') : '—')}</div><div class="mt-1"><span class="text-xs text-muted-foreground">完成：</span>${escapeHtml(row.task.completedAt ? new Date(row.task.completedAt).toLocaleString('zh-CN') : '—')}</div>` },
  { key: 'actions', title: '操作', width: 370, required: true, actionColumn: true, render: (row) => `<div class="flex flex-wrap justify-end gap-x-3 gap-y-2"><a data-nav="/fcs/craft/post-finishing/work-orders/${encodeURIComponent(row.task.postTaskId)}" class="text-xs font-medium text-blue-700 hover:underline">查看详情</a><a data-nav="/fcs/pda/post-finishing/execute?id=${encodeURIComponent(row.task.postTaskNo)}" class="text-xs text-blue-600 hover:underline">PDA 执行（优先）</a><a data-nav="/fcs/craft/post-finishing/print?type=POST_ORDER&id=${encodeURIComponent(row.task.postTaskId)}" class="text-xs text-blue-600 hover:underline">打印任务流转卡</a><a data-nav="/fcs/craft/post-finishing/audit-records?deliveryId=${encodeURIComponent(row.task.deliveryId)}" class="text-xs text-blue-600 hover:underline">查看全流程</a></div>` },
]
