...
export function renderPostFinishingAuditRecordsPage(): string {
  ...
  if (selected) {
    return `<div class="space-y-4 p-4" data-testid="post-finishing-audit-detail-page">
      ${renderPostFinishingPageHeader('差异与操作日志', '查看所选回货业务链的单据、差异与操作记录')}
      ${renderDetail(selected, activeDetailTab)}
    </div>`
  }
