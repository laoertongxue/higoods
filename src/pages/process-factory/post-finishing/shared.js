...
export function formatGarmentQty(value, unit = '件') {
    const safeValue = Number.isFinite(value) ? Number(value) : 0;
    return `${safeValue.toLocaleString('zh-CN')} ${unit}`;
}
export function renderPostFinishingPageHeader(title, description = '', actionHtml = '') {
    return `
...
export function renderPostMetricCard(label, value, description) {
    return `
...
export function renderPostSection(title, body) {
    return `
...
export function renderPostStatusBadge(status) {
...
export function renderPostTable(headers, rows, minWidth = 'min-w-[1120px]') {
    return `
...
export function renderPostAction(label, href, disabled = false) {
    if (disabled)
        return `<button type="button" class="rounded-md border px-2 py-1 text-xs opacity-50" disabled>${escapeHtml(label)}</button>`;
...
export function getPostListFilters() {
...
export function postFilterTextMatches(keyword, values) {
    const normalizedKeyword = keyword.trim().toLowerCase();
    if (!normalizedKeyword)
        return true;
    return values.some((value) => String(value ?? '').toLowerCase().includes(normalizedKeyword));
}
export function paginatePostRows(rows, filters) {
...
function buildPostQueryLink(overrides) {
    const current = getPostListFilters();
    const next = { ...current, ...overrides };
    const params = safeSearchParams();
    ['keyword', 'status', 'source', 'factory'].forEach((key) => {
...
export function renderPostPagination(pagination) {
