import { buildQualityDeductionAnalysisFilterOptions, buildQualityDeductionBreakdown, buildQualityDeductionDetails, buildQualityDeductionExportRows, buildQualityDeductionKpis, buildQualityDeductionTrend, createDefaultQualityDeductionAnalysisQuery, QUALITY_DEDUCTION_ANALYSIS_DIMENSION_LABEL, QUALITY_DEDUCTION_ANALYSIS_TIME_BASIS_LABEL, } from '../data/fcs/quality-deduction-analysis.ts';
import { appStore } from '../state/store.ts';
import { QUALITY_DEDUCTION_DISPUTE_STATUS_LABEL, QUALITY_DEDUCTION_FACTORY_RESPONSE_STATUS_LABEL, QUALITY_DEDUCTION_LIABILITY_STATUS_LABEL, QUALITY_DEDUCTION_QC_RESULT_LABEL, QUALITY_DEDUCTION_SETTLEMENT_IMPACT_STATUS_LABEL, } from '../data/fcs/quality-deduction-selectors.ts';
...
let routeQueryKey = '';
function getCurrentAnalysisSearchParams() {
    const pathname = appStore.getState().pathname;
    const [, query] = pathname.split('?');
    return new URLSearchParams(query ?? '');
}
function syncAnalysisStateFromRoute() {
    const pathname = appStore.getState().pathname;
    const [, query = ''] = pathname.split('?');
    if (query === routeQueryKey)
        return;
    routeQueryKey = query;
...
function currentRangeText() {
    if (!state.query.startDate && !state.query.endDate)
        return '全部时间';
    if (state.query.startDate && state.query.endDate)
        return `${state.query.startDate} 至 ${state.query.endDate}`;
    if (state.query.startDate)
        return `${state.query.startDate} 起`;
...
function renderStatusOptions(labelMap, currentValue, allLabel) {
    return renderSelectOptions([{ value: 'ALL', label: allLabel }, ...Object.entries(labelMap).map(([value, label]) => ({ value, label }))], currentValue);
}
    ...
    <div class="flex flex-col gap-3">
      <div class="flex flex-wrap gap-2">
        ${BREAKDOWN_DIMENSIONS.map((dimension) => `
            <button
              class="${state.breakdownDimension === dimension
      ...
      <div class="overflow-x-auto rounded-md border">
        <table class="w-full min-w-[860px] text-sm">
          ...
          <tbody>
            ${rows
        .map((row) => {
        const isActive = state.query.drilldownDimension === state.breakdownDimension &&
...
export function renderDeductionAnalysisPage() {
    ...
    const drilldownActive = state.query.drilldownDimension === state.breakdownDimension && Boolean(state.query.drilldownValue);
    return `
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-6">
          ...
          <label class="flex flex-col gap-1 text-sm">
            ...
            <select class="h-10 rounded-md border bg-background px-3" data-danalysis-filter="timeBasis">
              ${renderSelectOptions(Object.entries(QUALITY_DEDUCTION_ANALYSIS_TIME_BASIS_LABEL).map(([value, label]) => ({ value, label })), state.query.timeBasis)}
            </select>
          </label>
          <label class="flex flex-col gap-1 text-sm">
      ...
      <section class="rounded-xl border bg-card p-5">
        ...
        ? `
                <div class="flex items-center gap-2">
                  <span class="rounded-full bg-amber-100 px-3 py-1 text-xs text-amber-800">已按 ${escapeHtml(QUALITY_DEDUCTION_ANALYSIS_DIMENSION_LABEL[state.breakdownDimension])} 钻取：${escapeHtml(buildQualityDeductionBreakdown(state.query, state.breakdownDimension).find((row) => row.key === state.query.drilldownValue)?.label ?? state.query.drilldownValue ?? '')}</span>
                ...
                <table class="w-full min-w-[1720px] text-sm">
                  <thead>
                    <tr class="border-b bg-muted/40 text-left">
                      ...
                      <th class="px-4 py-2 font-medium">${escapeHtml(state.query.timeBasis === 'SETTLEMENT_CYCLE' ? '结算周期 / 归属时间' : '财务影响生效时间')}</th>
...
export function handleDeductionAnalysisEvent(target) {
    const filterNode = target.closest('[data-danalysis-filter]');
    if (filterNode instanceof HTMLInputElement || filterNode instanceof HTMLSelectElement) {
        ...
        if (!field)
            return false;
        state.query[field] = filterNode.value;
        return true;
    }
    const dimensionNode = target.closest('[data-danalysis-dimension]');
    if (dimensionNode) {
        const next = dimensionNode.dataset.danalysisDimension;
        if (!next)
            return false;
    ...
    const drilldownNode = target.closest('[data-danalysis-breakdown-value]');
    if (drilldownNode) {
        const value = drilldownNode.dataset.danalysisBreakdownValue;
        if (!value)
            return false;
        const isSame = state.query.drilldownDimension === state.breakdownDimension && state.query.drilldownValue === value;
    ...
    const actionNode = target.closest('[data-danalysis-action]');
    if (!actionNode)
        return false;
...
export function isDeductionAnalysisDialogOpen() {
    return false;
}
