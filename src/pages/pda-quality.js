...
import { confirmQualityDeductionFactoryResponse, submitQualityDeductionDispute, } from '../data/fcs/quality-deduction-repository';
import { getFutureMobileFactoryQcDetail, getFutureMobileFactoryQcSummary, listFutureMobileFactoryQcBuckets, } from '../data/fcs/quality-deduction-selectors';
import { listPostFinishingSourceStyleOptions, listPostFinishingSourceTaskOptions, } from '../data/fcs/post-finishing-domain';
...
function getCurrentFactoryOperatorName(factoryId) {
    const runtime = getPdaRuntimeContext();
    if (runtime?.userName)
...
function buildPdaQualityDetailHref(qcId) {
    const params = getCurrentSearchParams();
    const search = new URLSearchParams();
    const view = params.get('view');
    const cycleId = params.get('cycleId');
...
function getResultBadgeClass(label) {
    if (label === '合格')
...
function getResponseBadgeClass(label) {
    if (label.includes('待工厂处理'))
        return getBadgeClass('amber');
    if (label.includes('自动确认'))
        return getBadgeClass('purple');
...
function getDisputeBadgeClass(label) {
    if (label === '无异议')
        return getBadgeClass('gray');
    if (label === '待平台处理' || label === '平台处理中')
        return getBadgeClass('amber');
    if (label === '最终部分工厂责任')
...
function getSettlementBadgeClass(label) {
    if (label.includes('待确认') || label.includes('待平台处理'))
        return getBadgeClass('amber');
    if (label.includes('正式质量扣款流水'))
        return getBadgeClass('green');
    if (label.includes('预结算单'))
        return getBadgeClass('blue');
    if (label.includes('预付款批次'))
        return getBadgeClass('purple');
    return getBadgeClass('gray');
}
...
function renderSectionCard(title, body, _description) {
    return `
...
function renderEvidenceAssets(assets, emptyText, prefix) {
    if (assets.length === 0) {
            ...
            <article class="rounded-xl border bg-background px-3 py-3">
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  ...
                  <div class="mt-1 text-[11px] text-muted-foreground">${escapeHtml(prefix)} · ${asset.assetType === 'IMAGE' ? '图片' : asset.assetType === 'VIDEO' ? '视频' : '文档'}</div>
                </div>
...
function getListItemsByView(factoryId, view) {
...
function getViewCount(factoryId, view) {
...
function openDisputeSheet(qcId) {
...
function setCreateStyle(styleId) {
...
function toggleCreateSku(skuId, checked) {
...
function showPdaQualityToast(message) {
...
function renderCreateSkuRows(lines) {
    if (lines.length === 0) {
...
function renderSummaryCards(factoryId) {
...
function renderViewTabs(factoryId) {
    return `
    <div class="flex gap-2 overflow-x-auto pb-1">
      ${QUALITY_VIEWS.map((view) => {
        ...
        return `
          <button
            class="inline-flex shrink-0 items-center rounded-full border px-3 py-1.5 text-xs font-medium ${active ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background text-muted-foreground'}"
...
function renderListCard(item) {
    const pendingHint = item.factoryResponseStatus === 'PENDING_RESPONSE'
        ? item.isOverdue
      ...
      ${item.canConfirm || item.canDispute
        ? `
            <div class="mt-3 rounded-xl border border-primary/20 bg-primary/5 px-3 py-3">
              <div class="text-[11px] leading-5 text-muted-foreground">当前需要工厂处理，可先核对检查证据和金额影响。</div>
...
function renderListEmptyState(viewLabel) {
    return `
...
export function renderPdaQualityPage() {
...
function renderDetailSections(detail) {
    ...
    return [
        renderSectionCard('基本信息', renderInfoGrid([
            { label: '检查单号', value: escapeHtml(detail.qcNo) },
            ...
            { label: '检查场景', value: escapeHtml(detail.inspectionSceneLabel ?? '回货质检') },
            { label: '检查类型', value: escapeHtml(detail.inspectionTypeLabel ?? '质检') },
            { label: '检查方式', value: escapeHtml(detail.inspectionMethodLabel ?? '抽检') },
            { label: '回货环节', value: escapeHtml(detail.processLabel) },
            { label: '工厂', value: escapeHtml(detail.returnFactoryName) },
            { label: '接收方', value: escapeHtml(detail.receiverName) },
            { label: '检查人', value: escapeHtml(detail.inspectorUserName) },
            { label: '完成时间', value: escapeHtml(formatDateTime(detail.inspectedAt)) },
        ])),
        renderSectionCard('数量与责任', renderInfoGrid([
            { label: '工厂交出', value: formatQty(detail.declaredQty) },
            { label: '接收方实收', value: formatQty(detail.receivedQty) },
            { label: '质检数量', value: formatQty(detail.inspectedQty) },
        ...
        renderSectionCard('缺陷与检查证据', `
        <div class="rounded-xl bg-muted/40 px-3 py-3 text-xs leading-5 text-foreground">
          <div class="font-medium">缺陷说明</div>
          <div class="mt-1 text-muted-foreground">${escapeHtml(detail.unqualifiedReasonSummary || detail.defectItems.map((item) => `${item.defectName}×${item.qty}`).join('、') || '当前无额外缺陷说明')}</div>
        </div>
        <div class="mt-3 text-[11px] text-muted-foreground">检查证据 ${detail.warehouseEvidenceCount} 份</div>
        <div class="mt-2">${renderEvidenceAssets(detail.warehouseEvidenceAssets, '当前无检查证据。', '检查证据')}</div>
      `),
        renderSectionCard('工厂处理与金额影响', renderInfoGrid([
            ...
            {
                label: '正式质量扣款流水说明',
                value: escapeHtml(detail.formalLedgerStatusLabel ??
...
function renderBottomActionBar(detail) {
    if (detail.availableActions.length === 0)
        return '';
    return `
    <div class="sticky bottom-0 z-20 mt-4 rounded-t-2xl border border-border bg-background/95 px-4 py-3 backdrop-blur">
      <div class="mb-2 text-[11px] text-muted-foreground">当前需要工厂处理，请先核对数量与证据。</div>
      <div class="flex gap-2">
        <button
...
function renderConfirmDialog(detail) {
    if (state.confirmQcId !== detail.qcId)
        return '';
    return `
...
function renderDisputeSheet(detail) {
    if (state.disputeQcId !== detail.qcId)
        return '';
    return `
    <div class="fixed inset-0 z-[130]">
          ...
          <section class="rounded-2xl border bg-card px-4 py-4 shadow-sm">
            ...
            <select class="mt-2 h-10 w-full rounded-xl border bg-background px-3 text-sm" data-pda-quality-field="disputeReasonCode">
              ...
              ${DISPUTE_REASON_OPTIONS.map((option) => `
                  <option value="${escapeAttr(option.code)}" ${state.disputeForm.reasonCode === option.code ? 'selected' : ''}>
...
export function renderPdaQualityDetailPage(qcId) {
    if (!getPdaRuntimeContext()) {
        return renderPdaLoginRedirect();
    }
    const detail = getFutureMobileFactoryQcDetail(qcId, getCurrentFactoryId());
    if (!detail)
        return renderDetailEmptyState();
    return renderPdaFrame(`
      <div class="space-y-4 px-4 pb-5 pt-4">
...
export function handlePdaQualityEvent(target) {
    if (!ensurePdaSessionForAction())
        return true;
    const fieldNode = target.closest('[data-pda-quality-field]');
    if (fieldNode instanceof HTMLInputElement ||
        ...
        if (field === 'createSku' && fieldNode instanceof HTMLInputElement) {
            ...
            if (skuId)
                toggleCreateSku(skuId, fieldNode.checked);
            return true;
        }
    ...
    const actionNode = target.closest('[data-pda-quality-action]');
    if (!actionNode)
        return false;
