...
import { initialNotifications, initialUrges, listProgressExceptions, mockInternalUsers, generateNotificationId, generateUrgeId, } from '../data/fcs/store-domain-progress.ts';
import { buildHandoverOrderDetailLink, getHandoverLedgerRows, getHandoverOrderTimelineViews, getProductionOrderHandoverSummary, } from '../data/fcs/handover-ledger-view';
import { getTaskChainTaskById, getTaskChainTaskDisplayName, listTaskChainTasks, listTaskChainTenders, } from '../data/fcs/page-adapters/task-chain-pages-adapter';
const state = {
    lastQueryKey: '',
    activeTab: 'inbox',
    nRecipientType: 'ALL',
...
function isTargetTypeWithoutTechPack(value) {
...
function showProgressUrgeToast(message, tone = 'success') {
...
function getUrgeById(urgeId) {
    return initialUrges.find((item) => item.urgeId === urgeId);
}
function updateNotification(updated) {
...
function markAllNotificationsRead(filter) {
    const readAt = nowTimestamp();
    for (let i = 0; i < initialNotifications.length; i += 1) {
        const item = initialNotifications[i];
        if (item.readAt)
            continue;
        if (filter?.recipientType && item.recipientType !== filter.recipientType)
            continue;
        if (filter?.recipientId && item.recipientId !== filter.recipientId)
            continue;
...
function recomputeAutoNotifications() {
    const now = new Date();
    const nowStr = nowTimestamp(now);
    const newNotifications = [];
    const existingKeys = new Set(initialNotifications
        .filter((item) => {
    ...
    const warehouseOverdueByTask = new Map();
    handoverRows.forEach((row) => {
        ...
        if (Number.isNaN(occurredAtMs))
            return;
        if (now.getTime() - occurredAtMs < 24 * 60 * 60 * 1000)
            return;
    ...
    warehouseOverdueByTask.forEach((row) => {
        ...
        const notification = {
            ...
            deepLink: parseHandoverHref(buildHandoverOrderDetailLink({
                productionOrderId: row.productionOrderId,
                taskId: row.taskId,
    ...
    const objectionOverdueByTask = new Map();
    handoverRows.forEach((row) => {
        if (row.statusCode !== 'HANDOUT_OBJECTION_REPORTED' && row.statusCode !== 'HANDOUT_OBJECTION_PROCESSING')
            return;
        const occurredAtMs = parseDateTime(row.occurredAt);
        if (Number.isNaN(occurredAtMs))
            return;
    ...
    listUrgeTasks().forEach((task) => {
        ...
        if (now.getTime() - assignedAt.getTime() < 4 * 60 * 60 * 1000)
            return;
        const factory = indonesiaFactories.find((item) => item.id === task.assignedFactoryId);
        const notification = {
            level: 'WARN',
...
function getDefaultUrgeType(targetType, taskStatus) {
    switch (targetType) {
        case 'TASK':
...
function getAvailableUrgeTypes(targetType) {
    switch (targetType) {
        case 'TASK':
...
function isMaterialRelated(title, content, tags) {
...
function getRecipientOptions(recipientType) {
...
function inferRecipient(targetType, targetId) {
    switch (targetType) {
        case 'TASK': {
...
function parseHandoverHref(href) {
...
function getDeepLink(targetType, targetId) {
    const task = listUrgeTasks().find((item) => item.taskId === targetId);
    switch (targetType) {
        case 'TASK':
            return {
        ...
        case 'HANDOVER':
            return parseHandoverHref(buildHandoverOrderDetailLink({
...
function openDeepLink(deepLink, title) {
...
function applyTargetTypeFilterFromQuery(activeTab) {
...
function getFilteredNotifications() {
    return initialNotifications
        .filter((notification) => {
        if (state.nRecipientType !== 'ALL' && notification.recipientType !== state.nRecipientType)
            return false;
        if (state.nRecipientId !== 'ALL' && notification.recipientId !== state.nRecipientId)
            return false;
        if (state.nLevel !== 'ALL' && notification.level !== state.nLevel)
            return false;
        if (state.nTargetType !== 'ALL' && notification.targetType !== state.nTargetType)
            return false;
        if (state.nReadStatus === 'UNREAD' && notification.readAt)
            return false;
        if (state.nReadStatus === 'READ' && !notification.readAt)
            return false;
...
function getFilteredUrges() {
    return initialUrges
        .filter((urge) => {
        if (state.uUrgeType !== 'ALL' && urge.urgeType !== state.uUrgeType)
            return false;
        if (state.uToType !== 'ALL' && urge.toType !== state.uToType)
            return false;
        if (state.uToId !== 'ALL' && urge.toId !== state.uToId)
            return false;
        if (state.uTargetType !== 'ALL' && urge.targetType !== state.uTargetType)
            return false;
        if (state.uStatus !== 'ALL' && urge.status !== state.uStatus)
            return false;
...
function renderTargetTypeOptions(includeTechPack, selectedValue) {
...
function renderUrgeActionMenu(urge) {
...
function renderNotificationTable(rows) {
    return `
...
function renderNotificationDetailDrawer() {
    if (!state.notificationDetailId)
        return '';
...
function renderUrgeDetailDrawer() {
    if (!state.urgeDetailId)
        return '';
...
function renderNewUrgeDrawer() {
    if (!state.newUrgeOpen)
        return '';
    ...
    <div class="fixed inset-0 z-[55]">
      ...
      <section class="absolute inset-y-0 right-0 w-full max-w-[500px] overflow-y-auto border-l bg-background shadow-2xl">
        ...
        <footer class="sticky bottom-0 flex flex-wrap justify-end gap-2 border-t bg-background px-5 py-4">
          ...
          <button
            class="${toClassName('inline-flex h-9 items-center rounded-md px-4 text-sm', canSend
...
function updateField(field, node) {
    ...
    if (field === 'formTargetType' && node instanceof HTMLSelectElement) {
        if (!isTargetTypeWithoutTechPack(node.value))
            return;
...
export function renderProgressUrgePage() {
    syncFromQuery();
...
export function handleProgressUrgeEvent(target) {
    const fieldNode = target.closest('[data-urge-field]');
    ...
    const actionNode = target.closest('[data-urge-action]');
    if (!actionNode) {
...
export function isProgressUrgeDialogOpen() {
