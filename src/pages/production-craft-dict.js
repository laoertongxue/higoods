...
import { craftStageDict, getProcessCraftDictRowByCode, listAccessoryCrafts, listAuxiliaryCrafts, listCutPiecePartCrafts, listCuttingCrafts, listFabricCrafts, SAM_FACTORY_FIELD_GROUP_LABEL, listSamFactoryFieldDefinitions, listPreparationProcesses, listProcessCraftDictRows, listSpecialTypeCrafts, processCraftDictRows, } from '../data/fcs/process-craft-dict';
...
function filteredCraftRows() {
    ...
    return rows.filter((row) => {
        if (state.filterStage !== 'ALL' && row.stageCode !== state.filterStage)
            return false;
        ...
        if (!keyword)
            return true;
        return (row.craftCode.toLowerCase().includes(keyword) ||
...
function renderCompactDetailFields(fields) {
    return `
...
function renderSamFieldGroups(fieldKeys) {
    const groups = ['DEVICE', 'STAFF', 'ADJUSTMENT'].map((group) => ({
        group,
        label: SAM_FACTORY_FIELD_GROUP_LABEL[group],
...
function renderCraftBasicPanel(row) {
    return `
...
function renderIdealSamPanel(row) {
...
function renderCurrentStagePanel(row) {
    const formulaGuide = getFactorySupplyFormulaGuide(row.craftName);
    return `
...
function renderCraftReferenceSamPanel(row) {
    return `
...
function renderCraftBadgeList(items) {
    return `
...
function renderModernCraftItems(items) {
    return items.map((item) => `${item.craftName}（${item.targetObjectName}）`);
}
function renderDictionaryRebuildSummary() {
...
function renderCraftDetailSheet(row) {
    return `
    ...
    <aside class="fixed inset-y-0 right-0 z-[121] w-full max-w-[960px] overflow-y-auto border-l bg-background shadow-xl" data-testid="craft-dict-detail-sheet">
      ...
      <div class="space-y-4 p-5" data-testid="craft-dict-detail-body">
        ...
        .map(([tab, label]) => `
                  <button
                    class="inline-flex h-8 items-center rounded-md border px-3 text-xs font-medium ${state.detailTab === tab
...
export function renderProductionCraftDictPage() {
    ...
    <div class="space-y-4">
      ...
      <div class="space-y-4">
        ...
        .map((item) => `<option value="${item.stageCode}" ${state.filterStage === item.stageCode ? 'selected' : ''}>${escapeHtml(item.stageName)}</option>`)
        .join('')}
          </select>
...
export function handleProductionCraftDictEvent(target) {
    const fieldNode = target.closest('[data-craft-dict-field]');
    if (fieldNode instanceof HTMLInputElement || fieldNode instanceof HTMLSelectElement) {
        ...
        if (field === 'filterGranularity') {
            state.filterGranularity = fieldNode.value;
            state.page = 1;
            return true;
        }
    ...
    const actionNode = target.closest('[data-craft-dict-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'switch-detail-tab') {
        const tab = actionNode.dataset.detailTab;
        if (tab) {
            state.detailTab = tab;
        }
        return true;
...
export function isProductionCraftDictDialogOpen() {
    return Boolean(state.viewCraftCode);
}
export function closeProductionCraftDictDialog() {
    state.viewCraftCode = '';
    state.detailTab = 'CURRENT';
