...
import { buildPdaCuttingRoute, getPdaCuttingTaskSnapshot, } from '../data/fcs/pda-cutting-execution-source.ts';
import { listSpreadingResultGeneratedFeiTicketsByCutOrderId } from '../data/fcs/cutting/generated-fei-tickets.ts';
import { readSelectedExecutionOrderIdFromLocation, readSelectedExecutionOrderNoFromLocation, resolveSelectedExecutionOrderLine, } from './pda-cutting-context';
import { resolvePdaCuttingWritebackIdentity, resolvePdaCuttingWritebackOperator, } from '../data/fcs/pda-cutting-writeback-inputs.ts';
import { appendPdaCuttingStageWritebackRecord } from '../data/fcs/cutting/pda-cutting-stage-writeback.ts';
import { renderPdaCuttingEmptyState, renderPdaCuttingStatusChip, } from './pda-cutting-shared';
import { renderMaterialIdentityBlock } from './process-factory/cutting/material-identity';
import { renderPdaFrame } from './pda-shell';
function buildPickupHref(taskId, backHref, line) {
    const params = new URLSearchParams();
    params.set('tab', 'pickup');
    params.set('focusTaskId', taskId);
    if (line?.executionOrderId)
        params.set('executionOrderId', line.executionOrderId);
    if (line?.executionOrderNo)
        params.set('executionOrderNo', line.executionOrderNo);
    if (backHref)
        params.set('returnTo', backHref);
    return `/fcs/pda/handover?${params.toString()}`;
}
function buildActionHref(taskId, line, backHref) {
    if (line.currentStepCode === 'START')
        return backHref || '/fcs/pda/exec?tab=NOT_STARTED';
    if (line.currentStepCode === 'PICKUP')
        return buildPickupHref(taskId, backHref, line);
    const routeKey = line.primaryExecutionRouteKey;
...
function renderMiniMaterialField(line) {
    return `
...
function renderFeiTicketIds(cutOrderId) {
...
function renderTraceIdentity(detail) {
    return `
...
function renderCurrentReceiveBlock(detail, backHref) {
    return `
    <section class="rounded-2xl border bg-card p-3 shadow-sm">
...
function renderOrderLine(taskId, line, backHref, selected) {
...
function renderRecentActions(detail) {
    if (!detail.recentActions.length)
        return renderPdaCuttingEmptyState('暂无操作记录', '');
    return `
    <section class="rounded-2xl border bg-card p-3 shadow-sm">
...
export function renderPdaCuttingTaskDetailPage(taskId, options = {}) {
    ...
    if (!detail) {
        return renderPdaFrame(`<section class="space-y-3 px-3 py-4">
    ...
    const selectedLine = resolveSelectedExecutionOrderLine(detail, readSelectedExecutionOrderIdFromLocation(), readSelectedExecutionOrderNoFromLocation()) || detail.cutPieceOrders.find((line) => line.executionOrderId === detail.currentSelectedExecutionOrderId) || detail.cutPieceOrders[0] || null;
    ...
    `, 'exec', { disableTodoAutoOpen: true });
}
export function handlePdaCuttingTaskDetailEvent(target) {
    const button = target.closest('[data-pda-cutting-task-action="start-work"]');
    if (!button)
        return false;
    ...
    const feedback = button.closest('[data-pda-cutting-order-line]')?.querySelector('[data-pda-cutting-task-feedback]');
    if (!identity) {
        if (feedback) {
