...
import { getQcById as getQcByIdFromChain, resolveQcIdFromRouteKey, } from '../../data/fcs/quality-chain-adapter';
import { getPlatformQcDetailViewModelByRouteKey, getPlatformQcWorkbenchStats, getPlatformQcWorkbenchTabCounts, listPlatformQcListItems, matchesPlatformQcWorkbenchView, } from '../../data/fcs/quality-deduction-selectors';
import { initialDeductionBasisItems, initialQualityInspections, initialReturnInboundBatches, } from '../../data/fcs/store-domain-quality-seeds';
import { defaultResponsibility, } from '../../data/fcs/store-domain-quality-types';
import { blockTaskForReturnInboundQc, findReturnInboundBatchForQc, isReturnInboundInspection, isSewReturnInboundQc, requiresFinalLiabilityDecision, resolveReturnInboundTaskId, upsertDeductionBasisFromReturnInboundQc, } from '../../data/fcs/return-inbound-workflow';
import { normalizeQcForView, RETURN_INBOUND_PROCESS_LABEL, RETURN_INBOUND_QC_POLICY_LABEL, SEW_POST_PROCESS_MODE_LABEL, } from '../../data/fcs/return-inbound-qc-view';
...
const processTasks = listExecutionTaskFacts();
const NEEDS_AFFECTED_QTY = ['ACCEPT_AS_DEFECT'];
const RESULT_LABEL = {
    PASS: '合格',
    PARTIAL_PASS: '部分合格',
    FAIL: '不合格',
...
const DEDUCTION_DECISION_LABEL = {
    DEDUCT: '扣款',
    NO_DEDUCT: '不扣款',
};
...
function showQcRecordsToast(message, tone = 'success') {
...
function syncListViewFromRouteQuery() {
    ...
    if (routeView && allowedViews.includes(routeView)) {
        listState.activeView = routeView;
    }
}
function getCurrentDetailRouteId() {
    const pathname = appStore.getState().pathname;
...
function emptyAdjudicationDraft(overrides = {}) {
    return {
        result: '',
        comment: '',
        adjustedLiableQty: '',
...
function qcToForm(qc) {
    const isReturnInbound = qc.inspectionScene === 'RETURN_INBOUND' ||
    ...
    const refType = isReturnInbound ? 'RETURN_BATCH' : qc.refType === 'HANDOVER' ? 'HANDOVER' : 'TASK';
    ...
    return {
        ...
        rootCauseType: qc.rootCauseType ?? 'UNKNOWN',
...
function getQcById(qcId) {
    return getQcByIdFromChain(qcId);
}
function getReturnInboundBatchById(batchId) {
    return initialReturnInboundBatches.find((item) => item.batchId === batchId) ?? null;
}
function applyReturnInboundBatchToForm(form, batchId) {
...
function ensureDetailState(routeQcId) {
    ...
    if (!detailState || detailState.routeQcId !== routeQcId || detailState.queryKey !== queryKey) {
        ...
        detailState = {
            ...
            adjudication: emptyAdjudicationDraft(currentQcId
                ? (() => {
                    const detailVm = getPlatformQcDetailViewModelByRouteKey(routeQcId);
                    return detailVm
                        ? {
                            adjustedLiableQty: detailVm.deductionBasis?.deductionQty ?? detailVm.qcRecord.factoryLiabilityQty,
                            adjustedBlockedProcessingFeeAmount: detailVm.settlementImpact.blockedProcessingFeeAmount,
                            adjustedEffectiveQualityDeductionAmount: detailVm.settlementImpact.effectiveQualityDeductionAmount,
                        }
                        : {};
                })()
                : {}),
...
function getWorkbenchTabCounts() {
...
function getFilteredQcRows() {
    ...
    return getQcViewRows()
        ...
        if (listState.filterLiabilityStatus !== 'ALL' &&
            row.liabilityStatus !== listState.filterLiabilityStatus) {
            return false;
        }
        if (listState.filterFactoryResponseStatus !== 'ALL' &&
            row.factoryResponseStatus !== listState.filterFactoryResponseStatus) {
            return false;
        }
        if (listState.filterDisputeStatus !== 'ALL' &&
            row.disputeStatus !== listState.filterDisputeStatus) {
            return false;
        }
        if (listState.filterSettlementImpactStatus !== 'ALL' &&
            row.settlementImpactStatus !== listState.filterSettlementImpactStatus) {
            return false;
        }
        if (listState.filterDisposition !== 'ALL' &&
            row.disposition !== listState.filterDisposition) {
            return false;
        }
        ...
        if (keyword) {
            const match = row.qcId.toLowerCase().includes(keyword) ||
...
export { appStore, initialDeductionBasisItems, initialQualityInspections, initialReturnInboundBatches, defaultResponsibility, blockTaskForReturnInboundQc, findReturnInboundBatchForQc, isReturnInboundInspection, isSewReturnInboundQc, requiresFinalLiabilityDecision, resolveReturnInboundTaskId, upsertDeductionBasisFromReturnInboundQc, normalizeQcForView, RETURN_INBOUND_PROCESS_LABEL, RETURN_INBOUND_QC_POLICY_LABEL, SEW_POST_PROCESS_MODE_LABEL, escapeHtml, formatDateTime, toClassName, processTasks, NEEDS_AFFECTED_QTY, RESULT_LABEL, RESULT_CLASS, STATUS_LABEL, STATUS_CLASS, DISPOSITION_LABEL, DEDUCTION_DECISION_LABEL, DISPOSITION_CLASS, ROOT_CAUSE_LABEL, LIABILITY_LABEL, PARTY_TYPE_LABEL, listState, nowTimestamp, randomSuffix, showQcRecordsToast, getCurrentQueryString, getCurrentSearchParams, syncListViewFromRouteQuery, getCurrentDetailRouteId, emptyForm, qcToForm, getQcById, resolveQcIdFromRouteKey, getReturnInboundBatchById, applyReturnInboundBatchToForm, isSewReturnInboundFromForm, requiresFinalDecisionForForm, replaceQc, syncDetailFromQc, ensureDetailState, toInputValue, parseNumberField, parseAmountField, getQcViewRows, getFactoryOptions, getWarehouseOptions, getInspectorOptions, getWorkbenchStats, getWorkbenchTabCounts, getFilteredQcRows, };
