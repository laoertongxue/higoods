import { RETURN_INBOUND_PROCESS_LABEL, RETURN_INBOUND_QC_POLICY_LABEL, escapeHtml, formatDateTime, RESULT_LABEL, RESULT_CLASS, DISPOSITION_LABEL, DISPOSITION_CLASS, listState, toInputValue, getFilteredQcRows, getFactoryOptions, getWarehouseOptions, getInspectorOptions, getWorkbenchStats, getWorkbenchTabCounts, } from './context';
import { buildQcDeductionHref, buildQcDetailHref } from '../../data/fcs/quality-chain-adapter';
import { PLATFORM_QC_WORKBENCH_VIEW_LABEL, QUALITY_DEDUCTION_DISPUTE_STATUS_LABEL, QUALITY_DEDUCTION_FACTORY_RESPONSE_STATUS_LABEL, QUALITY_DEDUCTION_LIABILITY_STATUS_LABEL, QUALITY_DEDUCTION_SETTLEMENT_IMPACT_STATUS_LABEL, } from '../../data/fcs/quality-deduction-selectors';
...
function renderTextBadge(label, className) {
...
function renderDispositionBadge(result, disposition) {
...
function renderFactoryResponseBadge(row) {
    const className = row.factoryResponseStatus === 'PENDING_RESPONSE'
...
function renderDisputeBadge(row) {
    const className = row.disputeStatus === 'PENDING_REVIEW' || row.disputeStatus === 'IN_REVIEW'
        ? 'border-amber-200 bg-amber-50 text-amber-700'
...
function renderSettlementBadge(row) {
    const className = row.settlementImpactStatus === 'BLOCKED'
        ? 'border-orange-200 bg-orange-50 text-orange-700'
...
function renderViewTabs(tabCounts) {
    const views = [
        'ALL',
    ...
    <section class="rounded-md border bg-card px-4 py-3">
      <div class="flex flex-wrap gap-2">
        ...
        return `
              <button
                class="inline-flex h-9 items-center gap-2 rounded-md border px-3 text-sm ${active
            ? 'border-blue-200 bg-blue-50 text-blue-700'
...
function renderStatCard(params) {
    const toneClass = params.tone === 'orange'
        ? 'text-orange-600'
        : params.tone === 'amber'
            ? 'text-amber-600'
            : params.tone === 'blue'
...
function renderQuantitySummary(row) {
    return `
    <div class="space-y-1">
...
function renderFactoryResponseSummary(row) {
    return `
    <div class="space-y-1">
...
function renderDisputeSummary(row) {
    return `
    <div class="space-y-1">
...
function renderDeductionSummary(row) {
...
function renderSettlementSummary(row) {
    return `
    <div class="space-y-1">
...
export function renderQcRecordsPage() {
    ...
    <div class="flex flex-col gap-6 p-6">
      ...
      <section class="rounded-md border bg-card p-4">
        ...
        .map(([key, label]) => `<option value="${key}" ${listState.filterProcessType === key ? 'selected' : ''}>${label}</option>`)
        .join('')}
            </select>
          </div>
          ...
          <div>
            ...
            <select class="h-9 w-full rounded-md border bg-background px-3 text-sm" data-qcr-filter="factory">
              ...
              ${factoryOptions
        .map((item) => `<option value="${escapeHtml(item)}" ${listState.filterFactory === item ? 'selected' : ''}>${escapeHtml(item)}</option>`)
        .join('')}
            </select>
          </div>
          ...
          <div>
            ...
            <select class="h-9 w-full rounded-md border bg-background px-3 text-sm" data-qcr-filter="warehouse">
              ...
              ${warehouseOptions
        .map((item) => `<option value="${escapeHtml(item)}" ${listState.filterWarehouse === item ? 'selected' : ''}>${escapeHtml(item)}</option>`)
        .join('')}
            </select>
          </div>
        ...
        .map(([key, label]) => `<option value="${key}" ${listState.filterPolicy === key ? 'selected' : ''}>${label}</option>`)
        .join('')}
            </select>
          </div>
        ...
        .map(([key, label]) => `<option value="${key}" ${listState.filterLiabilityStatus === key ? 'selected' : ''}>${label}</option>`)
        .join('')}
            </select>
          </div>
        ...
        .map(([key, label]) => `<option value="${key}" ${listState.filterFactoryResponseStatus === key ? 'selected' : ''}>${label}</option>`)
        .join('')}
            </select>
          </div>
        ...
        .map(([key, label]) => `<option value="${key}" ${listState.filterDisputeStatus === key ? 'selected' : ''}>${label}</option>`)
        .join('')}
            </select>
          </div>
        ...
        .map(([key, label]) => `<option value="${key}" ${listState.filterSettlementImpactStatus === key ? 'selected' : ''}>${label}</option>`)
        .join('')}
            </select>
          </div>
          ...
          <div>
            ...
            <select class="h-9 w-full rounded-md border bg-background px-3 text-sm" data-qcr-filter="inspector">
              ...
              ${inspectorOptions
        .map((item) => `<option value="${escapeHtml(item)}" ${listState.filterInspector === item ? 'selected' : ''}>${escapeHtml(item)}</option>`)
        .join('')}
            </select>
          </div>
        ...
        : `
            ...
            return `
                ...
                : ''}
                          ...
                          <td class="px-4 py-3 align-top">${renderDispositionBadge(row.result, row.disposition)}</td>
