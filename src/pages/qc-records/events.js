import { appStore, listState, showQcRecordsToast, getCurrentDetailRouteId, ensureDetailState, parseNumberField, parseAmountField, nowTimestamp, getQcById, syncDetailFromQc, } from './context';
import { adjudicateDisputeCase } from '../../data/fcs/quality-deduction-lifecycle';
import { updateQcDispositionBreakdown, saveDraft, submitDetail, updateFormField, setResult, isDetailReadOnly, } from './actions';
export function handleQcRecordsEvent(target) {
    const listFilterNode = target.closest('[data-qcr-filter]');
    if (listFilterNode instanceof HTMLInputElement || listFilterNode instanceof HTMLSelectElement) {
        ...
        if (field === 'processType') {
            listState.filterProcessType = listFilterNode.value;
            return true;
        }
        if (field === 'policy') {
            listState.filterPolicy = listFilterNode.value;
            return true;
        }
        if (field === 'result') {
            listState.filterResult = listFilterNode.value;
            return true;
        }
        if (field === 'status') {
            listState.filterStatus = listFilterNode.value;
            return true;
        }
        if (field === 'disposition') {
            listState.filterDisposition = listFilterNode.value;
            return true;
        }
        if (field === 'liabilityStatus') {
            listState.filterLiabilityStatus = listFilterNode.value;
            return true;
        }
        if (field === 'factoryResponseStatus') {
            listState.filterFactoryResponseStatus = listFilterNode.value;
            return true;
        }
        if (field === 'disputeStatus') {
            listState.filterDisputeStatus = listFilterNode.value;
            return true;
        }
        if (field === 'settlementImpactStatus') {
            listState.filterSettlementImpactStatus = listFilterNode.value;
            return true;
        }
        if (field === 'inspector') {
    ...
    const detailFieldNode = target.closest('[data-qcd-field]');
    if (detailFieldNode instanceof HTMLInputElement ||
    ...
    const defectNode = target.closest('[data-qcd-defect-field]');
    ...
    const breakdownNode = target.closest('[data-qcd-breakdown]');
    ...
    const adjudicationFieldNode = target.closest('[data-qcd-adjudication-field]');
    if (adjudicationFieldNode instanceof HTMLInputElement ||
        ...
        if (field === 'result' && adjudicationFieldNode instanceof HTMLSelectElement) {
            detail.adjudication.result = adjudicationFieldNode.value;
    ...
    const listActionNode = target.closest('[data-qcr-action]');
    if (listActionNode) {
        ...
        if (action === 'set-view') {
            listState.activeView =
                listActionNode.dataset.qcrView ?? 'ALL';
            return true;
        }
    ...
    const detailActionNode = target.closest('[data-qcd-action]');
    if (!detailActionNode)
        return false;
    ...
    if (action === 'back-list') {
        ...
        appStore.navigate(pathname.startsWith('/fcs/pda/qc-records/')
            ? '/fcs/pda/settlement'
    ...
    if (action === 'set-result') {
        if (isDetailReadOnly(detail))
            return true;
        const result = detailActionNode.dataset.qcdResult;
    ...
    if (action === 'save-breakdown') {
        ...
        const result = updateQcDispositionBreakdown(detail.currentQcId, {
    ...
    if (action === 'submit-adjudication') {
        ...
        const adjudicationResult = adjudicateDisputeCase({
            ...
            adjustedLiableQty: detail.adjudication.result === 'PARTIALLY_ADJUSTED' && detail.adjudication.adjustedLiableQty !== ''
                ? Number(detail.adjudication.adjustedLiableQty)
                : undefined,
            adjustedBlockedProcessingFeeAmount: detail.adjudication.result === 'PARTIALLY_ADJUSTED' && detail.adjudication.adjustedBlockedProcessingFeeAmount !== ''
                ? Number(detail.adjudication.adjustedBlockedProcessingFeeAmount)
                : undefined,
            adjustedEffectiveQualityDeductionAmount: detail.adjudication.result === 'PARTIALLY_ADJUSTED' && detail.adjudication.adjustedEffectiveQualityDeductionAmount !== ''
                ? Number(detail.adjudication.adjustedEffectiveQualityDeductionAmount)
                : undefined,
            adjustmentReasonSummary: detail.adjudication.result === 'PARTIALLY_ADJUSTED'
