import { initialDeductionBasisItems, initialReturnInboundBatches, normalizeQcForView, RETURN_INBOUND_PROCESS_LABEL, RETURN_INBOUND_QC_POLICY_LABEL, SEW_POST_PROCESS_MODE_LABEL, escapeHtml, formatDateTime, toClassName, NEEDS_AFFECTED_QTY, RESULT_LABEL, RESULT_CLASS, STATUS_LABEL, STATUS_CLASS, DISPOSITION_LABEL, DEDUCTION_DECISION_LABEL, ROOT_CAUSE_LABEL, LIABILITY_LABEL, PARTY_TYPE_LABEL, processTasks, ensureDetailState, getCurrentSearchParams, getQcById, getReturnInboundBatchById, requiresFinalDecisionForForm, toInputValue, } from './context';
import { buildDeductionEntryHrefByBasisId, buildQcDetailHref, getQcChainFactByRouteKey, getSettlementImpactLabel, } from '../../data/fcs/quality-chain-adapter';
import { buildQualityDeductionConfirmationPrintLink, buildQualityDisputeProcessingPrintLink, } from '../../data/fcs/fcs-route-links';
import { getPlatformQcDetailViewModelByRouteKey, } from '../../data/fcs/quality-deduction-selectors';
import { renderPdaFrame } from '../pda-shell';
const INSPECTION_SCENE_LABEL = {
    RETURN_INBOUND: '回货质检',
...
const INSPECTION_METHOD_LABEL = {
    COUNT_ONLY: '数量复核',
    SAMPLING: '抽检',
    FULL_INSPECTION: '全检',
};
function getInspectionSceneLabel(qc) {
    if (!qc.inspectionScene)
        return '回货质检';
    return INSPECTION_SCENE_LABEL[qc.inspectionScene] ?? '回货质检';
}
function getInspectionMethodLabel(qc) {
    if (!qc.inspectionMethod)
        return '抽检';
    return INSPECTION_METHOD_LABEL[qc.inspectionMethod] ?? '抽检';
}
function renderDispositionOptions(selected) {
    return `
...
function renderRootCauseOptions(selected) {
...
function renderLiabilityStatusOptions(selected) {
...
function renderPartyTypeOptions(selected) {
    return `
...
function renderBreakdownCard(detail, existingQc) {
    const target = existingQc.affectedQty;
    const sum = (Number(detail.bdAcceptDefect) || 0) +
    ...
    <section class="rounded-md border bg-card">
      ...
      <div class="space-y-4 px-4 py-4">
        ...
        <div class="${toClassName('flex flex-wrap gap-4 rounded-md border px-3 py-2 text-sm', valid ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50')}">
...
function renderChainOverview(params) {
    const { qc, batchId, warehouseName, returnFactoryName, processLabel, basisCount, basisReadyCount, basisFrozenCount, basisAmountTotal, evidenceCount, settlementImpactLabel, settlementSummary, disputeSummary, } = params;
    const decisionText = qc.deductionDecision === 'DEDUCT'
    ...
    <section class="grid gap-3 md:grid-cols-3">
      ...
      <article class="rounded-md border bg-card px-4 py-3">
        ...
        <div class="mt-1 flex flex-wrap items-center gap-2">
          <span class="inline-flex rounded-md border px-2 py-0.5 text-xs ${qc.liabilityStatus === 'FACTORY'
            ...
            : qc.liabilityStatus === 'MIXED'
                ...
                : qc.liabilityStatus === 'CONFIRMED'
                    ...
                    : qc.liabilityStatus === 'DISPUTED'
                        ...
                        : 'border-slate-200 bg-slate-50 text-slate-600'}">${escapeHtml(LIABILITY_LABEL[qc.liabilityStatus] ?? qc.liabilityStatus)}</span>
        </div>
...
function renderDetailNotFound(qcId) {
    return `
...
function renderPcFieldGrid(items, columnsClass = 'md:grid-cols-2 xl:grid-cols-3') {
    return `
...
function renderPcSection(title, description, body) {
    return `
...
function renderOverviewCard(title, rows, footer) {
    return `
...
function getResultExplanation(result, inspectedQty, qualifiedQty, unqualifiedQty) {
    if (result === 'PASS') {
...
function formatAuditDetail(detail, resultLabel) {
    return escapeHtml(detail).replace(/\bPASS\b/g, '合格').replace(/\bFAIL\b/g, resultLabel);
}
function renderBasisLinkGroup(basisItems) {
...
const FACTORY_RESPONSE_ACTION_LABEL = {
    CONFIRM: '确认',
    DISPUTE: '发起异议',
    AUTO_CONFIRM: '自动确认',
};
...
function getFactoryResponseBadgeClass(status) {
    switch (status) {
...
function getDisputeBadgeClass(status) {
    switch (status) {
        case 'PENDING_REVIEW':
        case 'IN_REVIEW':
            return 'border-amber-200 bg-amber-50 text-amber-700';
...
function getLiabilityBadgeClass(status) {
    switch (status) {
        case 'FACTORY':
            return 'border-red-200 bg-red-50 text-red-700';
        case 'NON_FACTORY':
...
function getBasisBadgeClass(statusLabel) {
    if (statusLabel === '已生效')
        return 'border-emerald-200 bg-emerald-50 text-emerald-700';
    if (statusLabel === '已调整')
        return 'border-blue-200 bg-blue-50 text-blue-700';
...
function formatDeadlineSummary(deadline, isOverdue) {
    if (!deadline)
        return '—';
...
function renderEvidenceAssets(assets, emptyText) {
    if (assets.length === 0) {
...
function renderAdjudicationResultOptions(selected) {
    return `
...
function renderAdjudicationPanel(detailVm, detail) {
    if (!detailVm.canHandleDispute || !detailVm.disputeCase)
        return '';
...
function renderExistingQcPcDetail(detailVm, detail) {
    ...
    const statusClass = STATUS_CLASS[qcRecord.qcStatus];
    const statusLabel = STATUS_LABEL[qcRecord.qcStatus];
    const resultExplanation = getResultExplanation(detailVm.qcResultDisplay, qcRecord.inspectedQty, qcRecord.qualifiedQty, qcRecord.unqualifiedQty);
    const liabilitySummary = qcRecord.responsiblePartyType || qcRecord.responsiblePartyId
        ...
        : '待确认';
    const defectSummary = qcRecord.defectItems.length > 0
    ...
    const disputeSectionClass = focusSection === 'dispute'
    ...
    const logSection = qcRecord.auditLogs.length > 0
        ? renderPcSection('操作日志', '保留共享质检事实的操作日志，便于核对提交、判责、异议和结算写回节点。', `
            <ol class="space-y-3">
              ${qcRecord.auditLogs
            .map((log) => `
    ...
    <div class="flex flex-col gap-6 p-6">
      ...
      ${renderPcSection('基本信息', '平台统一查看检查来源、现场主体和责任对象。', renderPcFieldGrid([
      ...
      ${renderPcSection('缺陷与检查证据', '这里只展示现场缺陷和检查证据，用于先确认发生了什么。', `
          <div class="space-y-5">
            ${renderPcFieldGrid([
        {
            label: '缺陷类型',
            value: qcRecord.defectItems.length > 0
      ...
      ${renderPcSection('责任判定与不合格品处理', detailVm.showUnqualifiedHandling
        ? '该区块只说明谁负责、责任数量是多少，以及不合格品如何处理。'
        : '该记录全部合格，仅保留责任与金额空态，不展示完整不合格品处理流程。', detailVm.showUnqualifiedHandling
        ? `
              <div class="space-y-5">
                ${renderPcFieldGrid([
      ...
      ${renderPcSection('工厂响应与异议', '先看工厂是否已响应、是否超时，再顺着看异议提交、平台裁决和回写结果。', `
        ...
        : `
            ...
            {
                label: '金额变化',
                value: disputeCase.adjudicatedAmount !== undefined
      ...
      ${renderPcSection('扣款与结算', '这里统一查看扣款依据、正式质量扣款流水与预结算衔接，不再混入旧的调整主链口径。', `
          <div class="space-y-5">
        ...
        ? `
                    <div class="rounded-md border border-dashed bg-card px-4 py-4 text-sm text-muted-foreground">
                      兼容说明：当前仍保留旧兼容映射字段以保证历史页面可读，但新主链只认正式质量扣款流水；本条记录当前${escapeHtml(settlementAdjustment.summary)}。
                    </div>
                  `
        : ''}
            </div>
...
function renderQcRecordDetailPageByVariant(qcId, variant) {
    ...
    const needsQty = detail.form.disposition !== '' && NEEDS_AFFECTED_QTY.includes(detail.form.disposition);
    ...
    const sourceTaskForView = (inboundView?.sourceTaskId ? processTasks.find((item) => item.taskId === inboundView.sourceTaskId) ?? null : null) ??
        refTask ??
        null;
    const maxQty = selectedBatch?.returnedQty ?? refTask?.qty;
    const basisItems = chainFact?.basisItems ?? (detail.currentQcId
        ? initialDeductionBasisItems.filter((item) => item.sourceRefId === detail.currentQcId || item.sourceId === detail.currentQcId)
        : []);
    ...
    const sourceTypeLabel = inboundView?.isReturnInbound || detail.form.refType === 'RETURN_BATCH'
        ? '回货入仓批次'
    ...
    <div class="mx-auto flex max-w-3xl flex-col gap-5 px-4 py-6">
      <div class="flex items-start gap-3">
        ...
        ${existingQc
        ? `<span class="inline-flex rounded-md border px-2 py-0.5 text-xs ${STATUS_CLASS[existingQc.status]}">${STATUS_LABEL[existingQc.status]}</span>`
        : ''}
      </div>
      ...
      ${existingQc
        ? renderChainOverview({
            ...
            returnFactoryName: inboundView?.returnFactoryName || selectedBatch?.returnFactoryName || sourceTaskForView?.assignedFactoryName || '-',
            processLabel: inboundView?.processLabel ||
      ...
      <section class="rounded-md border bg-card">
        ...
        <div class="space-y-4 px-4 py-4">
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div class="space-y-1.5">
              <label class="text-sm">${detail.form.refType === 'RETURN_BATCH'
        ? '回货批次号'
        : detail.form.refType === 'TASK'
            ? '任务 ID'
            : '交接事件 ID'}</label>
        ...
        ? `
            ...
            .map((batch) => `<option value="${escapeHtml(batch.batchId)}" ${detail.form.refId === batch.batchId ? 'selected' : ''}>${escapeHtml(batch.batchId)} · ${escapeHtml(batch.processLabel ?? RETURN_INBOUND_PROCESS_LABEL[batch.processType])} · ${escapeHtml(batch.productionOrderId)}</option>`)
            .join('')}
                    </select>
                  `
        : `
      ...
      <section class="rounded-md border bg-card">
        ...
        : selectedBatch
          ...
          <div>
            ...
            <p>${inboundView?.sewPostProcessMode
      ...
      <section class="rounded-md border bg-card">
        ...
        .map((result) => `
                    <button
                      class="${toClassName('rounded-md border px-5 py-2 text-sm font-medium transition-colors', detail.form.result === result
        ? result === 'PASS'
            ? 'border-green-600 bg-green-600 text-white'
            : 'border-red-600 bg-red-600 text-white'
        : 'bg-background text-muted-foreground hover:border-foreground', readOnly && 'cursor-not-allowed opacity-70')}"
            ...
            ? `
                    <article class="rounded-md border bg-card">
                      ...
                      <div class="grid grid-cols-1 gap-3 px-4 py-4 text-sm md:grid-cols-2">
                        ...
                        <div>
                          ...
                          <p>${existingQc.responsiblePartyType
                ...
                : '-'}</p>
                        ...
                        <div>
                          ...
                          <p>${existingQc.deductionDecision
                ...
                : '-'}</p>
                        ...
                        <div>
                          ...
                          <p>${existingQc.deductionDecision === 'DEDUCT'
                ...
                .map((basis) => `
                              <div class="space-y-1.5 rounded-md border bg-background px-3 py-2.5 text-sm">
                                <div class="flex flex-wrap items-center gap-2">
                                  ...
                                  <span class="inline-flex rounded-md border px-2 py-0.5 text-xs ${basis.status === 'CONFIRMED'
                ? 'border-green-200 bg-green-100 text-green-800'
                : basis.status === 'DISPUTED'
                    ? 'border-yellow-200 bg-yellow-100 text-yellow-800'
                    : basis.status === 'VOID'
                        ? 'bg-muted text-muted-foreground'
                        : 'bg-muted text-muted-foreground'}">${basis.status === 'CONFIRMED' ? '已确认' : basis.status === 'DISPUTED' ? '争议中' : basis.status === 'VOID' ? '已作废' : '草稿'}</span>
                                </div>
                                ${basis.summary
...
export function renderQcRecordDetailPage(qcId) {
    return renderQcRecordDetailPageByVariant(qcId, 'web');
}
export function renderQcRecordMobileDetailPage(qcId) {
