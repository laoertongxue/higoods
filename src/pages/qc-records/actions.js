import { appStore, initialDeductionBasisItems, initialQualityInspections, initialReturnInboundBatches, defaultResponsibility, blockTaskForReturnInboundQc, findReturnInboundBatchForQc, isReturnInboundInspection, requiresFinalLiabilityDecision, resolveReturnInboundTaskId, upsertDeductionBasisFromReturnInboundQc, processTasks, showQcRecordsToast, getQcById, getReturnInboundBatchById, applyReturnInboundBatchToForm, requiresFinalDecisionForForm, replaceQc, syncDetailFromQc, nowTimestamp, randomSuffix, parseNumberField, parseAmountField, NEEDS_AFFECTED_QTY, } from './context';
...
function blockTaskForQuality(task, qcId, by, now) {
...
function upsertDeductionBasisFromQc(qc, parentTask, by, now) {
    const basisQty = qc.affectedQty && qc.affectedQty > 0
        ? qc.affectedQty
        : qc.defectItems.reduce((sum, item) => sum + item.qty, 0);
    if (basisQty <= 0)
        return;
    const existing = initialDeductionBasisItems.find((item) => item.sourceRefId === qc.qcId || item.sourceId === qc.qcId);
    ...
    const summary = qc.defectItems.length > 0
...
function submitQcRecord(qcId, by) {
    const qc = getQcById(qcId);
    if (!qc)
        return { ok: false, message: '质检单不存在' };
    const now = nowTimestamp();
    ...
    if (finalLiabilityRequired) {
        ...
        if (!qc.disposition)
            return { ok: false, message: '车缝回货入仓质检提交前必须填写不合格品处置方式' };
        if (!qc.deductionDecision)
            return { ok: false, message: '车缝回货入仓质检提交前必须明确是否扣款' };
    ...
    const updated = {
        ...
        deductionCurrency: finalLiabilityRequired && qc.deductionDecision === 'DEDUCT'
...
function updateQcDispositionBreakdown(qcId, breakdown, by) {
    const qc = getQcById(qcId);
    if (!qc)
        return { ok: false, message: '质检单不存在' };
    if (qc.result !== 'FAIL')
        return { ok: false, message: '仅 FAIL 质检单可保存处置拆分' };
    ...
    for (const basis of initialDeductionBasisItems) {
        if (basis.status === 'VOID')
            continue;
        if (!(basis.sourceRefId === qcId || basis.sourceId === qcId))
            continue;
...
function buildPayload(form, existing) {
    ...
    const basePayload = {
        ...
        affectedQty: isFail && needsQty && form.affectedQty !== ''
        ...
        deductionAmount: isFail && finalLiabilityRequired && form.deductionDecision === 'DEDUCT' && form.deductionAmount !== ''
            ? Number(form.deductionAmount)
            : undefined,
        deductionCurrency: isFail && finalLiabilityRequired && form.deductionDecision === 'DEDUCT'
            ? 'CNY'
            : undefined,
        deductionDecisionRemark: isFail && finalLiabilityRequired ? form.deductionDecisionRemark.trim() || undefined : undefined,
    ...
    if ((existing?.inspectionScene === 'RETURN_INBOUND' ||
...
function validateForm(form, forSubmit, existing) {
    if (!form.refId.trim())
        return '请填写引用 ID（回货批次号 / 任务 ID / 交接事件 ID）';
    ...
    if (!forSubmit)
        return null;
    if (form.result === 'FAIL') {
        if (form.defectItems.length === 0)
            return '不合格时至少填写一条缺陷明细';
        for (const defect of form.defectItems) {
            if (!defect.defectName.trim())
                return '缺陷名称不能为空';
            if (!defect.qty || defect.qty < 1)
                return '缺陷数量须大于等于 1';
        }
        if (!form.disposition)
            return '请选择不合格品处置方式';
        ...
        if (requiresFinalDecisionForForm(form, existing)) {
            if (!form.responsiblePartyType)
                return '车缝回货入仓质检提交前必须选择责任方类型';
            if (!form.responsiblePartyId.trim())
                return '车缝回货入仓质检提交前必须填写责任方';
            if (!form.disposition)
                return '车缝回货入仓质检提交前必须填写不合格品处置方式';
            if (!form.deductionDecision)
                return '车缝回货入仓质检提交前必须明确是否扣款';
...
function updateFormField(detail, field, value) {
    if (field === 'refType') {
        if (value === 'HANDOVER') {
    ...
    if (field === 'rootCauseType') {
        detail.form.rootCauseType = value || 'UNKNOWN';
        return;
    }
    if (field === 'liabilityStatus') {
        detail.form.liabilityStatus = value || 'DRAFT';
        return;
    }
    if (field === 'responsiblePartyType') {
        detail.form.responsiblePartyType = (value || '');
        return;
    }
    ...
    if (field === 'deductionDecision') {
        detail.form.deductionDecision = (value || '');
...
function setResult(detail, result) {
...
function isDetailReadOnly(detail) {
    if (!detail.currentQcId)
        return false;
    const existing = getQcById(detail.currentQcId);
    return existing?.status === 'SUBMITTED' || existing?.status === 'CLOSED';
}
export { createQc, submitQcRecord, updateQcDispositionBreakdown, saveDraft, submitDetail, updateFormField, setResult, isDetailReadOnly, };
