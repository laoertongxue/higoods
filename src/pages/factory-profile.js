...
import { listFactoryMasterRecords, removeFactoryMasterRecord, upsertFactoryMasterRecord, } from '../data/fcs/factory-master-store';
import { cooperationModeConfig, factoryStatusConfig, factoryTierConfig, factoryTypeConfig, typesByTier, } from '../data/fcs/factory-types';
import { getProcessDefinitionByCode, listCraftsByProcessCode, listProcessStages, listProcessesByStageCode, } from '../data/fcs/process-craft-dict';
import { DEFAULT_FACTORY_MOBILE_APP_ROLE_ID, createFactoryPdaUser, ensureFactoryPdaSeed, findFactoryPdaUserByLoginId, listAllFactoryPdaRoles, listAllFactoryPdaUsers, listFactoryPdaRoles, listFactoryPdaUsers, permissionCatalog, removeFactoryPdaDataByFactory, resetFactoryPdaUserPassword, replaceFactoryPdaRoles, setFactoryPdaUserRole, toggleFactoryPdaUserLock, } from '../data/fcs/store-domain-pda';
...
function cloneProcessAbility(item) {
    return {
...
function getPostBusinessActionOptions(factoryType) {
    return factoryType === 'SATELLITE_FINISHING'
        ? [...POST_BUSINESS_ACTIONS]
        : [POST_BUSINESS_ACTIONS[0]];
}
function isDedicatedPostFactoryType(factoryType) {
    return factoryType === 'SATELLITE_FINISHING';
}
...
function mapInitialPdaUsersByFactory() {
    const grouped = {};
    for (const user of listAllFactoryPdaUsers()) {
        ...
        if (!grouped[user.factoryId])
            grouped[user.factoryId] = [];
        grouped[user.factoryId].push(record);
    }
    return grouped;
}
function clonePdaUserRecord(user) {
    return {
        userId: user.userId,
        factoryId: user.factoryId,
        name: user.name,
...
function renderTestFactoryBadge(factory) {
...
function mapInitialPdaRolesByFactory() {
    const grouped = {};
    for (const role of listAllFactoryPdaRoles()) {
        if (!grouped[role.factoryId])
            grouped[role.factoryId] = [];
...
function ensureTypeForTier(data) {
...
function createFormData(factory) {
    if (!factory)
        return cloneFormDraft(DEFAULT_FORM_DATA);
    return {
        name: factory.name,
...
function getSelectedCraftCodes(processAbilities, processCode) {
    return processAbilities.find((item) => item.processCode === processCode)?.craftCodes ?? [];
}
function getSelectedCapacityNodeCodes(processAbilities, processCode) {
    return processAbilities.find((item) => item.processCode === processCode)?.capacityNodeCodes ?? [];
}
function upsertProcessAbility(processAbilities, processCode, craftCodes, capacityNodeCodes) {
    ...
    if (nextCraftCodes.length > 0 || (nextCapacityNodeCodes?.length ?? 0) > 0) {
        ...
        nextAbilities.push({
            ...
            processName: process?.processName ?? processCode,
            craftNames: processCode === 'POST_FINISHING'
            ...
            abilityName: processCode === 'SPECIAL_CRAFT' && nextCraftCodes.length === 1
            ...
            abilityScope: processCode === 'POST_FINISHING'
                ? 'PROCESS'
...
function openEditDialog(factoryId) {
...
function getCurrentDialogFactoryId() {
    return state.dialog.type === 'edit' ? state.dialog.factoryId : null;
}
function ensurePdaDataForFactory(factory) {
...
function getFactoryPdaUsers(factoryId) {
    return state.pdaUsersByFactory[factoryId] ?? [];
}
function getFactoryPdaRoles(factoryId) {
    return state.pdaRolesByFactory[factoryId] ?? [];
}
function setFactoryPdaUsers(factoryId, users) {
    state.pdaUsersByFactory = {
        ...state.pdaUsersByFactory,
        [factoryId]: users,
    };
}
function setFactoryPdaRoles(factoryId, roles) {
...
function setPdaError(message) {
    state.pdaError = message;
}
function clearPdaError() {
    state.pdaError = '';
}
function validatePdaPasswordDraft(password, confirmPassword) {
    if (!password)
        return '请输入登录密码';
    if (!confirmPassword)
        return '请再次输入登录密码';
...
function openRoleFormEdit(factoryId, roleId) {
...
function copyRolePermissions(factoryId, roleId) {
    const role = getFactoryPdaRoles(factoryId).find((item) => item.roleId === roleId);
    if (!role)
        return;
    state.pdaRoleFormPerms = [...role.permissionKeys];
...
function setRoleGroupPermissions(group, selectAll) {
    ...
    if (selectAll) {
        ...
        state.pdaRoleFormPerms = state.pdaRoleFormPerms.filter((item) => !groupPermissionKeys.includes(item));
    }
    clearPdaError();
}
async function createPdaUser(factoryId) {
...
function togglePdaUserLock(factoryId, userId) {
    toggleFactoryPdaUserLock(userId, 'ADMIN');
    setFactoryPdaUsers(factoryId, listFactoryPdaUsers(factoryId).map(clonePdaUserRecord));
    clearPdaError();
}
function setPdaUserRole(factoryId, userId, roleId) {
    const roles = getFactoryPdaRoles(factoryId);
...
function openPdaResetPassword(factoryId, userId) {
...
async function submitPdaResetPassword(factoryId) {
    const userId = state.pdaResetPasswordUserId;
    if (!userId) {
        setPdaError('未找到账号，无法重置密码。');
        return;
    }
    const passwordError = validatePdaPasswordDraft(state.pdaResetPasswordValue, state.pdaResetPasswordConfirm);
    if (passwordError) {
        setPdaError(passwordError);
        return;
    }
...
function savePdaRole(factoryId) {
    ...
    if (state.pdaEditingRoleId) {
        const nextRoles = roles.map((role) => role.roleId === state.pdaEditingRoleId
            ? {
                ...role,
                roleName,
...
function togglePdaRoleStatus(factoryId, roleId) {
    const roles = getFactoryPdaRoles(factoryId);
    const nextRoles = roles.map((role) => role.roleId === roleId
        ? {
            ...role,
...
function getVisibleFactories() {
    ...
    if (state.searchKeyword.trim()) {
        ...
        result = result.filter((factory) => {
            return (factory.name.toLowerCase().includes(keyword) ||
...
function getPagedFactories(filteredFactories) {
    const start = (state.currentPage - 1) * PAGE_SIZE;
    return filteredFactories.slice(start, start + PAGE_SIZE);
}
function getEditingFactory() {
...
function renderFactoryTableRows(factories) {
    ...
    return factories
        ...
        <tr class="border-b last:border-0 hover:bg-muted/30" data-factory-id="${factory.id}">
          ...
          <td class="px-3 py-3">
            ...
            .map((flag) => {
            const enabled = factory.eligibility[flag.key];
            return `<span class="inline-flex items-center justify-center rounded border px-1 py-0.5 text-[10px] font-medium ${enabled
                ? 'border-green-200 bg-green-50 text-green-700'
                : 'border-gray-200 bg-gray-100 text-gray-400'}">${flag.label}</span>`;
        })
            .join('')}
            </div>
          </td>
...
function renderPdaUsersSection(factoryId, pdaEnabled) {
    if (!factoryId) {
    ...
    const activeRoles = roles.filter((role) => role.status === 'ACTIVE');
    const resetTargetUser = state.pdaResetPasswordOpen && state.pdaResetPasswordUserId
        ...
        : `
            ...
            return `
                          ...
                          <td class="px-3 py-2">
                            <div class="space-y-1">
                              <select data-pda-user-role="${user.userId}" class="h-7 min-w-[120px] rounded border px-2 text-xs ${!pdaEnabled ? 'pointer-events-none bg-muted opacity-70' : ''}">
                                ${activeRoles
                .map((role) => `<option value="${role.roleId}" ${user.roleId === role.roleId ? 'selected' : ''}>${escapeHtml(role.roleName)}</option>`)
                .join('')}
                              </select>
                              ${roleDisabled
                ...
                : ''}
                          ...
                          <td class="px-3 py-2">
                            <span class="inline-flex rounded border px-2 py-0.5 text-xs ${user.status === 'ACTIVE'
                ? 'border-blue-200 bg-blue-50 text-blue-700'
                : 'border-slate-200 bg-slate-100 text-slate-700'}">${user.status === 'ACTIVE' ? '启用' : '锁定'}</span>
                          </td>
            ...
            <div class="rounded-md border bg-muted/30 p-4">
              ...
              <label class="mt-3 block space-y-1">
                ...
                <select data-pda-field="new-user-role" class="h-8 w-full rounded border px-2 text-sm">
                  ${activeRoles
            .map((role) => `<option value="${role.roleId}" ${state.pdaNewRoleId === role.roleId ? 'selected' : ''}>${escapeHtml(role.roleName)}</option>`)
            .join('')}
                </select>
              </label>
...
function renderPdaRoleForm(factoryId, pdaEnabled) {
    if (!state.pdaRoleFormOpen)
        return '';
    const roles = getFactoryPdaRoles(factoryId);
    const isEdit = Boolean(state.pdaEditingRoleId);
    const groups = permissionCatalog.reduce((acc, item) => {
        if (!acc[item.group])
            acc[item.group] = [];
        ...
        : `
              <label class="space-y-1">
                ...
                <select data-pda-field="role-copy-from" class="h-8 w-full rounded border px-2 text-sm">
                  ...
                  ${roles
            .map((role) => `<option value="${role.roleId}" ${state.pdaCopyFromRoleId === role.roleId ? 'selected' : ''}>${escapeHtml(role.roleName)}</option>`)
            .join('')}
                </select>
              </label>
            `}
...
function renderPdaRolesSection(factoryId, pdaEnabled) {
    if (!factoryId) {
        ...
        : `
            ...
            return `
                        <tr class="border-b last:border-0">
                          ...
                          <td class="px-3 py-2">
                            <span class="inline-flex rounded border px-2 py-0.5 text-xs ${role.status === 'ACTIVE'
                ? 'border-blue-200 bg-blue-50 text-blue-700'
                : 'border-slate-200 bg-slate-100 text-slate-700'}">${role.status === 'ACTIVE' ? '启用' : '禁用'}</span>
                          </td>
...
function renderPdaPermissionsSection() {
...
function renderFactoryDrawer() {
    ...
    const parentCandidates = state.factories.filter((factory) => factory.factoryTier === 'CENTRAL' && (!editingFactory || factory.id !== editingFactory.id));
    ...
    <div class="fixed inset-0 z-50">
      ...
      <section class="absolute inset-y-0 right-0 flex w-full flex-col border-l bg-background shadow-2xl sm:max-w-[720px]">
        ...
        .map(([key, config]) => `<option value="${key}" ${draft.status === key ? 'selected' : ''}>${escapeHtml(config.label)}</option>`)
        .join('')}
                  </select>
                </label>
                <label class="space-y-1.5">
        ...
        .map(([key, config]) => `<option value="${key}" ${draft.cooperationMode === key ? 'selected' : ''}>${escapeHtml(config.label)}</option>`)
        .join('')}
                  </select>
                </label>
              </div>
        ...
        .map(([key, config]) => `<option value="${key}" ${draft.factoryTier === key ? 'selected' : ''}>${escapeHtml(config.label)}</option>`)
        .join('')}
                  </select>
                </label>
                <label class="space-y-1.5">
                  ...
                  <select data-factory-field="factoryType" class="w-full rounded-md border px-3 py-2 text-sm">
                    ${availableTypes
        .map((type) => `<option value="${type}" ${draft.factoryType === type ? 'selected' : ''}>${escapeHtml(factoryTypeConfig[type].label)}</option>`)
        .join('')}
                  </select>
              ...
              <label class="space-y-1.5">
                ...
                <select data-factory-field="parentFactoryId" class="w-full rounded-md border px-3 py-2 text-sm">
                  ...
                  ${parentCandidates
        .map((factory) => `<option value="${factory.id}" ${draft.parentFactoryId === factory.id ? 'selected' : ''}>${escapeHtml(factory.name)} (${escapeHtml(factory.code)})</option>`)
        .join('')}
                </select>
        ...
        .map((item) => {
        const checked = draft.eligibility[item.key];
        return `
        ...
        : ''}
              ...
              <div class="grid w-full grid-cols-3 rounded-md border p-1">
                <button type="button" data-fast-page-render="true" data-factory-action="switch-pda-tab" data-pda-tab="users" class="rounded px-2 py-1 text-xs ${state.pdaTab === 'users' ? 'bg-blue-50 text-blue-700' : 'text-muted-foreground hover:bg-muted'}">账号列表</button>
                <button type="button" data-fast-page-render="true" data-factory-action="switch-pda-tab" data-pda-tab="roles" class="rounded px-2 py-1 text-xs ${state.pdaTab === 'roles' ? 'bg-blue-50 text-blue-700' : 'text-muted-foreground hover:bg-muted'}">角色管理</button>
                <button type="button" data-fast-page-render="true" data-factory-action="switch-pda-tab" data-pda-tab="permissions" class="rounded px-2 py-1 text-xs ${state.pdaTab === 'permissions' ? 'bg-blue-50 text-blue-700' : 'text-muted-foreground hover:bg-muted'}">权限矩阵</button>
              </div>
        ...
        : state.pdaTab === 'roles'
          ...
          <footer class="border-t px-6 py-4">
            ...
            <div class="flex items-center justify-between gap-3">
              ...
              <button type="submit" class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">${state.dialog.type === 'edit' ? '保存' : '创建工厂'}</button>
            </div>
          </footer>
        </form>
      </section>
...
export function renderFactoryProfilePage() {
    ...
    const availableTypes = state.tierFilter !== 'all'
        ? typesByTier[state.tierFilter] ?? []
        : Object.keys(factoryTypeConfig);
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <select data-factory-filter="tier" class="rounded-md border px-3 py-2 text-sm">
          ...
          ${Object.entries(factoryTierConfig)
        .map(([key, config]) => `<option value="${key}" ${state.tierFilter === key ? 'selected' : ''}>${escapeHtml(config.label)}</option>`)
        .join('')}
        </select>
        ...
        <select data-factory-filter="type" class="rounded-md border px-3 py-2 text-sm">
          ...
          ${availableTypes
        .map((type) => `<option value="${type}" ${state.typeFilter === type ? 'selected' : ''}>${escapeHtml(factoryTypeConfig[type].label)}</option>`)
        .join('')}
        </select>
        ...
        <select data-factory-filter="status" class="rounded-md border px-3 py-2 text-sm">
          ...
          ${Object.entries(factoryStatusConfig)
        .map(([key, config]) => `<option value="${key}" ${state.statusFilter === key ? 'selected' : ''}>${escapeHtml(config.label)}</option>`)
        .join('')}
        </select>
...
function updateTypeFilterByTier() {
    if (state.tierFilter === 'all')
        return;
    const available = typesByTier[state.tierFilter] ?? [];
    if (available.length === 0)
        return;
    if (!available.includes(state.typeFilter)) {
        state.typeFilter = 'all';
    }
}
function upsertFactory(data, editingFactory) {
    if (editingFactory) {
        const nextFactory = {
            ...editingFactory,
            ...data,
...
export function handleFactoryPageEvent(target) {
    const formField = target.closest('[data-factory-field]');
    if ((formField instanceof HTMLInputElement || formField instanceof HTMLSelectElement) && state.formDraft) {
        ...
        if (field === 'status') {
            setDraft((prev) => ({ ...prev, status: formField.value }));
            return true;
        }
        if (field === 'cooperationMode') {
            setDraft((prev) => ({ ...prev, cooperationMode: formField.value }));
            return true;
        }
        if (field === 'factoryTier') {
            const nextTier = formField.value;
            setDraft((prev) => {
        ...
        if (field === 'factoryType') {
            setDraft((prev) => ({ ...prev, factoryType: formField.value }));
            return true;
        }
    ...
    const processToggleField = target.closest('[data-factory-process-toggle]');
    if (processToggleField instanceof HTMLInputElement && state.formDraft) {
        ...
        setDraft((prev) => ({
            ...prev,
            processAbilities: upsertProcessAbility(prev.processAbilities, processCode, craftCodes, processCode === 'POST_FINISHING' ? capacityNodeCodes : undefined),
        }));
        return true;
    }
    const craftToggleField = target.closest('[data-factory-craft-toggle]');
    ...
    const nodeToggleField = target.closest('[data-factory-node-toggle]');
    if (nodeToggleField instanceof HTMLInputElement && state.formDraft) {
        ...
        const nodeCode = nodeToggleField.dataset.factoryNodeToggle;
        if (!processCode || !nodeCode)
            return true;
        setDraft((prev) => {
    ...
    const pdaField = target.closest('[data-pda-field]');
    ...
    const rolePermissionField = target.closest('[data-pda-role-perm]');
    if (rolePermissionField instanceof HTMLInputElement) {
        const permissionKey = rolePermissionField.dataset.pdaRolePerm;
        if (!permissionKey)
            return true;
        toggleRolePermission(permissionKey, rolePermissionField.checked);
        return true;
    }
    const userRoleField = target.closest('[data-pda-user-role]');
    ...
    const filterNode = target.closest('[data-factory-filter]');
    if (filterNode instanceof HTMLInputElement || filterNode instanceof HTMLSelectElement) {
        ...
        if (filter === 'pda')
            state.pdaFilter = value;
        state.currentPage = 1;
        return true;
    }
    const actionNode = target.closest('[data-factory-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'switch-pda-tab') {
        const tab = actionNode.dataset.pdaTab;
    ...
    if (action === 'toggle-add-user') {
        if (!state.pdaAddOpen) {
            const factoryId = getCurrentDialogFactoryId();
            if (factoryId) {
                const activeRole = getFactoryPdaRoles(factoryId).find((role) => role.status === 'ACTIVE' && role.roleId === DEFAULT_FACTORY_MOBILE_APP_ROLE_ID) || getFactoryPdaRoles(factoryId).find((role) => role.status === 'ACTIVE');
                if (activeRole) {
    ...
    if (action === 'sort') {
        const field = actionNode.dataset.sortField;
        if (!field)
            return true;
        if (state.sortField === field) {
...
export function handleFactoryPageSubmit(form) {
    if (form.dataset.factoryForm !== 'true')
        return false;
    if (!state.formDraft) {
...
export function isFactoryPageOpenDialog() {
