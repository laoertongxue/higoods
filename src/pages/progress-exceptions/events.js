import { state, OWNER_OPTIONS, isSubCategoryKey, getSubCategoryOptions, getCaseById, getExceptionTotalPages, getOrderById, filterCases, getProductionOrderHandoverSummary, buildHandoverOrderDetailLink, mockInternalUsers, nowTimestamp, } from './context';
import { getClaimDisputeByCaseId, updateClaimDisputePlatformHandling } from '../../state/fcs-claim-dispute-store';
import { getPdaPickupDisputeByCaseId, updatePdaPickupDisputePlatformHandling, } from '../../helpers/fcs-pda-pickup-dispute';
import { clearFilters, assignCaseOwner, confirmUnblock, confirmExtendTender, confirmPauseFollowUp, confirmPauseAllowContinue, openCloseDialog, closeCloseDialog, confirmCloseException, createUrge, showProgressExceptionsToast, openLinkedPage, } from './actions';
function openProductionOrderTechPackSnapshot(productionOrderId) {
    openLinkedPage(`技术包快照-${productionOrderId}`, `/fcs/production/orders/${encodeURIComponent(productionOrderId)}/tech-pack`);
}
function syncClaimDisputeHandleForm(caseId) {
    if (!caseId) {
...
function syncPickupDisputeHandleForm(caseId) {
    if (!caseId) {
...
function updateField(field, node) {
    ...
    if (field === 'subCategoryFilter' && node instanceof HTMLSelectElement) {
        state.subCategoryFilter = node.value;
        state.currentPage = 1;
        return;
    }
    ...
    if (field === 'claimDisputeHandleStatus' && node instanceof HTMLSelectElement) {
        state.claimDisputeHandleStatus = node.value;
        return;
    }
    ...
    if (field === 'pickupDisputeHandleStatus' && node instanceof HTMLSelectElement) {
        state.pickupDisputeHandleStatus = node.value;
        return;
    }
...
function handleRowAction(action, actionNode) {
    if (action === 'row-view') {
    ...
    if (action === 'row-handover') {
        ...
        openLinkedPage('交接链路', buildHandoverOrderDetailLink({
    ...
    if (action === 'row-handover-objection') {
        const orderId = actionNode.dataset.orderId || '';
        const taskId = actionNode.dataset.taskId || '';
        openLinkedPage('数量异议', buildHandoverOrderDetailLink({
            productionOrderId: orderId,
            taskId,
            focus: 'objection',
            source: '异常定位',
...
function handleDrawerAction(action, actionNode) {
    ...
    if (action === 'drawer-go-craft-dispute') {
        ...
        openLinkedPage('待加工仓', `/fcs/craft/cutting/warehouse-management/wait-process${cutOrderNo ? `?cutOrderNo=${encodeURIComponent(cutOrderNo)}` : ''}`);
        return true;
    }
...
function handleAction(action, actionNode) {
    ...
    if (action === 'assign-owner') {
        const caseId = actionNode.dataset.caseId;
        if (!caseId || !(actionNode instanceof HTMLSelectElement))
            return true;
    ...
    if (action === 'submit-pickup-dispute-handle') {
        ...
        if (!caseId)
            return true;
        const finalResolvedQty = state.pickupDisputeHandleStatus === 'RESOLVED' ? Number(state.pickupDisputeHandleResolvedQty) : undefined;
        ...
        syncPickupDisputeHandleForm(caseId);
        showProgressExceptionsToast(state.pickupDisputeHandleStatus === 'RESOLVED'
...
export function handleProgressExceptionsEvent(target) {
    const fieldNode = target.closest('[data-pe-field]');
    ...
    const actionNode = target.closest('[data-pe-action]');
    if (!actionNode) {
