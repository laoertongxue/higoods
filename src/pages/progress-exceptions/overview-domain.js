import { state, OWNER_OPTIONS, REASON_LABEL, SEVERITY_COLOR_CLASS, CATEGORY_LABEL, SUB_CATEGORY_LABEL, getFactoryById, getProcessTypeByCode, getSubCategoryOptions, getExceptionCases, getExceptionTotalPages, getCaseFactoryId, getPagedCases, getTaskById, getUnifiedCategory, getSubCategoryLabel, getCaseFactoryName, getCaseProcessName, getRelatedObjects, getSpuFromCase, hasUpstreamFilter, renderBadge, renderStatusBadge, escapeAttr, escapeHtml, } from './context';
...
export function renderActionMenu(exc) {
...
export function renderUpstreamHint() {
    if (!state.showUpstreamHint || !hasUpstreamFilter())
        return '';
    return `
    <section class="flex items-center justify-between rounded-lg border border-blue-200 bg-blue-50 px-4 py-2">
      <div class="flex flex-wrap items-center gap-2 text-sm text-blue-700">
        ...
        ${state.upstreamReasonCode ? renderBadge(`原因: ${REASON_LABEL[state.upstreamReasonCode] || state.upstreamReasonCode}`, 'border-blue-200 bg-white text-blue-700') : ''}
...
export function renderCategoryQuickSwitch() {
    const options = [
        { key: 'ALL', label: '全部' },
...
export function getFactoryFilterOptions() {
    const map = new Map();
      ...
      ${state.aggregateFilter
        ? `
            <div class="mt-2 flex items-center gap-2">
              ...
              ${renderBadge(state.aggregateFilter.type === 'reason'
            ...
            : state.aggregateFilter.type === 'factory'
                ...
                : getProcessTypeByCode(state.aggregateFilter.value)?.nameZh || state.aggregateFilter.value, 'border-border bg-background text-foreground')}
...
export function renderTable(cases) {
