import { state, OWNER_OPTIONS, DIRECT_CLOSE_REASON_SET, CLOSE_REASON_LABEL, CASE_STATUS_LABEL, CATEGORY_LABEL, SEVERITY_COLOR_CLASS, RESOLVE_RULE_LABEL, RESOLVE_SOURCE_LABEL, getCaseById, getTaskById, getOrderById, getFactoryById, getTenderById, getReasonLabel, getSubCategoryLabel, getUnifiedCategory, getMaterialIssueRows, getHandoverCaseSnapshot, getResolveJudgeResult, getTaskStatusLabel, getRelatedObjects, normalizeCaseStatus, parseTimestampToMs, renderBadge, renderStatusBadge, escapeAttr, escapeHtml, } from './context';
...
function isCuttingClaimDisputeCase(detailCase) {
    return detailCase.sourceModule === 'CUTTING_CLAIM_DISPUTE';
}
function isPdaPickupDisputeCase(detailCase) {
    return detailCase.sourceModule === 'PDA_PICKUP_DISPUTE';
}
function renderClaimDisputeSourcePanel(detailCase) {
...
function renderClaimDisputeActionPanel(detailCase) {
    const dispute = getClaimDisputeByCaseId(detailCase.caseId);
    if (!dispute)
        return '';
    return `
...
function renderPdaPickupDisputeSourcePanel(detailCase) {
...
function renderPdaPickupDisputeActionPanel(detailCase) {
    const dispute = getPdaPickupDisputeByCaseId(detailCase.caseId);
    if (!dispute)
        return '';
    const record = dispute.record;
...
export function renderBasicTab(detailCase) {
...
export function renderRelatedTab(detailCase) {
...
export function renderSourceTab(detailCase) {
    const firstOrderId = detailCase.relatedOrderIds[0] || '-';
    const firstTaskId = detailCase.relatedTaskIds[0] || '-';
  ...
  `;
    const formatMilestoneSnapshot = (snapshot) => {
        if (!snapshot?.required)
            return '无强制关键节点';
        const statusLabel = snapshot.status === 'REPORTED' ? '已上报' : '待上报';
        const fallbackRule = snapshot.targetQty && snapshot.targetQty > 0
    ...
    if (unifiedCategory === 'ASSIGNMENT') {
      ...
      <div class="space-y-3 rounded-lg border border-blue-200 bg-blue-50 p-4">
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          ${renderKv('任务是否仍未分出', task ? (task.assignmentStatus === 'UNASSIGNED' || !task.assignedFactoryId ? '是' : '否') : '是')}
        </div>
        <p class="text-xs text-muted-foreground">
          候选工厂：${tender?.invitedFactoryIds?.length
            ? escapeHtml(tender.invitedFactoryIds
                .slice(0, 4)
      ...
      <div class="space-y-3 rounded-lg border border-amber-200 bg-amber-50 p-4">
        ...
        ${detailCase.sourceType === 'FACTORY_PAUSE_REPORT'
            ? `
              ...
              <p class="text-xs text-muted-foreground">
                暂停凭证：${detailCase.pauseProofFiles?.length || 0} 个 ｜ 关键节点：${formatMilestoneSnapshot(detailCase.milestoneSnapshot)}
              </p>
            `
            : ''}
      </div>
...
export function renderActionsTab(detailCase) {
    const firstTaskId = detailCase.relatedTaskIds[0] || '';
    const firstOrderId = detailCase.relatedOrderIds[0] || '';
    const firstTenderId = detailCase.relatedTenderIds[0] || '';
    const unifiedCategory = getUnifiedCategory(detailCase);
...
export function renderTimelineTab(detailCase) {
    const renderAuditDetail = (detail) => escapeHtml(detail)
...
export function renderCloseDialog() {
    if (!state.closeDialogCaseId)
        return '';
    ...
    <div class="fixed inset-0 z-[60]" data-dialog-backdrop="true">
      ...
      <section class="absolute left-1/2 top-1/2 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background p-6 shadow-2xl">
        ...
        .map(([value, label]) => `<option value="${value}" ${state.closeReason === value ? 'selected' : ''}>${escapeHtml(label)}</option>`)
        .join('')}
            </select>
          </div>
...
export function renderUnblockDialog() {
    if (!state.unblockDialogCaseId)
        return '';
...
export function renderExtendDialog() {
    if (!state.extendDialogCaseId)
        return '';
...
export function renderPauseFollowUpDialog() {
    if (!state.pauseFollowUpCaseId)
        return '';
