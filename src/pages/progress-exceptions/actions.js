import { state, DIRECT_CLOSE_REASON_SET, CLOSE_REASON_LABEL, nowTimestamp, getExceptionCases, getCaseById, getTaskById, getResolveJudgeResult, getExecutionTaskFactById, appStore, appendCaseAction, appendCaseAuditLog, appendCaseStatusChangeAudit, markCaseResolved, maybeAutoCloseResolvedCase, markCaseClosed, allowContinueFromPauseException, recordPauseExceptionFollowUp, generateNotificationId, generateUrgeId, initialNotifications, initialUrges, normalizeCaseStatus, upsertProgressExceptionCase, extendTenderDeadlineFromRuntime, } from './context';
export function syncExceptionResolvedByBusiness() {
...
export function updateException(updated) {
    upsertProgressExceptionCase(updated);
}
export function updateTaskStatus(taskId, newStatus, by = 'Admin') {
...
export function extendTenderDeadline(tenderId, hours = 24) {
...
export function createUrge(payload) {
...
export function showProgressExceptionsToast(message, tone = 'success') {
...
export function openLinkedPage(title, href) {
    appStore.openTab({
        key: href,
        title,
        href,
...
export function assignCaseOwner(exc, userId, userName) {
    const now = nowTimestamp();
...
export function confirmExtendTender() {
...
export function confirmPauseFollowUp() {
...
export function confirmCloseException() {
    ...
    const closeDetail = [
        ...
        .filter(Boolean)
        .join('，');
    updateException(markCaseClosed(exc, {
        by: 'Admin',
        reasonCode: reason,
        detail: closeDetail,
