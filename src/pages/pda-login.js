...
import { authenticateFactoryPdaUserByCredentials, authenticateFactoryPdaUserByLoginId, clearPdaSession, createPdaSessionFromUser, setPdaSession, } from '../data/fcs/store-domain-pda.ts';
...
import { authenticateFactoryOnboardingAdmin, createPdaOnboardingSessionFromApplication, getFactoryOnboardingLoginFailureMessage, getPdaCurrentAuthSession, resolvePdaPostLoginRoute, } from '../data/fcs/factory-onboarding-flow.ts';
...
async function submitLogin() {
    ...
    if (onboardingApplication) {
        ...
        appStore.navigate(resolvePdaPostLoginRoute({ kind: 'ONBOARDING', session, application: onboardingApplication }, returnTo), { historyMode: 'replace' });
        return;
    }
    state.errorText = getFactoryOnboardingLoginFailureMessage(loginId, password);
}
export function renderPdaLoginPage() {
...
export async function handlePdaLoginEvent(target) {
    const fieldNode = target.closest('[data-pda-login-field]');
    ...
    const actionNode = target.closest('[data-pda-login-action]');
