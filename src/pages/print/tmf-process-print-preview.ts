...
function sourceIds(input: PrintDocumentBuildInput): string[] {
  try {
    const parsed = JSON.parse(input.sourceId)
    if (Array.isArray(parsed) && parsed.every((value) => typeof value === 'string' && value.trim())) return [...new Set(parsed)]
  } catch { /* single work order id */ }
...
export function renderTmfProcessPrintPreviewPage(): string {
  ...
  try {
    const documents = sourceIds(input).map((sourceId) => buildTmfProcessSheetPrintDocument({ ...input, sourceId }))
    const document = documents[0]
    if (typeof window !== 'undefined') window.setTimeout(() => undefined, 0)
    return `${renderUnifiedPrintStyles()}<div class="print-preview-root" data-skip-page-rerender="true"><div class="print-preview-toolbar print-hidden"><div class="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-white p-3 shadow-sm"><div><h1 class="text-lg font-semibold">${escapeHtml(document.documentTitle)}打印预览${documents.length > 1 ? ` · ${documents.length} 单` : ''}</h1><p class="mt-1 text-xs text-muted-foreground">打印前请在浏览器打印设置中关闭页眉和页脚。该提示不会被打印。</p></div><div class="flex flex-wrap gap-2"><a class="rounded-md border px-3 py-2 text-sm" href="${escapeHtml(document.printMeta.returnHref || '/fcs/craft/accessory/webbing/work-orders')}" data-nav="${escapeHtml(document.printMeta.returnHref || '/fcs/craft/accessory/webbing/work-orders')}">返回业务单据</a><button class="rounded-md border px-3 py-2 text-sm hover:bg-slate-50" data-print-preview-action="download-pdf">下载 PDF</button><button class="rounded-md border px-3 py-2 text-sm hover:bg-slate-50" data-print-preview-action="print">打印</button></div></div></div><p class="print-hidden px-3 text-sm" role="status" data-print-ready-feedback></p>${documents.map((value) => `<section class="break-after-page" data-tmf-batch-print-source="${escapeHtml(value.sourceId)}">${renderTmfProcessSheetTemplate(value)}</section>`).join('')}</div>`
  } catch (error) {
...
async function prepareTmfPrint(button: HTMLElement): Promise<void> {
  ...
  try {
    const input = readInput()
    const currents = sourceIds(input).map((sourceId) => buildTmfProcessSheetPrintDocument({ ...input, sourceId }))
    const printed = Array.from(root.querySelectorAll<HTMLElement>('[data-tmf-print-signature]'))
    if (printed.length !== currents.length || printed.some((node, index) => node.dataset.tmfPrintSignature !== tmfPrintFactsSignature(currents[index]))) throw new Error('加工要求、数量或生产状态已变化，请重新打开预览后核对。')
