...
import { buildPrintBarcodePayload, buildPrintQrPayload, createPrintDocumentId, formatPrintQty, getPrintGeneratedAt, } from '../../../data/fcs/print-service.ts';
import { getSettlementChangeRequests, getSettlementRequestById, getSettlementStatusLabel, } from '../../../data/fcs/settlement-change-requests.ts';
import { getProcessHandoverRecordById, listProcessHandoverDifferenceRecords, } from '../../../data/fcs/process-warehouse-domain.ts';
import { getQualityDeductionCaseFactByDisputeId, listQualityDeductionCaseFacts, listQualityDeductionDisputeCases, } from '../../../data/fcs/quality-deduction-repository.ts';
import { QUALITY_DEDUCTION_DISPUTE_STATUS_LABEL, QUALITY_DEDUCTION_QC_RESULT_LABEL, } from '../../../data/fcs/quality-deduction-selectors.ts';
...
const masterDataChangeRequests = [
    {
...
function getChangedFieldsSummary(request) {
    const changed = [];
...
function getDifferenceRequestSource(sourceId) {
    const difference = listProcessHandoverDifferenceRecords().find((item) => item.differenceRecordId === sourceId || item.differenceRecordNo === sourceId)
        || listProcessHandoverDifferenceRecords()[0];
    if (!difference)
        throw new Error('缺少差异处理申请单来源数据');
    const handover = getProcessHandoverRecordById(difference.handoverRecordId);
    return { difference, handover };
}
function getQualityCaseByPending(sourceId) {
    const caseFact = listQualityDeductionCaseFacts({ includeLegacy: true })
        .find((item) => item.pendingDeductionRecord?.pendingRecordId === sourceId
    ...
    if (!caseFact || !caseFact.pendingDeductionRecord)
        throw new Error('缺少待确认质量扣款记录');
    return caseFact;
}
function getQualityCaseByDispute(sourceId) {
    const direct = getQualityDeductionCaseFactByDisputeId(sourceId);
    if (direct?.disputeCase)
        return direct;
    ...
    if (!caseFact || !caseFact.disputeCase)
        throw new Error('缺少质量异议处理单来源数据');
    return caseFact;
}
function getSettlementRequest(sourceId) {
...
export function buildSettlementChangeRequestPrintDocument(input) {
    const request = getSettlementRequest(input.sourceId);
    const accountRows = (snapshot) => mapFields([
...
export function buildHandoverDifferenceRequestPrintDocument(input) {
...
function buildQualityDeductionFields(caseFact, pending) {
...
export function buildQualityDeductionConfirmationPrintDocument(input) {
...
export function buildQualityDisputeProcessingPrintDocument(input) {
...
export function buildMasterDataChangeRequestPrintDocument(input) {
...
export function renderBusinessRequestFormTemplate(document) {
