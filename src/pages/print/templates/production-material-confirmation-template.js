...
import { buildProductionConfirmationSnapshot, getOrCreateProductionConfirmation, getProductionConfirmationByOrderId, getProductionConfirmationSnapshotById, isProductionConfirmationPrintable, productionConfirmationStatusLabels, } from '../../../data/fcs/production-confirmation.ts';
import { productionOrders, productionOrderStatusConfig, } from '../../../data/fcs/production-orders.ts';
...
function calcLossQty(row) {
    if (row.plannedUsageQty === null || row.lossRate === null)
        return '暂无数据';
...
function objectizedUnitConsumption(row) {
    if (row.unitConsumption === null)
        return '暂无数据';
    return row.materialType === '面料'
        ? `${row.unitConsumption} 米 / 件`
        : `${row.unitConsumption} ${materialUnitLabel(row)} / 件`;
}
function objectizedPlannedUsage(row) {
    if (row.plannedUsageQty === null)
        return '暂无数据';
    return row.materialType === '面料'
...
function imageBlock(title, images, fallbackLabel = '暂无图片') {
    return {
        title,
...
export function resolveProductionPrintImages(productionOrderId) {
...
export function resolveMaterialPrintImages(productionOrderId) {
    const snapshot = buildProductionConfirmationSnapshot(productionOrderId);
    return [
        imageBlock('面料图', snapshot.imageSnapshot.materialImages),
        imageBlock('辅料图', snapshot.imageSnapshot.accessoryImages),
...
function buildSkuMatrixTable(snapshot) {
...
function buildMaterialTable(snapshot) {
    return {
...
function buildProcessTable(snapshot) {
    return {
...
function buildPatternTable(snapshot) {
    const rows = snapshot.patternSnapshot.rows.flatMap((pattern) => pattern.pieceRows.length
...
function buildHeaderFields(context) {
...
function buildProductionSections(context) {
    const { order, snapshot } = context;
    return [
        {
            sectionId: 'base',
...
function buildProductionTables(snapshot) {
    return [
...
export function buildProductionMaterialConfirmationPrintDocument(input) {
    return buildDocument(input);
}
export function buildProductionConfirmationPrintDocument(input) {
...
function renderFieldGrid(fields, columns = 4) {
    return `
...
function renderImageBlocks(blocks, prominent) {
    return `
...
export function renderProductionMaterialConfirmationTemplate(doc) {
    return `
