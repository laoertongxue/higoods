...
import { buildTaskDeliveryCardPrintDocByRecordId, resolveTaskPrintImage, } from '../../../data/fcs/task-print-cards.ts';
import { getDifferenceRecordsByHandoverRecordId, getProcessHandoverRecordById, getProcessWarehouseRecordById, listProcessWarehouseReviewRecords, } from '../../../data/fcs/process-warehouse-domain.ts';
import { getPostFinishingSourceLabel, getPostFinishingWorkOrderById, } from '../../../data/fcs/post-finishing-domain.ts';
import { createPrintDocumentId, formatPrintQty, getPrintGeneratedAt, } from '../../../data/fcs/print-service.ts';
const DELIVERY_TITLE_BY_VARIANT = {
    runtime: '任务交货卡',
    printing: '印花任务交货卡',
    dyeing: '染色任务交货卡',
...
function resolveDeliveryInput(input) {
...
function objectQtyNoun(objectType, qtyUnit) {
    if (objectType === '面料' || String(objectType).includes('面料'))
        return qtyUnit === '卷' ? '面料卷数' : '面料米数';
    if (objectType === '裁片' || String(objectType).includes('裁片'))
        return '裁片数量';
    if (objectType === '成衣' || String(objectType).includes('成衣') || qtyUnit === '件')
        return '成衣件数';
    return '对象数量';
}
...
function inferVariantFromText(text) {
    ...
    if (text.includes('车缝') || text.includes('缝制'))
        return 'sewing';
    if (text.includes('裁片') || text.includes('裁剪') || text.includes('裁床'))
        return 'cutting';
...
function inferVariantFromProcess(record) {
    if (record.craftType === 'PRINT')
        return 'printing';
    if (record.craftType === 'DYE')
        return 'dyeing';
    if (record.craftType === 'SPECIAL_CRAFT')
        return 'specialCraft';
    if (record.craftType === 'POST_FINISHING')
        return 'postFinishing';
    return inferVariantFromText(`${record.craftName} ${record.sourceTaskNo} ${record.sourceWorkOrderNo}`);
}
function resolveTargetRoute(record) {
    if (record.craftType === 'PRINT')
        return `/fcs/craft/printing/work-orders/${encodeURIComponent(record.sourceWorkOrderId)}?tab=handover`;
    if (record.craftType === 'DYE')
        return `/fcs/craft/dyeing/work-orders/${encodeURIComponent(record.sourceWorkOrderId)}?tab=handover`;
    if (record.craftType === 'POST_FINISHING')
        return `/fcs/craft/post-finishing/work-orders/${encodeURIComponent(record.sourceWorkOrderId)}?tab=handover`;
    if (record.craftType === 'SPECIAL_CRAFT')
        return `/fcs/process-factory/special-craft/tasks?handoverRecordId=${encodeURIComponent(record.handoverRecordId)}`;
...
function findFieldValue(rows, keyword, fallback = '待确认') {
...
function normalizeReceiptStatus(status) {
    ...
    if (!status)
        return '待确认收货';
    if (status === legacyWait || status === '已交出')
        return '交出待收货';
    if (status === legacyDone)
        return '收货确认中';
    if (status.includes('差异') || status.includes('驳回'))
        return '收货差异';
    return status;
}
function normalizeReceiptReviewStatus(status) {
    if (!status)
        return '待确认收货';
...
function buildDifferenceRows(differences) {
...
function buildLegacyDifferenceRows(doc, noun) {
...
function buildLineRows(lines, noun) {
...
function buildUnifiedLineRows(record, warehouse) {
    return [[
            record.objectType,
...
function getLegacyDeliveryVariant(doc, preferredVariant) {
    return preferredVariant || inferVariantFromText(`${doc.title} ${doc.processName} ${doc.craftName || ''} ${doc.taskNo || ''}`);
}
function buildDocumentFromLegacyDoc(input, doc, preferredVariant) {
...
function buildDocumentFromProcessHandover(input, record, preferredVariant) {
    const generatedAt = getPrintGeneratedAt();
...
export function buildTaskDeliveryCardPrintDocument(input, preferredVariant) {
...
export function buildRuntimeTaskDeliveryCardPrintDocument(input) {
    return buildTaskDeliveryCardPrintDocument(input, 'runtime');
}
export function buildPrintingDeliveryCardPrintDocument(input) {
    return buildTaskDeliveryCardPrintDocument(input, 'printing');
}
export function buildDyeingDeliveryCardPrintDocument(input) {
    return buildTaskDeliveryCardPrintDocument(input, 'dyeing');
}
export function buildSpecialCraftDeliveryCardPrintDocument(input) {
    return buildTaskDeliveryCardPrintDocument(input, 'specialCraft');
}
export function buildPostFinishingDeliveryCardPrintDocument(input) {
    return buildTaskDeliveryCardPrintDocument(input, 'postFinishing');
}
export function buildCuttingDeliveryCardPrintDocument(input) {
    return buildTaskDeliveryCardPrintDocument(input, 'cutting');
}
export function buildSewingDeliveryCardPrintDocument(input) {
...
export function renderTaskDeliveryCardTemplate(doc) {
