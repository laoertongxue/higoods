...
import { commonPickupPrintVersions, commonPickupScanRecords, commonPickupSlips, cuttingPickupPrintVersions, cuttingPickupScanRecords, cuttingPickupSlips, } from '../../../domain/pickup/mock.ts';
import { listCuttingSewingDispatchBatches, listCuttingSewingDispatchOrders, } from '../../../data/fcs/cutting/sewing-dispatch.ts';
...
function garmentQty(value) {
    return formatPrintQty(value, '件');
}
function rollQty(value) {
    return formatPrintQty(value, '卷');
}
function objectQtyText(summary, noun) {
    if (!summary)
        return '待确认';
...
function objectQtyDiff(expected, actual, noun) {
    if (!expected || !actual)
        return '待确认';
...
function pickupObjectNoun(slip) {
    if (slip.plannedQtySummary.length !== undefined || slip.configuredQtySummary.length !== undefined)
        return '面料米数';
    if (slip.materialType === 'GENERAL')
        return '辅料数量';
    if (slip.boundObjectType === 'CUT_PIECE_ORDER')
        return '裁片数量';
...
export function buildMaterialPrepSlipPrintDocument(input) {
...
function allPickupSlips() {
    return [...commonPickupSlips, ...cuttingPickupSlips];
}
export function buildPickupSlipPrintDocument(input) {
...
export function buildIssueSlipPrintDocument(input) {
        ...
        tables: [
            {
                ...
                rows: (batches.length ? batches : [{ dispatchBatchNo: order.dispatchOrderNo, plannedGarmentQty: order.plannedDispatchGarmentQty, plannedSkuQtyLines: [], feiTicketNos: [], status: order.status }])
                    .flatMap((batch) => (batch.plannedSkuQtyLines?.length ? batch.plannedSkuQtyLines : [{ colorName: '按生产单', sizeCode: '汇总', plannedGarmentQty: batch.plannedGarmentQty, dispatchedGarmentQty: batch.plannedGarmentQty, remainingGarmentQty: 0 }]).map((line) => [
                    '成衣',
                    '车缝裁片包',
...
export function buildSupplementMaterialSlipPrintDocument(input) {
...
export function renderMaterialSlipTemplate(doc) {
