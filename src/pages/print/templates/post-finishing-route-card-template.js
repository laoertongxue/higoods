...
import { getPostFinishingFlowText, getPostFinishingSourceLabel, getPostFinishingWorkOrderById, } from '../../../data/fcs/post-finishing-domain.ts';
...
function actionQty(action, primary) {
...
function getPostActionFields(order) {
...
function buildRouteRows(order) {
    ...
    rows.push({ node: '复检', action: order.recheckAction, qty: actionQty(order.recheckAction, order.recheckAction.confirmedGarmentQty ?? order.recheckAction.acceptedGarmentQty) }, {
        node: '交出',
...
export function buildPostFinishingRouteCardPrintDocument(input) {
...
export function renderPostFinishingRouteCardTemplate(doc) {
