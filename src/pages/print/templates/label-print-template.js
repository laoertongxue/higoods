...
import { getProcessHandoverRecordById, listProcessHandoverRecords, } from '../../../data/fcs/process-warehouse-domain.ts';
const TEMPLATE_BY_DOCUMENT = {
...
function generatedTicketToRecord(ticket) {
    return {
        ...ticket,
...
function listFeiRecords(documentType) {
    const projection = buildFeiTicketPrintProjection();
    const projected = (projection.ticketRecords || []);
...
function findFeiRecord(sourceId, documentType) {
...
function listFeiRecordsForSource(sourceId, documentType) {
    const records = listFeiRecords(documentType);
    if (sourceId.includes(',')) {
        const ids = sourceId.split(',').map((item) => item.trim()).filter(Boolean);
        return ids.map((id) => findFeiRecord(id, documentType)).filter(Boolean);
    }
    ...
    const byCutOrder = records.filter((item) => item.printableUnitId === sourceId
    ...
    if (byCutOrder.length)
        return byCutOrder;
    return [];
}
function resolveFeiTicketTargetRoute(record) {
...
function feiQr(record, documentType, mode, isVoid = false) {
...
function resolveFeiPrintVersion(record, mode) {
    const numericVersion = Number(record.version || 0);
    if (Number.isFinite(numericVersion) && numericVersion > 0)
        return `V${numericVersion}`;
    if (mode === '补打')
        return `R${Math.max(Number(record.reprintCount || 0), 1)}`;
    return toText(record.printVersionNo, 'V1');
}
function resolveFeiMarkerPlanNo(record) {
    return toText(record.sourceMarkerPlanNo ||
        record.markerPlanNo ||
        record.batchNo);
}
function resolveFeiSpreadingSessionNo(record) {
    return toText(record.sourceSpreadingSessionNo ||
        record.spreadingSessionNo ||
        record.sourceSpreadingNo ||
        record.spreadingNo);
}
function needsWideFeiLabel(item) {
    return item.labelFields.some((field) => String(field.value || '').length > 34);
}
function buildFeiLabelItem(record, input, mode) {
...
function buildBaseLabelDocument(input, options) {
...
function resolveFeiMode(documentType) {
...
export function buildFeiTicketLabelPrintDocument(input) {
    ...
    const paperType = items.length > 1
        ? 'A4_LABEL_GRID'
...
export function buildTransferBagLabelPrintDocument(input) {
...
export function buildCuttingOrderQrLabelPrintDocument(input) {
...
function objectQtyNoun(record) {
    if (record.objectType === '面料')
        return record.qtyUnit === '卷' ? '面料卷数' : '面料米数';
    if (record.objectType === '裁片')
        return '裁片数量';
    if (record.objectType === '成衣')
        return '成衣件数';
    return '对象数量';
}
export function buildHandoverQrLabelPrintDocument(input) {
...
function renderLabelItem(item, paperType) {
...
export function renderLabelPrintTemplate(doc) {
