...
export function getCurrentPrintSearchParams() {
...
export function renderFailure(failure, backHref) {
    return `
    ${renderPrintStyles()}
...
export function renderTextValue(value, fallback = '待确认') {
    if (value === undefined || value === null || String(value).trim().length === 0) {
        return fallback ? `<span class="text-muted-foreground">${escapeHtml(fallback)}</span>` : '';
    }
    return escapeHtml(String(value));
}
export function renderInfoGrid(rows, importantLabels = []) {
    return `
...
export function renderRouteRecordTable(rows) {
    return `
