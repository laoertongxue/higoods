...
import { getPdaCuttingTaskSnapshot, getPdaTaskFlowTaskById, } from '../data/fcs/pda-cutting-execution-source.ts';
import { buildPdaCuttingTaskDetailFocusHref, readPdaCuttingNavContext, } from './pda-cutting-nav-context';
...
function getLocationSearchParams(pathname) {
    const currentPathname = pathname ?? getCurrentPathname();
    const [, queryString = ''] = currentPathname.split('?');
    return new URLSearchParams(queryString);
}
export function readSelectedExecutionOrderIdFromLocation(pathname) {
    const value = getLocationSearchParams(pathname).get('executionOrderId')?.trim();
    return value ? value : null;
}
export function readSelectedExecutionOrderNoFromLocation(pathname) {
    const value = getLocationSearchParams(pathname).get('executionOrderNo')?.trim();
    return value ? value : null;
}
export function readPdaCuttingReturnToFromLocation(pathname) {
    const value = getLocationSearchParams(pathname).get('returnTo')?.trim();
    return value ? value : null;
}
export function resolveSelectedExecutionOrderLine(detail, selectedExecutionOrderId, selectedExecutionOrderNo) {
...
export function buildPdaCuttingExecutionContext(taskId, routeKey, pathname) {
...
export function buildPdaCuttingExecutionUnitContext(taskId, executionOrderIdFromPath, pathname) {
    const navContext = readPdaCuttingNavContext(pathname);
    const returnTo = navContext.returnTo || readPdaCuttingReturnToFromLocation(pathname);
