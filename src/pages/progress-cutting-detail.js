...
import { platformCuttingRiskMeta, platformCuttingStageMeta, platformCuttingUrgencyMeta, } from '../domain/cutting-platform/overview.helpers';
...
function renderPageHeader(recordId) {
    const detail = buildPlatformCuttingDetailView(recordId);
    if (!detail)
        return renderNotApplicable();
    const { row } = detail;
    return `
...
function renderBasicInfo(recordId) {
    const detail = buildPlatformCuttingDetailView(recordId);
    if (!detail)
        return '';
    const { row } = detail;
...
function renderChainProgress(recordId) {
    const detail = buildPlatformCuttingDetailView(recordId);
    if (!detail)
        return '';
    return `
...
function renderPickupSection(recordId) {
    const detail = buildPlatformCuttingDetailView(recordId);
    if (!detail)
        return '';
    const { row } = detail;
...
function renderExecutionSection(recordId) {
    const detail = buildPlatformCuttingDetailView(recordId);
    if (!detail)
        return '';
    const { row } = detail;
...
function renderReplenishmentSection(recordId) {
    const detail = buildPlatformCuttingDetailView(recordId);
    if (!detail)
        return '';
    const { row } = detail;
...
function renderWarehouseSection(recordId) {
    const detail = buildPlatformCuttingDetailView(recordId);
    if (!detail)
        return '';
    const { row } = detail;
...
function renderIssueSection(recordId) {
    const detail = buildPlatformCuttingDetailView(recordId);
    if (!detail)
        return '';
    return `
...
function renderAttentionSection(recordId) {
    const detail = buildPlatformCuttingDetailView(recordId);
    if (!detail)
        return '';
    return `
...
export function renderProgressCuttingDetailPage(recordId) {
    const detail = buildPlatformCuttingDetailView(recordId);
    if (!detail)
        return renderNotApplicable();
    return `
...
export function handleProgressCuttingDetailEvent(target) {
    const actionNode = target.closest('[data-platform-cutting-detail-action]');
...
export function extractProgressCuttingDetailId(pathname) {
