...
import { FULL_CAPABILITY_FACTORY_ID, listPostFinishingWaitHandoverWarehouseRecords, listPostFinishingWaitProcessWarehouseRecords, } from '../data/fcs/post-finishing-domain.ts';
import { renderPdaFrame } from './pda-shell';
import { buildWarehouseDifferenceText, escapeAttr, formatWarehouseDateTime, getMobileWarehouseSearchParams, getMobileWarehouseRuntimeContext, getWarehouseGeneratedModeLabel, getWaitProcessSourceActionLabel, renderCompactFieldList, renderMobilePageEmptyState, renderSectionFilterChips, renderStatusPill, renderWarehouseSummaryHeader, resolveInboundSourceRoute, } from './pda-warehouse-shared';
...
function getPostFinishingInboundRows() {
    const waitProcessRows = listPostFinishingWaitProcessWarehouseRecords().flatMap((record) => record.flowRecords
    ...
    const waitHandoverRows = listPostFinishingWaitHandoverWarehouseRecords().flatMap((record) => record.flowRecords
...
function renderPostFinishingInboundDetailDrawer() {
...
function renderPostFinishingInboundRecordsPage(runtimeFactoryName) {
...
export function renderPdaWarehouseInboundRecordsPage() {
    const runtime = getMobileWarehouseRuntimeContext();
    if (!runtime)
    ...
    if (runtime.factoryId === FULL_CAPABILITY_FACTORY_ID)
        return renderPostFinishingInboundRecordsPage(runtime.factoryName);
    const rows = getRows();
    const content = `
    <div class="space-y-4 px-4 pb-5 pt-4">
...
export function handlePdaWarehouseInboundRecordsEvent(target) {
