import { canEditTechnique, difficultyOptions, bomTriggerFieldLabel, escapeHtml, getTechniqueCraftOptions, getTechniqueProcessOptions, getTechniqueReferenceMetaByCraftCode, getSelectedDraftMeta, isBomDrivenPrepTechnique, isPrepStage, isTechPackReadOnly, stageCodeToName, stageOptions, state, } from './context.ts';
export function renderProcessTechniqueCard(item) {
    ...
    const autoSourceText = isBomGenerated
      ...
      <div class="mt-2 flex flex-wrap items-center gap-2 text-xs">
        ...
        <span class="rounded border ${item.linkageStatus === '待确认'
            ? 'border-amber-200 bg-amber-50 text-amber-700'
            : 'border-emerald-200 bg-emerald-50 text-emerald-700'} px-2 py-0.5 font-medium">状态：${escapeHtml(item.linkageStatus || '已生成')}</span>
      </div>
    `
        : '';
    ...
    `
        : '';
    const referenceValueText = item.referencePublishedSamValue !== null && item.referencePublishedSamUnitLabel
        ? `${item.referencePublishedSamValue} ${item.referencePublishedSamUnitLabel}`
        : '暂无平台参考';
    const targetObjectText = item.isSpecialCraft && item.selectedTargetObject
...
export function renderAddTechniqueDialog() {
    if (!state.addTechniqueDialogOpen)
        return '';
    ...
    const availableCraftOptions = getTechniqueCraftOptions(state.newTechnique.stageCode, state.newTechnique.processCode);
    ...
    const draftReferenceText = draftReferenceMeta.referencePublishedSamValue !== null && draftReferenceMeta.referencePublishedSamUnitLabel
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/45 p-4" data-dialog-backdrop="true">
      <section class="w-full max-w-lg rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
        ...
        <div class="space-y-4 px-6 py-4">
          ...
          <label class="space-y-1">
            ...
            <select class="w-full rounded-md border px-3 py-2 text-sm" data-tech-field="new-technique-process-code" ${isLockedPrepTechnique ? 'disabled' : ''}>
              ...
              ${processOptions
        .map((item) => `<option value="${item.processCode}" ${state.newTechnique.processCode === item.processCode ? 'selected' : ''}>${item.processName}</option>`)
        .join('')}
            </select>
          </label>
        ...
        .map((item) => `<option value="${item.craftCode}" ${state.newTechnique.craftCode === item.craftCode ? 'selected' : ''}>${item.craftName}</option>`)
        .join('')}
            </select>
          </label>
        ...
        <footer class="flex items-center justify-end gap-2 border-t px-6 py-4">
          ...
          <button class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 ${selectedMeta ? '' : 'pointer-events-none opacity-50'}" data-tech-action="save-technique">${isEdit ? '保存' : '确认新增'}</button>
        </footer>
      </section>
    </div>
  `;
