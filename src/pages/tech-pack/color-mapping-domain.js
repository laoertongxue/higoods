import { colorMappingStatusClass, colorMappingStatusLabel, escapeHtml, generatedModeLabel, getPatternById, getSkuOptionsForCurrentSpu, isTechPackReadOnly, state, } from './context.ts';
function renderReadonlyText(value, muted = false) {
    const normalized = String(value ?? '').trim();
    if (!normalized)
        return '<span class="text-muted-foreground">-</span>';
    return `<span class="${muted ? 'text-muted-foreground' : 'text-slate-800'}">${escapeHtml(normalized)}</span>`;
}
function renderReadonlySkuTags(codes, skuLabelByCode) {
    if (codes.length === 0)
        return '<span class="text-muted-foreground">全部 SKU</span>';
    return `
...
function renderReadonlyColorMappingLines(mapping, skuLabelByCode) {
...
export function renderColorMappingTab() {
    if (!state.techPack)
        return '';
    ...
    const skuLabelByCode = new Map(skuOptions.map((item) => [item.skuCode, `${item.color}/${item.size}（${item.skuCode}）`]));
    ...
    const confirmedCount = mappings.filter((item) => item.status === 'CONFIRMED' || item.status === 'AUTO_CONFIRMED').length;
            ...
            return `
                      <article class="rounded-lg border">
                        <header class="flex flex-wrap items-center justify-between gap-2 border-b px-3 py-2">
                          ...
                          <div class="flex flex-wrap items-center gap-2">
                            ${readonly ? '' : `<button
                              class="inline-flex h-7 items-center rounded border px-2 text-xs hover:bg-muted ${mapping.status === 'AUTO_DRAFT' || mapping.status === 'MANUAL_ADJUSTED'
                ? ''
                                              ...
                                              <td class="px-2 py-1.5">
                                                <div class="grid gap-1">
                                                  ...
                                                  >
                                                    ...
                                                    ${bomOptions
                        .map((bom) => `<option value="${bom.id}" ${line.bomItemId === bom.id ? 'selected' : ''}>${escapeHtml(`${bom.materialCode || bom.id} · ${bom.materialName}`)}</option>`)
                        .join('')}
                                                  </select>
                                                  <input
                                                    class="h-7 rounded border px-2 text-xs"
                                              ...
                                              <td class="px-2 py-1.5">
                                                ...
                                                >
                                                  ...
                                                  ${patternOptions
                        .map((pattern) => `<option value="${pattern.id}" ${line.patternId === pattern.id ? 'selected' : ''}>${escapeHtml(pattern.name)}</option>`)
                        .join('')}
                                                </select>
                                              </td>
                                              <td class="px-2 py-1.5">
                                                ...
                                                >
                                                  ...
                                                  ${pieceOptions
                        .map((piece) => `<option value="${piece.id}" ${line.pieceId === piece.id ? 'selected' : ''}>${escapeHtml(piece.name)}</option>`)
                        .join('')}
                                                </select>
                                              </td>
