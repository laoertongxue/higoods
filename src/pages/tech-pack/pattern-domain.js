import { TECH_PACK_PATTERN_CATEGORY_OPTIONS, calculatePatternTotalPieceQty, escapeHtml, formatPatternSpec, getBomColorOptionsForPattern, getPatternColorQuantityOptions, getPartTemplateOptions, getPatternPieceInstanceSpecialCraftOptions, getPatternPieceSpecialCraftOptionsFromCurrentTechPack, PATTERN_CRAFT_POSITION_OPTIONS, getPatternBySelectionKey, getSizeCodeOptionsFromSizeRules, isTechPackReadOnly, state, } from './context.ts';
function isAllowedPatternImage(value) {
...
function buildPrototypeDownloadUrl(fileName) {
    const payload = [
        'HiGood 技术包演示文件',
        `文件名：${fileName}`,
        '用途：技术包纸样管理下载演示',
...
function buildPatternPreviewUrl(title, fileName) {
    const svg = `
...
function renderPatternDownloadLink(fileName, label) {
...
function getPatternTechnicalFiles(pattern) {
...
function getPatternImagePreviewUrl(pattern) {
...
function renderPatternImagePreview(pattern) {
...
function renderPatternTechnicalFileLinks(pattern) {
...
function renderPatternFileBadge(label) {
    return `<span class="inline-flex rounded border px-2 py-0.5 text-xs">${escapeHtml(label)}</span>`;
}
function renderMaintainerStepStatus(status) {
    const normalized = String(status || '').trim();
    if (isTechPackReadOnly() && normalized === '已解析待确认')
        return '已完成';
...
function renderBindingStripCount(count) {
    return `${count} 条`;
}
function renderPatternBindingStripEditor() {
...
function renderPatternBindingStripDetail(pattern) {
...
function renderPatternDuplicateWarning() {
...
function renderPatternPiecePreview(previewSvg) {
    if (!previewSvg) {
        return '<span class="text-muted-foreground">暂无图片</span>';
    }
    return `
...
function renderTemplatePreview(previewSvg, emptyText = '-') {
    if (!previewSvg) {
...
function renderTemplateFlag(isTemplate) {
    return isTemplate ? '是' : '否';
}
function renderTemplateBindingSummary(row) {
    if (!row.isTemplate) {
...
function renderReadonlyTagList(labels, emptyText = '暂无数据') {
    if (labels.length === 0) {
        return `<span class="text-muted-foreground">${escapeHtml(emptyText)}</span>`;
    }
...
function renderPatternSizeSummary(selectedSizeCodes, sizeRange) {
    if (selectedSizeCodes.length > 0)
        return escapeHtml(selectedSizeCodes.join(' / '));
    return renderTextValue(sizeRange);
}
function getEnabledPatternColorQuantities(row) {
    return (row.colorPieceQuantities ?? []).filter((item) => item.enabled);
}
function renderPatternPieceColorQuantitySummary(row) {
    ...
    return `<div class="space-y-1">${enabledQuantities
        .map((quantity) => `<div class="text-xs"><span class="text-muted-foreground">${escapeHtml(quantity.colorName)}</span>：${escapeHtml(String(quantity.pieceQty || 0))} 片</div>`)
        .join('')}</div>`;
}
function renderPatternPieceQuantityWarnings(row) {
    const warnings = [];
    ...
    if (enabledQuantities.length === 0)
        warnings.push('请至少选择一个适用颜色。');
    if (enabledQuantities.some((item) => Number(item.pieceQty) === 0)) {
        warnings.push('已选择适用颜色但片数为 0，请确认。');
    }
    if (Number(row.totalPieceQty || 0) === 0)
        warnings.push('当前总片数为 0，请维护颜色片数。');
    if (Number.isFinite(Number(row.parsedQuantity))
...
function renderPatternPieceSpecialCraftSummary(row) {
    if (row.specialCrafts.length === 0) {
        return '<span class="text-muted-foreground">无</span>';
    }
    return renderReadonlyTagList(row.specialCrafts.map((craft) => craft.displayName || craft.craftName), '无');
}
function getPieceInstancesBySourcePieceId(sourcePieceId) {
    return state.newPattern.pieceInstances.filter((instance) => instance.sourcePieceId === sourcePieceId);
}
function renderPieceInstanceCraftSummary(sourcePieceId) {
...
function renderReadonlyPieceInstanceCraftSummary(pattern, sourcePieceId) {
    ...
    <div class="space-y-1">
      ...
      ${renderReadonlyTagList(instances
        .flatMap((instance) => instance.specialCraftAssignments.map((assignment) => `${instance.displayName}：${assignment.craftName}（${assignment.craftPositionName}）`))
        .slice(0, 3), '暂无逐片工艺')}
    </div>
  `;
}
...
function getWovenStatusMessage() {
    ...
    if (state.newPattern.patternParsing || state.newPattern.parseStatus === 'PARSING')
        return '解析中';
    if (hasDxf && !dxfValid)
        return 'DXF 文件格式不正确';
    if (hasRul && !rulValid)
        return 'RUL 文件格式不正确';
    if (!hasDxf && !hasRul)
        return '请成对上传 1 个 DXF 和 1 个 RUL 文件';
    if (hasDxf !== hasRul)
        return '还缺少 1 个配对文件';
    if (state.newPattern.parseStatus === 'FAILED')
        return state.newPattern.parseError || '解析失败';
...
function canParseWovenPattern() {
    const hasName = Boolean(state.newPattern.name.trim());
    const hasDxf = Boolean(state.newPattern.dxfFileName.trim());
    const hasRul = Boolean(state.newPattern.rulFileName.trim());
    if (!hasName || !hasDxf || !hasRul)
        return false;
    if (!hasExtension(state.newPattern.dxfFileName, '.dxf'))
        return false;
    if (!hasExtension(state.newPattern.rulFileName, '.rul'))
        return false;
    return !state.newPattern.patternParsing;
}
function canSavePatternForm() {
    if (!state.newPattern.name.trim())
        return false;
    const sizeOptions = getSizeCodeOptionsFromSizeRules();
    if (sizeOptions.length === 0 || state.newPattern.selectedSizeCodes.length === 0)
        return false;
    const colorOptions = getPatternColorQuantityOptions(state.newPattern.linkedBomItemId);
    if (colorOptions.length === 0)
        return false;
    const hasInvalidColorQuantities = state.newPattern.pieceRows.some((row) => row.colorPieceQuantities.length === 0
    ...
    if (state.newPattern.patternMaterialType === 'WOVEN') {
        ...
        if (state.newPattern.pieceRows.length === 0)
            return false;
        if (hasInvalidColorQuantities)
            return false;
        return !state.newPattern.pieceRows.some((row) => !row.name.trim() || Number(row.totalPieceQty) <= 0 || row.missingName);
    }
...
function renderLinkedBomLabel(patternId, linkedBomItemId, bomById) {
    const linkedBom = linkedBomItemId ? bomById.get(linkedBomItemId) : null;
    if (!linkedBom)
        return '<span class="text-sm text-muted-foreground">未关联</span>';
    return `
    <button
...
function renderPatternDetailPieceTable() {
...
export function renderPatternTab() {
    const bomById = new Map(state.bomItems.map((item) => [
        item.id,
        {
    ...
    <section class="space-y-4 rounded-lg border bg-card p-4" data-testid="pattern-management-page">
      ...
      <div class="space-y-3">
        ...
        : `
            ...
            .map((item) => {
            const pieceCount = Number.isFinite(item.patternTotalPieceQty) && item.patternTotalPieceQty > 0
...
export function renderPatternDialog() {
    if (!state.patternDialogOpen || !state.selectedPattern)
        return '';
    const pattern = getPatternBySelectionKey(state.selectedPattern);
    if (!pattern)
        return '';
    const linkedBom = pattern.linkedBomItemId.length > 0
        ? state.bomItems.find((item) => item.id === pattern.linkedBomItemId) ?? null
        : null;
    const image = getPatternImagePreviewUrl(pattern);
    const pieceTotal = Number.isFinite(pattern.patternTotalPieceQty) && pattern.patternTotalPieceQty > 0
...
function renderPatternSizeSelector() {
    const sizeOptions = getSizeCodeOptionsFromSizeRules();
...
function renderPatternPieceColorSelector(row) {
    const colorOptions = getPatternColorQuantityOptions(state.newPattern.linkedBomItemId);
    if (colorOptions.length === 0) {
        return '<span class="text-muted-foreground">请先维护物料清单颜色</span>';
    }
    const selectedColorSet = new Set(row.colorPieceQuantities
...
function renderPatternPieceColorCountEditor(row) {
...
function renderPatternPieceSpecialCraftSelector(row) {
    const specialCraftOptions = getPatternPieceSpecialCraftOptionsFromCurrentTechPack();
    if (specialCraftOptions.length === 0) {
        return '<span class="text-muted-foreground">无</span>';
    }
    const selectedCraftKeys = new Set(row.specialCrafts.map((item) => `${item.processCode}:${item.craftCode}:${item.selectedTargetObject}`));
...
function renderPatternPackagePieceEditorTable(isWoven) {
    if (state.newPattern.pieceRows.length === 0) {
...
function renderPatternPieceEditorTable(isWoven) {
    if (state.newPattern.pieceRows.length === 0) {
        return '<div class="rounded border border-dashed px-3 py-3 text-xs text-muted-foreground">暂无数据</div>';
    }
    const patternTotalPieceQty = calculatePatternTotalPieceQty(state.newPattern.pieceRows);
...
function renderPatternFormDialogLegacy() {
    if (!state.addPatternDialogOpen)
        return '';
    ...
    const parseButtonLabel = state.newPattern.patternParsing
        ? '解析中'
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <label class="space-y-1">
              ...
              <select class="w-full rounded-md border px-3 py-2 text-sm" data-tech-field="new-pattern-linked-bom-item">
                ...
                ${bomOptions
        .map((item) => `<option value="${item.id}" ${state.newPattern.linkedBomItemId === item.id ? 'selected' : ''}>${escapeHtml(`${item.materialCode} · ${item.materialName}`)}</option>`)
        .join('')}
              </select>
            </label>
            <label class="space-y-1">
...
export function renderPatternFormDialog() {
    if (!state.addPatternDialogOpen)
        return '';
    const bomOptions = state.bomItems;
    ...
    const parseButtonLabel = state.newPattern.patternParsing
        ...
        : state.newPattern.parseStatus === 'PARSED'
            ? '重新解析'
            : '解析纸样';
    const stepButtonClass = (step) => activeStep === step
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        : `
                <select class="w-full rounded-md border px-3 py-2 text-sm" data-tech-field="new-pattern-linked-bom-item">
                  <option value="">请选择关联物料</option>
                  ${bomOptions
            .map((item) => `<option value="${item.id}" ${state.newPattern.linkedBomItemId === item.id ? 'selected' : ''}>${escapeHtml(`${item.materialName} · ${item.materialCode}`)}</option>`)
            .join('')}
                </select>
              `}
        </label>
...
function renderPieceInstanceAssignmentList(instance) {
...
export function renderPieceInstanceSpecialCraftDialog() {
    if (!state.pieceInstanceCraftDialogOpen || !state.activePieceInstanceSourcePieceId)
        return '';
    const instances = getPieceInstancesBySourcePieceId(state.activePieceInstanceSourcePieceId);
    const activeInstance = instances.find((instance) => instance.pieceInstanceId === state.activePieceInstanceId)
        || instances[0];
...
export function renderPatternTemplateDialog() {
    if (!state.patternTemplateDialogOpen || !state.activePatternTemplatePieceId)
        return '';
            ...
            .map((record) => `
                            <tr class="border-b align-top last:border-0">
                              ...
                              <td class="px-3 py-2">
                                ...
                                <div class="mt-1 text-xs text-muted-foreground">${escapeHtml([record.standardPartName, record.sourcePartName].filter(Boolean).join(' · ') || '暂无数据')}</div>
                              </td>
