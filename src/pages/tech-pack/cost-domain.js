import { currencyOptions, customCostUnitOptions, escapeHtml, isTechPackReadOnly, materialUnitOptions, processUnitOptions, state, } from './context.ts';
export function renderCostTab() {
    const readonly = isTechPackReadOnly();
    const materialTotal = state.materialCostRows.reduce((sum, row) => sum + row.usage * (Number.parseFloat(row.price) || 0), 0);
    const processTotal = state.processCostRows.reduce((sum, row) => sum + (Number.parseFloat(row.price) || 0), 0);
    const customTotal = state.customCostRows.reduce((sum, row) => sum + (Number.parseFloat(row.price) || 0), 0);
