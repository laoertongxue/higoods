import { ensureTechPackPageState, escapeHtml, getChecklist, renderChecklist, renderStatusBadge, renderTabHeader, state, } from './context.ts';
...
import { renderPatternDialog, renderPatternFormDialog, renderPatternTab, renderPatternTemplateDialog, } from './pattern-domain.ts';
...
function renderTechPackSummary() {
    if (!state.currentTechnicalVersionId || !state.currentStyleId)
        return '';
...
function renderTechPackVersionLogsPanel() {
...
export function renderTechPackPage(rawSpuCode, options) {
    ...
    <div data-tech-pack-page-root="true" class="space-y-4">
      <header class="flex items-start justify-between">
        ...
        <div class="flex items-center gap-3">
          ...
          <button type="button" class="inline-flex items-center rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700 ${canRelease ? '' : 'pointer-events-none opacity-50'}" data-tech-action="open-release" title="${hasIncomplete ? '核心域未补全，暂不可发布' : state.techPack.status !== 'DRAFT' ? '只有草稿可以发布' : ''}">
