...
type TechniqueItem = {
  ...
  targetObject?: TechnicalProcessEntry['targetObject']
  targetObjectName?: TechnicalProcessEntry['targetObjectName']
  ...
  triggerField?: TechnicalProcessEntry['triggerField']
...
export function normalizeGarmentBomItem(item: BomItemRow): BomItemRow {
  ...
  return {
    ...
    usageProcessCodes: item.usageProcessCodes.filter((code) =>
...
const tabItems: Array<{ key: TechPackTab; icon: string; label: string }> = [
  ...
  { key: 'process', icon: 'scissors', label: '工艺路线' },
...
const bomUsageProcessOptions: Array<{
  code: string
  label: string
...
function mapBomTypeToRouteObjectType(type: BomItemRow['type'] | undefined): TechnicalProcessObjectType {
...
function resolveTechniqueRouteObjectDefaults(input: {
  id: string
  processCode: string
  selectedTargetObject?: string
  woolTaskType?: 'WHOLE_GARMENT' | 'PART_PANEL'
  ...
  const linkedBomItem = (input.linkedBomItemIds ?? [])
    .map((id) => state.bomItems.find((item) => item.id === id))
    .find((item): item is BomItemRow => Boolean(item))
  const linkedPatternId = input.linkedPatternIds?.[0]
  const bomObjectType = mapBomTypeToRouteObjectType(linkedBomItem?.type)
  if (input.processCode === 'CUT_PANEL') {
    return {
      routeObjectKey: linkedBomItem ? `BOM:${linkedBomItem.id}` : `PROCESS:${input.id}`,
      inputObjectType: 'FABRIC',
      outputObjectType: 'CUT_PIECE',
    }
  }
  if (input.processCode === 'WOOL') {
    return {
      routeObjectKey: linkedPatternId ? `PATTERN:${linkedPatternId}` : linkedBomItem ? `BOM:${linkedBomItem.id}` : `PROCESS:${input.id}`,
      inputObjectType: 'YARN',
      outputObjectType: input.woolTaskType === 'PART_PANEL' ? 'KNITTED_PANEL' : 'GARMENT',
    }
  }
  if (input.processCode === 'SEW') {
    return {
      routeObjectKey: `GARMENT:${state.currentStyleId || state.currentSpuCode || 'CURRENT'}`,
      inputObjectType: 'CUT_PIECE',
      outputObjectType: 'GARMENT',
    }
  }
  if (input.selectedTargetObject === '成衣') {
    return {
      routeObjectKey: linkedBomItem ? `BOM:${linkedBomItem.id}` : `GARMENT:${state.currentStyleId || state.currentSpuCode || 'CURRENT'}`,
      inputObjectType: 'GARMENT',
      outputObjectType: 'GARMENT',
    }
  }
  if (input.selectedTargetObject === '已裁部位') {
    return {
      routeObjectKey: linkedPatternId ? `PATTERN:${linkedPatternId}` : `PROCESS:${input.id}`,
      inputObjectType: 'CUT_PIECE',
      outputObjectType: 'CUT_PIECE',
    }
  }
  if (input.selectedTargetObject === '完整面料') {
    return {
      routeObjectKey: linkedBomItem ? `BOM:${linkedBomItem.id}` : `PROCESS:${input.id}`,
      inputObjectType: 'FABRIC',
  ...
  return {
    routeObjectKey: linkedBomItem ? `BOM:${linkedBomItem.id}` : linkedPatternId ? `PATTERN:${linkedPatternId}` : `PROCESS:${input.id}`,
    inputObjectType: bomObjectType,
    outputObjectType: bomObjectType,
  }
}
...
function withRouteDefaults(item: TechniqueItem, fallbackIndex: number): TechniqueItem {
  ...
  return {
    ...item,
    routeObjectKey: routeFields.routeObjectKey || item.routeObjectKey,
    inputObjectType: routeFields.inputObjectType || item.inputObjectType,
    outputObjectType: routeFields.outputObjectType || item.outputObjectType,
    consumedBomItemIds: routeFields.consumedBomItemIds
      ? [...routeFields.consumedBomItemIds]
      : item.consumedBomItemIds ? [...item.consumedBomItemIds] : undefined,
    predecessorEntryIds: routeFields.predecessorEntryIds
      ? [...routeFields.predecessorEntryIds]
      : item.predecessorEntryIds ? [...item.predecessorEntryIds] : undefined,
...
function normalizeTechniqueRoutes(items: TechniqueItem[]): TechniqueItem[] {
  const withDefaults = items.map(withRouteDefaults)
  let normalized: TechniqueItem[]
  try {
    normalized = projectProcessRouteLayoutFromPredecessors(
      materializeLegacyProcessRoutePredecessors(withDefaults),
    )
  } catch {
    normalized = normalizeProcessRouteEntries(withDefaults)
  }
...
function listPatternCraftRouteSpecs(patternItems: PatternItem[]): PatternCraftRouteSpec[] {
  const specs = new Map<string, PatternCraftRouteSpec>()
  patternItems.forEach((pattern) => {
    pattern.pieceRows.forEach((piece) => {
      ...
      if (instanceAssignments.length > 0) {
        instanceAssignments.forEach(({ assignment, sequenceNo }) => {
          ...
          specs.set(occurrenceKey, {
            id: `tech-pattern-${toRouteIdSegment(pattern.id)}-${toRouteIdSegment(piece.id)}-${toRouteIdSegment(assignment.craftCode)}-${assignment.craftPosition.toLowerCase()}`,
            patternId: pattern.id,
            bomItemId: pattern.linkedBomItemId,
      ...
      piece.specialCrafts.forEach((craft, index) => {
        ...
        if (specs.has(occurrenceKey)) return
        specs.set(occurrenceKey, {
          id: `tech-pattern-${toRouteIdSegment(pattern.id)}-${toRouteIdSegment(piece.id)}-${toRouteIdSegment(craft.craftCode)}`,
          patternId: pattern.id,
          bomItemId: pattern.linkedBomItemId,
          sourcePieceId: piece.id,
          pieceName: piece.name,
...
/**
 * 物料与纸样关联中的裁片工艺是路线节点的权威来源。
 * 本函数只同步 occurrence 与类型级对象，不复制生产单实际 SKU、批次或数量。
 */
function syncPatternDrivenTechniques(
  techniques: TechniqueItem[],
  patternItems: PatternItem[],
): TechniqueItem[] {
  const specs = listPatternCraftRouteSpecs(patternItems)
  const requiredIds = new Set(specs.map((spec) => spec.id))
  ...
  const generated = specs.map((spec, index): TechniqueItem => {
    ...
    return {
      ...
      stageCode: 'PROD',
      stage: '生产阶段',
      processCode: craft?.processCode || 'SPECIAL_CRAFT',
      process: craft?.processName || '辅助/特种工艺',
      craftCode: spec.craftCode,
      technique: spec.craftName,
      assignmentGranularity: craft?.assignmentGranularity || 'DETAIL',
      ruleSource: craft?.ruleSource || 'INHERIT_PROCESS',
      detailSplitMode: craft?.detailSplitMode || 'COMPOSITE',
      detailSplitDimensions: [...(craft?.detailSplitDimensions || ['PATTERN', 'GARMENT_SKU'])],
      defaultDocType: craft?.defaultDocType || 'TASK',
      taskTypeMode: craft?.taskTypeMode || 'CRAFT',
      isSpecialCraft: craft?.isSpecialCraft ?? true,
      selectedTargetObject: '已裁部位',
      targetObject: 'CUT_PIECE_PART',
      targetObjectName: '裁片部位',
      supportedTargetObjects: craft?.supportedTargetObjects ? [...craft.supportedTargetObjects] : ['CUT_PIECE'],
      supportedTargetObjectLabels: craft?.supportedTargetObjectLabels ? [...craft.supportedTargetObjectLabels] : ['已裁部位'],
      linkedBomItemIds: spec.bomItemId ? [spec.bomItemId] : [],
      linkedPatternIds: [spec.patternId],
      routeObjectKey: `PATTERN:${spec.patternId}:PIECE:${spec.sourcePieceId}`,
      inputObjectType: 'CUT_PIECE',
      ...
      routeUpdatedBy: existing?.routeUpdatedBy,
      routeUpdatedAt: existing?.routeUpdatedAt,
    }
  })
  const next = [...baseWithoutGenerated, ...generated]
  const validIds = new Set(next.map((item) => item.id))
  return normalizeTechniqueRoutes(next.map((item) => ({
    ...item,
    predecessorEntryIds: (item.predecessorEntryIds ?? []).filter((id) => validIds.has(id)),
  })))
}
...
function isBomDrivenPrepTechnique(
  ...
  return isBomDrivenPrepTechniqueFromBom({
    stageCode: item.stageCode,
    processCode: item.processCode,
...
function getSelectedDraftMeta():
  | {
      ...
      isSpecialCraft: boolean
      selectedTargetObject?: TechPackSpecialCraftTargetObject
      targetObject?: TechniqueItem['targetObject']
      targetObjectName?: TechniqueItem['targetObjectName']
      woolTaskType?: TechniqueItem['woolTaskType']
      downstreamTarget?: TechniqueItem['downstreamTarget']
      materialIssueMode?: TechniqueItem['materialIssueMode']
      linkedBomItemIds?: string[]
      linkedPatternIds?: string[]
      supportedTargetObjects?: SpecialCraftSupportedTargetObject[]
      supportedTargetObjectLabels?: SpecialCraftTargetObjectLabel[]
  ...
  if (state.newTechnique.stageCode && state.newTechnique.processCode && state.newTechnique.craftCode) {
    ...
    return {
      ...
      selectedTargetObject: selectedTargetObject || undefined,
...
function getBomColorOptionsForPattern(linkedBomItemId?: string): PatternBomColorOption[] {
  ...
  return getCachedDerived(
    ...
    () => {
      ...
      const allBomItems = (state.techPack?.bomItems ?? []).map((item): BomItemRow => ({
        id: item.id,
        type: item.type,
...
function normalizePatternColorPieceQuantities(
  ...
  const explicit = (input ?? [])
    .map((item): TechPackPatternColorPieceQuantity | null => {
...
function normalizePatternPieceSpecialCraftAssignments(
  ...
  return dedupeByKey(
    assignments
      .map((assignment, index): PatternPieceSpecialCraftAssignment | null => {
    ...
    () => {
      ...
      return dedupeByKey(
        state.techniques
          ...
          .map((item): TechPackPatternPieceSpecialCraft | null => {
          ...
          .filter((item): item is TechPackPatternPieceSpecialCraft => item?.processName === '特殊工艺'),
...
function normalizePatternPieceRows(
  rows: Array<Partial<PatternPieceRow> & Pick<PatternPieceRow, 'id' | 'name' | 'count'>>,
  patternId: string,
  linkedBomItemId = '',
): PatternPieceRow[] {
...
function toTechniqueItemFromEntry(entry: TechPackProcessEntry, fallbackIndex: number): TechniqueItem {
  ...
  const routeObjectDefaults = resolveTechniqueRouteObjectDefaults({
    ...
    selectedTargetObject: normalizedEntry.selectedTargetObject,
    woolTaskType: normalizedEntry.woolTaskType,
    linkedBomItemIds: normalizedEntry.linkedBomItemIds,
    linkedPatternIds: normalizedEntry.linkedPatternIds,
  })
  return {
    id: normalizedEntry.id || `tech-${fallbackIndex + 1}`,
    ...
    targetObject: normalizedEntry.targetObject,
      targetObjectName: String(normalizedEntry.targetObjectName || '') === '成衣半成品'
      ? '成衣'
...
function buildTechniquesFromTechPack(
  ...
  return syncBomDrivenPrepTechniques(
    techPack.processes.map((item, index): TechniqueItem => {
...
function syncTechPackToStore(options: { touch: boolean; persist?: boolean } = { touch: true, persist: true }): boolean {
  ...
  const next: TechPack = {
    ...
    patternFiles: state.patternItems.map((item): TechPackPatternFile => {
          ...
          specialCrafts:
            row.specialCrafts.length > 0
              ? row.specialCrafts.map((craft) => ({
                  ...
                  supportedTargetObjects: craft.supportedTargetObjects ? [...craft.supportedTargetObjects] : [],
                  supportedTargetObjectLabels: craft.supportedTargetObjectLabels ? [...craft.supportedTargetObjectLabels] : [],
                }))
              : undefined,
          bundleLengthCm: row.bundleLengthCm,
          bundleWidthCm: row.bundleWidthCm,
...
function renderStatusBadge(status: string): string {
  const config = techPackStatusConfig[status as TechPackStatus] ?? techPackStatusConfig.DRAFT
