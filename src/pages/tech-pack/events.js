...
import { parseFcsPatternFilePair, resolveFcsPatternFilePair, } from '../../data/fcs/fcs-pattern-file-parser.ts';
import { publishTechPackDraft, validateTechPackForPublish, } from '../../data/fcs/tech-packs.ts';
...
import { TECH_PACK_PATTERN_CATEGORY_OPTIONS, buildPatternDisplayFile, buildPatternFormStateFromItem, calculatePatternPieceTotalQty, calculatePatternTotalPieceQty, closeAllDialogs, canEditTechnique, copySystemDraftToManual, createEmptyMappingLine, createPatternBindingStrip, createPatternManagedFile, currentUser, dedupeStrings, escapeHtml, buildPatternPiecePartKey, getBaselineProcessByCode, getCraftOptionByCode, getBomColorOptionsForPattern, getPatternColorQuantityOptions, getPartTemplateRecordById, getPatternPieceSpecialCraftOptionsFromCurrentTechPack, getPatternMaterialTypeLabel, getPatternParseStatusLabel, getPatternById, getPatternPieceById, getSizeCodeOptionsFromSizeRules, getTechniqueReferenceMetaByCraftCode, isBomDrivenPrepTechnique, isPrepStage, getSelectedDraftMeta, getSkuOptionsForCurrentSpu, normalizePatternPieceRows, normalizePatternBindingStrips, generatePieceInstancesFromColorQuantities, summarizePieceInstances, findConfiguredPieceInstancesRemoved, getPatternPieceInstanceSpecialCraftOptions, PATTERN_CRAFT_POSITION_OPTIONS, hasEnabledColorPiece, hasInvalidColorPieceQty, hasPositiveEnabledColorPiece, isTechPackReadOnly, requestTechPackRender, resetAttachmentForm, resetBomForm, resetColorMappingToSystemSuggestion, resetPatternForm, resetSizeForm, resetTechniqueForm, state, stageNameToCode, syncMaterialCostRows, syncProcessCostRows, syncTechPackToStore, toTimestamp, touchMappingAsManual, updateColorMapping, updateColorMappingLine, } from './context.ts';
const PATTERN_IMAGE_PREVIEW_MODAL_ID = 'tech-pack-pattern-image-preview-modal';
function closePatternImagePreviewModal() {
...
function clearPatternParseState(status = 'NOT_PARSED') {
...
function revokeDraftDesignPreview() {
}
...
function buildDesignPlaceholderImage(fileName, sideType) {
...
function isImageDesignFile(file) {
...
async function buildDesignThumbnailDataUrl(originalFileDataUrl, fileName, sideType) {
...
async function applySelectedDesignFile(file) {
    ...
    try {
        ...
        const previewThumbnailDataUrl = isImageDesignFile(file)
            ...
            : buildDesignPlaceholderImage(file.name, state.newDesignSideType);
        if (state.selectedDesignFile !== file)
            return;
...
function getPatternPieceTemplateConflict(pieceId, templateId) {
    ...
    if (!partKey)
        return null;
    return (state.newPattern.pieceRows.find((row) => row.id !== pieceId
        && row.isTemplate
        && String(row.partTemplateId || '').trim()
        && buildPatternPiecePartKey(row) === partKey
        && String(row.partTemplateId || '').trim() !== templateId) ?? null);
}
function applyBomPrintRequirementChange(nextRequirement) {
...
function applyBomPrintSideModeChange(nextMode) {
...
function updatePatternPieceRow(pieceId, updater) {
    state.newPattern.pieceRows = state.newPattern.pieceRows.map((row) => row.id === pieceId ? updater(row) : row);
}
function clearPatternPieceTemplate(pieceId) {
...
function buildColorAllocationId(pieceId, colorName) {
    return `${pieceId}-${colorName.trim().replace(/\s+/g, '-')}`;
}
function syncPieceApplicableSkuCodes(pieceId) {
    updatePatternPieceRow(pieceId, (row) => ({
        ...row,
        applicableSkuCodes: dedupeStrings(row.colorAllocations.flatMap((allocation) => allocation.skuCodes ?? [])),
    }));
}
function syncColorAllocationsFromColorPieceQuantities(row) {
    ...
    const colorAllocations = row.colorPieceQuantities
        ...
        .map((quantity, index) => {
        const matched = colorOptions.find((option) => String(option.colorCode || option.colorName).trim().toLowerCase()
...
function normalizePackagePieceQty(value) {
...
function getPackagePieceQty(row) {
...
function sanitizePatternPackagePieceRow(row) {
...
function sanitizePatternPackagePieceRows(rows) {
    return rows.map(sanitizePatternPackagePieceRow);
}
function refreshPatternPackagePieceTotals() {
...
function refreshPatternPieceTotals() {
...
function clonePatternPieceRows(rows) {
    return rows.map((row) => ({
        ...row,
...
function clonePieceInstances(instances) {
    return instances.map((instance) => ({
        ...instance,
        specialCraftAssignments: instance.specialCraftAssignments.map((assignment) => ({ ...assignment })),
    }));
}
function applyPatternPieceRowsWithInstanceProtection(updater) {
    ...
    if (removedConfigured.length === 0)
        return;
    if (window.confirm('当前减少片数会删除已配置特殊工艺的裁片实例，是否继续？'))
        return;
...
function getPatternSizeCodeSet() {
...
function getPatternBomColorNameSet() {
    return new Set(getPatternBomColorOptions().map((item) => item.colorName.trim().toLowerCase()));
}
function getPatternSpecialCraftKeySet() {
    return new Set(getPatternPieceSpecialCraftOptionsFromCurrentTechPack().map((item) => `${item.processCode}:${item.craftCode}:${item.selectedTargetObject}`));
}
function toFileLastModifiedText(file) {
...
function applyPatternFileError(message, input) {
    state.newPattern.parseError = message;
    input.value = '';
}
function openPatternFileInput(inputId) {
...
function applyPatternMaterialTypeChange(nextType) {
...
async function startPatternParsing() {
    ...
    try {
        ...
        const normalizedRows = normalizePatternPieceRows(parsed.pieceRows, state.editPatternItemId || `PAT-${Date.now()}`, state.newPattern.linkedBomItemId);
...
function validatePatternForm() {
    ...
    if (invalidColorQtyRow)
        return '颜色片数必须为非负整数';
    const noEnabledColorRow = state.newPattern.pieceRows.find((row) => !hasEnabledColorPiece(row));
    if (noEnabledColorRow)
        return '请至少选择一个适用颜色';
    const noPositiveColorRow = state.newPattern.pieceRows.find((row) => !hasPositiveEnabledColorPiece(row));
    if (noPositiveColorRow)
        return '当前总片数为 0，请维护颜色片数';
    const invalidColorRow = state.newPattern.pieceRows.find((row) => row.colorPieceQuantities.some((quantity) => quantity.enabled && !bomColorNameSet.has(quantity.colorName.trim().toLowerCase())));
    if (invalidColorRow)
        return '适用颜色必须来自当前技术包颜色';
    const invalidSpecialCraftRow = state.newPattern.pieceRows.find((row) => row.specialCrafts.some((craft) => !validSpecialCraftKeys.has(`${craft.processCode}:${craft.craftCode}:${craft.selectedTargetObject}`)));
    if (invalidSpecialCraftRow)
        return '特殊工艺必须来自当前技术包且作用对象为已裁部位';
    const invalidTemplateRow = state.newPattern.pieceRows.find((row) => row.isTemplate && !String(row.partTemplateId || '').trim());
    if (invalidTemplateRow)
        return '选择了模板裁片后，必须指定具体部位模板';
    const templateIdsByPartKey = new Map();
    state.newPattern.pieceRows.forEach((row) => {
        if (!row.isTemplate || !String(row.partTemplateId || '').trim())
            return;
        const partKey = buildPatternPiecePartKey(row);
        if (!partKey)
            return;
        const current = templateIdsByPartKey.get(partKey) || new Set();
    ...
    if (state.newPattern.patternMaterialType === 'WOVEN') {
        if (state.newPattern.parseStatus !== 'PARSED')
            return '布料纸样需先解析纸样';
        if (state.newPattern.pieceRows.length === 0)
            return '布料纸样解析不到裁片明细，不能保存';
        const invalidRow = state.newPattern.pieceRows.find((row) => row.sourceType !== 'PARSED_PATTERN'
            || !row.name.trim()
            || Number(row.totalPieceQty) <= 0
            || row.missingName);
        if (invalidRow?.missingName)
            return '布料纸样存在名称缺失，不能保存';
        if (invalidRow)
            return '布料纸样解析结果不完整，不能保存';
        return null;
    }
    ...
    if (state.newPattern.pieceRows.length === 0)
        return '毛织纸样至少保留 1 行毛织部位明细';
    const invalidRow = state.newPattern.pieceRows.find((row) => row.sourceType !== 'MANUAL' || !row.name.trim() || Number(row.totalPieceQty) <= 0);
    if (invalidRow)
        return '毛织纸样部位名称和颜色片数必须填写完整';
    return null;
}
function validatePatternMerchandiserStep() {
    if (state.patternFormPurpose === 'PACKAGE') {
        if (!state.newPattern.name.trim())
            return '请填写纸样名称';
        if (!String(state.newPattern.type || '').trim())
            return '请选择纸样分类';
        if (state.newPattern.patternMaterialType === 'UNKNOWN')
            return '请选择纸样类型';
        return null;
    }
    if (!state.newPattern.linkedBomItemId.trim())
        return '请选择关联物料';
...
function validatePatternMakerStep() {
...
function validatePatternPackage() {
    const baseError = validatePatternMerchandiserStep();
    if (baseError)
        return baseError;
    if (state.newPattern.patternMaterialType === 'WOOL') {
        if (!state.newPattern.singlePatternFileName.trim() && !state.newPattern.file.trim())
            return '请上传毛织纸样 Zip 文件';
        if (!hasFileExtension(state.newPattern.singlePatternFileName || state.newPattern.file, ['.zip']))
            return '毛织纸样包只能上传 Zip 文件';
        if (state.newPattern.pieceRows.length === 0)
            return '请新增毛织部位明细';
        const invalidRow = state.newPattern.pieceRows.find((row) => row.sourceType !== 'MANUAL' || !row.name.trim() || Number(row.totalPieceQty) <= 0);
        if (invalidRow)
            return '毛织部位名称和片数必须填写完整';
        return null;
    }
    if (!state.newPattern.prjFile?.fileName)
        return '请上传纸样 PRJ 文件';
    if (!state.newPattern.markerImage?.fileName)
        return '请上传唛架图片';
    ...
    if (state.newPattern.patternMaterialType === 'WOVEN') {
        if (!state.newPattern.dxfFileName.trim())
            return '请上传 DXF 文件';
        if (!state.newPattern.rulFileName.trim())
            return '请上传 RUL 文件';
        if (!hasFileExtension(state.newPattern.dxfFileName, ['.dxf']))
            return '文件格式不正确，请上传 DXF 文件';
        if (!hasFileExtension(state.newPattern.rulFileName, ['.rul']))
            return '文件格式不正确，请上传 RUL 文件';
        if (state.newPattern.parseStatus !== 'PARSED')
            return '布料纸样需先解析部位信息';
        if (state.newPattern.pieceRows.length === 0)
            return '布料纸样解析不到部位明细，不能保存';
        const invalidRow = state.newPattern.pieceRows.find((row) => row.sourceType !== 'PARSED_PATTERN'
            || !row.name.trim()
            || Number(row.totalPieceQty) <= 0
            || row.missingName);
...
function updatePatternMaintainerStatuses(nextStatus) {
...
function buildPatternItemFromForm(nowId, finalStatus) {
    const isPatternPackage = state.patternFormPurpose === 'PACKAGE';
    const normalizedFormPieceRows = normalizePatternPieceRows(state.newPattern.pieceRows.map((row) => ({ ...row })), nowId, state.newPattern.linkedBomItemId);
    ...
    const selectedSizeCodes = dedupeStrings(state.newPattern.selectedSizeCodes.length > 0
    ...
    const normalizedPatternMaterialType = state.newPattern.patternMaterialType === 'UNKNOWN'
        ? 'WOVEN'
        : state.newPattern.patternMaterialType;
    const normalizedPatternFileMode = normalizedPatternMaterialType === 'WOOL'
        ? 'SINGLE_FILE'
        : state.newPattern.patternFileMode || 'PAIRED_DXF_RUL';
    ...
    const nextParseStatus = normalizedPatternMaterialType === 'WOOL'
        ? 'NOT_REQUIRED'
        : finalStatus === '已解析待确认' || finalStatus === '已完成'
            ...
            : state.newPattern.parseStatus === 'FAILED'
                ...
                : 'NOT_PARSED';
    return {
        recordKind: isPatternPackage ? 'PACKAGE' : 'MATERIAL_ASSOCIATION',
        ...
        isWoolted: state.newPattern.patternMaterialType === 'WOOL' ? '是' : '否',
        maintainerStepStatus: finalStatus,
        merchandiserInfoStatus: '已填写',
        patternMakerInfoStatus: finalStatus === '已解析待确认' || finalStatus === '已完成'
            ? '已解析'
            : finalStatus === '待解析'
                ? '待解析'
                : '未填写',
        ...
        parsedAt: normalizedPatternMaterialType === 'WOVEN' && (finalStatus === '已解析待确认' || finalStatus === '已完成')
        ...
        selectedSizeCodes,
        sizeRange,
        pieceRows: normalizedPieceRows.map((row) => isPatternPackage
            ? {
                ...row,
...
function updatePieceInstances(nextInstances) {
...
function refreshPieceInstanceSpecialCraftDom() {
    ...
    document.querySelectorAll('[data-testid="piece-instance-craft-summary"][data-piece-id]').forEach((summaryNode) => {
    ...
    const configuredTotal = document.querySelector('[data-testid="pattern-special-craft-configured-total"]');
    if (configuredTotal) {
        configuredTotal.textContent = `已配置特殊工艺裁片：${state.newPattern.specialCraftConfiguredPieceTotal} 片`;
    }
    const unconfiguredTotal = document.querySelector('[data-testid="pattern-special-craft-unconfigured-total"]');
    if (unconfiguredTotal) {
        unconfiguredTotal.textContent = `未配置特殊工艺裁片：${state.newPattern.specialCraftUnconfiguredPieceTotal} 片`;
    }
}
function schedulePieceInstanceSpecialCraftDomRefresh() {
...
function getActivePieceInstance() {
    return state.newPattern.pieceInstances.find((instance) => instance.pieceInstanceId === state.activePieceInstanceId) ?? null;
}
...
function savePatternFromTwoStep(finalStatus) {
    const nowId = state.editPatternItemId || `PAT-${Date.now()}`;
    const nextPattern = buildPatternItemFromForm(nowId, finalStatus);
    if (state.patternFormPurpose === 'PACKAGE') {
        const duplicateResult = checkDuplicatePattern({ id: nowId, ...nextPattern }, state.patternItems.filter((item) => item.recordKind === 'PACKAGE'));
    ...
    if (state.editPatternItemId) {
        state.patternItems = state.patternItems.map((item) => item.id === state.editPatternItemId ? { ...item, ...nextPattern } : item);
    }
    else {
...
function applyPatternPackageToAssociation(patternPackageId) {
    ...
    state.newPattern.pieceRows = normalizePatternPieceRows(state.newPattern.pieceRows, state.editPatternItemId || 'DRAFT', state.newPattern.linkedBomItemId);
    refreshPatternPieceTotals();
}
function handleTechPackField(node) {
    ...
    if (field === 'new-pattern-linked-bom-item') {
        ...
        applyPatternPieceRowsWithInstanceProtection(() => {
            state.newPattern.pieceRows = state.newPattern.pieceRows.map((row) => ({
                ...row,
                colorAllocations: row.colorAllocations.filter((allocation) => getBomColorOptionsForPattern(value).some((option) => option.colorName.trim().toLowerCase() === allocation.colorName.trim().toLowerCase())),
                colorPieceQuantities: row.colorPieceQuantities.filter((quantity) => nextColorOptions.some((option) => String(option.colorCode || option.colorName).trim().toLowerCase()
                    === String(quantity.colorId || quantity.colorName).trim().toLowerCase()
                    || option.colorName.trim().toLowerCase() === quantity.colorName.trim().toLowerCase())),
            }));
            state.newPattern.pieceRows = normalizePatternPieceRows(state.newPattern.pieceRows, state.editPatternItemId || 'DRAFT', state.newPattern.linkedBomItemId);
    ...
    if (field === 'new-pattern-binding-strip-name') {
        ...
        state.newPattern.bindingStrips = normalizePatternBindingStrips(state.newPattern.bindingStrips.map((item) => item.bindingStripId === bindingStripId ? { ...item, bindingStripName: value } : item));
        return true;
    }
    if (field === 'new-pattern-binding-strip-length-cm') {
        const bindingStripId = node.dataset.bindingStripId;
        state.newPattern.bindingStrips = normalizePatternBindingStrips(state.newPattern.bindingStrips.map((item) => item.bindingStripId === bindingStripId ? { ...item, lengthCm: Number(value) } : item));
        return true;
    }
    if (field === 'new-pattern-binding-strip-width-cm') {
        const bindingStripId = node.dataset.bindingStripId;
        state.newPattern.bindingStrips = normalizePatternBindingStrips(state.newPattern.bindingStrips.map((item) => item.bindingStripId === bindingStripId ? { ...item, widthCm: Number(value) } : item));
        return true;
    }
    if (field === 'new-pattern-binding-strip-remark') {
        const bindingStripId = node.dataset.bindingStripId;
        state.newPattern.bindingStrips = normalizePatternBindingStrips(state.newPattern.bindingStrips.map((item) => item.bindingStripId === bindingStripId ? { ...item, remark: value } : item));
        return true;
    }
    ...
    if (field === 'new-pattern-piece-count') {
        if (state.newPattern.patternMaterialType === 'WOVEN' && state.patternFormPurpose !== 'PACKAGE')
            return true;
        const pieceId = node.dataset.pieceId;
        if (!pieceId)
            return true;
        ...
        applyPatternPieceRowsWithInstanceProtection(() => {
            updatePatternPieceRow(pieceId, (row) => ({
                ...row,
                count: nextCount,
                colorPieceQuantities: row.colorPieceQuantities.map((quantity, index) => index === 0 || quantity.pieceQty > 0 ? { ...quantity, enabled: true, pieceQty: nextCount } : quantity),
            }));
        });
        return true;
    }
    ...
    if (field === 'new-pattern-piece-color-count') {
        ...
        if (!pieceId || (!colorId && !colorName))
            return true;
        const pieceQty = value.trim() === '' ? 0 : Number(value);
        applyPatternPieceRowsWithInstanceProtection(() => {
            updatePatternPieceRow(pieceId, (row) => ({
                ...row,
                colorPieceQuantities: row.colorPieceQuantities.map((quantity) => quantity.colorId === colorId || quantity.colorName === colorName
                    ? { ...quantity, pieceQty }
                    : quantity),
            }));
        });
        ...
        applyPatternPieceRowsWithInstanceProtection(() => {
            updatePatternPieceRow(pieceId, (row) => ({
                ...
                colorPieceQuantities: row.colorPieceQuantities.map((quantity) => {
                    if (quantity.colorId !== colorId && quantity.colorName !== colorName)
                        return quantity;
                    return {
                        ...quantity,
                        enabled: checked,
    ...
    if (field === 'new-bom-print-side-mode') {
        applyBomPrintSideModeChange((value === 'SINGLE' || value === 'DOUBLE' ? value : ''));
        return true;
    }
    ...
    if (field === 'new-bom-sku') {
        ...
        if (checked) {
            ...
            state.newBomItem.applicableSkuCodes = state.newBomItem.applicableSkuCodes.filter((code) => code !== skuCode);
        }
        return true;
    }
    if (field === 'new-bom-usage-process') {
        ...
        if (checked) {
            ...
            state.newBomItem.usageProcessCodes = state.newBomItem.usageProcessCodes.filter((code) => code !== processCode);
        }
        return true;
    }
    ...
    if (field === 'new-technique-craft-code') {
        ...
        const selectedTargetObject = craft?.isSpecialCraft && craft.supportedTargetObjectLabels?.length === 1
    ...
    if (field === 'new-technique-target-object') {
        state.newTechnique.selectedTargetObject = value;
        return true;
    }
    ...
    if (field === 'new-technique-assignment-granularity') {
        state.newTechnique.assignmentGranularity = (value || 'ORDER');
        return true;
    }
    ...
    if (field === 'new-technique-difficulty') {
        state.newTechnique.difficulty = value;
        return true;
    }
    ...
    if (field === 'bom-print') {
        ...
        if (!bomId)
            return true;
        state.bomItems = state.bomItems.map((item) => item.id === bomId
            ? {
                ...item,
    ...
    if (field === 'bom-dye') {
        ...
        if (!bomId)
            return true;
        state.bomItems = state.bomItems.map((item) => item.id === bomId ? { ...item, dyeRequirement: value } : item);
        syncTechPackToStore();
        return true;
    }
    if (field === 'bom-shrink') {
        ...
        if (!bomId)
            return true;
        state.bomItems = state.bomItems.map((item) => item.id === bomId ? { ...item, shrinkRequirement: normalizeBomRequirement(value) } : item);
        syncTechPackToStore();
        return true;
    }
    if (field === 'bom-wash') {
        ...
        if (!bomId)
            return true;
        state.bomItems = state.bomItems.map((item) => item.id === bomId ? { ...item, washRequirement: normalizeBomRequirement(value) } : item);
        syncTechPackToStore();
        return true;
    }
    ...
    if (field === 'material-price') {
        ...
        if (!rowId)
            return true;
        state.materialCostRows = state.materialCostRows.map((row) => row.id === rowId ? { ...row, price: value } : row);
    ...
    if (field === 'material-currency') {
        ...
        if (!rowId)
            return true;
        state.materialCostRows = state.materialCostRows.map((row) => row.id === rowId ? { ...row, currency: value } : row);
        syncTechPackToStore({ touch: false });
        return true;
    }
    if (field === 'material-unit') {
        ...
        if (!rowId)
            return true;
        state.materialCostRows = state.materialCostRows.map((row) => row.id === rowId ? { ...row, unit: value } : row);
        syncTechPackToStore({ touch: false });
        return true;
    }
    if (field === 'process-price') {
        ...
        if (!rowId)
            return true;
        state.processCostRows = state.processCostRows.map((row) => row.id === rowId ? { ...row, price: value } : row);
        syncTechPackToStore({ touch: false });
        return true;
    }
    if (field === 'process-currency') {
        ...
        if (!rowId)
            return true;
        state.processCostRows = state.processCostRows.map((row) => row.id === rowId ? { ...row, currency: value } : row);
        syncTechPackToStore({ touch: false });
        return true;
    }
    if (field === 'process-unit') {
        ...
        if (!rowId)
            return true;
        state.processCostRows = state.processCostRows.map((row) => row.id === rowId ? { ...row, unit: value } : row);
        syncTechPackToStore({ touch: false });
        return true;
    }
    if (field === 'custom-cost-name') {
        ...
        if (!rowId)
            return true;
        state.customCostRows = state.customCostRows.map((row) => row.id === rowId ? { ...row, name: value } : row);
        syncTechPackToStore({ touch: false });
        return true;
    }
    if (field === 'custom-cost-price') {
        ...
        if (!rowId)
            return true;
        state.customCostRows = state.customCostRows.map((row) => row.id === rowId ? { ...row, price: value } : row);
        syncTechPackToStore({ touch: false });
        return true;
    }
    if (field === 'custom-cost-currency') {
        ...
        if (!rowId)
            return true;
        state.customCostRows = state.customCostRows.map((row) => row.id === rowId ? { ...row, currency: value } : row);
        syncTechPackToStore({ touch: false });
        return true;
    }
    if (field === 'custom-cost-unit') {
        ...
        if (!rowId)
            return true;
        state.customCostRows = state.customCostRows.map((row) => row.id === rowId ? { ...row, unit: value } : row);
        syncTechPackToStore({ touch: false });
        return true;
    }
    if (field === 'custom-cost-remark') {
        ...
        if (!rowId)
            return true;
        state.customCostRows = state.customCostRows.map((row) => row.id === rowId ? { ...row, remark: value } : row);
        syncTechPackToStore({ touch: false });
        return true;
    }
    ...
    if (field === 'mapping-line-bom-item' ||
        ...
        if (field === 'mapping-line-bom-item') {
            ...
            updateColorMappingLine(mappingId, lineId, (line) => ({
                ...
                applicableSkuCodes: selectedBom && selectedBom.applicableSkuCodes.length > 0
        ...
        if (field === 'mapping-line-piece-id') {
            ...
            if (currentLine)
                patternId = currentLine.patternId;
            ...
            updateColorMappingLine(mappingId, lineId, (line) => ({
                ...
                applicableSkuCodes: piece && piece.applicableSkuCodes.length > 0
        ...
        if (field === 'mapping-line-skus') {
            updateColorMappingLine(mappingId, lineId, (line) => ({
                ...line,
                applicableSkuCodes: dedupeStrings(value
                    .split(',')
                    .map((item) => item.trim())
    ...
    if (field === 'new-design-side-type') {
        ...
        if (state.selectedDesignFile && !isImageDesignFile(state.selectedDesignFile)) {
            state.newDesignPreviewThumbnailDataUrl = buildDesignPlaceholderImage(state.selectedDesignFile.name, state.newDesignSideType);
        }
        return true;
    }
...
export function handleTechPackEvent(target) {
    const fieldNode = target.closest('[data-tech-field]');
    ...
    if (isTechPackReadOnly()) {
        const isReadonlySafeAction = action.startsWith('close-') ||
    ...
    if (action === 'save-pattern-merchandiser-step') {
        ...
        state.newPattern.parseError = '';
        updatePatternMaintainerStatuses('待版师维护');
        if (!savePatternFromTwoStep('待版师维护'))
            return true;
        state.addPatternDialogOpen = false;
        closePatternTemplateDialog(false);
        resetPatternForm();
    ...
    if (action === 'delete-pattern-binding-strip') {
        ...
        if (!bindingStripId)
            return true;
        if (!window.confirm('确认删除该捆条？'))
            return true;
        state.newPattern.bindingStrips = normalizePatternBindingStrips(state.newPattern.bindingStrips.filter((item) => item.bindingStripId !== bindingStripId));
        return true;
    }
    ...
    if (action === 'confirm-pattern-duplicate-warning') {
        ...
        state.patternDuplicateWarning = null;
        if (!savePatternFromTwoStep(warning.finalStatus))
            return true;
        state.addPatternDialogOpen = false;
        closePatternTemplateDialog(false);
        resetPatternForm();
    ...
    if (action === 'save-pattern-package') {
        ...
        state.newPattern.parseError = '';
        const finalStatus = state.newPattern.patternMaterialType === 'WOOL'
            ? '已解析待确认'
        ...
        if (!savePatternFromTwoStep(finalStatus))
            return true;
        state.addPatternDialogOpen = false;
        closePatternTemplateDialog(false);
        resetPatternForm();
    ...
    if (action === 'save-pattern-maker-step' || action === 'save-pattern-and-parse') {
        ...
        state.newPattern.parseError = '';
        const finalStatus = action === 'save-pattern-and-parse' || state.newPattern.parseStatus === 'PARSED' || state.newPattern.patternMaterialType === 'WOOL'
            ? '已解析待确认'
            : '待解析';
        updatePatternMaintainerStatuses(finalStatus);
        if (!savePatternFromTwoStep(finalStatus))
    ...
    if (action === 'clear-pattern-uploaded-files') {
        if (state.newPattern.patternMaterialType === 'WOVEN') {
            ...
            if (dxfInput instanceof HTMLInputElement)
                dxfInput.value = '';
            if (rulInput instanceof HTMLInputElement)
                rulInput.value = '';
        }
        else {
            clearWoolFileState();
    ...
    if (action === 'toggle-pattern-piece-color') {
        ...
        if (!pieceId || !colorName)
            return true;
        const colorOption = getPatternBomColorOptions().find((option) => option.colorName.trim().toLowerCase() === colorName.trim().toLowerCase()
            || String(option.colorCode || '').trim().toLowerCase() === String(colorId || '').trim().toLowerCase());
        if (!colorOption)
            return true;
        applyPatternPieceRowsWithInstanceProtection(() => {
            updatePatternPieceRow(pieceId, (row) => {
                const exists = row.colorPieceQuantities.some((quantity) => (colorId && quantity.colorId.trim().toLowerCase() === colorId.trim().toLowerCase())
                    || quantity.colorName.trim().toLowerCase() === colorName.trim().toLowerCase());
                return {
                    ...row,
                    colorPieceQuantities: exists
                        ? row.colorPieceQuantities.map((quantity) => (colorId && quantity.colorId.trim().toLowerCase() === colorId.trim().toLowerCase())
                            || quantity.colorName.trim().toLowerCase() === colorName.trim().toLowerCase()
    ...
    if (action === 'toggle-pattern-piece-special-craft') {
        ...
        const selectedTargetObject = actionNode.dataset.targetObject;
        if (!pieceId || !processCode || !craftCode || !selectedTargetObject)
            return true;
        const specialCraft = getPatternPieceSpecialCraftOptionsFromCurrentTechPack().find((item) => item.processCode === processCode
            && item.craftCode === craftCode
            && item.selectedTargetObject === selectedTargetObject);
        if (!specialCraft)
            return true;
        updatePatternPieceRow(pieceId, (row) => {
            const exists = row.specialCrafts.some((item) => item.processCode === processCode
                && item.craftCode === craftCode
                && item.selectedTargetObject === selectedTargetObject);
            const nextSpecialCrafts = exists
                ? row.specialCrafts.filter((item) => !(item.processCode === processCode
                    && item.craftCode === craftCode
                    && item.selectedTargetObject === selectedTargetObject))
    ...
    if (action === 'add-piece-instance-special-craft') {
        ...
        updatePieceInstances(state.newPattern.pieceInstances.map((instance) => {
            if (instance.pieceInstanceId !== activeInstance.pieceInstanceId)
                return instance;
    ...
    if (action === 'delete-piece-instance-special-craft') {
        ...
        if (!pieceInstanceId || !assignmentId)
            return true;
        updatePieceInstances(state.newPattern.pieceInstances.map((instance) => {
            if (instance.pieceInstanceId !== pieceInstanceId)
                return instance;
            const specialCraftAssignments = instance.specialCraftAssignments.filter((assignment) => assignment.assignmentId !== assignmentId);
            return {
                ...instance,
                specialCraftAssignments,
                status: specialCraftAssignments.length > 0 ? '已配置' : '未配置',
    ...
    if (action === 'apply-piece-instance-craft-to-same-color') {
        const activeInstance = getActivePieceInstance();
        if (!activeInstance || activeInstance.specialCraftAssignments.length === 0)
            return true;
        if (!window.confirm('是否将当前片的特殊工艺应用到同颜色全部片？'))
            return true;
        updatePieceInstances(state.newPattern.pieceInstances.map((instance) => {
            if (instance.sourcePieceId !== activeInstance.sourcePieceId
    ...
    if (action === 'save-bom') {
        if (!state.newBomItem.materialName.trim())
            return true;
        ...
        if (state.newBomItem.printRequirement !== '无'
        ...
        const nextBom = {
            ...
            colorLabel: (() => {
                ...
                if (state.newBomItem.applicableSkuCodes.length === 0)
                    return '全部SKU（当前未区分颜色）';
                if (state.newBomItem.colorLabel.trim())
                    return state.newBomItem.colorLabel.trim();
                const colors = dedupeStrings(state.newBomItem.applicableSkuCodes
            ...
            usageProcessCodes: state.newBomItem.usageProcessCodes.length > 0
            ...
            printSideMode: state.newBomItem.printRequirement === '无'
                ? ''
                : state.newBomItem.printSideMode,
            frontPatternDesignId: state.newBomItem.printRequirement === '无'
                ? ''
                : state.newBomItem.frontPatternDesignId,
            insidePatternDesignId: state.newBomItem.printRequirement === '无' || state.newBomItem.printSideMode !== 'DOUBLE'
                ? ''
                : state.newBomItem.insidePatternDesignId,
        };
    ...
    if (action === 'confirm-color-mapping') {
        ...
        if (!mappingId)
            return true;
        state.colorMaterialMappings = state.colorMaterialMappings.map((item) => item.id === mappingId
            ? {
                ...item,
                status: item.status === 'AUTO_DRAFT' || item.status === 'MANUAL_ADJUSTED'
                    ? 'CONFIRMED'
                    : item.status,
    ...
    if (action === 'save-technique') {
        ...
        if (editingTarget && !canEditTechnique(editingTarget))
            return true;
        if (!editingTarget && selectedMeta.stageCode === 'PREP') {
            return true;
        }
        const immutablePrepMeta = editingTarget && isBomDrivenPrepTechnique(editingTarget)
            ? {
        ...
        if (effectiveMeta.isSpecialCraft) {
            const duplicate = state.techniques.some((item) => item.id !== state.editTechniqueId
        ...
        if (state.editTechniqueId) {
            state.techniques = state.techniques.map((item) => item.id === state.editTechniqueId ? nextItem : item);
        }
        else {
    ...
    if (action === 'delete-technique') {
        ...
        if (!techId)
            return true;
        const target = getTechniqueById(techId);
        if (target && isBomDrivenPrepTechnique(target))
            return true;
        state.techniques = state.techniques.filter((item) => item.id !== techId);
        syncProcessCostRows();
        syncTechPackToStore();
    ...
    if (action === 'save-size') {
        if (!state.techPack || !state.newSizeRow.part.trim())
            return true;
        const row = {
            id: `size-${Date.now()}`,
    ...
    if (action === 'save-design') {
        if (!state.techPack || !state.newDesignName.trim() || !state.newDesignFileName.trim())
            return true;
        if (!state.newDesignOriginalFileDataUrl.trim()) {
            window.alert('设计稿文件处理中，请稍后再保存');
            return true;
        }
        const previewThumbnailDataUrl = state.newDesignPreviewThumbnailDataUrl
    ...
    if (action === 'delete-design') {
        const designId = actionNode.dataset.designId;
        if (!state.techPack || !designId)
            return true;
        const referencedByBom = state.bomItems.find((item) => item.frontPatternDesignId === designId || item.insidePatternDesignId === designId);
        if (referencedByBom) {
    ...
    if (action === 'save-attachment') {
        if (!state.techPack || !state.newAttachment.fileName.trim())
            return true;
        state.techPack = {
            ...state.techPack,
            attachments: [
    ...
    if (action === 'delete-attachment') {
        ...
        if (!state.techPack || !attachmentId)
            return true;
        state.techPack = {
            ...state.techPack,
