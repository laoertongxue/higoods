import { escapeHtml, getPatternDesignPreviewUrl, isTechPackReadOnly, state, } from './context.ts';
export function renderDesignTab() {
    ...
    const renderDesignGroup = (title, items) => `
...
export function renderAttachmentsTab() {
    const techPack = state.techPack;
    if (!techPack)
        return '';
    const readonly = isTechPackReadOnly();
...
export function renderAddDesignDialog() {
    if (!state.addDesignDialogOpen)
        return '';
    return `
    <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/45 p-4" data-dialog-backdrop="true">
      <section class="w-full max-w-md rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
        ...
        <footer class="flex items-center justify-end gap-2 border-t px-6 py-4">
          ...
          <button type="button" class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 ${state.newDesignName.trim() && state.newDesignFileName.trim() ? '' : 'pointer-events-none opacity-50'}" data-tech-action="save-design">确认</button>
        </footer>
      </section>
    </div>
  `;
}
export function renderAddAttachmentDialog() {
    if (!state.addAttachmentDialogOpen)
        return '';
    return `
    <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/45 p-4" data-dialog-backdrop="true">
      <section class="w-full max-w-md rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
        ...
        <footer class="flex items-center justify-end gap-2 border-t px-6 py-4">
          ...
          <button class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 ${state.newAttachment.fileName.trim() ? '' : 'pointer-events-none opacity-50'}" data-tech-action="save-attachment">确认</button>
        </footer>
      </section>
    </div>
  `;
