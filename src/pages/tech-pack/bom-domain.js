import { dedupeStrings, bomRequirementOptions, bomUsageProcessOptions, dyeOptions, escapeHtml, getPatternDesignPreviewAssetById, getPatternDesignOptionsBySide, getSkuOptionsForCurrentSpu, isTechPackReadOnly, printOptions, state, } from './context.ts';
function renderTextValue(value) {
    return value.trim().length > 0 ? escapeHtml(value) : '<span class="text-muted-foreground">-</span>';
}
function renderPrintSideModeLabel(mode) {
    if (mode === 'SINGLE')
        return '单面印';
    if (mode === 'DOUBLE')
        return '双面印';
...
function renderBomPrintBindingCell(item, side) {
...
export function renderDesignThumbnailPreviewDialog() {
    if (!state.designPreviewDialogOpen)
        return '';
...
export function renderBomTab() {
    ...
    const deriveColorLabel = (item) => {
        if (item.colorLabel.trim())
            return item.colorLabel.trim();
        if (item.applicableSkuCodes.length === 0)
            return '全部SKU（当前未区分颜色）';
        const colors = dedupeStrings(item.applicableSkuCodes
    ...
    const groups = Array.from(groupsByColor.values()).sort((a, b) => {
        if (a.colorLabel.startsWith('全部'))
            return -1;
        if (b.colorLabel.startsWith('全部'))
            return 1;
...
export function renderBomFormDialog() {
    if (!state.addBomDialogOpen)
        return '';
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/45 p-4" data-dialog-backdrop="true">
      <section class="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
        ...
        .join('')}
            ...
            <label class="space-y-1">
              ...
              <select class="w-full rounded-md border px-3 py-2 text-sm" data-tech-field="new-bom-color-label">
                ...
                ${colorOptions
        .map((option) => `<option value="${escapeHtml(option)}" ${state.newBomItem.colorLabel === option ? 'selected' : ''}>${escapeHtml(option)}</option>`)
        .join('')}
              </select>
            </label>
            <label class="space-y-1">
        ...
        ? `
                  <label class="space-y-1">
                    ...
                    <select class="w-full rounded-md border px-3 py-2 text-sm" data-tech-field="new-bom-front-pattern-design-id">
                      ...
                      ${frontDesignOptions
            .map((item) => `<option value="${item.id}" ${state.newBomItem.frontPatternDesignId === item.id ? 'selected' : ''}>${escapeHtml(item.name)}</option>`)
            .join('')}
                    </select>
                  </label>
                `
        ...
        ? `
                  <label class="space-y-1">
                    ...
                    <select class="w-full rounded-md border px-3 py-2 text-sm" data-tech-field="new-bom-inside-pattern-design-id">
                      ...
                      ${insideDesignOptions
            .map((item) => `<option value="${item.id}" ${state.newBomItem.insidePatternDesignId === item.id ? 'selected' : ''}>${escapeHtml(item.name)}</option>`)
            .join('')}
                    </select>
                  </label>
                `
        ...
        <footer class="flex items-center justify-end gap-2 border-t px-6 py-4">
          ...
          <button type="button" class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 ${state.newBomItem.materialName.trim() ? '' : 'pointer-events-none opacity-50'}" data-tech-action="save-bom">确认</button>
        </footer>
      </section>
    </div>
  `;
