import { initialAllocationByTaskId, initialAllocationEvents, initialDeductionBasisItems, initialQualityInspections, initialReturnInboundBatches, listDyePrintOrdersStore, } from '../data/fcs/store-domain-quality-seeds';
import { applyQualitySeedBootstrap } from '../data/fcs/store-domain-quality-bootstrap';
import { processTasks } from '../data/fcs/process-tasks';
import { deriveDyePrintSettlementRelation, resolveDefaultReturnInboundQcPolicy, } from '../data/fcs/store-domain-quality-types';
import { applyReturnInboundPassWriteback, blockTaskForReturnInboundQc, createQcFromReturnInboundBatch, createReturnInboundBatchRecord, updateReturnInboundBatchStatus, upsertDeductionBasisFromReturnInboundQc, } from '../data/fcs/return-inbound-workflow';
...
const DBI_STATUS_LABEL = {
    DRAFT: '草稿',
    CONFIRMED: '已确认',
    DISPUTED: '争议中',
    VOID: '已作废',
};
const DBI_STATUS_CLASS = {
...
function mapDyePrintToReturnInboundProcessType(processType) {
...
function showDyePrintToast(message, tone = 'success') {
...
function getProcessorOptions(orders) {
...
function createDyePrintOrder() {
    ...
    const order = {
        ...
        settlementRelation: deriveDyePrintSettlementRelation(state.createForm.processorFactoryId, 'PROCESSOR', state.createForm.processorFactoryId),
...
function startDyePrintOrder(dpId) {
    const order = getOrders().find((item) => item.dpId === dpId);
    if (!order)
        return { ok: false, message: `加工单 ${dpId} 不存在` };
    if (order.status !== 'DRAFT')
        return { ok: false, message: '只有草稿状态的加工单可以开始加工' };
    replaceOrder({
        ...order,
        status: 'PROCESSING',
...
function closeDyePrintOrder(dpId) {
    const order = getOrders().find((item) => item.dpId === dpId);
    if (!order)
        return { ok: false, message: `加工单 ${dpId} 不存在` };
    if (order.status === 'CLOSED')
        return { ok: false, message: '加工单已关闭' };
    replaceOrder({
        ...order,
        status: 'CLOSED',
        updatedAt: nowTimestamp(),
...
function addDyePrintReturn(dpId, payload) {
    const order = getOrders().find((item) => item.dpId === dpId);
    if (!order)
        return { ok: false, message: `加工单 ${dpId} 不存在` };
    if (order.status === 'CLOSED')
        return { ok: false, message: '加工单已关闭，不能登记回货' };
    ...
    const nextStatus = totalQty >= order.plannedQty ? 'COMPLETED' : totalQty > 0 ? 'PARTIAL_RETURNED' : order.status;
    replaceOrder({
        ...order,
    ...
    if (payload.result === 'PASS') {
        ...
        if (!passResult.ok)
            return { ok: false, message: passResult.message };
        updateReturnInboundBatchStatus({
            batches: initialReturnInboundBatches,
            batchId: linkedReturnInboundBatchId,
...
function updateField(field, node) {
    ...
    if (field === 'create.productionOrderId') {
        ...
        if (state.createErrors.productionOrderId)
            delete state.createErrors.productionOrderId;
        return;
    }
    if (field === 'create.relatedTaskId') {
        state.createForm.relatedTaskId = value;
        if (state.createErrors.relatedTaskId)
            delete state.createErrors.relatedTaskId;
        return;
    }
    if (field === 'create.processorFactoryId') {
        ...
        if (state.createErrors.processorFactoryId)
            delete state.createErrors.processorFactoryId;
        return;
    }
    if (field === 'create.processType') {
        state.createForm.processType = value;
        return;
    }
    if (field === 'create.plannedQty') {
        state.createForm.plannedQty = value;
        if (state.createErrors.plannedQty)
            delete state.createErrors.plannedQty;
        return;
    }
    ...
    if (field === 'return.disposition') {
        state.returnForm.disposition = value;
        return;
    }
...
export function handleDyePrintOrdersEvent(target) {
    const filterNode = target.closest('[data-dye-filter]');
    if (filterNode instanceof HTMLInputElement || filterNode instanceof HTMLSelectElement) {
        ...
        if (field === 'status') {
            state.filterStatus = filterNode.value;
            return true;
        }
        if (field === 'processor') {
    ...
    const fieldNode = target.closest('[data-dye-field]');
    ...
    const actionNode = target.closest('[data-dye-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'submit-return') {
        ...
        const result = addDyePrintReturn(targetOrder.dpId, {
            ...
            disposition: state.returnForm.result === 'FAIL'
                ? state.returnForm.disposition
                : undefined,
...
export function isDyePrintOrdersDialogOpen() {
