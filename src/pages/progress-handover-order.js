...
import { buildHandoverPageLink, getHandoverLedgerRows, getHandoverOrderTimelineViewById, getProductionOrderHandoverSummary, } from '../data/fcs/handover-ledger-view';
import { buildHandoverOrderLink, buildQualityRecordLink, buildTaskDetailLink, } from '../data/fcs/fcs-route-links.ts';
...
function renderStatusBadge(row) {
    const toneClass = row.statusTone === 'success'
...
function renderPostRouteCard(orderId) {
    ...
    <section class="rounded-lg border bg-card p-4">
      <div class="flex flex-wrap items-center gap-2">
        ...
        <span class="inline-flex rounded border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs text-slate-700">${escapeHtml(getPostExecutionModeLabel(route.postExecutionMode))}</span>
...
function isRowFocused(row) {
    if (state.taskIdHint && row.taskId === state.taskIdHint)
        return true;
    if (!state.focusHint)
        return false;
...
function isSectionFocused(section) {
    if (state.taskIdHint && section.taskId === state.taskIdHint)
        return true;
    if (!state.focusHint)
        return false;
    if (state.focusHint === 'pickup')
    ...
    if (state.focusHint === 'warehouse-confirm')
        return section.processStatusLabel === '已交出待接收方确认';
...
function resolveSectionTone(section) {
    if (section.processStatusTone === 'success')
        return 'bg-emerald-100 text-emerald-700 border-emerald-200';
    if (section.processStatusTone === 'warning')
        return 'bg-amber-100 text-amber-700 border-amber-200';
    if (section.processStatusTone === 'danger')
        return 'bg-red-100 text-red-700 border-red-200';
    if (section.processStatusTone === 'info')
...
function syncStateFromPath(pathname) {
...
function createEmptyProcessSection(taskId, taskNo, seq, processName) {
    return {
        taskId,
        taskNo,
        seq,
...
function getCompleteProcessSections(orderId, view) {
    ...
    const sections = processSkeleton.map((node) => {
        const matchedTask = orderTasks.find((task) => task.seq === node.seq) ||
            orderTasks.find((task) => task.processNameZh === node.processName);
        if (!matchedTask) {
            return createEmptyProcessSection(`${orderId}-SEQ-${node.seq}`, `${orderId}-T${node.seq}`, node.seq, node.processName);
        }
        const existed = sectionByTaskId.get(matchedTask.taskId);
        if (!existed) {
            return createEmptyProcessSection(matchedTask.taskId, getTaskDisplayNo(matchedTask), matchedTask.seq, matchedTask.processNameZh);
        }
        return {
            ...existed,
...
function renderProcessTab(view) {
...
export function renderProgressHandoverOrderPage(productionOrderId) {
...
export function handleProgressHandoverOrderEvent(target) {
    const actionNode = target.closest('[data-handover-order-action]');
    if (!actionNode)
        return false;
...
export function isProgressHandoverOrderDialogOpen() {
    return false;
}
