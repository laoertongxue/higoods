...
import { createPatternAsset, listPatternAssets } from '../data/pcs-pattern-library.ts';
...
import { PATTERN_TASK_TEAMS, listPatternTaskMembersByTeam, } from '../data/pcs-pattern-task-team-config.ts';
import { reviewPatternTaskByBuyer, transferPatternTaskToChinaTeam, } from '../data/pcs-pattern-task-flow-service.ts';
import { getPatternTechPackActionMeta, getRevisionTechPackActionLabel, isTechPackGenerationAllowedStatus, } from '../data/pcs-tech-pack-task-generation.ts';
...
import { FIRST_SAMPLE_FACTORY_OPTIONS, updateFirstSampleTaskDetailAndSync, } from '../data/pcs-first-sample-project-writeback.ts';
import { FIRST_ORDER_SAMPLE_FACTORY_OPTIONS, updateFirstOrderSampleTaskDetailAndSync, } from '../data/pcs-first-order-sample-project-writeback.ts';
import { completePatternTask, completePlateMakingTask, completeRevisionTask, createDownstreamTasksFromRevision, createFirstSampleTaskWithProjectRelation, createPatternTask, createPlateMakingTask, createFirstOrderSampleTaskWithProjectRelation, createRevisionTask, syncExistingProjectEngineeringTaskNodes, } from '../data/pcs-task-project-relation-writeback.ts';
...
import { PATTERN_TASK_SOURCE_TYPE_LIST, PLATE_TASK_SOURCE_TYPE_LIST, REVISION_TASK_SOURCE_TYPE_LIST, } from '../data/pcs-task-source-normalizer.ts';
...
const PATTERN_DEMAND_SOURCE_OPTIONS = ['预售测款通过', '改版任务', '设计师款'];
const PATTERN_PROCESS_OPTIONS = ['数码印', '烫画', '直喷'];
const PATTERN_COLOR_DEPTH_OPTIONS = ['浅色', '深色', '中间值'];
const PATTERN_DIFFICULTY_OPTIONS = ['A++', 'A+', 'A', 'B', 'C', 'D'];
const SAMPLE_SITE_OPTIONS = ['all', '深圳', '雅加达'];
const SAMPLE_CHAIN_MODE_OPTIONS = ['复用首版结论', '新增首单样衣确认', '替代布与正确布双确认'];
const SAMPLE_SPECIAL_REASON_OPTIONS = ['定位印', '大货量大', '工厂参照样', '正确布确认', '其它'];
const initialRevisionCreateDraft = () => ({
    bindingMode: 'project',
    sourceType: '测款触发',
    projectId: '',
    styleId: '',
...
const initialRevisionDetailDraft = () => ({
...
const initialPatternCreateDraft = () => ({
    bindingMode: 'project',
    sourceType: '项目模板阶段',
    projectId: '',
    styleId: '',
...
const initialPatternDetailDraft = () => ({
    artworkVersion: '',
    difficultyGrade: 'A',
...
const initialSampleCreateDraft = () => ({
    projectId: '',
    title: '',
    ownerName: '',
...
const initialFirstOrderCreateDraft = () => ({
...
const initialFirstSampleDetailDraft = () => ({
    sampleCode: '',
    sampleImageIdsText: '',
...
function buildFirstSampleDetailDraft(task) {
    return {
...
const initialFirstOrderDetailDraft = () => ({
    patternVersion: '',
    artworkVersion: '',
    samplePlanLinesText: '',
    finalReferenceNote: '',
...
const initialFirstSampleStartDraft = () => ({
    operatorName: '当前用户',
    startedAt: nowText(),
    planFinishAt: '',
    startNote: '',
});
const initialFirstSampleResultDraft = () => ({
    sampleCode: '',
    submittedBy: '当前用户',
    completedAt: nowText(),
    sampleImageIds: [],
...
const initialFirstOrderStartDraft = () => ({
    operatorName: '当前用户',
    startedAt: nowText(),
    planFinishAt: '',
    factoryConfirmNote: '',
});
const initialFirstOrderResultDraft = () => ({
    sampleCode: '',
    submittedBy: '当前用户',
    completedAt: nowText(),
    finalReferenceNote: '',
...
function serializeFirstOrderSamplePlanLines(lines) {
    return (lines || [])
        .map((line) => [
        line.sampleRole,
        line.materialMode,
...
function parseFirstOrderSamplePlanLines(text, fallback) {
    const lines = text
        .split('\n')
        .map((item) => item.trim())
        .filter(Boolean)
        ...
        return {
            ...
            sampleRole: (sampleRole || fallbackLine?.sampleRole || '复用首版结论'),
            materialMode: (materialMode || fallbackLine?.materialMode || '沿用首版'),
            ...
            status: (status || fallbackLine?.status || '待确认'),
            note: note || fallbackLine?.note || '',
        };
    });
    return lines.length > 0 ? lines : fallback;
}
const SAMPLE_PLAN_ROLE_OPTIONS = ['复用首版结论', '替代布确认样', '正确布确认样', '工厂参照确认'];
const SAMPLE_PLAN_MATERIAL_OPTIONS = ['沿用首版', '替代布', '正确布'];
const SAMPLE_PLAN_STATUS_OPTIONS = ['待确认', '已确认', '需补样', '已取消'];
function buildEmptyFirstOrderSamplePlanLine(index) {
    return {
...
function getFirstOrderSamplePlanLinesFromText(text, fallback) {
...
function updateFirstOrderSamplePlanLineText(text, fallback, index, key, value) {
    ...
    if (key === 'quantity') {
        ...
        nextLine.materialMode = value;
    }
    else if (key === 'status') {
        nextLine.status = value;
    }
...
function addFirstOrderSamplePlanLineText(text, fallback) {
    const lines = getFirstOrderSamplePlanLinesFromText(text, fallback);
    return serializeFirstOrderSamplePlanLines([...lines, buildEmptyFirstOrderSamplePlanLine(lines.length)]);
}
function removeFirstOrderSamplePlanLineText(text, fallback, index) {
    const lines = getFirstOrderSamplePlanLinesFromText(text, fallback);
    if (lines.length <= 1)
        return serializeFirstOrderSamplePlanLines(lines);
    return serializeFirstOrderSamplePlanLines(lines.filter((_, itemIndex) => itemIndex !== index));
}
function buildFirstOrderDetailDraft(task) {
    return {
...
function buildFirstOrderResultDraft(task) {
    return {
...
function getFirstOrderSourceFirstSampleLabel(projectId) {
    const matched = listFirstSampleTasks()
        .filter((task) => task.projectId === projectId)
        .find((task) => task.workItemTypeCode === 'FIRST_SAMPLE');
    if (!matched)
        return '未找到同商品项目的首版样衣结果';
    return `${matched.firstSampleTaskCode}${matched.sampleCode ? ` · ${matched.sampleCode}` : ''}`;
}
const state = {
    notice: null,
    revisionList: { search: '', status: 'all', owner: 'all', source: 'all', quickFilter: 'all', currentPage: 1 },
    revisionTab: 'plan',
    ...
    plateList: { search: '', status: 'all', owner: 'all', source: 'all', quickFilter: 'all', currentPage: 1 },
    plateTab: 'overview',
    ...
    patternList: { search: '', status: 'all', owner: 'all', source: 'all', quickFilter: 'all', currentPage: 1 },
    patternTab: 'plan',
    ...
    firstSampleList: { search: '', status: 'all', owner: 'all', source: 'all', quickFilter: 'all', currentPage: 1, site: 'all' },
    firstSampleTab: 'overview',
    ...
    firstOrderList: { search: '', status: 'all', owner: 'all', source: 'all', quickFilter: 'all', currentPage: 1, site: 'all' },
    firstOrderTab: 'overview',
...
const firstSampleAcceptanceMap = new Map();
const firstOrderConclusionMap = new Map();
function nowText() {
    const now = new Date();
...
function renderStatusBadge(status, sample = false) {
...
function renderPageHeader(title, actionLabel, action) {
...
function renderMetricButton(label, value, active, quickFilter, actionPrefix) {
    return `
    <button
      type="button"
      class="${escapeHtml(toClassName('rounded-xl border px-4 py-4 text-left shadow-sm transition hover:border-blue-300 hover:shadow', active ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white'))}"
...
function renderPagination(currentPage, total, actionPrefix) {
...
function isOverdue(dateTime, done) {
    if (!dateTime || done)
        return false;
    return dateTime.slice(0, 10) < todayText();
}
function projectButton(projectId, projectCode, projectName) {
    if (!projectId)
        return '<span class="text-slate-400">未关联商品项目</span>';
...
function projectNodeButton(projectId, projectNodeId, label) {
    if (!projectId || !projectNodeId)
        return '<span class="text-slate-400">未关联项目节点</span>';
...
function styleArchiveButton(styleId, styleCode, styleName) {
    if (!styleId)
        return '<span class="text-slate-400">待选择款式档案</span>';
...
function styleArchiveLinkByProject(projectId) {
    const style = findStyleArchiveByProjectId(projectId);
    if (!style)
        return '<span class="text-slate-400">待建立</span>';
...
function revisionTaskNewTabLink(revisionTaskId, revisionTaskCode) {
    if (!revisionTaskId)
        return '<span class="text-slate-400">未创建改版任务</span>';
...
function styleArchiveLink(styleId, styleCode, styleName, projectId = '') {
    if (styleId)
        return styleArchiveButton(styleId, styleCode, styleName);
    if (projectId)
        return styleArchiveLinkByProject(projectId);
...
function techPackLinkByProject(projectId, technicalVersionId, fallbackLabel) {
    const style = findStyleArchiveByProjectId(projectId);
    if (!style || !technicalVersionId)
        return '<span class="text-slate-400">未生成</span>';
...
function buildStyleArchiveOptions() {
        ...
        ? `
              <label class="flex flex-col gap-2 text-sm text-slate-600">
                ...
                <select class="h-10 rounded-md border border-slate-200 px-3 text-sm text-slate-900 outline-none focus:border-blue-500" data-pcs-engineering-field="${escapeHtml(input.siteField)}">
                  ${input.siteOptions.map((option) => `<option value="${escapeHtml(option)}" ${String(listState.site) === option ? 'selected' : ''}>${escapeHtml(option === 'all' ? '全部' : option)}</option>`).join('')}
                </select>
              </label>
            `
        : ''}
...
function renderTabBar(current, options, action) {
    return `
...
function renderKeyValueGrid(items, columns = 3) {
    return `
...
function renderSectionCard(title, body, subtitle) {
    return `
...
function renderDialog(open, title, body, closeAction, submitAction, submitLabel) {
    if (!open)
        return '';
    return `
...
function serializePlateTemplateLinks(links) {
...
function parsePlateTemplateLinks(value) {
...
function ensureRevisionDetailDraft(task) {
    if (!task)
        return initialRevisionDetailDraft();
...
function ensurePlateDetailDraft(task) {
    if (!task)
        return initialPlateDetailDraft();
...
function ensurePatternDetailDraft(task) {
    if (!task) {
...
function renderTaskSaveBar(action, taskId, label = '保存任务') {
    return `
...
function renderTextInput(label, field, value, placeholder = '') {
    return `
    <label class="flex flex-col gap-2 text-sm text-slate-600">
...
function renderSelectInput(label, field, value, options) {
    return `
    <label class="flex flex-col gap-2 text-sm text-slate-600">
      <span>${escapeHtml(label)}</span>
...
function buildRevisionOwnerOptions() {
    const ownerNames = new Set();
    listProjects().forEach((project) => {
        if (project.ownerName)
            ownerNames.add(project.ownerName);
    });
    listRevisionTasks().forEach((task) => {
        if (task.ownerName)
            ownerNames.add(task.ownerName);
    });
...
function renderDateTimeInput(label, field, value) {
    return `
    <label class="flex flex-col gap-2 text-sm text-slate-600">
      <span>${escapeHtml(label)}</span>
...
function renderImageList(imageIds, emptyText = '暂无图片') {
...
function renderSmallImage(imageId) {
    if (!imageId)
        return '<span class="text-slate-400">未上传</span>';
...
function renderImageUploader(label, field, imageIds, emptyText = '暂无图片') {
    return `
    <div class="space-y-3">
      <div class="flex flex-wrap items-center justify-between gap-3">
...
function extractFileLabel(fileId) {
    if (!fileId)
        return '';
...
function renderFileUploader(label, field, fileIds, emptyText = '未上传', accept = '') {
    return `
    <div class="space-y-3">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <p class="text-sm font-medium text-slate-900">${escapeHtml(label)}</p>
...
function buildMockFileId(file) {
    return `mock-file://${Date.now()}-${encodeURIComponent(file.name)}`;
}
function appendImageValues(field, values) {
...
function appendFileValues(field, values) {
...
function removeListValue(scope, index) {
    if (index < 0)
        return false;
...
function updateRevisionMaterialLine(index, key, value) {
...
function updatePlateMaterialLine(index, key, value) {
...
function updatePlatePatternImageLine(index, key, value) {
...
function newRevisionMaterialLine(prefix) {
    return {
...
function newPlateMaterialLine(prefix) {
    return {
        lineId: `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
        materialImageId: '',
        materialName: '',
...
function newPlatePatternImageLine(prefix) {
    return {
        lineId: `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
        imageId: '',
        materialPartName: '',
...
function renderImageThumbnailGrid(imageUrls, removable = false) {
    if (!imageUrls.length)
        return '';
    return `
...
function renderRevisionEvidenceUploader(imageUrls) {
    return `
...
function getProjectDefaultValues(projectId) {
...
function getRevisionTaskStyle(task) {
    if (task.styleId) {
        return {
            styleId: task.styleId,
...
function getRevisionDownstreamTasks(task) {
    return listPatternTasks().filter((item) => item.upstreamObjectId === task.revisionTaskId || item.upstreamObjectCode === task.revisionTaskCode);
}
function getRevisionDownstreamFlag(task) {
...
function getRevisionScopeText(scopeCodes, scopeNames) {
    return scopeNames.join('、') || scopeCodes.join('、') || '-';
}
function canCreateRevisionPatternTask(scopeCodes, sourceType, projectId) {
    return scopeCodes.includes('PRINT') && sourceType === '测款触发' && Boolean(projectId);
}
function renderRevisionMaterialLineEditor(lines) {
    return `
    <div class="space-y-3">
      <div class="flex items-center justify-between gap-3">
...
function renderPlateMaterialLineEditor(lines) {
    return `
    <div class="space-y-3">
      <div class="flex items-center justify-between gap-3">
...
function renderPlatePatternImageLineEditor(lines) {
    return `
    <div class="space-y-3">
      <div class="flex items-center justify-between gap-3">
...
function renderLineInput(key, index, module, label, value) {
    return `
    <label class="flex flex-col gap-2 text-sm text-slate-600">
      <span>${escapeHtml(label)}</span>
...
function buildPatternPreviewDataUrl(taskName) {
    const svg = `
...
function createPatternAssetFromTask(taskId) {
    const task = getPatternTaskById(taskId);
    if (!task)
        return { ok: false, message: '未找到花型任务。' };
...
function renderEmptyDetail(title, listPath) {
    return `
...
function renderHeaderMeta(title, subtitle, badges, actions) {
    return `
    <section class="rounded-xl border bg-white px-4 py-4 shadow-sm">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
...
function renderDataTable(headers, rows, emptyText, footer = '') {
    return `
...
function renderProjectContext(task) {
    ...
    return renderSectionCard('项目与来源', renderKeyValueGrid([
...
function renderRevisionContext(task) {
    if (!task)
        return '';
    ...
    const referenceObjectText = task.referenceObjectId
        ...
        : '—';
    return renderSectionCard('来源与关联', renderKeyValueGrid([
        { label: '来源类型', value: escapeHtml(task.sourceType) },
...
function getRevisionTasksFiltered() {
    ...
    return tasks.filter((task) => {
        ...
        if (state.revisionList.status !== 'all' && task.status !== state.revisionList.status)
            return false;
        if (state.revisionList.owner !== 'all' && task.ownerName !== state.revisionList.owner)
            return false;
        if (state.revisionList.source !== 'all' && task.sourceType !== state.revisionList.source)
            return false;
        if (state.revisionList.quickFilter === 'mine' && task.ownerName !== '李版师')
            return false;
        if (state.revisionList.quickFilter === 'pending-review' && task.status !== '待确认')
            return false;
        if (state.revisionList.quickFilter === 'confirmed-no-output' && !(task.projectId && task.status === '已确认' && !task.linkedTechPackVersionId))
            return false;
        if (state.revisionList.quickFilter === 'blocked' && task.status !== '异常待处理')
            return false;
        if (state.revisionList.quickFilter === 'overdue' && !isOverdue(task.dueAt, task.status === '已完成' || task.status === '已取消'))
            return false;
        return true;
    });
}
function renderRevisionListPage() {
    const tasks = listRevisionTasks();
...
function renderRevisionIssues(task) {
    if (!task)
        return '';
    return renderSectionCard('问题点与证据', `
...
function renderRevisionMaterialRows(task) {
    const lines = task?.materialAdjustmentLines || [];
    if (lines.length === 0)
        return '<div class="rounded-lg border border-dashed border-slate-200 px-4 py-8 text-center text-sm text-slate-500">暂无面辅料变化</div>';
    return `
    <div class="overflow-x-auto">
      <table class="min-w-full divide-y divide-slate-200 text-sm">
...
function renderRevisionDownstream(task) {
    if (!task)
        return '';
    ...
    const emptyText = !task.projectId
        ...
        : !task.revisionScopeCodes.includes('PRINT')
            ? '未涉及花型'
            : '暂无下游任务';
    return renderSectionCard('下游任务', rows.length > 0
        ? `
          <div class="space-y-3">
            ${rows.map((row) => `
...
function renderRevisionDetailPage(revisionTaskId) {
    syncExistingProjectEngineeringTaskNodes('系统同步');
    const task = getRevisionTaskById(revisionTaskId);
    if (!task)
        return renderEmptyDetail('改版任务', '/pcs/patterns/revision');
    ...
    const header = renderHeaderMeta(`${task.revisionTaskCode} · ${task.title}`, subtitleParts.join(' · '), `${renderStatusBadge(task.status)}<span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">${escapeHtml(task.priorityLevel)}优先</span>`, actions);
    ...
    const styleCompare = renderSectionCard('旧款 / 新款对比', `
      ${renderKeyValueGrid([
    ...
    `);
    const revisionPlan = renderSectionCard('改版说明', `
      ${renderKeyValueGrid([
    ...
    `);
    const patternAndDrafts = renderSectionCard('纸样与设计稿', `
      ${renderKeyValueGrid([
    ...
    `);
    const liveRetest = renderSectionCard('回直播验证', `
      ${renderKeyValueGrid([
    ...
    `);
    const samples = renderSectionCard('关联样衣', relatedSamples.length > 0
        ? `
          <div class="space-y-3">
    ...
    const outputs = renderSectionCard('产出物', `
      <div class="grid gap-4 md:grid-cols-2">
        <div class="rounded-lg border border-slate-200 p-4">
    ...
    const aside = `
    <div class="space-y-4">
      ${renderSectionCard('关键摘要', renderKeyValueGrid([
...
function renderRevisionCreateDialog() {
...
function getPlateTasksFiltered() {
    ...
    return tasks.filter((task) => {
        ...
        if (state.plateList.status !== 'all' && task.status !== state.plateList.status)
            return false;
        if (state.plateList.owner !== 'all' && task.ownerName !== state.plateList.owner)
            return false;
        if (state.plateList.source !== 'all' && task.sourceType !== state.plateList.source)
            return false;
        if (state.plateList.quickFilter === 'mine' && task.ownerName !== '王版师')
            return false;
        if (state.plateList.quickFilter === 'pending-review' && task.status !== '待确认')
            return false;
        if (state.plateList.quickFilter === 'confirmed-no-output' && !(task.status === '已确认' && !task.linkedTechPackVersionId))
            return false;
        if (state.plateList.quickFilter === 'blocked' && task.status !== '异常待处理')
            return false;
        if (state.plateList.quickFilter === 'overdue' && !isOverdue(task.dueAt, task.status === '已完成' || task.status === '已取消'))
            return false;
        return true;
    });
}
function renderPlateMaterialRows(task) {
    const lines = task?.materialRequirementLines || [];
    if (lines.length === 0)
        return '<div class="rounded-lg border border-dashed border-slate-200 px-4 py-8 text-center text-sm text-slate-500">暂无面辅料明细</div>';
    return `
    <div class="overflow-x-auto">
      <table class="min-w-full divide-y divide-slate-200 text-sm">
        <thead class="bg-slate-50"><tr>${['面辅料', 'SKU', '印花要求', '数量', '单价', '金额', '备注'].map((item) => `<th class="px-3 py-2 text-left font-medium text-slate-500">${item}</th>`).join('')}</tr></thead>
...
function renderPlatePatternImageRows(task) {
    const lines = task?.patternImageLineItems || [];
    if (lines.length === 0)
        return '<div class="rounded-lg border border-dashed border-slate-200 px-4 py-8 text-center text-sm text-slate-500">暂无唛架图片</div>';
    return `
    <div class="space-y-3">
      ${lines.map((line) => `
...
function renderFileChips(title, fileIds) {
    return `
    <div class="rounded-lg border border-slate-200 p-4">
...
function renderPlateTemplateRows(task) {
    const links = task?.partTemplateLinks || [];
    if (links.length === 0)
        return '<div class="rounded-lg border border-dashed border-slate-200 px-4 py-8 text-center text-sm text-slate-500">暂无模板关联</div>';
    return `
    <div class="space-y-3">
      ${links.map((link) => `
        <div class="rounded-lg border border-slate-200 px-4 py-3">
...
function renderPlateCreateDialog() {
...
function renderPlateDetailPage(plateTaskId) {
    syncExistingProjectEngineeringTaskNodes('系统同步');
    const task = getPlateMakingTaskById(plateTaskId);
    if (!task)
        return renderEmptyDetail('制版任务', '/pcs/patterns');
    ...
    const header = renderHeaderMeta(`${task.plateTaskCode} · ${task.title}`, `${task.projectCode} · ${task.projectName} · ${formatDateTime(task.updatedAt)}`, `${renderStatusBadge(task.status)}<span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">${escapeHtml(task.patternVersion || '待定版次')}</span>`, [
    ...
    const downstream = renderSectionCard('下游打样', downstreamFirst.length + downstreamPre.length > 0
        ? `
          <div class="space-y-3">
            ${[
...
function getPatternTasksFiltered() {
    ...
    return tasks.filter((task) => {
        ...
        if (state.patternList.status !== 'all' && task.status !== state.patternList.status)
            return false;
        if (state.patternList.owner !== 'all' && task.ownerName !== state.patternList.owner)
            return false;
        if (state.patternList.source !== 'all' && task.sourceType !== state.patternList.source)
            return false;
        if (state.patternList.quickFilter === 'mine' && task.ownerName !== '林小美')
            return false;
        if (state.patternList.quickFilter === 'pending-review' && task.buyerReviewStatus !== '待买手确认')
            return false;
        if (state.patternList.quickFilter === 'confirmed-no-output' && !(task.status === '已确认' && !task.linkedTechPackVersionId))
            return false;
        if (state.patternList.quickFilter === 'blocked' && task.status !== '异常待处理')
            return false;
        if (state.patternList.quickFilter === 'overdue' && !isOverdue(task.dueAt, task.status === '已完成' || task.status === '已取消'))
            return false;
        return true;
    });
}
function renderPatternListPage() {
    const tasks = listPatternTasks();
...
function renderPatternCreateDialog() {
...
function renderPatternDetailPage(patternTaskId) {
    syncExistingProjectEngineeringTaskNodes('系统同步');
    const task = getPatternTaskById(patternTaskId);
    if (!task)
        return renderEmptyDetail('花型任务', '/pcs/patterns/colors');
    const techPackAction = getPatternTechPackActionMeta(task.patternTaskId);
    ...
    const header = renderHeaderMeta(`${task.patternTaskCode} · ${task.title}`, `${task.projectCode} · ${task.projectName} · ${formatDateTime(task.updatedAt)}`, `${renderStatusBadge(task.status)}<span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">${escapeHtml(task.artworkVersion || '待确认版本')}</span>`, [
    ...
    const assignmentSection = renderSectionCard('团队与成员分配', `
      ${renderKeyValueGrid([
    ...
    `);
    const colorReviewSection = renderSectionCard('难易程度与颜色确认', `
      <div class="grid gap-4 md:grid-cols-2">
    ...
    `);
    const buyerReviewSection = renderSectionCard('买手确认', `
      ${renderKeyValueGrid([
    ...
    `);
    const completionReviewSection = renderSectionCard('完成确认', `
      <div class="grid gap-4 md:grid-cols-2">
    ...
    `);
    const librarySection = renderSectionCard('花型库沉淀', `
      ${asset
        ? renderKeyValueGrid([
...
function buildSampleMilestones(task) {
...
function renderSamplePlanLines(lines) {
...
function renderFirstOrderPlanCellSelect(scope, index, key, value, options) {
    return `
...
function renderFirstOrderPlanCellInput(scope, index, key, value, placeholder = '', type = 'text') {
    return `
...
function renderFirstOrderReferenceFactorySelect(scope, index, line) {
...
function renderFirstOrderSamplePlanEditTable(scope, lines) {
...
function getFirstSampleActionLabel(status) {
    if (status === '草稿' || status === '待处理')
        return '开始打样';
    if (status === '打样中')
        return '提交结果';
    if (status === '待确认')
...
function getFirstOrderActionLabel(status) {
    if (status === '草稿' || status === '待处理')
        return '开始首单';
    if (status === '打样中')
        return '提交首单结果';
...
function getFirstSampleTasksFiltered() {
    ...
    return tasks.filter((task) => {
        ...
        if (state.firstSampleList.status !== 'all' && task.status !== state.firstSampleList.status)
            return false;
        if (state.firstSampleList.owner !== 'all' && task.ownerName !== state.firstSampleList.owner)
            return false;
        if (state.firstSampleList.source !== 'all' && task.sourceType !== state.firstSampleList.source)
            return false;
        if (state.firstSampleList.site !== 'all' && task.targetSite !== state.firstSampleList.site)
            return false;
        if (state.firstSampleList.quickFilter === 'sampling' && task.status !== '打样中')
            return false;
        if (state.firstSampleList.quickFilter === 'confirming' && task.status !== '待确认')
            return false;
        if (state.firstSampleList.quickFilter === 'rework' && task.status !== '需改版')
            return false;
        if (state.firstSampleList.quickFilter === 'passed' && task.status !== '已通过')
            return false;
        return true;
    });
}
function renderFirstSampleRevisionCell(task) {
    if (task.status !== '需改版')
        return '<span class="text-slate-400">-</span>';
    const revision = findRevisionTaskByFirstSample(task);
    if (!revision)
        return '<span class="text-xs font-medium text-orange-700">未创建改版任务</span>';
    return `
    <div class="space-y-1">
...
function renderFirstSampleRevisionAction(task) {
    if (task.status !== '需改版')
        return '';
    const revision = findRevisionTaskByFirstSample(task);
    if (revision) {
...
function renderFirstSampleListPage() {
    const tasks = listFirstSampleTasks();
...
function renderFirstSampleCreateDialog() {
...
function renderFirstSampleStartDialog() {
...
function renderFirstSampleResultDialog() {
...
function renderFirstSampleAcceptanceDialog() {
...
function getFirstSampleDetailDraft(task) {
...
function parseSampleImageIdsText(value) {
    return value
        .split(/[\n,，、]/)
        .map((item) => item.trim())
        .filter(Boolean);
}
function saveFirstSampleDetail(taskId) {
...
function getFirstOrderDetailDraft(task) {
...
function saveFirstOrderDetail(taskId) {
...
function renderFirstSampleReworkCard(task) {
    if (task.status !== '需改版')
        return '';
    const revision = findRevisionTaskByFirstSample(task);
    const reworkReason = [
...
function renderFirstSampleDetailPage(firstSampleTaskId) {
    const task = getFirstSampleTaskById(firstSampleTaskId);
    if (!task)
        return renderEmptyDetail('首版样衣打样', '/pcs/samples/first-sample');
    getFirstSampleDetailDraft(task);
    const acceptance = firstSampleAcceptanceMap.get(task.firstSampleTaskId) ||
    ...
    const header = renderHeaderMeta(`${task.firstSampleTaskCode} · ${task.title}`, `${task.projectCode} · ${task.projectName} · ${formatDateTime(task.updatedAt)}`, `${renderStatusBadge(task.status, true)}<span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">${escapeHtml(task.targetSite)}</span>`, [
    ...
    const resultSection = renderSectionCard('打样结果', `
      <div class="space-y-4">
        ${renderKeyValueGrid([
    ...
    `);
    const acceptanceSection = renderSectionCard('验收与结论', `
      <div class="space-y-4">
        <div class="grid gap-4 md:grid-cols-2">
          <div class="rounded-lg border border-slate-200 p-4">
...
function getFirstOrderTasksFiltered() {
    ...
    return tasks.filter((task) => {
        ...
        if (state.firstOrderList.status !== 'all' && task.status !== state.firstOrderList.status)
            return false;
        if (state.firstOrderList.owner !== 'all' && task.ownerName !== state.firstOrderList.owner)
            return false;
        if (state.firstOrderList.source !== 'all' && task.sourceType !== state.firstOrderList.source)
            return false;
        if (state.firstOrderList.site !== 'all' && task.targetSite !== state.firstOrderList.site)
            return false;
        if (state.firstOrderList.quickFilter === 'sampling' && task.status !== '打样中')
            return false;
        if (state.firstOrderList.quickFilter === 'confirming' && task.status !== '待确认')
            return false;
        if (state.firstOrderList.quickFilter === 'rework' && task.status !== '需改版')
            return false;
        if (state.firstOrderList.quickFilter === 'passed' && task.status !== '已通过')
            return false;
        return true;
    });
}
function renderFirstOrderListPage() {
    const tasks = listFirstOrderSampleTasks();
...
function renderFirstOrderCreateDialog() {
...
function renderFirstOrderStartDialog() {
...
function renderFirstOrderResultDialog() {
...
function renderFirstOrderConclusionDialog() {
    const body = `
    <div class="space-y-4">
      <div class="grid gap-4 md:grid-cols-2">
...
function renderFirstOrderDetailPage(firstOrderSampleTaskId) {
    const task = getFirstOrderSampleTaskById(firstOrderSampleTaskId);
    if (!task)
        return renderEmptyDetail('首单样衣打样', '/pcs/samples/first-order');
    if (state.firstOrderDetailDraftTaskId !== task.firstOrderSampleTaskId) {
        state.firstOrderDetailDraftTaskId = task.firstOrderSampleTaskId;
        state.firstOrderDetailDraft = buildFirstOrderDetailDraft(task);
    }
    ...
    const header = renderHeaderMeta(`${task.firstOrderSampleTaskCode} · ${task.title}`, `${task.projectCode} · ${task.projectName} · ${formatDateTime(task.updatedAt)}`, `${renderStatusBadge(task.status, true)}${conclusion ? `<span class="rounded-full bg-emerald-50 px-2.5 py-1 text-xs text-emerald-700">${escapeHtml(conclusion.result)}</span>` : ''}`, [
    ...
    const conclusionSection = renderSectionCard('首单确认', `
...
function updateListPage(listState, step, total) {
    const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));
    listState.currentPage = Math.min(totalPages, Math.max(1, listState.currentPage + step));
}
function findRevisionTaskByFirstSample(task) {
    return listRevisionTasks().find((revision) => revision.projectId === task.projectId &&
        (revision.upstreamObjectId === task.firstSampleTaskId ||
            revision.upstreamObjectCode === task.firstSampleTaskCode));
}
function buildRevisionDraftFromFirstSampleRework(task, operatorName, fitSummary, artworkSummary) {
...
function openRevisionCreateFromFirstSampleRework(task, operatorName) {
    ...
    state.revisionCreateDraft = buildRevisionDraftFromFirstSampleRework(task, operatorName, task.fitConfirmationSummary || task.productionReadinessNote || task.note || '首版样衣验收需改版。', task.artworkConfirmationSummary);
    setNotice('已根据首版样衣需改版结论预填改版任务，请确认后创建。');
}
function advanceFirstSampleTask(taskId) {
    const task = getFirstSampleTaskById(taskId);
    if (!task)
        return;
...
function advanceFirstOrderTask(taskId) {
    const task = getFirstOrderSampleTaskById(taskId);
    if (!task)
        return;
    if (task.status === '草稿' || task.status === '待处理') {
...
function submitFirstSampleCreate() {
    const draft = state.firstSampleCreateDraft;
    if (!draft.projectId) {
        setNotice('请先选择商品项目。');
        return;
...
function submitFirstSampleStart() {
...
function submitFirstSampleResult() {
...
function submitFirstOrderCreate() {
    const draft = state.firstOrderCreateDraft;
    if (!draft.projectId) {
        setNotice('请先选择商品项目。');
        return;
...
function submitFirstOrderStart() {
...
function submitFirstOrderResult() {
...
function generateRevisionTechPack(taskId) {
...
function generatePlateTechPack(taskId) {
...
function generatePatternTechPack(taskId) {
    const task = getPatternTaskById(taskId);
    if (!task)
        return;
    try {
        ...
        setNotice(result.logType === '花型写入技术包'
...
export function renderPcsRevisionTaskPage() {
    syncExistingProjectEngineeringTaskNodes('系统同步');
    return renderRevisionListPage();
}
export function renderPcsRevisionTaskDetailPage(revisionTaskId) {
    return renderRevisionDetailPage(revisionTaskId);
}
export function renderPcsPlateMakingTaskPage() {
    syncExistingProjectEngineeringTaskNodes('系统同步');
    return renderPlateListPage();
}
export function renderPcsPlateMakingTaskDetailPage(plateTaskId) {
    return renderPlateDetailPage(plateTaskId);
}
export function renderPcsPatternTaskPage() {
    syncExistingProjectEngineeringTaskNodes('系统同步');
    return renderPatternListPage();
}
export function renderPcsPatternTaskDetailPage(patternTaskId) {
    return renderPatternDetailPage(patternTaskId);
}
export function renderPcsFirstSampleTaskPage() {
    return renderFirstSampleListPage();
}
export function renderPcsFirstSampleTaskDetailPage(firstSampleTaskId) {
    return renderFirstSampleDetailPage(firstSampleTaskId);
}
export function renderPcsFirstOrderSampleTaskPage() {
    return renderFirstOrderListPage();
}
export function renderPcsFirstOrderSampleTaskDetailPage(firstOrderSampleTaskId) {
    return renderFirstOrderDetailPage(firstOrderSampleTaskId);
}
export function handlePcsEngineeringTaskInput(target) {
    const fieldNode = target.closest('[data-pcs-engineering-field]');
    if (!fieldNode)
        return false;
    ...
    if (fieldNode instanceof HTMLInputElement && fieldNode.type === 'file') {
        ...
        if (field === 'revision-material-line-image' && lineIndex >= 0) {
            ...
            if (imageUrl)
                state.revisionDetailDraft.materialAdjustmentLines[lineIndex].materialImageId = URL.createObjectURL(imageUrl);
            fieldNode.value = '';
            return true;
        }
        if (field === 'plate-material-line-image' && lineIndex >= 0) {
            const imageUrl = files.find((file) => file.type.startsWith('image/'));
            if (imageUrl)
                state.plateDetailDraft.materialRequirementLines[lineIndex].materialImageId = URL.createObjectURL(imageUrl);
            fieldNode.value = '';
            return true;
        }
        if (field === 'plate-pattern-line-image' && lineIndex >= 0) {
            const imageUrl = files.find((file) => file.type.startsWith('image/'));
            if (imageUrl)
                state.plateDetailDraft.patternImageLineItems[lineIndex].imageId = URL.createObjectURL(imageUrl);
            fieldNode.value = '';
            return true;
        }
    ...
    if (fieldNode instanceof HTMLInputElement
        ...
        if (field === 'first-order-create-plan-line' && index >= 0 && key) {
            state.firstOrderCreateDraft.samplePlanLinesText = updateFirstOrderSamplePlanLineText(state.firstOrderCreateDraft.samplePlanLinesText, createDefaultSamplePlanLines(state.firstOrderCreateDraft.sampleChainMode), index, key, fieldNode.value);
            return true;
        }
        if (field === 'first-order-result-plan-line' && index >= 0 && key) {
            state.firstOrderResultDraft.samplePlanLinesText = updateFirstOrderSamplePlanLineText(state.firstOrderResultDraft.samplePlanLinesText, [], index, key, fieldNode.value);
            return true;
        }
    }
    if (field === 'revision-search' && fieldNode instanceof HTMLInputElement) {
        state.revisionList.search = fieldNode.value;
        state.revisionList.currentPage = 1;
        return true;
    }
    if (field === 'revision-status' && fieldNode instanceof HTMLSelectElement) {
        state.revisionList.status = fieldNode.value;
        state.revisionList.currentPage = 1;
        return true;
    }
    if (field === 'revision-owner' && fieldNode instanceof HTMLSelectElement) {
        state.revisionList.owner = fieldNode.value;
        state.revisionList.currentPage = 1;
        return true;
    }
    if (field === 'revision-source' && fieldNode instanceof HTMLSelectElement) {
        state.revisionList.source = fieldNode.value;
        state.revisionList.currentPage = 1;
        return true;
    }
    if (field === 'plate-search' && fieldNode instanceof HTMLInputElement) {
        state.plateList.search = fieldNode.value;
        state.plateList.currentPage = 1;
        return true;
    }
    if (field === 'plate-status' && fieldNode instanceof HTMLSelectElement) {
        state.plateList.status = fieldNode.value;
        state.plateList.currentPage = 1;
        return true;
    }
    if (field === 'plate-owner' && fieldNode instanceof HTMLSelectElement) {
        state.plateList.owner = fieldNode.value;
        state.plateList.currentPage = 1;
        return true;
    }
    if (field === 'plate-source' && fieldNode instanceof HTMLSelectElement) {
        state.plateList.source = fieldNode.value;
        state.plateList.currentPage = 1;
        return true;
    }
    if (field === 'pattern-search' && fieldNode instanceof HTMLInputElement) {
        state.patternList.search = fieldNode.value;
        state.patternList.currentPage = 1;
        return true;
    }
    if (field === 'pattern-status' && fieldNode instanceof HTMLSelectElement) {
        state.patternList.status = fieldNode.value;
        state.patternList.currentPage = 1;
        return true;
    }
    if (field === 'pattern-owner' && fieldNode instanceof HTMLSelectElement) {
        state.patternList.owner = fieldNode.value;
        state.patternList.currentPage = 1;
        return true;
    }
    if (field === 'pattern-source' && fieldNode instanceof HTMLSelectElement) {
        state.patternList.source = fieldNode.value;
        state.patternList.currentPage = 1;
        return true;
    }
    if (field === 'first-sample-search' && fieldNode instanceof HTMLInputElement) {
        state.firstSampleList.search = fieldNode.value;
        state.firstSampleList.currentPage = 1;
        return true;
    }
    if (field === 'first-sample-status' && fieldNode instanceof HTMLSelectElement) {
        state.firstSampleList.status = fieldNode.value;
        state.firstSampleList.currentPage = 1;
        return true;
    }
    if (field === 'first-sample-owner' && fieldNode instanceof HTMLSelectElement) {
        state.firstSampleList.owner = fieldNode.value;
        state.firstSampleList.currentPage = 1;
        return true;
    }
    if (field === 'first-sample-source' && fieldNode instanceof HTMLSelectElement) {
        state.firstSampleList.source = fieldNode.value;
        state.firstSampleList.currentPage = 1;
        return true;
    }
    if (field === 'first-sample-site' && fieldNode instanceof HTMLSelectElement) {
        state.firstSampleList.site = fieldNode.value;
        state.firstSampleList.currentPage = 1;
        return true;
    }
    if (field === 'first-order-search' && fieldNode instanceof HTMLInputElement) {
        state.firstOrderList.search = fieldNode.value;
        state.firstOrderList.currentPage = 1;
        return true;
    }
    if (field === 'first-order-status' && fieldNode instanceof HTMLSelectElement) {
        state.firstOrderList.status = fieldNode.value;
        state.firstOrderList.currentPage = 1;
        return true;
    }
    if (field === 'first-order-owner' && fieldNode instanceof HTMLSelectElement) {
        state.firstOrderList.owner = fieldNode.value;
        state.firstOrderList.currentPage = 1;
        return true;
    }
    if (field === 'first-order-source' && fieldNode instanceof HTMLSelectElement) {
        state.firstOrderList.source = fieldNode.value;
        state.firstOrderList.currentPage = 1;
        return true;
    }
    if (field === 'first-order-site' && fieldNode instanceof HTMLSelectElement) {
        state.firstOrderList.site = fieldNode.value;
        state.firstOrderList.currentPage = 1;
        return true;
    }
    if (fieldNode instanceof HTMLInputElement || fieldNode instanceof HTMLTextAreaElement || fieldNode instanceof HTMLSelectElement) {
        ...
        switch (field) {
            case 'revision-create-binding-mode':
                state.revisionCreateDraft.bindingMode = value;
            ...
            case 'revision-create-source-type':
                state.revisionCreateDraft.sourceType = value;
                state.revisionCreateDraft.projectId = '';
                state.revisionCreateDraft.styleId = '';
            ...
            case 'revision-create-project': {
                ...
                state.revisionCreateDraft.createPatternTask = canCreateRevisionPatternTask(state.revisionCreateDraft.scopeCodes, state.revisionCreateDraft.sourceType, value);
                return true;
            }
            ...
            case 'revision-create-owner':
                state.revisionCreateDraft.ownerName = value;
                return true;
            case 'revision-create-title':
                state.revisionCreateDraft.title = value;
                return true;
            case 'revision-create-due-at':
                state.revisionCreateDraft.dueAt = fromDateTimeLocalValue(value);
                return true;
            case 'revision-create-issue-summary':
                state.revisionCreateDraft.issueSummary = value;
                return true;
            case 'revision-create-evidence-summary':
                state.revisionCreateDraft.evidenceSummary = value;
                return true;
            case 'revision-create-note':
                state.revisionCreateDraft.note = value;
                return true;
            case 'revision-detail-participants':
                state.revisionDetailDraft.participantNamesText = value;
                return true;
            case 'revision-detail-version':
                state.revisionDetailDraft.revisionVersion = value;
                return true;
            case 'revision-detail-base-style-code':
                state.revisionDetailDraft.baseStyleCode = value;
                return true;
            case 'revision-detail-base-style-name':
                state.revisionDetailDraft.baseStyleName = value;
                return true;
            case 'revision-detail-target-style-code':
                state.revisionDetailDraft.targetStyleCodeCandidate = value;
                return true;
            case 'revision-detail-target-style-name':
                state.revisionDetailDraft.targetStyleNameCandidate = value;
                return true;
            case 'revision-detail-sample-qty':
                state.revisionDetailDraft.sampleQty = value;
                return true;
            case 'revision-detail-style-preference':
                state.revisionDetailDraft.stylePreference = value;
                return true;
            case 'revision-detail-pattern-maker-name':
                state.revisionDetailDraft.patternMakerName = value;
                return true;
            case 'revision-detail-suggestion':
                state.revisionDetailDraft.revisionSuggestionRichText = value;
                return true;
            case 'revision-detail-paper-print-at':
                state.revisionDetailDraft.paperPrintAt = fromDateTimeLocalValue(value);
                return true;
            case 'revision-detail-delivery-address':
                state.revisionDetailDraft.deliveryAddress = value;
                return true;
            case 'revision-detail-pattern-area':
                state.revisionDetailDraft.patternArea = value;
                return true;
            case 'revision-detail-new-pattern-spu':
                state.revisionDetailDraft.newPatternSpuCode = value;
                return true;
            case 'revision-detail-pattern-change-note':
                state.revisionDetailDraft.patternChangeNote = value;
                return true;
            case 'revision-detail-live-retest-status':
                state.revisionDetailDraft.liveRetestStatus = value;
                return true;
            case 'revision-detail-live-retest-relations':
                state.revisionDetailDraft.liveRetestRelationIdsText = value;
                return true;
            case 'revision-detail-live-retest-summary':
                state.revisionDetailDraft.liveRetestSummary = value;
                return true;
            case 'plate-create-binding-mode':
                state.plateCreateDraft.bindingMode = value;
            ...
            case 'plate-create-source-type':
                state.plateCreateDraft.sourceType = value;
                return true;
            ...
            case 'plate-create-owner':
                state.plateCreateDraft.ownerName = value;
                return true;
            case 'plate-create-pattern-maker':
                state.plateCreateDraft.patternMakerName = value;
                return true;
            case 'plate-create-title':
                state.plateCreateDraft.title = value;
                return true;
            case 'plate-create-due-at':
                state.plateCreateDraft.dueAt = fromDateTimeLocalValue(value);
                return true;
            case 'plate-create-style-code':
                state.plateCreateDraft.productStyleCode = value;
                return true;
            case 'plate-create-product-history-type':
                state.plateCreateDraft.productHistoryType = value;
                return true;
            case 'plate-create-pattern-area':
                state.plateCreateDraft.patternArea = value;
                return true;
            case 'plate-create-pattern-type':
                state.plateCreateDraft.patternType = value;
                return true;
            case 'plate-create-size-range':
                state.plateCreateDraft.sizeRange = value;
                return true;
            case 'plate-create-note':
                state.plateCreateDraft.note = value;
                return true;
            case 'plate-detail-participants':
                state.plateDetailDraft.participantNamesText = value;
                return true;
            case 'plate-detail-version':
                state.plateDetailDraft.patternVersion = value;
                return true;
            case 'plate-detail-product-history-type':
                state.plateDetailDraft.productHistoryType = value;
                return true;
            case 'plate-detail-pattern-maker':
                state.plateDetailDraft.patternMakerName = value;
                return true;
            case 'plate-detail-sample-confirmed-at':
                state.plateDetailDraft.sampleConfirmedAt = fromDateTimeLocalValue(value);
                return true;
            case 'plate-detail-pattern-area':
                state.plateDetailDraft.patternArea = value;
                return true;
            case 'plate-detail-color-requirement':
                state.plateDetailDraft.colorRequirementText = value;
                return true;
            case 'plate-detail-new-pattern-spu':
                state.plateDetailDraft.newPatternSpuCode = value;
                return true;
            case 'plate-detail-template-links':
                state.plateDetailDraft.partTemplateLinksText = value;
                return true;
            case 'pattern-create-binding-mode':
                state.patternCreateDraft.bindingMode = value;
            ...
            case 'pattern-create-source-type':
                state.patternCreateDraft.sourceType = value;
                return true;
            ...
            case 'pattern-create-style-id':
                ...
                return true;
            case 'pattern-create-owner':
                state.patternCreateDraft.ownerName = value;
                return true;
            case 'pattern-create-title':
                state.patternCreateDraft.title = value;
                return true;
            case 'pattern-create-due-at':
                state.patternCreateDraft.dueAt = fromDateTimeLocalValue(value);
                return true;
            case 'pattern-create-style-code':
                state.patternCreateDraft.productStyleCode = value;
                return true;
            case 'pattern-create-demand-source':
                state.patternCreateDraft.demandSourceType = value;
                return true;
            case 'pattern-create-process-type': {
                state.patternCreateDraft.processType = value;
                state.patternCreateDraft.artworkType = value === '烫画' ? '烫画' : '印花';
                state.patternCreateDraft.patternMode = value;
                return true;
            }
            case 'pattern-create-request-qty':
                state.patternCreateDraft.requestQty = value;
                return true;
            case 'pattern-create-fabric-sku':
                state.patternCreateDraft.fabricSku = value;
                return true;
            case 'pattern-create-fabric-name':
                state.patternCreateDraft.fabricName = value;
                return true;
            case 'pattern-create-team': {
                const nextTeamCode = value;
            ...
            case 'pattern-create-member':
                state.patternCreateDraft.assignedMemberId = value;
                return true;
            case 'pattern-create-artwork-name':
                state.patternCreateDraft.artworkName = value;
                return true;
            case 'pattern-create-artwork-type':
                state.patternCreateDraft.artworkType = value;
                return true;
            case 'pattern-create-pattern-mode':
                state.patternCreateDraft.patternMode = value;
                return true;
            case 'pattern-create-category':
                state.patternCreateDraft.patternCategoryCode = value;
                return true;
            case 'pattern-create-style-tags':
                state.patternCreateDraft.patternStyleTagsText = value;
                return true;
            case 'pattern-create-note':
                state.patternCreateDraft.note = value;
                return true;
            case 'pattern-detail-version':
                state.patternDetailDraft.artworkVersion = value;
                return true;
            case 'pattern-detail-difficulty':
                state.patternDetailDraft.difficultyGrade = value;
                return true;
            case 'pattern-detail-color-depth':
                state.patternDetailDraft.colorDepthOption = value;
                return true;
            case 'pattern-detail-physical-note':
                state.patternDetailDraft.physicalReferenceNote = value;
                return true;
            case 'pattern-detail-color-note':
                state.patternDetailDraft.colorConfirmNote = value;
                return true;
            case 'pattern-detail-buyer-note':
                state.patternDetailDraft.buyerReviewNote = value;
                return true;
            case 'pattern-detail-transfer-reason':
                state.patternDetailDraft.transferReason = value;
                return true;
            case 'pattern-detail-category':
                state.patternDetailDraft.patternCategoryCode = value;
                return true;
            case 'pattern-detail-style-tags':
                state.patternDetailDraft.patternStyleTagsText = value;
                return true;
            ...
            case 'first-sample-create-owner':
                state.firstSampleCreateDraft.ownerName = value;
                return true;
            case 'first-sample-create-title':
                state.firstSampleCreateDraft.title = value;
                return true;
            case 'first-sample-create-tech-pack-id':
                state.firstSampleCreateDraft.sourceTechPackVersionId = value;
                return true;
            ...
            case 'first-sample-create-factory':
                ...
                return true;
            case 'first-sample-create-site':
                state.firstSampleCreateDraft.targetSite = value;
                return true;
            case 'first-sample-create-material-mode':
                state.firstSampleCreateDraft.sampleMaterialMode = value;
                return true;
            case 'first-sample-create-purpose':
                state.firstSampleCreateDraft.samplePurpose = value;
                return true;
            case 'first-sample-create-note':
                state.firstSampleCreateDraft.note = value;
                return true;
            case 'first-sample-start-operator':
                state.firstSampleStartDraft.operatorName = value;
                return true;
            case 'first-sample-start-at':
                state.firstSampleStartDraft.startedAt = fromDateTimeLocalValue(value);
                return true;
            case 'first-sample-start-plan-finish-at':
                state.firstSampleStartDraft.planFinishAt = fromDateTimeLocalValue(value);
                return true;
            case 'first-sample-start-note':
                state.firstSampleStartDraft.startNote = value;
                return true;
            case 'first-sample-result-sample-code':
                state.firstSampleResultDraft.sampleCode = value;
                return true;
            case 'first-sample-result-submitted-by':
                state.firstSampleResultDraft.submittedBy = value;
                return true;
            case 'first-sample-result-completed-at':
                state.firstSampleResultDraft.completedAt = fromDateTimeLocalValue(value);
                return true;
            case 'first-sample-result-note':
                state.firstSampleResultDraft.resultNote = value;
                return true;
            case 'first-sample-acceptance-confirmed-by':
                state.firstSampleAcceptanceConfirmedBy = value;
                return true;
            case 'first-sample-acceptance-confirmed-at':
                state.firstSampleAcceptanceConfirmedAt = fromDateTimeLocalValue(value);
                return true;
            case 'first-sample-acceptance-result':
                state.firstSampleAcceptanceResult = value;
                return true;
            case 'first-sample-acceptance-note':
                state.firstSampleAcceptanceNote = value;
                return true;
            case 'first-sample-acceptance-artwork-summary':
                state.firstSampleAcceptanceArtworkSummary = value;
                return true;
            case 'first-sample-detail-sample-code':
                state.firstSampleDetailDraft.sampleCode = value;
                return true;
            case 'first-sample-detail-sample-images':
                state.firstSampleDetailDraft.sampleImageIdsText = value;
                return true;
            case 'first-sample-detail-fit-summary':
                state.firstSampleDetailDraft.fitConfirmationSummary = value;
                return true;
            case 'first-sample-detail-artwork-summary':
                state.firstSampleDetailDraft.artworkConfirmationSummary = value;
                return true;
            case 'first-sample-detail-production-note':
                state.firstSampleDetailDraft.productionReadinessNote = value;
                return true;
            case 'first-sample-detail-reuse-note':
                state.firstSampleDetailDraft.reuseAsFirstOrderBasisNote = value;
                return true;
            case 'first-sample-detail-reuse-confirmed-at':
                state.firstSampleDetailDraft.reuseAsFirstOrderBasisConfirmedAt = value;
                return true;
            case 'first-sample-detail-reuse-confirmed-by':
                state.firstSampleDetailDraft.reuseAsFirstOrderBasisConfirmedBy = value;
                return true;
            case 'first-sample-detail-confirmed-at':
                state.firstSampleDetailDraft.confirmedAt = value;
                return true;
            ...
            case 'first-order-create-owner':
                state.firstOrderCreateDraft.ownerName = value;
                return true;
            case 'first-order-create-title':
                state.firstOrderCreateDraft.title = value;
                return true;
            case 'first-order-create-factory':
                ...
                return true;
            case 'first-order-create-site':
                state.firstOrderCreateDraft.targetSite = value;
                return true;
            case 'first-order-create-chain-mode':
                state.firstOrderCreateDraft.sampleChainMode = value;
                ...
                return true;
            case 'first-order-create-pattern-version':
                state.firstOrderCreateDraft.patternVersion = value;
                return true;
            case 'first-order-create-artwork-version':
                state.firstOrderCreateDraft.artworkVersion = value;
                return true;
            case 'first-order-create-final-reference-note':
                state.firstOrderCreateDraft.finalReferenceNote = value;
                return true;
            case 'first-order-create-note':
                state.firstOrderCreateDraft.note = value;
                return true;
            case 'first-order-start-operator':
                state.firstOrderStartDraft.operatorName = value;
                return true;
            case 'first-order-start-at':
                state.firstOrderStartDraft.startedAt = fromDateTimeLocalValue(value);
                return true;
            case 'first-order-start-plan-finish-at':
                state.firstOrderStartDraft.planFinishAt = fromDateTimeLocalValue(value);
                return true;
            case 'first-order-start-factory-note':
                state.firstOrderStartDraft.factoryConfirmNote = value;
                return true;
            case 'first-order-result-sample-code':
                state.firstOrderResultDraft.sampleCode = value;
                return true;
            case 'first-order-result-submitted-by':
                state.firstOrderResultDraft.submittedBy = value;
                return true;
            case 'first-order-result-completed-at':
                state.firstOrderResultDraft.completedAt = fromDateTimeLocalValue(value);
                return true;
            case 'first-order-result-final-reference-note':
                state.firstOrderResultDraft.finalReferenceNote = value;
                return true;
            case 'first-order-result-note':
                state.firstOrderResultDraft.resultNote = value;
                return true;
            case 'first-order-conclusion-result':
                state.firstOrderConclusionResult = value;
                return true;
            case 'first-order-conclusion-note':
                state.firstOrderConclusionNote = value;
                return true;
            case 'first-order-conclusion-confirmed-by':
                state.firstOrderConclusionConfirmedBy = value;
                return true;
            case 'first-order-conclusion-confirmed-at':
                state.firstOrderConclusionConfirmedAt = fromDateTimeLocalValue(value);
                return true;
            case 'first-order-detail-pattern-version':
                state.firstOrderDetailDraft.patternVersion = value;
                return true;
            case 'first-order-detail-artwork-version':
                state.firstOrderDetailDraft.artworkVersion = value;
                return true;
            case 'first-order-detail-sample-code':
                state.firstOrderDetailDraft.sampleCode = value;
                return true;
            case 'first-order-detail-final-reference-note':
                state.firstOrderDetailDraft.finalReferenceNote = value;
                return true;
            case 'first-order-detail-sample-plan-lines':
                state.firstOrderDetailDraft.samplePlanLinesText = value;
                return true;
            case 'first-order-detail-conclusion-result':
                state.firstOrderDetailDraft.conclusionResult = value;
                return true;
            case 'first-order-detail-conclusion-note':
                state.firstOrderDetailDraft.conclusionNote = value;
                return true;
            case 'first-order-detail-confirmed-at':
                state.firstOrderDetailDraft.confirmedAt = fromDateTimeLocalValue(value);
                return true;
            case 'first-order-detail-confirmed-by':
                state.firstOrderDetailDraft.confirmedBy = value;
                return true;
...
export function handlePcsEngineeringTaskEvent(target) {
    const actionNode = target.closest('[data-pcs-engineering-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'open-revision-create') {
        state.revisionCreateOpen = true;
        state.revisionCreateDraft = initialRevisionCreateDraft();
        clearNotice();
        return true;
    }
    if (action === 'close-revision-create') {
        state.revisionCreateOpen = false;
        return true;
    }
    if (action === 'submit-revision-create') {
        submitRevisionCreate();
        return true;
    }
    if (action === 'open-plate-create') {
        state.plateCreateOpen = true;
        state.plateCreateDraft = initialPlateCreateDraft();
        clearNotice();
        return true;
    }
    if (action === 'close-plate-create') {
        state.plateCreateOpen = false;
        return true;
    }
    if (action === 'submit-plate-create') {
        submitPlateCreate();
        return true;
    }
    if (action === 'open-pattern-create') {
        state.patternCreateOpen = true;
        state.patternCreateDraft = initialPatternCreateDraft();
        clearNotice();
        return true;
    }
    if (action === 'close-pattern-create') {
        state.patternCreateOpen = false;
        return true;
    }
    if (action === 'submit-pattern-create') {
        submitPatternCreate();
        return true;
    }
    if (action === 'open-first-sample-create') {
        state.firstSampleCreateOpen = true;
        return true;
    }
    if (action === 'close-first-sample-create') {
        state.firstSampleCreateOpen = false;
        return true;
    }
    if (action === 'submit-first-sample-create') {
        submitFirstSampleCreate();
        return true;
    }
    if (action === 'close-first-sample-start') {
        state.firstSampleStartOpen = false;
        return true;
    }
    if (action === 'submit-first-sample-start') {
        submitFirstSampleStart();
        return true;
    }
    if (action === 'close-first-sample-result') {
        state.firstSampleResultOpen = false;
        return true;
    }
    if (action === 'submit-first-sample-result') {
        submitFirstSampleResult();
        return true;
    }
    if (action === 'open-first-order-create') {
        state.firstOrderCreateOpen = true;
        return true;
    }
    if (action === 'close-first-order-create') {
        state.firstOrderCreateOpen = false;
        return true;
    }
    if (action === 'submit-first-order-create') {
        submitFirstOrderCreate();
        return true;
    }
    if (action === 'add-first-order-create-plan-line') {
        state.firstOrderCreateDraft.samplePlanLinesText = addFirstOrderSamplePlanLineText(state.firstOrderCreateDraft.samplePlanLinesText, createDefaultSamplePlanLines(state.firstOrderCreateDraft.sampleChainMode));
        return true;
    }
    if (action === 'remove-first-order-create-plan-line') {
        state.firstOrderCreateDraft.samplePlanLinesText = removeFirstOrderSamplePlanLineText(state.firstOrderCreateDraft.samplePlanLinesText, createDefaultSamplePlanLines(state.firstOrderCreateDraft.sampleChainMode), Number(actionNode.dataset.lineIndex || '-1'));
        return true;
    }
    ...
    if (action === 'remove-first-order-result-plan-line') {
        state.firstOrderResultDraft.samplePlanLinesText = removeFirstOrderSamplePlanLineText(state.firstOrderResultDraft.samplePlanLinesText, [], Number(actionNode.dataset.lineIndex || '-1'));
        return true;
    }
    if (action === 'set-revision-quick-filter') {
        state.revisionList.quickFilter = actionNode.dataset.quickFilter || 'all';
        state.revisionList.currentPage = 1;
        return true;
    }
    if (action === 'set-plate-quick-filter') {
        state.plateList.quickFilter = actionNode.dataset.quickFilter || 'all';
        state.plateList.currentPage = 1;
        return true;
    }
    if (action === 'set-pattern-quick-filter') {
        state.patternList.quickFilter = actionNode.dataset.quickFilter || 'all';
        state.patternList.currentPage = 1;
        return true;
    }
    if (action === 'set-first-sample-quick-filter') {
        state.firstSampleList.quickFilter = actionNode.dataset.quickFilter || 'all';
        state.firstSampleList.currentPage = 1;
        return true;
    }
    if (action === 'set-first-order-quick-filter') {
        state.firstOrderList.quickFilter = actionNode.dataset.quickFilter || 'all';
        state.firstOrderList.currentPage = 1;
        return true;
    }
    if (action === 'change-revision-page') {
        updateListPage(state.revisionList, Number(actionNode.dataset.pageStep || '0'), getRevisionTasksFiltered().length);
        return true;
    }
    if (action === 'change-plate-page') {
        updateListPage(state.plateList, Number(actionNode.dataset.pageStep || '0'), getPlateTasksFiltered().length);
        return true;
    }
    if (action === 'change-pattern-page') {
        updateListPage(state.patternList, Number(actionNode.dataset.pageStep || '0'), getPatternTasksFiltered().length);
        return true;
    }
    if (action === 'change-first-sample-page') {
        updateListPage(state.firstSampleList, Number(actionNode.dataset.pageStep || '0'), getFirstSampleTasksFiltered().length);
        return true;
    }
    if (action === 'change-first-order-page') {
        updateListPage(state.firstOrderList, Number(actionNode.dataset.pageStep || '0'), getFirstOrderTasksFiltered().length);
        return true;
    }
    if (action === 'set-revision-tab') {
        state.revisionTab = actionNode.dataset.tab || 'plan';
        return true;
    }
    if (action === 'set-plate-tab') {
        state.plateTab = actionNode.dataset.tab || 'overview';
        return true;
    }
    if (action === 'set-pattern-tab') {
        state.patternTab = actionNode.dataset.tab || 'plan';
        return true;
    }
    if (action === 'set-first-sample-tab') {
        state.firstSampleTab = actionNode.dataset.tab || 'overview';
        return true;
    }
    if (action === 'set-first-order-tab') {
        state.firstOrderTab = actionNode.dataset.tab || 'overview';
        return true;
    }
    if (action === 'toggle-revision-scope') {
        ...
        state.revisionCreateDraft.createPatternTask = canCreateRevisionPatternTask(state.revisionCreateDraft.scopeCodes, state.revisionCreateDraft.sourceType, state.revisionCreateDraft.projectId);
        return true;
    }
    ...
    if (action === 'revision-generate-tech-pack') {
        generateRevisionTechPack(actionNode.dataset.taskId || '');
        return true;
    }
    if (action === 'plate-generate-tech-pack') {
        generatePlateTechPack(actionNode.dataset.taskId || '');
        return true;
    }
    if (action === 'pattern-generate-tech-pack') {
        generatePatternTechPack(actionNode.dataset.taskId || '');
        return true;
    }
    ...
    if (action === 'first-sample-advance') {
        advanceFirstSampleTask(actionNode.dataset.taskId || '');
        return true;
    }
    if (action === 'save-first-sample-detail') {
        saveFirstSampleDetail(actionNode.dataset.taskId || state.firstSampleDetailDraftTaskId);
        return true;
    }
    if (action === 'close-first-sample-acceptance') {
        state.firstSampleAcceptanceOpen = false;
        return true;
    }
    ...
    if (action === 'first-order-advance') {
        advanceFirstOrderTask(actionNode.dataset.taskId || '');
        return true;
    }
    if (action === 'close-first-order-start') {
        state.firstOrderStartOpen = false;
        return true;
    }
    if (action === 'submit-first-order-start') {
        submitFirstOrderStart();
        return true;
    }
    if (action === 'close-first-order-result') {
        state.firstOrderResultOpen = false;
        return true;
    }
    if (action === 'submit-first-order-result') {
        submitFirstOrderResult();
        return true;
    }
    if (action === 'close-first-order-conclusion') {
        state.firstOrderConclusionOpen = false;
        return true;
    }
    if (action === 'submit-first-order-conclusion') {
        ...
        if (!state.firstOrderConclusionConfirmedBy.trim()) {
            setNotice('请填写确认人。');
            return true;
        }
        if (!state.firstOrderConclusionConfirmedAt.trim()) {
            setNotice('请填写确认时间。');
            return true;
        }
...
export function isPcsEngineeringTaskDialogOpen() {
    return (state.revisionCreateOpen
...
export function resetPcsEngineeringTaskState() {
    clearNotice();
...
export function resetPcsEngineeringTaskRepositories() {
