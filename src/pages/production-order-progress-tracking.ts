...
import { renderProductionObjectCodeButton, renderProductionOrderIdentityCell } from '../data/fcs/production-order-identity.ts'
import {
  addNaturalDays,
...
const columns: StandardListColumn<OnlineProductionProgressRecord>[] = [
  { key: 'base', title: '基础信息', width: 230, required: true, freezeable: true, render: (row) => `<div class="flex gap-2"><button data-progress-action="preview-image" data-url="${row.imageUrl}" data-label="${escapeHtml(row.spu)}"><img src="${row.imageUrl}" alt="${escapeHtml(row.spu)}款式实拍图" class="h-16 w-14 rounded border object-cover"/></button><div class="text-xs"><b>Spu：${renderProductionObjectCodeButton({ objectType: 'PRODUCTION_ORDER', objectId: row.productionOrderNo, label: row.spu })}</b>${renderProductionOrderIdentityCell({ productionOrderNo: row.productionOrderNo, demandNo: row.demandNo, saleType: row.saleType })}<p class="mt-1"><span class="rounded bg-orange-50 px-1 text-orange-600">${row.sampleType}</span> <span class="rounded bg-blue-50 px-1 text-blue-600">${row.saleType}</span></p><p>做货难度：${escapeHtml(row.difficulty)}</p></div></div>` },
