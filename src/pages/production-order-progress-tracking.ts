...
const columns: StandardListColumn<OnlineProductionProgressRecord>[] = [
  ...
  { key: 'time', title: '时间', width: 225, render: (row) => { const fulfillment = highestReturnRisk(row); return `<dl class="grid grid-cols-[74px_1fr] gap-x-2 text-[11px]"><dt>生产下单</dt><dd>${row.orderAt}</dd><dt>拆解</dt><dd>${row.breakdownAt || '-'}</dd><dt>首次配料</dt><dd>${row.firstMaterialAt}</dd><dt>首次领料</dt><dd>${row.firstPickupAt || '-'}</dd><dt>裁片开始</dt><dd>${row.cuttingStartedAt || '-'}</dd><dt>裁片完成</dt><dd>${row.cuttingCompletedAt || '-'}</dd><dt>车缝派单</dt><dd>${row.sewingAssignedAt || '-'}</dd><dt>首次登记</dt><dd>${row.firstRegistrationAt || '-'}</dd><dt>首次入库</dt><dd>${row.firstInboundAt || '-'}</dd>${fulfillment?.node && fulfillment.progress ? `<dt class="mt-1 text-red-700">最近节点</dt><dd class="mt-1 text-red-700">${fulfillment.node.deadline} · ${escapeHtml(fulfillment.progress.factoryName)} · 缺${Math.max(0, fulfillment.node.targetQty - fulfillment.node.confirmedQty)}件</dd>` : ''}</dl>` } },
