...
import { createMaterialArchive, createMaterialSkuRecord, getMaterialArchiveById, getMaterialArchiveCategoryOptions, getMaterialSkuRecordById, getMaterialStats, getMaterialSkuSpecMeta, listMaterialArchives, listMaterialLogRecordsByMaterialId, listMaterialSkuRecordsByMaterialId, listMaterialUsageRecordsByMaterialId, updateMaterialSkuRecord, } from '../data/pcs-material-archive-repository.ts';
...
function createEmptySkuEditorState() {
    return {
        open: false,
        materialId: '',
        materialSkuId: null,
...
function resetCreateState(kind) {
    state.create = {
        open: false,
        kind,
        materialName: '',
...
function buildSkuEditorSpecValue(kind, record) {
    if (kind === 'fabric') {
...
function openSkuEditor(materialId, materialSkuId = null) {
...
function buildSkuEditorInput(material) {
...
function renderDrawerShell(title, body, footer) {
    return `
...
function getFilteredRecords(kind) {
...
function renderListTable(kind, records) {
    const rows = records
        .map((record) => {
...
function renderSkuEditorDrawer() {
    if (!state.skuEditor.open || !state.skuEditor.materialId)
        return '';
    ...
    <div class="space-y-4">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ${renderFormField(specMeta.primaryLabel, renderTextInput('sku-editor-color-name', state.skuEditor.colorName, specMeta.primaryPlaceholder), true)}
        ${renderFormField(specMeta.secondaryLabel, renderTextInput('sku-editor-spec-value', state.skuEditor.specValue, specMeta.secondaryPlaceholder), true)}
      </div>
      <div class="grid gap-4 md:grid-cols-2">
...
function renderBarcodeDrawer() {
    if (!state.barcode.open || !state.barcode.materialId)
        return '';
...
function renderLogDrawer() {
    if (!state.log.open || !state.log.materialId)
        return '';
...
function renderOverviewTab(material, skuRecords, usageRecords) {
...
function renderSkuSpecText(kind, item) {
    if (kind === 'fabric') {
...
function renderSkuTab(kind, skuRecords) {
...
function renderUsageTab(usageRecords) {
...
function renderDetailPage(kind, materialId) {
    const material = getMaterialArchiveById(materialId);
    ...
    const logs = listMaterialLogRecordsByMaterialId(material.materialId);
    const tabContent = state.detail.activeTab === 'overview'
...
function syncDefaultBarcodeSelection(materialId) {
...
function updateFilterField(field, value) {
    ...
    if (statusMatch) {
        ...
        state.filters[kind].status = (value || 'all');
        return true;
    }
    return false;
}
export function handlePcsMaterialArchiveInput(target) {
    ...
    if (updateFilterField(field, value))
        return true;
    switch (field) {
...
export function handlePcsMaterialArchiveEvent(target) {
    ...
    switch (action) {
        case 'close-notice':
            ...
            return true;
        case 'open-create': {
            const kind = actionNode.dataset.kind || 'fabric';
        ...
        case 'open-log':
            ...
            return true;
        case 'set-detail-tab':
            state.detail.activeTab = actionNode.dataset.value || 'overview';
            return true;
        case 'close-drawers':
...
export function isPcsMaterialArchiveDialogOpen() {
    return state.create.open || state.skuEditor.open || state.barcode.open || state.log.open;
}
export function renderPcsFabricArchiveListPage() {
    return renderListPage('fabric');
}
export function renderPcsFabricArchiveCreatePage() {
    resetCreateState('fabric');
    state.create.open = true;
    return renderListPage('fabric');
}
export function renderPcsAccessoryArchiveListPage() {
    return renderListPage('accessory');
}
export function renderPcsAccessoryArchiveCreatePage() {
    resetCreateState('accessory');
    state.create.open = true;
    return renderListPage('accessory');
}
export function renderPcsYarnArchiveListPage() {
    return renderListPage('yarn');
}
export function renderPcsYarnArchiveCreatePage() {
    resetCreateState('yarn');
    state.create.open = true;
    return renderListPage('yarn');
}
export function renderPcsConsumableArchiveListPage() {
    return renderListPage('consumable');
}
export function renderPcsConsumableArchiveCreatePage() {
    resetCreateState('consumable');
    state.create.open = true;
    return renderListPage('consumable');
}
export function renderPcsMaterialArchiveDetailPage(kind, materialId) {
