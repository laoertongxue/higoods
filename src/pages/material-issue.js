import { listMaterialIssueSheetsFromRuntime, } from '../data/fcs/store-domain-dispatch-process';
import { getExecutionTaskFactById, listExecutionTaskFacts, } from '../data/fcs/page-adapters/task-execution-adapter';
...
const localIssueAdditions = [];
const localIssueOverrides = new Map();
...
function showMaterialIssueToast(message, tone = 'success') {
...
function getMutableSheetById(issueId) {
...
function updateMaterialIssueStatus(input, by) {
    const { issueId, nextStatus, remark } = input;
    const sheet = getMutableSheetById(issueId);
    if (!sheet)
        return { ok: false, message: `领料需求单 ${issueId} 不存在` };
    if (!nextStatus)
        return { ok: false, message: '目标状态不能为空' };
...
function renderCreateDialog() {
    ...
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      ...
      <div class="absolute left-1/2 top-1/2 max-h-[86vh] w-full max-w-xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl border bg-background p-6 shadow-2xl" data-dialog-panel="true">
        ...
        .map((task) => `<option value="${escapeHtml(task.taskId)}" ${state.createForm.taskId === task.taskId ? 'selected' : ''}>${escapeHtml(task.taskId)} ${escapeHtml(task.processNameZh)}</option>`)
        .join('')}
            </select>
          </div>
...
function renderEditDialog(editSheet) {
    if (!editSheet)
        return '';
    return `
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
...
function renderStatusDialog(statusSheet) {
    if (!statusSheet)
        return '';
    return `
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      ...
      <div class="absolute left-1/2 top-1/2 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background p-6 shadow-2xl" data-dialog-panel="true">
        ...
        .map((value) => `<option value="${value}" ${state.nextStatus === value ? 'selected' : ''}>${STATUS_LABEL[value]}</option>`)
        .join('')}
            </select>
          </div>
        ...
        <div class="mt-6 flex justify-end gap-2">
          ...
          <button class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 ${!state.nextStatus ? 'pointer-events-none opacity-50' : ''}" data-mis-action="submit-status">保存</button>
        </div>
      </div>
    </div>
  `;
}
export function renderMaterialIssuePage() {
                          ...
                          <tr class="border-b last:border-b-0">
                            ...
                            <td class="px-4 py-3">
                              <div class="flex flex-wrap gap-1">
                                ...
                                <button class="inline-flex h-8 items-center rounded-md border px-2 text-xs hover:bg-muted ${!canStatus ? 'pointer-events-none opacity-50' : ''}" data-mis-action="open-status" data-issue-id="${escapeHtml(sheet.issueId)}">状态变更</button>
...
export function handleMaterialIssueEvent(target) {
    const filterNode = target.closest('[data-mis-filter]');
    ...
    const fieldNode = target.closest('[data-mis-field]');
    ...
    const actionNode = target.closest('[data-mis-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'submit-status') {
        if (!state.statusIssueId || !state.nextStatus)
            return true;
...
export function isMaterialIssueDialogOpen() {
