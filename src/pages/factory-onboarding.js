...
import { FACTORY_ONBOARDING_NODE_OPTIONS, FACTORY_ONBOARDING_REQUIRED_FIELD_OPTIONS, FACTORY_ONBOARDING_REVIEW_RESULTS, FACTORY_ONBOARDING_STATUS_OPTIONS, normalizeReviewResult, } from '../data/fcs/factory-onboarding-domain.ts';
import { FACTORY_SAMPLE_ISSUE_METHOD_OPTIONS, FACTORY_SAMPLE_CAPACITY_CONCLUSION_OPTIONS, FACTORY_SAMPLE_QUALITY_CONCLUSION_OPTIONS, FACTORY_SAMPLE_REVIEW_REQUIRED_ITEM_OPTIONS, FACTORY_SAMPLE_REVIEW_RESULT_OPTIONS, FACTORY_SAMPLE_VERIFICATION_PURPOSE_OPTIONS, normalizeSampleReviewResult, } from '../data/fcs/factory-sample-verification-domain.ts';
import { getFactoryOnboardingCurrentNodeSummary, getLatestReviewRecord, getLatestSupplementRecord, listFactoryOnboardingStatusBuckets, listSelectableProcessCraftOptions, reviewFactoryOnboardingApplication, convertOnboardingToOfficialFactory, } from '../data/fcs/factory-onboarding-flow.ts';
import { getAvailableOnboardingPpicOptions, listFactoryOnboardingApplications, updateOnboardingPpic, } from '../data/fcs/factory-onboarding-store.ts';
import { createSampleIssuePayload, issueSampleForOnboarding, reviewFactorySample, } from '../data/fcs/factory-sample-verification-flow.ts';
import { getSampleVerificationByApplicationId, getSampleVerificationById, } from '../data/fcs/factory-sample-verification-store.ts';
const state = {
    statusFilter: 'ALL',
    processFilter: 'ALL',
    craftFilter: 'ALL',
...
function createDefaultSampleReviewDraft() {
    return {
        sampleReviewResult: '已通过',
        sampleReviewOpinion: '',
        resubmitAllowed: false,
...
function syncDialogStateFromQuery(applications) {
    ...
    if (dialog === 'ppic') {
        ...
        if (state.ppicDialogApplicationId === applicationId)
            return;
    ...
    if (state.selectedApplicationId || state.reviewApplicationId)
        return;
    if (dialog === 'review') {
        if (application.status !== '待平台审核')
            return;
    ...
    if (dialog === 'sample-review') {
        if (application.status !== '待平台审核样衣')
            return;
        if (state.sampleReviewApplicationId === applicationId)
            return;
    ...
    if (dialog === 'sample-issue') {
        if (application.status !== '待样衣验证' || application.sampleVerificationId || getSampleVerificationForApplication(application))
            return;
        if (state.sampleIssueApplicationId === applicationId)
            return;
    ...
    if (dialog === 'conversion') {
        ...
        if (application.status !== '样衣审核通过待转正式' || sampleVerification?.status !== '样衣审核通过')
            return;
...
function getSampleVerificationForApplication(application) {
    if (!application)
        return null;
...
function isIssuedSampleStatus(application) {
    return [
        '待工厂确认收样',
        '待工厂提交样衣审核',
        '待平台审核样衣',
...
function renderReferenceFiles(files, emptyText) {
    if (files.length <= 0)
        return `<div class="rounded-xl border border-dashed px-3 py-2 text-muted-foreground">${escapeHtml(emptyText)}</div>`;
    return `
    <div class="space-y-2">
      ${files.map((file) => `
...
function getReferenceFileType(fileName) {
...
function buildReferenceFilesFromInput(input) {
...
function updateSampleReviewFilesFromInput(input, field) {
    if (field !== 'sampleReview-bossIdentityFiles')
        return;
...
function getLatestActionName(application) {
    const action = application.actionLogs[application.actionLogs.length - 1];
    return action?.actionName || '—';
}
function getPrimaryFactoryTypeLabel(code) {
    const map = {
        CUTTING_FACTORY: '裁床厂',
...
function getApplicationMobilePhone(application) {
...
function getApplicationStatusDisplay(status) {
    if (status === LEGACY_PLATFORM_REJECTED_STATUS)
        return '平台审核退回';
    if (status === LEGACY_SAMPLE_REJECTED_STATUS)
        return '样衣审核退回';
    return status;
}
function getSampleStatusDisplay(status) {
    if (status === LEGACY_SAMPLE_REJECTED_STATUS)
        return '样衣审核退回';
    if (status === '样衣审核通过')
        return '待转正式合作';
...
function renderStatusChip(label) {
    ...
    const className = displayLabel === '已转正式合作'
...
function renderDetailStatusPanel(application) {
...
function filterApplications(applications) {
    return applications.filter((item) => {
        if (state.statusFilter !== 'ALL' && item.status !== state.statusFilter)
            return false;
        if (state.nodeFilter !== 'ALL' && item.currentNode !== state.nodeFilter)
            return false;
        if (state.reviewResultFilter !== 'ALL') {
            const latestReview = getLatestReviewRecord(item);
            if (!latestReview || normalizeReviewResult(latestReview.reviewResult) !== state.reviewResultFilter)
                return false;
        }
        if (state.ppicFilter === 'UNASSIGNED' && item.assignedPpicId)
            return false;
        if (state.ppicFilter !== 'ALL' && state.ppicFilter !== 'UNASSIGNED' && item.assignedPpicId !== state.ppicFilter)
            return false;
        if (state.processFilter !== 'ALL' && !item.selectedCapabilities.some((cap) => cap.processCode === state.processFilter))
            return false;
        if (state.craftFilter !== 'ALL' && !item.selectedCapabilities.some((cap) => cap.craftCode === state.craftFilter))
            return false;
        if (state.keyword) {
            const text = [
                item.applicationNo,
...
function renderSampleVerificationDetail(application) {
    const sampleVerification = getSampleVerificationForApplication(application);
...
function renderDetailDrawer(application) {
    if (!application)
        return '';
...
function renderReviewDialog(application) {
    if (!application)
        return '';
...
function renderSampleIssueDialog(application) {
    if (!application)
        return '';
...
function renderSampleReviewDialog(application) {
    if (!application)
        return '';
    const sampleVerification = getSampleVerificationForApplication(application);
    if (!sampleVerification)
...
function renderConversionDialog(application) {
    if (!application)
        return '';
    const sampleVerification = getSampleVerificationForApplication(application);
...
function renderPpicDialog(application) {
    if (!application)
        return '';
    const options = getAvailableOnboardingPpicOptions();
...
export function renderFactoryOnboardingPage() {
...
function updateSampleIssueField(field, value, checked) {
    state.sampleIssueErrorText = '';
    ...
    if (!field.startsWith('sampleIssue-'))
        return;
    const key = field.replace('sampleIssue-', '');
...
function updateSampleReviewField(field, value, checked) {
    state.sampleReviewErrorText = '';
    ...
    if (!field.startsWith('sampleReview-'))
        return;
    const key = field.replace('sampleReview-', '');
    const nextDraft = {
        ...state.sampleReviewDraft,
        [key]: value,
    };
    if (key === 'sampleReviewResult') {
        ...
        if (value !== '未通过')
            nextDraft.requiredResubmitItems = [];
    }
    state.sampleReviewDraft = nextDraft;
}
function readSampleReviewDraftFromDom() {
    if (typeof document === 'undefined')
        return state.sampleReviewDraft;
    const result = document.querySelector('[data-factory-onboarding-field="sampleReview-sampleReviewResult"]:checked')?.value;
    const sampleReviewOpinion = document.querySelector('[data-factory-onboarding-field="sampleReview-sampleReviewOpinion"]')?.value;
    const sampleQualityConclusion = document.querySelector('[data-factory-onboarding-field="sampleReview-sampleQualityConclusion"]:checked')?.value;
    const capacityConclusion = document.querySelector('[data-factory-onboarding-field="sampleReview-capacityConclusion"]:checked')?.value;
    const bossIdentityNo = document.querySelector('[data-factory-onboarding-field="sampleReview-bossIdentityNo"]')?.value;
    const remark = document.querySelector('[data-factory-onboarding-field="sampleReview-remark"]')?.value;
    const requiredResubmitItems = Array.from(document.querySelectorAll('[data-factory-onboarding-field="sampleReviewRequiredItem"]:checked'))
        .map((item) => item.value);
...
function updateField(field, value, checked) {
    ...
    if (field === 'reviewRequiredField') {
        ...
        if (checked) {
            if (!state.reviewRequiredFields.includes(fieldValue))
                state.reviewRequiredFields.push(fieldValue);
        }
        else {
...
export async function handleFactoryOnboardingEvent(target) {
    const fileNode = target.closest('[data-factory-onboarding-file]');
    if (fileNode instanceof HTMLInputElement) {
        updateSampleReviewFilesFromInput(fileNode, fileNode.dataset.factoryOnboardingFile || '');
        return true;
    }
    const fieldNode = target.closest('[data-factory-onboarding-field]');
    ...
    const actionNode = target.closest('[data-factory-onboarding-action]');
    ...
    if (action === 'switch-detail-tab') {
        state.detailTab = actionNode?.dataset.tab || 'basic';
        return true;
    }
    ...
    if (action === 'submit-ppic') {
        ...
        state.ppicErrorText = '';
        try {
            const dialog = actionNode?.closest('[data-factory-onboarding-dialog="ppic"]') || document;
            const nextPpicId = dialog.querySelector('[data-factory-onboarding-field="ppicDraftId"]')?.value || state.ppicDraftId;
            const changeReason = dialog.querySelector('[data-factory-onboarding-field="ppicChangeReason"]')?.value || state.ppicChangeReason;
            const updated = updateOnboardingPpic(state.ppicDialogApplicationId || '', nextPpicId, '平台运营员', changeReason);
            state.ppicDialogApplicationId = null;
    ...
    if (action === 'submit-sample-issue') {
        ...
        state.sampleIssueErrorText = '';
        try {
            const result = issueSampleForOnboarding(state.sampleIssueApplicationId || '', state.sampleIssueDraft, state.sampleIssueDraft.issuedBy || '平台样衣员');
            state.sampleIssueApplicationId = null;
    ...
    if (action === 'submit-sample-review') {
        ...
        try {
            ...
            const result = reviewFactorySample(sampleVerification.verificationId, state.sampleReviewDraft, '平台样衣审核员');
            state.sampleReviewApplicationId = null;
            state.selectedApplicationId = result.application.applicationId;
            state.detailTab = 'sample';
    ...
    if (action === 'submit-conversion') {
        ...
        state.conversionErrorText = '';
        try {
            const result = await convertOnboardingToOfficialFactory(state.conversionApplicationId || '', '平台转档员');
            state.conversionApplicationId = null;
            state.selectedApplicationId = result.application.applicationId;
...
export function isFactoryOnboardingDialogOpen() {
