...
import { ensureHandoverOrderForStartedTask, getHandoverOrderById, listHandoverOrdersByTaskId, } from '../data/fcs/pda-handover-events.ts';
import { getTaskProcessDisplayName, } from '../data/fcs/page-adapters/task-execution-adapter';
import { isCuttingSpecialTask, } from '../data/fcs/pda-cutting-execution-source.ts';
import { getSpecialCraftFeiTicketScanSummary, listCuttingSpecialCraftFeiTicketBindings, } from '../data/fcs/cutting/special-craft-fei-ticket-flow.ts';
import { getSpecialCraftTaskWorkOrderById } from '../data/fcs/special-craft-task-orders.ts';
import { getDifferenceRecordsByWorkOrderId, getHandoverRecordsByWorkOrderId, getWarehouseRecordsByWorkOrderId, } from '../data/fcs/process-warehouse-domain.ts';
...
import { PAUSE_REASON_OPTIONS, getPauseHandleStatus, getTaskMilestoneProofHint, getTaskMilestoneState, isTaskMilestoneProofSatisfied, isTaskMilestoneReported, reportTaskMilestone, reportTaskPause, syncMilestoneOverdueExceptions, } from '../data/fcs/pda-exec-link';
import { buildTaskQrValue } from '../data/fcs/task-qr.ts';
import { TEST_FACTORY_ID } from '../data/fcs/factory-mock-data.ts';
import { getPrintExecutionNodeRecord, getPrintOrderHandoverSummary, getPrintReviewRecordByOrderId, getPrintWorkOrderById, getPrintWorkOrderByTaskId, getPrintWorkOrderStatusLabel, listPrintMachineOptions, } from '../data/fcs/printing-task-domain.ts';
import { acceptWoolWorkOrder, completeWoolPickupHead, confirmWoolHandoverReceipt, confirmWoolPickupRecord, getWoolAllowedActions, getWoolWorkOrderByTaskId, getWoolWorkOrderKindLabel, getWoolWorkOrderStatusLabel, getWoolYarnUsageSummary, markWoolFeiTicketsPrinted, scheduleWoolMachines, submitWoolHandover, updateWoolWorkOrderNodeStatus, } from '../data/fcs/wool-task-domain.ts';
import { getDyeExecutionNodeRecord, getDyeOrderHandoverSummary, getDyeReviewRecordByOrderId, getDyeWorkOrderById, getDyeWorkOrderByTaskId, getDyeWorkOrderStatusLabel, getSampleWaitTypeLabel, listDyeVatOptions, } from '../data/fcs/dyeing-task-domain.ts';
import { bindSpecialCraftFeiTicket, getPostFinishingWorkOrderForMobile, startDyeMaterialWaitWriteback, startDyeSampleWaitWriteback, } from '../data/fcs/process-execution-writeback.ts';
import { executeMobileProcessAction, getProcessActionOperationRecordsBySource, getProcessActionOperationRecordsByTask, } from '../data/fcs/process-action-writeback-service.ts';
import { formatProcessQuantityWithUnit, getQuantityLabel, } from '../data/fcs/process-quantity-labels.ts';
import { getPostFinishingFlowText, getPostFinishingSourceLabel, getPostFinishingTaskById, getSewingFactoryPostTaskById, listPostFinishingQcOrderEntities, listPostFinishingRecheckOrderEntities, listPostFinishingWaitQcSkuItems, listPostFinishingWorkOrders, finishSewingFactoryPostTask, startSewingFactoryPostTask, transferSewingFactoryPostTaskToManagedFactory, } from '../data/fcs/post-finishing-domain.ts';
import { buildMobileExecutionListLocatePathForTask, getMobileExecutionTaskById, getMobileExecutionTaskBySource, getMobileExecutionTaskSourceInfo, getMobileTaskTabKey, isMobileTaskVisibleForFactory, MOBILE_EXECUTION_TASK_TAB_LABELS, } from '../data/fcs/mobile-execution-task-index.ts';
import { getMobileTaskProcessType, getMobileTaskAccessResult, listPdaMobileExecutionTasks, } from '../data/fcs/process-mobile-task-binding.ts';
...
function mapPostFinishingStatusToTaskStatus(status) {
    if (status.includes('差异'))
        return 'BLOCKED';
    if (status.includes('中'))
        return 'IN_PROGRESS';
    if (status.includes('已交出') || status.includes('已收货') || status.includes('已完成'))
        return 'DONE';
    return 'NOT_STARTED';
}
function mapPostFinishingOrderToTask(order, seq) {
    return {
...
function mapSewingFactoryPostTaskToProcessTask(task, seq) {
    return {
...
function getRootTaskDisplayNo(task) {
...
function getTaskQrValue(task) {
    return task.taskQrValue || buildTaskQrValue(task.taskId);
}
function getReceiverKindLabel(kind) {
    if (kind === 'WAREHOUSE')
        return '仓库';
...
function getReceiverDisplayText(task) {
    if (task.receiverName?.trim())
        return task.receiverName.trim();
    return getReceiverKindLabel(task.receiverKind) || '未配置';
}
function getHandoverOrderStatusLabel(status) {
    if (!status)
        return '未生成';
    const labelMap = {
        AUTO_CREATED: '已创建',
...
function canTaskUseHandover(task) {
    return task.handoverAutoCreatePolicy === 'CREATE_ON_START' || Boolean(task.taskQrValue);
}
function syncTaskHandoverFields(task, handoverOrder) {
    if (!handoverOrder)
        return;
...
function getTaskHandoverOrder(task) {
...
function renderHandoverOrderCard(handoverOrder) {
...
function renderPrintingStatusBadge(label, tone) {
...
function getReceiptStatusMeta(status) {
    if (status === 'FULL_HANDOVER')
        return { label: '全部交出', tone: 'success' };
    if (status === 'PARTIAL_HANDOVER')
        return { label: '部分交出', tone: 'warning' };
    if (status === 'HANDOVER_DIFFERENCE')
        return { label: '收货差异', tone: 'danger' };
    if (status === 'WAIT_RECEIVE' || status === 'HANDOVER_WAIT_RECEIVE')
        return { label: '交出待收货', tone: 'warning' };
    return { label: '待收货确认', tone: 'muted' };
}
function canOperatePrintingNode(task) {
...
function renderPrintingTaskCard(task, printOrder, handoverOrder) {
    ...
    const handoverBadge = printOrder.status === 'WAIT_HANDOVER'
...
function canOperateDyeingNode(task) {
    const currentFactoryId = getPdaSession()?.factoryId || task.assignedFactoryId || TEST_FACTORY_ID;
    return isMobileTaskVisibleForFactory(task, currentFactoryId) && task.status !== 'NOT_STARTED' && task.status !== 'CANCELLED';
}
function getExecDetailSearchParams() {
    const pathname = appStore.getState().pathname;
    const [, queryString = ''] = pathname.split('?');
    return new URLSearchParams(queryString);
}
function resolveExecDetailBackHref(task) {
...
function getExecDetailAccessNotice(access) {
    if (access.canOpenMobileExecution)
        return null;
...
function renderDyeingTaskCard(task, dyeOrder, handoverOrder) {
    ...
    const handoverBadge = dyeOrder.status === 'WAIT_HANDOVER'
        ? renderPrintingStatusBadge('待交出', 'warning')
    ...
    const postProcessRows = [
        { label: '脱水', code: 'DEHYDRATE', record: dehydrateNode, requireFinished: Boolean(dyeNode?.finishedAt) },
        { label: '烘干', code: 'DRY', record: dryNode, requireFinished: Boolean(dehydrateNode?.finishedAt) },
        { label: '定型', code: 'SET', record: setNode, requireFinished: Boolean(dryNode?.finishedAt) },
        { label: '打卷', code: 'ROLL', record: rollNode, requireFinished: Boolean(setNode?.finishedAt) },
        { label: '包装', code: 'PACK', record: packNode, requireFinished: Boolean(rollNode?.finishedAt) },
    ];
    return `
    <article class="rounded-lg border bg-card">
      <header class="border-b px-4 py-3">
        ...
        <div class="grid gap-3 xl:grid-cols-2">
          ...
          <section class="rounded-lg border bg-background p-3 xl:col-span-2">
            ...
            <div class="mt-3 space-y-2">
              ${postProcessRows
        .map(({ label, code, record, requireFinished }) => `
...
function renderWoolTaskCard(task, woolOrder, handoverOrder) {
    ...
    const renderAllowedActionButton = (action) => `
    <button
      type="button"
  ...
  `;
    ...
    const yarnStatusText = woolOrder.yarnReceipt.receivedWeightKg <= 0
        ...
        : yarnDiff !== 0
            ? '领料有差异'
            : '已领料';
    const visibleNodes = woolOrder.nodes.filter((node) => node.status !== '未开始'
...
function getReportedQtyLabel(unitLabel) {
    if (unitLabel === '卷')
        return '上报布卷数（卷）';
    if (unitLabel === '层')
        return '上报铺布层数（层）';
...
function getMilestoneDisplayUnitLabel(task, fallback) {
    const woolOrder = getWoolWorkOrderByTaskId(task.taskId);
    if (woolOrder?.kind === 'PART_PANEL')
        return '片';
    if (woolOrder?.kind === 'WHOLE_GARMENT')
        return '件';
    return fallback;
}
const MOCK_START_PROOF = {
    'PDA-EXEC-007': [
...
function getCurrentPathWithoutSearch() {
...
function syncDialogStateWithQuery(task) {
    const taskId = task.taskId;
    const pathname = appStore.getState().pathname;
    const key = `${taskId}|${pathname}`;
    if (detailState.initializedPathKey === key)
        return;
...
function showPdaExecDetailToast(message) {
...
function addProofFile(scope, type) {
    ...
    if (scope === 'start')
        detailState.startProofFiles = next;
    else if (scope === 'milestone')
        detailState.milestoneProofFiles = next;
    else
        detailState.pauseProofFiles = next;
}
function removeProofFile(scope, id) {
    const next = scope === 'start'
...
function isProofRequirementSatisfied(requirement, proofFiles) {
    if (requirement === 'NONE')
        return true;
...
function renderProofUploadSection(files, scope, helperText) {
    return `
    <div class="space-y-3">
...
function renderProofViewSection(files) {
    if (files.length === 0) {
        return `
...
function mutateStartTask(taskId, by, payload) {
...
function getWoolOrderForTask(task) {
    if (!task)
        return undefined;
    return getWoolWorkOrderByTaskId(task.taskId);
}
function promptWoolKgValue(label, currentValue = 0, allowZero = true) {
...
function startWoolOrderFromMobile(task, startTime, yarnUsageWeightKg) {
...
function reportWoolMilestoneFromMobile(task, reportAt) {
    const woolOrder = getWoolOrderForTask(task);
    if (!woolOrder)
        return false;
...
function finishWoolOrderFromMobile(task) {
    const woolOrder = getWoolOrderForTask(task);
    if (!woolOrder)
        return false;
    ...
    woolOrder.nodes.forEach((node) => {
        ...
        if (node.status !== '已跳过') {
            updateWoolWorkOrderNodeStatus(woolOrder.woolOrderId, node.nodeName, '已完成', '工厂端操作员', operatedAt, node.nodeName === '缝盘' ? { yarnLossWeightKg } : {});
        }
    });
    return true;
}
function isSpecialCraftExecutionTask(task, displayProcessName = getTaskProcessDisplayName(task)) {
    if (getWoolWorkOrderByTaskId(task.taskId))
        return false;
    const stage = task.stage;
    const processStage = task.processStage;
...
function getSpecialCraftExecBindings(task) {
    const taskNo = task.taskNo || task.taskId;
    const rootTaskNo = task.rootTaskNo || '';
    return listCuttingSpecialCraftFeiTicketBindings().filter((binding) => binding.taskOrderId === task.taskId
...
function resolveSpecialCraftPdaObjectMeta(workOrder) {
...
function getSpecialCraftWorkOrderForPdaTask(task, bindings = getSpecialCraftExecBindings(task)) {
...
function getSpecialCraftPdaBaseQty(task, workOrder, binding, objectMeta) {
...
function renderSpecialCraftExecutionPanel(task, status, displayProcessName) {
    const bindings = getSpecialCraftExecBindings(task);
    if (!isSpecialCraftExecutionTask(task, displayProcessName) && bindings.length === 0)
        return '';
...
function getPostFinishingActionLabel(actionType, phase) {
    if (actionType === '扫码收货')
        return phase === 'start' ? '开始扫码收货' : '确认收货入库';
    if (phase === 'start') {
        return actionType === '后道' ? '开始后道' : actionType === '质检' ? '开始质检' : '开始复检';
    }
    return actionType === '后道' ? '完成后道' : actionType === '质检' ? '完成质检' : '完成复检';
}
function getPostFinishingActionCode(actionType, phase) {
    if (actionType === '扫码收货')
        return phase === 'start' ? 'POST_RECEIVE_START' : 'POST_RECEIVE_FINISH';
    if (actionType === '质检')
        return phase === 'start' ? 'POST_QC_START' : 'POST_QC_FINISH';
    if (actionType === '后道')
        return phase === 'start' ? 'POST_PROCESS_START' : 'POST_PROCESS_FINISH';
    return phase === 'start' ? 'POST_RECHECK_START' : 'POST_RECHECK_FINISH';
}
function renderPostFinishingActionButton(order, actionType, phase, disabled = false) {
    return `
    <button
      type="button"
...
function canPostFinishingManagedFactoryOperate(order) {
    return order.isDedicatedPostFactory;
}
function renderPostFinishingActionPanel(order) {
    const actions = [];
...
function listUnifiedPostFinishingMobileOperationRecords(order) {
...
function renderPostFinishingMobileOperationRecords(order) {
...
function renderPdaPostFinishingTaskPage(execId, task) {
...
function renderPdaPostFinishingExecutionPage(execId, order) {
...
function renderPdaSewingPostTaskPage(taskId, task) {
...
export function renderPdaExecDetailPage(taskId) {
    ...
    const milestoneProofTitle = milestone.proofRequirement === 'NONE'
        ? '关键节点凭证（当前配置：不要求凭证）'
        : `关键节点凭证（当前配置：${milestone.proofRequirementLabel}）`;
    const startProofTitle = startRule.proofRequirement === 'NONE'
        ? '开工凭证（当前配置：不要求凭证）'
        : `开工凭证（当前配置：${startRule.proofRequirementLabel}）`;
    const startProofHint = startRule.proofRequirement === 'NONE'
        ? '当前配置不要求开工凭证，可按现场需要补充上传'
        : `当前开工配置要求：${startRule.proofRequirementLabel}`;
    const startRiskText = startDueInfo.startRiskStatus === 'OVERDUE'
        ? '开工已逾期'
    ...
    const assignedFactory = task.assignedFactoryId
        ...
        : undefined;
    const pauseReasonLabel = task.pauseReasonLabel || '';
    const pauseReportedAt = task.pauseReportedAt || '';
    ...
    const taskQrValue = getTaskQrValue(task);
    const receiverDisplayText = getReceiverDisplayText(task);
        ...
        ? `
              <div class="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs">
                <div class="flex items-center gap-1.5 font-medium text-red-700">
                  ...
                  已上报暂停：${escapeHtml(task.pauseReasonLabel || blockReasonLabel(task.blockReason))}
                </div>
  ...
  `;
    const renderWoolWorkflowOperationButtons = () => woolOrder && woolWorkflowOperationActions.length
        ? `
          <div class="${woolWorkflowOperationActions.length > 1 ? 'grid grid-cols-2 gap-2' : 'space-y-2'}">
            ...
            .map((action) => `
                <button
                  type="button"
                  class="inline-flex h-9 w-full items-center justify-center rounded-md ${action.tone === 'primary'
            ? 'bg-primary text-primary-foreground hover:bg-primary/90'
            : 'border hover:bg-muted'} text-sm font-medium"
                  data-pda-execd-action="wool-workflow"
                  data-wool-workflow-action="${escapeHtml(action.code)}"
                  data-wool-order-id="${escapeHtml(woolOrder.woolOrderId)}"
                  ${action.nodeName ? `data-node-name="${escapeHtml(action.nodeName)}"` : ''}
        ...
        `
        : '';
    const renderDetailAuxiliaryButtons = () => {
        const buttons = [];
    ...
    <div class="space-y-4 bg-background p-4 pb-6">
      ...
      <article class="rounded-lg border bg-card">
        ...
        ? `
                    ...
                    <span class="text-xs font-medium ${deadline?.label === '执行逾期'
            ? 'text-red-700'
            : deadline?.label === '即将逾期'
                ? 'text-amber-700'
                : ''}">${escapeHtml(task.taskDeadline || '')}</span>
                  `
        : ''}
          </div>
      ...
      ${mobileTaskAccess.canOpenMobileExecution && printWorkOrder ? renderPrintingTaskCard(task, printWorkOrder, handoverOrder) : ''}
      ${mobileTaskAccess.canOpenMobileExecution && dyeWorkOrder ? renderDyeingTaskCard(task, dyeWorkOrder, handoverOrder) : ''}
      ${mobileTaskAccess.canOpenMobileExecution && woolOrder ? renderWoolTaskCard(task, woolOrder, handoverOrder) : ''}
...
export function handlePdaExecDetailEvent(target) {
    const fieldNode = target.closest('[data-pda-execd-field]');
    if (fieldNode instanceof HTMLInputElement ||
        ...
        if (field === 'pauseReasonCode' && fieldNode instanceof HTMLSelectElement) {
            detailState.pauseReasonCode = fieldNode.value;
            return true;
        }
    ...
    const actionNode = target.closest('[data-pda-execd-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'wool-workflow' && actionNode.dataset.woolOrderId && actionNode.dataset.woolWorkflowAction) {
        ...
        if (workflowAction === 'ACCEPT')
            acceptWoolWorkOrder(orderId, '工厂端操作员', operatedAt);
        if (workflowAction === 'CONFIRM_PICKUP')
            confirmWoolPickupRecord(orderId, '工厂端操作员', operatedAt);
        if (workflowAction === 'COMPLETE_PICKUP')
            completeWoolPickupHead(orderId, '工厂端操作员', operatedAt);
        if (workflowAction === 'SCHEDULE_MACHINE')
            scheduleWoolMachines(orderId, '工厂端排产员', operatedAt);
        if (workflowAction === 'START_FLAT') {
            ...
            if (yarnUsageWeightKg === null)
                return true;
            updateWoolWorkOrderNodeStatus(orderId, '横机成片', '进行中', '工厂端操作员', operatedAt, { yarnUsageWeightKg });
        }
        if (workflowAction === 'REPORT_FLAT_MILESTONE')
            updateWoolWorkOrderNodeStatus(orderId, '横机成片', '进行中', '工厂端操作员', operatedAt);
        if (workflowAction === 'COMPLETE_FLAT')
            updateWoolWorkOrderNodeStatus(orderId, '横机成片', '已完成', '工厂端操作员', operatedAt);
        if (workflowAction === 'START_LINKING')
            updateWoolWorkOrderNodeStatus(orderId, '缝盘', '进行中', '工厂端操作员', operatedAt);
        ...
        if (workflowAction === 'START_IRONING')
            updateWoolWorkOrderNodeStatus(orderId, '熨烫', '进行中', '工厂端操作员', operatedAt);
        if (workflowAction === 'COMPLETE_IRONING')
            updateWoolWorkOrderNodeStatus(orderId, '熨烫', '已完成', '工厂端操作员', operatedAt);
        if (workflowAction === 'START_PACKING')
            updateWoolWorkOrderNodeStatus(orderId, '包装', '进行中', '工厂端操作员', operatedAt);
        if (workflowAction === 'COMPLETE_PACKING')
            updateWoolWorkOrderNodeStatus(orderId, '包装', '已完成', '工厂端操作员', operatedAt);
        if (workflowAction === 'SKIP_PACKING')
            updateWoolWorkOrderNodeStatus(orderId, '包装', '已跳过', '工厂端操作员', operatedAt);
        if (workflowAction === 'PRINT_FEI_TICKET')
            markWoolFeiTicketsPrinted(orderId, '工厂端操作员', operatedAt);
        if (workflowAction === 'SUBMIT_HANDOVER')
            submitWoolHandover(orderId, '工厂端操作员', operatedAt);
        if (workflowAction === 'CONFIRM_HANDOVER_RECEIPT')
            confirmWoolHandoverReceipt(orderId, '接收仓库', operatedAt);
        showPdaExecDetailToast('毛织任务状态已更新，Web端同步更新');
        return true;
    }
    if ((action === 'wool-node-start' || action === 'wool-node-complete' || action === 'wool-node-skip')
        && actionNode.dataset.woolOrderId
        && actionNode.dataset.nodeName) {
        const nextStatus = action === 'wool-node-start' ? '进行中' : action === 'wool-node-complete' ? '已完成' : '已跳过';
    ...
    if (action === 'view-handover-order' || action === 'new-handover-record') {
        ...
        appStore.navigate(action === 'new-handover-record'
    ...
    if (action === 'dye-start-sample-wait'
        ...
        try {
            if (action === 'dye-start-sample-wait') {
                const waitTypeText = window.prompt('请输入等待类型：样衣 / 色样', dyeOrder.sampleWaitType === 'WAIT_COLOR_CARD' ? '色样' : '样衣')?.trim() || '';
            ...
            const nodeCode = actionNode.dataset.nodeCode;
            if (!nodeCode)
                return true;
            const nodeLabelMap = {
                DEHYDRATE: '脱水',
    ...
    if (action === 'special-bind-fei-ticket' ||
        ...
        try {
            ...
            const baseQty = getSpecialCraftPdaBaseQty(task, workOrder, sourceBinding, objectMeta) || 1;
            const finishQty = Math.max(baseQty - Number(detailState.specialCraftScrapQty || 0) - Number(detailState.specialCraftDamageQty || 0), 0);
            executeMobileProcessAction({
                sourceType: 'SPECIAL_CRAFT',
                sourceId,
                taskId,
    ...
    if (action === 'post-start-action'
        ...
        const actionType = actionNode.dataset.postActionType;
        if (!postOrderId)
            return true;
        try {
            ...
            const rejectedQty = actionType === '质检'
                ? Number(window.prompt('请输入不合格成衣件数', '0')?.trim() || '0')
                : 0;
            const acceptedQty = actionType === '复检'
                ? submittedQty
                : actionType === '质检'
    ...
    if (action === 'add-proof-image') {
        const scope = actionNode.dataset.proofScope || 'start';
    ...
    if (action === 'confirm-start') {
        ...
        const yarnUsageWeightKg = woolOrderForStart
            ? promptWoolKgValue('请输入本次纱线加工领用数量（kg）', getWoolYarnUsageSummary(woolOrderForStart).processingUsageWeightKg
                || woolOrderForStart.yarnReceipt.receivedWeightKg
                || woolOrderForStart.yarnReceipt.plannedWeightKg, false)
            : undefined;
        if (yarnUsageWeightKg === null)
            return true;
        ...
        try {
            ...
            const updatedTask = getTaskFactById(taskId);
    ...
    if (action === 'report-milestone') {
        ...
        if (!isTaskMilestoneProofSatisfied(task, detailState.milestoneProofFiles)) {
            const milestone = getTaskMilestoneState(task);
            const proofHint = milestone.proofRequirement === 'IMAGE'
                ? '请至少上传 1 项关键节点图片凭证'
