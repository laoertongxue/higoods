...
function renderPrepRecordActions(record: MaterialPrepRecord): string {
  ...
  switch (record.recordStatus) {
    ...
    case 'CONFIRMED':
      return `<span class="rounded-md bg-emerald-50 px-3 py-1.5 text-xs text-emerald-700">已确认，可车缝领料</span>`
    case 'REJECTED':
      return `
  ...
  return `
    <section class="rounded-lg border bg-card p-4">
      ...
      <div class="mt-2 rounded-md border border-dashed bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
        配料记录按 DRAFT → PICKED → STAGED → CONFIRMED 流转；打回后可从 STAGED 重新确认；已确认的配料可被车缝领料；每条记录整体确认，记录内物料明细不单独确认。
      </div>
