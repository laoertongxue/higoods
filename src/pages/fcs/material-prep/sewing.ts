...
const categoryLabel = '车缝配料'
const pageBasePath = '/fcs/material-prep/sewing'
...
function renderPrepRecordActions(record: MaterialPrepRecord): string {
  ...
  switch (record.recordStatus) {
    ...
    case 'CONFIRMED':
      return `<span class="rounded-md bg-emerald-50 px-3 py-1.5 text-xs text-emerald-700">已确认，可车缝接收</span>`
    case 'REJECTED':
      return `
...
function renderDetailTabs(projection: MaterialPrepOrderProjection, activeTab: MaterialPrepDetailTab): string {
  const tabs: Array<{ key: MaterialPrepDetailTab; label: string; count?: string }> = [
    ...
    { key: 'pickup', label: '与配料记录关联的接收记录', count: `${projection.pickupRecords.length + projection.rejectRecords.length} 条` },
  ]
  return `
...
function renderProductionDemand(projection: MaterialPrepOrderProjection): string {
  ...
  return `
    ...
    ${renderCutPieceReleaseCard(projection)}
...
function renderPickupRecords(projection: MaterialPrepOrderProjection): string {
  ...
  return `
    <section class="rounded-lg border bg-card p-4">
      <h3 class="text-base font-semibold">与配料记录关联的接收记录</h3>
      <div class="mt-3 grid gap-3 text-sm lg:grid-cols-2">
        <div class="rounded-md border bg-muted/20 px-3 py-2">
          ...
          <div class="divide-y">
            ${records.length ? records.map((record) => `
              <div class="px-3 py-3 text-sm">
                ...
                <div class="mt-1 text-xs text-muted-foreground">接收：${formatQty(record.pickedQty)} / ${record.rollCount} 卷 / ${escapeHtml(record.receiverName)} / ${escapeHtml(record.pickedAt)}</div>
    ...
    <div class="space-y-5 p-6">
      <header>
        <div>
          ...
          <p class="mt-2 text-sm text-muted-foreground">按生产单组织${escapeHtml(categoryLabel)}配料，让配料人员知道哪些无库存可配、哪些部分有库存可配、哪些全部都有充足库存、哪些被打回重配、哪些已配齐。车缝配料数量受裁片放行约束限制。</p>
        </div>
      </header>
...
export function handleFcsSewingPrepEvent(target: HTMLElement): boolean {
