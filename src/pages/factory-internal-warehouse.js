...
import { createFactoryWarehouseArea, createFactoryWarehouseLocation, createFactoryWarehouseShelf, createFactoryWarehouseStocktakeOrder, completeFactoryWarehouseStocktakeOrder, approveFactoryWarehouseStocktakeDifferenceReview, executeFactoryWarehouseAdjustmentOrder, findFactoryInternalWarehouseById, findFactoryWarehouseInboundRecordBySourceRecordId, findFactoryWarehouseOutboundRecordByHandoverRecordId, getFactoryWarehouseFilterStatusOptions, getFactoryWarehouseInboundSourceLabel, getFactoryWarehouseKindOptions, getFactoryWarehouseOutboundSourceLabel, getFactoryWarehousePositionLabel, getFactoryWarehouseProcessOptions, getFactoryWarehouseSourceRecordTypeLabel, getFactoryWarehouseSummary, getFactoryWarehouseTimeRangeOptions, listFactoryInternalWarehouseFactoryOptions, listFactoryInternalWarehouses, listFactoryWaitHandoverStockItems, listFactoryWaitProcessStockItems, listFactoryWarehouseInboundRecords, listFactoryWarehouseNodeRows, listFactoryWarehouseOutboundRecords, listFactoryWarehouseAdjustmentOrdersByStocktake, listFactoryWarehouseStocktakeDifferenceReviewsByOrder, listFactoryWarehouseStocktakeOrders, rejectFactoryWarehouseStocktakeDifferenceReview, toggleFactoryWarehouseNodeStatus, updateFactoryWarehouseNodeRemark, updateFactoryWarehouseStocktakeLine, updateWaitHandoverStockLocation, updateWaitProcessStockLocation, } from '../data/fcs/factory-internal-warehouse';
import { buildHandoverOrderLink, buildHandoverRecordLink, buildProductionOrderLink, buildTaskDetailLink, } from '../data/fcs/fcs-route-links';
...
function getStatusTone(label) {
    if (label.includes('差异') || label.includes('异议'))
        return 'red';
    if (label.includes('待交出') || label.includes('待确认') || label.includes('待领料'))
        return 'amber';
    if (label.includes('已交出') || label.includes('已出库'))
        return 'blue';
    if (label.includes('已回写') || label.includes('已完成') || label.includes('已入'))
        return 'green';
    return 'slate';
}
...
function withinTimeRange(value, timeRange) {
    if (timeRange === 'ALL')
        return true;
...
function matchesWarehouseFilter(warehouseId, warehouseMap) {
    if (!state.filters.warehouseKind)
        return true;
    const warehouse = warehouseMap.get(warehouseId);
    return warehouse?.warehouseKind === state.filters.warehouseKind;
}
function matchesCommonFactoryFilters(input, warehouseMap) {
    if (state.filters.factoryId && input.factoryId !== state.filters.factoryId)
        return false;
    if (state.filters.processCode && input.processCode !== state.filters.processCode)
        return false;
    if (input.warehouseId && !matchesWarehouseFilter(input.warehouseId, warehouseMap))
        return false;
    if (!matchesKeyword(input.keywordTokens, state.filters.keyword.trim().toLowerCase()))
        return false;
    if (!withinTimeRange(input.timeValue, state.filters.timeRange))
        return false;
    return true;
}
function getWaitProcessRows() {
    const warehouseMap = getWarehouseMap();
    return listFactoryWaitProcessStockItems().filter((item) => {
        if (!matchesCommonFactoryFilters({
...
function getInboundRows() {
    const warehouseMap = getWarehouseMap();
    return listFactoryWarehouseInboundRecords().filter((item) => matchesCommonFactoryFilters({
        factoryId: item.factoryId,
        warehouseId: item.warehouseId,
        processCode: item.processCode,
...
function getOutboundRows() {
    const warehouseMap = getWarehouseMap();
    return listFactoryWarehouseOutboundRecords().filter((item) => matchesCommonFactoryFilters({
        factoryId: item.factoryId,
        warehouseId: item.warehouseId,
        processCode: item.processCode,
...
function getLocationRows() {
    ...
    return listFactoryWarehouseNodeRows(state.filters.factoryId || undefined).filter((item) => {
        ...
        if (state.filters.processCode) {
            ...
            if (!processCodes.has(state.filters.processCode))
                return false;
        }
        if (state.filters.warehouseKind && warehouse.warehouseKind !== state.filters.warehouseKind)
            return false;
        if (!matchesKeyword([item.factoryName, item.warehouseName, item.areaName, item.shelfNo, item.locationNo, item.remark], state.filters.keyword.trim().toLowerCase()))
            return false;
        return true;
    });
}
function getStocktakeRows() {
    ...
    return listFactoryWarehouseStocktakeOrders().filter((item) => {
        if (state.filters.factoryId && item.factoryId !== state.filters.factoryId)
            return false;
        if (state.filters.warehouseKind && item.warehouseKind !== state.filters.warehouseKind)
            return false;
        if (!matchesKeyword([item.stocktakeOrderNo, item.factoryName, item.warehouseName, item.createdBy], state.filters.keyword.trim().toLowerCase()))
            return false;
        if (!withinTimeRange(item.createdAt, state.filters.timeRange))
            return false;
...
function getTargetWarehouseForMaintenance() {
    if (!state.filters.factoryId)
        return null;
...
function renderSummaryCards() {
    ...
    const progressSummary = progressSnapshots.reduce((result, item) => {
    ...
    const cards = [
        { label: '待加工数量', value: formatQty(progressSummary.waitProcessQty || summary.waitProcessQty), tone: 'green' },
        { label: '待交出数量', value: formatQty(progressSummary.waitHandoverQty || summary.waitHandoverQty), tone: 'blue' },
        { label: '今日入库', value: formatQty(progressSummary.todayInboundQty), tone: 'green' },
        { label: '今日出库', value: formatQty(progressSummary.todayOutboundQty), tone: 'violet' },
        { label: '入库差异', value: String(progressSummary.inboundDifferenceCount), tone: 'red' },
        { label: '出库差异', value: String(progressSummary.outboundDifferenceCount), tone: 'red' },
        { label: '异议中', value: String(progressSummary.objectionCount), tone: 'red' },
        { label: '盘点差异', value: String(progressSummary.stocktakeDifferenceCount || summary.stocktakeDifferenceCount), tone: 'amber' },
        { label: '待审核差异', value: String(progressSummary.stocktakeWaitReviewCount || summary.stocktakeWaitReviewCount), tone: 'amber' },
        { label: '已调整', value: String(progressSummary.stocktakeAdjustedCount || summary.stocktakeAdjustedCount), tone: 'green' },
        { label: '超时未处理', value: String(progressSummary.overdueCount), tone: 'amber' },
    ];
    return `
...
function renderTableWrapper(headers, body, minWidthClass = 'min-w-[1600px]') {
    return `
...
function getWaitProcessSourceActionLabel(item) {
    return item.sourceRecordType === 'HANDOVER_RECEIVE' ? '交出接收' : '领料确认';
}
function getWaitProcessSourceStatusLabel(item) {
    if (item.status === '差异待处理')
        return '差异待处理';
    if (item.status === '待领料')
        return '待领料';
    return '已确认';
}
function getWaitHandoverWritebackStatusLabel(item) {
    if (item.status === '异议中')
        return '异议中';
    if (item.status === '差异')
        return '差异';
    if (item.status === '已回写')
        return '已回写';
    if (typeof item.receiverWrittenQty === 'number')
        return '已回写';
    return '未回写';
}
function getOutboundQrDisplayText(value) {
    return value ? '已生成' : '—';
}
function renderLinkedRecordButton(action, recordId, label, emptyText = '—') {
    if (!recordId || !label)
...
function renderWaitHandoverTab() {
...
function renderOutboundTab() {
    ...
    const body = rows.length === 0
        ...
        : rows
            .map((item) => {
            const handoverHref = item.handoverOrderId ? buildHandoverOrderLink(item.handoverOrderId) : '/fcs/pda/handover';
            const recordHref = item.handoverOrderId && item.handoverRecordId ? buildHandoverRecordLink(item.handoverOrderId, item.handoverRecordId) : handoverHref;
            return `
              <tr class="border-b last:border-b-0">
...
function renderLocationTab() {
    ...
    const body = rows.length === 0
        ...
        : rows
            .map((item) => {
            const addButton = item.rowType === 'AREA'
...
function renderActiveTab() {
    const content = state.activeTab === 'WAIT_PROCESS'
...
function renderWaitProcessDetail(item) {
    const inboundRecord = findFactoryWarehouseInboundRecordBySourceRecordId(item.sourceRecordId);
    return `
    <div class="space-y-4">
...
function getLocationOptionsFromDrawer(warehouseId, areaName, shelfNo) {
...
function renderLocationSelect(label, field, value, options) {
    return `
    <label class="space-y-2">
...
function renderLocationDrawer(title, warehouseId, areaName, shelfNo, locationNo, saveAction) {
...
function renderWaitHandoverDetail(item) {
    const outboundRecord = item.handoverRecordId ? findFactoryWarehouseOutboundRecordByHandoverRecordId(item.handoverRecordId) : undefined;
    return `
    <div class="space-y-4">
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
...
function renderOutboundDetail(record) {
    return `
    <div class="space-y-4">
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
...
function renderNodeRemarkDrawer(drawerState) {
    return renderDetailDrawer({
...
function renderStocktakeDetail(order) {
...
export function renderFactoryInternalWarehousePage() {
    return `
...
function updateDrawerLocationField(field, value) {
    if (state.drawer.type !== 'WAIT_PROCESS_POSITION' && state.drawer.type !== 'WAIT_HANDOVER_POSITION')
        return;
...
function toggleNodeStatus(target) {
    const rowType = target.dataset.rowType;
    const warehouseId = target.dataset.warehouseId;
    const areaId = target.dataset.areaId;
    if (!rowType || !warehouseId || !areaId)
...
function handleFieldUpdate(field, node) {
    ...
    if (field === 'warehouseKind' && node instanceof HTMLSelectElement) {
        state.filters.warehouseKind = node.value;
        return true;
    }
    ...
    if (field === 'timeRange' && node instanceof HTMLSelectElement) {
        state.filters.timeRange = node.value;
        return true;
    }
...
export function handleFactoryInternalWarehouseEvent(target) {
    const fieldNode = target.closest('[data-factory-warehouse-field]');
    if (fieldNode) {
    ...
    const actionNode = target.closest('[data-factory-warehouse-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'switch-tab') {
        state.activeTab = actionNode.dataset.tab || 'WAIT_PROCESS';
        state.notice = '';
        return true;
    }
    ...
    if (action === 'save-node-remark' && state.drawer.type === 'NODE_REMARK') {
        const success = updateFactoryWarehouseNodeRemark(state.drawer.rowType, {
...
export function isFactoryInternalWarehouseDialogOpen() {
