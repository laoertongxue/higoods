...
import { PDA_MOCK_BIDDING_TENDERS, PDA_MOCK_QUOTED_TENDERS, } from '../data/fcs/pda-mobile-mock';
import { getTaskProcessDisplayName, getTaskStageDisplayName, } from '../data/fcs/page-adapters/task-execution-adapter';
import { getPdaTaskFlowTaskById, isCuttingSpecialTask, resolvePdaTaskExecPath, } from '../data/fcs/pda-cutting-execution-source.ts';
import { getPdaMobileExecutionTaskById, listPdaMobileExecutionTasks, } from '../data/fcs/process-mobile-task-binding.ts';
import { canFactoryAccessSpecialCraftPdaTask } from '../data/fcs/special-craft-pda-scope.ts';
import { acceptPostFinishingTask, rejectPostFinishingTask, } from '../data/fcs/post-finishing-domain.ts';
import { renderPdaFrame } from './pda-shell';
import { buildPdaCuttingDirectExecEntryHref, buildPdaCuttingTaskDetailNavHref, } from './pda-cutting-nav-context';
...
function getReceiveDetailSearchParams() {
    const pathname = appStore.getState().pathname;
    const [, queryString = ''] = pathname.split('?');
    return new URLSearchParams(queryString);
}
function resolveReceiveBackHref() {
...
function getReceiveDetailTab(task) {
    const returnTo = getReceiveDetailSearchParams().get('returnTo');
    if (returnTo) {
...
function renderSectionCard(title, icon, body) {
    return `
...
function showTaskReceiveDetailToast(message) {
...
function getTaskPricing(task) {
    ...
    const priceStatusColor = priceStatus === '按标准价派单'
...
function renderCuttingTaskRollupCard(task) {
    return `
    <article class="rounded-lg border bg-card">
      <header class="border-b px-4 py-3">
...
function renderCuttingTaskActionCard(task) {
    ...
    return renderSectionCard('进入处理', 'arrow-right-circle', `
...
function renderRejectDialog(taskId) {
...
function renderTaskStyleCard(task) {
...
function renderReceiveStatusChips(task, tab) {
    const sceneBadge = tab === 'pending-quote'
    ...
    <div class="flex flex-wrap gap-2">
      ...
      ${task.assignmentStatus
        ? renderBadge(task.assignmentStatus === 'ASSIGNED'
            ? '已分配'
            : task.assignmentStatus === 'AWARDED'
                ? '已中标'
                : task.assignmentStatus === 'BIDDING'
                    ? '竞价中'
                    : task.assignmentStatus, 'border-border bg-background text-muted-foreground')
        : ''}
      ${task.acceptanceStatus
        ? renderBadge(task.acceptanceStatus === 'ACCEPTED'
            ? '已接单'
            : task.acceptanceStatus === 'REJECTED'
                ? '已拒单'
                : '待接单', task.acceptanceStatus === 'ACCEPTED'
...
function renderReceiveSpecificSection(task, tab) {
    ...
    const execStatus = task.status === 'DONE'
        ? '已完工'
    ...
    if (tab === 'pending-quote' && pendingQuoteTender) {
        return renderSectionCard('招标信息', 'gavel', `
        <div class="grid grid-cols-2 gap-3">
    ...
    if (tab === 'quoted' && quotedTender) {
        return renderSectionCard('已报价信息', 'badge-cent', `
        <div class="grid grid-cols-2 gap-3">
    ...
    if (tab === 'awarded') {
        return renderSectionCard('中标信息', 'award', `
        <div class="grid grid-cols-2 gap-3">
    ...
    return renderSectionCard('直接派单信息', 'clipboard-list', `
      <div class="grid grid-cols-2 gap-3">
...
function renderPdaTaskReceiveCuttingDetailPage(task) {
    const factory = task.assignedFactoryId
        ? getFactoryMasterRecordById(task.assignedFactoryId)
        : undefined;
    const factoryDisplayName = formatFactoryDisplayName(factory?.name || task.assignedFactoryName, factory?.code || factory?.id || task.assignedFactoryId);
...
export function renderPdaTaskReceiveDetailPage(taskId) {
    if (!getPdaRuntimeContext()) {
        return renderPdaLoginRedirect();
    }
    const task = getTaskFactById(taskId);
    if (isCuttingSpecialTask(task)) {
        return renderPdaTaskReceiveCuttingDetailPage(task);
    }
    if (!task) {
        const content = `
      <div class="flex min-h-[760px] flex-col bg-background">
...
export function handlePdaTaskReceiveDetailEvent(target) {
    if (!ensurePdaSessionForAction())
        return true;
    const fieldNode = target.closest('[data-pda-trd-field]');
    ...
    const actionNode = target.closest('[data-pda-trd-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'confirm-reject') {
        const taskId = actionNode.dataset.taskId;
        if (!taskId || !state.rejectReason.trim())
            return true;
        const factoryName = getFactoryName(getCurrentFactoryId());
