...
import { formalizeStyleArchive, getStyleArchiveFormalizationCheck, } from '../data/pcs-project-style-archive-generation.ts';
import { activateTechPackVersionForStyle, createManualTechnicalDataVersionDraftFromCurrent, } from '../data/pcs-project-technical-data-writeback.ts';
import { listProjectChannelProductsSnapshot, } from '../data/pcs-channel-product-project-repository.ts';
import { buildSkuFixture } from '../data/pcs-product-archive-fixtures.ts';
import { STYLE_ARCHIVE_STATUS_RULES, isStyleArchiveFormalized, resolveStyleArchiveBusinessStatus, } from '../data/pcs-product-lifecycle-governance.ts';
import { getStyleArchiveById, listStyleArchives, updateStyleArchive } from '../data/pcs-style-archive-repository.ts';
import { createSkuArchive, createSkuArchiveBatch, findSkuArchiveByCode, getSkuArchiveById, listSkuArchives, listSkuArchivesByStyleId, updateSkuArchive, } from '../data/pcs-sku-archive-repository.ts';
import { buildTechnicalVersionListByStyle } from '../data/pcs-technical-data-version-view-model.ts';
import { listTechnicalDataVersions, listTechnicalDataVersionsByStyleId, } from '../data/pcs-technical-data-version-repository.ts';
import { listProjectWorkspaceColors, listProjectWorkspaceSizes, } from '../data/pcs-project-config-workspace-adapter.ts';
const STYLE_DETAIL_TABS = [
...
function createDefaultStyleCompletionState() {
    return {
        open: false,
        styleId: '',
        styleName: '',
...
function createDefaultManualTechPackVersionState() {
    return {
        open: false,
        styleId: '',
    };
...
function resetSkuCreateState() {
    state.skuCreate = createDefaultSkuCreateState();
}
function resetStyleCompletionState() {
...
function resetManualTechPackVersionState() {
    state.manualTechPackVersion = createDefaultManualTechPackVersionState();
}
export function resetPcsProductArchiveState() {
    state.notice = null;
    state.styleList = {
        search: '',
        status: 'all',
...
function ensurePageDataReady() {
    if (productArchiveDataReady)
        return;
...
function normalizeTextToken(value, fallback) {
...
function buildAutoSkuCode(styleCode, color, size, print) {
...
function resolveLatestVersionMeta(styleId) {
...
function resolveStyleMappingHealth(skus) {
    if (skus.some((item) => item.mappingHealth === 'CONFLICT'))
        return 'CONFLICT';
    if (skus.some((item) => item.mappingHealth === 'MISSING') || skus.length === 0)
        return 'MISSING';
    return 'OK';
}
function groupByStyleId(records) {
    ...
    records.forEach((record) => {
        ...
        if (current)
            ...
            grouped.set(record.styleId, [record]);
    });
    return grouped;
}
function groupChannelProductsByProjectId(records) {
    const grouped = new Map();
    records.forEach((record) => {
        if (!record.projectId)
            return;
        ...
        if (current)
            ...
            grouped.set(record.projectId, [record]);
    });
    return grouped;
}
function groupTechnicalVersionsByStyleId(records) {
...
function hasPersistedSkuArchiveStore() {
    return (typeof localStorage !== 'undefined' &&
        typeof localStorage.getItem === 'function' &&
        Boolean(localStorage.getItem(SKU_ARCHIVE_STORAGE_KEY)));
}
function buildSkuSummariesByStyleId(styles) {
...
function buildProductArchiveListContext() {
    ensurePageDataReady();
...
function listStyleChannelProducts(style, context) {
    ...
    const matchedRecords = records.filter((item) => item.styleId === style.styleId || (!!style.sourceProjectId && item.projectId === style.sourceProjectId));
    const uniqueRecords = new Map(matchedRecords.map((item) => [item.channelProductId, item]));
    return Array.from(uniqueRecords.values());
}
function countOnSaleChannelProducts(records) {
    return records.filter((item) => item.channelProductStatus === '已生效' && item.upstreamSyncStatus === '已更新').length;
}
function buildStyleListItems(context = buildProductArchiveListContext()) {
    return context.styles.map((style) => {
        ...
        const currentVersion = versions.find((item) => item.technicalVersionId === style.currentTechPackVersionId) || versions[0] || null;
...
function filterStyleItems(items) {
    ...
    return items.filter((item) => {
        ...
        if (state.styleList.status !== 'all' && item.displayStatus !== state.styleList.status)
            return false;
        if (state.styleList.version === 'has' && !item.hasEffectiveTechPack)
            return false;
        if (state.styleList.version === 'none' && item.hasEffectiveTechPack)
            return false;
        if (state.styleList.mapping !== 'all' && item.mappingHealth !== state.styleList.mapping)
            return false;
        return true;
    });
}
function getFilteredStyleItems() {
    return filterStyleItems(buildStyleListItems());
}
function getStyleStats(items) {
    return {
        total: items.length,
...
function getFilteredSkuItems() {
    ...
    return listSkuArchives().filter((item) => {
        ...
        if (state.skuList.status !== 'all' && item.archiveStatus !== state.skuList.status)
            return false;
        if (state.skuList.mapping !== 'all' && item.mappingHealth !== state.skuList.mapping)
            return false;
        if (state.skuList.styleId && item.styleId !== state.skuList.styleId)
            return false;
        return true;
    });
}
...
function renderStatusBadge(status, type) {
    const meta = type === 'style' ? STYLE_ARCHIVE_STATUS_RULES[status] : SKU_STATUS_META[status];
    return renderBadge(meta.label, meta.className);
}
function renderStyleLifecycleBadge(style) {
    return renderStatusBadge(resolveStyleArchiveBusinessStatus(style), 'style');
}
function renderMappingBadge(health) {
    const meta = MAPPING_META[health];
    return renderBadge(meta.label, meta.className);
}
function renderMetricButton(title, value, description, action, extraData = '') {
    const clickable = action !== 'noop';
    const className = toClassName('rounded-lg border bg-white px-4 py-3 text-left shadow-sm', clickable && 'hover:border-slate-300 hover:bg-slate-50');
    return `
...
function renderFormField(label, control, required = false, hint = '') {
    return `
...
function renderReadonlyValue(value, placeholder = '—') {
...
function renderCheckbox(field, checked, label, extraData = '') {
    return `
...
function formatCurrency(value, currency = '人民币') {
    if (!Number.isFinite(value))
        return '-';
    if (currency === '美元')
        return `$${value.toFixed(2)}`;
    if (currency === '印尼盾')
        return `Rp${Math.round(value).toLocaleString('zh-CN')}`;
    return `¥${value.toFixed(2)}`;
}
...
function renderStyleImagePreviewButton(url, title, size = 'md', highlighted = false) {
    if (!url)
        return renderArchiveImage(url, title, size);
    return `
    <button
      type="button"
...
function renderStyleStats(items) {
...
function renderStyleFilters(total) {
    return `
...
function renderStyleTable(items) {
    const rows = items
        .map((item) => `
...
function openStyleCompletionDrawer(style) {
...
function renderStyleFormalizationPanel(style) {
    ...
    <section class="rounded-lg border bg-white p-5 shadow-sm">
      ...
      <div class="mt-4 grid gap-4 lg:grid-cols-[1fr,1fr]">
        ...
        .map((item) => renderBadge(item.label, missingCount > 0 ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'))
        .join('')}
          </div>
        </div>
      </div>
...
function renderStyleCompletionDrawer() {
    if (!state.styleCompletion.open)
        return '';
    ...
    const renderControlledTextField = (label, field, value, placeholder, required = false) => renderFormField(label, alreadyFormalized ? renderReadonlyValue(value) : renderTextInput(field, value, placeholder), required, alreadyFormalized ? '正式建档后只读' : '');
    const renderControlledTextareaField = (label, field, value, placeholder, rows, required = false) => renderFormField(label, alreadyFormalized ? renderReadonlyValue(value) : renderTextarea(field, value, placeholder, rows), required, alreadyFormalized ? '正式建档后只读' : '');
...
function submitStyleCompletion() {
...
function renderSkuFilters(total) {
...
function renderSkuCreateDrawer() {
    if (!state.skuCreate.open)
        return '';
...
function renderStyleDetailOverview(style) {
    ...
    const alreadyFormalized = isStyleArchiveFormalized(style);
    const currentTechPackHref = style.currentTechPackVersionId && style.currentTechPackVersionCode
...
function renderStyleDetailVersions(style) {
...
function renderManualTechPackVersionDrawer() {
    if (!state.manualTechPackVersion.open)
        return '';
...
function renderStyleDetailSpecifications(style) {
    const skus = listSkuArchivesByStyleId(style.styleId);
    const rows = skus
        .map((item) => `
        <tr class="border-t border-slate-100 align-top">
...
function renderStyleDetailMappings(style) {
...
function renderStyleDetailChannels(style) {
...
function renderStyleDetailLogs(style) {
    const logs = [
...
function renderStyleDetailPage(styleId) {
    ensurePageDataReady();
    ...
    const tabButtons = STYLE_DETAIL_TABS.map((tab) => `
    ...
    `).join('');
    const tabContent = state.styleDetail.activeTab === 'overview'
...
function buildSkuChannelMappingRows(sku) {
...
function renderSkuDetailOverview(sku) {
    return `
...
function renderSkuDetailMappings(sku) {
...
function renderSkuDetailVariants(sku) {
    const rows = buildSkuChannelMappingRows(sku)
        .map((item) => `
        <tr class="border-t border-slate-100 align-top">
          <td class="px-4 py-3 text-sm text-slate-700">${escapeHtml(item.channel)}</td>
...
function renderSkuDetailCodeMappings(sku) {
    const rows = [
        {
            type: '老系统 SKU',
...
function renderSkuDetailLogs(sku) {
    const logs = [
...
function renderSkuDetailPage(skuId) {
    ensurePageDataReady();
    ...
    const tabButtons = SKU_DETAIL_TABS.map((tab) => `
    ...
    `).join('');
    const tabContent = state.skuDetail.activeTab === 'overview'
...
function renderSkuStyleLink(sku) {
...
function renderPcsStyleListPage() {
...
export function renderPcsStyleArchiveListPage() {
    return renderPcsStyleListPage();
}
export function renderPcsStyleArchiveDetailPage(styleId) {
    return renderStyleDetailPage(styleId);
}
export function renderPcsSpecificationListPage() {
    return renderPcsSkuListPage();
}
export function renderPcsSpecificationDetailPage(skuId) {
...
function submitSkuCreate() {
    ...
    const skuCode = state.skuCreate.mode === 'import'
    ...
    try {
        const created = createSkuArchive(buildSkuRecord({
            style,
...
function submitSkuBatchCreate() {
    ...
    const records = state.skuCreate.batchColors.flatMap((color) => state.skuCreate.batchSizes
        .map((size) => {
        const skuCode = buildAutoSkuCode(style.styleCode, color, size, state.skuCreate.batchPrint);
        if (findSkuArchiveByCode(skuCode))
            return null;
...
function openManualTechPackVersionDrawer(style) {
    state.manualTechPackVersion = {
        open: true,
        styleId: style.styleId,
    };
}
function submitManualTechPackVersion() {
    const style = state.manualTechPackVersion.styleId ? getStyleArchiveById(state.manualTechPackVersion.styleId) : null;
    if (!style) {
        state.notice = '请选择款式。';
        return;
...
function resolveClosestNode(target, selector) {
...
function resolveFieldValue(target) {
    const input = target;
    return {
...
function toggleBatchValue(list, value, checked) {
    if (checked) {
        return list.includes(value) ? list : [...list, value];
    }
    return list.filter((item) => item !== value);
}
function handleStyleImageUpload(target) {
    const input = target;
...
export function handlePcsProductArchiveInput(target) {
    ...
    switch (field) {
        ...
        case 'style-list-status':
            state.styleList.status = (value || 'all');
            return true;
        case 'style-list-version':
            state.styleList.version = (value || 'all');
            return true;
        case 'style-list-mapping':
            state.styleList.mapping = (value || 'all');
            return true;
        case 'sku-list-search':
            state.skuList.search = value;
            return true;
        case 'sku-list-status':
            state.skuList.status = (value || 'all');
            return true;
        case 'sku-list-mapping':
            state.skuList.mapping = (value || 'all');
            return true;
...
export function handlePcsProductArchiveEvent(target) {
    ...
    switch (action) {
        ...
        case 'open-sku-create':
            ...
            state.skuCreate.mode = actionNode.dataset.mode || 'single';
        ...
        case 'style-quick-filter': {
            ...
            if (filter === 'reset') {
                ...
                state.styleList.status = (actionNode.dataset.value || 'all');
            }
            else if (filter === 'version') {
                state.styleList.version = (actionNode.dataset.value || 'all');
            }
            else if (filter === 'mapping') {
                state.styleList.mapping = (actionNode.dataset.value || 'all');
            }
            return true;
        }
        case 'sku-quick-filter': {
            ...
            if (filter === 'reset') {
                ...
                state.skuList.status = (actionNode.dataset.value || 'all');
            }
            else if (filter === 'mapping') {
                state.skuList.mapping = (actionNode.dataset.value || 'all');
            }
            return true;
        }
        ...
        case 'toggle-sku-status': {
            ...
            const nextStatus = sku.archiveStatus === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE';
        ...
        case 'set-style-detail-tab':
            state.styleDetail.activeTab = actionNode.dataset.value || 'overview';
            return true;
        case 'set-sku-detail-tab':
            state.skuDetail.activeTab = actionNode.dataset.value || 'overview';
            return true;
...
export function isPcsProductArchiveDialogOpen() {
