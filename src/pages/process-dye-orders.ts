...
function renderSourceDetail(order: PrepProcessOrderFact): string {
  ...
  if (order.sourceType === 'STOCK') { rows.push(`<div><span class="text-muted-foreground">备货物料：</span>${escapeHtml(order.stockMaterial?.materialName || '-')}</div>`); return rows.join('') }
...
function installListColumnDragEvents(): void {
  ...
  document.addEventListener('dragstart', (event) => {
    const target = event.target instanceof Element ? event.target.closest<HTMLElement>('[data-process-dye-orders-root] [data-standard-list-column-drag]') : null
    if (!target) return
    draggedListColumnKey = target.dataset.dragSource || ''
    event.dataTransfer?.setData('text/plain', draggedListColumnKey)
  })
  document.addEventListener('dragover', (event) => { if (event.target instanceof Element && event.target.closest('[data-process-dye-orders-root] [data-drop-target]')) event.preventDefault() })
  document.addEventListener('drop', (event) => {
    const target = event.target instanceof Element ? event.target.closest<HTMLElement>('[data-process-dye-orders-root] [data-drop-target]') : null
  ...
  if (listAction) {
    ...
    if (action === 'toggle-column-visibility' || action === 'toggle-column-freeze') {
      ...
      if (column && !column.actionColumn) {
        const visibleKeys = action === 'toggle-column-visibility' && !column.required ? (state.preferences.visibleKeys.includes(key) ? state.preferences.visibleKeys.filter((item) => item !== key) : [...state.preferences.visibleKeys, key]) : state.preferences.visibleKeys
        const frozenKeys = action === 'toggle-column-freeze' && column.freezeable ? (state.preferences.frozenKeys.includes(key) ? state.preferences.frozenKeys.filter((item) => item !== key) : [...state.preferences.frozenKeys, key]) : state.preferences.frozenKeys
