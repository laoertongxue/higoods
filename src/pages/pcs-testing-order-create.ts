...
import { renderLabeledInput, renderLabeledSelect } from '../components/ui/form.ts'
...
const state = { styleId: '', buyerName: '', skuCodes: [] as string[] }
...
export function renderPcsTestingOrderCreatePage(): string {
  ...
  return `<div data-testing-create-page data-skip-page-rerender="true" class="mx-auto max-w-7xl space-y-4 p-4">
    <header class="flex items-center justify-between"><div><h1 class="text-xl font-semibold">新建测款单</h1><p class="mt-1 text-sm text-slate-500">关联商品档案，指定买手并选择本次测款规格。</p></div><a data-nav="/pcs/testing/orders" href="/pcs/testing/orders" class="text-sm text-slate-600 hover:text-blue-700">返回测款单列表</a></header>
    <section class="rounded-lg border bg-white p-5"><h2 class="mb-4 font-semibold">测款安排</h2><div class="grid gap-5 md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
      ...
      ${renderLabeledInput('买手', { id: 'testing-create-buyer', prefix: 'testing-create', field: 'buyer', value: state.buyerName, placeholder: '填写负责本次测款的买手', required: true }, true)}
...
export function handlePcsTestingOrderCreateInput(target: Element): boolean {
  ...
  if (field === 'buyer') state.buyerName = (target as HTMLInputElement).value
...
export function handlePcsTestingOrderCreateEvent(target: HTMLElement): boolean {
  ...
  if (!state.buyerName.trim()) { feedback.textContent = '请填写负责本次测款的买手。'; document.getElementById('testing-create-buyer')?.focus(); return true }
  ...
  const result = createTestingOrder({ styleId: state.styleId, buyerName: state.buyerName, skuCodes: state.skuCodes })
  if (!result.ok || !result.order) { feedback.textContent = result.message || '创建失败，请核对商品档案。'; return true }
  state.styleId = ''; state.buyerName = ''; state.skuCodes = []
