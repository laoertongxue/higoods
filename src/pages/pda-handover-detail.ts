...
function openWaterHandoverConfirm(head: PdaHandoverHead): { ok: boolean; message: string } {
  ...
  const order = getWaterSolubleWorkOrderByTaskId(head.taskId)
  const availableQty = Number(Math.max(0, (order?.completedQty || 0) - (order?.handoverQty || 0)).toFixed(6))
  if (!order || availableQty <= 0) return { ok: false, message: '当前没有已完成未交出的水溶物料。' }
...
function validateWaterHandoverConfirm(head: PdaHandoverHead, token: string | undefined): { ok: boolean; message: string } {
  ...
  if (
    ...
    || confirm.approvedQty !== Number(Math.max(0, (order?.completedQty || 0) - (order?.handoverQty || 0)).toFixed(6))
...
function nowTimestamp(date: Date = new Date()): string {
  return nowDisplayTimestamp(date)
}
...
function renderPdaHandoverSourceIdentity(head: PdaHandoverHead): string {
  const source = getPdaHandoverSourceDisplay(head)
  const businessTypeLabel = head.sourceBusinessType === 'WATER_SOLUBLE_WORK_ORDER'
    ? '水溶加工单'
    : head.sourceBusinessType === 'DYE_WORK_ORDER'
      ? '染色加工单'
      : ''
  ...
  return `${businessTypeRow}${typeRow}${renderFieldHtmlRow(source.label, renderPdaHandoverObjectCode({
...
function renderPickupCurrentPanel(
  ...
  const sourceQtyLabel = '仓库交付数量'
  const sourceTimeLabel = '仓库交付时间'
  const waitSourceText = '待仓库扫码交付'
  ...
  return `
    ...
    <div class="pt-1 text-xs text-blue-700">待仓库交付后确认接收。</div>
  `
}
  ...
  return `
    <article data-testid="pickup-record-card" class="space-y-2.5 rounded-lg border ${selected ? 'border-primary bg-primary/5 ring-1 ring-primary/10 shadow-sm' : 'bg-card shadow-sm'} p-3">
      ...
      <div data-testid="pickup-record-qr" class="flex flex-wrap items-start justify-between gap-2 text-xs">
        ...
        <p class="max-w-[220px] text-[11px] text-muted-foreground">仓库扫码对象固定为接收记录。</p>
      </div>
...
function renderPickupHeadDetail(head: PdaHandoverHead): string {
  const records = getPdaPickupRecordsByHead(head.handoverId)
  if (head.simpleCutPieceReceipt && records[0]?.simpleCutPiece) {
    const record = records[0], p = record.simpleCutPiece!
    return `<article class="space-y-3 rounded-xl border bg-white p-3" data-simple-cut-piece-receipt><h2 class="text-base font-semibold">裁片接收记录 · 已接收</h2><p class="text-sm text-emerald-700">仓库确认即 PPIC 和工厂已接收</p><div class="flex items-center gap-3"><button type="button" data-pda-image-preview-url="${escapeHtml(p.styleImageUrl)}" data-pda-image-preview-title="${escapeHtml(p.styleName)}"><img class="h-20 w-20 object-contain" src="${escapeHtml(p.styleImageUrl)}" alt="${escapeHtml(p.styleName)}" onload="this.nextElementSibling.hidden=true" onerror="this.hidden=true;this.nextElementSibling.hidden=false;this.nextElementSibling.textContent='图片加载失败，点击重试预览'"><span>图片加载中</span></button><div class="min-w-0 text-sm">${escapeHtml(p.styleCode)}<br>${escapeHtml(p.styleName)}</div></div><dl class="space-y-2 break-words text-xs"><div>记录号：${escapeHtml(p.handoverRecordNo)}</div><div>任务单：${escapeHtml(p.taskSheetNo)}</div><div>任务：${escapeHtml(p.taskNo)} · ${escapeHtml(p.taskTypeLabel)}</div><div>生产单：${escapeHtml(p.productionOrderNo)}</div><div>接收工厂：${escapeHtml(p.factoryName)}</div><div>领取 PPIC：${escapeHtml(p.ppicName)}</div><div>仓库操作人：${escapeHtml(record.warehouseHandedBy || '')} / ${escapeHtml(record.warehouseOperatorId || '')}</div><div>来源：${escapeHtml(record.sourceChannel || '')}</div><div>接收时间：${escapeHtml(record.receivedAt || '')}</div><div>本批：${p.tickets.length} 张菲票 / ${p.totalPieceQty} 片；不使用中转袋</div></dl><h3 class="font-semibold">本批菲票明细</h3>${p.tickets.map((t) => `<div class="rounded-lg border p-3 text-xs"><b>${escapeHtml(t.feiTicketNo)}</b><p class="mt-1">${escapeHtml(t.skuCode)} / ${escapeHtml(t.color)} / ${escapeHtml(t.size)} / ${escapeHtml(t.partName)}</p><p>编号范围：${escapeHtml(t.pieceRange)} / ${t.pieceQty} 片</p></div>`).join('')}${p.replacementFabricTickets?.length ? `<section class="space-y-2" data-replacement-fabric-receipt><h3 class="font-semibold">本批随交换片布</h3><p class="text-sm">${escapeHtml(mixedBagSummary(p.replacementFabricTickets))}</p>${p.replacementFabricTickets.map(ticket => renderMixedBagTicket({ ...ticket, sewingTaskId: p.runtimeTaskId, sewingTaskNo: p.taskNo, receiverFactoryId: p.factoryId, receiverFactoryName: p.factoryName })).join('')}</section>` : ''}<p class="text-xs text-slate-500">此处是本批接收结果，任务剩余数量与生产进度继续按任务记录计算。</p></article>`
  }
  ...
  return `
    ...
    <details class="rounded-xl border bg-card" data-testid="pickup-record-history">
      ...
      <div class="space-y-2 border-t p-3">${records.length ? records.map((record) => renderPickupRecordItem(record)).join('') : '<div class="py-4 text-center text-xs text-muted-foreground">暂无接收记录</div>'}</div>
    </details>
  `
}
  ...
  return `
    ...
    ${renderSectionCard(
      ...
      <div class="flex flex-wrap items-center justify-between gap-2 rounded-md border bg-muted/20 px-2.5 py-2 text-xs">
        ...
        ${
          isWoolHandover || (isCompleted && !completionCheck.ok) || (waterAccess !== null && !waterAccess.ok)
            ? ''
    ...
    <article class="rounded-xl border bg-card p-3 shadow-sm" data-testid="pda-handout-summary">
      ...
      <div class="mt-3 grid grid-cols-2 gap-x-3 gap-y-1 rounded-lg border bg-muted/15 px-2 py-2 text-xs">
        ...
        ${head.sourceBusinessType === 'WATER_SOLUBLE_WORK_ORDER'
          ? renderFieldRow('物料', `${head.materialName || '—'} / ${head.materialCode || '—'}`)
          : ''}
      </div>
      <div class="mt-3 inline-flex items-center rounded border border-border bg-background px-2 py-1 text-[10px]">交出物类型：${escapeHtml(profile.objectTypeLabel)}</div>
      <div class="mt-3 grid grid-cols-3 gap-2 rounded-lg bg-muted/30 px-2 py-2 text-center text-xs">
        <div><div class="text-[10px] text-muted-foreground">${escapeHtml(profile.primaryQtyLabel)}</div><div class="mt-1 font-semibold">${profile.totalPlannedQty} ${escapeHtml(profile.displayUnit)}</div></div>
        <div><div class="text-[10px] text-muted-foreground">${escapeHtml(profile.writtenQtyLabel)}</div><div class="mt-1 font-semibold">${profile.totalWrittenQty} ${escapeHtml(profile.displayUnit)}</div></div>
        <div><div class="text-[10px] text-muted-foreground">${escapeHtml(profile.pendingQtyLabel)}</div><div class="mt-1 font-semibold ${profile.totalPendingQty > 0 ? 'text-amber-700' : 'text-emerald-700'}">${profile.totalPendingQty} ${escapeHtml(profile.displayUnit)}</div></div>
      </div>
    </article>
...
export function renderPdaHandoverDetailPage(eventId: string): string {
  ...
  const runtime = getPdaRuntimeContext()
  if (
    (head.processBusinessCode === 'WOOL' || head.simpleCutPieceReceipt)
    ...
    const content = `
      <div class="space-y-4 p-4">
        ...
        <article class="rounded-lg border bg-card px-4 py-8 text-center text-sm text-muted-foreground">
          ${head.simpleCutPieceReceipt ? '该裁片接收记录' : '该毛织交出'}不属于当前登录工厂，不能查看或确认接收。
        </article>
      </div>
    `
    return renderPdaFrame(content, 'handover')
  ...
  const content = `
    <div class="space-y-3 bg-background p-4 pb-6">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <span class="text-sm font-semibold">${escapeHtml(head.simpleCutPieceReceipt ? '接收记录' : head.headType === 'PICKUP' ? '确认接收' : isPostReturnLinkOnlyView(head) ? '核对车缝回货' : '发起交出')}</span>
        </div>
        <div class="w-16"></div>
      </div>
...
export async function handlePdaHandoverDetailEvent(target: HTMLElement): Promise<boolean> {
  ...
  if (action === 'complete-handout-head') {
    ...
    const result = await markPdaHandoutHeadCompleted(handoverId, nowTimestamp(), getPdaRuntimeContext()?.userName)
    showPdaHandoverDetailToast(result.message)
    return true
  }
