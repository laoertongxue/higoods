...
function getAllItems() {
    ...
    const startDueItems = processTasks
        .filter((task) => task.taskId.startsWith('PDA-EXEC-') &&
        ...
        deadlineLabel: '开工时限',
        deadline: item.dueInfo.startDueAt,
        statusLabel: '待开工',
        riskNote: typeof item.dueInfo.remainingMs === 'number'
            ? `距开工时限不足 ${formatRemainingHours(item.dueInfo.remainingMs)} 小时`
            : '距开工时限不足 24 小时',
        href: `/fcs/pda/exec/${item.task.taskId}?action=start`,
    }));
    const execInProgressItems = processTasks
        .filter((task) => task.taskId.startsWith('PDA-EXEC-') &&
        task.assignedFactoryId === selectedFactoryId &&
        task.acceptanceStatus === 'ACCEPTED' &&
    ...
    const qualityDueSoonItems = listFutureMobileFactorySoonOverdueQcItems(selectedFactoryId).map((item) => ({
...
function getFilteredItems(items) {
    let list = state.activeCategory === '全部' ? items : items.filter((item) => item.category === state.activeCategory);
    const keyword = state.search.trim().toLowerCase();
    if (keyword) {
        list = list.filter((item) => {
            return (item.taskId?.toLowerCase().includes(keyword) ||
...
function renderFieldRow(label, value, urgent = false) {
    return `
...
function renderDueSoonCard(item) {
...
export function renderPdaNotifyDueSoonPage() {
    ...
    <div class="flex min-h-[760px] flex-col bg-muted/30">
      ...
      <section class="sticky top-[88px] z-10 border-b bg-background">
        ...
        return `
              <button
                class="shrink-0 whitespace-nowrap rounded-full border px-3 py-1.5 text-xs font-medium transition-colors ${toClassName(active
...
export function handlePdaNotifyDueSoonEvent(target) {
    if (!ensurePdaSessionForAction())
        return true;
    const fieldNode = target.closest('[data-pda-due-field]');
    ...
    const actionNode = target.closest('[data-pda-due-action]');
    if (!actionNode)
        return false;
