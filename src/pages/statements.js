...
import { buildStatementDraftLines, getStatementDetailViewModel, getStatementListItems, getStatementSourceItemById, listStatementBuildCandidates, listStatementBuildScopes, } from '../data/fcs/store-domain-statement-source-adapter';
import { createStatementFromEligibleLedgers, findOpenStatementByPartyAndCycle, getLatestStatementAppeal, getOpenStatementAppeal, getStatementSettlementProgressView, getStatementDraftById, initialStatementDrafts, resolveStatementAppeal, startStatementAppealHandling, syncStatementDraftFromBuild, } from '../data/fcs/store-domain-settlement-seeds';
...
function showStatementsToast(message, tone = 'success') {
...
function getFactoryFeedbackStatusLabel(status) {
    return FACTORY_FEEDBACK_LABEL[status];
}
function getFactoryFeedbackStatusBadge(status) {
    return FACTORY_FEEDBACK_BADGE[status];
}
function getFactoryAppealStatusLabel(status) {
    if (status === 'SUBMITTED')
        return '已提交';
...
function getResolutionResultLabel(result) {
    if (result === 'UPHELD')
        return '维持当前口径';
    if (result === 'REOPEN_REQUIRED')
        return '退回重算';
    return '当前未处理';
}
function getStatementAppealRecords(draft) {
    if (draft.appealRecords?.length)
        return draft.appealRecords;
    return draft.factoryAppealRecord ? [draft.factoryAppealRecord] : [];
}
function getStatementOverviewCounts(listItems, buildScopes) {
    return {
...
function getStatementPartyOptions(listItems) {
    return Array.from(new Map(listItems.map((item) => [item.settlementPartyId, item.settlementPartyLabel])).entries())
        .map(([value, label]) => ({ value, label }))
        .sort((left, right) => left.label.localeCompare(right.label, 'zh-CN'));
}
function getStatementCycleOptions(listItems) {
    return Array.from(new Map(listItems
        .filter((item) => item.settlementCycleId && item.settlementCycleLabel)
        .map((item) => [item.settlementCycleId, item.settlementCycleLabel])).entries())
        .map(([value, label]) => ({ value, label }))
        .sort((left, right) => right.label.localeCompare(left.label, 'zh-CN'));
}
function getFilteredStatementListItems(items) {
    const keyword = state.keyword.trim().toLowerCase();
    return items.filter((item) => {
        if (state.filterParty !== '__ALL__' && item.settlementPartyId !== state.filterParty)
            return false;
        if (state.filterCycle !== '__ALL__' && item.settlementCycleId !== state.filterCycle)
            return false;
        if (state.filterStatus !== '__ALL__' && item.status !== state.filterStatus)
            return false;
        if (state.filterFeedback !== '__ALL__' && item.factoryFeedbackStatus !== state.filterFeedback)
            return false;
        if (keyword) {
            const haystack = [
                item.statementId,
...
function getBuildFactoryOptions(scopes) {
    return Array.from(new Map(scopes.map((item) => [item.settlementPartyId, item.settlementPartyLabel])).entries())
        .map(([value, label]) => ({ value, label }))
        .sort((left, right) => left.label.localeCompare(right.label, 'zh-CN'));
}
function getBuildCycleOptions(scopes, factoryId) {
    return scopes.filter((item) => item.settlementPartyId === factoryId);
}
function getSelectedBuildScope(scopes) {
    if (!state.buildFactoryId || !state.buildCycleId)
        return null;
    return (scopes.find((item) => item.settlementPartyId === state.buildFactoryId && item.settlementCycleId === state.buildCycleId) ?? null);
}
function getEditingDraft() {
    if (!state.editingStatementId)
        return null;
    return getStatementDraftById(state.editingStatementId);
}
function resetAppealResolutionState() {
    state.processingAppealStatementId = null;
    state.appealResolutionResult = '';
    state.appealResolutionComment = '';
}
function getBuildCandidates(scopes) {
...
function getBuildLines(scopes) {
    ...
    if (!selectedScope)
        return [];
    return buildStatementDraftLines(selectedScope.settlementPartyId, selectedScope.settlementCycleId).map((item) => ({
        ...item,
        lineTypeZh: LINE_GRAIN_LABEL[item.statementLineGrainType ?? 'OTHER_SOURCE_OBJECT'] ?? '其它来源行',
        sourceTypeZh: item.sourceLabelZh ??
...
function getBuildLineSummary(lines) {
    return {
...
function openBuildView(scopes, statement) {
...
function resetBuildState(scopes) {
    const firstScope = scopes[0];
    state.editingStatementId = null;
    state.buildFactoryId = firstScope?.settlementPartyId ?? '';
    state.buildCycleId = firstScope?.settlementCycleId ?? '';
...
function createStatementDraftFromScope(scope, remark, by) {
    const lines = buildStatementDraftLines(scope.settlementPartyId, scope.settlementCycleId);
    if (!lines.length)
        return { ok: false, message: '当前工厂和结算周期暂无可生成的对账明细行' };
...
function confirmStatementDraft(statementId, by) {
    const draft = initialStatementDrafts.find((item) => item.statementId === statementId);
    if (!draft)
        return { ok: false, message: `对账单 ${statementId} 不存在` };
    if (draft.status === 'PENDING_FACTORY_CONFIRM' || draft.status === 'READY_FOR_PREPAYMENT')
        return { ok: true };
    if (draft.status === 'CLOSED')
        return { ok: false, message: '已关闭的对账单不可确认' };
...
function closeStatementDraft(statementId, by) {
    const draft = initialStatementDrafts.find((item) => item.statementId === statementId);
    if (!draft)
        return { ok: false, message: `对账单 ${statementId} 不存在` };
    if (draft.status !== 'DRAFT')
        return { ok: false, message: '当前仅草稿可关闭' };
    draft.status = 'CLOSED';
    draft.updatedAt = nowTimestamp();
    draft.updatedBy = by;
    return { ok: true };
...
function renderStatementListRows(items) {
    return items
        .map((item) => `
        <tr class="border-b last:border-b-0">
          ...
          <td class="px-4 py-3">
            <span class="inline-flex rounded-md px-2 py-0.5 text-xs ${getFactoryFeedbackStatusBadge(item.factoryFeedbackStatus)}">${escapeHtml(getFactoryFeedbackStatusLabel(item.factoryFeedbackStatus))}</span>
          </td>
...
function renderBuildCandidateRows(items) {
    if (!items.length) {
...
function renderBuildLineRows(lines) {
    if (!lines.length) {
...
function renderStatementLedgerSectionRows(lines, ledgerType) {
    if (!lines.length) {
...
function renderDetailDialog(detail) {
    ...
    const showAppealProcessing = state.processingAppealStatementId === detail.draft.statementId && openAppeal;
    return `
        ...
        <div class="flex-1 overflow-auto px-6 py-5">
          ...
          <section class="mt-4 rounded-lg border bg-card p-4">
            ...
            <dl class="mt-3 grid gap-3 text-sm md:grid-cols-2">
              ...
              <div class="flex items-start justify-between gap-3"><dt class="text-muted-foreground">最新申诉</dt><dd class="max-w-[70%] text-right text-xs text-muted-foreground">${latestAppeal
        ? escapeHtml(`${latestAppeal.reasonName} · ${latestAppeal.submittedAt} · ${getFactoryAppealStatusLabel(latestAppeal.status)}`)
        : '当前无工厂申诉'}</dd></div>
...
function renderListView(listItems, buildScopes) {
    ...
    <section class="rounded-xl border bg-background p-4">
      ...
      <div class="mt-4 flex flex-wrap items-center gap-3">
        ...
        <select class="h-9 w-52 rounded-md border bg-background px-3 text-sm" data-stm-list-filter="party">
          ...
          ${partyOptions
        .map((item) => `<option value="${escapeHtml(item.value)}" ${state.filterParty === item.value ? 'selected' : ''}>${escapeHtml(item.label)}</option>`)
        .join('')}
        </select>
        <select class="h-9 w-56 rounded-md border bg-background px-3 text-sm" data-stm-list-filter="cycle">
          ...
          ${cycleOptions
        .map((item) => `<option value="${escapeHtml(item.value)}" ${state.filterCycle === item.value ? 'selected' : ''}>${escapeHtml(item.label)}</option>`)
        .join('')}
        </select>
        ...
        .map(([value, label]) => `<option value="${value}" ${state.filterFeedback === value ? 'selected' : ''}>${escapeHtml(label)}</option>`)
        .join('')}
        </select>
      </div>
...
function renderBuildView(scopes) {
    const editingDraft = getEditingDraft();
    ...
    const duplicatedStatement = selectedScope == null
        ? null
        : findOpenStatementByPartyAndCycle(selectedScope.settlementPartyId, selectedScope.settlementCycleId);
    const blockingStatement = duplicatedStatement && duplicatedStatement.statementId !== state.editingStatementId
        ? duplicatedStatement
        : null;
    return `
    <section class="rounded-xl border bg-background p-4">
                ...
                <div class="mt-3 grid gap-3 md:grid-cols-2">
                  <label class="grid gap-1 text-sm">
                    ...
                    <select class="h-9 rounded-md border bg-background px-3 text-sm" data-stm-build-field="factory" ${editingDraft ? 'disabled' : ''}>
                      ...
                      ${factoryOptions
            .map((item) => `<option value="${escapeHtml(item.value)}" ${state.buildFactoryId === item.value ? 'selected' : ''}>${escapeHtml(item.label)}</option>`)
            .join('')}
                    </select>
                  </label>
                  <label class="grid gap-1 text-sm">
                    ...
                    <select class="h-9 rounded-md border bg-background px-3 text-sm" data-stm-build-field="cycle" ${editingDraft ? 'disabled' : ''}>
                      ...
                      ${cycleOptions
            .map((item) => `<option value="${escapeHtml(item.settlementCycleId)}" ${state.buildCycleId === item.settlementCycleId ? 'selected' : ''}>${escapeHtml(item.settlementCycleLabel)}</option>`)
            .join('')}
                    </select>
                  </label>
                </div>
    ...
    <div class="flex flex-col gap-6 p-6">
      ...
      <section class="rounded-xl border bg-background p-4">
        <div class="flex flex-wrap items-center gap-2">
          <button
            class="inline-flex h-9 items-center rounded-full border px-4 text-sm ${state.activeView === 'LIST' ? 'border-blue-300 bg-blue-50 text-blue-700' : 'hover:bg-muted'}"
          ...
          <button
            class="inline-flex h-9 items-center rounded-full border px-4 text-sm ${state.activeView === 'BUILD' ? 'border-blue-300 bg-blue-50 text-blue-700' : 'hover:bg-muted'}"
...
export function handleStatementsEvent(target) {
    const listFilterNode = target.closest('[data-stm-list-filter]');
    if (listFilterNode instanceof HTMLInputElement || listFilterNode instanceof HTMLSelectElement) {
        ...
        if (field === 'status') {
            state.filterStatus = listFilterNode.value;
            return true;
        }
        if (field === 'feedback') {
            state.filterFeedback = listFilterNode.value;
            return true;
        }
        return true;
    }
    const buildFieldNode = target.closest('[data-stm-build-field]');
    ...
    const appealFieldNode = target.closest('[data-stm-appeal-field]');
    if (appealFieldNode instanceof HTMLSelectElement || appealFieldNode instanceof HTMLTextAreaElement) {
        ...
        if (field === 'result' && appealFieldNode instanceof HTMLSelectElement) {
            state.appealResolutionResult = appealFieldNode.value;
            return true;
        }
    ...
    const actionNode = target.closest('[data-stm-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'open-detail') {
        ...
        if (state.processingAppealStatementId !== statementId)
            resetAppealResolutionState();
        return true;
    }
    ...
    if (action === 'generate') {
        ...
        if (!result.ok) {
            ...
            if (result.existingStatementId)
                state.detailStatementId = result.existingStatementId;
            return true;
        }
...
export function isStatementsDialogOpen() {
