...
// 当前入口需要同时读取正式生产任务、毛织交出和成衣图案工艺交出；各域只注册只读来源解析器。
import '../data/fcs/runtime-process-tasks.ts'
import '../data/fcs/wool-task-domain.ts'
import '../data/fcs/special-craft-task-orders.ts'
import { appStore } from '../state/store.ts'
import { escapeHtml } from '../utils.ts'
import { getPdaRuntimeContext, renderPdaLoginRedirect } from './pda-runtime.ts'
import { clearPdaSewingSelfReturnMode, getPdaSewingSelfReturnModeLock } from '../data/fcs/pda-sewing-self-return-mode.ts'
...
// 公共机由后道管理员开启；本次登记人是现场填写姓名的车缝送货人，不把开启人记成送货人。
export function resolveSewingSelfReturnRegistrationActor(
  runtime: { userId: string; userName: string; roleId: string; factoryId: string },
  deliveryPersonName: string,
  lock: ReturnType<typeof getPdaSewingSelfReturnModeLock>,
): { actorId: string; actorName: string; roleName: string } {
  if (lock?.active && lock.factoryId === runtime.factoryId && lock.openedBy === runtime.userName
    && runtime.roleId === 'ROLE_ADMIN') {
    const name = deliveryPersonName.trim()
    if (!name) throw new Error('请填写送货人姓名。')
    return { actorId: `PUBLIC-PDA-COURIER:${lock.factoryId}:${encodeURIComponent(name)}`, actorName: name, roleName: '车缝厂送货人员' }
  }
  return { actorId: runtime.userId, actorName: runtime.userName, roleName: runtime.roleId }
}
  ...
  if (!state.resolvedOrder || !state.returnIndex) {
    return `<section class="rounded-2xl border bg-white p-4 shadow-sm">
      <div class="flex items-start gap-3">
        ...
        <div><h2 class="text-base font-semibold">扫描本次回货来源码</h2><p class="mt-1 text-xs text-slate-500">必须扫描完整来源码；存在多个来源任务时，码中会包含任务标识。不展示待办列表。</p></div>
      </div>
      <div class="mt-4 flex gap-2">
        <input class="h-11 min-w-0 flex-1 rounded-xl border px-3 text-sm" value="${escapeHtml(state.scanValue)}" placeholder="PFRETURN:生产单号:回货序号[:来源任务]" data-pda-sewing-self-return-field="scanValue" data-pda-scan-enter="true" data-skip-page-rerender="true" />
...
function renderSkuCards(): string {
  ...
  return `<section class="space-y-3" data-testid="return-sku-list">
    <div><h2 class="text-base font-semibold">登记 ${state.resolvedOrder.skus.length} 个 SKU 回货数量</h2><p class="mt-1 text-xs text-slate-500">未随本批回货的 SKU 填 0；本次至少一个 SKU 大于 0，累计不能超过来源任务可交出数量。</p></div>
    ...
    ${state.resolvedOrder.skus.map((sku) => `<article class="rounded-2xl border bg-white p-3 shadow-sm" data-return-sku-card="${escapeHtml(sku.skuId)}">
      <div class="flex gap-3">
        ...
        <div class="min-w-0 flex-1"><div class="truncate text-sm font-semibold">${escapeHtml(sku.skuCode)}</div><div class="mt-1 text-xs text-slate-500">${escapeHtml(sku.colorName)} / ${escapeHtml(sku.sizeName)}</div><div class="mt-1 text-[11px] text-slate-500">本来源可登记 ${sku.plannedQty} 件</div><div class="mt-1 truncate text-[11px] text-slate-400">${escapeHtml(sku.barcode)}</div></div>
        <label class="w-28 text-xs font-medium text-slate-600">本次登记<input type="number" min="0" step="1" inputmode="numeric" value="${escapeHtml(state.quantities[sku.skuId] || '')}" class="mt-1 h-11 w-full rounded-xl border px-3 text-right text-base font-semibold" data-pda-sewing-self-return-field="quantity" data-sku-id="${escapeHtml(sku.skuId)}" data-skip-page-rerender="true" aria-invalid="false" placeholder="件" /></label>
      </div>
    </article>`).join('')}
  </section>`
}
...
function renderEvidence(): string {
  ...
  return `<section class="rounded-2xl border bg-white p-4 shadow-sm">
    ...
    <div class="mt-3 grid gap-3">
      ...
      <label class="text-xs text-slate-600">上传现场凭证图片<input type="file" accept="image/*" class="mt-1 block w-full rounded-xl border p-2 text-xs" data-pda-sewing-self-return-field="evidenceFile" data-skip-page-rerender="true" /></label>
      ...
      <div class="grid grid-cols-3 gap-2" data-return-evidence-preview>${renderEvidenceImages()}</div>
    </div>
  </section>`
}
...
export function handlePdaSewingSelfReturnEvent(target: HTMLElement, event?: Event): boolean {
  ...
  if (fieldNode) {
    ...
    if (field === 'quantity' && fieldNode.dataset.skuId) {
      ...
      const hasInvalidQuantity = Object.values(state.quantities).some((value) => (
        value !== '' && (!Number.isInteger(Number(value)) || Number(value) < 0)
      ))
      state.errorText = hasInvalidQuantity ? '回货登记数量必须是不小于 0 的整数件数。' : ''
      fieldNode.setAttribute('aria-invalid', hasInvalidQuantity ? 'true' : 'false')
    ...
    if (field === 'evidenceFile' && event?.type === 'change' && fieldNode.files?.[0]) {
      ...
      state.evidenceImages.forEach(image => { if (image.previewUrl.startsWith('blob:')) URL.revokeObjectURL(image.previewUrl) })
      state.evidenceImages = [{ name: file.name, previewUrl: URL.createObjectURL(file) }]
      const preview = document.querySelector<HTMLElement>('[data-return-evidence-preview]')
      if (preview) preview.innerHTML = renderEvidenceImages()
    }
    return true
  }
  ...
  try {
    ...
    if (action === 'submit') {
      ...
      const record = registerPostFinishingFactoryReturn({
        ...
        executionTaskId: state.resolvedOrder.executionTaskId,
        returnIndex: state.returnIndex,
        triggerSource: '公共PDA自助回货',
        idempotencyKey: `PUBLIC-PDA:${state.resolvedOrder.productionOrderNo}:${state.resolvedOrder.executionTaskId}:${state.returnIndex}`,
        ...
        actor: resolveSewingSelfReturnRegistrationActor(runtime, state.deliveryPersonName, getPdaSewingSelfReturnModeLock()),
      })
