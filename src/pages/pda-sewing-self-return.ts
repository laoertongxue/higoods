...
function renderScanBlock(result: PostFinishingSewingSelfReturnScanResult | null): string {
  return `
    <section class="rounded-2xl border bg-white p-4 shadow-sm">
      ...
      <textarea
        class="mt-3 min-h-24 w-full rounded-xl border bg-white px-3 py-2 text-xs leading-5 text-slate-700 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
      ...
      ${result ? `
        <div class="mt-3 rounded-xl border border-blue-100 bg-blue-50 px-3 py-3 text-xs leading-5 text-blue-800">
          <div class="font-semibold">${escapeHtml(result.productionConfirmationNo)} / ${escapeHtml(result.productionOrderNo)}</div>
          <div>车缝任务：${escapeHtml(result.sourceTaskNo)}</div>
          <div>车缝工厂：${escapeHtml(result.sourceFactoryName)}</div>
          <div>默认入库：后道待加工仓 / ${escapeHtml(result.defaultAreaName)} / ${escapeHtml(result.defaultLocationCode)}</div>
        </div>
      ` : ''}
    </section>
  `
  ...
  return `
    <section class="rounded-2xl border bg-white p-4 shadow-sm">
      <div class="flex items-start justify-between gap-3">
        ...
        <button type="button" class="rounded-full border px-3 py-1 text-xs" data-pda-sewing-self-return-action="fill-planned-qty">填入计划数</button>
      </div>
      ...
      <div class="mt-4 grid grid-cols-1 gap-3">
        ...
        <label class="block text-xs font-medium text-slate-600">
          现场凭证说明
          <textarea class="mt-1 min-h-20 w-full rounded-xl border px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" data-pda-sewing-self-return-field="evidenceText" data-skip-page-rerender="true" placeholder="例如：纸质确认单拍照留存、包装箱数、签名说明">${escapeHtml(state.evidenceText)}</textarea>
        </label>
      </div>
...
function renderExitBlock(): string {
  return `
    <section class="rounded-2xl border bg-white p-4 shadow-sm">
      <h2 class="text-sm font-semibold text-slate-950">退出车缝现场交货登记模式</h2>
      <p class="mt-1 text-xs leading-5 text-slate-500">退出需要验证当前后道工厂管理员账号密码。</p>
      ${renderNotice(state.exitErrorText, 'error')}
      <div class="mt-3 grid grid-cols-1 gap-3">
        <input class="h-11 rounded-xl border px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" value="${escapeHtml(state.exitLoginId)}" data-pda-sewing-self-return-field="exitLoginId" data-skip-page-rerender="true" placeholder="管理员账号" />
        <input type="password" class="h-11 rounded-xl border px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" value="${escapeHtml(state.exitPassword)}" data-pda-sewing-self-return-field="exitPassword" data-skip-page-rerender="true" placeholder="管理员密码" />
      </div>
      <button type="button" class="mt-3 h-11 w-full rounded-xl border border-slate-300 bg-white text-sm font-semibold text-slate-800" data-pda-sewing-self-return-action="exit-mode">验证并退出</button>
    </section>
  `
}
  ...
  return `
    <section class="min-h-screen bg-slate-100 text-slate-950" data-testid="pda-sewing-self-return-locked-page">
      ...
      <main class="mx-auto max-w-[480px] space-y-3 px-4 py-4 pb-8">
        <div class="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-800">
          当前 PDA 已锁定为车缝厂自助回货，只保留扫码提交和管理员退出。
        </div>
...
export async function handlePdaSewingSelfReturnEvent(target: HTMLElement): Promise<boolean> {
  ...
  if (fieldNode) {
    ...
    if (field === 'evidenceText') state.evidenceText = value
  ...
  if (action === 'fill-planned-qty') {
    try {
      fillPlannedQty(resolvePostFinishingSewingSelfReturnScan(state.scanValue))
    } catch (error) {
      state.errorText = error instanceof Error ? error.message : '请先扫描生产确认单。'
    }
    return true
  }
