...
import { buildCapacityStatusBadge, buildCapacityCalendarData, buildCapacityBottleneckData, buildCapacityRiskData, buildFactoryCalendarData, filterCapacityRiskTaskRows, summarizeProductionOrderRisk, } from '../data/fcs/capacity-calendar';
import { createCapacityCalendarOverride, expireCapacityCalendarOverride, getCapacityCalendarOverrideById, listCapacityCalendarOverrides, removeCapacityCalendarOverride, updateCapacityCalendarOverride, } from '../data/fcs/capacity-calendar-overrides';
import { listLegacyLikeDeductionBasisForTailPages, listLegacyLikeDyePrintOrdersForTailPages, listLegacyLikeExceptionsForTailPages, listLegacyLikeProcessTasksForTailPages, listLegacyLikeQualityInspectionsForTailPages, } from '../data/fcs/page-adapters/long-tail-pages-adapter';
...
function renderBadge(text, tone = 'secondary', className = '') {
...
function formatCapacityScopeText(processName, craftName) {
    const process = (processName ?? '').trim();
    const craft = (craftName ?? '').trim();
    if (!process)
        return craft;
    if (!craft || craft === process)
        return process;
...
function renderMetricStatCard(label, value, valueClass = '') {
    return `
    <article class="rounded-lg border bg-card">
      <div class="px-4 pb-4 pt-4">
        <p class="text-xs font-medium leading-snug text-muted-foreground">${escapeHtml(label)}</p>
...
function createEmptyPoliciesForm() {
    return {
        factoryId: '',
        processCode: '',
        craftCode: '',
...
function buildPoliciesFormFromOverride(overrideId) {
    const record = getCapacityCalendarOverrideById(overrideId);
    if (!record)
        return createEmptyPoliciesForm();
    return {
...
function getPoliciesProcessOptions(factoryId) {
...
function getPoliciesCraftOptions(factoryId, processCode) {
...
function openPoliciesEditor(mode, overrideId = '') {
...
function buildPoliciesOverrideInput() {
    return {
...
function resolvePoliciesOverrideStateLabel(startDate, endDate) {
    const today = new Date().toISOString().slice(0, 10);
    if (endDate < today)
        return { label: '已过期', tone: 'outline' };
    if (startDate > today)
        return { label: '未来生效', tone: 'secondary' };
    return { label: '生效中', tone: 'default' };
}
function getCurrentCapacityQueryString() {
    const pathname = appStore.getState().pathname;
    const [, query = ''] = pathname.split('?');
    return query;
}
function getCurrentCapacitySearchParams() {
    return new URLSearchParams(getCurrentCapacityQueryString());
}
function syncCapacityStateFromRoute() {
    const pathname = appStore.getState().pathname;
    ...
    if (pathOnly === '/fcs/capacity/overview') {
        ...
        if (tab === 'comparison' || tab === 'unallocated' || tab === 'unscheduled')
            state.overviewTab = tab;
    }
    state.querySignature = pathname;
}
function renderCapacityRouteContextBanner(page) {
    if (state.routeContext.source !== 'cuttable-pool')
        return '';
    if (page === 'overview') {
...
function renderTabButton(page, tab, current, label) {
...
function orderFactoryName(orderId) {
...
function getOrderDyeStatus(orderId) {
    const orderDyes = legacyLikeDyePrintOrders.filter((item) => item.productionOrderId === orderId);
    if (orderDyes.length === 0)
        return '无染印';
    if (orderDyes.some((item) => item.availableQty > 0))
        return '可继续';
    return '生产暂停';
}
function getOrderQcPendingCount(orderId) {
    return legacyLikeQualityInspections.filter((item) => item.productionOrderId === orderId && item.status !== 'CLOSED').length;
}
function getOrderOpenExceptionCount(orderId) {
    return legacyLikeExceptions.filter((item) => item.relatedOrderIds.includes(orderId) && item.caseStatus !== 'CLOSED').length;
}
...
function matchesOverviewKeyword(keyword, ...parts) {
    if (!keyword)
        return true;
    return parts.some((part) => String(part ?? '').toLowerCase().includes(keyword));
}
function getVisibleComparisonRows(rows, keyword) {
    const filtered = rows.filter((row) => matchesOverviewKeyword(keyword, row.date, row.factoryId, row.factoryName, row.processCode, row.processName, row.craftCode, row.craftName, ...row.taskIds));
    if (keyword)
        return filtered.slice(0, 120);
    return filtered.slice(0, 80);
}
function renderCapacityComparisonTable(rows, keyword) {
...
function renderUnallocatedRows(rows, keyword) {
    const filtered = rows
        .filter((row) => matchesOverviewKeyword(keyword, row.date, row.processCode, row.processName, row.craftCode, row.craftName, ...row.taskIds, ...row.assignmentStatuses))
        .slice(0, 50);
...
function renderUnscheduledRows(rows, keyword) {
    const filtered = rows
        .filter((row) => matchesOverviewKeyword(keyword, row.taskId, row.productionOrderId, row.demandType, row.processCode, row.processName, row.craftCode, row.craftName, row.assignmentStatus, row.assignmentMode, row.factoryName))
        .slice(0, 50);
    if (filtered.length === 0) {
...
function renderFactoryCalendarPagination(totalRows) {
...
function renderFactoryCalendarMainTable(rows, selectedRowKey) {
    if (rows.length === 0) {
...
function renderFactoryCalendarSourceTable(sources, emptyText) {
    if (sources.length === 0) {
...
function renderFactoryCalendarDetailPanel(calendar, selectedRow) {
    if (!selectedRow) {
        return `
...
function getOverviewFactoryRows() {
    ...
    for (const task of processTasks) {
        const factoryId = task.assignedFactoryId ??
    ...
    for (const qc of legacyLikeQualityInspections.filter((item) => item.status !== 'CLOSED')) {
        const basis = legacyLikeDeductionBasisItems.find((item) => item.sourceRefId === qc.qcId || item.sourceId === qc.qcId);
        const factoryId = basis?.factoryId ??
    ...
    return [...map.values()].map((item) => {
        const loadStatus = item.blockedCount > 0 ? '存在生产暂停' : item.taskCount >= 10 ? '高占用' : item.taskCount >= 1 ? '正常' : '空闲';
        return {
...
function renderOverviewFactoryTable(keyword) {
    ...
    return rows
        .map((row) => {
        const loadTone = row.loadStatus === '存在生产暂停'
            ? 'destructive'
            : row.loadStatus === '高占用'
                ? 'default'
                : row.loadStatus === '空闲'
        ...
        <tr class="border-b last:border-0">
          ...
          <td class="px-3 py-3 text-center text-sm">${row.taskCount}</td>
          <td class="px-3 py-3 text-center text-sm">${row.blockedCount > 0 ? renderBadge(String(row.blockedCount), 'destructive') : '<span class="text-muted-foreground">0</span>'}</td>
          <td class="px-3 py-3 text-center text-sm">${row.orderCount}</td>
          <td class="px-3 py-3 text-center text-sm">${row.dyeCount}</td>
          <td class="px-3 py-3 text-center text-sm">${row.qcPendingCount > 0 ? renderBadge(String(row.qcPendingCount), 'default') : '<span class="text-muted-foreground">0</span>'}</td>
...
function renderOverviewOrderTable(keyword) {
    const rows = getOverviewOrderRows().filter((row) => {
        if (!keyword)
            return true;
        return (includesKeyword(toLower(row.productionOrderId), keyword) || includesKeyword(toLower(row.mainFactory), keyword));
    });
    if (rows.length === 0) {
    ...
    return rows
        .map((row) => {
        const dyeTone = row.dyeStatus === '生产暂停'
            ? 'destructive'
            : row.dyeStatus === '可继续'
                ? 'default'
                : 'outline';
        const pressureTone = row.pressure === '高风险'
            ? 'destructive'
        ...
        <tr class="border-b last:border-0">
          ...
          <td class="px-3 py-3 text-center text-sm">${row.qcPending > 0 ? renderBadge(String(row.qcPending), 'default') : '<span class="text-muted-foreground">0</span>'}</td>
...
export function renderCapacityOverviewPage() {
    ...
    const hiddenComparisonCount = Math.max(calendar.comparisonRows.filter((row) => matchesOverviewKeyword(keyword, row.date, row.factoryId, row.factoryName, row.processCode, row.processName, row.craftCode, row.craftName, ...row.taskIds)).length - visibleComparisonRows.length, 0);
    ...
    const tabSummaryMap = {
        comparison: {
            title: '工厂供需明细',
            description: '按 日期 / 工厂 / 工序 / 工艺 聚合。已占用只消费占用工时对象，已冻结只消费冻结工时对象，剩余 = 供给 - 已占用 - 已冻结。',
            panel: `
...
function renderRiskConclusionBadge(conclusion) {
    const tone = conclusion === 'PAUSED' || conclusion === 'EXCEEDS_WINDOW'
        ? 'destructive'
        : conclusion === 'TIGHT'
            ? 'default'
            : conclusion === 'CAPABLE'
...
function renderRiskSamValue(value) {
    return value == null ? '—' : formatSamValue(value);
}
function renderRiskFactoryCell(row) {
    if (!row.factoryName) {
        return `<div class="text-sm text-muted-foreground">未落厂</div>`;
    }
    const bindingLabel = row.factoryBindingKind === 'COMMITTED'
        ? '已落厂占用'
        : row.factoryBindingKind === 'FROZEN_PENDING'
            ? '已冻结待确认'
            : '未落厂';
    const scopeLine = row.factoryBindingKind === 'FROZEN_PENDING' && (row.bindingFactoryCount ?? 0) > 1
...
function renderRiskWindowText(row) {
...
function renderRiskTaskTable(rows) {
    if (rows.length === 0) {
...
function renderRiskOrderTable(rows) {
    if (rows.length === 0) {
...
export function renderCapacityRiskPage() {
    syncDispatchCapacityUsageLedger();
    ...
    const craftOptions = riskData.craftOptions.filter((item) => state.riskProcessCode ? item.processCode === state.riskProcessCode : true);
    return `
...
function filterBottleneckCraftRows(rows, keyword) {
    if (!keyword)
        return rows;
...
function filterBottleneckDateRows(rows, keyword) {
    if (!keyword)
        return rows;
    return rows.filter((row) => includesKeyword(toLower(row.date), keyword)
        || row.detailRows.some((detail) => [detail.factoryName, detail.processName, detail.craftName].some((value) => includesKeyword(toLower(value), keyword))));
}
function filterBottleneckUnallocatedRows(rows, keyword) {
    if (!keyword)
        return rows;
    return rows.filter((row) => [
        row.taskId,
...
function filterBottleneckUnscheduledRows(rows, keyword) {
    if (!keyword)
        return rows;
    return rows.filter((row) => [
        row.taskId,
...
function renderBottleneckCraftTable(rows, selectedRowKey) {
    if (rows.length === 0) {
...
function renderBottleneckDateTable(rows, selectedDate) {
    if (rows.length === 0) {
...
function renderBottleneckDemandTableSection(title, description, tableHtml, testId) {
    return `
...
function renderBottleneckUnallocatedTable(rows) {
    if (rows.length === 0) {
        return `
      <table class="w-full text-sm">
...
function renderBottleneckUnscheduledTable(rows) {
    if (rows.length === 0) {
        return `
      <table class="w-full text-sm">
...
function renderBottleneckCraftDetailPanel(row) {
    return `
...
function renderBottleneckDateDetailPanel(row) {
    return `
...
export function renderCapacityBottleneckPage() {
    syncDispatchCapacityUsageLedger();
    ...
    const selectedCraftValue = state.bottleneckCraftCode && state.bottleneckProcessCode
    ...
    <div class="space-y-6" data-capacity-bottleneck-page>
      ...
      <section class="space-y-4">
        ...
        : state.bottleneckTab === 'date'
            ...
            : `
                <div class="space-y-4">
                  ${renderBottleneckDemandTableSection('待分配需求', '业务上仍未最终落厂的任务会继续留在需求池；如果已在多家工厂形成冻结，会同时占用这些工厂能力，但不会从待分配池消失。', renderBottleneckUnallocatedTable(filteredUnallocatedRows), 'bottleneck-unallocated-section')}
                  ${renderBottleneckDemandTableSection('未排期需求', '已有标准工时但缺少有效日期窗口的任务会单独列出，不会被强行落到某一天。', renderBottleneckUnscheduledTable(filteredUnscheduledRows), 'bottleneck-unscheduled-section')}
                </div>
              `}
      </section>
    </div>
...
export function renderCapacityConstraintsPage() {
    syncCapacityStateFromRoute();
    syncDispatchCapacityUsageLedger();
    ...
    const scopedRows = state.routeContext.orderIds.length
        ? calendar.rows.filter((row) => [...row.committedSources, ...row.frozenSources].some((source) => state.routeContext.orderIds.includes(source.productionOrderId)))
        : calendar.rows;
    ...
    const selectedRow = scopedRows.find((row) => row.rowKey === state.constraintsDetailRowKey)
        ?? scopedRows[0]
        ?? null;
    ...
    const windowText = calendar.dates.length
        ...
        : '暂无日期窗口';
    const selectedCraftValue = calendar.selectedCraftCode && calendar.selectedProcessCode
    ...
    <div class="space-y-5" data-testid="capacity-constraints-page">
      ...
      <section class="grid gap-3 rounded-xl bg-slate-50/80 p-4 md:grid-cols-2 xl:grid-cols-4" data-testid="capacity-constraints-filters">
        ...
        .map((factory) => `<option value="${escapeHtml(factory.id)}" ${factory.id === calendar.selectedFactoryId ? 'selected' : ''}>${escapeHtml(factory.label)}</option>`)
        .join('')}
          </select>
        </label>
        <label class="space-y-1 text-sm">
        ...
        .map((option) => `<option value="${option.value}" ${option.value === calendar.windowDays ? 'selected' : ''}>${escapeHtml(option.label)}</option>`)
        .join('')}
          </select>
        </label>
        <label class="space-y-1 text-sm">
        ...
        .map((process) => `<option value="${escapeHtml(process.processCode)}" ${process.processCode === calendar.selectedProcessCode ? 'selected' : ''}>${escapeHtml(process.processName)}</option>`)
        .join('')}
          </select>
        </label>
        <label class="space-y-1 text-sm">
        ...
        .map((craft) => `<option value="${escapeHtml(`${craft.processCode}::${craft.craftCode}`)}" ${`${craft.processCode}::${craft.craftCode}` === selectedCraftValue ? 'selected' : ''}>${escapeHtml(formatCapacityScopeText(craft.processName, craft.craftName))}</option>`)
        .join('')}
          </select>
        </label>
      </section>
...
function renderCapacityPoliciesOverrideDrawer() {
    if (!state.policiesEditorMode)
        return '';
    ...
    const selectedProcessCode = state.policiesForm.processCode && processOptions.some((item) => item.value === state.policiesForm.processCode)
...
export function renderCapacityPoliciesPage() {
    ...
    const overrideRowsHtml = overrideRows.length === 0
...
export function handleCapacityEvent(target) {
    const policiesFieldNode = target.closest('[data-capacity-policies-field]');
    if (policiesFieldNode instanceof HTMLInputElement ||
        policiesFieldNode instanceof HTMLSelectElement ||
        policiesFieldNode instanceof HTMLTextAreaElement) {
        const field = policiesFieldNode.dataset.capacityPoliciesField;
        if (!field)
            return false;
    ...
    const filterNode = target.closest('[data-capacity-filter]');
    if (filterNode instanceof HTMLInputElement || filterNode instanceof HTMLSelectElement) {
        ...
        if (filter === 'overview-keyword')
            state.overviewKeyword = value;
        if (filter === 'risk-keyword')
            state.riskKeyword = value;
        ...
        if (filter === 'risk-conclusion')
            state.riskConclusion = value;
        if (filter === 'bottleneck-keyword')
            state.bottleneckKeyword = value;
    ...
    const actionNode = target.closest('[data-capacity-action]');
    if (!actionNode)
        return false;
