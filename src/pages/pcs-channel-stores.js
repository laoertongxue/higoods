...
const PAYOUT_DETAIL_TABS = [
    { key: 'overview', label: '基本信息' },
    { key: 'stores', label: '关联店铺' },
    { key: 'attachments', label: '附件与日志' },
];
const initialStoreCreateDraft = {
    channel: '',
    storeName: '',
    storeCode: '',
    platformStoreId: '',
...
const initialPayoutCreateDraft = {
    name: '',
    payoutChannel: '',
    identifier: '',
    ownerType: '',
...
const payoutAccountRecords = new Map();
const syncErrors = [
    {
        id: 'E-001',
        type: 'product',
...
function getStoreOwnerOptions(team) {
    return STORE_TEAM_OPTIONS.find((item) => item.team === team)?.owners ?? [];
}
function getDefaultCurrencyByCountry(country) {
    if (country === '印尼')
        return 'IDR';
    if (country === '越南')
        return 'VND';
...
function getDefaultTimezoneByCountry(country) {
    if (country === '印尼')
        return 'Asia/Jakarta';
    if (country === '越南')
...
function ensureSeeded() {
    if (storeRecords.size > 0 && payoutAccountRecords.size > 0)
        return;
    const accounts = [
        {
            id: 'PA-001',
...
function resetStoreCreateDraft() {
    state.storeCreateDraft = { ...initialStoreCreateDraft };
}
function resetPayoutCreateDraft() {
...
function getRelatedStoreCount(accountId) {
    return getRelatedStoresForAccount(accountId).length;
}
function updateStore(storeId, updater) {
    ensureSeeded();
...
function appendStoreLog(record, log) {
    return {
        ...record,
        updatedAt: log.time,
...
function appendAccountLog(record, log) {
    return {
        ...record,
        updatedAt: log.time,
        updatedBy: log.operator,
...
function syncStoreDetailState(storeId) {
    const routeKey = appStore.getState().pathname;
    if (state.storeDetail.routeKey === routeKey && state.storeDetail.storeId === storeId)
        return;
...
function syncPayoutDetailState(accountId) {
    const routeKey = appStore.getState().pathname;
    if (state.payoutDetail.routeKey === routeKey && state.payoutDetail.accountId === accountId)
        return;
    state.payoutDetail = {
        routeKey,
        accountId,
...
function syncSyncPageState() {
    const routeKey = appStore.getState().pathname;
    if (state.syncPage.routeKey === routeKey)
        return;
...
function renderMetricButton(title, value, tone, action, extraData = '') {
    return `
...
function renderDrawerShell(title, description, body, footer) {
    return `
...
function renderModalShell(title, description, body, footer, widthClass = 'max-w-2xl') {
    return `
    <div class="fixed inset-0 z-50">
...
function renderFormField(label, control, required = false) {
    return `
...
function renderTextInput(field, value, placeholder, type = 'text') {
...
function renderTextArea(field, value, placeholder, rows = 4) {
...
function renderCheckbox(field, checked, label) {
    return `
...
function getFilteredStores() {
    ...
    return listStores().filter((store) => {
        ...
        if (!matchesKeyword)
            return false;
        if (state.storeList.channel !== 'all' && store.channel !== state.storeList.channel)
            return false;
        if (state.storeList.country !== 'all' && store.country !== state.storeList.country)
            return false;
        if (state.storeList.status !== 'all' && store.status !== state.storeList.status)
            return false;
        if (state.storeList.linkedOnly && store.projectStoreIds.length === 0)
            return false;
        return true;
    });
}
...
function getFilteredSyncErrors() {
    syncSyncPageState();
    const keyword = state.syncPage.search.trim().toLowerCase();
    return syncErrors.filter((record) => {
        if (record.type !== state.syncPage.activeTab)
            return false;
        const matchesKeyword = keyword.length === 0 ||
            [record.objectId, record.objectName, record.errorType, record.errorMsg].join(' ').toLowerCase().includes(keyword);
        if (!matchesKeyword)
            return false;
        if (state.syncPage.storeFilter !== 'all' && record.storeId !== state.syncPage.storeFilter)
            return false;
        if (state.syncPage.statusFilter !== 'all' && record.status !== state.syncPage.statusFilter)
            return false;
        return true;
    });
}
function getSyncStats(type) {
...
function getFilteredPayoutAccounts() {
    ...
    return listPayoutAccounts().filter((account) => {
        ...
        if (!matchesKeyword)
            return false;
        if (state.payoutList.ownerType !== 'all' && account.ownerType !== state.payoutList.ownerType)
            return false;
        if (state.payoutList.legalEntity !== 'all' && account.ownerRefId !== state.payoutList.legalEntity)
            return false;
        if (state.payoutList.country !== 'all' && account.country !== state.payoutList.country)
            return false;
        if (state.payoutList.status !== 'all' && account.status !== state.payoutList.status)
            return false;
        return true;
    });
}
...
function openStoreEditDrawer(storeId) {
    const store = getStoreById(storeId);
    if (!store)
        return;
    state.storeEditDrawer = {
...
function createPayoutAccount() {
    ...
    const ownerName = draft.ownerType === 'LEGAL'
...
function saveStoreEdit() {
    ...
    if (!storeId)
        return;
    updateStore(storeId, (store) => appendStoreLog({
        ...store,
...
function refreshStoreAuth(storeId, mode) {
...
function changeStorePayoutBinding() {
    ...
    updateAccount(account.id, (record) => appendAccountLog(record, {
        time: timestamp,
        action: '绑定店铺',
        operator: '当前用户',
    ...
    if (previousAccountId && previousAccountId !== account.id) {
        updateAccount(previousAccountId, (record) => appendAccountLog(record, {
            time: timestamp,
            action: '解绑店铺',
            operator: '当前用户',
...
function renderStoreListPageContent() {
  ...
  `;
    const filters = renderCard('筛选条件', `
      <div class="grid gap-4 lg:grid-cols-5">
        ${renderFormField('关键词', renderTextInput('store-list-search', state.storeList.search, '搜索店铺名/内部编码/平台店铺ID'))}
        ${renderFormField('渠道', renderSelect('store-list-channel', state.storeList.channel === 'all' ? '' : state.storeList.channel, CHANNEL_OPTIONS.map((item) => ({ value: item, label: item })), '全部渠道'))}
        ${renderFormField('国家/区域', renderSelect('store-list-country', state.storeList.country === 'all' ? '' : state.storeList.country, STORE_COUNTRY_OPTIONS.map((item) => ({ value: item, label: item })), '全部区域'))}
        ${renderFormField('店铺状态', renderSelect('store-list-status', state.storeList.status === 'all' ? '' : state.storeList.status, [
...
function renderStoreCreateDrawer() {
    if (!state.storeCreateDrawerOpen)
        return '';
    const draft = state.storeCreateDraft;
    ...
    <div class="space-y-6">
      <section class="space-y-4">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ${renderFormField('渠道', renderSelect('store-create-channel', draft.channel, CHANNEL_OPTIONS.map((item) => ({ value: item, label: item })), '选择渠道'), true)}
      ...
      <section class="space-y-4">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ${renderFormField('国家/区域', renderSelect('store-create-country', draft.country, STORE_COUNTRY_OPTIONS.map((item) => ({ value: item, label: item })), '选择国家/区域'), true)}
          ${renderFormField('结算币种', renderSelect('store-create-settlement-currency', draft.settlementCurrency, CURRENCY_OPTIONS.map((item) => ({ value: item, label: item })), '选择结算币种'))}
      ...
      <section class="space-y-4">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ${renderFormField('所属团队', renderSelect('store-create-team', draft.team, STORE_TEAM_OPTIONS.map((item) => ({ value: item.team, label: item.team })), '选择所属团队'), true)}
          ${renderFormField('店铺负责人', renderSelect('store-create-store-owner', draft.storeOwner, ownerOptions.map((item) => ({ value: item, label: item })), draft.team ? '选择店铺负责人' : '请先选择所属团队'), true)}
        </div>
      </section>
    </div>
  `;
...
function renderStoreOverviewTab(store) {
...
function renderStoreAuthTab(store) {
    const statusMeta = AUTH_STATUS_META[store.authStatus];
    const summaryClass = store.authStatus === 'CONNECTED'
        ? 'border-green-200 bg-green-50 text-green-700'
        : store.authStatus === 'EXPIRED'
            ? 'border-yellow-200 bg-yellow-50 text-yellow-700'
            : 'border-red-200 bg-red-50 text-red-700';
    const summaryText = store.authStatus === 'CONNECTED'
        ? `授权有效期至 ${store.tokenExpireAt || '-'}`
        : store.authStatus === 'EXPIRED'
            ? '授权已过期，请尽快重新授权。'
            : '平台连接校验失败，请重新授权或人工校验。';
    return renderCard('授权状态', `
...
function renderStorePoliciesTab() {
    return renderCard('上架策略配置', `
      <div class="space-y-5">
        <div class="rounded-lg border border-slate-200 bg-slate-50 p-4">
          ${renderCheckbox('store-policy-allow-listing', state.storePolicyDraft.allowListing, '允许该店铺发起商品上架')}
        </div>
        <div class="grid gap-4 md:grid-cols-2">
          ${renderFormField('库存同步模式', renderSelect('store-policy-inventory-sync-mode', state.storePolicyDraft.inventorySyncMode, INVENTORY_SYNC_OPTIONS, '选择库存模式'))}
...
function renderStorePayoutTab(store) {
    const binding = getCurrentBinding(store);
    ...
    const rows = store.bindings
      ...
      `)
        .join('');
    return `
    ${renderCard('当前有效提现账号', currentCard, `
...
function renderStoreSyncTab() {
    return renderCard('数据同步状态', `
...
function renderStoreLogsTab(store) {
...
function renderStoreDetailDialogs() {
    return renderStoreEditDrawer();
}
function renderStoreEditDrawer() {
    if (!state.storeEditDrawer.open)
        return '';
...
function renderStoreAuthDialog(store) {
    if (!state.storeAuthDialog.open)
        return '';
...
function renderStorePayoutDialog() {
    if (!state.storePayoutDialog.open)
        return '';
    const availableAccounts = listPayoutAccounts().filter((account) => account.status === 'ACTIVE');
    const body = `
    <div class="space-y-4">
      ${renderFormField('新的提现账号', renderSelect('store-payout-account-id', state.storePayoutDialog.payoutAccountId, availableAccounts.map((account) => ({ value: account.id, label: `${account.name} / ${account.identifierMasked}` })), '选择提现账号'), true)}
...
function renderStoreBindingHistoryDialog(store) {
    if (!state.storeBindingHistoryDialog.open)
        return '';
    const rows = store.bindings
        .map((binding) => `
        <tr class="border-t border-slate-100 align-top">
...
function renderPayoutCreateDrawer() {
    if (!state.payoutCreateDrawerOpen)
        return '';
    const draft = state.payoutCreateDraft;
    const body = `
    <div class="space-y-6">
      ${renderFormField('账号名称', renderTextInput('payout-create-name', draft.name, '如：HiGOOD LIVE Limited - TikTok Payout'), true)}
      ${renderFormField('提现渠道', renderSelect('payout-create-channel', draft.payoutChannel, PAYOUT_CHANNEL_OPTIONS.map((item) => ({ value: item, label: item })), '选择提现渠道'), true)}
      ${renderFormField('账号标识（脱敏展示）', renderTextInput('payout-create-identifier', draft.identifier, '如：卡号尾号 / 钱包 ID'))}
      ${renderFormField('归属类型', renderSelect('payout-create-owner-type', draft.ownerType, [
        { value: 'LEGAL', label: '法人 (公司)' },
        { value: 'PERSONAL', label: '个人' },
    ], '选择归属类型'), true)}
      ${draft.ownerType === 'LEGAL'
        ? renderFormField('法人主体', renderSelect('payout-create-owner-ref-id', draft.ownerRefId, LEGAL_ENTITIES.map((item) => ({ value: item.id, label: `${item.name} (${item.country})` })), '选择法人主体'), true)
        : renderFormField('个人姓名', renderTextInput('payout-create-owner-ref-id', draft.ownerRefId, '输入个人姓名'), true)}
      <div class="grid gap-4 md:grid-cols-2">
        ${renderFormField('国家/区域', renderSelect('payout-create-country', draft.country, ACCOUNT_COUNTRY_OPTIONS.map((item) => ({ value: item.code, label: item.label })), '选择国家/区域'), true)}
        ${renderFormField('币种', renderSelect('payout-create-currency', draft.currency, CURRENCY_OPTIONS.map((item) => ({ value: item, label: item })), '选择币种'), true)}
      </div>
    </div>
  `;
    const footer = `
...
function renderStoreDetailSummary(store) {
    const projectStoreText = store.projectStoreIds.length > 0
...
function renderSyncPageContent() {
  ...
  `;
    ...
    <div class="space-y-5 p-4">
      ${renderNotice()}
      ${renderPageHeader('同步状态与错误回执', '查看商品同步和订单同步的错误信息与处理状态。', `
      ...
      ${renderCard('筛选条件', `
          <div class="grid gap-4 md:grid-cols-4">
            ...
            ${renderFormField('店铺', renderSelect('sync-store-filter', state.syncPage.storeFilter === 'all' ? '' : state.syncPage.storeFilter, listStores().map((store) => ({ value: store.id, label: store.storeName })), '全部店铺'))}
            ${renderFormField('状态', renderSelect('sync-status-filter', state.syncPage.statusFilter === 'all' ? '' : state.syncPage.statusFilter, [
        { value: '待处理', label: '待处理' },
        { value: '已重试', label: '已重试' },
        { value: '已恢复', label: '已恢复' },
    ], '全部状态'))}
        ...
        `)}
      ${renderCard(state.syncPage.activeTab === 'product' ? '商品同步错误' : '订单同步错误', `
          <div class="overflow-x-auto">
            <table class="min-w-full text-left text-sm">
              <thead class="bg-slate-50 text-slate-500">
                <tr>
...
function renderPayoutAccountListContent() {
    ...
    <div class="space-y-5 p-4">
      ...
      ${renderCard('筛选条件', `
          <div class="grid gap-4 lg:grid-cols-5">
            ...
            ${renderFormField('归属类型', renderSelect('payout-list-owner-type', state.payoutList.ownerType === 'all' ? '' : state.payoutList.ownerType, [
        { value: 'LEGAL', label: '法人' },
        { value: 'PERSONAL', label: '个人' },
    ], '全部类型'))}
            ${state.payoutList.ownerType === 'LEGAL'
        ? renderFormField('法人主体', renderSelect('payout-list-legal-entity', state.payoutList.legalEntity === 'all' ? '' : state.payoutList.legalEntity, LEGAL_ENTITIES.map((item) => ({ value: item.id, label: item.name })), '全部法人'))
        : ''}
            ${renderFormField('国家/区域', renderSelect('payout-list-country', state.payoutList.country === 'all' ? '' : state.payoutList.country, ACCOUNT_COUNTRY_OPTIONS.map((item) => ({ value: item.code, label: item.label })), '全部区域'))}
            ${renderFormField('状态', renderSelect('payout-list-status', state.payoutList.status === 'all' ? '' : state.payoutList.status, [
        { value: 'ACTIVE', label: '启用' },
        { value: 'INACTIVE', label: '停用' },
    ], '全部状态'))}
          </div>
...
function renderPayoutAccountDetailContent(account) {
    const relatedStores = getRelatedStoresForAccount(account.id);
    const tabs = PAYOUT_DETAIL_TABS.map((tab) => `
  ...
  `;
    const stores = renderCard('关联店铺（当前/历史绑定）', `
      <div class="overflow-x-auto">
        <table class="min-w-full text-left text-sm">
          <thead class="bg-slate-50 text-slate-500">
            <tr>
    ...
    ${renderCard('操作日志', `
      ...
      `)}
  `;
    const content = state.payoutDetail.activeTab === 'overview'
        ? overview
...
export function renderPcsChannelStoreListPage() {
    ensureSeeded();
    return renderStoreListPageContent();
}
export function renderPcsChannelStoreDetailPage(storeId) {
    ensureSeeded();
...
export function renderPcsChannelStoreSyncPage() {
    ensureSeeded();
    return renderSyncPageContent();
}
export function renderPcsPayoutAccountListPage() {
    ensureSeeded();
    return renderPayoutAccountListContent();
}
export function renderPcsPayoutAccountDetailPage(accountId) {
    ensureSeeded();
...
export function handlePcsChannelStoresInput(target) {
    const fieldNode = target.closest('[data-pcs-channel-store-field]');
    if (!fieldNode)
        return false;
    ...
    if (field in storeCreateFields && (fieldNode instanceof HTMLInputElement || fieldNode instanceof HTMLSelectElement)) {
        ;
        state.storeCreateDraft[storeCreateFields[field]] = fieldNode.value;
    ...
    if (field in storeEditFields && fieldNode instanceof HTMLInputElement) {
        ;
        state.storeEditDrawer.draft[storeEditFields[field]] = fieldNode.value;
        return true;
    }
    ...
    if (field in policyStringFields && (fieldNode instanceof HTMLInputElement || fieldNode instanceof HTMLSelectElement)) {
        ;
        state.storePolicyDraft[policyStringFields[field]] = fieldNode.value;
        return true;
    }
    ...
    if (field in payoutCreateFields && (fieldNode instanceof HTMLInputElement || fieldNode instanceof HTMLSelectElement)) {
        ;
        state.payoutCreateDraft[payoutCreateFields[field]] = fieldNode.value;
...
export function handlePcsChannelStoresEvent(target) {
    const actionNode = target.closest('[data-pcs-channel-store-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'set-auth-method') {
        state.storeAuthDialog.method = actionNode.dataset.value || 'oauth';
        return true;
    }
    ...
    if (action === 'prev-store-auth-step') {
        if (state.storeAuthDialog.step > 1) {
            state.storeAuthDialog.step = (state.storeAuthDialog.step - 1);
        }
        return true;
    }
    if (action === 'complete-store-auth') {
        if (!state.storeAuthDialog.storeId)
            return true;
    ...
    if (action === 'set-store-detail-tab') {
        state.storeDetail.activeTab = actionNode.dataset.value || 'overview';
        return true;
    }
    if (action === 'set-sync-tab') {
        state.syncPage.activeTab = actionNode.dataset.value || 'product';
    ...
    if (action === 'payout-quick-filter') {
        ...
        const value = actionNode.dataset.value ?? '';
        if (filter === 'status')
            state.payoutList.status = value;
        if (filter === 'owner')
            state.payoutList.ownerType = value;
        return true;
    }
    ...
    if (action === 'set-payout-detail-tab') {
        state.payoutDetail.activeTab = actionNode.dataset.value || 'overview';
        return true;
    }
...
export function isPcsChannelStoresDialogOpen() {
    return Boolean(state.storeCreateDrawerOpen ||
