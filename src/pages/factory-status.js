import { indonesiaFactories, tierLabels, typeLabels, } from '../data/fcs/indonesia-factories';
...
function executeSingleChange() {
    const targetFactory = getTargetFactory();
    if (!targetFactory)
        return;
    const { date, full } = getNow();
    state.factories = state.factories.map((factory) => factory.id === targetFactory.id
        ? {
            ...factory,
    ...
    const history = {
        ...
        oldStatus: targetFactory.status,
    ...
    const log = {
        ...
        oldValue: statusConfig[targetFactory.status].label,
...
function executeBatchChange() {
    if (state.selectedIds.length === 0)
        return;
    const { date, full } = getNow();
    const targetFactories = state.factories.filter((factory) => state.selectedIds.includes(factory.id));
    state.factories = state.factories.map((factory) => state.selectedIds.includes(factory.id)
        ? {
            ...factory,
            status: state.pendingStatus,
            updatedAt: date,
        ...
        : factory);
    const histories = targetFactories.map((factory, index) => ({
        id: `h-${Date.now()}-${index}`,
...
function renderSingleChangeDialog() {
    if (!state.changeDialogOpen)
        return '';
    const targetFactory = getTargetFactory();
    return `
      ...
      <section class="absolute left-1/2 top-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background shadow-2xl" data-dialog-panel="true">
        ...
        <div class="space-y-4 px-6 py-4">
          ...
          <div class="space-y-1.5">
            ...
            <div>${targetFactory ? renderStatusBadge(targetFactory.status) : '-'}</div>
          </div>
        ...
        .map((status) => `<option value="${status}" ${state.pendingStatus === status ? 'selected' : ''}>${statusConfig[status].label}</option>`)
        .join('')}
            </select>
...
function renderBatchChangeDialog() {
    if (!state.batchChangeDialogOpen)
        return '';
    return `
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      <button class="absolute inset-0 bg-black/45" data-status-action="close-dialog" aria-label="关闭"></button>
...
function renderConfirmDialog() {
    if (!state.confirmDialogOpen)
        return '';
    const warningText = state.pendingStatus === 'BLACKLISTED'
        ...
        ? `
                <table class="w-full text-sm">
                  ...
                  <tbody>
                    ${state.changeLogs
            .map((log) => {
            const targetNames = log.targetNames.length > 2
...
export function renderFactoryStatusPage() {
    ...
    const allSelected = pagedFactories.length > 0 && pagedFactories.every((factory) => state.selectedIds.includes(factory.id));
    return `
    <div class="space-y-6">
      ...
      <section class="flex flex-wrap items-center gap-4 rounded-lg bg-muted/30 p-4">
        ...
        <select data-status-filter="status" class="w-[140px] rounded-md border px-3 py-2 text-sm">
          ...
          ${Object.keys(statusConfig)
        .map((status) => `<option value="${status}" ${state.statusFilter === status ? 'selected' : ''}>${statusConfig[status].label}</option>`)
        .join('')}
        </select>
        ...
        : pagedFactories
            ...
            return `
                ...
                : ''}
                          ...
                          <td class="px-3 py-3">${renderStatusBadge(factory.status)}</td>
...
export function handleFactoryStatusEvent(target) {
    const statusField = target.closest('[data-status-field]');
    ...
    const actionNode = target.closest('[data-status-action]');
    if (!actionNode) {
    ...
    if (action === 'open-single-change') {
        ...
        state.pendingStatus = factory.status;
        state.pendingReason = '';
        state.pendingNote = '';
        state.reasonError = '';
        state.pendingBatchConfirm = false;
...
export function isFactoryStatusDialogOpen() {
    return (state.changeDialogOpen ||
