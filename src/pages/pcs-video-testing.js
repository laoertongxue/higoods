...
import { ACCOUNTING_STATUS_META, SESSION_STATUS_META, VIDEO_PLATFORM_META, VIDEO_PURPOSE_META, getVideoEvidence, getVideoItems, getVideoLogs, getVideoRecordById, getVideoSamples, } from '../data/pcs-testing.ts';
import { getVideoTestRecordById, listVideoTestRecords, } from '../data/pcs-video-testing-repository.ts';
...
import { getProjectRelationProjectLabel, listProjectRelationsByVideoRecord, listVideoRecordProjectRelationCandidates, replaceVideoRecordProjectRelations, } from '../data/pcs-project-relation-repository.ts';
...
function buildRecommendation(intent, views, clicks, orders, gmv) {
    if (intent !== 'TEST') {
...
function buildDecisionRef(projectRef, accounted) {
    if (!projectRef || !accounted)
        return null;
...
function inferTestingProjectIdentity(item) {
    ...
    const projectNamePart = searchText.includes('SPU-20260110-001') || searchText.includes('印尼风格碎花连衣裙')
        ? '印尼风格碎花连衣裙测款项目'
...
function getVideoWorkItemSnapshot(record) {
    const actionItem = record.items.find((item) => item.evaluationIntent === 'TEST') ?? record.items[0] ?? null;
    const linkedChannelProduct = actionItem?.relatedProjectId
...
function buildExposureType(intent, index) {
    if (intent === 'TEST')
        return index % 2 === 0 ? '试穿讲解' : '对比展示';
    if (index % 2 === 0)
        return '上身展示';
    return '搭配展示';
}
function buildItemComments(likes) {
    return Math.max(1, Math.round(likes * 0.038));
}
function buildItemShares(likes) {
    return Math.max(1, Math.round(likes * 0.018));
}
function inferPlatformCode(record) {
    const matched = Object.entries(VIDEO_PLATFORM_META).find(([, meta]) => record.includes(meta.label));
    return matched?.[0] ?? 'OTHER';
}
function inferAccountingStatus(baseRecord, repoRecord, items) {
...
function buildFallbackEvidence(record) {
    const evidence = [];
    if (record.videoUrl) {
        evidence.push({
...
function buildFallbackSamples(record) {
...
function buildFallbackLogs(record) {
    return [
...
function buildItemFromBase(item, index, projectIdByCode, accounted) {
    ...
    const relatedProjectId = projectRef || inferredProject
...
function buildFallbackItems(record) {
    if (!record)
        return [];
    const intent = record.legacyProjectRef ? 'TEST' : 'SELL';
...
function buildRecordViewModel(recordId) {
    const repoRecord = getVideoTestRecordById(recordId);
    const baseRecord = getVideoRecordById(recordId);
    if (!repoRecord && !baseRecord)
        return null;
    const relationProjectMap = new Map(listProjectRelationsByVideoRecord(recordId).map((relation) => [relation.projectCode, relation.projectId]));
    const items = baseRecord
        ? getVideoItems(recordId).map((item, index) => buildItemFromBase(item, index, relationProjectMap, baseRecord.testAccountingStatus === 'ACCOUNTED'))
...
function syncDetailState(recordId) {
    const routeKey = appStore.getState().pathname;
    if (state.detail.routeKey === routeKey && state.detail.recordId === recordId)
        return;
...
function getFilteredRecords() {
    ...
    return getRecords().filter((record) => {
        ...
        if (!matchesKeyword)
            return false;
        if (state.list.status !== 'all' && record.status !== state.list.status)
            return false;
        if (state.list.purpose !== 'all' && !record.purposes.includes(state.list.purpose))
            return false;
        if (state.list.platform !== 'all' && record.platformCode !== state.list.platform)
            return false;
        if (state.list.accounting !== 'all' && record.testAccountingStatus !== state.list.accounting)
            return false;
        if (state.list.quickFilter === 'reconciling' && record.status !== 'RECONCILING')
            return false;
        if (state.list.quickFilter === 'canClose' && !(record.status === 'RECONCILING' && record.itemCount > 0))
            return false;
        if (state.list.quickFilter === 'pendingAccounting' && record.testAccountingStatus !== 'PENDING')
            return false;
        if (state.list.quickFilter === 'accounted' && record.testAccountingStatus !== 'ACCOUNTED')
            return false;
        return true;
    });
}
...
function getPrimaryProject(record) {
...
function isDisplayableRecord(record) {
    return Boolean(getPrimaryProject(record) &&
        record.publishedAt &&
...
function renderKpis() {
    ...
    <section class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ${cards
        .map((card) => `
            <button type="button" class="${toClassName('rounded-lg border p-4 text-left transition hover:shadow-sm', card.tone, state.list.quickFilter === card.key ? 'ring-2 ring-blue-500' : '')}" data-pcs-video-testing-action="set-quick-filter" data-value="${card.key}">
              ...
              <div class="grid gap-4 md:grid-cols-2">
                <label class="space-y-1">
                  ...
                  <select class="h-10 w-full rounded-md border border-slate-200 px-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" data-pcs-video-testing-field="create-platform">
                    ...
                    ${Object.keys(VIDEO_PLATFORM_META)
...
function renderCloseDialog() {
    ...
    if (!record)
        return '';
    return renderModalShell('完成记录（关账）', `确认完成「${record.title}」的关账动作，关账后记录将只读，仅可补充证据。`, `
...
function renderAccountingDialog() {
    ...
    return renderModalShell('完成测款核对（入账）', `确认对「${record.title}」的 TEST 条目完成核对并写入测款结论。`, `
      <div class="rounded-lg bg-slate-50 p-4 text-sm text-slate-600">
        <p><span class="font-medium text-slate-900">记录编号：</span>${escapeHtml(record.id)}</p>
      ...
      <section class="space-y-2">
        ...
        <div class="space-y-2 rounded-lg border border-slate-200 p-3 text-sm text-slate-600">
          ${testItems
        .map((item) => `<div class="rounded-md bg-blue-50 px-3 py-2 text-blue-800">• ${escapeHtml(item.projectRef ? `项目维度：${item.projectRef}` : `商品维度：${item.productRef}`)}</div>`)
        .join('')}
        </div>
      </section>
      <label class="space-y-1">
      ...
      <button type="button" class="${toClassName('inline-flex h-9 items-center rounded-md border px-3 text-sm', state.accountingDialog.confirmed ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50')}" data-pcs-video-testing-action="toggle-accounting-confirmed">
...
function renderEditDrawer() {
    ...
    return renderDrawerShell(`编辑条目 - ${item.id}`, `
      <div class="space-y-6">
...
function renderDetailHeader(record, relatedProjects, mode = 'page') {
    return `
    <section class="rounded-lg border bg-white">
...
function renderOverviewTab(record) {
    return `
    <div class="space-y-6">
...
function renderItemsTab(record) {
    return `
    <div class="space-y-4">
      ...
      <section class="rounded-lg border bg-white">
        ...
        .map((item) => `
                    <tr class="hover:bg-slate-50/80">
                      ...
                      <td class="px-4 py-3">${item.evaluationIntent === 'TEST'
...
function renderReconcileTab(record) {
    const checks = [
...
function renderEvidenceTab(record) {
    return `
    <div class="space-y-4">
      <div class="flex items-center justify-between">
...
function renderAccountingTab(record, testItems) {
    return `
    <div class="space-y-4">
      <section class="rounded-lg border bg-white">
...
function renderDetailSidebar(record, relatedProjects) {
...
function renderDetailContent(record, mode = 'page') {
    const relatedProjects = Array.from(new Map(listProjectRelationsByVideoRecord(record.id).map((relation) => [
        relation.projectId,
    ...
    if (state.detail.activeTab === 'items')
        activeContent = renderItemsTab(record);
    if (state.detail.activeTab === 'reconcile')
        activeContent = renderReconcileTab(record);
    if (state.detail.activeTab === 'evidence')
        activeContent = renderEvidenceTab(record);
    if (state.detail.activeTab === 'accounting')
        activeContent = renderAccountingTab(record, testItems);
    if (state.detail.activeTab === 'logs')
        activeContent = renderLogsTab(record);
    return `
...
export function renderPcsVideoTestingListPage() {
    ensureRecordStore();
...
export function renderPcsVideoTestingDetailPage(recordId) {
    ensureRecordStore();
    syncDetailState(recordId);
    return renderPcsVideoTestingListPage();
}
export function handlePcsVideoTestingInput(target) {
    const fieldNode = target.closest('[data-pcs-video-testing-field]');
    if (!fieldNode)
        return false;
    ...
    if (field in createInputFields && fieldNode instanceof HTMLInputElement) {
        ;
        state.createDraft[createInputFields[field]] = fieldNode.value;
        return true;
    }
    if (field === 'create-platform' && fieldNode instanceof HTMLSelectElement) {
        state.createDraft.platform = (fieldNode.value || '');
        return true;
    }
    ...
    if (field === 'edit-intent' && fieldNode instanceof HTMLSelectElement) {
        state.editDrawer.draft.evaluationIntent = fieldNode.value;
        return true;
    }
    ...
    if (field in editInputFields && fieldNode instanceof HTMLInputElement) {
        ;
        state.editDrawer.draft[editInputFields[field]] = fieldNode.value;
        return true;
    }
...
export function handlePcsVideoTestingEvent(target) {
    const actionNode = target.closest('[data-pcs-video-testing-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'confirm-close-record') {
        ...
        if (!recordId)
            return true;
        updateRecord(recordId, (record) => appendLog({
            ...record,
            status: 'COMPLETED',
            completedBy: '当前用户',
            completedAt: nowText(),
    ...
    if (action === 'save-edit-item') {
        ...
        if (!recordId || !itemId)
            return true;
        const candidateMap = new Map(listVideoRecordProjectRelationCandidates(recordId).map((item) => [item.projectId, item]));
        const resolvedDraftProject = resolveProjectIdentity(draft.projectRef);
        let notice = '短视频条目已保存。';
        updateRecord(recordId, (record) => {
            const items = record.items.map((item) => {
                const baseItem = item.id === itemId
                    ? {
                        ...item,
                    ...
                    : item;
                const normalizedBaseItem = draft.evaluationIntent === 'TEST' && baseItem.evaluationIntent === 'TEST'
                    ? {
                        ...baseItem,
            ...
            const nextProjectIds = Array.from(new Set(items.flatMap((item) => {
                const resolvedProject = resolveProjectIdentity(item.projectRef);
                if (!item.projectRef || !resolvedProject) {
                    if (item.projectRef)
                        blockedReasons.push(`未找到项目 ${item.projectRef}`);
                    return [];
                }
...
export function isPcsVideoTestingDialogOpen() {
