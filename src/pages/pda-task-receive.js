...
import { resolveTaskChainTenderId, } from '../data/fcs/page-adapters/task-chain-pages-adapter';
import { listPdaAwardedTenderNoticesByFactoryId, listPdaBiddingTendersByFactoryId, listPdaQuotedTendersByFactoryId, PDA_MOCK_QUOTED_TENDERS, } from '../data/fcs/pda-mobile-mock';
...
import { getMobileTaskProcessType, getPdaMobileExecutionTaskById, listPdaMobileExecutionTasks, } from '../data/fcs/process-mobile-task-binding.ts';
import { acceptPostFinishingTask, FULL_CAPABILITY_FACTORY_ID, rejectPostFinishingTask, } from '../data/fcs/post-finishing-domain.ts';
import { renderPdaFrame } from './pda-shell';
import { buildPdaCuttingDirectExecEntryHref, buildPdaCuttingTaskDetailNavHref, buildPdaTaskListFocusHref, buildPdaTaskReceiveDetailNavHref, readPdaCuttingNavContext, } from './pda-cutting-nav-context';
import { buildPdaCuttingTaskEntryAction, getPdaCuttingTaskStateBadgeClass, } from './pda-cutting-task-rollup';
import { createInitialPdaReceiveSubmittedTenderIds, filterReceiveActiveBiddingTenders, filterReceiveAwardedTaskFacts, filterReceivePendingAcceptTasks, filterReceiveQuotedTenders, } from '../data/fcs/pda-receive-scope.ts';
...
const submittedQuotes = new Map(PDA_MOCK_QUOTED_TENDERS.map((item) => [
    item.tenderId,
    {
...
function isPostFinishingDirectOnlyFactory(factoryId) {
...
function getTaskProductionOrderNo(task) {
    if (!task)
        return '-';
    return task.productionOrderNo || task.productionOrderId;
}
...
function syncQuoteDialogWithQuery(activeBiddingTenders) {
    const tenderId = getCurrentSearchParams().get('quoteTenderId');
    if (!tenderId || state.activeTab !== 'pending-quote')
        return;
...
function getDeadlineStatus(deadline) {
    ...
    if (diff < 0)
        return { label: '接单逾期', variant: 'destructive' };
    if (hours < 4)
        return { label: '即将逾期', variant: 'default' };
    return { label: '正常', variant: 'secondary' };
}
function getDeadlineBadgeClass(variant) {
...
function getTaskPricing(task) {
    const directPrice = task.directPrice ?? task.dispatchPrice;
...
function mutateAcceptTask(taskId, by) {
    const now = nowTimestamp();
    const task = getTaskFactById(taskId);
    if (!task)
        return;
    const woolOrderId = task.woolOrderId;
...
function mutateRejectTask(taskId, reason, by) {
    const now = nowTimestamp();
    const task = getTaskFactById(taskId);
    if (!task)
        return;
...
function showTaskReceiveToast(message) {
...
function getTabCounts(pendingAcceptTasks, activeBiddingTenders, quotedTenders, awardedTenders) {
    return {
...
function getQuotedTenders(selectedFactoryId) {
...
function getActiveBiddingTenders() {
...
function getAwardedTenders(selectedFactoryId) {
    const taskFacts = listPdaMobileExecutionTasks();
    const awardedTaskIds = new Set(listPdaAwardedTenderNoticesByFactoryId(selectedFactoryId).map((item) => item.taskId));
    return filterReceiveAwardedTaskFacts(taskFacts, selectedFactoryId)
        ...
        execStatus: task.acceptanceStatus === 'REJECTED'
            ? '已拒接'
...
function formatPendingAcceptTaskQty(task) {
    return `${task.qty} 件`;
}
function formatPendingAcceptDispatchPrice(task) {
    const pricing = getTaskPricing(task);
    if (pricing.directPrice == null)
        return '-';
    const unit = task.dispatchPriceUnit || pricing.unit || '件';
    return `${pricing.directPrice.toLocaleString()} ${pricing.currency}/${unit}`;
}
function renderPendingAcceptFieldGrid(task) {
    return `
...
function getFocusedTaskId() {
    ...
    if (!navContext.highlightTask)
        return null;
    return navContext.focusTaskId || navContext.taskId || null;
}
function shouldHighlightTask(taskId) {
    return getFocusedTaskId() === taskId;
}
function scheduleTaskFocus(taskId) {
    if (!taskId || typeof document === 'undefined' || typeof window === 'undefined')
        return;
    const focusToken = `${appStore.getState().pathname}::${taskId}`;
    if (lastFocusedTaskToken === focusToken)
        return;
    lastFocusedTaskToken = focusToken;
    window.requestAnimationFrame(() => {
        const card = document.querySelector(`[data-pda-cutting-task-card-id="${taskId}"]`);
        card?.scrollIntoView({ block: 'center' });
    });
}
function buildCuttingTaskListReturnTo(taskId) {
...
function buildCuttingTaskReceiveDetailHref(task) {
...
function buildCuttingTaskDetailEntryHref(task) {
...
function buildCuttingTaskExecHref(task) {
...
function renderCuttingTaskOrderSummary(task) {
    return `
    <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
      ${renderFieldRow('生产单号', getTaskProductionOrderNo(task))}
...
function renderPendingAcceptCuttingTask(task, factoryName) {
...
function renderPendingAcceptTask(task, factoryName) {
    if (isCuttingSpecialTask(task)) {
        return renderPendingAcceptCuttingTask(task, factoryName);
    }
    const deadlineStatus = getDeadlineStatus(task.acceptDeadline || '');
    return `
    <article class="overflow-hidden rounded-lg border bg-card">
      <div class="space-y-2 p-3">
        <div class="flex items-center justify-between gap-2">
          <span class="truncate font-mono text-sm font-semibold">${escapeHtml(getTaskDisplayNo(task))}</span>
          <span class="inline-flex shrink-0 items-center rounded border px-1.5 text-[10px] ${toClassName(getDeadlineBadgeClass(deadlineStatus.variant), deadlineStatus.label === '即将逾期' ? 'bg-amber-500 text-white border-amber-500' : '')}">${escapeHtml(deadlineStatus.label)}</span>
        </div>
...
function renderPendingQuoteItem(tender) {
...
function renderQuotedItem(tender) {
    const task = getTaskFactById(tender.taskId);
    const processName = task ? getTaskProcessDisplayName(task) : tender.processName;
    return `
    <article class="overflow-hidden rounded-lg border bg-card">
...
function renderAwardedCuttingTask(task, item) {
...
function renderAwardedItem(item) {
    const task = getTaskFactById(item.taskId);
    if (task && isCuttingSpecialTask(task)) {
        return renderAwardedCuttingTask(task, item);
    }
...
function renderQuoteDialog(quotingTender) {
    if (!state.quoteDialogOpen)
        return '';
...
export function renderPdaTaskReceivePage() {
    ...
    const tabCounts = getTabCounts(pendingAcceptTasks, activeBiddingTenders, allQuotedTenders, awardedTenders);
    ...
    <div class="flex min-h-[760px] flex-col bg-background">
        ...
        ? `
              <div class="flex gap-2">
                <select class="h-8 flex-1 rounded-md border bg-background px-2 text-xs" data-pda-tr-field="processFilter">
                  ...
                  ${processOptions
            .map((processName) => `<option value="${escapeHtml(processName)}" ${state.processFilter === processName ? 'selected' : ''}>${escapeHtml(processName)}</option>`)
            .join('')}
                </select>
      ...
      <div class="sticky top-[auto] z-20 flex border-b bg-background">
        ...
        return `
            <button
              class="flex-1 border-b-2 py-2.5 text-xs font-medium transition-colors ${toClassName(active ? 'border-primary text-primary' : 'border-transparent text-muted-foreground')}"
              data-pda-tr-action="switch-tab"
              data-tab="${tab.key}"
            >
              ${escapeHtml(tab.label)}
              ${tabCounts[tab.key] > 0
            ? `<span class="ml-1 inline-block rounded-full px-1.5 py-0 text-[10px] leading-4 ${toClassName(active ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground')}">${tabCounts[tab.key]}</span>`
            : ''}
            </button>
          `;
    }).join('')}
...
export function handlePdaTaskReceiveEvent(target) {
    if (!ensurePdaSessionForAction())
        return true;
    const fieldNode = target.closest('[data-pda-tr-field]');
    ...
    const actionNode = target.closest('[data-pda-tr-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'open-detail') {
        ...
        if (taskId) {
            const task = getTaskFactById(taskId);
            if (isCuttingSpecialTask(task)) {
                appStore.navigate(buildCuttingTaskReceiveDetailHref(task));
            }
            else {
    ...
    if (action === 'open-exec') {
        ...
        if (taskId) {
            const task = getTaskFactById(taskId);
            if (isCuttingSpecialTask(task)) {
                appStore.navigate(buildCuttingTaskExecHref(task));
            }
            else {
    ...
    if (action === 'confirm-reject') {
        if (!state.rejectReason.trim() || !state.rejectingTaskId)
            return true;
