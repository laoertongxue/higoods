import { state, candidateFactories, createDefaultAutoDispatchConfig, getVisibleRows, getFactoryOptions, nowTimestamp, getCreateTenderTask, getTaskAllocatableGroups, getSelectableTenderFactoryIds, openAppRoute, supportsDetailAssignment, } from './context.ts';
import { applyAutoAssign, openDispatchDialog, closeDispatchDialog, confirmDirectDispatch, setTaskAssignMode, batchSetTaskAssignMode, } from './dispatch-domain.ts';
import { openCreateTender, closeCreateTender, confirmCreateTender, openViewTender, closeViewTender, closePriceSnapshot, } from './tender-domain.ts';
function getTenderSelectableFactoryIds() {
    const task = getCreateTenderTask();
    if (!task)
        return new Set(candidateFactories.map((factory) => factory.id));
...
export function handleDispatchBoardEvent(target) {
    const fieldNode = target.closest('[data-dispatch-field]');
    ...
    const actionNode = target.closest('[data-dispatch-action]');
    if (!actionNode) {
    ...
    if (action === 'clear-all-pool') {
        state.createTenderError = null;
        state.createTenderForm.selectedPool = new Set();
        state.createTenderForm.mainFactoryId = '';
        state.createTenderForm.mainFactoryName = '';
        return true;
    }
