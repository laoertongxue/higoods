import { state, pathZh, resultZh, resultBadgeClass, colLabel, colHeaderColor, colBg, priceStatusLabel, priceStatusClass, productionOrders, deriveAssignPath, deriveAssignResult, deriveKanbanCol, getEffectiveTender, formatDeadlineBadge, getDeadlineStatus, currentCheckpoint, isAffectedByTaskSet, formatScopeLabel, formatTaskNo, formatPublishedSamNumber, hasTender, getPriceStatus, getStandardPrice, resolveTaskPublishedSam, calcRemaining, escapeHtml, } from './context.ts';
function renderKanbanCard(task, dyePendingTaskIds, qcPendingOrderIds, exceptionTaskIds) {
    ...
    const checkpoint = currentCheckpoint(task, assignResult, tender, dyePendingTaskIds, qcPendingOrderIds, hasException);
    ...
    const directSummary = (() => {
        if (assignResult !== 'DIRECT_ASSIGNED')
            return '';
...
function renderKanbanView(rows, dyePendingTaskIds, qcPendingOrderIds, exceptionTaskIds) {
    const cols = {
        UNASSIGNED: [],
        BIDDING: [],
        AWAIT_AWARD: [],
    ...
    <div class="flex gap-3 overflow-x-auto pb-4 pt-2">
      ${colOrder
        ...
        return `
            ...
            : cols[col]
                .map((task) => renderKanbanCard(task, dyePendingTaskIds, qcPendingOrderIds, exceptionTaskIds))
                .join('')}
                </div>
              </div>
            </section>
...
function renderDispatchListPagination(total, currentPage, pageCount, pageSize) {
    const pages = [];
...
function renderListView(rows, exceptionTaskIds) {
        ...
        : pageRows
            ...
            return `
                ...
                : '<span class="text-xs text-muted-foreground">—</span>'}
                          ...
                          <td class="px-3 py-3 text-xs">${isDirect && task.assignedFactoryName
                ? `<span class="font-medium text-green-700">${escapeHtml(task.assignedFactoryName)}</span>`
                : '<span class="text-muted-foreground">—</span>'}</td>

                          <td class="px-3 py-3 text-xs text-muted-foreground">${isDirect && task.acceptDeadline ? escapeHtml(task.acceptDeadline.slice(0, 16).replace('T', ' ')) : '—'}</td>

                          <td class="px-3 py-3 text-xs text-muted-foreground">${isDirect && task.taskDeadline ? escapeHtml(task.taskDeadline.slice(0, 16).replace('T', ' ')) : '—'}</td>

                          <td class="px-3 py-3 text-xs tabular-nums text-muted-foreground">${isDirect ? `${std.price.toLocaleString()} ${escapeHtml(std.currency)}/${escapeHtml(std.unit)}` : '—'}</td>

                          <td class="px-3 py-3 text-xs tabular-nums">${isDirect && task.dispatchPrice != null
                ...
                : '<span class="text-xs text-muted-foreground">—</span>'}
                          ...
                          <td class="px-3 py-3 font-mono text-xs">${isBid && tender ? `<span class="text-orange-700">${escapeHtml(tender.tenderId)}</span>` : '<span class="text-muted-foreground">—</span>'}</td>
                          ...
                          <td class="px-3 py-3 text-xs">${assignResult === 'AWARDED' && awardedFactory
                ? `<span class="font-medium text-green-700">${escapeHtml(awardedFactory)}</span>`
                : '<span class="text-muted-foreground">—</span>'}</td>

                          <td class="px-3 py-3 text-xs tabular-nums">${assignResult === 'AWARDED' && awardedPrice != null
