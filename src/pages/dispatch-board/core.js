import { state, getVisibleRows, getExceptionTaskIds, deriveKanbanCol, isAffectedByTaskSet, getDispatchDialogTasks, getCreateTenderTask, getViewTenderTask, getPriceSnapshotTask, getFactoryOptions, isRuntimeSewingTask, createDefaultAutoDispatchConfig, getAutoDispatchConfigKeyFromTask, escapeHtml, } from './context.ts';
import { renderDirectDispatchDialog } from './dispatch-domain.ts';
import { renderCreateTenderSheet, renderViewTenderSheet, renderPriceSnapshotSheet, } from './tender-domain.ts';
import { renderListView } from './board-domain.ts';
import { listProcessCraftDictRows } from '../../data/fcs/process-craft-dict.ts';
const dispatchListTabs = [
...
function getDispatchListTabKey(task, exceptionTaskIds) {
    return deriveKanbanCol(task, isAffectedByTaskSet(task, exceptionTaskIds));
}
function getDispatchListTabCounts(rows, exceptionTaskIds) {
    const counts = dispatchListTabs.reduce((result, tab) => {
        result[tab.key] = 0;
        return result;
    }, {});
...
function renderDispatchListTabs(counts) {
    return `
    <section class="flex gap-2 overflow-x-auto rounded-lg border bg-card p-2" data-dispatch-list-tabs="true">
      ${dispatchListTabs
        ...
        return `
            <button
              class="inline-flex shrink-0 items-center gap-2 rounded-md border px-3 py-2 text-sm transition ${active ? 'border-blue-200 bg-blue-50 text-blue-700 shadow-sm' : 'border-transparent hover:bg-muted'}"
              data-dispatch-action="switch-list-tab"
              data-tab="${tab.key}"
            >
              <span class="font-medium">${escapeHtml(tab.label)}</span>
              <span class="rounded-full px-2 py-0.5 text-xs font-semibold ${exceptionTone
            ? 'bg-red-100 text-red-700'
            : active
                ? 'bg-blue-100 text-blue-700'
                : 'bg-muted text-muted-foreground'}">${counts[tab.key]}</span>
            </button>
          `;
    })
        .join('')}
...
function getAutoDispatchConfigRows(rows) {
...
function sortAutoDispatchDictionaryRows(left, right) {
...
function renderAutoDispatchToolbar() {
    return `
...
function renderAutoAssignMessage() {
    if (!state.autoAssignMessage)
        return '';
    ...
    <section class="space-y-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
      ...
      <div class="space-y-1">
        ...
        ? `<div class="flex flex-wrap gap-2">
            ...
            .map((item) => `<span class="rounded-full border border-blue-200 bg-white px-2 py-0.5 text-xs">${escapeHtml(item.label)}：${item.count} 条</span>`)
            .join('')}
              </div>`
...
function renderAutoDispatchConfigDialog(rows) {
    if (!state.autoDispatchConfigOpen)
        return '';
                            ...
                            <tr class="border-b last:border-b-0" data-auto-dispatch-config-row="${escapeHtml(item.configKey)}">
                              ...
                              <td class="px-3 py-3">
                                <select class="h-8 w-full rounded-md border bg-background px-2 text-xs" data-dispatch-field="auto.factoryId" data-process-code="${escapeHtml(item.configKey)}">
                                  ...
                                  ${factoryOptions
                .map((factory) => `<option value="${escapeHtml(factory.id)}" ${config.factoryId === factory.id ? 'selected' : ''}>${escapeHtml(factory.name)}</option>`)
                .join('')}
                                </select>
                              </td>
                              <td class="px-3 py-3">
...
function renderDispatchBoardInner() {
    ...
    const listRows = state.listTab === 'ALL'
        ? allRows
