import { state, validateRuntimeBatchDispatchSelection, createDispatchCapacityEvaluationContext, createDispatchStandardTimeEvaluationContext, getTaskAllocatableGroups, supportsDetailAssignment, dispatchRuntimeTaskByDetailGroups, applyRuntimeDirectDispatchMeta, setRuntimeTaskAssignMode, batchSetRuntimeTaskAssignMode, batchDispatchRuntimeTasks, isRuntimeSewingTask, getTaskById, getVisibleRows, getDispatchDialogTasks, getDispatchDialogValidation, getStandardPrice, getAutoDispatchConfigKeyFromTask, emptyDispatchForm, describeDispatchCapacityConstraintDecision, formatPublishedSamNumber, fromDateTimeLocal, nowTimestamp, escapeHtml, formatScopeLabel, formatTaskNo, createFreezeFromDirectDispatch, resolveAllocatableGroupPublishedSam, resolveAllocatableGroupFactoryCapacityConstraint, resolveAllocatableGroupFactoryStandardTimeJudgement, resolveTaskPublishedSam, resolveTaskFactoryCapacityConstraint, resolveTaskFactoryStandardTimeJudgement, syncDispatchCapacityUsageLedger, } from './context.ts';
function setTaskAssignMode(taskId, mode, by) {
    setRuntimeTaskAssignMode(taskId, mode, by);
}
function batchSetTaskAssignMode(taskIds, mode, by) {
    batchSetRuntimeTaskAssignMode(taskIds, mode, by);
}
function batchDispatch(taskIds, factoryId, factoryName, acceptDeadline, taskDeadline, remark, by, dispatchPrice, dispatchPriceCurrency, dispatchPriceUnit, priceDiffReason) {
...
function getDispatchSingleTask() {
    const tasks = getDispatchDialogTasks();
    return tasks.length === 1 ? tasks[0] : null;
}
function getDirectDispatchGroups(task) {
    return getTaskAllocatableGroups(task);
}
function getDirectDispatchAssignments(groups) {
    return groups
        .map((group) => {
        const selected = state.dispatchForm.factoryByGroupKey[group.groupKey];
        if (!selected?.factoryId || !selected.factoryName)
            return null;
        return {
...
function getConstraintTone(snapshot) {
    if (!snapshot)
...
function renderCapacityConstraintSummary(snapshot, placeholder, testId) {
    if (!snapshot) {
...
function getStandardTimeJudgementTone(snapshot) {
    if (!snapshot)
        return 'border-slate-200 bg-slate-50 text-slate-600';
    if (snapshot.status === 'EXCEEDS_WINDOW')
        return 'border-red-200 bg-red-50 text-red-700';
...
function renderStandardTimeJudgementSummary(snapshot, placeholder, testId) {
    if (!snapshot) {
        return `<div class="rounded-md border border-dashed px-3 py-2 text-xs text-muted-foreground"${testId ? ` data-${testId}="empty"` : ''}>${escapeHtml(placeholder)}</div>`;
    }
...
function openDispatchDialog(taskIds) {
...
function applyAutoAssign() {
    ...
    const assignedCountByProcessCraft = new Map();
...
function confirmDirectDispatch() {
    const tasks = getDispatchDialogTasks();
    if (tasks.length === 0)
        return;
    ...
    if (detailMode && singleTask) {
        ...
        for (const group of groups) {
            const selected = state.dispatchForm.factoryByGroupKey[group.groupKey];
            if (!selected?.factoryId)
                continue;
            const constraint = resolveAllocatableGroupFactoryCapacityConstraint(singleTask, group, selected.factoryId, selected.factoryName, evaluationContext ?? undefined);
        ...
        for (const assignment of result.resultAssignments) {
            ...
            assignmentsByTaskId.set(assignment.taskId, {
                ...current,
                publishedSamTotal: (current.publishedSamTotal ?? 0) + (assignment.publishedSamTotal ?? 0),
            });
        }
    ...
    if (singleTask) {
        const constraint = resolveTaskFactoryCapacityConstraint(singleTask, state.dispatchForm.factoryId, state.dispatchForm.factoryName, evaluationContext ?? undefined);
        if (constraint?.hardBlocked) {
            state.dispatchDialogError = `当前工厂不可派单：${constraint.reason}`;
            return;
        }
    }
    const result = batchDispatch(tasks.map((task) => task.taskId), state.dispatchForm.factoryId, state.dispatchForm.factoryName, acceptDeadline, taskDeadline, state.dispatchForm.remark, '跟单A', validation.dispatchPrice, validation.stdCurrency, validation.stdUnit, state.dispatchForm.priceDiffReason);
    if (!result.ok) {
...
function renderDetailDispatchMode(task, groups, factoryOptions, evaluationContext, samEvaluationContext) {
    const assignmentGranularity = task.assignmentGranularity ?? 'ORDER';
    const isSewingTask = isRuntimeSewingTask(task);
    const assignmentGranularityLabel = {
        ORDER: '按生产单',
        COLOR: '按颜色',
        SKU: '按SKU',
        DETAIL: '按明细行',
    ...
    <div class="space-y-3 rounded-md border bg-muted/15 p-3">
      ...
      <div class="overflow-x-auto rounded-md border">
        ...
        const selectedConstraint = selectedFactoryId
            ? resolveAllocatableGroupFactoryCapacityConstraint(task, group, selectedFactoryId, selectedFactory?.factoryName, evaluationContext)
            : null;
        const selectedSamJudgement = selectedFactoryId
            ? resolveAllocatableGroupFactoryStandardTimeJudgement(task, group, selectedFactoryId, samEvaluationContext)
            : null;
        const unitSamText = groupSam.publishedSamPerUnit && groupSam.publishedSamUnit
            ? `${formatPublishedSamNumber(groupSam.publishedSamPerUnit)} ${escapeHtml(groupSam.publishedSamUnit)}`
            : '--';
        const totalSamText = groupSam.publishedSamTotal != null && groupSam.publishedSamUnit
        ...
        return `
            ...
            .join('')}
                        ...
                        <div class="mt-2" data-dispatch-group-constraint="${escapeHtml(group.groupKey)}">
                          ${renderCapacityConstraintSummary(selectedConstraint, '选择工厂后，将按该分配单元的任务日期窗口校验产能日历状态。')}
                        </div>
                        <div class="mt-2" data-dispatch-group-sam-judgement="${escapeHtml(group.groupKey)}">
                          ${renderStandardTimeJudgementSummary(selectedSamJudgement, '选择工厂后，将显示该分配单元在未来窗口内的标准工时判断。')}
                        </div>
                      </td>
                    </tr>
                  `;
...
function renderDirectDispatchDialog(tasks, factoryOptions) {
    if (!state.dispatchDialogTaskIds || tasks.length === 0)
        return '';
    ...
    const hasSelectedDetailFactory = detailMode && groups.some((group) => Boolean(state.dispatchForm.factoryByGroupKey[group.groupKey]?.factoryId));
    ...
    const selectedTaskConstraint = !isBatch && state.dispatchForm.factoryId
        ? resolveTaskFactoryCapacityConstraint(refTask, state.dispatchForm.factoryId, state.dispatchForm.factoryName, evaluationContext)
        : null;
    const selectedTaskSamJudgement = !isBatch && state.dispatchForm.factoryId
        ? resolveTaskFactoryStandardTimeJudgement(refTask, state.dispatchForm.factoryId, samEvaluationContext)
        : null;
    const detailBlocked = detailMode &&
        groups.some((group) => {
            const selected = state.dispatchForm.factoryByGroupKey[group.groupKey];
            if (!selected?.factoryId)
                return false;
            const constraint = resolveAllocatableGroupFactoryCapacityConstraint(refTask, group, selected.factoryId, selected.factoryName, evaluationContext);
            return Boolean(constraint?.hardBlocked);
        });
    const unitSamText = taskSam.publishedSamPerUnit && taskSam.publishedSamUnit
        ? `${formatPublishedSamNumber(taskSam.publishedSamPerUnit)} ${escapeHtml(taskSam.publishedSamUnit)}`
        : '--';
    const totalSamText = taskSam.publishedSamTotal != null && taskSam.publishedSamUnit
        ? `${formatPublishedSamNumber(taskSam.publishedSamTotal)} ${escapeHtml(taskSam.publishedSamUnit.replace(/^分钟\//, '分钟'))}`
        : '--';
    const selectionError = state.dispatchDialogError ??
        (!selectionValidation.valid ? selectionValidation.reason ?? '批量派单条件不满足' : '');
    const detailMainFactorySelected = !detailMode ||
        !isRuntimeSewingTask(refTask) ||
        Boolean(state.dispatchForm.mainFactoryGroupKey &&
            state.dispatchForm.factoryByGroupKey[state.dispatchForm.mainFactoryGroupKey]?.factoryId);
    const canSubmit = selectionValidation.valid &&
        validation.valid &&
      ...
      <section class="absolute left-1/2 top-1/2 max-h-[90vh] w-full max-w-3xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl border bg-background p-6 shadow-2xl" data-dialog-panel="true">
        ...
        : `<div class="space-y-1.5">
                  ...
                  <select class="h-9 w-full rounded-md border bg-background px-3 text-sm" data-dispatch-field="dispatch.factoryId">
                    ...
                    ${factoryOptions
            .map((factory) => `<option value="${escapeHtml(factory.id)}" ${state.dispatchForm.factoryId === factory.id ? 'selected' : ''}>${escapeHtml(factory.name)}</option>`)
            .join('')}
                  </select>
                  <div data-dispatch-task-constraint="selected-factory">
                    ${renderCapacityConstraintSummary(selectedTaskConstraint, isBatch
            ? '批量派单当前不逐任务做产能日历校验。'
            : '选择工厂后，将按当前任务日期窗口校验产能日历状态。')}
                  </div>
                  <div data-dispatch-task-sam-judgement="selected-factory">
                    ${renderStandardTimeJudgementSummary(selectedTaskSamJudgement, isBatch
        ...
        ? `<div class="flex items-center justify-between gap-2">
                    ...
                    <span class="text-sm font-medium tabular-nums ${!validation.changed
            ? 'text-green-700'
            : (validation.diff ?? 0) > 0
                ? 'text-amber-700'
                : 'text-blue-700'}">
...
export { setTaskAssignMode, batchSetTaskAssignMode, batchDispatch, openDispatchDialog, closeDispatchDialog, applyAutoAssign, confirmDirectDispatch, renderDirectDispatchDialog, };
