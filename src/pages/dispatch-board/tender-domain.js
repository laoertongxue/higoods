import { state, candidateFactories, createDispatchCapacityEvaluationContext, createDispatchStandardTimeEvaluationContext, getTaskById, getTaskAllocatableGroups, supportsDetailAssignment, hasTender, getEffectiveTender, getStandardPrice, formatScopeLabel, formatTaskNo, fromDateTimeLocal, nowTimestamp, describeDispatchCapacityConstraintDecision, createRuntimeTaskTenderByDetailGroups, upsertRuntimeTaskTender, isRuntimeSewingTask, getCreateTenderTask, priceStatusLabel, priceStatusClass, getPriceStatus, emptyCreateTenderForm, formatPublishedSamNumber, escapeHtml, resolveAllocatableGroupPublishedSam, resolveAllocatableGroupFactoryCapacityConstraint, resolveAllocatableGroupFactoryStandardTimeJudgement, resolveTaskPublishedSam, resolveTenderFactoryCapacityConstraint, resolveTenderFactoryStandardTimeJudgement, syncDispatchCapacityUsageLedger, } from './context.ts';
...
function renderConstraintBadge(snapshot) {
    if (!snapshot)
        return '<span class="inline-flex rounded border border-slate-200 bg-slate-50 px-1.5 py-0 text-[10px] text-slate-600">待校验</span>';
...
function renderStandardTimeJudgementBlock(snapshot, options) {
    if (!snapshot) {
...
function openCreateTender(taskId) {
...
function closeViewTender() {
    state.viewTenderTaskId = null;
}
function closePriceSnapshot() {
    state.priceSnapshotTaskId = null;
}
function renderCreateTenderSheet(task) {
    if (!task || !state.createTenderTaskId)
        return '';
    ...
    const candidateFactoryConstraints = new Map(selectedPoolFactories.map((factory) => [
        factory.id,
        resolveTenderFactoryCapacityConstraint(task, factory.id, factory.name, constraintGroups, evaluationContext),
    ]));
    const candidateFactorySamJudgements = new Map(selectedPoolFactories.map((factory) => [
        factory.id,
    ...
    const maxValid = state.createTenderForm.maxPrice !== '' &&
    ...
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      ...
      <section class="absolute inset-y-0 right-0 flex w-full flex-col border-l bg-background shadow-2xl sm:max-w-[560px]" data-tender-sheet="true">
        <header class="border-b bg-background px-6 py-4">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-muted-foreground">${detailSupported
        ...
        ? `<div class="space-y-2">
            ...
            const groupUnitSamText = groupSam.publishedSamPerUnit && groupSam.publishedSamUnit
                ? `${formatPublishedSamNumber(groupSam.publishedSamPerUnit)} ${escapeHtml(groupSam.publishedSamUnit)}`
                : '--';
            const groupTotalSamText = groupSam.publishedSamTotal != null && groupSam.publishedSamUnit
        ...
        const detailStatusBlocks = selected && detailMode
            ? detailGroups
                .map((group) => {
                const groupConstraint = resolveAllocatableGroupFactoryCapacityConstraint(task, group, factory.id, factory.name, evaluationContext);
                return `
        ...
        const detailSamBlocks = selected && detailMode
            ? detailGroups
                .map((group) => {
                const groupJudgement = resolveAllocatableGroupFactoryStandardTimeJudgement(task, group, factory.id, samEvaluationContext);
                return `
	                    ...
	                    <button class="flex w-full items-start gap-3 px-3 py-2.5 text-left transition-colors ${selected ? 'bg-orange-50' : 'hover:bg-muted/40'} ${disabled ? 'cursor-not-allowed opacity-60' : ''}" data-dispatch-action="toggle-pool" data-factory-id="${escapeHtml(factory.id)}" ${disabled ? 'disabled' : ''} data-tender-factory-option="${escapeHtml(factory.id)}" data-tender-factory-status="${escapeHtml(constraint?.status ?? 'PENDING')}">
	                      ...
	                      <span class="flex-1 min-w-0 space-y-0.5">
	                        <span class="flex items-center gap-1.5 flex-wrap">
	                          ...
	                          ${factory.processTags
            .map((tag) => `<span class="inline-flex rounded border border-blue-200 bg-blue-50 px-1.5 py-0 text-[10px] text-blue-700">${escapeHtml(tag)}</span>`)
            .join('')}
	                        </span>
        ...
        <footer class="border-t px-6 py-4">
          <div class="flex justify-end gap-2">
            <button class="rounded-md border px-4 py-2 text-sm hover:bg-muted" data-dispatch-action="close-create-tender">取消</button>
	            <button class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 ${valid && !hasBlockedSelectedPool ? '' : 'pointer-events-none opacity-50'}" data-dispatch-action="confirm-create-tender">确认创建招标单</button>
	          </div>
	        </footer>
      </section>
    </div>
...
function renderViewTenderSheet(task) {
    if (!task || !state.viewTenderTaskId)
        return '';
    ...
    const taskSam = resolveTaskPublishedSam(task);
    const viewUnitSamText = (tenderSam.publishedSamPerUnit ?? taskSam.publishedSamPerUnit) &&
        (tenderSam.publishedSamUnit ?? taskSam.publishedSamUnit)
        ? `${formatPublishedSamNumber(tenderSam.publishedSamPerUnit ?? taskSam.publishedSamPerUnit)} ${escapeHtml((tenderSam.publishedSamUnit ?? taskSam.publishedSamUnit))}`
        : '--';
    const resolvedSamUnit = tenderSam.publishedSamUnit ?? taskSam.publishedSamUnit;
    const viewTotalSamText = (tenderSam.publishedSamTotal ?? taskSam.publishedSamTotal) != null && resolvedSamUnit
    ...
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      ...
      <section class="absolute inset-y-0 right-0 flex w-full flex-col border-l bg-background shadow-2xl sm:max-w-[480px]">
        ...
        ? `<div class="flex flex-wrap gap-1.5 rounded-md border px-3 py-2">${poolNames
            .map((name) => `<span class="inline-flex rounded-full border border-orange-200 bg-orange-50 px-2 py-0.5 text-xs text-orange-800">${escapeHtml(name)}</span>`)
            .join('')}</div>`
...
function renderPriceSnapshotSheet(task) {
    if (!task || !state.priceSnapshotTaskId)
        return '';
    const std = getStandardPrice(task);
    ...
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      ...
      <section class="absolute inset-y-0 right-0 flex w-full flex-col border-l bg-background shadow-2xl sm:max-w-[360px]">
        ...
        ? `<div class="space-y-1">
            ...
            : ''}
                    <span class="text-sm font-medium tabular-nums ${diff === 0 ? 'text-green-700' : diff > 0 ? 'text-amber-700' : 'text-blue-700'}">${diff === 0
            ? '0（0%）'
            : `${diff > 0 ? '+' : ''}${diff.toLocaleString()} ${escapeHtml(std.currency)}/${escapeHtml(std.unit)}（${diff > 0 ? '+' : ''}${diffPct}%）`}</span>
                  </div>
                </div>`
        : ''}
...
function confirmCreateTender() {
    ...
    const minPrice = Number(state.createTenderForm.minPrice);
    const maxPrice = Number(state.createTenderForm.maxPrice);
    const detailGroups = state.createTenderForm.mode === 'DETAIL' && supportsDetailAssignment(task)
        ? getTaskAllocatableGroups(task)
        : [];
    const evaluationContext = createDispatchCapacityEvaluationContext();
    const valid = state.createTenderForm.selectedPool.size > 0 &&
        state.createTenderForm.minPrice !== '' &&
        Number.isFinite(minPrice) &&
        minPrice > 0 &&
        state.createTenderForm.maxPrice !== '' &&
    ...
    const blockedSelections = selectedPoolIds
        ...
        const constraint = resolveTenderFactoryCapacityConstraint(task, factoryId, factory?.name, detailGroups, evaluationContext);
    ...
    if (state.createTenderForm.mode === 'DETAIL' && supportsDetailAssignment(task)) {
        ...
        if (!result.ok || !result.createdTaskIds)
            return;
        result.createdTaskIds.forEach((childTaskId, index) => {
            ...
            upsertRuntimeTaskTender(childTaskId, {
                tenderId: childTenderId,
                biddingDeadline,
                taskDeadline,
                publishedSamPerUnit: childTaskSam.publishedSamPerUnit,
    ...
    upsertRuntimeTaskTender(task.taskId, {
        tenderId: state.createTenderForm.tenderId,
        biddingDeadline: fromDateTimeLocal(state.createTenderForm.biddingDeadline),
        taskDeadline: fromDateTimeLocal(state.createTenderForm.taskDeadline),
        publishedSamPerUnit: taskSam.publishedSamPerUnit,
...
export { openCreateTender, closeCreateTender, openViewTender, closeViewTender, closePriceSnapshot, renderCreateTenderSheet, renderViewTenderSheet, renderPriceSnapshotSheet, confirmCreateTender, };
