...
import { buildCuttingSettlementFocusBuckets, buildCuttingSettlementInputEmptyStateText, buildCuttingSettlementInputStats, buildGroupContributionHeadline, buildOperatorContributionHeadline, buildSettlementReviewText, cuttingInputReviewStatusMeta, cuttingRecommendedScoreBandMeta, cuttingScoreFocusMeta, cuttingSettlementFocusMeta, filterCuttingSettlementInputViews, hasCuttingSettlementInputFilters, } from '../domain/cutting-settlement/helpers';
...
function renderProcessSelect(label, field, value, options) {
    return `
    <label class="space-y-2">
      <span class="text-sm font-medium text-foreground">${escapeHtml(label)}</span>
      <select
...
function updateRow(recordId, updater) {
    const row = state.rows.find((item) => item.id === recordId);
    if (!row)
        return;
    updater(row);
...
function applyCommonReviewPatch(row, next) {
    ...
    if (next.latestActionText)
        row.latestActionText = next.latestActionText;
    if (next.suggestedActionText)
        row.suggestedActionText = next.suggestedActionText;
    syncReviewFlags(row);
}
function markSettlementAttention(recordId) {
...
function markScoreAttention(recordId) {
    updateRow(recordId, (row) => {
...
function markReviewing(recordId) {
    updateRow(recordId, (row) => {
        applyCommonReviewPatch(row, {
...
function confirmInput(recordId) {
    updateRow(recordId, (row) => {
        applyCommonReviewPatch(row, {
...
function saveProcessDraft() {
    ...
    applyCommonReviewPatch(row, {
        ...
        suggestedActionText: state.processDraft.reviewStatus === 'NEED_MORE_INFO'
...
export function renderCuttingSettlementInputPage() {
    refreshRuntimeRows();
    return `
...
export function handleCuttingSettlementInputEvent(target) {
    const fieldNode = target.closest('[data-cutting-settlement-field]');
    if (fieldNode) {
        const field = fieldNode.dataset.cuttingSettlementField;
        if (!field)
            return false;
    ...
    const processFieldNode = target.closest('[data-cutting-settlement-process-field]');
    if (processFieldNode) {
        const field = processFieldNode.dataset.cuttingSettlementProcessField;
        if (!field)
            return false;
    ...
    const actionNode = target.closest('[data-cutting-settlement-action]');
...
export function isCuttingSettlementInputDialogOpen() {
