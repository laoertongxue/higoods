...
function canCreateRevisionPatternTask(scopeCodes: string[], _sourceType: RevisionTaskSourceType, formalTargetId: string): boolean {
  return hasRevisionPatternDownstreamScope(scopeCodes) && Boolean(formalTargetId)
}
...
function renderRevisionDownstream(task: ReturnType<typeof getRevisionTaskById>): string {
  ...
  const emptyText = suggestedTypes.length === 0
      ? '当前改版范围暂无可创建的下游任务'
      : '暂无下游任务'
  const canCreateSuggestedDownstream = suggestedTypes.some((type) => task.projectId || type === 'PRINT')
  return renderSectionCard(
    '下游任务',
    `
...
function getRevisionTechPackStatusText(task: ReturnType<typeof getRevisionTaskById>): string {
  if (!task) return '-'
  if (!task.projectId) return '独立任务不生成项目技术包版本'
...
function getRevisionMissingItems(task: ReturnType<typeof getRevisionTaskById>, style: { styleId: string; styleCode: string }): string[] {
  ...
  if (task.projectId && (task.status === '已确认' || task.status === '待确认') && !task.linkedTechPackVersionId && !task.generatedNewTechPackVersionFlag) missing.push('生成改版技术包版本')
  if (task.status === '已生成技术包') {
    getRevisionTaskCompletionMissingFields(task).filter((item) => task.projectId || item !== '新技术包版本').forEach((item) => {
...
function getRevisionNextActionText(task: ReturnType<typeof getRevisionTaskById>, missingItems: string[]): string {
  ...
  if (task.status === '已确认') return task.projectId ? '生成改版技术包版本' : '完成独立改版任务'
...
function renderRevisionDetailPage(revisionTaskId: string): string {
  ...
  const actions = [
    ...
    ...(task.projectId && task.status === '已确认'
      ...
      : []),
    ...(task.status === '已生成技术包' || (!task.projectId && task.status === '已确认')
  ...
  const businessInfo = renderSectionCard('业务来源', renderKeyValueGrid([
    ...
    { label: '任务归属', value: task.projectId ? projectButton(task.projectId, task.projectCode, task.projectName) : '<span class="text-slate-700">独立改版任务</span>' },
...
function renderRevisionCreateDialog(): string {
  ...
  const canCreatePatternTask = canCreateRevisionPatternTask(draft.scopeCodes, draft.sourceType, showProjectField ? draft.projectId : draft.styleId)
  const body = `
    <div class="grid gap-4 md:grid-cols-2">
...
function renderFirstOrderCreateDialog(): string {
  ...
  const sourceFirstSampleOptions = draft.projectId
    ? listFirstOrderSourceFirstSampleOptions(draft.projectId)
    : []
  const body = `
    <div class="space-y-4">
      <div class="grid gap-4 md:grid-cols-2">
      ...
      ${renderSelectInput(
        '来源首版样衣',
        'first-order-create-source-first-sample',
        draft.sourceFirstSampleTaskId,
        sourceFirstSampleOptions.map((item) => ({ value: item.taskId, label: item.label })),
      )}
...
function submitRevisionCreate(): void {
  ...
  if (draft.createPatternTask && canCreateRevisionPatternTask(draft.scopeCodes, draft.sourceType, result.task.projectId || result.task.styleId || result.task.styleCode)) {
...
function submitFirstOrderCreate(): void {
  ...
  const sourceFirstSample = listFirstOrderSourceFirstSampleOptions(draft.projectId)
    .find((item) => item.taskId === draft.sourceFirstSampleTaskId)
  if (!sourceFirstSample) {
    setNotice('请选择当前商品项目内已通过的来源首版样衣。')
    return
  }
  ...
  const result = createFirstOrderSampleTaskWithProjectRelation({
    ...
    upstreamModule: '首版样衣打样',
    upstreamObjectType: '首版样衣打样任务',
    upstreamObjectId: sourceFirstSample.taskId,
    upstreamObjectCode: sourceFirstSample.taskCode,
...
// 列表分派注册：各模块在此声明列、行、状态与统计读取，供公共列表骨架按模块分派
registerEngineeringListModule('revision', {
  getColumns: () => REVISION_LIST_COLUMNS,
  getRows: () => getRevisionListRows(),
  getState: () => state.revisionList,
  getEmptyText: () => '暂无改版任务数据',
  getStatsHtml: () => renderRevisionListStats(),
})
registerEngineeringListModule('plate', {
  getColumns: () => PLATE_LIST_COLUMNS,
  getRows: () => getPlateListRows(),
  getState: () => state.plateList,
  getEmptyText: () => '暂无制版任务数据',
  getStatsHtml: () => renderPlateListStats(),
})
registerEngineeringListModule('firstSample', {
  getColumns: () => FIRST_SAMPLE_LIST_COLUMNS,
  getRows: () => getFirstSampleListRows(),
  getState: () => state.firstSampleList,
  getEmptyText: () => '暂无首版样衣打样数据',
  getStatsHtml: () => renderFirstSampleListStats(),
})
registerEngineeringListModule('firstOrder', {
  getColumns: () => FIRST_ORDER_LIST_COLUMNS,
  getRows: () => getFirstOrderListRows(),
  getState: () => state.firstOrderList,
  getEmptyText: () => '暂无首单样衣打样数据',
  getStatsHtml: () => renderFirstOrderListStats(),
})
...
if (field === 'color-search' && fieldNode instanceof HTMLInputElement) { state.colorList.search = fieldNode.value; state.colorList.currentPage = 1; refreshEngineeringList('color'); return true }
  if (field === 'color-status' && fieldNode instanceof HTMLSelectElement) { state.colorList.status = fieldNode.value; state.colorList.currentPage = 1; refreshEngineeringList('color'); return true }
  if (field === 'color-owner' && fieldNode instanceof HTMLSelectElement) { state.colorList.owner = fieldNode.value; state.colorList.currentPage = 1; refreshEngineeringList('color'); return true }
  if (field === 'color-source' && fieldNode instanceof HTMLSelectElement) { state.colorList.source = fieldNode.value; state.colorList.currentPage = 1; refreshEngineeringList('color'); return true }
if (field === 'purchase-search' && fieldNode instanceof HTMLInputElement) { state.purchaseList.search = fieldNode.value; state.purchaseList.currentPage = 1; refreshEngineeringList('purchase'); return true }
  if (field === 'purchase-status' && fieldNode instanceof HTMLSelectElement) { state.purchaseList.status = fieldNode.value; state.purchaseList.currentPage = 1; refreshEngineeringList('purchase'); return true }
  if (field === 'purchase-owner' && fieldNode instanceof HTMLSelectElement) { state.purchaseList.owner = fieldNode.value; state.purchaseList.currentPage = 1; refreshEngineeringList('purchase'); return true }
  if (field === 'purchase-source' && fieldNode instanceof HTMLSelectElement) { state.purchaseList.source = fieldNode.value; state.purchaseList.currentPage = 1; refreshEngineeringList('purchase'); return true }
if (field === 'tech-pack-search' && fieldNode instanceof HTMLInputElement) { state.techPackList.search = fieldNode.value; state.techPackList.currentPage = 1; refreshEngineeringList('techPack'); return true }
  if (field === 'tech-pack-status' && fieldNode instanceof HTMLSelectElement) { state.techPackList.status = fieldNode.value; state.techPackList.currentPage = 1; refreshEngineeringList('techPack'); return true }
  if (field === 'tech-pack-owner' && fieldNode instanceof HTMLSelectElement) { state.techPackList.owner = fieldNode.value; state.techPackList.currentPage = 1; refreshEngineeringList('techPack'); return true }
  if (field === 'tech-pack-source' && fieldNode instanceof HTMLSelectElement) { state.techPackList.source = fieldNode.value; state.techPackList.currentPage = 1; refreshEngineeringList('techPack'); return true }
    ...
    switch (field) {
      ...
      case 'revision-create-project': {
        ...
        state.revisionCreateDraft.createPatternTask = canCreateRevisionPatternTask(
          ...
          state.revisionCreateDraft.bindingMode === 'project' ? value : state.revisionCreateDraft.styleId,
        )
        return true
      }
      ...
      case 'first-order-create-project': {
        ...
        state.firstOrderCreateDraft.sourceFirstSampleTaskId = listFirstOrderSourceFirstSampleOptions(value)[0]?.taskId || ''
        return true
      }
      case 'first-order-create-source-first-sample': state.firstOrderCreateDraft.sourceFirstSampleTaskId = value; return true
...
export function handlePcsEngineeringTaskEvent(target: HTMLElement, event?: Event): boolean {
  ...
  if (action === 'set-color-quick-filter') { state.colorList.quickFilter = actionNode.dataset.quickFilter || 'all'; state.colorList.currentPage = 1; refreshEngineeringList('color', true); return true }
  if (action === 'set-purchase-quick-filter') { state.purchaseList.quickFilter = actionNode.dataset.quickFilter || 'all'; state.purchaseList.currentPage = 1; refreshEngineeringList('purchase', true); return true }
  if (action === 'set-tech-pack-quick-filter') { state.techPackList.quickFilter = actionNode.dataset.quickFilter || 'all'; state.techPackList.currentPage = 1; refreshEngineeringList('techPack', true); return true }
  ...
  if (action === 'toggle-revision-scope') {
    ...
    state.revisionCreateDraft.createPatternTask = canCreateRevisionPatternTask(
      state.revisionCreateDraft.scopeCodes,
      state.revisionCreateDraft.sourceType,
      state.revisionCreateDraft.bindingMode === 'project'
        ? state.revisionCreateDraft.projectId
        : state.revisionCreateDraft.styleId,
    )
    return true
  ...
  if (action === 'toggle-revision-create-pattern-task') {
    if (!canCreateRevisionPatternTask(
      state.revisionCreateDraft.scopeCodes,
      state.revisionCreateDraft.sourceType,
      state.revisionCreateDraft.bindingMode === 'project'
        ? state.revisionCreateDraft.projectId
...
export function resetPcsEngineeringTaskState(): void {
  ...
  state.colorList = { search: '', status: 'all', owner: 'all', source: 'all', quickFilter: 'all', currentPage: 1 }
  state.purchaseList = { search: '', status: 'all', owner: 'all', source: 'all', quickFilter: 'all', currentPage: 1 }
  state.techPackList = { search: '', status: 'all', owner: 'all', source: 'all', quickFilter: 'all', currentPage: 1 }
