import { state, cycleTypeConfig, pricingModeConfig, getSettlementRequestById, getSettlementVersionHistory, getSettlementStatusClass, getSettlementStatusLabel, maskBankAccountNo, escapeHtml, } from './context';
import { buildSettlementChangeRequestPrintLink } from '../../data/fcs/fcs-route-links.ts';
export function renderSettlementRequestDetailDialog() {
    if (state.dialog.type !== 'request-detail')
        return '';
    const request = getSettlementRequestById(state.dialog.requestId);
    if (!request)
        return '';
    const currentEffectiveInfo = state.effectiveInfos.find((item) => item.factoryId === request.factoryId) ?? null;
    ...
    const targetVersionHint = request.status === 'APPROVED' ? request.targetVersionNo : `${request.currentVersionNo} -> ${request.targetVersionNo}`;
      ...
      <section class="absolute inset-y-0 right-0 w-full border-l bg-background shadow-2xl sm:max-w-[680px]" data-dialog-panel="true">
        ...
        ? `
            ...
            ? request.signedProofFiles
                .map((file) => `<p>${escapeHtml(file.name)} · ${escapeHtml(file.uploadedAt)} · ${escapeHtml(file.uploadedBy)}</p>`)
                .join('')
...
export function renderSettlementRequestPrintDialog() {
            ...
            <article class="mx-auto max-w-[680px] rounded-lg border bg-card p-6 text-sm" id="settlement-print-area">
              ...
              <section class="mt-4 rounded-md border p-3 text-xs leading-6">
                <p>
                  ...
                  开户名“${escapeHtml(request.before.accountHolderName)}”、证件号“${escapeHtml(request.before.idNumber)}”、银行“${escapeHtml(request.before.bankName)}”、账号“${escapeHtml(maskBankAccountNo(request.before.bankAccountNo))}”、支行“${escapeHtml(request.before.bankBranch || '—')}”
                  变更为
                  开户名“${escapeHtml(request.after.accountHolderName)}”、证件号“${escapeHtml(request.after.idNumber)}”、银行“${escapeHtml(request.after.bankName)}”、账号“${escapeHtml(maskBankAccountNo(request.after.bankAccountNo))}”、支行“${escapeHtml(request.after.bankBranch || '—')}”。
                </p>
              </section>
