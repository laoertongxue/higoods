import { state, CURRENCIES, cycleTypeConfig, pricingModeConfig, ruleModeConfig, ruleTypeConfig, escapeHtml, syncSettlementRequestState, resetInitEditor, hasInitializedSettlement, getInitDraftByFactory, getUninitializedFactories, } from './context';
export function renderInitFactoryPickerDialog() {
    if (state.dialog.type !== 'init-factory-picker')
        return '';
    ...
    const candidates = keyword
        ? allCandidates.filter((item) => item.factoryName.toLowerCase().includes(keyword) || item.factoryId.toLowerCase().includes(keyword))
        : allCandidates;
    const selectedFactory = (state.initSelectedFactoryId
...
export function renderSettlementInitPage(factoryId) {
    ...
    if (initialized) {
      ...
      <div class="space-y-4">
        <div class="rounded-lg border bg-card p-6">
          ...
          <div class="mt-4">
            <button class="rounded-md border px-3 py-2 text-sm hover:bg-muted" data-nav="/fcs/factories/settlement/${escapeHtml(factoryId)}">前往详情</button>
          </div>
        </div>
      </div>
    `;
    ...
    <section class="space-y-3 rounded-lg border bg-card p-4">
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        .map((cycleType) => `<option value="${cycleType}" ${state.initConfigDraft.cycleType === cycleType ? 'selected' : ''}>${escapeHtml(cycleTypeConfig[cycleType].label)}</option>`)
        .join('')}
          </select>
        </label>
        <label class="space-y-1">
        ...
        .map((pricingMode) => `<option value="${pricingMode}" ${state.initConfigDraft.pricingMode === pricingMode ? 'selected' : ''}>${escapeHtml(pricingModeConfig[pricingMode].label)}</option>`)
        .join('')}
          </select>
        </label>
        <label class="space-y-1">
          ...
          <select data-settle-init-field="config.currency" class="h-9 w-full rounded-md border px-3 text-sm">
            ${CURRENCIES.map((currency) => `<option value="${currency}" ${state.initConfigDraft.currency === currency ? 'selected' : ''}>${currency}</option>`).join('')}
          </select>
        </label>
        <label class="space-y-1">
          <span class="text-xs text-muted-foreground">结算日规则</span>
          <input data-settle-init-field="config.settlementDayRule" class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.initConfigDraft.settlementDayRule || '')}" placeholder="例如：每月25日" />
        </label>
      </div>
    </section>
  `;
    ...
    <section class="space-y-3 rounded-lg border bg-card p-4">
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
        <label class="space-y-1">
          ...
          <input data-settle-init-field="account.accountHolderName" class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.initAccountDraft.accountHolderName)}" />
        </label>
        <label class="space-y-1">
          <span class="text-xs text-muted-foreground">证件号</span>
          <input data-settle-init-field="account.idNumber" class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.initAccountDraft.idNumber)}" />
        </label>
        <label class="space-y-1">
          <span class="text-xs text-muted-foreground">银行名称</span>
          <input data-settle-init-field="account.bankName" class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.initAccountDraft.bankName)}" />
        </label>
        <label class="space-y-1">
          <span class="text-xs text-muted-foreground">银行账号</span>
          <input data-settle-init-field="account.bankAccountNo" class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.initAccountDraft.bankAccountNo)}" />
        </label>
        <label class="space-y-1 md:col-span-2">
          <span class="text-xs text-muted-foreground">开户支行</span>
          <input data-settle-init-field="account.bankBranch" class="h-9 w-full rounded-md border px-3 text-sm" value="${escapeHtml(state.initAccountDraft.bankBranch)}" />
        </label>
      </div>
    </section>
  `;
      ...
      <div class="overflow-hidden rounded-md border">
        ...
        : state.initRulesDraft
            ...
            .join('')}
                          ...
                          <td class="px-2 py-2">
                            <input type="date" data-settle-init-field="rule.${index}.effectiveFrom" class="h-8 w-full rounded border px-2 text-xs" value="${escapeHtml(rule.effectiveFrom)}" />
                          </td>
                          <td class="px-2 py-2">
  ...
  `;
    ...
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="mt-1 text-sm text-muted-foreground">${escapeHtml(state.initEditorFactoryName)} · ${escapeHtml(factoryId)}</p>
          </div>
        </div>
      ...
      ${state.initErrorText
        ? `<div class="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">${escapeHtml(state.initErrorText)}</div>`
        : ''}
