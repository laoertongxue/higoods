import { bankAccounts, penaltyRules, settlementProfiles, settlementSummaries, } from '../../data/fcs/settlement-mock-data';
import { renderConfirmDialog } from '../../components/ui/dialog';
import { cycleTypeConfig, pricingModeConfig, ruleModeConfig, ruleTypeConfig, settlementStatusConfig, } from '../../data/fcs/settlement-types';
import { approveSettlementRequest, createSettlementVersionFromCurrent, followupSettlementRequest, getSettlementChangeRequests, getSettlementEffectiveInfos, getSettlementInitDraftByFactory, getSettlementInitDrafts, getSettlementRequestById, getSettlementStatusClass, getSettlementStatusLabel, getSettlementVersionHistory, initializeSettlementInfo, markSettlementRequestPrinted, rejectSettlementRequest, saveSettlementInitDraft, setSettlementRequestPaperArchived, submitSettlementSignedProof, uploadSettlementSignedProof, verifySettlementRequest, } from '../../data/fcs/settlement-change-requests';
import { escapeHtml } from '../../utils';
import { appStore } from '../../state/store';
export const PAGE_SIZE = 10;
export const CURRENCIES = ['CNY', 'USD', 'EUR', 'HKD'];
...
export function syncSettlementRequestState() {
...
export function maskBankAccountNo(accountNo) {
    const raw = accountNo.replace(/\s+/g, '');
    if (raw.length <= 8)
        return raw;
    return `${raw.slice(0, 4)} **** **** ${raw.slice(-4)}`;
}
export function getChangedFieldsSummary(request) {
    const changed = [];
...
export function getFactoryProfiles(factoryId) {
    return state.profiles.filter((profile) => profile.factoryId === factoryId);
}
export function getFactoryAccounts(factoryId) {
    return state.accounts.filter((account) => account.factoryId === factoryId);
}
export function getFactoryRules(factoryId) {
    return state.rules.filter((rule) => rule.factoryId === factoryId);
}
export function getFilteredSummaries() {
    let result = [...state.summaries];
    if (state.searchKeyword.trim()) {
        const keyword = state.searchKeyword.toLowerCase();
        result = result.filter((summary) => summary.factoryName.toLowerCase().includes(keyword) ||
...
export function getPagedSummaries(filteredSummaries) {
...
export function resetInitEditor(factoryId) {
...
export function saveCurrentInitDraft(operator) {
    if (!state.initEditorFactoryId)
        return { ok: false, message: '当前未选择工厂' };
...
export function getVersionRecordById(factoryId, versionId) {
    return getSettlementVersionHistory(factoryId).find((item) => item.versionId === versionId) ?? null;
}
export function resetProfileForm(factoryId) {
...
export function resetAccountForm(account) {
    if (account) {
        state.accountForm = {
...
export function resetRuleForm(rule) {
    if (rule) {
        state.ruleForm = {
...
export function openProfileDrawer(factoryId) {
    resetProfileForm(factoryId);
    state.profileActiveTab = 'config';
    state.dialog = { type: 'profile-drawer', factoryId };
}
export function openAccountDrawer(factoryId, accountId) {
...
export function openRuleDrawer(factoryId, ruleId) {
    const rule = ruleId
...
export { renderConfirmDialog, cycleTypeConfig, pricingModeConfig, ruleModeConfig, ruleTypeConfig, settlementStatusConfig, approveSettlementRequest, createSettlementVersionFromCurrent, followupSettlementRequest, getSettlementChangeRequests, getSettlementEffectiveInfos, getSettlementInitDraftByFactory, getSettlementInitDrafts, getSettlementRequestById, getSettlementStatusClass, getSettlementStatusLabel, getSettlementVersionHistory, initializeSettlementInfo, markSettlementRequestPrinted, rejectSettlementRequest, saveSettlementInitDraft, setSettlementRequestPaperArchived, submitSettlementSignedProof, uploadSettlementSignedProof, verifySettlementRequest, escapeHtml, appStore, };
