import { state, CURRENCIES, cycleTypeConfig, pricingModeConfig, ruleModeConfig, ruleTypeConfig, settlementStatusConfig, renderConfirmDialog, escapeHtml, getVersionRecordById, maskBankAccountNo, getFactoryProfiles, hasInitializedSettlement, getFactoryName, getSettlementVersionHistory, closeDialog, } from './context';
import { getSettlementVersionUsageStats } from '../../data/fcs/store-domain-settlement-seeds';
function renderSettlementVersionViewDialog() {
    if (state.dialog.type !== 'version-view')
        return '';
    ...
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      ...
      <section class="absolute inset-y-0 right-0 w-full border-l bg-background shadow-2xl sm:max-w-[680px]" data-dialog-panel="true">
        <div class="flex h-full flex-col">
          <header class="border-b px-6 py-4">
            <div class="flex items-center justify-between">
              ...
              <span class="inline-flex rounded border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs">${escapeHtml(record.versionNo)}</span>
            </div>
            <p class="mt-1 text-sm text-muted-foreground">${escapeHtml(record.factoryName)} · ${escapeHtml(record.effectiveAt)}</p>
          </header>
          <div class="flex-1 space-y-4 overflow-y-auto px-6 py-5 text-sm">
            ...
            <section class="rounded-md border p-4">
              ...
              <div class="mt-2 grid grid-cols-2 gap-3 text-xs">
                <p class="text-muted-foreground">结算周期：<span class="font-medium text-foreground">${escapeHtml(cycleTypeConfig[record.settlementConfigSnapshot.cycleType].label)}</span></p>
                <p class="text-muted-foreground">计价方式：<span class="font-medium text-foreground">${escapeHtml(pricingModeConfig[record.settlementConfigSnapshot.pricingMode].label)}</span></p>
                <p class="text-muted-foreground">结算日规则：<span class="font-medium text-foreground">${escapeHtml(record.settlementConfigSnapshot.settlementDayRule || '—')}</span></p>
                <p class="text-muted-foreground">币种：<span class="font-medium text-foreground">${escapeHtml(record.settlementConfigSnapshot.currency)}</span></p>
              </div>
            </section>
            <section class="rounded-md border p-4">
              <p class="text-sm font-semibold">收款账号</p>
              <div class="mt-2 grid grid-cols-2 gap-3 text-xs">
                <p class="text-muted-foreground">开户名：<span class="font-medium text-foreground">${escapeHtml(record.receivingAccountSnapshot.accountHolderName)}</span></p>
                <p class="text-muted-foreground">证件号：<span class="font-medium text-foreground">${escapeHtml(record.receivingAccountSnapshot.idNumber)}</span></p>
                <p class="text-muted-foreground">银行名称：<span class="font-medium text-foreground">${escapeHtml(record.receivingAccountSnapshot.bankName)}</span></p>
                <p class="text-muted-foreground">银行账号：<span class="font-medium text-foreground">${escapeHtml(maskBankAccountNo(record.receivingAccountSnapshot.bankAccountNo))}</span></p>
                <p class="text-muted-foreground">开户支行：<span class="font-medium text-foreground">${escapeHtml(record.receivingAccountSnapshot.bankBranch || '—')}</span></p>
              </div>
            </section>
            <section class="rounded-md border p-4">
        ...
        : record.defaultDeductionRulesSnapshot
            .map((item) => {
            const valueText = item.ruleMode === 'PERCENTAGE' ? `${item.ruleValue}%` : `${item.ruleValue} 元`;
            return `
...
function renderProfileDrawer() {
    if (state.dialog.type !== 'profile-drawer')
        return '';
    const configTab = `
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
        <label class="space-y-1">
          ...
          <select data-settle-field="profile.cycleType" class="h-9 w-full rounded-md border px-3 text-sm ${state.profileErrors.cycleType ? 'border-red-600' : ''}">
            ${Object.keys(cycleTypeConfig)
        .map((cycleType) => `<option value="${cycleType}" ${state.profileForm.cycleType === cycleType ? 'selected' : ''}>${escapeHtml(cycleTypeConfig[cycleType].label)}</option>`)
        .join('')}
          </select>
        ...
        <label class="space-y-1">
          ...
          <select data-settle-field="profile.pricingMode" class="h-9 w-full rounded-md border px-3 text-sm ${state.profileErrors.pricingMode ? 'border-red-600' : ''}">
            ${Object.keys(pricingModeConfig)
        .map((pricingMode) => `<option value="${pricingMode}" ${state.profileForm.pricingMode === pricingMode ? 'selected' : ''}>${escapeHtml(pricingModeConfig[pricingMode].label)}</option>`)
        .join('')}
          </select>
        ...
        <label class="space-y-1">
          ...
          <select data-settle-field="profile.currency" class="h-9 w-full rounded-md border px-3 text-sm ${state.profileErrors.currency ? 'border-red-600' : ''}">
            ${CURRENCIES.map((currency) => `<option value="${currency}" ${state.profileForm.currency === currency ? 'selected' : ''}>${currency}</option>`).join('')}
          </select>
                        ...
                        <tr class="border-b last:border-0">
                          <td class="px-2 py-2">
                            <select data-settle-profile-rule-field="rule.${index}.ruleType" class="h-8 w-full rounded border px-2 text-xs">
                              ${Object.keys(ruleTypeConfig)
            .map((ruleType) => `<option value="${ruleType}" ${rule.ruleType === ruleType ? 'selected' : ''}>${escapeHtml(ruleTypeConfig[ruleType].label)}</option>`)
            .join('')}
                            </select>
                          </td>
                          <td class="px-2 py-2">
                            <select data-settle-profile-rule-field="rule.${index}.ruleMode" class="h-8 w-full rounded border px-2 text-xs">
                              ${Object.keys(ruleModeConfig)
            .map((ruleMode) => `<option value="${ruleMode}" ${rule.ruleMode === ruleMode ? 'selected' : ''}>${escapeHtml(ruleModeConfig[ruleMode].label)}</option>`)
            .join('')}
                            </select>
                          </td>
                          <td class="px-2 py-2">
                          ...
                          <td class="px-2 py-2">
                            <input type="date" data-settle-profile-rule-field="rule.${index}.effectiveFrom" class="h-8 w-full rounded border px-2 text-xs" value="${escapeHtml(rule.effectiveFrom)}" />
                          </td>
                          <td class="px-2 py-2">
                            <button type="button" class="rounded-md border px-2 py-1 text-xs hover:bg-muted ${state.profileRulesDraft.length <= 1 ? 'pointer-events-none opacity-50' : ''}" data-settle-action="remove-profile-rule" data-rule-index="${index}">删除</button>
                          </td>
                        </tr>
                      `)
            .join('')}
...
function renderAccountDrawer() {
    if (state.dialog.type !== 'account-drawer')
        return '';
        ...
        : ''}
              ...
              <label class="space-y-1.5">
                ...
                <select data-settle-field="account.currency" class="w-full rounded-md border px-3 py-2 text-sm ${state.accountErrors.currency ? 'border-red-600' : ''}">
                  ${CURRENCIES.map((currency) => `<option value="${currency}" ${state.accountForm.currency === currency ? 'selected' : ''}>${currency}</option>`).join('')}
                </select>
...
function renderRuleDrawer() {
    if (state.dialog.type !== 'rule-drawer')
        return '';
            ...
            <div class="flex-1 space-y-6 overflow-y-auto px-6 py-6">
              <label class="space-y-1.5">
                ...
                <select data-settle-field="rule.ruleType" class="w-full rounded-md border px-3 py-2 text-sm ${state.ruleErrors.ruleType ? 'border-red-600' : ''}">
                  ${Object.keys(ruleTypeConfig)
        .map((ruleType) => `<option value="${ruleType}" ${state.ruleForm.ruleType === ruleType ? 'selected' : ''}>${escapeHtml(ruleTypeConfig[ruleType].label)}</option>`)
        .join('')}
                </select>
        ...
        : ''}
              ...
              <label class="space-y-1.5">
                ...
                <select data-settle-field="rule.ruleMode" class="w-full rounded-md border px-3 py-2 text-sm ${state.ruleErrors.ruleMode ? 'border-red-600' : ''}">
                  ${Object.keys(ruleModeConfig)
        .map((ruleMode) => `<option value="${ruleMode}" ${state.ruleForm.ruleMode === ruleMode ? 'selected' : ''}>${escapeHtml(ruleModeConfig[ruleMode].label)}</option>`)
        .join('')}
                </select>
...
function renderSettleConfirmDialog() {
    if (state.dialog.type !== 'confirm')
        return '';
    const title = state.dialog.actionType === 'setDefault' ? '设为默认账户' : '确认禁用';
    const description = state.dialog.actionType === 'setDefault'
...
function renderDetailProfileTab(factoryId, currentProfile, currentVersionNo) {
    if (!currentProfile) {
        return `
...
function renderDetailAccountsTab(effectiveInfo) {
    if (!effectiveInfo) {
        return `
      <div class="rounded-lg border bg-card p-6 text-center text-muted-foreground">
        暂无当前生效收款账号
...
function renderDetailRulesTab(rules) {
    return `
    <div class="space-y-4">
      <div class="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-700">
      ...
      <div class="overflow-x-auto rounded-md border">
        ...
        : rules
            ...
            const valueText = rule.ruleMode === 'PERCENTAGE' ? `${rule.ruleValue}%` : `${rule.ruleValue} 元`;
            return `
                        <tr class="border-b last:border-0">
...
function renderDetailHistoryTab(factoryId) {
    const records = getSettlementVersionHistory(factoryId).sort((a, b) => b.effectiveAt.localeCompare(a.effectiveAt));
    return `
    <div class="overflow-x-auto rounded-md border">
      <table class="w-full text-sm">
        <thead class="border-b bg-muted/30">
        ...
        : records
            .map((record) => {
            const configText = `${cycleTypeConfig[record.settlementConfigSnapshot.cycleType].label} · ${pricingModeConfig[record.settlementConfigSnapshot.pricingMode].label} · ${record.settlementConfigSnapshot.currency}`;
            const statusText = record.status === 'EFFECTIVE' ? '生效中' : '已失效';
            const statusClass = record.status === 'EFFECTIVE'
...
export function renderSettlementDetailPage(factoryId) {
    if (!hasInitializedSettlement(factoryId)) {
      ...
      <div class="space-y-4">
        <div class="rounded-lg border bg-card p-6">
          ...
          <div class="mt-4">
            <button class="rounded-md border px-3 py-2 text-sm hover:bg-muted" data-nav="/fcs/factories/settlement/new/${escapeHtml(factoryId)}">前往初始化</button>
          </div>
        </div>
      </div>
    `;
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              ${currentProfile
        ? `<span class="inline-flex rounded border px-2 py-0.5 text-xs ${settlementStatusConfig[currentProfile.isActive ? 'ACTIVE' : 'INACTIVE'].color}">${currentProfile.isActive ? '生效中' : '已失效'}</span>`
        : ''}
            </div>
...
function updateSettlementField(field, node) {
    ...
    if (field === 'profile.pricingMode') {
        state.profileForm.pricingMode = value;
    ...
    if (field === 'account.status') {
        state.accountForm.status = value;
        return;
    }
    if (field === 'rule.ruleType') {
        state.ruleForm.ruleType = value;
        state.ruleErrors.ruleType = undefined;
        return;
    }
    if (field === 'rule.ruleMode') {
        state.ruleForm.ruleMode = value;
    ...
    if (field === 'rule.status') {
        state.ruleForm.status = value;
    }
}
function hydrateRequestOperateForm(requestId) {
