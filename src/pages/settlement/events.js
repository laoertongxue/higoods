import { PAGE_SIZE, state, today, normalizeAccountMask, closeDialog, syncSettlementRequestState, getFilteredSummaries, getFilteredRequests, saveCurrentInitDraft, resetInitEditor, openProfileDrawer, openAccountDrawer, openRuleDrawer, getFactoryName, createSettlementVersionFromCurrent, initializeSettlementInfo, verifySettlementRequest, markSettlementRequestPrinted, uploadSettlementSignedProof, submitSettlementSignedProof, followupSettlementRequest, approveSettlementRequest, rejectSettlementRequest, setSettlementRequestPaperArchived, getSettlementRequestById, appStore, } from './context';
...
export function handleSettlementEvent(target) {
    const filterNode = target.closest('[data-settle-filter]');
    if (filterNode instanceof HTMLInputElement || filterNode instanceof HTMLSelectElement) {
        ...
        if (filter === 'cycleType')
            state.filterCycleType = value;
        if (filter === 'status')
            state.filterStatus = value;
        state.currentPage = 1;
        return true;
    }
    const requestFilterNode = target.closest('[data-settle-request-filter]');
    if (requestFilterNode instanceof HTMLInputElement || requestFilterNode instanceof HTMLSelectElement) {
        ...
        if (filter === 'search')
            state.requestSearchKeyword = value;
        if (filter === 'status')
            state.requestFilterStatus = value;
        state.requestPage = 1;
        return true;
    }
    const requestFieldNode = target.closest('[data-settle-request-field]');
    if (requestFieldNode instanceof HTMLInputElement ||
        ...
        if (!field)
            return true;
        if (field === 'verifyRemark')
            state.requestOperateForm.verifyRemark = requestFieldNode.value;
        if (field === 'followupRemark')
            state.requestOperateForm.followupRemark = requestFieldNode.value;
        if (field === 'rejectReason')
            state.requestOperateForm.rejectReason = requestFieldNode.value;
    ...
    const initFieldNode = target.closest('[data-settle-init-field]');
    if (initFieldNode instanceof HTMLInputElement ||
        ...
        if (field === 'config.cycleType') {
            state.initConfigDraft.cycleType = value;
            return true;
        }
        if (field === 'config.pricingMode') {
            state.initConfigDraft.pricingMode = value;
            return true;
        }
    ...
    const fieldNode = target.closest('[data-settle-field]');
    ...
    const profileRuleFieldNode = target.closest('[data-settle-profile-rule-field]');
    if (profileRuleFieldNode instanceof HTMLInputElement ||
    ...
    const actionNode = target.closest('[data-settle-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'open-init-editor') {
        if (!state.initSelectedFactoryId)
            return true;
    ...
    if (action === 'remove-init-rule') {
        ...
        if (index < 0 || index >= state.initRulesDraft.length)
            return true;
        if (state.initRulesDraft.length <= 1)
            return true;
    ...
    if (action === 'submit-init-settlement') {
        ...
        if (!state.initAccountDraft.accountHolderName.trim() ||
        ...
        if (state.initRulesDraft.length === 0 ||
        ...
        state.rules.push(...state.initRulesDraft.map((item, index) => ({
        ...
        state.summaries = state.summaries.map((item) => item.factoryId === factoryId
            ? {
                ...item,
                cycleType: state.initConfigDraft.cycleType,
                pricingMode: state.initConfigDraft.pricingMode,
    ...
    if (action === 'remove-profile-rule') {
        const index = Number(actionNode.dataset.ruleIndex ?? '-1');
        if (index < 0 || index >= state.profileRulesDraft.length)
            return true;
        if (state.profileRulesDraft.length <= 1)
            return true;
    ...
    if (action === 'open-confirm') {
        const factoryId = actionNode.dataset.factoryId;
        const actionType = actionNode.dataset.confirmType;
        const itemId = actionNode.dataset.itemId;
        if (!factoryId || !actionType || !itemId)
            return true;
        state.accountActionMenuId = null;
        state.ruleActionMenuId = null;
        state.dialog = {
    ...
    if (action === 'confirm-action') {
        ...
        if (actionType === 'disableAccount') {
            state.accounts = state.accounts.map((account) => account.id === itemId
                ? {
                    ...account,
                    status: 'INACTIVE',
                }
        ...
        if (actionType === 'setDefault') {
            state.accounts = state.accounts.map((account) => account.factoryId === factoryId
                ? {
                    ...account,
        ...
        if (actionType === 'disableRule') {
            state.rules = state.rules.map((rule) => rule.id === itemId
                ? {
                    ...rule,
                    status: 'INACTIVE',
                }
...
export function handleSettlementSubmit(form) {
    ...
    if (formType === 'profile') {
        ...
        const errors = {};
        if (!state.profileForm.cycleType)
            errors.cycleType = '请选择结算周期';
        if (!state.profileForm.pricingMode)
            errors.pricingMode = '请选择计价方式';
        if (!state.profileForm.currency)
            errors.currency = '请选择币种';
        if (!state.profileForm.effectiveFrom)
            errors.effectiveFrom = '请选择生效日期';
        if (Object.keys(errors).length > 0) {
            state.profileErrors = errors;
            return true;
        }
        if (state.profileRulesDraft.length === 0 ||
            state.profileRulesDraft.some((rule) => !rule.effectiveFrom || Number(rule.ruleValue) <= 0)) {
            state.profileRulesError = '请先补全扣款规则后再创建新版本';
            return true;
        }
        state.profiles = state.profiles.map((profile) => profile.factoryId === factoryId && profile.isActive
            ? {
                ...profile,
                isActive: false,
        ...
        const rulesSnapshot = state.profileRulesDraft.map((rule) => ({
        ...
        state.rules.push(...state.profileRulesDraft.map((rule, index) => ({
            id: `pr-${Date.now()}-${index}`,
            factoryId,
            ruleType: rule.ruleType,
            ruleMode: rule.ruleMode,
    ...
    if (formType === 'account') {
        ...
        const errors = {};
        if (!state.accountForm.accountName.trim())
            errors.accountName = '请输入账户名称';
        if (!state.accountForm.bankName.trim())
            errors.bankName = '请输入银行名称';
        if (!state.accountForm.accountMasked.trim())
            errors.accountMasked = '请输入账号';
        if (!state.accountForm.currency)
            errors.currency = '请选择币种';
        if (Object.keys(errors).length > 0) {
    ...
    if (formType === 'rule') {
        ...
        const errors = {};
        if (!state.ruleForm.ruleType)
            errors.ruleType = '请选择规则类型';
        if (!state.ruleForm.ruleMode)
            errors.ruleMode = '请选择计算方式';
        if (state.ruleForm.ruleValue <= 0)
            errors.ruleValue = '请输入有效数值';
        if (!state.ruleForm.effectiveFrom)
            errors.effectiveFrom = '请选择生效日期';
        if (Object.keys(errors).length > 0) {
        ...
        if (ruleId) {
            state.rules = state.rules.map((rule) => rule.id === ruleId
                ? {
                    ...rule,
                    ...state.ruleForm,
                }
