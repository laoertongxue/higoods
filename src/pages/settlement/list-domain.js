import { PAGE_SIZE, state, cycleTypeConfig, pricingModeConfig, settlementStatusConfig, getSettlementStatusClass, getSettlementStatusLabel, escapeHtml, maskBankAccountNo, getChangedFieldsSummary, syncSettlementRequestState, getFilteredSummaries, getPagedSummaries, getFilteredRequests, getPagedRequests, } from './context';
...
function renderPagination(total) {
    ...
    <div class="flex items-center gap-1">
      ...
      ${Array.from({ length: totalPages }, (_, index) => index + 1)
        .map((page) => `<button data-settle-action="goto-page" data-page="${page}" class="rounded-md border px-3 py-1 text-sm ${page === state.currentPage ? 'bg-blue-600 text-white' : 'hover:bg-muted'}">${page}</button>`)
        .join('')}
...
function renderRequestPagination(total) {
    ...
    <div class="flex items-center gap-1">
      ...
      ${Array.from({ length: totalPages }, (_, index) => index + 1)
        .map((page) => `<button data-settle-action="request-goto-page" data-page="${page}" class="rounded-md border px-3 py-1 text-sm ${page === state.requestPage ? 'bg-blue-600 text-white' : 'hover:bg-muted'}">${page}</button>`)
        .join('')}
...
function renderRequestStats(requests) {
    ...
    const stats = [
        { label: '待审核', value: pendingReview, status: 'PENDING_REVIEW' },
        { label: '已通过', value: approvedCount, status: 'APPROVED' },
        { label: '未通过', value: rejectedCount, status: 'REJECTED' },
...
export function renderSettlementListPage() {
    ...
    <div class="space-y-6">
      ...
      ${state.listView === 'effective'
        ? `
            ...
            .map((cycleType) => `<option value="${cycleType}" ${state.filterCycleType === cycleType ? 'selected' : ''}>${escapeHtml(cycleTypeConfig[cycleType].label)}</option>`)
            .join('')}
                  </select>
                </label>
                ...
                <label class="space-y-2">
                  ...
                  <select data-settle-filter="status" class="w-full rounded-md border px-3 py-2 text-sm">
                    ...
                    ${Object.keys(settlementStatusConfig)
            .map((status) => `<option value="${status}" ${state.filterStatus === status ? 'selected' : ''}>${escapeHtml(settlementStatusConfig[status].label)}</option>`)
            .join('')}
                  </select>
                </label>
            ...
            : pagedSummaries
                ...
                const effectiveInfo = state.effectiveInfos.find((item) => item.factoryId === summary.factoryId) ?? null;
                return `
                    ...
                    : `<span class="inline-flex items-center rounded border px-2 py-0.5 text-xs bg-amber-50 text-amber-700 border-amber-200"><i data-lucide="x" class="mr-1 h-3 w-3"></i>未配置</span>`}
                                ...
                                <td class="px-3 py-3">
                                  <span class="inline-flex rounded border px-2 py-0.5 text-xs ${status.color}">${escapeHtml(status.label)}</span>
                                </td>
