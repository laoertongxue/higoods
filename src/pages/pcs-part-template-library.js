...
import { getPartTemplateRecordById, listPartTemplatePackages, listPartTemplateRecords, listSiblingPartTemplateRecords, recommendPartTemplateRecords, savePartTemplatePackage, updatePartTemplateRecord, } from '../data/pcs-part-template-library';
import { PART_TEMPLATE_STANDARD_NAME_OPTIONS, parsePartTemplateFiles, resolveTemplateFilePair, suggestStandardPartName, } from '../utils/pcs-part-template-parser';
...
function getPartDraftKey(part) {
    return `${part.sourceBlockIndex}-${part.sourceBlockName}`;
}
function getPartTemplatePreviewUrl(record) {
    if (!record.previewSvg)
        return '';
    return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(record.previewSvg)}`;
}
function renderPartTemplateFileLink(fileName, fileUrl) {
    if (!fileUrl) {
...
function resetCreateDrawerState() {
...
function getSelectedTemplateFiles() {
    return [state.selectedDxfFile, state.selectedRulFile].filter((file) => file instanceof File);
}
function getSingleUploadError(file, expectedExtension, label) {
    if (!file)
        return null;
...
function getCreateUploadError() {
    ...
    return (getSingleUploadError(state.selectedDxfFile, 'dxf', 'DXF 文件') ??
        getSingleUploadError(state.selectedRulFile, 'rul', 'RUL 文件'));
}
function getParserBadgeClass(status) {
    if (status === '解析成功')
...
function getMachineBadgeClass(status) {
    if (status === '可模板机处理')
...
function formatFileModifiedTime(file) {
    return formatDateTime(new Date(file.lastModified).toISOString());
}
function getCreateUploadStage() {
    if (state.parsing)
        return 'parsing';
...
function getCreateUploadStageMeta() {
...
function renderShapeTags(values) {
    if (values.length === 0)
        return '<span class="text-xs text-gray-400">待补充</span>';
    return values
        .slice(0, 3)
        .map((value) => `<span class="inline-flex rounded-full border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-xs text-indigo-700">${escapeHtml(value)}</span>`)
        .join('');
}
function renderSuitabilityBadge(record) {
    ...
    const className = record.shapeDescription?.templateMachineSuitability === 'high'
...
function renderSourceInfoLine(record) {
...
function renderCandidateTags(values) {
    if (values.length === 0)
        return '<span class="text-xs text-gray-400">未识别</span>';
    return values
        .slice(0, 4)
        .map((value) => `<span class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs text-slate-600">${escapeHtml(value)}</span>`)
        .join('');
}
function getRecords() {
    ...
    return getRecords().filter((record) => {
        const matchesKeyword = keyword.length === 0 ||
            [
                ...
                .toLowerCase()
                .includes(keyword);
        const matchesStandard = state.standardPartFilter === '全部' || record.standardPartName === state.standardPartFilter;
        const matchesSize = state.sizeFilter === '全部' || (record.sizeCode ?? '-') === state.sizeFilter;
        const matchesParser = state.parserStatusFilter === '全部' || record.parserStatus === state.parserStatusFilter;
        const matchesMachine = state.machineReadyFilter === '全部' || record.machineReadyStatus === state.machineReadyFilter;
...
function renderFilters(records) {
    ...
    <section class="rounded-lg border bg-white p-4">
      <div class="grid gap-3 lg:grid-cols-[minmax(0,1.5fr)_220px_180px_180px_180px_auto]">
        ...
        .map((value) => `<option value="${escapeHtml(value)}" ${state.standardPartFilter === value ? 'selected' : ''}>${escapeHtml(value)}</option>`)
        .join('')}
          </select>
        </div>
        <div>
          ...
          <select class="h-9 w-full rounded-md border px-3 text-sm" data-part-template-field="sizeFilter">
            <option value="全部">全部</option>
            ${options.sizes
        .map((value) => `<option value="${escapeHtml(value)}" ${state.sizeFilter === value ? 'selected' : ''}>${escapeHtml(value)}</option>`)
        .join('')}
          </select>
        </div>
        <div>
          ...
          <select class="h-9 w-full rounded-md border px-3 text-sm" data-part-template-field="parserStatusFilter">
            ${['全部', '解析成功', '待人工矫正', '解析异常']
        .map((value) => `<option value="${value}" ${state.parserStatusFilter === value ? 'selected' : ''}>${value}</option>`)
        .join('')}
          </select>
        </div>
        <div>
          ...
          <select class="h-9 w-full rounded-md border px-3 text-sm" data-part-template-field="machineReadyFilter">
            ${['全部', '可模板机处理', '待评估', '不适用']
        .map((value) => `<option value="${value}" ${state.machineReadyFilter === value ? 'selected' : ''}>${value}</option>`)
        .join('')}
          </select>
        </div>
...
function renderRecordRows(records) {
    ...
    return records
        ...
        const recommendationPreview = recommendPartTemplateRecords({
            ...
            grainLines: [],
            pointLayerStats: {},
            metrics: record.width !== undefined &&
...
function renderPartTemplatePreviewDialog() {
    if (!state.previewOpen || !state.previewRecordId)
        return '';
...
function renderPartTemplatePackageDialog() {
    if (!state.packageDialogOpen || !state.packageDialogRecordId)
        return '';
...
function renderParsedSummary(result) {
    return `
...
function renderPartDraftCards(result) {
    return result.parts
        .map((part) => {
...
function renderCreateDrawer() {
    ...
    const canParse = !!state.templateName.trim() &&
...
function renderDetailRecommendations(record) {
    const matches = recommendPartTemplateRecords({
        ...
        rawTextLabels: [],
        insertRefs: [],
    }, 3).filter((item) => item.record.id !== record.id);
    if (matches.length === 0) {
...
function renderDetailDrawer() {
    if (!state.detailDrawerOpen || !state.detailRecordId)
        return '';
...
function renderStandardNameDatalist() {
    return `
...
function startParsingTemplate() {
    ...
    void parsePartTemplateFiles({
        ...
        .then((result) => {
        if (requestId !== currentParseRequestId)
            return;
        state.parseResult = result;
        state.draftStandardNames = Object.fromEntries(result.parts.map((part) => [getPartDraftKey(part), suggestStandardPartName(part)]));
        state.parseError = null;
    })
        .catch((error) => {
        if (requestId !== currentParseRequestId)
...
function saveParsedRows() {
    if (!state.parseResult) {
        state.parseError = '请先完成模板解析。';
        return;
    }
    const rows = state.parseResult.parts.map((part) => ({
        part,
...
export function handlePcsPartTemplateLibraryEvent(target) {
    const actionNode = target.closest('[data-part-template-action]');
...
export function handlePcsPartTemplateLibraryInput(target) {
    const fieldNode = target.closest('[data-part-template-field]');
...
export function isPcsPartTemplateLibraryDialogOpen() {
    return state.createDrawerOpen || state.detailDrawerOpen || state.previewOpen || state.packageDialogOpen;
}
export function renderPcsPartTemplateLibraryPage() {
    return renderPage();
}
