...
function renderWorkspace(): string {
  ...
  return renderStandardListPage({
    ...
    overlaysHtml: [state.showColumnSettings ? renderStandardListColumnSettings({ title: '分拨任务列设置', columns, preferences: state.preferences, eventPrefix: EVENT_PREFIX, maxFrozenWidth: 400 }) : '', state.detailIdx >= 0 && seedTasks[state.detailIdx] ? renderStandardRowDetailDialog({ title: '二次分拨列表详情', columns, row: seedTasks[state.detailIdx], eventPrefix: EVENT_PREFIX }) : ''].join(''),
  })
}
...
export function handleCollectionSortingEvent(target: HTMLElement, event?: Event): boolean {
  ...
  if (action === 'export') { exportStandardListRows({ fileName: '二次分拨列表', columns, rows: filteredRows() }); return true }
  return false
}
