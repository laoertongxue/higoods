...
function renderWorkspace(): string {
  ...
  return renderStandardListPage({
    ...
    overlaysHtml: [state.showColumnSettings ? renderStandardListColumnSettings({ title: '波次管理列设置', columns, preferences: state.preferences, eventPrefix: EVENT_PREFIX, maxFrozenWidth: 400 }) : ''].join(''),
  })
}
...
export function handleWaveManageEvent(target: HTMLElement, event?: Event): boolean {
  ...
  if (action === 'export') { exportStandardListRows({ fileName: '波次管理', columns, rows: filteredRows() }); return true }
  return false
}
