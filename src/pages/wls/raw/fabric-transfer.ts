...
const seedOrders: FabricTransfer[] = [
  { transferNo: 'TR-FT-20260716-001', fromWarehouse: '原料仓A', toWarehouse: '中转仓', material: '面料', spu: 'SPU-FAB-10001', sku: 'SKU-FAB-40001', productName: '主身面料-白色', qty: 200, status: '已签收', creator: '原料仓文员-小陈', createTime: '2026-07-16 08:00', completeTime: '2026-07-16 13:00' },
  { transferNo: 'TR-FT-20260716-002', fromWarehouse: '原料仓B', toWarehouse: '中转仓', material: '面料', spu: 'SPU-FAB-10002', sku: 'SKU-FAB-40002', productName: '里料-黑色', qty: 150, status: '运输中', creator: '原料仓文员-小刘', createTime: '2026-07-16 08:30', completeTime: '-' },
  { transferNo: 'TR-FT-20260716-003', fromWarehouse: '原料仓A', toWarehouse: '车间线边仓', material: '面料', spu: 'SPU-FAB-10003', sku: 'SKU-FAB-40003', productName: '口袋布-灰色', qty: 100, status: '待出库', creator: '原料仓文员-小陈', createTime: '2026-07-16 09:00', completeTime: '-' },
  { transferNo: 'TR-FT-20260716-004', fromWarehouse: '原料仓C', toWarehouse: '中转仓', material: '面料', spu: 'SPU-FAB-10004', sku: 'SKU-FAB-40004', productName: '领衬面料', qty: 80, status: '待签收', creator: '原料仓文员-小刘', createTime: '2026-07-16 09:30', completeTime: '-' },
  { transferNo: 'TR-FT-20260716-005', fromWarehouse: '原料仓B', toWarehouse: '车间线边仓', material: '面料', spu: 'SPU-FAB-10005', sku: 'SKU-FAB-40005', productName: '袖里面料', qty: 120, status: '已签收', creator: '原料仓文员-小陈', createTime: '2026-07-16 10:00', completeTime: '2026-07-16 15:00' },
  { transferNo: 'TR-FT-20260716-006', fromWarehouse: '原料仓A', toWarehouse: '中转仓', material: '面料', spu: 'SPU-FAB-10006', sku: 'SKU-FAB-40006', productName: '面料-条纹', qty: 300, status: '已取消', creator: '原料仓文员-小刘', createTime: '2026-07-16 10:30', completeTime: '-' },
  { transferNo: 'TR-FT-20260716-007', fromWarehouse: '原料仓C', toWarehouse: '中转仓', material: '面料', spu: 'SPU-FAB-10007', sku: 'SKU-FAB-40007', productName: '面料-格子', qty: 250, status: '运输中', creator: '原料仓文员-小陈', createTime: '2026-07-16 11:00', completeTime: '-' },
  { transferNo: 'TR-FT-20260715-001', fromWarehouse: '原料仓B', toWarehouse: '中转仓', material: '面料', spu: 'SPU-FAB-10008', sku: 'SKU-FAB-40008', productName: '面料-纯色蓝', qty: 180, status: '待出库', creator: '原料仓文员-小刘', createTime: '2026-07-15 14:00', completeTime: '-' },
]
...
const EVENT_PREFIX = 'wls-raw-fabric-transfer'
const DATASET_PREFIX = EVENT_PREFIX.replace(/-([a-z0-9])/g, (_, c: string) => c.toUpperCase())
const PREFERENCE_KEY = '/wls/raw/fabric-transfer:list-columns'
...
const columns: StandardListColumn<FabricTransfer>[] = [
...
function filteredRows(): FabricTransfer[] {
...
function renderWorkspace(): string {
  ...
  return renderStandardListPage({
    ...
    overlaysHtml: [state.showColumnSettings ? renderStandardListColumnSettings({ title: '面料调拨列设置', columns, preferences: state.preferences, eventPrefix: EVENT_PREFIX, maxFrozenWidth: 400 }) : '', state.detailIdx >= 0 && seedOrders[state.detailIdx] ? renderStandardRowDetailDialog({ title: '原料仓面料调拨单详情', columns, row: seedOrders[state.detailIdx], eventPrefix: EVENT_PREFIX }) : ''].join(''),
  })
}
...
export function renderRawFabricTransfer(): string {
  resetStandardListEntryTransientStateOnRouteEntry(state, Boolean(rootElement()))
  ensurePreferencesLoaded()
  return `<div data-${EVENT_PREFIX}-root data-skip-page-rerender="true"><div data-${EVENT_PREFIX}-workspace>${renderWorkspace()}</div></div>`
}

export function handleRawFabricTransferEvent(target: HTMLElement, event?: Event): boolean {
  ...
  if (action === 'export') { exportStandardListRows({ fileName: '原料仓面料调拨单', columns, rows: filteredRows() }); return true }
  return false
}
