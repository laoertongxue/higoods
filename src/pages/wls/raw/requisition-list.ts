...
const columns: StandardListColumn<RawRequisitionOrder>[] = [
  ...
  { key: 'actions', title: '操作', width: 100, required: true, actionColumn: true,
    render: () => `<div class="flex items-center gap-1"><button class="rounded border border-slate-200 px-2 py-0.5 text-xs text-slate-600 hover:bg-slate-50" data-${EVENT_PREFIX}-action="view">查看</button></div>` },
]
...
function renderWorkspace(): string {
  ...
  return renderStandardListPage({
    ...
    overlaysHtml: state.showColumnSettings ? renderStandardListColumnSettings({ title: '领料单列设置', columns, preferences: state.preferences, eventPrefix: EVENT_PREFIX, maxFrozenWidth: 400 }) : '',
  })
}
