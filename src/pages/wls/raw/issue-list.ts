...
const columns: StandardListColumn<RawIssueOrder>[] = [
  ...
  { key: 'actions', title: '操作', width: 180, required: true, actionColumn: true,
    render: r => `<div class="flex items-center gap-1"><button class="rounded border border-slate-200 px-2 py-0.5 text-xs text-slate-600 hover:bg-slate-50" data-${EVENT_PREFIX}-action="view" data-${EVENT_PREFIX}-idx="${ seedIssues.indexOf(r) }">查看</button>${r.pickingStatus === '草稿' ? `<button class="rounded border border-emerald-300 px-2 py-0.5 text-xs text-emerald-700 hover:bg-emerald-50" data-${EVENT_PREFIX}-action="factory-confirm" data-${EVENT_PREFIX}-idx="${ seedIssues.indexOf(r) }">工厂确认</button><button class="rounded border border-red-300 px-2 py-0.5 text-xs text-red-600 hover:bg-red-50" data-${EVENT_PREFIX}-action="factory-reject" data-${EVENT_PREFIX}-idx="${ seedIssues.indexOf(r) }">工厂拒绝</button>` : ''}</div>` },
]
...
function renderWorkspace(): string {
  ...
  return renderStandardListPage({
    ...
    overlaysHtml: [state.showColumnSettings ? renderStandardListColumnSettings({ title: '配料单列设置', columns, preferences: state.preferences, eventPrefix: EVENT_PREFIX, maxFrozenWidth: 400 }) : '', state.detailIdx >= 0 && seedIssues[state.detailIdx] ? renderStandardRowDetailDialog({ title: '原料配料单列表详情', columns, row: seedIssues[state.detailIdx], eventPrefix: EVENT_PREFIX }) : '', state.confirmIdx >= 0 && seedIssues[state.confirmIdx] ? renderSimpleConfirmDialog({
      prefix: EVENT_PREFIX,
      closeAction: 'cancel-confirm',
      confirmAction: 'run-confirm',
      title: state.confirmKind === 'factory-reject' ? '确认拒绝领取' : '确认工厂领取',
      description: `配料单 ${seedIssues[state.confirmIdx].outboundOrderNo}（${seedIssues[state.confirmIdx].receivingUnit}）${state.confirmKind === 'factory-reject' ? '将被标记为拒绝领取，需要重新配料。' : '将由工厂确认领取，确认后进入拣货。'}`,
      confirmLabel: state.confirmKind === 'factory-reject' ? '确认拒绝' : '确认领取',
      danger: state.confirmKind === 'factory-reject',
    }) : ''].join(''),
  })
}
...
export function handleRawIssueListEvent(target: HTMLElement, event?: Event): boolean {
  ...
  if (action === 'factory-confirm' || action === 'factory-reject') {
    state.confirmKind = action
    state.confirmIdx = Number(actionNode.dataset['wlsRawIssueListIdx'])
    refreshWorkspace()
    return true
  }
  ...
  if (action === 'run-confirm') {
    const row = seedIssues[state.confirmIdx]
    if (row) {
      const rejected = state.confirmKind === 'factory-reject'
      row.factoryConfirm = rejected ? '工厂拒绝领取' : '工厂确认领取'
      showListFeedback(`配料单 ${row.outboundOrderNo} 已由${row.receivingUnit}${rejected ? '拒绝领取，需重新配料' : '确认领取，进入拣货'}`)
    }
    state.confirmKind = ''
    state.confirmIdx = -1
    refreshWorkspace()
  ...
  if (action === 'export') { exportStandardListRows({ fileName: '原料配料单列表', columns, rows: filteredRows() }); return true }
  return false
}
