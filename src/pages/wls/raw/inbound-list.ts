...
function renderWorkspace(): string {
  ...
  return renderStandardListPage({
    ...
    overlaysHtml: [state.showColumnSettings ? renderStandardListColumnSettings({ title: '上架单列设置', columns, preferences: state.preferences, eventPrefix: EVENT_PREFIX, maxFrozenWidth: 400 }) : '', state.detailIdx >= 0 && seedOrders[state.detailIdx] ? renderStandardRowDetailDialog({ title: '原料仓上架单详情', columns, row: seedOrders[state.detailIdx], eventPrefix: EVENT_PREFIX }) : ''].join(''),
  })
}
...
export function handleRawInboundListEvent(target: HTMLElement, event?: Event): boolean {
  ...
  if (action === 'export') { exportStandardListRows({ fileName: '原料仓上架单', columns, rows: filteredRows() }); return true }
  return false
}
