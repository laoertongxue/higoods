...
function renderWorkspace(): string {
  ...
  return renderStandardListPage({
    ...
    overlaysHtml: [state.showColumnSettings ? renderStandardListColumnSettings({ title: '面料盘点单列设置', columns, preferences: state.preferences, eventPrefix: EVENT_PREFIX, maxFrozenWidth: 400 }) : '', state.detailIdx >= 0 && seedOrders[state.detailIdx] ? renderStandardRowDetailDialog({ title: '原料仓面料盘点单详情', columns, row: seedOrders[state.detailIdx], eventPrefix: EVENT_PREFIX }) : ''].join(''),
  })
}
...
export function handleRawFabricInventoryCountEvent(target: HTMLElement, event?: Event): boolean {
  ...
  if (action === 'start') {
    const row = seedOrders[Number(actionNode.dataset['wlsRawFabricInventoryCountIdx'])]
  ...
  if (action === 'export') { exportStandardListRows({ fileName: '原料仓面料盘点单', columns, rows: filteredRows() }); return true }
  return false
}
