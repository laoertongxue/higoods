...
const columns: StandardListColumn<RawArrivalOrder>[] = [
  ...
  { key: 'actions', title: '操作', width: 120, required: true, actionColumn: true,
    render: r => `<div class="flex items-center gap-1"><button class="rounded border border-slate-200 px-2 py-0.5 text-xs text-slate-600 hover:bg-slate-50" data-${EVENT_PREFIX}-action="view" data-${EVENT_PREFIX}-idx="${ seedOrders.indexOf(r) }">查看</button>${r.status === '待收货' ? `<button class="rounded bg-blue-600 px-2 py-0.5 text-xs text-white hover:bg-blue-700" data-${EVENT_PREFIX}-action="receive" data-${EVENT_PREFIX}-idx="${ seedOrders.indexOf(r) }">开始收货</button>` : ''}</div>` },
]
...
function renderWorkspace(): string {
  ...
  return renderStandardListPage({
    ...
    overlaysHtml: [state.showColumnSettings ? renderStandardListColumnSettings({ title: '到货清单列设置', columns, preferences: state.preferences, eventPrefix: EVENT_PREFIX, maxFrozenWidth: 400 }) : '', state.detailIdx >= 0 && seedOrders[state.detailIdx] ? renderStandardRowDetailDialog({ title: '原料仓到货清单详情', columns, row: seedOrders[state.detailIdx], eventPrefix: EVENT_PREFIX }) : ''].join(''),
  })
}
...
export function handleRawArrivalListEvent(target: HTMLElement, event?: Event): boolean {
  ...
  if (action === 'receive') {
    const row = seedOrders[Number(actionNode.dataset['wlsRawArrivalListIdx'])]
    if (row && row.status === '待收货') {
      row.status = '收货中'
      showListFeedback(`到货单 ${row.arrivalNo} 已开始收货，应收 ${row.qty} 件`)
    }
    refreshWorkspace()
    return true
  }
  if (action === 'close-detail') { state.detailIdx = -1; refreshWorkspace(); return true }
  if (action === 'export') { exportStandardListRows({ fileName: '原料仓到货清单', columns, rows: filteredRows() }); return true }
  return false
}
