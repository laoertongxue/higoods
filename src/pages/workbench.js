import { listLegacyLikeDeductionBasisForTailPages, listLegacyLikeDyePrintOrdersForTailPages, listLegacyLikeProcessTasksForTailPages, listLegacyLikeQualityInspectionsForTailPages, listLegacyLikeSettlementBatchesForTailPages, listLegacyLikeStatementDraftsForTailPages, } from '../data/fcs/page-adapters/long-tail-pages-adapter';
...
const LIABILITY_STATUS_ZH = {
    DRAFT: '草稿',
    CONFIRMED: '已确认',
    DISPUTED: '争议中',
    VOID: '已作废',
};
const STATEMENT_STATUS_ZH = {
    DRAFT: '草稿',
    CONFIRMED: '已确认',
    CLOSED: '已关闭',
};
...
function renderSectionTable(title, header, rowsHtml, emptyText, colspan) {
    return `
...
function buildTodos() {
    ...
    const occupiedIds = new Set(initialStatementDrafts
...
export function renderOverviewPage() {
    ...
    const recentQcRows = recentQc
        .map((qc) => {
        const resultClass = qc.result === 'PASS'
            ? 'bg-green-50 text-green-700 border-green-200'
            : qc.result === 'FAIL'
                ? 'bg-red-50 text-red-700 border-red-200'
                : '';
        const liabilityClass = qc.liabilityStatus === 'DISPUTED'
...
export function getBadgeClassByStatus(status) {
