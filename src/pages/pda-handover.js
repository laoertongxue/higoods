...
import { deriveHandoutObjectProfile, getPdaCompletedHeads, getPdaHandoverRecordsByHead, getPdaHandoutHeads, getPdaPostFinishingCompletedHeads, getPdaPostFinishingHandoutHeads, getPdaPostFinishingPickupHeads, getPdaPickupHeads, } from '../data/fcs/pda-handover-events';
import { getHandoverOrderQrDisplayValue, getHandoverOrderStatusLabel, getReceiverDisplayName, getReceiverKindLabel, } from '../data/fcs/task-handover-domain';
...
function renderPartyChip(kind, name) {
    return `
...
function getExecutorLabel(head) {
    if (head.executorKind === 'WAREHOUSE_WORKSHOP')
        return '仓内后道';
    return '外部工厂';
}
function getPickupSummaryMeta(head) {
...
function getHandoutSummaryMeta(head) {
    if (head.summaryStatus === 'NONE') {
        return {
            label: '暂无交出记录',
            className: 'border-border bg-background text-muted-foreground',
...
function renderHandoutQrBlock(head, objectTypeLabel, size) {
...
function renderHandoutObjectBlock(head, compact = false) {
...
function renderOpenHeadCard(head) {
    ...
    if (head.headType === 'PICKUP') {
        const pickupHint = head.objectionCount > 0
...
function renderDoneHeadCard(head) {
...
export function renderPdaHandoverPage() {
    ...
    <div class="flex min-h-[760px] flex-col bg-background">
      <div class="shrink-0 px-4 pb-2 pt-4">
        ...
        return `
              <button
                class="${toClassName('rounded-lg border p-2 text-center transition-colors', active ? 'border-primary bg-primary text-primary-foreground' : 'border-transparent bg-muted/40')}"
...
export function handlePdaHandoverEvent(target) {
    if (!ensurePdaSessionForAction())
        return true;
    const actionNode = target.closest('[data-pda-handover-action]');
    if (!actionNode)
        return false;
