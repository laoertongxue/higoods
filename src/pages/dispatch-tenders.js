...
function calcPriceSummary(quotes, currency, unit) {
    const prices = quotes
        .filter((quote) => quote.hasQuoted && quote.quotePrice != null)
        .map((quote) => quote.quotePrice);
    if (prices.length === 0) {
        return {
            maxStr: '暂无报价',
            minStr: '暂无报价',
...
function formatDeviation(quotePrice, standardPrice, currency, unit) {
...
function getEffectiveAward(tender) {
...
function toEffectiveTender(tender) {
...
function getViewTender() {
    if (!state.viewTenderId)
        return null;
...
function getFilteredTenders() {
    ...
    return allTenders.filter((tender) => {
        if (state.statusFilter !== 'ALL' && tender.status !== state.statusFilter)
            return false;
        if (keyword) {
            return (tender.tenderId.toLowerCase().includes(keyword) ||
...
function getCreateValidation() {
    ...
    const createValid = state.form.taskId.trim() !== '' &&
...
function openViewDrawer(tenderId) {
...
function showTenderToast(message) {
...
function confirmAwardInView() {
    const tender = getViewTender();
    if (!tender)
        return;
    const effective = toEffectiveTender(tender);
    if (effective.status !== 'AWAIT_AWARD')
        return;
    const selectedQuote = tender.factoryQuotes.find((quote) => quote.factoryName === state.viewAwardFactoryName &&
        quote.hasQuoted &&
        quote.quotePrice != null);
    if (!selectedQuote || selectedQuote.quotePrice == null)
        return;
    const priceOutOfRange = selectedQuote.quotePrice < tender.minPrice || selectedQuote.quotePrice > tender.maxPrice;
    if (priceOutOfRange && state.viewAwardReason.trim() === '')
        return;
    state.localAwards = {
        ...state.localAwards,
        [tender.tenderId]: {
    ...
    showTenderToast(`定标完成：${selectedQuote.factoryName}，中标价 ${selectedQuote.quotePrice.toLocaleString()} ${tender.currency}/${tender.unit}`);
    closeViewDrawer();
}
function renderViewTenderSheet(tender) {
    if (!tender || !state.viewTenderId)
        return '';
    ...
    const priceOutOfRange = selectedPrice != null && (selectedPrice < tender.minPrice || selectedPrice > tender.maxPrice);
    const needReason = priceOutOfRange;
    const canConfirm = effectiveStatus === 'AWAIT_AWARD' &&
        state.viewAwardFactoryName !== '' &&
        selectedPrice != null &&
        (!needReason || state.viewAwardReason.trim() !== '');
    const avgPrice = quotedCount > 0
        ? Math.round(quotedRows.reduce((sum, quote) => sum + quote.quotePrice, 0) / quotedCount)
        : null;
    return `
      ...
      <section class="absolute inset-y-0 right-0 flex w-full flex-col border-l bg-background shadow-2xl sm:max-w-[600px]">
        ...
        .map(([key, value]) => `
                  <div class="flex items-center justify-between gap-2 text-sm">
                    ...
                    <span class="text-right text-xs ${key === '距招标结束'
        ? remaining === '已截止'
        ...
        .map((quote) => {
        const deviation = quote.hasQuoted && quote.quotePrice != null
        ...
        .join('')}
          ...
          <div class="space-y-2 pb-4">
            <p class="text-sm font-semibold">${effectiveStatus === 'AWARDED'
        ? '定标结果'
        ...
        : effectiveStatus === 'AWAIT_AWARD'
            ? `<div class="space-y-3 rounded-md border border-purple-200 bg-purple-50/40 p-3">
                ...
                : quotedRows
                    ...
                    const deviation = formatDeviation(quote.quotePrice, tender.standardPrice, tender.currency, tender.unit);
                    const isHigh = quote.quotePrice > tender.maxPrice;
                    const isLow = quote.quotePrice < tender.minPrice;
                    return `
                        ...
                        : ''}
                                          ...
                                          <span class="mt-0.5 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                                            <span class="font-medium text-foreground tabular-nums">${quote.quotePrice.toLocaleString()} ${escapeHtml(tender.currency)}/${escapeHtml(tender.unit)}</span>
                                            <span class="tabular-nums ${deviation.className}">偏差 ${escapeHtml(deviation.text)}</span>
                ...
                : '<span class="ml-1 text-muted-foreground">（可选）</span>'}
                      ...
                      <div class="flex items-center justify-between pt-1">
                        <p class="text-xs text-muted-foreground">${state.viewAwardFactoryName
...
function renderCreateTenderSheet() {
    if (!state.createOpen)
        return '';
                    ...
                    <button class="flex w-full items-start gap-2.5 px-3 py-2 text-left hover:bg-muted/30 ${checked ? 'bg-blue-50' : ''}" data-tender-action="toggle-pool" data-factory-id="${escapeHtml(factory.id)}">
                      ...
                      <span class="min-w-0 flex-1">
                        <span class="flex flex-wrap items-center gap-1.5">
                          ...
                          <span class="rounded px-1 py-0 text-[10px] ${factory.currentStatus === '正常'
            ? 'bg-green-50 text-green-700'
            : 'bg-amber-50 text-amber-700'}">${escapeHtml(factory.currentStatus)}</span>
                        </span>
...
function renderRow(tender) {
    ...
    <tr class="border-b last:border-b-0">
      ...
      <td class="whitespace-nowrap px-3 py-3 text-xs tabular-nums">${quoted > 0 ? `<span class="text-red-700">${escapeHtml(summary.maxStr)}</span>` : '<span class="text-muted-foreground">暂无报价</span>'}</td>
      <td class="whitespace-nowrap px-3 py-3 text-xs tabular-nums">${quoted > 0 ? `<span class="text-blue-700">${escapeHtml(summary.minStr)}</span>` : '<span class="text-muted-foreground">暂无报价</span>'}</td>
      ...
      <td class="whitespace-nowrap px-3 py-3 text-xs">${tender.awardedFactory
        ? `<span class="font-medium text-green-700">${escapeHtml(tender.awardedFactory)}</span>`
        : '<span class="text-muted-foreground">—</span>'}</td>
      <td class="whitespace-nowrap px-3 py-3 text-xs tabular-nums">${tender.awardedPrice != null
...
export function renderDispatchTendersPage() {
...
function confirmCreateTender() {
    ...
    const poolNames = selectedPoolIds.map((factoryId) => CANDIDATE_FACTORIES.find((factory) => factory.id === factoryId)?.name ?? factoryId);
    const newTender = {
...
export function handleDispatchTendersEvent(target) {
    const fieldNode = target.closest('[data-tender-field]');
    ...
    const actionNode = target.closest('[data-tender-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'clear-all-pool') {
        state.form.selectedPool = new Set();
        return true;
    }
...
export function isDispatchTendersDialogOpen() {
