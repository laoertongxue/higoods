import { findFactoryInternalWarehouseById, findFactoryWarehouseInboundRecordBySourceRecordId, findFactoryWarehouseOutboundRecordByHandoverRecordId, getFactoryWarehousePositionStatusOptions, listFactoryInternalWarehouses, } from '../data/fcs/factory-internal-warehouse.ts';
...
export function getMobileWarehouseRuntimeContext() {
...
export function renderMobileWarehouseLoginRedirect() {
    return renderPdaLoginRedirect();
}
export function getCurrentFactoryWarehouseByKind(warehouseKind) {
    const runtime = getPdaRuntimeContext();
    if (!runtime)
        return undefined;
    return listFactoryInternalWarehouses().find((item) => item.factoryId === runtime.factoryId && item.warehouseKind === warehouseKind);
}
export function renderWarehouseSummaryHeader(title, subtitle, overview) {
    return `
...
export function renderWarehouseActionCards(cards) {
    return `
        ...
        .map((card) => `
            ...
            >
              <div class="flex items-center justify-between gap-2">
                ...
                <span class="rounded-full px-2 py-0.5 text-[11px] ${toClassName(card.status === 'danger'
        ? 'bg-destructive/10 text-destructive'
        : card.status === 'warning'
            ? 'bg-amber-500/10 text-amber-600'
            : 'bg-muted text-muted-foreground')}">${escapeHtml(card.countText)}</span>
              </div>
...
export function renderStatusPill(status) {
    const className = status.includes('差异') || status.includes('异议')
        ? 'bg-destructive/10 text-destructive'
        : status.includes('待')
...
export function renderSectionFilterChips(activeValue, options, field) {
    return `
    <div class="flex gap-2 overflow-x-auto pb-1">
      ${options
        .map((option) => `
            <button
              type="button"
              class="whitespace-nowrap rounded-full border px-3 py-1.5 text-xs ${toClassName(option.value === activeValue ? 'border-primary bg-primary/10 text-primary' : 'text-muted-foreground')}"
...
export function renderMobilePageEmptyState(title, description) {
    return `
...
export function renderCompactFieldList(rows) {
    return `
...
export function getMobileWarehouseSearchParams() {
...
export function getWaitProcessSourceActionLabel(item) {
    return item.sourceRecordType === 'HANDOVER_RECEIVE' ? '交出接收' : '领料确认';
}
export function getWaitProcessSourceStatusLabel(item) {
    return item.status === '差异待处理' ? '差异待处理' : '已确认';
}
export function getWaitHandoverWritebackStatusLabel(item) {
    if (item.status === '异议中')
        return '异议中';
    if (item.status === '差异')
        return '差异';
...
export function getWarehouseGeneratedModeLabel() {
    return '自动转单';
}
export function getWarehouseQrDisplayText(value) {
    return value ? '已生成' : '—';
}
export function resolveWarehouseInboundRecordRoute(sourceRecordId) {
    if (!sourceRecordId)
...
export function resolveWarehouseOutboundRecordRoute(handoverRecordId) {
    if (!handoverRecordId)
...
export function resolveInboundSourceRoute(record) {
    if (record.sourceRecordType === 'HANDOVER_RECEIVE' && record.sourceRecordId) {
        ...
        if (handoverRecord?.handoverId)
            return `/fcs/pda/handover/${handoverRecord.handoverId}`;
    ...
    if (record.sourceRecordType === 'MATERIAL_PICKUP' && record.sourceRecordId) {
        ...
        if (pickupRecord?.handoverId)
            return `/fcs/pda/handover/${pickupRecord.handoverId}`;
    }
    return '/fcs/pda/handover?tab=pickup';
}
export function resolveTaskRoute(taskId) {
    return taskId ? `/fcs/pda/exec/${taskId}` : '/fcs/pda/exec';
}
export function resolveOutboundRoute(record) {
    if (record.handoverOrderId)
        return `/fcs/pda/handover/${record.handoverOrderId}`;
    return '/fcs/pda/handover';
}
export function resolveWaitProcessSourceRoute(item) {
...
export function resolveWaitHandoverRoute(item) {
    if (item.handoverOrderId)
        return `/fcs/pda/handover/${item.handoverOrderId}`;
    return '/fcs/pda/handover';
}
export function getWarehousePositionOptions(warehouseId) {
    ...
    const shelfOptionsByArea = Object.fromEntries(warehouse.areaList.map((area) => [
        area.areaName,
        area.shelfList.map((shelf) => ({ value: shelf.shelfNo, label: shelf.shelfName })),
    ]));
    const locationOptionsByShelf = Object.fromEntries(warehouse.areaList.flatMap((area) => area.shelfList.map((shelf) => [
        shelf.shelfNo,
        shelf.locationList.map((location) => ({ value: location.locationNo, label: location.locationName })),
    ])));
    return { areaOptions, shelfOptionsByArea, locationOptionsByShelf };
}
export function getWarehousePositionStatusLabels() {
    return getFactoryWarehousePositionStatusOptions();
}
export function formatWarehouseDateTime(value) {
    return value ? formatDateTime(value) : '-';
}
export function buildWarehouseDifferenceText(value) {
    if (typeof value !== 'number')
        return '-';
    return `${value}`;
}
export function buildStocktakeStatusText(line) {
    return line.status;
}
export function buildStocktakeOrderSummary(order) {
