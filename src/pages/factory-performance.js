import { indonesiaFactories, kpiTemplateLabels, tierLabels, typeLabels, } from '../data/fcs/indonesia-factories';
...
function calculateScore(data) {
    const score = data.onTimeRate * 0.4 +
...
function checkRisks(data) {
    const risks = [];
    if (data.onTimeRate < 90)
        risks.push('准时交付率低于90%');
    if (data.defectRate > 3)
        risks.push('残次率超过3%');
    return risks;
}
...
function generatePerformanceData() {
    return indonesiaFactories.map((factory) => {
        const score = factory.performanceScore ||
            calculateScore({
...
function getFilteredList() {
    ...
    if (state.keyword.trim()) {
        ...
        result = result.filter((factory) => factory.factoryName.toLowerCase().includes(keyword) ||
...
function getPagedList(filteredList) {
    const start = (state.currentPage - 1) * PAGE_SIZE;
    return filteredList.slice(start, start + PAGE_SIZE);
}
function getSummary(filteredList) {
    ...
    const sum = filteredList.reduce((acc, factory) => ({
...
function validateForm() {
    const errors = {};
    if (!state.form.factoryId)
        errors.factoryId = '请选择工厂';
...
function submitForm() {
    ...
    state.performanceList = state.performanceList.map((factory) => factory.factoryId === state.form.factoryId
        ? {
            ...factory,
...
function renderSummaryCard(icon, iconClass, label, value) {
    return `
...
function renderFormDialog() {
    ...
    const factorySelector = editingFactory
        ? ''
        : `
        <div class="space-y-2">
          ...
          <select data-perf-field="factoryId" class="w-full rounded-md border px-3 py-2 text-sm">
            ...
            .map((factory) => `<option value="${factory.factoryId}" ${state.form.factoryId === factory.factoryId ? 'selected' : ''}>${escapeHtml(factory.factoryName)}</option>`)
            .join('')}
          </select>
...
function renderLogDialog() {
    if (state.dialog.type !== 'log')
        return '';
    const content = state.changeLogs.length > 0
        ? `
        <table class="w-full text-sm">
          <thead class="border-b">
            <tr>
...
export function renderFactoryPerformancePage() {
...
export function handleFactoryPerformanceEvent(target) {
    const formField = target.closest('[data-perf-field]');
    if (formField instanceof HTMLInputElement ||
        ...
        if (field === 'factoryId')
            state.form.factoryId = value;
        if (field === 'period')
            state.form.period = value;
        if (field === 'onTimeRate')
            state.form.onTimeRate = Number(value);
        if (field === 'defectRate')
            state.form.defectRate = Number(value);
        if (field === 'rejectRate')
            state.form.rejectRate = Number(value);
        if (field === 'disputeRate')
            state.form.disputeRate = Number(value);
        if (field === 'note')
            state.form.note = value;
        return true;
    }
    const filterNode = target.closest('[data-perf-filter]');
    if (filterNode instanceof HTMLInputElement || filterNode instanceof HTMLSelectElement) {
        ...
        if (filter === 'keyword')
            state.keyword = value;
        if (filter === 'month')
            state.monthFilter = value;
        return true;
    }
    const actionNode = target.closest('[data-perf-action]');
    if (!actionNode) {
...
export function isFactoryPerformanceDialogOpen() {
