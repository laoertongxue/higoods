import { applyPrepaymentBatchForPayment, canStatementEnterPrepayment, closePrepaymentBatch, createPaymentWriteback, createPrepaymentBatch, getFeishuPaymentApprovalById, getOpenStatementAppeal, getPaymentWritebackById, getStatementSettlementProgressView, initialSettlementBatches, initialStatementDrafts, syncFeishuPaymentApprovalStatus, } from '../data/fcs/store-domain-settlement-seeds';
...
function formatAmount(value, currency = 'IDR') {
    return `${currency} ${value.toLocaleString('zh-CN', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
    })}`;
}
function buildBatchesHref(view = state.activeView) {
    if (view === 'IN_PROGRESS')
        return '/fcs/settlement/batches';
    const params = new URLSearchParams();
    params.set('view', view === 'PAYMENT' ? 'payment' : view === 'COMPLETED' ? 'completed' : 'history');
    return `/fcs/settlement/batches?${params.toString()}`;
}
function getCurrentBatchSearchParams() {
    const pathname = appStore.getState().pathname;
    const [, query] = pathname.split('?');
    return new URLSearchParams(query || '');
}
function syncBatchesStateFromRoute() {
...
function showBatchesToast(message, tone = 'success') {
...
function getStatementCurrency(statement) {
...
function getWritebackStatus(batch, approval, writeback) {
    if (writeback)
        return 'WRITTEN_BACK';
    if (approval?.status === 'PAID')
        return 'PAID_PENDING_WRITEBACK';
...
function getPaidAmount(batch, approval, writeback) {
    if (writeback)
        return writeback.amount;
    if (approval?.status === 'PAID')
        return approval.amount;
    return batch.paymentAmount ?? 0;
}
function getPendingAmount(batch, approval, writeback) {
    return Math.max(0, batch.totalPayableAmount - getPaidAmount(batch, approval, writeback));
}
function getPrimarySnapshot(batch) {
    return batch.settlementProfileSnapshotRefs?.[0];
}
function isHistoryBatch(batch) {
    return batch.status === 'CLOSED';
}
function isPaymentViewBatch(batch) {
    return batch.status === 'FEISHU_PAID_PENDING_WRITEBACK';
}
function isCompletedViewBatch(batch) {
    return batch.status === 'PREPAID';
}
function isInProgressBatch(batch) {
    return !isPaymentViewBatch(batch) && !isCompletedViewBatch(batch) && !isHistoryBatch(batch);
}
function canApplyPayment(batch, approval) {
    if (approval && approval.status !== 'REJECTED' && approval.status !== 'CANCELED')
        return false;
    return batch.status === 'READY_TO_APPLY_PAYMENT' || batch.status === 'DRAFT';
}
function canSyncApproval(approval, writeback) {
    if (!approval || writeback)
        return false;
    return (approval.status === 'CREATED' ||
        approval.status === 'APPROVING' ||
        approval.status === 'APPROVED_PENDING_PAYMENT');
}
function canCreateWriteback(batch, approval, writeback) {
    return Boolean(batch.feishuApprovalId && approval?.status === 'PAID' && !writeback);
}
function canCloseBatch(batch, writeback) {
    return batch.status === 'PREPAID' && Boolean(writeback);
}
function getOccupiedStatementIds() {
    return new Set(initialSettlementBatches
        .filter((item) => item.status !== 'CLOSED')
        .flatMap((item) => item.statementIds));
}
function getCandidateStatements() {
    const occupiedIds = getOccupiedStatementIds();
    return initialStatementDrafts.filter((item) => canStatementEnterPrepayment(item) && !occupiedIds.has(item.statementId));
}
function getPartyOptions(candidates) {
    const seen = new Map();
...
function getFilteredPool(candidates) {
    const keyword = state.poolKeyword.trim().toLowerCase();
    return candidates.filter((item) => {
        if (state.poolParty !== '__all__' && item.settlementPartyId !== state.poolParty)
            return false;
        if (!keyword)
            return true;
        const haystack = [
...
function matchesBatchKeyword(batch) {
...
function sortBatches(items, anchor) {
    const getAnchor = (item) => {
        const approval = getBatchApproval(item);
        const writeback = getBatchWriteback(item);
        if (anchor === 'closed')
            return item.closedAt ?? item.prepaidAt ?? writeback?.writtenBackAt ?? item.updatedAt ?? item.createdAt;
        if (anchor === 'updated')
            return approval?.latestSyncedAt ?? writeback?.writtenBackAt ?? item.updatedAt ?? item.createdAt;
        return item.createdAt;
    };
    return [...items].sort((left, right) => getAnchor(right).localeCompare(getAnchor(left)));
}
function getBatchesByView(view, includeKeyword = true) {
    const all = includeKeyword
        ? initialSettlementBatches.filter((item) => matchesBatchKeyword(item))
        : [...initialSettlementBatches];
    if (view === 'IN_PROGRESS')
        return sortBatches(all.filter((item) => isInProgressBatch(item)), 'updated');
    if (view === 'PAYMENT')
        return sortBatches(all.filter((item) => isPaymentViewBatch(item)), 'updated');
    if (view === 'COMPLETED')
        return sortBatches(all.filter((item) => isCompletedViewBatch(item)), 'updated');
    return sortBatches(all.filter((item) => isHistoryBatch(item)), 'closed');
}
function getWorkbenchCounts(candidates) {
    return {
...
function getSelectedAmount(candidates) {
...
function getBatchLifecycleRecords(batch, approval, writeback) {
    const records = [
        {
            title: '创建预付款批次',
...
function getBatchDetailViewModel(batch) {
    const approval = getBatchApproval(batch);
    const writeback = getBatchWriteback(batch);
    return {
        batch,
...
function renderWorkbenchCard(label, value, note, active, action) {
    return `
    <article class="${toClassName('rounded-lg border bg-card transition-colors', active ? 'border-blue-300 shadow-sm' : '')}">
...
function renderStatsSection(counts) {
    return `
...
function renderViewSwitcher(counts) {
    const entries = [
    ...
    <section class="flex flex-col gap-3 rounded-lg border bg-card p-4">
      <div class="inline-flex flex-wrap gap-2">
        ...
        .map((entry) => `
              <button
                class="${toClassName('inline-flex items-center rounded-md border px-3 py-1.5 text-sm transition-colors', state.activeView === entry.view
...
function renderCandidatePool(candidates) {
    ...
    const selectedFactoryIds = new Set(Array.from(state.selected)
    ...
    const allSelected = filteredPool.length > 0 && filteredPool.every((item) => state.selected.has(item.statementId));
    return `
    <section class="space-y-3 rounded-lg border bg-card p-4">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <select class="h-9 w-56 rounded-md border bg-background px-3 text-sm" data-batch-field="poolParty">
          ...
          ${partyOptions
        .map((item) => `<option value="${escapeHtml(item.key)}" ${state.poolParty === item.key ? 'selected' : ''}>${escapeHtml(item.label)}</option>`)
        .join('')}
        </select>
      </div>
...
function renderBatchActions(batch, approval, writeback) {
    const actions = [
    ...
    if (canApplyPayment(batch, approval)) {
        actions.push(`<button class="inline-flex h-7 items-center rounded-md border px-2 text-xs hover:bg-muted" data-batch-action="apply-payment" data-batch-id="${escapeHtml(batch.batchId)}">申请付款</button>`);
    }
    else if (canSyncApproval(approval, writeback)) {
        actions.push(`<button class="inline-flex h-7 items-center rounded-md border px-2 text-xs hover:bg-muted" data-batch-action="sync-approval" data-batch-id="${escapeHtml(batch.batchId)}">同步飞书状态</button>`);
    }
    else if (canCreateWriteback(batch, approval, writeback)) {
        actions.push(`<button class="inline-flex h-7 items-center rounded-md border px-2 text-xs hover:bg-muted" data-batch-action="create-writeback" data-batch-id="${escapeHtml(batch.batchId)}">创建打款回写</button>`);
    }
    else if (canCloseBatch(batch, writeback)) {
        actions.push(`<button class="inline-flex h-7 items-center rounded-md border px-2 text-xs hover:bg-muted" data-batch-action="close-batch" data-batch-id="${escapeHtml(batch.batchId)}">关闭批次</button>`);
    }
    return `<div class="flex flex-wrap gap-1">${actions.join('')}</div>`;
}
function renderBatchRows(view, items) {
    if (items.length === 0) {
        const emptyText = view === 'IN_PROGRESS'
            ? '暂无待申请或审批中的预付款批次'
            : view === 'PAYMENT'
                ? '暂无已付款待回写的预付款批次'
                : view === 'COMPLETED'
    ...
    const timeLabel = view === 'PAYMENT' ? '最近同步' : view === 'COMPLETED' ? '预付时间' : view === 'HISTORY' ? '关闭时间' : '创建时间';
    return `
    <div class="overflow-x-auto rounded-md border">
      <table class="w-full min-w-[1180px] text-sm">
        <thead>
        ...
        const primaryTime = view === 'PAYMENT'
...
function renderCurrentViewList() {
    ...
    <div class="fixed inset-0 z-50" data-dialog-backdrop="true">
      ...
      <section class="absolute left-1/2 top-1/2 flex max-h-[90vh] w-full max-w-6xl -translate-x-1/2 -translate-y-1/2 flex-col rounded-xl border bg-background p-6 shadow-2xl" data-dialog-panel="true">
          ...
          <section class="rounded-lg border bg-card p-4">
            ...
            <dl class="mt-3 space-y-2 text-sm">
              ...
              <div class="flex items-start justify-between gap-3">
                ...
                <dd class="text-right text-xs">${escapeHtml(primarySnapshot
        ? `${primarySnapshot.receivingAccountSnapshot.bankName} · ${maskBankAccountNo(primarySnapshot.receivingAccountSnapshot.bankAccountNo)}`
        : '当前未绑定')}</dd>
              </div>
            </dl>
          </section>
        ...
        ? `
                ...
                `
        : `<p class="mt-3 text-sm text-muted-foreground">${detail.approval?.status === 'PAID'
...
export function handleBatchesEvent(target) {
    const fieldNode = target.closest('[data-batch-field]');
    ...
    const actionNode = target.closest('[data-batch-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'switch-view') {
        ...
        if (view)
            appStore.navigate(buildBatchesHref(view));
        return true;
    }
    ...
    if (action === 'toggle-select-all') {
        ...
        if (allSelected)
            ...
            state.selected = new Set(filteredPool.map((item) => item.statementId));
        return true;
    }
...
export function isBatchesDialogOpen() {
