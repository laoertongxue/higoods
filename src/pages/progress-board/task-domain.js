import { state, TASK_RISK_LABEL, TASK_STATUS_LABEL, PROCESS_STAGE_GROUP_LABEL, ASSIGNMENT_STATUS_LABEL, ASSIGNMENT_STATUS_COLOR_CLASS, BLOCK_REASON_LABEL, stageLabels, getTaskHandoverSummary, getTaskPickupSummary, getTaskHandoutSummary, getTaskProgressFact, getTaskStageGroup, getOrderById, getFactoryById, getTaskRisks, getTaskDisplayName, getOrderSpuCode, getOrderSpuName, getTaskTenderId, getTenderById, getTaskById, getTaskKpiStats, getTaskKanbanGroups, BLOCK_REASON_OPTIONS, renderBadge, escapeAttr, escapeHtml, } from './context.ts';
import { resolveTaskStandardTimeSnapshot } from '../../data/fcs/process-tasks.ts';
import { buildTaskDeliveryCardPrintLink, buildTaskRouteCardPrintLink, } from '../../data/fcs/fcs-route-links.ts';
import { PLATFORM_PROCESS_STATUS_CLASS, getPlatformStatusForRuntimeTask, } from '../../data/fcs/process-platform-status-adapter.ts';
import { listPlatformProcessResultViews } from '../../data/fcs/platform-process-result-view.ts';
...
function renderTaskRiskBadges(risks) {
    if (!risks.length)
...
let platformResultViewByTaskId = null;
function getPlatformResultViewByTaskId(taskId) {
    if (!platformResultViewByTaskId) {
        ...
        for (const view of listPlatformProcessResultViews()) {
            ...
            if (taskIdFromLink)
                platformResultViewByTaskId.set(decodeURIComponent(taskIdFromLink), view);
        }
    }
    return platformResultViewByTaskId.get(taskId);
}
function getTaskPlatformSummary(task) {
...
function renderTaskChainStatusCell(summary, action, taskId) {
    return `
    <button
...
function renderPlatformStatusCell(task) {
...
function renderEmptySubsection(title, hint) {
    return `
...
function renderDrawerSectionTable(title, headers, rows, emptyText, attrs = '') {
    if (!rows.length)
        return renderEmptySubsection(title, emptyText);
    return `
...
function renderTaskDeliveryCardAction(recordId) {
    if (!recordId) {
...
function renderPickupTab(task) {
    ...
    const draftOnlyRows = requestRows.length === 0
        ? pickupSummary.draftRows.map((row) => [
            escapeHtml('草稿待确认'),
            escapeHtml(row.lines
    ...
    const pickupRecordRows = pickupSummary.pickupHeads.flatMap((head) => {
        ...
        return records.map((record) => [
            ...
            escapeHtml(record.finalResolvedAt ||
    ...
    <div class="space-y-4" data-progress-task-tab-panel="pickup">
      ...
      ${renderDrawerSectionTable('领料需求', ['领料需求单号', '物料 / 面料概况', '领料方式', '当前状态', '更新时间'], requestRows.length > 0 ? requestRows : draftOnlyRows, '当前任务暂无领料需求或仍处于草稿阶段。', 'data-progress-task-pickup-section=\"requests\"')}
      ${renderDrawerSectionTable('仓库发料执行', ['单号', '类型', '当前状态', '计划物料对象', '已发物料对象', '差异物料对象'], executionRows, '当前任务暂无仓库发料执行记录。', 'data-progress-task-pickup-section=\"execution\"')}
      ${renderDrawerSectionTable('领料记录', ['领料单号', '当前状态', '应领物料对象', '已领物料对象', '最新领料时间', '差异 / 异议'], pickupRecordRows, '当前任务暂无领料记录。', 'data-progress-task-pickup-section=\"records\"')}
    </div>
  `;
}
function renderHandoverTab(task) {
    const handoutSummary = getTaskHandoutSummary(task.taskId);
    const handoutHeadRows = handoutSummary.handoutHeads.map((head) => [
        escapeHtml(head.handoverId),
        escapeHtml(head.summaryStatus === 'HAS_OBJECTION'
            ? '交出异议中'
    ...
    const handoutRecordRows = handoutSummary.handoutRecords.map((record) => [
        ...
        escapeHtml(record.status),
        escapeHtml(typeof record.warehouseWrittenQty === 'number'
    ...
    <div class="space-y-4" data-progress-task-tab-panel="handover">
      ...
      ${renderDrawerSectionTable('交出单', ['交出单号', '当前状态', '应交对象', '已交对象', '最新操作时间'], handoutHeadRows, '当前任务暂无交出单。', 'data-progress-task-handover-section=\"heads\"')}
      ${renderDrawerSectionTable('交出记录', ['记录号', '提交时间', '状态', '回写对象', '接收方回写时间', '是否有异议', '操作'], handoutRecordRows, '当前任务暂无交出记录。', 'data-progress-task-handover-section=\"records\"')}
      ${disputeRows.length > 0
        ? renderDrawerSectionTable('差异 / 异议', ['异议原因', '异议状态', '后续说明', '处理时间'], disputeRows, '当前任务暂无差异或异议。', 'data-progress-task-handover-section=\"disputes\"')
...
function renderTaskListView(filteredTasks) {
...
function renderTaskKanbanView(filteredTasks) {
...
function renderTaskDimension(filteredTasks) {
      ...
      <section class="absolute inset-y-0 right-0 w-full max-w-[600px] overflow-y-auto border-l bg-background shadow-2xl" data-progress-task-drawer="true">
        ...
        ? `
                <div class="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <p class="text-xs text-muted-foreground">分配方式</p>
                    <p class="mt-1">${task.assignmentMode === 'DIRECT'
            ? renderBadge('派单', 'border-slate-200 bg-slate-100 text-slate-700')
            : renderBadge('竞价', 'border-blue-200 bg-blue-100 text-blue-700')}</p>
                  </div>
                  <div>
...
function renderBatchConfirmDialog() {
    if (!state.confirmDialogType)
        return '';
    const title = state.confirmDialogType === 'start' ? '批量标记开始' : '批量标记完工';
    const desc = state.confirmDialogType === 'start'
        ? '确认将选中的任务批量标记为进行中？'
        : '确认将选中的任务批量标记为已完成？';
    return `
    <div class="fixed inset-0 z-[60]" data-dialog-backdrop="true">
