import { state, LIFECYCLE_LABEL, getTaskById, getProductionOrderHandoverSummary, getTaskHandoverSummary, buildHandoverOrderDetailLink, getFilteredTasks, getFactoryById, } from './context.ts';
import { showProgressBoardToast, copyToClipboard, openLinkedPage, clearTaskFilters, handleTaskKpiClick, handlePoKpiClick, setTaskSelected, openOrderDetail, openTaskDetail, handleBatchUrge, openBatchDialog, confirmBatchAction, requestTaskStatusChange, createUrge, confirmTaskBlock, } from './actions.ts';
...
function handleTaskAction(action, actionNode) {
...
function handleOrderAction(action, actionNode) {
...
function handleAction(action, actionNode) {
    ...
    if (action === 'kpi-filter') {
        ...
        if (kpi)
            handleTaskKpiClick(kpi);
        return true;
    }
    if (action === 'po-kpi-filter') {
        const lifecycle = actionNode.dataset.lifecycle;
    ...
    if (action === 'open-task-detail') {
        if (actionNode.closest('[data-progress-stop="true"]'))
            return false;
        const taskId = actionNode.dataset.taskId;
        if (taskId) {
            openTaskDetail(taskId);
    ...
    if (action === 'task-status-start') {
        ...
        if (task)
            requestTaskStatusChange(task, 'IN_PROGRESS');
        return true;
    }
    if (action === 'task-status-finish') {
        ...
        const task = taskId ? getTaskById(taskId) : null;
        if (task)
            requestTaskStatusChange(task, 'DONE');
        return true;
    }
    if (action === 'task-status-block') {
        ...
        const task = taskId ? getTaskById(taskId) : null;
        if (task)
            requestTaskStatusChange(task, 'BLOCKED');
        return true;
    }
...
export function handleProgressBoardEvent(target) {
    const fieldNode = target.closest('[data-progress-field]');
    ...
    const actionNode = target.closest('[data-progress-action]');
    if (!actionNode) {
