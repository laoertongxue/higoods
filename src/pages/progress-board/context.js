...
import { stageLabels } from '../../data/fcs/process-types.ts';
import { productionOrders } from '../../data/fcs/production-orders.ts';
import { indonesiaFactories } from '../../data/fcs/indonesia-factories.ts';
import { generateCaseId, generateNotificationId, generateUrgeId, listProgressExceptions, upsertProgressExceptionCase, initialNotifications, initialUrges, } from '../../data/fcs/store-domain-progress.ts';
...
import { buildHandoverOrderDetailLink, getHandoverLedgerRows, getProductionOrderHandoverSummary, getTaskHandoverSummary, } from '../../data/fcs/handover-ledger-view.ts';
import { sortProductionProgressByDefaultDueDate } from '../../data/fcs/progress-statistics-linkage.ts';
import { listMaterialRequestDraftsByOrder, } from '../../data/fcs/material-request-drafts.ts';
import { listWarehouseExecutionDocsByRuntimeTaskId, } from '../../data/fcs/warehouse-material-execution.ts';
import { getPdaHandoverRecordsByHead, getPdaPickupRecordsByHead, listPdaHandoverHeads, } from '../../data/fcs/pda-handover-events.ts';
import { getDefaultSubCategoryKeyFromReason, getUnifiedCategoryFromReason, } from '../../data/fcs/progress-exception-taxonomy.ts';
import { getTaskChainTaskById, getTaskChainTaskDisplayName, getTaskChainTenderById, listTaskChainTasks, resolveTaskChainTenderId, } from '../../data/fcs/page-adapters/task-chain-pages-adapter.ts';
import { listProgressFacts } from '../../data/fcs/store-domain-progress.ts';
...
function createTaskBoardSummaryCache() {
    return {
...
const PROCESS_STAGE_GROUP_LABEL = {
    PREP: '准备阶段',
    PROD: '生产阶段',
    POST: '后道阶段',
};
function getTaskStageGroup(stage) {
    if (stage === 'POST')
        return 'POST';
    if (stage === 'PREP' || stage === 'MATERIAL' || stage === 'WAREHOUSE')
        return 'PREP';
    return 'PROD';
}
...
const ASSIGNMENT_STATUS_COLOR_CLASS = {
...
function getTaskProgressFact(taskId) {
    if (!taskBoardSummaryCache.progressFactsByTaskId.has(taskId)) {
        taskBoardSummaryCache.progressFactsByTaskId.set(taskId, getAllProgressFacts().find((fact) => fact.runtimeTaskId === taskId || fact.baseTaskId === taskId));
    }
    return taskBoardSummaryCache.progressFactsByTaskId.get(taskId);
}
function getTaskDraftRows(task) {
    if (!taskBoardSummaryCache.draftRowsByTaskId.has(task.taskId)) {
        taskBoardSummaryCache.draftRowsByTaskId.set(task.taskId, listMaterialRequestDraftsByOrder(task.productionOrderId).filter((draft) => draft.taskId === task.taskId));
    }
    return taskBoardSummaryCache.draftRowsByTaskId.get(task.taskId) ?? [];
}
function getTaskExecutionDocs(taskId) {
...
function getPickupRecordsForHead(headId) {
...
function getHandoutRecordsForHead(headId) {
...
function getTaskPickupSummary(taskId) {
    ...
    const executionRows = getTaskExecutionDocs(taskId).filter((doc) => doc.docType === 'ISSUE');
    ...
    const latestOccurredAt = pickLatestTimestamp([
        ...
        ...pickupRecords.map((row) => row.finalResolvedAt ||
            row.factoryConfirmedAt ||
            row.receivedAt ||
            row.warehouseHandedAt ||
            row.submittedAt),
    ]);
    const hasDifference = pickupRecords.some((record) => ['OBJECTION_REPORTED', 'OBJECTION_PROCESSING', 'OBJECTION_RESOLVED'].includes(record.status));
    const finalReceivedCount = countBy(pickupRecords, (record) => record.status === 'RECEIVED' || record.status === 'OBJECTION_RESOLVED');
    ...
    const preparingCount = countBy(executionRows, (row) => row.status === 'PREPARING' || row.status === 'PARTIALLY_PREPARED');
    const readyCount = countBy(executionRows, (row) => ['READY', 'ISSUED', 'IN_TRANSIT', 'RECEIVED', 'CLOSED'].includes(row.status));
...
function getTaskHandoutSummary(taskId) {
    ...
    const objectionRows = handoutRecords.filter((record) => ['OBJECTION_REPORTED', 'OBJECTION_PROCESSING', 'OBJECTION_RESOLVED'].includes(record.status));
    ...
    const disputeText = objectionRows[0]?.objectionReason ??
...
function deriveBlockpoint(tasks) {
    if (tasks.some((task) => task.status === 'BLOCKED' && task.blockReason === 'QUALITY'))
        return '质检未已完成';
    if (tasks.some((task) => task.status === 'BLOCKED' && task.blockReason === 'MATERIAL'))
        return '染印回货未完成';
    if (tasks.some((task) => task.status === 'BLOCKED')) {
        return `${tasks.filter((task) => task.status === 'BLOCKED').length} 个任务生产暂停中`;
    }
    if (tasks.some((task) => task.assignmentStatus === 'UNASSIGNED'))
        return '后道任务未分配';
    if (tasks.some((task) => task.assignmentStatus === 'BIDDING'))
        return '存在竞价未定标任务';
    return '—';
}
function deriveNextAction(lifecycle, tasks) {
    switch (lifecycle) {
        case 'PREPARING':
            return '完成技术包并发布';
        case 'PENDING_ASSIGN':
...
function deriveLifecycle(tasks, order) {
    if (tasks.length === 0)
        return 'PREPARING';
...
function buildOrderUrgencyLevel(order, qty) {
...
function getPoViewRows() {
    ...
    return orderIds.map((orderId) => {
        ...
        if (tasks.some((task) => task.status === 'BLOCKED'))
            risks.push('有生产暂停');
        if (tasks.some((task) => getTaskRisks(task).includes('TASK_OVERDUE')))
            risks.push('逾期风险');
        if (tasks.some((task) => getTaskRisks(task).includes('TENDER_OVERDUE')))
            risks.push('竞价逾期');
        if (order?.techPackSnapshot?.status !== 'RELEASED')
            risks.push('技术包未发布');
        return {
...
function getFilteredPoRows(rows) {
...
function getPoKanbanGroups(rows) {
    const groups = {
        PREPARING: [],
        PENDING_ASSIGN: [],
        IN_EXECUTION: [],
...
function getFilteredTasks() {
    ...
    return listBoardTasks().filter((task) => {
        ...
        if (state.statusFilter !== 'ALL' && task.status !== state.statusFilter)
            return false;
        if (state.assignmentStatusFilter !== 'ALL' && task.assignmentStatus !== state.assignmentStatusFilter)
            return false;
        if (state.assignmentModeFilter !== 'ALL' && task.assignmentMode !== state.assignmentModeFilter)
            return false;
        if (state.processFilter !== 'ALL' && task.processCode !== state.processFilter)
            return false;
        if (state.stageFilter !== 'ALL' && getTaskStageGroup(task.stage) !== state.stageFilter)
            return false;
        if (state.factoryFilter !== 'ALL' && task.assignedFactoryId !== state.factoryFilter)
            return false;
        if (state.riskFilter !== 'ALL') {
            const risks = getTaskRisks(task);
            if (state.riskFilter === 'blockedOnly' && task.status !== 'BLOCKED')
                return false;
            if (state.riskFilter === 'tenderOverdueOnly' && !risks.includes('TENDER_OVERDUE'))
                return false;
            if (state.riskFilter === 'rejectedOnly' && !risks.includes('DISPATCH_REJECTED'))
                return false;
            if (state.riskFilter === 'taskOverdueOnly' && !risks.includes('TASK_OVERDUE'))
                return false;
        }
        return true;
    });
}
function getTaskKanbanGroups(tasks) {
    return {
...
function syncPresetFromQuery() {
    ...
    if (!state.initializedByQuery) {
        ...
        if (presetStatus)
            state.statusFilter = presetStatus;
        if (presetAssignmentStatus)
            state.assignmentStatusFilter = presetAssignmentStatus;
        if (presetRisk)
            state.riskFilter = presetRisk;
    }
    if (presetTaskId) {
...
export { appStore, escapeHtml, toClassName, state, TASK_STATUS_LABEL, PROCESS_STAGE_GROUP_LABEL, ASSIGNMENT_STATUS_LABEL, STATUS_COLOR_CLASS, ASSIGNMENT_STATUS_COLOR_CLASS, LIFECYCLE_LABEL, LIFECYCLE_COLOR_CLASS, LIFECYCLE_ICON, TASK_RISK_LABEL, BLOCK_REASON_LABEL, BLOCK_REASON_OPTIONS, stageLabels, productionOrders, indonesiaFactories, generateCaseId, generateNotificationId, generateUrgeId, listProgressExceptions, upsertProgressExceptionCase, initialNotifications, initialUrges, syncPdaStartRiskAndExceptions, syncMilestoneOverdueExceptions, buildHandoverOrderDetailLink, getHandoverLedgerRows, getProductionOrderHandoverSummary, getTaskHandoverSummary, getDefaultSubCategoryKeyFromReason, getUnifiedCategoryFromReason, getTaskChainTaskById, getTaskChainTaskDisplayName, getTaskChainTenderById, listTaskChainTasks, resolveTaskChainTenderId, nowTimestamp, getCurrentQueryString, getCurrentSearchParams, getOrderById, getFactoryById, getTenderById, listBoardTasks, getTaskDisplayName, getTaskById, resetTaskBoardSummaryCache, getTaskPickupSummary, getTaskHandoutSummary, getTaskProgressFact, getTaskStageGroup, getTaskTenderId, getTaskDependencies, getOrderSpuCode, getOrderSpuName, getOrderQty, parseDateTime, deriveBlockpoint, deriveNextAction, deriveLifecycle, getTaskRisks, getTaskKpiStats, getPoViewRows, getPoKpiStats, getFilteredPoRows, getPoKanbanGroups, getFilteredTasks, getTaskKanbanGroups, getUniqueFactories, getExceptionsByTaskId, nextUrgeAuditLogId, nextExceptionAuditLogId, nextTaskAuditLogId, nextOrderAuditLogId, syncPresetFromQuery, escapeAttr, renderBadge, };
