import { state, LIFECYCLE_LABEL, LIFECYCLE_COLOR_CLASS, LIFECYCLE_ICON, getFactoryById, getPoKanbanGroups, getPoKpiStats, getFilteredPoRows, listBoardTasks, getTaskDisplayName, TASK_STATUS_LABEL, STATUS_COLOR_CLASS, renderBadge, escapeAttr, escapeHtml, } from './context.ts';
import { buildProductionConfirmationLink, buildProductionOrderLink, buildQualityRecordLink, buildTaskDetailLink, } from '../../data/fcs/fcs-route-links.ts';
import { getQcViewRows } from '../qc-records/context.ts';
function getStartedTaskCount(row) {
    return row.inProgressTasks + row.doneTasks + row.blockedTasks;
}
function getPendingHandoverCount(row) {
    return row.handoverStatusLabel.includes('待交出') ? Math.max(row.handoverPendingCount, 1) : 0;
}
function getPendingWritebackCount(row) {
    return row.handoverStatusLabel.includes('回写') ? Math.max(row.handoverPendingCount, 1) : 0;
}
function getDifferenceCount(row) {
    return row.handoverStatusLabel.includes('差异') ? Math.max(row.handoverObjectionCount, 1) : 0;
}
function getQcStatusLabel(row) {
    if (row.lifecycle === 'PENDING_QC')
        return '待处理';
    if (row.lifecycle === 'PENDING_SETTLEMENT' || row.lifecycle === 'CLOSED')
        return '已完成';
    return '正常';
}
function getRecheckStatusLabel(row) {
    return row.risks.some((risk) => risk.includes('质检') || risk.includes('复检')) ? '待处理' : '正常';
}
function getRiskStatusLabel(row) {
    return row.risks.length > 0 ? '风险' : '正常';
}
function getReceivingQcLink(orderId) {
    const row = getQcViewRows().find((item) => item.productionOrderId === orderId && item.qc.inspectionScene !== 'POST_FINAL_RECHECK');
    return row ? buildQualityRecordLink(row.qcId) : '';
}
function getFinalRecheckLink(orderId) {
    const row = getQcViewRows().find((item) => item.productionOrderId === orderId && item.qc.inspectionScene === 'POST_FINAL_RECHECK');
    return row ? buildQualityRecordLink(row.qcId) : '';
}
function renderOrderActionMenu(row) {
...
function renderOrderListView(rows) {
    return `
...
function renderOrderKanbanView(rows) {
    const groups = getPoKanbanGroups(rows);
    return `
    <div class="grid grid-cols-6 gap-3">
      ${Object.keys(LIFECYCLE_LABEL)
        .map((lifecycle) => {
...
function renderOrderDimension(rows) {
...
function renderOrderDrawer(rows) {
...
function renderDetailSummaryChip(label, value) {
    return `
