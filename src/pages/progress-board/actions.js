import { state, BLOCK_REASON_LABEL, nowTimestamp, getTaskById, getFactoryById, getTaskTenderId, getTenderById, listBoardTasks, getUnifiedCategoryFromReason, getDefaultSubCategoryKeyFromReason, generateCaseId, generateNotificationId, generateUrgeId, listProgressExceptions, upsertProgressExceptionCase, initialNotifications, initialUrges, appStore, } from './context.ts';
...
function createOrUpdateExceptionFromSignal(signal) {
    ...
    const existed = listProgressExceptions().find((item) => item.sourceType === signal.sourceType &&
    ...
    if (existed) {
        ...
        if (!existed.unifiedCategory)
            existed.unifiedCategory = unifiedCategory;
        if (!existed.subCategoryKey)
            existed.subCategoryKey = subCategoryKey;
        existed.auditLogs = [
            ...existed.auditLogs,
            {
    ...
    const s1Reasons = ['TENDER_OVERDUE', 'NO_BID', 'FACTORY_BLACKLISTED', 'HANDOVER_DIFF'];
    const s2Reasons = [
        'DISPATCH_REJECTED',
        'ACK_TIMEOUT',
...
function updateTaskStatus(taskId, newStatus, blockReason, blockRemark, by = 'Admin') {
...
function showProgressBoardToast(message, tone = 'success') {
...
function handleTaskKpiClick(type) {
    clearTaskFilters();
    switch (type) {
        case 'notStarted':
...
function handlePoKpiClick(lifecycle) {
    state.poKeyword = '';
    state.poLifecycleFilter = lifecycle;
}
function setTaskSelected(taskId, checked) {
    if (checked) {
...
function confirmBatchAction() {
    if (!state.confirmDialogType || state.confirmTaskIds.length === 0)
        return;
    const newStatus = state.confirmDialogType === 'start' ? 'IN_PROGRESS' : 'DONE';
...
function requestTaskStatusChange(task, nextStatus) {
...
export { showProgressBoardToast, copyToClipboard, openLinkedPage, clearTaskFilters, handleTaskKpiClick, handlePoKpiClick, setTaskSelected, openOrderDetail, openTaskDetail, handleBatchUrge, openBatchDialog, confirmBatchAction, requestTaskStatusChange, createUrge, confirmTaskBlock, };
