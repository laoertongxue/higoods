...
import { ACCOUNTING_STATUS_META, LIVE_PURPOSE_META, SESSION_STATUS_META, getLiveSessionById, getLiveSessionItems, getLiveSessionLogs, getLiveSessionSamples, } from '../data/pcs-testing.ts';
import { getLiveSessionRecordById, listLiveProductLinesBySession, listLiveSessionRecords, } from '../data/pcs-live-testing-repository.ts';
import { findProjectChannelProductByLiveLine, } from '../data/pcs-channel-product-project-repository.ts';
import { findProjectByCode, getProjectById, listProjects, } from '../data/pcs-project-repository.ts';
import { getProjectRelationProjectLabel, listLiveProductLineProjectRelationCandidates, listProjectRelationsByLiveProductLine, replaceLiveProductLineProjectRelations, } from '../data/pcs-project-relation-repository.ts';
...
const STATUS_OPTIONS = ['all', 'DRAFT', 'RECONCILING', 'COMPLETED', 'CANCELLED'];
const ACCOUNTING_OPTIONS = ['all', 'NONE', 'PENDING', 'ACCOUNTED'];
const ITEM_INTENT_META = {
...
function getStatusCodeByLabel(label) {
    const matched = Object.entries(SESSION_STATUS_META).find(([, meta]) => meta.label === label);
    return matched?.[0] ?? 'DRAFT';
}
function getAccountingCodeByValue(value) {
    if (value === 'PENDING' || value === 'ACCOUNTED' || value === 'NONE')
        return value;
    const matched = Object.entries(ACCOUNTING_STATUS_META).find(([, meta]) => meta.label === value);
...
function buildSegmentTime(dateTime, offsetMinutes) {
...
function buildRecommendation(intent, orderQty, clickQty, gmvAmount) {
    if (intent !== 'TEST') {
...
function inferTestingProjectIdentity(item) {
    if (item.intent !== 'TEST')
        return null;
    ...
    const projectNamePart = searchText.includes('SPU-A001') || searchText.includes('印尼风格碎花连衣裙')
        ? '印尼风格碎花连衣裙测款项目'
...
function getWorkItemStatusLabel(status) {
...
function getLiveWorkItemSnapshot(session) {
    const actionItem = session.items.find((item) => item.intent === 'TEST') ?? session.items[0] ?? null;
    const linkedChannelProduct = actionItem?.relatedProjectId && actionItem.liveLineId
        ? findProjectChannelProductByLiveLine(actionItem.relatedProjectId, actionItem.liveLineId)
        : null;
    const mainImageUrl = linkedChannelProduct?.listingImages.find((item) => item.imageId === linkedChannelProduct.listingMainImageId)?.imageUrl ||
...
function buildSessionItemsFromBase(sessionId, baseItems, startedAt) {
...
function buildSessionItemsFromLines(sessionId, startedAt, testItemCount) {
    return listLiveProductLinesBySession(sessionId).map((line, index) => {
        ...
        const primaryRelation = relations[0] ?? null;
        const intent = index < testItemCount ? 'TEST' : 'SELL';
        const inferredProject = inferTestingProjectIdentity({
            intent,
...
function buildSessionFromRepository(sessionId) {
...
function inferSite(channelName) {
    if (channelName.includes('雅加达') || channelName.includes('ID') || channelName.includes('TikTok / 商品中心直播间'))
        return '雅加达';
    if (channelName.includes('MY'))
        return '吉隆坡';
    if (channelName.includes('VN'))
        return '胡志明';
    return '深圳';
}
...
function appendSessionLog(session, log) {
    return {
        ...session,
        updatedAt: log.time,
...
function syncCreateDrawerStateFromQuery() {
    const routeKey = appStore.getState().pathname;
    if (state.createRouteKey === routeKey)
        return;
...
function syncDetailState(sessionId) {
    const routeKey = appStore.getState().pathname;
    if (state.detail.routeKey === routeKey && state.detail.sessionId === sessionId)
        return;
...
function getFilteredSessions() {
    ...
    return getSessions().filter((session) => {
        ...
        if (!matchesKeyword)
            return false;
        if (state.list.status !== 'all' && session.status !== state.list.status)
            return false;
        if (state.list.purpose !== 'all' && !session.purposes.includes(state.list.purpose))
            return false;
        if (state.list.accounting !== 'all' && session.testAccountingStatus !== state.list.accounting)
            return false;
        if (state.list.quickFilter === 'reconciling' && session.status !== 'RECONCILING')
            return false;
        if (state.list.quickFilter === 'readyToClose' && !(session.status === 'RECONCILING' && session.endAt))
            return false;
        if (state.list.quickFilter === 'pendingAccounting' && session.testAccountingStatus !== 'PENDING')
            return false;
        if (state.list.quickFilter === 'accounted' && session.testAccountingStatus !== 'ACCOUNTED')
            return false;
        if (state.list.quickFilter === 'abnormal' && !(!session.endAt && session.status !== 'DRAFT' && session.status !== 'CANCELLED'))
            return false;
        return true;
    });
}
...
function renderDrawerShell(title, body, closeAction) {
    return `
    <div class="fixed inset-0 z-50">
...
function findProjectByHints(hints) {
    ...
    const exact = projects.find((project) => normalizedHints.some((hint) => [project.projectCode, project.projectName, project.styleCodeName, project.styleNumber, project.linkedStyleCode || '']
        .map((value) => value.trim().toLowerCase())
        .includes(hint)));
    if (exact) {
        return { projectId: exact.projectId, label: exact.projectName };
    }
    const fuzzy = projects.find((project) => normalizedHints.some((hint) => [project.projectCode, project.projectName, project.styleCodeName, project.styleNumber, project.linkedStyleCode || '']
        .some((value) => {
...
function getPrimaryProject(session) {
...
function isDisplayableSession(session) {
    return Boolean(getPrimaryProject(session) &&
        session.endAt &&
...
function renderPager(totalPages) {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 px-4 py-3 text-xs text-slate-500">
      ...
      <div class="flex items-center gap-1">
        ...
        ${visiblePages
        .map((page) => `<button type="button" class="${toClassName('inline-flex h-8 min-w-8 items-center justify-center rounded-md border px-2 text-xs', page === state.list.currentPage ? 'border-blue-600 bg-blue-600 text-white' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50')}" data-pcs-live-testing-action="set-page" data-page="${page}">${page}</button>`)
        .join('')}
...
function renderKpis() {
    ...
    <section class="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
      ${cards
        .map((card) => `
            <button type="button" class="${toClassName('rounded-lg border p-4 text-left transition hover:shadow-sm', card.tone, state.list.quickFilter === card.key ? 'ring-2 ring-blue-500' : '')}" data-pcs-live-testing-action="set-quick-filter" data-value="${card.key}">
...
function renderListPageDialogs() {
    return renderCreateDrawer();
}
function renderSessionDetailHeader(session, relatedProjects, mode = 'page') {
...
function renderOverviewTab(session, testItems) {
    const checks = [
...
function renderItemsTab(session) {
    return `
    <div class="space-y-4">
...
function renderEvidenceTab(session) {
...
function renderAccountingTab(session, testItems) {
    return `
    <div class="space-y-4">
      <section class="rounded-lg border bg-white">
...
function renderLogsTab(session) {
    return `
    <div class="space-y-4">
      <section class="rounded-lg border bg-white p-4">
        <div class="mb-4">
...
function renderDetailSidebar(session, relatedProjects) {
    ...
    <div class="space-y-4">
      ...
      <section class="rounded-lg border bg-white p-4">
        ...
        .map((row) => `<div class="flex justify-between gap-3"><span class="text-slate-500">${escapeHtml(row.label)}</span><span class="max-w-[150px] text-right text-slate-900">${escapeHtml(row.value)}</span></div>`)
        .join('')}
        </div>
        <div class="mt-4">
          <button
...
function renderDetailContent(session, mode = 'page') {
    const testItems = session.items.filter((item) => item.intent === 'TEST');
    const relatedProjects = Array.from(new Map(session.items
        .filter((item) => item.relatedProjectId && item.projectRef)
        .map((item) => [item.relatedProjectId, { projectId: item.relatedProjectId, label: getProjectRelationProjectLabel(item.relatedProjectId) }])).values());
    let activeContent = renderOverviewTab(session, testItems);
    if (state.detail.activeTab === 'items')
        activeContent = renderItemsTab(session);
    if (state.detail.activeTab === 'reconcile')
        activeContent = renderReconcileTab();
    if (state.detail.activeTab === 'evidence')
        activeContent = renderEvidenceTab(session);
    if (state.detail.activeTab === 'accounting')
        activeContent = renderAccountingTab(session, testItems);
    if (state.detail.activeTab === 'logs')
        activeContent = renderLogsTab(session);
    return `
        ...
        <div class="space-y-4">
          <section class="rounded-lg border bg-white p-2">
            <div class="grid gap-2 md:grid-cols-3 xl:grid-cols-7">
              ${DETAIL_TAB_OPTIONS
        .map((tab) => `
                    <button type="button" class="${toClassName('inline-flex h-10 items-center justify-center rounded-md px-3 text-sm transition', state.detail.activeTab === tab.key ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200')}" data-pcs-live-testing-action="set-detail-tab" data-tab="${tab.key}">${escapeHtml(tab.label)}</button>
                  `)
        .join('')}
            </div>
          </section>
...
function renderCloseDialog() {
    ...
    return renderModalShell('完成直播测款（关账）', `确认完成「${session.title}」的关账动作，状态将流转为“已关账”。`, `
      ...
      <section class="space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button type="button" class="${toClassName('inline-flex h-9 items-center rounded-md border px-3 text-sm', state.closeDialog.completionType === 'normal' ? 'border-blue-600 bg-blue-600 text-white' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50')}" data-pcs-live-testing-action="set-completion-type" data-value="normal">正常完成</button>
          <button type="button" class="${toClassName('inline-flex h-9 items-center rounded-md border px-3 text-sm', state.closeDialog.completionType === 'abnormal' ? 'border-amber-600 bg-amber-500 text-white' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50')}" data-pcs-live-testing-action="set-completion-type" data-value="abnormal">异常完成</button>
        </div>
...
function renderAccountingDialog() {
    if (!state.accountingDialog.open)
        return '';
    ...
    return renderModalShell('完成测款核对（入账）', `确认对「${session.title}」的 TEST 行完成核对并回写入账结果。`, `
      <section class="space-y-3">
      ...
      <section class="space-y-3">
        ...
        <div class="rounded-lg bg-blue-50 p-4 text-sm text-blue-800">
          ...
          <ul class="mt-2 list-disc space-y-1 pl-5">
            ${testItems
        .map((item) => `<li>${escapeHtml(item.projectRef || item.productRef || item.productName)}：追加 1 条直播证据与结果建议。</li>`)
        .join('')}
          </ul>
        </div>
      </section>
...
function renderEditDrawer() {
    ...
    return renderDrawerShell('编辑明细行', `
      <div class="space-y-6">
        <section class="space-y-3">
          ...
          <div class="grid gap-4 md:grid-cols-2">
            <label class="space-y-1">
              ...
              <select class="h-10 w-full rounded-md border border-slate-200 px-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" data-pcs-live-testing-field="edit-intent">
                ${['SELL', 'TEST', 'REVIEW'].map((intent) => `<option value="${intent}" ${draft.intent === intent ? 'selected' : ''}>${escapeHtml(`${ITEM_INTENT_META[intent].label} (${intent})`)}</option>`).join('')}
              </select>
            </label>
            <label class="space-y-1">
...
export function renderPcsLiveTestingListPage() {
    ensureSessionStore();
...
export function renderPcsLiveTestingDetailPage(sessionId) {
    ensureSessionStore();
    syncDetailState(sessionId);
    return renderPcsLiveTestingListPage();
}
export function handlePcsLiveTestingInput(target) {
    const fieldNode = target.closest('[data-pcs-live-testing-field]');
    if (!fieldNode)
        return false;
    ...
    if (textFields.has(field) && fieldNode instanceof HTMLInputElement) {
        const key = field.replace(/^create-/, '').replaceAll('-', '');
        if (field === 'create-project-ref')
            state.createDraft.projectRef = fieldNode.value;
        if (field === 'create-title')
            state.createDraft.title = fieldNode.value;
        if (field === 'create-live-account')
            state.createDraft.liveAccount = fieldNode.value;
        if (field === 'create-anchor')
            state.createDraft.anchor = fieldNode.value;
        if (field === 'create-start-at')
            state.createDraft.startAt = fieldNode.value;
        if (field === 'create-end-at')
            state.createDraft.endAt = fieldNode.value;
        if (field === 'create-exposure')
            state.createDraft.exposure = fieldNode.value;
        if (field === 'create-click')
            state.createDraft.click = fieldNode.value;
        if (field === 'create-cart')
            state.createDraft.cart = fieldNode.value;
        if (field === 'create-order')
            state.createDraft.order = fieldNode.value;
        if (field === 'create-gmv')
            state.createDraft.gmv = fieldNode.value;
        void key;
        return true;
    }
    ...
    if (field === 'edit-intent' && fieldNode instanceof HTMLSelectElement) {
        state.editDrawer.draft.intent = fieldNode.value;
        return true;
    }
...
export function handlePcsLiveTestingEvent(target) {
    const actionNode = target.closest('[data-pcs-live-testing-action]');
    if (!actionNode)
        return false;
    ...
    if (action === 'set-quick-filter') {
        const value = actionNode.dataset.value || 'all';
    ...
    if (action === 'confirm-close-session') {
        ...
        if (!sessionId)
            return true;
        updateSession(sessionId, (session) => appendSessionLog({
            ...session,
            status: 'COMPLETED',
    ...
    if (action === 'save-edit-item') {
        ...
        const testLineIds = Array.from(new Set(session.items
            .filter((item) => item.intent === 'TEST' && item.liveLineId)
            .map((item) => item.liveLineId)));
        let notice = '明细行已保存。';
        if (shouldSyncProject) {
            if (!draft.projectRef.trim()) {
                ...
                const candidate = currentItem.liveLineId
                    ? listLiveProductLineProjectRelationCandidates(currentItem.liveLineId).find((item) => item.projectId === resolvedProject.projectId)
                    : null;
                if (candidate && !candidate.eligible) {
                    notice = `明细行已保存；直播测款正式关联未回写：${candidate.disabledReason || '当前项目不满足关联条件。'}`;
                }
                else {
                    const errors = testLineIds.flatMap((lineId) => replaceLiveProductLineProjectRelations(lineId, [resolvedProject.projectId], '当前用户').errors);
        ...
        updateSession(sessionId, (session) => {
            const items = session.items.map((item) => item.id === itemId
                ? {
                    ...item,
                    intent: draft.intent,
                    projectRef: shouldSyncProject
                        ? resolvedProject?.projectCode ?? (draft.projectRef.trim() || null)
                        : resolvedProject?.projectCode ?? (draft.projectRef.trim() || null),
                    relatedProjectId: shouldSyncProject ? resolvedProject?.projectId ?? null : resolvedProject?.projectId ?? null,
...
export function isPcsLiveTestingDialogOpen() {
    return Boolean(state.createDrawerOpen ||
