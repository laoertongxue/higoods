...
import { getCuttingMainlineSession, listCuttingMainlineSessions, } from '../data/fcs/cutting/cutting-mainline.ts';
...
function toneForSession(session) {
    if (session.statusTab === 'DONE')
        return 'green';
    if (session.statusTab === 'IN_PROGRESS')
        return 'blue';
    if (session.statusTab === 'BLOCKED')
        return 'red';
    return 'amber';
}
...
function renderStep(label, status, action, enabled = true) {
    return `
...
function getActualTimeText(session) {
    if (session.actualStartAt && session.actualEndAt)
        return `${session.actualStartAt} 至 ${session.actualEndAt}`;
    if (session.actualStartAt)
        return `${session.actualStartAt} 起`;
    return '未开始';
}
function getSession(taskId, sessionId) {
...
export function renderPdaCuttingExecutionUnitPage(taskId, executionOrderId) {
    const session = getSession(taskId, executionOrderId);
    if (!session) {
        return renderPdaFrame(`<section class="space-y-3 px-3 py-3"><button class="rounded-lg border px-2.5 py-1.5 text-sm" data-nav="/fcs/pda/exec">返回</button><div class="rounded-2xl border border-dashed px-3 py-8 text-center text-sm">未找到铺布现场任务</div></section>`, 'exec', { disableTodoAutoOpen: true });
    }
...
export function handlePdaCuttingExecutionUnitEvent(target) {
    const button = target.closest('[data-pda-cutting-unit-action]');
    if (!button)
        return false;
    const action = button.dataset.pdaCuttingUnitAction || '现场动作';
    const feedback = document.querySelector('[data-pda-cutting-action-feedback]');
    if (feedback) {
