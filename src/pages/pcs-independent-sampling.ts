...
function renderDialogHost(): string {
  ...
  return `<div data-independent-sampling-dialogs data-skip-page-rerender="true">${ui.createType ? renderCreateDialog(ui.createType) : ''}${renderPreview()}</div>`
}

function refreshDialogs(): void {
  if (typeof document === 'undefined') return
  document.querySelectorAll<HTMLElement>('[data-independent-sampling-dialogs]').forEach((host) => {
    host.innerHTML = `${ui.createType ? renderCreateDialog(ui.createType) : ''}${renderPreview()}`
  })
}
...
function listColumns(type: EngineeringIndependentSamplingType): StandardListColumn<EngineeringIndependentSamplingRecord>[] {
  return [
    ...
    { key: 'bom', title: 'BOM 与价格', width: 170, render: (row) => `<a class="text-blue-700 hover:underline" href="/pcs/technical-data/bom-pricing/${escapeHtml(row.bomDraftVersionId)}">${row.bomVersionIds.length} 个颜色版本</a>` },
...
export function handlePcsIndependentSamplingEvent(target: HTMLElement): boolean {
  ...
  if (action === 'create') { const type = node.dataset.samplingType as EngineeringIndependentSamplingType; run(() => { const created = createEngineeringIndependentSampling({ samplingType: type, sourceStyleId: type === 'REVISION' ? value('sourceStyleId') : undefined, targetStyleId: value('targetStyleId'), merchandiser: CURRENT_PCS_ENGINEERING_USER, selectedTaskTypes: checkedTaskTypes('taskType'), createdAt: new Date().toISOString().replace('T', ' ').slice(0, 19) }); ui.createType = ''; window.location.href = `/pcs/engineering/${type === 'REVISION' ? 'revision' : 'design'}-sampling/${created.samplingTaskId}` }, '任务已创建。'); return true }
