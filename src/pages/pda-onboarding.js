...
import { FACTORY_ONBOARDING_NODE_OPTIONS, normalizeReviewResult, } from '../data/fcs/factory-onboarding-domain.ts';
import { activateOnboardingSession, applyMachineValidation, calculateOnboardingCompleteness, canEditOnboardingApplication, canSubmitOnboardingApplication, createCapabilityFromSelection, createDefaultMachineDraft, createDefaultOnboardingDraft, getCompletenessMissingItems, getFactoryOnboardingCurrentNodeSummary, getPrimaryFactoryType, inferFactoryTypesFromCapabilities, getLatestReviewRecord, getLatestSupplementRecord, getOnboardingStatusActionLabel, getPdaOnboardingApplicationFromSession, listSelectableProcessCraftOptions, logoutPdaAccess, saveFactoryOnboardingDraft, submitFactoryOnboardingApplication, } from '../data/fcs/factory-onboarding-flow.ts';
...
import { normalizeSampleReviewResult } from '../data/fcs/factory-sample-verification-domain.ts';
import { confirmFactoryReceivedSample, submitFactorySampleReview, } from '../data/fcs/factory-sample-verification-flow.ts';
...
function hydrateDraftFromApplication(application) {
    if (!application) {
...
function syncMachineValidation() {
...
function syncPageState() {
    ...
    const application = (queryApplicationId ? getFactoryOnboardingApplicationById(queryApplicationId) : null)
        || sessionApplication
        || runtimeApplication
        || null;
...
function renderNodeStatusChip(label, tone) {
    const className = tone === 'done'
...
function getPdaSampleStatusLabel(status) {
...
function buildNodeSnapshot(application, nodeName) {
...
function renderFlowCard(application) {
    return `
...
function renderCurrentStatusCard(application) {
    if (!application) {
        return `
...
function renderPdaReferenceFiles(files, emptyText) {
    if (files.length <= 0)
...
function renderFactorySampleSubmittedInfo(sampleVerification) {
    if (!sampleVerification?.factorySubmittedAt)
        return '';
    return `
...
function renderReceiveInfo(sampleVerification) {
    if (!sampleVerification?.factoryReceivedAt)
        return '';
    return `
...
function renderFactorySampleSubmitForm(isResubmit = false) {
...
function renderSampleVerificationCard(application) {
...
function renderSampleReceiveDialog(application) {
    if (!state.sampleReceiveVerificationId)
        return '';
    const sampleVerification = getSampleVerificationForApplication(application);
    if (!sampleVerification || sampleVerification.verificationId !== state.sampleReceiveVerificationId)
        return '';
...
function getFactoryTypeLabel(code) {
    const map = {
        CUTTING_FACTORY: '裁床厂',
...
function renderCompletenessCard(application) {
    ...
    const toneClass = completeness.completenessScore >= 95
        ? 'border-emerald-200 bg-emerald-50 text-emerald-700'
...
function renderField(label, inputHtml, required = false) {
    return `
    <label class="block space-y-1.5">
...
function renderCapabilityTags(capabilities, readonly) {
...
function renderCapabilityPicker(readonly) {
...
function getSelectedCraftOptions(processCode) {
    return state.draft.selectedCapabilities.filter((item) => item.processCode === processCode);
}
function renderMachineTable(readonly) {
...
function renderMachineRow(machine, index, readonly) {
...
function getDraftFieldValue(field) {
    switch (field) {
        case '工厂简称':
...
function renderRecords(application) {
    if (!application)
        return '';
    return `
    <section class="rounded-[28px] border border-slate-200 bg-white p-3 shadow-sm">
...
function renderActionArea(application) {
...
function renderMobileFlowSection(application) {
    const steps = [
    ...
    const currentNodeLog = application?.nodeLogs.find((item) => item.nodeName === currentNode) ||
    ...
    const activeStep = steps.find((item) => item.nodeNames.includes(currentNode)) || steps[0];
    return `
    <section class="rounded-[28px] border border-slate-200 bg-white p-3 shadow-sm" data-testid="pda-onboarding-flow-card">
      ...
      <div class="-mx-3 mt-3 overflow-x-auto px-3 pb-1">
        <div class="flex min-w-max gap-2">
          ${steps.map((step) => {
        const relatedLogs = application?.nodeLogs.filter((item) => step.nodeNames.includes(item.nodeName)) || [];
        const nodeLog = relatedLogs.find((item) => item.nodeStatus === '进行中') || relatedLogs[relatedLogs.length - 1] || null;
        const isActive = step.nodeNames.includes(currentNode);
...
function renderOnboardingBody(application) {
    const readonly = !canEditOnboardingApplication(application);
    return `
...
export function renderPdaOnboardingPage() {
...
function toggleCapability(processCode, craftCode) {
...
function updateMachineDraft(index, field, value) {
...
function updateDraftField(field, value) {
    ...
    if (field.startsWith('admin-')) {
        ...
        state.draft.adminAccount[adminField] = value;
        return;
    }
    if (field === 'effectiveWorkerCount' || field === 'machineTotalCount') {
        ;
        state.draft[field] = Number(value) || 0;
        return;
    }
    ;
...
function updateIdentityFileFromInput(input) {
...
function updateSampleFilesFromInput(input, field) {
    const files = buildReferenceFilesFromInput(input);
    if (field === 'factorySamplePhotos')
        state.sampleSubmissionDraft.factorySamplePhotos = files;
    if (field === 'factorySampleVideos')
        state.sampleSubmissionDraft.factorySampleVideos = files;
    if (field === 'factorySubmissionFiles')
        state.sampleSubmissionDraft.factorySubmissionFiles = files;
    if (field === 'factorySitePhotos')
        state.sampleSubmissionDraft.factorySitePhotos = files;
    if (field === 'factorySiteVideos')
        state.sampleSubmissionDraft.factorySiteVideos = files;
    if (field === 'bossIdentityFiles')
        state.sampleSubmissionDraft.bossIdentityFiles = files;
    state.sampleSubmissionErrorText = '';
}
function updateSampleReceiveField(field, value) {
    if (!field.startsWith('sampleReceive-'))
        return false;
    const key = field.replace('sampleReceive-', '');
...
function updateSampleSubmissionField(field, value) {
    if (!field.startsWith('sampleSubmission-'))
        return false;
    const key = field.replace('sampleSubmission-', '');
    state.sampleSubmissionDraft = {
        ...state.sampleSubmissionDraft,
        [key]: value,
    };
...
function openSampleReceiveDialog(verificationId) {
    const application = syncPageState();
    const sampleVerification = getSampleVerificationForApplication(application);
...
async function handleConfirmSampleReceive() {
    state.sampleReceiveErrorText = '';
    state.errorText = '';
    state.successText = '';
    try {
        const sampleVerification = getCurrentSampleVerification();
        const result = confirmFactoryReceivedSample(state.sampleReceiveVerificationId || sampleVerification?.verificationId || '', state.sampleReceiveDraft, state.sampleReceiveDraft.factoryReceivedBy || '工厂管理员');
...
async function handleSubmitFactorySampleReview() {
    state.sampleSubmissionErrorText = '';
    state.errorText = '';
    state.successText = '';
    try {
        ...
        const result = submitFactorySampleReview(sampleVerification.verificationId, state.sampleSubmissionDraft, application?.adminAccount.adminName || application?.applicantName || '工厂管理员');
        hydrateDraftFromApplication(result.application);
...
export async function handlePdaOnboardingEvent(target) {
    const fileNode = target.closest('[data-pda-onboarding-file]');
    ...
    const fieldNode = target.closest('[data-pda-onboarding-field]');
    if (fieldNode instanceof HTMLInputElement || fieldNode instanceof HTMLTextAreaElement || fieldNode instanceof HTMLSelectElement) {
        ...
        if (updateSampleReceiveField(field, fieldNode.value))
            return true;
        if (updateSampleSubmissionField(field, fieldNode.value))
            return true;
        updateDraftField(field, fieldNode.value);
        return true;
    }
    const machineFieldNode = target.closest('[data-pda-onboarding-machine-field]');
    if (machineFieldNode instanceof HTMLInputElement || machineFieldNode instanceof HTMLTextAreaElement || machineFieldNode instanceof HTMLSelectElement) {
        ...
        const field = machineFieldNode.dataset.pdaOnboardingMachineField;
        if (index >= 0 && field) {
            updateMachineDraft(index, field, machineFieldNode.value);
            return true;
        }
    }
    const actionNode = target.closest('[data-pda-onboarding-action]');
    ...
    if (action === 'remove-machine') {
        ...
        if (index >= 0)
            state.draft.machines.splice(index, 1);
        syncMachineValidation();
        return true;
    }
