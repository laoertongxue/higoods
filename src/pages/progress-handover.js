...
import { findPdaHandoverRecord, followupPdaHandoverObjection, getPdaCompletedHeads, getPdaHandoutHeads, getPdaPickupRecordsByHead, getPdaPickupHeads, markPdaHandoutHeadCompleted, markPdaPickupHeadCompleted, resolvePdaHandoverObjection, writeBackHandoverRecord, } from '../data/fcs/pda-handover-events';
import { getRecordReceiverWrittenAt, getRecordReceiverWrittenQty, getReceiverDisplayName, } from '../data/fcs/task-handover-domain';
import { buildHandoverOrderDetailLink, getHandoverOrderTimelineViews, getHandoverLedgerRows, getProductionOrderHandoverSummary, getHandoverPreviewStats, } from '../data/fcs/handover-ledger-view';
import { buildHandoverOrderLink, buildHandoverQrLabelPrintLink, buildProductionOrderLink, buildQualityRecordLink, buildTaskDeliveryCardPrintLink, buildTaskDetailLink, } from '../data/fcs/fcs-route-links.ts';
import { getPostExecutionModeLabel, getPostProcessRouteByProductionOrderId, getPostRouteNodeLabel, } from '../data/fcs/post-process-route';
...
const LEDGER_EVENT_FILTER_OPTIONS = [
...
const LEDGER_STATUS_FILTER_OPTIONS = [
...
const ORDER_BOTTLENECK_FILTER_OPTIONS = [
...
function isHandoverFocus(value) {
...
function getFocusLabel(focus) {
    ...
    if (focus === 'handout')
        return '待交出';
    if (focus === 'warehouse-confirm')
        return '待接收方确认';
    return '异议处理';
}
function showProgressHandoverToast(message, tone = 'success') {
...
function openLinkedPage(title, href) {
    appStore.openTab({
        key: href,
        title,
        href,
...
function getLedgerRowById(rowId) {
    return getLedgerRows().find((row) => row.rowId === rowId);
}
function findOrderIdByTaskId(taskId) {
    if (!taskId)
        return '';
...
function matchEventTypeFilter(row, filter) {
    if (filter === 'ALL')
        return true;
    if (filter === 'PICKUP') {
...
function normalizeStatusFilter(value) {
    if (!value)
        return 'ALL';
    if (isLedgerStatusFilter(value))
        return value;
    return 'ALL';
}
function matchStatusFilter(row, filter) {
    if (filter === 'ALL')
        return true;
    if (filter === 'DONE')
        return row.statusGroup === 'DONE';
...
function hasDiffOrObjection(row) {
    if (typeof row.qtyDiff === 'number' && row.qtyDiff !== 0)
        return true;
    return (row.statusCode === 'HANDOUT_HAS_OBJECTION' ||
...
function syncFromQuery() {
    ...
    if (!eventType && state.focusHint && !(isOrdersTab && (po || taskId || focus))) {
        if (state.focusHint === 'pickup')
            state.filterEventType = 'PICKUP';
        if (state.focusHint === 'handout')
            state.filterEventType = 'HANDOUT';
        if (state.focusHint === 'warehouse-confirm') {
            state.filterEventType = 'WAREHOUSE';
            if (state.filterStatus === 'ALL')
                state.filterStatus = 'PENDING_WAREHOUSE';
        }
        if (state.focusHint === 'objection') {
            state.filterEventType = 'OBJECTION';
            if (state.filterStatus === 'ALL')
                state.filterStatus = 'HAS_OBJECTION';
        }
    }
    state.showUrlFilterBanner = Boolean(po ||
        process ||
        taskId ||
        eventType ||
        status ||
...
function getFilteredRows(rows) {
    return rows.filter((row) => {
        ...
        if (state.filterPo && row.productionOrderId !== state.filterPo)
            return false;
        if (state.filterProcess && row.processName !== state.filterProcess)
            return false;
        if (state.filterTaskId && row.taskId !== state.filterTaskId)
            return false;
        if (!matchEventTypeFilter(row, state.filterEventType))
            return false;
        if (!matchStatusFilter(row, state.filterStatus))
            return false;
        if (state.filterHasDiff === 'YES' && !hasDiffOrObjection(row))
            return false;
        if (state.filterHasDiff === 'NO' && hasDiffOrObjection(row))
            return false;
        return true;
    });
}
function getTimelineBaseRows(rows) {
    return rows.filter((row) => {
        if (state.filterPo && row.productionOrderId !== state.filterPo)
            return false;
        if (state.filterTaskId && row.taskId !== state.filterTaskId)
...
function findPdaHeadByHandoverId(handoverId) {
    return [...getPdaPickupHeads(), ...getPdaHandoutHeads(), ...getPdaCompletedHeads()].find((head) => head.handoverId === handoverId);
}
function renderSourceFacts(row) {
    ...
    if (row.sourceType === 'PICKUP_RECORD' && row.recordId) {
      ...
      <div class="space-y-1 text-sm">
        ...
        <p><span class="text-muted-foreground">实际数量：</span>${typeof pickupRecord.qtyActual === 'number'
    ...
    if (row.sourceType === 'HANDOUT_RECORD' && row.recordId) {
      ...
      <div class="space-y-1 text-sm">
        ...
        <p><span class="text-muted-foreground">接收方确认：</span>${typeof getRecordReceiverWrittenQty(handoutRecord) === 'number'
            ? `${getRecordReceiverWrittenQty(handoutRecord)} ${escapeHtml(head.qtyUnit)}`
            : '待接收方确认'}</p>
        <p><span class="text-muted-foreground">异议状态：</span>${handoutRecord.status === 'OBJECTION_REPORTED'
            ? '已发起异议'
...
function renderCurrentStatusText(row) {
    if (row.statusCode === 'HANDOUT_RECORD_PENDING_WRITEBACK')
        return '接收方还未确认实收数量，先等待收货确认。';
    if (row.statusCode === 'HANDOUT_OBJECTION_REPORTED')
        return '工厂已发起异议，待平台跟进。';
    if (row.statusCode === 'HANDOUT_OBJECTION_PROCESSING')
        return '平台正在跟进异议，等待处理结论。';
    if (row.statusCode === 'HANDOUT_OBJECTION_RESOLVED')
        return '异议已处理完成，可继续关注是否完成。';
    if (row.statusCode === 'PICKUP_PENDING')
        return '当前任务还没开始领料，先推进领料。';
    if (row.statusCode === 'HANDOUT_PENDING')
        return '当前任务还没发起交出记录，先推进交出。';
    if (row.statusCode === 'HEAD_COMPLETED')
        return '当前交接已关闭，这条链路无需额外处理。';
    return '当前状态正常，按下一步建议继续跟进。';
}
function buildExceptionHrefByRow(row) {
...
function renderBadge(label, className, icon) {
    return `
...
function renderUrlBanner() {
    if (!state.showUrlFilterBanner)
        return '';
    return `
...
function renderEventsPreviewCards(rows) {
...
function renderEventsFilters(rows) {
    ...
    <section class="flex flex-wrap items-center gap-3 rounded-lg border bg-card p-4">
      ...
      <select class="h-9 w-[210px] rounded-md border bg-background px-3 text-sm" data-handover-field="filterPo">
        ...
        ${orderOptions
        .map((item) => `<option value="${escapeAttr(item)}" ${state.filterPo === item ? 'selected' : ''}>${escapeHtml(item)}</option>`)
        .join('')}
      </select>
      ...
      <select class="h-9 w-[170px] rounded-md border bg-background px-3 text-sm" data-handover-field="filterProcess">
        ...
        ${processOptions
        .map((item) => `<option value="${escapeAttr(item)}" ${state.filterProcess === item ? 'selected' : ''}>${escapeHtml(item)}</option>`)
        .join('')}
      </select>
...
function renderRowActionMenu(row) {
    ...
    const canHandleObjection = (row.statusCode === 'HANDOUT_OBJECTION_REPORTED' || row.statusCode === 'HANDOUT_OBJECTION_PROCESSING') &&
    ...
    let primaryAction = '';
    if (canWriteback) {
        primaryAction = `<button class="flex w-full items-center rounded px-2 py-1.5 text-left text-sm hover:bg-muted" data-handover-action="open-writeback-dialog" data-record-id="${escapeAttr(row.recordId || '')}"><i data-lucide="clipboard-check" class="mr-2 h-4 w-4"></i>接收方确认</button>`;
    }
    else if (canHandleObjection) {
        primaryAction = `<button class="flex w-full items-center rounded px-2 py-1.5 text-left text-sm hover:bg-muted" data-handover-action="open-objection-dialog" data-record-id="${escapeAttr(row.recordId || '')}"><i data-lucide="message-square" class="mr-2 h-4 w-4"></i>处理异议</button>`;
    }
    else if (canMarkPickupComplete) {
        primaryAction = `<button class="flex w-full items-center rounded px-2 py-1.5 text-left text-sm hover:bg-muted" data-handover-action="mark-pickup-complete" data-handover-id="${escapeAttr(row.handoverId)}"><i data-lucide="check-check" class="mr-2 h-4 w-4"></i>标记领料完成</button>`;
    }
    else if (canMarkHandoutComplete) {
        primaryAction = `<button class="flex w-full items-center rounded px-2 py-1.5 text-left text-sm hover:bg-muted" data-handover-action="mark-handout-complete" data-handover-id="${escapeAttr(row.handoverId)}"><i data-lucide="check-check" class="mr-2 h-4 w-4"></i>标记交出完成</button>`;
    }
    return `
...
function renderEventsDimension(rows, allRows) {
    return `
    <div class="space-y-4">
...
function getOrdersPreviewStats(views) {
    return {
...
function renderOrdersPreviewCards(views) {
...
function getOrderProcessFilterOptions(views) {
...
function getFilteredOrderViews(views, rows) {
    return views.filter((view) => {
        ...
        if (state.ordersObjectionFilter === 'YES' && view.objectionCount <= 0)
            return false;
        if (state.ordersObjectionFilter === 'NO' && view.objectionCount > 0)
            return false;
...
function renderOrdersFilters(views) {
    ...
    <section class="flex flex-wrap items-center gap-3 rounded-lg border bg-card p-4">
      ...
      <select class="h-9 w-[170px] rounded-md border bg-background px-3 text-sm" data-handover-field="ordersProcessFilter">
        <option value="">工序工艺</option>
        ${processOptions
        .map((item) => `<option value="${escapeAttr(item)}" ${state.ordersProcessFilter === item ? 'selected' : ''}>${escapeHtml(item)}</option>`)
        .join('')}
      </select>
...
function renderOrdersDimension(rows) {
...
function renderTimelineProcessStatusBadge(section) {
    const className = section.processStatusTone === 'success'
    ...
    return `<span class="inline-flex items-center rounded border px-2 py-0.5 text-xs ${className}">${escapeHtml(section.processStatusLabel)}</span>`;
}
function buildDefaultExpandedOrderIds(views) {
...
function ensureTimelineExpandedState(views) {
...
function isTimelineSectionFocused(view, section) {
    if (state.timelineAnchorOrderId && state.timelineAnchorOrderId !== view.productionOrderId)
        return false;
    ...
    if (!focus)
        return false;
    if (focus === 'pickup')
        return section.processStatusLabel === '待领料';
    if (focus === 'handout')
        return section.processStatusLabel === '待交出' || section.processStatusLabel === '已领料待交出';
    if (focus === 'warehouse-confirm')
        return section.processStatusLabel === '待接收方确认';
    if (focus === 'objection')
        return section.processStatusLabel === '有异议' || section.processStatusLabel === '异议处理中';
    return false;
}
function renderTimelineSection(view, section) {
...
function renderTimelineOrderCard(view) {
...
function scheduleTimelineAnchorScroll() {
...
function getReceivingQcHref(productionOrderId) {
    const row = getQcViewRows().find((item) => item.productionOrderId === productionOrderId && item.qc.inspectionScene !== 'POST_FINAL_RECHECK');
    return row ? buildQualityRecordLink(row.qcId) : '';
}
function getFinalRecheckHref(productionOrderId) {
    const row = getQcViewRows().find((item) => item.productionOrderId === productionOrderId && item.qc.inspectionScene === 'POST_FINAL_RECHECK');
    return row ? buildQualityRecordLink(row.qcId) : '';
}
function resolveRouteStageLabel(params) {
    ...
    if (stage === 'receivingQc') {
        if (currentNode === 'WAIT_RECEIVING_QC')
            return '待回货质检';
        if (currentNode === 'WAIT_SEW_HANDOVER' || currentNode === 'WAIT_RECEIVER_WRITEBACK')
            return '未开始';
        return '已完成';
    }
    if (stage === 'postExecution') {
        if (!requiresPostExecution)
            return '车缝厂已含后道';
        if (currentNode === 'WAIT_POST_EXECUTION')
            return '待后道';
    ...
    if (stage === 'finalRecheck') {
        if (currentNode === 'WAIT_FINAL_RECHECK')
            return '待复检';
        if (currentNode === 'WAIT_WAREHOUSE_HANDOVER' || currentNode === 'CLOSED')
            return '已完成';
        return '未开始';
    }
    if (currentNode === 'WAIT_WAREHOUSE_HANDOVER')
        return '待交成衣仓';
    if (currentNode === 'CLOSED')
        return '已关闭';
    return '未开始';
}
function renderPostRouteSummary(orderId) {
    const route = getPostProcessRouteByProductionOrderId(orderId);
    if (!route)
        return '<span class="text-xs text-muted-foreground">后道路由：未配置</span>';
    const receivingQcHref = getReceivingQcHref(orderId);
    const finalRecheckHref = getFinalRecheckHref(orderId);
    const modeLabel = getPostExecutionModeLabel(route.postExecutionMode);
    const currentNodeLabel = getPostRouteNodeLabel(route.currentNode);
    return `
    <div class="space-y-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-3">
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="font-medium text-foreground">后道路由：${escapeHtml(modeLabel)}</span>
        <span class="text-muted-foreground">接收方：${escapeHtml(route.managedPostFactoryName)}</span>
        <span class="text-muted-foreground">当前节点：${escapeHtml(currentNodeLabel)}</span>
      </div>
      <div class="grid gap-2 text-xs md:grid-cols-4">
        <div class="rounded-md border bg-white px-3 py-2">
          ...
          ${receivingQcHref
        ? `<button type="button" class="mt-2 inline-flex rounded-md border px-2 py-1 text-[11px] hover:bg-muted" data-handover-action="open-quality-record" data-title="质检记录" data-href="${escapeAttr(receivingQcHref)}">质检记录</button>`
        : ''}
        </div>
        <div class="rounded-md border bg-white px-3 py-2">
        ...
        <div class="rounded-md border bg-white px-3 py-2">
          ...
          ${finalRecheckHref
        ? `<button type="button" class="mt-2 inline-flex rounded-md border px-2 py-1 text-[11px] hover:bg-muted" data-handover-action="open-quality-record" data-title="复检记录" data-href="${escapeAttr(finalRecheckHref)}">复检记录</button>`
        : ''}
        </div>
        <div class="rounded-md border bg-white px-3 py-2">
...
function renderTimelineOrderSummaryCard(view, rows) {
    const latestEvent = getLatestOrderEvent(view.productionOrderId, rows);
    ...
    const focus = highlighted && state.timelineAnchorFocus
        ? state.timelineAnchorFocus
    ...
    >
      ...
      <td class="px-3 py-3 text-xs text-muted-foreground">
        <div class="space-y-2">
          <div>${escapeHtml(getOrderProcessSummary(view))}</div>
          ${renderPostRouteSummary(view.productionOrderId)}
        </div>
      </td>
      <td class="px-3 py-3 text-xs">
        <span class="inline-flex items-center rounded-md bg-muted px-2 py-1 text-muted-foreground">${escapeHtml(summary.primaryActionHint || view.currentBottleneckHint)}</span>
      </td>
...
function renderOrdersTable(views, rows) {
    return `
    <section class="overflow-hidden rounded-lg border bg-card">
      <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
...
function renderTimelineTab(rows) {
    const timelineViews = getHandoverOrderTimelineViews(rows);
    return `
    <section class="space-y-4">
...
function renderDetailDrawer(rows) {
    if (!state.detailRowId)
        return '';
    ...
    const canHandleObjection = (row.statusCode === 'HANDOUT_OBJECTION_REPORTED' || row.statusCode === 'HANDOUT_OBJECTION_PROCESSING') &&
        Boolean(row.recordId);
    const headType = row.sourceType === 'PICKUP_HEAD' ? 'PICKUP' : row.sourceType === 'HANDOUT_HEAD' ? 'HANDOUT' : null;
    const sourceTypeLabel = row.sourceType === 'PICKUP_HEAD'
        ? '领料单'
    ...
    const directActions = [];
    if (canWriteback) {
        directActions.push(`<button class="inline-flex h-8 items-center rounded-md border border-emerald-200 bg-emerald-50 px-3 text-sm text-emerald-700 hover:bg-emerald-100" data-handover-action="open-writeback-dialog" data-record-id="${escapeAttr(row.recordId || '')}">接收方确认</button>`);
    }
    if (canHandleObjection) {
        directActions.push(`<button class="inline-flex h-8 items-center rounded-md border border-red-200 bg-red-50 px-3 text-sm text-red-700 hover:bg-red-100" data-handover-action="open-objection-dialog" data-record-id="${escapeAttr(row.recordId || '')}">处理异议</button>`);
    }
    if (headType === 'PICKUP') {
        directActions.push(`<button class="inline-flex h-8 items-center rounded-md border border-emerald-200 bg-emerald-50 px-3 text-sm text-emerald-700 hover:bg-emerald-100" data-handover-action="mark-pickup-complete" data-handover-id="${escapeAttr(row.handoverId)}">标记领料完成</button>`);
    }
    if (headType === 'HANDOUT') {
        directActions.push(`<button class="inline-flex h-8 items-center rounded-md border border-emerald-200 bg-emerald-50 px-3 text-sm text-emerald-700 hover:bg-emerald-100" data-handover-action="mark-handout-complete" data-handover-id="${escapeAttr(row.handoverId)}">标记交出完成</button>`);
    }
    if (row.sourceType === 'HANDOUT_RECORD' && row.recordId) {
        directActions.push(`<button class="inline-flex h-8 items-center rounded-md border px-3 text-sm hover:bg-muted" data-handover-action="print-delivery-card" data-handover-id="${escapeAttr(row.handoverId)}" data-record-id="${escapeAttr(row.recordId)}">打印任务交货卡</button>`, `<button class="inline-flex h-8 items-center rounded-md border px-3 text-sm hover:bg-muted" data-handover-action="print-handover-qr" data-record-id="${escapeAttr(row.recordId)}">打印交出二维码</button>`);
    }
    return `
        ...
        <div class="space-y-5 px-5 py-5">
          ...
          <section class="space-y-3 border-t pt-4">
            ...
            <div class="space-y-1 text-sm">
              ...
              <p>交出单：${escapeHtml(row.handoverId)}</p>
        ...
        : ''}
            ...
            <div class="space-y-2">
              ...
              <div class="flex flex-wrap gap-2">
                <button class="inline-flex h-8 items-center rounded-md border px-3 text-sm hover:bg-muted" data-handover-action="open-pda-detail" data-handover-id="${escapeAttr(row.handoverId)}">查看交出详情</button>
...
function renderWritebackDialog() {
    if (!state.writebackRecordId)
        return '';
    ...
    <div class="fixed inset-0 z-[60]" data-dialog-backdrop="true">
      ...
      <section class="absolute left-1/2 top-1/2 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background p-6 shadow-2xl">
        ...
        <div class="mt-4 space-y-2 text-sm">
          ...
          <p><span class="text-muted-foreground">接收方：</span>${escapeHtml(getReceiverDisplayName(head))}</p>
...
function renderObjectionDialog() {
    if (!state.objectionRecordId)
        return '';
    ...
    <div class="fixed inset-0 z-[60]" data-dialog-backdrop="true">
      ...
      <section class="absolute inset-y-6 left-1/2 w-full max-w-2xl -translate-x-1/2 overflow-y-auto rounded-xl border bg-background p-6 shadow-2xl">
        <header class="space-y-1">
          ...
          <p class="text-sm text-muted-foreground">记录平台跟进或处理结果</p>
        </header>
        ...
        <div class="mt-4 space-y-3 text-sm">
          ...
          <div class="rounded-md border bg-muted/20 p-3">
            ...
            <p class="mt-1"><span class="text-muted-foreground">接收方实收数量：</span>${typeof getRecordReceiverWrittenQty(record) === 'number' ? `${getRecordReceiverWrittenQty(record)} ${escapeHtml(head.qtyUnit)}` : '—'}</p>
...
function updateField(field, node) {
    ...
    if (field === 'ordersBottleneckFilter' && node instanceof HTMLSelectElement) {
        state.ordersBottleneckFilter = (node.value || 'ALL');
        state.ordersPage = 1;
        return;
    }
...
function handleAction(action, actionNode) {
    ...
    if (action === 'mark-handout-complete') {
        ...
        showProgressHandoverToast(`交出单已关闭：${handoverId}`);
        return true;
    }
    if (action === 'open-writeback-dialog') {
        ...
        state.writebackReturnNo = record.receiverRemark || record.diffReason || '';
        state.writebackQty =
            typeof getRecordReceiverWrittenQty(record) === 'number'
                ? String(getRecordReceiverWrittenQty(record))
                : String(record.submittedQty ?? record.plannedQty ?? '');
        state.writebackAt = getRecordReceiverWrittenAt(record)
            ? getRecordReceiverWrittenAt(record).replace(' ', 'T').slice(0, 16)
    ...
    if (action === 'confirm-writeback') {
        ...
        if (qty !== submittedQty && !state.writebackReturnNo.trim()) {
            showProgressHandoverToast('数量有差异时请填写差异原因', 'error');
            return true;
        }
        const updated = writeBackHandoverRecord({
            handoverRecordId: recordId,
            receiverWrittenQty: qty,
            receiverWrittenAt: `${state.writebackAt.replace('T', ' ')}:00`,
            receiverWrittenBy: '接收方扫码员',
            receiverRemark: state.writebackReturnNo.trim() || undefined,
            diffReason: qty === submittedQty ? undefined : state.writebackReturnNo.trim() || '接收方实收数量与工厂交出数量不一致',
        });
        if (!updated) {
            showProgressHandoverToast('当前记录无法确认收货，请刷新后重试', 'error');
            return true;
    ...
    if (action === 'goto-order') {
        ...
        if (orderId) {
            openLinkedPage(`生产单 ${orderId}`, buildProductionOrderLink(orderId));
        }
        return true;
    }
    if (action === 'goto-task') {
    ...
    if (action === 'open-order-detail') {
        ...
        if (orderId) {
            ...
            const taskId = actionNode.dataset.taskId || '';
            openLinkedPage(`交接详情-${orderId}`, buildHandoverOrderDetailLink({
...
export function renderProgressHandoverPage() {
    const html = renderPage();
    scheduleTimelineAnchorScroll();
    return html;
}
export function handleProgressHandoverEvent(target) {
    const fieldNode = target.closest('[data-handover-field]');
    ...
    const actionNode = target.closest('[data-handover-action]');
    if (!actionNode) {
...
export function isProgressHandoverDialogOpen() {
