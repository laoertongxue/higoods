import { getProjectChannelProductById, listProjectChannelProducts, } from '../data/pcs-channel-product-project-repository.ts';
import { CHANNEL_PRODUCT_STATUS_RULES, resolveChannelProductBusinessStatus, } from '../data/pcs-product-lifecycle-governance.ts';
...
function getChannelLabel(channelCode) {
...
function getStoreLabel(record) {
    if (record.storeId === 'store-shopee-01')
        return '虾皮马来西亚店';
    if (record.storeId === 'store-mini-program-01')
        return '微信小程序商城';
    if (record.storeId === 'store-lazada-01')
        return 'Lazada 菲律宾店';
    if (record.storeId === 'store-tiktok-01')
        return '抖音商城旗舰店';
    return record.storeName || '-';
}
function getViewLabel(record) {
    return CHANNEL_PRODUCT_STATUS_RULES[resolveChannelProductBusinessStatus(record)].label;
}
function getLinkageDescription(record) {
...
function renderChannelStatusBadge(status) {
    if (status === '已生效')
        return renderBadge(status, 'bg-emerald-100 text-emerald-700');
    if (status === '已作废')
        return renderBadge(status, 'bg-rose-100 text-rose-700');
    if (status === '已上架待测款')
        return renderBadge(status, 'bg-blue-100 text-blue-700');
    if (status === '已上传待确认')
        return renderBadge(status, 'bg-amber-100 text-amber-700');
    return renderBadge(status, 'bg-slate-100 text-slate-600');
}
function renderBusinessStatusBadge(record) {
...
function renderUpstreamStatusBadge(status) {
    if (status === '已更新')
        return renderBadge(status, 'bg-emerald-100 text-emerald-700');
    if (status === '待更新')
        return renderBadge(status, 'bg-amber-100 text-amber-700');
...
function renderSpecLineSummary(record) {
    return `
    <div class="space-y-1">
...
function getListingMainImage(record) {
    const mainImage = record.listingImages.find((item) => item.imageId === record.listingMainImageId) ||
...
function renderDetailButton(label, href) {
    if (!href) {
...
function renderSpecLineRows(record) {
...
export function renderPcsChannelProductListPage() {
...
export function renderPcsChannelProductDetailPage(channelProductId) {
