...
export function renderPcsPatternLibraryConfigPage() {
    syncState();
    return `
    <div class="space-y-4">
...
export function handlePcsPatternLibraryConfigEvent(target) {
    const actionNode = target.closest('[data-pattern-library-config-action]');
    ...
    if (action === 'toggle-rule') {
        ...
        if (rule === 'primaryColor')
            state.rulePrimaryColor = !state.rulePrimaryColor;
        if (rule === 'usageType')
            state.ruleUsageType = !state.ruleUsageType;
        if (rule === 'category')
            state.ruleCategory = !state.ruleCategory;
        if (rule === 'filenameTokens')
            state.ruleFilenameTokens = !state.ruleFilenameTokens;
        return true;
    }
    if (action === 'save') {
...
export function handlePcsPatternLibraryConfigInput(target) {
    syncState();
    const field = target.dataset.patternLibraryConfigField;
    if (!field)
        return false;
    state[field] = target.value;
    return true;
}
export function isPcsPatternLibraryConfigDialogOpen() {
    return false;
}
