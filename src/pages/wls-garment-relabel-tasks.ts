...
const controller = createProcessOrderListController({
  ...
  emptyText: '暂无成衣仓换码任务；后道或仓储发起包含成衣仓未售成衣的整色替换后会自动进入这里。',
...
function renderInner(): string {
  ...
  return renderStandardListPage({
    ...
    statsHtml: renderStandardListStats([
      ...
      { label: '成衣仓未售换码数量', value: formatQty(rows.reduce((sum, task) => sum + task.lines.reduce((lineSum, line) => lineSum + line.qty, 0), 0)) },
