...
import { createPatternAsset, getPatternCategorySecondaryList, persistPatternParsedFile, getPatternLibraryConfig, getPatternTaskSummary, listPatternApplicableCategoryOptions, listPatternDuplicateCandidates, waitForPatternLibraryPersistence, } from '../data/pcs-pattern-library';
import { buildPatternCategoryPath, getPatternSimilarityStatusText, PatternParseService, PatternTagService, validatePatternSubmitEligibility, } from '../utils/pcs-pattern-library-services';
...
function setFlashNotice(message) {
    if (typeof window !== 'undefined') {
...
function attachPreviewUrls(parsedFile) {
    return {
        ...parsedFile,
...
function releaseParsedFileUrls(parsedFile) {
    if (!parsedFile)
        return;
...
function clearUploadInputs() {
    if (typeof document === 'undefined')
        return;
    const singleInput = document.getElementById(SINGLE_UPLOAD_INPUT_ID);
    const batchInput = document.getElementById(BATCH_UPLOAD_INPUT_ID);
    if (singleInput)
        singleInput.value = '';
    if (batchInput)
        batchInput.value = '';
...
function focusCreateField(field) {
    if (typeof document === 'undefined')
        return;
    const node = document.querySelector(`[data-pattern-library-create-field="${field}"]`);
...
function getStatusBannerText() {
    ...
    if (state.mode === 'batch') {
        ...
        if (failedCount > 0 && successCount === 0)
            return '解析失败，请重试';
        if (successCount > 0)
            return '已完成解析，可保存草稿或提交审核';
        return '解析中';
    }
    if (!state.parsedFile)
        return '待上传并解析';
    if (state.parsedFile.parseStatus === 'failed')
        return '解析失败，请重试';
    if (state.parsedFile.parseStatus === 'success')
        return '已完成解析，可保存草稿或提交审核';
    return '待上传并解析';
}
...
function getCreateDefaultsFromParsed(parsedFile) {
    ...
    if (!state.form.patternName)
        state.form.patternName = parsedFile.originalFilename.replace(/\.[^.]+$/, '');
    if (!state.form.primaryColors)
        state.form.primaryColors = parsedFile.dominantColors.join(' / ');
    if (!state.form.categoryPrimary && primarySuggestion) {
        state.form.categoryPrimary = primarySuggestion.tag_name;
    }
    if (state.form.categoryPrimary
...
function buildDraftInput(parsedFile, submitForReview) {
    return {
        ...
        maintenanceStatus: state.form.maintenanceStatus,
        ...
        license: {
            license_status: state.form.licenseStatus,
...
async function handleSave(submitForReview) {
    ...
    if (!validation.valid) {
        ...
        if (validation.field)
            focusCreateField(validation.field);
        return true;
    }
    try {
        state.submitting = true;
...
function renderSourceTaskCard() {
    if (!state.form.sourceTaskId)
        return '';
...
function renderUploadControls() {
    ...
    const selectedLabel = state.mode === 'batch'
...
export function handlePcsPatternLibraryCreateEvent(target) {
    syncModeFromQuery();
    const actionNode = target.closest('[data-pattern-library-create-action]');
    ...
    if (action === 'select-batch-files') {
        const files = Array.from(actionNode.files ?? []);
        if (files.length > 0) {
    ...
    if (action === 'set-duplicate-action') {
        state.duplicateAction = actionNode.dataset.duplicateAction || 'force-new';
        return true;
    }
...
export function handlePcsPatternLibraryCreateInput(target) {
    syncModeFromQuery();
    const field = target.dataset.patternLibraryCreateField;
    if (!field)
        return false;
...
export function isPcsPatternLibraryCreateDialogOpen() {
    return false;
}
export function renderPcsPatternLibraryCreatePage() {
    syncModeFromQuery();
    return renderForm();
}
