import { getFactoryMobileTodoActionRoute, getFactoryMobileTodoSummary, getFactoryMobileTodos, } from '../data/fcs/factory-mobile-todos.ts';
...
function getCurrentFactoryTodos() {
...
function renderFilterChips() {
    ...
    <div class="flex gap-2 overflow-x-auto pb-1">
      ${FILTERS.map((item) => `
          <button
            type="button"
            class="whitespace-nowrap rounded-full border px-3 py-1.5 text-xs ${toClassName(state.filter === item.value ? 'border-primary bg-primary/10 text-primary' : 'text-muted-foreground')}"
      ...
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0 flex-1">
          <div class="flex flex-wrap items-center gap-2">
            ...
            <span class="rounded-full px-2 py-0.5 text-[11px] ${toClassName(todo.priority === '紧急'
        ? 'bg-destructive/10 text-destructive'
        : todo.priority === '加急'
            ? 'bg-amber-500/10 text-amber-700'
            : 'bg-primary/10 text-primary')}">${escapeHtml(todo.priority)}</span>
...
export function renderPdaNotifyPage() {
    const runtime = getPdaRuntimeContext();
    if (!runtime)
...
export function handlePdaNotifyEvent(target) {
    if (!ensurePdaSessionForAction())
        return true;
    const fieldNode = target.closest('[data-pda-notify-field]');
    if (!fieldNode)
        return false;
    if (fieldNode.dataset.pdaNotifyField === 'filter' && fieldNode.dataset.value) {
        state.filter = fieldNode.dataset.value;
        return true;
    }
    return false;
}
