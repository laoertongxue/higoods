...
function renderBatchCompatPage(targetPath, title, note) {
...
export function renderPaymentSyncPage() {
    const pageBoundary = getSettlementPageBoundary('payment-sync');
    return renderBatchCompatPage('/fcs/settlement/batches?view=payment', '预付款批次', pageBoundary.pageIntro);
}
export function handlePaymentSyncEvent(_target) {
    return false;
}
export function isPaymentSyncDialogOpen() {
    return false;
}
