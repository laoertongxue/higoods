...
import { resetPrintingWorkOrderBusinessStore, getPrintingWorkOrderById, updatePrintingRollBarcode, createPrintingDispatch, listPrintingDispatchDocuments, scanPrintingDispatchRoll, confirmPrintingDispatch, voidPrintingDispatch, copyPrintingRollBarcode, deletePrintingRollBarcodes, importPrintingRollLengths, markPrintingRollBarcodesPrinted, handoverPrintingOutput, capturePrintProcessMutationState, restorePrintProcessMutationState } from '../src/data/fcs/printing-task-domain.ts'
resetPrintingWorkOrderBusinessStore()
const initialState = capturePrintProcessMutationState()
for (const [workOrderId, targetId, targetName] of [
  ['PWO-PRINT-005', 'WH-FABRIC-001', '面料中央仓(GKP)'],
  ['PWO-PRINT-006', 'WH-FABRIC-002', '面料备用仓'],
] as const) {
  const workOrder = initialState.workOrders.find(([key]) => key === workOrderId)?.[1]
  assert.ok(workOrder)
  workOrder.receiverName = targetName
  workOrder.targetTransferWarehouseId = targetId
  workOrder.targetTransferWarehouseName = targetName
}
restorePrintProcessMutationState(initialState)
...
markPrintingRollBarcodesPrinted(id, rolls.map(roll => roll.id), '测试标签打印员')
...
scanPrintingDispatchRoll(doc, first.barcode, '测试扫码员')
scanPrintingDispatchRoll(doc, second.barcode, '测试扫码员')
...
one.receiverName='面料中央仓(GKP)';one.targetTransferWarehouseId='WH-FABRIC-001';one.targetTransferWarehouseName='面料中央仓(GKP)'
...
secondView.historicalInputQuantityUnknown=false;secondView.historicalRollQuantitiesUnknown=false;secondView.actualInput.receivedQty=10;secondView.actualInput.usedQty=10;secondView.actualInput.usedRollCount=1;secondView.actualInput.receivedRollCount=1;secondView.output.completedQty=0;secondView.output.completedRollCount=0;secondView.barcodes=secondView.barcodes.slice(0,1);secondView.barcodes[0].lengthY=0
secondView.productionStartedAt='2026-09-14 10:00:00';secondView.productionActions=[{id:'BATCH-PRINT-FINISH',stage:'PRINT',action:'FINISH',qty:10,operatorName:'测试印制员',at:'2026-09-14 10:10:00',requirementVersion:1}]
...
const secondOutput=getPrintingWorkOrderById(two.printOrderId)!
const secondRollId=secondOutput.barcodes.at(-1)!.id
updatePrintingRollBarcode(two.printOrderId,secondRollId,{lengthY:10,gsm:secondOutput.output.gsm,widthCm:secondOutput.output.widthCm,vatNo:'BATCH-TEST',warehouseName:'本厂待交出仓',remark:'批量测试实测'})
const secondRoll=getPrintingWorkOrderById(two.printOrderId)!.barcodes.find(roll=>roll.id===secondRollId)!
markPrintingRollBarcodesPrinted(one.printOrderId,[getPrintingWorkOrderById(one.printOrderId)!.barcodes[0].id],'批量测试标签打印员')
markPrintingRollBarcodesPrinted(two.printOrderId,[secondRoll.id],'批量测试标签打印员')
const batch=createPrintingDispatch([{workOrderId:one.printOrderId,barcodeIds:[one.businessView!.barcodes[0].id]},{workOrderId:two.printOrderId,barcodeIds:[secondRoll.id]}],'批量测试')
assert.equal(listPrintingDispatchDocuments().find(doc=>doc.id===batch)!.lines.length,2)
scanPrintingDispatchRoll(batch, getPrintingWorkOrderById(one.printOrderId)!.barcodes[0].barcode, '批量测试扫码员')
scanPrintingDispatchRoll(batch, secondRoll.barcode, '批量测试扫码员')
assert.equal(listPrintingDispatchDocuments().find(doc=>doc.id===batch)!.scanCompletedAt?.length! > 0,true)
console.log('PASS 多加工单合并、标签打印和扫卷核对')
