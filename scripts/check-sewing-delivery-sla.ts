...
import {
  ...
  getSewingDeliverySlaSnapshotById,
...
const stableAwardedDemoTaskId = 'TASKGEN-202603-083-002__ORDER'
const stableAwardedDemoTask = getRuntimeTaskById(stableAwardedDemoTaskId)
assert.equal(stableAwardedDemoTask?.assignmentStatus, 'AWARDED', '主 SLA 回归隔离前必须确认 083 是稳定已中标演示任务')
const slaFixtureTaskId = 'TEST-SLA-INDEPENDENT-SEWING-001'
const slaFixtureSource = getRuntimeTaskById('TASKGEN-202603-084-003__ORDER')
assert(slaFixtureSource, '主 SLA 回归必须存在可克隆的独立车缝事实源')
const slaFixtureState = captureRuntimeDirectDispatchState()
slaFixtureState.reassignedTasks.push([slaFixtureTaskId, {
  ...structuredClone(slaFixtureSource),
  taskId: slaFixtureTaskId,
  taskNo: slaFixtureTaskId,
  baseTaskId: slaFixtureTaskId,
  rootTaskNo: slaFixtureTaskId,
...
restoreRuntimeDirectDispatchState(slaFixtureState)

const delayedDemoTaskId = 'TASKGEN-202603-0015-001__ORDER'
const delayedDemoTask = listRuntimeProcessTasks().find((task) => task.taskId === delayedDemoTaskId)
assert(delayedDemoTask, '进度看板必须提供稳定的含车缝确认延迟演示任务')
assert.equal(delayedDemoTask.acceptanceStatus, 'ACCEPTED', '确认延迟演示任务必须已经接单并启动履约时钟')
const delayedDemoFactory = indonesiaFactories.find((factory) => factory.id === 'ID-F021')
assert(delayedDemoFactory, '确认延迟演示工厂必须存在于工厂主档')
assert.equal(delayedDemoFactory.status, 'ACTIVE', '确认延迟演示工厂主档必须启用')
assert.equal(delayedDemoFactory.type, 'MICRO_SEWING', '确认延迟演示工厂必须是车缝类型')
assert.equal(delayedDemoTask.assignedFactoryId, delayedDemoFactory.id, '确认延迟演示任务必须分配给启用的车缝工厂')
assert.equal(delayedDemoTask.assignedFactoryName, delayedDemoFactory.name, '确认延迟演示任务工厂名称必须与主档一致')
const delayedDemoView = getSewingDeliverySlaView(delayedDemoTaskId, '2026-07-12 12:00:00')
assert(delayedDemoView, '确认延迟演示任务必须由真实 SLA 快照和 PDA 事实生成履约视图')
assert.equal(delayedDemoView.projection.snapshot.factoryId, delayedDemoFactory.id, '确认延迟 SLA 快照工厂必须与任务一致')
assert.equal(delayedDemoView.projection.snapshot.factoryName, delayedDemoFactory.name, '确认延迟 SLA 快照工厂名称必须与主档一致')
const delayedDemoHead = listPdaHandoverHeads().find((head) => head.handoverId === 'HOH-SLA-DELAY-DEMO-001')
assert(delayedDemoHead, '确认延迟演示任务必须存在 PDA 交出单')
assert.equal(delayedDemoHead.factoryId, delayedDemoFactory.id, '确认延迟交出单工厂必须与 SLA 快照一致')
assert.equal(delayedDemoHead.sourceFactoryName, delayedDemoFactory.name, '确认延迟交出单工厂名称必须与主档一致')
const delayedDemoRecord = getPdaHandoverRecordsByHead(delayedDemoHead.handoverId).find((record) => record.recordId === 'SLA-DEMO-RECEIVER-DELAY-001')
assert(delayedDemoRecord, '确认延迟演示任务必须存在 PDA 实收事实')
assert.match(delayedDemoRecord.factorySubmittedBy ?? '', new RegExp(delayedDemoFactory.name), '确认延迟实收事实提交方必须与交出单工厂一致')
const delayedDemoOrder = productionOrders.find((order) => order.productionOrderId === delayedDemoTask.productionOrderId)
assert.equal(delayedDemoOrder?.mainFactoryId, delayedDemoFactory.id, '确认延迟演示生产单必须由唯一有效车缝工厂确定主工厂')
assert.equal(delayedDemoOrder?.auditLogs.some((log) => log.detail.includes(`${delayedDemoFactory.name} → ${delayedDemoFactory.name}`)), false, '确认延迟演示种子不得记录同值主工厂调整日志')
const delayedDemoMilestone = delayedDemoView.projection.milestones.find((milestone) => milestone.receiverDelayRecords.length > 0)
assert(delayedDemoMilestone, '确认延迟演示任务必须存在由交出/实收时点计算出的延迟节点')
assert.equal(delayedDemoMilestone.receiverDelayRecords[0]?.recordId, 'SLA-DEMO-RECEIVER-DELAY-001')
assert.equal(delayedDemoMilestone.receiverDelayRecords[0]?.submittedAt, '2026-07-05 08:00:00')
assert.equal(delayedDemoMilestone.receiverDelayRecords[0]?.receivedAt, '2026-07-05 12:00:00')
const kpiTaskCountBeforeDemoSearch = listRuntimeProcessTasks().length
assert.equal(listRuntimeProcessTasks().length, kpiTaskCountBeforeDemoSearch, '演示事实不得复制运行时任务或污染任务 KPI 分母')
...
try {
  const sewingTenderTaskId = slaFixtureTaskId
...
const batchAwardProbe = `
  ...
  try {
    const taskId = 'TASKGEN-202603-086-002__ORDER'
...
try {
  const batchTaskId = 'TASKGEN-202603-086-002__ORDER'
...
try {
  ...
  assert.throws(() => applyRuntimeDirectDispatchMeta({
    ...redispatchInput,
    factoryId: 'ID-F004',
    factoryName: '泗水车缝厂',
    businessAssignedAt: '2026-07-02 08:00:00',
    operatedAt: '2026-07-02 10:00:00',
  }), /普通入口覆盖|改派/, '同一含车缝任务不得通过普通入口再次派单')
  const redispatchHistory = listSewingDeliverySlaSnapshotHistory(redispatchTaskId)
  assert.equal(redispatchHistory.length, 1, '非法普通入口再次派单不得生成第二条履约快照')
  assert.equal(redispatchHistory[0].active, true)
} finally {
...
const independentSewingTaskIds = [
  'TASKGEN-202603-0015-001__ORDER',
  'TASKGEN-202603-086-002__ORDER',
  'TASKGEN-202603-084-003__ORDER',
]
const cleanIndependentDirectState = captureRuntimeDirectDispatchState()
cleanIndependentDirectState.taskOverrides = cleanIndependentDirectState.taskOverrides.filter(([taskId]) => !independentSewingTaskIds.includes(taskId))
restoreRuntimeDirectDispatchState(cleanIndependentDirectState)
const independentSewingTasks = independentSewingTaskIds.map((taskId) => getRuntimeTaskById(taskId))
...
assert.doesNotMatch(continuousDispatchSource, /data-continuous-dispatch-action="switch-dialog-mode"/, '直接派单与发起竞价必须使用独立弹窗，不得在弹窗内切换模式')
...
try {
  const viewTask = listRuntimeProcessTasks().find((task) => task.taskId === 'TASKGEN-202603-084-003__ORDER')
  assert(viewTask, '履约视图测试需要至少一个适用含车缝时效的运行时任务')
  assert.notEqual(viewTask.taskId, delayedDemoTaskId, '履约视图边界测试必须使用与稳定演示事实隔离的任务身份')
  assert.notEqual(classifySewingDeliverySla(viewTask), null, '履约视图边界测试任务必须适用含车缝时效')
  const viewTaskId = viewTask.taskId
  const conflictTask = listRuntimeProcessTasks().find((task) => task.taskId === 'TASKGEN-202603-086-002__ORDER')
  assert.notEqual(conflictTask?.taskId, stableAwardedDemoTaskId, '分配冲突回归必须使用与稳定中标演示任务隔离的事实样本')
  assert(conflictTask, '字段冲突测试需要另一个适用含车缝时效的运行时任务')
  assert.notEqual(conflictTask.taskId, delayedDemoTaskId, '字段冲突测试必须使用与稳定演示事实隔离的任务身份')
  assert.notEqual(classifySewingDeliverySla(conflictTask), null, '字段冲突测试任务必须适用含车缝时效')
...
try {
  const reassignmentFixtureTemplateId = 'TASKGEN-202603-084-003__ORDER'
  const reassignmentFixtureTaskId = 'TEST-REASSIGN-SEWING-SOURCE'
  const cleanReassignmentFixture = captureRuntimeDirectDispatchState()
  cleanReassignmentFixture.taskOverrides = cleanReassignmentFixture.taskOverrides.filter(([taskId]) => taskId !== reassignmentFixtureTaskId)
  const reassignmentFixtureTemplate = getRuntimeTaskById(reassignmentFixtureTemplateId)
  assert(reassignmentFixtureTemplate, '改派测试需要现有独立车缝任务作为模板')
  cleanReassignmentFixture.reassignedTasks.push([reassignmentFixtureTaskId, {
    ...structuredClone(reassignmentFixtureTemplate),
    taskId: reassignmentFixtureTaskId,
    taskNo: reassignmentFixtureTaskId,
    baseTaskId: reassignmentFixtureTaskId,
    rootTaskNo: reassignmentFixtureTaskId,
    scopeQty: 1400,
    baseQty: 1400,
    qty: 1400,
    assignmentMode: 'HOLD',
  ...
  restoreRuntimeDirectDispatchState(cleanReassignmentFixture)
  const sourceBase = getRuntimeTaskById(reassignmentFixtureTaskId)
  assert(sourceBase, '改派测试需要固定独立车缝任务')
  assert.notEqual(sourceBase.taskId, delayedDemoTaskId, '改派事实边界测试必须使用与稳定演示事实隔离的任务身份')
  ...
  assert.equal(sumSewingDeliveryConfirmedReceiptQty(source.taskId, '2026-07-08 10:00:00'), 123, '改派操作时点只计当时已确认的 MATCHED、WRITTEN_BACK_DIFF、DIFF_ACCEPTED')
  assert.equal(sumSewingDeliveryConfirmedReceiptQty(source.taskId, '2026-07-08 11:00:00'), 140, '操作后确认的17件必须进入旧任务当前态，不得为保持改派余量而隐藏')
  ...
  upsertPdaHandoutRecordMock({ recordId: 'REC-REASSIGN-RECEIVER-DELAY-A', handoverRecordId: 'REC-REASSIGN-RECEIVER-DELAY-A', handoverId: reassignmentHeadId, handoverOrderId: reassignmentHeadId, taskId: source.taskId, sourceTaskId: source.taskId, sequenceNo: 3, submittedQty: 148, plannedQty: 148, qtyUnit: '件', factorySubmittedAt: '2026-07-04 07:00:00', factorySubmittedBy: '原工厂操作员', factoryProofFiles: [], status: 'WRITTEN_BACK', handoverRecordStatus: 'WRITTEN_BACK_MATCHED', receiverWrittenQty: 148, receiverWrittenAt: '2026-07-05 12:00:00', receiverWrittenBy: '仓库收货员' })
  upsertPdaHandoutRecordMock({ recordId: 'REC-REASSIGN-RECEIVER-DELAY-Z', handoverRecordId: 'REC-REASSIGN-RECEIVER-DELAY-Z', handoverId: reassignmentHeadId, handoverOrderId: reassignmentHeadId, taskId: source.taskId, sourceTaskId: source.taskId, sequenceNo: 4, submittedQty: 149, plannedQty: 149, qtyUnit: '件', factorySubmittedAt: '2026-07-04 07:30:00', factorySubmittedBy: '原工厂操作员', factoryProofFiles: [], status: 'WRITTEN_BACK', handoverRecordStatus: 'WRITTEN_BACK_MATCHED', receiverWrittenQty: 149, receiverWrittenAt: '2026-07-05 12:00:00', receiverWrittenBy: '仓库收货员' })
  ...
  assert.equal(oldViewAfterLateReceipt?.confirmedReceivedQty, 447, '旧任务后续实收必须继续推进旧历史快照投影并跨过30%节点')
  assert.equal(oldViewAfterLateReceipt?.submittedQty, 524, '旧历史视图已交出只统计同task有效提交；WAIT可计已交出，错挂与VOIDED不计')
  const inactiveBatchView = listSewingDeliverySlaViews('2026-07-09 10:00:00', [source.taskId]).find((view) => view.runtimeTaskId === source.taskId)
  assert.equal(inactiveBatchView?.confirmedReceivedQty, 447, '批量inactive投影必须与单查复用同一confirmed合法状态矩阵')
  assert.equal(inactiveBatchView?.submittedQty, 524, '批量inactive已交出不得被同head错挂task或VOIDED污染')
  ...
  assert.deepEqual(delayedMilestone.receiverDelayRecords.map((record) => [record.recordId, record.affectedQty]), [['REC-REASSIGN-RECEIVER-DELAY-A', 148], ['REC-REASSIGN-RECEIVER-DELAY-Z', 149]], '同一确认批次跨节点时应按两条延迟记录稳定分摊297件受影响数量')
...
assert.match(runtimeReadBridgeSource, /normalizeModuleOwner[\s\S]*resolverInstallToken/, '只读解析器必须按模块归属支持安全热替换与释放')
...
assert.match(runtimeProcessTaskSource, /installRuntimeTaskReadResolver\([\s\S]*import\.meta\.url[\s\S]*import\.meta\.hot\?\.dispose/, 'runtime 模块必须声明安装归属并在 HMR dispose 时释放当前 resolver')
...
try {
  const sewingWorkbenchHtml = renderSewingDispatchWorkbenchPage()
  assert.match(sewingWorkbenchHtml, /车缝任务 \/ 生产单/)
  assert.match(sewingWorkbenchHtml, /SKU 数 \/ 任务数量/)
  assert.match(sewingWorkbenchHtml, /可分配状态/)
  assert.doesNotMatch(sewingWorkbenchHtml, /<th[^>]*>毛织片<\/th>/)
  assert.doesNotMatch(sewingWorkbenchHtml, /<th[^>]*>特种工艺裁片<\/th>/)
  assert.doesNotMatch(sewingWorkbenchHtml, /min-w-\[2260px\]/)
  ...
  assert.match(directDialogHtml, /确认唯一主工厂|确认主工厂/)
  assert.match(directDialogHtml, /交付完成/)
  assert.match(directDialogHtml, /30% 回货/)
  assert.match(directDialogHtml, /2026-\d{2}-\d{2} \d{2}:\d{2}/, '时效必须展示具体年月日时分')
  ...
  restoreSewingDispatchWorkbenchPageState({
    ...openedState,
    dispatchFactoryIdByRowId: Object.fromEntries(directRows.map((row) => [row.rowId, directFactory.id])),
    dispatchMainFactoryIdByProductionOrderId: { [fixtureSource.productionOrderId]: directFactory.id },
  })
  ...
  assert.equal(directDraft.runtimeTaskIds.length, directRows.length, '真实 handler 必须为每个完整 SKU 生成独立工厂归属')
  assert.equal(directDraft.qty, directRows.reduce((sum, row) => sum + row.remainingQty, 0), '真实 handler 必须提交所有完整 SKU 的全部待分配数量')
  ...
  directRows.forEach((row) => assert.equal(listSewingDispatchWorkbenchRows().some((remaining) => remaining.taskId === fixtureTaskId && remaining.skuCode === row.skuCode), false, `${row.skuCode} 必须整 SKU 派出且不保留数量残行`))
  ...
  const secondDirect = createSewingDispatchWorkbenchDraft({ actionType: '直接派单', rowIds: [secondFactoryRow.rowId], factoryIdByRowId: { [secondFactoryRow.rowId]: secondFactory.id }, businessAssignedAt: '2026-07-10 08:30:00', operatedAt: '2026-07-10 10:00:00', by: '跟单A' })
  ...
  const explicitSwitch = createSewingDispatchWorkbenchDraft({ actionType: '直接派单', rowIds: [explicitSwitchRow.rowId], factoryIdByRowId: { [explicitSwitchRow.rowId]: secondFactory.id }, businessAssignedAt: '2026-07-10 09:00:00', operatedAt: '2026-07-10 10:00:00', mainFactoryIdByProductionOrderId: { [fixtureSource.productionOrderId]: secondFactory.id }, by: '跟单A' })
  ...
  const multiDirect = createSewingDispatchWorkbenchDraft({ actionType: '直接派单', rowIds: [multiFirstRow.rowId, multiSecondRow.rowId], factoryIdByRowId: { [multiFirstRow.rowId]: secondFactory.id, [multiSecondRow.rowId]: secondFactory.id }, businessAssignedAt: '2026-07-10 09:10:00', operatedAt: '2026-07-10 10:00:00', mainFactoryIdByProductionOrderId: { [fixtureSource.productionOrderId]: directFactory.id, [secondOrderId]: secondFactory.id }, by: '跟单A' })
  ...
  const invalidMulti = createSewingDispatchWorkbenchDraft({ actionType: '直接派单', rowIds: [invalidMultiRow.rowId], factoryIdByRowId: { [invalidMultiRow.rowId]: secondFactory.id }, businessAssignedAt: '2026-07-10 09:20:00', operatedAt: '2026-07-10 10:00:00', mainFactoryIdByProductionOrderId: { [secondOrderId]: 'NO-SUCH-MAIN' }, by: '跟单A' })
  ...
  restoreSewingDispatchWorkbenchPageState({ ...workbenchDispatchPageState, selectedTaskIds: new Set([bidRow.taskId]), dispatchSelectedRowIds: new Set([bidRow.rowId]), dispatchOpen: true, dispatchActionType: '发起竞价', dispatchBatchFactoryId: '', dispatchFactoryIdByRowId: {}, dispatchBusinessAssignedAt: '2026-07-10T09:00', dispatchOperatedAt: '2026-07-10 10:00:00', dispatchError: '' })
  assert.equal(handleSewingDispatchWorkbenchEvent(workbenchActionTarget('confirm-dispatch')), true)
  ...
  const futureTaskRows = listSewingDispatchWorkbenchRows().filter((row) => row.taskId === futureRow.taskId && row.completeKitQty >= row.remainingQty)
  const futureFactoryMap = Object.fromEntries(futureTaskRows.map((row) => [row.rowId, directFactory.id]))
  restoreSewingDispatchWorkbenchPageState({ ...workbenchDispatchPageState, selectedTaskIds: new Set([futureRow.taskId]), dispatchSelectedRowIds: new Set(futureTaskRows.map((row) => row.rowId)), dispatchOpen: true, dispatchActionType: '直接派单', dispatchBatchFactoryId: '', dispatchFactoryIdByRowId: futureFactoryMap, dispatchBusinessAssignedAt: '2026-07-10T11:01', dispatchOperatedAt: '2026-07-10 10:00:00', dispatchError: '' })
  assert.equal(handleSewingDispatchWorkbenchEvent(workbenchActionTarget('confirm-dispatch')), true)
  ...
  restoreSewingDispatchWorkbenchPageState({ ...workbenchDispatchPageState, selectedTaskIds: new Set([futureRow.taskId]), dispatchSelectedRowIds: new Set(futureTaskRows.map((row) => row.rowId)), dispatchOpen: true, dispatchActionType: '直接派单', dispatchBatchFactoryId: '', dispatchFactoryIdByRowId: futureFactoryMap, dispatchBusinessAssignedAt: '', dispatchOperatedAt: '2026-07-10 10:00:00', dispatchError: '' })
  assert.equal(handleSewingDispatchWorkbenchEvent(workbenchActionTarget('confirm-dispatch')), true)
...
try {
  ...
  const handlerSource = getRuntimeTaskById('TASKGEN-202603-086-002__ORDER')!
...
const adversarialProjectionSnapshot = createSewingDeliverySlaSnapshot({ assignmentId: 'ADVERSARIAL-PROJECTION', runtimeTaskId: 'TASK-ADVERSARIAL-PROJECTION', productionOrderId: 'PO-ADVERSARIAL', factoryId: 'F-ADVERSARIAL', factoryName: '对抗测试厂', assignedQty: 10, acceptedAt: '2026-07-01 00:00:00', slaKind: 'INDEPENDENT_SEWING' })
const asOfProjection = projectSewingDeliverySla(adversarialProjectionSnapshot, [
  { recordId: 'DUPLICATE-GLOBAL', submittedQty: 1, submittedAt: '2026-07-01 01:00:00', receivedQty: 1, receivedAt: '2026-07-02 01:00:00' },
  { recordId: 'DUPLICATE-GLOBAL', submittedQty: 2, submittedAt: '2026-07-01 01:00:00', receivedQty: 2, receivedAt: '2026-07-02 02:00:00' },
  { recordId: 'FUTURE-RECEIPT', submittedQty: 10, submittedAt: '2026-07-02 03:00:00', receivedQty: 10, receivedAt: '2026-07-03 00:00:00' },
], '2026-07-02 12:00:00')
assert.equal(asOfProjection.confirmedReceivedQty, 2, '履约投影只能累计观察时点前事实，且同 recordId 只取最新有效版本')
const reversedInputProjection = projectSewingDeliverySla(adversarialProjectionSnapshot, [...[
  { recordId: 'FUTURE-RECEIPT', submittedQty: 10, submittedAt: '2026-07-02 03:00:00', receivedQty: 10, receivedAt: '2026-07-03 00:00:00' },
  { recordId: 'DUPLICATE-GLOBAL', submittedQty: 2, submittedAt: '2026-07-01 01:00:00', receivedQty: 2, receivedAt: '2026-07-02 02:00:00' },
  { recordId: 'DUPLICATE-GLOBAL', submittedQty: 1, submittedAt: '2026-07-01 01:00:00', receivedQty: 1, receivedAt: '2026-07-02 01:00:00' },
]], '2026-07-02 12:00:00')
assert.deepEqual(reversedInputProjection, asOfProjection, 'recordId 去重与平局策略必须与输入顺序无关')
const cappedAttributionProjection = projectSewingDeliverySla(adversarialProjectionSnapshot, [
...
try {
  ...
  assert.throws(() => saveSewingDeliverySlaSnapshot({ ...collisionBase, factoryId: 'F-B' }), /快照ID冲突/, '相同任务与分配编号也不得覆盖已保存的快照事实')
  assert.equal(getSewingDeliverySlaSnapshotById(collisionBase.snapshotId)?.runtimeTaskId, 'TASK-COLLISION-A')
  assert(getSewingDeliverySlaView('TASK-COLLISION-A', '2026-07-02 00:00:00'), '运行时任务不存在时单条查询应保留历史快照可见性')
  assert(listSewingDeliverySlaViews('2026-07-02 00:00:00', ['TASK-COLLISION-A']).some((view) => view.runtimeTaskId === 'TASK-COLLISION-A'), '批量查询必须与单条查询共享历史快照资格')
} finally {
  restoreSewingDeliverySlaSnapshotStore(collisionState)
}
