...
try {
  ...
  assert.equal(oldViewAfterLateReceipt?.submittedQty, 220, '旧历史视图已交出只统计同task有效提交；WAIT可计已交出，错挂与VOIDED不计')
  ...
  assert.equal(inactiveBatchView?.confirmedReceivedQty, 143, '批量inactive投影必须与单查复用同一confirmed合法状态矩阵')
  assert.equal(inactiveBatchView?.submittedQty, 220, '批量inactive已交出不得被同head错挂task或VOIDED污染')
