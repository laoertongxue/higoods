...
try {
  ...
  assert.equal(oldViewAfterLateReceipt?.confirmedReceivedQty, 430, '旧任务后续实收必须继续推进旧历史快照投影并跨过30%节点')
  assert.equal(oldViewAfterLateReceipt?.submittedQty, 507, '旧历史视图已交出只统计同task有效提交；WAIT可计已交出，错挂与VOIDED不计')
  ...
  assert.equal(inactiveBatchView?.confirmedReceivedQty, 430, '批量inactive投影必须与单查复用同一confirmed合法状态矩阵')
  assert.equal(inactiveBatchView?.submittedQty, 507, '批量inactive已交出不得被同head错挂task或VOIDED污染')
...
assert.match(runtimeReadBridgeSource, /HMR[\s\S]*resolveRuntimeTask = resolver/, '只读解析器必须允许热重载模块替换为最新 resolver')
...
assert.doesNotThrow(() => installRuntimeTaskReadResolver(() => null), '只读桥必须允许 HMR 产生的新 resolver 引用替换旧引用')
assert.equal(readRuntimeTaskById('TASKGEN-202603-0015-001__ORDER'), null, '替换后必须委托最新 resolver')
