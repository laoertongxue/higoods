...
const testFactory = factories.find((factory) => factory.id === TEST_FACTORY_ID)
assert(testFactory, '缺少全能力测试工厂')
assert(testFactory.isTestFactory === true, '全能力测试工厂必须带 isTestFactory 标识')
assert(testFactory.testFactoryScope === 'ALL_PROCESS_CRAFT', '全能力测试工厂缺少 ALL_PROCESS_CRAFT 范围标识')
const testFactoryProfile = profiles.find((profile) => profile.factoryId === TEST_FACTORY_ID)
assert(testFactoryProfile, '全能力测试工厂缺少产能档案')
assert(listFactoryCapacityEntries(TEST_FACTORY_ID).length > 0, '全能力测试工厂缺少可展示产能条目')
assert(listFactoryCapacityEquipments(TEST_FACTORY_ID).length > 0, '全能力测试工厂缺少最小设备档案')
...
assert(PROFILE_PAGE_SOURCE.includes('测试工厂'), '工厂产能档案页面缺少测试工厂标签')
