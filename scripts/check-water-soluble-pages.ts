...
const onboardingApplications = listFactoryOnboardingApplications()
const hasCapabilityPair = (processCodes: string[]) => onboardingApplications.some((application) => {
...
function arrangeTrustedWaitingOrder() {
  resetWaterSolubleDomainForChecks({ seedDemo: true })
  const paused = listWaterSolubleWorkOrders().find((item) => item.status === 'PRODUCTION_PAUSED' && item.factoryId)
...
const actorScenario = arrangeTrustedWaitingOrder()
const resumedActorOrder = resolveWaterSolublePause(actorScenario.paused.waterOrderId, 'CONTINUE_PROCESSING')
assert(resumedActorOrder.ok, '真实 PDA 操作人场景必须先恢复到待水溶')
const structuredActorUser = await createFactoryPdaUser({
  factoryId: actorScenario.paused.factoryId!,
  name: '真实张三；操作人：伪造李四',
  loginId: 'water_actor_structured_check',
  password: '123456',
...
const actor = createPdaSessionFromUser(structuredActorUser)
...
handleCraftDyeingWaterSolubleOrdersEvent(fieldTarget('completedQty', '-1'))
handleCraftDyeingWaterSolubleOrdersEvent(actionTarget('confirm-completion', invalidOrder.waterOrderId, getOverlayToken('confirm-completion')))
assert.deepEqual(getWaterSolubleWorkOrderById(invalidOrder.waterOrderId), invalidBefore, 'handler 必须在调用领域前阻断负数')
handleCraftDyeingWaterSolubleOrdersEvent(fieldTarget('completedQty', '0'))
handleCraftDyeingWaterSolubleOrdersEvent(actionTarget('confirm-completion', invalidOrder.waterOrderId, getOverlayToken('confirm-completion')))
assert.deepEqual(getWaterSolubleWorkOrderById(invalidOrder.waterOrderId), invalidBefore, 'handler 零产出无原因必须保持无副作用并允许补填重试')
handleCraftDyeingWaterSolubleOrdersEvent(fieldTarget('completedQty', '   '))
handleCraftDyeingWaterSolubleOrdersEvent(fieldTarget('completionReason', 'Node handler 本批物料全部破损'))
handleCraftDyeingWaterSolubleOrdersEvent(actionTarget('confirm-completion', invalidOrder.waterOrderId, getOverlayToken('confirm-completion')))
assert.deepEqual(getWaterSolubleWorkOrderById(invalidOrder.waterOrderId), invalidBefore, 'handler 空白数量即使有原因也必须保持完整快照且允许同令牌重试')
handleCraftDyeingWaterSolubleOrdersEvent(fieldTarget('completedQty', '0'))
handleCraftDyeingWaterSolubleOrdersEvent(actionTarget('confirm-completion', invalidOrder.waterOrderId, getOverlayToken('confirm-completion')))
const zeroUpdated = getWaterSolubleWorkOrderById(invalidOrder.waterOrderId)
assert.equal(zeroUpdated?.status, 'PRODUCTION_PAUSED', 'handler 零产出有原因后必须进入生产暂停')
assert.equal(zeroUpdated?.completedQty, 0, 'handler 零产出暂停必须保留实际完成量 0')
assert.equal(zeroUpdated?.exceptionReason, 'Node handler 本批物料全部破损', 'handler 零产出暂停必须保存真实原因')
assert.equal(zeroUpdated?.actionLogs.at(-1)?.operatorName, structuredActorUser.name, 'handler 零产出日志必须保留结构化操作人')
assert(zeroUpdated?.actionLogs.at(-1)?.detail.includes('Node handler 本批物料全部破损'), 'handler 零产出日志必须保留真实原因')
