...
async function main(): Promise<void> {
  ...
  await page.waitForFunction(() => document.body.innerText.includes('已配齐待接收'), undefined, { timeout: 10_000 })
  await page.locator('a[href*="difference=1"]').first().waitFor({ timeout: 10_000 })

  const openReturnDuration = await measure('接收差异上报打开退回入口', async () => {
    ...
    await assertNoPageErrors(pageErrors, '接收差异上报')
    await page.waitForFunction(() => document.body.innerText.includes('上报接收差异'), undefined, { timeout: 10_000 })
    await page.locator('[data-pda-warehouse-action="toggle-cutting-pickup-difference"]').first().click()
    await page.waitForFunction(() => {
      const text = document.body.innerText
      return text.includes('上报接收差异') && text.includes('提交差异并叫主管')
