...
function warehouseDomainSlug(operation: SpecialCraftOperationDefinition): 'auxiliary' | 'special-type' {
  return operation.managementDomain === 'AUXILIARY_CRAFT_FACTORY' ? 'auxiliary' : 'special-type'
}
...
const dispatchPageSource = read('src/pages/process-factory/cutting/warehouse-hub.ts')
const returnPageSource = read('src/pages/process-factory/cutting/warehouse-hub.ts')
...
;[
  ...
  'getSpecialCraftTaskOrderById',
  'listSpecialCraftTaskOrders',
  'listSpecialCraftGenerationBatches',
].forEach((item) => assertContains(taskOrdersSource, item, `特殊工艺任务缺少关键能力：${item}`))

assertContains(generationSource, 'getSpecialCraftGenerationKey', '任务幂等键必须保留')
assertContains(generationSource, 'targetObject', '任务仍需按作用对象合并')
assertContains(generationSource, "'WAIT_ASSIGN'", '任务仍需保留待分配占位合并口径')
assertContains(taskOrdersSource, 'partName', '任务单必须按裁片部位拆分')
assertContains(taskOrdersSource, 'factoryId ||', '任务单必须按执行工厂或待分配占位拆分')
...
const sampleOperation =
  listEnabledSpecialCraftOperationDefinitions().find((operation) =>
    renderSpecialCraftDomainWaitHandoverWarehousePage(warehouseDomainSlug(operation)).includes('打印任务交货卡'),
...
;[
  ...
  '待交出',
  '差异',
].forEach((item) => assertContains(taskOrdersPageSource, item, `加工单列表缺少数量或差异字段：${item}`))
...
assertContains(sewingDispatchSource, 'currentQty > 0', '已回仓裁片交出仍需 currentQty > 0')
assertContains(sewingDispatchSource, "summary.returnStatus.includes('已回仓')", '交出必须按回仓结果判断特殊工艺状态')
assertContains(sewingDispatchSource, '差异待处理不阻断裁片交出', '特殊工艺差异待处理只能形成交出后差异提示')

assertContains(sewingDispatchSource, '差异待处理不阻断', '特殊工艺差异不得阻断正数量回仓交出')
...
assert(tasks.length > 0, '必须保留特殊工艺任务数据')
assert(tasks.every((item) => item.demandLines && item.demandLines.length > 0), '任务单必须包含需求明细')
assert(tasks.every((item) => item.taskOrderId && item.partName), '任务单必须关联裁片部位')
assert(bindings.length > 0, '必须保留特殊工艺菲票绑定')
assert(bindings.every((item) => item.taskOrderId), '菲票绑定必须关联任务单')
...
assert('taskOrderCount' in parentSummary || 'childWorkOrderCount' in parentSummary, '任务聚合必须包含子工艺单数')
...
assert(eligible.every((item) => item.qty > 0), '可交出菲票必须保留正数当前数量')
