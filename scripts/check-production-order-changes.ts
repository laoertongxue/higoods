...
assert.ok(
  orders.some((order) => (order as any).changeType === 'QUANTITY_CHANGE'),
  '生产单变更必须覆盖修改生产单需求数量',
)
assert.ok(
  orders.some((order) => (order as any).changeType === 'MATERIAL_REPLACEMENT'),
  '生产单变更必须覆盖替换物料',
)
...
const materialOrder = orders.find((order) => (order as any).changeType === 'MATERIAL_REPLACEMENT')
