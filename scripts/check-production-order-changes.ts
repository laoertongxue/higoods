...
import { productionOrderChangeResultLabels } from '../src/data/fcs/production-tech-pack-change-domain.ts'
...
const removedCopy = ['待主管确认', '相关负责人', '通知相关负责人处理', '适用颜色', '适用尺码', '从哪里开始用新物料', '提交审核']
...
removedCopy.forEach((text) => {
  assert.ok(!newHtml.includes(text), `新增页不应出现旧文案「${text}」`)
})
