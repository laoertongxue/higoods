...
export const PROCESS_ROUTE_FULL_STAGE_DELIVERY: DeliveryEntry[] = [
  ...
  entry('PROD-004', '整件毛织加工单', 'PROD', '保留并接入', '通用任务项关系', ['src/data/fcs/wool-domain/types.ts', relationCarrier, 'src/pages/process-factory/wool/stage-order-detail.ts'], ['check:wool-fact-workflow', 'check:process-order-task-relations']),
  entry('PROD-005', '部位毛织加工单', 'PROD', '保留并接入', '通用任务项关系', ['src/data/fcs/wool-domain/types.ts', relationCarrier, 'src/pages/process-factory/wool/stage-order-detail.ts'], ['check:wool-fact-workflow', 'check:wool-warehouse-unified-model', 'check:process-order-task-relations']),
