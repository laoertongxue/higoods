...
const handedWithoutDebt = leaderRows.find((row) => row.quantitySummaries.includes('裁片已齐，不欠片'))
assert.ok(handedWithoutDebt, '已交出不欠片任务必须明确显示裁片已齐')
assert.ok(handedWithoutDebt!.sourceLinks.some((source) => source.label === '交出与欠片'))
assert.ok(!handedWithoutDebt!.sourceLinks.some((source) => source.label === '补料跟进'), '当前不欠片的任务不得提供补料跟进入口')
const confirmedDebt = leaderRows.find((row) => row.quantitySummaries.some((fact) => fact.startsWith('裁片欠片 ')))
assert.ok(confirmedDebt?.sourceLinks.some((source) => source.label === '交出与欠片'))
assert.ok(confirmedDebt?.sourceLinks.some((source) => source.label === '补料跟进'))
