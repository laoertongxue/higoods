...
const factoryWarehouseSeeds = listFactoryInternalWarehouses()
const seededWarehouseFactory = factoryWarehouseSeeds.find((warehouse) => {
  const factoryId = warehouse.factoryId
  return (
    listFactoryWaitProcessStockItems().some((item) => item.factoryId === factoryId)
    && listFactoryWaitHandoverStockItems().some((item) => item.factoryId === factoryId)
    && listFactoryWarehouseInboundRecords().some((item) => item.factoryId === factoryId)
    && listFactoryWarehouseOutboundRecords().some((item) => item.factoryId === factoryId)
    && listFactoryWarehouseStocktakeOrders().some((item) => item.factoryId === factoryId)
  )
})
const factoryId = seededWarehouseFactory?.factoryId || factoryWarehouseSeeds[0]?.factoryId || 'ID-F001'
const factoryName = seededWarehouseFactory?.factoryName || factoryWarehouseSeeds[0]?.factoryName || '示例工厂'
...
const printingFactory = mockFactories.find((item) => item.factoryType === 'CENTRAL_PRINT')
const dyeFactory = mockFactories.find((item) => item.factoryType === 'CENTRAL_DYE')
const cuttingFactory = mockFactories.find((item) => item.factoryType === 'CENTRAL_CUTTING')
const finishingFactory = mockFactories.find((item) => item.factoryType === 'SATELLITE_FINISHING')
const sewingFactory = mockFactories.find((item) => item.factoryType === 'SATELLITE_SEWING')
...
;[
  ...
  assertContains(cuttingWaitHandoverActionsSource, token, `裁床待交出仓动作未直达操作页：${token}`)
})
...
assert.deepEqual(
  ...
  '仓管视图概览 6 张卡顺序必须保持：待加工仓 / 待交出仓 / 入库记录 / 出库记录 / 盘点 / 差异',
)
assert(warehouseCards.some((card) => card.title === '待加工仓'), '仓管视图概览缺少待加工仓卡片')
assert(warehouseCards.some((card) => card.title === '待交出仓'), '仓管视图概览缺少待交出仓卡片')
assert(warehouseCards.some((card) => card.title === '入库记录'), '仓管视图概览缺少入库记录卡片')
assert(warehouseCards.some((card) => card.title === '出库记录'), '仓管视图概览缺少出库记录卡片')
assert(warehouseCards.some((card) => card.title === '盘点'), '仓管视图概览缺少盘点卡片')
assert(warehouseCards.some((card) => card.title === '差异'), '仓管视图概览缺少差异卡片')
...
assertContains(warehouseViewSource, 'isSewingFactory', '仓管视图层缺少车缝厂轻量判断 helper')
...
assert(listFactoryWarehouseAdjustmentOrders().some((item) => item.adjustmentType === '盘盈单'), '缺少盘盈单 mock 数据')
assert(listFactoryWarehouseAdjustmentOrders().some((item) => item.adjustmentType === '盘亏单'), '缺少盘亏单 mock 数据')
assert(listFactoryWarehouseInboundRecords().some((item) => item.sourceRecordType === 'STOCKTAKE_ADJUSTMENT'), '库存流水缺少盘盈调整入库')
assert(listFactoryWarehouseOutboundRecords().some((item) => item.remark?.includes('盘亏审核通过')), '库存流水缺少盘亏调整出库')
...
assert(printingFactory && dyeFactory && cuttingFactory && finishingFactory && sewingFactory, '不同工厂类型样例不完整，无法校验仓管覆盖')
const typedOverviews = [
  getFactoryMobileWarehouseOverview(printingFactory.id, printingFactory.name),
  getFactoryMobileWarehouseOverview(dyeFactory.id, dyeFactory.name),
  getFactoryMobileWarehouseOverview(cuttingFactory.id, cuttingFactory.name),
  getFactoryMobileWarehouseOverview(finishingFactory.id, finishingFactory.name),
]
assert(typedOverviews.every((item) => item.isSewingLightweight === false), '非车缝工厂仓管必须继续使用完整 6 卡口径')
const sewingOverview = getFactoryMobileWarehouseOverview(sewingFactory.id, sewingFactory.name)
assert(sewingOverview.isSewingLightweight, '车缝厂仓管必须走轻量接收回写口径')
assert(sewingOverview.waitProcessCount === 0, '车缝厂仓管不应伪造待加工仓数据')
assert(sewingOverview.waitHandoverCount === 0, '车缝厂仓管不应伪造待交出仓数据')
assert(sewingOverview.pendingTransferBagReceiveCount >= 0, '车缝厂仓管缺少待收中转袋统计')
assert(sewingOverview.receivedTransferBagCount >= 0, '车缝厂仓管缺少已收中转袋统计')
assert(sewingOverview.feiTicketWritebackCount >= 0, '车缝厂仓管缺少菲票回写统计')
...
assertContains(waitProcessSource, 'data-pda-warehouse-action="confirm-cutting-wp-pickup"', '待加工仓页面必须承接中转仓接收确认动作')
assertContains(waitProcessSource, '确认全部接收', '待加工仓页面必须明确中转仓实物全部接收口径')
