...
assert.equal((viewSectionHtml.match(/定位裁（激光切）演示工厂35 · 待加工仓库位图/g) || []).length, 1, '工厂仓库名称与库位统计必须合并在同一头部卡片')
...
assert.match(warehouseMapSource, /@page \{ size: 100mm 100mm/, '库位标签必须使用 100mm × 100mm 打印版式')
assert.match(warehouseMapSource, /data-location-label-line="location"[\s\S]*data-location-label-line="area"[\s\S]*data-location-label-line="guide"[\s\S]*data-location-label-line="qr"[\s\S]*data-location-label-line="barcode"/, '库位标签必须按库位编号、库区仓名、层位指示、二维码、条形码五段排列')
