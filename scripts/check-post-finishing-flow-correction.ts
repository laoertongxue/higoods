...
assert.ok(workOrders.length >= 5, '后道阶段处理 Mock 不足')
...
const forbidden = ['后道任务', '车缝+后道', '>开始后道<', '>完成后道<']
...
assert.ok(pdaSource.includes('实际工序完成成衣件数') && pdaSource.includes('关联实际工序单号'), 'PDA 必须使用实际工序口径')

console.log(JSON.stringify({
  后道阶段实际工序: POST_STAGE_PROCESSES.map((item) => item.name),
  流程节点: POST_STAGE_FLOW_NODES.map((item) => item.name),
  实际工序单数: workOrders.length,
  无实际工序路线: '收货→质检→复检→交接',
  命名门禁: '通过',
}, null, 2))
