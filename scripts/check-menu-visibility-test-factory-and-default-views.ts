...
const taskProgressSource = read('src/pages/progress-board/core.ts')
...
const specialCraftGroups = buildSpecialCraftMenuGroups()
const pfosSpecialCraftGroups = menusBySystem.pfos.filter((group) => group.title === '辅助工艺工厂管理' || group.title === '特种工艺工厂管理')
assert(pfosSpecialCraftGroups.length === specialCraftGroups.length, 'PFOS 缺少辅助工艺或特种工艺菜单组')
assert(enabledOperations.length === 13, '启用特殊工艺菜单数量必须为 13')
assert(flattenMenuItems(specialCraftGroups).length === enabledOperations.length + specialCraftGroups.length * 2, 'PFOS 全局特殊工艺菜单必须显示全部启用加工单和仓库菜单')
assert(flattenMenuItems(buildSpecialCraftMenuGroupsForFactory(TEST_FACTORY_ID)).length > 0, '全能力测试工厂应可见匹配工厂上下文的特殊工艺菜单')
...
assert(buildSpecialCraftProgressSnapshots().every((item) => item.factoryId !== TEST_FACTORY_ID), '默认特殊工艺进度必须排除测试工厂')
...
assertContains(progressLinkageSource, 'buildSpecialCraftProgressSnapshots(options: ProgressStatisticsBuildOptions = {})', '特殊工艺进度 builder 缺少 includeTestFactories 能力入口')
...
assertContains(taskProgressSource, '任务进度跟踪', '任务进度页面必须使用新名称')
assertNoToken(taskProgressSource, '任务进度看板', '任务进度页面不得继续显示旧名称')
assertNoToken(taskProgressSource, 'switch-dimension', '任务进度页面不得保留生产单维度切换')
assertNoToken(taskProgressSource, 'switch-view', '任务进度页面不得保留列表/看板切换')

assertContains(cuttingProgressModelSource, "'CUT_ORDER' | 'PRODUCTION_ORDER'", '裁床进度模型缺少 CUT_ORDER / PRODUCTION_ORDER 双维度')
assertContains(cuttingProgressSource, "viewDimension: 'PRODUCTION_ORDER'", '裁床进度默认维度必须为 PRODUCTION_ORDER')
assertContains(cuttingProgressSource, '裁片单主表', '裁床进度缺少裁片单维度主表')
assertContains(cuttingProgressSource, '生产单列表', '裁床进度缺少生产单维度列表')
...
assertContains(progressLinkageSource, 'targetObjectSummary', '特殊工艺进度缺少作用对象消费')
assertContains(progressLinkageSource, 'receiveDifferenceTicketCount', '特殊工艺进度缺少接收差异菲票字段')
assertContains(progressLinkageSource, 'returnDifferenceTicketCount', '特殊工艺进度缺少回仓差异菲票字段')
