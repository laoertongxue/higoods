...
assert(enabledOperations.length === 6, '启用特殊工艺菜单数量必须为 6')
assert(flattenMenuItems(buildSpecialCraftMenuGroups()).length === 6, 'PFOS 全局特殊工艺菜单必须继续全量显示 6 个启用菜单')
assert(flattenMenuItems(buildSpecialCraftMenuGroupsForFactory(TEST_FACTORY_ID)).length === 6, '全能力测试工厂应可见全部启用特殊工艺菜单')
...
assertContains(progressLinkageSource, '作用对象', '特殊工艺进度缺少作用对象消费')
assertContains(progressLinkageSource, '接收差异菲票', '特殊工艺进度缺少接收差异菲票字段')
assertContains(progressLinkageSource, '回仓差异菲票', '特殊工艺进度缺少回仓差异菲票字段')
