...
assert(allStoreTasks.length > 0, '特殊工艺任务单数据源必须存在生产单自动产出任务')
assert(allStoreTasks.every((task) => task.generationSource === 'PRODUCTION_ORDER'), '特殊工艺任务单数据源不得混入原型 seed 或手工任务')
