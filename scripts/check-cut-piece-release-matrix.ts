...
const sewingDispatchSource = readFileSync(resolve(process.cwd(), 'src/pages/unified-dispatch-workbench.ts'), 'utf8')
...
assert.match(sewingDispatchSource, /renderSewingPreparationOverview\(task, dialog\.selectedSkuCodes\)/, '统一任务分配必须真实接入裁片放行摘要')
assert.match(sewingDispatchSource, /getCutPieceDispatchReadinessForTask\(/, '统一任务分配必须读取裁片放行与分配占用事实')
assert.match(sewingDispatchSource, /assertCutPieceReleaseDispatchAvailable\(/, '提交含裁片任务前必须执行放行余量硬门禁')
...
assert.doesNotMatch(sewingDispatchSource, /renderBadge\(summary\.decision/, '车缝摘要不得继续显示旧通用决策徽标')
