...
assert(completedQc.postTaskNo, '质检完成后必须生成后道任务')
...
task = savePostFinishingPostSkuAdjustment({
  ...
  skuId: firstLine.sku.skuId,
  adjustmentMode: 'DECREASE',
  defectReasonQuantities: [{ reason: '污渍', quantity: 1 }],
  returnQty: 0,
  actor: POST_FINISHING_ACCEPTANCE_ACTORS.postOperator,
  nowMs: nextTime(),
})
assert.deepEqual(task.draftLines?.[0].defectReasonQuantities, [
  { reason: '污渍', quantity: 1 },
  { reason: '车缝不良', quantity: 1 },
...
assert.equal(task.startedBy?.actorId, webFallbackActor.actorId, 'Web 应急接管必须切换当前操作人')
assert.equal(task.lastTakeoverReason, 'PDA故障，转Web继续', 'Web 应急接管必须留存原因')
...
task = savePostFinishingPostSkuAdjustment({
  ...
  adjustmentMode: 'INCREASE',
  defectReasonQuantities: [{ reason: '污渍', quantity: allDefectLine.expectedQty }],
  returnQty: 0,
  actor: webFallbackActor,
  nowMs: nextTime(),
})
...
for (const action of ['填报后道完成数量', '增加后道瑕疵', '减少后道瑕疵', 'Web应急接管后道', '完成后道']) {
...
assert(!pdaSource.includes('请先返回后道单填写并保存该 SKU 的完成数量') && !workOrderDetailSource.includes('请先返回后道单填写并保存该 SKU 的完成数量'), 'PDA/Web 调整瑕疵不得依赖先填完成数量')
