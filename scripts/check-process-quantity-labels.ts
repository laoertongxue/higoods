...
assert(specialOrders.every((order) => order.unit === '片' || order.unit === '件'), '特殊工艺执行单位必须为片或件')
...
assertIncludes('src/pages/process-factory/special-craft/task-detail.ts', '绑定菲票', '特殊工艺任务详情必须展示菲票关系')
