...
for (const title of ['加工单／商品','加工投入／上游','加工要求','处理进度','加工产出／下游','时间','数量','操作','更多筛选','全选本页','全选筛选结果']) assert(html.includes(title), title)
...
for (const title of ['UNIFIED-RECEIPT','验收员','供料记录','交接记录','确认损耗', row.sampleNote]) assert(detail.includes(title), title)
console.log('染色统一列表专项通过：接收5600未用/未损耗、9列、分组、筛选、未知数量汇总、逐笔记录。')
...
assert.equal(printed.imageBlocks.find(item => item.title === '目标色样')?.imageUrl, row.sampleImageUrl, '流程卡必须使用正式目标色样')
assert.notEqual(row.sampleImageUrl, row.inputMaterials[0]?.imageUrl, '不得将投入物料图当正式色样')
assert.equal(printFields.find(item => item.label === '布料供应商')?.value, row.supplierName, '流程卡供应商必须读取当前加工单数据')
console.log('染色流程卡专项通过：已收未用量、确认损耗、供料方、正式目标色样。')
...
const rowWithoutNext = rows.find(item => !item.matchStatus && !item.downstreamLinks.length)
assert(rowWithoutNext)
const noNextDetail = renderDyeWorkOrderOverlay({type: 'view', dyeOrderId: rowWithoutNext.dyeOrderId})
if (rowWithoutNext.downstreamPartner) assert(noNextDetail.includes(rowWithoutNext.downstreamPartner.name), '已有正式下游接收对象必须展示')
else assert(noNextDetail.includes('下道工序待确定'), '没有正式下游时不能从接收对象推断工序')
assert(noNextDetail.includes(rowWithoutNext.receiverName), '实际接收对象必须展示')
if (rowWithoutNext.receiverWarehouseName !== rowWithoutNext.receiverName) assert(noNextDetail.includes(rowWithoutNext.receiverWarehouseName), '仓库与单位不同应分别展示')
...
console.log('染色上下游身份回归通过：正式下游与接收单位/仓分离，无正式关系时明确待确定。')
