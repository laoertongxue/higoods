...
function main(): void {
  ...
  assertIncludes(specialProcessPage, "'confirm-receive': 'BINDING_CONFIRM_RECEIVE'", '捆条加工单缺少确认接收动作')
  assertIncludes(specialProcessPage, "'process-report': 'BINDING_PROCESS_REPORT'", '捆条加工单缺少加工填报动作')
  assertIncludes(specialProcessPage, "'submit-handover': 'BINDING_SUBMIT_HANDOVER'", '捆条加工单缺少发起交出动作')
  assertIncludes(specialProcessPage, "'complete-order': 'BINDING_COMPLETE_ORDER'", '捆条加工单缺少完成加工单动作')
  assertIncludes(specialProcessPage, 'data-binding-action-detail', '数量动作必须选择具体捆条规格')
  assertIncludes(specialProcessPage, 'data-binding-action-qty', '确认接收、加工填报和发起交出必须录入本次数量')
  assertIncludes(specialProcessPage, 'data-binding-confirmation-key', '每次加工单动作必须携带唯一确认号')
  assertIncludes(specialProcessPage, 'executeBindingProcessActionWithWarehouse', '捆条动作必须同时写回加工单和仓库/交出事实')
  assertIncludes(specialProcessPage, 'event.stopPropagation()', '加工单动作弹窗内部按钮不允许冒泡后重复处理')
  assertNotIncludes(specialProcessPage, 'data-cutting-binding-action="record-cutting"', '捆条加工单不得保留旧记录裁剪动作')
  assertNotIncludes(specialProcessPage, 'data-cutting-binding-action="finish"', '捆条加工单不得保留旧手动结束加工动作')
  assertNotIncludes(specialProcessPage, 'submit-record-cutting', '捆条加工单不得保留旧记录裁剪提交入口')
  assertNotIncludes(specialProcessPage, 'submit-finish', '捆条加工单不得保留旧结束加工提交入口')
