...
assert.match(page, /createMasterTaskPage/, '花型任务必须使用生产准备单任务页面骨架')
assert.match(page, /PATTERN_ARTWORK/, '花型任务必须绑定生产准备单的花型任务类型')
assert.match(common, /listEngineeringTasksByType/, '花型任务必须从生产准备单任务读取')
