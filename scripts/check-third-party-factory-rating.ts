...
assert.ok(ratingPageSource.includes('renderTrialAssessmentRecords'), '评级详情必须渲染独立试产考核记录')
assert.ok(ratingPageSource.includes('listThirdPartyFactoryTrialAssessmentRecords'), '评级详情必须读取独立试产考核记录')
assert.ok(ratingPageSource.includes('factoryLiabilityDefectReasonItems'), '评级详情必须展示工厂责任瑕疵原因明细')
assert.ok(ratingPageSource.includes('timelinessGrade'), '评级详情必须展示试产时效评级')
assert.ok(ratingPageSource.includes('qualityGrade'), '评级详情必须展示试产质量评级')
assert.ok(ratingPageSource.includes('autoRatingGrade'), '评级详情必须展示试产自动评级')
...
const multiRoundTrialRecord = trialAssessmentRecords.find((item) => item.assessmentRound >= 3)
assert.ok(multiRoundTrialRecord, '必须有多轮试产记录用于验证详情抽屉')
const multiRoundDetailHtml = renderRatingPageWithSearch(`?viewFactoryId=${encodeURIComponent(multiRoundTrialRecord.factoryId)}`)
for (const requiredText of ['试产考核记录', '第 3 轮', '第 2 轮', '质检情况', '返工', '工厂责任瑕疵原因', '系统建议', '人工结论']) {
  assert.ok(multiRoundDetailHtml.includes(requiredText), `评级详情试产考核记录必须展示 ${requiredText}`)
}
assert.ok(multiRoundDetailHtml.includes('已派出未交出'), '评级详情必须用中文展示已派出未交出状态')
assert.ok(!multiRoundDetailHtml.includes('TRIAL_DISPATCHED'), '评级详情不得暴露试产英文状态码')
