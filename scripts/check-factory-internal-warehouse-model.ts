...
assertContains(dataSource, 'levelNo?: number', '共享库位模型缺少层号字段')
assertContains(dataSource, 'positionNo?: number', '共享库位模型缺少层内位置号字段')
assertContains(dataSource, 'shelfSequence?: number', '共享货架模型缺少货架序号字段')
assertContains(dataSource, 'code?: string', '共享库区模型缺少库区代码字段')
;[
  'listFactoryInternalWarehouses',
  'listFactoryWaitProcessStockItems',
  'listFactoryWaitHandoverStockItems',
...
assertContains(dataSource, 'approveAndExecuteFactoryWarehouseStocktakeDifferenceReview', 'PDA 端缺少盘点差异确认并调整 helper')
assert(stocktakeOrders.some((item) => item.factoryId === 'FACTORY-ONBOARD-0035'), '入驻裁床演示工厂35缺少盘点 mock 数据')
...
assert(
  listFactoryWarehouseAdjustmentOrders().some((item) => item.adjustmentType === '盘盈单'),
  '盘点 mock 缺少盘盈单场景',
)
assert(
  listFactoryWarehouseAdjustmentOrders().some((item) => item.adjustmentType === '盘亏单'),
  '盘点 mock 缺少盘亏单场景',
)
assert(
  listFactoryWarehouseInboundRecords().some((item) => item.sourceRecordType === 'STOCKTAKE_ADJUSTMENT' && item.remark?.includes('盘盈')),
  '库存流水缺少盘盈库存调整入库记录',
)
assert(
  listFactoryWarehouseOutboundRecords().some((item) => item.remark?.includes('盘亏审核通过')),
  '库存流水缺少盘亏库存调整出库记录',
)
const pdaReview = listFactoryWarehouseStocktakeDifferenceReviews().find((item) => item.reviewStatus === '待审核')
if (pdaReview) {
  const pdaAdjustment = approveAndExecuteFactoryWarehouseStocktakeDifferenceReview({
    reviewId: pdaReview.reviewId,
    operatorName: 'PDA检查脚本',
    remark: 'PDA 盘点差异确认并调整',
  })
  assert(pdaAdjustment?.status === '已完成', 'PDA 盘点差异确认后必须直接完成库存调整')
}
...
assert(
  outboundRecords
    .filter((item) => !item.remark?.includes('盘亏审核通过'))
    .every((item) => !!item.handoverOrderNo && !!item.handoverRecordNo && !!item.handoverRecordQrValue),
  '普通出库记录必须关联交出单、交出记录与交出二维码',
)
...
;[
  ...
  assertContains(dataSource, token, `待加工仓模型字段缺少：${token}`)
})
...
;[
  ...
  'handoverOrderNo',
  'handoverRecordNo',
  'handoverRecordQrValue',
  'receiverWrittenQty',
  'abnormalReason',
].forEach((token) => {
  assertContains(dataSource, token, `待交出仓模型字段缺少：${token}`)
})

assertContains(dataSource, "stocktakeScope: '全盘'", '盘点范围必须是全盘')
assert(stocktakeOrders.every((item) => item.stocktakeScope === '全盘'), '盘点模型只允许全盘')

assert(!routeSource.includes('/fcs/factory/warehouse'), 'FCS 路由不应保留工厂仓库总览页')
assert(!rendererSource.includes('renderFactoryInternalWarehousePage'), 'FCS 渲染器不应保留工厂仓库总览页')
...
;['待加工仓', '待交出仓', '库区库位'].forEach((token) => {
  assertContains(specialCraftWarehouseSource, token, `特殊工艺仓库管理缺少：${token}`)
})
assert(
  specialCraftWarehouseSource.includes('入库记录') || specialCraftWarehouseSource.includes('入仓记录'),
  '特殊工艺仓库管理缺少入库/入仓记录入口',
)
assert(
  specialCraftWarehouseSource.includes('出库记录') || specialCraftWarehouseSource.includes('交出记录'),
  '特殊工艺仓库管理缺少出库/交出记录入口',
)
;[
  buildToken('新增', '库存'),
...
assert(!fcsMenus.some((item) => item.href === '/fcs/factory/warehouse' || item.key === 'factories-warehouse'), 'FCS 菜单不应保留工厂仓库入口')
...
assertNoTerms(dataSource, legacyWarehouseTerms, '数据源码仍保留旧仓库主文案')
assertNoTerms(dataSource, noWmsTerms, '工厂内部仓源码不应扩展为 WMS 能力')
assertNoTerms(dataSource, noCycleTerms, '盘点不应包含抽盘或循环盘')
