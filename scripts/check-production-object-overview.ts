...
assert.equal(overview.executionSummary.length, 4, '总览必须包含 4 个执行状态摘要块')
...
assert.ok(searchPanel.includes('输入生产单 / 需求单 / SPU / SKU / 面辅料SKU / 配料单 / 发料单'), '搜索 placeholder 错误')
...
const relationSurface = uiModule.renderProductionObjectOverviewSurface(overview.objectType, overview.objectKey, 'relationship')
...
const timelineSurface = uiModule.renderProductionObjectOverviewSurface(overview.objectType, overview.objectKey, 'timeline')
assert.ok(timelineSurface.includes('计划时间'), '生产时间线必须展示计划时间')
assert.ok(timelineSurface.includes('实际时间'), '生产时间线必须展示实际时间')

const materialFlowSurface = uiModule.renderProductionObjectOverviewSurface(overview.objectType, overview.objectKey, 'material-flow')
assert.ok(materialFlowSurface.includes('采购'), '面辅料流转必须展示采购阶段')
assert.ok(materialFlowSurface.includes('到仓'), '面辅料流转必须展示到仓阶段')
assert.ok(materialFlowSurface.includes('配料'), '面辅料流转必须展示配料阶段')
assert.ok(materialFlowSurface.includes('发料'), '面辅料流转必须展示发料阶段')
assert.ok(materialFlowSurface.includes('签收'), '面辅料流转必须展示签收阶段')

const responsibilitySurface = uiModule.renderProductionObjectOverviewSurface(overview.objectType, overview.objectKey, 'responsibility')
assert.ok(responsibilitySurface.includes('判断依据'), '责任分析必须展示判断依据')

const crossQuerySurface = uiModule.renderProductionObjectOverviewSurface(overview.objectType, overview.objectKey, 'cross-query')
assert.ok(crossQuerySurface.includes('生产问题聚合查询'), '跨单查询必须展示管理查询标题')
