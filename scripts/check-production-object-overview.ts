...
const partialMaterialSearchPanel = uiModule.renderProductionObjectSearchPanel('FLSZ260617')
const materialTypeSearchPanel = uiModule.renderProductionObjectSearchPanel('面料')
...
for (const group of ['物料资源', '相关生产对象', '相关采购与仓储', '异常线索']) {
  ...
  assert.ok(hasSearchGroup(partialMaterialSearchPanel, group), `搜索部分物料编码时必须展示 ${group} 分组`)
}
assert.ok(hasSearchGroup(materialTypeSearchPanel, '物料资源'), '搜索物料类型时必须进入物料资源分组')
for (const group of ['最佳匹配', '当前卡点', '生产主线', '关联执行']) {
  assert.ok(!hasSearchGroup(materialSearchPanel, group), `搜索物料编码时不应追加 ${group} 分组`)
  assert.ok(!hasSearchGroup(partialMaterialSearchPanel, group), `搜索部分物料编码时不应追加 ${group} 分组`)
  assert.ok(!hasSearchGroup(materialTypeSearchPanel, group), `搜索物料类型时不应追加 ${group} 分组`)
...
assert.ok(!orderSearchPanel.includes('data-object-type="MATERIAL"'), '业务单据搜索里的 MATERIAL 结果不得打开生产对象总览')
assert.ok(!getSearchGroupHtml(materialSearchPanel, '异常线索').includes('data-object-type="MATERIAL"'), '物料分组下的异常线索不得把 MATERIAL 渲染成生产对象总览入口')
