...
const woolPageSource = readFileSync('src/pages/process-factory/wool/stage-orders.ts', 'utf8')
const pendingSource = readFileSync('src/pages/process-factory/wool/pending-receipts.ts', 'utf8')
const stageDisplaySource = readFileSync('src/pages/process-factory/wool/stage-display.ts', 'utf8')
...
assertContains(woolPageSource, '加工单号 / 任务号 / 款号 / 款名 / 内部货号', '毛织加工单筛选标签必须包含内部货号')
for (const label of ['全部加工单', '未加工', '待开工', '加工中', '加工完成', '已完成']) {
  assertContains(woolPageSource, `label: '${label}'`, `两阶段列表必须保留 ${label} Tab`)
}
assertContains(woolPageSource, 'function tabCounts(rows = baseRows())', 'Tab 数量必须基于同一筛选结果')
...
assertContains(woolPageSource, '凭证', '加工和交出事实弹窗保留凭证')
assertContains(woolPageSource, '历史摘要', '加工和交出事实弹窗展示历史摘要')
assertContains(woolPageSource, '累计有效加工填报', '加工填报按 SKU 展示累计有效加工数量')
assertContains(woolPageSource, '累计有效交出', '缝盘交出按 SKU 展示累计有效交出数量')
assertContains(woolPageSource, 'getWoolOutputHandoverAvailableQty', '缝盘交出使用统一可交出余额')
assertContains(woolPageSource, 'pieceAvailableQty', '横机按外发片计算可交出余额')
assertContains(woolPageSource, '/fcs/craft/wool/pending-receipts?workOrderId=', '两阶段接收导航统一待接收页')
assertContains(woolPageSource, "actions.includes('RECEIVE_PIECES')", '缝盘接收工艺片回货')
assertContains(woolPageSource, "order.stage === 'KNITTING' && actions.includes('ASSOCIATE_MACHINE')", '横机设备关联仅横机阶段')
assertNotContains(woolPageSource, 'renderReceiptDialog', '不得保留旧单内接收弹窗')
assertNotContains(woolPageSource, 'addWoolYarnReceipt', '不得另写一套纱线实收')
assertContains(pendingSource, 'confirmFactoryMaterialReceipt', '统一接收使用共享实收命令')
assertContains(pendingSource, 'grossKg', '纱线接收保留重量计算')
assertContains(pendingSource, 'pieceQty', '片接收使用片数量')
assertContains(stageDisplaySource, '内部货号', '两阶段款式信息显示内部货号')
assertNotContains(woolPageSource, 'advanceWoolOrderToWarehouseInbound', '不得保留旧完工入仓入口')

console.log('毛织内部货号与两阶段统一接收契约检查通过')
