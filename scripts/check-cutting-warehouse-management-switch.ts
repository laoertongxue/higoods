...
const movedTitles = ['裁床仓', '样衣仓', '特殊工艺发料', '特殊工艺回仓', '裁片交出', '裁片仓']
...
;['进入裁片仓', '进入特殊工艺回仓', '进入裁片交出'].forEach((item) => assertIncludes(waitHandoverHtml, item, `待交出仓汇总缺少：${item}`))
;['裁片仓', '裁片交出', '特殊工艺回仓'].forEach((item) => assertNotIncludes(waitProcessHtml, item, `待加工仓汇总不得出现：${item}`))
