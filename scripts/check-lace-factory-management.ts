...
assert.doesNotMatch(pageHtml, /当前操作身份/)
assert.doesNotMatch(pageHtml, /当前查看身份/)
...
for (const action of ['撤销完成', '取消生产单']) assert.match(workOrderListHtml, new RegExp(action))
assert.doesNotMatch(workOrderListHtml, /data-lace-work-orders-field="actorRole"/)
...
assert.doesNotMatch(recoverableInteractionSource, /currentLaceDetailActor/)
assert.match(recoverableInteractionSource, /currentReceiptActor/)
assert.match(recoverableInteractionSource, /actor:\s*LACE_FACTORY_SUPERVISOR/)
...
assert.doesNotMatch(purchaseDemandSource, /currentDemandActor/)
assert.doesNotMatch(purchaseDemandSource, /data-lace-demand-field="actorRole"/)
assert.match(purchaseDemandSource, /markPurchaseChangeViewed\(purchaseOrderId, PLATFORM_ADMIN\)/)
