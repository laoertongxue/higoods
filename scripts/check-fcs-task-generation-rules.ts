...
async function main(): Promise<void> {
  ...
  assertIncludes(continuousDispatchSource, 'paginateStandardListRows', '连续任务分配列表必须通过标准列表模型分页切片，避免一次性渲染全部行')
  ...
  assertIncludes(continuousDispatchSource, 'renderStandardListTable', '连续任务分配表格必须使用标准列表表格组件')
  assertIncludes(continuousDispatchSource, 'renderStandardListColumnSettings', '连续任务分配列表必须提供标准列设置能力')
  assertIncludes(continuousDispatchSource, 'actionColumn: true', '连续任务分配操作列必须声明为标准固定操作列')
