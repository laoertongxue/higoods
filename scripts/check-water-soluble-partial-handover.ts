...
assert.equal(receiveAndStartWaterForCheck(order.waterOrderId, { qty: 1, receiptId: 'RECEIVE-FIRST', upstreamRecordId: 'CHECK-SOURCE-FIRST' }).ok, true)
...
for (const [index, cumulativeOutput] of [1, order.plannedQty].entries()) {
  if (index === 1) {
    assert.equal(receiveAndStartWaterForCheck(order.waterOrderId, { qty: order.plannedQty - 1, receiptId: 'RECEIVE-SECOND', upstreamRecordId: 'CHECK-SOURCE-SECOND' }).ok, true)
    assert.equal(receiveAndStartWaterForCheck(order.waterOrderId, { qty: order.plannedQty - 1, receiptId: 'RECEIVE-SECOND', upstreamRecordId: 'CHECK-SOURCE-SECOND' }).ok, false, '重复接收不能累计')
  }
...
receiveAndStartWaterForCheck(split.waterOrderId, { qty: 10, receiptId: 'INPUT-10', upstreamRecordId: 'CHECK-SOURCE-SPLIT' })
