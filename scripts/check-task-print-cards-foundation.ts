...
const printPreviewSource = read('src/pages/print/print-preview.ts')
const routeTemplateSource = read('src/pages/print/templates/task-route-card-template.ts')
const deliveryTemplateSource = read('src/pages/print/templates/task-delivery-card-template.ts')
...
const printPagesSource = routePageSource + deliveryPageSource + routeTemplateSource + deliveryTemplateSource
...
assertIncludes(routeLinksSource, "documentType: 'TASK_ROUTE_CARD'", '任务流转卡 route builder 未使用统一打印预览页')
assertIncludes(routeLinksSource, "documentType: 'TASK_DELIVERY_CARD'", '任务交货卡 route builder 未使用统一打印预览页')
assertIncludes(routeLinksSource, 'handoverRecordId', '任务交货卡 route builder 未按交出记录生成链接')
...
assertIncludes(progressTaskSource, '打印任务流转卡', '任务进度跟踪缺少打印任务流转卡入口')
assertIncludes(progressTaskSource, "buildTaskRouteCardPrintLink('RUNTIME_TASK', task.taskId)", '任务进度跟踪入口必须使用 RUNTIME_TASK + taskId')
...
assertIncludes(printPreviewSource, 'window.print()', '统一打印预览页缺少打印按钮')
assertIncludes(printPagesSource + read('src/pages/print/task-card-shared.ts'), '商品图片', '任务打印预览页缺少图片区')
assertIncludes(routeTemplateSource, 'print-qr-box', '任务流转卡缺少二维码区')
assertIncludes(routeTemplateSource, 'qr.title', '任务流转卡二维码标题必须来自打印文档')
assertIncludes(deliveryTemplateSource, 'print-qr-box', '任务交货卡缺少二维码区')
assertIncludes(deliveryTemplateSource, '交出二维码', '任务交货卡缺少交出二维码定义')
assertIncludes(domainSource, '第几次交货', '任务交货卡页头缺少第几次交货')
...
assertIncludes(productionConfirmationSource, 'renderUnifiedPrintPreviewPage', '生产确认单必须接入统一打印预览页')
assertIncludes(printPreviewSource, 'window.print()', '生产确认单打印按钮被破坏')
assertIncludes(read('src/pages/print/templates/material-slip-template.ts'), '配料单', '配料单打印能力被破坏')
assertIncludes(read('src/pages/process-factory/cutting/fei-tickets.ts') + read('src/pages/print/templates/label-print-template.ts'), '菲票', '菲票打印能力被破坏')
assertIncludes(read('src/pages/process-factory/cutting/transfer-bags.ts') + read('src/pages/print/templates/label-print-template.ts'), '中转袋', '袋码 / 中转单打印能力被破坏')
