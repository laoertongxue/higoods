...
;['基本信息', '接收领料', '后道记录', '质检记录', '复检记录', '交出记录', '流转记录'].forEach((label) => {
...
assertIncludes(workOrdersPage, 'buildUnifiedPrintPreviewRouteLink', '后道列表打印任务流转卡必须进入统一打印预览')
assertIncludes(workOrdersPage, "sourceType: 'POST_FINISHING_WORK_ORDER'", '后道列表打印任务流转卡必须使用后道单打印来源')
assertIncludes(taskPrintCards, 'POST_FINISHING_WORK_ORDER', '任务流转卡旧底座必须继续兼容后道单来源')
...
;['确认接收领料', '开始后道', '完成后道', '开始质检', '完成质检', '开始复检', '完成复检', '发起交出'].forEach((label) => {
  assertIncludes(pdaExecDetailPage, label, `PDA 后道详情缺少动作 ${label}`)
})
;['计划成衣件数', '接收成衣件数', '已完成后道成衣件数', '复检确认成衣件数', '交出成衣件数'].forEach((label) => {
  assertIncludes(pdaExecDetailPage + detailPage + workOrdersPage + warehousePage, label, `后道数量字段缺少 ${label}`)
})

const sewingDoneOrder = postOrders.find((order) => order.routeMode === '车缝厂已做后道')
assert(sewingDoneOrder, '必须存在车缝厂已做后道场景')
let blockedPost = false
try {
  startPostFinishingAction(sewingDoneOrder!.postOrderId, '后道', {
    operatorName: '检查脚本',
...
assert(blockedPost, '车缝厂已完成后道时，后道工厂不能再次执行后道')
