...
assert.ok(workOrders.length >= 5, '后道加工单记录不足')
...
;['完整质检单号', '领取质检单', '一单一质检员'].forEach((text) => {
  assert.ok((qcOrdersSource + qcWorkbenchSource).includes(text), `Web 统一质检页缺少 ${text}`)
})
;['开始后道', '完整后道加工单号', '核对无误，开始后道', '完成后道并生成复检单'].forEach((text) => {
  assert.ok((detailSource + workOrdersSource).includes(text), `Web 后道加工单缺少 ${text}`)
})
;['领取复检单', '完整复检单号', '复检合格', '复检瑕疵', 'SKU 条码是否正确', '提交复检并生成后道出货单'].forEach((text) => {
  assert.ok(recheckOrdersSource.includes(text), `Web 复检单缺少 ${text}`)
})
...
;['阶段任务', '实际工序单', '车缝+后道', 'PDA 执行（优先）', 'Web 应急', 'Web应急'].forEach((text) => {
...
console.log(JSON.stringify({
  后道加工单记录: workOrders.length,
  质检记录: qcOrders.length,
  复检记录: recheckOrders.length,
  Web质检_Web后道_Web复检_PDA兼容_打印实际工序动作: '通过',
}, null, 2))
