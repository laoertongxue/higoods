...
assertContains(webNumberingPageSource, 'flex flex-wrap items-center justify-end gap-2', 'Web 打编号右上角按钮必须成组靠右展示')
...
assertContains(webNumberingPageSource, '员工统计排行', '员工计件汇总必须展示员工统计排行')
assertContains(webNumberingPageSource, 'set-summary-period', '员工统计排行必须支持周期筛选')
assertContains(webNumberingPageSource, "mode: 'day', label: '按日'", '员工统计排行缺少按日筛选')
assertContains(webNumberingPageSource, "mode: 'week', label: '按周'", '员工统计排行缺少按周筛选')
assertContains(webNumberingPageSource, "mode: 'month', label: '按月'", '员工统计排行缺少按月筛选')
assertContains(webNumberingPageSource, 'renderSummaryPeriodPicker', '员工统计排行切换周期后必须展示对应的日期/周/月选择器')
assertContains(webNumberingPageSource, "summaryDate: string", '员工统计排行缺少按日选择状态')
assertContains(webNumberingPageSource, "summaryWeek: string", '员工统计排行缺少按周选择状态')
assertContains(webNumberingPageSource, "summaryMonth: string", '员工统计排行缺少按月选择状态')
assertContains(webNumberingPageSource, "day: { label: '选择日期', type: 'date' }", '按日筛选必须选择日期')
assertContains(webNumberingPageSource, "week: { label: '选择周', type: 'week' }", '按周筛选必须选择周')
assertContains(webNumberingPageSource, "month: { label: '选择月份', type: 'month' }", '按月筛选必须选择月份')
assertContains(webNumberingPageSource, 'selectedPeriodKey && period.key !== selectedPeriodKey', '员工统计排行必须按当前选择周期过滤数据')
assertNotContains(webNumberingPageSource, "renderOperatorPeriodSummaryTable('按周汇总'", '员工计件汇总不应拆成按周模块')
assertNotContains(webNumberingPageSource, "renderOperatorPeriodSummaryTable('按月汇总'", '员工计件汇总不应拆成按月模块')
