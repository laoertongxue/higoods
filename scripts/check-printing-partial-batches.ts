...
receivePrintingInput(id, { actualSku: order.plannedInput.sku, receivedQty: 100, receivedRollCount: 2, receiverName: '分批验收' })
...
handoverPrintingOutput(id, { qty: 50, barcodeIds: read().barcodes.map((item) => item.id), operatorName: '分批验收', receiverName: '下游' })
receivePrintingHandover(id, { receivedQty: 50, receiverName: '下游' })
...
handoverPrintingOutput(id, { qty: 100, barcodeIds: nextRolls.map((item) => item.id), operatorName: '分批验收', receiverName: '下游' })
receivePrintingHandover(id, { receivedQty: 100, receiverName: '下游' })
