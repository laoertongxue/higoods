...
assert(washCraft.processCode === 'WASHING', '洗水必须挂在准备阶段洗水工序下')
assert(washCraft.generatesExternalTask, '洗水必须产出任务')
...
assert(postFinishing.generatesExternalTask, '后道父任务必须产出任务')
assert(postFinishing.requiresTaskQr, '后道父任务必须具备任务二维码')
