...
const sampleBatch = dispatchBatches[0]
const sampleOrder = productionOrders.find((order) => order.productionOrderId === sampleBatch.productionOrderId)
assert(sampleOrder, '示例发料批次必须关联生产单')
const sampleSnapshot = getProductionOrderTechPackSnapshot(sampleOrder.productionOrderId)
assert(sampleSnapshot, '示例发料批次必须能读取技术包快照')
const requiredResult = buildRequiredCutPiecesForSewingDispatch(sampleOrder, sampleSnapshot, sampleBatch.plannedSkuQtyLines)
...
assert(dataSource.includes('特殊工艺差异') && dataSource.includes('特殊工艺异议中'), '特殊工艺差异和异议中必须阻断交出')
