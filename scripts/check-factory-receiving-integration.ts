...
if(process.env.RECEIVING_REPLAY_FILE){
 ...
 wool.validateWoolStore(wool.readWoolStore());assert.equal(queries.listWoolWarehouseStocks('WAIT_PROCESS').filter(s=>s.objectSkuCode==='YARN-COTTON-MIXED'&&s.batchNo==='Y260911-01').reduce((n,s)=>n+s.currentQty,0),8.101)
...
assert.equal(queries.listWoolWarehouseStocks('WAIT_PROCESS').find(s=>s.objectSkuCode==='YARN-COTTON-MIXED'&&s.batchNo==='Y260911-01')?.currentQty,9.101)
...
assert(link.listReceivingAllocationTargets('INT-WOOL-L1').some(t=>t.id==='WOOL-STAGE-001:KNITTING'))
link.allocateReceivedMaterialToOrder({id:'INT-ALLOC-WOOL',receiptLineId:'INT-WOOL-L1',woolOrderId:'WOOL-STAGE-001:KNITTING',qty:5,operatorName:'周哥',at:'2026-09-11 16:15:00'})
const allocated=wool.readWoolStore();wool.validateWoolStore(allocated)
assert.equal(queries.listWoolWarehouseStocks('WAIT_PROCESS').filter(s=>s.objectSkuCode==='YARN-COTTON-MIXED'&&s.batchNo==='Y260911-01').reduce((n,s)=>n+s.currentQty,0),9.101)
assert.equal(queries.getWoolWorkOrderReadinessProjection('WOOL-STAGE-001:KNITTING').yarnReceiptsBySku.get('YARN-COTTON-MIXED')?.receivedQty,5)
const commands=await import('../src/data/fcs/wool-domain/commands.ts')
commands.issueWoolYarn('WOOL-STAGE-001:KNITTING',{commandId:'INT-ISSUE',yarnSkuCode:'YARN-COTTON-MIXED',batchNo:'Y260911-01',issuedQty:2,issuedBy:'周哥',issuedAt:'2026-09-11 16:20:00'})
commands.returnWoolYarn('WOOL-STAGE-001:KNITTING',{commandId:'INT-RETURN',yarnSkuCode:'YARN-COTTON-MIXED',batchNo:'Y260911-01',returnedQty:1,returnedBy:'周哥',returnedAt:'2026-09-11 16:30:00'})
wool.validateWoolStore(wool.readWoolStore())
assert.equal(queries.listWoolWarehouseStocks('WAIT_PROCESS').filter(s=>s.objectSkuCode==='YARN-COTTON-MIXED'&&s.batchNo==='Y260911-01').reduce((n,s)=>n+s.currentQty,0),8.101)
...
const beforeLocations=queries.listWoolWarehouseStocks('WAIT_PROCESS').filter(s=>s.objectSkuCode==='YARN-COTTON-MIXED'&&s.batchNo==='Y260911-01');assert(beforeLocations.every(s=>s.physicalLocationId))
...
const stocks=queries.listWoolWarehouseStocks('WAIT_PROCESS').filter(s=>s.objectSkuCode==='YARN-COTTON-MIXED'&&s.batchNo==='Y260911-01');assert.equal(stocks.find(s=>s.physicalLocationId===secondPosition.location.locationId)!.currentQty,1.938);assert.equal(new Set(stocks.map(s=>s.physicalLocationId)).size,2)
