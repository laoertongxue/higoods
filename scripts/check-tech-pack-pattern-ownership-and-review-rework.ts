...
assert.equal(current.buyerReview?.status, '无需审核', '买手范围无差异时应自动无需审核')
...
assert.equal(current.buyerReview?.status, '无需审核', '场景1：未选买手时买手保持无需审核')
...
assert.equal(current.buyerReview?.status, '无需审核', '场景2：未指定买手时买手保持无需审核')
