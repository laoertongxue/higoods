...
function main(): void {
  ...
  assert(dataSource.includes('getActiveProcessOptions()'), '任务产值风险筛选未从工序工艺字典读取')
  assert(dataSource.includes('getActiveCraftOptionsByProcess()'), '任务产值风险工艺筛选未从工序工艺字典读取')
  assert(dataSource.includes('assertProcessCraftExists('), '任务产值风险数据未按工序工艺字典校验')
  assert(dataSource.includes('resolveDemandIdentityFromSourceEntry('), '任务产值风险未补齐 sourceEntryId 的技术包回捞')
  assert(dataSource.includes('getProductionOrderProcessEntries(task.productionOrderId)'), '任务产值风险未从技术包工序定义兜底解析旧任务')
  ...
  assert(getActiveCraftOptionsByProcess().length > 0, '任务产值风险缺少工序工艺字典选项')
  ...
  ;[
    ...
    assert(riskSection.includes(token), `任务产值风险页面缺少字段：${token}`)
  })
  ...
  ;[
    ...
    assert(!riskSection.includes(token), `任务产值风险页面不应展示专属细维度：${token}`)
  })

  assert(riskSection.includes('已冻结待确认任务数'), '任务产值风险页缺少“已冻结待确认”KPI')
  assert(riskSection.includes('data-capacity-risk-task-table'), '任务产值风险表缺少测试锚点')
  assert(riskSection.includes('data-capacity-risk-order-table'), '生产单风险表缺少测试锚点')
  assert(!riskSection.includes('染印'), '任务产值风险页仍残留染印统计')
  assert(!riskSection.includes('质检'), '任务产值风险页仍残留质检统计')
  assert(!riskSection.includes('裁片 - 定位裁'), '任务产值风险页仍残留固定样例')
  ...
  console.log('任务产值风险与工艺瓶颈检查通过：筛选来自工序工艺字典，风险页无印花/染色专属细维度，固定右侧卡片已移除。')
}
