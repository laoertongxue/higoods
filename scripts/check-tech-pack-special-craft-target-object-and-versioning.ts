...
function buildValidationBasePack(): TechPack {
  return {
    ...createDraftTechPack('SPU-CHECK-TARGET', '作用对象校验款'),
...
assert(enabledOperations.length >= 2, 'PFOS 至少必须保留烫画和直喷两个启用特殊工艺菜单')
...
assertIncludes(contextSource, 'bundleLengthCm', '纸样捆条长度必须进入技术包数据模型')
assertIncludes(contextSource, 'bundleWidthCm', '纸样捆条宽度必须进入技术包数据模型')
assertIncludes(eventsSource, '捆条长度必须大于 0', '纸样事件缺少捆条长度校验')
assertIncludes(eventsSource, '捆条宽度必须大于 0', '纸样事件缺少捆条宽度校验')
