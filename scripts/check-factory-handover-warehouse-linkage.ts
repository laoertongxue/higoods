...
assertContains(sewingDispatchSource, 'submitCuttingSewingDispatchBatch', '裁床交出缺少提交本次交出 helper')
assertContains(sewingDispatchSource, 'createFactoryHandoverRecord', '裁床交出必须复用现有交出记录')
assertContains(sewingDispatchSource, 'linkHandoverRecordToOutboundRecord', '裁床交出必须复用出库联动')
assertContains(sewingDispatchSource, 'syncSewingReceiveWritebackToDispatch', '车缝厂回写必须同步本次交出状态')
...
assert(!listFactoryWarehouseInboundRecords().some((record) => record.factoryName.includes('车缝厂')), `不应为${buildToken('车', '缝')}接收方生成内部仓记录`)
...
const writtenBackOutbound = listFactoryWarehouseOutboundRecords().find((record) =>
  record.status === '已回写'
  && Boolean(findFactoryWaitHandoverStockItemByHandoverRecordId(record.handoverRecordId || '')),
)
