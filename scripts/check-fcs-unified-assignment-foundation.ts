...
assert.equal(printDocument.templateCode, 'PRODUCTION_CONTRACT_SPK_ID_V1')
const printText = JSON.stringify(printDocument)
assert(printText.includes('Jadwal Pengembalian Bertahap'))
...
const contractMasterTemplateSource = read('src/pages/print/templates/production-contract-master-template.ts')
const productionContractsSource = read('src/data/fcs/production-contracts.ts')
...
assert(contractMasterTemplateSource.includes('PRODUCTION_CONTRACT_SPK_ID_V1'))
assert(contractMasterTemplateSource.includes('dayCalculationId') && productionContractsSource.includes("dayCalculationId: 'KALENDER'"))
assert(!contractPrintSource.includes('生产加工合同') && !contractPrintSource.includes('PRODUCTION PROCESSING CONTRACT'))
