...
export default function TaskDetailPage() {
  ...
  // 直接派单价格状态
  const std = (task as any).standardPrice as number | undefined
  const direct = (task as any).directPrice as number | undefined
  ...
  // 接单截止时限状态
  const acceptDeadlineStr: string = (task as any).acceptDeadline || ''
  const deadlineStatusLabel = (() => {
    if (!acceptDeadlineStr) return null
    const diff = new Date(acceptDeadlineStr).getTime() - Date.now()
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant="outline">
                {task.assignmentStatus === 'ASSIGNED' ? '已分配' : task.assignmentStatus}
              </Badge>
              {task.acceptanceStatus && (
                <Badge variant={task.acceptanceStatus === 'ACCEPTED' ? 'default' : task.acceptanceStatus === 'REJECTED' ? 'destructive' : 'outline'}>
                  {task.acceptanceStatus === 'ACCEPTED' ? '已接单' : task.acceptanceStatus === 'REJECTED' ? '已拒单' : '待接单'}
                </Badge>
              )}
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="grid grid-cols-2 gap-3 text-sm">
              {acceptDeadlineStr && (
                <div>
                  <span className="text-muted-foreground">接单截止时间:</span>
                  <div className="font-medium">{acceptDeadlineStr}</div>
                  {deadlineStatusLabel && (
                    <div className={`text-xs font-medium mt-0.5 ${deadlineStatusLabel.color}`}>{deadlineStatusLabel.label}</div>
                  )}
                </div>
              )}
              {(task as any).taskDeadline && (
                <div>
                  <span className="text-muted-foreground">任务截止时间:</span>
                  <div className="font-medium">{(task as any).taskDeadline}</div>
                </div>
              )}
              <div>
                <span className="text-muted-foreground">币种 / 单位:</span>
                <div className="font-medium">{currency} / {unit}</div>
              </div>
              {std != null && (
                <div>
                  <span className="text-muted-foreground">工序标准价:</span>
                  <div className="font-medium">{std.toLocaleString()} {currency}/{unit}</div>
                </div>
              )}
              {direct != null && (
                <div>
                  <span className="text-muted-foreground">直接派单价:</span>
                  <div className="font-medium text-primary">{direct.toLocaleString()} {currency}/{unit}</div>
                  {priceStatus && (
                    <div className={`text-xs font-medium mt-0.5 ${priceStatusColor}`}>{priceStatus}</div>
                  )}
                </div>
              )}
            </div>
            {(task as any).priceDiffReason && (
              <>
                <Separator />
                <div className="text-sm">
                  <span className="text-muted-foreground">价格偏差原因:</span>
                  <div className="mt-1 bg-amber-50 text-amber-700 rounded p-2 text-xs">{(task as any).priceDiffReason}</div>
                </div>
              </>
            )}
            {(task as any).dispatchRemark && (
              <>
                <Separator />
                <div className="text-sm">
                  <span className="text-muted-foreground">派单备注:</span>
                  <div className="mt-1 bg-muted/50 rounded p-2 text-xs">{(task as any).dispatchRemark}</div>
                </div>
              </>
            )}
          </CardContent>
