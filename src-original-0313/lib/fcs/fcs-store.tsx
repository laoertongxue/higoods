...
type FcsAction =
  ...
  | { type: 'BATCH_DISPATCH'; payload: { taskIds: string[]; factoryId: string; factoryName: string; acceptDeadline?: string; taskDeadline?: string; dispatchRemark?: string; dispatchedBy?: string; dispatchPrice?: number; dispatchPriceCurrency?: string; dispatchPriceUnit?: string; priceDiffReason?: string } }
...
function fcsReducer(state: FcsState, action: FcsAction): FcsState {
  switch (action.type) {
    ...
    case 'BATCH_DISPATCH': {
      const { taskIds, factoryId, factoryName, acceptDeadline, taskDeadline, dispatchRemark, dispatchedBy, dispatchPrice, dispatchPriceCurrency, dispatchPriceUnit, priceDiffReason } = action.payload
      const now = new Date().toISOString().replace('T', ' ').slice(0, 19)
      ...
      const updatedTasks = state.processTasks.map(task => {
        if (taskIds.includes(task.taskId)) {
          ...
          return {
            ...
            acceptDeadline: acceptDeadline ?? task.acceptDeadline,
            taskDeadline: taskDeadline ?? task.taskDeadline,
            dispatchRemark: dispatchRemark ?? task.dispatchRemark,
            dispatchedAt: now,
            dispatchedBy: dispatchedBy ?? 'Admin',
            dispatchPrice: dispatchPrice ?? task.dispatchPrice,
            dispatchPriceCurrency: dispatchPriceCurrency ?? task.dispatchPriceCurrency,
            dispatchPriceUnit: dispatchPriceUnit ?? task.dispatchPriceUnit,
            priceDiffReason: priceDiffReason ?? task.priceDiffReason,
            updatedAt: now,
...
interface FcsContextType {
  ...
  batchDispatch: (taskIds: string[], factoryId: string, factoryName: string, acceptDeadline?: string, taskDeadline?: string, dispatchRemark?: string, dispatchedBy?: string, dispatchPrice?: number, dispatchPriceCurrency?: string, dispatchPriceUnit?: string, priceDiffReason?: string) => void
...
export function FcsProvider({ children }: { children: ReactNode }) {
  ...
  const batchDispatch = (taskIds: string[], factoryId: string, factoryName: string, acceptDeadline?: string, taskDeadline?: string, dispatchRemark?: string, dispatchedBy?: string, dispatchPrice?: number, dispatchPriceCurrency?: string, dispatchPriceUnit?: string, priceDiffReason?: string) => {
    dispatch({ type: 'BATCH_DISPATCH', payload: { taskIds, factoryId, factoryName, acceptDeadline, taskDeadline, dispatchRemark, dispatchedBy, dispatchPrice, dispatchPriceCurrency, dispatchPriceUnit, priceDiffReason } })
  }
  ...
  const updateTenderOrderStatus = (
    ...
    if (!ALLOWED[order.status]?.includes(nextStatus)) {
      return { ok: false, message: '当前招标单状态不���许切换到目标状态' }
    }
    const ts = new Date().toISOString().replace('T', ' ').slice(0, 19)
    dispatch({
  ...
  const applyQcAllocationWriteback = (
    ...
    if (qc.refType === 'RETURN_BATCH') {
      ...
      if (!taskId) return { ok: false, message: `无法确定 RETURN_BATCH 批次 ${batchId} ��应的 taskId` }
    } else {
      taskId = qc.refId
    }
    const bd = qc.dispositionQtyBreakdown
  ...
  const markReturnBatchPass = (
    ...
    dispatch({
      ...
      payload: {
        ...
        deltaScrappedQty: 0,
        noteZh: `回货批次 ${batchId} 合�����行：可用量+${batch.returnedQty}`,
        createdAt: ts,
        createdBy: resolvedBy,
      },
    })
  ...
  const generateStatementDraft = (
    ...
    if (!basisIds.length) return { ok: false, message: '请至少选择一��扣款依据' }
